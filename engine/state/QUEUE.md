...
| 회사 | 국가·분류 | 상태 |
|---|---|---|
| **Adevinta** | EU · 커머스(분류광고 마켓플레이스) | **275 기능 1개** — `pilot-with-real-work`(백로그로 도구를 잰다, 결정 8). 도메인 ① 1개 · **도메인 ② 0개(재료 2편 확보)**. **🔴 275 의 발견 — 2024년 결론이 2026년에 뒤집혔다**(Copilot 이 *"going to stay"* → 추정 대비 **1%**, Claude Code 를 **2,000명+** 에 배포). **⚠️ 개별 장터(leboncoin·Marktplaats)의 시스템 글 미확인.** **⚠️ 매출 실액은 확인 불가**(2024 상장폐지) |
