...
| 회사 | 국가·분류 | 상태 |
|---|---|---|
| **Klaviyo** | US · SaaS | ⭐ **프로파일 완료(2026-09-06) — 도메인 2개.** `마감 시각이 다른 일을 한 줄에서 처리한다`(EDF 스케줄러·풀 기반 컨슈머 프록시) · `잃지 않고 넘긴다`(RabbitMQ → AWS MSK 무중단 이관, 7일 리플레이). ⭐ **연도 확인 완료 — 최신 2026-04-23 으로 살아 있다.** ⏳ **기능 0/2 — 다음 사이클은 확장이다.** ⚠️ **개별 글 주소는 검색으로만 얻는다(Medium 해시). 못 받은 두 편은 open_questions 에.** |
