...
| 회사 | 국가·분류 | 근거 |
|---|---|---|
| **Wiz** | 기타(이스라엘) · SaaS(클라우드 보안) | ⭐ **`AI 를 넣었다가 다시 빼는` 이야기다.** `From Concept to Context Engine`(2026-08-27) 본문 확인 — **만든 것**(**`분석 파이프라인의 단계마다 다른 전문 하위 에이전트를 두고, 각 단계에 가장 알맞은 모델 등급과 짝지었다`**) · **초기 대가를 그대로 적는다**(**`느리고 비쌌다. 하지만 기존 스캐너가 완전히 놓치던 민감 데이터를 찾아냈다.`**) · ⭐ **거절한 것이 뾰족하다**(**`결정론적 로직을 AI 계층에서 완전히 빼내고, 맥락 이해가 진짜로 필요한 작업에만 AI 를 남겼다`**) · **수치**(**오탐률 거의 0 으로 떨어지고 재현율은 높게 유지** · 한 주에 **수백 개 공개 버킷의 약 2,000개 고유 파일에서 민감 데이터 발견 2,700건**). 최신 글 **2026-09-03**. ⚠️ **다수는 제품 발표와 보안 연구라 첫 화면으로는 판정할 수 없다** — 이 글 하나를 열어 확정했다(Snyk 전례). ⚠️ **`Wix`(이미 있음)와 다른 회사다.** ✅ `r.jina.ai` |
