...
| 회사 | 국가·분류 | 상태 |
|---|---|---|
| **Airbnb** | US · 커머스(숙박 마켓플레이스) | 도메인 3(이 규모를 관측하는 비용을 감당한다[tech 4] / 여러 제품을 하나의 데이터 구조로 담는다[tech 0 · ⏳ 미독] / 신원 그래프를 벤더에서 자기 것으로 옮긴다[tech 0 · ⏳ 미독]) · **기능 1**(small-enough-to-fix) · ⚠️ **목록이 헤더만 온다 — 개별 글 주소로 바로 간다** |
