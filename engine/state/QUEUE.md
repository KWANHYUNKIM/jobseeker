...
| 회사 | 국가·분류 | 상태 |
|---|---|---|
| **Wix** | 기타(이스라엘) · SaaS(웹사이트 빌더) | **294 도메인 ② 두 번째 기능 완료 — 기능 3개.** ① `에이전트에게 당번을 맡기되 열쇠는 안 준다`(`context-before-fix`. **남은 글 다섯 편**) · ② `멈추지 않고 갈아 끼운다`(`read-first-then-write` 인프라 층 + `chatroom-by-chatroom` 앱 층 — **정독 완료, 더 팔 것 없음**). **⭐⭐ 같은 문제를 두 층에서 정반대로 푼 대비 확보.** **다음 사이클에 도메인 ① 을 더 파거나 닫는다.** 실적 — Q2 2026 매출 **5억 6,300만 달러 · 15% 성장** |
