...
| 회사 | 국가·분류 | 상태 |
|---|---|---|
| **컬리** | KR · 커머스(신선식품) | **기능 1개 · 도메인 2개.** 260 에서 `misdelivery-detection`(문 앞을 알아본다) — **결정 8**. **질문을 바꿔 희소성을 피했다**('오배송인가' 대신 '늘 보던 문 앞인가'). **남은 재료** — 도메인 ② `남의 물건을 대신 받아 준다`(입고 outbox, **아직 기능 없음** · 두 번째 글 후보는 `2025-kafka-streams-window`) · **세 번째 도메인 후보** 딜리버리 암호화(2025-06, **버린 대안 둘에 이유 명시** · 다른 문제라 안 넣었다) · `access-block` 2부작(무중단 점검, 2025-10) · OMS MSA(2025-05). **엔지니어링만 7페이지(70편+)** |
