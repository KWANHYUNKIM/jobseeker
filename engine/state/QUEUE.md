...
- **Doximity** (US · 소셜) — 사이클 211 에서 열었다. **40번째 회사, 의료 첫 곳.**
  **도메인 3개** — `배치에서 실시간에 가까워지기`(CDC 본문 확인) ·
  `루비 모놀리스를 프로덕션에서 굴린다`(프로파일링 본문 확인) ·
  `임상 현장의 AI`(Ask 본문 확인 — **⚠️ 그 글에 버린 대안이 없다**).
  **기능 0개** — 다음 사이클부터. **`ui_map` 을 넣었다**(토스·Yelp 다음 세 번째).
  ✅ **IR 은 브라우저로 받았다** — `press.doximity.com` 은 본문이 비지만
  **`investors.doximity.com/news/news-details/.../default.aspx` 는 `get_page_text` 로 전문이 온다**(표까지).
  FY2026 매출 **$644.9M**(+13%) · 조정 EBITDA 마진 **55.5%** · 총이익률 **89.1%** ·
  활성 처방자 **80만 명 초과** · 미국 의사의 **85% 이상**이 회원.
  ⚠️ **매출 부문별 구성은 없다**(10-K 를 봐야 할 수도). **블로그 92편 중 4편만 읽었다.**
