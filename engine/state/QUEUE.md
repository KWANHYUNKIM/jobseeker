...
| 회사 | 국가·분류 | 상태 |
|---|---|---|
| **CyberAgent** | JP · 광고 | **기능 1개 · 도메인 2개.** 248 에서 `ad-pacing`(고르게 뿌린다) — **결정 7 · 버린 대안 셋에 이유가 각각**(로그 비 오차 · D 항 제거 · 베르누이 추첨). **남은 재료** — ② `같은 것을 여러 번 읽지 않게 한다`(캐시 2편, 도메인 `tech` 만 차 있다) · **세 번째 도메인 후보**: 광고 배송 고속화(64498, **자기 수치 있다** 700 RPS · CPU 32%→16%) + 플랫폼 이설(61355, **버린 대안 넷** · 자기 수치 0) · **네 번째 후보**: 관측 3편(60707 · 61628 · 60208, 미개봉) |
