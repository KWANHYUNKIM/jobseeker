...
- **Grafana Labs** (US · SaaS) — 사이클 161 에서 열었다. **27번째 회사.** 프로파일 + 도메인 2개
  (시계열 수집·저장 · 질의 실행) + 기능 1개(`mimir-next-gen`) 완료.
  **다음 기능은 `질의 실행`** — 자료 확인 완료(시간 분할 + 질의 샤딩, 고카디널리티 질의 **10분의 1**,
  37.7초 → 3.9초, **고객 질의의 60%만 샤딩됨**, TSDB 격리 비활성화로 인제스터 p99 **90% 감소**).
  두 번째 글로 `docs/mimir/latest/references/architecture/query-sharding/` 이 유력하다.
  ⚠️ **`grafana.com/blog/tags/engineering/` 는 404** — `WebSearch(allowed_domains=["grafana.com"])` 를 쓸 것.
  안 읽은 글: `a-year-in-mimir`(2023-03), `maintainers-tell-all`(2022-05), 차세대 구조 두 번째 글.
...
| **Adyen** | NL/EU · 핀테크 | `adyen.com/knowledge-hub` (Design to Duty 연재) | ✅ **본문 확인.** 2부를 열어 버린 안·이유·수치가 명시돼 있음을 봤다 | **유럽 핀테크**이고 축이 토스·스트라이프와 다르다 — **회계 원장과 직접 만들 것인가 빌릴 것인가.** 확인한 것: 스트리밍을 **Kafka 로 안 간 이유가 "그때는 Kafka 가 exactly-once 전달을 지원하지 않아서"** 였고 **PostgreSQL + 자바로 직접 만들었다**(토픽 같은 기능이 없는 것을 알면서). 샤딩은 **가맹점·결제수단 같은 도메인 지식 대신 라운드로빈** — 어차피 집계는 전 샤드를 물어야 하니 단순함·견고함·클러스터 증감의 유연함을 골랐다. 수치: **결제 한 건당 회계 DB 에 약 50행**, 초당 회계 삽입 **수천 건**(결제는 초당 수백 건), 트래픽이 **한 자릿수 배 이상** 늘었다. 그리고 **"이 틀이 20배를 더 갈 수 있을지, 아니면 Kafka 로 갈아야 할지 사내에서 논쟁 중"** 이라고 적는다 — 미해결을 적는 회사는 드물다. **비교 문서 후보 ⑧(외부에 맡길 것인가 직접 운영할 것인가)의 정반대 사례**이기도 하다 |
