...
| 회사 | 국가·분류 | 상태 |
|---|---|---|
| **Adevinta** | EU · 커머스(분류광고 마켓플레이스) | **277 기능 3개** — `pilot-with-real-work` · `account-metadata-spine` · `policy-inventory-cost`(규칙이 보려면 기억해야 한다, 결정 6). **🔴 277 의 발견 — 이 회사 안에서 사고를 적는 글과 안 적는 글이 갈린다**(정책 엔진 편에는 OOMKill 로 배포가 막힌 기록이 있는데, 거버넌스 편은 계정 732개까지의 과정만 자세하고 사고도 폐기 절차도 없다). **⚠️ 개별 장터 시스템 글 미확인.** **⚠️ 매출 실액 확인 불가**(2024 상장폐지) |
