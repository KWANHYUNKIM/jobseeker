...
| 회사 | 국가·분류 | 상태 |
|---|---|---|
| **하이퍼커넥트** | KR · 소셜(실시간 영상) | **기능 1개 · 도메인 2개.** 255 에서 `objective-relaxation`(가정을 쌓아 목표를 옮긴다) — **결정 8**, 목표 지표 3부작을 다 읽었다. **⭐ 세 글이 서로를 링크하고, 가정이 실제로 깨진 기록이 있다**(DAU 를 올렸더니 *"PUR 이 떨어졌습니다"*). **남은 재료** — 도메인 ② `서로 다른 시각에 도착한 것을 이어 붙인다`(Flink 2025-06 **재독 완료** + 이벤트 기반 추천 2022-01, **둘 다 tech 에 들어 있다 — 바로 기능을 쓸 수 있다**) · **세 번째 도메인 후보** 쿠버네티스 3편(2025-05 · 2023-07 · 2020-11, 미개봉). **⚠️ 사업 수치를 못 찾았다** — Match Group IR 에 아자르가 안 나온다 |
