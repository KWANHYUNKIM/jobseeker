...
| 회사 | 국가·분류 | 상태 |
|---|---|---|
| **monday.com** | 기타(이스라엘) · SaaS(협업 보드) | **286 기능 2개** — ① `board-query-engine`(결정 8) · ② `agent-with-guardrails`(길을 잃지 않게 잘게 자른다, 결정 8). **⭐⭐ 같은 시스템을 두 목소리로 쓴 회사** — 만든 사람 쪽은 **8 person-years → 6 person-months**, 에이전트 1인칭 쪽은 **밤 11시의 사고**. **⚠️ 도메인 ① 에 두 번째 기능 재료 있음**(아이템 ID 생성, **UUIDv7 후일담**). **⚠️ 관측·신뢰성 4편 미독.** **⚠️ 총 ARR 미확인** |
