...
| 회사 | 국가·분류 | 상태 |
|---|---|---|
| **Modal** | US · SaaS | ⭐ **기능 1/2**(2026-09-06). ✅ `전역 조율 없이 스케줄한다`(임계 경로에서 데이터 저장소를 걷어낸다 — 결정 8개, 그림 2장, 하드 프라블럼 2개). ⏳ 남은 하나 — `서버를 사용자 옆으로 옮긴다`. ⭐ **세 번째 도메인 후보 발견 — `시작 시간을 미리 벌어 둔다`(CRIU 메모리 스냅샷, import torch 5초 → 1.05초).** 완주 판정 때 세울지 정한다. |
