...
- **Spotify** (EU · 스트리밍) — 도메인 3개 중 2개에 기능이 있다(`rap`, `data-platform`).
  남은 하나는 `콘텐츠 수집·트랜스코딩` 인데 **사고 보고서 하나뿐**이라 두 번째 글이 필요하다.
  ✅ **다음 사이클의 더 나은 후보: 실험 플랫폼을 새 도메인으로 여는 것** — 본문 확인 완료.
  `when-can-llms-replace-humans-in-a-b-tests`(2026-08)는 **음성 결과를 담은 좋은 글**이다:
  보정 없는 LLM 예측은 인간 처치 효과의 **39%만** 회복하고, 선형 OLS 보정은 반증 검정에서
  **3.8 표준오차** 떨어져 탈락한다. ML 기반 보정만 표본오차 안에 든다. 두 번째 글은
  `better-experiments-with-llm-evals-a-funnel-not-a-fork`(2026-05).
  **'실험'은 이 엔진 25곳 통틀어 없던 도메인이다.**
