...
|---|---|---|---|---|
| **ZOZO** | JP · 커머스 | `techblog.zozo.com` | WebFetch ✅ (최신 2026-09-01) | 패션 커머스인데 무신사·잘란도와 다른 자리를 판다 — **검색 그 자체**다. `wear-hybrid-search`(2026-08-27)가 OpenSearch 하이브리드 검색을 벡터 양자화·워밍업·스코어 조정까지 쪼개 적었다. **버린 대안**(프로덕트·이진 양자화 → SQfp16), **대가**(fp16 이 검색에 20ms 오버헤드, 워밍업으로도 스파이크를 다 못 막아 0.5초 타임아웃 필요), **수치**(색인 13.2GB→5.2GB 60.6% 감소, p99 200ms→최대 20초→워밍업 후 369ms, fp32 대비 상위 20건 중복률 95% 이상)가 한 글에 다 있다. |
| **쏘카** | KR · 모빌리티 | `tech.socar.kr` (`tech.socarcorp.kr` 에서 301) | WebFetch ✅ (최신 2026-07-27) | 우버·Careem·배민과 같은 모빌리티인데 **차를 직접 소유해 빌려주는 쪽**이라 문제의 모양이 다르다(차량 상태·주차·장비사). `agentic-engineering-in-production`(2026-07-22)이 장애 자가복구 에이전트를 다루는데 **버린 대안**(OpenAI·Claude Agent SDK 를 벤더 종속과 저수준 제어 부족으로 버리고 LangGraph, 원인별 에이전트 분리도 폐기하고 단계 기반으로), **대가**(복구 한 건에 LLM 비용 몇백 원, 프롬프트 한 줄 바꾸면 검증에 30분, 실패 없는 장비사 변경은 여전히 미감지), **수치**(연 실패 약 30만 건·환불 16.5만 건, 대응 4시간→5분 이내, 2개월간 7,267건 복구, 장비사 약 50곳)가 나란히 있다. |
| **카카오뱅크** | KR · 핀테크 | `tech.kakaobank.com` | WebFetch ✅ (최신 2026-07-27) | 토스·Adyen·Stripe 는 결제였고 **은행 라이선스를 가진 곳은 아직 없다** — 심사·여신·FDS 가 새 도메인이다. 두 편을 열어 확인했다. `2605-no-wait-collateral-review-system-design` 은 **버린 대안**(외부 워크플로우 엔진 대신 검증된 사내 Deferred Batch 확장)과 **대가**(작업 현황·오류 추적이 복잡해져 전용 대시보드를 따로 만들어야 했다)를 적고, `2606-sequence-based-fds-model` 은 **버린 대안**(ARIMA 계열·RNN/LSTM/GRU·표준 ViT)과 **수치**(추론 평균 20ms, 일 1,800만 건, 일 2억 8,800만 레코드, 이상거래 0.1% 이하)를 준다. ⚠️ 다만 두 글 다 **정확도 지표(precision·recall)는 없다.** |
