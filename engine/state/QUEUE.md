...
| 회사 | 국가·분류 | 근거 |
|---|---|---|
| **Coinbase** | US · 핀테크(암호자산 거래소) | ⭐ **대가를 문장마다 명시하는 드문 글이다.** `How Coinbase built a governed platform for internal operations`(2026-08-18) 본문 확인 — **거절한 것**(**`느슨한 독립 도구들의 모음`** 대신 우산 서비스) · ⭐ **중앙화의 양날을 한 문장에**(**`너무 조금 중앙화하면 다시 난립이고, 너무 많이 중앙화하면 플랫폼 팀이 다른 모든 팀이 기다리는 대상이 된다`**) · **인가는 안전을 가용성과 바꾸지 않는다**(**`확신 있는 판단에 이르지 못하면 언제나 거부한다`** — 대가는 **`새로운 종류의 호출자가 추가될 때마다 fail-closed 경계를 다시 검증해야 한다`**) · ⭐ **받아들인 값을 적는다**(**`모든 것을 한 서비스로 라우팅하면 지연이 늘고 위험이 집중된다. 다른 방법으로는 얻을 수 없는 일관성을 위해 그것을 받아들였다`**) · **설정 기반의 양면**(**`설정 주도 동작은 양날이다 … 설정을 코드만큼 조심스럽게 리뷰하고 버전 관리하고 지켜야 하는 것으로 만든다`**) · **오류를 전송 계층이 아니라 페이로드 수준에 두어 `추적과 모니터링이 의도적으로 드러내지 않으면 페이로드 수준 오류를 놓친다`**. 최신 글 **2026-09-02**. ⏳ **다른 축도 있다** — `CEEcil: 사람 같은 기억을 가진 지원 「팀원」 만들기`(2026-08-28) · `Coinbase 의 기업용 AI 에이전트`(2025-12). ✅ `r.jina.ai` |
