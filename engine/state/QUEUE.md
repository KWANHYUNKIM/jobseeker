...
- **Mercari** (JP · 커머스) — 사이클 202 에서 열었다. **38번째 회사.**
  **도메인 후보 3개** — `잔액과 여신의 추상`(backend 분류 4편, 그중 2편 본문 확인) ·
  `비밀번호를 없앤 다음에 남는 문제`(security 분류 3편, 그중 2편 본문 확인) ·
  `국내용으로 만든 것을 국경 밖으로`(infrastructure 분류 4편 + 다통화 잔액 1편, **전부 제목만**).
  **기능 0개** — 다음 사이클부터.
  ✅ **사업 수치는 IR 데이터시트(CSV)로 전부 받았다** — `https://pdf.irpocket.com/C4385/xoA3/ieAo/YOsY/Kz3T.csv`
  (FY2026.6 매출 229,293 · 마켓플레이스 테이크레이트 **10.1%** vs 미국 **32.6%** · 핀테크 매출의 **60.7%가 여신**).
  ⚠️ **영문 블로그의 '더 보기' 링크 주소를 못 찾았다**(`/en/blog/page/2/` 는 404) — **분류별 목록으로 우회한다**
  (`/en/blog/category/<backend|security|infrastructure|ai|...>/`, 각 10편).
  ⚠️ **`국내용으로 만든 것을 국경 밖으로` 는 본문을 읽기 전에 기능을 열지 않는다.**
