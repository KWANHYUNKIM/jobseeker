...
|---|---|---|---|---|
| **Roblox** | US · 게임 | `about.roblox.com/newsroom` | ✅ WebFetch (2026-08-21 확인 · `blog.`→`corp.`→`about.` 로 301 두 번) | **게임 카테고리가 비어 있다.** 그리고 2021-10 의 **73시간 장애 사후분석이 최상급 자료**다 — Consul 스트리밍의 Go 채널 경합과 BoltDB 프리리스트가 쓰기당 7.8MB 로 부푼 것이 근본 원인이고, **텔레메트리가 Consul 에 의존해 장애 중 관측이 안 됐다**는 순환 의존까지 적혀 있다. 진단이 네 번 헛돌았다는 것도. 화염 그래프·커널 프로파일 포함. 4.2GB 로그 중 실제 데이터는 489MB |
| **Slack** | US · SaaS(메시징) | `slack.engineering` | ✅ WebFetch (2026-08-21 확인) | **Discord 와 같은 실시간 메시징인데 축이 다르다** — 기업용이라 `조직 하나에 워크스페이스 수천 개`가 문제가 된다(Unified Grid). 2021-06-30 us-east-1 의 **회색 장애**(구성요소마다 가용성 판단이 엇갈림)가 셀 아키텍처 이전을 낳았고, 잡 큐에서 **가용성을 일관성보다 위에 두어 잡 유실 위험을 감수**한다고 명시한다. Vitess 도 쓴다 — **Figma·Notion 이 버린 것을 고른 회사**라 비교 재료다 |
| **Airbnb** | US · 마켓플레이스 | `medium.com/airbnb-engineering` | ⚠️ **미검증** — WebFetch 403 (STATE 자료 접근 지도), 브라우저 필요 | 지금까지 없는 축 — **재고가 남의 집**이고 검색이 지리+날짜다. 커머스(배민·쿠팡·무신사·Shopify)와도, 모빌리티(Uber)와도 다르다. **다만 접근을 확인하지 못했다** — 브라우저가 이 세션에서 두 번 응답 불능이었다. 파기 전에 접근부터 다시 확인할 것 |
