...
| 회사 | 국가·분류 | 상태 |
|---|---|---|
| **하이퍼커넥트** | KR · 소셜(실시간 영상) | **기능 3개 · 도메인 3개 — 열어 둔 도메인이 다 찼다.** 255 `objective-relaxation` · 256 `stream-join` · 257 `raid-recovery`(결정 6, **복구 18~24시간에서 약 1시간**). **남은 재료** — ARM64 이전(2023-07, **도메인 tech 에 들어 있다**, 실패 목록이 길지만 **버린 대안이 없다**) · 클러스터 이전(2020-11, **미개봉이고 6년 전**). **다음 사이클에 회사를 닫는다** |
