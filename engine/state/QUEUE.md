...
|---|---|---|---|---|
| **Sansan** | JP · SaaS | `buildersbox.corp-sansan.com` | ✅ **본문을 열어 확인했다**(`/entry/2024/05/27/110000`). 목록도 열린다 | **기준을 넉넉히 넘겼다(3/3).** 열어 본 글은 `マイクロサービスアーキテクチャのリポジトリ構成を漸進的にモノレポに移行した話`(2024-05-27) — **버린 대안 둘**(빅뱅 이관 / OSS 도구를 안 쓰고 직접 스크립트를 쓴 이유를 넷 든다), **대가 넷**(저장소가 커져 초기 셋업 경험이 나빠질 수 있다 · **CI/CD 잡 이관이 모노레포 이관보다 손이 갔다** · 커밋이 빨리 흘러 분기 기준이 금세 낡는다 · ⭐ 병합 충돌 걱정은 **기우로 판명됐다고 스스로 적는다**), **수치**(저장소 **40개** · **20명 미만** · **100만 줄**, 설정 포함 550만). **블로그가 살아 있다** — 최신 글이 2026-08-25 다. **이관·아키텍처 글이 여럿이라 도메인이 여럿 설 가능성이 크다**(GitHub Enterprise 1000 저장소 이관 2025-12 · Swift Concurrency 이관 2026-03 · Jetpack Compose 이관 2025-11 · EC2 탈출과 모듈러 모놀리스화) |
| **freee** | JP · SaaS | `developers.freee.co.jp` | ✅ **본문을 열어 확인했다**(`/entry/migrate-to-auth-microservice`) | **⭐ 이 사이트에 드문 종류의 글이다 — 후회를 적는다.** 열어 본 글은 `freeeの礎となる認証認可基盤のマイクロサービス化プロジェクトの経緯と振り返り`(2022-08-31) — **4년 가까이 걸린 프로젝트의 회고**다. **대가가 아주 굵다**: DDD 로 새로 설계해 *"旧基盤との挙動の違いが大量に発生しました"* · 레거시의 부정 데이터를 옛 기반은 흘려보냈는데 새 기반은 에러를 낸다 · **견적 부족으로 연기 다발** · *"開発メンバーの精神が摩耗する状態となっていました"*. **후회와 교훈을 명시한다** — *"完了までに時間がかかったことを踏まえるとこれを良しとするのはよくありません"* 이고 *"旧機能のリファクタはマイクロサービス化の前に着手すべし"* 다. **수치**: **2019년~2022년 6월** · QA 가 **14제품 633 테스트 케이스**로 사내 최대. 다른 재료도 있다 — MySQL→Aurora 이관과 샤딩 · 권한 관리 기반 마이크로서비스 |
| **SmartHR** | JP · SaaS | `tech.smarthr.jp` | ✅ **본문을 열어 확인했다**(`/entry/2026/06/11/192713`) | **수치가 이번 조사에서 가장 강하다(3/3).** 열어 본 글은 `SaaS管理ユニットはなぜモジュラーモノリスから始め、マイクロサービスへ移行しているのか`(2026-06-11) — **CI 30분 이상 → 약 1분**, **배포 1일 1회 → 수시**, 종업원 데이터 조회 **3.59~4.46배**(1,000명 207ms→57.6ms · 50,000명 13.7초→3.07초), 행 크기 **약 71% 감소**. **버린 대안**은 처음부터 별도 서비스로 만드는 안(인프라·데이터 연동 준비에 시간이 걸려 IdP 기능을 약 6개월에 못 낸다). **대가도 적는다** — 이관이 아직 안 끝났고 DB 분리는 앞으로의 일이며 **벤치마크가 엄밀하지 않다고 스스로 밝힌다**. **⭐⭐ 그리고 freee 와 정면으로 대비된다** — 같은 '인증·권한 기반을 제품에서 떼어낸다' 문제를 **freee 는 모놀리스에서 곧장 4년에 걸쳐**(후회 명시), **SmartHR 은 모듈러 모놀리스로 시작해 Packwerk 로 경계를 먼저 보이게 한 뒤** 떼어낸다. **비교 문서 재료로 굵다** |
