...
| 회사 | 국가·분류 | 상태 |
|---|---|---|
| **Flipkart** | IN · 커머스 | 프로파일 완료 · 도메인 3(광고 예산을 초당 100만 이벤트에서 지킨다[tech 0 · ⏳ 미독] / 검색이 무엇을 관련 있다고 할지 정한다[tech 0 · ⏳ 미독] / 5억 고객의 공급망을 계획한다[tech 0 · ⏳ 미독]) · **기능 0** · ⚠️ **WebFetch 403 — `r.jina.ai` 를 앞에 둔다.** ⚠️ **도메인 셋이 전부 제목만 보고 세운 것이라 첫 확장에서 본문이 기대에 못 미치면 도메인을 고친다** |
