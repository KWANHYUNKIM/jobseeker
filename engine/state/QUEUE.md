...
| 회사 | 국가·분류 | 상태 |
|---|---|---|
| **LinkedIn** | US · 소셜(구인·구직) | **회사 프로파일까지 썼다**(사이클 227, 43번째). 도메인 2개 — `랭킹 모델을 얼마나 빨리 다시 만들 수 있는가`(본문 1편) · `자기 개발 과정에 AI 를 붙일 때 무엇을 통제하는가`(본문 1편, 두 번째 글 `ai/qa-agent-...` 미독). **기능 0개.** ⚠️ **블로그 분류 페이지가 WebFetch 로 안 열려 글 편수를 세지 못했다** |
