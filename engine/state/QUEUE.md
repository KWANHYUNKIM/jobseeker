...
| 회사 | 국가·분류 | 상태 |
|---|---|---|
| **Cybozu (サイボウズ)** | JP · SaaS | **308 새 도메인 ② 를 열고 기능까지 — 도메인 2개 · 기능 2개.** ① `이십 년 된 것을 멈추지 않고 옮긴다`(`same-node-rolling`) · ② `테넌트가 사만 개일 때 문을 여는 법`(`issuer-from-domain` — **결정 7개**, 설정을 4만 개 두는 대신 주소에서 조립. **그 값을 사용자가 치른다고 회사가 적는다**). **⚠️ 이 블로그는 글의 두께가 크게 다르다** — 세 편 읽고 하나만 썼다. 실적 — 2025년 매출 **374억 3천만 엔(+26.1%)** · 영업이익 **101억 엔(+106.4%)** |
