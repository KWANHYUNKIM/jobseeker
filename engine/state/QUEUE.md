...
| 회사 | 국가·분류 | 상태 |
|---|---|---|
| **CyberAgent** | JP · 광고 | **기능 2개 · 도메인 2개 — 열어 둔 도메인이 다 찼다.** 248 `ad-pacing`(결정 7) · 249 `two-tier-cache`(결정 7, **버린 대안 넷**). **세 번째 도메인 후보(재료 확인됨)** — `광고 배송을 어디서 어떻게 굴릴 것인가`: 고속화(64498, 700 RPS · CPU 32%→16%) + 이설(61355, **버린 대안 넷** · 자기 수치 0). **네 번째 후보** — 관측 3편(60707 · 61628 · 60208, 미개봉). **다음 사이클에 세 번째 도메인을 열지 회사를 닫을지 정한다** |
