...
| 회사 | 국가·분류 | 상태 |
|---|---|---|
| **Wayfair** | US · 커머스(대형 물품) | **회사 프로파일까지 썼다**(사이클 240, 44번째). 도메인 2개 — `창고와 사진에서 물건의 상태를 판정한다`(글 3편 · 본문 1편 확인) · `분류가 아니라 생김새로 찾게 한다`(글 2편 · 본문 1편 확인). **기능 0개.** 다음 사이클에 손상 판정 쪽부터 쓰되 **`vision-language-models-in-the-wild`(2026-01-30)를 먼저 읽는다** |
