...
| 회사 | 국가·분류 | 상태 |
|---|---|---|
| **Adevinta** | EU · 커머스(분류광고 마켓플레이스) | **276 기능 2개** — `pilot-with-real-work`(백로그로 도구를 잰다) · `account-metadata-spine`(누가 쓰는지를 인프라로 둔다). 각각 결정 8개. **⚠️ 정책 엔진(OPA→Kyverno, 8GB→2.7GB)이 도메인 ② 의 다음 기능 재료로 남아 있다.** **⚠️ 개별 장터(leboncoin·Marktplaats)의 시스템 글 미확인.** **⚠️ 매출 실액은 확인 불가**(2024 상장폐지) |
