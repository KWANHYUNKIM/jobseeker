...
*(비어 있다 — 다음 사이클이 대기에서 하나를 올린다.)*

## 대기

| 회사 | 국가·분류 | 근거 |
|---|---|---|
| **Atlassian** | AU · SaaS(협업 도구) | ⭐ **설계 목표가 뒤집힌 이야기다.** `Online index migration and shard-scaling in OpenSearch with AOSC`(2026-07-24) 본문 확인 — **문제**(**300개 넘는 클러스터 · 1.7PB** 에서 무중단으로 살아 있는 색인의 모양을 바꿔야 했다) · ⭐ **`설계 목표가 빠르게에서 의도적으로 느리게, 꾸준하게, 조절 가능하게로 뒤집혔다`** · **거절한 대안**(**`그 모든 데이터를 외부 프로세스로 흘려보내는 것은 느리고 부서지기 쉬웠을 것이다`** → 클러스터 안에서 돌리되 샤드 서비스와 클러스터 매니저 서비스라는 두 역할을 만들어야 했다) · **받아들인 값**(컷오버 때 소스 색인에 짧은 쓰기 차단 — **`애플리케이션이 잠깐 (재시도 가능한) 쓰기 오류를 본다`**, **50TB 색인에서도 대부분 2~5초, 최대 30초**). ⚠️ **블로그 대부분은 사업·AI 채택 글이라 첫 화면으로는 판정할 수 없다** — 이 글 하나를 열어 확정했다(Snyk·Wiz 전례). ✅ `r.jina.ai` |
