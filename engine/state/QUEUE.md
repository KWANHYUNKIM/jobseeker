...
| 회사 | 국가·분류 | 상태 |
|---|---|---|
| **ScyllaDB** | IL/US · SaaS(데이터베이스) | 도메인 2(아무것도 공유하지 않고 코어마다 하나씩 둔다[tech 5] / **남의 API 를 그대로 받아 주면서 그 아래를 다시 짓는다[tech 0 · ⏳ 미독]**) · **기능 1**(move-the-sharing-into-the-kernel) · ✅ WebFetch 로 본문이 온다. ⚠️ **슬러그 추측 금지 — 검색으로 주소를 받는다** |
