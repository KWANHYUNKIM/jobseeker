...
| 회사 | 국가·분류 | 상태 |
|---|---|---|
| **CyberAgent** | JP · 광고 | **기능 3개 · 도메인 3개 — 열어 둔 도메인이 다 찼다.** 248 `ad-pacing` · 249 `two-tier-cache` · 250 `env-split-migration`(**결정 8**, 이 회사에서 버린 대안이 가장 많은 글인데 **롤백 서술이 한 줄도 없다**). **남은 재료** — 고속화(64498, 700 RPS · CPU 32%→16%, **다른 문제라 안 넣었다**) · 관측 3편(60707 · 61628 · 60208, **미개봉**). **다음 사이클에 네 번째 도메인을 열지 회사를 닫을지 정한다** |
