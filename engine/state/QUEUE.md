...
| 회사 | 국가·분류 | 상태 |
|---|---|---|
| **Wix** | 기타(이스라엘) · SaaS(웹사이트 빌더) | **292 도메인 ① 첫 기능 완료 — 기능 1개.** ① `에이전트에게 당번을 맡기되 열쇠는 안 준다`(**두 편 읽음** — AirBot + 자가치유 두뇌. `context-before-fix` 결정 8개, **버린 대안 셋**: 포맷팅 에이전트(해 보고 되돌림) · LangChain · Google ADK 포크) · ② `멈추지 않고 갈아 끼운다`(무중단 DB 이관 — 283 확인, **정독 필요**). **⚠️ 도메인 ① 에 남은 글 다섯 편.** **실적 확보** — Q2 2026 매출 **5억 6,300만 달러 · 15% 성장** |
