...
|---|---|---|---|---|
| **Razorpay** | IN · 핀테크(결제 게이트웨이) | `engineering.razorpay.com` | ⚠️ **WebFetch 403 → `r.jina.ai` 로 뚫린다**(2026-09-06 되짚기 성공) | **최신 글이 8월**이고 목록이 **자기 시스템 이야기 일색**이며 **수치가 붙어 있다.** ✅ **본문 확인함**(`메트릭 비용 62% 절감`, 2026-08-24) — **결정마다 거절한 대안과 대가가 명시적이다.** ① **스크레이프 간격 30초 → 60초**: **경보 규칙과 대시보드를 감사해 분 미만 해상도에 의존하는 것이 있는지 봤고 하나도 없었다** → **하루 샘플 약 4,500억 → 1,700억.** ② **active-standby vmagent**(리더 선출 사이드카): 스크레이프 도구를 통째로 바꾸는 길을 거절(팀 간 조율 필요), **대가는 페일오버 로직과 스플릿브레인 방지의 운영 복잡도** — **장애 유형에 따라 페일오버 7~22초, 리스 만료 15초.** ③ **쓰지 않는 고카디널리티 메트릭을 relabel 로 버린다**: 각 팀에 제거를 요청하는 대신 **중앙에서** — **90일간 상위 500개 중 167개가 어디에도 안 쓰였고 하루 500억 샘플을 지웠다.** ⭐ **인도 핀테크인데 결제 게이트웨이는 이 사이트에 없고, `비용을 깎는 이야기` 라는 결도 새롭다.** ⏳ 후보 글: `CI 를 스팟 인스턴스로`(8/5) · `데이터 웨어하우스 갱신 10배`(7/14) · `보안 트리아지 750시간 → 2시간` |
