...
없다 — 1Password 를 완주하고 비웠다.

## 대기

| 회사 | 국가·분류 | 1차 자료 | 접근 | 왜 이 회사인가 |
|---|---|---|---|---|
| **DoorDash** | US · 배달·물류 | `careersatdoordash.com/blog` | ✅ 자체 호스팅. 검색으로 글 주소가 잘 나온다 | ⚠️ **AI 에이전트 축의 다섯 번째 회사가 된다** — `Delegating Engineering Work To Cloud-Based Agents`(2026-08)에서 **한 달에 엔지니어링 작업 13만 건**을 자동화하고 **주당 자동 코드 리뷰 2.5만 건**을 돌린다고 적는다. **Sentry·ClickHouse·Duolingo·Ramp 와 나란히 놓을 다섯 번째 답이다.** 그 밖에도 결이 좋다 — 모놀리스에서 마이크로서비스로 간 회고가 **단일 마스터 포스트그레스의 한계**와 **기술 선택 비교(Python 대 Java · REST 대 gRPC · RabbitMQ 대 Kafka/Cadence)**를 다루고, `Reducing the Migration's Pain Points` 라는 **아픈 곳을 제목에 건 글**도 있다. 서비스 메시 이전(2부작)과 Kafka 기반 `Iguazu` 이벤트 플랫폼(**하루 수천억 건**)도 있다. ⚠️ **배달 축은 이미 다섯 곳이 있지만**(배민·쿠팡·Grab·메이투안·Careem) **미국 자리는 비어 있고, 무엇보다 AI 에이전트 비교의 재료가 크다** |
