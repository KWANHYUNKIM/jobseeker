...
|---|---|---|---|---|
| **Deliveroo** | UK · 커머스(음식 배달) | `deliveroo.engineering` | ✅ 자체 호스팅. 검색으로 글이 나온다 | **최신 글이 2026-02-27** 로 살아 있다 — `AI Hub` 와 **`Governance as Code`** 로 **거버넌스를 플랫폼에 심어** 엔지니어가 빠르게 움직이면서도 규정을 지키게 한다. ⚠️ **DoorDash 가 인수한 회사라 축이 겹치는데**(2026년 2분기 실적에서 인수 효과를 따로 밝힌다) **결이 다르다** — DoorDash 의 Flux 는 **에이전트가 설 자리**를 만들고, 이쪽은 **규정 준수를 코드로 심는다.** **`AI 를 어디까지 믿을 것인가` 축의 여섯 번째가 될 수 있고, 특히 `그 선을 무엇이 지키나` 에 새 답을 준다 — 거버넌스 자체를 코드로.** ⏳ **본문은 아직 안 읽었다** |
