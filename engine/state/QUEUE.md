...
| **Mercari** | JP · 커머스 | `engineering.mercari.com` | ✅ **본문까지 확인**(`20260730 risk-based-authentication`) | **C2C 마켓인데 자체 결제·여신을 가진 자리**다 — Vinted(C2C, 결제는 플랫폼 안이지만 여신 없음)와도, LY(메시징)와도, Allegro Pay(전문 판매자 마켓)와도 다르다. 읽은 글에 **수치와 버린 접근이 있다**: **약 1,300만 명이 패스키를 등록**했고 패스키를 요구한 곳에서 *"대규모 피싱 금전 피해가 사실상 0"* 이다. **순수 패스키 강제를 거절한 이유**가 분명하다 — 이탈과 잠김이 *"곧바로 문의로 이어진다"*. 대신 **자격증명을 네 단계 스펙트럼**(비밀번호 → 이메일 링크 → 패스키+이메일 → 패스키 전용)으로 두고 **전환마다 위험을 평가**한다(세션 자세 vs 계정 자세를 **비대칭으로** 가중). 다른 글: `20260626 merpay-exchange-abstraction`(**Exchange 추상 — 결제·충전·이체 API 를 하나로**, 버린 접근 있음) · `20260608 partner-credit-service`(**여신 한도 관리** — 외부 후불 서비스·외부 청구 대행·법인카드를 각각 왜 버렸는지). ⚠️ **Merpay 쪽 두 글은 수치가 거의 없다** — 사업 수치는 **TSE 상장사라 IR PDF 로** 받을 것 |
| **Yelp** | US · 기타 | `engineeringblog.yelp.com` | ✅ **본문까지 확인**(`2026/05 partition-access-visualizations`) | **지역 검색·리뷰가 이 사이트에 없다** — 네이버·구글(문서 색인)과도, 마켓플레이스와도 다른 자리다. 읽은 글이 수치로 단단하다: **페타바이트급 데이터 레이크의 S3 비용을 33% 줄였고**, 30일 미접근 객체는 **40%**, 90일 미접근은 **81%** 싸진다는 계단을 이용한다. **수천 개 테이블**을 Iceberg 로 옮겼다. **버린 것**은 *"이해관계자 대화·문서·구전 — 금세 낡는 출처"* 이고, 대신 **S3 서버 접근 로그로 파티션 단위 사용을 IAM 역할에 붙여** 그림으로 만든다(일 배치는 대각선, 백필은 수직선, 애드혹은 흩뿌려진 점). 다른 글: `2026/05 webpack → Rspack`(**빌드 시간 약 50% 감소**) · `2026/08 Flow 모노레포 → TypeScript`(버린 이유 명시) · `2026/04 서버 주도 UI 를 네 플랫폼에 일관되게` · `2026/07 Training Orchestrator`. **상장사라 IR 이 있다** |
