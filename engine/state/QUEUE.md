...
|---|---|---|---|---|
| **Flipkart** | IN · 커머스 | `blog.flipkart.tech` | ⚠️ **WebFetch 403 → `r.jina.ai` 로 뚫린다**(2026-09-06 되짚기 성공) | **최신 글이 7월**이고 **`Re-architecting` · `Optimizing` · `Guaranteeing` 같은 제목이 줄지어 있다.** ⏳ **본문은 아직 안 읽었다** — 목록과 제목만 확인했다. ⭐ **인도 최대 커머스이고 Meesho·Zepto 와 같은 시장을 다르게 판다.** ⏳ 후보 글: **`고위험 고규모: 초당 100만 이벤트에서 광고 예산 정확도를 보장한다`**(2026-01-22, ⭐ **Zepto 의 광고 분석과 나란히 놓을 자리**) · `Rate Card 엔진 재설계`(2026-02-24) · `인도 공급망의 맥박 — 5억 고객을 위한 계획`(2026-05-05) · `검색 관련성 라벨링을 LLM 으로`(2026-07-31, ⭐ **Etsy 의 3단 증류와 견줄 자리**) |
