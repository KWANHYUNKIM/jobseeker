...
| 회사 | 국가·분류 | 상태 |
|---|---|---|
| **Temporal** | US · SaaS(내구성 있는 실행) | 프로파일 완료 · 도메인 3(상태를 이력으로 두고 재생해서 되살린다[tech 4] / 확장의 단위를 워크플로 하나로 잡는다[tech 3] / 관리형에서는 영속 계층을 직접 만든다[tech 3]) · **기능 2**(replay-must-match / shard-count-is-forever) |
