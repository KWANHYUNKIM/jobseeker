...
|---|---|---|---|---|
| **Dropbox** | US · 저장 인프라 | `dropbox.tech/infrastructure` | ✅ **본문 확인** — 목록·기사 모두 WebFetch 로 읽힘 | **아무도 없는 축 — 물리 저장 장치를 직접 굴린다.** Magic Pocket 으로 AWS 를 떠나 엑사바이트를 자기 하드웨어에 얹었고, SMR 드라이브를 페타바이트 규모로 처음 깔았고, SSD 캐시를 **떼어서** 쓰기 처리량을 올렸다. 지금까지 20곳은 전부 남의 클라우드 위에 있다. 첫 기능은 **동기화 엔진 재작성**(2016~2020, 4년) — '리팩터링으로는 데이터 모델을 못 바꾼다'는 판단이 통째로 적혀 있다 |
| **GitHub** | US · 개발자 도구 | `github.blog/engineering` | ✅ **본문 확인** — Blackbird 글이 특히 좋다 | **개발자 도구 축이 통째로 비어 있다.** 그리고 Blackbird 코드 검색 글은 이 엔진에서 만난 것 중 **버린 대안을 숫자로 계산해 둔** 드문 자료다 — ripgrep 방식을 2048코어 완전 병렬로 돌려도 0.01 QPS 라 경제적으로 불가능하다고 못 박는다. 저장소 4500만 · 코드 115TB → 색인 25TB · 문서 155억 · p99 100ms. 첫 기능은 **코드 검색(Blackbird)** |
