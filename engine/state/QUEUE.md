...
|---|---|---|---|---|
| **Wise** | EU(UK) · 핀테크(국경 간 송금) | `medium.com/wise-engineering` — 읽은 글은 **2025-02-12** | ⚠️ **Medium 이라 목록은 안 오고 개별 글만 브라우저로 열린다.** 글 주소는 검색으로 찾았다 | **`wise-tech-stack-2025-update` 전문을 읽었다 — 수치가 이 엔진에서 손꼽게 많다.** **사업** — 활성 고객 **1,560만** · 분기당 약 **360억 파운드** 이동 · **전송의 약 65%가 즉시 도착** · 엔지니어 **850명 이상** · 서비스 **1,000개 이상** · Java 저장소 **700개 이상**. **이전이 여럿이고 전후가 다 있다** — iOS 는 Xcode 모듈 250개를 Xcodegen→Tuist, Cocoapods→SPM 으로 옮겨 **무변경 빌드 28초 → 2초**; Kubernetes 는 2018년의 Terraform+JSONNET+ConcourseCI 에서 CRP(RKE2·Rancher·**Helm 이 JSONNET 대체**·ArgoCD)로 가며 **클러스터 6개 → 20개 이상**; CI 는 CircleCI→GitHub Actions 로 옮기고 캐시를 미리 채워 **빌드 15% 단축**(월 **50만 빌드**에서 **월 1,000시간 이상** 절약); CD 는 사내 도구 Octopus→Spinnaker 로 가며 **자동 카나리 분석**(트래픽 **5%** · **30분** 분석 · 자동 롤백)으로 **2024년에만 사고 유발 가능 배포를 수백 건 차단**; 관측은 Thanos→Grafana Mimir 로 LGTM 스택 통일, **초당 약 600만 메트릭 샘플 · 최대 테넌트 1억 5천만 활성 시리즈**. **Data Archives 는 1,000억 건 이상**을 아카이브한다. ⚠️ **다만 스택 소개 글이라 `무엇을 버렸나` 는 이전 목록으로만 나오고 `무엇을 잃었나` 가 거의 없다.** **수치로 기준을 넘지만 기능을 쓸 때 대가 칸을 채우려면 다른 글이 필요하다.** ⭐ **재미있는 연결** — 워크플로 엔진으로 **Temporal 을 채택했다**(방금 판 회사다). 전환·복구 시험 자동화에 쓴다 |
