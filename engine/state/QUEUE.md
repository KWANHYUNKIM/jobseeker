...
| 회사 | 국가·분류 | 근거 |
|---|---|---|
| **Perplexity** | US · 검색 | ⭐ **자체 서빙 엔진 `ROSE`(Runtime-Optimized Serving Engine)를 직접 만들고 공개한다.** **파이썬·PyTorch 로 모델 정의를 유연하게 두고, 서빙 로직과 배치 스케줄링만 러스트로 옮기는 중** — ⭐ **`어디를 유연하게 두고 어디를 빠르게 할 것인가`가 언어 경계로 드러나는 드문 사례다.** MoE 서빙용 **RDMA 최적화로 대역폭 효율 97.1%**, **2025-11 arXiv 논문 1편 + `fabric-lib` 커널 오픈소스**, **`Lily`(애플 실리콘 로컬 추론 엔진)로 로컬·클라우드 하이브리드**. ⚠️ **글이 `perplexity.ai/hub/blog` 와 arXiv 로 나뉘어 있다.** |
