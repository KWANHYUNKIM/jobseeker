...
| 회사 | 국가·분류 | 상태 |
|---|---|---|
| **Perplexity** | US · 검색 | ⭐ **기능 1/3**(2026-09-06). ✅ `요청을 GPU 앞까지 나른다`(Ivy·Tulip — 결정 6개, 그림 2장). ⏳ 남은 둘 — `모델을 GPU 위에서 돌린다`(ROSE) · `GPU 끼리 직접 말하게 한다`(fabric-lib, **arXiv 본문이 직접 열린다**). ⚠️⚠️ **`perplexity.ai/hub/blog` 는 목록도 개별 글도 403 이다** — 회사 글은 **3자 기사가 인용한 것**으로 읽고 출처에 매체를 적는다. |
