...
| 회사 | 국가·분류 | 근거 |
|---|---|---|
| **Trendyol** | 기타(튀르키예) · 커머스 | ⭐ **튀르키예 축의 첫 자리.** `Managing Ad Packages`(2026-03-24) 본문 확인 — **버린 구조를 정확히 적는다**(**`모든 것이 NoSQL 안에 광고 문서로 살았다. 애플리케이션 로직이 각 문서 안의 하나의 type 필드로 분기했다.`**) · **왜 못 견뎠는지**(**`새 광고 솔루션을 추가하려면 여러 서비스에 걸쳐 코드를 바꿔야 했다. 데이터만으로 시스템을 확장할 방법이 없었다.`**) → **정규화된 관계형 모델의 PostgreSQL 로.** ⚠️ **약점 — 포기한 것을 안 적는다**(NoSQL 의 유연성·수평 확장을 무엇과 바꿨는지 없고, 개선 차트는 있는데 본문에 수치가 없다). **제목에 거절과 대가가 드러나는 글이 여럿이다**(`5분에서 30초로` · `Context 에서 Zustand 로`). ✅ `medium.com/trendyol-tech` 은 `r.jina.ai` 로 온다 |
