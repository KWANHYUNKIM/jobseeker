...
*(없다 — LinkedIn 을 사이클 230 에서 닫았다. **큐가 0 이고 후보 조사도 소득이 없었다** — 다음은 **비교 문서**다.)*

## 대기

| 회사 | 국가·분류 | 1차 자료 | 접근 | 왜 이 회사인가 |
|---|---|---|---|---|
| **Wayfair** | US · 커머스(대형 물품) | `aboutwayfair.com/careers/tech-blog` | ✅ **본문까지 확인**(`real-time-package-damage-detection-at-scale-integrating-yolo-in-warehouse-operations`, 2026-03-26) | **가구·대형 물품이라 자리가 다르다.** Target 은 매장이 배송 거점이고 쿠팡은 자체 물류센터인데, 여기는 **창고 컨베이어 위에서 물리적 손상을 카메라로 검사한다** — 소프트웨어가 물리 세계를 판정하는 자리다(이 구분은 이 사이트의 것). 블로그는 살아 있고(최신 **2026-07-17**) 태그별 편수가 나온다 — **Data Science 90 · ML/AI 68 · Computer Vision 10 · Cloud 8**. 읽은 글은 **포장 손상 실시간 탐지**(Tarragon) — 문제는 *"손상된 배송은 교체 지연과 불편으로 고객 경험에 영향을 주고, 물류 부담을 키워 비용도 올린다"*. ⚠️ **버린 대안은 명시돼 있지 않다.** 대신 **대가와 한계가 여섯 줄이나 적혀 있다**: *"우리 창고 환경에 맞는 라벨된 데이터가 없다는 것이 큰 과제였다"* · *"손상이 드물어서 라벨링할 손상 예시를 수동으로 찾는 것은 '건초 더미에서 바늘 찾기' 문제다"* · **"사람 주석자들이 이미지 수준 손상/무손상 라벨의 16% 에서 의견이 갈렸다"** · *"노이즈 데이터를 처음에 포함시키면 성능이 떨어진다"* · 카메라 노출이 일정하게 어긋난 **캘리브레이션 문제**를 찾았는데 *"고치는 데 하드웨어 벤더와 몇 달간 조율이 필요했다"* · *"클라우드 추론은 로컬 실행 대비 약 250ms 지연을 더한다"*(SLA 는 **1초**). 수치 — 주석은 **20% 이중 · 80% 단일**, 불일치 **16%**, 기준선 정확도 **3.3%**, 모델은 그 대비 **12배 이상**, 손상 등급 **0~2 대 3~4**. 스택은 **YOLO · 파인튜닝한 Gemini 2.0 Flash 분류기 · Vertex AI · 여섯 각도 라인 카메라 · Pub/Sub**, 배치는 **켄터키 Erlanger 창고**. **축이 될 만한 것**: *사람도 16% 에서 합의 못 하는 판정을 모델에 시킨다* |


### 확인해 둔 후보 (아직 검증 안 됨)

- **CyberAgent**(JP · 미디어/광고) `developers.cyberagent.co.jp/blog/` — ⚠️ **블로그는 활발한데 연 글이 얇았다**(사이클 239). 최신 **2026-08-26**, 태그 편수가 **Engineers 138 · ABEMA 125 · Designers 115**. 열어 본 글(`archives/64965` — Bucketeer A/B 테스트의 승자 선택)은 **영어이고 읽기 쉬운데 버린 대안이 없고 대가가 한 줄뿐**이다(*"트래픽 분할 자체가 어긋나면 그 위의 결과는 오해를 부를 수 있다"*). **다음에 볼 것은 `ABEMA` 태그** — 일본의 실시간 스트리밍이라 넷플릭스·유튜브와 자리가 다를 수 있다.
