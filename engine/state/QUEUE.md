...
| 회사 | 국가·분류 | 상태 |
|---|---|---|
| **Sansan** | JP · SaaS | 332 에서 **세 번째 도메인 `사람과 기계가 같이 도는 공정을 잰다` 를 열고 기능 `measure-the-mixed-line` 까지 썼다** — **도메인 3개 · 기능 3개 · 결정 6개.** **🔴 331 의 판단을 정정했다** — 데이터화 공정에 사람 확인 단계가 있고(GEES), 그 지연이 단말에서 즉시 답을 주기로 한 이유다. 남은 것 — 실물 원본 관리 축(대담 형식) · 이관 글 여럿 |
