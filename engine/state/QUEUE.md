...
- **Zalando** (EU · 커머스) — 사이클 164 에서 열었다. **28번째 회사.** 프로파일 + 도메인 2개
  (내부 트래픽 라우팅 · 상품·오퍼 서빙) + 기능 1개(`client-side-lb`) 완료.
  **다음 기능은 `상품·오퍼 서빙`** — 자료 확인 완료(`posts/2025/03/event-driven-to-api.html`:
  PODS 로 상품/오퍼 분리, **페이로드 90% 이상이 안 바뀐 채 실려 다녔고 사이버위크에 최대 30분 지연**,
  PRAPI 는 단건 p99 **10ms 미만**, 캐시가 다 안 들어가 일관 해시로 카탈로그 분할, DynamoDB 쓰기
  용량 병목, GC 압력 때문에 ByteArray 로 보관, **약 350개 팀·수천 앱** 때문에 Accept 헤더로 옛 형식
  병행). **두 번째 글이 필요하다** — `posts/2017/11/why-event-driven.html`(이벤트 주도의 대가를
  스스로 적은 글)이 유력하다. 그 밖에 `tags/scalability.html`, `posts/2016/07/scaling-our-tech-organization-and-architecture.html`.
