...
- **LY Corporation (LINE)** (JP · 메시징) — 사이클 146 에서 열었다. 회사 프로파일 + 도메인 3개
  (스트리밍 파이프라인 · 메시지 암호화 · 데이터 레이크 통합) + 기능 1개(`kafka-e2ee`) 완료.
  남은 도메인 2개에 기능이 없다. 1차 자료는 `techblog.lycorp.co.jp/en` — ⚠️ **URL 을 추측하지 말고
  목록 페이지에서 딸 것**(WebFetch 가 목록을 읽을 때 호스트를 `www.lycorp.co.jp` 로 바꿔 내놓는다.
  그대로 쓰면 404 다 — `techblog.` 로 고쳐야 한다).
