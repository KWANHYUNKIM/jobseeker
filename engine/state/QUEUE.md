...
| 회사 | 국가·분류 | 상태 |
|---|---|---|
| **Sansan** | JP · SaaS | 333 에서 **네 번째 도메인 `화면을 되돌려도 종이는 안 돌아온다` 를 열고 기능 `paper-does-not-undo` 까지 썼다**(이 회사 첫 `ui` 포함) — **도메인 4개 · 기능 4개 · 결정 6개.** **⭐ 네 도메인이 같은 공정의 네 층이다** — 개발 환경 · 인식 · 공정 실적 · 실물. 남은 것은 안 읽은 이관 글 여럿뿐이라 **다음 사이클은 닫는 판단**이다 |
