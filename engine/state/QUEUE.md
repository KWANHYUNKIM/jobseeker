...
| 회사 | 국가·분류 | 상태 |
|---|---|---|
| **Klaviyo** | US · SaaS | ⭐ **기능 1/2**(2026-09-06). ✅ `마감 시각으로 줄을 세운다`(EDF 우선순위 큐 — 결정 6개, 그림 2장, 세대 1). ⏳ **마지막 하나** — `잃지 않고 넘긴다`(RabbitMQ → AWS MSK 무중단 이관, 7일 리플레이). ⚠️ **완주 판정 때 수익원 둘(구독·SMS)이 기능과 이어지는지 먼저 본다.** |
