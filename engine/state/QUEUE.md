...
- **Careem** (AE · 모빌리티) — 사이클 200 에서 열었다. **37번째 회사, 중동 첫 회사.**
  ...
  **도메인 2개**(사이클 201 에서 셋 → 둘로 고쳤다. 엔지니어링 글이 **여섯 편뿐**이라 저장·전송을
  **`인프라 비용이 새는 자리`** 하나로 합쳤고, `화면을 서버가 정한다` 는 **수치가 없어 보류**했다).
  **기능 1개 완료**(`cost-leaks`, 사이클 201). **보류 도메인은 세지 않으므로 done 으로 닫아도 된다.**
