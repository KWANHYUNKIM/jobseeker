...
| 회사 | 국가·분류 | 상태 |
|---|---|---|
| **CyberAgent** | JP · 광고 | **기능 4개 · 도메인 4개 — 열어 둔 도메인이 다 찼다.** 248 `ad-pacing` · 249 `two-tier-cache` · 250 `env-split-migration` · 251 `alert-context`(**결정 6**, 회사가 *"알림의 형해화"* 라고 이름 붙인 문제). **남은 재료** — Vector(60707, **도메인 tech 에 들어 있다**, 초당 2만 이벤트 · Datadog Observability Pipelines 를 버린 이유 셋) · 고속화(64498, 다른 문제) · Datadog APM Inferred Services(60208, **미개봉**). **다음 사이클에 Vector 기능을 쓰고 닫는다** |
