...
| 회사 | 국가·분류 | 상태 |
|---|---|---|
| **Turso** | US · SaaS | ⭐ **기능 1/2**(2026-09-06). ✅ `재현을 설계로 만든다`(시드를 주면 같은 버그가 다시 난다 — 결정 8개, 그림 2장). ⭐⭐ **`--doublecheck` 로 결정론 자체를 검사한다.** ⏳ **마지막 하나** — `동기 인터페이스를 비동기로 연다`. ⚠️ **완주 판정 때 수익원 둘이 기능과 이어지는지 본다.** |
