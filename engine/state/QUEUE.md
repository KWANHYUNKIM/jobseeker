...
| **TigerBeetle** | 기타 · SaaS | `tigerbeetle.com/blog` | ✅ **본문까지 확인**(`a-database-without-dynamic-memory` · `a-trillion-transactions`) | **하드웨어에 가장 가까운 후보다** — 금융 거래 전용 원장 데이터베이스이고, **동적 메모리 할당을 아예 하지 않는다**(연결 수·메시지 크기·메시지당 질의 수를 전부 시작 시점이나 컴파일 시점에 고정하고 총 메모리를 곱셈과 덧셈으로 계산한다). 이유가 성능만이 아니다 — *"정적 할당은 과부하를 쉽게 다루게 해 준다. 객체와 배열 크기가 고정이면 잘못 설정된 클라이언트가 연결이나 메시지를 너무 많이 보내도 끝없이 자라는 버퍼가 없다."* 계정·이체 메시지는 **128바이트**, 메시지당 기본 최대 질의 **8,191개**. `a-trillion-transactions` 는 **확장의 일곱 단계**(로그 → 스냅샷 → LSM → 복제 상태 기계 → 수평 샤딩 → 저장/컴퓨트 분리 → 대각 확장)를 세우고 **OLTP 수평 샤딩을 명시적으로 거절한다**(캐시 지역성 상실 · 암달의 법칙 · *"단일 수평 확장 클러스터는 연 100만 달러가 들었을 것"*). 복구 추정도 있다 — 128 TiB 를 100Gb NIC 로 **2.9시간**, 인덱스 재구성까지면 **약 6시간**. **Zig 로 쓰였고 결정론적 시뮬레이션 테스트**가 별도 글로 있다(`a-tale-of-four-fuzzers`, `protocol-aware-deterministic-simulation-testing`). ⚠️ **비상장·소규모라 사업 수치가 거의 없을 것** — business_model 은 얇게 잡고 기술에 무게를 둘 것 |
| **Careem** | AE/기타 · 모빌리티 | `engineering.careem.com` | ✅ **본문까지 확인**(`benchmarking-kafka-compression...`) | **중동 첫 회사다** — 이 사이트에 중동·아프리카·라틴아메리카가 아직 0곳이다. 두바이 기반 슈퍼앱(라이드헤일링 + 배달 + 결제). 읽은 글에 **수치와 버린 대안이 둘 다** 있다: 실제 프로덕션 메시지 **20만 건**(약 4.37MB JSON)으로 압축 코덱 다섯을 비교해 **LZ4** 를 골랐다 — 처리량 **150 → 336 msg/s**, P99.9 지연 **약 140ms → 35ms**, **저장 88.2% 절감**, CPU **+8.3%**. **버린 이유가 각각 다르다** — Gzip 은 *"극도로 느리고"*, Zstd 는 압축은 좋은데 **CPU 부담**, Snappy 는 전 지표에서 LZ4 에 밀렸다. 1KB Avro 프로덕션 토픽에서 **저장·대역폭 약 45% 감소**로 검증까지 했다. 다른 글: `scaling-amazon-rds-for-mysql-performance-on-aws`, `app-engine-server-driven-ui`(서버 주도 UI), `using-facial-recognition-technology-fraud-detection`(중복 기사 계정 탐지). ⚠️ **소유 구조를 개설 사이클에 확인할 것** — 라이드헤일링은 Uber 가 인수했고 슈퍼앱은 별도 법인으로 알려져 있다. **1차 자료로 확인 전에는 쓰지 말 것** |
