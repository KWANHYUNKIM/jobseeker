...
| 회사 | 국가·분류 | 상태 |
|---|---|---|
| **Cybozu (サイボウズ)** | JP · SaaS | **307 도메인 ① 첫 기능 완료 — 도메인 1개 · 기능 1개.** ① `이십 년 된 것을 멈추지 않고 옮긴다`(`same-node-rolling` — **결정 7개**, StatefulSet 을 버린 이유 명시, **가용성을 얻고 분산을 내준 대가**, 커널까지 직접 조사). **⚠️ 두 번째 편(Cattage)은 얇았고 문제도 달랐다** — 한 편에 기댄다는 사실을 밝혀 뒀다. **⭐ 사내 쿠버네티스 기반(Neco)을 여러 팀이 쓰게 하는 것이 별도 도메인 후보.** 실적 — 2025년 매출 **374억 3천만 엔(+26.1%)** · 영업이익 **101억 엔(+106.4%)** |
