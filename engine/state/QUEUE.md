...
| 회사 | 국가·분류 | 상태 |
|---|---|---|
| **Meesho** | IN · 커머스(소셜 커머스) | 도메인 4(판매자에게 언제 얼마를 왜 주는지 설명한다[tech 4] / 주소를 못 쓰는 나라에서 배달지를 찾는다[tech 0 · ⏳ 미독] / 사기 네트워크를 실시간으로 끊는다[tech 0 · ⏳ 미독] / 10억 대 기기의 인터넷에 물건을 보여 준다[tech 0 · ⏳ 미독]) · **기능 1**(fast-but-wrong-is-still-wrong) · ⚠️ **`www.meesho.io/blog` 는 WebFetch 403 — `r.jina.ai` 를 앞에 둔다** |
