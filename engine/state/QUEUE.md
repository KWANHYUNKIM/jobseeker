...
|---|---|---|---|---|
| **하이퍼커넥트** | KR · 소셜(실시간 영상) | `hyperconnect.github.io` | ✅ WebFetch 로 목록·본문 다 열린다 | **2018~2026 에 150편 넘게 쌓였고 최신이 2026-04-22 다.** 묶음이 넷으로 뚜렷하다 — **추천/ML 23편+**(아자르 추천 모델 2024-11 · 추천 실험 시스템 2025-08 · LLM-as-a-Judge 2026-04) · **실시간 인프라 15편+**(Flink KeyedProcessFunction 스트림 조인 2025-06 · 이벤트 기반 라이브 스트리밍 추천 2022-01 · WebRTC) · **쿠버네티스 17편+**(ScyllaDB 클러스터를 Super Disk 와 RAID 로 2025-05 · **1년에 워크로드 절반을 ARM64 로** 2023-07 · **5일 만에 프로덕션 클러스터 이전** 2020-11) · **데이터 파이프라인 10편+**(Kafka · CDC). **자리** — 이 사이트에 **실시간 1:1 영상 매칭이 없다.** 당근은 지역 중고거래, Discord 는 음성 커뮤니티라 축이 다르다. **낯선 사람을 실시간으로 이어 주는 문제**(매칭 지연 · 영상 품질 · 추천)가 통째로 비어 있다. ⚠️ **본문을 아직 안 열었다** — 파기 전에 한 편 확인한다 |
| **컬리** | KR · 커머스(신선식품) | `helloworld.kurly.com` | ✅ WebFetch 로 목록·본문 다 열린다 | **253 에서 본문을 열어 확인했다** — `/blog/2026-delivery-domain-rag`(2026-05-21)가 **세 단계를 거치며 앞의 둘을 왜 버렸는지 인용문으로 적는다**: ① 역색인은 *"키워드 정확 매칭은 의미 가중을 줄 수 없어서 단어가 같으면 다 끌고 옵니다"* 이고 *"색인 유지 자체가 부채"* · ② 전문 임베딩은 **문서의 85% 가 512 토큰 한계를 넘겨** 잘렸고(가장 긴 문서 7,396 토큰 중 **약 93% 가 임베딩에 반영 안 됨**) *"임베딩 모델은 학습 데이터에 거의 등장하지 않은 토큰에 대해 의미 시그널을 만들지 못합니다"* · ③ 골라 쓰는 것은 **요약 임베딩 + FTS5 하이브리드**. **남는 대가도 적는다** — *"일반어 노이즈만 일부 잔존"*, *"summary 품질 편차가 남습니다"*. **⚠️ 다만 이 글은 사내 개발 도구다.** **본업 쪽 재료가 따로 있다** — `/blog/image-based-misdelivery-detection`(**배송 완료 사진으로 오배송을 On-device 로 탐지**, 2026-04-28) · `/blog/2026-outbox-pattern-and-retry-topic`(**입고 시스템의 외부 인입 데이터 동기화**, 2026-01-12). **도메인을 열 때 이 둘부터 본다.** **자리** — 쿠팡은 종합 익일배송, 배민은 음식 배달인데 **여기는 신선식품 콜드체인**이라 **문 앞에 두고 가는 배송의 판정**이 소프트웨어 문제가 된다(확인되면) |
