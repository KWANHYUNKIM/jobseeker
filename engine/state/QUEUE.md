...
|---|---|---|---|---|
| **ScyllaDB** | IL/US · SaaS(데이터베이스) | `www.scylladb.com` | ⚠️ **판정 뒤집힘**(2026-09-06). 첫 화면은 고객 사례·제품 글이라 접었는데 **엔지니어링 글은 결이 다르다.** ✅ 본문 열림. ⚠️ **슬러그 추측 금지** — 검색으로 주소를 받는다(한 번 404 를 냈다) | ✅ **본문 확인함**(`비대칭 io_uring 백엔드`, 2026-07-22) — **세 대안을 나란히 놓고 거절 이유를 댄다.** ① **마스터/프록시 샤드** — `smp::submit_to 의 성능 비용은 받아들일 수 없다. 폴링에 의존해 완료까지 수 밀리초가 걸릴 수 있다` ② **공유 io_uring 인스턴스** — `liburing 이 스레드 안전하지 않아 동기화가 필요한데, 뮤텍스를 넣으면 경합이 심해지고 Seastar 의 shared-nothing 철학을 위반한다`(락프리는 **기술적으로 불가능**하다고 판단) ③ **선택: 샤드마다 자기 인스턴스 + 커널 수준 공유**(`IORING_SETUP_ATTACH_WQ`). ⚠️ **그리고 자기 방식이 진 경우를 그대로 적는다** — **`순수 I/O 워크로드에서는 새 백엔드가 linux-aio 만큼의 처리량을 내지 못한다`**(네트워킹 코어가 **주기의 40~50% 를 메모리 복사에 쓴다**). **계산이 섞이면 이긴다.** ⭐ **shard-per-core 라는 다른 접근이고 ClickHouse 와 견줄 자리다** |
