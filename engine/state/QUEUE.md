...
| 회사 | 국가·분류 | 상태 |
|---|---|---|
| **Modal** | US · SaaS | ⭐ **프로파일 완료(2026-09-06) — 도메인 2개.** `전역 조율 없이 스케줄한다`(워커가 자기 진실 원천 · 임계 경로에 데이터 저장소 없음 · 100만 개를 1분 안에) · `서버를 사용자 옆으로 옮긴다`(분산 I/O 플레인 · p50 80ms 감소 · Go 재작성). ⏳ **기능 0/2 — 다음은 확장, 스케줄링부터.** ⭐ **㊳축(유휴를 어떻게 다루나)의 네 번째 답 — 유휴에 아예 값을 안 받는다.** ⚠️ **나머지 글은 사례·모델 지원 발표라 얇을 수 있다.** |
