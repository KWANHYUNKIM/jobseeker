...
| 회사 | 국가·분류 | 상태 |
|---|---|---|
| **Target** | US · 커머스(리테일 물류) | **기능 1개**(`split-shipments`, 사이클 220). 도메인 2개 중 **`주문을 어디서 몇 개의 배송으로 쪼갤 것인가` 착수**, **`비용을 청구하지 않고 보여준다` 는 아직 0개** — 그 도메인은 본문을 한 편만 읽었으니 **`accelerating-developer-velocity-dbaas`(2025-10)를 먼저 읽고** 쓴다 |
