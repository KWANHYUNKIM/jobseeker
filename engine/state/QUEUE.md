...
| 회사 | 국가·분류 | 상태 |
|---|---|---|
| **Sansan** | JP · SaaS | 335 에서 도메인 ⑤ 의 두 번째 기능 `coordinator-in-between` 을 쓰고 **그 도메인 이름을 `십 년 된 앱을 한 번에 못 옮긴다` 로 고쳤다**(334 의 이름이 Compose 편을 못 담았다) — **도메인 5개 · 기능 6개.** 남은 것은 대담·회고뿐이라 **다음 사이클은 닫는 판단**이다 |
