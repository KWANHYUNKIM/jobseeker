...
| 회사 | 국가·분류 | 상태 |
|---|---|---|
| **bol** | EU · 커머스 | 352 에서 **새 도메인 `모델이 먹을 데이터를 층으로 나눠 만든다`** 를 열고 `slow-down-to-go-fast` 를 썼다 — **도메인 2개 · 기능 2개.** **⭐⭐ 같은 회사 안의 대비를 찾았다** — 데이터 사이언스 쪽은 단계마다 품질을 보는데 3TB 이관 쪽엔 검증 이야기가 아예 없다. **⚠️ 두 도메인 다 한 편씩만 읽었다.** 남은 것 — Flyway 도커 편 · shifting left/right 두 편 · 추천 · 수요 예측 |
