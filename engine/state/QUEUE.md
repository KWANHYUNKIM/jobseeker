...
| 회사 | 국가·분류 | 상태 |
|---|---|---|
| **SmartHR** | JP · SaaS | 346 에서 **새 도메인 `한 덩어리를 그대로 두고 버티게 만든다`** 를 열고 `share-a-few-connections` 를 썼다 — **도메인 2개 · 기능 2개.** **⭐⭐ 두 도메인이 같은 단일 실패점에 정반대로 답한다** — 하나는 떼어내고 하나는 앞에 세워 버틴다. 새 도메인은 **두 편 규칙 미충족.** 남은 것 — 연말정산 부하 시험 · 횡단 프런트엔드 · 불구합 집약 · Sandbox 테스트 · Policy-as-Code 등 과거 글 셋 |
