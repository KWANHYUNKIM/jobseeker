...
|---|---|---|---|---|
| **Meituan (美团)** | CN · 커머스 | `tech.meituan.com` — **1차 자료이고 살아 있다**(최신 2026-08-27) | **WebFetch 로 목록과 본문 모두 열린다.** 중국어 | **298 에서 본문 한 편을 실제로 열어 확인했다**(`美团搜索3.0`, 8천~1만 자). **기준 셋을 다 넘는다** — **버린 대안 셋에 이유가 각각 있다**(세 번의 forward pass 는 *"推理效率有优化空间"* / 복잡한 지시문은 *"过于复杂的任务指令反而降低表征质量"* / CPV 정보를 넣었더니 *"排序评估效果反而下降"* 이라 빼고 *"信息质量比信息量更重要"* 로 정리) · **한계를 목록으로 적는다**(*"四个明显短板"*, 그리고 *"这一结论有阶段局限性"* 로 결론의 유효 기간까지 밝힌다) · **수치가 촘촘하다**(주문 +0.20% · 롱테일 NDCG@5 +2.21pp · Q2I-Order-AUC +11.02pp · 질의 커버리지 81.24%→98.92%). **그리고 중국은 지금 ByteDance 하나뿐이다.** |
