...
| 회사 | 국가·분류 | 근거 |
|---|---|---|
| **Cursor** | US · SaaS(AI 코딩 에이전트) | ⭐ **거절한 대안과 대가와 수치가 다 있다.** `Git at any scale`(2026-08-18) 본문 확인 — **문제**(Spokes 의 3단계 커밋 합의 알고리즘에 **`근본적인 한계가 있었다 — 모든 단계의 지연이 클러스터 안 모든 서버 중 가장 느린 것에 묶인다`**) · **고른 것**(합의 기반 복제 대신 **S3 객체 저장소를 쓰는 WAL 시스템 Continuity**) · ⭐ **내준 것**(**`복제본들의 정족수 대신 단일 로컬 저장소와 참조 트랜잭션을 동기화한다`**) · **수치**(표준 S3 에서 **초당 최대 120 푸시**, S3 Express One Zone 에서 **초당 300 푸시 이상**, 읽기는 **복제본 100개까지 일관된 선형 확장**, 조건부 GET 일관성 확인 **평균 10ms 미만**) · **얻은 것**(**라우팅 테이블이 없어져 저장소 위치 관리의 운영 복잡도가 사라진다**). ⭐ **WarpStream·Neon 과 같은 축**(객체 저장소를 진실 원천으로). ⏳ **다른 축도 두껍다** — `직접 관리하는 머신에서 클라우드 에이전트를 돌린다`(2026-09-02) · `클라우드 에이전트가 빌드로 3배 빨리 시작한다`(2026-08-13) · `NVL72 를 위한 오픈소스 MoE 메가커널` · `Cursor 라우터는 어떻게 작업에 맞는 모델을 고르나` · 에이전트 보안 인증(AIUC-1). ⚠️ **2026년 8월 SpaceX 에 합류했다 — ㉗축 재료.** ✅ `r.jina.ai` |
