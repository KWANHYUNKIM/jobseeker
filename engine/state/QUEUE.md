...
- **Careem** (AE · 모빌리티) — 사이클 200 에서 열었다. **37번째 회사, 중동 첫 회사.**
  `AE` 를 validate.py COUNTRIES 와 index.json countries 에 추가했다.
  프로파일 + 도메인 지도 + 도메인 3개(**거대 단일 데이터베이스 줄이기** · **이벤트 파이프라인 비용** ·
  **화면을 서버가 정한다**). 기능 0개 — **다음 사이클은 `거대 단일 데이터베이스 줄이기`**
  (본문 정독 완료, 수치가 가장 두껍다: 270TB → 78TB · InnoDB HLL 58.7억 · 언두 로그 5.7TB → 5.7GB ·
  SLA 1ms vs 실패 20ms vs 최종 0.54ms · 버린 대안이 *"약 3.5개월"* 로 계산돼 있다).
  ⚠️ **두 번째 글이 필요한 도메인** — `화면을 서버가 정한다` 는 **수치가 하나도 없다.**
  ⚠️ **소유 구조·매출을 확인 못 했다**(`careem.com/about` 은 404). 지어내지 말 것.
