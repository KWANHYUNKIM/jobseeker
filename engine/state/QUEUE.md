...
| 회사 | 국가·분류 | 상태 |
|---|---|---|
| **Block (Square)** | US · 핀테크 | ⭐ **프로파일 완료(2026-09-06) — 도메인 4개.** `확률적으로 틀리는 것을 고친다`(Moneybot 자가치유) · `키를 만드는 난수`(COLDCARD 펌웨어) · `에이전트가 팀으로 일한다`(Buzz 벌 계층·릴레이) · `노는 컴퓨트를 모은다`(MeshLLM). **다섯 편을 본문까지 읽었고 넷 다 대가가 명시돼 있다.** ⏳ **기능 0/4 — 다음 사이클은 확장이다.** ⚠️ **Square 본업(결제·정산·대출 심사) 자료는 아직 못 읽었다.** |
