...
|---|---|---|---|---|
| **ClickHouse** | US · SaaS(분석 DB) | `clickhouse.com/blog` | ✅ WebFetch 로 본문이 온다 | ⚠️ **판정이 뒤집혔다** — 앞서 `updates-in-clickhouse-1-purpose-built-engines` 를 읽고 **일반론 교육 글**이라 떨어뜨리며 `ai-first-data-warehouse 는 결이 다를 수 있다` 고 남겨 뒀는데 **그게 맞았다.** **자기 내부 데이터 창고(2.1PB) 이야기**다. 사내 AI 어시스턴트 `DWAINE` 에 **사용자 250명 이상 · 하루 200 메시지 · 대화 50~70건**, **분석 워크로드의 70% 를 AI 가 처리**하고 DWH 팀 지원 요청이 **50~70% 줄었다**(15~30분 걸리던 SQL 작업이 즉시). **⭐ 남긴 30% 가 이 글의 핵심이다** — 공인 재무 지표·실시간 운영 모니터링·반복 표준 보고는 **일부러 전통 BI 로 남겼고**, 가드레일로 **DWAINE 출력만으로 중요 결정을 못 하게** 한다. 선택도 적는다 — Claude 4.0(AWS Bedrock) · **벤더 종속을 피하려 LibreChat** · MCP · 자체 호스팅 |
