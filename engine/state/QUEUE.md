...
| 회사 | 국가·분류 | 상태 |
|---|---|---|
| **Block (Square)** | US · 핀테크 | ⭐ **기능 4/4 — 도메인은 다 찼다**(2026-09-06). ✅ Moneybot 자가치유 · ✅ COLDCARD 난수 자문 · ✅ 벌 계층 · ✅ MeshLLM. ⚠️ **그래도 `done` 이 아니다 — 수익원 `커머스 인에이블먼트(Square)` 가 어느 기능과도 안 이어진다.** **다음 사이클에서 Square 본업(결제·정산·대출 심사) 자료를 찾아 도메인을 더 세우거나, 없다는 사실로 닫는다.** ⚠️ **`--gaps` 는 이걸 못 집는다**(빈 도메인이 없어 `정리할 것이 없다` 로 넘어간다). |
