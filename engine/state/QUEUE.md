...
- **Vinted** (EU · 커머스) — 사이클 181 에서 열었다. **33번째 회사, 발트 첫 회사.**
  프로파일 + 도메인 지도 + 도메인 3개(**아이템 검색** · **데이터베이스 샤딩** · **자체 데이터센터 망**).
  **기능 2개 완료**(`item-search` 182 · `vitess-sharding` 183).
  **남은 것은 `자체 데이터센터 망`** 하나 — 자료 둘(Clos+eBGP 2020 · clocksource 2025) 확인 완료.
  **채우면 done 으로 닫고 다음은 Zerodha 개설이다.**
  ⚠️ **`vinted.com/pricing` 은 404 다** — 구매자 보호 수수료 요율의 1차 문서를 못 찾았고
  **매출 구성도 회사가 나누어 밝히지 않는다.** 지어내지 말 것.
