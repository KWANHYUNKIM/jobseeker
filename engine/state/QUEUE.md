...
| 회사 | 국가·분류 | 상태 |
|---|---|---|
| **LinkedIn** | US · 소셜(구인·구직) | **기능 2개**(`ai-code-review` 228 · `qa-agent` 229). `자기 개발 과정에 AI 를 붙일 때 무엇을 통제하는가` 도메인이 찼다. **`랭킹 모델을 얼마나 빨리 다시 만들 수 있는가` 는 여전히 0개** — 본문이 증류 글 한 편뿐이고 목록·분류 페이지를 못 여니 **다음 사이클에 `hold_reason` 을 붙이고 닫을지 정한다** |
