...
|---|---|---|---|---|
| **bol** | EU · 리테일 | `techlab.bol.com` | ✅ **본문을 열어 확인했다**(`/en/blog/splitting-the-data-of-the-monolith-because-who-needs-to-sleep-anyway/`) | **이번 조사에서 가장 강하다 — 3/3 이 아니라 3/3+ 다.** 열어 본 글은 `Splitting the data of the monolith`(2024-11-07) — **모놀리스의 4TB 중 3TB 를 두 마이크로서비스로 옮긴 이야기**(상점 주문·고객·부가세 2TB, 물류 이벤트 1TB). **⭐ 버린 대안이 셋이고 각각 이유가 붙는다** — ① 중개 앱을 2031년까지 유지(불필요한 앱을 무한정 떠안는다) ② 스케줄 잡으로 데이터를 밀기(클라우드 비용과 소요 시간을 모른다) ③ 전 물류 이벤트를 리플레이(비용이 더 크고 **과거 데이터 불일치를 손으로 고쳐야 한다**). **🔴 실패를 셋이나 적는다** — 한 테이블이 **3일 넘게 돌다 미완으로 멈췄고**(서브셀렉트가 O(N제곱)이었다), DMS 가 **18시간 잘 돌다 외래키 하나 때문에 반복 실패해 2주를 조사에 썼고**, 인덱스 생성이 **1시간 넘게 걸려 Flyway 트랜잭션이 롤백되며 재시작 루프**에 빠졌다. **수치가 촘촘하다** — DB CPU **4 퍼센트 → 15 퍼센트**(병렬화), 한 테이블 **6시간 → 7시간 → 10~12시간**(스크립트를 O(2N)으로 고쳐서), DMS 잡 하나당 **19~23시간**(두 잡을 순차로만 돌릴 수 있다), 준비 1~2시간 · 실행 1~2일 · 복구 3~6시간, **전체 2~3개월.** **⚠️ 최신 글이 2024-11-07 이라 그 뒤가 없다**(재방문 가치는 낮을 수 있다). 기술 글 비중은 열 편 중 일곱으로 높다. **⭐ 축이 새롭다 — EU 리테일이고 이 사이트에 없는 결이다.** |
