...
- **Yelp** (US · 검색) — 사이클 207 에서 열었다. **39번째 회사.**
  **도메인 3개** — `안 쓰는 데이터를 알아보는 법`(2편, 1편 본문 확인) ·
  `제자리에서 갈아 끼운다`(5편, 1편 본문 확인) ·
  `광고비가 매출인 회사의 매출 운영`(4편, 1편 본문 확인 — **⚠️ 그 한 편에 버린 대안이 없었다**).
  **기능 0개** — 다음 사이클부터. **`ui_map` 을 넣었다**(토스 다음으로 두 번째).
  ✅ **사업 수치는 IR 첫 화면에서 받았다** — 2026 Q2 순매출 **$376M**, 유료 광고 지점 **51만 곳**,
  누적 리뷰 **3.3억 건**, 앱 순 기기 **2,800만 대**, 소유권 주장 지점 **840만 곳**.
  ⚠️ **매출 구성(광고/기타·업종별)은 못 찾았다** — 분기 실적·보도자료 페이지가 **JS 렌더링**이라
  WebFetch 로 본문이 안 온다. **브라우저로는 열릴 수 있다.**
  ⚠️ **블로그는 `/page/N/` 로만 넘긴다**(끝 슬래시 없으면 http 로 301). **43페이지 중 3페이지만 봤다.**
