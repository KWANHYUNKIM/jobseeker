...
| 회사 | 국가·분류 | 상태 |
|---|---|---|
| **Paytm** | IN · 핀테크 | 321 에서 네 번째 도메인 `재는 일을 사지 않고 만든다` 를 열고 기능 `own-the-measuring`(재는 일을 직접 떠안는다)까지 썼다 — **도메인 4개 · 기능 2개 · 결정 6개.** ⚠️ **블로그 열 편을 전부 읽었다.** 남은 도메인 둘(거래 기록 · 앞단에서 끝낸다)은 **재료가 기준을 못 넘겨 기능 없이 남는다** — 이유는 `open_questions` 에 있다. **다음 사이클은 닫는 판단이다** |
