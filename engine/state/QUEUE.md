...
|---|---|---|---|---|
| **Cloudflare** | US · 인프라 | `blog.cloudflare.com` | ✅ WebFetch (2026-08-21 확인) | **인프라 자체가 제품인 첫 회사.** 지금까지 판 15곳은 전부 자기 서비스를 지켰지만 여기는 **남의 트래픽이 곧 내 문제**다. Workers KV 재설계 글에 단일 공급자로 줄인 결정 → 2시간+ 장애 → 이중화로 되돌린 과정이 대가까지 다 있다(p99 5ms 대 200ms, 중간값 288바이트, 초당 수백만 GET). RYOW 일관성 회귀를 스스로 찾아 고친 기록도 있다 |
| **Datadog** | US · SaaS(관측) | `datadoghq.com/blog/engineering` | ✅ WebFetch (2026-08-21 확인) | **남의 시스템을 보는 것이 제품**인 축. Husky 글이 정확히 한 번 수집과 멀티테넌시를 함께 다루는데, 배민 주문 접수의 `정확히 한 번`과 Shopify 멀티테넌시가 여기서 한 문제로 만난다. 버린 대안이 명시적이다(ID 전량 메모리 보관·무작위 라우팅·단순 일관 해싱). Rust 시계열 엔진 글은 Redis 를 떠난 이유를 대가로 적는다 |
| **Notion** | US · SaaS(문서) | `notion.com/blog` | ✅ WebFetch (2026-08-21 확인) | **블록 하나가 모든 것인 데이터 모델**이 새 축이다. 샤딩은 Figma 와 겹치지만 **그게 오히려 비교 재료** — Figma 는 논리(뷰)→물리 순서로 갔고 Notion 은 480 논리 샤드를 32→96 물리로 재분배했다. Citus·Vitess 를 버린 이유("클러스터링 로직이 불투명하고 분포를 우리가 통제하고 싶었다")가 Figma 의 "직접 만든다"와 같은 자리다 |
...
- Airbnb — `medium.com/airbnb-engineering` (브라우저 필요)
