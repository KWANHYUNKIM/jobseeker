...
| 회사 | 국가·분류 | 근거 |
|---|---|---|
| **Block (Square)** | US · 핀테크 | ⭐ **`큰 이름인데 아직 없는 회사` 각도, 네 조사 연속.** `engineering.block.xyz` 최신 **2026-09-01**. **자체 시스템을 이름 대고 판다** — `Buzz`(사내 에이전트 플랫폼)를 **다섯 편 연작**으로 쓰고(`MeshLLM 으로 컴퓨트 공유` · `릴레이 직접 굴리기` · `토큰 효율과 팀 구성` · `에이전트 설정` · `프로젝트`), `Autohealing Moneybot`(LLM 확률적 실패를 고치는 자동 수리 루프) · `BlockTabBench`(테이블 모델 평가) · **COLDCARD 펌웨어의 RNG 폴백·32비트 재시드**까지 층이 다르다. ⭐ **결제(Square)·비트코인 하드웨어(COLDCARD)·사내 에이전트가 한 회사 안에 있어 도메인이 저절로 갈린다.** |
| **Klaviyo** | US · SaaS | `klaviyo.tech` — **제목이 전부 내부 구조다**: `카프카로 분산 우선순위 큐 만들기` · `분산 ML 워크로드를 돌리는 안 부서지는 시스템` · `Aurora Blue/Green 배포 경험` · `대규모 에이전트 시스템을 안전하게`. ⭐ **`분산 우선순위 큐`는 이 엔진에 없는 축이다**(큐는 많지만 우선순위를 붙인 이야기는 아직 없다). ⚠️ **목록의 최신 날짜가 `Apr 23` 로 연도가 안 붙어 있다** — 파기 전에 개별 글에서 연도를 확인한다. |
