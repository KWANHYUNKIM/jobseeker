...
| 회사 | 국가·분류 | 상태 |
|---|---|---|
| **Block (Square)** | US · 핀테크 | ⭐ **기능 2/4**(2026-09-06). ✅ `확률적으로 틀리는 것을 고친다`(Moneybot 자가치유 루프) · ✅ `키를 만드는 난수`(COLDCARD RNG 자문 — 결정 8개, 그림 2장, 연표 2세대). ⏳ 남은 둘 — `에이전트가 팀으로 일한다`(Buzz) · `노는 컴퓨트를 모은다`(MeshLLM). ⚠️ **Square 본업(결제·정산·대출 심사) 자료는 아직 못 읽었다.** |
