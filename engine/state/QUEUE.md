...
| 회사 | 국가·분류 | 상태 |
|---|---|---|
| **Freshworks** | IN · SaaS | 325 에서 둘째 기능 `rename-before-push`(밀기 직전에 이름을 갈아 끼운다)를 썼다 — **도메인 2개 · 기능 2개 · 결정 6개.** **🔴 325 에서 블로그 범위를 정정했다** — `/saas/` 아래에 엔지니어링 글이 더 있다(무중단 Rails 업그레이드 · Mozilla rr 로 메모리 손상 디버깅 · CloudFront 사설 콘텐츠 · 기능 토글 · 문자열 보간). **다음 사이클은 그것들을 열어 새 도메인이 서는지 본다** |
