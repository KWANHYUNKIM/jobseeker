...
*(없다 — Target 을 사이클 224 에서 닫았다. 다음은 **후보 조사**다 — 큐가 1/3 이다.)*

## 대기

| 회사 | 국가·분류 | 1차 자료 | 접근 | 왜 이 회사인가 |
|---|---|---|---|---|
| **bol.com** | EU(NL) · 커머스 | `techlab.bol.com/en` | ✅ **본문까지 확인**(`blog/splitting-the-data-of-the-monolith-because-who-needs-to-sleep-anyway/`, 2024-11-07) | **모놀리스의 데이터를 쪼개는 이야기가 수치로 빽빽하다** — **4TB 중 3TB** 를 두 마이크로서비스로(**약 2TB** 주문·고객·VAT, **약 1TB** 물류 이벤트), 이관 잡 하나가 **19~23시간**, 전체 **2~3개월**, DMS 잡이 *"18시간을 잘 돌다가"* 실패, 문제의 테이블 하나는 **사흘 넘게** 걸리다 중단 → 최적화 후 **약 7시간** → 최종 **10~12시간**, CPU 사용률 **4% → 약 15%**, 조사에만 **약 2주**. **버린 대안 셋에 장단점이 각각** 적혀 있다 — 중개 앱을 2031년까지 돌리기(*"불필요한 애플리케이션을 하나 더 유지"*) · 스케줄 잡(*"클라우드 비용 증가, 소요 시간이 불확실"*) · 물류 이벤트 전량 재생(*"더 큰 클라우드 비용, 더 오래 걸림, 과거 불일치를 수동으로 고쳐야"*). *"모든 경우에 트레이드오프가 너무 컸다"* 며 전문가에게 물었다는 것도 적는다. **Careem `cost-leaks`(DMS Serverless 로 270TB→78TB)와 나란히 놓을 재료**다. ⚠️ **최신 글이 2024-11 이라 블로그가 멈춰 있을 수 있다 — 열 때 최신 글을 먼저 확인할 것** |
| **Grab** | SG · 모빌리티(슈퍼앱) | `engineering.grab.com` | ✅ **본문까지 확인**(`counter-service-storage-migration`, 2026-07-03) | **동남아가 이 사이트에 0곳이다.** 블로그가 크고 살아 있다 — **최신 글 2026-08-21**, **24페이지 × 10편 ≈ 240편**, 분류별 목록이 열린다(`/categories/engineering/` **84편** · Data 33 · Data Science 24 · ML 23 · Security 23 · Database 20 · Optimization 19). 읽은 글은 **안티프라우드의 카운터 서비스 저장소 이관** — *"사용자의 최근 승차 요청이나 카드의 결제 실패 횟수 같은, 시간 창 안의 개수 질문에 답하는 데 쓰인다"*. **버린 대안이 셋, 이유가 각각 있다**: 보조 인덱스 방식과 BatchGet 방식이 *"같은 벽에 부딪혔다 — Aerospike 의 기본 인덱스는 레코드당 64바이트이고 메모리에 상주한다. 수십억 레코드에서는 인덱스 메모리가 제약이 된다"*, 보조 인덱스는 *"우리에게 필요 없는 오버헤드와 운영 복잡성을 더했다"*; **인덱스를 NVMe 로 옮기는 실험은 되돌렸다** — *"노드를 더 좋게 더 많이 넣어도 조금 나아질 뿐 문제를 완화하지 못했다. 그래서 인메모리 인덱스로 되돌렸다."* **대가를 스스로 적는다** — 복제 계수를 3에서 2로 낮추며 *"남은 위험인 동시 다중 AZ 장애는 이 워크로드에서 감수할 만했다"*, Rust 클라이언트가 **동기라서** `tokio::task::spawn_blocking` 으로 다리를 놨고, 클라이언트가 *"초기화 때만 시드 호스트명을 해석하고 토폴로지가 갱신될 때 다시 해석하지 않았다"*, p99 스파이크가 QPS 가 아니라 **핫키의 I/O** 를 따라갔다. 수치 — **수만 QPS**, **하루 약 10억 요청**, 섀도 램프 **5%→20%→50%→100%**, 노드당 메모리 100GB 중 **50GB**, 디스크 **약 1TB**(기존 약 3TB), **p99 읽기 약 50% 개선**, **노드당 비용 약 45~50% 절감**. **자리**: Careem(중동 승차)과 달리 **안티프라우드가 도메인으로 서 있고**, 쿠팡·Target 과 달리 **자기 차량도 자기 창고도 없는 양면 시장**이다 |
| **Spotify** | SE · 스트리밍(음악) | `engineering.atspotify.com` | ✅ **본문까지 확인**(`2026/7/indexing-the-data-lake-for-online-point-queries`) | **이 사이트의 스트리밍 둘(넷플릭스·유튜브)이 다 영상이다 — 음악이 없다.** 블로그는 살아 있다(**최신 글 2026-08-13**), 묶이는 주제는 Developer Tools 6 · Developer Experience 5 · Platform 5 · Data 4. 읽은 글은 **데이터 레이크에 점 조회를 얹는 이야기**(Random Access Parquet) — *"포털이나 사용자의 청취 이력을 쓰는 개인화 기능 같은 온라인 서비스는 사용자별 데이터를 대화형 속도로 조회하고 페이지 넘김 할 수 있어야 한다"*. **버린 대안에 이유가 붙어 있다**: 분산 SQL 엔진 — *"Trino 나 BigQuery 같은 분산 SQL 엔진은 단일 행 조회에도 잡 스케줄링과 질의 계획에 몇 초를 더한다"*; 인코딩 — *"각 프레임 안의 값은 앞 프레임 없이 해석 가능해야 하므로 델타·런렝스 인코딩이 배제된다"*. **대가가 다섯 줄이나 명시돼 있다** — *"배치 분석은 블롭 안에서 필드별 컬럼 프루닝과 조건 푸시다운을 잃는다"*, *"인터리브된 단일 컬럼을 읽는 기존 리더는 다른 컬럼의 데이터도 죽은 공간으로 읽어 I/O 를 배로 늘린다"*, *"거친 파티셔닝은 배치 질의의 파티션 프루닝 입도를 떨어뜨린다"*. 수치 — GCS **30~100ms**/요청(S3 Express One Zone·GCS Rapid 는 **한 자릿수 ms**), 하루 **1,000 파일** × 90일 = **90,000 파케이 파일** → 후보가 **90,000 → 90 → 12일치**, **4MB 페이지에서 100바이트**를 뽑고, 키당 **연 365 파일**(주간 단위면 52), *"테라바이트를 색인하면 기가바이트, 페타바이트를 색인하면 테라바이트"*. **자리**: PlanetScale·ClickHouse·TigerBeetle 은 **DB 를 파는 회사**인데 여기는 **자기 데이터 레이크 위에 조회 계층을 얹는 쪽**이다 |
