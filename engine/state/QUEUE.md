...
| 회사 | 국가·분류 | 근거 |
|---|---|---|
| **VictoriaMetrics** | 다국적 · 인프라(시계열 DB·관측) | ⭐ **이미 판 두 회사가 고른 도구를 만든 곳이다** — **Airbnb 와 Razorpay 가 둘 다 `vmagent` 를 골랐다.** `The Life of a Metric`(v1.147.0 기준) 본문 확인 — **설계 근거**(**`디스크의 데이터는 TSID 로 정렬되고, TSID 는 MetricID 앞에 세 개의 묶음 필드를 둔다 … 그래서 같은 메트릭·job·instance 의 시리즈가 디스크에서 나란히 놓여 압축이 잘 되고 스캔이 싸진다`**) · **임계값이 있는 결정**(**`zstd 가 어떤 블록을 최소 약 10% 만큼 줄이지 못하면 델타 인코딩된 평문 바이트를 그대로 둔다`**) · ⭐ **비용의 정체를 말한다**(**`태어나는 것이 이 메트릭 생애에서 비싼 부분이고 자라는 것은 싸다 … 원시 샘플 양이 아니라 카디널리티가 시계열 DB 를 짓누른다`**). ⚠️ **위험 — 블로그의 나머지가 제품 발표 위주다**(`Q2 2026 이상 탐지 업데이트` 류). **이 글 하나는 확실하지만 두 번째 기능에서 자료가 마를 수 있다** — 그때는 Go 내부 글(`How Go's Built-in Map Works` 등)로 내려간다 |
