...
| 회사 | 국가·분류 | 상태 |
|---|---|---|
| **Turso** | US · SaaS | ⭐ **프로파일 완료(2026-09-06) — 도메인 2개.** `재현을 설계로 만든다`(결정론적 시뮬레이션 테스트 · 시드로 재현 · SQLite 를 정답지로 한 차분 테스트 · Antithesis) · `동기 인터페이스를 비동기로 연다`(io_uring · 620ns→506ns · 파일 형식 호환 유지). ⭐⭐ **재작성의 이유가 성능이 아니라 `SQLite 의 테스트 스위트가 독점`이라는 것이다.** ⏳ **기능 0/2 — 다음은 확장, 재현부터.** ⚠️ **`데이터베이스의 LLVM` 글이 404 라 그 축은 도메인으로 안 세웠다.** |
