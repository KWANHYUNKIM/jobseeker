...
| 회사 | 국가·분류 | 상태 |
|---|---|---|
| **monday.com** | 기타(이스라엘) · SaaS(협업 보드) | **285 기능 1개** — `board-query-engine`(열로 세워 두고 두 층을 겹쳐 읽는다, 결정 8). ① 1개 · ② 0개(**Morphex 재료 확보, 두 번째 편 필요**). **⭐ 3년 간격 두 편으로 썼다** — 2023 Redis+Cassandra → 2026 DuckDB+WAL, **그때 남긴 숙제가 다음 판 설계가 됐다**. **⚠️ 관측·신뢰성 묶음 4편 미독.** **⚠️ 총 ARR 실액 미확인** |
