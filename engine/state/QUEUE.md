...
|---|---|---|---|---|
| **Twilio (Segment)** | US · SaaS(통신·데이터) | `twilio.com/en-us/blog` | ✅ WebFetch 로 본문이 온다. ⚠️ **대부분은 고객용 아키텍처 가이드** — 자기 시스템 글은 검색으로 골라야 한다 | ✅ **본문 확인함**(`Strangler Fig 로 Segment 의 알림 아키텍처를 바꾸다`, 2025-09-22) — **자기 시스템 사례 연구다.** **모놀리식 알림 서비스를 한 번에 다시 쓰는 길(big bang)을 버리고** 점진 교체를 골랐고, **API 운영 · 이벤트 처리(Kafka) · 시간 기반 스케줄링** 셋으로 갈랐다. ⚠️ **대가를 적는다** — **전환 기간에 레거시와 새 시스템에 이중 쓰기**를 유지해 일관성과 롤백을 얻었고, **수십 개 독립 서비스 대신 워크플로 오케스트레이션(Orkes)을 골라 `통합 플랫폼의 운영 단순성` 을 `순수 마이크로서비스 모듈성` 보다 앞에 뒀다.** ⚠️ **수치가 없다.** ⭐ **`한 번에 갈아엎지 않고 목 졸라 죽이기` 라는 이전 전략이 이 사이트에 아직 없다** |
