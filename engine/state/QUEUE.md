...
| 회사 | 국가·분류 | 상태 |
|---|---|---|
| **Meituan (美团)** | CN · 커머스 | **301 새 도메인 ② 를 열고 기능까지 — 도메인 2개 · 기능 2개.** ① `검색이 무엇을 위로 올릴지 정한다`(`semantic-before-text`) · ② `무엇을 잘한다고 부를지 정한다`(`binarize-the-rubric` — **결정 8개**, 안 되는 방식 다섯 · 한계 다섯 · 미해결 다섯). **⭐⭐ Snap 의 '자를 어떻게 세우는가' 축에 다섯째·여섯째를 더했다.** **🔴 옛 글은 404 — 홈의 href 만 믿는다.** 실적 — Q2 2026 매출 **1,050억 위안(+14.4%)** |
