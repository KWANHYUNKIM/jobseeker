...
|---|---|---|---|---|
| **Fastly** | US · CDN·엣지 컴퓨트 | `fastly.com/blog` + `/blog/category/engineering` | ✅ WebFetch 로 목록·본문 다 온다 | **캐시 무효화 전파가 이 엔진에 없는 문제다.** 자체 퍼징 시스템 `Powderhorn` 글이 **중앙집중과 분산을 나란히 놓고 버린 이유를 적는다**(단일 장애점 · 네트워크 장애 시 일부 서버 도달 불가 → *"available as long as a customer could reach any Fastly server"*). **Bimodal Multicast** 로 직접 브로드캐스트(UDP)와 가십을 겹쳐 쓴다 — **논문 기반이라 research 칸의 재료가 된다.** 프로덕션 수치도 있다(POP 간 패킷 손실 **0.1% 미만** · 피어 3분의 2가 막힌 분할에서 **p95 약 10초** · DDoS **8분** 장애 중 **p95 1분 미만**). ⚠️ 첫 화면은 보안 리서치·제품 발표라 **첫 화면으로 판정하지 않는다 규칙을 다시 썼다.** Cloudflare 와 축이 겹치지만 **엣지 무효화라는 다른 문제**이고 비교 문서 재료가 된다 |
| **Monzo** | EU(UK) · 핀테크(네오뱅크) | `monzo.com/blog` (기술 태그 + 연도별 장애 회고) | ✅ WebFetch 로 목록·본문 다 온다 | **영국 네오뱅크 자리가 비어 있다**(kakaobank·toss·nubank 는 있다). **장애 회고를 자기 이름으로 쓴다** — 2019-07-29 장애 글이 **설정 플래그 하나의 오해**를 그대로 적는다: `auto_bootstrap: false` 가 데이터 스트리밍만 막는 줄 알았는데 **새 서버가 활성 상태로 합류하는지도 같이 정했다.** 빈 서버 6대가 즉시 읽기·쓰기를 받아 `not found` 를 돌려줬다. **⚠️ 1대로 테스트하고 운영에 6대를 넣었다.** 대가도 적는다 — 단일 클러스터는 운영이 단순한 대신 **한 번의 변경이 전체에 닿는다**(쪼개면 격리는 좋아지고 운영 복잡도는 는다). **수치** — 노드 **21대·복제 3** · 장애 **약 2시간** · 데이터 정합 복구 **8시간**. 데이터 쪽도 규모가 크다 — **dbt 모델 12,000개 이상·팀 100개 이상**, 4계층·사내 도구 `Modelgen`·CI 가 PR 을 막는 데이터 표준, 이전 30% 시점에 **비용 약 40%↓ · 랜딩 약 25%↓** |
