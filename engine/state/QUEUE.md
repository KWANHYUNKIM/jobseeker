...
| 회사 | 국가·분류 | 상태 |
|---|---|---|
| **컬리** | KR · 커머스(신선식품) | **259 회사 프로파일 완료. 도메인 2개 · 기능 0개.** ① `문 앞에 두고 간 것이 맞는지 판정한다`(On-device 오배송 탐지 — **양치기 소년 현상을 이름 붙여 적는다** · 채널마다 임계를 다르게) · ② `남의 물건을 대신 받아 준다`(3PL 입고 outbox — **버린 대안에 이유가 명시적**). **각 도메인의 두 번째 글을 아직 안 읽었다** — 엔지니어링 카테고리가 **7페이지(70편+)** 이고 1쪽에만 본업 글이 여섯이다(Kafka Streams 윈도우 2025-12 · 딜리버리 암호화 2025-06 · OMS MSA 2025-05 · 서비스 점검 2부작 2025-10). **⚠️ 사업 수치를 하나도 못 찾았다** — kurlycorp.com DNS 없음 |
