...
- **Allegro** (EU · 커머스) — 사이클 176 에서 열었다. **32번째 회사, 동유럽 첫 회사.**
  프로파일 + 도메인 지도 + 도메인 3개(**검색 적합성 평가** · **프론트엔드 배포** ·
  **저장을 어디에 둘 것인가**). 기능 1개 완료(`search-relevance-judge`, 사이클 177).
  **다음은 `프론트엔드 배포`**(자료 정독 완료: opbox-web · 126팀 · 의존성 1,206개), 그 다음이
  `저장을 어디에 둘 것인가`. **둘 다 끝나면 done 으로 닫는다.**
  ⚠️ **검색 랭킹 본체와 Allegro Pay 여신 자료는 못 찾았다** — 억지로 도메인을 열지 말 것.
