...
| 회사 | 국가·분류 | 상태 |
|---|---|---|
| **Wix** | 기타(이스라엘) · SaaS(웹사이트 빌더) | **293 도메인 ② 첫 기능 완료 — 기능 2개.** ① `에이전트에게 당번을 맡기되 열쇠는 안 준다`(**두 편 읽음** — `context-before-fix`. **남은 글 다섯 편**) · ② `멈추지 않고 갈아 끼운다`(**두 편 정독** — `read-first-then-write`, 결정 8개. **버린 대안 셋**: 덤프·DMS·다중소스 복제). **⭐ 같은 문제를 인프라 층과 앱 층에서 따로 푼 대비가 남아 있다** — Inbox 편은 별도 기능 후보. **실적 확보** — Q2 2026 매출 **5억 6,300만 달러 · 15% 성장** |
