...
| 회사 | 국가·분류 | 상태 |
|---|---|---|
| **하이퍼커넥트** | KR · 소셜(실시간 영상) | **기능 2개 · 도메인 2개 — 열어 둔 도메인이 다 찼다.** 255 `objective-relaxation`(결정 8, **가정이 실제로 깨진 기록**) · 256 `stream-join`(결정 8, **버린 대안 다섯**). **세 번째 도메인 후보** — 쿠버네티스/인프라 3편(ScyllaDB Super Disk 2025-05 · ARM64 2023-07 · 클러스터 이전 2020-11, **미개봉이고 둘이 오래됐다**). **다음 사이클에 그 도메인을 열지 회사를 닫을지 정한다.** **⚠️ 사업 수치를 못 찾았다** — Match Group IR 에 아자르가 안 나온다 |
