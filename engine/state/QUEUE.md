...
| 회사 | 국가·분류 | 상태 |
|---|---|---|
| **Perplexity** | US · 검색 | ⭐ **기능 2/3**(2026-09-06). ✅ `요청을 GPU 앞까지 나른다`(Ivy·Tulip) · ✅ `모델을 GPU 위에서 돌린다`(ROSE — CUDA 그래프 지연 캡처, LazyTensor). ⏳ **마지막 하나** — `GPU 끼리 직접 말하게 한다`(fabric-lib, **arXiv 본문이 직접 열린다**). ⚠️ 회사 블로그는 목록·개별 글 모두 403 — 3자 인용으로 읽는다. |
