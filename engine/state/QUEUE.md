...
| 회사 | 국가·분류 | 상태 |
|---|---|---|
| **Sansan** | JP · SaaS | 334 에서 **다섯 번째 도메인 `컴파일러가 엄해지기 전에 옮긴다` 를 열고 기능 `layer-by-layer-async` 까지 썼다** — **도메인 5개 · 기능 5개 · 결정 7개.** **⭐ 다섯 도메인이 같은 회사의 다섯 층이다** — 개발 환경 · 인식 · 공정 실적 · 실물 · 앱의 동시성 모델. **⚠️ 이 도메인은 글 한 편으로 썼다**(밝혔다). 다음은 Jetpack Compose 이관 계획을 보고 닫는 판단 |
