...
| 회사 | 국가·분류 | 상태 |
|---|---|---|
| **freee** | JP · SaaS | 339 에서 첫 기능 `refactor-before-you-cut`(떼어내기 전에 옛것을 정리했어야 했다)를 썼다 — **도메인 1개 · 기능 1개 · 결정 8개.** ⚠️ **도메인을 글 한 편으로 썼다**(밝혔다). **⭐⭐ SmartHR 과의 정반대 순서 대비를 `open_questions` 에 남겼다** — 다음 비교 문서의 씨앗이다 |
