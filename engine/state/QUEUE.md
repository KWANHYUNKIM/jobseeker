...
*(없다 — Doximity 를 사이클 214 에서 닫았다. 다음은 후보 조사다.)*

## 대기

| 회사 | 국가·분류 | 1차 자료 | 접근 | 왜 이 회사인가 |
|---|---|---|---|---|
| **Moniepoint** | NG · 핀테크 | `engineering.moniepoint.com` | ✅ **본문까지 확인**(`save-as-you-transact...`, `high-availability-in-production...`) | **아프리카가 이 사이트에 0곳이다.** 핀테크는 두껍지만 **자리가 다르다** — 토스(개인 슈퍼앱)·Stripe(개발자 API)·Adyen(가맹점 인프라)·Zerodha(증권)와 달리 **나이지리아의 사업자 뱅킹과 POS 에이전트 망** 위에 서 있다. **수치가 단단하다**: **월 거래액 170억 달러**, **1,000만 이상의 사업자·개인**, 목표 **99.99% 가동률**, *"0.1% 가용성 저하 = 수천 건의 거래"*, 서비스는 *"피크의 2배까지 지연 곡선이 평평한지"* 를 보여야 배포된다. **버린 대안도 이유와 함께 적는다**: ① 거래 서비스가 저축 서비스를 직접 호출하는 안(*"데모에서는 되고 프로덕션에서는 무너진다"* — 저축이 모든 거래의 임계 경로가 된다) ② **2단계 중복 제거**(*"매 이벤트마다 DB 왕복 두 번이 들고 경쟁 구간이 열린다"*) → insert-first ③ 기본 Spring Boot Actuator 헬스체크(*"프로덕션에서 예측 불가능하고 서비스를 불필요하게 로테이션에서 튕겨낸다"*). **대가도 한 줄로 적혀 있다** — 오래된 이벤트를 버리는 것에 대해 *"약간의 완전성을 훨씬 큰 신뢰와 맞바꿨다"* |
