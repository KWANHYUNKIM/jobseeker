...
| 회사 | 국가·분류 | 상태 |
|---|---|---|
| **Zepto** | IN · 커머스(퀵커머스) | 프로파일 완료 · 도메인 3(재고가 계속 바뀌는 시장에서 검색한다[tech 0 · ✅ 본문 확인] / 10분을 지키려고 사람과 경로를 배치한다[tech 0 · ⏳ 미독] / 광고를 팔려면 하루 10억 이벤트를 세야 한다[tech 0 · ⏳ 미독]) · **기능 0** · ⚠️ **`blog.zepto.com` 은 `r.jina.ai` 를 앞에 둔다**(Medium 계열이라 개별 글은 브라우저도 된다) |
