...
- **Canva** (AU · SaaS) — 사이클 158 에서 열었다. **호주 첫 회사**(validate.py 의 COUNTRIES 에
  `AU` 를 추가해야 했다). 프로파일 + 도메인 2개(세션·인증 게이트웨이 · 인쇄 주문 라우팅) +
  기능 1개(`session-revocations`) 완료. ✅ 블로그가 잘 읽히고 **버린 안과 대가가 명시**돼 있다.
  **다음 기능은 `인쇄 주문 라우팅`** — 본문 확인 완료(최소비용유량 변형 O(VE) + 후회, 다익스트라를
  버린 이유가 명시, p99 50ms · 가용성 99.999%). **두 번째 글이 필요하다** — 아직 못 찾았다.
  안 읽은 글: `snowpipe-streaming`, `image-replacement-...-reverse-image-search`.
