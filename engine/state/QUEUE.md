...
|---|---|---|---|---|
| **CyberAgent** | JP · 미디어·광고 | `developers.cyberagent.co.jp/blog/` | ✅ WebFetch 로 목록·본문 다 열린다 | **246 에서 본문을 열어 확인했다** — `archives/65145`(저지연 광고 배송의 캐시 전략 쇄신, 2026-08-04)이 **버린 대안 둘에 각각 이유를 적는다**: 안1(최근 Deal ID 를 외부 저장소에 두고 새 Pod 이 시작 시 일괄 취득) 은 *"신구 Pod 에서 서로 다른 취득 경로를 유지해야 한다"* · *"용도에 비해 구성이 과잉"*, 안2(로컬 캐시를 없애고 전부 공유 캐시로) 는 *"모든 캐시 취득에 네트워크 I/O 가 발생해 레이턴시에 영향"* · 공유 캐시가 병목 · 단일 장애점. 고른 것은 **L1(로컬) + L2(공유 Valkey) 2단 캐시**. **대가도 다섯 줄** — 롤링 업데이트 중 신구 스키마 충돌, L2 미스 시 *"응답 시간이 다른 Pod 에 좌우되고 구현도 복잡해진다"*, 공유 캐시 장애 범위, 스키마 해시가 바뀌는 릴리스는 여전히 단계적 배포가 필요, **지금도 안전을 우선해 L2 를 10분 데운 뒤 나머지 Pod 을 편다.** 수치도 소속이 분명하다 — Pod 4개 동시 기동에 **Spanner p99 약 4배**, 릴리스 **약 40분 → 약 15분**, TTL **L1 1분 / L2 4분**. 스택 **Go · GKE · Spanner · Memorystore for Valkey · Argo CD/Rollouts · singleflight · Datadog**. **자리** — 이 사이트에 **광고를 파는 회사가 없다**(넷플릭스·유튜브는 미디어, 구글 없음). 그리고 **ABEMA 태그 125편**이 일본의 실시간 스트리밍이라 넷플릭스·유튜브와 다른 축일 수 있다. **⚠️ 239 에서 본 Bucketeer 글은 얇았다** — 글마다 밀도가 다르니 도메인을 열 때 **광고 인프라 쪽부터** 본다 |
