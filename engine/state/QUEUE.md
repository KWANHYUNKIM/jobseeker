...
*(없다 — Mercari 를 사이클 205 에서 닫았다. 다음은 후보 조사다.)*
...
| **Doximity** | US · 기타(의료) | `technology.doximity.com` | ✅ **본문까지 확인**(`articles/from-batch-snapshots-to-near-real-time-data`, 2026-08-04) | **의료가 이 사이트에 0곳이다.** 의사용 전문 네트워크라 축이 완전히 다르다 — 개인 사용자도 가맹점도 아닌 **면허 있는 전문가**가 사용자이고, 그래서 신원 확인과 데이터 취급의 제약이 커머스·핀테크와 다르다(이 관찰은 이 사이트의 것). 읽은 글이 **버린 대안을 넷 다 밝힌다**: ① 초기 적재를 Debezium 스냅샷에 맡기지 않고 기존 배치 스냅샷을 쓴다(*"커넥터가 관리하는 MySQL 스냅샷은 전역·테이블 락을 잠시 잡을 수 있다"*, 큰 테이블의 두 번째 풀스캔도 피한다) ② 토픽↔테이블 1:1 매핑을 버리고 **팬인/팬아웃**(1:1 은 *"새 소스 테이블마다 토픽 매핑과 커넥터 배포가 필요"*) ③ 매 이벤트 즉시 병합을 버리고 **base+delta 뷰**(병합을 미뤄도 뷰로 보이므로 신선도가 안 깨지고, 물리 병합을 묶음으로 돌린다) ④ **binlog 만 보는 것을 버렸다** — *"캐스케이드로 생긴 자식 삭제는 바이너리 로그에 없어"* 고아 행이 남는다. 그래서 외래키 정의를 읽어 **합성 삭제 이벤트**를 만든다. **수치**: 표본 이벤트의 **95%가 Kafka 발행 후 8분 안에**, **99%가 13분 안에** 조회 가능 |
