...
- **Moniepoint** (NG · 핀테크) — 사이클 216 에서 열었다. **41번째 회사, 아프리카 첫 곳.**
  `NG` 를 **validate.py COUNTRIES 와 index.json countries 양쪽**에 추가했다.
  **도메인 2개** — `멈추면 안 되는 것을 프로덕션에서 굴린다`(글 3편 이상, 2편 본문 확인) ·
  `돈의 기록이 맞다는 것을 어떻게 아는가`(2편, 1편 본문 확인).
  **⚠️ 블로그가 작다 — 전부 8편 남짓이고 `payments` 와 `data` 분류는 비어 있다.**
  Careem(6편에 도메인 셋은 과했다) 교훈대로 **2개만** 열었다. **기능 0개** — 다음 사이클부터.
  **`ui_map` 을 넣었다**(토스·Yelp·Doximity 다음 네 번째).
  ✅ **사업 수치는 회사 홈페이지에서 받았다** — 계정 **2,000만+** · 월 TPV **$17B** ·
  일 결제 **2,600만 건** · 분당 API **2억** · *"연간 2,500억 달러 이상"* · **나이지리아 최대 머천트 어콰이어러**.
  ⚠️ **비상장이라 매출·수수료율은 없다.** ⚠️ **사용자 수가 자료마다 다르다**(블로그 1,000만 vs 홈페이지 2,000만 계정).
  📌 **다음 사이클에 읽을 것** — 가용성 도메인은 `what-running-kafka-on-vms-taught-us-about-systems-thinking` 과
  `scheduled-jobs-in-java-spring-kubernetes-part-1/2`, 대사 도메인은 **`system-migration-data-accuracy`(99.9%)**.

## 대기

| 회사 | 국가·분류 | 1차 자료 | 접근 | 왜 이 회사인가 |
|---|---|---|---|---|
| **Target** | US · 커머스(리테일 물류) | `tech.target.com` | ✅ **본문까지 확인**(`blog/balancing-speed-and-accuracy-digital-fulfillment`, 2026-04-09) | **물류 전문이 이 사이트에 0곳이다.** 쿠팡(자체 물류센터)·Zalando·Allegro 와 달리 **매장 자체를 배송 거점으로 쓰는** 리테일이라 자리가 다르다. 읽은 글이 **주문을 어느 거점에서 몇 개의 배송으로 쪼갤지**를 다루는데 **버린 대안 둘에 이유가 각각** 있다: **브루트포스**는 *"주문의 품목 수가 늘면 복잡도가 지수로 자라"* *"계산상 비현실적"* 이 되고(**품목 5개면 배송 조합 31가지, 10개면 1,023가지 — 2ⁿ−1**), **그리디**는 *"동적 비용과 미묘한 요소를 놓쳐 비용이 더 든다"*. 회사의 한 줄이 축을 요약한다 — *"하나는 시간에서 실패하고 다른 하나는 해의 질에서 실패한다"*. 고른 것은 **그리디로 시작해 지역 탐색으로 다듬는 혼합**(Set Reassignment · Set Merging)이고 결과가 **브루트포스 대비 계산 시간 약 8% 로 비용 격차의 약 91% 를 메운다**. 공급망 글이 더 있다(`The Math Behind Target's Inventory and Delivery Optimization` 2025-11-18 등). **상장사(NYSE: TGT)라 IR 도 있다** |
