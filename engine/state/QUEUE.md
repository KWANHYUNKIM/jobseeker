...
|---|---|---|---|---|
| **monday.com** | 기타(이스라엘) · SaaS(협업 도구) | `https://engineering.monday.com/` — **자체 호스팅**. **2026 년 글이 24편**(최신 2026-08-28) | ✅ 목록·본문 모두 열림 | **283 에서 `mondaydb-3-solving-htap-for-a-trillion-table-system`(2026-05-19)을 열어 기준 셋을 다 확인했다.** **버린 대안** — DuckDB 안에 쓰기 버퍼링과 보드당 여러 파일+union 질의를 검토하다 *"Instead, we stepped back and asked: what if we don't write to DuckDB from the write path at all?"* 로 방향을 틀었고, 범용 DB 도 거절했다(*"We did not build a general-purpose database"*). **대가** — **DuckDB 는 파일당 writer 가 하나**라 *"For a system handling thousands of concurrent writes per second… this sounds like a dealbreaker"* · **보드를 가로지르는 읽기는 결과적 일관성**으로 *"typically converging within 500 milliseconds"* · 따뜻한 캐시를 잃으면 *"higher latency costs until it rebuilds"*. **수치** — 보드 로드 **5배** · 큰 보드 **20배** · 집계 **거의 50배** · **인프라 비용 40~60% 감소** · **100만+ 조직의 보드 읽기 트래픽 100%** · 캐시 히트율 **95% 이상** · WAL 동기 중앙값 **3ms**. **⭐ 구조가 배치·속도·서빙 세 층**이고 오브젝트 스토리지의 DuckDB 파일 + WAL 이 진실이라 **어느 노드든 어느 보드든 서빙할 수 있다.** 다른 후보 편 — `How We Scaled Board Item ID Generation`(04-09) · `Redesign Authorization to monday.com Scale`(04-26) · `Fueling the AI SRE: Standardized Observability for 600+ Services`(08-10) |
