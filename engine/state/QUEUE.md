...
| 회사 | 국가·분류 | 근거 |
|---|---|---|
| **Turso** | US · SaaS | ⭐ **새 각도 — `오픈소스를 내는 인프라 회사`가 통했다.** `blog.turso.tech` 최신 **2026-09-03**. ⭐ **`우리는 Postgres 를 러스트로 짓고 있다 — 데이터베이스의 LLVM 을 써서`**(2026-07-16)가 이 엔진에 없는 각도다. **형식 검증으로 SQLite 버그를 찾는 글**과 **동시 쓰기·멀티테넌시 구조** 논의가 있다. 최근 3편은 SQLite 암호화 연작(키 관리·마이그레이션·BYOK, 09-01~09-03). ⚠️ **최근 연작이 튜토리얼 성격이라 심층 글은 앞쪽에 있다.** |
| **Materialize** | US · SaaS | ⭐⭐ **`첫 화면으로 판정하지 않는다`가 살린 후보다.** 목록 8편 중 7편이 제품·개념 글이라 접을 뻔했는데, **개별 글 `Dictionary Compression in Materialize`(2026-08-03)가 완벽한 심층 글이었다.** **쓰이지 않는 태그 바이트 약 128개로 자주 나오는 컬럼 값을 표현**하고, **Misra-Gries 스트리밍 알고리즘으로 적재 중에 인기 값을 찾아** LSM 트리 수명주기에 걸쳐 통계를 유지한다. **약 1TB 지식 그래프 워크로드에서 피크 메모리 981.0 GiB → 428.4 GiB(56% 감소)**, 같은 인스턴스에서 자원 2배 절감. ⚠️ **대가를 적는다** — **컬럼형 엔진만큼의 압축률은 못 내고**, **압축 효율이 데이터 변화에 따라 나빠지면 `OOM 루프`에 빠질 위험**이 있다. ⭐ **차분 데이터플로라는 학술 배경이 있어 논문 각도도 있다.** |
