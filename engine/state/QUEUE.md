...
| 회사 | 국가·분류 | 근거 |
|---|---|---|
| **Neon** | US · SaaS(서버리스 Postgres) | ⭐ **거절한 자료구조를 이름을 대며 왜 안 맞는지까지 적는 드문 글이다.** `WAL + S3: Lakebase storage for the era of agents`(2026-08-24) 본문 확인 — **문제**(**`읽기는 키와 LSN 을 대고, 저장 시스템은 그 LSN 이거나 그 이전에 그 키를 덮는 가장 가까운 레이어를 찾아야 한다. 그건 기하 문제이고, 수천만 개 레이어에 걸쳐 어떻게 푸는지가 자명하지 않다`**) · ⭐ **거절**(**`선형 스캔은 너무 느리고, 뻔한 공간 구조들은 맞지 않는다 — R-트리는 「이 점 아래 첫 번째 레이어」 가 아니라 포함 질의에 답하고, 세그먼트 트리는 레이어 수가 아니라 좌표 공간의 크기에 따라 확장한다`**) · **고른 것**(각 LSN 의 과거 루트를 유지하는 **영속 이진 탐색 트리** — 현재 읽기와 과거 읽기가 같은 비용). ⭐ **WarpStream 과 같은 축**(객체 저장소 위의 저장 엔진). ⚠️ **약점 — 수치 트레이드오프가 없다**(밀리초·비용 비교 없이 구조 원리만). ⏳ **Databricks 인수 흔적이 `Lakebase` 라는 이름에 있다 — ㉗축 재료.** ✅ `r.jina.ai` |
