...
|---|---|---|---|---|
| **1Password** | CA · 보안(비밀번호 관리) | `1password.com/blog` · `support.1password.com` | ✅ WebFetch 로 본문이 온다 | **보안 축 첫 자리다** — 이 엔진에 비밀번호·자격증명을 맡는 회사가 없었다. **⭐⭐ 대가가 이 엔진에서 본 것 중 가장 극단적이다** — 복호화 키가 서버에 절대 가지 않아 회사가 **정책이 아니라 기술적으로**(`technically incapable`) 금고를 못 읽는다. **그래서 못 하는 기능을 그대로 적는다**: **계정 비밀번호 복구 불가**(비밀번호나 Secret Key 를 잃으면 접근이 끝난다) · **서버 측 금고 검색 불가** · **실제 자격증명으로 유출 확인 불가**(대신 기기에서 계산하고 부분 해시만 보낸다). ⚠️ **그 제약을 우회하려고 기밀 컴퓨팅 엔클레이브**(`sealed processing room`)를 만들었다 — **구조로 막아 놓고 그 구조를 다시 구조로 뚫는 자리**라 팔 값이 크다. **⭐ Fastly 프록시와 정확히 대비된다** — 거기서는 `구조가 막는 것`과 `약속이 막는 것`이 한 문단에 섞여 있었는데, 여기는 **거의 전부를 구조로 막고 그 값을 기능 삭제로 치른다.** **수치** — 128비트 키 · **34자 Secret Key** · AES-GCM-256 |
