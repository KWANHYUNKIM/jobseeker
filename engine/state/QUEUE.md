...
| 회사 | 국가·분류 | 상태 |
|---|---|---|
| **컬리** | KR · 커머스(신선식품) | **기능 4개 · 도메인 4개 — 열어 둔 도메인이 다 찼다.** 260 `misdelivery-detection` · 261 `inbound-sync` · 262 `access-block` · 263 `encryption-module`(결정 6, **직접 만든 이유가 명시적** — 후보 라이브러리에 취약점이 보고돼 있었다). **남은 재료 하나** — 재고 정산 Kafka Streams 윈도(2025-12, **읽어 뒀다**). **다음 사이클에 회사를 닫는다** |
