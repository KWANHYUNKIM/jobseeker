...
| 회사 | 국가·분류 | 상태 |
|---|---|---|
| **Wix** | 기타(이스라엘) · SaaS(웹사이트 빌더) | **296 도메인 ③ 두 번째 기능 — 도메인 3개 · 기능 5개.** ① `context-before-fix` · ② `read-first-then-write` + `chatroom-by-chatroom`(**정독 완료**) · ③ `agent-ready-org` + `compile-then-publish`(코드 샘플 2,000개를 AI 로 고치고 **컴파일러를 문지기로** 세운 기록. **결정 7개**). **⭐ '모델을 손대지 않는다' 가 네 번째로 확인됐다.** **⚠️ `From Weeks to Hours` 는 주소 못 찾음(재시도 금지).** 실적 — Q2 2026 매출 **5억 6,300만 달러 · 15% 성장** |
