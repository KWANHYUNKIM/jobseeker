...
| 회사 | 국가·분류 | 상태 |
|---|---|---|
| **Freshworks** | IN · SaaS | 326 에서 세 번째 도메인 `한 코드베이스가 두 판을 동시에 안다` 를 열고 기능 `know-both-versions` 까지 썼다 — **도메인 3개 · 기능 3개 · 결정 7개.** 안 읽은 `/saas/` 글 넷 남음(**Mozilla rr 메모리 손상 디버깅** · CloudFront 사설 콘텐츠 · 기능 토글 · 문자열 보간). ⚠️ **목록 페이지가 안 열려 글 전체 목록은 확인 못 했다** |
