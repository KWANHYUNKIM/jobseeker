...
|---|---|---|---|---|
| **Grafana Labs** | US/SE · SaaS(관측) | `grafana.com/blog` | ✅ WebFetch 로 본문이 온다. ⚠️ **태그 목록 주소는 404** — 심층 글은 검색으로 주소를 받는다 | ✅ **본문 확인함**(`Mimir 차세대 아키텍처`, 2026-01-02) — **거절한 옛 구조와 그 이유가 명시적이다.** 옛 구조는 **카산드라에서 온 복제 계수 3 + 쿼럼**이었고 **`복제 계수 3은 비싸다 — 수집 경로에서 각 데이터의 사본 셋을 들고 있어야 한다`**. 게다가 **인제스터가 읽기와 쓰기 경로를 묶어 놓아 `인제스터가 과부하되면 쓰기 경로도 영향을 받는다`**. 새 구조는 **쓰기와 읽기 사이에 Kafka 를 놓아**(gapless consumption) **복제를 3 → 2, 쿼럼을 2 → 1** 로 낮췄고, **WarpStream 으로 교차 AZ 전송 비용(GB당 0.02달러)을 객체 스토리지로 옮겼다.** **Grafana Cloud 의 Mimir TCO 25% 감소** · 활성 시계열 **수십억(클라우드) ~ 수백억(오픈소스)**. ⏳ Loki 도 **Kafka 기반 수집 계층으로 재설계**했다(Grafana 13, 2026). ⭐ **`저장 엔진` 축이 두터워진다**(ClickHouse · ScyllaDB 와 함께) |
