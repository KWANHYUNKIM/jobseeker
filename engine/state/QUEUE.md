...
| 회사 | 국가·분류 | 상태 |
|---|---|---|
| **Monzo** | EU(UK) · 핀테크(네오뱅크) | 프로파일 완료 · 도메인 3(서비스 3,000개를 한 저장소에서 같은 모양으로 굴린다[tech 4] / 은행의 상태를 한 클러스터에 몰아 두고 그 대가를 치른다[tech 3] / 팀 100개가 만드는 데이터에 계약을 건다[tech 4]) · **기능 1**(one-team-drives-all-migrations) |
