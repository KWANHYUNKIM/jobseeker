...
| **Vinted** | LT/EU · 커머스 | `vinted.engineering` | ✅ **본문까지 확인.** 목록과 개별 글이 모두 WebFetch 로 열린다 | **리투아니아 — 발트 첫 회사이고, 유럽 최대 중고 패션 C2C 마켓플레이스.** 당근과 같은 C2C 이지만 **국경을 넘는다**(24개 언어 · 50개 이상 국가-언어 조합). 본문을 읽은 `2026/04/22 personalized-search-autocomplete` 는 **수치와 버린 대안을 함께 적는다** — 후보 **1억 2,500만 개**, **초당 4,700 질의 · P99 31ms**, Vespa 요청의 **62% 정확 접두사 / 21% 퍼지-1 / 17% 퍼지-2**, LTR 로 **CTR +8%**, A/B **35회 이상**. 버린 것도 명시한다 — **대소문자 혼용은 CTR 이 조금 나았는데도 버렸고**, **범주 한정 제안은 CTR +2.4% 인데 구매 확률이 약 1.3% 낮아져 접었다**(⑬ '누구의 손해로 만드는 이득' 재료). 그 밖에 **Elasticsearch → Vespa 이관**(2024), **Vitess 샤딩 4부작**, **Clos/eBGP 데이터센터 망 재설계**, **동일 서버인데 성능이 다른 원인이 clocksource 였던 조사기**가 있다 |

| **Zerodha** | IN/기타 · 핀테크 | `zerodha.tech/blog` | ✅ **본문까지 확인**(`working-with-postgresql`) | **인도 첫 회사이자 증권·거래소 첫 회사.** 인도 최대 리테일 증권사이고 **자체 호스팅을 고집한다** — *"우리는 모든 DB 인스턴스를 맨 EC2 위에서 직접 호스팅하고 튜닝하고 관리한다."* 읽은 글에 수치와 버린 대안이 둘 다 있다: 샤드 4대에 **수천억 행 · 약 20TB**, 하루 **수천만 행** 삽입, 피크에 **초당 4~5만 질의**, 시작은 **32코어·64GB EC2 두 대**. **버린 것 셋** — 일 단위 파티셔닝(범위 질의가 크게 느려짐), Redis 캐시(리포트 수십 종의 필터·정렬에 너무 복잡), **Citus 샤딩 확장(스키마를 고쳐야 해서 거부)**. 자동증가 ID 와 외래키를 **일부러 버린 것**도 적는다. 다른 글: `1-5-million-pdfs-in-25-minutes`(**PDF 150만 장을 25분에**), `logging-at-zerodha`(**ELK → ClickHouse+Vector**), `from-native-to-react-native-to-flutter`(두 번 갈아탄 기록 — **⑫ '해 보고 되돌린 것' 재료**). ⚠️ **블로그가 2024년 3월에 멈춰 있다**(글 12편) — 최신 수치는 없다고 보고 들어갈 것 |
