...
| 회사 | 국가·분류 | 상태 |
|---|---|---|
| **1Password** | CA · 보안(비밀번호 관리) | 프로파일 완료 · 도메인 3(맡은 것을 자기가 못 읽게 스스로 막는다[tech 5] / 못 읽는데 처리해야 할 때 봉인된 방을 만든다[tech 5] / **기업의 접근 권한을 제로 지식을 깨지 않고 자동으로 준다[tech 0 — 자료 미확보]**) · **기능 1**(two-secrets-and-no-way-back) |
