...
| 회사 | 국가·분류 | 상태 |
|---|---|---|
| **Databricks** | US · SaaS | ⭐ **기능 1/2**(2026-09-06). ✅ `로그만 넘기고 나머지는 뒤에서 만든다`(쓰기에서 전체 페이지를 빼고 읽기를 유계로 지킨다 — 결정 7개, 그림 2장). ⏳ **마지막 하나** — `카디널리티에서 시계열 DB 를 지킨다`(Pantheon). ⭐ **`docs.databricks.com` 은 열리지만 주소를 추측하면 404 다 — 검색으로 얻는다.** ⚠️⚠️ **Neon 과 구조가 같다(인수). 비교 문서에서 두 번 세지 않는다.** |
