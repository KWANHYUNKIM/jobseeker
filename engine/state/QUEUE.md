...
| 회사 | 국가·분류 | 상태 |
|---|---|---|
| **SmartHR** | JP · SaaS | 347 에서 `measure-before-the-season` 을 썼다 — **도메인 2개 · 기능 3개.** ✅ **두 도메인 다 두 편 규칙 충족.** **⭐⭐ 같은 회사가 두 번 다 데이터베이스에서 막혔는데 한 번은 늦었고 한 번은 이르다** — 커넥션 풀러는 장애 뒤, 부하 시험은 정점 전. 남은 것 — 횡단 프런트엔드 · 불구합 집약 · Sandbox 테스트 · Policy-as-Code 등 과거 글 셋 |
