...
|---|---|---|---|---|
| **Snyk** | UK/IL · SaaS(개발자 보안) | `snyk.io/blog` | ✅ WebFetch 로 본문이 온다 | ⏸️ **보류를 풀었다**(2026-09-06). 첫 화면이 제품 발표라 접었는데 **`New Agentic Architecture for Snyk Agent Fix` 는 결이 다르다.** ✅ **본문 확인함** — **버린 방식이 명시적이다**(정적 파인튜닝 → 동적 few-shot: 언어·규칙마다 **수천 개의 수동 학습 샘플과 긴 파인튜닝**이 필요했다). **대가도 적는다** — 예전에는 **안전하지 않은 코드를 그냥 걸러내 개발자에게 아무 수정도 안 남겼는데** 이제 **실패한 수정을 버리는 대신 다시 생각하게 한다**(agentic retries). **수치** — 자체 벤치마크 약 150건에서 **Opus 4.6 단독 74.6% → Snyk 인텔리전스 결합 85.4%(+14.48%)**, 전문가가 쓴 수정 **3만 5천 건 이상**. ⭐ **`AI 를 어디까지 믿나` 축의 일곱 번째이고, 앞의 여섯 곳과 달리 AI 가 고치는 대상이 보안 취약점이다** |
