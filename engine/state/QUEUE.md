...
| **LinkedIn** | US · 소셜(구인·구직) | `linkedin.com/blog/engineering` | ✅ **본문까지 확인**(`infrastructure/the-training-infrastructure-behind-ai-powered-job-search-eight-x-faster-multi-teacher-distillation`, 2026-08-06) | **이 사이트에 구인·구직 그래프가 없다** — 소셜은 인스타그램·Discord·Slack·당근인데 전부 다른 결이다. 블로그는 자체 호스팅이고 살아 있다(**최신 글 2026-08-13**). 읽은 글은 **채용 검색 랭킹 모델의 학습 인프라** — *"학습 인프라의 속도와 효율이 우리가 얼마나 빨리 랭킹 모델을 개선할 수 있는지를 결정하는 1차 제약이다."* **대가를 한 문장으로 적는다** — *"증류는 교사 모델의 품질을 지키면서 컴팩트한 0.6B 학생을 GPU당 초당 약 22,000 요청으로 서빙하게 해 준다. 하지만 그 교환은 비용을 서빙에서 학습으로 밀어낸다."* 그리고 *"오프라인 증류는 선행 비용을 더한다 — 첫 단계가 캐시를 만들어야 한다"*, 단일 노드 방식은 *"천장이 낮았다: 교사와 학생이 같은 GPU 를 두고 경쟁했다"*, 수십억 파라미터 교사를 그대로 서빙하는 것은 *"시작조차 불가능"*. 수치 — **2일 → 5시간 미만**(전체 **8배**), 온라인 경로 **약 45시간 → 약 10시간**, 교사 파인튜닝 **약 25시간 → 12시간 미만**, 캐시 생성 **2시간 미만**, **2,000만 레코드**, 배치 **75% 증가**, 하드웨어 FLOPs 활용률 **2배**. 스택은 **PyTorch FSDP/FSDP2 · HSDP · Liger 커널 · Ray · H200 · HDFS**. **자리**: 이 레포 자체가 채용 사이트라 **'구인 검색을 어떻게 랭킹하는가'** 가 특히 값이 있다 |
### 확인해 둔 후보 (아직 검증 안 됨)
