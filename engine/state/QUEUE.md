...
|---|---|---|---|---|
| **Paystack** | NG · 핀테크(아프리카 결제) | ⚠️ **`paystack.com/blog/category/engineering-design`** (옛 `blog.paystack.com` 이 아니다) | ✅ 브라우저로 목록·본문이 온다 | ⚠️ **아프리카 축 첫 자리다** — 87개 회사 중 하나도 없었고, 앞서 `아프리카 — 두 번째로 빈손` 이라 적었던 자리다. **⭐⭐⭐ Plaid 와 같은 마감에 다른 답을 냈다** — **아마존이 2024년 10월 Aurora MySQL v2(5.7 기반) 지원 중단을 발표**했고, **Plaid 는 플랫폼을 갈아탔는데**(TiDB) **Paystack 은 업그레이드를 해냈다**(MySQL 8.0). **같은 외부 마감, 갈린 답 — 비교 문서의 완벽한 재료다.** **⭐⭐ 핵심 글이 제목부터 대가를 약속한다** — `How we pulled off a **zero incident** migration across 11 production databases`(2026-02-28). **아프리카 전역의 가맹점 결제를 처리하는 프로덕션 Aurora 클러스터 11개**를 올렸고, 구성이 **① 이전 성공의 재정의 ② 이중 계층 호환성 테스트 ③ 블루-그린 실행 ④ 명확한 롤백 경로** 다. 3자 케이스 스터디에는 **54만 달러 절감·100% 가동률**이 나온다. ⚠️ **글이 드문드문하다** — 2023-09 · 2024-09 · 2024-11 · 2025-05 · 2025-07 · 2026-02 이고 **여러 편이 커뮤니티·통합 라이브러리·디자인** 글이다. 시스템 주제로 묶이는 것은 **DB 이전**과 **SAST 취약점 관리**(2025-05) 둘이다. **한 편이 아주 좋고 나머지가 얇은 회사이므로 도메인을 크게 잡지 않는다** |
