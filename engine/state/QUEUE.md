...
|---|---|---|---|---|
| **Zepto** | IN · 커머스(퀵커머스) | `blog.zepto.com` | ⚠️ **`r.jina.ai` 로 뚫린다**(2026-09-06 되짚기 성공). Medium 계열이라 브라우저도 병행 | **최신 글이 이틀 전**이고 목록이 전부 자기 시스템 글이다. ✅ **`Building Search for a 10-Minute World`(2026-06) 확인** — **MoE 랭킹 → 피처 스토어 재랭킹 → 재고·가격의 권위 해석 단계**로 이어지고 **색인이 초~분 단위로 뒤처진다는 것을 인정하고 설계에 반영한다.** **검색 플랫폼만 분당 100만 요청 이상.** ⭐ **`10분 배송` 이라는 제약이 이 사이트에 없는 축이다** — 재고가 실시간으로 바뀌는 커머스. ⏳ 후보 글: `Route IQ`(잡음 섞인 GPS 로 배달 경로 복원, 2026-08-24) · `하루 10억 이벤트 광고 분석`(2026-07-27) · `성능 엔지니어링 2부`(2025-12-15). 2차 자료로 **AWS 의 OpenSearch OR2 사례 글**도 있다 |
