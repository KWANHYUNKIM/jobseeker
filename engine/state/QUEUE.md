...
| 회사 | 국가·분류 | 근거 |
|---|---|---|
| **WarpStream** | US · 인프라(Kafka 호환 스트리밍) | ⭐ **Grafana Labs 글이 교차 AZ 비용 비교로 든 회사를 따라갔다.** `One Weird Trick to Make Rapid Storage 40x Faster`(2026-07-20) 본문 확인 — **거절한 대안**(**`장수명 파일에 append 하는 모드로 WarpStream 을 재설계할 수 있었지만 크고 침습적인 변경이었을 것이다`**) · **대가**(**`Rapid Bucket 의 실효 저장 비용이 일반 지역 GCS 버킷보다 10배 이상 높다`** → **지연을 줄이려 Rapid Bucket 정족수에 먼저 안착시키고 몇 분 뒤 지역 GCS 로 비동기 압축**) · ⭐ **기대와 실제가 다른 것을 그대로 적는다**(40배를 기대했는데 **P99 3배 감소** — **`쓰는 데이터 양이 늘면 Rapid 와 일반 GCS 의 격차가 줄고, 이 측정에는 압축 같은 버킷 외 작업도 들어간다`**) · **한계**(**Lightning Topics 와 비호환**). 최신 글 **2026-08-18**(`Orbit` 자동 Kafka 이전). ✅ `r.jina.ai` 로 목록·본문 모두 온다 |
