...
|---|---|---|---|---|
| **Cybozu (サイボウズ)** | JP · SaaS | `blog.cybozu.io` — **1차 자료이고 살아 있다**(최신 2026-08-27) | **WebFetch 로 목록과 본문 모두 열린다.** 일본어 | **305 에서 본문 한 편을 실제로 열어 확인했다**(`20년 넘은 SQLite 기반 앱을 쿠버네티스로`, 2026-08-27). **기준 셋을 다 넘는다** — **버린 대안에 이유가 명시된다**(*"案 1: StatefulSet を利用する…更新時は古い Pod を削除してから新しい Pod を作成するため、リリースのたびにダウンタイムが発生する"* 라 Deployment + Pod Affinity + ReadWriteOnce 로 갔다) · **한계를 여럿 적는다**(같은 볼륨의 배타 제어가 되려면 **파드가 같은 노드에 있어야 한다** · 대상 파일이 **수억 개**라 *"月 1 回の定期メンテナンスの時間内にすべてを転送することはできません"* · 커널 동작을 직접 조사해 검증해야 했다) · **수치가 있다**(약 **300만 파일을 15분**에 전송 · 페이지 캐시 검증에 5GB 랜덤 데이터와 977.2M 캐시 · 파드 메모리 상한 1Gi). **⭐ 20년 된 C++ CGI 애플리케이션을 옮긴 이야기라 이 사이트에 없는 종류다.** **그리고 일본은 지금 셋뿐이다.** |
