...
| 회사 | 국가·분류 | 상태 |
|---|---|---|
| **Meituan (美团)** | CN · 커머스 | **300 도메인 ① 첫 기능 완료 — 도메인 1개 · 기능 1개.** ① `검색이 무엇을 위로 올릴지 정한다`(`semantic-before-text` — **결정 8개**, 버린 것마다 이유, 못 푼 것 넷을 논문 이름까지 대며 적는다). **🔴 이 블로그는 옛 글을 내린다 — 2022~2025-05 글이 전부 404 다.** 그래서 이 기능은 **글 한 편에 기댄다**(그 사실을 밝혀 뒀다). **살아 있는 것은 2025-10 이후로 보인다.** 실적 — Q2 2026 매출 **1,050억 위안(+14.4%)** · 조정 순이익 **25억 위안** |
