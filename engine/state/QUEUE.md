...
|---|---|---|---|---|
| **Cygames** | JP · 게임 | `tech.cygames.co.jp` | ⚠️ WebFetch 는 헤더만 온다 → **`r.jina.ai` 로 목록·본문이 온다**(2026-09-06 되짚기 성공) | ⭐ **게임 축의 첫 자리다** — 이 사이트에 게임 회사가 없다. ✅ **본문 확인함**(`출시 시점의 TiDB`, 2025-10-03) — **`Shadowverse: Worlds Beyond` 출시일 초당 50만 질의, 이후 피크 65만 QPS 를 무중단으로** 넘겼고 **40만 QPS 중에 TiKV 노드를 롤링 업데이트**했다. **결정과 대가가 있다** — **통계 정보 이력 기능을 껐다**(메모리를 제약해서, 안정성 우선). ⚠️ **다만 이 글은 컨퍼런스 발표 후속 요약이라 깊이가 얕다** — **`스마트폰 게임 아키텍처`(archives/3706)** 가 본체일 수 있으니 그것부터 연다. ⭐ **Plaid 와 같은 TiDB 인데 쓰는 이유가 다르다** — 하나는 **관리형 MySQL 을 떠나며**, 하나는 **게임의 샤딩 복잡도를 피하려고** |
