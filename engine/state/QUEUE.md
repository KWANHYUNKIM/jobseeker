...
| 회사 | 국가·분류 | 상태 |
|---|---|---|
| **컬리** | KR · 커머스(신선식품) | **기능 2개 · 도메인 2개 — 열어 둔 도메인이 다 찼다.** 260 `misdelivery-detection`(결정 8) · 261 `inbound-sync`(결정 7, **재시도 창 24시간의 근거가 업무 데드라인**). **세 번째 도메인 후보 셋** — 재고 정산 Kafka Streams 윈도(2025-12, **읽었다**, 버린 대안 하나 + 대가 넷) · 딜리버리 암호화(2025-06, **읽었다**, 버린 대안 둘에 이유 명시) · `access-block` 2부작 무중단 점검(2025-10, 미개봉). **다음 사이클에 세 번째 도메인을 열지 회사를 닫을지 정한다.** **엔지니어링만 7페이지(70편+)** |
