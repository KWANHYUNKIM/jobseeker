...
| 회사 | 국가·분류 | 상태 |
|---|---|---|
| **SmartHR** | JP · SaaS | 345 에서 첫 기능 `when-a-rule-is-not-enough` 를 썼다 — **도메인 1개 · 기능 1개.** ✅ **두 편 규칙 채움.** **⭐⭐ 같은 회사가 Packwerk 를 두 방향으로 썼고 결과가 갈렸다** — 새로 만드는 쪽은 됐고 이미 얽힌 쪽은 안 됐다. 남은 것 — 커넥션 풀러(2년 전 장애) · 연말정산 부하 시험 · 횡단 프런트엔드 · 불구합 집약 · Sandbox 테스트 |
