...
| 회사 | 국가·분류 | 상태 |
|---|---|---|
| **PayFit** | EU · SaaS | 355 에서 첫 기능 `name-it-before-you-cut` 를 썼다 — **도메인 1개 · 기능 1개.** 결정 7개. **⭐⭐ 급여 명세가 전에는 '내부 구현 상세' 였다** — 이름이 볼 수 있는 것을 정한다. **⚠️ 두 편 규칙 미충족**(DDD 편 하나만 썼다). 다음 첫 순위는 **JetLang 본문**(이 회사의 기반인데 한 줄 설명만 확인). **⚠️ 엔지니어링 태그 여덟 편, 최신 2024-07-08** |
