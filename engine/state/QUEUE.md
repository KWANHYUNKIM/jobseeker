...
| 회사 | 국가·분류 | 상태 |
|---|---|---|
| **Paytm** | IN · 핀테크 | 320 에서 첫 기능 `leave-what-costs-more`(다 옮기지 않기로 정한다)를 썼다 — **도메인 3개 · 기능 1개 · 결정 6개.** 다음은 도메인 ②(거래 기록을 어디까지 맞출 것인가)인데 **수치가 0 이라 한 편 더 읽어야 한다**, 또는 도메인 ③ 의 재료인 Redis 클러스터 글(2023-01-05). ⚠️ **블로그가 2023-04 에서 멈췄고 안 읽은 글이 넷 남았다** |
