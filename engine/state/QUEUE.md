...
|---|---|---|---|---|
| **Plaid** | US · 핀테크(금융 데이터 연결) | ⚠️ **`plaid.com/blog`** (옛 `engineering.plaid.com` 은 Medium 이고 비어 있다) | ✅ 자체 도메인. 검색으로 글 주소가 나온다 | ⚠️ **접근 지도의 판정이 낡아 있었다** — `engineering.plaid.com` 이 Medium 이라 접었는데 **회사가 자체 도메인으로 옮겼다.** **⭐⭐ 수치가 이 엔진에서 본 이전 이야기 중 가장 촘촘하다** — Aurora 에서 **큰 테이블·높은 행 경합에 약 12k writes/sec 병목**을 만났고, **6명 팀이 데이터베이스 234개**를 **약 100개 서비스**에서 **약 2년 반**에 걸쳐 자체 호스팅 TiDB 로 옮겼다. **⭐⭐ 전후 비교가 아주 구체적이다** — 2023년 초 Aurora 5.6→5.7 업그레이드에 **계획 다운타임 104분 + 엔지니어링 26주**가 들었는데, 2025년 **모든 TiDB 클러스터 업그레이드는 약 1 엔지니어링 주에 다운타임 0** 이다(**다운타임 100% 제거 · 노력 96% 감소**). 서비스별 컷오버도 **3~4주 → 약 1주**, 쓰기 다운타임 **약 5분 → 60초 미만**. **⭐ 핵심 글** — `plaid.com/blog/switching-to-tidb/`(2026-03-31, `Cutting over: Our journey from AWS Aurora MySQL to TiDB`). ⚠️ **DB 이전 축은 Etsy(Vitess)와 겹치는데, 그것이 오히려 값이다** — **같은 문제(관리형 MySQL 의 쓰기 한계)에 한쪽은 샤딩 추상화로, 다른 쪽은 분산 SQL 로 답했다.** **비교 문서의 재료다** |
