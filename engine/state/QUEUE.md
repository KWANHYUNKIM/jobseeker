...
|---|---|---|---|---|
| **Pinterest** | US · 소셜(발견·추천) | `medium.com/pinterest-engineering` | ✅ **`r.jina.ai` 로 목록·본문이 온다** | **최신 글이 며칠 전**이고 자기 시스템 글이다. ✅ **본문 확인함**(`홈피드용 조건부 학습 검색의 확장`, 2026-08-26) — **거절한 대안과 대가가 수치와 함께 있다.** ① **핀 ID 임베딩**: 이미지 시그니처로 시작했다가 **`충분한 암기와 견딜 만한 충돌을 위해 20GB 크기의 id 임베딩 테이블을 사전학습하게 됐다`** → **양자화 VAE 로 만든 시맨틱 ID** 와 **계층마다 하나씩 다섯 개의 작은 임베딩 테이블 + MLP 융합**으로 바꿨다(**메모리와 표현력의 맞바꿈**). ② **조건별 모델을 통합**: 따로 두면 **`학습 파이프라인·피처 업그레이드·실험·서빙 설정·인덱싱에 걸쳐 일이 중복된다`** — **조건별 최적화를 일부 내주고 운영 단순성을 샀다.** ③ **서빙을 GPU 로**: **`7자리 비용 절감과 p90 모델 지연 85% 감소(80ms → 12ms)`**, 병목이 **네트워크에서 CPU 로** 옮겨 갔다. ④ 학습 효율 **2배**(요청 수준 중복 제거 · M-Falcon 으로 조건 토큰 평탄화). ⭐ **추천·피드 축에 Instagram·YouTube·Snap 이 있지만 `조건부 검색` 이라는 각도는 없다** |
