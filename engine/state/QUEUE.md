...
| 회사 | 국가·분류 | 상태 |
|---|---|---|
| **Wix** | 기타(이스라엘) · SaaS(웹사이트 빌더) | **295 새 도메인 ③ 을 열고 기능까지 — 도메인 3개 · 기능 4개.** ① `에이전트에게 당번을 맡기되 열쇠는 안 준다`(`context-before-fix`) · ② `멈추지 않고 갈아 끼운다`(`read-first-then-write` + `chatroom-by-chatroom`, **정독 완료**) · ③ `에이전트가 들어올 수 있게 집을 고친다`(`agent-ready-org` — 저장소 표준·하네스 중앙화·검토 우선. **결정 7개**). **⚠️ `From Weeks to Hours` 는 주소를 못 찾았다**(두 번 시도, ⑱ 이 안 통한 첫 사례). 실적 — Q2 2026 매출 **5억 6,300만 달러 · 15% 성장** |
