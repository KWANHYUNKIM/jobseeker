...
|---|---|---|---|---|
| **Booking.com** | EU(NL) · 여행 | 목록 `blog.booking.com`(최신 2026-07-29) · 본문 `medium.com/booking-com-development` | ⚠️ **목록은 WebFetch 로, 본문은 브라우저 `get_page_text` 로.** 실제로 열어 확인했다 | **`Breaking the Loop`(2026-02-19) 전문을 읽었다 — 이 엔진이 찾는 결의 교과서다.** **축이 순환 의존성이다**: 백업 오케스트레이터(Bacula)의 카탈로그가 **자기가 지켜야 할 온프렘 DB 안에 살고 있었다** — 그 DB 를 잃으면 그것을 복구할 방법도 함께 잃는다. 그걸 끊으려고 카탈로그만 Amazon RDS 로 뺐다. **버린 대안이 뚜렷하다** — 복제 기반 이전을 *"deliberately rejected"*(구·신 Bacula 스키마 충돌과 온프렘→RDS 복제 구성의 마찰), 다중 리전이 기본인 Aurora 도 *"deliberately chose"* RDS 를 골라 뺐다(Terraform 모듈 도그푸딩과 비용). **실패를 그대로 적는다** — KMS 암호화 키는 **생성 시점에 다중 리전으로 정해야 하고 나중에 못 바꾼다**는 걸 몰라 스냅샷 → 복호화 → 재암호화로 클러스터를 다시 만들었다(*"a quite painful lesson"*). **수치** — MySQL 클러스터 **250개 이상** · 하루 약 **1,000개 백업** · 연결 약 200 · 초당 쿼리 약 92 · `db.r7g.2xlarge` → **2개월 뒤 `xlarge` 로 축소**. **연도가 붙어 있다** — 계획 2024 Q2, 착수 Q3, 완료 2025. **여행 축이 trivago(메타서치) 하나뿐이라 자리도 맞는다.** 다른 회고도 있다: `A Migration Adventure`(2026-07-27, 저장소 수천 개를 SaaS 형상관리로) |
