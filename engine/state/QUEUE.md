...
| 회사 | 국가·분류 | 상태 |
|---|---|---|
| **Sansan** | JP · SaaS | 331 에서 **두 번째 도메인 `좁혀야 이기는 인식`(이 회사의 심장)을 열고 기능 `narrow-to-win` 까지 썼다** — **도메인 2개 · 기능 2개 · 결정 5개.** 연구개발부 연재 두 편으로 받쳤다(명함 데이터화 2022-10-27 · NineOCR 2022-11-17). **⭐ 만든 초해상을 안 쓴다고 적는 드문 글이다.** 남은 것 — 실물 원본 관리 축(대담 형식, 버린 대안 0) · 이관 글 여럿 · **연재 제2회를 못 찾았다** |
