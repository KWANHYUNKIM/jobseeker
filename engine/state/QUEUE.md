...
| 회사 | 국가·분류 | 상태 |
|---|---|---|
| **LinkedIn** | US · 소셜(구인·구직) | **기능 1개**(`ai-code-review`, 사이클 228). `자기 개발 과정에 AI 를 붙일 때 무엇을 통제하는가` 착수(글 2편 다 읽음). **`랭킹 모델을 얼마나 빨리 다시 만들 수 있는가` 는 아직 0개** — 본문이 증류 글 한 편뿐이라 **두 번째 글을 먼저 찾아야 한다**. QA 에이전트를 두 번째 기능으로 쓸 수도 있다 |
