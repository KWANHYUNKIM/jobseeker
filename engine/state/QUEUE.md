...
|---|---|---|---|---|
| **Airbnb** | US · 커머스(숙박 마켓플레이스) | `medium.com/airbnb-engineering` | ⚠️ **목록 페이지는 헤더만 온다** — **글 주소를 검색으로 받아 `r.jina.ai` 에 태운다** | **2026년 글이 이전·마이그레이션 이야기로 줄지어 있다.** ✅ **본문 확인함**(`OpenTelemetry 와 vmagent 로 고volume 메트릭 파이프라인 만들기`, 2026-04) — ⚠️ **거절한 대안을 다섯 개 나열한다**: **Veneur**(광범위한 재작성 필요) · **recording rules** · **OpenTelemetry Collector 자체 집계** · **Vector** · **m3aggregator** — 그리고 **스트리밍 집계 능력과 감당할 만한 코드베이스** 때문에 vmagent 를 골랐다. ⚠️ **대가를 명시한다** — **고카디널리티 서비스를 델타 시간성으로 바꾸자 장애 때 이상한 스파이크 대신 눈에 보이는 데이터 공백이 생기는데, 메모리 압박을 다루려고 의도적으로 감수한 절충**이다. **수치** — **초당 1억 샘플 이상** · JVM 서비스의 메트릭 CPU **10% → 1% 미만** · **`이전 벤더 기반 아키텍처 대비 비용이 대략 한 자릿수 규모로 줄었다`**. ⭐ **Razorpay 와 같은 vmagent 를 쓴다** — **사는 쪽 둘이 같은 도구로 다른 문제를 푼 자리다.** ⏳ 후보 글: `다제품 세계를 위한 데이터 아키텍처 진화`(2026-06, **이중 파이프라인 검증과 고통스럽도록 느린 폐기 주기**) · `통합 지식 그래프로 신원 그래프 확장`(2026-05, **같은 질의가 자체와 벤더에서 성능이 크게 달랐다**) |
