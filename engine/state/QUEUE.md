...
| 회사 | 국가·분류 | 상태 |
|---|---|---|
| **Perplexity** | US · 검색 | ⭐ **프로파일 완료(2026-09-06) — 도메인 3개.** `요청을 GPU 앞까지 나른다`(Ivy·Tulip, 러스트) · `모델을 GPU 위에서 돌린다`(ROSE, 파이썬 — CUDA 그래프·LazyTensor) · `GPU 끼리 직접 말하게 한다`(fabric-lib, RDMA 400Gbps, **MLSys 2026 논문**). ⏳ **기능 0/3 — 다음은 확장, RDMA 부터.** ⚠️ **블로그 목록이 403 이라 개별 글 주소를 3자 기사에서 캔다.** |
