...
| 회사 | 국가·분류 | 상태 |
|---|---|---|
| **컬리** | KR · 커머스(신선식품) | **기능 3개 · 도메인 3개 — 열어 둔 도메인이 다 찼다.** 260 `misdelivery-detection` · 261 `inbound-sync` · 262 `access-block`(결정 7, **출발점이 권한이다** — *"개발팀에겐 nginx 접근 권한이 없습니다"*). **남은 재료 둘은 읽어 뒀다** — 재고 정산 Kafka Streams 윈도(2025-12) · 딜리버리 암호화(2025-06, **버린 대안 둘에 이유 명시**). **다음 사이클에 네 번째 도메인을 열지 회사를 닫을지 정한다** |
