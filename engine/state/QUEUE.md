...
- **Spotify** (EU · 스트리밍) — 사이클 153 에서 열었다. **유럽 첫 회사.** 프로파일 + 도메인 3개
  (데이터 레이크 조회 · 데이터 플랫폼 · 콘텐츠 수집·트랜스코딩) + 기능 1개(`rap`) 완료.
  ✅ **본문 확인 완료** — `engineering.atspotify.com` 은 잘 읽히고, RAP 글은 대가를 항목마다
  적어 두었다(`NOT STATED` 로 답하는 부분도 있어 요약을 신뢰할 만하다).
  ⚠️ **추천·오디오 전송 글이 블로그에 없다** — 최근 글이 데이터 플랫폼·실험·개발자 도구·AI 에
  몰려 있다. 다음 기능은 **데이터 플랫폼**(2부가 나왔는지 확인) 또는 **콘텐츠 수집·트랜스코딩**
  (사고 보고서 하나로는 얇으니 두 번째 글을 찾을 것), 아니면 **실험 플랫폼**을 새 도메인으로
  여는 것(`when-can-llms-replace-humans-in-a-b-tests`, `better-experiments-with-llm-evals`).
