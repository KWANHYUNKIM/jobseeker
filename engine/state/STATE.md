...
**Chainguard — 기능 하나를 채웠다(`rebuild-until-it-converges`).** 빈 도메인 둘. 회사 154개, 큐 1/3.

### 한 번에 맞히기를 요구하면 사람이 붙어야 한다

이번 기능의 축이다. 상류가 흔들리고 시험이 깨지는 일이 늘 있는데 실패를 예외로 다루면 그때마다
사람이 개입해야 한다. 그래서 **여분을 두고 다시 시도해 수렴시킨다** — 대신 그 재시도가 몰림을
만들어 다음 도메인을 부른다.

### AI 를 넣을 자리를 명시적으로 갈랐다

**규칙으로 짤 수 있는 것은 규칙으로 짜고**, AI 는 부품 추론이나 CVE 백포팅처럼 구조가 없는
판단에만 둔다 — 나머지를 검증 가능한 구조적 도구로 둔 이유가 환각을 막는 것이라고 적는다.
⚠️ 다만 **그 경계의 구체적 기준은 글에 없다.**

### 컨테이너는 담아 두지 못한다

Athena 샌드박싱 글(2026-07-29)을 이번에 읽어 넣었다. 다루는 것이 **공개 전 취약점·실제로 도는
악용 코드·공개 전 패치**라 오픈소스 빌드와 위험이 다르다. 그래서 마이크로VM(QEMU/KVM · 게스트별
전용 커널 · gVisor 네트워크 스택 · 작업마다 조립하는 임시 파일 시스템 · 읽을 때 발급하는 짧은
수명 자격 증명)을 골랐다. 물린 이유가 한 줄이다 — **"컨테이너는 담아 두지 못한다."**

### 다음 사이클 — 확장 (남은 두 도메인 중 하나)

`한꺼번에 몰리는 일을 길들인다` 가 수치가 가장 두껍다 — 물린 대안 셋(지터 · 두 레인 워크큐 +
리컨실러별 리더 선출 · 초기 파킹이 만든 디스패처 병목)과 50만 키 → 분당 350키, 파킹 5,000키
15분 미만이 다 있다.
