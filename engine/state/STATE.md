...
**없다 — 예순여섯 번째 후보 조사를 돌았다. 큐 1/3.** 다음 사이클은 **신규**(Let's Encrypt).

### 없애야만 없앨 수 있는 위험

OCSP 종료 글이 통과한 근거다. 응답자를 돌리는 한 **방문자의 IP 와 방문 사이트를 즉시 알게
된다** — 보관하지 않아도 **법적으로 수집하도록 강제당할 수 있다.** 그래서 **자기가 오래
돌리던 서비스를 스스로 접었다.** 데이터를 안 모으는 것과 **모을 수 없게 만드는 것**은 다르다.

### 어느 글을 먼저 열었느냐로 판정이 갈릴 뻔했다

먼저 연 `from-90-to-45` 는 **업계 표준이 강제한 변경**이라 물린 대안도 정량 비용도 없었다 —
그것만 봤다면 접었을 것이다. `ending-ocsp` 를 열어서야 자기 결정과 대가가 나왔다.
**맵박스에 이어 두 번째다.**

### 물린 것을 이름 대어 적는다

**OCSP Must Staple 은 여러 해 동안 넓은 브라우저 지원을 얻지 못했고**, OCSP Stapling 을
구현한 웹서버는 **심각한 다운타임 위험**을 만든다. 표준을 만들어 두고도 **채택되지 않은
것**을 실패로 적는 대목이다.

### Home Assistant 는 접었다

홈 자동화 축(완전히 빈 축)을 노렸는데 릴리스 공지·제품 발표·파트너십이 지배한다. 유일하게
결정을 다룬 글이 **EU 규제 이야기**라 자기 시스템의 기술 결정이 아니다.
