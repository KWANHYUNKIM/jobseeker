...
**아무 회사도 파는 중이 아니다 — 30곳이 전부 done, 비교 문서는 21개다.** 사이클 173 은 문서를 썼다.
**다음 사이클은 31번째 회사 trivago 를 연다**(회사–문서 번갈아).

> **⚠️ trivago 를 열 때 — 본문을 아직 한 편도 읽지 않았다.** `tech.trivago.com` **접속만** 확인했다.
> **팔기 전에 반드시 한 편을 정독할 것** — `2026-06-12 how-we-cut-kafka-consumer-deployment-costs-by-83`
> (**Kafka 컨슈머 배포 비용 83% 절감**, 계층별 성능 조사, **P1 사고 19건**)이 유력하다. 그 밖에
> `2026-03-27 unifying-internal-apis-a-different-use-case-for-graphql-gateways`(**고객 대면이 아닌
> 내부 API 통합** — GraphQL 게이트웨이의 다른 용례라고 스스로 구분), `2026-02-18 from-always-on-to-on-demand-scaling-kafka-sinks-with-keda`(상시 → 온디맨드), `2022-05-16 warp-a-web-application-rewrite-project`.
> **축: 여행 메타서치** — **자기 재고 없이 남의 재고를 모아 비교해 판다**(쿠팡·아마존·무신사와 다름).
> 국가는 `EU`(독일). 상장사(NASDAQ: TRVG)라 IR 로 매출을 confirmed 로 확인해 볼 것.

> **미사용 기능 현황 — 전체 119개 중 쓰임 69, 미사용 50.** 아직 여유가 있다.

> **이번에 쓴 문서: `벽이 되는 것은 크기가 아니라 개수다`** (LY `hdfs-bridge` · Grab
> `iceberg-lake`·`counter-service` · Discord `voice-sfu` · Notion `logical-shard-reshard`).
> 축: 용량은 대개 '얼마나 큰가'로 이야기되는데 **먼저 무너진 건 개수**였고, 그 개수는
> **데이터가 아니라 관리하는 쪽**에서 값을 받았다 — 네임노드는 파일마다 메모리, 인덱스는
> 레코드마다 64바이트, S3 는 요청마다 요금, 메타스토어는 파티션마다 조회 시간.
> **가장 좋은 대목 둘**: ① **개수를 늘린 행동들이 각각 그 자리에서는 옳은 선택**이었다(병렬로
> 쓰고, 잘게 나누고, 창마다 세고, 사람을 더 부르고) ② **'더 크게 사서' 해결한 곳이 하나도 없다** —
> 넷은 세는 단위나 구조를 바꿨고 하나(Notion)는 **개수를 계약으로 만들었다**(논리 샤드 480개 불변).
> **Discord 와 Notion 이 오랜만에 들어갔다.**

> **문서 쓰는 절차가 네 번 연속 값을 했다** — ① 쓰인 features 집합 ② **미사용 목록 + 남은 개수**
> ③ **정규식으로 축 표지 훑기**(이번엔 `개수|파일 수|레코드 수|카디널리티|작은 파일|파티션당`,
> 그리고 2차로 `카디널리티|시리즈 수|샤드 수|목록 조회|항목 수|파티션 수`) ④ 파일 열어 confirmed
> 확인. **2차 정규식에서 Notion 이 나온 것이 이번 문서의 전환점**이었다 — 개수를 벽으로 만난
> 넷과 달리 **개수를 아예 못 바꾸게 고정한** 사례라 문서에 결이 하나 더 생겼다.

> **아직 안 쓴 비교 축** — ⑬ **'누구의 손해로 만드는 이득'**(Grab 합승: 기사 39.13km→23.72km,
> 승객 +4.5분·+6분, 회사가 *"형식화에 따라 회사·승객·기사에게 다른 영향"* 이라 적음 — 배민·우버·
> 쿠팡에 같은 결이 있는지 **확인 필요**) · ⑫ 해 보고 되돌린 것(Grab NVMe 인덱스는 **이번 문서에서
> 썼다** — 재료 재확인) · ⑨ 같이 몰린다 · ② 틀려도 되는 일 vs 안 되는 일 · ⑤ 시스템 두 벌.

**큐 잔량 2/3.** trivago → Allegro.

마지막 사이클: 173 (2026-08-22) — 비교 문서 `count-not-size`. 문서 21개.
