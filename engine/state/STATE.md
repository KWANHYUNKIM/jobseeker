...
**없다. 309 에서 Cybozu(サイボウズ, 53번째)를 `done` 으로 닫았다 — 도메인 2개 · 기능 2개.**

**닫은 근거** — 남은 묶음에서 한 편을 더 **열어 보고** 판단했다. `govulncheck 오탐 줄이기`
(2026-08-05)는 **한계가 상세한데**(심볼 정보를 지우면 모듈 전체가 영향받는 것으로 판정 ·
사람이 손으로 바이너리를 꺼내야 함 · 관련 이슈에 *"進展は全くありません"*) **수치가 0 이고
결정 여섯 중 대부분이 도구 버그 우회다**(PURL 을 jq·sed 로 손보기). **실제 설계 결정은 셋
남짓이라 기능으로 쓰지 않았다.** **이 회사에서 여섯 편을 읽고 둘만 기능이 됐다.**

**`business_model` 에 관찰 넷을 적었다**(전부 이 사이트의 정리라고 밝혔다)
1. **⭐ 20년 넘은 제품을 아직 팔기 때문에 멈추지 않고 계속 옮겨야 한다** — 새로 만드는
   이야기가 아니라 **옛 가정을 지키면서 새 바닥에 올리는 이야기**다. `fcntl` 잠금 때문에
   **StatefulSet 을 버리고**(*"リリースのたびにダウンタイムが発生する"*) 파드를 한 노드에
   묶었다 — **가용성을 얻고 분산을 내줬다.**
2. **⭐ 관리 대상이 너무 많으면 저장을 없앤다** — 테넌트 **4만 건 초과**라 설정 대신 주소에서
   발급자를 **조립**해 관리 대상을 0개로. **🔴 그 값을 사용자가 매번 주소를 입력하며 치른다고
   회사가 스스로 적는다**(*"…スムーズではありません"*).
3. **⭐ 문서를 믿지 않고 커널까지 내려가 재 본다** — *"カーネルの挙動を調べました"*
   (`O_DIRECT` · 타임스탬프 · 5GB → 977.2M 캐시 · `bpftrace`).
4. **⚠️ 글의 두께가 크게 다르다** — 두꺼운 둘(이관 · OIDC), 중간 둘(샤딩 · govulncheck),
   얇은 하나(Cattage). **최근 목록의 절반 넘게가 행사·채용·회고다.**
   **그래서 목록이 아니라 본문으로만 판단할 수 있는 회사다** — 305 에서 목록만 봤으면
   회사 자체를 떨어뜨렸을 것이다.

**⚠️ 못 채운 것** — **2026년 영업이익 전망이 +4.1% 인 이유**(매출은 +12.7%) · 제품별 매출
내역 · 요금 구조 · 부문별 이익률 · **kintone 자체의 앱 실행 엔진이나 데이터 모델 이야기**.
...
**후보 조사에서 볼 곳**(298·305 에서 아홉 곳을 보고 둘만 올렸다 — **"1차 자료가 있다"와
"쓸 수 있다"는 다르다**)
- **분포** — **53곳.** US 22 로 포화 · 한국 아홉으로 편중 · **일본 넷** · 중국 둘 ·
  동남아 하나 · 인도 하나 · 유럽 일곱. **채울 곳은 중국·인도·유럽 쪽이다.**
- **아직 안 본 곳** — 중국은 Kuaishou·JD·Alibaba 계열을 안 봤다(**Xiaohongshu 는 블로그가
  없었다**). 일본은 Sansan·freee·SmartHR·Money Forward 를 안 봤다. 유럽은 SoundCloud·
  Trade Republic·Personio 를 안 봤다.
...
- **📌 kakao·roblox 가 `in_progress` 로 남아 있다** — 각각 `hold_reason` 달린 도메인 하나
  때문이고 **새 단서가 없으면 hold 를 유지한 채 닫는 것도 정당하다**(290). **이 둘을 닫는
  것도 한 사이클이 된다.**

**⭐⭐⭐ 비교 문서가 계속 밀려 있다. 축이 넷이고 하나는 재료가 여섯이다.**
1. **`문지기를 무엇으로 세우는가` — 가장 굵다.** Wix 컴파일러(타입 정의로 컴파일해 통과한
   ...
   monday.com Morphex. **⭐ 핵심 둘** — **① 대가가 한 방향으로 모인다: 검사할 수 있는 것만
   검사되고 그 몫이 얼마인지는 어디에도 없다** · **② 문지기의 성질이 갈린다: 컴파일러는 못
   잡는 범위가 분명한데 AI 문지기는 무엇을 못 보는지가 흐릿하다.**
   **⭐ 그리고 Cybozu 가 반대 방향 재료를 준다** — govulncheck 편은 문지기가 **너무 많이
   잡는 것**(오탐)을 심볼 도달성으로 줄인다.
2. **`에이전트에게 무엇을 못 하게 하는가`** — 재료 일곱, 세 갈래.
3. **`자를 어떻게 세우는가`** — Snap 넷 + Meituan 둘.
4. **`무중단으로 갈아 끼우기`** — Wix 두 편(인프라 층·앱 층) + **Cybozu**(애플리케이션이
