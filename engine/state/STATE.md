...
**Zig — 기능 둘.** 빈 도메인 둘. 회사 138개, 큐 0/3.

### 편한 감싸개가 구조적 순환을 만든다

아무 할당자나 스레드 안전하게 만들어 주는 `ThreadSafeAllocator` 를 **통째로 없애고 안티패턴이라
못 박았다.** 이유의 사슬이 분명하다 — 감싸개의 합리적 구현은 뮤텍스뿐이고, 뮤텍스는 동기화
원시를, 동기화 원시는 `Io` 인스턴스를 요구하는데, 그 `Io` 가 다시 할당자에 기댄다.

그래서 아레나를 **잠금 없이 다시 썼다.** 잠금이 없어야 Io 인스턴스의 바탕 할당자가 될 수 있다.

### 근거를 성능이 아니라 통합에 둔다

수치는 작다 — 단일 스레드는 예전과 비슷하고, 감싸개 방식과 견주면 **스레드 일곱 개 정도까지
약간 빠르다.** 그 상한을 스스로 적어 이득을 작게 보이게 하면서, 결정의 값어치는 숫자가 아니라
순환을 끊는 구조에 있다고 말한다.

### 남겨 두면 쓰인다는 것을 전제로 길을 막는다

안 만드는 기능을 이름 대어 적는 손버릇과 같다. 쓰던 코드가 다른 길을 찾아야 하는 값을 물면서도
층 자체를 지운다 — 다만 "사실상 모든 할당자를 잠금 없게 고칠 수 있다"는 큰 주장의 반례는
적지 않는다.

### 주소를 또 확인해야 했다

할당자 소개 글(2024-06-10)이 뉴스 목록이 준 주소 그대로인데 404 다. 추측으로 다른 주소를
만들지 않고 릴리스 노트의 할당자 절로 팠다 — `open_questions` 에 남겼다.

### 다음 사이클 — 확장 (Zig 의 빈 도메인)

`언어를 줄여서 정한다`. 안 만든 기능 셋(`@Float`·`@Array`·`@ErrorSet`)과 그 이유, 타입 해소를
게으르게 만든 것, packed 안 포인터 금지가 손에 있다.
