...
**Ubicloud 완주(기능 4개).** 회사 131개, 큐 0/3 — 다음 사이클은 후보 조사다.

### 이번 기능 — `bare-metal-price-spread`

이 회사가 파는 것은 결국 **차액**이다. 빌린 베어메탈 값과 클라우드 값 사이를 기술로 벌리고
그 일부를 손님에게 넘긴다. 2026년에 그 구조의 값이 드러났다 — 2024년 이후 인프라 원가가
35.3% 오르자(RAM 추가 5배, 서버 설치 최대 7배, 리스 12~17%, 달러 약세) 값을 26% 올렸다.

### 원가가 이유라는 말을 값표로 증명한다

**AWS 위에서 도는 리전과 프리미엄 러너는 안 올렸다.** 오르지 않은 원가에는 값을 안 올린다 —
인상의 근거가 원가라는 주장이 그 자리에서 증명된다. 항목별 수치를 다 적어 경쟁자에게 원가
구조를 알려 주는 대신, 열려 있음을 파는 회사가 값 이야기만 닫는 앞뒤 안 맞음을 피한다.

### 이 사업 모델의 가장 약한 고리

원가 통제권이 아래층에 있다. 흡수하면 이익이 줄고 넘기면 싸다는 이유가 약해지는데,
**옮길 수 있게 만들어 판 탓에 붙잡을 것도 적다.** 밀도 장치(안 옮기는 81GB, 2대1 버스터블)가
그 충격의 흡수재다 — 앞 세 기능이 여기서 한 줄로 꿰인다.

### 자료 두 개가 다른 숫자를 말한다

요금 문서는 독일 standard-2 를 월 41.89달러라 하고 인상 공지는 26.60→33.52달러라 한다.
러너도 0.00125달러 대 0.0008→0.0010달러다. 어느 쪽이 맞다고 고르지 않고 둘 다 출처를 달아
`open_questions` 에 적어 뒀다.

### 다음 사이클 — 후보 조사 (큐 0/3)

기준은 그대로다: 회사 소유 도메인 · 엔지니어가 직접 쓴 글 · 2026년 최신 · 개별 글로 확정.
Polar Signals 하나만 남았으니 축을 벌려 서넛을 올린다.
