...
**없다 — 문체 보수 사이클이었다.** DoorDash 를 배민 문체로 다시 썼다(2.38 → 0.0). 회사 115개, 큐 2/3. 38곳 남았다.

### 빨리 가려고 멈춘 회사가 셋이 됐다

첫 기능에서 앞 두 사이클의 축을 이어 받았다.

- Ramp 는 고객마다 다른 규칙을 코드로 못 받겠다고 판단해 기능 대신 재료를 만들었다.
- 볼(bol)은 옛 모놀리스의 과거 4년 치를 두고 갈 수 없어 함께 옮겼다.
- DoorDash 는 셋 중 가장 크게 멈췄다 — **2019년 말 신규 기능 개발을 전부 멈췄다.**

그런데 끝까지 동결할 수는 없었다. 사업이 커야 했으니 스택을 바꾸고 기능을 옮기면서 새 기능도
만들어야 했다. 이 회고가 드문 것은 다른 데 있다 — **가장 어려웠던 것이 기술이 아니라 사람이었다고
이름까지 붙여 적는다(IKEA 효과).** 자기 조직의 인지 편향을 회고에 적는 회사는 이 엔진에서 처음이다.

### 노트북을 떠난 두 회사 — 천장을 낮추는 쪽과 바닥을 까는 쪽

둘째 기능을 Razorpay 와 붙였다. 노트북 방식의 한계를 꼽는 목록이 거의 같다.

- Razorpay 는 격리를 커널에 맡겨 **에이전트가 무엇까지 할 수 있는지를 가뒀다**.
- DoorDash 는 에이전트를 만들지 않고 **에이전트가 설 자리를 만들었다** — 환경·도구·권한·통합·제약.

한쪽은 권한의 천장을 낮추고 다른 쪽은 바닥을 깐다. 그래서 이 회사가 남긴 교훈도 플랫폼 쪽이다 —
재사용 가능한 워크플로는 플랫폼이 있다고 저절로 생기지 않는다.

### 무엇을 볼지 미리 정하는 쪽과 미루는 쪽

셋째 기능을 관측 두 회사와 붙였다. Honeycomb 은 이벤트 하나로 값을 매겨 아무나 되묻게 하고,
Datadog 은 보고 있는 것만 모은다는 원칙을 수집기에 심었다. **둘 다 무엇을 볼지를 먼저 정하는
쪽인데, DoorDash 는 파이프라인을 종류마다가 아니라 이벤트마다 만들어 그 결정을 뒤로 미룬다.**

### 도메인을 사업이 아니라 무엇을 다시 지었느냐로 잘랐다

음식 배달·마트·광고로 가르면 셋이 한 칸에 뭉친다. 회사가 공개한 시스템 글이 전부 다시 짓는
이야기이기 때문이다. 셋째 도메인이 이벤트 파이프라인 한 갈래로만 서 있는 것은 배차 자체를
아직 못 읽어서라는 것도 도메인 설명에 적어 뒀다.

### 다음 사이클 — 문체 보수 (사다리 3순위)

--gaps 가 정한다. 지금 맨 위는 etsy(2.33)지만 그건 이 시점의 예상이지 확정이 아니다.
