...
**turbopuffer — 기능 둘.** 남은 빈 도메인은 색인 하나(블로그 미독). 회사 143개, 큐 1/3.

### 파는 것은 저장이 아니라 따뜻함이다

같은 데이터가 캐시에 있느냐 없느냐로 **874밀리초와 14밀리초**를 오간다. 60배 넘는 차이가 곧
제품의 성질이라, 값도 온도로 나눠 매긴다(차가운 GB 당 0.02달러, 따뜻한 0.1달러).

### 안전한 기본값은 빠른 기본값이 아니다

기본을 **강한 일관성**에 뒀다 — 방금 넣은 문서가 검색에 안 나오면 제품이 고장 난 것처럼 보이기
때문이다(재구성). 더 빠르게 원하면 최종 일관성으로 바꿔 10밀리초 아래가 되지만 **최악의 경우
한 시간까지 낡는다**고 상한을 적는다.

### 쓰기를 묶으면 쓰고 바로 읽기가 느려진다

객체 저장소는 쓰기 횟수가 곧 값이라 네임스페이스당 초당 WAL 한 항목으로 묶는데, 기본이 강한
일관성이라 그 **최대 1초가 쓰고 바로 읽는 경로의 지연**으로 이어진다(재구성).

### 따뜻함은 우리가 못 정한다

어떤 네임스페이스가 자주 쓰이는지는 손님의 접근 패턴이 정한다. 가끔 쓰는 쪽에는 차가운 값이
기본 경험이 되는데, 그 숫자를 감추지 않고 문서에 나란히 적는다.

### 다음 사이클 — 확장 (turbopuffer 의 마지막 빈 도메인)

`색인이 읽을 양을 정한다`. 블로그(역색인 설계·FTS v2·BM25·MAXSCORE·SIMD)를 열어야 지금
`unknown` 인 tech 두 칸이 풀린다.
