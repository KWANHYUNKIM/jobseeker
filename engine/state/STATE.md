...
**Deliveroo** (UK · 커머스 — 음식·식료품 배달) — **프로파일까지 썼다**(PROMPT 3단계). 도메인 3개 · **기능 0.** 회사 **89개.**

### 이번 사이클 — 신규(6순위). 큐의 마지막 한 곳을 올렸다

⚠️ **이 엔진이 처음 만난 종류의 벽을 만났고, 그것이 이번 사이클의 가장 큰 수확이다.**

- ⚠️ **`deliveroo.engineering` 은 Cloudflare 오류 1009 — 한국(KR) IP 를 국가 단위로 막는다.** WebFetch 도 브라우저 `get_page_text` 도 **똑같이 차단 페이지를 받는다.** **지금까지의 403 은 도구를 바꾸면 뚫렸는데(Medium·Netflix·Etsy·DoorDash) 이것은 도구의 문제가 아니다.**
- ✅ **뚫는 법 — 텍스트 추출 프록시를 앞에 둔다.** `https://r.jina.ai/<원래 주소>` 로 **본문 전문이 마크다운으로 온다.** WebFetch 로 부르면 요약이 오므로 **브라우저 navigate → `get_page_text` 로 받아야 원문 인용을 쓸 수 있다.** 접근 지도에 올렸다.
- ⚠️ **GitHub Pages 사이트라 `/blog/` 는 404 다** — 목록은 **루트**이고 더 보기는 `/articles/pageN`. **슬러그를 추측하지 않는다는 규칙이 또 한 번 값을 했다.**

### 이번 회사를 고른 이유 — 규모가 아니라 각도다

**같은 그룹이 된 도어대시와 결이 다르다.** 도어대시의 Flux 는 **에이전트가 설 자리**를 만들고(`Primitives, not workflows`), 딜리버루는 **규정 준수 자체를 코드로 심는다**(`Governance as Code`). ⚠️ **`AI 를 어디까지 믿을 것인가` 축의 여섯 번째이고, 특히 `그 선을 무엇이 지키나` 에 새 답을 준다.**

읽은 것에서 건진 것 셋(2026-02-27 글):

- ⚠️ **문제는 AI 를 만드는 것이 아니라 팀마다 따로 만든 것을 다시 모으는 일이었다** — **비슷한 관측·보고를 팀들이 나란히 만들었고, 엔지니어링과 법무 사이의 거버넌스가 급격히 복잡해졌으며, 규정 준수가 팀별 맞춤 작업으로 자라났다.**
- ⚠️ **강제 지점을 배포 파이프라인에 둔다** — 표준 모델 버저닝 정책을 **MLOps 로 강제해 비준수 모델을 스테이징에 못 올리게** 막는다. **다만 회사가 `현재 작업 중` 이라고 적는다 — 계획이지 이미 도는 장치가 아니다.**
- ⚠️ **정책을 데이터셋으로 만든다** — 3면 마켓플레이스라 **고객·파트너·라이더 에이전트에 같은 톤 정책**이 걸리므로 **하나의 벤치마크 데이터셋**으로 묶고, **법무·프라이버시 팀이 회의 대신 그 데이터셋에 직접 기여**한다.

### 다음 사이클 — 이 회사를 확장한다(2순위)

`status: in_progress` 라 `--gaps` 가 **확장**을 부른다. **⭐ `AI 를 회사 규모로 굴리면서 규정을 지킨다` 부터 판다** — 자료를 이미 읽었고 결정·대가가 충분하다. ⏳ 그 다음 후보 둘의 본문은 아직 안 읽었다: `experimentation-principles`(2025-02-06 — **Etsy 의 `부분의 합이 전체가 아니다` 와 견줄 자리**) · `3ds-optimisation`(2023-05-17 — 승인률과 사기의 맞바꿈).

⚠️ **큐가 0/3 이 됐다.** 이 회사를 다 판 뒤 **3순위(후보 조사)** 가 걸린다. ⏳ 되짚을 목록 — `www.meesho.io/blog` · `blog.zepto.com` · `bytes.swiggy.com` · `www.gojek.io/blog`(429) · `tech.cygames.co.jp` · `www.scylladb.com/blog`. ⏸️ Snyk 보류 — `New Agentic Architecture for Snyk Agent Fix` 를 열고 결론 낸다.

### ⚠️ 비교 문서 재료 두 축 (초안 유지)

**① `AI 에이전트를 어디까지 믿나` 6곳** — Sentry(조직의 신뢰) / ClickHouse(접근 범위, **세 겹 중 한 겹만 기계가 강제**) / Duolingo(사람 리뷰 → **병목**) / Ramp(**사람의 신원**) / DoorDash(**primitives, not workflows**) / **Deliveroo(**거버넌스를 코드로 — 강제 지점을 배포 파이프라인에**)**.
