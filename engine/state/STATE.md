...
**없다 — 예순네 번째 후보 조사를 돌았다. 큐 1/3.** 다음 사이클은 **신규**(Elastic).

### 하나의 점수로 합치면 서로를 상쇄한다

Elastic 의 샤드 밸런싱 글이 통과한 근거다. 옛 밸런서는 쓰기 부하·디스크·샤드 수·인덱스
균형을 **가중치로 합쳐 하나의 점수**로 만들었는데, 그래서 **"한 지표의 높은 값이 다른
지표의 낮은 값을 상쇄"** 했다 — 쓰기 부하가 높아도 샤드 수가 적으면 아무것도 안 한다.
게다가 **가중치를 손대면 OOM 으로 이어질 수 있었다.**

### 그래서 독립된 판정자로 쪼갰다

`IndexBalance` · `HeapUsage` · `WriteLoad` 셋이 **각각 따로** 판정하고, 판정값에
**`NOT_PREFERRED`** 를 더했다(YES/NO 옆에). 블루스카이가 모더레이션을 **한 겹의 권한에서
층으로** 옮긴 것과 결이 같다 — **하나로 합치지 않고 나눠 두는** 쪽이다.

### 무엇을 감수하는지 명시한다

**"샤드는 그래도 `NOT_PREFERRED` 노드로 재배치되고 어느 정도 성능 저하를 감수한다 —
데이터 노드 OOM 을 무릅쓰는 것보다 나은 선택으로."** 나쁜 것 둘 중 무엇을 고를지를
적어 두는 문장이다.

### 첫 화면으로 판정하지 않는 규칙이 두 번 값을 했다

`elastic.co/blog` 첫 화면은 제품 공지와 고객 사례가 지배해 접을 뻔했다. **`/search-labs`**
로 갔다가 다시 **`/search-labs/blog`** 로 들어가서야 기술 글 목록이 나왔다 — **두 단계를
더 따라갔다.**

### Algolia 는 못 읽었다

블로그가 **"No Products Found!!!"** 만 뜬다. 카테고리 링크를 따라가도 같다 — 이 도구로는
**목록이 렌더링되지 않는 페이지**다.
