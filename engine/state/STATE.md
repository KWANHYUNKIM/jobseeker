...
**없다 — Plaid 를 완주했다.** 큐가 **0/3** 이다.

### 이번 사이클 — 계좌 연결 `은행마다 다른 규칙을 대신 흡수하되, 새는 것을 문서에 적는다` 와 완주

⚠️ **자료를 세 번 찾아야 했다.** ① 검색으로 후보를 받고 ② `api-progress-update` 를 열었더니 **정책·홍보 글**이었고(버린 대안 0·대가 0) ③ **Fastly 때 제품 문서가 1급이었던 것을 떠올려 개발자 문서(`plaid.com/docs/link/oauth`)를 열어** 자료를 얻었다.

- ⚠️ **파는 것은 `하나의 API` 인데 뒤에는 1만 2,000곳의 서로 다른 규칙이 있다** — **문서가 은행 이름을 적어 예외를 나열한다**: Chase 는 **사용자가 동의 기간을 바꿀 수 있어** 기존 연결이 만료일을 얻고(만료되면 **ACH 용 토큰 계좌번호도 멈춘다**), Capital One 은 **잔액 조회에 신선도를 지정해야** 하고 못 맞추면 **호출이 실패한다.** **감추면 런타임에 놀라고, 다 노출하면 하나의 API 가 아니다 — 그 사이를 문서로 메운다**(재구성).
- ⚠️ **자격증명을 갖지 않은 대가가 명확하다** — OAuth 라 비밀번호가 없으니 **끊긴 연결을 자동으로 다시 이을 수 없다.** `ITEM_LOGIN_REQUIRED` 가 되고 **사람이 다시 와야 한다.** 그래서 **동의 만료 웹훅을 듣고 끊기기 전에 알리라**고 문서가 권한다.
- ⚠️ **혼자 정할 수 없는 결정이 있다** — 미국·캐나다에 오픈 파이낸스 의무가 없어 **API 채택에 조율된 산업 노력**이 필요했다. 그래서 **`연결의 75% 가 API`** 가 성과로 발표된다 — **나머지가 아직 스크래핑이라는 뜻이기도 하다.**

**완주 판정**: 도메인 2/2 · 고아 없음 · 수익원 둘 다 연결. **경고도 2건에서 1건으로 줄었다.**

⚠️ **다만 이 회사에는 큰 구멍이 남았다** — **핵심 제품의 엔지니어링 회고를 못 찾았다.** 정책 글과 개발자 문서로 기능을 썼고, **1만 2,000곳의 변화를 어떻게 감지하고 따라가는지**는 비어 있다. **재연결 비율도 공개하지 않는다.**

### 다음 사이클 — 큐가 0/3 이라 후보 조사

⏳ **되짚을 목록** — `blog.paystack.com` · `www.meesho.io/blog` · `blog.zepto.com` · `bytes.swiggy.com` · `www.gojek.io/blog`(429) · `tech.cygames.co.jp` · ScyllaDB(목록이 안 읽힌다). **되짚을 때는 브라우저로 열고 검색으로 현재 주소도 함께 받는다.**

⏳ **안 두드린 새 이름** — Deliveroo · Delivery Hero · Snyk/Okta(보안 둘째 자리).
