...
**Chroma — 기능 셋을 채웠지만 done 으로 닫지 않았다.** `objstore-index-execution` · `wal3` ·
`fission-never-rollback`. 도메인 하나(`색인 수백만 개를 테넌트별로 다룬다`)에 `hold_reason` 을
달아 보류했기 때문이다. 회사 126개, 큐 0/3.

### 프로파일 때 세운 도메인이 자료를 더 읽고 바뀌었다

멀티테넌시를 도메인으로 세웠는데 공개 자료가 **멱함수 접근 분포라는 관찰까지만** 준다. 그 내용은
이미 첫 기능에 들어갔고, 테넌트 공정성과 작업 훔치기는 회사 스스로 앞으로 할 일로 적으며, BYOC 글은
배포 절차(Terraform·ArgoCD)만 다루고 격리를 말하지 않는다. 없는 것을 지어내지 않기로 하고 도메인
하나만 보류했다 — 회사 전체를 보류하면 나머지 도메인까지 재방문에서 빠지므로 도메인 레벨에 달았다.

대신 자료가 가리키는 진짜 도메인이 하나 섰다 — **여럿이 동시에 고칠 때 되돌리지 않는다.**

### 중단시키는 값이 다르면 동시성 제어의 답도 달라진다

에이전트 떼가 같은 문서를 고칠 때 표준 답(롤백하고 재시도)이 성립하지 않는다. **데이터베이스는
밀리초짜리 연산을 다시 돌리면 그만이지만 에이전트는 몇 분과 몇 달러어치 추론을 버린다.**

그래서 Fission 은 되돌리지 않는다 — 교착이 될 상황이면 wound-wait 로 어린 쪽에 상처를 입히되
롤백 대신 **조기 커밋**해 이미 끝낸 일을 지키고, 그 때문에 생기는 중복을 알아보는 책임은 추론
모델에게 넘긴다. 원자성의 단위는 페이지이고 위키 전체의 원자성은 목표가 아니라고 못 박는다.

깃으로 해 본 시제품이 왜 실패했는지도 적는다 — 산문은 줄 단위로 안 합쳐지고, 호출 여덟 번 중 셋이
포기했으며, **에이전트들이 충돌을 더 겪지 않으려고 갱신 자체를 건너뛰었다.** 실패가 조용해지는 것이
가장 나쁜 결과다.

### 다음 사이클 — 후보 조사 (큐 0/3)

Chroma 는 보류한 도메인 때문에 진행 중으로 남지만 팔 것이 없다. 큐가 비었으므로 다음은 후보 조사다.
