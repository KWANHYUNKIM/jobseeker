...
**Flipkart** (IN · 커머스) — 도메인 3개 · **기능 2개.** 남은 도메인은 **`5억 고객의 공급망을 계획한다`** 하나다(⏳ 미독, 글 2편).

### 이번 사이클 — `사람의 다수결을 배우고 사람을 넘어선다`

건진 것 넷:

- ⚠️ **관련성은 정답이 있는 문제가 아니라 기준을 정해야 하는 문제다** — **`Wakefit 금속 침대` 에 나무 Wakefit 침대가 걸리면 브랜드로는 맞고 소재로는 틀리다.** **주석자마다 무엇을 더 무겁게 보느냐가 달라 라벨이 흔들렸고 그 흔들림이 하류 모델의 잡음이 됐다.** ⚠️ **사람을 늘려도 이 문제는 오히려 나빠진다**(재구성).
- ⚠️ **자료의 분포가 곧 모델의 성격이 된다** — 무작위 표집은 관련 쪽으로 쏠려 **거절을 못 배우게** 한다. 질의마다 세 버킷에서 최소 하나씩 넣은 것이 **`모델에게 나쁜 매치를 거절하는 강한 능력을 줬다`**.
- ⚠️ **설명을 사람이 쓰지 않고 모델이 만들되 결과로 걸렀다** — **정답으로 이어진 추론만 남긴다.** **생성한 것을 결과로 거르는 방식이라 사람이 할 수 없는 규모의 품질 관문이 된다**(재구성).
- ⚠️ **검증을 모델 점수가 아니라 판단으로 했다** — **22주간 수백만 쌍에서 자동 보고서와 수동 보고서의 NDCG 차이가 1% 미만.** **모델의 정확도가 아니라 `그 점수로 내리는 결론이 같은가` 를 봤다**(재구성).

**수치** — 인간 주석자 대비 **3단계 +2.3% · 이진 +2.9%** · 최고 상용 LLM 대비 **+12% 이면서 비용 약 30%** · 추론 코치 **약 80억**, 교사 **320억.**

### ⭐ Etsy 와 정면으로 견줄 자리다 — 사람의 자리가 다르다

**Etsy** 는 **`LLM 은 인간 판단을 대체하지 않고 정렬해 증폭한다`** 고 적고 **3단 증류**로 갔다. **Flipkart** 는 **`인간 주석자보다 정확하다`** 를 수치로 내놓고 **사람을 가이드라인 정의로 옮겼다.** ⚠️ **같은 문제(검색 관련성)에 대한 두 답이고, 둘 다 사람을 없애지 않지만 남기는 자리가 다르다.** open_questions 에 적어 뒀다.

### ⚠️ 회사가 스스로 적은 한계 셋

**이미지를 안 본다**(관련성 신호가 많은데) · **약 80억 모델은 실시간에 너무 느리다(>100ms)** 라 **5~10억으로 증류해야 한다** · **이커머스에 튜닝돼 있어 새 도메인엔 few-shot 이 필요하다.** ⚠️ **정확한 모델이 곧바로 사용자 화면으로 가지는 않는다** — 지금은 **배치로 라벨을 만들어 하류 모델의 자료로 쓴다**(재구성).

### 다음 사이클 — 마지막 도메인, 그리고 완주 판정

`--gaps` 가 **`5억 고객의 공급망을 계획한다`** 를 부를 것이다. ⏳ **글 2편 미독** — `인도 공급망의 맥박`(2026-05-05) · `시계와의 경주: 공급망 용량 최적화`(2026-04-15). **URL 은 목록에서 받아 뒀다.** 자료가 서면 **같은 사이클에서 완주 판정까지** 한다.

⚠️ **큐는 1/3**(Twilio). ⏳ **보강 거리 둘** — Grafana Labs 인용 대조 · Razorpay 보안 트리아지 글. ⏳ **Flipkart 의 Rate Card 엔진 글**도 남아 있다(어느 도메인인지 본문을 봐야 안다).
...
**③ `자기 성과를 어디까지 주장하나` 4곳 + Flipkart** — Snyk / Grafana Labs / ScyllaDB / Razorpay. ⚠️ **Flipkart 도 한계 셋을 스스로 적는다.**
...
**⑨ `한 번에 갈아엎을 것인가 목 졸라 죽일 것인가`** — ⏳ Twilio / Etsy / Plaid / Paystack.

**⑩ `관측 비용을 어디까지 줄이나`** — Razorpay / Grafana Labs / Honeycomb.

**⑪ `깨질 걸 알면서 고른 의존을 어떻게 다루나`** — Razorpay / Plaid / Paystack / ScyllaDB.

**⑫ `빠른 숫자와 정확한 숫자를 어떻게 가르나`** — Flipkart / Zepto / Deliveroo.

**⑬ `애매할 때 어느 쪽으로 넘어지나`** — Flipkart / Razorpay / Zepto / Paystack.

⭐ **⑭ 재료가 찼다 — `검색 관련성을 누가 정하나`** — **Etsy**(사람이 기준을 만들고 LLM 이 증폭, 3단 증류) / **Flipkart**(모델이 인간 주석자를 넘어서고 사람은 가이드라인으로) / ⏳ **Zepto**(구어 질의 교정과 exploit/explore).
