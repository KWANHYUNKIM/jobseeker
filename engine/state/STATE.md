...
**Sansan(JP · SaaS) `in_progress` — 도메인 5개 · 기능 6개.** 335 에서 도메인 ⑤ 의 **두 번째
기능 `coordinator-in-between`(사이에 조정자를 두고 두 판을 함께 굴린다)** 을 썼다.
**결정 5개 · 그림 3장 · 생각 4개 · 난제 2건 · 연결 1건.** 재료는
`SansanのAndroid View→Jetpack Composeへの移行計画`(2025-11-17).
**판정: 버린 대안 ✅(전면 재작성) · 대가 ✅✅ · 수치 ✅ → 3/3 통과.**

**🔴 그리고 334 의 도메인 이름을 고쳤다**(정정을 `open_questions` 에 남겼다).
334 에서 `컴파일러가 엄해지기 전에 옮긴다` 로 열었는데 **Android 쪽에는 그런 강제가 없다** —
Swift 쪽은 **Swift 6 의 SCC 가 기본이 되는 시한**이 있지만 Compose 쪽은 **3년이라는 자기
계산**뿐이다. **이름이 두 편을 다 못 담아** `십 년 된 앱을 한 번에 못 옮긴다` 로 바꿨다
(`index.json` 의 `domains` 배열과 기능 둘의 `domain` 도 함께 고쳤다).
**334 에 적었던 "글 한 편으로 썼다" 도 이제 맞지 않아 그 항목을 고쳤다.**

**⭐ 두 기능이 한 도메인에서 만난다** — 둘 다 **십 년짜리 앱**, 둘 다 **한 번에 못 옮긴다는
전제**(*"一気に移行することはできません"* / *"3年かかる計算になります"*), 둘 다 **두 판을
나란히 굴리고**, 둘 다 **AI 에게 반복을 맡긴다.**
**⭐⭐ 그런데 갈리는 곳이 둘이다** — **순서를 어디서 끌어내는가**(iOS 는 VIPER 의존 방향,
Android 는 새로 만든 **ViewCoordinator 사이 층**)와 **시한을 누가 정했는가**(Swift 6 대 자기 계산).

**⭐ Compose 편에서 가장 값이 큰 것** — **계획이 도구의 미래에 걸려 있다고 스스로 적는다.**
3년이라는 숫자를 내놓고 **AI 가 줄여 줄 것에 기대는데 모델이 충분히 자동화해 줄지는
불확실하다**고 인정한다. ⚠️ **기대만큼 안 됐을 때의 대안은 글에 없다** — 난제로 적었다.
그리고 **검증된 것은 화면 두 개뿐**인데 **원래 문제가 상태 관리가 제각각이라는 것이었으므로
가장 이상한 화면들이 남아 있을 가능성이 크다**(이 지적은 이 사이트의 것).

---

**다음 사이클 — Sansan 을 닫는 판단이다. 남은 것은 대담과 회고뿐이다.**
- **남은 글** — `【Sansan Tech Talk】名刺アプリ「Eight」の技術的挑戦 〜EC2からの移行と
  モジュラーモノリス化への道〜`(**대담 형식으로 보인다**) · `レイヤードアーキテクチャを振り返る`
  (2019-04-21, **회고**라 대가가 있을 수 있다) · `AIと並走して作る、現場と地続きのシステム設計`
  (2026-06-26) · 모바일 개발 대담 2부작(2026-08-21·08-25).
  **⚠️ 제목만으로 떨어뜨리지 않는다**(298 의 교훈) — **회고 편을 먼저 연다.**
- **기준을 넘기는 것이 없으면 닫는다.** 닫을 때 `business_model` 에 적을 관찰:
  **⭐⭐ 다섯 도메인이 같은 회사의 다섯 층이다** — 개발 환경 · 인식 · 공정 실적 · 실물 ·
  **십 년 된 앱**(iOS 와 Android 둘 다). **이만큼 층이 다른 회사는 이 사이트에 드물다.**
  **⭐ 글의 두께가 층마다 다르다** — 개발 환경과 앱 쪽은 **대가를 아낌없이 적고**, **인식
  쪽은 성과 중심이며**, **Digitization 쪽은 대담·운영 서사가 많다**(추정).
  **⭐ 이 회사는 자기가 만든 것을 안 쓴다고 적는다**(초해상). **⚠️ 그런데 가장 아프다고 말한
  항목의 정확도는 공개하지 않는다**(공개한 것은 주소 98.3% 하나).
  **⭐⭐ 다섯 번 다 점진을 골랐는데 버린 이유가 매번 달랐다** — 저장소(개발을 안 멈추려고) ·
  GitHub Enterprise(코드 프리즈가 길어서) · 스캔 센터(위험을 나누려고) · Swift
  Concurrency(한 번에 못 옮겨서) · **Compose(다시 만들 수 없어서)**.
  **⭐ 그리고 마지막 둘은 AI 를 이관 인력으로 쓴다** — 하나는 이미 돌리고 있고(Claude Code
  Skills), 하나는 **앞으로의 개선에 기댄다고 적는다.**

**📌 그 밖**
- **⚠️ 확인 못 한 GEES 99.9% 는 안 썼다.**
- **큐 2/3**(freee · SmartHR). **⚠️ Personio(EU)는 아직 안 봤다 — 다음 후보 조사의 첫 순위.**
- **⭐⭐ freee 와 SmartHR 이 정반대 순서로 같은 문제를 풀었다**(비교 문서 재료).
- **⭐ 비교 문서로 쓸 재료 다섯** — ① 320 Paytm → `reversible-first.md`·`where-it-piles-up.md`
  ② 326 Freshworks → *"언제 롤백 경로를 닫는지 정한 곳이 없다"* ③ 333 Sansan → **되돌릴 수
  없는 것이 화면 밖에 있다** ④ 334 Sansan → **되돌릴 자리를 남기려고 중간 상태의 비용을
  감수한다** ⑤ **⭐ 335 Sansan → 이관 계획이 도구의 미래에 걸려 있다고 적는다.**
  **⭐⭐ 이관·되돌리기 축의 재료가 이제 아주 두껍다 — 비교 문서 사이클을 한 번 끼울 때다.**
- **후보 조사 누적 열아홉 곳 중 일곱.**
- **⚠️ 낡은 메모를 나르지 않는다**(314·317·325). **⚠️ 두 방향 규칙**(298·305·318·321).
  ...
  **⭐ 도메인 이름이 좁으면 다음 재료가 안 들어온다**(334→335, 새 교훈).
