...
**Linear — 프로파일만 썼다(도메인 둘, 기능 0).** 회사 152개, 큐 1/3.

### 로컬 사본을 주면 서버의 일이 바뀐다

클라이언트마다 로컬 데이터베이스를 두는 순간, 서버가 하는 일은 화면을 그려 주는 것이 아니라
**각 클라이언트가 놓친 변경을 골라 주는 것**이 된다. 마지막으로 적용한 ID 를 체크포인트로
받아 그 뒤만 돌려준다 — 작업 공간 전체를 다시 받게 할 수는 없어서다.

### 규모가 그 고르기를 무너뜨렸다

가장 큰 작업 공간이 하루에 **백만 건에 가까운 동기화 액션**을 만들고, 잠깐 끊긴 클라이언트도
**수십만 건 뒤처져** 돌아온다. **중앙값은 멀쩡한데 꼬리 지연이 출렁였다** — 포스트그레스가
넓어지는 ID 범위에 배열 겹침 술어로 상관없는 행까지 시험하고 버려서, p95·p99 가 작업 공간
크기에 비례해 올라갔다. 읽기 복제본을 늘려도 교집합 계산 자체는 안 준다.

### 튜닝이 아니라 질의의 모양을 바꿨다

2차 색인을 **turbopuffer** 로 옮겼다 — 역색인과 포스팅 리스트로 행을 거르는 대신 집합
교집합을 바로 구한다. **turbopuffer 는 이 엔진에 이미 있다**(만드는 쪽을 판 뒤 쓰는 쪽을 보는
네 번째 사례). GIN 색인은 이 규모에서 쓰기 값이 말이 안 돼 물렸다.

### 도메인을 억지로 셋으로 늘리지 않았다

기술 글이 사실상 한 편이라 **도메인을 둘로 뒀다.** 없는 것을 지어내지 않는다는 규칙이
도메인 수에도 적용된다.

### 다음 사이클 — 확장 (첫 도메인)

`끊겼다 돌아온 클라이언트를 따라잡힌다` 와 `읽는 길의 모양을 바꾼다` 둘 다 같은 글이 재료다.
**⚠️ 지연 백분위의 구체적 값이 글에 숫자로 없다**(그래프로 '거의 평평하다'고만 보인다) —
없는 수치를 지어내지 않는다.
