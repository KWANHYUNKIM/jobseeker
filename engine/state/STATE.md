...
**DoorDash** (US · 커머스) — **프로파일까지 썼고, 이 엔진에서 드물게 `eras`(회사 연표) 3시기를 세웠다.** 도메인 3개, 기능은 아직 0이다.

### 이번 사이클 — 신규(6순위), DoorDash 프로파일

⚠️ **접근 지도를 정정해야 했다** — 앞 사이클에 `careersatdoordash.com/blog` 를 `✅ 자체 호스팅, 검색으로 주소가 잘 나온다` 고 적었는데, **WebFetch 는 403 을 준다.** 주소가 나오는 것과 본문이 읽히는 것은 다른 문제였다. **브라우저(`get_page_text`)로는 전문이 그대로 온다** — 그 방법으로 두 편을 읽었다.

**연표가 선 이유는 회고 글이 연도를 촘촘히 적기 때문이다:**

| 시기 | 무엇 |
|---|---|
| **2013~2017** | Django 하나로 프런트도 백엔드도. **단일 PostgreSQL** ⚠️ **이때 공짜였던 것이 나중에 전부 비용이 된다**(내부 호출 지연 없음·하위 호환 걱정 없음·단일 질의 집계) |
| **2017~2019** | 금이 가고 위기가 왔다. 무관한 경로의 **연쇄 테스트 실패** · 롤백 비용 상승 · **2018년 신뢰성 위기로 코드 프리즈** |
| **2019~** | **신규 기능 개발을 전면 중단**하고 Kotlin·gRPC 로 재설계. 16개월 뒤 **마이크로서비스 약 50개, 그중 20개가 크리티컬 패스** |

건진 것 셋:

- ⚠️ **가장 아픈 문장은 2018년 대응에 있다** — 신뢰성 문제를 영역별로 고치기는 했는데 **`왜 아키텍처가 그렇게 취약한지에 대한 분석은 포함되지 않았다`.** **증상을 고치고 원인을 안 물은 한 해가 통째로 지나갔다**(재구성).
- ⚠️ **가장 어려웠던 것이 기술이 아니라 사람이었다** — 오래 만든 것에 애착이 생기는 **IKEA 효과**를 회사가 이름으로 부르며 `disagree and commit` 으로 넘어갔고 **새 피가 필요했다**고 적는다.
- ⚠️ **안티패턴을 그대로 적는다** — 하류 API 가 없을 때 **데이터베이스를 직접 봤고**, 그것이 접근 패턴을 흐려 **테이블 이전을 더 어렵게 만들었다.** 게다가 **코로나로 주문량이 급증하는 중**이었다.

**Flux(에이전트 플랫폼)도 전문을 읽었다** — Firecracker microVM 샌드박스(**p95 5초 미만**) · Agent Gateway(플레이북이 선언한 범위만 부여, 모든 행동 로그) · 플레이북(**단일 YAML, 에이전트 단계와 결정론적 단계를 섞는다**) · 호출 표면 다섯.

### 다음 사이클 — 확장

**두 도메인 다 자료가 익었다.** 마이크로서비스 쪽은 **연표까지 섰고**, Flux 쪽은 **버린 대안이 명시적이다**(호스팅 에이전트는 `민감한 코드를 3자에 보내거나 3자가 내부로 들어오는 길을 열거나` 를 강요한다).

⚠️ **세 번째 도메인(주문·배차 이벤트)은 이 회사의 본체인데 tech 가 0 이다.** **그 차례가 오면 기능보다 자료를 먼저 찾는다** — 네 번 연속으로 통한 규칙이다. 찾을 곳: `Iguazu` 이벤트 플랫폼 · 서비스 메시(Linkerd→Envoy) 글. **브라우저로 열어야 한다.**
