...
**없다 — 이번 사이클은 후보 조사였다. 큐를 0/3 에서 1/3 로 채웠다.** 회사 162개.

### 올린 하나 — Metabase

**BI·분석 도구 축**이 비어 있었다. 취약점 사후 기록(2026-08-27)을 본문까지 열어 확인했다 —
자기 코드의 구멍을 **네 겹으로 나눠** 적는다: 스키마가 추가 키를 거부하지 않았고, **자기들이
2025년 11월에 한 인증 리팩터**가 `token`·`password` 만 넘기던 것을 본문 전체로 바꿨으며,
신뢰 못 할 입력의 사용자 ID 를 그대로 조회했고, HoneySQL 의 `raw` 키가 리터럴 SQL 이었다.

### 자기 리팩터가 원인이라고 적는다

**되돌린 기록**의 한 종류다 — 무엇을 바꿔서 무엇이 열렸는지를 시간(2025년 11월)까지 적는다.
영향도 숨기지 않는다: **클라우드 고객의 3% 미만**이 패치 전에 침해됐고 0.58 이상 전 버전이
취약했다. 8월 3일 신고 → **8월 6일 패치** → 8월 27일 공개.

### AI 를 낀 정찰을 기록에 남겼다

초기 공격 요청의 **user-agent 가 LLM 의 가드레일을 우회하려는 문자열**이었다고 적는다.
이 엔진의 자료에서 처음 보는 종류의 관찰이다.

### 보안 공지가 지배하는 블로그에서 한 편을 건졌다

목록만 보면 "보안 공지·행사 글뿐"인데, 그 보안 글 가운데 **'무슨 일이 있었나'** 하나가 진짜
사후 기록이었다. 지난 사이클의 교훈(**제목만으로 접어서도 안 된다**)이 이어진다.

### 접은 하나 — 의료 축은 계속 비어 있다

**Medplum** 은 제품 공지·고객 사례·월간 업데이트가 지배한다. 월간 글이 '서버 범위 구독' 같은
기능을 언급하지만 구현 대가를 판 글은 아니다.

### 다음 사이클 — 신규 (대기의 Metabase)

⚠️ 나머지 블로그는 보안 공지와 제품·행사 글이 지배하므로 **도메인을 억지로 늘리지 않는다.**
