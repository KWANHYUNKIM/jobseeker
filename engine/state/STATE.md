...
**Replicate**(ML 추론 배포) — 도메인 2개 중 **1개에 기능이 들어갔다**(`boot-in-under-a-second`).
남은 빈 도메인 하나: '속도를 위해 바뀐 출력의 값을 치른다'. 큐 1/3.

### 기술 개선을 요금 구조에 넣었다

부팅을 줄인 것을 **청구에서 빼 준다** — '빠르게 부팅하는 파인튠'만 처리 시간을 청구한다.
이것은 회사가 그 시간을 **자기 원가로 인정했다**는 뜻이고, 개선을 마진으로만 삼지 않고
**상품으로 내놓은** 자리다.

### 수치는 내는데 방법을 안 적는다

이 회사 글에서 되풀이되는 공백이다. 2023년 콜드 부트 글은 **"몇 분에서 1초 미만"** 이라는
결과만 있고 **가중치 캐싱인지 베이스 공유인지 메커니즘이 없다.** 2025년 컴파일 캐시 글은
CI/CD 식 구조를 적어 그나마 낫지만, **캐시 유효성 판단도 미스 동작도 무효화 정책도 없다.**
`failure` 도해에 두 글이 함께 비운 칸으로 세웠다.

### 첫 번째 사용자는 언제나 느리다

캐시도 라우팅도 **이미 한 번 한 일**이 있어야 작동한다. 그래서 새 모델·새 버전의 첫 요청은
전부를 치르고, **첫 실행 로딩 약 2.5초는 남는다.**

### ⚠️ 사고 원인이 잡혔다 — `ast.py`

기능 스크립트를 실행하자 **앞서 완주한 astral.json 이 또 0개짜리로 덮어써졌다.** 검증이
`index.json` 불일치로 잡았고 커밋 전이라 되돌렸다. 원인은 스크래치패드에 둔 파일 이름
**`ast.py`** 였다 — 스크립트를 실행하면 그 디렉터리가 `sys.path` 맨 앞에 들어가서,
**파이썬 표준 라이브러리 `ast` 대신 그 파일이 임포트되고 그대로 실행된다.** 그것이 astral
프로파일을 처음부터 다시 쓰는 스크립트였다.

**지난 사이클에 'heredoc 이 깨져서'라고 적은 것은 틀렸다.** 그때도 같은 파일이 원인이었다
(SyntaxError 를 처리하며 표준 모듈을 임포트하는 길에 걸렸다). 파일 이름을
`astral_profile.py` 로 바꾸고 `__pycache__` 를 지웠다. **표준 라이브러리와 같은 이름을
스크래치패드에 두지 않는다.**

### 다음 사이클

남은 도메인 하나 — 양자화로 바뀐 출력을 보상하는 쪽. 대가와 보상책이 선명하지만
**개선 전 수치가 없다.**
