...
**없다 — Turso 를 완주했다.** 도메인 **2** · 기능 **2** · `status: done`. 회사 115개. 큐 2/3.

### 이번 사이클 — `제어를 돌려주는 데이터베이스`

⭐ **한 문장이 이 도메인 전체다** — **`SQLite 가 로컬 디스크의 세상에 맞춰 조율된 동기적 단일 라이터 C 라이브러리라면, 터소는 비동기 I/O 와 다중 버전 동시성 제어를 중심으로 엔진을 다시 짓는다 — 서버리스와 엣지 런타임이 SQLite 에 요구해 왔으나 얻지 못한 것들이다.`**

⚠️ **이 회사가 파는 것이 정확히 엣지 데이터베이스다** — **자기가 파는 자리에서 원본이 안 맞았다**(재구성).

- ⭐ **동기 API 의 문제는 느린 것이 아니라 막는 것이다**(재구성) — **`데이터가 곧바로 소비할 수 있게 준비되지 않으면 호출자에게 돌아갈 수 있게`** 주 진입점을 확장했다(리눅스는 `io_uring`).
- ⭐ **바꿀 것과 못 바꿀 것을 먼저 갈랐다** — **같은 SQL 방언·같은 디스크 파일 형식·같은 C API 는 묶어 뒀다.** ⚠️ **구조를 자유롭게 바꾸려고 다시 쓰는 건데 이 셋은 영원한 제약이 된다.** ⭐ **얻는 것은 재작성의 가장 큰 위험(아무도 안 쓴다)을 없애는 것이다**(재구성).
- ⚠️ **단일 라이터는 MVCC 로 풀려 하는데 아직 개발 중이고, 전체가 아직 베타다** — **`프로덕션 배포에 앞서 SQLite 수준의 신뢰성을 확보하는 데 무게를 둔다.`** ⭐ **호환을 내걸었으므로 신뢰성 기준도 원본이 정한다.**

### ⭐⭐ Turso 완주 관찰 — 두 도메인이 원인과 감당이다

**`테스트를 물려받을 수 없다`가 재작성을 불렀고**, 재작성이 **비동기와 메모리 안전**을 가능하게 했으며, 그 **긴 시간을 견디는 방법이 결정론적 시뮬레이션 테스트**다(재구성).

⭐ **한 바퀴가 닫힌다** — **검증 능력이 없어서 다시 썼고, 다시 쓰는 위험을 검증 능력으로 감당한다.**

⚠️ **그리고 비동기와 MVCC 가 늘린 사건 순서의 경우의 수를 시뮬레이터가 받는다** — **`서로 다른 사건 순서로 수년치 실행`.**

### 다음 사이클 — 신규 (6순위)

**큐 2/3.** ⏳ 대기 — **Materialize**(⭐ 사전 압축 981.0 → 428.4 GiB · OOM 루프 위험) · **Zed**(⭐ Cursor 와 정면 대비).

⏳ **Turso 보강 거리** — SQLite 암호화 연작 3편 · `Turso v0.7.0` 릴리스 노트(동시 쓰기·멀티테넌시) · **`데이터베이스의 LLVM`의 뜻**(글이 404) · **축소(shrinking)를 하는지.** ⚠️ **옛 글은 `Limbo` 로 검색해야 나온다**(이름이 바뀌었다).
