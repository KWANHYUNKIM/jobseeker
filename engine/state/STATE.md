...
**없다 — 문체 보수 사이클이었다.** Duolingo 를 배민 문체로 다시 썼다(2.24 → 0.0). 회사 115개, 큐 2/3. 35곳 남았다.

### 사내 에이전트 플랫폼이 여섯이 됐고 막으려는 것이 다 다르다
...
- Sentry — 품질(매일 밤 심문)
- Block — 사람이 중계기가 되는 것
- DoorDash — 에이전트가 설 자리 자체(프리미티브)
- Ramp — 신원(시킨 사람의 토큰을 빌린다)
- Duolingo — **채택**

앞의 다섯이 만든 다음에 생기는 문제를 말한다면 **여기는 만들어도 안 쓰이는 문제를 말한다.**
세 번의 실패가 전부 기술이 아니라 거기서 났다. 그리고 편의를 끝까지 밀자 앞 회사들의 문제가
뒤늦게 따라온다 — 아무나 @멘션하는 봇이 200개 도구에 닿으면 권한 우회로가 된다. 답은
Razorpay·Ramp 와 같은 방향이었다.

### 두 힘이 반대로 당기는 모양을 두 번째로 봤다

첫 기능을 Etsy 와 붙였다. 저쪽은 의미적 관련성을 올렸더니 참여 지표가 떨어졌고, 여기는 밀어내면
배우지만 떠나면 구독이 끝난다. **가르치는 힘과 붙잡는 힘이 같은 손잡이에 달려 있다.**
다만 이 회사는 얼마나 미는 것이 맞는지의 기준을 적지 않고, 더 배웠다는 주장에 수치가 없다.

### 에이전트를 감시하는 에이전트가 둘이 됐는데 보는 이유가 다르다

- Sentry 는 **결론이 틀릴까 봐** 본다(도구가 다 성공해도 결론은 틀릴 수 있다).
- Duolingo 는 **사람이 다 못 읽을까 봐** 본다(파이프라인이 리뷰보다 빨리 PR 을 만든다).

병목이 사라진 것이 아니라 옮겨 갔을 뿐이라는 것을 회사가 그대로 적는다. 사고가 난 뒤에 규칙을
세우는 순서(테스트용 에이전트가 프로덕션 소스를 고쳤다)도 Monzo·Wise 와 같다.

### 도메인을 제품이 아니라 무엇을 자동으로 고르게 했느냐로 잘랐다

학습자에게 보여 줄 것을 모델이 고르고, 엔지니어가 쓸 도구를 플랫폼이 고르고, 테스트를 파이프라인이
쓴다. 셋 다 사람이 하던 선택을 기계로 옮긴 자리이고, **셋 다 그 뒤에 생긴 문제가 본론이다.**

### 다음 사이클 — 문체 보수 (사다리 3순위)

--gaps 가 정한다. 지금 맨 위는 anthropic(2.22, 272개)이지만 그건 이 시점의 예상이지 확정이 아니다.
