...
**Hatena — 기능 둘을 채웠다.** 빈 도메인 하나(`25년 된 Perl 을 계속 굴린다`). 회사 150개,
**큐 0/3**.

### 범용 벤치가 무엇을 말해 준 것인가

이번 기능(`measure-then-choose`)의 축이다. 같은 CPU 가 범용 잣대(DKbench)로는 **1.03** 인데
운영 코드로는 **0.91** 이라 값 대비 성능 순위가 통째로 뒤집힌다 — 범용 기준 1위는 c8g, 실제
기준 1위는 c8a(1.38)다. 그래서 운영 코드 11가지로 자체 마이크로벤치를 만들어 함께 돌린다.

### 예측할 수 없는 것은 고르지 않는다

Fargate 는 평균이 나쁘지 않은데 **최저 성능이 평균의 75%**까지 내려가고 0.5 vCPU 이하에서는
가장 오래된 세대만 나온다. 뽑기를 빼는 대신 확장 꼬리 시간을 문다 — 50개 태스크에 **88.5초 대
72.2초**. 자체 관리 EC2 는 고르지 않고 **분기점만 적었다**(약 25대, 운영 부담 월 1,000달러 가정).

### 벤더가 말하는 향상은 내 향상이 아니다

Aurora 글이 그걸 수치로 보인다. AWS 이론값은 합쳐서 **약 182%**인데 실측은 **무거운 질의
20~40% 단축**이다. 다만 세대를 올리며 크기를 3분의 2로 줄이니 예약 인스턴스 시간당 **4.178달러
→ 3.357달러** — 빨라지면서 싸졌다.

### 잰 결과를 어디까지 쓸지도 적는다

**"Perl 단일 코어를 잰 것이라 Go·멀티스레드·I/O 중심 부하에는 옮길 수 없다"**고 못 박는다.
그리고 "CPU 에 기대는 처리의 최적화가 아직 부족하다"고 스스로 적는다.

### 다음 사이클 — 확장 (마지막 빈 도메인)

`25년 된 Perl 을 계속 굴린다`. 키워드 30만 개로 만든 **4MB 짜리 정규식**이 Perl 5.42.0 에서
OOM 을 냈고 원인은 5.29.9 의 변경이었다. **못 고쳐서 기능을 일단 접었다가** 나중에 고친 기록이라
대가가 분명하다.
