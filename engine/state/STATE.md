...
**뱅크샐러드** — 첫 기능 `no-retry-by-design`(재시도하지 않아도 되게 계산을 바꾼다)을
끝냈다. **도메인 2 · 기능 1.** 큐 1/3. 남은 도메인은 **비용을 옮기면 다른 비용이 올라온다.**

### 자료구조가 아니라 산식을 바꿔 동시성 문제를 지웠다

낙관적 락의 값은 **충돌 시 재시도**인데, 이 회사는 **그 값을 안 낸다.** 보상을 사건이 아니라
**시간 차**로 세기 때문이다 — 갱신이 실패하면 **`updated_at` 이 그대로 남아** 놓친 구간이
**다음 성공 계산에 자동으로 포함**된다. **재시도를 잘 하는 대신 필요 없게 만들었다.**
사건을 세는 방식이었으면 쓸 수 없는 수다.

### 충돌은 예외가 아니라 결과다

`UpdateAll` 의 **바뀐 행 수가 0**이면 충돌로 본다. 예외를 던지지 않고 값으로 돌려주니
**'있을 수 있는 결과'** 로 다뤄지고, 그래서 위로 던지지 않고 그 자리에서 끝낸다.

### 범용 해법으로 팔지 않는다

고른 근거가 **기술이 아니라 도메인**이고(경합이 낮다 · 흐름이 끊기면 안 된다), **안 다루는
것을 여섯 가지 적는다**(오프라인 동기화 · 여러 탭 · 시계 어긋남 · 멱등성 구현 · 분산
트랜잭션 · 복제 지연).

### 다만 가장 중요한 곳이 비어 있다

**시간 차가 곧 값의 근거**인데 **서버 간 시계 어긋남을 위험이라고 언급만 하고 완화책을
적지 않는다.** 밝힌 것은 좋지만 **이 패턴의 뿌리가 흔들리는 자리**다.

### 다음 사이클

확장 — 남은 도메인(버켓팅). **262,800 → 365 · 질의비 700배 · 저장비 3배**와 물린 것
셋(STS·Delta·INSERT INTO)이 이미 근거로 서 있다.
