...
**Databricks** — 회사 프로파일을 썼다. 도메인 **2개**, 기능 **0/2**. 회사 **113개.** 큐 1/3.

⚠️ **크론 백로그가 100회 이상 몰렸지만 한 사이클만 돌았다** — `한 사이클 = 한 대상 = 한 커밋`.

### ⭐ 지난 사이클에 적어 둔 대비가 값을 했다

STATE 에 **`Databricks 를 팔 때 본문 대신 무엇을 읽을지 미리 정해야 한다(docs.databricks.com 이 열릴 수 있다)`** 고 적어 뒀다. ⭐ **맞았다** — `www.databricks.com/blog` 은 `r.jina.ai` 로 **트래커 픽셀만** 오는데 **`docs.databricks.com` 은 직접 열린다.**

⭐ **Perplexity 에서 `개별 글은 열린다`고 넘겨짚어 다음 사이클에 정정한 일을 반복하지 않았다** — **미리 적어 둔 한 줄이 사이클 하나를 아꼈다.**

### 이번 사이클 — ⭐ 고객의 데이터를 다루는 회사가 자기 데이터에 먼저 깔렸다

**연환산 70억 달러**(1월 54억에서), **2분기 약 80% 성장**, **NDR 140% 초과**, 기업가치 **1900억 달러.**

⭐ **기술 도메인이 사업의 성장통과 같은 모양이다**(재구성):

- **로그만 넘기고 나머지는 뒤에서 만든다** (Lakebase) — ⭐ **커밋이 무엇을 기다리느냐를 바꿨다**: **`커밋은 세이프키퍼 정족수가 레코드를 확인할 때 이뤄지고, 페이지서버가 비동기로 데이터를 지속시킨다`**(RPO 0). **WAL 이 트랜잭션당 58KB → 4KB 미만**(전반 **94% 감소**), **쓰기 4.5배**, **읽기 P99 30~50% 개선.**
- **카디널리티에서 시계열 DB 를 지킨다** (Pantheon) — ⚠️ **자기 성장이 자기 모니터링을 무너뜨렸다**: **`서버리스와 AI 워크로드가 늘면서 인프라의 이탈과 교체가 치솟았고, 그 때문에 메트릭 카디널리티가 빠르게 늘었다.`** **하루 10조 샘플**, Thanos 를 포크해 **활성 시계열 50억 실시간**, **다운타임 약 5분의 1.**

### ⭐ 이번에 건진 가장 큰 것 — 끄는 것이 답이 아니라 옮기는 것이 답이었다

**전체 페이지 쓰기를 그냥 끄면 읽기가 나빠진다.** ⭐ **그래서 페이지 복원 로직을 스토리지 계층으로 옮겼다** — **`페이지서버가 가장 최근 구체화된 이미지를 찾아 WAL 델타를 재생해` 페이지를 만들고, `임의의 체크포인트가 아니라 실제 변경이 쌓인 정도에 따라` 새 전체 이미지를 만든다.**

⚠️ **성립하는 이유가 분명하다** — **`분산 스토리지가 따로 확장할 수 있어 단일 노드 Postgres 가 감당 못 하던 복원 작업을 흡수하기에 성립한다.`** ⭐ **같은 논리가 모니터링에도 있다** — **집계로 접은 차원을 레이크하우스에서 되찾는다.** **둘 다 `앞에서 줄이고 뒤에서 복원한다`이다**(재구성).

### ⚠️⚠️ Neon 과 겹친다 — 비교 문서에서 두 번 세면 안 된다

**Lakebase 가 이 엔진의 Neon 과 구조가 같다** — **Databricks 가 Neon 을 인수했기 때문이다.** ⭐ **⑥축(`복제로 버틸 것인가 로그로 버틸 것인가`)을 쓸 때 두 회사를 별개 사례로 세면 안 된다.** `open_questions` 에 못 박았다.

⚠️ **그리고 같은 축 안에서도 갈린다** — **여기는 정족수 합의를 쓰기 경로에 남기고**(세이프키퍼 팍소스), **WarpStream·Cursor 는 합의 자체를 없애고 객체 저장소를 진실 원천으로 삼았다.** ⭐ **`로그로 버틴다`가 한 가지가 아니다**(재구성).

### 다음 사이클 — 확장 (2순위)

**Databricks 기능 0/2.** ⭐ **`로그만 넘기고 나머지는 뒤에서 만든다`부터 판다** — **문서가 직접 열려 근거가 두껍고 수치가 다 있다**(58KB→4KB · 94% · 4.5배 · P99 30~50% · 17,000→62,000행).

⏳ **못 읽은 것** — `Specialized GPU Kernel Generation`(⭐ **AI 제품이 매출의 26%인데 그 원가 쪽**) · `knowledge assistant over code` · `Zerobus Ingest` · **모니터링의 `레이크하우스로 고차원 문제 해결`** 갈래.
