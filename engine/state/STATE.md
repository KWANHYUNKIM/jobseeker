...
**Airbnb** (US · 커머스) — 도메인 3개 · **기능 1개**(`small-enough-to-fix`). 남은 둘은 **본문 미독**이다(데이터 아키텍처 · 신원 그래프).

### 이번 사이클 — `고칠 수 있는 크기인지를 먼저 본다`

⭐ **거절 이유가 기술만이 아니라는 것이 이 기능의 발견이다.**

건진 것 넷:

- ⚠️ **포크는 처음에 싸고 나중에 비싸다** — **`Veneur 를 계속 쓰려면 Prometheus 의 프로토콜과 데이터 모델을 지원하기 위한 상당한 재작성이 필요했고, 그 포크는 지속적인 유지 부담이었다.`** **떠난 이유의 절반이 기술이 아니라 유지의 무게였다**(재구성).
- ⚠️ **목표가 무엇이냐에 따라 같은 도구가 답이 되기도 하고 아니기도 한다** — recording rules 를 거절한 이유: **`질의 성능 개선에는 훌륭하지만, 원시 데이터를 먼저 TSDB 에 저장해야 하므로 우리의 비용 절감 목표를 무너뜨린다.`** **성능을 목표로 삼았다면 골랐을 답이다**(재구성).
- ⚠️ **`남들이 쓴다` 가 근거가 되지 못한 자리** — m3aggregator 는 **`다른 회사들에서 검증됐지만 아키텍처가 우리가 원하는 것보다 복잡해 보였다`**, Vector 는 **`내장 확장 능력이 없고 Rust 로 쓰였는데 Airbnb 에서 널리 쓰이지 않는다`**. **도구의 언어가 선택 기준이 된다 — 고쳐야 할 때 고칠 사람이 있느냐를 본 것이다**(재구성).
- ⚠️ **고른 근거의 마지막 줄이 기능 이름이다** — **`작은 코드베이스(약 1만 줄)라 필요하면 이해하고 고치기 쉽다.`** **`우리가 고칠 수 있는가` 가 도구 선택의 기준으로 명시돼 있다.**

**수치** — **초당 1억 샘플 이상**(수백 개 집계기) · JVM 메트릭 CPU **10% → 1% 미만** · **비용 한 자릿수 규모 감소.**

### ⚠️ 또 하나의 `어느 쪽으로 넘어지나`

**고카디널리티 서비스를 델타 시간성으로 바꾸며** — **`예기치 못한 실패가 데이터에 눈에 보이는 공백을 남긴다 — 메모리 사용 감소와 안정성 향상의 대가로 받아들인 절충이다.`** ⚠️ **누적이면 장애가 이상한 스파이크로, 델타면 빈칸으로 나타난다 — 어느 쪽도 진실은 아니지만 빈칸이 덜 속인다**(재구성). ⭐ **Flipkart(덜 쓰는 쪽) · Razorpay(수집을 멈추는 쪽)와 같은 결이다.**

### ⚠️ 이번 사이클에 내가 한 실수 — 식별자 필드에 강조 기호

**`connections[].to` 에 백틱을 넣어 커밋했다가 검증 경고를 보고 같은 사이클 안에서 고쳤다**(amend). ⚠️ **`to` 는 식별자·대조에 쓰이는 필드라 강조를 넣으면 화면에 새어 나오고 값끼리의 대조가 깨진다.** **이번 세션에서 Paystack 의 `name` 에 백틱을 넣은 것과 같은 부류의 실수이고 두 번째다.** ⚠️ **커밋 전에 `validate.py` 출력의 경고까지 읽는다** — 통과만 보고 넘어갔다.

### ⭐ 같은 도구, 다른 문제 — connections 에 적었다

**Razorpay 와 Airbnb 가 둘 다 vmagent 를 쓰는데 출발점이 다르다** — 한쪽은 **`안 쓰는 메트릭을 찾아 지우는`** 문제였고 한쪽은 **`StatsD 와 자체 포크가 확장 한계에 닿은`** 문제였다. **도구가 같아도 문제가 다르면 쓰는 법이 다르다**(재구성).

### 다음 사이클 — 확장(2순위)

`--gaps` 가 **`여러 제품을 하나의 데이터 구조로 담는다`**(2026-06)를 부를 것이다. ⏳ **본문 미독** — **URL 은 확보해 뒀다.** ⚠️ **`medium.com/airbnb-engineering` 은 목록이 헤더만 오니 개별 글 주소로 바로 간다.**

⚠️ **큐는 1/3**(Pinterest). ⏳ **보강 거리 셋** — Grafana Labs 인용 대조 · Razorpay 보안 트리아지 글 · Flipkart Rate Card 엔진 글.

### ⚠️ 비교 문서 재료 (초안 유지)

**① `AI 에이전트를 어디까지 믿나` 9곳** — Sentry / ClickHouse / Duolingo / Ramp / DoorDash / Deliveroo / Snyk / Cygames / Razorpay.

**② `관리형 MySQL 의 한계` 3곳 + Cygames** — Etsy / Plaid / Paystack.

**③ `자기 성과를 어디까지 주장하나` 6곳** — Snyk / Grafana Labs / ScyllaDB / Razorpay / Flipkart / **Airbnb**(거절한 대안 다섯과 받아들인 대가를 나열).
...
**⑧ `제약을 없애지 못할 때 어디서 갚는가`** — Cygames / Zepto / Grafana Labs.

**⑨ `한 번에 갈아엎을 것인가 목 졸라 죽일 것인가`** — Twilio(두 방향) / Etsy / Plaid / Paystack / ⏳ Airbnb(데이터 아키텍처).

**⑩ ⭐ `관측 비용을 어디까지 줄이나` — 재료가 찼다** — **Razorpay · Airbnb**(사는 쪽, 같은 vmagent 를 다른 이유로) / **Grafana Labs · Honeycomb**(파는 쪽).

**⑪ `깨질 걸 알면서 고른 의존을 어떻게 다루나`** — Razorpay / Plaid / Paystack / ScyllaDB.

**⑫ `빠른 숫자와 정확한 숫자를 어떻게 가르나`** — Flipkart / Zepto / Deliveroo.

**⑬ ⭐ `애매할 때 어느 쪽으로 넘어지나` — 다섯 곳** — Flipkart(둘) / Razorpay / Zepto / Paystack / **Airbnb**(스파이크 대신 빈칸).

**⑭ `검색 관련성을 누가 정하나`** — Etsy / Flipkart / ⏳ Zepto / ⏳ Pinterest.

**⑮ `언제 쪼개고 언제 합치나`** — Twilio / DoorDash / Deliveroo / ScyllaDB / ⏳ Pinterest.

**⑯ `빌려 쓰던 것을 언제 자기 것으로 만드나`** — Airbnb / ScyllaDB / Plaid / Cygames(반대 방향).
