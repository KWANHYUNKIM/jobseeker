...
**Perplexity** — 기능 **2/3.** 회사 111개. 큐 0/3. ⚠️ **남은 도메인 하나**(`GPU 끼리 직접 말하게 한다`)를 채우면 완주다.

### 이번 사이클 — `수천 개의 그래프를 미리 만들지 않는다 (ROSE)`

⭐ **한 문장이 이 기능 전체다** — **CUDA 그래프는 켜면 빨라지는데, 미리 다 만들면 서버가 안 뜬다.**

**`CUDA 그래프는 순전파의 모든 커널을 드라이버 호출 한 번으로 띄우는 데 필요한 메타데이터를 담아, 반복되는 파이썬과 파이토치 오버헤드를 없앤다.`** ⭐ **엔진이 파이썬인 것과 이 결정이 짝이다**(재구성) — **파이썬으로 쓰면서 파이썬 오버헤드를 지운다.**

⚠️ **그런데 그래프가 `시퀀스 수 × 토큰 수` 조합마다 필요하다.** 토큰 수를 **64 또는 256 버킷**으로 묶어 줄여도 **`수천 개의 그래프가 나오고 캡처에 몇 분이 걸린다.`** ⭐ **그래서 미리 만들지 않고 `실제 트래픽이 올 때 지연 캡처한다`** — **시작 시간을 운영 시간으로 옮겼다**(재구성). ⚠️ **대가는 아직 안 잡힌 조합의 첫 요청이 캡처 비용을 문다는 것이다.**

건진 것 셋 더:

- ⭐ **패딩이 두 곳에 있고 성격이 다르다**(재구성) — **입력 패딩은 피하고**(들쭉날쭉한 입력을 지원하는 어텐션 커널로 디스패치), **그래프 키에서는 버킷 패딩을 받아들인다.** **하나는 계산 낭비고 하나는 조합 폭발을 막는 값이다.**
- ⭐ **LLM 커널을 재사용하되 그대로는 안 쓴다** — **`KV 캐시를 건너뛰고 들쭉날쭉한 입력을 지원하는 어텐션 커널로 디스패치해 패딩 오버헤드를 피한다.`** **임베딩은 다음 토큰을 만들지 않으므로 KV 캐시가 군더더기다**(재구성).
- ⭐ **결과를 값이 아니라 약속으로 돌려준다** — **`LazyTensor 는 페이지 잠금 메모리의 호스트 버퍼를 cudaMemcpyAsync 이벤트와 함께 추적해, 러스트 비동기 태스크가 앞 배치의 결과를 기다리는 동안 CPU 가 다음 배치를 준비하게 한다.`** ⚠️ **파이썬·러스트 경계를 넘는 비동기라 디버깅이 어려워진다**(재구성).

**결과** — **`BGE-M3 와 pplx-embed-1-0.6b 에서 p50 과 p99 모두 기준선(vLLM v0.22.0)을 이긴다.`**

### ⭐ 두 기능이 같은 문장의 앞뒤다

**Ivy·Tulip 은 `GPU 를 놀리지 않으려고` 앞을 러스트로 옮겼고, ROSE 는 `파이썬을 쓰면서 파이썬 값을 안 치르려고` 그래프를 쓴다**(재구성). ⭐ **둘 다 같은 압력에서 나온다 — 사용량으로 파니 GPU 이용률이 곧 마진이다.**

### 다음 사이클 — 확장, 그리고 완주 (2순위)

**Perplexity 기능 2/3.** ⏳ 마지막 — **`GPU 끼리 직접 말하게 한다`**(fabric-lib · arXiv 2510.27656 · MLSys 2026). ⭐ **arXiv 본문이 직접 열리므로 이 회사에서 유일하게 1차 자료로 쓸 수 있는 도메인이다.**

⚠️ **완주 판정 때 수익원 셋**(구독 · 기업 좌석·광고·커머스 · 개발자 API)**이 기능과 이어지는지 본다** — Block 에서 그것 때문에 사이클이 하나 더 들었다.

⚠️ **큐가 0/3 이다** — 완주 직후 **후보 조사**를 반드시 끼운다. ⏳ Anthropic · Databricks · Snowflake · Anduril.
