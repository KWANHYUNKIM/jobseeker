...
**freee(JP · SaaS) `in_progress` — 도메인 2개 · 기능 3개.** 341 에서 **새 도메인**
`멈출 수 없는 데이터베이스를 돌아가는 채로 바꾼다` 를 열고 그 안에
**`swap-the-floor-before-the-peak`(정점이 오기 전에 바닥을 갈아 끼운다)** 를 썼다.
**결정 7개 · 그림 3장 · 생각 4개 · 난제 2건 · 연결 1건.**

**⑭ `--gaps` 는 [신규 = SmartHR]** 을 가리켰다(큐 1/3, 열린 도메인이 다 차서 사다리가
아래로 내려갔다). **회사 완주 규칙이 위라 freee 를 계속 팠다**(CLAUDE.md).

**재료와 판정** — `確定申告を乗り越えるDBパフォーマンス改善 - Aurora 移行の舞台裏`
(2021-04-07, `/entry/migrating-mysql-to-aurora`). **3/3** — 버린 대안(Spanner·TiDB) ·
대가(되돌리기 준비가 *"とにかく大変"*) · 수치(2020-11 승인 → 2021-01 완료, 되돌릴 수
있는 상태 2~3주).

**⚠️ 223 규칙으로 갈랐다** — 기존 도메인은 **여러 제품이 함께 기대던 것을 떼어내는**
이야기인데, 이 글은 **회계 DB 의 바닥 자체를 정해진 기한 전에 갈아 끼우는** 이야기다.
**다른 문제라 새 도메인을 열었고 이름을 넓게 지었다**(334→335 의 교훈).

**✅ 새 도메인도 두 편 규칙을 지켰다** — 두 번째로 `RDS Proxyを用いたオンライン
スイッチオーバー`(2023-08-01, `/entry/online-rdbms-switchover-with-rds-proxy`)를 읽어
도메인 `tech` 세 번째 항목을 채웠다. **기능은 첫 편으로 하나만 썼다.**

**341 이 새로 낸 것**
- **⭐ 기술의 우열이 아니라 달력이 답을 정했다** — NewSQL 이면 쓰기까지 갈라지는데도
  *"MySQL から NewSQL への移行は構成の変更があまりに大きく、確定申告に間に合わせるのは難しい"*
  라 버렸다. **그 대가로 쓰기는 여전히 한 대다.**
- **검증은 본 프로덕션 쿼리 재생** — *"ワークロードも非常に多様"* 라 인공 시나리오를 버렸다.
  Percona pt-upgrade 를 고쳐 썼고 뒤에 *"独自にツールを開発した方が柔軟性が高かった"* 라 적는다.
- **⭐ 검증에서 몇 배 느린 쿼리가 나왔는데 원인이 검증 환경이었다**(테스트 인스턴스가 작아
  buffer pool 부족). **나쁜 숫자가 제품 탓인지 환경 탓인지를 가르는 일이 남는다**(이 사이트의 정리).
- **되돌릴 길을 직접 만들었다** — Aurora 에 마스터 승격 기능이 없어 **양방향 레플리케이션 +
  쓰기 가능 마스터를 하나로 제한**. *"切り戻すための仕組みや手順の整備がとにかく大変"*.
- **되돌릴 수 있는 창은 2~3주** — *"MySQL に戻せる状態を2〜3週間維持して、無事に年末年始も超えられた"*.
- **⭐⭐ 같은 기준이 3년을 건너 반복된다** — 2023년에는 Blue/Green 이 있었는데도
  *"切り戻し手法を確立できないため、導入を見送りました"*. **되돌릴 길이 서지 않으면 안 넘긴다.**
- **⭐ 무중단이라는 말의 실제 뜻** — Route 53 CNAME 은 *"最大数分間"*, RDS Proxy 로 줄여도
  *"経験的に10秒程度"* 이고 **실행 중이던 트랜잭션은 끊긴다.** **줄지만 영이 되지는 않는다.**
- **⭐⭐ 두 도메인이 같은 아픔의 두 층이다**(`connections` 에 적었다) — 바닥을 갈아 끼우는 데
  **석 달**, 위에 얽힌 것을 떼어내는 데 **1,454일**. **아래를 통째로 바꾸는 일보다 위에 얽힌
  것을 푸는 일이 오래 걸렸다.**
- **⚠️ 추정 하나를 명시로 남겼다**(`open_questions`) — 340 이 말한 "먼저 해 봤지만 부족했던
  Aurora 이관"이 이 건일 가능성이 있으나(시기가 겹친다) **회사가 같은 건이라 말한 적은 없다.**

---

**다음 사이클 — freee 를 닫거나 한 축을 더 연다.**
- **1순위 — 새 도메인의 두 번째 기능.** 제목만 확인한 다섯 편이 있다:
  `pt-online-schema-changeの導入時に検討したこと` · `複数の検証環境でのDB相乗り化` ·
  `freee人事労務の給与計算ロジックにLocal Write Forwardingを導入した話` ·
  `安全なデータベース削除オペレーションの自動化事例` ·
  `Aurora 3.04.2 での DDL の予期しない挙動と Rails での対策`. **⑰ 주소를 추측하지 않는다.**
...
- **4순위 — 닫는다.** 그때 `business_model` 에 적을 관찰(이 사이트의 정리라고 밝힌다):
  **⭐ 후회를 적고 되먹임 고리까지 짚는다** — *"完了までに時間がかかったことを踏まえるとこれを良しとするのはよくありません"* ·
  *"リリース回数が少ないために振り返りの頻度も多くなく、改善施策が出しづらい状況になった"*.
  **⭐⭐ 같은 프로젝트를 두 글이 다른 각도로 다룬다**(왜 4년이 걸렸나 / 어떻게 갈아 끼웠나) —
  **둘을 겹쳐야 4년의 모양이 보인다.** **⭐ 되돌릴 길이 서지 않으면 안 넘긴다는 기준이
  2020년과 2023년에 똑같이 나온다.** **⚠️ 최근 글의 결은 AI 쪽으로 크게 기울어 있고
  이런 굵은 회고는 2022년 언저리에 몰려 있다**(추정).
...
**분포** — 57곳(일본 여섯 · 인도 셋) · 비교 문서 32편 · **운영 사실 27개.**
...
다르면 다른 기능이다**(339→340) · **⭐ 새로: 같은 회사의 같은 아픔이라도 층이 다르면 다른
도메인이다**(341).
