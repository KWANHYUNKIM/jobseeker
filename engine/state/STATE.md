...
**없다 — 카카오뱅크를 닫았다.** 도메인 2 · 기능 2 · `status: done`.
`sequence-fds`(결정 8 · 논문 2) · `collateral-review`(결정 6) · 그림 6.
**완료 63곳 · ⚠️ 큐 대기 0/3 · 비교 문서 33편.**

### ⭐ 다음 사이클 — 사다리 3순위 **후보 조사**

**큐가 0 이고 파는 중인 회사가 없다.** PROMPT 사다리에서 **큐가 0 이면 후보 조사가 3순위**이고
지금 위쪽(보수·확장)이 비었으므로 **곧바로 여기다.** 목표는 `## 대기` 표에 **3곳**.
- **⚠️ 지난 세션에서 WebSearch 예산이 막힌 적이 있다 — 새 세션이면 먼저 시도한다.**
- **⚠️ 요약만 보고 판정하면 과대평가가 난다**(354) — **'무엇이 없는지' 를 함께 묻는다**(352).
- **⚠️ 글 본문까지 열어** 버린 대안·대가·수치가 실제로 있는지 본다. **결정은 있는데 대가가 없는
  회사는 떨어뜨린다**(Monzo·GetYourGuide 를 그렇게 뺐다).
- **이미 판 축과 겹치지 않는 곳**을 고른다. 최근 다섯 회사가 커머스(ZOZO)·모빌리티(쏘카)·
  핀테크(카카오뱅크)였다. **게임은 Roblox 하나뿐이고, 물류·의료·교육·산업 쪽이 얇다.**

### ⭐ 그 다음 후보 — 비교 문서 ② 를 쓸 수 있다 (세 회사가 모였다)

**"정확도를 못 재는 것을 어떻게 믿는가"** — 대리 지표로 옮겨 간 세 사례가 갖춰졌다.
- **카카오뱅크 FDS** — precision·recall 없이 **t-SNE 군집**과 사각지대 축소로 대신한다.
- **ZOZO 애매한 검색** — **"nDCG 같은 정량 지표가 아니라 목시 평가"**.
- **쏘카 예약 테트리스** — **가상 예약**이라는 대리 지표. 회사가 **"가장 어려운 부분"** 이라고 적었다.

**공통점**: 셋 다 **정답 집합을 만들 수 없어** 대리 지표로 옮겼고, **그 대리 지표가 무엇을 좋다고
볼지를 정한다.** **서로 답하지 않은 질문**: 대리 지표가 실제 목표와 어긋나기 시작한 것을
무엇으로 아는가 — 세 회사 다 답이 없다. ⚠️ **새 주장을 만들지 말고 각 회사 페이지의
`confirmed` 만 엮는다.**

**비교 문서 ①** "내가 못 고치는 것이 조용히 바뀐다"(ZOZO·쏘카)도 재료가 있으나 **두 곳뿐**이다.

### 이번 사이클에서 배운 것

- **⭐ 비동기로 옮기면 관측이 함께 사라진다.** 카카오뱅크가 그 값을 먼저 적었다 —
  **"작업 수행 현황 파악, 오류 추적, 운영 조치가 복잡해질 수 있는 단점도 존재"** 해서
  **상태 테이블과 전용 대시보드를 함께 설계**했다. **비동기 전환을 다루는 기능에서는
  '무엇을 잃고 무엇으로 되샀는가' 가 결정 하나가 된다.**
- **재시도는 기다림을 없애지 못하고 뒤로 미룬다.** 지연을 키우는 재시도(+5분·+30분)는
  상대를 보호하는 장치이지 속도 장치가 아니다.
- **오류 기록의 트랜잭션 경계도 결정이다.** `REQUIRES_NEW` 로 나누면 롤백돼도 이유가 남지만
  **업무 상태와 기록을 한 번에 못 본다.**
- **두 사이클 미룬 자리는 STATE 에 '먼저 원문을 연다' 로 적어 두면 실제로 지켜진다.**
  큐 메모의 내용을 원문 확인 전에는 옮기지 않았고(341·342), 이번에 열어 보니 메모보다
  훨씬 두꺼웠다(재시도 정책·트랜잭션 경계·상태 4종·생산성 50%).

### 카카오뱅크 자료 접근 — 재방문 사이클을 위해 남긴다

- **`tech.kakaobank.com` 은 WebFetch 로 본문까지 열린다.** 목록은 **페이지네이션이 있어 한 번에
  4건만** 보인다 — 글 주소를 직접 열거나 검색으로 찾는다.
- **⚠️ 실적을 언론 보도에서 옮겼다**(아시아경제 2026-02-04). IR 원문과 대조하지 않았다.
- **⚠️ FDS 정확도 지표가 없다**(t-SNE 군집으로 대신). **"약 30%를 … 모니터링" 이 무엇의 30%인지
  못 읽었다.** **SUSPENDED 와 ERROR 를 가르는 조건도 글에 없다.** 셋 다 `open_questions` 에 있다.
- **`eras` 는 세우지 않았다.** FDS 가 **포인트 기반 → 시퀀스 기반** 세대 교체를 분명히 말하지만
  **앞 세대가 언제부터였는지가 없다.** 다른 글에서 잡히면 그때 세운다.
