...
**Depot — 회사 프로파일까지 썼다(in_progress).** 회사 130개, 큐 2/3.

도메인 셋을 세웠고 기능은 아직 0개다.

- **마이크로VM 을 초 단위로 띄운다** — `Why I reimplemented LVM (with worse guarantees)`(2026-08-13)
- **이미지를 다 받지 않고 시작한다** — `Why SOCI belongs in the build`(2026-08-04)
- **시간을 팔면서 시간을 줄인다** — 요금 페이지(분과 초로 매긴다)

### 과금 단위가 이상한 자리를 만든다

파는 단위가 분과 초다(도커 빌드 분당 0.04달러, 자체 CI 는 vCPU 당 초당 0.00005달러). 그러면
**빨라질수록 같은 일의 매출이 준다.** 그런데도 빠르게 만드는 이유는 비교 대상이 이미 쓰고 있는
남의 CI 라서, 옮겨 올 이유를 속도로 만들어야 하기 때문이다(재구성).

이 엔진에서 시간을 파는 회사가 여럿인데(Modal 은 유휴에 안 받고, Materialize 는 뷰를 유지하는
시간을 받는다) 여기는 **대체재를 파는 자리**라 축이 다르다.

### 요구가 표준 도구의 한계를 드러낸다

마이크로VM 을 1초 안에, 초당 200건까지 띄워야 한다. 그런데 LVM 은 볼륨 그룹 전체에 잠금을 걸어
연산을 직렬화하고 연산당 약 100밀리초를 먹는다 — 그 요구에서는 쓸 수 없는 도구가 된다.

### 버린 보장을 이름 대며 적는 회사다

크래시 안전성·삭제 복구·메타데이터 보존을 버렸다고 명시하고 근거를 댄다 — 하이퍼바이저가 죽으면
어차피 휘발성 VM 이 사라진다. 거절한 대안도 이름을 댄다(ZFS·Btrfs·Stratis·루프 장치). **무엇을 안
지키기로 했는지가 이렇게 분명한 글이 이 엔진에 드물다**(재구성).

### 다음 사이클 — 확장 (Depot 의 빈 도메인)

LVM 재구현 글이 가장 두껍다. 커널 device-mapper 데이터면은 그대로 두고 제어면만 바꿔 약 100배를
얻은 이야기다.
