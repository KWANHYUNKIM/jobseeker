...
**없다 — 문체 보수 사이클이었다.** 그라파나 랩스를 배민 문체로 다시 썼다(0.87 → 0.0). 회사 115개, 큐 2/3. 5곳 남았다.

### 변환기에 조사 붙이기를 넣었다

앞 사이클(윅스)에서 손으로 하던 정리를 변환기에 넣었더니 이번에는 손볼 조사가 하나도 안 남았다.
**같은 손질을 열여덟 번 하고 나서야 도구에 넣은 셈이다.**

### 자기 흠을 적는 회사들 중 여기는 안 적을 수가 없다

ClickHouse(고치는 대신 무엇을 고쳤는지) · Honeycomb(나빠진 구간) · DoorDash(IKEA 효과)에 이어,
그라파나 랩스는 **코드가 열려 있어 아키텍처를 바꾸는 과정 자체를 숨길 수 없다.** 그래서 자기가
골랐던 설계를 이름 붙여 부정하는 문장이 나온다(복제 계수 3은 비싸다).

그리고 남의 규모까지 감당해야 한다는 제약이 선택을 좁힌다 — **모르는 사람의 온프렘 클러스터에서도
도는 구조라야 한다.**

### 벽이 읽는 양이 아니라 읽는 손의 개수다

Datadog 은 100조 개를 안 읽는 방법을 질의 계층에 넣었고 Honeycomb 은 아무것도 미리 정하지 않고 다
훑는데, 여기의 벽은 **PromQL 엔진이 단일 스레드**라는 것이다. 데이터를 아무리 잘 나눠 둬도 질의가
한 코어에서 돈다 — **문제가 데이터의 배치가 아니라 질의의 배치였다.**

### 색인을 안 하기로 한 값을 읽을 때 치른다

Honeycomb 과 정확히 같은 자리인데, 이 회사는 그 값을 수치로 적는다 — **청크의 최대 97%가 로그
본문이고 질의에 쓸 수 있는 것은 3%뿐이다.** 그 위에 병목이 셋 더 있었다는 것까지 함께 적는다.

### 다음 사이클 — 문체 보수 (사다리 3순위)

--gaps 가 정한다. 지금 맨 위는 monday(0.86)지만 그건 이 시점의 예상이지 확정이 아니다.
