...
**없다 — 일흔다섯 번째 후보 조사를 돌았고 통과가 0곳이다.** 큐 0/3 이라 다음 사이클도
**후보 조사**다.

### 도구 조합을 하나 얻었다

**WebFetch 가 403/404 를 주는 곳도 `curl` + 텍스트 추출로 본문을 읽을 수 있다.**
소스그래프가 그랬다 — WebFetch 는 403 인데 curl 로는 본문이 그대로 나와서 **글 둘을 열고
판정**했다. **medium 사각지대(mixi·리디·루닛)에도 통할지 다음에 시험한다.**

### 막힌 곳 다시 두드리기는 절반만 값을 했다

앞 사이클에서 엠쓰리를 그렇게 찾았는데, 이번에는 넷을 두드려 **주소는 다 열렸지만 내용이
아니었다** — 루닛은 **다운로드 센터**(브로슈어·임상 자료), dbt Labs 는 **회사 소식과
합병 공지**, incident.io 는 **curl 200 / WebFetch 404** 로 아직 못 읽었다. **주소가 열리는
것과 자료가 있는 것은 다르다.**

### 잘 짜인 글이 곧 우리 자료는 아니다

소스그래프의 **90개 요구사항·10개 범주·3년 비용 모델**은 구조가 훌륭하다. 그런데
**'우리가 이렇게 만들었다'가 아니라 '당신이 만들면 이만큼 든다'** 다 — **build-vs-buy
영업 논거**이지 자기 결정의 기록이 아니다. **판정 기준을 넓히지 않았다.**

### 회사 단위로 팔 수 없는 자료가 또 나왔다

**허깅페이스**는 블로그가 **커뮤니티 기고**로 채워져 있다 — **데보션(SKT)과 같은 이유**로
이 엔진의 단위(한 회사)와 안 맞는다. 이런 사례가 둘이 됐으니 **후보를 볼 때 '주인이
하나인가'를 먼저 보는 것**이 값을 한다.

### 다음 사이클

**후보 조사**(큐 0/3). 하나도 못 채운 것을 채운 척하지 않는다.
