...
**Bun — 도메인 셋을 다 채우고 done 으로 닫았다.** `bun-install` · `zig-to-rust` · `v8-api-shim`.
회사 124개, 큐 2/3.
...
호환을 다룬 회사가 이 엔진에 여럿인데 여기가 가장 깊다. Dolt 는 와이어 프로토콜을 흉내 냈고
QuestDB 는 자기 것을 만들었으며 DuckDB 는 표준을 물렸다. Bun 은 **남의 엔진 내부 메모리 배치**를
맞춘다 — 객체 앞머리의 맵 포인터, 오프셋 12의 instance type, 태그드 포인터와 NaN 박싱 사이의 번역.

그럴 수밖에 없는 이유가 분명하다. 엔진 중립 인터페이스(Node-API)가 있는데도 인기 네이티브 모듈이
V8 API 를 직접 쓰고, **V8 의 인라인 함수가 그 배치를 직접 읽기 때문에** 함수만 흉내 내는 길이
애초에 없다. 첫 시도(JSValue 를 그대로 재해석)가 잠깐 되다가 무너진 것이 그 증거다.

남은 손잡이는 하나다 — instance_type 을 찾을 만큼만 세워 두고 **통제할 수 있는 느린 경로로
흐름을 끌어오는 것.** 빠른 경로를 내주고 개입 지점을 사는 교환이다.

### 세 도메인이 원인과 결과로 이어진다

시스템 층까지 내려가니 메모리 관리가 손에 들어왔고(1→2), 그 손이 틀리는 것을 컴파일러에 맡겼다.
그리고 호환을 겨냥해 쌓아 둔 언어 중립 테스트가 있었기에 언어를 통째로 바꾸고도 같은 물건인지
물을 수 있었다(3→2). **호환이 만든 자산이 이전을 가능하게 한 셈이다.**

### 다음 사이클 — 신규 (Tigris Data)

큐가 2/3 이고 대기 맨 위가 Tigris Data 다. FoundationDB 위의 다중 리전 객체 저장소라 이 엔진의
`객체 저장소를 진실 원천으로` 축의 반대편이 된다.
