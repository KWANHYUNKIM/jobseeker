...
**Meesho** (IN · 커머스) — 도메인 4개 · **기능 1개**(`fast-but-wrong-is-still-wrong`). 남은 셋은 **본문 미독**이다.

### 이번 사이클 — `빠른데 틀린 정산은 여전히 나쁜 정산이다`

건진 것 넷:

- ⚠️ **가장 눈에 띄는 개선(처리 속도)을 첫째로 두지 않았다** — 회사의 문장: **`속도만으로는 충분하지 않다. 빠른데 틀린 정산은 여전히 나쁜 정산이다.`** 그래서 **정정·환수·보상 없이 첫 거래에서 맞았는가**(FTRP)를 지표로 삼아 **95% 이상**으로 올렸다. ⚠️ **그런데 속도도 함께 좋아졌다**(P90 35~40시간 → 약 25시간) — **정확성을 앞에 두는 것이 반드시 느려지는 것은 아니었다**(재구성).
- ⚠️ **같은 총액인데 순서만 바꿔도 다른 제품이 된다** — **모든 회수를 정산 처리 전에** 끝내도록 순서를 바꿨다. **회계로는 어느 쪽이든 같지만 받은 돈이 나중에 줄어드는 경험은 신뢰를 갉는다**(재구성).
- ⚠️ **셀프서브로 붙이는 속도를 얻고 규칙의 일관성을 잃었다** — 내부 팀이 스스로 회수 로직을 정의하게 하자 **예정 회수율이 사실상 0 에서 98% 이상**이 됐는데, **정확성의 책임도 함께 흩어졌다.** 회사가 남은 과제로 **`규칙을 전반에 걸쳐 일관되고 표준화하는 일`** 을 꼽는 자리가 정확히 여기다.
- ⚠️ **판매자의 현금 흐름이 고객의 반품 권리에 묶여 있다** — **D+8 은 기술 제약이 아니라 두 이해관계자 사이에 그은 선**이다(7일 반품 창 + 하루). **회사가 고른 것은 최적화가 아니라 규칙과 설명이다**(재구성).

### ⚠️ 이번 사이클의 도구 기록

**브라우저 `get_page_text` 가 두 사이클 연속 응답하지 않았다**(`Can't interact with browser-internal or unparseable URLs`). **`r.jina.ai` + WebFetch 로 인용을 나눠 받아 메웠다** — WebFetch 는 요약을 주지만 **`verbatim 으로 인용하라` 고 지정하면 원문 문장을 돌려준다.** ⚠️ **브라우저가 막힐 때의 차선책으로 기록해 둔다.**

### 다음 사이클 — 같은 회사의 남은 도메인 셋

`--gaps` 가 **`주소를 못 쓰는 나라에서 배달지를 찾는다`** 를 부를 것이다. ⏳ **`r.jina.ai` 를 앞에 두고 읽는다:**
- `https://www.meesho.io/blog/teaching-a-machine-to-read-indian-addresses-how-we-built-geoindia-2` (2026-05-07)
- `https://www.meesho.io/blog/teaching-a-geocoder-what-every-delivery-agent-already-knows` (2026-05-25)
- 그 다음: `trustmesh-...`(사기, 2026-07-15) · `the-pixel-gap-...`(이미지, 2026-06-24)

⚠️ **큐는 2/3 이다**(Zepto · Snyk). 이 회사를 다 판 뒤 7순위(버퍼 보충)가 걸릴 수 있다. ⏳ **Swiggy 는 브라우저가 살아나면 다시 본다.**

### ⚠️ 비교 문서 재료 세 축 (초안 유지)

**① `AI 에이전트를 어디까지 믿나` 6곳 + ⏳ Snyk** — Sentry / ClickHouse / Duolingo / Ramp / DoorDash / Deliveroo(**검사 가능한 것만 강제된다**).

**② `관리형 MySQL 의 한계` 3곳** — Etsy(Vitess) / Plaid(TiDB) / Paystack(MySQL 8.0 제자리).
