...
**Duolingo** (US · 교육) — 도메인 3개 · **기능 2개**. 남은 것은 테스트 하나이고, **자료는 이미 다 있다.**

### 이번 사이클 — 에이전트 `세 번 갈아엎고 나서 편의가 이긴다는 것을 인정한다`

**이 엔진에서 가장 선명한 연표를 세웠다 — 1년 사이 네 세대.**

| 시기 | 무엇 | 왜 넘어갔나 |
|---|---|---|
| 2024-11 | 로컬 MCP | **설정이 고통스럽다** — 설정·자격증명 찾기, `mcp.json` 손 편집. 소수만 썼다 |
| 2025-05 | 중앙 설정 페이지 | **깨진다** — Node·Python·Go·Docker, stdio·SSE·HTTP. `npm 버전 불일치·좀비 프로세스` |
| 2025-08 | 표준화된 HTTP MCP | **깨지진 않는데 안 쓴다** — `클릭 하나도 너무 많다` |
| 2025-09 | 슬랙 앱 | 지금. 주간 **300명(회사의 30%)** · 찬성률 **약 80%** |

건진 것 셋:

- ⚠️ **세 번의 실패가 전부 같은 원인이었고, 기술 문제를 다 풀고 나서야 진짜 문제가 보였다** — `not every engineer wants to configure a local AI environment; most just want answers`. 결론을 회사가 문장으로 적는다: **`Convenience wins. A Slack App people can just @-mention is far more adoptable than any MCP-setup guide, no matter how polished.`**
- ⚠️ **사람이 있는 곳으로 가면 도구의 성질이 바뀐다** — 개인 편집기 안의 도구는 혼자 쓰는 것인데 **채널의 봇은 여러 사람 앞에서 답한다.** 그래서 채널별 맥락·권한 통제·프라이버시 규칙이 **새로 필요해졌다**(재구성).
- ⚠️ **편의를 극단까지 밀면 봇이 권한 우회로가 된다** — 아무나 @멘션하는데 봇은 도구 **200개 이상**에 닿는다. 그 문을 한 문장으로 닫는다: `Engineers shouldn't gain access through the bot that they couldn't get directly`.

**3단계가 헛수고가 아니었다** — 표준화된 HTTP·단일 토큰·예측 가능한 URL 위에서만 4단계가 성립한다(재구성).

### 다음 사이클 — 테스트, 그러면 완주가 보인다

`--gaps` 가 **테스트를 사람이 아니라 파이프라인이 쓴다** 를 부른다. **자료가 이미 다 있다** — Temporal + S3(DynamoDB 를 버린 이유 포함) · 라벨 트리거와 주기적 백필 · **SwiftLint 실패율 13.6%** · **리뷰 병목** · **자동 수정 에이전트가 프로덕션 파일을 고친 사고** · 17주 PR 250개·테스트 약 85,000줄·MVVM 커버리지 9→30%.

**두 기능이 이미 이어져 있다** — 둘 다 Temporal 위에 있고 **둘 다 사람이 마지막 문을 지키는데, 저쪽에서는 그 문이 병목이 됐다.**
