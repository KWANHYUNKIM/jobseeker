...
**Sansan(JP · SaaS) `in_progress` — 도메인 4개 · 기능 4개.** 333 에서 **네 번째 도메인
`화면을 되돌려도 종이는 안 돌아온다` 를 열고 기능 `paper-does-not-undo`(실물이 있는 공정을
소프트웨어로 감싼다)를 썼다. 결정 6개 · 그림 3장 · 생각 4개 · 난제 2건 · 연결 1건 ·
⭐ 이 회사 첫 `ui`(현장 작업자의 스마트폰 화면, 핀 4개, 재구성이라고 밝힘).**

**두 편으로 받쳤다** — `当たり前を作っていくオペレーション：Bill Oneスキャンセンターの挑戦`
(2024-10-23, **새로 읽음**)과 `サービスごとに異なる原本管理を、一つの仕組みへ`(2026-08-20).
**스캔 센터 편 판정: 버린 대안 ❌ · 대가 ✅✅ · 수치 ✅ → 2/3 통과.**

**⭐⭐ 이 축은 이 사이트에 없던 종류다** — 다른 회사들의 되돌리기는 전부 화면 안에서 끝나는데
(트래픽을 이전 색으로 · 캐시 키를 버전별로 · 옛 경로를 살려 두기) **여기는 상자가 이미
옮겨져 있다**: *"システムのステータスだけを戻したとしても、実際の原本はもう別の場所に
あるかもしれない"*. **그래서 답이 되돌리기가 아니라 애초에 틀리지 않게 하는 쪽으로 모인다** —
관계없는 버튼을 감추고, QR 통합 ID 로 서비스별 규칙을 없애고, 누구나 이해하는 규칙을 세운다.

**⭐ 다른 축 둘도 굵다**
- **도착할 때 아무것도 모른다** — 누구 것인지, 몇 장인지, 크기가 얼마인지가 열기 전엔 안
  정해진다. **물건이 먼저 오고 데이터가 뒤따른다**(이 대비는 이 사이트의 것).
- **늘릴 수 없는 자원으로 10배의 진폭을 받는다** — *"ある日の10倍以上…その翌日は急にその
  10分の1以下"* 인데 *"物理オペレーションは場所・人・機材など制約が多く"* 다. 그래서 **미리
  두 배로 잡고** 그 대가를 스스로 적는다: *"これが閑散の原因となっています"*.

**수치** — 고객 **3,000사 이상** · ARR **84억 엔 이상** · 새 센터 **7개월** 기한 · 다시 설계 후
**처리량 3배** · 원본 관리는 **엔지니어 2명이 1.5개월**에 첫 릴리스.

---

**다음 사이클 — Sansan 을 닫는 판단이다. 남은 것은 이관 글들뿐이다.**
- **먼저 볼 것**(⚠️ 223 규칙으로 **이미 연 도메인 `저장소를 옮기면 붙어 있던 것이 따라오지
  않는다` 에 붙는지** 실물로 정한다) — Swift Concurrency 이관(2026-03-19) · Jetpack Compose
  이관(2025-11-17) · Eight 의 EC2 탈출과 모듈러 모놀리스화 · 레이어드 아키텍처 회고
  (2019-04-21) · 모바일 개발 대담 2부작(2026-08-21·08-25). **⑰ 주소는 검색으로 확인한다.**
- **기준을 넘기는 것이 없으면 닫는다.** 닫을 때 `business_model` 에 적을 관찰:
  **⭐⭐ 네 도메인이 같은 공정의 네 층이다** — **개발 환경**(저장소 이관) · **인식**(모델
  정확도) · **공정 실적**(사람과 기계가 몇 건 했나) · **실물**(종이가 어디 있나).
  **이 넷을 다 가진 회사는 이 사이트에 드물다.**
  **⭐ 그런데 글의 두께가 층마다 다르다** — 개발 환경 쪽은 **대가를 아낌없이 적고**(CI/CD 가
  이관보다 손이 갔다 · Secrets 미이관 · 병합 충돌 걱정은 기우였다), **인식 쪽은 성과 중심이며**
  (NineOCR 편은 버린 대안도 한계도 없다), **Digitization 쪽은 대담·운영 서사가 많다**(추정).
  **⭐ 그리고 이 회사는 자기가 만든 것을 안 쓴다고 적는다** — 초해상이 저해상에서 **30% 이상**
  올리는데도 뺐다(*"認識に問題があるほどの低解像画像が流れていないため"*).
  **⚠️ 가장 아프다고 말한 항목의 정확도는 공개하지 않는다** — 이메일·전화가 한 글자도 안 된다고
  해 놓고 **공개한 수치는 주소 98.3% 하나**다.

**📌 그 밖**
- **⚠️ 확인 못 한 수치** — 검색 조각의 **GEES 정확도 99.9%** 는 본문에서 못 봐서 **안 썼다**.
- **큐 2/3**(freee · SmartHR). **⚠️ Personio(EU)는 아직 안 봤다 — 다음 후보 조사의 첫 순위.**
- **⭐⭐ freee 와 SmartHR 이 정반대 순서로 같은 문제를 풀었다**(비교 문서 재료).
- **⭐ 비교 문서로 쓸 재료 셋** — ① 320 `leave-what-costs-more`(Paytm) → `reversible-first.md`
  와 `where-it-piles-up.md` 의 빈칸. ② 326 `know-both-versions`(Freshworks) →
  `reversible-first.md` 의 *"언제 롤백 경로를 닫는지 정한 곳이 없다"* 에 **첫 재료**.
  ③ **⭐ 333 의 `paper-does-not-undo` 가 `reversible-first.md` 에 전혀 없던 축을 준다** —
  **되돌릴 수 없는 것이 화면 밖에 있다.** 그 문서는 되돌릴 길을 만드는 네 가지 방법을 세는데,
  **여기는 그 방법들이 애초에 안 통하는 자리다.**
- **후보 조사 누적 열아홉 곳 중 일곱.**
