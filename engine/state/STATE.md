...
**Classi**(JP · 학교 ICT SaaS) — 회사 프로파일까지 썼다. **도메인 둘 · 기능 0.** 큐 0/3.

### 두 글이 다 '기술이 아닌 것'을 원인으로 지목한다

하나는 **학교 도메인의 복잡성**(학년도를 넘나드는 업무 논리 · **같은 반 이름이 해마다 다른
실체**), 다른 하나는 **조직 조건**(핵심 인력이 로테이션될 때마다 동력을 잃었다). **코드
밖에서 원인을 찾는 회사**라는 것이 이 프로파일의 축이다.

### 한 줄로 맡겼다 몇 주를 버렸다

**'집계 기간을 가변으로 해 두세요'** 한 줄을 AI 에 던졌더니 **부분적으로만 동작**했다 —
특정 조건에서 0 이나 틀린 값, 그리고 **고찰 텍스트가 지정 기간 밖의 데이터까지 참조**.
원인은 코딩 버그가 아니라 **코드에 안 적힌 도메인 규칙**이었고, **PR 을 머지하지 않은 채
닫았다.**

### 검수하려면 알아야 한다

**'AI 가 낸 코드가 옳은지 평가하려면 사람 쪽이 시스템 내부 구조와 사업이 다루는 도메인을
이해하고 있어야 한다.'** 그래서 역할을 가른다 — **도메인 지식이 필요한 설계 판단은 사람,
코딩 구현은 AI.** **모르면 맡길 수도 없다**는 말이기도 하다.

### 제도가 쓰는 언어가 곧 제품의 언어다

**'개별 최적화된 배움'** 에서 갈라지는 **'지도의 개별화'·'학습의 개성화'** 를 그대로 가져와
**학생의 러닝 사이클**과 **교사의 코칭 사이클**로 옮긴다. **헨리가 병원의 인시던트 레벨을
빌린 것**과 같은 결이다 — **일본 회사들이 제도의 말을 제품에 그대로 들인다.**

### 다음 사이클

확장 — 빈 도메인 둘 중 하나. **AI 실패 쪽이 원인 분석이 날카롭고**(학년도·반 이름),
리아키텍트 쪽은 **멈춘 이유와 고친 방법**이 짝을 이룬다. 큐가 0/3 이라 그 뒤에 후보
조사가 뜬다.
