...
**Block (Square)** — 기능 **3/4.** 회사 109개. 큐 2/3. ⚠️ **남은 도메인 하나**(`노는 컴퓨트를 모은다`)를 채우면 완주다.

### 이번 사이클 — `벌 계층으로 팀을 짠다`

⭐ **이 회사가 자기 가정을 수치로 깼다** — 에이전트를 더 붙이면 나아진다는 가정이다:

**`Terminal-Bench 2.1 에서 열두 가지 팀 구성을 돌렸다. 그중 어느 것도 값을 생각하면 솔로 SmartBee 를 앞서지 못했다.`** 이유까지 적는다 — **`몇 분에 끝나는 작은 작업은 나눌 만한 구조가 없다. 작은 작업에서 에이전트를 더 붙이는 것은 대개 같은 것을 두 번 설명하는 비용을 사는 것이다.`**

⭐ **그런데 긴 작업에서는 뒤집힌다** — 솔로 **15개·59.1%** → SmartBee + WorkerBee 둘 **20개·71.5%**. **`평균 보상 +12.4점, 그중 11.4점이 추가 완료분이다.`** ⚠️ **가르는 축이 작업의 난이도가 아니라 길이다.**

건진 것 셋 더:

- ⭐ **가장 비싼 조합이 가장 좋지 않다** — **xhigh 노력의 Opus 5 가 $140.63 에 75.0%** 인데 **high 노력의 GPT-5.6 Luna 가 $4.98 에 같은 75.0%** 다. ⭐ **원인이 실력이 아니라 시계다** — **`88개 작업 중 17개에서 벽시계 타임아웃에 걸렸다.`**
- ⭐ **그래서 노력을 계층별로 거꾸로 준다** — **`QuickBee 와 WorkerBee 는 high 로 돌려라. SmartBee 의 토큰은 조율과 판단과 가장 어려운 결정에 써라.`** **싼 모델은 세게, 비싼 모델은 medium.**
- ⭐ **에스컬레이션이 사람 앞에서 멈춘다** — **`코디네이터 하나, 워커 여럿, 당신에게 닿기 전에 멈추는 에스컬레이션. 진짜로 새로운 예외만 당신에게 온다.`** 사람이 준 답은 메모리에 쌓여 같은 질문이 두 번 오지 않는다.

### ⭐ 에이전트를 플레이어블 캐릭터로 다룬다

**`각각이 이름, 암호학적 신원, 성격, 그리고 채널 안의 제 자리를 갖는다.`** 성격은 **에이전트 지시**(백스토리이자 직무 기술서)와 **코어 메모리**(세이브 파일)에 담기고, 코어 메모리는 **`세션이 시작될 때마다 시스템 프롬프트에 주입되어 프롬프트와 똑같은 방식으로 행동을 빚는다.`**

⭐ **그리고 소유가 곧 신뢰다** — **`당신 것으로 암호학적으로 검증된 다른 에이전트의 메시지는 당신의 메시지처럼 다뤄진다.`** ⚠️ **에이전트는 부르기 전엔 안 움직인다** — **`벤치에 앉아 있다가 자기에게 온 이벤트가 떨어질 때`** 만.

### ⭐ 비교 문서 재료가 하나 늘었다 — `에이전트를 몇으로 둘 것인가`

**Cursor 는 `모델을 고르는 라우터`로, Block 은 `계층과 작업 길이`로 답한다.** ⚠️ **둘 다 벤치마크를 못 믿는다는 데서 출발하는데 방향이 반대다** — Cursor 는 **사용자의 다음 행동**으로 옮겼고, Block 은 **벤치마크를 돌리되 가격과 벽시계까지 같이 본다**(가격 5.5배에 점수 8.9점).

### 다음 사이클 — 확장, 그리고 완주 (2순위)

**Block 기능 3/4.** ⏳ 마지막 도메인 — **`노는 컴퓨트를 모은다`**(MeshLLM: Iroh QUIC NAT 통과, 층 단위 분산 추론, 드래프트 모델·n-gram·MTP). **이걸 채우면 `status: done` 조건 셋을 다시 확인한다.**

⚠️ **읽을 것이 남았다** — `Square 본업`(결제·정산·대출 심사) 자료를 아직 못 찾았다. ⏳ 미독 — `BlockTabBench` · `Buzz 프로젝트` · `Microfilm`. **⭐ 남은 물음 — Bitkey 는 난수를 어떻게 만드나.**
