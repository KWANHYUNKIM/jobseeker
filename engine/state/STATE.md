...
**1Password** (CA · 보안) — 도메인 3개 · **기능 2개**. **남은 것은 프로비저닝 하나이고, 그것이 완주를 막고 있다.**

### 이번 사이클 — 기밀 컴퓨팅 `우리를 믿으라고 하지 않으려고 믿을 곳을 바깥으로 옮긴다`

건진 것 넷:

- ⚠️ **앞 도메인이 만든 제약이 여기서 청구서로 돌아온다** — 못 읽게 만들어 놓았으니 기업 리포팅을 하려면 어딘가에서 평문을 봐야 하는데, **기기는 성능이 안 되고**(`cannot be decrypted and aggregated on-device...due to performance limitations`) **서버는 봐서는 안 된다.** **선택지 셋 중 둘이 막힌 상태에서 세 번째를 만들어야 했다**(재구성).
- ⚠️ **핵심은 신뢰를 없앤 것이 아니라 옮긴 것이다** — `우리를 믿어라` 대신 **AWS 의 하드웨어 격리 · 제3자가 통제하는 로그(Rekor) · 독립 감사자**를 믿게 한다. **믿을 곳을 자기 밖으로 흩어 놓으면 한 곳이 배신해도 드러난다.** 대신 **그 바깥들에 묶인다.**
- ⚠️ **검증을 사용자 기기에 심었다** — 클라이언트가 **서명되고 공개 로그에 게시된 코드가 아니면 접속을 거부한다.** 회사가 몰래 바꾸려면 **사용자 손에 있는 클라이언트도 고쳐야 한다.** 로그에는 `every single version of code ever run in the enclave` 가 남는다.
- **Rust 를 고른 이유가 안전만이 아니다** — `use the exact same tried and tested libraries already in use today for cryptography and safe logging`. **이미 검증된 것을 그대로 가져다 쓰는 것이 새 코드를 줄이는 가장 확실한 방법이다.** 로깅은 **컴파일 타임에 `loggable` 트레이트로 강제**한다.

⚠️ **한 회사 안에서 글의 결이 갈리는 아홉 번째 사례다** — 제로 지식 글은 **못 하는 기능을 목록으로** 적는데, **이 글에는 한계도 버린 대안도 성능·비용 수치도 없다.** 얻은 것만 적는다.

### 다음 사이클 — 프로비저닝, 기능보다 자료가 먼저다

⚠️ **매출의 75% 이상이 걸린 축인데 `tech` 가 0 이다**(검증 경고). `--gaps` 가 이 도메인을 부를 텐데, **기능을 쓰기 전에 자료를 찾는 것이 그 사이클의 일이다** — Fastly 보안·Monzo 플랫폼·ClickHouse 엔진에서 **세 번 연속으로 통한 규칙**이다.

찾을 곳: `next-generation-automated-provisioning` · `automated-provisioning-hosted-by-1password` · `support.1password.com` 의 프로비저닝 문서(**Fastly 때 제품 문서가 1급이었다**).

⚠️ **못 찾으면 도메인을 지우지 않는다** — 매출 75% 짜리 축을 숨기면 그림이 거짓이 된다. **`open_questions` 에 남기고 완주를 미룬다.**
