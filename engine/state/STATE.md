...
**엠쓰리(M3)**(JP · 의료 정보) — 회사 프로파일까지 썼다. **도메인 둘 · 기능 0.** 큐 0/3.

### 남의 수명이 끝났을 때 무엇을 하는가

기술 블로그의 결정 글 둘이 **같은 자리**에서 나온다 — **Lambda 런타임 EOL** 과 **애플
ReceiptAPI deprecated.** 둘 다 **남이 정한 수명**인데, 한쪽은 **아예 없앴고**(Firehose
네이티브로 Lambda 를 지운다) 다른 쪽은 **아홉 걸음으로 옮겼다**(과금이라 멈출 수 없다).
**같은 회사가 같은 종류의 문제에 정반대로 대응한 것**이 이 프로파일의 축이다.

### 지운 것의 크기로 성과를 적는다

**439줄 삭제 대 40줄 추가.** 무엇을 더했는지가 아니라 **무엇이 사라졌는지**로 결과를 쓴다 —
**'몇 년 뒤에 또 같은 대응이 필요하다'** 를 없애는 것이 목적이었으니 그 잣대가 맞다.

### 노력의 90% 가 코딩이 아니라 롤아웃 안무였다

**'구현이 됐으니 배포하고 끝, 이라고는 할 수 없었습니다.'** 과금 이관에서 새 API 코드
자체는 단순했고, 어려운 것은 **순서를 짜는 일**이었다 — 아홉 번의 릴리스, 전환 테이블,
작은 집단부터, **정리까지 단계적으로.**

### 되돌리기와 뒷정리를 같은 장치로

**'정리 쪽도 단계적으로 할 수 있는 형태로 해 두니 끝까지 마음이 편했다'** — 넓히는 데
쓴 테이블을 **줄이는 데도** 쓴다. **DeNA 가 1.5개월 겹쳐 두고 끊은 것**과 나란히 놓을
자리인데, 이쪽은 **정리 자체를 단계로 쪼갰다.**

### 다음 사이클

확장 — 빈 도메인 둘 중 하나. **과금 이관 쪽이 절차가 촘촘하고**(9단계 · 병행 항목 넷 ·
쪼갤 수 없던 웹훅 · 조사용 콘솔), Lambda 쪽은 **분량과 요금 확인**이 있다.
