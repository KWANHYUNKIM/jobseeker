...
**Classi** — 첫 기능 `decide-what-not-to-delegate`(AI 에게 무엇을 맡기지 않을지 정한다)를
끝냈다. **도메인 2 · 기능 1.** 큐 0/3. 남은 도메인은 **멈춰 있던 이유를 조직에서 찾는다.**

### 두 판단이 한 원칙으로 모인다

**맡겼다 실패한 것**(집계 기간 가변화 — 한 줄 지시, 몇 주 뒤 PR 을 닫았다)과 **아예 안
맡긴 것**(SQL 작성 — 검증된 템플릿에 학교 ID 만)이 같은 자리로 온다. **도메인 판단은
사람, 구현은 AI.** 그리고 **틀리면 안 되는 자리는 잘 시키는 대신 못 하게 한다.**

### 부분적으로만 동작하는 결과가 가장 위험하다

완전히 깨지면 바로 아는데, **어떤 그래프는 맞고 특정 조건에서만 틀리면** 검수를 통과한다.
게다가 **고찰 텍스트가 지정 기간 밖의 데이터까지 참조**해도 읽는 사람은 모른다.

### AI 를 탓하고 끝내지 않는다

원인이 **AI 쪽이 아니라 사람 쪽이 도메인 규칙을 정리해 두지 않은 것**이라는 데까지 간다.
**'AI 가 낸 코드가 옳은지 평가하려면 사람이 시스템 내부 구조와 도메인을 이해하고 있어야
한다'** — **모르면 맡길 수도 없다**는 말이다.

### 안 맡길 자리를 고르는 기준은 '결과가 밖으로 나가는가'로 보인다

영업 리포트는 **학교에 직접 전달**되니 **학교 간 혼입은 되돌릴 수 없다.** 그래서 그 자리만
손을 묶었다 — **같은 회사 안에서 기준이 다른 것**이 오히려 분명한 선이다. 컨벡스가
**`count()` 를 뺏고 넓은 읽기 집합은 문서로 맡긴 것**과 같은 모양이다.

### 원칙은 남겼는데 절차로는 안 갔다

실패에서 **원칙 둘**을 남겼지만 **체크리스트나 도구는 없다.** 엠쓰리(**'확인 체크리스트를
정례화'**)·쿡패드(**'구현 전에 선임과 이해를 맞춘다'**)와 대비된다 — **원칙만으로는 다음
사람이 같은 실수를 한다.**

### 다음 사이클

확장 — 남은 도메인(리아키텍트 리부트). **멈춘 이유와 고친 방법이 짝을 이루고** 있다.
큐가 0/3 이라 그 뒤에 후보 조사가 뜬다.
