...
**없다 — Twilio 완주.** 도메인 2/2 · **기능 2개.** 회사 **97개.** ⚠️ **큐가 0/3 이라 다음은 후보 조사(3순위)다.**

### 이번 사이클 — `자율을 주면 격차가 생긴다`

건진 것 넷:

- ⚠️ **좋은 이유로 고른 구조가 문제가 됐다** — 마이크로 프론트엔드를 고른 근거는 **`팀들이 독립적인 릴리스 주기와 자기 기술 스택에 대한 자율성을 가졌다`** 였다. **그런데 `도구, 라이브러리, 기술 스택이 시간이 갈수록 갈라져 팀 간 협업에 마찰을 더했다.`**
- ⚠️ **이 기능의 이름이 나온 자리** — **`성공과 개발자 효과성이 고르지 않게 분포했다 — 어떤 팀은 번창했고 어떤 팀은 고전했다.`** **그리고 격차의 끝에는 아예 못 옮긴 팀이 있다** — **`서버 렌더링 애플리케이션(대부분 PHP)에서 옮기지 않기로 하는 힘든 결정`.** **자율은 평균을 올리지만 분산도 키운다**(재구성).
- ⚠️ **조직의 문제를 푼 방법이 제품의 성능 문제가 됐다** — 팀 사이의 결합을 풀려고 넣은 **프록시가 모든 첫 요청 앞에 놓여** **`최악의 경우 첫 바이트가 고객에게 닿기까지 꼬박 1초`** 가 걸렸다.
- ⚠️ **자율을 거두는 대신 발언권을 줬다** — **오픈소스 기여 모델**로 **`팀들이 플랫폼 로드맵에 영향을 주고 개선을 모든 팀이 쓸 수 있게`** 했고, **`제품 팀을 우리 고객처럼 대한 것`** 이 문서·핵심 기능의 개선을 낳았다. ⚠️ **강제 대신 설득을 고른 대가로 안 옮기는 팀이 남는다**(재구성).

### ⭐ 한 회사 안에서 두 방향이 같은 태도를 쓴다

**알림은 모놀리스를 쪼갰고 콘솔은 마이크로 프론트엔드를 합쳤다.** ⚠️ **그런데 둘 다 `한 번에 갈아엎지 않는다`** — 저기서는 **피처 플래그로 읽기·쓰기를 따로**, 여기서는 **페이지 라우트 하나씩(한 스프린트 안에).** ⭐ **방향은 반대인데 방법이 같다는 것이 이 회사에서 나온 관찰이다**(재구성).

### ⚠️ 오래된 자료로 지금을 말하지 않았다

**이 도메인의 자료가 2021년 4월 글 하나뿐이다.** **5년이 지났고 그 뒤가 어떻게 됐는지, `옮기지 않기로 한` 팀들이 결국 어떻게 됐는지 모른다.** ⚠️ **그 사실을 metrics·도메인 설명·결정·연결·open_questions 다섯 군데에 적었다** — **재방문 사이클의 좋은 후보다.**

### 다음 사이클 — 후보 조사(3순위)

⚠️ **큐가 0/3 이다.** ⏳ **없는 축** — 헬스케어 · 물류 B2B · 산업/제조 · 통신(⚠️ **Twilio 를 넣었지만 본체 자료가 없어 통신 인프라 축은 아직 비어 있다**). ⏳ 되짚을 목록 — `www.gojek.io/blog`(429) · **Klarna**(목록은 열리나 최신이 2025-06, 본문 판정 미완) · ⛔ `bytes.swiggy.com`.
...
**③ `자기 성과를 어디까지 주장하나` 5곳** — Snyk / Grafana Labs / ScyllaDB / Razorpay / Flipkart.
...
**⑨ `한 번에 갈아엎을 것인가 목 졸라 죽일 것인가`** — **Twilio(두 방향 다)** / Etsy / Plaid / Paystack.
...
**⑬ `애매할 때 어느 쪽으로 넘어지나`** — Flipkart(둘) / Razorpay / Zepto / Paystack.

**⑭ `검색 관련성을 누가 정하나`** — Etsy / Flipkart / ⏳ Zepto.

**⑮ ⭐ `언제 쪼개고 언제 합치나` — 재료가 찼다** — **Twilio**(한 회사 안에 두 방향, 같은 방법) / **DoorDash**(모놀리스 → 마이크로서비스 약 50개) / **Deliveroo**(팀마다 만든 것을 중심 인프라로) / **ScyllaDB**(공유의 자리를 커널로 내렸다).
