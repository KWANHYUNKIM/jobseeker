...
**Modal** — 기능 **1/2.** 회사 114개. ⚠️ **큐 0/3** — 남은 도메인 하나를 채우면 완주고, 그다음은 반드시 후보 조사다.

### 이번 사이클 — `임계 경로에서 데이터 저장소를 걷어낸다`

⭐ **쿠버네티스를 안 쓴 이유를 수식으로 적는 드문 글이다** — 스케줄링 **`O(n × p)`** 에 기본 직렬화, 그리고 더 나쁜 것: **`각 노드가 하트비트 간격마다 최소 한 번은 etcd 에 써야 해서, 기저 부하가 파드 생성과 완전히 무관하게 O(노드) 다.`** ⚠️ **아무것도 안 만들어도 노드 수만큼 든다**(재구성).

⚠️ **자기 이전 구조도 같은 병으로 진단한다** — **`강한 일관성 때문에 전역 조율이 필요했고, 쉽게 샤딩할 수 없는 Postgres 에 O(샌드박스) 만큼 써야 했다.`** ⭐ **증상이 방향을 알려 줬다** — **`샌드박스 이탈률이 높으면 거대한 이벤트 백로그가 생겼다`**: **빨리 죽는 것이 많을수록 시스템이 느려졌다**(재구성).

### ⭐⭐ 답이 세 겹으로 일관된다 — 아래로 내리고, 늦추고, 틀릴 수 있게 한다

- **진실 원천을 아래로** — **`모든 워커가 자기 자신의 진실 원천이다.`** 상태를 **Redis 스트림에 발행**하고 스케줄러가 **비동기로 소비**해 **메모리 캐시**에 대고 판단한다.
- **저장을 늦춰서** — ⭐ **`샌드박스 생성의 임계 경로에 데이터 저장소가 아예 없다.`** 메타데이터와 결과는 **대체로 비동기로** 쓴다.
- **최종 판단도 아래로** — 스케줄러가 낡은 캐시로 고르고 **워커가 `현재 용량에 따라 받거나 거절한다`.** ⭐ **스케줄러가 틀릴 수 있다는 것을 전제로 짰다**(재구성).

**맞바꾼 것을 문장으로 적는다** — **`샌드박스를 만들고 돌리는 임계 경로 전반에서 전역 일관성을 확장성과 성능과 맞바꿨다.`** ⚠️ **`전반에서`가 중요하다** — 부분 완화가 아니다.

**결과** — **100만 개를 1분 안에**, **시작 중앙값 0.5초 미만**, **스케줄링 지연 수십 밀리초**, **`규모가 커져도 눈에 띄는 저하가 없다`.**

### ⭐ 남은 병목을 밝히면서 해법까지 적는다

**모든 워커가 단일 Redis 스트림에 발행**하는데 **`부하 시험상 10만 워커를 훌쩍 넘겨도 유효하고, 순서에 의존하지 않으므로 스트림을 더 만들면 된다.`** ⭐ **`순서에 의존하지 않는다`가 미래의 여유를 만들었다**(재구성) — 순서를 썼다면 나누기 어려웠다.

### ⭐ 그리고 다음 병목이 커널 안에 있었다

**빨라진 것 자체가 새 실패를 만들었다** — **`새 스케줄러가 컨테이너를 워커로 너무 빨리 밀어 넣어, 한꺼번에 시작하는 많은 컨테이너가 네트워킹을 설정할 때 리눅스 커널의 rtnl 락을 두고 경합해 시작이 수십 초 걸렸다.`** ⭐ **애플리케이션 층에서 아무리 잘 만들어도 그 아래에 직렬화 지점이 있다**(재구성). 회사가 인정한다: **`규모에서의 꼬리는 실재한다.`**

### ⭐ 세 번째 도메인 후보를 찾았다 — `시작 시간을 미리 벌어 둔다`

**CRIU 기반 메모리 스냅샷.** **`import torch` 예제가 5초 → p50 1.05초·p0 0.69초**, 초기화가 무거운 함수는 **3~10배.** **워커 메모리의 분산 파일 캐시는 캐시 없이 내려받는 것보다 3~5배.** ⚠️ **이번 대상이 스케줄링이라 stack 근거로만 쓰고 도메인은 안 세웠다** — **다음 확장에서 판단한다.**

### 다음 사이클 — 확장 (2순위)

**Modal 기능 1/2.** ⏳ 남은 도메인 — **`서버를 사용자 옆으로 옮긴다`**(분산 I/O 플레인 · p50 80ms 감소 · Go 재작성 · Valkey · 2MiB 임계값). ⚠️ **완주 판정 때 세 번째 도메인(스냅샷)을 세울지 함께 정한다** — 세우면 기능이 하나 더 필요하다.

⚠️ **큐 0/3 이다** — 완주 직후 **후보 조사**를 반드시 끼운다.
