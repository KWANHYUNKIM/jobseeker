...
**Cockroach Labs**(US · 분산 SQL) — 도메인 2개 중 **1개에 기능이 들어갔다**(`value-separation`).
남은 빈 도메인 하나: '분산 트랜잭션 위에 벡터 색인을 얹는다'. 큐 0/3.

### 어려운 것은 해법이 아니라 무엇이 불필요한 일인지 알아보는 것

이번 기능의 축이다. **"정렬은 키로만 정해지는데 값까지 옮긴다"** — 한 문장으로 낭비의
정체를 짚고 나면 해법은 거의 따라 나온다. 값을 밖으로 빼고 핸들만 남기면 된다.

### 논리 식별자를 넣어 재작성의 자유를 샀다

핸들에 물리 오프셋을 박았다면 blob 을 다시 쓸 때마다 **SST 를 전부 고쳐야** 한다 — 분리한
의미가 사라진다. `BlobFileID` 라는 논리 식별자와 LSM 이 들고 있는 매핑, 그리고 재작성된
인덱스 안의 **BlockID 재매핑 열**이 그 값을 치른 대가다.

### 줄이려던 일이 부분적으로 돌아온다

갈라 둔 참조는 시간이 지나면 뒤섞인다. 그래서 **blob 참조 깊이**(한 SST 를 훑을 때 작업
집합에 들어오는 blob 파일의 최대 개수)를 재고, 커지겠으면 **새 blob 파일을 써서 깊이를
1 로 되돌린다.** 그 되돌리는 일이 결국 값을 다시 쓰는 컴팩션이다.

### 최적화의 성패가 자기 층 밖에 있다

MVCC 가비지를 갈라 둬도 **`AS OF SYSTEM TIME` 질의가 읽기 대역폭을 지배**해 이득이 상쇄된다.
저장 엔진에서 아무리 잘 해도 위층이 옛 판을 계속 읽어 가면 소용이 없다 — 회사가 그것을
**47% 를 내놓은 같은 글에서** 적는다.

### 수치를 크게 낼수록 조건을 함께 적는다

작은 값 부하에서는 효과가 덜하다고 적고, MVCC 쪽 이득은 아직 못 쓴다고 적는다. 반대로
**왜 하필 256바이트인지, 재작성 임계값이 얼마인지, p99 읽기 지연이 어떻게 되는지**는
비워 뒀다 — `failure` 도해에 여섯 가지로 세웠다.

### 다음 사이클

남은 도메인 하나 — '분산 트랜잭션 위에 벡터 색인을 얹는다'(C-SPANN). 자료가 두껍다
(RaBitQ 94% 압축 · 팬아웃 100 · 100억 벡터에 5단계 · 분할·병합·재배정 · 접두 열).
그것으로 이 회사는 완주다.
