...
**Railway — 기능 둘.** 남은 빈 도메인은 CDN 하나. 회사 136개, 큐 2/3.

### 자원이 모자란 게 아니었다

빌드 노드를 **CPU 20%에서 이미 늘리고 있었다.** 오래 도는 빌드 하나가 공유 VM 을 다른 손님에게
못 쓰게 만드는 구조가 문제였지 자원이 부족한 게 아니었다. 256코어·512GB 한 대를 마이크로VM
여덟 개로 갈라 방마다 buildkitd 를 돌리니 시끄러운 이웃이 갇힌다 — 시간당 6만 6천 건.

### 초는 안 하는 일에서 나온다

네 겹을 한 프로세스로 합쳐 gRPC 홉과 워크플로 왕복을 지워 **약 20초**, 이미지 레이어를 풀지
않고 OCI 매니페스트에서 바로 읽어 **약 10초**. 둘 다 더 빠르게 만든 게 아니라 하던 일을 없앴다.

### 자기 금속이 남의 클라우드보다 늦게 받았다

새 빌더를 베어메탈에 먼저 내놨다가 **브리지 네트워킹이 불안정해 되돌리고** GCP·AWS 에 먼저
배포한 뒤 돌아와 망을 고쳤다. 떠나기로 한 클라우드가 아직 안전판으로 남아 있는 자리다 —
직접 만든 망은 직접 고쳐야 한다는 값이 여기서 드러난다.

### 방 안에서 다시 나눈다

방을 갈라 놓고도 그 안에서 cgroup v2 로 제어 프로세스에 바닥을 준다. runc 작업자가 buildkitd 를
굶기면 그 방 전체가 멈추기 때문이다 — 격리를 두 층으로 겹친 셈이다.

### 다음 사이클 — 확장 (Railway 의 마지막 빈 도메인)

`엣지도 남의 것을 안 쓴다`. 히카리 넷(edge-cp·keeper·데이터면·WASM 게스트), POP 60·노드 180,
WASM 10μs·스레드 로컬 풀, BGP 커뮤니티, 3천만 RPS 의 진짜 뜻이 손에 있다.
