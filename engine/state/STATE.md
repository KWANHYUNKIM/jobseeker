...
**없다 — Cygames 완주.** 도메인 3/3 · **기능 3개.** 회사 **95개.** ⚠️ **큐가 0/3 이라 다음은 후보 조사(3순위)다.**

### 이번 사이클 — `에이전트를 여럿 두고 서로 비판하게 한다`

건진 것 넷:

- ⚠️ **하나로 두면 그 하나가 틀렸을 때 걸러 낼 것이 없다** — 그래서 **관점(보안·품질)과 모델(Claude·Codex)을 함께 갈라 넷**을 만들고 **각각 독립적으로** 지적 후보를 내게 했다. **다른 관점과 다른 모델이 같은 실수를 할 확률이 낮다는 전제다**(재구성).
- ⚠️ **모아서 합치는 대신 서로 비판하게 했다** — 2라운드에서 **에이전트들이 서로의 지적을 검토한다.** **여러 의견을 나열하면 사람이 다시 걸러야 하는데, 서로 걸러 내면 사람에게 가는 목록이 짧아진다**(재구성).
- ⚠️ **품질의 값을 숫자로 적는다** — **처리 단위당 토큰 약 8배, 시간 약 2배.** 그리고 **`현 단계에서는 비용 최소화보다 품질 검증을 우선한다`** 고 밝힌다. ⚠️ **`현 단계에서는` 이라는 말이 이 선택이 잠정적임을 드러낸다**(재구성).
- ⚠️ **어디까지 읽을지를 AI 에게 맡겼다** — **`리포지토리 전체를 참조할 수 있게 해 두었고, 구체적으로 어디까지 확인할지는 AI 가 판단`**. **변경된 파일만 보면 그 변경이 다른 곳을 깨뜨리는지 알 수 없으니 필요한 설계인데, 그만큼 비용이 요청마다 달라진다**(재구성).

### ⭐ 비교 문서 ①축에 여덟 번째이자 새 답이 들어왔다

**`AI 에이전트를 어디까지 믿나`** — 앞의 일곱 곳은 **사람이나 규칙이 AI 를 감독하는 방식**을 말한다(Sentry 조직의 신뢰 / ClickHouse 접근 범위 / Duolingo 사람 리뷰 → 병목 / Ramp 사람의 신원 / DoorDash primitives / Deliveroo 검사 가능한 것만 강제 / Snyk 벤치마크로 잰다). ⚠️ **Cygames 는 다르다 — AI 를 여럿 두고 서로 검증하게 한다.** 그리고 **그 값을 8배 토큰으로 적는다.** ⭐ **`감독을 누가 하는가` 라는 축에서 유일하게 `다른 AI 가` 라고 답한 곳이다**(재구성).

### 다음 사이클 — 후보 조사(3순위)

⚠️ **큐가 0/3 이다.** ⏳ 되짚을 목록 — `www.gojek.io/blog`(429) · ⛔ `bytes.swiggy.com`(세 번 두드리고 접었다). **새 이름도 필요하다** — 지금 없는 축: **헬스케어 · 물류 B2B · 산업/제조 · 통신.**

⚠️ **이번 세션에서 판 회사가 여덟이다**(Deliveroo · Meesho · Zepto · Snyk · Grafana · ScyllaDB · Cygames, 그리고 앞서 Paystack). **전부 큐에서 나왔다.**

### ⚠️ 비교 문서 재료 (초안 유지)

**① ⭐ `AI 에이전트를 어디까지 믿나` 8곳 — 재료가 넘친다** — Sentry / ClickHouse / Duolingo / Ramp / DoorDash / Deliveroo / Snyk / **Cygames(에이전트끼리 서로 비판)**.
...
**⑧ `제약을 없애지 못할 때 어디서 갚는가`** — Cygames / Zepto / Grafana.
