...
**쿡패드** — 첫 기능 `migrate-whole-then-fix-embedded`(통째로 옮기면 안에 박힌 것부터
깨진다)를 끝냈다. **도메인 2 · 기능 1.** 큐 0/3. 남은 도메인은 **바꾼 뒤에 나빠지지
않았음을 증명한다.**

### 순서를 고를 수 없는 이관이 있다

**'글로벌판 레시피의 ID 는 이관해 보지 않으면 정해지지 않는다'** — 새 식별자가 **이관의
결과로** 정해지니 **먼저 옮기고 그다음에 참조를 고칠** 수밖에 없다. 그 사이에는 링크가
어긋나 있다.

### 참조 무결성을 데이터베이스가 못 지켜 주는 자리

레시피 본문의 **소개문·재료·순서·요령** 네 군데에 `レシピID:12345` 가 **사용자 손으로
쓰여** 있다. 스키마 밖의 텍스트라 **이관이 조용히 깨뜨린다** — 렌더의 숨은 DNS 의존성과
같은 결이다(평소에는 안 보이는 연결).

### 한 군데 고쳤다고 다 고친 것이 아니다

**'레시피 편집 때에는 내가 구현을 넣은 곳과 다른 곳의 코드가 도는 것을 뒤늦게
알아차렸다.'** 게다가 **사내 도구 Queuery 와 손 실행의 SQL 이스케이프 거동이 달라**
확인 결과까지 어긋났다. 카카오페이가 **프록시가 자기 호출을 못 잡는다**고 적은 것과
같은 자리다.

### 배운 것을 절차로 적는다

**'다음엔 잘하자'가 아니라** 셋을 절차로 남겼다 — **도메인을 아는 사람에게 먼저 묻기**,
**구현 전에 선임과 이해 맞추기**, **확인 체크리스트를 정례화하기.**

### 다음 사이클

확장 — 남은 도메인(검증). **RBO 97.2/95.1 · 2만 키워드 · 인터리빙의 중복 문제**가 이미
근거로 서 있다. 큐가 0/3 이라 그 뒤에 후보 조사가 뜬다.
