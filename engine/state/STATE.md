...
**없다 — 문체 보수 사이클이었다.** VictoriaMetrics 를 배민 문체로 다시 썼다(3.78 → 0.0). 회사 115개, 큐 2/3. 67곳 남았다.

### 이번 사이클에서 가장 값진 것 — 견줄 자리가 저절로 나온다

문체 보수는 새 조사를 안 하는 사이클인데, 다시 읽는 동안 견줄 자리가 눈에 들어온다.
VictoriaMetrics 는 카디널리티가 비용의 정체라고 말하며 압축을 파는 회사이고,
Databricks 는 서버리스와 AI 워크로드가 늘자 인스턴스가 짧게 살다 사라지면서 라벨 조합이
폭발해 시계열 DB 를 직접 포크한 회사다. **같은 문제를 파는 쪽과 깔린 쪽에서 다 보게 됐다.**

여기에 Airbnb 와 Razorpay 가 둘 다 vmagent 를 골랐다는 이야기가 원래 있었으니, 이 회사
하나에 사는 쪽과 파는 쪽과 깔린 쪽이 다 모인다. 처음 팔 때는 못 넣었던 연결이다 —
Databricks 를 나중에 팠기 때문이다.

### 이번 사이클 — VictoriaMetrics

비용의 정체를 앞세웠다. 한 시리즈에 백만 개 샘플은 싸지만 백만 개 시리즈에 샘플 하나씩은
그렇지 않다는 것이고, 이유가 태어나는 순간에 있다. 새 시리즈가 처음 나타나면 색인 쓰기가
열네 번 일어나는데 그다음부터는 샘플이 아무리 쌓여도 싸다.

도메인 경계의 근거도 적었다. 저장과 질의를 따로 볼까 했으나 카디널리티가 둘을 같은 이유로
무겁게 만들어 한 경계로 뒀고, 기술이 아닌 것(오픈소스를 사업으로 만드는 일)을 도메인으로
세운 이유는 무료로 다 주는 결정이 제품 구조를 정하기 때문이라고 밝혔다.

기능 이름에 핵심을 붙였다. `백만 개 샘플은 싸고 백만 개 시리즈는 비싸다 — 비용의 정체`,
`추적하지 않고 기다린다 — 리드를 안 잡는 오픈소스 회사`.

### 다음 사이클 — 문체 보수 (사다리 3순위)

무거운 순으로 vercel(3.75) · atlassian · trendyol · cygames · payfit 이 남았다.
