...
**없다 — 문체 보수 사이클이었다.** 타깃을 배민 문체로 다시 썼다(1.06 → 0.0, 기호 175개). 회사 115개, 큐 2/3. 11곳 남았다.

### 규칙을 시스템이 지키게 하는 넷 — 여기만 안 막는다

- Deliveroo — 규정 준수를 회의에서 배포 파이프라인으로.
- Monzo — 데이터 표준을 CI 에 심어 어기는 것만 막는다.
- Booking.com — 그 검사를 더 앞으로 당긴다.
- 타깃 — **막지 않고 보여 준다.** 청구서 대신 점수판을 세우고, 벌칙에 대한 두려움보다 긍정적
  강화가 더 나은 채택을 이끈다는 연구를 근거로 든다.

다만 보여 주는 층과 막는 층이 갈려 있다 — 같은 회사의 DBaaS 는 쿼터가 걸린 온디맨드
프로비저닝을 쓴다. **점수는 안 막고 플랫폼이 막는다.**

### 매장을 거점으로 쓰는 대가가 계산 문제로 돌아온다

쿠팡은 물류센터를 깔아 어디서 낼지를 미리 정했고, 웨이페어는 보내는 것이 커서 반품이 비싸다.
타깃은 **재고가 파는 곳과 보내는 곳에 동시에 있어 주문마다 다시 푼다.** 그래서 얼마나 좋은 답을
얼마나 빨리 낼지를 값으로 흥정하는 자리가 된다.

짝을 이루는 재고 배치 기능에는 이 모델이 답할 수 없는 질문도 적었다 — 주어진 창고망 안에서만
최적이라 **창고를 어디에 둘 것인가는 모델의 바깥이다.**

### A/B 테스트를 버린 자리

Etsy 는 팀마다 도는 실험이 서로 간섭한다는 것을 공개하며 얼마나 잘게 나눠 볼지를 물었고,
타깃은 **아예 A/B 테스트를 버렸다**(규모에서 한 번에 한 실험은 비현실적이라서). 대신 값을 앞에서
낸다 — 첫 며칠간 여러 배열을 탐색하는 동안 손님 일부는 덜 좋은 화면을 봤다.

### 언제를 맞히는 문제

Duolingo 가 언제 다시 보여 줄지와 무엇을 보여 줄지를 두 모델로 갈랐다면, 타깃은 **그 언제가 곧
쿠폰의 시점**이다. 그리고 못 맞히는 것을 적는다 — 앱인지 Target.com 인지 매장인지는 향후 과제다.
**사는 시점은 맞혀도 사는 자리는 아직 모른다.**

### 다음 사이클 — 문체 보수 (사다리 3순위)

--gaps 가 정한다. 지금 맨 위는 freee(1.0)지만 그건 이 시점의 예상이지 확정이 아니다.
