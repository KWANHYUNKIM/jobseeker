...
**Bun — 기능 하나를 채웠다(`bun-install`, 설치를 시스템 콜 수로 잰다).** 회사 124개, 큐 2/3.
빈 도메인 둘 — 러스트 이전 · API 호환.

### 자를 바꾸면 답이 바뀐다

이 기능이 다시 푼 것은 알고리즘이 아니라 **재는 대상**이다. 기존 도구들이 느린 I/O 를 스레드 풀로
감추게 설계된 사이 병목이 시스템 콜로 옮겨 갔다는 것 — 시스템 콜 하나가 모드 전환에만 1,000~1,500
사이클이니 400만 회면 사이클 수십억이 거기로 간다.

같은 설치의 시스템 콜 수가 그 주장의 증거다: yarn 4,046,507(futex 61.76%) · npm 996,978 ·
pnpm 456,930 · **Bun 165,743(futex 0.46%)**. futex 비율이 특히 말해 주는 것이 있다 — 다른 도구가
느린 이유의 절반 이상이 디스크가 아니라 **스레드들이 서로 기다리는 것**이었다.

### 싼 길을 묻고 안 되면 내려간다

파일 복사가 맥은 clonefile 한 번(1.27초 대 2.96초), 리눅스는 하드링크(109ms 대 325ms) →
ioctl_ficlone → copy_file_range → sendfile → 일반 복사 다섯 갈래다. 성능이 커널과 파일시스템의
조합에 매인다는 것을 받아들인 구조다.

### 기계에 빠른 것과 사람이 다룰 수 있는 것

이진 락파일은 JSON 파싱을 아예 없앴는데 **PR 에서 검토가 안 되고 충돌을 병합할 수 없어 되돌렸다.**
대신 기계만 보는 매니페스트 캐시는 이진으로 남겼다(캐시된 설치 9ms 대 npm 363ms). 이 선을 실제로
그어 본 기록이라 이 엔진에서 드문 자리다.

### 다음 사이클 — 확장 (Bun 의 빈 도메인)

러스트 이전 글은 이미 읽어 뒀다. 이 층까지 내려간 대가로 메모리 관리가 손에 들어왔고 그것이 언어를
바꾸는 결정으로 이어진다 — 두 도메인을 `connections` 로 걸어 뒀다.
