...
**Antithesis — 기능 하나를 채웠다(`deterministic-hypervisor`, 코어 하나로 몰아 세계를 재현되게
만든다).** 회사 119개, 큐 1/3. 빈 도메인 둘 — 탐색 · 되돌려 보기.

### 사업 모델이 기술 층을 정했다

남에게 파는 물건이라 고객의 코드를 고칠 수 없다. 자기 시스템을 시험하려고 결정론을 짓는 회사는
코드를 그 규율에 맞춰 쓰면 되지만(Turso·TigerBeetle), 여기는 이미 쓰인 남의 바이너리를 그대로
받아야 한다. 그래서 결정론을 언어도 라이브러리도 아닌 **코드 아래**에서 만든다 — 하이퍼바이저다.

바닥부터 짓지는 않았다. FreeBSD 의 bhyve 를 결정론 쪽으로 포크해 장치 모델과 가상화 처리를
물려받고 자기 문제 한 축에만 손댔다.

### 비결정성이 들어오는 문 넷을 하나씩 닫는다

시간(어떤 시간 소스를 읽어도 가상 시간을 준다) · 인터럽트(명령어 수를 기준으로 같은 자리에 놓는다) ·
멀티코어(한 시스템을 코어 하나에 묶는다) · 환경(네트워크와 저장소를 방 안에서 흉내 낸다). 넷 중
셋이 자유도를 없애는 방식이라, 이 회사의 결정론은 **정교한 기록이 아니라 단순화**로 얻어진다(재구성).

### 파는 물건의 결함을 수치로 적는다

성능 모니터링 카운터가 정밀 모드에서도 완전히 결정론적이지 않고, 임계 이벤트를 CPU 가 알리기까지
수십 명령어가 더 지나간다. 그래서 약 1조 명령어에 하나꼴로 잘못 센다고 적고 원인을 모른다는 것까지
밝힌다. **파는 것이 재현성인 회사가 자기 재현성이 완전하지 않다고 쓴 자리**라 값이 크다.

이 엔진에서 못 하는 것을 적는 회사들(Block 의 다섯 기능, DuckDB 의 세 기능) 옆에 놓인다. 다만
성능 수치는 하나도 없다 — 처리량이 꽤 높다고만 하고 대지 않는다.

### 다음 사이클 — 확장 (Antithesis 의 빈 도메인)

탐색 알고리즘이 무엇을 신호로 삼는지는 공개 자료에 없다. 그 도메인을 팔 때 `Debugging in the
Multiverse` 와 `Breaking the WAL` 을 먼저 연다.
