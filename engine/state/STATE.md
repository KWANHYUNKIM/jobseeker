...
**없다 — 문체 보수 사이클이었고, 이것으로 문체 빚이 0이 됐다.** 링크드인(0.56 → 0.0)이 마지막이었다. 회사 115개, 큐 2/3.

### 문체 보수 국면이 끝났다 — 서른네 사이클

`--gaps` 의 문체 목록이 비었다. 이 국면에서 한 일을 한 번만 정리해 둔다.

- **기호를 걷어낸 회사 34곳.** 가장 무거웠던 것은 컬리(489) · 메이퇀(462) · 하이퍼커넥트(457) ·
  윅스(414) · 스냅(409) 순이었다.
- **validate 가 세지 않는 `🔴` 가 일곱 회사에서 나왔다**(산산 273 · 컬리 165 · SmartHR 158 ·
  윅스 176 · 메이퇀 220 · 하이퍼커넥트 96 · 사이보우즈 64). 같은 시기에 쓴 문서들의 공통 습관이다.
- **원문 인용을 우리말로 풀었다.** 가장 많았던 것이 윅스(215종) · 스냅(190여) · 그랩(100여) ·
  프리(일본어 100여) · 메이퇀(중국어 60여)이다.
- **인용을 풀면 조사가 붕 뜬다**는 것을 열여덟 번 손으로 고치고 나서야 변환기에 넣었다.
  넣은 다음 사이클(그라파나 랩스)부터는 손볼 것이 하나도 안 남았다.
- **검증이 두 번 막았다** — 변환기가 `solves` 값이 `—` 하나였던 칸을 비워 버린 것을 모달과 그랩에서
  각각 잡았다. 둘 다 채우고 115개를 다시 훑었다.
- **데이터가 스스로와 어긋난 것도 고쳤다** — Razorpay 의 open_questions, 패스틀리의 도메인 why,
  페이스택의 `country`(본문은 나이지리아인데 값이 `기타`였다).

**그리고 이 국면의 진짜 산출물은 기호를 지운 것이 아니라 축이다.** 회사마다 앞선 회사와 견주는
문장을 넣으면서, 이 엔진에 흩어져 있던 결정들이 축으로 묶였다 — 관측 비용을 치르는 여덟 가지 답,
사내 에이전트가 막으려는 여덟 가지, 이전에서 과거를 대하는 일곱 가지, 재는 것을 의심하는 다섯
가지, 남의 규칙을 다루는 여섯 가지처럼.

### 다음 사이클 — 신규 (사다리 7순위)

문체 목록이 비면서 사다리가 3순위에서 7순위로 내려갔다. QUEUE 맨 위는 **Materialize** 이고,
그다음이 Zed 다. 큐가 2/3 이라 이 회사를 끝낸 뒤에는 후보 조사를 한 번 끼우게 된다.
