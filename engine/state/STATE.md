...
**Feldera — 회사 프로파일까지 썼다(in_progress).** 회사 123개, 큐 0/3.

도메인 셋을 세웠고 기능은 아직 0개다.

- **어떤 SQL 이든 증분으로 바꾼다** — DBSP(VLDB 2023 최우수 논문상) + 실제 운영 프로그램 벤치(2026-02-04)
- **유지해야 할 상태를 무엇에 어떻게 담을지 정한다** — `Nobody ever got fired for using a struct`(2026-03-02)
- **파이프라인이 어디서 느린지 보이게 만든다** — 시각 프로파일러(제목만 확인)

### 이론을 앞에 세우는 회사다

학술 배경 회사가 이 엔진에 넷이 됐는데(Materialize 의 차분 데이터플로 · DuckDB 의 CWI ·
CedarDB 의 TU 뮌헨 · Feldera 의 DBSP) **논문을 판매 문장의 맨 앞에 두는 것은 여기가 처음**이다.
성능이 아니라 무엇까지 되느냐를 판다 — 조인이 아무리 많아도 증분화한다는 것.

증거의 모양도 그에 맞는다. 속도가 아니라 프로그램의 크기를 낸다 — 입력 테이블 61개, 출력 뷰 33개,
조인 217개·집계 27개·선형 연산자 287개짜리 실제 운영 프로그램을 16코어 한 대에서 돌려 입력이
바뀌면 약 200ms 에 모든 출력을 갱신하고 정상 상태 메모리가 15GB 다.

### 오픈코어의 선을 규모에 그었다

MIT 오픈소스가 **단일 노드·단일 컨테이너**까지다. 전체 SQL 과 모든 커넥터, 메모리보다 큰
데이터셋까지 주되 여러 대로 늘리는 것이 엔터프라이즈다. 바로 앞에 판 QuestDB 는 복제·TLS 같은
운영 안전장치를 유료로 뺐는데, 여기는 규모를 유료로 뺐다.

### 병목이 계산이 아니라 보관 형식이었다

컬럼이 700개 넘는 워크로드에서 메모리 40바이트짜리 행이 보관 형태로는 88바이트가 됐다 —
`Option<T>` 래퍼가 판별자를 요구해서다. 자료 배치를 갈아엎는 대신 직렬화 형식만 고쳐(널을 비트맵으로,
값은 조밀하게, 널 많은 행은 희소 표현) 절반으로 줄였다. 증분 유지의 값이 상태를 계속 들고 있는
것이라는 사실이 여기서 디스크 I/O 로 나타난다.

### 다음 사이클 — 확장 (Feldera 의 빈 도메인)

Materialize 와의 대비(차분 데이터플로 대 DBSP, 메모리 원가 대 직렬화 크기)를 기능에서 세운다.
