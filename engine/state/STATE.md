...
**Perplexity** — 회사 프로파일을 썼다. 도메인 **3개**, 기능 **0/3**. 회사 **111개.** ⚠️ **큐 0/3 — 이 회사를 다 판 뒤 후보 조사다.**

### 이번 사이클 — ⭐ 언어 경계가 곧 아키텍처인 회사

⭐ **CPU 쪽은 러스트, GPU 쪽은 파이썬, 그 사이는 RDMA.** 세 서비스 이름이 그대로 경계다:

- **Ivy** — **러스트 HTTP 게이트웨이.** **`CPU 쪽 일 — JSON 파싱, 토큰화, 입력 템플릿, 배치 쪼개기`** 를 맡고 자체 gRPC 로 옮긴다.
- **Tulip** — **러스트 gRPC 서버**(tokio · tonic). **스케줄링과 배치.**
- **ROSE** — ⭐ **여기만 파이썬이다.** **`커널·레이어·모델 정의를 제공하고 CUDA 그래프를 관리한다.`**

⚠️ **왜 이렇게 갈랐는지가 사업에 있다**(재구성) — **사용량 기반 과금으로 옮긴 뒤 ARR 이 1억 → 5억 달러가 됐다.** ⭐ **사용량으로 팔면 추론 원가가 곧 마진이라, 인프라를 직접 만드는 것이 기술 취향이 아니라 사업 결정이다.**

### 건진 것 넷

- ⭐ **별도의 임베딩 엔진을 안 만들었다** — **`LLM 스택의 프리필과 디코드 커널을 다시 쓴다.`** 이유가 정확하다: **배치 임베딩은 계산 병목인 프리필을 닮았고, 온라인 임베딩은 메모리 병목인 디코드를 닮았다.**
- ⭐ **CUDA 그래프 캡처가 너무 오래 걸려 시작을 못 하는 문제를 시간으로 흩었다** — **`지연 캡처로 몇 분치 즉시 작업을 몇 시간에 걸쳐 흩는다.`** ⚠️ **대가는 초기에 아직 캡처 안 된 경로가 있다는 것이다**(재구성).
- ⭐ **선착순 스케줄러를 쓰면서 그 근거를 댄다** — **`지연이 시퀀스 수가 아니라 토큰 수에 대략 비례한다.`** **10억 파라미터 미만 모델에서 512 토큰쯤이면 GPU 가 포화된다.**
- ⭐ **`LazyTensor` 로 언어 경계를 성능으로 바꾼다** — **`러스트 비동기 태스크가 N 번째 배치를 기다리는 동안 CPU 가 N+1 번째를 큐에 넣는다.`**

### ⭐ 논문이 있는 도메인이다 — MLSys 2026

**`RDMA Point-to-Point Communication for LLM Systems`**(arXiv 2510.27656). ⭐ **문제 정의가 이식성이다** — **`분리 추론, 전문가 혼합 라우팅, 비동기 강화 미세조정`** 이 유연한 점대점 통신을 요구하는데 **기존 구현이 특정 NIC 에 묶여 `추론 엔진에 넣기 어렵고 하드웨어 공급자를 옮기기 어렵다.`**

**fabric-lib 이 균일한 인터페이스를 준다** — **순서 가정 없는 완료 통지를 위한 `ImmCounter` + 단방향 `WriteImm`**, **GPU 하나에 붙은 여러 NIC 를 투명하게 관리.** **ConnectX-7 과 AWS EFA 양쪽 400Gbps**, **조 단위 파라미터 모델의 RL 가중치 갱신 1.3초**, **EFA 에서 처음으로 쓸 만한 MoE 디코드 지연.** ⚠️ **집합 통신을 대체하지 않고 `보완한다`고 스스로 적는다.**

### ⚠️ 자료 접근에 벽이 있다

**`perplexity.ai/hub/blog` 목록 페이지가 403 이다**(`r.jina.ai` 로도). ⭐ **개별 글 주소는 열린다** — 임베딩 글의 주소를 **3자 기사(MarkTechPost)에서** 얻었다. **목록이 막히면 3자 요약에서 원문 주소를 캐낸다** — Klaviyo 에서 본문 문구로 찾은 것과 같은 계열의 우회다.

### 다음 사이클 — 확장 (2순위)

**Perplexity 기능 0/3.** ⭐ **`GPU 끼리 직접 말하게 한다`부터 판다** — **논문이 있어 근거가 가장 두껍고**(arXiv 본문 + 오픈소스), **거절한 대안(NIC 종속 구현)과 수치가 다 있다.**

⏳ **못 읽은 것** — `Lily`(애플 실리콘 로컬 추론) · ROSE 의 러스트 이관 범위 · **검색 본업의 기술 자료**(색인·랭킹·인용 검증). ⚠️ **Block 에서 겪은 것을 기억한다 — 본업 자료가 안 보이면 목록을 더 넘겨야 한다.**
