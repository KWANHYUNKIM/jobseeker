...
**없다 — TigerData 를 완주시켰다(도메인 2 · 기능 2 · `status: done`).** 회사 174개, **큐 0/3.**

### 파는 물건의 가장 아픈 곳을 첫 문단에 둔다

연속 집계 글이 **여는 장면으로** 자기 설계의 부담을 보여 준다 — 버퍼된 공장 데이터가 원래
타임스탬프로 도착해 집계가 **41,900 에서 43,100 으로 조용히 올라갔고 아무도 몰랐다.**
숫자는 **'빠르게'** 그리고 **'그럴듯하게'** 돌아온다. **틀린 숫자가 틀려 보이지 않는다.**

### 남을 물리면서 자기 값도 같이 적는다

**전체 재계산 · 클릭하우스식 삽입 트리거 뷰 · 스트리밍 데이터플로**를 이름 대어 물리고
각각의 대가를 적은 뒤, **같은 문장 안에** 자기 대가를 넣는다 — **"일은 작게 유지되고 이미
돌리는 데이터베이스 안에 머문다. 그 대신 누군가는 그 창의 올바른 너비를 골라야 한다."**
174개 회사를 파면서 이런 형태의 문장은 드물었다.

### 정책들이 서로를 무력화한다

보존 정책이 `start_offset` 안의 원시 청크를 지우면 **정정 자체가 불가능해진다.** 계층형이면
정정이 층마다 한 번씩 기다린다. **각각은 합리적인 설정인데 겹치면 조용히 서로를 깎고,
시스템이 그 조합을 검사해 주지 않는다.**

### 두 도메인이 같은 구조다

한쪽은 **읽을 때 몇 개를 열지**, 다른 쪽은 **다시 계산할 때 어디까지 거슬러 갈지**를 정한다.
둘 다 **창을 좁히면 싸지고 넓히면 정확해진다.** 그리고 보존 정책이 원시 청크를 지우면
이쪽의 정정이 막히는 데서 둘이 만난다.

### 다음 사이클 — 후보 조사 (큐가 0/3 이다)

**CAD 축 세 번, 의료 축 여덟 번, 보험 축 한 번 실패.** 못 읽은 곳: Hugging Face ·
Lunit · Blender(도메인 전체 403) · Sourcegraph(도메인 전체 403) · Lemonade(403) ·
dbt Labs · incident.io(404 네 번) · M3 · Guardian · Flexport · Cal.com.
**이 세션의 WebSearch 예산은 끝났다** — 목록 페이지에 href 를 물어 받는 방법만 남았다.
