...
**CyberAgent (JP · 광고) — 45번째. 기능 2개 · 도메인 2개. 열어 둔 도메인이 다 찼다. 큐 잔량 0/3.**

249 에서 **`two-tier-cache`(두 층으로 나눠 읽는다)** 를 썼다. 결정 7 · 그림 3 · 생각 4 · 스택 6 ·
난제 2 · ui 6핀 · 연결 1(`ad-pacing` 과 **추정**으로 이었다).

**📌 원문 재독이 또 옳았다**(223·229·243 에 이어 네 번째). 요약에는 없던 것이 셋 나왔다.
- **버린 대안 둘이 더 있었다** — ③ **Pod 간 조율(분산 singleflight)**: *"Go 의 singleflight 같은 처리를
  Pod 간에 실현하는 이미지입니다"* 라고 설명한 뒤 버렸고 이유가 *"대기나 폴링으로 응답 시간이 다른
  Pod 에 좌우되고 구현도 복잡해집니다"*. ④ **전용 갱신 워커**: *"상주하는 컴포넌트가 늘어나는 데
  더해, 캐시 구조가 바뀔 때마다 worker 도 따라서 릴리스해야 합니다."*
- **TTL 을 나눈 이유가 명시돼 있었다** — *"L2 의 기한 직전에 취득한 값이 L1 에 저장된 경우에도 L1 은
  1분에 실효되므로, Spanner 로부터 취득한 지 **최대 5분 이내**의 값을 반환할 수 있습니다."*
  **층을 늘리면서 신선도 상한을 기존과 같게 유지한 것**이 이 기능의 축이다.
- **Schema Hash 의 정체** — *"구조체의 필드명·타입·직렬화용 메타데이터로부터 Schema Hash 를 자동
  생성해 키에 포함했습니다."* 구조체가 바뀌면 키 공간이 저절로 갈린다. **다만 그날은 새 키 공간이
  비어 있어 여전히 단계적 배포가 필요하다** — **자동화가 문제를 없앤 게 아니라 조건부로 만들었다.**
- **Deal 정의** — *"하나의 Deal 에는 배송 대상이 되는 광고 정보나 타게팅 조건이 연결되어 있습니다."*

**🔴 버린 대안 넷 중 셋이 같은 이유였다 — 컴포넌트를 늘리지 않는다.** 외부 저장소는 *"용도에 비해
구성이 과잉"*, 전용 워커는 상주 컴포넌트가 는다, Pod 간 조율은 구현 복잡도. **성능이 아니라 유지
비용이 이 설계의 기준이었다**(이 정리는 이 사이트의 것이다). 넷째(전부 공유 캐시)만 성능·가용성 이유다.

**🚨 다음 사이클 — 세 번째 도메인을 열지 회사를 닫을지 정한다.**
**세 번째 도메인 후보의 재료가 이미 확인돼 있다** — `광고 배송을 어디서 어떻게 굴릴 것인가`:
**64498**(필터링 고속화 — **2코어에서 약 700 RPS 한계** · vtprotobuf 로 **Marshal CPU 32%→16%** ·
버린 대안 하나 easyproto · 대가 하나: 고부하 시 논블로킹 큐가 상한에 닿아 *"예상 이상의 로그가 결손"*
· *"기간 관계로 대응하지 못했습니다"*) + **61355**(플랫폼 이설 — **버린 대안 넷에 이유가 각각**:
기존 구성 개수 · PSA+VPN(다운타임) · Cloud Run Domain Mapping(헤어핀 라우팅 지연 · Cloud Armor/CDN
불가) · Routes 기반 VPN 직결(Pod CIDR 이 BGP 로 안 퍼진다) · **대가 다섯**: DMS 제약 · Memorystore
직결 곤란 · 쓰기 실패 시 CrashLoopBackOff 위험 · 인증서 최대 72시간 · TCP 프록시는 임시 우회로 ·
**자기 수치 0**, 기간 약 반년뿐).
**⚠️ 둘은 이미 읽었으니 도메인을 열면 그 자리에서 기능까지 쓸 수 있다.** 다만 **둘이 정말 한 도메인인지
먼저 판단한다** — 고속화는 **얼마나 빨리**(코드 수준 처리량), 이설은 **어디서**(인프라 배치)라
**223 의 규칙상 서로 다른 문제일 수 있다.** 그렇다면 **이설 쪽이 버린 대안 넷으로 더 단단하니 그쪽을 연다.**
**네 번째 후보** — 관측 3편(Vector 60707 · Grafana Alert 61628 · Datadog Inferred Services 60208).
**아직 한 편도 안 열었다.**

**IR 에서 아직 못 찾은 것** — **세그먼트별(광고/미디어/게임) 수치**와 **ABEMA 시청자 지표(WAU/MAU)**.
`www.cyberagent.co.jp/en/ir/library/results/` 나 통합보고서(`report.cyberagent.co.jp`)를 열어 본다.
...
**📌 이 회사에서 굳어진 관찰(유지한다)**
- **글마다 밀도가 극단적으로 다르다** — 자기 수치를 내는 글(캐시 65145 · 고속화 64498)과 안 내는 글
  (페이싱 63908 · 이설 61355)이 반반이다. **버린 대안의 수와는 상관이 없다** — 페이싱은 버린 대안이
  셋인데 수치가 0 이고, 이설은 넷인데 0 이다.
- **같은 시스템의 다른 층을 다루는 글들이 서로를 전혀 언급하지 않는다** — 페이싱↔고속화↔이설,
  광고 캐시↔ABEMA 본체 캐시. 후자는 **같은 문제를 각각 겪고 같은 답(Valkey 공유층)에 이르렀는데도** 그렇다.
...
⑨ **`index.json` 의 회사 엔트리는 `domain` 키를 쓴다**(`domains_count` 가 아니다 — 247 에서 고쳤다).
⑩ **새 카테고리는 `validate.py` 의 CATEGORIES 와 `schema.json` 양쪽에 넣는다**(국가 코드와 같다).
⑪ **mermaid 라벨에 수식 기호(=, ×, ÷, 위첨자)를 넣지 않는다** — 말로 푼다(248).
⑫ **요약이 아니라 원문을 다시 읽는다** — 223·229·243·**249** 네 번 다, 요약이 없다던 결정을 원문에서 찾았다.
