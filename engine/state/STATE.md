...
**Meituan(美团, 52번째) `in_progress` — 도메인 3개 · 기능 3개.** 302 에서 **새 도메인 ③
`AI 가 코드를 쓰는 팀을 굴린다` 를 열고**(`tech` 둘 채움, `index.json` 최상위 `domains` 에도
넣음) **기능 `align-people-then-machines`(사람끼리 먼저 맞추고 기계를 맞춘다)까지 썼다.
결정 8개.** 읽은 편은 `/2026/05/07/Agent-AI-Coding.html`.

**무엇이 나왔나** — 평가 플랫폼 코드가 **5만 줄에서 31만 줄**로 불고 팀은 **3배**가 됐는데
**코드의 90% 이상을 AI 가 쓰면서 규범이 없었다.** 31만 줄의 호출 사슬을 사람이 못 훑는다.
- **⭐ 답의 순서** — *"先'人人对齐'，再'人机对齐'"*. 근거는 *"很多团队以为配置好 AI Rule
  就完事了，但真正的瓶颈在人，不在工具"*(진짜 병목은 도구가 아니라 사람이다).
- **분업** — *"AI 很适合帮我们把问题'看全'，但什么问题最重要…还是要由人来判断"*. 사람이 P0·P1
  경계를 긋고 AI 가 전수 조사해 **눈으로 못 찾을 성능 위험 10개**를 짚었다.
- **🔴 해 보고 버린 것** — **테스트 케이스 전자동 생성.** 이유가 *"틀렸다"* 가 아니라
  *"反而增加 Review 负担"*(되레 리뷰 부담을 늘린다)이다.
- **리뷰가 병목이 되자 문을 앞으로 옮겼다** — 제출 전 AI 감사를 통과해야 사람에게 가고,
  사람 리뷰어는 업무 로직만 본다. **상급 모델이 하급을 심사하고 벤더를 교차**시킨다.
- **전용 리팩터링 시간 0일** — 부채를 업무 요구의 곁다리로 쪼갰다. 패키지 **10개 이상** 이관.
- **⚠️ 없는 것** — 비용·토큰 지출·투자 대비 효과 · 부채가 실제로 줄었는지의 사후 지표 ·
  통합 테스트나 점진 배포 이야기.

**⭐⭐ 이 회사의 반복 패턴이 굳었다**(`open_questions` 에 남겼다) — **기준을 하나로 만드는
일이 자동화보다 먼저다.** 평가 편은 **한 명의 중재자**를 세운 뒤에야 기계 채점을 붙였고
(일치율 85~90% 문턱), AI 코딩 편은 **사람끼리 합의한 뒤에야** AI 규칙을 만들었다.
**그리고 회사가 그 연결을 스스로 적는다** — *"管理 AI Coding 与评测 Agent 的底层逻辑一模一样"*.

**⭐ 비교 문서 재료가 또 늘었다** — **문지기를 세우는 축**에서 **Wix 는 컴파일러**를,
**Meituan 은 AI** 를 문 앞에 세웠다. **컴파일러는 못 잡는 범위가 분명한데 AI 문지기는
무엇을 못 보는지가 흐릿하다**(이 대비는 이 사이트의 것이다).

**다음 사이클 — 도메인 ② 를 한 번 더 파거나 새 도메인을 연다. 살아 있는 주소만 쓴다.**
- **도메인 ② 의 남은 글**(홈 href 로 확인) — `/2026/06/29/LongCat-VitaBench-2.0.html` ·
  `/2026/06/12/LongCat-WBench.html` · `/2026/07/24/LongCat-MineExplorer.html`.
  **⚠️ LoHoSearch 처럼 벤치마크 소개 글은 버린 대안이 없을 수 있다** — 열어 보고 판단한다.
- **새 도메인 후보 — 자체 대규모 모델**: `/2026/06/30/LongCat2.0.html` ·
  `/2026/07/12/LongCat-2.0-Open-source.html`(**국산 칩 추론**) · `/2026/04/02/LongCat-Next.html` ·
  `/2026/01/26/EvoCUA.html` · `/2026/07/28/CatPaw-LongCat.html`. **다섯 편이라 두 편 규칙을
  지킬 수 있다.**
- **결정이 5개 안 나오면 Meituan 을 닫는다.** `business_model` 에 적을 패턴이 이미 굳었다 —
  **① 기준을 하나로 만드는 일이 자동화보다 먼저다**(두 도메인에서 반복) · **② 결론에 유효
  기간을 붙인다**(*"这一结论有阶段局限性"*) · **③ 실패와 뜻밖의 결과를 그대로 적는다**
  (LoRA 가 전체 미세조정보다 나았다 / 테스트 전자동 생성을 버렸다 / 클론 실패는 Wix 였고
  여기서는 *"反而增加 Review 负担"*) · **⚠️ 다만 자를 만든 LoHoSearch 편만은 자기 한계를
  한 줄도 안 적는다.**

**그 밖에**
- **🔴 `tech.meituan.com` 은 옛 글을 내린다**(㉑). 2022~2025-05 글과 `/tags/` 목록이 **전부
  404**. **검색이 주는 옛 주소를 믿지 말고 홈(`/`)의 href 만 쓴다.**
...
- **⭐⭐⭐ 비교 문서 축이 셋으로 늘었다.**
  ...
  ② `자를 어떻게 세우는가` — Snap 넷 + Meituan 둘 = **여섯 방식**.
  ③ **`문지기를 무엇으로 세우는가`** — Wix 컴파일러 · Meituan AI 감사 · Wix AirBot 의 타입
  강제 · monday.com Morphex. **③ 이 302 에서 새로 생겼다.**
