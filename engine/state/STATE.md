...
**없다 — Proton 을 완주시켰다(도메인 2 · 기능 2 · `status: done`).** 회사 166개, **큐 0/3.**

### 파는 것의 약점을 자기 도구 설명서에 적어 뒀다

이 회사를 다 파고 나서 가장 놀란 대목이다. 주력 제품이 **브라우저에서 열리는 메일**인데,
자기들이 만든 암호 라이브러리 README 에 **"일반 웹 호스팅으로 배포한 자바스크립트 암호
앱은 설치형 앱보다 사용자에게 낮은 보안을 준다"** 고 적는다. 사후 기록에 자기 한계를
적는 태도와 같은 결이다.

### 공개와 감사는 다른 일이다

회사 오픈소스 페이지는 "전 세계 수천 명의 전문가가 독립적으로 감사했다"고 쓴다. 저장소를
열어 보니 **OpenPGP.js 는 Cure53 이 두 번 봤고 GopenPGP 는 README 에 감사 정보가 없다.**
**라이브러리 문서가 회사 페이지보다 정확했다** — 요약을 믿지 말고 저장소를 열라는 사례를
하나 더 얻었다.

### 암호에서 호환이 깨지면 되돌릴 수 없다

기능이 깨지면 고치면 되지만 **읽을 수 없게 된 메일은 읽을 수 없다.** 그래서 새 규격과 새
AEAD 방식이 좋아도 기본으로 못 켠다 — 플래그 뒤에 둔다. 반대로 GopenPGP V3 는 V2 와
호환을 **깨고** 대신 V2 를 브랜치로 계속 살린다. 같은 회사가 두 자리에서 반대로 골랐다.

### 두 도메인이 같은 선택의 두 얼굴이다

암호를 남의 라이브러리에 안 맡기는 것과 인프라를 남의 클라우드에 안 맡기는 것. 대가도
같은 종류다 — **유지 부담을 계속 지고, 사고가 나면 스스로 골라야 한다.**

### 심은 대비

**페러스 시스템즈**(남의 언어 툴체인을 인증한다) ↔ **Proton**(자기 암호를 남이 쓰게 내놓고
감사를 붙인다) — 둘 다 '내가 옳다'가 아니라 **'밖에서 확인할 수 있다'**를 판다. **1Password**
(설치형 중심) ↔ Proton(웹에서도 열린다) — README 의 경고가 정확히 그 차이를 가리킨다.

### 다음 사이클 — 후보 조사 (큐가 0/3 이다)

**의료 축은 일곱 번째로 비어 있다.** 지난 두 조사에서 확인된 방법은 분명하다 — **접은 곳
목록에서 근거가 "편수"나 "첫 화면 인상"인 것을 다시 연다.** 못 읽은 곳: Lunit(medium 403) ·
Blender(403) · dbt Labs · incident.io(404 네 번) · Guardian · Flexport · Cal.com.
Sourcegraph 는 세 번째 403 으로 목록에서 뺐다.
