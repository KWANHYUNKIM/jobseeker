...
**Duolingo** (US · 교육) — **프로파일까지 썼다.** 도메인 3개, 기능은 아직 0이다. **교육 축 첫 자리다.**

### 이번 사이클 — 신규(6순위), Duolingo 프로파일

⚠️ **무료 사용자가 압도적으로 많은 구조가 기술 요구를 정한다** — 매일 **5,870만 명**이 들어오는데 돈을 내는 건 **1,270만 명**이다. 그러면 **한 사람당 계산이 아주 싸야 하고 동시에 내일 다시 오게 만들어야 한다.** 그 둘을 한꺼번에 푸는 자리가 **다음 문제를 고르는 일**이고, **머무르게 하는 일이 곧 매출이다**(재구성).

세운 도메인 셋:
- **언제 무엇을 다시 보여 줄지 고른다** — 회사가 이 문제를 **논문으로 낸다**(2016 간격 반복·2018 제2언어 습득 모델링·2020 언어 평가). ⚠️ **그런데 확인한 것이 10년 전 논문까지다** — 매출의 86% 가 걸린 축인데 **지금 무엇으로 고르는지 1급 자료를 못 찾았다.**
- **에이전트를 한 번 정의해 여러 곳에 태운다** — ⚠️ **여기 이르기까지 세 번 갈아엎었고 연도가 다 붙어 있다**(로컬 MCP 2024-11 → 중앙 설정 생성기 2025-05 → HTTP MCP 서버 2025-08 → 슬랙 앱). **버린 이유가 각각 다르다** — 설정이 고통스럽다 · 취약하다(npm 버전 불일치·좀비 프로세스) · **`even one click is still too much`**.
- **테스트를 사람이 아니라 파이프라인이 쓴다** — ⚠️ **값은 성과가 아니라 부딪힌 벽에 있다**(리뷰 병목 · 린트 못 돌려 실패율 13.6% · **자동 수정 에이전트가 프로덕션 파일을 고친 사고**).

### 다음 사이클 — 확장

**에이전트 도메인이 가장 익었다** — 버린 대안 셋에 이유가 붙어 있고 **연표를 세울 연도가 다 있다.** 테스트 도메인도 자료가 충분하다.

⚠️ **학습 도메인은 자료부터 찾아야 한다** — Fastly 보안 때와 같은 모양이다(매출 큰 축인데 1차 자료가 얇다). **그 차례가 오면 기능을 쓰기 전에 찾는다.** 찾을 곳: `research.duolingo.com` 의 개별 논문 · 회사 블로그의 학습 알고리즘 글.
