...
**없다 — Airbnb 완주.** 도메인 3/3 · **기능 3개.** 회사 **98개.** ⚠️ **큐 1/3**(Pinterest).

### 이번 사이클 — `저장은 남에게 맡기고 로직은 내가 쥔다`

건진 것 넷:

- ⚠️ **관리형인데 운영이 남았다** — 3자 PaaS 를 떠난 이유 중 가장 이상한 것: **`최적 성능을 유지하려면 주기적인 수동 인스턴스 재부팅이 필요했다.`** **남에게 맡긴 값을 치르면서 운영도 하고 있었던 셈이다**(재구성). 게다가 **벤더의 질의 최적화기가 내는 실행 계획을 고칠 수 없다** — **문제를 알아도 손댈 수 없는 자리가 남는다.**
- ⚠️ **떠나되 어디까지 떠날지를 계층으로 갈랐다** — **JanusGraph 의 플러그형 구조로 저장을 DynamoDB 에 맡기고 `그래프 로직 계층에 대한 완전한 통제를 유지했다`.** **분산 저장을 다시 만들지 않고 AWS 의 확장성을 쓰면서 빠르게 반복한다.**
- ⚠️ **로직을 쥔 값이 곧바로 나왔다** — **JanusGraph 의 무거운 기본 잠금을 DynamoDB 조건부 쓰기로 갈아 끼웠다.** **저장을 고를 수 있으면 그 저장의 원시 기능으로 상위 계층의 무거운 부분을 대신할 수 있다**(재구성) — **벤더 PaaS 였다면 손댈 수 없는 자리다.** 쓰기 P95 **353ms → 156ms**, 용량 **10배.**
- ⚠️ **엔진만 고쳐서는 안 됐다** — **`느리고 배치되지 않는 백엔드 질의로 폴백하는` Path 단계**를 클라이언트 질의에서 걷어내야 했다. **이전이 서버 팀만의 일이 아니게 된다**(재구성).

**수치** — **70억 노드 · 110억 엣지 · 하루 500만 새 엣지** · 읽기 **32~93% 개선** · 종단 **P95 2.1초 → 1.0초 · P99 5.0초 → 2.5초** · **수동 재부팅 소멸.**

### ⚠️ 성과를 문제와 같은 자리에서 말한다

**원래 문제가 `롱테일 지연` 과 `주기적 재부팅` 이었으니 성과도 P95·P99 와 운영 부담으로 말한다.** ⚠️ **평균으로 말했다면 문제를 말하지 않은 것이 된다**(재구성).

### ⭐ 이 회사의 세 기능이 한 방향을 가리킨다

**빌려 쓰던 것을 하나씩 자기 것으로 바꾼다** — 관측은 **벤더 기반에서 오픈소스 스택으로**, 데이터 구조는 **한 제품용에서 여러 제품용으로**, 신원 그래프는 **벤더 PaaS 에서 자체 플랫폼으로.** ⚠️ **그리고 셋 다 `어디까지 직접 할지` 를 명시적으로 갈랐다** — vmagent 는 **고칠 수 있는 크기**라서, 데이터는 **도메인마다 고르되 섞지 않게**, 그래프는 **저장은 빌리고 로직은 쥐게.** ⭐ **회사 하나를 세 사이클에 걸쳐 파야 보이는 결이다**(재구성).

### 다음 사이클 — 신규(6순위)

`--gaps` 가 **큐 맨 위(Pinterest)를 `in_progress` 로 만들고 3단계부터** 를 부를 것이다. ⚠️ **큐가 1/3 이라 그 뒤에 후보 조사가 걸린다.**
...
**③ `자기 성과를 어디까지 주장하나` 6곳** — Snyk / Grafana Labs / ScyllaDB / Razorpay / Flipkart / Airbnb.
...
**⑨ `한 번에 갈아엎을 것인가 목 졸라 죽일 것인가`** — Twilio(두 방향) / Etsy / Plaid / Paystack / Airbnb.
...
**⑬ `애매할 때 어느 쪽으로 넘어지나` 5곳** — Flipkart(둘) / Razorpay / Zepto / Paystack / Airbnb.

**⑭ `검색 관련성을 누가 정하나`** — Etsy / Flipkart / ⏳ Zepto / ⏳ Pinterest.

**⑮ `언제 쪼개고 언제 합치나`** — Twilio / DoorDash / Deliveroo / ScyllaDB / Airbnb(규칙을 준 유일한 사례) / ⏳ Pinterest.

**⑯ ⭐ `빌려 쓰던 것을 언제 자기 것으로 만드나` — 재료가 찼다** — **Airbnb**(관측·그래프를 벤더에서, **어디까지 빌릴지를 계층으로 가른다**) / **ScyllaDB**(남의 SDK 를 감싸되 안에서 갈라진다) / **Plaid**(관리형 Aurora 를 떠나 TiDB 로) / **Cygames**(반대로 관리형 TiDB Cloud 를 골랐다) / **Twilio**(포크를 버렸다 — Airbnb 의 Veneur 와 같은 이유).
