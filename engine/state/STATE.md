...
**없다 — Restate 를 완주했다(도메인 2 · 기능 2).** 큐 1/3 이라 다음 사이클은 **후보 조사**다.

### 감출 수 없는 것만 수치로 적는다

이 회사에 대해 두 사이클 내리 **'정량이 없다'** 고 적었는데, 지역 복제 글에는 **있다** —
**P99 지연이 약 30초까지 치솟고**, 넘김 창은 **1분 미만**이며, 넘긴 뒤에도 **P50 이 약간
높아진 채로 남는다.** 자기에게 불리한 쪽이다. **넘어가는 동안의 지연은 응용에 그대로
보이니 감출 수가 없다.** 반대로 아직 안 적은 것들(엔진 지연·처리량·저널 크기·운영 비용·
RPO)은 **바깥에서 안 보이는 것들**이다. **'정량이 없는 회사'라고 뭉뚱그리면 이 선이
사라진다** — open_questions 에서 범위를 좁혀 적었다.

### 상태의 원본을 없애면 어긋날 것이 없다

**'로그가 근거다.'** 상태는 어디에도 원본으로 없고 처리기가 **임시 저장소 위 RocksDB** 로
되살린다. 그래서 **노드를 잃는 일이 사고가 아니라 평범한 일**이 된다. 팔로워도 물화는
하되 **호출은 리더만 일으킨다** — 상태는 따라 만들면서 부수효과는 하나만 낸다.

### 위층의 규칙이 아래층의 비용을 정한다

위층이 **걸음마다, 실행 전에** 적기로 했으므로 아래층의 **동기 쿼럼 왕복이 걸음마다** 붙는다.
**위층에서 없앤 조율의 값이 아래층에 그대로 남아 있다** — 두 도메인을 따로 읽으면 안 보이는
자리다.

### 확인이 아니라 시간으로 푼 곳

S3 지역 간 복제가 비동기라 스냅샷이 닿았는지 모르는 채로 로그를 자르면 근거가 사라진다.
**60분을 기다린다** — 확인하지 않고 시간으로 넉넉히 잡는다.

### 미완을 미완이라 적는다

분산 배치가 **'정말 어렵다'** 고 말하고 **'1월에 첫 버전'** 을 약속하는 데서 멈춘다.
**앞으로 될 것을 된 것처럼 쓰지 않는다** — 이 원장에서 자주 못 보는 태도다.

### 다음 사이클

**후보 조사**(큐 1/3). 지속 실행 축은 템포럴·인지스트에 이어 셋이 됐다.
