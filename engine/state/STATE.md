...
**Airbyte — 프로파일만 썼다(도메인 하나, 기능 0).** 회사 162개, **큐 0/3**.

### 중간에 서 있는 것이 곧 값이다

파는 것이 **커넥터 600개 이상**이니 성능 문제는 한 곳이 아니라 **모든 커넥터가 지나는 길**에
있다. 3판에서는 데이터가 소스에서 **오케스트레이터를 거쳐** 목적지로 표준 입출력 파이프를 타고
흘렀는데, 4판은 **유닉스 도메인 소켓으로 직결**한다.

### 몇 초만 사는 파일에는 다른 저울을 쓴다

스테이징 형식을 CSV+gzip 에서 Avro+Snappy 로 바꾼 이유가 좋다 — **파일이 조금 커지는 대신
압축·해제가 훨씬 싸고, `COPY INTO` 전에 몇 초만 사는 파일에서는 압축률보다 처리량이 이긴다.**
같은 형식 선택이 오래 보관하는 파일이었다면 반대로 갔을 것이다(재구성).

### 새 컬럼 하나가 전부를 다시 처리하게 만들면 안 된다

3판의 **전체 테이블 소프트 리셋**을 버린 이유다 — 수백만 행 테이블에서 컬럼 하나 때문에 시간과
데이터브릭스 연산 값이 함께 들었다. 10GB 기준 처리량은 **9.54 → 42.47 MB/s(4.5배).**

### 도메인을 하나만 세웠다

커넥터 600개를 **만들고 유지하는 방법**(생성 도구·시험 체계)을 다룬 글을 못 찾았다 — 먹스·
파이어웍스·프레임워크에서 한 판단을 이어 쓴다.

### 다음 사이클 — 확장 (그 하나뿐인 도메인)

`소스에서 목적지로 옮기는 길을 짧게 만든다`. 물린 구조·형식 맞바꿈·스키마 처리·수치가 한 글에
다 있다. **⚠️ 오케스트레이터가 하던 일이 어디로 갔는지는 글에 없다** — 재구성으로 적는다.

### ⚠️ 큐가 0/3 이다

Airbyte 를 완주시킨 뒤 **후보 조사 사이클을 끼운다.**
