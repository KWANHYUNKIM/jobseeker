...
**없다 — 문체 보수 사이클이었다.** Oxide Computer 를 배민 문체로 다시 썼다(0.96 → 0.0). 회사 115개, 큐 2/3. 7곳 남았다.

### 사업 모델과 기술 구조가 같은 문장인 회사

**벤더에 티켓을 열고 기다리지 않으려고 층을 다 갖는다** — 이 한 문장이 파는 방식(랙 한 대를 통째로,
구독 없이)과 기술 선택(BMC 도 운영체제도 직접)을 동시에 설명한다. 그래서 도메인을 제품이 아니라
**층을 다 가져서 할 수 있게 된 일**로 잘랐다.

### 표준 답을 안 쓰기로 한 회사들 중 여기가 가장 아래층이다

- 사이보우즈 — 상태 있는 워크로드의 표준(StatefulSet)을 멈춤 때문에 버렸다.
- 터소 — 물려받을 수 없는 테스트 스위트 때문에 데이터베이스를 통째로 다시 썼다.
- Oxide — **BMC 를 다시 만든다.** 완전히 감사 가능한 보안 사슬을 팔려면 맨 아래 한 칸이
  블랙박스이면 안 되기 때문이다.

운영체제까지 직접 만든 이유도 적혀 있다 — 기존 운영체제에서 원한 것이 그것들이 주는 것도, 주려던
것도 아니었다. 그리고 **러스트 같은 메모리 안전 언어에서도 메모리 보호는 필수**라며 태스크와
드라이버를 분리된 보호 영역에 둔다 — 언어가 막아 주는 것과 구조가 막아 주는 것을 가른 자리다.

### 층마다 박힌 기본값 — 옮기는 것보다 붙어 있던 것

산산은 저장소를 옮기며 CI/CD 설정이 더 힘들었다고 적었고, DoorDash 는 코드가 아니라 기능을 옮기는
것이 어려웠다고 적었다. Oxide 의 1500 도 같은 모양이다 — 게스트에게 값을 알리는 경로, 드라이버,
캡슐화 데이터 경로, 운영자 스위치까지 **네 층에 같은 숫자가 따로 박혀 있었다.**

그리고 켜는 것보다 무서운 실패가 있다 — 끝에서 끝까지 켜져 있지 않으면 **TCP 악수는 끝나고 그다음
애플리케이션이 멈춰 아무것도 못 보낸다.**

### 다음 사이클 — 문체 보수 (사다리 3순위)

--gaps 가 정한다. 지금 맨 위는 wix(0.95, 414개)지만 그건 이 시점의 예상이지 확정이 아니다.
