...
**Zig — 기능 셋.** 남은 빈 도메인은 표준 라이브러리 하나. 회사 138개, 큐 0/3.

### 언어를 정하는 일은 안 만들 것을 고르는 일이다

릴리스 노트에 넣은 것만큼 **안 넣은 것과 그 이유**를 적는다. `@Float` 은 실행시 부동소수
타입이 다섯뿐이라, `@Array` 는 보통 배열 문법으로 되니까, `@Opaque`·`@Optional`·`@ErrorUnion` 은
문법을 그냥 쓰면 되니까, `@ErrorSet` 은 언어를 단순하게 하려고 재구성 자체를 없앴다.

만능이던 `@Type` 은 목적별 여덟 개로 쪼갰다 — 근거가 실사용이다: **흔한 작업에 쓰기가
어설펐다.**

### 표현할 수 없는 것은 만들 수 없게 한다

packed 안의 포인터를 금지한 이유가 분명하다 — 바이트 정렬이 아닌 포인터를 담은 상수는 대부분의
이진 형식으로 표현할 수 없다. packed 유니온의 남는 비트도 막고, 바탕 타입을 추론하는 타입은
extern 에서 못 쓰게 했다: **남의 ABI 와 만나는 자리에서 암묵을 허용하지 않는다.**

### 멀쩡하던 코드가 깨지는 것을 감수한다

의존 순환 규칙을 단순화해 예전에 통과하던 코드가 이제 순환으로 잡힐 수 있다. 그 값을 내고
얻은 것은 **왜 순환인지가 더 분명해진 것** — 명세의 어려움을 줄이는 쪽에 무게를 둔다.

### 세 기능이 한 손버릇을 가리킨다

비동기를 키워드로 안 넣고 인자로 풀었고(1), 감싸개를 안티패턴이라 부르며 지웠고(2), 만들 수
있는 기능도 안 만든 이유를 적는다(3). **길을 열어 두면 그 길로 간다**는 판단이 세 자리에서 같다.

### 다음 사이클 — 확장 (Zig 의 마지막 빈 도메인)

`표준 라이브러리를 직접 쓴다`. 자체 deflate(zlib 대비 9.7% 빠르고 압축률 1% 손해)와 NtDll
이전, LLVM 회귀 우회가 손에 있다.
