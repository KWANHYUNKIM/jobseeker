...
**없다 — 카카오페이를 완주했다(도메인 2 · 기능 2).** 큐 0/3 이라 다음 사이클은 **후보 조사**다.

### 같은 회사가 같은 도구를 두 방향으로 쓴다

레거시를 걷어낼 때는 **AOP 로 감추고**(코드를 안 건드리고 옆에서 견준다), 로깅·캐시·
트랜잭션에는 **AOP 를 버리고 코틀린 후행 람다로 드러낸다.** 앞은 **한시적이고 코드를 안
건드려야** 하고, 뒤는 **상시이고 코드가 알아야** 하기 때문으로 보인다 — **목적이 도구를
정한다.**

### 함정을 없앤 것이 아니라 바꿨다

AOP 는 **모르게 안 걸리는** 함정이 있고(프록시가 `this` 호출을 못 잡는다), 람다는
**잊어서 안 감싸는** 함정이 있다. 다만 **뒤엣것은 코드를 읽으면 보인다** — 그 차이로 골랐다.

### 무엇을 얼마나 내주는지 적는 회사가 늘고 있다

**처리량 20% 감소 · 평균 190→233ms · 정점 P99 548→1,305ms · CPU·힙·스레드 상승**을 표로
낸다. 트리거닷데브가 **런타임 교체의 대가를 성과보다 길게** 적은 것과 같은 결이다.

### 시작하는 법은 있는데 끝내는 법이 없다

**Shadow 비교를 언제 끄는지**가 어디에도 없다. 20%를 **얼마 동안** 감수하는지, 무엇을 보고
**'이제 됐다'** 고 하는지가 빠져 있다 — 이 원장에서 **끝내는 조건을 안 적는 것**이 반복된다.

### '같다'를 기계가 정할 수 없다

**BigDecimal `100.0` ≠ `100.000`**, 컬렉션 순서. 비교 자동화가 **도메인 지식 앞에서
멈춘다** — 정규화 규칙은 사람이 도메인마다 다시 넣어야 한다.
