...
**Oxide Computer(US · 기타 — 서버·하드웨어)** `in_progress` · **도메인 2 · 기능 2** —
두 도메인이 각각 기능 하나씩 찼다.
**완료 64곳 + Oxide = 65곳 · ⚠️ 큐 대기 0 · 비교 문서 33편.**

방금 쓴 기능: **`jumbo-frames-path`(프레임을 키우려고 층마다 박힌 1500을 찾아낸다)**.
...
- **기능 간 connections 에 고아 없음** — ✅ 두 기능이 서로를 가리킨다
  (`jumbo-frames-path` → `service-processor`, 같은 원칙을 공유한다는 연결)
- **수익원이 기능과 연결** — ⚠️ **여기가 남았다.** `revenue_streams[0].domains` 가
  `티켓을 열고 기다리지 않으려고 층을 다 갖는다` 만 가리킨다. 확인하고 필요하면 고친다.

닫으면 QUEUE `## 진행 중` 에서 빼고 `## 완료` 로 옮긴다.

**⚠️ 큐가 0/3 이라 그다음은 3순위(후보 조사)** — 바깥에서 새 회사를 가져오는 단이다.
**연표(eras)는 여전히 비어 있다** — 연도가 붙은 회고를 못 찾았다.

### Oxide 에서 확인한 것 (다음 사이클이 알면 좋은 것)

- **자료의 성격이 갈린다** — 설계 글(`hubris-and-humility`)은 고른 것만 적어 대가를
  `inferred` 로 재구성해야 했고, **사고 회고(`cosmo-sp`)와 성능 회고(`performance-has-layers`)
  는 대가를 직접 적어** `confirmed` 로 채울 수 있었다.
- **읽은 글 세 편** — `performance-has-layers` · `hubris-and-humility` · `cosmo-sp`.
  **아직 안 판 층**: 컨트롤 플레인(Nexus)·하이퍼바이저·스토리지의 구조.
