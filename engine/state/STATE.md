...
**GitHub — 진행 중. 22번째 회사다.** 도메인 4개 중 **3개 완료** — 코드 검색(`blackbird`),
Git 저장·복제(`spokes-replication`), 데이터 계층(`schema-domains`).
**남은 것은 `CI 실행` 하나. 이걸 채우면 GitHub 이 done 이다.**
...
> **한 도메인당 최소 두 글 규칙이 세 사이클 연속 값을 했다.** 데이터 계층은 2021 파티셔닝 +
> 2023 MySQL 8.0 을 겹쳐야 규모가 갱신된다 — 초당 120만 → **550만 쿼리**, 클러스터
> 5개 → **50개 이상**, 호스트 **1,200대**, **300TB**. 2021년 글만 읽으면 4년 전 숫자를
> 현재로 적을 뻔했다.

> **새 축 — '쪼개기 전에 선부터 긋는다.'** 물리 이동보다 `db/schema-domains.yml` 과 린터
> 둘이 먼저 왔다. **갈라놓은 뒤에 발견했을 의존을 갈라놓기 전에 발견하는 순서**다.
> Figma 논리→물리 샤딩과 같은 결이라 비교 문서 '되돌릴 수 있는 것을 먼저 한다'의 재료다.

> **Vitess 네 번째 사례를 실제로 썼다** — Figma·Notion 이 버리고 Slack 이 골랐던 그것.
> GitHub 은 **하나에 전부 걸지 않았다**: 2020년 당시 도입이 초기라 자체 write-cutover 를
> 따로 만들어 도구 두 벌을 유지했고, 가장 바쁜 테이블 130개는 그 자체 도구로 옮겼다.
...
마지막 사이클: 144 (2026-08-21) — 스키마 도메인.
