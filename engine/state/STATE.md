...
**Antithesis — 도메인 셋을 다 채우고 done 으로 닫았다.** `deterministic-hypervisor` ·
`ordinary-workload-rare-race` · `multiverse-debugging`. 회사 119개, 큐 1/3.

완주 기준 셋을 다 넘겼다 — 도메인마다 기능 1개 이상, 기능 셋이 한 줄로 이어져 고아가 없고,
수익원이 기능과 닿는다.

### 관측 축의 아홉 번째 답 — 남기지 않고 다시 만든다

이 엔진에서 관측 비용을 다룬 회사가 여덟이었다(VictoriaMetrics · Honeycomb · Datadog ·
Razorpay · Databricks · CyberAgent · Paytm · monday). **여덟 다 무엇을 얼마나 남길 것인가로
풀었다.** 안티테시스는 그 질문 자체를 지운다 — 세계를 다시 만들 수 있게 해 두고 계측은 나중에
켠다. 실패 직전으로 되감아 네트워크 덤프를 처음부터 잡고 있던 셈으로 만들고, 결함 주입을 끄고
같은 지점을 다시 밟아 변인을 가른다.

회사가 문제의 뿌리를 한 문장으로 짚는다 — **미래의 내가 무엇을 알고 싶어 할지를 지금 알아맞혀야
한다는 것.** 로그를 늘리는 것은 답이 아니라 그 문제를 미루는 방법이라는 뜻이다(재구성).

### 코어 하나라는 값을 두 번 회수한다

하이퍼바이저에서 치른 대가(시스템 하나가 물리 코어 하나)가 탐색에서 한 번, 되감기에서 또 한 번
회수된다. 스케줄을 이쪽이 정하니 드문 순서를 골라 밟을 수 있고, 같은 시드가 같은 지점으로 데려가니
계측을 나중에 켜도 그 실행이 달라지지 않는다.

### 세 글의 정직함이 고르지 않다

하이퍼바이저 글은 카운터가 1조 명령어에 하나 어긋난다고 적고 원인을 모른다는 것까지 밝혔다. 탐색
글은 못 찾는 조건을 보였다. 그런데 **디버깅 글은 할 수 있는 일만 열거하고 한계를 적지 않는다.**
같은 회사 안에서도 글마다 이렇게 갈리는 것을 그대로 남겼다.

### 다음 사이클 — 신규 (Dolt)

큐가 1/3 이다. 대기에 Dolt 하나가 남았으므로 그것을 판 뒤 후보 조사를 끼운다.
