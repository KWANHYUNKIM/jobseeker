...
**없다 — Render 를 완주했다(도메인 2 · 기능 2).** 큐 0/3 이라 다음 사이클은 **후보 조사**다.

### 고침이 값을 했다고 나중 사건으로 보인다

**2022년 1월**에 DNS 가 제어 평면과 함께 죽어 갈라 놓았고, **2024년 5월에 제어 평면이 다시
무너졌을 때 그 영향이 갇혔다.** **고쳤다고 쓰는 것과 고침이 값을 했다고 보이는 것은
다르다** — 이 원장에서 후자를 보인 회사는 드물다.

### 상태가 복구 시간을 정한다

같은 사고인데 **무상태는 14분, 디스크 달린 것은 3시간 53분**이었다. 개별 디스크 작업은
**'길어야 몇 분'** 이었는데 **수십만 개가 한꺼번에**라 몇 시간이 됐다. 그래서 고침이
**개별을 빠르게 하는 것이 아니라 배치로 묶는 것**(처리량 한 자릿수 향상)이었다.

### 안전장치를 늘리면 검증해야 할 것도 늘어난다

코드를 **기능 플래그 뒤에 두고 시험했는데**, **그 플래그 코드의 미묘한 버그가 문제 코드를
비운영에서 한 번도 안 돌게 막았다.** **통과한 시험이 아무것도 시험하지 않았다.** 고침은
시험 인프라 표준화인데 **플래그 자체를 어떻게 검증할지는 안 적는다.**

### 기술 원인 밖의 잘못까지 쓰는 회사가 둘이 됐다

렌더는 **공개까지 12분**과 **'「부분 장애」나 「전체 장애」 대신 「성능 저하」를 잘못
사용했다'** 를 적고, 트리거닷데브는 옆 지역으로 보낸 판단을 **'큰 실수였다'** 고 적는다.
**장애와 대응을 나눠 적는 회사**를 이 원장이 둘 갖게 됐다.

### 다만 렌더는 보상을 안 적는다

트리거닷데브가 **계정당 최소 $10 크레딧과 전액 환불**을 적은 것과 달리, 렌더의 RCA 에는
**영향 범위 비율도 금전적 영향도 보상도 없다.** open_questions 에 남겼다.
