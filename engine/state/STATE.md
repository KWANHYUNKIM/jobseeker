...
**없다 — 후보 조사 사이클이었다.** 큐를 **0/3 → 2/3** 으로 채웠다. 다음 사이클은 `--gaps` 가 **신규(6순위)**를 부른다 — 대기 맨 위는 **Paystack** 이다.

### 이번 사이클 — 후보 조사 (스물한 번째)

⚠️ **앞 사이클에 세운 되짚기 규칙이 그대로 통했다** — `브라우저로 열고, 검색으로 현재 주소도 받는다`.

- **✅ Paystack (❌ → ✅)** — ⚠️ **주소가 바뀐 두 번째 사례다.** 접근 지도에 **`blog.paystack.com` 403** 이라 적혀 있었는데 실제로는 **`paystack.com/blog/category/engineering-design`** 다. **⭐ 아프리카 축 첫 자리**다 — 87개 회사 중 하나도 없었고, 앞서 `아프리카 — 두 번째로 빈손` 이라 적었던 자리다.
- **✅ Deliveroo** — 새 이름. 최신 글이 **2026-02-27**(`AI Hub` · **Governance as Code**). ⏳ 본문은 안 읽었다.
- **⏸️ Snyk** — 제품 발표 성격이 강한데 **`New Agentic Architecture for Snyk Agent Fix` 는 결이 다를 수 있다**(버린 방식이 명시적이다). **다음에 그 글을 열고 결론을 낸다.**
- **❌ Delivery Hero** — 검색이 자체 기술 블로그를 주지 않는다.

### ⚠️ 이번 사이클의 가장 큰 발견 — 같은 마감에 갈린 답

**Plaid 를 움직인 것과 같은 외부 마감이 Paystack 도 움직였다** — **아마존이 2024년 10월 Aurora MySQL v2(5.7 기반) 지원 중단을 발표**했다.

| 회사 | 같은 마감 앞에서 | 결과 |
|---|---|---|
| **Plaid** | ⚠️ **플랫폼을 갈아탔다** — `어차피 큰 업그레이드가 필요하니 그 노력을 이전에 쓴다` | 자체 호스팅 TiDB · 6명이 2년 반 · **관리형을 떠난 대가**(벤더 버그가 내 일이 됐다) |
| **Paystack** | **업그레이드를 해냈다** — MySQL 8.0 · 블루-그린 · 명확한 롤백 | **11개 프로덕션 클러스터 · 무사고** · 3자 자료로 54만 달러 절감·100% 가동률 |

⚠️ **DB 이전 축에 이제 세 곳이 찼다** — Etsy(샤딩 추상화 · Vitess) · Plaid(분산 SQL · TiDB) · Paystack(제자리 업그레이드 · MySQL 8.0). **같은 문제(관리형 MySQL 의 한계와 마감)에 세 가지 다른 답이다.** **AI 에이전트 축에 이어 두 번째 비교 문서 재료이고, 이쪽이 더 선명하다.**

### 다음 사이클 — 신규(Paystack)

⚠️ **글이 드문드문한 회사다** — 시스템 주제로 묶이는 것은 **DB 이전**과 **SAST 취약점 관리** 둘뿐이고 나머지는 커뮤니티·라이브러리·디자인 글이다. **도메인을 크게 잡지 않는다.**
