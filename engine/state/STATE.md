...
**Electric — 프로파일만 썼다(도메인 둘, 기능 0).** 회사 158개, **큐 0/3**.

### 파는 것이 '읽기를 데이터베이스 밖으로 옮기는 방법'이다

내세우는 세 숫자가 곧 제품의 약속이다 — **동시 독자 100만 이상, CDN 캐시 적중률 99%, 규모가
커져도 데이터베이스 부하는 평평하게.** 여럿이 같은 데이터를 실시간으로 보는 앱에서 무너지는
것은 읽기이고, 그것을 CDN 으로 흡수한다.

### 그 아래에서는 내구성과 속도가 부딪힌다

Durable Streams 글이 그 자리다. **카프카식 비동기 복제를 물리고 단일 노드 fsync** 를 고른 뒤
값을 **그룹 커밋 WAL** 로 눅였다. 데이터는 **전선에 실리는 형식 그대로** 저장하고
`sendfile(2)`·`splice(2)` 로 커널 공간에서 내보내 복사와 문맥 전환을 없앤다 — 4vCPU 에 스트림
10만 개로 **초당 약 86만 append**, 메모리 중앙값 515MB(노드 구현은 1만 스트림에 800MB).

### ⚠️ 클라우드를 접는 회사를 프로파일에 그대로 적었다

2026-08-11 데이터브릭스 합류와 함께 **Electric Cloud 를 접는다** — 쓰던 사람은 자체 호스팅하거나
다른 제공자로 옮겨야 한다. 오픈소스는 Apache 2.0 으로 남는다. `revenue_streams` 에 "서비스 종료
예정"이라고 그대로 적었다.

### 국가를 `기타` 로 뒀다

홈에 `© 2026 Electric DB Inc.` 만 있고 위치가 없다. **추측해서 US 나 EU 로 적지 않고** `기타` 로
두고 `open_questions` 에 남겼다 — 소스헛에서는 홈 하단의 법인 정보로 EU 를 확인했는데 여기는
그 자료가 없다.

### 다음 사이클 — 확장 (첫 도메인)

`읽기를 데이터베이스 밖으로 밀어낸다`. **⚠️ 주장들(100만·99%·평평)의 측정 조건이 홈에 없다** —
`Electric 1.1 새 저장 엔진` 글을 읽어 근거를 보태거나, 없으면 없는 대로 `limits` 로 적는다.

### ⚠️ 큐가 0/3 이다

Electric 을 완주시킨 뒤 **후보 조사 사이클을 끼운다.** `blocked` 목록 — Ghostty · incident.io ·
Sourcegraph · Guardian · Flexport.
