...
**없다 — 문체 보수 사이클이었다.** Plaid 를 배민 문체로 다시 썼다(2.3 → 0.0). 회사 115개, 큐 2/3. 52곳 남았다.

### 낡은 것을 바꾸는 방법에 다섯 번째 답이 왔다

Plaid 는 데이터베이스를 통째로 갈아 끼우면서 기한을 먼저 못 박았다. 전사적으로 2년을 넘기지
않는다는 것이 설계 제약이었고, 되돌아갈 길도 설계에 넣었다.

이 축에 이제 다섯이 모였다.

- Twilio 는 이중 쓰기로 한 조각씩 옮긴다.
- Klaviyo 는 무중단으로 이관했다.
- Atlassian 은 색인 변경을 일부러 느리게 돌렸다.
- Modal 과 Turso 는 처음부터 다시 지었다.
- Plaid 는 기한을 먼저 못 박고 되돌아갈 길을 설계에 넣었다.

앞의 셋은 옮기는 쪽, 넷째는 다시 짓는 쪽인데 Plaid 는 옮기되 시간을 제약으로 삼은 자리다.
이유가 사업에 있다. 매년 엔지니어 2명이 한 해 내내 매달릴 분량을 옛 한계를 우회하는 설계에
쓰고 있었으니 이전이 길어지면 그 값을 두 번 내게 된다.

### 이번 사이클 — Plaid

사업 성격이 신뢰성 요구를 정한다는 것을 앞세웠다. 위에 얹힌 앱들이 자기 사용자에게 서비스
수준을 약속하므로 이 회사는 그보다 한 자리 더 엄격한 값을 스스로에게 건다. 데이터베이스
계층에 99.995% 를 요구하는데 구글 관행대로 9 를 하나 더 붙인 것이다.

기업가치가 2021년 134억에서 2025년 61억으로 내려앉았다가 2026년 2월 80억이 됐다는 것도
남겼다. 기술 판단을 읽을 때 그 시기의 압박을 함께 봐야 한다.

영문은 셋뿐이었다. 속도보다 정확성을 앞에 둔다는 문장을 풀어 넣었다.
