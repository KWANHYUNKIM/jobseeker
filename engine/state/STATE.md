...
**없다 — 이번 사이클은 보수였다.** ⚠️ **후보 조사를 하려다 중복 등록을 발견해 그것부터 고쳤다.** 회사 **95 → 94개**(중복 하나 제거).

### ⚠️ 이번 사이클 — 내가 만든 결함을 내가 고쳤다

**Grafana Labs 가 두 번 등록돼 있었다.**

- **2026-09-04** — `grafana-labs` slug 로 **이미 완주**했다(기능 2: `mimir-next-gen` · `query-sharding`, **eras 2시기까지 세워 둔 상태**).
- **2026-09-06** — 후보 조사에서 그것을 못 보고 **`grafana` 로 새로 올려 팠다**(기능 2: Mimir · Loki).

⚠️ **더 나쁜 것은 같은 자료로 기능을 두 번 썼다는 점이다** — **`mimir-next-gen`("자기가 고른 구조를 이름 붙여 부정하기")과 `let-the-log-remember-not-the-replicas`("사본을 세 벌 두는 대신 로그가 기억하게 한다")가 같은 Mimir 차세대 아키텍처 글에서 나왔다.** PROMPT 1단계의 **`같은 기능을 두 번 쓰지 않는다`** 를 어긴 것이다.

**처리** — **slug 는 한 번 정하면 바꾸지 않는다**(공개 URL)는 규칙대로 **먼저 있던 `grafana-labs` 를 정본으로 삼고**, 겹치지 않는 **로그(Loki) 기능만 그쪽으로 옮겼다**(도메인 이름도 정본의 결에 맞춰 `로그 수집·저장` 으로). **중복 파일과 index 항목을 지우고**, 삭제된 기능을 가리키던 connections 를 `mimir-next-gen` 으로 고쳤다. **index 의 죽은 도메인 이름 하나도 함께 정리했다**(297 → 296).

### ⚠️ 왜 못 봤나 — 다음 사이클을 위한 규칙

**후보 조사에서 `grafana` 라는 슬러그로만 index 를 조회했고 `grafana-labs` 를 못 찾았다.** ⚠️ **슬러그가 다르면 이름이 같아도 안 걸린다.** **앞으로 후보를 올릴 때는 슬러그가 아니라 `name_en` 으로 대조한다.** 이번에 전수 점검을 돌렸고 **다른 중복은 없다**(회사 파일 94개).

### 아직 안 한 일 — 다음 사이클의 거리

- ⏳ **보강: 정본의 `mimir-next-gen` 과 이번에 확인한 인용을 대조한다** — **끊김 없는 소비(gapless consumption) · 복제 3→2·쿼럼 2→1 · WarpStream 과 교차 AZ 전송 GB당 0.02달러 · `더 확장성이 좋다고는 말하지 않겠다`** 가 정본에 없으면 더한다. **open_questions 에 적어 뒀다.**
- ⏳ **후보 조사** — 큐가 **0/3** 이다. 이번 사이클이 보수로 바뀌어 못 했다. **지금 없는 축: 헬스케어 · 물류 B2B · 산업/제조 · 통신.** ⚠️ **Flexport 는 검색해 봤지만 엔지니어링 심층 글을 못 찾았다**(제품 릴리스와 3자 기사뿐). ⏳ 되짚을 목록 — `www.gojek.io/blog`(429) · ⛔ `bytes.swiggy.com`.

### ⚠️ 비교 문서 재료 (초안 유지)

**① `AI 에이전트를 어디까지 믿나` 8곳** — Sentry / ClickHouse / Duolingo / Ramp / DoorDash / Deliveroo / Snyk / Cygames(**에이전트끼리 서로 비판**).
...
**⑤ `한 코어에 하나씩인가, 여러 코어가 나눠 쓰나`** — ScyllaDB 대 ClickHouse. ⭐ **Grafana Labs 의 `query-sharding`("데이터는 나눠 뒀는데 질의가 한 코어에서 돈다")이 여기 붙는다.**
