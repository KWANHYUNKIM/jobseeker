...
**없다 — Convex 를 완주했다(도메인 2 · 기능 2).** 큐 0/3 이라 다음 사이클은 **후보 조사**다.

### 잘 관리하는 대신 관리할 것을 없앤다

안 주는 것 셋이 같은 모양이다. `select` 도 `count()` 도 **싸 보이지만 비싸서** 안 주고,
클라이언트의 직접 연결도 안 준다 — **'클라이언트가 데이터베이스에 직접 닿을 수 없으니,
잘못 설정할 행 단위 필터라는 것이 없다.'** **레츠 인크립트가 응답자를 없앤 것과 같은 수**인데
이유가 다르다 — 저쪽은 **강제당할 수 있어서**, 이쪽은 **틀릴 수 있어서**다.

### 닫아 둔 문 하나가 두 가지를 준다

클라이언트가 DB 에 못 닿는 것은 **트랜잭션과 결정성을 위한 구조**인데 **그 문이 보안까지
준다.** 하나의 결정이 두 도메인에 값을 낸다 — 도메인을 따로 읽으면 안 보이는 자리다.

### 위험이 사라졌는지 옮겨 갔는지가 열려 있다

'잘못 설정할 자리가 없다'는 참이지만 **자동 강제 계층이 없어 검사를 함수마다 손으로 쓴다.**
실수의 종류가 **설정 실수에서 누락으로** 바뀐 것인데, **두 방식의 실제 사고율을 재지
않는다.** 주장이 구조로만 서 있다.

### 남의 방식을 틀렸다고 하지 않는다

파이어베이스·수파베이스의 행 단위 보안이 **'이유가 있어서 존재한다'** 고 적고, 그
플랫폼에서 이 모델을 택하려면 **'Convex 가 공짜로 주는 것 일부를 포기해야 할 것'** 이라
덧붙인다. **이 원장에서 자주 못 보는 태도**라 적어 둔다.

### 잠그는 자리를 스스로 고르는데 그 기준은 안 적는다

`select`·`count()` 는 **뺏고**, 넓은 읽기 집합은 **문서로 안내한다.** 둘 다 조용히 비싸지는
것인데 대접이 다르다 — **어느 글도 그 기준을 적지 않는다.** open_questions 에 남겼다.
