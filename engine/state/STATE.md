...
**없다 — 문체 보수 사이클이었다.** 그랩을 배민 문체로 다시 썼다(0.96 → 0.0, 기호 184개). 회사 115개, 큐 2/3. 8곳 남았다.

### 변환기가 비운 solves 가 두 번째로 나왔다

모달에서 겪은 것과 같다 — `solves` 값이 통째로 `—` 하나였던 칸이 빈 문자열이 됐고 검증이 잡았다.
빈 칸을 실제 문장으로 채우고 115개 회사를 다시 훑었다(0건). **같은 변환기가 두 번 같은 자리를
다쳤다는 것이 이번 사이클의 기록이다.**

### 벽이 되는 것은 크기가 아니라 개수다

VictoriaMetrics 는 백만 개 샘플은 싸고 백만 개 시리즈는 비싸다며 비용의 정체가 카디널리티라고
못박는데, 그랩에서는 **수십억 레코드가 되면 데이터가 아니라 인덱스가 먼저 메모리를 다 쓴다.**
데이터 레이크 쪽도 같은 모양이다 — 데이터가 아니라 카탈로그가 먼저 무너진다.

### 바꾸기 전에 아는 방법을 만든 셋

- 타깃 — 규모에서 A/B 가 비현실적이라 밴딧으로 갔다.
- 웨이페어 — 다른 해결책을 골랐다면 어땠을지 모른다는 사정을 그대로 적었다.
- 그랩 — **배차의 A/B 는 실제 승객과 기사와 차가 움직이기 때문에 비싸다.**

그래서 시뮬레이터를 만들되 절대적으로 완벽할 필요는 없다고 명시한다. 진짜 문제는 따로 있다 —
**레버가 보상 함수에 약한 영향만 주면 접근 자체가 실패하는데, 그것을 실험을 다 돌린 뒤에야 안다.**

### 사내 에이전트 여덟 번째 — 믿는 대신 가둔다

Razorpay 와 가장 가깝다(격리를 커널에 맡긴다). 다만 그랩은 **에이전트가 혼란에 빠지거나 못 믿을
코드를 돌릴 수 있다고 전제하고** 시작하며, 금지가 아니라 관찰로 푼다(네트워크 접근을 막는 대신
관찰 가능하게). 그리고 Duolingo 의 채택 교훈이 여기서도 나온다 — **안전한 길이 모든 팀에게 Vault
정책 문법을 배우라고 요구하면 팀들은 그 길을 우회한다.**

짝을 이루는 다음 기능은 시간으로 말한다 — **추론 루프는 오후 한나절, 운영용 껍데기는 두 주.**

### 다음 사이클 — 문체 보수 (사다리 3순위)

--gaps 가 정한다. 지금 맨 위는 oxide(0.96)지만 그건 이 시점의 예상이지 확정이 아니다.
