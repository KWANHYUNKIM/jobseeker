...
**Ferrous Systems — 기능 하나를 채웠다(`certified-subset-boundary`).** 빈 도메인 둘.
회사 148개, 큐 2/3.

### 규격이 요구하는 것은 좋은 코드가 아니다

**"인증된 것이 인증 안 된 것에 절대 닿지 않는다"**는 증명이다. 규칙을 한 문장으로 세웠다 —
검증된 함수 집합은 **함수 호출 연산에 대해 닫혀 있어야 한다.** 그런데 러스트는 `do_io()?`
한 줄이 숨은 함수를 부르니 손으로는 못 센다.

### 물린 대안 넷을 전부 이유와 함께 적는다

**클리피 린트**(의존성에 재귀 적용 안 되고 특정 패닉 패턴만) · **효과 타입 시스템**(새 언어를
만드는 일이 된다) · **링커 시점 검사**(패닉에만 통하고 최적화 수준에 좌우되며 `panic = "abort"`
와 안 맞는다) · **`cfg` 로 core 포크**(유지 불가능한 타깃 변종 `aarch64-unknown-none.subset`).
이 엔진이 찾는 바로 그 형태의 글이다.

### 정밀도를 한쪽으로 기울인다

**위음성보다 위양성.** 인증에서는 놓친 하나가 전부를 무너뜨리니, 멀쩡한 코드를 잘못 잡는 편을
택한다. 패스도 두 겹이다 — THIR(`cargo check`, 빠른 피드백) + MIR(`cargo build`, 제네릭과
크레이트 경계 너머).

### 검사기가 컴파일러를 죽인 날

`ferrocene::unvalidated` 린트의 질의들이 순환을 만들었는데, **그 순환을 잡아 오류로 알렸어야
할 상류 장치가 오히려 무한 재귀**했고 스택 보호도 없었다(8MB · 100만 프레임 초과 · 디버깅
일주일). 우회하지 않고 상류를 고쳤다.

### 다음 사이클 — 확장 (남은 두 도메인 중 하나)

`남의 언어를 받아 인증한다` 에 `Query cycles` 자료가 있다(3개월 주기 대 rustc 6주, 상류 수정,
CI 의 시간·메모리 한계). `인증 문서까지 열어 놓고 판다` 는 릴리스와 제품 페이지가 재료다.
