...
**Sansan(JP · SaaS) `in_progress` — 도메인 1개 · 기능 1개.** 330 에서
**`rebind-what-was-attached`(그릇을 옮기고 붙어 있던 것을 다시 맨다)** 를 썼다.
**결정 7개 · 그림 3장 · 생각 4개 · 난제 2건.** 재료는 **모노레포 이관**(2024-05-27)과
**GitHub Enterprise 이관**(2025-12-19) **두 편**이다.

**⑫ 가 다섯 번째로 값했다 — 재독이 새 재료를 냈다.**
- **⭐ 순서를 재서 정했다** — `gh` 로 저장소마다 마지막 갱신일을 받아 **변경 빈도로 순위**를
  매기고 **위험 낮은 것부터 순차 → 변경이 잦은 것은 프로젝트 사이 틈에 → 빈 자리는 저빈도로**
  메웠다. **일정을 기술이 아니라 남의 일정에 맞춰 짠 것이다.**
- **옮기는 중에 원본이 계속 움직였다** — *"移行の真っ最中に元リポジトリにコミットが入ったり
  など細かいトラブルはいろいろありました"*. **해결책은 안 적는다.**
- **⚠️ git 이력을 어떻게 했는지 한 줄도 없다.** 빌드 시간 변화 측정도 없다.
- CI/CD 는 **Azure Pipelines** 였고 **같은 이관 스크립트 안에서** 함께 옮겼다.
- .NET 의 Stephen Toub 이 한 번에 커밋한 선례를 참조한다.

**⭐ 기능의 축** — **옮기는 것은 그릇이고, 값을 치르는 것은 그 그릇에 매여 있던 줄들이다.**
도구는 저장소만 옮긴다 — **CI/CD 잡 · Secrets 와 Variables · 외부 연동 · OIDC `sub` 클레임 ·
팀과 권한**은 사람이 다시 맨다. 모노레포 편이 그것을 한 문장으로 적는다:
*"モノレポの移行よりも、微妙に統一されきっていなかったCI/CDジョブの移行に手間取りました"*.

**⭐⭐ 다른 회사와 이어지는 자리를 `open_questions` 에 남겼다** — Paytm 의
`leave-what-costs-more`(320)와 **둘 다 옮기기 전에 안 옮길 것을 정했는데 그 뒤가 다르다.**
Paytm 은 **안 옮긴 것을 어떻게 했는지 적고**(남기거나 지운다, 근거는 *"likely not in use"*),
**Sansan 은 남은 약 300개의 행방을 안 적는다.** ⚠️ **옛 Organization 을 남겼다면 없애려던
계정 수작업이 거기 남는다**(추정).

⚠️ **330 에서 인용문에 키릴 문자가 섞인 것을 검증 전에 잡아 고쳤다** — 혼용 검사(한국어)만으로는
안 걸린다. **`[Ѐ-ӿ]` 도 함께 본다**(새 운영 사실).

---

**다음 사이클 — 열린 도메인이 다 찼다. 이 회사의 심장을 판다.**
- **⭐ 1순위 — R&D · 머신러닝 분류를 연다.** 매출을 끄는 것은 **명함·청구서를 읽어 데이터로
  바꾸는 일**(Bill One **+41.3%**)인데 아직 그쪽 글을 한 편도 안 읽었다. **분류 목록이
  열린다** — `buildersbox.corp-sansan.com/archive/category/...` 형태다(⑰ 주소는 목록에서
  실제 href 를 얻는다). **R&D 347편 · 머신러닝 64편**이 가장 크다.
- **⭐⭐ 2순위 — 실물이 있는 도메인.** `サービスごとに異なる原本管理を、一つの仕組みへ`
  (2026-08-20)는 **되돌리기가 화면 안에서 안 끝난다**는 축을 드러낸다 —
  *"システムのステータスだけを戻したとしても、実際の原本はもう別の場所にあるかもしれない"*.
  **⚠️ 대담 형식이라 버린 대안이 0 이라 아직 도메인으로 안 열었다. 같은 주제의 기술 글이
  하나 더 나오면 이 사이트에 없는 축이다**(다른 회사들의 되돌리기는 전부 화면 안에서 끝난다).
- **안 읽은 글** — Swift Concurrency 이관(2026-03-19) · Jetpack Compose 이관(2025-11-17) ·
  Eight 의 EC2 탈출과 모듈러 모놀리스화 · 레이어드 아키텍처 회고(2019-04-21) ·
  AI 와 함께 만드는 시스템 설계(2026-06-26) · 모바일 개발 대담 2부작.
  **⚠️ 이관 글이 많은데 문제가 서로 같은지는 열어서 정한다**(223 규칙) — **이미 연 도메인에
  더 붙일 수 있는지도 함께 본다.**
