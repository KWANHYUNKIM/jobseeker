...
**Paytm(IN · 핀테크) `in_progress` — 도메인 4개 · 기능 2개.** 321 에서 **네 번째 도메인
`재는 일을 사지 않고 만든다` 를 열고 기능 `own-the-measuring`(재는 일을 직접 떠안는다)을
썼다. 결정 6개 · 그림 3장 · 생각 4개 · 난제 2건 · 연결 2건.**
재료는 `Building an Open-source Monitoring System with Prometheus`(2022-08-31).

**⚠️ `--gaps` 는 도메인 ②(거래 기록)를 가리켰는데 거기를 안 썼다.** 감이 아니라 **재료를
열어 재고 나서** 그랬다 — 아래에 근거가 있다. **이번 사이클에 안 읽은 글 넷을 전부 열었다.**

**✅ 넘긴 것 하나 — Prometheus(2022-08-31)**
- **대가 ✅** — *"Cardinality was a bigger challenge for us to manage the availability of
  Prometheus on such a large scale"*. 자체 파이썬 감시기와 **잡별 스크랩 지표 한도**로 막는다.
- **수치 ✅✅** — 분당 **수백만 이벤트**, 일 **수십억**, **최근 40개월 100% 가용성**,
  비용은 유료 도구의 **2% 미만**, **한 사람이 운영**.
- **⭐ 기능의 축** — **산 것을 만들면 값이 사라지는 게 아니라 모양이 바뀐다.** 청구서는 2% 로
  내려갔는데 **관측 시스템을 지키려고 관측에 구멍을 냈다** — 한도에 걸린 지표는 안 들어오고
  **무엇이 잘렸는지 알 방법이 글에 없다.**
- **⭐⭐ `connections` 둘을 320 의 기능과 걸었다.** ① **없는 도구는 파이썬으로 짧게 만들어
  붙이는 버릇** — 이관 때는 접근 로그 분석기, 여기서는 카디널리티 감시기. ② **확인한 것을
  적는 정도가 정반대다** — 이관 글의 검증은 한 문장인데 관측 글은 40개월 100% 를 숫자로 낸다
  (⚠️ 다만 그 100% 를 어떻게 쟀는지는 이쪽도 안 적는다).

**❌ 떨어뜨린 것 셋**(전부 `open_questions` 에 이유와 수치까지 남겼다)
- **Redis 클러스터**(2023-01-05) — 수치는 굵은데(MTU **8,400만** · 가맹점 **2,830만** ·
  수십만 TPS · 셋업 **60초 미만**) **버린 대안도 대가도 0줄.** 게다가 **글의 틀이 캐시가 아니라
  클러스터 운영**이라 도메인 ③ 과도 다른 문제다(223 규칙).
- **루팅/탈옥 기기 차단**(2022-11-16) — **기술 서술이 사실상 없다.** RBI 회람을 근거로 든
  **정책 안내문**이다.
- **가짜 SMS 탐지**(2022-03-03) — **자동 탐지가 아니라 사용자 안내문**이다. 사람이 알아보는
  다섯 가지를 적는다.
- **그래서 `기기와 문자를 얼마나 믿을 것인가` 도메인은 열지 않았다.**

**⚠️ 두 편 규칙을 한 번 어겼고 그 사실을 도메인 `why` 에 밝혔다** — 관측을 다루는 글이
**블로그 열 편 중 이것 하나뿐**이다(300·307 에서도 같은 방식으로 밝혔다).

---

**다음 사이클 — Paytm 을 닫는 판단이다. 새 재료가 없다.**
**`paytm.com/blog/engineering/` 의 열 편을 전부 읽었다.** 남은 둘은 기능을 못 쓴다.
- **② 거래 기록을 어디까지 맞출 것인가** — 재료가 한 편뿐이고 **버린 대안 셋은 있으나 근거가
  한 줄씩이고 수치가 0** 이다. **대가를 적으면 대부분이 이 사이트의 지적이 된다**(STYLE.md —
  대가 칸이 비면 그 결정은 쓰지 않는다). **이 회사 안에 두 번째 글이 없다.**
- **③ 앞단에서 끝낸다** — CDN 글(버린 대안 0 · 대가 0)과 Redis 글(같은 문제도 아니다)뿐.
- **닫을 때 할 일** — 회사 `status` → `done` · `index.json` status → `done` ·
  **QUEUE 진행 중 행을 빼고 완료 불릿로 되돌린다**(완료 섹션은 **불릿**이고 표가 아니다 — ⑦.
  삽입은 `'\n## 완료\n'` 줄 경계 매치 — ⑥). **⚠️ 도메인 넷 중 둘이 기능 없이 남는다는 사실을
  `business_model` 에 이 사이트의 정리로 적는다.**

**📌 그 밖**
- **⭐ 320 의 기능이 비교 문서 두 편의 빈칸과 이어진다** — `reversible-first.md` 의
  *"되돌리지 못해 앞으로 밀고 나간 사례가 없다"* 에 **첫 재료**이고, `where-it-piles-up.md` 의
  **갚지 않고 이사한 쪽**(Careem 270TB→78TB)에서 **한 걸음 더 간다.**
  **다음 비교 문서 사이클이 두 문서를 갱신할 수 있다.**
- **큐 잔량 1/3**(Freshworks). **Paytm 을 닫으면 [신규]로 Freshworks 가 열리는데, 그 전에
  후보 조사를 한 번 끼우는 편이 낫다** — 다음 조사는 **일본**(Sansan·freee·SmartHR·Money
  Forward)이나 **유럽**(Personio). **인도는 소진됐고 Trade Republic 은 Medium 호스팅이다.**
- **후보 조사 누적 열여섯 곳 중 넷.**
- **⚠️ 낡은 메모를 나르지 않는다**(314·317). **⚠️ 두 방향 규칙**(298·305·318) — 한 편은 반드시
  열고 첫 편이 얇으면 한 편 더 연다. **321 은 그 반대 방향도 보여 줬다** — 넷을 다 열어 셋을
  떨어뜨렸는데, **안 열었으면 Redis 글을 제목만 보고 올렸을 것이다.**
