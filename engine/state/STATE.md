...
**Cerebras — 기능 둘을 채웠다.** `defect-tolerant-wafer` 와 `weights-stay-put`(가중치를 옮기지
않는다). 회사 127개, 큐 2/3. 빈 도메인 하나 — 커지는 모델.

### 문제를 정의하는 순간 답이 하드웨어로 좁혀진다

느린 이유를 **가중치 이동**이라고 정의하면 소프트웨어로 고칠 길이 사라진다. 연산 유닛을 늘려도
가중치가 매 단계 칩 밖에서 건너오면 그 시간이 답을 정하고, 칩을 더 붙이면 그 단계가 오히려
비싸진다. 이 회사는 그 정의를 받아들이고 **스스로를 칩을 만들 수밖에 없는 자리로 몰아넣는다**(재구성).

답은 모델을 칩 위에 통째로 올리는 것이다 — SRAM 44GB 가 코어 90만 개 옆, 총대역폭 21 PB/s.
모델은 층 단위로 굵게 나눠 가중치는 제자리에 두고 **활성값만 단계 사이로 보낸다.** 웨이퍼가
여럿일 때도 텐서·전문가 통신은 안에 가두고 밖으로는 활성값만 보낸다.

### 구조를 바꾼 뒤에도 소프트웨어에 남는 몫이 컸다

같은 하드웨어에서 세 배가 더 나왔다 — 핵심 커널(MatMul·리듀스/브로드캐스트·원소별 연산·활성 함수)을
다시 쓰고, 웨이퍼 입출력을 연산과 비동기로 돌리고, 투기적 디코딩을 얹어 Llama 3.1-70B 가 초당
2,100토큰, 응답 전체가 0.4초(GPU 는 1.1~4.2초)가 됐다.

### 지키기로 한 선과 인정한 흔들림

**가중치를 16비트로 유지한다** — 양자화로 얻을 속도를 포기하고 출력 정확도가 그대로임을 외부
확인으로 못 박는다. 파는 것이 빠른 답이지 다른 답이 아니라는 선이다. 반대로 투기적 디코딩이
만드는 속도 편차는 그대로 밝힌다 — **평균에서 위아래 20%는 정상이라고 적는다.**

### 다음 사이클 — 확장 (마지막 빈 도메인)

커지는 모델을 담을 곳(여러 웨이퍼, CS-6 의 DRAM 적층, CS-5 의 목표 수치)을 채우면 done 이 된다.
다만 그 도메인의 자료는 아직 계획과 목표뿐이라는 것도 적어 뒀다.
