...
**Paystack** (NG · 핀테크) — 도메인 2개 · **기능 1개**(`success-is-nothing-changed-after`). **남은 것은 핵심 제품 하나이고, 그것이 완주를 막고 있다.**

### 이번 사이클 — 이전 `제시간에 끝내는 것과 아무것도 안 바뀌는 것은 다르다`

건진 것 넷:

- ⚠️ **계획을 쓰기 전에 이전 기록을 먼저 읽었고, 거기서 방향이 나왔다** — **`정비 시간 안에 끝내는 것은 성공이 아니다`.** 예전에는 **직후에 멀쩡해 보이다가 나중에 문제가 드러났고, 어떤 질의는 잘못된 결과를 돌려주기 시작했다**(트래픽이 재개된 뒤에야 조사). **`제시간에 끝냈다` 는 즉시 검증되는데 `아무것도 안 바뀌었다` 는 시간이 지나야 안다**(재구성).
- ⚠️ **두 층이 서로 다른 것을 잡는다** — DB 쪽 검사는 **예약어 충돌**을 잡고, 앱 쪽 스테이징은 **ORM·커넥션 풀·실행 계획·백그라운드 작업**을 잡는다. **하나만 했으면 다른 하나를 놓쳤을 것이다**(재구성).
- ⚠️ **쉬워 보이는 것이 가장 늦게 터졌다** — 가장 복잡한 클러스터는 첫 번째로 무사히 갔는데, **`가장 쉬울 거라 예상했던` 클러스터**에서 드라이런 중 **복제가 테이블마다 다르게 동작**했다(어떤 것은 처음 보는 방식으로 실패). **멈추고 재평가해 여러 복제 방식을 조합**해 풀었다.
- ⚠️ **쓰지 않은 안전장치의 값을 적는다** — `In practice, that safeguard was never needed, but **knowing it was there made it possible to move forward confidently.**`

**결과** — 11개 전부를 마감 전에, **사후 사고 0**, 3시간 계획을 **2시간 미만**에, **첫 2년 17만 달러 이상** 절감.

### ⚠️ 이번 사이클에 내가 한 실수 — 기능 이름에 강조 기호를 넣었다

**`name` 은 식별자·대조에 쓰이는 필드라 강조 기호를 넣으면 안 되는데 백틱을 넣었다.** 검증이 경고로 잡아 줘서 커밋 전에 고쳤다(`제시간에 끝내는 것과 아무것도 안 바뀌는 것은 다르다`). **앞서 `one_liner`·`products` 에서 같은 실수를 한 적이 있고, `name` 도 같은 부류라는 것을 이번에 배웠다.**

### 다음 사이클 — 핵심 제품, 없으면 없다고 적는다

⚠️ **`아프리카에서 사업체가 돈을 받게 한다` 의 tech 가 0 이다.** **블로그의 시스템 글이 DB 이전과 SAST 둘뿐이라** 자료가 없을 가능성이 크다.

**그때는 완주를 미루고 `확인했고 없었다` 를 기록한다**(PROMPT 2‴단계 — 그것도 정상적인 사이클 결과다). ⏳ 찾아볼 곳: `How we manage vulnerabilities at Paystack`(2025-05, SAST) · 개발자 문서(`paystack.com/docs`) — **Fastly·Plaid 에서 제품 문서가 답이었던 전례가 둘 있다.**
