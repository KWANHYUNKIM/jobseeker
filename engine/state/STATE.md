...
**Cerebras — 도메인 셋을 다 채우고 done 으로 닫았다.** `defect-tolerant-wafer` ·
`weights-stay-put` · `weight-streaming-scale-out`. 회사 127개, 큐 2/3.
...
**움직이는 것을 줄인다.** 그 문장이 조건에 따라 셋으로 갈린다.

- 웨이퍼를 자르지 않아야 메모리가 연산 옆에 온다(그러려면 결함을 견뎌야 한다).
- 모델이 들어가면 얹어 두고 활성값만 옮긴다.
- 안 들어가면 흘려보내되, 파라미터 갱신은 메모리 쪽 코프로세서가 해서 왕복을 없앤다.

마지막 것이 특히 이 회사답다 — 갱신하려고 엔진으로 데이터를 가져오면 없애려던 이동이 되살아나니
연산을 메모리 쪽으로 보낸다.

### 한계를 스스로 적는 회사다

결함이 한곳에 몰리면 칩 전체가 죽을 수 있고 실제 수율은 100%가 아니다 · 면적의 절반인 메모리와
인터커넥트는 코어 크기의 이점이 그대로 안 간다 · 투기적 디코딩은 속도 편차를 ±20% 만든다 ·
희소 가속은 약 3,000배에서 포화되고 그 원인이 부분합 리듀스의 고정 네 사이클이다. **수치를 크게
내거는 회사가 그 수치의 천장도 같이 적는다.**

### 우리가 채운 빈칸

층 단위 분할의 파이프라인 균형(한 단계가 느리면 전체가 느려진다)을 회사가 다루지 않아 난제로
남겼고, 초당 750토큰의 배치·컨텍스트 조건과 실제 수율, 가격, 학습 쪽 최신 상태도 못 봤다.

### 다음 사이클 — 신규 (Ladybird)

큐가 2/3 이고 대기 맨 위가 Ladybird 다. 브라우저 엔진 축이 처음이고, 회사가 아니라 비영리라
수익 구조를 어떻게 적을지가 프로파일의 과제다.
