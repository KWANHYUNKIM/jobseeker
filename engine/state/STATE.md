...
**Inngest — 회사 프로파일까지 썼다(in_progress).** 회사 129개, 큐 0/3.

도메인 셋을 세웠고 기능은 아직 0개다.

- **중단된 자리에서 이어 붙인다** — 요금 페이지(실행 = 함수 실행 + 각 단계)
- **함수마다 자기 큐를 준다** — `fair multi-tenant queuing`(2024-01-22)
- **큐의 바닥을 갈아 끼운다** — `FoundationDB, Part 1`(2026-08-24)

### 과금 단위가 제품의 성질을 드러낸다

**실행 하나는 함수 실행 한 번에 그 안의 각 단계를 더한 것**이다. 코드를 `step.run()` 으로 잘게
나눌수록 중단에서 이어 붙일 자리가 늘고 청구서도 늘어난다. 동시성에도 따로 값을 매긴다(동시 단계
25개 추가마다 25달러) — 파는 것이 계산이 아니라 **동시에 얼마나 밀어 넣을 수 있느냐**이고, 그것을
공정하게 나누는 일이 곧 제품이다(재구성).

이 엔진에서 과금 단위가 기술 문제를 정한 회사가 셋이 됐다 — Materialize(크레딧이 곧 상태 크기) ·
Zed(수락된 편집 예측만 센다) · Inngest(실행 = 함수 + 단계).

### 보장을 더할수록 느려지는 구간에 들어갔다

공정성 장치를 하나 더할 때마다 저장소에 연산이 붙는데, 그 저장소가 메모리와 단일 스레드에 묶여
있었다. 두 해 동안 우회했다 — 동적 리스를 둔 수평 샤드, 제약 강제를 별도 서비스로, 키별 다중 버전
공정성. **같은 벽에 되풀이해 부딪혔다**고 적고 결국 바닥을 FoundationDB 로 바꾼다.

### 축이 셋과 이어진다

Temporal(지속 실행 축의 다른 답) · Tigris(같은 FoundationDB 바닥) · **Chroma 에서 자료가 없어
보류한 테넌트 공정성** — 한 회사에서 못 판 것을 여기서 판다.

### 다음 사이클 — 확장 (Inngest 의 빈 도메인)

공정 큐 글이 가장 두껍다. 시끄러운 이웃과 선두 차단을 함수마다의 큐와 상위 발견 큐로 푸는 이야기다.
