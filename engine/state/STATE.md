...
**없다 — 문체 보수 사이클이었다.** 스냅을 배민 문체로 다시 썼다(0.77 → 0.0, 기호 409개). 회사 115개, 큐 2/3. 3곳 남았다.

### 병목은 사라지지 않고 옮겨 간다

이 회사의 개발 도구 세 벌이 한 문장으로 이어진다 — **모두가 코드를 더 빨리 낼 수 있게 되면 병목은
사라지지 않고 코드 리뷰로 옮겨 간다.** 그래서 도메인을 제품이 아니라 **무엇이 사람의 속도를
넘어섰느냐**로 잘랐다(코드가 넘어선 자리, 컴퓨터가 화면을 떠나 얼굴 앞으로 온 자리).

### AI 가 만든 것을 거르는 답이 넷이 됐다

Duolingo(두 번째 리뷰어) · 윅스(컴파일 관문) · 메이퇀(올리기 전 스스로 검사)에 이어, 스냅은
**지적에 인용된 심벌이 실제로 맥락 안에 있는지 확인한다.** 순진한 방법을 해 보고 왜 안 되는지도
적는다 — 없는 것을 지어내고, 돌릴 때마다 자기 말과 어긋나고, 결과가 들쭉날쭉하다. 결론도
조심스럽다 — **AI 리뷰가 아직 사람 리뷰를 대체하지는 않고 그 모양을 바꾼다.**

### 검색을 똑똑하게 만들 것인가, 여러 번 돌릴 것인가

Honeycomb(색인 없이 다 훑는다) · Datadog(안 읽는 방법을 질의 계층에)과 달리, 스냅은 **빠른 검색
기본 요소 하나를 만들고 에이전트가 그것을 엮게 한다** — 검색 시스템은 멍청하고 지능은 루프에 있다.
근거가 시간에 있다: **모델이 좋아지면 우리가 아무것도 안 바꿔도 검색이 좋아지는 쪽**을 골랐다.

### 여럿의 흔적으로 한 번의 신호를 대신하는 셋

Zepto(사람들이 다닌 길로 GPS 를 고친다) · Meesho(기사가 실제로 간 곳을 정답으로)에 이어, 스냅은
**서로의 얼굴을 봐서 좌표를 맞춘다.** 그리고 못 재는 것을 적는다 — 믿을 만한 고정밀 기준이 없어
확정적인 수치를 못 내놓는다고 밝히고 정성적 관찰에 기댄다.

### 재는 자를 새로 만들되 결과를 부풀리지 않는다

메이퇀에 이어 스냅도 자를 만드는데, **무거운 추론이 새로운 공간 능력을 열어 준 것은 아니고 대체로
그러지 못했다**고 정직하게 적는다.

### 다음 사이클 — 문체 보수 (사다리 3순위)

--gaps 가 정한다. 지금 맨 위는 bol(0.73)이지만 그건 이 시점의 예상이지 확정이 아니다.
