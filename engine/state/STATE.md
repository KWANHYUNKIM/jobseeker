...
**Protomaps — 프로파일만 썼다.** 회사 146개, 큐 1/3(Ente 만 남음).
도메인 둘이 비어 있고 둘 다 자료를 확보했다.

### 낱개로 올리면 값이 무너진다

타일을 파일 하나하나로 올리면 **3억 개에 요청 요금만 1,500달러가 들고 며칠이 걸린다.** 그래서
피라미드 전체를 파일 하나에 담고 클라이언트가 HTTP 범위 요청으로 필요한 조각만 읽게 한다 —
같은 타일을 중복 제거해 전 세계 벡터 기본 지도에서 **70% 넘게** 줄인다.

### 읽기 전용이라는 성질이 전부를 정한다

파일 전체를 다시 쓰지 않고는 제자리 갱신이 안 된다(CSV·JSON 과 같다고 적는다). 그래서 자주
바뀌는 데이터에는 애초에 안 맞고, 그 사실이 다음 도메인의 출발점이 된다.

### 파는 쪽이 쓰지 말라고 적는다

`You Might Not Want PMTiles`(2024-05-22)가 적격 조건 셋을 못 박고(웹에서 보고 · 수 MB 를 넘고 ·
하루 한 번 이하로 바뀌고) 아니면 **GeoJSON·PostGIS·GeoParquet 을 이름 대어 권한다.** 이 엔진의
후보 판정을 통과시킨 것도 이 글 한 편이었다.

### 남이 팔던 것을 안 사도 되게 만드는 쪽

비상업 사용은 완전히 공짜, 상업적으로 쓰면 깃허브 스폰서, 자기 저장소에 올리면 무제한 무료다.
"이미 쓰고 있는 클라우드 저장소 위에서 지도 요금을 월 수백 달러에서 몇 푼으로" — **값을 줄이는
것이 곧 제품**인 조직이다.

### 다음 사이클 — 확장 (Protomaps 의 빈 도메인)

`서버 없이 지도를 낸다` 부터. 단일 파일·범위 요청·중복 제거·MBTiles 와의 갈림이 손에 있다.
