...
**Signal — 기능 둘.** 빈 도메인 둘(서버에 맡기는 것 · 비용). 회사 133개, 큐 2/3.

### 아무도 안 쓰는 방어는 방어가 아니다

안전 번호를 만나서 맞춰 보라는 절차를 만들어 뒀지만 실제로 하는 사람이 드물다. 그래서 기계가
대신 확인하게 만들었는데, **그러려면 어딘가를 믿어야 한다** — 우리를 믿으라고 할 수 없으니
밖에서 보는 눈 둘(클라우드플레어·트레일오브비츠)을 세웠다. 못 믿겠으면 끄고 예전 방식을
쓸 수 있게 남겼다.

### 서명 규칙 하나가 갈라 보이기를 막는다

감사자는 각 판과 색인책에 **한 번만 서명한다.** 그래서 서기가 50판을 두 모습으로 만들어
두 사람에게 달리 보여 주려 해도 유효한 서명은 한쪽에만 붙는다. 원장을 못 고치게 만드는 대신
서명의 유일함으로 거짓말을 막은 자리다.

### 공개해야 검증되는데 공개하면 드러난다

식별자는 VRF, 값은 키 해시로 가려 **감사자도 평문을 못 본다.** 조회에는 인증을 요구하지 않아
누가 누구를 찾았는지 계정에 남지 않는다. 가린 대가로 상대의 전화번호를 알아야 확인이 된다 —
사용자명으로만 연결된 사이는 자동 확인이 안 된다.

### 막는 것이 아니라 잡는 것

개입을 그 순간 막지 못하고 사후에 어긋남을 드러낼 뿐이며, 그마저 양쪽이 다 확인해야 완성된다.
어긋나도 **자동으로 세션을 끊지 않는다** — 정상적인 번호 변경과 서버의 개입을 기기가 못 가르기
때문이다. 계정을 통째로 뺏긴 경우는 이 방법으로 못 잡는다고 스스로 적는다.

### 다음 사이클 — 확장 (Signal 의 빈 도메인)

`서버가 알면 안 되는 것을 서버에 맡긴다`. 백업 글(2025-09-08)과 ORAM 글(2022-08-19)을
아직 안 읽었다 — 지금 이 도메인의 tech 한 칸이 `unknown` 이다.
