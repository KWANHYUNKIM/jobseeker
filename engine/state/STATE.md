...
**Jane Street — 기능 하나를 채웠다(`modes-for-refcounting`).** 빈 도메인 둘. 회사 137개, 큐 1/3.

### 정확성이 아니라 시점이 문제였다

OCaml 에서 파이썬 객체를 감싸면 `Py_INCREF` 는 제때 하는데 짝이 되는 decref 가 **OCaml GC 를
기다린다.** 틀리지는 않지만 큰 데이터프레임이나 GPU 메모리를 그 사이 내내 붙들고 있다.

### 빠르게 만든 게 아니라 없앨 일을 찾았다

`@ local` 은 이 값이 호출을 벗어나지 않는다는 뜻이라 **컴파일러가 그걸 증명하면 incref 를 할
이유 자체가 사라진다.** `@ unique` 는 유일한 손잡이라는 뜻이라 반환 때 포인터를 그대로 넘긴다.
`borrow_` 로 유일함을 잃지 않고 빌려준다.

### 러스트와 갈라선 자리를 명시한다

"소유권 추적의 짐을 API 를 쓰는 모두에게 지우지 않는다" — 모드를 안 쓰면 예전처럼 GC 에 맡겨
쓸 수 있게 두고, 그 코드에서는 지연이 그대로 남는 것을 감수한다. **안전을 선택적으로 파는**
설계라 이 엔진의 다른 언어·런타임 회사(Bun·Zig 후보)와 견줄 축이 하나 생겼다.

### 열되 기본이 아니게

`newref`·`decref` 를 손으로 쓰는 길을 남기되 코드가 지저분해진다고 적고, 예외가 나도 정리되게
`ppx_release` 로 감싼다.

### 다음 사이클 — 확장 (Jane Street 의 빈 도메인)

`커널의 기본값을 의심한다`. FUSE 재현 장치·bpftrace·와이어샤크 플러그인·NFQUEUE·리눅스 6.16
패치 둘이 손에 있다.
