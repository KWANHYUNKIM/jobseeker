...
**Polar Signals — 프로파일만 썼다.** 회사 132개, 큐 0/3. 도메인 셋이 다 비어 있고 셋 다
자료를 확보했다.

### 관측 축의 새 답 — 프로파일을 상시로 남긴다

로그를 얼마나 남길지, 추적을 얼마나 샘플링할지로 갈리던 축에 이 회사가 다른 답을 놓는다.
Antithesis 가 남기지 않고 다시 만드는 쪽이었다면 여기는 **디버깅 때 잠깐 켜던 프로파일러를
늘 켜 두는** 쪽이다. 그래서 기술 문제가 하나로 모인다 — 늘 켜 둬도 괜찮을 만큼 싸게 찍는 법.

### 파는 단위가 표본이다

월 1억 표본당 50달러, vCPU 하나가 월 600만 표본. **초당 19번이라는 표본 주기가 요금표와
직접 이어진다** — 앞의 Depot·Ubicloud 처럼 여기서도 과금 단위가 기술을 정한다.

### 벌써 눈에 걸린 것 — 두 길 중 하나만 유지한다

스택 되짚기에 프레임 포인터와 DWARF `.eh_frame` 두 길이 있는데, 파르카는 `.eh_frame` 만 쓰고
**프레임 포인터를 따라가려는 시도조차 하지 않는다.** 많은 컴파일러가 `-fomit-frame-pointer`
로 그것을 생략하니 두 길을 다 유지할 값을 안 낸다는 선택이다.

되짚는 코드는 eBPF 로 커널 안, 대상 프로세스의 주소 공간에서 돈다 — 스택을 사용자 공간으로
복사하지 않아 문맥 전환이 없고 깊은 스택이 잘리지 않는다. 대신 검증기가 허락하는 일만 할 수
있어 조율·DWARF 해석·네트워크는 사용자 공간 Go 로 갈라 뒀다.

### 다음 사이클 — 확장 (Polar Signals 의 빈 도메인)
