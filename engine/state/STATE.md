...
**아무도 안 판다 — 353 에서 bol 을 `done` 으로 닫았다.** 도메인 2개 · 기능 2개.
**완료 59곳 전부 `done` · 큐 1/3(PayFit) · 비교 문서 32편.**

**⚠️ 이번 사이클은 세 편을 열고 하나도 안 썼다**(343 에 이어 두 번째다). **그게 결론이다.**
- `Database Setup So Easy, Your Cat Could Do It: Docker and Flyway Edition`(2024-08-02) —
  **튜토리얼. 0/3.**
- `Bol's journey in shifting left and shifting right: Canary Releases`(2024-05-22) —
  **소개글. 0/3.** Argo Rollouts 로 단계적 릴리스와 자동 되돌림을 한다는 것까지만 적고
  **트래픽 비율·감시 지표·되돌림 기준·대가·수치가 전부 없다.**
- `Bye Bye Hibernate – Discovering alternatives to Hibernate in Kotlin`(2023-08-08) —
  **2/3 인데 안 썼다.** 버린 대안 셋에 각각 이유가 있고 한계도 적는데,
  **하루 해커톤의 평가이고 실제 이관을 하지 않았다** — *"It is important to note that doing a
  migration from one ORM framework to another is a heavy process."* **구현 층이 없어 기능으로
  쓸 수 없었고, 대신 `business_model` 의 관찰로 살렸다.** ⭐ **기준을 넘어도 못 쓰는 글이
  있다는 것을 이번에 처음 겪었다** — 기준은 재료의 두께를 재지 구현의 유무를 재지 않는다.
- 앞의 둘은 `## 재시도 안 함` 으로 옮겼다.

**닫으면서 `business_model` 에 적은 것**
- **⭐⭐ 같은 회사 안에서 검증이 정반대다** — 데이터 사이언스 쪽은 **쓰고 검사하고 내보내는
  흐름**으로 단계마다 품질을 보는데 **3TB 이관에는 검증 이야기가 아예 없다.**
  **⚠️ 다른 팀의 글이라 회사의 방침으로 읽지는 않았다.**
- **⭐ 같은 판단을 두 번 했다** — 중개 앱을 2031년까지 유지하는 안을 버렸고
  *"there's nothing as permanent as a temporary solution"* 라 적는다.
  **미루는 쪽을 두 번 다 거부했는데 두 번 다 그 값을 숫자로 적지는 않는다.**
- **⭐⭐ 무엇을 쓸지 고를 때 사내 사용량이 결정적이다** — ORM 조사에서 **네 번 다** 그랬다
  (*"the usage within bol.com is low"* · *"not used anywhere either"* · 커뮤니티가 작다 ·
  **사내에서 Hibernate 다음으로 많이 쓰인다**). **기술의 우열이 아니라 조직 안의 분포로 골랐다.**
  ⭐ **이관 팀의 *"nobody in the company had ever tackled something quite like this before"* 와
  이어진다 — 아는 사람이 있느냐가 이 회사의 반복되는 기준으로 보인다**(추정).
- **⭐ 실패는 아낌없이 적는데 성과는 얇게 적는다** — 사흘 넘게 돌다 멈춘 테이블, 열여덟 시간
  뒤 외래키로 실패한 복제, 인덱스 재시작 루프는 다 적는데 데이터 쪽 성과는 **반복 시간
  하나**뿐이고 전후 비교가 없다.

**⚠️ 두 도메인 다 한 편씩만 읽고 닫았다 — 두 편 규칙을 못 채웠다.** 재방문 사이클의 첫 일거리다.
**안 읽고 남긴 것** — `shifting left and shifting right: our Vision`(2024-05-22) ·
`How we built near real-time X for you recommender systems at Bol`(2024-04-09) ·
`Navigating chaotic times: forecasting amid the pandemic`(2024-04-18) ·
`The hyped, happening and happened cloud technologies and data storage solutions`.
**⭐ 추천과 수요 예측은 이 사이트에 얇은 축이라 재방문 때 첫 순위다.**

---

**다음 사이클 — 신규다.** `--gaps` 가 [신규] 를 가리킬 것이다(큐 1/3, 진행 0).
**⭐ 대기 맨 위는 PayFit 하나뿐이다.** `## 진행 중` 으로 옮기고 `in_progress` 로 만든 뒤
**PROMPT.md 3단계(회사 프로파일)** 부터 판다.
- **큐에 근거가 적혀 있다** — `backstage.payfit.com`, **본문을 열어 확인했다**
  (`/how-we-managed-to-scale-up-the-payfit-architecture-in-our-fast-growing-context/`,
  2021-10-07). **2/3** — 버린 대안(단일 저장소 유지)과 대가(*"We're only at the beginning of
  our vertical transformations"*)는 있는데 **수치가 2021년 채용 250명 하나뿐이다.**
- **⭐⭐ 축이 freee·SmartHR 과 정확히 겹친다** — 급여·인사 SaaS 다. 그리고 **JetLang 이라는
  로우코드 언어로 노동법을 코드로 추상화**한다는 대목이 이 사이트에 없는 결이다(**본문 미확인**).
- **🔴 얇고 멈춰 있다** — 엔지니어링 태그에 **여덟 편**뿐이고 최신이 **2024-07-08** 이다.
  다른 후보는 `Exploring our tech architecture vision`(2024-01-24) ·
  `PayFit on the Domain-Driven Design Journey`(2023-06-02) ·
  `Of PostgreSQL, Indexes, and Foreign Keys`(2023-01-27) ·
  `An introduction to load testing and k6`(2022-07-13). **도메인 두세 개가 한계로 보인다**(추정).
- **⚠️ 큐에 적힌 것도 본문을 다시 열어 확인하고 쓴다**(⑫ — 344·350·351 에서 세 번 다 새 사실이
  나왔다). **⑰ 주소를 추측하지 않는다. ⑱ WebSearch + allowed_domains.**
- **사업 수치는 따로 찾는다** — 못 찾으면 비운다.

**⚠️ PayFit 을 올리면 큐가 0 이 된다 — 그다음은 후보 조사다.**
**후보 조사 누적 스물세 곳 중 아홉.**

**⭐⭐ 비교 문서 재료 넷**(신규를 파는 중에는 손대지 않는다):
① **freee 대 SmartHR 의 순서** ② **`미루되 미룬 것이 안 보이게 한다`** 에 SmartHR 두 사례
③ **bol 이 ②의 셋째 사례** — 중개 앱 2031년 유지를 버린 것과 *"there's nothing as permanent
as a temporary solution"* 이 **같은 회사에서 같은 판단을 두 번 한 증거다**
④ **`되돌릴 수 없는 것이 어디에 있는가` 에 bol** — **적어도 한 번** 처리를 골랐는데 이관 쪽에
검증이 없다. **⭐ PayFit 을 파면 ①에 세 번째 회사가 붙을 수 있다**(추정).

**분포** — **59곳 전부 `done`**(US 22 · KR 9 · JP 7 · **EU 8** · IN 3 · 기타 3 · CN 2 ·
CA·AU·SG·AE·NG 각 1) · 비교 문서 32편 · **운영 사실 51개.**

**⚠️ 나르지 않는다** — 낡은 메모(314·317·325·348·349) · 두 방향 규칙(298·305·318·321) ·
...
나온다**(351) · **글을 열 때 '무엇이 없는지' 를 함께 묻는다**(352) ·
**⭐ 새로: 기준을 넘어도 못 쓰는 글이 있다 — 실제로 실행하지 않은 평가에는 구현 층이 없다.
그런 글은 기능이 아니라 회사 관찰로 살린다**(353).
