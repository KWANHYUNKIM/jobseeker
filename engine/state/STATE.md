...
**Inngest — 기능 하나를 채웠다(`step-memoization`, 끝난 단계는 다시 돌지 않는다).**
회사 129개, 큐 0/3. 빈 도메인 둘 — 공정 큐 · 저장소 이전.

### 결정론을 어디에 요구하느냐가 갈린다

중단된 일을 이어 붙이는 표준 답은 결정론적 재생이다 — 같은 코드 경로를 다시 걷게 하고 개발자에게
규칙을 지운다. 이 회사는 반대로 간다: **조율 계층에 결정론을 요구하지 않고 각 단계의 결과를 기억한다.**
단계 이름을 카운터와 함께 해시해 상태에서 찾고, 있으면 실행하지 않고 저장된 결과를 주입한다 —
배포가 바뀌어도 끝난 단계는 다시 돌지 않는다.

대신 조건 하나가 남는다. **함수를 처음부터 다시 돌리는 모델이라 단계 밖의 코드는 재개마다 다시
실행된다** — 결정론적이지 않은 일은 반드시 `step.run()` 안에 넣으라는 규약이 그 자리를 메운다.
이 엔진에서 Temporal 과 정면으로 갈리는 지점이라 `connections` 에 걸었다.

### 과금과 원가가 같은 모양이다

각 단계가 별개의 HTTP 요청이고 결과가 그때그때 바깥에 저장된다. 그래서 실행을 함수 한 번이 아니라
**함수 + 각 단계**로 세는 셈법이 원가의 모양을 그대로 따른다. 잘게 나눌수록 안전해지고 값도 오르며,
경계는 사용자가 정한다(재구성).

### 실행을 남의 인프라에서 돌린다

코드를 플랫폼으로 가져오지 않고 단계마다 사용자 앱에 HTTP 요청을 보낸다. 왕복이 붙는 대신 사용자의
런타임·의존성·비밀이 그대로 있는 자리에서 코드가 돈다.

### 다음 사이클 — 확장 (Inngest 의 빈 도메인)

단계가 잘게 나뉠수록 큐에 들어가는 항목이 늘고 공정성 문제가 커진다 — 공정 큐 글이 그 자리를
다룬다. 이미 읽어 뒀다.
