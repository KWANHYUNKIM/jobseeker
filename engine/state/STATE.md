...
**없다 — Ghostty 를 완주시켰다(도메인 3 · 기능 3 · `status: done`).** 회사 159개, **큐 0/3.**

### 예외로 만든 것을 흔한 것처럼 되돌리면 안 된다

이번 기능(`never-reuse-the-odd-one`)의 알맹이다. 페이지가 두 종류인데(고정 크기 표준은 풀로
돌리고, 더 큰 비표준은 `munmap` 으로 직접 없앤다), 스크롤백 정리 최적화가 **비표준 페이지를
메타데이터상 표준으로 되돌리고** 밑의 큰 `mmap` 은 그대로 뒀다. 그래서 해제할 때 풀 페이지로
여겨 **`munmap` 이 영영 안 불렸다.**

### 예외 경로는 드물어서 시험되지 않는다

1.0 때부터 있던 버그가 몇 년을 살아남은 이유다. **클로드 코드의 출력 양상**이 그 조건을 규모
있게 만들어 내서야 드러났다 — 하테나에서 30만 키워드 정규식이 언어 구현의 회귀를 드러낸 것과
같은 자리다(**평범한 시험이 만들지 않는 부하**).

### 근거 없이 정교해지지 않는다

"비표준 페이지가 얼마나 자주 나오는지 재서 가정을 동적으로 조정하자"는 제안을 **"뒷받침할 데이터
없이는 이르다"**며 물렸다. 대신 흔한 경우에 맞춘 단순한 규칙으로 되돌렸다.

### 이 프로젝트 안에서 같은 물음이 세 번 나왔다

**누가 이 메모리를 쥐고 있나** — Zig 과 GTK 의 수명이 어긋나 한쪽만 해제되던 자리, 메타데이터와
`mmap` 이 어긋나 `munmap` 이 안 불린 자리, 그리고 코어를 남에게 주려면 더 깨끗해야 한다는 자리.
`connections` 에 그 축을 심었다.

### 다음 사이클 — ⚠️ 후보 조사 (큐가 0/3 이다)

`blocked` 목록 — **incident.io**(404×3) · **Sourcegraph**(403) · **Guardian** · **Flexport** ·
**Framework**(LPCAMM2 딥다이브 본문 미확인). **지난 두 사이클에서 Fireworks 와 Ghostty 를 그
목록에서 되살렸다** — 접은 것과 못 읽은 것을 갈라 적는 습관이 계속 값을 한다.
