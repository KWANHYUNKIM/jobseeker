...
**Paytm(IN · 핀테크) `in_progress` — 도메인 3개 · 기능 1개.** 320 에서 도메인 ① 의 첫 기능
**`leave-what-costs-more`(다 옮기지 않기로 정한다)** 를 썼다. **결정 6개 · 그림 3장 ·
생각 4개 · 난제 2건.** 재료는 `Achieving Seamless Data Migration`(2023-04-12).

**⑫ 가 또 맞았다 — 재독이 새 재료를 넷 냈다**(319 에서 이미 읽은 글이다).
- **매니페스트 생성에 수 시간**이 걸렸고, AWS 지원과 상의해 **완료된 잡을 취소하고 작은
  배치로 쪼갰다** — *"To mitigate potential rate limiting and network bandwidth issues"*.
- **⭐ 재처리가 왜 2배인지의 진짜 이유** — S3 Batch 가 *"essentially performing a scan or a
  series of short List API calls on the entire 5 billion objects, incurring charges for all
  of the objects instead of just the millions within the prefix"*. **접두사 안의 수백만 개만
  손대려 해도 50억 개어치 값이 붙는다.**
- **에러 처리 자체가 없다** — *"S3 Batch Operations does not provide built-in error handling
  mechanisms"*. 재시도 없음과 별개의 한계다.
- **자체 도구** — 파이썬으로 S3 접근 로그를 **host 헤더 기준**으로 훑어 직접 접근되는 객체의
  **일간 리포트**를 만든다. CDN 페일오버는 **특정 HTTP 응답 코드**로 넘어간다.

**⭐ 기능의 축** — **양쪽 끝에서 안 옮길 것을 정한다.** 시작 전에 **15억 개**를 지웠고, 끝에서
권한 문제로 실패한 **수백만 개**를 재처리하지 않고 남겼다(*"likely not in use"*).
**⚠️ 회사가 이 둘을 하나의 원리로 적지는 않는다 — 이 묶음은 이 사이트의 것이다.**

**⚠️ 이 기능에서 가장 큰 빈칸** — **검증이 한 문장이다**(*"we conducted thorough testing"*).
50억 개를 옮기고 **개수 대조도 체크섬도 없고**, 수백만 개의 실패를 무엇으로 발견했는지도,
*"likely not in use"* 의 근거도 없다. `open_questions` 에 남겼다.

**⭐⭐ 비교 문서 두 편의 빈칸과 이어진다**(`open_questions` 에 남겼다) —
`reversible-first.md` 의 **아직 없는 것**에 *"되돌리지 못해 앞으로 밀고 나간 사례가 없다"* 가
적혀 있는데 **이 기능이 그 첫 재료**이고, `where-it-piles-up.md` 의 **갚지 않고 이사한 쪽**
(Careem 270TB→78TB)에서 **Paytm 은 한 걸음 더 간다 — 옮기다 실패한 것을 두고 갔다.**
**다음 비교 문서 사이클이 두 문서를 갱신할 수 있다.**

---

**다음 사이클 — 회사를 완주할 때까지 갈아타지 않는다. 남은 도메인 둘.**
- **② 거래 기록을 어디까지 맞출 것인가** — 재료는 `How Paytm Handles Millions of Digital
  Transactions Safely`(2022-09-16). Paxos 단일 리더(Raft·다중 리더를 버림) · 논리 DB 층 ·
  LSM · InnoDB · 강/약/중 일관성. **⚠️ 수치가 하나도 없다** — *"millions"* 와 *"superfast"*
  뿐이다. **기능을 쓰기 전에 한 편 더 읽는다**(두 편 규칙).
- **③ 앞단에서 끝낸다** — CDN 글은 기술 재료로만 썼다(버린 대안 0 · 대가 0).
  **기능은 안 읽은 `How Paytm Redis Clusters is Managing High Workloads`(2023-01-05)에서
  나올 가능성이 크다**(추정). **이쪽을 먼저 여는 편이 나을 수 있다.**
- **안 읽은 글 넷** — Redis 클러스터(2023-01-05) · 루팅/탈옥 기기 차단(2022-11-16) ·
  가짜 SMS 탐지(2022-03-03) · Prometheus 관측(2022-08-30). **뒤의 둘이 묶이면 네 번째
  도메인 감이다**(기기와 문자를 얼마나 믿을 것인가, **추정**).

**📌 그 밖**
- **⚠️ 블로그가 2023년 4월에서 멈췄다.** 글 열 편이 전부고 재방문으로 새 것이 나올 가능성이 낮다.
- **⚠️ 사업의 심장이 글에 없다** — 매출이 두 배로 는 **가맹점 대출 중개**(FY26 1분기 561
  크로르, +100%)를 다루는 글이 한 편도 없다. 정산도 없다.
- **큐 잔량 1/3**(Freshworks). **Paytm 을 다 판 뒤 후보 조사를 한 번 끼운다** — 다음 조사는
  **일본**(Sansan·freee·SmartHR·Money Forward)이나 **유럽**(Personio).
  **인도는 소진됐고 Trade Republic 은 Medium 호스팅이다.**
- **후보 조사 누적 열여섯 곳 중 넷.**
- **⚠️ 낡은 메모를 나르지 않는다**(314·317) — 후보를 나르기 전에 파일부터 대조한다.
- **⚠️ 두 방향 규칙**(298·305·318) — 한 편은 반드시 열고, **첫 편이 얇으면 한 편 더 연다.**
- **분포** — **54곳.** US 22 포화 · 한국 아홉 · 일본 넷 · EU 일곱 · 중국 둘 · 동남아 하나 ·
  **인도 둘** · 아프리카 하나 · 라틴아메리카 0(재시도 금지). **비교 문서 31편.**
