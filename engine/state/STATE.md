...
**turbopuffer 완주(기능 3개).** 회사 143개, 큐 0/3 — 다음 사이클은 후보 조사다.

### 숫자 하나가 셋을 동시에 정한다

포스팅 목록을 **약 256개짜리 블록**으로 자르는데, 그 크기가 압축 프레임(128 단위 비트패킹)과
MAXSCORE 블록 건너뛰기 효율과 키-값 메타데이터 분산을 **한꺼번에** 맞춘다. 세 이유가 한
숫자에 모이면 그게 답이라는 식의 결정이다.

### 메타데이터가 데이터를 이기고 있었다

예전에는 벡터 쪽 클러스터에 맞춰 잘랐는데 **조각 중앙값이 포스팅 1.5개**였다. 클러스터는
벡터를 위한 단위지 용어를 위한 단위가 아니었다 — 두 색인이 같은 저장소를 쓰되 자르는 규칙을
갈랐다.

### 압축은 줄이는 게 아니라 푸는 속도로 고른다

비트패킹이 단일 코어 초당 약 7억 4,100만 포스팅(약 4.5GB/s)이고 Zstd 같은 범용 압축은 약
1GB/s 다. 질의 경로에서는 **푸는 속도가 곧 지연**이라 그쪽을 골랐다.

### 세 기능이 한 줄로 꿰인다

진실을 객체 저장소에 둬 값을 두 자릿수 내리고(1), 캐시 층으로 지연을 되사고(2), 색인이 읽을
양을 줄여 왕복 안에 넣는다(3). **안 읽는 것이 곧 절약**이라는 문장이 세 자리에서 같다.

### 다음 사이클 — 후보 조사 (큐 0/3, Deno 만 남았다)
