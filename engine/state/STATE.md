...
**SourceHut — 프로파일만 썼다(도메인 둘, 기능 0).** 회사 155개, 큐 0/3.

### 안 하는 것으로 자기를 정의한다

추적도 광고도 없고, **모든 기능이 자바스크립트 없이 돌며, AI 기능은 하나도 없다.** 100%
자유·오픈소스이고 스스로 공개 알파라고 밝힌다. 이 엔진에서 "무엇을 안 넣었는가"를 제품 설명의
첫머리에 두는 회사는 드물다.

### 국가를 EU 로 정정했다

후보 조사 때 US 로 두고 "확인 필요"를 적어 뒀는데, 홈 하단에 **네덜란드 법인**(Hoorn, KVK
84165251, BTW NL003921490B16)이 있었다. 프로파일 사이클에서 확인해 고쳤다.

### 매일 겪는 것과 드물게 겪는 것

도메인 둘이 그 둘이다. **푸시 한 번의 길**(다섯 단계 — 디스패치·식별·인가·실행·후속)이 처음에는
단계마다 파이썬 시작 값 최대 1초에 SQL 왕복 20회 초과, **평균 60초에 육박**했다. 고로 다시 쓰고
질의를 합쳐 **가져오기 왕복 1회·푸시 최대 2회**가 됐다. 그리고 **무너진 날** — 170시간.

### 클라우드플레어 후원을 물린 이유가 적혀 있다

DDoS 로 상류가 서브넷을 널 라우트로 막았을 때, 후원 제안을 받는 대신 OVH NAT 을 세웠다 —
**SSH·SMTP·IRC 의 종단간 암호화를 지키려고**, 그리고 처음 견적보다 값도 쌌다고 밝힌다.

### 다음 사이클 — 확장 (첫 도메인)

`밀어 넣는 길을 짧게 만든다` 가 수치가 두껍다. **⚠️ 고로 옮기는 데 든 값(기간·인원)은 글에
없다** — 지어내지 말고 `limits` 로 적는다.
