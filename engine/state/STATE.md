...
**CyberAgent (JP · 광고) — 45번째. 회사 프로파일 완료. 도메인 2개 · 기능 0개. 큐 잔량 0/3.**

**⚠️ 카테고리 `광고` 를 새로 만들었다** — `validate.py` 의 CATEGORIES 와 `schema.json` 양쪽에 넣었다
(국가 코드 추가와 같은 패턴). **이 사이트에 광고를 파는 회사가 없었다.**

**ABEMA 태그(125편) 1쪽 14편을 봤고 두 묶음이 섰다.**
- **광고 배송 3편** — 페이싱 제어(63908) · 배송 고속화(64498) · 플랫폼 이설(61355)
- **캐시 2편** — 광고 배송 캐시(65145) · ABEMA 본체 다층 캐시(64293)
- (셋째 후보 — **관측 3편**: Vector 도입 60707 · Grafana Alert 61628 · Datadog Inferred Services 60208.
  **아직 한 편도 안 열었다.** 세 번째 도메인 후보로 남긴다.)

**본문 세 편을 읽었다(246 의 한 편 포함).**

**⭐ 페이싱 글(63908)이 이 회사 최고다.** **버린 대안 셋에 이유가 각각 다르다** —
① **선형 차 정규화**(e = t/T − N/G)를 버리고 **로그 비 오차**(e = −log(N/r))를 골랐다(비대칭 포화 ·
비율에 자연스러운 형태 · ∂e/∂N 이 N 에 반비례). ② **PID 의 D 항을 뺐다**(요청 스파이크를 채터링으로
증폭한다). ③ **임계값 결정론**을 버리고 **베르누이 추첨**을 골랐다(임계로 자르면 중간 구간이 영영
안 나간다). 대가도 적는다 — 포화대의 **I 항 폭주**(Anti-Windup 필요) · 확률 판정의 분산 ·
**G = 10³ 대 10⁶ 에서 진폭 1000배**.

**🔴 그런데 이 글에 이 회사 자신의 수치가 하나도 없다.** 인용된 숫자가 **전부 참고문헌의 타사 사례**다
(Verizon Media 0.5% · DoorDash 7%→4.7% · Roku 5분 · MicroAd 수십 초 · Ogury 하루 수십억).
**도입 전후를 잰 값이 없다.** 반면 같은 회사의 캐시 글(65145)은 자기 수치를 촘촘히 적는다
(Spanner p99 약 4배 · 릴리스 40분→15분 · TTL L1 1분/L2 4분 · L2 를 10분 데운다).
**글쓴이·팀에 따라 관행이 다른 것으로 보인다**(추측으로 적었다). **기능을 쓸 때 반드시 유의한다.**

**🔴 두 시스템이 같은 문제를 각각 풀고 서로를 언급하지 않는다.** 광고 배송과 ABEMA 본체가 둘 다
*"Pod 이 늘면 캐시가 흩어진다"* 를 겪고 둘 다 **Valkey 를 공유 중간층**으로 넣었다. **밀도는 크게
다르다** — 광고 쪽은 버린 대안 둘 + 대가 다섯 줄, ABEMA 본체 쪽은 **한 달짜리 인턴이 쓴 글**이라
버린 대안이 없고 개선 폭도 안 나온다.

**IR — 찾은 것과 못 찾은 것.** `www.cyberagent.co.jp/en/ir/` 는 **껍데기**였고 `library/trend5year/`
에서 받았다: **FY2025 순매출 8,740.30억엔**, **영업이익 717.02억엔**, 5년 추이(영업이익이 **FY2023
223.51억엔 바닥** 후 회복). ⚠️ **세그먼트별(광고/미디어/게임) 수치가 없다** — 다음에 `library/results/`
나 통합보고서(`report.cyberagent.co.jp`)를 연다. ⚠️ **ABEMA 시청자 지표(WAU/MAU)도 못 찾았다.**

**🚨 다음 사이클 — `--gaps` 가 [진행 중 확장]을 가리킬 것이다. 도메인 ①(페이싱)의 기능을 쓴다.**
**⚠️ 기능을 쓸 때는 그 도메인의 글을 두 편 이상 읽는다**(열일곱 번 확인된 규칙). 아직 안 연 둘 —
`archives/64498`(ABEMA 광고 배송 고속화, 2026-07-09) · `archives/61355`(광고 배송 플랫폼 이설,
2025-12-25). **⚠️ 페이싱 글에 자기 수치가 없으니, 두 편에서 실제 값이 나오는지 특히 본다.**

**⚠️ 큐가 0 이다 — CyberAgent 를 완주하면 곧바로 후보 조사다.**

**🔶 재시도 여지가 남은 둘(접지 않는다)** — **Gojek** `www.gojek.io/blog` **429**(두 번 다 429, 일시적일
수 있다) · **Cygames** `tech.cygames.co.jp` **헤더만 오고 목록이 JS 로 그려진다**(브라우저
`get_page_text` 로는 열릴 수 있다 — **게임 자리가 Roblox 하나뿐이라 값이 크다**, `/archives/` 는 404).

**아직 안 열어 본 후보** — Sea/Shopee(다른 주소?) · GoTo/Tokopedia · Traveloka · Robinhood ·
Plaid · Adevinta · Skyscanner. **📌 238 의 기준(유효)** — 자체 서비스를 굴리는 회사부터.
**📌 246 이 확인한 것** — 403 과 DNS 없음이 절반이 넘는다. 그래도 **여덟 곳에 한 곳은 나온다.**

**비어 있는 자리** — 라틴아메리카 0(접었다 · 재시도 금지) · **게임 1(Roblox — Cygames 가 실마리)** ·
...
⑨ **`index.json` 의 회사 엔트리는 `domain` 키를 쓴다**(`domains_count` 가 아니다 — 247 에서 잘못 넣었다가 고쳤다). 키는 `slug·name·name_en·country·category·status·updated_at·one_liner·domain·features_done`.
