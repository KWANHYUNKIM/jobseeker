...
**Linear — 기능 하나를 채웠다(`catch-up-not-full-download`).** 빈 도메인 하나. 회사 152개, 큐 1/3.

### 놓친 것만 골라 주는 것이 서버의 일이다

로컬 우선의 약속은 **끊겨도 쓸 수 있고 돌아오면 곧 최신이 된다**는 것이다. 그 약속을 지키는
길은 하나뿐이다 — 작업 공간 전체를 다시 주지 않고 체크포인트 뒤만 순서대로 주는 것.

### 고르는 일이 곧 값이다

권한·구독·ID 범위 **세 겹 필터**가 요청마다 교집합을 만든다. 필터를 늘릴수록 이 길이 비싸진다고
회사가 직접 적는다. 그래서 **늦은 보강** — 거를 때는 메타데이터만 들고 다니고 실제로 갈 것만
내용을 가져온다.

### 최근 것과 과거 것을 다른 출처에서 읽는다

2차 색인의 복제 지연이 **p50 약 1초, p95 몇 초**라서 방금 일어난 변경은 포스트그레스가 권위
있는 출처다. 경계에서 일부러 겹치게 두고 중복을 제거한다.

### 다음 사이클 — 확장 (마지막 빈 도메인)

`읽는 길의 모양을 바꾼다`. 같은 글이 재료다 — 꼬리 지연이 출렁인 이유(배열 겹침 술어로
상관없는 행까지 시험), 물린 GIN 색인(쓰기 값), turbopuffer 의 포스팅 리스트, 그림자 모드 검증,
turbopuffer 장애 시 포스트그레스 폴백.
