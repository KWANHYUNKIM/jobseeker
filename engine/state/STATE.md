...
**Inngest — 기능 둘을 채웠다.** `step-memoization` 과 `fair-queue-and-constraints`(함수마다 큐를
주고, 제약은 큐 밖으로 뺀다). 회사 129개, 큐 0/3. 빈 도메인 하나 — 저장소 이전.

### 불공정을 그림 하나로 정의한다

"한 사람이 5,000건을 넣고 그다음에 다른 사람이 하나를 넣으면, 그 5,000건을 다 처리해야 그 하나가
처리된다." 공유 큐를 쓰는 한 이 선두 차단은 사라지지 않고 **무작위 샤딩은 확률적 공정성일 뿐**이다.

답은 큐를 잘게 쪼개는 것이다 — 코드의 모든 브랜치에 있는 모든 함수마다 큐 하나. 그러려면 워커가
큐를 구독하는 방식을 버려야 한다. 그래서 일이 있는 함수를 지연순으로 담은 **발견 큐**를 두고 워커가
스스로 찾아가며, 훑어본 후보를 지연·용량·무료냐 유료냐로 가중치를 준 무작위 순서로 섞어 집는다.

### 같은 종류의 판단이 두 곳에 흩어져 있으면 따로 못 늘린다

흐름 제어가 갈라져 있었다 — 속도 제한은 스케줄링 때, 동시성과 스로틀링은 큐 안쪽 처리 중에. 그래서
셋이 막혔다: 제약 로직이 큐 코드와 얽히고, 동기 엔드포인트에는 못 걸고, 큐와 따로 못 늘렸다.
제약을 **별도 서비스**로 떼어(획득·연장·반납, 레디스 Lua 로 검사와 예약을 원자적으로) 따로 샤딩한다.

### 사람이 손대지 않아도 낫게 만든다

워커가 죽었는지 느린지는 밖에서 알 수 없다. 그래서 **시간 제한 리스**를 주고 만료되면 청소
프로세스가 회수하며, 재시도의 이중 지출은 멱등 키로 막는다. 만료를 기다리는 동안 용량이 노는 것이
그 대가다.

### 다음 사이클 — 확장 (마지막 빈 도메인)

제약 API 를 떼어 낸 것이 **FoundationDB 이전의 바탕**이라고 회사가 적는다. 저장소 이전을 채우면
Inngest 가 done 이 된다 — 다만 그 글 1부에는 수치가 없다는 것도 적어 뒀다.
