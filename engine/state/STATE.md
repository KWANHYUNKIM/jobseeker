...
**Ladybird — 기능 하나를 채웠다(`from-scratch-wpt`, 처음부터 짓되 남의 안전한 코드는 가져다
쓴다).** 회사 128개, 큐 1/3. 빈 도메인 둘 — 러스트 이전 · 프로세스 분리.

### 처음부터의 경계를 실용으로 그었다

엔진 코드는 다른 브라우저에서 가져오지 않는다. 그런데 **메모리 안전이 걸린 자리에는 검증된 남의
구현을 넣는다** — 텍스트 인코딩은 파이어폭스의 encoding_rs, GIF 디코딩은 Wuffs. 디코더는 수십 년째
취약점이 나오는 자리라 원칙보다 결과를 고른 셈이다(재구성). 여기에 문자열·ArrayBuffer 힙 분리와
기본으로 켜진 샌드박스(seccomp·Landlock·Seatbelt)를 더해 안전을 세 겹으로 쌓는다.

### 진도를 밖의 자로 센다

표준을 얼마나 구현했는지는 스스로 말할 수 없다. 그래서 **웹 플랫폼 테스트 서브테스트 수를 매달
공개한다** — 이달에 2,075,546 에서 2,078,912 로 3,366개를 더 통과했다. 후원으로 사는 조직이
진도를 증명하는 방식이자, 남이 만든 시험지가 목표가 되는 구조이기도 하다(재구성).

성능도 같은 태도다 — 크로미움 대비 배수를 목록으로 두고 나쁜 것부터 좁힌다(fillStyle 파싱 90배 →
2배, 리스트 삽입·삭제 120배 초과 → 20배 미만).

### 오픈소스인데 코드 기여를 안 받는다

6월에 개발을 유지보수자 전용으로 바꿨다. 브라우저는 오픈소스로 남고 커뮤니티는 테스트와 표준
논의로 참여한다. 표준 전체를 감당하며 속도를 내야 하는 조직이 검토 부담을 줄인 선택으로 읽힌다 —
**이 엔진에서 기여를 닫은 오픈소스를 보는 것은 처음이다**(재구성).

### 다음 사이클 — 확장 (Ladybird 의 빈 도메인)

러스트 이전이 Bun 과 정면 대비다 — 저쪽은 11일에 한 번에, 여기는 서브시스템 단위로 두 구현을
함께 돌려 대조하며 넘긴다.
