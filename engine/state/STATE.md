...
**DeNA** — 첫 기능 `overlap-then-cut`(겹쳐 두는 기간을 사고, 끊는 순간을 각오한다)을
끝냈다. **도메인 2 · 기능 1.** 큐 0/3. 남은 도메인은 **깨진 자리를 소스와 바이너리까지
파고든다.**

### 옮기는 동안은 조용하고 값은 끊을 때 나온다

Okta → 자체 IdP 에서 **350개 넘는 앱**을 **겹쳐 두고**(사용자는 Okta 로 로그인, 뒤에서
자체 IdP 가 인증) **1.5개월** 양쪽을 열어 뒀다. 그런데 정책을 **전환 모드에서 실제
강제로** 바꾸는 순간 **문의가 폭증**했다 — **사전에 알렸는데도.** **겹쳐 두는 동안
아무것도 안 해도 됐기 때문**이다.

### 이 원장에서 '끝내는 법'을 적은 드문 사례

카카오페이는 **Shadow 비교를 언제 끄는지 안 적었는데**, DeNA 는 **1.5개월 뒤 Okta 를
껐다**고 적고 **끄는 순간 무슨 일이 났는지**까지 쓴다. 앞 사이클에 적어 둔 빈칸이
다른 회사에서 채워진 자리다.

### 최악을 기준선으로 두면 판단이 선다

요금 증가를 **'두려워하던 몇 배 규모의 증가'와 견주어** 1.4배를 받아들인다. 절대 금액만
보면 '늘었다'로 끝나는데, **기준선을 세워** 판단했다. 그리고 **양쪽 요금을 다 적어** 읽는
쪽이 자기 기준으로 다시 볼 수 있게 했다.

### 보이는 것을 안 바꾸고 밑을 갈아 끼운다

**레츠 인크립트가 CRL URL 을 먼저 넣고 OCSP URL 을 뺀 것과 같은 순서**다 — 대체가 자리
잡은 뒤에 없앤다.

### 다음 사이클

확장 — 남은 도메인(MHA 추적). **10년 전 커밋까지 거슬러 간 조사**가 근거인데, **대응이
후속 글로 미뤄져 있다.** 큐가 0/3 이라 그 뒤에 후보 조사가 뜬다.
