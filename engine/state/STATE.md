...
**없다 — curl 을 완주시켰다(도메인 2 · 기능 2 · `status: done`).** 회사 178개, **큐 0/3.**

### 널리 쓰인다는 것이 곧 두 방향의 부담이다

이 프로젝트를 다 파고 나서 남는 문장이다. **들어오는 것도 나가는 것도 감당이 안 된다** —
보고는 몰려서 한 달간 문을 닫아야 했고, 배포된 바이너리는 손댈 수 없다. 그리고 **둘 다
상업 지원으로 풀리지 않았다**(신규 가입 0).

### 안 보이는 것과 없는 것은 다르다

마이크로소프트는 **정적으로 빌드해 `libcurl.dll` 을 아예 배포하지 않는다** — 그래서 윈도우
자체는 이 문제의 원인이 아니다. 그런데 같은 사실이 다른 것도 말한다: **가장 널리 배포된
curl 이 스캐너에 안 보이는 형태로 깔려 있다.** 동적 링크는 드러나서 문의가 몰리고, 정적
링크는 안 드러나되 갱신되지도 않는다 — **드러나는 위험과 숨은 위험 중 어느 쪽이 나은지가
분명하지 않다.**

### 할 수 없는 일을 분명히 하는 것도 지원이다

찾아온 사용자에게 **"우리는 못 고친다"** 고 말하는 것은 불친절해 보인다. 그런데 **원래 빌드
설정을 모르는 채 덮으면 "거의 확실히 크래시한다"** — 도우려다 망가뜨리는 쪽이 더 나쁘다.

### 상류가 할 수 있는 일은 전부 남을 거쳐야 닿는다

백포트도 장기 안정 릴리스도 조언도 **벤더가 받아 다시 빌드해야** 사용자에게 간다. 안 하는
벤더를 강제할 수단이 없고, 사용자는 **어느 벤더인지 스스로 찾아야** 한다.

### 심은 대비

**모질라**(AI 하네스로 버그를 만들어 내는 쪽) ↔ **curl**(받는 쪽에서 문을 닫는다) — 같은
비대칭의 양쪽 끝. **체인가드**(남의 취약점을 고쳐 다시 빌드해 판다) ↔ **curl**(상류에서
수정을 내지만 벤더를 거쳐야 닿는다) — 같은 사슬의 양 끝.

### 다음 사이클 — 후보 조사 (큐가 0/3 이다)

**CAD 축 세 번, 의료 축 여덟 번, 보험·위성 각 한 번 실패.** 못 읽은 곳: Hugging Face ·
Lunit · Blender(403) · Sourcegraph(403) · Lemonade(403) · OpenAI(403) · GitLab(403) ·
HashiCorp(500) · Algolia(목록이 안 뜬다) · Planet Labs · dbt Labs · incident.io(404 네 번) ·
M3 · Guardian · Flexport · Cal.com. **WebSearch 예산은 끝났다.**
