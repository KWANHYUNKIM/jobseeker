...
**curl**(오픈소스 프로젝트) — 회사 프로파일까지 썼다. **도메인 둘 · 기능 0.** 큐 0/3.

### 소수가 세상의 절반을 맡으면

두 도메인이 같은 뿌리에서 나온다. curl 은 거의 모든 기기에 들어가 있는데 **보고를 실제로
처리하는 사람은 소수**이고, **나가서 남이 다시 배포한 바이너리는 손댈 수 없다.** 기술 과제가
코드가 아니라 **위치**에서 온다.

### 모질라와 마주 보는 쌍이 데이터에 들어갔다

모질라는 AI 하네스로 버그 271개를 **만들어 내며** 비대칭 비용을 적었고, curl 은 **받는
쪽에서 한 달간 문을 닫았다.** 감수한 것도 적는다 — **CNA 의 72시간 응답 요구.**

### 상업 지원이 답이 아니라는 것이 드러났다

한 달 실험 뒤 **상업 지원 계약에 새로 가입한 곳이 0** 이었다. **취약점 대응 보장에 돈을
낼 동기가 사용자에게 없다**는 뜻이다 — 오픈소스 지속 가능성 논의에 그대로 쓸 수 있는
관측이다.

### 우리는 런타임에 패치해 줄 수 없다

**수천 개 앱이 `libcurl.dll` 을 깔고 일부는 `system32` 에 넣는다.** 버전도 취약점도
제각각인데 **"그 정확한 파일을 빌드한 사람이 그것을 갱신해야 하는 사람"** 이다. 프로젝트가
할 수 있는 것은 **장기 안정 릴리스와 백포트, 그리고 조언**뿐이다.

### 기호 규칙을 어길 뻔했다

회사 JSON 본문에 `⚠️` 를 아홉 개 썼다가 검사에서 잡아 전부 걷어냈다. **STATE·QUEUE 에는
쓰지만 회사 데이터에는 안 쓴다** — 뷰어가 읽는 본문이기 때문이다.

### 다음 사이클

빈 도메인 둘 중 하나. **보고를 감당하는 쪽이 자료가 두껍다**(중단 결정 · CNA 위험 · 상업
지원 0 · 불균형 문장 · 주기적 반복 계획). DLL 쪽은 **수치가 없지만** 제약이 선명하다.
