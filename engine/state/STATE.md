...
**아무것도 안 판다 — CyberAgent 를 닫았다(45번째, 도메인 4개 · 기능 5개). 큐 잔량 0/3.**

252 에서 **`observability-pipeline`(보내기 전에 걸러 낸다)** 을 쓰고 회사를 닫았다.
결정 7 · 그림 3 · 생각 4 · 스택 6 · 난제 2 · ui 6핀 · 연결 1(`alert-context` 와 **대비**).

**📌 원문 재독이 여섯 번째로 옳았다**(223·229·243·249·250·252). 요약에 없던 것이 넷 나왔다.
- **L4 에서 L7 로 바꾼 근거가 실측이다** — *"처음에는 kubernetes 의 서비스(L4 LB)로 … 제공하고
  있었습니다만, **Pod 별 메트릭을 보니 커넥션의 편향이 보였으므로** haproxy(L7 LB)를 경유시키고
  있습니다."* **관측 파이프라인을 관측해서 관측 파이프라인을 고친 자리다.**
- **Datadog Agent 를 앞에 두는 이유** — *"Vector 에 직접 애플리케이션 컨테이너의 표준 출력 등을
  접속시키지 않고 datadog-agent 를 경유시킴으로써 kubernetes 의 메타데이터로부터 datadog 관련 태그가
  생성되어 부가된 상태로 Vector 에 데이터가 전송됩니다."*
- **VRL 400줄이 하는 일 다섯** — ddtags 구조화 · 컨테이너 제외(**Vector 자신의 출력**과 **haproxy
  info 로그** 포함) · Argo Workflow 로그 정규화 · timestamp 정규화 · 태그 삭제로 카디널리티 감소.
- **🔴 버린 쪽의 장점을 인정한다** — 관리형에만 있는 *"실시간으로 로그를 흘리고 적용 결과를 확인하면서
  운용할 수 있는 구조"*. **이 회사 여섯 편 중 버린 대안의 좋은 점을 적은 것은 여기 하나다.**

**🔴 회사를 닫으며 `business_model` 에 패턴 세 개를 적었다**(이 사이트의 정리라고 밝혔다).
① **버린 대안과 대가는 잘 적는데 자기 개선 폭은 반만 적는다** — 여섯 편 중 **자기 수치를 낸 것은 셋**
(캐시 · 고속화 · Vector), **안 낸 것도 셋**(페이싱은 숫자가 전부 타사 참고문헌 · 이설은 글 전체에 셋뿐 ·
알림은 고치기 전만 센다). **버린 대안의 수와 수치의 유무가 상관이 없다.**
② **같은 시스템의 다른 층을 다루는 글들이 서로를 전혀 언급하지 않는다** — 광고 캐시↔ABEMA 본체
캐시는 **같은 문제를 각각 겪고 같은 답(Valkey)에 이르렀는데도** 그렇고, **관측조차 두 계열로 갈려 있다**
(광고 배송은 Datadog, 워크로드 감시는 Grafana).
③ **다섯 기능의 답이 전부 '줄이기' 다** — 한꺼번에 나가는 것 · 원본을 읽는 횟수 · 환경이 섞여 생기는
것 · 보내는 알림 건수 · 보내는 데이터. **광고를 팔면서 지면도 굴려 양쪽에서 값이 붙기 때문으로 보인다.**

**안 연 글** — Datadog APM Inferred Services(60208). 그리고 **고속화(64498)는 읽었지만 기능으로
안 썼다** — **얼마나 빨리**(코드 수준 처리량)라 어느 도메인과도 다른 문제였고, 버린 대안이 하나뿐이라
결정 5개가 안 섰다. **도메인 tech 에도 안 넣었다.**

**IR 에서 끝내 못 찾은 것** — **세그먼트별(광고/미디어/게임) 수치**와 **ABEMA 시청자 지표(WAU/MAU)**.
받은 것은 5년 실적뿐 — **FY2025 순매출 8,740.30억엔 · 영업이익 717.02억엔**(FY2023 223.51억엔 바닥,
**이유는 이 자료에 없다**). `library/results/` 나 통합보고서(`report.cyberagent.co.jp`)가 남았다.

**🚨 다음 사이클 — 후보 조사다. `--gaps` 가 그렇게 가리킬 것이다. 목표 3곳.**

**🔶 먼저 두드릴 둘(재시도 여지가 있다)**
- **Cygames** `tech.cygames.co.jp` — **헤더만 오고 목록이 JS 로 그려진다**(`/archives/` 는 404).
  **브라우저 `get_page_text` 로 열어 본다.** **게임 자리가 Roblox 하나뿐이라 값이 크다.**
  ⚠️ CyberAgent 자회사지만 **별도 회사·별도 블로그**다.
- **Gojek** `www.gojek.io/blog` — **429 가 두 번**. 시간이 지났으니 다시 두드린다. 동남아 두 번째.

**아직 안 열어 본 후보** — Sea/Shopee(다른 주소?) · GoTo/Tokopedia · Traveloka · Robinhood ·
Plaid · Adevinta · Skyscanner.
**📌 238 의 기준(유효)** — 개발자 대상 제품 회사의 블로그는 대개 영업 채널이다. **자체 서비스를
굴리는 회사부터.** 인프라(Vercel·Neon·Elastic·MongoDB·Confluent)는 뒤로.
**📌 246 이 확인한 것** — 403 과 DNS 없음이 절반이 넘는다. 그래도 **여덟 곳에 한 곳은 나온다.**
**📌 후보를 올릴 때 제목만 보지 않는다** — 목록을 열어 최신 글 날짜와 묶음을 보고, **본문 한 편을
실제로 연다.** 버린 대안과 대가가 없으면 안 올린다.
...
**📌 비교 문서 재료(27편 있음, 둘이 재료가 찼다)**
- **`되돌릴 수 있는 것을 먼저 한다`(reversible-first)에 CyberAgent 이설이 반례로 붙는다** —
  그 문서는 *"되돌림을 실제로 쓴 기록이 거의 없다"* 로 끝나는데 **여기는 되돌림 계획 자체가 없다.**
  대신 **임시 부품을 절차서에 올려 철거 계획과 세트로 관리한다**(`미루되 미룬 것이 안 보이게 한다` 와도 닿는다).
- **새 축 `관측에 값을 무엇으로 치르는가`** — CyberAgent `observability-pipeline`(돈: 이중 과금을
  피해 직접 굴리고 **관측 도구 자신의 로그를 뺀다**) + `alert-context`(주의력: *"알림의 형해화"*) +
  Yelp `partition-access`(재려고 만든 접근 로그가 **하루 TiB**) + Zerodha `log-storage`(색인 대신 압축,
  **행당 340바이트에서 17.38바이트**) + Doximity `prod-profiling`(*"프로파일러는 쓰기에 너무
  만족스러워서 첫 수로 삼고 싶어진다. 그러면 안 된다"*). **다섯이라 넉넉하다.**
