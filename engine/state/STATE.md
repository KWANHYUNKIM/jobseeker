...
**DuckDB — 도메인 셋을 다 채우고 done 으로 닫았다.** `quack-protocol` · `async-remote-read` ·
`peg-parser`. 회사 118개, 큐 2/3.

완주 기준 셋을 다 넘겼다 — 도메인마다 기능 1개 이상, 기능 셋이 서로 이어져 고아가 없고,
수익원(재단 회비)이 기능과 닿는다.

### 물려받은 것을 오래 쓰다 자기 것으로 갈아 끼운다

이 회사에서 같은 모양이 두 번 나왔다. 프로토콜에서는 포스트그레스 와이어와 Arrow Flight 를
이름 대며 물리고 자체 형식을 들었고, 파서에서는 2018년 시작 때부터 쓰던 PostgreSQL 유래
YACC/Bison 파서를 PEG 로 갈아 끼웠다. 둘 다 이유가 같다 — **남이 정한 형식에 묶이면 우리
일정이 남의 일정이 된다.**

갈아 끼우는 방식은 조심스럽다. 새 파서는 v1.2 에서 자동완성용 연구 시제품으로 들어와 v1.5 에서
옵트인이 됐다가 v2.0 에서 기본이 된다. 그리고 내부 AST 는 그대로 둬서 옵티마이저부터 실행기까지
뒷단이 교체를 모른 채 지나간다.

### 대가를 먼저 적는 회사인데, 딱 한 자리가 얇다

세 기능이 전부 자기 한계를 문장으로 남긴다 — 미리 읽기는 네트워크가 빠르면 메모리에 쌓인다 ·
병렬 삽입은 8스레드에서 정체하고 그 위는 PostgreSQL 이 앞선다 · 되돌아가기는 괄호 19개에
10.640초가 걸렸다. 그런데 **옛 파서와 같은 언어를 받아들인다는 동등성을 어떻게 검증했는지는
안 적는다.** 다르게 동작하는 질의를 만나면 알려 달라고 사용자에게 요청한다. 수치를 촘촘히 대는
이 회사에서 가장 얇은 자리라 `research` 의 난제로 남겨 뒀다.

### 다음 사이클 — 신규 (Antithesis)

큐가 2/3 이고 대기 맨 위가 Antithesis 다. 결정론적 시뮬레이션 테스트가 제품인 회사라
Turso·TigerBeetle 과 정면으로 붙는다.
