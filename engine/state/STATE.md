...
**Redpanda**(US · 스트리밍 엔진) — 도메인 2개 중 **1개에 기능이 들어갔다**
(`durability-as-a-dial`). 남은 빈 도메인 하나: '교차 AZ 비용을 오브젝트 스토리지로
옮긴다'. 큐 0/3.

### 성능은 갖춰 쟀는데 내구성은 안 쟀다

이번 기능에서 가장 눈에 띄는 비대칭이다. 성능은 **인스턴스 종류(im4gen.8xlarge 9대)·저장
매체 둘·파티션 1,395와 2,000·프로듀서 240과 500·처리량·프로듀서 설정까지** 갖춰 쟀다.
그런데 **내구성 쪽에는 같은 정성이 없다** — 장애 주입도, 브로커가 죽은 뒤의 복구 절차도,
실제로 얼마나 잃는지도 없다. **감수하라고 하면서 그 크기를 재 주지 않는다.**

### 안전하다는 근거가 기본값인데 그 기본값을 안 적는다

'상관 장애 때 소량 손실'이 드물다는 근거가 **'기본 플러시 간격이 낮다'** 인데, 정작
**`flush.ms`·`flush.bytes` 의 실제 기본값이 글에 없다.** 읽는 사람이 스스로 위험을 잴 수
없다.

### 덜 극적인 칸까지 낸 것이 벤치마크를 믿게 만든다

EBS 의 113ms→8.5ms 만 냈다면 훨씬 좋아 보였을 것이다. **NVMe 의 10ms→4ms** 와 **극한
부하에서 양쪽이 33ms 로 만나 이득이 사라지는 것**까지 함께 낸다.

### 물러설 선을 흔한 장애와 상관 장애 사이에 그었다

더 빠르게 하려면 **복제 전에 ack** 을 낼 수도 있었는데 그러지 않았다. 브로커 하나가 죽는
일은 드물지 않기 때문이다 — **건너뛴 것은 디스크 지속뿐이고 복제는 그대로 한다.**

### 심은 대비

**티커비틀**(모든 거래를 잃지 않는 것을 전제로 데이터베이스를 짓는다) ↔ **Redpanda**('모든
스트림이 기록 시스템으로 이어지는 것은 아니다') — **같은 인프라 층에서 전제가 정반대**다.

### 다음 사이클

남은 도메인 하나 — Cloud Topics. 대가는 수치로 적지만(0.25초 또는 4MB · L0 흩어진 읽기)
**물린 대안을 이름 대어 적지 않는다.** 그것으로 Redpanda 는 완주다.
