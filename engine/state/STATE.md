...
**Twilio** (US · SaaS — 통신 API) — 도메인 2개 · **기능 1개**(`intent-and-execution-apart`). 남은 도메인은 **`쪼개진 콘솔을 다시 하나로 합친다`** 하나다(✅ 자료 읽음, ⚠️ 2021년 글).

### 이번 사이클 — `의도와 실행 사이에 선을 긋는다`

건진 것 넷:

- ⚠️ **결합의 증상이 한 문장에 있다** — **`이벤트 처리의 문제가 즉시 모든 사용자의 API 가용성에 영향을 줬다.`** **API 운영·이벤트 처리·작업 스케줄링은 부하의 모양도 허용되는 지연도 다른데 한 프로세스에 묶여 있었다**(재구성). 그래서 **`어떤 컴포넌트의 실패든 시스템 전체의 장애로 번질 수 있었다.`**
- ⚠️ **그은 선이 명확하다** — **`제어 평면이 「무엇」을 소유한다 — 경보 규칙·임계값·설정을 정의하는 일. 데이터 평면이 「어떻게」를 소유한다 — 워크플로를 실행하고 대규모로 알림을 보내는 일.`** 얻는 것도 셋으로 적는다: **제어 평면이 흔들려도 데이터 평면은 계속 처리** · **각 평면이 독립적으로 확장** · **`의도와 실행 사이에 날카로운 경계를 그으면 팀이 한쪽을 다른 쪽을 방해하지 않고 진화시킬 수 있다.`**
- ⚠️ **쪼개는 것과 흩뜨리는 것은 다르다** — **`수십 개의 독립 서비스를 관리하는 대신, 오케스트레이션 플랫폼은 복잡도를 시각적이고 관리 가능한 워크플로로 표현하게 해 준다.`** ⭐ **같은 회사가 콘솔에서 정확히 그 대가를 치렀다** — 마이크로 프론트엔드가 파편화를 낳아 결국 다시 합쳤다(재구성).
- ⚠️ **`옮기는 동안` 이라는 상태의 값을 치른다** — **`세심하게 조율된 이중 쓰기`** 로 두 시스템의 데이터를 맞추고 **피처 플래그로 읽기와 쓰기를 따로** 옮긴다. **빅뱅 재작성이 피하는 비용이 정확히 이것이다**(재구성).

### ⚠️ 이 글에는 수치가 없다

**처리량·지연·이전 기간·실패율이 하나도 없고 `하루 수백만 건` 이 유일한 규모 표현이다.** **얻은 것을 `운영 안정성을 유지하면서 점진적으로 개선할 수 있는 능력` 처럼 능력으로 말한다.** ⚠️ **읽는 쪽에서는 그 능력이 얼마나 값을 했는지 확인할 방법이 없다**(재구성) — metrics 에 그 사실을 항목으로 적었다.

### 다음 사이클 — 마지막 도메인, 그리고 완주 판정

`--gaps` 가 **`쪼개진 콘솔을 다시 하나로 합친다`** 를 부를 것이다. **자료를 이미 읽었으니 바로 쓴다** — ⚠️ **다만 2021년 4월 글이라 낡았고, 결정이 다섯 개 이상 나오는지 먼저 본다.** 자료가 서면 **같은 사이클에서 완주 판정까지** 한다.

⚠️ **큐가 0/3 이다.** 이 회사를 다 판 뒤 **3순위(후보 조사)** 가 걸린다.
...
**⑨ ⭐ `한 번에 갈아엎을 것인가 목 졸라 죽일 것인가` — 재료가 찼다** — **Twilio**(Strangler Fig · 이중 쓰기 · 읽기와 쓰기를 따로) / Etsy(테이블마다 1%→100%) / Plaid(앞으로 실패하는 길) / Paystack(제자리 업그레이드).
...
**⑮ `언제 쪼개고 언제 합치나`** — ⭐ **Twilio 한 회사 안에 두 방향이 다 있다** / DoorDash / Deliveroo.
