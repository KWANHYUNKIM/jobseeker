...
**Wise(EU/UK · 핀테크—국경 간 송금)** `in_progress` · **도메인 3 · 기능 2** —
78곳 · ⚠️ 큐 0/3.

방금 쓴 기능: **`deploy-as-events`(배포를 한 번의 거래가 아니라 지켜보는 흐름으로 바꾼다)**.

### ⭐⭐ 회사가 먼저 못 박는다 — 도구 교체가 아니다

`This wasn't just a tool swap` · `a paradigm shift from viewing deployments as simple
transactions to seeing them as orchestrated sequences of events`.
**흐름이 되면 중간에 검증과 되돌리기를 끼워 넣을 자리가 생긴다** — 카나리가 그 자리다.

### ⭐ 두 숫자가 같은 저울 위에 있다

**트래픽 5%** 와 **30분**. 적게 보내면 닿는 범위가 작지만 **신호도 약하고**, 오래 보면
지표가 움직이지만 **배포마다 그 시간이 붙는다.** ⚠️ **그 값을 어떻게 정했는지는 글에
없다** — 회사가 적는 것은 **그 값으로 막아 낸 것**(2024년 수백 건 · 수천 시간)뿐이다.
실패 그림을 그 저울로 그렸다.

### ⚠️ 이 회사 글의 일관된 성격 — 얻은 것만 적는다

**사내 도구 Octopus 를 버린 대가도, CI 이전 자체의 비용도 안 적는다.**
⏳ **다만 본문이 `our earlier post about the state of our CI/CD pipeline` 을 가리킨다** —
**그 글을 찾으면 대가가 나올 수 있다.** `open_questions` 에 남겼다.

### 다음 사이클 — 마지막 도메인이자 이 회사에서 가장 정직한 글

**`관측 스택을 옮기고 나서야 진짜 비용을 만난다`** — Mimir 압축 글(전문 읽음).
**여기서는 회사가 자기 실수를 직접 적는다** — 압축기 최적화를 뒤로 미뤘고, **저성능
알림이 없었다.** 증상 넷이 **어느 것도 압축을 안 가리켰다.**
**약 25만 블록** · 가장 큰 테넌트에 **두 달치 백로그** · 메타데이터 동기화만 **약 9시간**.
고친 뒤 **S3 70%↓ · AZ 간 전송 40%↓ · 질의 지연 90%↓.**
여기까지 채우면 **완주 조건에 닿는다.**
