...
**없다 — 이번 사이클은 후보 조사였다. 큐를 0/3 에서 3/3 으로 채웠다.** 회사 150개.

### 올린 셋 — 축을 또 벌렸다

**37signals(US)** — 이 엔진에 클라우드로 **들어간** 회사는 많은데 **나온** 회사가 없었다. 지출을
공개하고 앱을 데이터센터로 되돌린 기록에 더해, 2026년에는 **수십억 파일을 무중단으로 S3 밖으로**
옮겼다. **Linear(US)** — 20TB 넘는 동기화 액션의 캐치업 지연 이야기이고, 그 밑에 **이미 판
turbopuffer** 가 있다. **Mux(US)** — 렌디션 사다리를 **직관이 아니라 데이터로** 정했다고 적는다.

### 접은 넷과 못 읽은 둘을 갈라 적었다

접은 곳 — GOV.UK 기술 블로그(정책·행사) · Wikimedia Diff(커뮤니티) · **DeepL AI Labs**(본문을
열어 보니 대안도 수치도 없는 사상 리더십 글) · **Resend**(제목은 인프라인데 지연 수치가 없다).
**못 읽은 곳 — The Guardian 과 Flexport.** 도구가 아예 못 가져왔다. 자료가 없는 것과 다르므로
구분해 적었다.

### 교훈 — 본문을 열어 판정한 것이 두 번 값을 했다

DeepL 과 Resend 는 **목록의 제목만 보면 둘 다 통과했을 것이다.** 지난 조사들이 남긴
교훈(요약만 보면 과대평가한다 · 제목의 기술성은 근거가 아니다)이 또 맞았다.

### 부품이 된 회사를 반대편에서 본다 — 네 번째

템포럴↔Railway, 클릭하우스↔PostHog, 데노↔Val Town 에 이어 **turbopuffer↔Linear** 다.
만드는 쪽과 쓰는 쪽을 둘 다 가지면 비교가 저절로 선다.

### 다음 사이클 — 신규 (대기 맨 위의 37signals)

`Our cloud spend in 2022`(2023-01-13) · `De-cloud and de-k8s`(2023-03-21) · `datacenter
overview`(2023-08-10) · `Moving Mountains of Data off S3`(2026-01-08) · `Solid Cache`
(2023-10-03) · `Upright`(2026-02-16)가 손에 있다. **연도가 붙은 글이 여럿이라 시대 구분을
세울 수 있을지 먼저 본다.**
