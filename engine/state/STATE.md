...
**없다 — 문체 보수 사이클이었다.** Cursor 를 배민 문체로 다시 썼다(4.2 → 0.0). 회사 115개, 큐 2/3.

### 이번 사이클에서 만든 것 — 재사용할 변환기

Neon 은 전부 손으로 고쳤는데 71곳을 그렇게 하면 사이클이 감당이 안 된다. 그래서 1차 변환기를
스크래치패드에 만들었다. 기호와 백틱 인용과 굵게를 걷어내고 줄머리에 남는 대시와 겹공백을
정리한다. 도해(code)는 건드리지 않는다.

다만 변환기만으로는 배민 문체가 안 된다. 기호를 빼도 문장이 이어지지 않기 때문이다.
그래서 절차를 둘로 나눴다.

1. 변환기로 1차 정리 — 기호·인용·굵게가 0 이 된다.
2. 읽는 사람이 먼저 보는 곳을 손으로 다시 쓴다 — one_liner, business_model,
   domains[].why, features[].name, business.why. 그리고 인용부호가 빠져 어색해진 자리를 다듬는다.

Cursor 에서는 마지막 단계가 특히 필요했다. 인용을 풀면 "복제본을 더할수록 푸시 처리량은 더
나빠진다 를 끊는다" 처럼 조사가 붕 뜬 문장이 남는다. 그런 자리를 열다섯 군데 고쳤다.

### 이번 사이클 — Cursor

사업 모델을 다시 쓰면서 성장 곡선이 이 회사의 기술 문제를 거의 다 설명한다는 것을 앞세웠다.
3개월마다 두 배로 자라면 인프라가 그 속도를 따라가야 한다.

도메인 경계의 근거를 셋 다 적었다. 저장 계층과 라우팅을 따로 볼까 했으나 라우팅 테이블을
없앤 것이 진실 원천을 S3 로 옮긴 결과라 한 경계로 뒀고, 시작 시간을 줄이는 일과 실행 위치를
옮기는 일은 같은 목표를 향해 있어 한 경계로 뒀으며, 모델을 고르는 일과 빠르게 돌리는 일은
라우터가 고른 모델을 실제로 돌리는 값이 선택의 전제라 한 경계로 뒀다.

앞선 회사와 견주는 문장도 넣었다. 맞았는지 어떻게 아느냐는 물음에 Block 은 표본 수와 잔여
여지로 재고 Anthropic 은 재는 자와 무대까지 의심하는데, Cursor 는 사용자의 다음 행동에서 읽는다.

기능 이름은 배민을 따라 주제와 핵심을 붙였다. `복제본을 더할수록 푸시가 느려졌다 — 합의를
버린 자리`, `실행만 옮기고 생각은 남긴다 — 손과 머리를 가르는 일`, `사용자가 다음에 무엇을
하는지로 잰다 — 묻지 않고 읽는 평가`.

### 다음 사이클 — 문체 보수 (사다리 3순위)

70곳이 남았다. 변환기가 있으니 사이클마다 한 회사씩 돌리고 앞머리만 손으로 고치면 된다.
