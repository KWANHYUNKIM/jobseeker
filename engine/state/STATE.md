...
**없다 — `in_progress` 0 · 완료 55곳.** 328 에서 **후보 조사를 해서 세 곳을 다 채웠다.
큐 3/3 — 목표 달성은 이번이 처음이다.**

**✅ 셋 다 본문을 열어 확인했고 셋 다 기준(버린 대안·대가·수치 중 둘)을 넘겼다.**
- **Sansan**(JP · SaaS, `buildersbox.corp-sansan.com`) — 열어 본 글은 **모노레포 점진 이관**
  (2024-05-27). **버린 대안 둘**(빅뱅 이관 / OSS 도구 대신 직접 스크립트, 이유 넷) ·
  **대가 넷**(⭐ **CI/CD 잡 이관이 모노레포 이관보다 손이 갔다** · 커밋이 빨리 흘러 분기
  기준이 낡는다 · 저장소가 커진다 · **병합 충돌 걱정은 기우로 판명됐다고 스스로 적는다**) ·
  **수치**(저장소 40개 · 20명 미만 · 100만 줄). **블로그가 살아 있다**(최신 2026-08-25)이고
  **이관 글이 여럿이라 도메인이 여럿 설 가능성이 크다.**
- **freee**(JP · SaaS, `developers.freee.co.jp`) — 열어 본 글은 **인증·인가 기반
  마이크로서비스화 회고**(2022-08-31). **⭐ 이 사이트에 드문 종류다 — 후회를 적는다.**
  DDD 재설계로 *"旧基盤との挙動の違いが大量に発生しました"*, 레거시의 부정 데이터가 새
  기반에서 에러, 견적 부족으로 연기 다발, *"開発メンバーの精神が摩耗する状態となっていました"*.
  교훈이 **"잘게 릴리스했어야"** 와 **"마이크로서비스화 전에 옛 기능을 리팩터했어야"** 다.
  **수치** — 2019~2022.6(4년 가까이) · QA 가 **14제품 633 테스트 케이스**로 사내 최대.
- **SmartHR**(JP · SaaS, `tech.smarthr.jp`) — 열어 본 글은 **왜 모듈러 모놀리스로 시작해
  마이크로서비스로 옮기는가**(2026-06-11). **수치가 이번 조사에서 가장 강하다** — CI
  **30분 이상 → 약 1분**, 배포 **1일 1회 → 수시**, 조회 **3.59~4.46배**(5만 명 13.7초→3.07초),
  행 크기 **약 71% 감소**. **버린 대안**은 처음부터 별도 서비스로 만드는 안. **대가**도 적고
  **벤치마크가 엄밀하지 않다고 스스로 밝힌다.**

**⭐⭐ 이번 조사에서 가장 값이 큰 발견 — freee 와 SmartHR 이 같은 문제를 정반대 순서로 풀었다.**
둘 다 **인증·권한 기반을 제품에서 떼어내는** 이야기인데, **freee 는 모놀리스에서 곧장
마이크로서비스로 가서 4년이 걸렸고 그것을 후회로 적는다.** **SmartHR 은 모듈러 모놀리스로
시작해 Packwerk 로 경계를 먼저 보이게 한 뒤 떼어낸다** — 그리고 **떼어내기 전에 HTTP 통신으로
바꿔 인터페이스를 미리 길렀다.** **비교 문서 재료로 굵다**(이 대비는 이 사이트의 것이다).

**⚠️ Personio(EU)는 안 봤다** — 세 곳을 채워서 멈췄다. 다음 후보 조사 때 첫 순위다.

**후보 조사 누적 성적 — 열아홉 곳 중 일곱**(298: 6→1, 305: 3→1, 311: 3→0, 318: 4→2,
**328: 3→3**). **328 이 처음으로 목표를 채웠다** — 일본 SaaS 는 접근이 잘 되고 회고 문화가
있다(**추정**).

---

**다음 사이클 — `--gaps` 가 [신규]를 가리킬 것이다**(큐 3/3, `in_progress` 0).
**⑭ 대로 출력을 실제로 보고 따른다.**
- **[신규]면 대기 표 맨 위 Sansan 을 `in_progress` 로 만들고 PROMPT.md 3단계(회사 프로파일)부터.**
  319·323 에서 한 방식 그대로 — **비즈니스 모델 · 제품 · 도메인 후보까지 쓰고 커밋, 기능은
  다음 사이클.** ⚠️ **목록 페이지가 열리는 회사이니 실제로 열어 글을 세고 주제 묶음을 본다.**
- **⭐⭐ 그런데 비교 문서 재료도 굵어졌다.** 사다리에서 비교 문서는 8순위라 [신규]가 위지만,
  **다음다음 사이클에 쓸 후보로 STATE 에 남긴다**:
  ① 320 `leave-what-costs-more`(Paytm) → `reversible-first.md` 의 *"되돌리지 못해 앞으로
  밀고 나간 사례가 없다"* 와 `where-it-piles-up.md` 의 **갚지 않고 이사한 쪽**.
  ② 326 `know-both-versions`(Freshworks) → `reversible-first.md` 의 *"언제 롤백 경로를
  닫는지 정한 곳이 없다"* 에 **첫 재료**이고, `swap-while-running.md` 의 **앱이 아는가
  모르는가** 축에서 **앱이 아는 쪽** 재료이기도 하다(**추정 — 그 문서를 다시 읽고 확인한다**).

**📌 운영 사실에 더할 것**
- **⭐ 후보 조사에서 주소가 실제로 열리는지도 확인한다**(327 의 교훈, 328 에서 실천했다).
  Freshworks 는 글이 있는데 **절반이 안 열렸다.** 이번엔 **셋 다 본문을 열어 보고 올렸다.**
- **⚠️ 낡은 메모를 나르지 않는다**(314·317·325). **⚠️ 두 방향 규칙**(298·305·318·321).
- **분포** — **55곳.** US 22 포화 · 한국 아홉 · **일본 넷(큐에 셋 더 대기)** · EU 일곱 ·
  중국 둘 · 동남아 하나 · 인도 셋 · 아프리카 하나 · 라틴아메리카 0(재시도 금지).
  **비교 문서 31편.**
