...
**Adyen — 29번째 회사.** 사이클 167 에서 열었다. 프로파일 + 도메인 2개 + 기능 1개
(`accounting-ledger`). **다음 기능은 `결제 처리 경로`** — 1부 자료는 이미 읽었고 두 번째 글로
`the-adyen-way-of-engineering--engineers-as-designers-architects-coders-and-testers` 를 읽으면 된다.

> **이 회사의 축 — 병목이 결제가 아니라 장부에 있다.** 결제는 초당 수백 건인데 **한 건이 회계
> DB 에 약 50행**을 남겨 삽입이 **초당 수천 건**이 된다. **바쁜 쪽과 눈에 띄는 쪽이 다르다.**
> 그리고 순매출을 처리 볼륨으로 나누면 **약 0.17%**(이 사이트의 계산)라, **처리 원가가 곧
> 이익률**인데 **한 건이라도 틀리면 그건 돈**이라 정확성을 깎아 원가를 줄일 수 없다.

> **가장 값진 대목 — 거절했던 이유가 사라진 뒤.** Kafka 를 안 쓴 이유는 취향이 아니라 사실이었다
> (**"그때는 exactly-once 전달을 지원하지 않았다"**). 그래서 PostgreSQL + 자바로 직접 만들었는데,
> **그 사실은 영구적이지 않다.** 근거가 사라진 뒤에도 자체 구현은 남고 그 위에 최적화와 운영
> 지식이 쌓인다 — **바꿀 이유와 안 바꿀 이유가 같은 곳에서 자란다**(해석). 회사는 결론 대신
> **질문을 공개했다**: *"이 틀이 20배를 더 갈 수 있을지, 아니면 Kafka 같은 오픈소스로 갈아야
> 할지 사내에서 논쟁 중이다."* 판정 기준(**사내 20배 규칙**)이 있어서 가능한 기록이다.

> **⚠️ 비교 축 ⑧(직접 운영 vs 맡길 것인가)의 재료가 이제 찼다.** Canva `session-revocations`(Redis 를
> *"클러스터를 우리가 운영해야 한다"* 며 버림)는 **이미 쓰였지만**, 남은 것으로도 충분하다:
> **Adyen `accounting-ledger`**(Kafka 대신 직접) · Spotify `podcast-previews`(K8s 대신 관리형
> Dataflow, 대신 Beam 업그레이드에서 의존성 지옥) · Grafana `mimir-next-gen`(**이미 쓰임**) ·
> ByteDance `muxflow`(**이미 쓰임**) — **미사용만 세면 둘이니, 문서를 쓰기 전에 반드시 미사용
> 기능 목록을 다시 뽑아 확인할 것.** Adyen 의 `결제 처리 경로` 기능을 채우면 **CockroachDB 를
> 가져다 쓴 것**과 **온프레미스 결정**이 같은 축의 재료로 더해진다.

> **🚫 인도 후보는 두 번 막혔다.** `blog.zomato.com` 에 이어 **정규 URL `www.zomato.com/blog/...`
> 도 60초 타임아웃**. 새 단서가 없으면 다시 꺼내지 않는다. 큐는 **2/3 유지**(Grab 하나 남음) —
> **다음 후보 탐색은 아프리카·중동·동유럽 쪽에서 WebFetch 가 실제로 열리는 곳을 먼저 확인할 것.**

**큐 잔량 2/3 — 목표 미달.** Grab.

마지막 사이클: 167 (2026-08-22) — Adyen 개설 + 회계 원장.
