...
**없다 — 문체 보수 사이클이었다.** Anthropic 을 배민 문체로 다시 썼다(2.22 → 0.0, 기호 272개). 회사 115개, 큐 2/3. 34곳 남았다.

### 재는 도구를 의심하는 셋 — 의심하는 자리가 다 다르다

첫 기능에서 축이 셋으로 찼다.

- Etsy 는 **자기 실험 체계**를 의심한다(좋아 보여서 고른 처치일수록 효과를 부풀려 본다).
- Ramp 는 **재현의 조건**을 의심한다(설정을 고정해도 상류 데이터가 흔들린다).
- Anthropic 은 **무대**를 의심한다 — 모델을 안 바꾸고 인프라 설정만 바꿔도 6%포인트가 갈리는데,
  그 폭이 리더보드 상위 모델들 사이의 간격보다 크다.

평가를 어디서 얻느냐에서 Cursor 와도 갈린다. 저쪽은 사용자에게 묻지 않고 다음 행동을 읽고,
여기는 실험실 안의 무대를 먼저 고친다. 파는 것이 남의 제품에 들어가는 부품이라 벤치마크가 곧
영업 자료이기 때문이다.

### 뇌와 손을 가르는 이유가 둘 — 속도냐 수명이냐

- Cursor 는 **에이전트 속도**를 감당하려고 실행만 옮기고 생각은 남겼다.
- Anthropic 은 **수명** 때문에 가른다. 특정 구현보다 오래 살아남을 작은 인터페이스 묶음을 만들어
  컨테이너가 애완동물이 되는 것을 피한다.

그리고 DoorDash 와 같은 방향으로 끝난다 — 둘 다 지금 만드는 에이전트가 오래 안 갈 것을 전제로
그 밑을 짓는다.

### 자기 흠을 적는 회사가 넷이 됐다

- ClickHouse — 고치는 대신 무엇을 고쳤는지만 적어 둔다.
- Honeycomb — 브로커 교체가 8~12시간에서 48~72시간으로 나빠진 구간을 남긴다.
- DoorDash — 가장 어려웠던 것이 기술이 아니라 사람이었다고 IKEA 효과라 이름 붙인다.
- Anthropic — **여러 겹의 리뷰와 시험과 자체 사용을 다 통과하고도 버그가 운영에 닿았다**는 것,
  그리고 내부 사용도 평가도 그것을 재현하지 못했다는 것.

이 자리에서 앞의 두 도메인이 되돌아온다. 재는 자를 의심한 첫 도메인도 뇌와 손을 가른 둘째
도메인도 이 사고를 막지 못했다 — 셋째 도메인이 사후에 서 있는 것이 이 회사 문서의 구조 자체다.

### 도메인을 제품이 아니라 신뢰를 어떻게 만드느냐로 잘랐다

모델·API·클로드 코드로 가르면 셋이 다 한 칸에 들어가면서 이 회사가 공개한 것의 성격이 사라진다.

### 다음 사이클 — 문체 보수 (사다리 3순위)

--gaps 가 정한다. 지금 맨 위는 perplexity(2.21)지만 그건 이 시점의 예상이지 확정이 아니다.
