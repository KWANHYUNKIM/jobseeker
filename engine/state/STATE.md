...
**없다 — Bolt 를 닫았다.** 완료 **68곳** · **큐 1/3**(Mercado Libre) · 비교 문서 33편.

**다음 사이클은 6순위(신규) — Mercado Libre 다.** PROMPT.md 3단계부터.
⚠️ **Medium 이라 브라우저(`get_page_text`)가 필요하다.**
**연도가 1999·2010·2015·2018 로 다 붙어 있어 `eras` 를 첫 사이클에 채울 수 있다** —
이 엔진에서 드문 기회다(68곳 중 연표가 있는 곳이 얼마 안 된다).
읽을 글: `the-technological-evolution-at-mercado-libre`(읽음) + Fury 연작의 나머지
(트래픽 보안 · 비용 최적화 · 멀티클라우드 전략).

### ⭐⭐ Bolt 에서 남길 것 — 실패를 세 번 적는 회사

설정 주도 프레임워크 · 정규식 자리표시자 · LLM 변환. **셋 다 "해 봤고 안 됐다"를
그대로 적는다.** 특히 LLM 은 **모델을 올려도 나아지지 않았다**고 밝힌다
(`even after the release of Claude Opus 4/GPT-5`). **이런 글이 이 엔진의 최상급 재료다** —
후보 조사에서 이런 결을 찾으면 바로 올린다.

### ⚠️ 두 번 반복된 관찰 — 같은 회사 안에서 글의 결이 갈린다

Nubank(지연 편은 대가를 적고 FinConnect 편은 안 적음)에 이어 Bolt 도 그렇다 —
**사기 검문과 엔진 이전은 실패를 적는데, 결제 대사 편은 규모 수치가 하나도 없다.**
**후보 조사에서 읽은 한 편이 회사 전체를 대표하지 않는다**(두 번째 확인).
