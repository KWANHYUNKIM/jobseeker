...
**PostHog — 프로파일만 썼다.** 회사 140개, 큐 1/3(Val Town 만 남음).
도메인 셋이 다 비어 있고 셋 다 자료를 확보했다.

### 만드는 쪽 옆에 쓰는 쪽을 세웠다

이 엔진에 클릭하우스는 **만드는 쪽**으로 이미 있는데 쓰는 쪽이 없었다. 여기는 그 데이터베이스의
제약을 자기 핸드북에 적고 그에 맞춰 구조를 바꾼 기록이다 — **갱신과 삭제가 비싸서 제자리 수정
대신 변화를 새 행으로 모형화**하고, 웨어하우스 용도로는 "그냥 안 맞았다"고 적는다.

### 한 클러스터를 함께 쓰다 데였다

초기에는 하나였는데 **무거운 배치 작업이 사용자 질의와 자원을 다투면서 성능이 나빠졌다.** 지금은
워크로드마다 전용 클러스터를 둔다 — 대화형·적재·세션 리플레이·로그·배치 내보내기·API·내부 도구.

### 카프카가 완충재이자 함정이다

트래픽이 튀거나 클러스터를 못 써도 견디려고 앞에 카프카를 둔다. 그런데 핸드북이 위험도 적는다 —
**적재 중에 카프카 테이블을 질의하면 오프셋이 진행돼 데이터를 잃을 수 있다.**

### 안 되는 것을 옆에 둔다

클릭하우스로 웨어하우스를 못 하겠다고 판단하고 덕DB 를 붙이되 포스트그레스 호환 감싸개
(Duckgres)를 앞에 세웠다. 성능도 갈아 적는다 — 작은 데이터의 콜드 질의는 덕DB 가 앞서고,
큰 데이터에 구체화 뷰와 색인을 다듬으면 클릭하우스가 늘 앞선다.

### 다음 사이클 — 확장 (PostHog 의 빈 도메인)

`한 데이터베이스로 안 되는 것을 옆에 둔다` 부터. 대가가 이름 대어 갈려 있다(수평 대 수직 확장,
조율 복잡도 대 개발 편의).
