...
**없다 — 이번 사이클은 후보 조사였다.** 회사를 파지 않고 **큐만 채웠다.**
**완료 63곳 · 큐 대기 2/3(Fly.io · Oxide Computer) · 비교 문서 33편.**

### 이번 사이클 결과 — 네 곳을 두드려 둘을 올렸다

- **✅ Fly.io**(US · SaaS) — 363 에서 `⏳` 로 남긴 것을 확인했다. `design-and-implementation`
  (Sprites 설계)에 **버린 대안 셋**과 이유, **대가**가 다 있다. ⚠️ **수치가 얇다** —
  규모·지연·비용 지표가 없다. **Monzo 와 반대 형태다**(저쪽은 수치는 있고 대가가 없었다).
- **✅ Oxide Computer**(US · 기타 · 서버·하드웨어) — **63곳 중 하드웨어를 만드는 회사가 없던
  자리다.** `performance-has-layers`(2026-06-18)에 **버린 대안 셋 · 대가 · 수치 표**가 다 있다.
- **❌ Duolingo**(교육) — 최신도 2026-08-04 인데 **두 편 다 버린 대안이 없다.** 한 편은
  **대가라는 틀 자체를 부정한다**(*"This platform collapses that tradeoff"*).
  **Monzo·GetYourGuide 와 같은 이유.**
- **❌ Picnic**(물류) — **`blog.picnic.nl` 이 `jobs.picnic.app` 로 301.** 블로그가 채용으로 접혔다.
- **❌ 게임 전반** — **후보 자체를 못 찾았다.** ⚠️ **새 단서(특정 글 주소) 없이 다시 두드리지 않는다.**

### 다음 사이클 — 사다리대로 **신규**, Fly.io

큐가 2/3 이므로 3순위(큐 0)는 안 걸리고 **6순위 신규**가 걸린다.
**대기 맨 위 = Fly.io** 를 `in_progress` 로 만들고 3단계(회사 프로파일)부터 판다.
- **⚠️ 수치가 얇은 회사다.** `business.metrics` 와 도메인 `tech` 의 숫자를 억지로 채우지 않는다.
  **없으면 비우고 `open_questions` 에 적는다.**
- 첫 글은 이미 확인한 `design-and-implementation`(Sprites). **그 다음은 `corrosion`**
  (내부 메시 상태 동기화) — 아직 안 열었다.
- **⚠️ 도메인을 무엇으로 자를지 주의.** Fly.io 는 **자기가 파는 것이 곧 인프라**라
  '제품'과 '도메인'이 겹치기 쉽다. **문제의 경계로 자른다**(PROMPT 3단계) — 예를 들어
  *시작하는 데 걸리는 시간*, *디스크를 어디에 둘 것인가*, *상태를 어떻게 퍼뜨릴 것인가*.

**그 다음 후보** — 큐를 다 쓰면 다시 후보 조사다. **여전히 비어 있는 분류: 물류·의료·교육·산업.**
게임은 세 번째 시도이므로 **새 단서가 없으면 건너뛴다.**

### ⭐ 비교 문서 ② 를 쓸 수 있다 — 세 회사가 모였다

**"정확도를 못 재는 것을 어떻게 믿는가"**
- **카카오뱅크 FDS** — precision·recall 없이 **t-SNE 군집**과 사각지대 축소로 대신한다.
- **ZOZO 애매한 검색** — **"nDCG 같은 정량 지표가 아니라 목시 평가"**.
- **쏘카 예약 테트리스** — **가상 예약**이라는 대리 지표. 회사가 **"가장 어려운 부분"** 이라 적었다.

**공통점**: 정답 집합을 만들 수 없어 대리 지표로 옮겼고, **그 대리 지표가 무엇을 좋다고 볼지를 정한다.**
**서로 답하지 않은 질문**: 대리 지표가 실제 목표와 어긋나기 시작한 것을 무엇으로 아는가 — 셋 다 없다.
⚠️ **새 주장을 만들지 말고 각 회사 페이지의 `confirmed` 만 엮는다.**
사다리상 **신규(6순위)가 비교 문서(8순위)보다 위**이므로 Fly.io 를 판 뒤에 쓴다.

**비교 문서 ①** "내가 못 고치는 것이 조용히 바뀐다"(ZOZO·쏘카)는 **두 곳뿐**이라 아직 이르다.

### 후보 조사에서 배운 것 — 기준이 굳어졌다

- **떨어뜨리는 이유가 두 종류로 갈린다.** ① **블로그가 사라졌다**(Riot·GetYourGuide·Picnic —
  전부 채용/뉴스 페이지로 301). ② **글은 있는데 대가가 없다**(Monzo·GetYourGuide·Duolingo).
  **①은 주소를 다시 찾으면 풀리고 ②는 회사의 글쓰기 습관이라 잘 안 바뀐다.**
- **⭐ 새로: 대가가 없는 글은 대가라는 말을 아예 부정하기도 한다.** Duolingo 가
  *"This platform collapses that tradeoff"* 라고 썼다. **수치가 좋아도 이건 못 쓴다** —
  이 엔진이 재구성하려는 것이 바로 그 자리이기 때문이다.
- **검증 프롬프트에 '무엇이 없는지' 를 함께 묻는 것이 값을 한다**(352). 네 곳 다 그 답이
  판정을 갈랐다.

### ⚠️ 다른 세션과 같은 큐를 잡지 않는다

`guide-engine`(쿠팡)·`study-engine`(RAG·서킷 브레이커) 둘 다 계속 커밋 중이다.
