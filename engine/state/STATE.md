...
**Cerebras — 기능 하나를 채웠다(`defect-tolerant-wafer`, 결함을 피하지 않고 견디게 설계한다).**
회사 127개, 큐 2/3. 빈 도메인 둘 — 가중치 이동 · 커지는 모델.

### 존재의 조건을 먼저 팠다

이 회사의 나머지 전부가 웨이퍼를 자르지 않는다는 결정 위에 서 있는데, 그 결정이 오랫동안
불가능하다고 여겨진 이유가 수율이다 — **칩이 클수록 수율이 지수적으로 나빠진다.** 46,225㎟ 짜리
칩은 그 계산으로 성립하지 않는다.

답은 그 규칙을 뒤집는 것이 아니라 **전제를 바꾸는 것**이었다. 결함 하나가 칩 전체를 죽인다는
전제를 깨면 된다 — 코어를 약 0.05㎟(H100 코어의 1% 남짓)로 잘게 만들어 결함이 죽이는 면적을
줄이고, 라우팅을 동적으로 다시 짜 죽은 자리를 지나가며, 여벌 코어를 7%만 둔다. 같은 TSMC 5나노
웨이퍼에서 **H100 크기 GPU 72개가 361㎟ 를 잃을 때 웨이퍼 한 장은 2.2㎟ 만 잃는다.** 물리 코어
97만 개 중 90만 개를 써 활용률 93%다.

### 자기 주장의 크기를 스스로 깎는다

164배라는 숫자만 내세울 수도 있는데 한계를 같은 글에 적는다 — 결함이 한곳에 몰리면 내결함 영역을
넘어 칩 전체가 죽을 수 있고, 실제 수율은 100%에 가깝지 않으며, **면적의 약 절반인 메모리·캐시·
인터커넥트는 코어 크기의 이점이 그대로 적용되지 않아 따로 내결함 설계가 필요하다.**

### 도구의 한계를 절차로 또 썼다

목록 페이지에는 모델 발표와 제휴 글이 많아 이 글이 안 보였다. **검색으로 찾아냈다** — 티그리스에서
얻은 절차(목록이 얇으면 검색으로 내려간다)를 두 번째로 썼다.

### 다음 사이클 — 확장 (Cerebras 의 빈 도메인)

메모리 벽 글은 읽어 뒀다. 결함 내성이 성립해야 웨이퍼 위 SRAM 44GB 와 패브릭 53.5 PB/s 가 있고,
그것이 있어야 가중치를 안 옮긴다는 이야기가 선다 — 두 도메인을 `connections` 로 걸어 뒀다.
