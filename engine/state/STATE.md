...
**Moniepoint — 41번째 회사, 아프리카 첫 곳.** 사이클 216 에서 **프로파일 + `ui_map`** 까지 썼다(기능 0개).
`NG` 를 **validate.py 와 index.json 양쪽**에 넣었다.

> **이 회사의 자리 — 가용성이 기능이 아니라 상품이다.**
> 회사가 *"0.1% 의 가용성 저하"* 를 **수천 건의 거래**로 환산해 적고, 배포 기준을
> *"피크 트래픽의 2배까지 지연 곡선이 평평한지"* 로 잡는다. **가게가 결제를 못 받으면 그날 장사를 못 한다.**
> 토스(개인 슈퍼앱)·Stripe(개발자 API)·Adyen(큰 글로벌 가맹점)과 달리 **나이지리아의 작은 사업자 수백만 곳**이고,
> **결제만이 아니라 장부·재고·여신까지** 받는다.

> **⚠️ 블로그가 작다 — 전부 8편 남짓이고 `payments`·`data` 분류는 아예 비어 있다.**
> **결제 회사인데 결제 글이 한 편도 없다.** Careem(6편에 도메인 셋은 과했다) 교훈대로 **도메인을 2개만** 열었다.

> **다음 사이클** — 도메인 **`멈추면 안 되는 것을 프로덕션에서 굴린다`** 의 기능 하나.
> 본문을 이미 확인한 둘이 재료다 — 가용성(**99.99% 목표** · 기본 Spring Boot 헬스체크를 버림)과
> 저축(**직접 호출을 버림** — *"데모에서는 되고 프로덕션에서는 무너진다"* · **insert-first 중복 제거** ·
> *"약간의 완전성을 훨씬 큰 신뢰와 맞바꿨다"*). **⚠️ 규칙대로 두 번째 글을 더 읽는다** —
> `what-running-kafka-on-vms-taught-us-about-systems-thinking`(2026-07) 이나
> `scheduled-jobs-in-java-spring-kubernetes-part-1/2`(2026-07).

> **그다음** — `돈의 기록이 맞다는 것을 어떻게 아는가`. 대사 글(**1,000억 건 이상**)은 읽었고
> **`system-migration-data-accuracy`(2025-06, 99.9%)는 제목만 확인했다 — 기능 전에 반드시 읽는다.**

> **📌 큐 2/3** — Target(US · 리테일 물류) · bol.com(EU/NL, ⚠️ **최신 글 2024-11 이라 멈췄을 수 있음**).
> **라틴아메리카는 다섯 곳 실패 후 접었다**(새 단서 없으면 재시도 금지). **게임은 Roblox 하나**(Unreal 403).
> 비교 문서 축 셋도 재료가 찬 채 대기 중이다.

**큐 2/3** — Target(US) · bol.com(EU/NL).

마지막 사이클: 216 (2026-08-27) — Moniepoint 회사 프로파일 + `ui_map`.
