...
**(회사는 없다. 사이클 233 은 비교 문서 `which-way-it-leans` 를 썼다 — 25편째.)**

**🚩 재료가 차 있던 비교 문서 축 셋을 다 썼다** — 231 `machine-says-so` · 232 `after-you-measure` ·
233 `which-way-it-leans`. **다음 사이클은 보강(사다리 5순위)이다.**

**233 의 축: 무엇을 내줬는가가 아니라 오차를 어느 쪽으로 몰았는가.**
다섯 사례에서 중요한 것이 **얼마나**가 아니라 **어느 쪽으로**였다. 열쇠 문장은 Doximity 에서
나온다 — 겹침과 간극 중 **겹침을 받아들였다**(재생돼도 *"결국 수렴"*).
**겹친 것은 시간이 지우지만 빠진 것은 시간이 못 만든다**(이 문장은 이 사이트의 정리).

**새로 보인 것 셋.**
- **기운 방향이 전부 '나중에 고칠 수 있는 쪽' 이다** — 중복 쪽(멱등이 흡수) · '안 쓰인다' 쪽
  (내리는 결정에만 쓴다) · 버리되 DLQ 로 흔적 · 주문 한 건 안쪽(누적 안 됨) · 추천 안 함 쪽
  (사용자가 직접 찾을 수 있다). **오차를 없앨 수 없으니 되돌릴 수 있는 쪽으로 몰았다.**
- **내준 양을 아는 곳은 둘뿐이다**(Yelp 0.001% · Target 9%). **버린 것이 관측 가능한 자리에
  있느냐가 갈랐다** — 늦게 온 로그는 도착하니 세어지고 브루트포스는 돌려 볼 수 있다.
  나머지 셋은 **버린 것이 애초에 시스템에 안 들어온다.**
- **Doximity 는 구멍을 못 없애면 운영에 등록한다** — *"건너뛴 캐스케이드 모니터링과 메타데이터
  신선도가 운영 모델의 일부"*.

**🚨 다음 사이클 — 보강(사다리 5순위). 지금 경고 15건이 그대로 할 일 목록이다.**
`python3 engine/validate.py` 출력이 정확히 그 목록이다.
1. **문자열 엔티티 14건 정규화** — `domain_model.entities` 가 `"이름 — 설명"` 문자열인데
   스키마는 `{name, what}` 객체다. **discord 4**(message-storage · presence-fanout · voice-sfu ·
   automod-layers) · **figma 1**(multiplayer-doc) · **netflix 1**(adaptive-playback) ·
   **shopify 4**(pods-isolation · bfcm-readiness · checkout-reservations · storefront-renderer) ·
   **youtube 4**(uvq-quality · content-id · watch-next-ranking · ggc-edge).
   ⚠️ **뷰어는 ` — ` 로 갈라 읽으므로 화면은 지금도 정상이다** — 고치는 것은 형식 통일이다.
   **기계적이라 한 사이클에 몰아서 할 수 있다.** 스크립트로 ` — ` 첫 번째를 기준으로 갈라
   `{"name": 앞, "what": 뒤}` 로 바꾸되, **갈라지지 않는 항목이 있으면 손대지 말고 남긴다.**
2. **kakao 의 빈 도메인 tech** — `domains[동기화·다중 기기]` 에 `tech` 가 없다(STYLE.md 2번).
   **자료를 찾아야 하므로 1번과 같은 사이클에 넣지 않는다.** `tech.kakao.com` 은 **브라우저로만**
   본문이 읽혔는데 **브라우저가 지금 탭을 못 만든다**(228 에서 확인) — 열리지 않으면
   **도메인 레벨 `hold_reason`** 을 붙이는 것도 정직한 답이다.

**🔴 비교 문서 절차(231~233 에서 굳었다)** — `domains/index.json` 을 읽고 **기존 문서 하나를
통째로 읽어 짜임을 따른다**(제목 → 인용 블록[비교 대상·확인/추정 규칙·교차 참조·범위] →
도입 한 문단 + 증거 하나 → 표 → 갈래 해설 → `## 이 사이트에 아직 없는 것`). **표와 산문만 쓴다.**
**재료는 기억이 아니라 회사 파일에서 직접 읽는다.** `validate.py` 는 비교 문서를 안 보므로
**`companies`/`features` 값이 실제 slug·key 와 맞는지 직접 검산한다.**

**⚠️ 큐는 0 이고 후보 조사가 막혀 있다**(230 에서 여덟 곳 두드려 0곳). **보강까지 끝나면 그때
다시 후보를 찾는다** — 그전에 **기존 43곳 목록과 반드시 대조한다**(226 에서 Grab 파일을 덮어썼다).
**비어 있는 자리** — 라틴아메리카 0(접었다) · 게임 1(Roblox) · 아프리카 1(Moniepoint) ·
인도 1(Zerodha) · 여행 1(trivago). **동남아는 Grab 이 있어 비어 있지 않다.**

**남은 비교 문서 씨앗(재료가 아직 덜 찼다)** — '미루되 미룬 것이 안 보이게 한다'
(Doximity `cdc-overlap` base+delta · Careem `cost-leaks` 언두 로그 · PlanetScale
`concurrency-limits`) · '되돌릴 수 있는 것을 먼저 한다'(Figma 논리→물리 샤딩 · WebGPU 폴백 ·
Shopify 섀도 모드 · 토스 카나리 · Yelp `in-place-swap`). ⚠️ **후자는 233 과 결이 가까우니
쓸 때 범위를 갈라야 한다** — 233 은 *오차의 방향*, 저건 *작업의 순서*다.
