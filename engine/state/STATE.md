...
**Kraken Technologies(EU/UK · 에너지)** `in_progress` · **도메인 3 · 기능 2** —
67곳 · ⚠️ 큐 0/3.

방금 쓴 기능: **`types-after-the-fact`(이미 커진 코드에 타입을 뒤늦게, 만지는 것부터
입힌다)**. 결정 7개가 **전부 `confirmed`** 다 — 이 회사는 고른 이유와 버린 값을 매번 적는다.

### 두 기능이 정면으로 부딪힌다 — 그게 이 회사의 모양이다

**하루 100번 넘게 내보내는데 mypy 가 CI 파이프라인에서 가장 느리다**
(`the slowest thing in our CI pipeline`). 그런데 `follow_imports` 를 `silent` 로
낮추지 않고 `normal` 로 둔다(`We don't use it at Kraken (yet)`) — **빠짐없음을 사고
속도를 내줬다.** 두 기능의 `connections` 가 서로를 가리키게 해 뒀다.

### ⭐ 사업 구조가 타입 문제로 그대로 나타나는 자리

**고객사마다 추상 모델을 확장한 구체 모델이 따로 있다.** 그래서 추상 모델을 가리키는
코드는 **가능한 모든 구체 모델의 유니온**을 상대해야 하고, 그걸 풀려고 플러그인이
**앱을 띄워 런타임 인트로스펙션을 한다** — **정적으로 보려고 한 번 살려 보는** 역설이다.
다음 도메인(남의 전기 사업)이 바로 이 이야기의 다른 쪽 얼굴이다.

### ⚠️ 회고 하나가 값지다 — 일괄 변경의 창은 닫힌다

Black 을 **한 번의 거대한 PR** 로 적용했는데, 회사가 돌아보며 적는다:
`admittedly something that would be much harder to do now that the codebase has grown.`
**그래서 그다음 큰 변경(타입)은 2.5년짜리 점진 작업이 됐다**(이 인과는 재구성이다).

### 다음 사이클 — 마지막 도메인

**남의 전기 사업을 같은 코드 위에 올린다.** 자료는 `upgrading-rabbit-mq...`(읽음) +
제품 소개(읽음) + **`django-kubernetes-health-checks`(2026-02-11, 아직 안 읽음)**.
여기까지 채우면 **Kraken 이 완주 조건에 닿는다** — 수익원이 기능과 이어지는지 함께 본다.

### 아직 못 잡은 것

- **Octopus Energy 와의 관계**를 1차 문장으로 못 찾았다.
- **과금·가격·매출 비공개.**
- **연표(eras) 비었다.** ⚠️ 블로그가 **2015년부터** 있다 — `django-elb-health-checks`(2016) ·
  `django-project-structure`(2018) · `python-interfaces-a-la-go`(2019) 가 연도 붙은 재료다.
  **2016년 글과 2026년 글이 같은 주제(Django 헬스체크와 지속 배포)라 나란히 놓으면 시간축이 보일 수 있다.**
