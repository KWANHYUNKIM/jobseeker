...
**없다 — 문체 보수 사이클이었다.** Pinterest 를 배민 문체로 다시 썼다(2.4 → 0.0). 회사 115개, 큐 2/3. 54곳 남았다.

### 권한을 어디에 두나 — 셋이 다른 답을 냈다

Pinterest 의 세 번째 기능이 이 축이다. 인프라를 코드로 관리하면 코드를 고칠 수 있는 사람이
인프라를 고칠 수 있게 되는데, 수백 개의 테라폼 워크스페이스가 여러 레포에 흩어져 있으면
그 하나하나가 전체를 건드릴 수 있는 문이 된다. 그래서 코드가 자기 권한을 스스로 정하지
못하게 파이프라인 쪽에 뒀다.

- Coinbase 는 흩어진 관리 도구들을 중앙 인가와 감사 계층으로 모았다. 다만 너무 많이
  중앙화하면 플랫폼 팀이 모두가 기다리는 대상이 된다는 것도 같이 적는다.
- Modal 은 반대로 전역 조율을 없애고 모든 워커를 자기 진실 원천으로 만들었다.
- Pinterest 는 권한을 코드 밖에 두되 중앙에 모으지는 않는다.

배치를 정하는 일과 권한을 정하는 일은 중앙에 두는 값이 다르다는 것을 Coinbase 사이클에서
적었는데, Pinterest 가 그 사이의 세 번째 자리를 채운다.

### 이번 사이클 — Pinterest

기술 이야기가 무엇을 보여 줄지로 모인다는 것을 앞세웠다. 홈피드에 무엇을 꺼내 올지,
사용자를 어떤 쓰임새로 이해할지, 그것을 굴리는 인프라를 어떻게 안전하게 둘지다.

참여와 유지가 다르다는 문장도 그대로 살렸다. 사용자가 오늘 사워도우 레시피 열 개를
저장하고도 다음 달에 이탈할 수 있고, 추천은 이미 좋아하는 것을 더 보여 주는 데는 뛰어났지만
오래 남게 할 새 관심사를 찾아 주지는 못했다.

도메인 경계의 근거도 적었다. 조건을 고르는 일과 후보를 꺼내는 일을 따로 볼까 했으나
조건이 곧 검색기의 입력이라 한 경계로 뒀다.
