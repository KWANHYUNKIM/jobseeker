...
**bol(EU · 커머스) `in_progress` — 도메인 1개 · 기능 1개.** 351 에서 첫 기능
**`move-four-years-of-past`(과거를 두고 갈 수 없어 사 년 어치를 옮긴다)** 를 썼다.
**결정 7개 · 그림 3장 · 생각 4개 · 난제 2건 · 연결 0건**(첫 기능이라 이을 곳이 없다).

**⑭ `--gaps` 가 [확장] bol 을 정확히 지목했다.**

**⭐ ⑫ 가 또 값을 냈다 — 이미 두 번 읽은 글인데 세 번째 읽기가 넷을 더 냈다**
(`open_questions` 에 적었다):
- **전환 순간의 처리** — *"we must stop the old application, while we are collecting messages
  in the new applications to process everything at least once"*. **옛 앱을 멈추고 새 앱이
  모아 두었다가 적어도 한 번 처리한다.** ⚠️ **적어도 한 번은 두 번일 수도 있다는 뜻이고,
  정산에서 중복은 크레딧을 두 번 내는 일이다**(이 지적은 이 사이트의 것이다).
- **리허설 방식** — 포크한 앱에서 테스트를 다 끄고(스프링 컨텍스트 로딩만 남긴다)
  **별도 Google 프로젝트**에 **서비스 간 통신을 끈 채** 승격 데이터베이스를 붙여 돌려 봤다.
  ⚠️ **그래서 연동에서 생기는 문제는 이 연습에서 안 보인다.**
- **준비 단계에 새 인덱스를 만드는 일도 들어 있다** — 걷기만 하는 게 아니었다.
- **상태 테이블은 주 테이블의 마지막 처리 id 를 남겨** 중단 뒤 재개를 가능하게 했다.

**⚠️ 그리고 빈칸을 여럿 확인했다** — **검증 이야기가 아예 없다**(행 수 대조·체크섬·정합성
절차 어느 것도 없다) · 전환에 얼마나 멈췄는지 · 중복을 무엇으로 걸렀는지 · 어떤 인덱스와
제약을 걷었는지 · **왜 병렬화가 되레 느려졌는지** · 승격이 레플리카 승격을 뜻하는지와 옛
데이터베이스의 뒷일.

**⭐⭐ 난제 둘이 이 기능의 값이다**
- **자원이 노는 것과 자원이 병목인 것을 가르는 일** — CPU 가 **4퍼센트**인 것을 보고 병렬화로
  **15퍼센트**까지 올렸는데 6시간짜리가 **약 7시간**이 됐다. **병목이 알고리즘이면 병렬화는
  조율 비용만 더한다.** 결국 서브셀렉트를 임시 테이블로 바꿔 **10~12시간.**
- **옮긴 것이 맞는지 어떻게 아는가** — 3TB 를 행 단위로 대조하는 일은 옮기는 일만큼 들고,
  **스키마가 바뀌었으니 단순 체크섬도 안 맞는다.** **⭐ 이 사이트가 읽은 Paytm 도 50억 개를
  옮기고 검증이 한 문장이었다 — 큰 이관에서 반복되는 빈칸으로 보인다**(추정).

**⚠️ 맺음말의 교훈이 전부 사람에 대한 것이다** — 여러 사람과 여러 번 이야기하라 · 겸손하고
참을성 있게 도움을 청하라 · 작게 나눠라. **기술 교훈이 하나도 없다.** *"nobody in the company
had ever tackled something quite like this before"* 와 맞물리는 것으로 보인다(추정).

---

**다음 사이클 — 두 편 규칙을 채운다.**
- **⭐ 1순위** — `Fast is slow, slow is fast - rethinking Our Data Engineering Process`
  (2024-11-07). **같은 날 글이라 이어질 가능성이 있다**(추정). **223 규칙으로 실물을 보고
  같은 도메인인지 새 도메인인지 정한다.**
- **2순위** — `Bol's journey in shifting left and shifting right: our Vision` 과
  `... Canary Releases`(둘 다 2024-05-22, **두 편이 짝이라 파기 좋다**) ·
  `Database Setup So Easy, Your Cat Could Do It: Docker and Flyway Edition`(2024-08-02,
  **Flyway 가 이번 기능에도 나왔다 — 이어질 수 있다**, 추정).
...
- **⚠️ 주소는 목록에서 얻은 경로만 쓰고 열리는지 확인한다**(⑰·327·328). **블로그 최신이
  2024-11-07 이다.**

**⚠️ 큐 1/3 — bol 을 완주하면 후보 조사를 한 번 끼운다.** 대기는 **PayFit** 하나
(`backstage.payfit.com`, 2/3, 얇고 2024-07 에서 멈춤). **후보 조사 누적 스물세 곳 중 아홉.**

**⭐⭐ 비교 문서 재료 셋**(회사를 파는 중에는 손대지 않는다):
① **freee 대 SmartHR 의 순서** — freee 가 후회로 적은 순서를 SmartHR 이 실제로 밟아 보고
**모듈러 모놀리스로 경계만 정리하는 안을 버렸다**(인가가 과금·테넌트에 의존해야 해서 **의존
허용 규칙이 곧 경계 설계가 되고** 배포 독립성과 가용성은 규칙으로 안 풀린다).
② **`미루되 미룬 것이 안 보이게 한다`** 에 SmartHR 두 사례 — 미룬 쪽은 **놀고 있는 접속**,
미리 잰 쪽은 **연 1회 정점의 부하 시험**.
③ **bol 이 ②의 셋째 사례** — 버린 첫 대안이 **중개 앱을 2031년까지 그대로 돌리는 것**인데,
**미루는 쪽을 값으로 계산해 거부했다.** ⚠️ **다만 그 값을 숫자로 적지는 않는다.**
④ **⭐ 새로: `되돌릴 수 없는 것이 어디에 있는가` 에 붙을 재료도 생겼다** — bol 은 **적어도
한 번** 처리를 골랐고, 정산에서 중복은 크레딧을 두 번 내는 일이다. **검증이 없다는 사실과
함께 보면 더 무겁다**(추정 — 쓸지는 완주 뒤에 정한다).
...
**`category`·`country` 는 허용값 집합이 있다**(350) · **⭐ 새로: 같은 글을 세 번째 읽어도
새 사실이 나온다 — 요약은 '무엇이 없는지' 를 특히 잘 떨어뜨린다**(351).
