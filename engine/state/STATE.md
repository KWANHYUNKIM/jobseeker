...
**Antithesis — 회사 프로파일까지 썼다(in_progress).** 회사 119개, 큐 1/3.

도메인 셋을 세웠고 기능은 아직 0개다.

- **결정론을 밑바닥에서 만든다** — `So you think you want to write a deterministic hypervisor?`(2024-03-20)
- **이상한 상황을 스스로 찾아다닌다** — 제품 소개 + `Finding bugs in Raft implementations`(2026-07-27)
- **찾은 것을 다시 돌려 원인까지 데려간다** — 제품 소개(시간여행 디버깅·인과 분석)

### 같은 기법을 쓰는 세 번째 회사인데 자리가 다르다

Turso 는 SQLite 테스트 스위트를 물려받을 수 없어 재현되는 시스템을 스스로 지었고 TigerBeetle 도
자기 데이터베이스를 그렇게 시험한다. 안티테시스는 **그 능력을 남에게 판다** — 자기 시스템을 위한
도구가 아니라 남의 시스템을 넣을 방을 만들어 파는 쪽이다. 그래서 제약이 반대 방향으로 생긴다.
스스로 지은 쪽은 자기 코드를 결정론에 맞춰 쓰면 되지만, 파는 쪽은 남이 이미 쓴 코드를 그대로 받아야
해서 결정론을 하이퍼바이저까지 내려가 만든다.

### 재현을 위해 코어 하나로 몰아넣는다

여러 코어가 동시에 움직이면 순서가 임의로 섞이므로 그 자유도를 아예 없앤다 — 하이퍼바이저 한 벌이
물리 코어 하나에서만 돈다. 처리량은 48~96코어 서버에 VM 을 여럿 띄워 벌고, 코어 간 동기화를 피한
덕에 오히려 성능 이득이 있었다고 적는다. 시간을 읽으려는 모든 시도에는 가상 시간을 돌려주고,
성능 모니터링 카운터가 약 1조 명령어에 하나꼴로 잘못 센다는 것까지 밝힌다.

### 찾은 버그가 곧 광고다

라프트 구현 하나를 한 시간 돌려 성격이 다른 버그 셋을 갈랐다(합의 깨짐 · 리더 이양 후 교착 ·
스냅샷 설치 라이브락). 노드를 죽이지도 않고 네트워크 분할과 교란만으로 재현했다. 제품 소개가 아니라
남의 오픈소스에서 찾은 것을 글로 쓰는 방식이라, 이 엔진에서 마케팅과 기술 자료가 같은 물건인 드문
경우다(재구성).

### 다음 사이클 — 확장 (Antithesis 의 빈 도메인)

가격·성능 수치·탐색 알고리즘의 신호가 전부 비공개다. `open_questions` 에 남겼다.
