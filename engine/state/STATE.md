...
**없다 — Oxide 를 닫았다.** 완료 **65곳** · ⚠️ **큐 1/3**(Nubank) · 비교 문서 33편.

### 다음 사이클은 Nubank 신규다 (PROMPT.md 3단계부터)

**BR · 핀테크. 라틴아메리카 첫 자리다** — 65곳 어디에도 브라질·아르헨티나·멕시코가 없었다.
`building.nubank.com` 은 WebFetch 로 목록·본문 다 열린다(최신 2026-09-04).
읽을 글은 QUEUE 의 대기 행에 적어 뒀다 — 지연 76% 편은 **버린 대안·대가·수치가 다 있고**,
`finconnect`(나라별로 흩어진 연동을 한 층으로)와 immutable-data 편이 남아 있다.
**Clojure·Datomic 으로 은행을 만든 곳**이라 언어·저장소 축이 65곳과 겹치지 않는다.

### Oxide 를 닫으며 고친 것

**`revenue_streams` 에 빈 수익원이 하나 들어 있었다** — `⚠️ 그 밖의 수익원은 확인하지
못했다` 를 수익원 배열에 넣어 뒀는데, 완주 기준 셋째("각 수익원이 최소 한 기능과
연결된다")에 그대로 걸린다. **모르는 것은 수익원 자리가 아니라 `open_questions` 에
적는다** — 옮겼다. 그리고 랙 판매의 `domains` 에 IPv6 도메인을 더했다(회사가 게스트
사이 처리량을 수치로 공개하므로 **성능 자체가 판매 논리의 일부다**).

### ⚠️ 사다리의 사각지대 — 닫는 일은 `--gaps` 가 부르지 않는다

이번에도 `--gaps` 는 **신규(Nubank)** 를 지목했다. Oxide 는 빈 도메인이 없어 2순위
(확장)에 안 걸리고, 큐가 차 있으니 곧장 6순위로 내려간다. **그대로 따랐으면
`in_progress` 인 회사를 둔 채 다음 회사를 여는 셈이었다**("한 회사를 완주할 때까지
다음 회사로 안 넘어간다" 위반). **`--gaps` 가 신규를 지목하면, 그 전에 `in_progress`
회사가 남아 있는지 먼저 본다.**
