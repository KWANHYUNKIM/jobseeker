...
**ByteDance — 24번째 회사, 중국 첫 회사.** 사이클 149 에서 열었다. 프로파일 + 도메인 3개 +
기능 1개(`monolith`)까지 갔다. **다음 사이클은 `--gaps` 가 기능 없는 도메인을 가리킬 것이다 —
`로컬 저장 엔진`(LavaStore)과 `그래프·분석 데이터`(ByteGraph·ByteHTAP)** 둘 다 **초록까지만** 읽었다.

> **⚠️ 논문 읽는 법 — 이번에 확립했다.** `arxiv.org/abs/<id>` 는 **초록만** 준다(WebFetch 가
> "full PDF 가 필요하다"고 답한다). **본문은 `ar5iv.labs.arxiv.org/html/<id>` 로 읽으면
> 전문이 나온다** — 실험 표·인용문까지 그대로 뽑힌다. VLDB 는 `vldb.org/pvldb/volNN/pXXXX-저자.pdf`
> 직링크가 있고, dl.acm.org 은 초록까지는 읽힌다.

> **다음 기능 후보** — ① **LavaStore**(PVLDB vol.17 p3799): 쓰기 폭주 + 비용 + 조회 꼬리 지연을
> 동시에 맞추려고 RocksDB 를 골라 고쳤다. **GC 를 컴팩션에서 떼어 낸 KV 분리**와 WAL 전용 엔진
> 타입이 핵심. **PDF 를 WebFetch 하면 로컬에 저장되므로 Read 로 정독할 것.**
> ② **ByteGraph**(PVLDB vol.15) — TikTok·Douyin·Toutiao 의 그래프 부하.
> ③ ByteHTAP(PVLDB vol.15) — 아직 제목만 봤다.

> **이 회사가 이 엔진에 처음인 것 — 자료가 블로그가 아니라 논문이다.** 블로그는 '우리가 이렇게
> 했다'로 끝나는데 논문은 **버린 대안이 실험 표로 나란히 있고**(충돌 있는 테이블 대 없는 테이블,
> 동기화 5시간·1시간·30분), **맞바꿈의 손실을 사용자 수로 계산해 보인다**(10일마다 15,000명의
> 하루치). `research.papers` 에 그 관찰을 적어 두었다. **비상장이라 매출은 전부 보도·추정이다.**

> **새 축 — '틀려도 되는 일이라서 가능한 맞바꿈.'** 논문이 직접 적었다: **"시스템 신뢰성은
> 실시간 학습을 위해 맞바꿀 수 있다."** 그런데 이 계산이 성립하는 이유는 추천이 **틀려도 다음
> 것을 또 보여 주면 되는** 종류의 일이기 때문이다 — 토스 송금원장이나 스트라이프 자금 기록은
> 같은 계산을 할 수 없다. **비교 문서로 아주 좋은 축이다**(정확성의 값이 도메인마다 다르다).

> **비교 문서 후보 다섯** — ① **완벽한 탐지를 포기하고 확률로 좁히는 자리**(LY 어테스테이션 ·
> Discord 안전 · 유튜브 Content ID · 토스 이상거래 — **가장 유망**). ② **틀려도 되는 일 vs 틀리면
> 안 되는 일**(ByteDance 추천 ↔ 토스·스트라이프 원장). ③ 양 끝 중 하나를 고르는 대신 축을 바꾸는 수
> (LY 공유 KEK · GitHub 일회용 러너 · Dropbox SSD 캐시). ④ 데이터 레이크의 작은 파일 문제.
> ⑤ 합병·인수로 시스템이 두 벌이 된 회사(지금은 LY 뿐).

**큐 잔량 2/3 — 목표 미달.** Spotify → Canva. 다음 신규 사이클 전에 후보를 하나 채워야 한다.

마지막 사이클: 149 (2026-08-21) — ByteDance 개설 + Monolith.
