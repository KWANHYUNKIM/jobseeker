...
**Godot Engine**(게임 엔진) — 회사 프로파일까지 썼다. **도메인 하나 · 기능 0.** 큐 0/3.

### 파는 것이 없다는 것이 기술 부담의 성격을 정한다

엔진 자체를 팔지 않고 재단의 개발 기금과 기업 후원으로 굴러간다. 그런데 엔진은 데스크톱·
모바일·웹·콘솔에 걸쳐 도는데 **그 아래 플랫폼은 전부 남의 것**이다. 그래서 **플랫폼이 주는
것을 그대로 흘려보내지 않고 대신 받아 내는 일**이 계속 생긴다.

### 도구를 파는 쪽과 그 도구로 게임을 만드는 쪽이 함께 떠받친다

후원사 목록에 **Arm · JetBrains · DeNA** 와 **Mega Crit · Re-Logic** 이 같은 표에 있다.
이 엔진의 처지를 한눈에 보여 주는 자료다.

### 도메인을 하나만 세웠다

블로그 첫 화면이 릴리스 공지와 개발 스냅숏이 지배해 **렌더러·물리·GDScript 컴파일러·씬
시스템 같은 코어를 판 글을 못 찾았다.** 목록 다음 쪽을 넘길 방법이 검색뿐인데 이 세션의
예산이 끝났다. **억지로 도메인을 늘리지 않는다** — 먹스·딥그램·메타베이스와 같은 처리다.

### 소재지를 확인하지 못했다

홈에 `godot.foundation` 링크만 있고 어느 나라 법인인지 적혀 있지 않아 `country` 를
**'기타'** 로 두고 `open_questions` 에 적었다. 큐에는 EU 라고 적어 뒀는데 **근거가 없었다** —
Fastmail 때와 같은 일이 반복됐으니, 큐에 국가를 적을 때도 출처를 확인해야 한다.

### 다음 사이클

도메인 하나에 기능이 없다 — `--gaps` 가 확장으로 부를 것이다. 자료는 마우스 폴링 글 한
편이지만 두껍다(진단·고침 세 부분·물린 대안·고친 뒤에도 남는 부담).
