...
**VictoriaMetrics** (기타 · SaaS) — 도메인 2개 · **기능 1개**(`cardinality-not-volume`). 남은 하나는 **`오픈소스를 사업으로 만든다`**(미독)이다. 회사 101개 · 큐 0/3.

### 이번 사이클 — `백만 개 샘플은 싸고 백만 개 시리즈는 비싸다`

⭐ **회사가 직관을 뒤집는 한 문장으로 연다** — **`한 시리즈에 백만 개 샘플은 싸다. 백만 개 시리즈에 샘플 하나씩은 그렇지 않다.`** 그리고 이유가 태어나는 순간에 있다 — **새 시리즈 하나가 색인 쓰기 열네 번**(**`전역 색인에 포스팅 리스트 갱신 일곱 번, 그리고 오늘의 일별 색인에 같은 일곱 번`**)**을 부르고, 그 뒤 붙는 샘플들은 이미 만들어 둔 자리에 얹히기만 한다**(재구성).

건진 것 넷:

- ⚠️ **읽기를 싸게 하려고 쓰기를 두 배로 냈다** — **일별 색인**이 시간 창 질의로 하여금 **`그 날들에 실제로 살아 있던 시리즈만`** 보게 하는데, **그 대가가 바로 색인 쓰기 일곱 번 → 열네 번**이다. **대가가 같은 문단 안에 적혀 있다.**
- ⚠️ **사소해 보이는 규칙이 최악의 비용을 부른다** — **`안정적인 순서는 스크레이프마다 조회 키를 안정적으로 유지해 캐시가 제 일을 하게 해 준다. 푸시할 때마다 라벨을 재정렬하면 매번 다른 키가 나온다.`** ⚠️ **같은 시리즈인데 키가 달라지면 매번 `새로 태어난 것` 이 되고 그게 가장 비싼 경우다**(재구성).
- ⚠️ **압축은 알고리즘보다 배치가 먼저다** — **`한 컬럼 안의 숫자들은 타임스탬프와 그 값 사이보다 훨씬 서로 닮았고, 유사성이야말로 압축이 먹고 사는 것이다.`** 그 위에서 **`모든 델타가 동일하면 수천 개의 타임스탬프로 이뤄진 블록 전체가 첫 타임스탬프와 간격만으로 저장된다.`**
- ⭐ **압축을 언제 포기할지를 숫자로 박았다** — **`zstd 가 어떤 블록을 최소 약 10% 만큼 줄이지 못하면 델타 인코딩된 평문 바이트를 그대로 둔다.`** **이득이 작으면 쓸 때와 읽을 때 두 번 손해라서 아예 버린 것이다**(재구성).

⚠️ **버리는 일도 새로 만들지 않았다** — **`병합이 보존 기한을 이미 지난 개별 블록들을 건너뛴다`** 이고, 다운샘플링은 **`각 구간의 마지막 샘플을 남기고 나머지는 그냥 버린다`**(**평균·집계를 계산하지 않아 구간 안의 최댓값·최솟값이 사라진다**, 재구성).

### ⚠️ 후보 조사의 판정이 두 번 다 맞았다

후보 조사 때 **`설계 근거는 있지만 거절한 대안이 약하다`** 고 적었는데 그대로였다 — **`~ 대신 ~ 한다`** 가 컬럼 저장 한 곳에만 나오고 나머지는 서술이라, **이 기능의 tradeoff 상당수가 인용된 사실 위에 세운 재구성이다.** 그리고 **`두 번째 기능에서 자료가 마를 수 있다`** 는 경고도 맞아서 **도메인이 둘뿐이다.** ⭐ **후보 조사에서 위험을 적어 두는 습관은 유지한다 — 두 번 다 값을 했다.**

⚠️ **검증 경고를 또 하나 배웠다** — `connections[].to` 에 **`vmagent`** 라고 썼더니 **`feature key 처럼 생겼는데 그런 기능이 없다`** 고 잡았다. **`수집 에이전트 (vmagent)`** 로 고쳤다. **커밋 전에 통과만 보지 말고 경고까지 읽는다**(이 세션에서 세 번째로 값을 한 규칙이다).

### 다음 사이클 — 확장(2순위)

`--gaps` 가 **`오픈소스를 사업으로 만든다`**(`Open Source Is Good Marketing`)를 부를 것이다. ⏳ **본문 미독이고 개별 주소를 아직 못 받았다** — **⚠️ 슬러그를 추측하지 말고 검색으로 받는다**(이 세션에서 두 번 어긴 규칙). **이걸 쓰면 VictoriaMetrics 완주다.**

⚠️ **큐가 0/3 이다.** 완주 뒤 **후보 조사**가 걸린다. ⭐ **언급 각도를 먼저 쓴다.**
...
**③ `자기 성과를 어디까지 주장하나` 8곳** — Snyk / Grafana Labs / ScyllaDB / Razorpay / Flipkart / Airbnb / Pinterest / WarpStream.

**④ `인도 규모에서 무엇이 달라지나` 4곳** — Meesho / Zepto / Razorpay / Flipkart.

**⑤ ⭐ `한 코어에 하나씩인가, 여러 코어가 나눠 쓰나`** — ScyllaDB / ClickHouse / Grafana Labs / **VictoriaMetrics**(**`CPU 별 버퍼`** 로 시작한다).

**⑥ `복제로 버틸 것인가 로그로 버틸 것인가`** — Grafana Labs / WarpStream.

**⑦ `추상화가 무엇을 가리는가`** — Plaid / ScyllaDB / ⏳ Paystack.

**⑧ ⭐ `제약을 없애지 못할 때 어디서 갚는가`** — Cygames / Zepto / Grafana Labs / Pinterest / WarpStream / **VictoriaMetrics**(⚠️ **태어나는 값을 못 줄이니 나란히 두는 쪽으로 갚는다**).

**⑨ `한 번에 갈아엎을 것인가 목 졸라 죽일 것인가` 7곳** — Twilio(두 방향) / Etsy / Plaid / Paystack / Airbnb / Pinterest / WarpStream.

**⑩ ⭐⭐ `관측 비용을 어디까지 줄이나`** — **사는 쪽**: Razorpay · Airbnb(둘 다 vmagent) · WarpStream / **파는 쪽**: Grafana Labs · Honeycomb · **VictoriaMetrics**. ⚠️ **`vmagent` 자체의 설계 글은 끝내 못 찾았다 — 절반만 있다.**
...
**⑬ ⭐ `애매할 때 어느 쪽으로 넘어지나` 8곳** — Flipkart(둘) / Razorpay / Zepto / Paystack / Airbnb / Pinterest / WarpStream / **VictoriaMetrics**(⚠️ **압축 이득이 애매하면 안 하는 쪽으로 — 10% 임계**).
...
**⑰ ⭐⭐ `기억시킬 것인가 압축할 것인가`** — Pinterest(20GB 테이블 → 시맨틱 ID) / Zepto / Grafana Labs / **VictoriaMetrics**(⚠️ **가장 구체적이다** — **압축을 언제 포기할지의 임계값(10%)** · **컬럼 배치가 알고리즘보다 먼저** · **다운샘플링은 계산하지 않고 그냥 버린다**).
...
**㉔ `무엇을 무료로 두고 무엇을 파나`** — ⏳ **VictoriaMetrics**(`오픈소스는 좋은 마케팅이다` — **다음 사이클에 읽는다**) / Grafana Labs / ClickHouse / Snyk.

⏳ **㉕ 새로 보인다 — `새 일을 만들 것인가 도는 일에 얹을 것인가`** — **VictoriaMetrics**(**만료를 별도 삭제 작업이 아니라 이미 도는 LSM 병합에 얹었다**) / **WarpStream**(**관측을 새 인프라가 아니라 이미 있는 저장 엔진에** · **감시를 이미 있는 복제 경로에**) / ⏳ Etsy · Plaid(확인 필요). ⚠️ **새 구성요소를 안 늘리는 것 자체가 설계 원칙인 회사들이 있다**(재구성).
