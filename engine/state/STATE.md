...
**Zed — 회사 프로파일까지 썼다(in_progress).** 회사 117개, 큐 0/3.

도메인 셋을 세웠고 기능은 아직 0개다.

- **에이전트에게 무엇을 허락할 것인가** — `Sandboxing`(2026-08-05)
- **다음 편집을 미리 맞힌다** — `We Rebuilt Zeta from the Training Data Up`(2026-03-25)
- **글자를 무엇에 담을 것인가** — `Zed Decoded: Rope & SumTree`(2024-04-23)

### 과금 단위가 맞혔을 때만 센다

무료 등급이 재는 것이 요청 수가 아니라 **수락된 편집 예측** 2,000건이다. 빗나간 예측의 값은
회사가 치른다. 그래서 이 회사의 기술 문제가 값을 깎는 쪽이 아니라 **맞힐 확률을 올리는 쪽**으로
몰린다 — 바로 앞에 판 Materialize 와 정확히 반대다. 저쪽은 도는 시간이 곧 청구서라 바이트를
깎는 글만 쓴다.

### 에이전트를 가두는 세 번째 답이 나왔다

이 엔진에서 사내 에이전트를 다룬 회사가 여럿인데 Zed 는 파는 제품 안에 그것을 넣은 쪽이다.

- Razorpay — 격리는 애플리케이션이 아니라 **인프라(커널)의 속성**이라고 못 박았다.
- 그랩 — 에이전트가 혼란에 빠지거나 뚫릴 수 있다고 **전제하고** 제어면을 바깥에 뒀다.
- Zed — 같은 결론에 **정렬 문제**로 도달한다. 에이전트가 사용자가 승인할 만한 행동을 스스로
  가려내지 못하는 한 가두는 것이 최선이라고 적고, 맥 Seatbelt·리눅스 bubblewrap 으로 내린다.

버린 답을 함께 적은 것이 값어치다 — **명령어 패턴 규칙**은 셸 이스케이프·변수 치환·다른
인터프리터로 우회되므로 버렸다. 그리고 `.git` 쓰기는 승인 흐름에서 **아예 뺐다**(훅이 울타리
밖에서 돌기 때문). 덮지 못하는 곳도 스스로 나열한다 — 편집 도구·일반 터미널·LSP·MCP·build.rs.

### 연표는 세우지 않았다

GPUI 1→2, CRDT 협업처럼 갈아엎은 사건이 분명히 있는데 읽은 자료에 **무엇이 언제부터였는지가
연도로 없다.** 글의 발행일을 시작 연도로 옮기는 것이 이 판을 망치는 가장 흔한 방법이라
`open_questions` 로 넘겼다.

### 다음 사이클 — 후보 조사 (큐 0/3)

Zed 를 진행 중으로 옮기며 대기가 비었다. 기능을 파기 전에 후보 조사를 한 번 끼운다 —
바닥을 치고 나서 채우면 팔 회사가 떨어질 때마다 사이클을 하나씩 버리게 된다.
