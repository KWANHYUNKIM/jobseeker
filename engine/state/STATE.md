...
**아무도 안 판다 — 358 은 비교 문서 사이클이었다.**
**`same-problem-three-answers`(제품이 늘자 경계가 무너졌다 — 같은 문제에 세 회사가 낸 세
가지 답)** 를 썼다. **완료 60곳 전부 `done` · 큐 0/3 · 비교 문서 33편.**
...
**freee `refactor-before-you-cut`·`swap-without-anyone-noticing` / SmartHR
`when-a-rule-is-not-enough` / PayFit `name-it-before-you-cut`.** 셋 다 급여·인사 SaaS 이고
**제품이 늘며 경계가 무너진** 같은 출발점인데 **무엇부터 손댔는지가 다르다** —
freee 는 **떼어냈고**(1,454일), SmartHR 은 **규칙으로 그었고**(새 영역은 약 반년에 첫 기능),
PayFit 은 **이름을 맞췄다**(2년).

**⭐⭐ 이 문서의 심장 — 후회와 그 후회를 실행해 본 사례가 나란히 있다**(**추정**으로 적었다).
freee 가 *"旧機能のリファクタはマイクロサービス化の前に着手すべし"* 라 후회한 것을
**SmartHR 이 실제로 밟았고**, **새로 만드는 것에는 통했지만**(CI 30분 이상에서 약 1분 ·
조회 3.59~4.46배 · 행 크기 약 71% 감소) **이미 얽힌 인증에는 부족했다.** 이유가 둘이다 —
인가가 과금·테넌트에 의존해야 해서 *"認可よりも変更頻度の高い領域に認可が引っ張られてしまいます"*
이고, 애초에 모듈러 모놀리스로는 *"デリバリ（リリースの独立性）と可用性を解消できません"*.

### ⭐ 표를 만들고 나서 보인 것

**방법의 차이보다 재는 정도의 차이가 크다.** **SmartHR 만 성과 수치를 낸다.**
freee 는 **기간과 규모를 아주 자세히 적으면서 무엇이 좋아졌는지는 안 적고**,
PayFit 은 **2년을 쓰고 아무 수치도 없다.**
**⭐ 짐작**(추정으로 적었다) — 조율 비용과 개발 속도는 인과를 못 가른다. **SmartHR 만 숫자를
낸 것은 그쪽이 잰 것이 조율이 아니라 CI 시간과 쿼리 지연 같은 기계적인 값이기 때문으로 보인다.**

**⚠️ 그리고 정직하게 두 가지를 단서로 달았다** — **셋 다 아직 안 끝났다**(freee 만 완료 선언,
SmartHR 인증 리프트는 시작 단계, PayFit 은 옛 구조에서 옮기는 중)는 것과,
**세 회사의 규모가 한 자릿수씩 다르다**는 것(freee 유료 고객 60만 곳 이상 · SmartHR 등록
사수 80,000사 · PayFit 고객 10,000곳).

### ⑫ 가 또 값을 냈다

**프롬프트가 준 SmartHR 수치가 기능 메트릭에는 없었다** — 회사 파일을 직접 열어 확인하니
**프로파일 단계의 출처 요약에 있었다.** 그리고 **인용 열아홉 개를 하나씩 파일에서 있는지
확인한 뒤에 썼다.** 전부 있었다. **낡은 메모를 나르지 않는 규칙이 비교 문서에서도 통한다.**

---

**다음 사이클 — 남은 비교 문서 재료 셋 중 하나.**
- **⭐ 1순위 — `미루되 미룬 것이 안 보이게 한다 — 그럼 그것은 어디에 쌓이는가` 갱신.**
  지금 Doximity·Careem·PlanetScale 셋으로 돼 있는데 **세 사례를 더할 수 있다**:
  **SmartHR 의 미룬 쪽**(단일 실패점인 줄 알면서 언제 심해질지 몰라 미뤘고, 미룬 것이 **놀고
  있는 접속**에 쌓였으며 청구서는 **장애 빈도가 월 1회에서 동시다발로 오르며** 왔다),
  **SmartHR 의 미리 잰 쪽**(연 1회 정점을 부하 시험으로 앞당겨 보고 목표의 4분의 1이라는 것을
  정점 전에 알았다), **bol**(중개 앱을 2031년까지 유지하는 안을 값으로 계산해 거부했고
  *"there's nothing as permanent as a temporary solution"* 라고도 적는다).
  **⚠️ 기존 문서의 구조를 먼저 읽고 그 결에 맞춘다.**
- **2순위 — `되돌릴 수 없는 것이 어디에 있는가` 에 bol 을 더한다.** **적어도 한 번** 처리를
  골랐고 정산에서 중복은 크레딧을 두 번 내는 일인데 **이관 쪽에 검증 이야기가 아예 없다.**
- **3순위 — 한국 회사 인용 전수 확인.** 357 에서 **hyperconnect 128 · kurly 110** 을 한국어
  원문 인용이라 일부러 안 고쳤다. **표본 열여섯 개는 다 원문이었으나 전수는 안 봤고**, 그중
  **이 사이트가 쓴 우리말 정리를 인용 표시로 감싼 것**이 섞여 있을 수 있다.
...
CA·AU·SG·AE·NG 각 1) · **비교 문서 33편** · **운영 사실 62개.**

**⚠️ 나르지 않는다** — 낡은 메모(314·317·325·348·349 · **358 에서도 프롬프트가 준 수치가
기능 메트릭에 없었다**) · 두 방향 규칙(298·305·318·321·354) · 주소가 열리는지도 확인한다
(327·328) · **한 글에 없다고 회사에 없는 것이 아니다**(331→332) · **도메인 이름은 넓게**
(334→335) · **자기 문서 인용도 우리말은 굵게**(337) · **각도가 다르면 다른 기능**(339→340) ·
**층이 다르면 다른 도메인**(341) · **확인 안 된 인용 금지**(341·342) · **읽은 글을 미독 목록에
남기지 않는다**(342·345) · **여러 편을 열고 하나도 안 쓰는 사이클이 정상**(343·353) ·
**`domain_map`·`diagrams` 는 객체**(344) · **`features_done` 을 같이 올린다**(345) ·
**정반대 답은 `connections` 에**(346) · **셸에 백틱 금지**(347·356 — **긴 문서는 Write 툴로
직접 쓴다**) · **닫을 때 낡은 문장 다시 본다**(348) · **URL 끝 16진수 해시는 Medium**(349) ·
...
스크립트부터 의심한다**(356) · **기계적 보수는 예외를 먼저 가른다**(357) ·
**⭐ 새로: 비교 문서를 쓸 때 인용을 하나씩 회사 파일에서 있는지 확인하고 쓴다 — 358 에서
열아홉 개를 다 확인했고 그 덕에 지어낸 것이 하나도 없다**(358).
