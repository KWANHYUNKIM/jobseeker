...
**카케하시(KAKEHASHI)**(JP · 약국 SaaS) — 회사 프로파일까지 썼다. **도메인 둘 · 기능 0.** 큐 0/3.

### 두 도메인이 같은 물음의 두 답이다

**나중에 반드시 바뀌는 것을 어떻게 미리 다루는가.** 데이터 쪽은 **과거를 안 고치고 이력을
쌓는다**(버전마다 전체 정의를 유효 기간과 함께 남기고 델타를 순서대로 적용), 인증 쪽은
**빠진 경우를 컴파일러가 잡게 한다**(Discriminated Union + exhaustiveness 검사).
**하나는 시간축에서, 하나는 타입에서** 변화를 받는다.

### 고치는 것이 위험한 자리가 있다

스키마를 바꾸면 **과거 예측 데이터 자체를 다시 써야** 했는데, 그것이 **위험하고 감사
추적을 흐린다.** 그래서 **쌓는 층은 불변**으로 두고 **분석 층만 재생성** 가능하게 만들었다 —
**되돌릴 수 없는 것을 아예 안 건드리는 구조**다.

### 시장을 숫자로 적는다

**전국 약 6만 점의 조제약국**. 그리고 약국 안에 머물지 않고 **병원·약국·환자를 잇는 약
900증례 규모 PHR 실증**(AMED 채택)으로 넓히는 중이다. 창업 **10주년**.

### 물린 것을 이름 대어 적는 쪽과 안 적는 쪽이 한 회사에 있다

인증 글은 **CSR/SPA 를 물린 이유**(**'한순간을 위해 SPA 를 초기화하는 비용'**)와 선례
(**Keycloak 메인테이너가 SSR 유지를 명시**)까지 적는데, 데이터 층 글은 **물린 대안을
아예 안 적는다.** 같은 회사 안에서 글마다 다르다 — **뱅크샐러드·DeNA 에 이어 네 번째로
같은 관찰**이다.

### 다음 사이클

확장 — 빈 도메인 둘 중 하나. **데이터 층 쪽이 문제를 셋으로 이름 대어** 적고, 인증 쪽은
**물린 것과 남은 문제**(프런트엔드 지견 부족)가 분명하다. 큐가 0/3 이라 그 뒤에 후보
조사가 뜬다.
