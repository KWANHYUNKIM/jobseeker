...
**Wayfair (US · 커머스/대형 물품) — 44번째. 기능 2개 · 열어 둔 도메인 2개가 다 찼다.**

242 에서 **`beyond-taxonomy`** 를 썼다. 결정 7 · 그림 3 · 생각 4 · 스택 6 · 난제 2 · ui 4핀.

**두 번째 글을 먼저 읽은 것이 또 값을 했다**(241 에 이어 열다섯 번째).
**두 글이 서로 다른 자리에서 같은 적을 지목한다** — 시각 탐색은 *"표준 시스템은 하드
필터처럼 작동한다: 사용자의 사진을 러브시트로 분류하면 검색을 그 카테고리로만 제한한다"*,
고객 관심 생성은 *"전통적인 고객 이해 모델은 … **고정된 분류 체계에 기대고**, 방대한 학습
데이터를 요구하며, 암묵적 패턴이나 잠재된 관심을 자주 놓친다"*.
**사진으로 찾기와 말로 짐작하기가 같은 문제의 두 입구다.** ⚠️ **두 글은 서로를 언급하지 않는다.**

**⚠️ 두 글 다 새 시스템의 한계를 한 줄도 안 적는다** — 적혀 있는 대가는 전부 **옛 방식**의 것이다.
그래서 대가 칸의 상당수가 **이 사이트의 지적**이고, 전부 그렇게 밝혀 적었다:
분류를 떼어 내면 **왜 이 결과가 나왔는지를 카테고리로 설명할 수 없다** · **YOLOX 가 쿠션을
잡으면 뒤가 다 틀린다** · **압축 70% 가 무엇을 버렸는지 모른다** · **LLM-as-a-Judge 가 틀릴 수
있다는 이야기가 없다**(보고된 개선폭이 전부 그 판정을 통과한 숫자다).

**🚨 다음 사이클의 선택 — 열어 둔 도메인이 둘 다 찼다.**
- **손상 도메인의 두 번째 기능**: `how-we-validate-product-dimensions-at-scale-with-multimodal-genai`
  (2026-01-15). 그 도메인은 이미 본문 2편을 읽었으니 **이 글만 읽으면 바로 쓸 수 있다.**
  ⚠️ **`damage-judgment` 와 뭉개지 않는다** — 저건 *손상*, 이건 *치수*다(223 의 교훈).
- **또는 세 번째 도메인** — **Data Science 태그 90편을 아직 한 편도 안 봤다.**
  `how-we-used-ai-as-a-lab-assistant-to-run-110-experiments-in-three-days`(2026-06-15,
  **태그 검증 비용 94% 절감**)가 눈에 띈다. **태그 목록이 `?tag=Data%20Science` 로 열린다.**
- **또는 Wayfair 를 done 으로 닫는다.**
