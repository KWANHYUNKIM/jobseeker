...
**없다 — 이번 사이클은 후보 조사였다. 큐를 0/3 에서 1/3 로 채웠다.** 회사 160개.

### 비어 있던 음성 축을 채웠다 — Deepgram

`Why TTS Latency Grows With Response Length` 를 본문까지 열어 확인했다. **대가를 먼저 적는다** —
스트리밍 TTS 는 배치보다 **문맥을 5~20배 적게 보고 그래서 고유명사 발음 정확도가 떨어진다.**
병렬 디코딩(전부 계산한 뒤 내주기)을 물리고 토큰 단위 스트리밍을 골랐다.

### 측정 방법론까지 적는 글이다

**총 지연 = 네트워크 + TTFB + 오디오 합성**이고 **마지막 항이 데모가 감추는 것**이라고 적는다.
그리고 그 격차를 수치로 보인다 — 70 QPS 에서 짧은 입력은 136.56ms, **7초 오디오는 925.51ms.**
p95 TTFB 는 200ms 미만(정상 상태 약 90ms), 기준으로 ITU-T G.114 를 든다.

### 날짜 없는 자료를 어떻게 다룰지 정했다

`/learn` 은 학습 허브라 **어느 글에도 발행일이 없다.** 지난 조사에서 그래서 미뤘는데, 이번에
본문을 열어 보니 내용은 확실했다. **날짜가 없다는 사실을 큐에 적어 두고 올린다** — 연표를 세울
수 없다는 뜻이므로 프로파일에서 `eras`·`history` 를 만들지 않는다.

### `/engineering` 섹션까지 봐야 판정이 정확해진다

**Sanity** 를 접은 과정이 그렇다. 블로그만 봤다면 "제품 글뿐"이라고 접었을 텐데, 따로 있는
엔지니어링 섹션까지 보고서야 **"코어 시스템(콘텐츠 레이크·GROQ·실시간 협업) 글이 없다"**는
정확한 판정이 됐다. 있는 것은 에이전트 글과 도구 이야기였다.

### 다음 사이클 — 신규 (대기의 Deepgram)

TTS 지연 글 외에 `Batch TTS Deep Dive` 와 `real-time voice AI stack` 이 있다.
**⚠️ 전부 날짜가 없다 — 연표를 세우지 않는다.**
