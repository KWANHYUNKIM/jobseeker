...
**Depot — 기능 둘을 채웠다.** 남은 빈 도메인은 하나(`시간을 팔면서 시간을 줄인다`).
회사 130개, 큐 2/3.

### 이번 기능 — `soci-in-build` (색인을 만들 자리는 빌드다)

레이어가 tar+gzip 순차 스트림이라 가운데 파일 하나를 얻으려면 앞을 다 풀어야 한다. 그런데
앱은 시작할 때 엔트리포인트와 초기 의존성만 있으면 된다. 그 어긋남에 SOCI 의 zTOC 가 들어간다
(AWS 사례로 ML 컨테이너 7분 → 21초).

**요점은 색인을 만드는 자리다** — 푸시한 뒤 다시 받아 색인하지 말고, **이미 레이어를 손에 들고
있는 빌드에서** BuildKit 이 만들게 고쳤다. 값은 압축 레이어를 읽는 정도라 빌드에 견주면 작다.

### 호환을 깨지 않는 쪽을 골랐다

eStargz·Nydus 를 이름 대며 대비하되 **레이어를 다시 쓰지 않는다** — 원본 압축 레이어를 그대로
두고 옆에 별도 OCI 아티팩트로 메타데이터를 얹는다. 모르는 런타임에서 이미지가 그대로 돈다.
대신 이득도 아는 쪽에서만 난다 — **만드는 쪽만으로는 끝나지 않는 최적화**다.

그래서 기본으로 켜지 않는다(옵트인, 압축 지정 필요). 조건이 붙는 기능의 정직한 기본값이다.

### 앞 회사와 견준 자리

빌드 옆에 붙는 부산물이라는 점에서 앞의 `in-process-volume-allocator` 와 정반대다 — 저쪽은
우리 하드웨어라 다 통제했고, 이쪽은 **받는 쪽 생태계의 협조가 필요하다.**

### 다음 사이클 — 확장 (Depot 의 마지막 빈 도메인)

`시간을 팔면서 시간을 줄인다` — 요금은 분·초 단위인데 빠르게 만들수록 같은 일의 매출이 준다.
`the-fibonacci-benchmark`(2026-08-26)를 아직 안 읽었다.
