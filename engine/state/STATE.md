...
**Airbnb** (US · 커머스 — 숙박 마켓플레이스) — **프로파일까지 썼다**(PROMPT 3단계). 도메인 3개 · 기능 0. 회사 **98개.**

### 이번 사이클 — 신규(6순위)

⚠️ **이 회사가 2026년에 공개한 기술 이야기 셋이 모두 `옮기는 이야기` 다** — **관측을 벤더 기반에서 오픈소스 스택으로**(2026-04) · **데이터 아키텍처를 한 제품에서 여러 제품 구조로**(2026-06) · **신원 그래프를 벤더 PaaS 에서 자체 인프라로**(2026-05). **규모가 커진 회사가 빌려 쓰던 것을 하나씩 자기 것으로 바꾸는 국면으로 보인다**(재구성). 도메인을 그 셋으로 세웠다.

**규모** — 활성 등록 **800만 개 이상** · 호스트 **500만 명 이상** · **220개가 넘는 나라와 지역** · 2026년 2분기 매출 **36억 달러(+17%)** · 총예약액 **272억 달러(+16%)** · 예약된 숙박·좌석 **1억 4,830만 건(+10%).**

### 읽어 둔 것 — 메트릭 글에서 건진 것 넷 (다음 사이클의 재료)

- ⚠️ **버린 이유가 둘 다 구조적이다** — **StatsD 의 UDP 전송은 패킷 손실 위험을 만들고**, **Veneur 의 설계는 현대적인 Prometheus 데이터 모델과 잘 맞지 않았다.**
- ⚠️ **거절한 대안을 다섯 개 나열한다** — **Veneur**(광범위한 재작성 필요) · **recording rules** · **OpenTelemetry Collector 자체 집계** · **Vector** · **m3aggregator.** 고른 근거는 **스트리밍 집계 능력과 감당할 만한 코드베이스**다.
- ⚠️ **순서를 명시한다** — **기능을 점진적으로 옮기는 대신 `먼저 모든 메트릭이 새 시스템으로 흐르게 하고, 그다음에 대시보드·경보·사용자 도구를 다룬다`.**
- ⚠️ **대가를 숨기지 않는다** — **고카디널리티 서비스를 델타 시간성으로 바꾸자 장애 때 이상한 스파이크 대신 눈에 보이는 데이터 공백이 생긴다.** **메모리 압박을 다루려고 의도적으로 감수한 절충**이다.

**수치** — **초당 1억 샘플 이상** · JVM 서비스의 메트릭 CPU **10% → 1% 미만** · **`이전 벤더 기반 아키텍처 대비 비용이 대략 한 자릿수 규모로 줄었다`.**

### 다음 사이클 — 확장(2순위)

`--gaps` 가 **`이 규모를 관측하는 비용을 감당한다`** 를 부를 것이다. **자료를 이미 읽었으니 바로 기능을 쓴다.** ⭐ **⑩축(`관측 비용을 어디까지 줄이나`)에서 Razorpay 와 같은 vmagent 를 다른 이유로 골랐다** — 그 대비를 connections 에 적는다.
...
**① `AI 에이전트를 어디까지 믿나` 9곳** — Sentry / ClickHouse / Duolingo / Ramp / DoorDash / Deliveroo / Snyk / Cygames / Razorpay. ⏳ **Airbnb 의 `에이전트가 코드의 60%를 쓴다` 는 3자 기사라 회사 글로 확인해야 한다.**
...
**⑩ ⭐ `관측 비용을 어디까지 줄이나`** — **Razorpay · Airbnb**(사는 쪽, 둘 다 vmagent) / **Grafana Labs · Honeycomb**(파는 쪽).
...
⏳ **⑯ 새로 보인다 — `빌려 쓰던 것을 언제 자기 것으로 만드나`** — **Airbnb**(관측·신원 그래프를 벤더에서 자체로) / **ScyllaDB**(남의 SDK 를 감싸되 안에서 갈라진다) / **Plaid**(관리형 Aurora 를 떠나 TiDB 로) / **Cygames**(반대로 관리형 TiDB Cloud 를 골랐다).
