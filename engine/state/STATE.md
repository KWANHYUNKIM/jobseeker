...
**카케하시(KAKEHASHI)** — 첫 기능 `leave-it-for-whoever-reads-later`(나중에 읽을 사람을
위해 지금 남긴다)를 끝냈다. **도메인 2 · 기능 1.** 큐 0/3. 남은 도메인은 **여러 제품을
한 팀이 떠받친다.**

### 되돌릴 수 없는 작업을 아예 안 하는 구조

과거를 고치는 것이 위험하다면 **고칠 일이 없게** 만든다 — **쌓는 층은 불변**, **분석 층만
재생성**, 스키마는 **버전 이력으로 쌓아 읽을 때 델타를 적용**한다. 대가는 **저장 중복과
재생성 시간**인데 **그 크기는 수치로 없다.**

### 거창한 이름이 시작을 막는다

**'시맨틱 레이어'·'온톨로지'** 라는 말 때문에 **엄격한 구조를 먼저 갖추려다** 아무것도 안
하게 된다. 그래서 **평범한 말로 적기**부터 했다 — **`status = 3` 의 `3` 이 무엇인지**를
쓰고, **'태그별로 집계하면 중복 계수될 수 있다'** 같은 **함정까지** 남긴다.

### 읽는 쪽이 늘면 두는 곳이 바뀐다

**AI 가 읽을 수 있느냐**가 조건이 됐다. 예전에는 BI 도구 안이 자연스러웠는데 지금은
**깃허브·노션·컨플루언스처럼 열린 곳**이어야 한다. **Classi 가 'AI 가 낸 코드를 평가하려면
사람이 도메인을 알아야 한다'** 고 적은 것과 짝이다 — **둘 다 도메인을 밖으로 꺼내는 일**이다.

### 자기 방법을 과장하지 않는다

**'시맨틱 레이어나 메트릭 정의의 구조는 그 자체가 마법이 아니다'** — **Garbage In, Garbage
Out** 은 그대로라 나중에 **dbt 로 구조 자체를 정비**하러 갔다. **문서로 시작하되 거기서
끝내지 않은 것**이 이 글을 믿게 만든다.

### 다음 사이클

확장 — 남은 도메인(공통 기반). **SSR 과 OIDC · Discriminated Union · 물린 CSR/SPA ·
남은 프런트엔드 지견 부족**이 이미 근거로 서 있다. 큐가 0/3 이라 그 뒤에 후보 조사가 뜬다.
