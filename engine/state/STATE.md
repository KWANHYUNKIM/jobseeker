...
**없다 — 후보 조사 사이클이었다.** 큐를 **0/3 → 1/3** 으로 채웠다. 다음 사이클은 `--gaps` 가 **신규(6순위)**를 부른다 — 대기 맨 위는 **Plaid** 다.

### 이번 사이클 — 후보 조사 (스무 번째)

**403 목록을 브라우저로 되짚다가 더 큰 것을 배웠다.**

- **✅ Plaid (❌ → ✅)** — ⚠️ **접었던 이유 자체가 낡아 있었다.** `engineering.plaid.com` 이 Medium 커스텀 도메인이라 접었는데, **브라우저로 열어 보니 그 Medium 은 비어 있고 회사는 `plaid.com/blog` 로 옮겨 자체 호스팅한다.** **수치가 이 엔진에서 본 이전 이야기 중 가장 촘촘하다** — Aurora 에서 **약 12k writes/sec 병목**, **6명이 DB 234개**를 **약 100개 서비스**에서 **2년 반**에 걸쳐 TiDB 로. 업그레이드 비용이 **26 엔지니어링 주·다운타임 104분 → 1주·다운타임 0**(노력 **96% 감소**).
- **⏸️ ScyllaDB — 보류.** 브라우저로는 열리는데 **목록이 렌더링되지 않는다.** 제목 하나는 좋아 보인다(`Asymmetric io_uring Backend for Seastar`). ⚠️ **DB 벤더 축이 이미 셋**이라 우선순위는 낮다.

### ⚠️ 이번에 배운 것 — 접근 지도의 ❌ 는 그 시점의 사실이지 영원한 사실이 아니다

**앞 사이클에서는 `403 은 브라우저로 뚫린다` 를 배웠는데, 이번에는 그것으로 부족했다.** Plaid 는 **뚫는 문제가 아니라 주소가 바뀐 문제**였다 — **회사가 블로그를 옮기면 옛 판정이 통째로 틀린 것이 된다.**

**되짚을 때는 두 가지를 함께 해야 한다** — ① **브라우저로 열어 본다**(403 을 뚫는다) ② **검색으로 현재 주소를 다시 받는다**(회사가 옮겼는지 본다).

남은 되짚기 목록 — `blog.paystack.com` · `www.meesho.io/blog` · `blog.zepto.com` · `bytes.swiggy.com` · `www.gojek.io/blog`(429) · `tech.cygames.co.jp`.

### 다음 사이클 — 신규(Plaid)

⚠️ **DB 이전 축이 Etsy(Vitess)와 겹치는데 그것이 오히려 값이다** — **같은 문제(관리형 MySQL 의 쓰기 한계)에 한쪽은 샤딩 추상화로, 다른 쪽은 분산 SQL 로 답했다.** **비교 문서의 재료가 하나 더 생겼다**(AI 에이전트 축에 이어 둘째다).
...
| `plaid.com/blog` | ✅ **자체 호스팅.** 옛 `engineering.plaid.com` 은 Medium 이고 **스토리가 비어 있다** |
