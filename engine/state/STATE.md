...
**AuthZed — 기능 하나를 채웠다(`zedtoken-consistency`).** 빈 도메인 하나. 회사 142개, 큐 2/3.

### 캐시해야 빠르고 캐시하면 낡는다

권한이 모든 요청 앞에 서니 캐시 없이는 병목이 된다. 그런데 캐시된 권한은 **방금 뺏은 접근을
아직 허용**할 수 있다 — 잔지바 논문이 새 적 문제라 부른 것이다.

이 회사는 그 선택을 **부르는 쪽에게 넘긴다.** 네 수준(빠른 쪽·이 시점 이후·바로 그 시점·무조건
최신)을 주고 요청마다 고르게 한다.

### 안전해 보이는 기본값이 가장 비싸다

기본은 읽기가 `minimize_latency`, 쓰기가 `fully_consistent` 다. **가장 정확한 수준은 캐시를
대놓고 우회해 지연에 크게 영향을 준다**고 문서가 적는다 — 아무 데나 그걸 쓰면 권한이 제품
전체의 지연이 된다.

### 시점을 불투명한 토큰으로 넘긴다

ZedToken 은 저장소의 한 시점을 가리키되 안이 안 보인다. 애플리케이션이 자기 데이터 옆에 들고
다니다가 물을 때 함께 보낸다 — **일관성의 책임이 부르는 쪽으로 일부 넘어간다.**

### 영원한 시점은 없다

정확한 시점을 요구할 수 있게 하되 쓰레기 수집이 지나가면 만료 오류로 실패한다. 저장을 무한정
늘리지 않는 대신 쓰는 쪽이 그 오류를 다뤄야 한다.

### 다음 사이클 — 확장 (AuthZed 의 마지막 빈 도메인)

`권한 그래프를 빨리 훑는다`. 계획기가 캐시로 얻는 이득이 일관성 수준을 올리는 순간 사라진다는
연결까지 이미 적어 뒀다.
