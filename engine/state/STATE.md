...
**Bluesky**(US · 분산 소셜) — 회사 프로파일까지 썼다. **도메인 둘 · 기능 0.** 큐 0/3.

### 광고를 안 받는 이유를 윤리가 아니라 구조로 적는다

**"우리는 공개 소셜 네트워크이고 코드가 전부 오픈소스라, 데이터에 관해서는 해자가 없다."**
사용자가 자기 데이터를 소유하고 **언제든 떠날 수 있게** 만든 설계가 데이터를 파는 사업과
애초에 맞지 않는다는 것이다. 그래서 첫 유료 상품이 **커스텀 도메인**인데, 이것도 프로토콜에서
나온다 — **'사용자명은 그냥 도메인'** 이라 도메인을 파는 것이 곧 정체성을 파는 일이 된다.

### 고른 것과 프로토콜이 허용하는 것을 갈라 적는다

서버 대 서버 대신 중앙 릴레이를 골랐으면서 **"우리 방식은 규범적인 것이 아니다. 프로토콜은
실제로 양쪽 다 되도록 설계돼 있다"** 고 못 박는다. 그리고 대가도 적는다 — 릴레이는 **자원을
많이 먹어** 전체 네트워크 릴레이는 소수만 존재할 것이다.

### 물린 것을 비유로 적는다

중앙 집중 모더레이션을 **"모든 분쟁을 대법원 수준에서 해결하는 것"** 에 빗대 물리고, 순수
페더레이션은 **"다른 네트워크에서는 옮기는 일이 대개 어렵고 비싸다"** 로 물린다. 대신 세
층으로 쌓았다(자동 필터 · 서버 기본값 · **커뮤니티 라벨 구독**).

### 심은 대비

**Proton**(비영리 재단이 최대 주주라 광고를 안 받는다) ↔ **Bluesky**(프로토콜 구조상 광고가
막혀 있다) — **같은 자리에 소유 구조로 간 쪽과 기술 구조로 간 쪽**이다.

### 다음 사이클

빈 도메인 둘 중 하나. **페더레이션 쪽이 자료가 더 두껍다**(고른 이유·이득 셋·대가·규범이
아니라는 선언). 모더레이션 쪽은 물린 대안 둘이 선명하지만 **커뮤니티 라벨링이 그때 미완성**
이었어서 그 뒤를 확인해야 한다.
