...
**없다 — 이번 사이클은 후보 조사였다.** 큐 **0/3 → 3/3.** 회사 92개.

### 이번 사이클 — 되짚기가 또 통했고 새 축이 둘 열렸다

- **✅ ScyllaDB** (❌ → ✅) — **첫 화면으로 판정하지 않는다는 규칙이 또 맞았다.** 고객 사례·제품 글 사이에 엔지니어링 글이 섞여 있다. ✅ 본문 확인(`비대칭 io_uring 백엔드`, 2026-07-22) — **세 대안을 나란히 놓고 거절 이유를 대고**, ⚠️ **자기 방식이 순수 I/O 워크로드에서 기존(linux-aio)보다 못하다는 것까지 그대로 적는다**(네트워킹 코어가 주기의 40~50% 를 메모리 복사에 쓴다).
- **✅ Cygames** (❌ → ✅) — **`r.jina.ai` 로 뚫렸다.** ⭐ **게임 축의 첫 자리다.** ✅ 본문 확인(`출시 시점의 TiDB`, 2025-10-03) — 출시일 **50만 QPS**, 피크 **65만**, **40만 QPS 중 롤링 업데이트**, 그리고 **통계 이력 기능을 껐다**(메모리 제약, 안정성 우선). ⚠️ **컨퍼런스 후속 요약이라 얕다** — 아키텍처 글이 본체일 수 있다.
- **✅ Grafana Labs** — 새 이름. ⭐ **관측·저장 엔진 축.** ✅ 본문 확인(`Mimir 차세대 아키텍처`, 2026-01-02) — **옛 구조(카산드라식 복제 3 + 쿼럼)를 거절한 이유가 명시적이고**(`복제 계수 3은 비싸다` · `인제스터가 과부하되면 쓰기 경로도 영향을 받는다`), **Kafka 를 사이에 놓아 복제 3→2, 쿼럼 2→1** 로 낮췄다. **TCO 25% 감소.**

### ⛔ Swiggy — 세 번 두드리고 접었다

`bytes.swiggy.com` 은 **`r.jina.ai` 가 목록·개별 글 모두 422** 이고 **브라우저는 다섯 사이클째 응답이 없다.** ⚠️ **원 도메인 `swiggybytes.medium.com` 은 열리는데 거기 있는 것은 인물·커리어 글이고 최신이 2024-07 이다** — **같은 회사의 두 주소가 다른 내용을 담고 접근성도 다르다.** 시스템 글의 슬러그를 medium 호스트에 붙여도 **404**(별개 퍼블리케이션). **회사 사정이 아니라 도구 한계**이므로 접근 지도에 그렇게 갈라 적었다.

### ⚠️ 이번 사이클에 내가 한 실수 — 슬러그를 추측했다

**ScyllaDB 글 주소를 날짜 경로 형식으로 지어내 404 를 받았다.** **접근 지도에 `슬러그 추측 금지` 가 이미 적혀 있는데도** 그랬다(Ramp·Honeycomb·Sentry 에서 세 번 겪은 실수다). **검색으로 정정했고, 이번엔 규칙을 어긴 것 자체를 기록에 남긴다.**

### 다음 사이클 — 신규(6순위)

`--gaps` 가 **큐 맨 위(Grafana Labs)를 `in_progress` 로 만들고 3단계부터** 를 부를 것이다.

⚠️ **이번 세션에서 판 네 회사가 전부 큐에서 나왔다** — 후보 조사가 밀리면 파는 쪽이 멈춘다.

### ⚠️ 비교 문서 재료 (초안 유지)

**① `AI 에이전트를 어디까지 믿나` 7곳** — Sentry / ClickHouse / Duolingo / Ramp / DoorDash / Deliveroo / Snyk.

**② `관리형 MySQL 의 한계` 3곳** — Etsy / Plaid / Paystack. ⏳ **Cygames 가 TiDB 로 붙을 수 있다** — **Plaid 는 관리형 MySQL 을 떠나며, Cygames 는 게임의 샤딩 복잡도를 피하려고** 같은 것을 골랐다.

**③ `실험 결과를 얼마나 믿나` 3곳** — Etsy / Deliveroo / **Snyk**. **셋 다 자기 측정 도구의 한계를 공개한다.**

**④ `인도 규모에서 무엇이 달라지나` 2곳** — Meesho / Zepto.

⏳ **⑤ 다섯째가 열릴 수 있다 — `한 코어에 하나씩인가, 여러 코어가 나눠 쓰나`** (ScyllaDB 의 shared-nothing 대 ClickHouse). ⏳ **⑥ `복제로 버틸 것인가 로그로 버틸 것인가`**(Grafana Mimir 가 복제 3 → Kafka 로 옮긴 이야기).

⚠️ **비교 문서(8순위)는 이 사다리에서 영영 안 온다** — 3·7순위가 큐를 다시 채우므로 "위가 전부 비었다" 가 성립하지 않는다. **PROMPT.md 의 문제이고 사이클이 고칠 것이 아니다.**
