...
**Chroma — 회사 프로파일까지 썼다(in_progress).** 회사 126개, 큐 0/3.

도메인 셋을 세웠고 기능은 아직 0개다.

- **색인을 객체 저장소 위에 올린다** — `Retrieval powered by object storage`
- **쓰기 로그도 객체 저장소 위에 짓는다** — `wal3: Chroma's Write-Ahead Log`
- **색인 수백만 개를 테넌트별로 다룬다** — 같은 글의 멱함수 분포 + 제품 페이지

### 지연을 내주고 값을 샀다

검색 인프라의 원가는 대개 메모리와 SSD 인데 그 자리를 객체 저장소로 바꾼다 — GB 당 월 0.02달러와
메모리의 5달러를 나란히 놓고 최대 10배 싸다고 판다. 대가는 35~100밀리초다.

**그 교환이 성립하는 근거를 고객의 성질에서 찾는 것이 이 회사의 특징이다** — 언어 모델을 여러 번
호출하느라 종단 지연이 이미 긴 워크로드라면 그 정도는 묻힌다는 것. 값 이야기를 성능 이야기로
받아치지 않고 **누가 쓰느냐**로 답한다.

### 워크로드의 모양이 인프라를 정했다

전통적인 검색은 큰 색인 하나인데 여기는 **작은 색인 수백만 개**이고 접근이 멱함수 분포라 대부분이
거의 안 쓰인다. 그래서 안 쓰이는 색인에 자원을 두지 않는 서버리스가 성립한다. 자주 쓰는 컬렉션만
SSD·메모리 캐시가 당겨 준다.

### 앞 회사와 층이 붙는다

바로 앞에 판 Tigris 는 객체 저장소를 **만드는** 쪽이고 Chroma 는 그 위에 색인과 로그를 **올리는**
쪽이다. wal3 에서 카프카(디스크 함대를 운영해야 한다)·WarpStream(오픈소스가 아니다)·CDC(처음부터
지어야 한다)를 이름 대며 물리는데, **WarpStream 은 이 엔진에 있는 회사**다.

### 다음 사이클 — 확장 (Chroma 의 빈 도메인)

wal3 가 거절한 대안에 이름과 이유가 다 붙어 가장 두껍다. 글에 발행일이 없다는 것도 적어 뒀다.
