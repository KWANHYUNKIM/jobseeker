...
**없다 — 문체 보수 사이클이었다.** 터소를 배민 문체로 다시 썼다(2.05 → 0.0, 기호 171개). 회사 115개, 큐 2/3. 30곳 남았다.

### 재현을 다루는 넷 — 앞에 놓느냐 뒤에 붙이느냐

첫 기능에서 이 축이 넷으로 찼다.

- Anthropic 은 신고를 재현하지 못하자 **재는 자와 무대**를 의심했다.
- Etsy 는 자기 실험 체계로 의심을 돌려 **승자의 저주**를 공개했다.
- Ramp 는 재현을 설정 파일로 사면서도 그 약속이 **상류 데이터**에 걸려 있다고 조건을 적는다.
- 터소만 재현을 **앞에 놓았다.**

앞의 셋이 이미 있는 시스템에서 재현을 되찾으려는 이야기라면, **여기는 재현이 안 되는 시스템을
물려받을 수 없어서 처음부터 다시 짓는 쪽을 골랐다.** 직원 19명인 회사가 데이터베이스를 통째로
다시 쓴 이유가 사업이 아니라 남의 테스트 스위트가 독점이라는 데 있다.

### 물려받은 것을 어디까지 지킬 것인가 — 여기가 가장 멀리 갔다

- Etsy 는 16년 된 샤딩을 두고 **데이터를 안 옮긴 채 접근 계층만** 바꿨다.
- Discord 는 카산드라를 떠나며 **저장 계층을** 러스트로 다시 썼다.
- 터소는 **인터페이스만 남기고 엔진을 통째로** 다시 짓는다.

그 이유가 파는 물건에서 나온다. 파는 것이 엣지 데이터베이스인데 SQLite 는 로컬 디스크의 세상에
맞춰 조율된 동기적 단일 라이터 라이브러리다 — **자기가 파는 자리에서 원본이 안 맞았다.**

### 도메인을 제품이 아니라 다시 쓰면서 무엇을 넣었느냐로 잘랐다

관리형 서비스와 오픈소스 엔진으로 가르면 뿌리가 안 보인다. 검증을 설계에 넣은 이야기와
인터페이스를 바꾼 이야기가 **포크를 접고 새로 짓기로 한 하나의 결정에서 갈라져 나온다.**

### 다음 사이클 — 문체 보수 (사다리 3순위)

--gaps 가 정한다. 지금 맨 위는 monzo(2.03)지만 그건 이 시점의 예상이지 확정이 아니다.
