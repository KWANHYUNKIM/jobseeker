...
**1Password** (CA · 보안) — **프로파일까지 썼다.** 도메인 3개, 기능은 아직 0이다. **보안 축 첫 자리다.**

### 이번 사이클 — 신규(6순위), 1Password 프로파일

⚠️ **이 회사는 사업 모델과 기술 구조가 정면으로 부딪히는 자리에 서 있다** — 남의 비밀을 **맡아 주는 장사인데 맡은 것을 자기가 못 읽는다.** 정책이 아니라 **`technically incapable`** 이다. **그래서 남들이 파는 기능 몇 가지를 아예 팔 수 없다**(비밀번호 복구 · 서버 측 검색 · 실제 자격증명으로 하는 유출 확인).

**그 제약이 곧 제품의 값이기도 하다** — 못 읽는다는 것이 **털려도 남이 못 읽는다**는 뜻이라 기업이 맡길 이유가 된다(재구성). 실제로 **매출의 75% 이상이 기업**에서 나온다.

세운 도메인 셋:
- **맡은 것을 자기가 못 읽게 스스로 막는다** — 두 비밀(계정 비밀번호 + **128비트 Secret Key**) · PBKDF2 · **SRP**(비밀번호를 네트워크로 안 보낸다) · AES-GCM-256. ⚠️ **업계의 보통 답을 인용하며 거부한다** — `trust the server, use access controls, audit the logs, and **hope** the infrastructure isn't compromised`. **`hope` 라는 단어가 이 회사의 입장을 말해 준다.**
- **못 읽는데 처리해야 할 때 봉인된 방을 만든다** — AWS Nitro Enclaves · **Rust**(클라이언트 암호 라이브러리 재사용) · ⚠️ **클라이언트가 서명되고 제3자 투명성 로그에 공개된 코드가 아니면 접속을 거부한다** · **컴파일 타임에 강제되는 안전 로깅**(`loggable` 트레이트) · 2024년 9월 독립 감사에서 주요 취약점 0.
- ⚠️ **기업의 접근 권한을 제로 지식을 깨지 않고 자동으로 준다** — **매출 75%가 걸린 축인데 1차 자료를 아직 못 읽었다.** `tech` 를 비워 뒀고 검증이 경고로 잡는다. **Fastly 보안 때와 같은 의도한 공백이다.**

### 다음 사이클 — 확장

**제로 지식 도메인이 가장 익었다** — 버린 대안이 인용까지 붙어 있고 **못 하는 기능 목록**이 그대로 대가다. 기밀 컴퓨팅 도메인도 자료가 충분하다.

⚠️ **프로비저닝 도메인 차례가 오면 기능보다 자료를 먼저 찾는다** — 세 회사 연속으로 통한 규칙이다(Fastly · Monzo · ClickHouse).

⏳ **연표 실마리** — 3자 자료에 **2023년 말 암호화 형식 변경**(고정 길이 페이로드 → 가변 길이·패딩으로 금고 크기 추정 차단)이 있다. **회사 자료로 확인되면 시간축이 된다.**
