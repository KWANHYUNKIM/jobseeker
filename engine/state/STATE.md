...
**카카오뱅크(KR · 핀테크) — 회사 프로파일까지 썼다.** `in_progress` · **도메인 2 · 기능 0.**
**완료 62곳 + 카카오뱅크 = 63곳 · ⚠️ 큐 대기 0/3 · 비교 문서 33편.**

### ⚠️ 큐가 0 이 됐다 — 사다리 3순위가 곧 걸린다

카카오뱅크를 꺼내면서 **`## 대기` 가 비었다.** PROMPT 사다리에서 **큐가 0 이면 후보 조사가 3순위**이고,
그것은 **확장(2순위)보다 아래, 신규(6순위)보다 위**다. 즉 **카카오뱅크를 다 판 뒤**가 아니라
**확장이 끝나는 시점마다** 후보 조사가 먼저 걸린다.
**⚠️ 지난 세션에서 WebSearch 예산이 막힌 적이 있다 — 새 세션이면 먼저 시도한다.**
**⚠️ 요약만 보고 판정하면 과대평가가 난다**(354) — **'무엇이 없는지' 를 함께 묻는다**(352).

### 다음 사이클 — 카카오뱅크의 첫 기능

`--gaps` 가 **확장 2순위**로 잡을 것이다. 빈 도메인은 하나 —
**`빌려줄지 정하는 데 사람을 기다리게 하지 않는다`(tech 비었음)**.
**바로 그 글을 읽어야 한다** — `tech.kakaobank.com/posts/2605-no-wait-collateral-review-system-design/`
(2026년 5월 6일). **이번 사이클에서 제목과 날짜만 확인했고 본문은 안 열었다.**
큐 조사 메모에 **외부 워크플로우 엔진 대신 검증된 사내 Deferred Batch 를 확장했고, 그 대가로
작업 현황·오류 추적이 복잡해져 전용 대시보드를 따로 만들어야 했다**는 내용이 있으나
**원문으로 확인하지 않아 옮기지 않았다.** 먼저 원문을 열고 그 다음에 쓴다.

FDS 도메인(`한 건이 아니라 순서를 본다`)은 **tech 3 개를 이미 채웠으므로** 기능을 쓸 때
**겹치지 않게** 해야 한다 — tech 가 *무엇으로 풀었나*를 가져갔으니 기능은 *어떤 갈림길이 있었나*
(ViT 를 고르기까지 버린 것들, 20ms 예산이 모델을 좁힌 순서, 0.1% 불균형, XAI 를 잃고 되찾은 일)로 쓴다.

### 카카오뱅크 자료 접근 — 다음 실행이 헛돌지 않게

- **`tech.kakaobank.com` 은 WebFetch 로 본문까지 열린다.** 목록 페이지는 **페이지네이션이 있어
  한 번에 4건만** 보였다 — 더 필요하면 `./posts/` 경로를 직접 열거나 검색으로 찾는다.
- **⚠️ 실적을 언론 보도에서 옮겼다**(아시아경제 2026-02-04). `kakaobank.com` 의 IR 페이지는
  이번에 열지 않았다. **1차 자료로 대조하지 않았다는 사실을 `open_questions` 에 적어 뒀다.**
- **⚠️ FDS 정확도 지표가 없다.** 회사가 precision·recall 대신 **t-SNE 군집 시각화**로 대신한다.
  **얼마나 잡고 얼마나 헛짚는지는 공개 자료로 알 수 없다** — 기능을 쓸 때 지표 칸을 억지로 채우지 않는다.
- **`eras` 는 세우지 않았다.** FDS 가 **포인트 기반 → 시퀀스 기반** 세대 교체를 분명히 말하지만
  **앞 세대가 언제부터였는지가 없다.** 기능 단계에서 `history` 로 쓸 수 있는지 다시 본다.

### 비교 문서 재료 — 두 편이 쌓였다, 아직 안 썼다

**① "내가 못 고치는 것이 조용히 바뀐다"** — **ZOZO**(관리형 추천 API, *"종류·분포·건수가 모두
정상인 채 추천의 내용만 열화하는 경우는 검지할 수 없다"*)와 **쏘카**(장비사 50여 곳, *실패 없는
조용한 변경은 미탐지*). **둘 다 실패를 신호로 삼기 때문에 실패 없는 변경을 못 잡는다.**

**② "정확도를 못 재는 것을 어떻게 믿는가"** — **카카오뱅크 FDS**(precision·recall 없이 t-SNE
군집으로 대신), **ZOZO 애매한 검색**(*"nDCG 같은 정량 지표가 아니라 목시 평가"*),
**쏘카 예약 테트리스**(*가상 예약*이라는 대리 지표). **셋 다 정답 집합을 만들 수 없어 대리
지표로 옮겨 갔고, 대리 지표가 무엇을 좋다고 볼지를 정한다.** ⭐ **이쪽이 더 새롭다.**

### 이 세션에서 배운 것 — 다음 회사에도 걸린다

- **제목의 마지막 낱말이 기능으로 쓸 수 있는지를 가른다**(ZOZO). 「検証」은 오프라인,
  「構築と運用」·「対策」은 운영이었다. **본문에서 '본번·릴리스·A/B' 를 먼저 찾는다**(353 의 연장).
- **남에게 의존하는 자리는 그 자체로 도메인이 된다**(ZOZO·쏘카).
- **버린 대안이 인프라 선택에도 있다**(쏘카: AWS SQS 를 페이로드 256KB 한계로 버리고 Pub/Sub).
  **큐·인스턴스 선택도 결정 칸에 들어간다.**
- **⭐ 새로: 지연 예산이 모델 선택을 미리 좁힌다.** 카카오뱅크가 RNN·LSTM·GRU 를 버린 이유가
  성능만이 아니라 **"순차 처리가 필요해 GPU 병렬화에 불리"** 하고 **"20ms 내에 다수 요청을
  처리해야 하는 요구사항"** 이었다. **비기능 요구가 모델 후보를 자르는 순서를 기능에 적는다.**

### ⚠️ 다른 세션과 같은 큐를 잡지 않는다

`guide-engine`(쿠팡)·`study-engine` 둘 다 계속 커밋 중이다.
