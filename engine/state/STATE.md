...
**Rive — 기능 하나를 채웠다(`own-the-rasterizer`).** 빈 도메인 둘. 회사 149개, 큐 1/3.

### 기능 목록이 가장 약한 플랫폼에 맞춰진다

남의 렌더러를 쓸 때의 진짜 값이 이거였다 — 한 백엔드에서 못 쓰면 그 기능은 **어디에도** 못
낸다. 그래서 그리는 층을 직접 만들고 오픈소스로 열었다. 기법은 **특별한 하드웨어를 요구하지
않는 쪽**이다 — 삼각형 병렬 래스터화는 이미 모든 GPU 에 있으니 벡터 경로를 그 형태로 환원한다.

### 수식을 손에 쥐고 있는데 정보를 없앨 이유가 없다

페더링 글의 태도다. 굽고 흐리는 대신 **정규 분포 함수의 적분으로 덮임을 구한다** — 가장자리에서
정확히 50%. 겹치는 모서리는 1차원 표본이 아니라 2차원 적분으로 다루고, 시계 방향 채우기 규칙이
그 해법을 가능하게 했다고 적는다.

### 자기 자랑거리를 스스로 깎는 글이 또 나왔다

`Vector, raster, or both?`(2025-08-14)가 **"벡터는 공짜가 아니다"**라고 적는다 — 경로·곡선·
클리핑 마스크마다 비용이 붙고 저사양 기기에서 CPU/GPU 가 치솟으니 단순화하거나 래스터로 구우라고
권한다. **프로토맵스와 같은 자리다**(자기 것을 쓰지 말아야 할 때를 적는다).

### 다음 사이클 — 확장 (남은 두 도메인 중 하나)

`런타임 안에 언어를 들인다` 가 자료가 두껍다 — Luau 글 하나에 요구사항 일곱, 물린 대안 셋,
프로토콜 다섯(Converter·Node·Layout·Test·PathEffect), C++ API 에서 타입 정의 생성까지 있다.
`디자이너가 만든 것이 그대로 제품에 실린다` 는 상태 기계·데이터 바인딩 글을 더 읽어야 한다.
