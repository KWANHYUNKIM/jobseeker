...
**없다 — 문체 보수 사이클이었다.** Razorpay 를 배민 문체로 다시 썼다(2.81 → 0.0). 회사 115개, 큐 2/3. 45곳 남았다.

### 결제 회사인데 도메인 셋이 다 결제가 아니다

Razorpay 는 인도 온라인 결제의 절반을 처리하는데, 공개적으로 쓰는 기술 글은 관측 비용과
CI 비용과 사내 AI 다. 문체 보수를 하면서 이 경계를 사업 모델에 적어 뒀다 — **결제 도메인을
안 세운 것은 자료를 못 찾아서이지 없다고 판단해서가 아니다.** 이 구분을 안 적으면 읽는 사람은
이 회사에 결제 기술이 없다고 읽게 된다.

같은 회사에서 도메인 셋을 가른 근거도 한 줄씩 적었다. 앞의 둘은 비용을 깎는 이야기고
셋째는 사람의 시간을 깎는 이야기다.

### 관측 비용 — 같은 문제를 파는 쪽에서 본 회사가 이미 둘 있었다

첫 기능에 비교 축을 세웠다. 셋이 같은 문제에 다른 층에서 답한다.

- VictoriaMetrics 는 비용을 짓누르는 것이 샘플 양이 아니라 카디널리티라고 말한다.
- Datadog 은 보고 있는 것만 모은다는 원칙을 수집기에 심었다.
- Razorpay 는 그 도구를 쓰는 쪽이고(수집기가 vmagent 다) 답이 가장 단순하다 —
  도구도 원칙도 안 바꾸고, 감사로 안 쓰이는 것을 찾아 수집 계층에서 버렸다.

**줄이기 전에 아무도 안 쓴다는 것부터 확인한 것이 이 기능의 전부다.** 아마 안 쓸 것이다와
확인했더니 안 썼다는 서로 다른 근거다.

### 이 실패가 곧 지나갈 것인가 — 신흥시장 결제사 둘이 같은 질문에 걸린다

둘째 기능에 Moniepoint 와의 대비를 넣었다.

- Moniepoint 는 일시적 실패가 서비스를 로테이션에서 튕겨내는 것을 막으려고 헬스체크를 직접 만들었다.
- Razorpay 는 일시적 실패와 영구적 실패를 갈라 헛된 재시도를 줄이는 것을 남은 일로 적는다.

층이 다를 뿐 질문은 하나다. 셋째 기능의 ClickHouse 대비(세 겹 중 한 겹만 기계가 강제한다 ↔
여기서는 그 겹이 커널이다)는 이미 있던 것을 살렸다.

### 고친 것 하나 — 데이터가 스스로와 어긋나 있었다

`open_questions` 가 도메인 셋 중 둘이 미독이라고 적어 뒀는데 도메인 셋의 `why` 는 전부
본문 확인함에 날짜까지 달려 있었다. 뒤에 읽고 앞을 안 고친 자국이다. 실제로 남은 미독
(보안 트리아지 글)만 남기고 고쳤다.

### 다음 사이클 — 문체 보수 (사다리 3순위)

--gaps 가 정한다. 지금 맨 위는 honeycomb(2.8)지만 그건 이 시점의 예상이지 확정이 아니다.
