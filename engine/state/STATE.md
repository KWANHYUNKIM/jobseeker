...
**SourceHut — 기능 하나를 채웠다(`five-stages-and-two-roundtrips`).** 빈 도메인 하나.
회사 155개, **큐 0/3**.

### 깃이 느린 게 아니라 깃을 부르기까지가 느렸다

푸시 한 번이 다섯 단계를 지나는데, 처음에는 **단계마다 파이썬 시작 값이 최대 1초**(VM 초기화와
SQLAlchemy 임포트)였다. 실제 일은 시작하지도 않았는데 몇 초가 갔다.

### 코드를 모으려다 병목을 만들었다

같은 로직을 웹과 SSH 양쪽에 두지 않으려고 웹 애플리케이션 워커에 맡겼는데, 그 워커가 깃 작업까지
받으며 **HTTP 요청과 깃 푸시의 평균 시간이 60초에 육박**했다. 걷어내고 따로 처리했다.

### 왕복 20번이 최대 2번이 됐다

인가에 필요한 저장소·소유자·미는 사람의 자격·접근 목록을 따로 묻고 있었다. 서브쿼리와 조인으로
합쳐 **가져오기 왕복 1회, 푸시 최대 2회**가 됐다.

### 소프트웨어로 못 줄이는 자리를 밝힌다

다 줄이고 나서 남은 것은 **서버가 바쁠 때의 I/O 처리량**이고, 이건 하드웨어 문제라 더 나은
서버를 들이고 있다고 적는다.

### 다음 사이클 — 확장 (마지막 빈 도메인)

`무너진 것을 통째로 적는다`. 재료 둘이 손에 있다 — **170시간 장애**(2024-01-19: 상류 널 라우트 ·
대서양 건너 7일 이전 · 클라우드플레어 대신 OVH NAT · 백업 30초/12시간 · 사후 작업 약 60건)와
**장애가 될 뻔한 날**(2020-10-08: GC 크론잡이 저장 모형을 바꿨는데 스냅숏 정책을 안 고쳐 닷새에
100GiB→25GiB, **스냅숏 여섯 개 지우자 500GiB 즉시 회수**).

### ⚠️ 큐가 0/3 이다

SourceHut 을 완주시킨 뒤 **후보 조사 사이클을 끼운다.**
