...
**Ente — 기능 하나를 채웠다(`on-device-inference`).** 빈 도메인 둘. 회사 147개, **큐 0/3**.

### 서버가 못 보니 폰이 본다

종단간 암호화가 서버의 눈을 가리자 검색·얼굴·글자 인식이 **전부 폰의 일**이 됐다. 성능 예산이
서버 랙에서 사용자 기기로 옮겨 간다 — 5만 장이면 **25억 번 대조에 30분**이고, USearch 의 HNSW
색인으로 **30초 아래**까지 내렸다(임베딩은 512차원).

### 같은 축의 판단이 2년 만에 뒤집혔다

2023년 CLIP 을 얹을 때는 **ONNX 를 물리고** GGML(clip.cpp)을 골랐다 — 런타임 없는 얇은 C 층이
이유였다. 그런데 2025년 OCR 에서는 **ONNX 런타임 모바일**을 쓴다. 무엇이 바뀌었는지 어느 글도
안 적어서 `open_questions` 에 남겼다. **같은 회사 안에서 반대 방향을 찾은 첫 사례다.**

### 비공개 의존을 거절하는 것이 판정 기준이다

OCR 에서 구글 ML 킷은 "결과는 좋지만" 플레이 서비스에 매이고 비공개라 **시작부터 탈락**이었다.
테서랙트는 정확도로 탈락. 남은 길은 PaddleOCR v5 를 뜯어 ONNX 로 바꾸고 전·후처리를 코틀린으로
다시 쓰는 것이었다.

### 다음 사이클 — 확장 (Ente 의 남은 두 도메인 중 하나)

`열 수 없게 만들고 그것을 증명한다` 가 자료가 있다(X25519+ML-KEM-768, 캐스트 흐름부터,
HPKE, 러스트 단일 구현). `ente.com/ML` 백서를 읽으면 앞 기능도 층이 는다.

### ⚠️ 큐가 0/3 이다

Ente 를 완주시킨 뒤 **후보 조사 사이클을 끼운다.** 판정 기준을 이어 쓴다 — 회고를 먼저,
되돌린 기록을, **자기 것을 쓰지 말라고 적는 글**을 찾는다.
