...
**(회사는 없다. 사이클 232 는 비교 문서 `after-you-measure` 를 썼다 — 24편째.)**

**축: 재 보고 나서 무엇을 골랐는가. 다섯 사례 중 잰 값이 그대로 답이 된 것이 하나도 없다.**
Target 은 표에서 1등(SLRC **0.143/1300h**)을 떨어뜨리고 **0.131/250h** 를 골랐고,
Careem 은 **좋게 나온 값을 착시로 읽었으며**(Gzip CPU — *"프로듀서가 놀고 있어서"*),
Yelp 는 **정확한 자를 일부러 안 샀다**(CloudTrail $1/100만 건 vs 서버 접근 로그 $0.25 정액).

**문서를 쓰면서 새로 보인 것 셋.**
- **Yelp 만 오차의 방향까지 계산했다.** 부정확한 자를 사면서 *"놓친 접근은 안 쓰인다 쪽으로
  기운다"* 는 것을 알기에 **지우는 결정이 아니라 내리는 결정에** 쓴다. **부정확한 자를 어디에
  쓸 수 있는지까지 정한 사례는 여기 하나다.**
- **손대려던 곳과 실제로 손댄 곳이 다른 사례가 둘.** Careem 은 삭제가 아니라 **언두 로그
  처리**가 문제였고(HLL 58.7억 · 언두 5.7TB), Doximity 는 느린 코드가 아니라 **캐시가 걸린 층**
  이었다(Redis 4.5만 번 → 1번, p95 214초 → 69초).
- **버린 대안에 값을 매긴 곳이 셋뿐이다** — Careem(자연 감소 **약 3.5개월**) · Yelp($1 vs $0.25) ·
  Target(1300h vs 250h). **Zerodha 와 Doximity 는 버린 쪽 이유를 성질로만 적고 값을 안 매겼다.**

**⚠️ 프롬프트가 경고한 대로였다** — EBS 400ms / S3 4~5초는 `log-storage` 가 아니라 다른 기능의
값이라 **쓰지 않았다.** 재료는 전부 회사 파일에서 직접 읽었다.

**남은 비교 문서 축 하나 — 다음 사이클에 쓴다.**
**'무엇을 잃기로 정했는가'**(재료 다섯) — Moniepoint `critical-path`(*"약간의 완전성을 훨씬 큰
신뢰와 맞바꿨다"*) · Doximity `cdc-overlap`(겹침은 받고 간극은 못 받는다) ·
...
⚠️ **Yelp `partition-access` 와 Target `repurchase-timing` 이 232 문서와 겹친다** — 232 는
*재 보고 무엇을 골랐나*, 다음 문서는 *고르면서 오차를 어느 쪽으로 몰았나*다. **인용 블록에서
`after-you-measure` 를 교차 참조하고 범위를 갈라 적는다.**

**🔴 비교 문서 절차(231~232 에서 굳었다)** — `domains/index.json` 을 읽고 **기존 문서 하나를
통째로 읽어 짜임을 따른다**(제목 → 인용 블록[비교 대상·확인/추정 규칙·교차 참조·범위] →
도입 한 문단 + 증거 하나 → 표 → 갈래 해설 → `## 이 사이트에 아직 없는 것`).
**재료는 기억이 아니라 회사 파일에서 직접 읽는다.** `validate.py` 는 비교 문서를 안 보므로
**`companies`/`features` 값이 실제 slug·key 와 맞는지 직접 검산한다.**

**📌 그다음 사이클 — 축 셋을 다 쓰고 나면 보강(사다리 5순위)이다.**
**지금 경고 15건이 그대로 할 일 목록이다** — 문자열 엔티티 14건 정규화
(discord 4 · figma 1 · netflix 1 · shopify 4 · youtube 4)와 **kakao 의 빈 도메인 tech**.
**기계적인 작업이라 한 사이클에 몰아서 할 수 있다.**

**⚠️ 큐는 0 이고 후보 조사가 막혀 있다**(230 에서 여덟 곳 두드려 0곳).
