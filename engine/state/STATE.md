...
**아무도 안 판다 — 362 는 보수 사이클이었고, 인용 표기 정리가 끝났다.**
**완료 60곳 전부 `done` · 큐 0/3 · 비교 문서 33편.**

**⑭ `--gaps` 는 [후보 조사]** 를 가리켰다. **⚠️ 지난 세션에서 WebSearch 예산이 소진돼 막혀
있었는데, 362 에서 WebFetch 는 되는 것을 확인했다**(주소를 아는 글은 열린다). **새 세션이면
WebSearch 도 다시 될 수 있다 — 다음에 후보 조사를 시도해 볼 만하다.**

### 남아 있던 13건을 손으로 갈랐다

- **`build-or-buy.md` 4건 — 전부 굵게로.** Adyen · Spotify · Cloudflare 인용이라 다 영어 원문의
  번역이었다. **쿠팡 인용은 그 넷에 없었다.**
- **`reversible-first.md` 9건 — 8건 굵게, 1건 원문 인용으로 보존.**

### ⭐⭐ 토스 인용을 원문으로 확인하다가 두 가지가 드러났다

원문(`toss.tech/article/payments-legacy-2`)을 열어 대조했다.

1. **인용이 반만 원문이었다.** 문서에는 **PREPARE 대비 CONFIRM 비율이 정상 범위를 벗어나면
   자동으로 레거시로 원복** 이라고 통으로 인용해 뒀는데, 원문은
   *"PREPARE 대비 CONFIRM 비율이 정상 범위를 벗어나면"* 까지이고 그 뒤는
   *"이상 징후 감지 시 자동으로 트래픽을 레거시로 롤백"* 이다. **앞뒤를 이어 붙인 의역이었다.**
   → **원문 부분만 인용으로 남기고 나머지는 굵게로 풀었다.**
2. **또 하나는 원문에 아예 없었다.** **자동 롤백을 걸려면 정상 범위를 판정할 지표와 관측
   체계를 먼저 만들어야 한다**는 문장은 **이 사이트가 정리한 대가**이지 토스의 말이 아니다.
   `toss.json` 에서도 그것은 `tradeoff` 칸의 산문이었다. → **굵게.**

**⭐ 정정을 문서 안에도 남겼다**(PROMPT.md 2″단계) — `reversible-first.md` 의
`이 사이트에 아직 없는 것` 절에 한 줄 더했다.

### ⚠️ 361 과 같은 실수를 또 할 뻔했다

토스 인용을 원문 형태로 되살려 놓고 **그 뒤에 일괄 치환을 돌려서 방금 살린 것을 다시 먹었다.**
확인 단계에서 잡아 되돌렸다. **⭐ 표적 수정과 일괄 치환을 같은 스크립트에서 돌리면 순서가
서로를 덮는다 — 일괄을 먼저 돌리고 표적을 나중에 하거나, 표적 부분을 치환 대상에서 빼야 한다.**
