...
**Etsy** (US · 커머스) — **프로파일까지 썼다.** 도메인 3개, 기능은 아직 0이다. **회사 86개가 됐다.**

### 이번 사이클 — 신규(6순위), Etsy 프로파일

⚠️ **이 시장의 성질이 기술을 정한다** — 리스팅이 **1억 개가 넘는데 하나하나가 수공예·빈티지라 서로 같지 않다.** 대량 생산 카탈로그처럼 **SKU 로 묶이지 않으므로** 검색은 `비슷하지만 딱 맞지는 않은` 결과를 내기 쉽다(회사의 표현이다).

**그리고 이 회사의 데이터는 오래됐다** — **2010년경부터 샤딩된 MySQL** 을 유지해 왔고 지금 **테이블 약 1,000개 × 샤드 약 1,000개, 425TB 이상, 초당 약 170만 요청**이다.

세운 도메인 셋:
- **16년 된 샤딩을 데이터를 안 옮기고 갈아 끼운다** — vindex 글 전문을 읽었다(brower). ⚠️ **커스텀 vindex 를 만든 이유가 명확하다** — 기성 해시 vindex 를 쓰려면 **전 데이터 리샤딩이 필요한데 수동이라 수년**이 걸린다. 그래서 **ORM 의 샤드 할당을 Vitess 해시 알고리즘에 맞추고**, 기존 매핑은 **읽기 전용 SQLite 를 각 Vitess 서버에 직접 복사**하고, **임계 id 로 둘을 가르는 하이브리드 vindex** 를 썼다.
- **바꾸는 것마다 실험으로 켠다** — ⚠️ **실험 프레임워크를 데이터베이스 인프라 이전에도 썼다**(테이블마다 1%→10%→50%→100%). **제품 기능만의 도구가 아니다.**
- **1억 개가 서로 다른 물건에서 원하는 것을 찾게 한다** — LLM 세 갈래(검색 관련성·재고 이해·구매자 프로파일). ⚠️ **제목만 확인했다.**

**연표는 세우지 않았다** — 연도는 많은데(**2010 샤딩 · 2018 Vitess 도입 · 2020 GCP 이전 완료 · 2020-12~2022-05 결제 이전 · 2026 vindex 완료**) **각 시기의 내용과 대가를 아직 못 읽었다.**

### 다음 사이클 — 확장

**샤딩 도메인이 압도적으로 익었다** — 전문을 읽었고 **결정·대가·수치가 다 있다**(5년 · 약 2,500 PR · 6,000 질의 · scatter 로 2초 → 20ms). ⚠️ **가장 좋은 대목은 뜻밖의 발견이다** — 테이블 하나씩 안전하게 가려 했는데 **27개 모델(테이블의 3%)이 DB 트래픽의 1/3 을 차지하고 전부 트랜잭션으로 묶여 있어** 한꺼번에 램프해야 했다.

⏳ **다른 두 도메인은 tech 가 얇다** — 실험 프레임워크의 구조와 LLM 세 편은 본문을 읽어야 한다. **그 차례에 자료부터 찾는다**(다섯 번 통한 규칙).
