...
**없다 — Hatena 를 완주시켰다(도메인 3 · 기능 3 · `status: done`).** 회사 150개, **큐 0/3.**

### 이번 사이클이 채운 것 — `keep-a-25-year-old-runtime`

언어를 갈아엎지 않고 **버전을 올리며 사는** 쪽의 값이 무엇인지 보여 주는 사건이다. 키워드
30만 개로 만든 **4MB 짜리 정규식**이 Perl 5.42.0 에서 컴파일에만 기가바이트를 먹어 OOM 킬러가
요청 프로세스를 죽였다. 원인은 **5.29.9 의 변경** — 긴 점프가 필요한지 확인하기 전에 명령열을
거의 다 만들어 놓고, 필요하면 통째로 버리고 다시 컴파일한다.

### 못 고치는 동안은 기능을 접었다

처음 시도가 실패해 키워드 링크를 접어 뒀다(기간은 글에 없다). 나중에 **AI 로 자동 이분 탐색과
코드 분석**을 돌려 약 20분에 풀었다고 적는다.

### 우회 대신 상류 — 두 번째 사례를 잡았다

고침을 **perl5 저장소에 PR #24555 로 내고 논의나 조정 없이 그대로 병합**됐다. 이건 방금 판
**Ferrous Systems 와 같은 결정**이다 — 저쪽은 인증 린트가 상류 질의 시스템의 버그를 드러냈고,
여기는 오래된 기능이 정규식 컴파일러의 회귀를 드러냈다. 둘 다 자기 쪽에서 피하지 않았다.
`connections` 에 그 축을 심었다.

### 다음 사이클 — ⚠️ 후보 조사 (큐가 0/3 이다)

판정 기준을 이어 쓴다 — 회고를 먼저, 되돌린 기록, 자기 것을 쓰지 말라고 적는 글, 사고를
공개하는 글. **형식이 아니라 대가가 기준이다.** 이번 회사에서 하나 더 얻었다: **상류에 고쳐
보낸 기록**은 판정을 통과시킨다 — 그런 글에는 원인 분석과 맞바꿈이 반드시 들어 있다.
`name_en` 150개를 먼저 출력해 중복부터 확인한다.
