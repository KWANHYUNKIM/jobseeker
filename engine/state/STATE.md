...
**Booking.com(EU/NL · 커머스—여행)** `in_progress` · **도메인 2 · 기능 0** — 프로파일만
세웠다. **70곳** · ⚠️ 큐 0/3.
⚠️ **본문은 브라우저(`get_page_text`)로 `medium.com/booking-com-development` 를 연다.**

### ⚠️ `category` 에 '여행' 은 없다 — 커머스로 넣었다

`CATEGORIES` 는 핀테크·커머스·소셜·메시징·스트리밍·검색·광고·모빌리티·게임·SaaS·기타
열한 개다. **trivago 는 `검색`, 이곳은 예약 중개라 `커머스`** 로 넣었다.
**새 회사를 넣을 때 분류를 지어내지 않는다** — 검증이 잡아 준다.

### 두 도메인이 같은 이야기의 두 규모다

**작은 결함이 번져 믿음이 무너진다** — 데이터 쪽은 그것을 `깨진 유리창` 으로 설명하고,
백업 쪽은 그것이 **순환 의존성**이라는 구조로 굳은 사례다. 회사가 그 굳는 과정을
직접 적는다: 시스템이 커지는 동안 되돌아볼 시간이 없었고, **클라우드 이전 같은 큰
계기가 있을 때** 고친다고.

### 다음 사이클 — 어디부터든 자료가 있다

- **지키는 것 안에 지킬 수단을 두지 않는다** — `Breaking the Loop`(전문 읽음).
  **20분 분량이고 실패까지 적는다**(KMS 키를 나중에 다중 리전으로 못 바꿔 클러스터
  재구축, `a quite painful lesson`). 버린 대안 둘(복제 기반 이전 · Aurora)도 명시적이다.
  **연도가 있다** — 계획 2024 Q2, 착수 Q3, 완료 2025.
- **품질을 마지막 관문이 아니라 생애주기로 옮긴다** — `Broken Windows of Data`(전문 읽음).
  **수치가 좋다**(dbt 모델 약 1,000개 · 검사 통과 95% 초과 · 승인까지 중앙값 **2시간 →
  1시간 미만** · MR 물량 7개월에 3배 이상). **너무 엄하게 시작해 마찰이 생겼던 것까지
  적는다.**

### 아직 못 잡은 것

1. **Booking.com 단독 매출** — 공개 자료가 모회사(Booking Holdings) 기준이다.
2. **Perl 모놀리스 → Java·Kotlin 이전**이 있다고 알려져 있으나 **1차 자료로 확인 못 했다.**
   확인하면 `eras` 의 재료다.
3. 안 읽은 글 — `A Migration Adventure` · `Kotlin Multiplatform in Production` ·
   `The Causality Gap` · `Stability in Jetpack Compose`.
