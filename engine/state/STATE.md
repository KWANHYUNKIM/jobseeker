...
**Zig 완주(기능 4개).** 회사 138개, **큐 0/3 — 다음 사이클은 후보 조사다.**

### 이긴 숫자와 진 숫자를 같은 표에 적는다

직접 쓴 deflate 는 시간이 9.7% 빠르고 캐시 미스가 47.3% 적은데 **압축률은 zlib 이 1.00% 낫다**
(최고 수준에서 0.77%). 이긴 것만 적으면 쓰는 쪽이 자기 워크로드에 맞는지 판단할 수 없다.

### 감싸면 남의 결함까지 받는다

zlib 를 감싸는 순간 그쪽이 자기 방식으로 메모리를 잡고 자기 방식으로 막는다 — **입출력을
인자로 받고 할당을 감추지 않는 규칙이 거기서 끊긴다.** 그래서 이력 창도 쓰는 쪽 버퍼 안에 둔다.

윈도우는 상위 DLL 을 거치지 않고 가장 낮은 안정된 시스템콜(NtDll)까지 내려가는 이전을 끝냈다.
그 덕에 Batch 와 취소가 윈도우에서 제대로 구현됐다.

### 남긴 의존에도 이유를 적는다

인증서용 암호 함수(kernel32·crypt32)만 외부에 남기고 **실수가 아니라 의도한 선택**이라고 밝힌다.
안 만든 기능의 이유를 적는 손버릇이 여기서도 나온다.

### 네 기능이 한 문장으로 모인다

감춰진 것을 만들지 않는다 — 입출력도 할당도 인자로 드러내고, 언어는 안 만든 이유까지 적으며,
표준 라이브러리는 감싸지 않고 진 자리까지 적는다. **릴리스 노트 한 편에서 회사 하나가 나왔다.**

### 다음 사이클 — 후보 조사 (큐 0/3, 비었다)

기준은 그대로 — 회사 소유 도메인 · 엔지니어가 직접 쓴 글 · 자기 시스템의 의사결정과 대가.
최근에 데이터센터·트레이딩·언어로 벌려 놨으니 또 다른 축을 찾는다.
