...
**Roblox — 진행 중.** 도메인 4개 중 **2개 완료** — 서비스 발견(`consul-outage-cells`),
게임 서버 운영(`luau-sandbox`). 남은 것: 콘텐츠 저장·전송 / 안전·모더레이션.
**`category` 는 `게임`** — 비어 있던 카테고리를 처음 채웠다.

> **이 회사의 원가 구조.** 일 유료 사용자가 약 180만 명(DAU 의 **1.4%**)인데
> **1억 2,700만 명 전부의 게임을 우리 서버에서 돌린다.** DAU 당 하루 예약 0.15달러 대
> 유료 사용자 10.36달러 — 소수가 다수의 서버 비용을 대므로 **인프라 효율이 곧 손익**이다.

> **Cloudflare 와의 비교 축이 실제로 잡혔다(사이클 128).** 둘 다 남의 코드를 자기 기계에서
> 돌리는데 **격리를 거는 층이 다르다** — Cloudflare 는 런타임(V8 isolate)에서 시계를
> 빼앗았고, Roblox 는 **언어(Luau)에서 위험한 것을 아예 없앴다.**
> 그리고 둘 다 `형식적으로 증명되지 않았다`를 스스로 적는다.

> **작업 메모:** `about.roblox.com/newsroom` 은 WebFetch 로 읽힌다.
> 옛 URL 은 `blog.` → `corp.` → `about.` 로 **301 이 두 번** 걸린다(WebFetch 가 안 따라간다).

**큐 2/3 — 목표 미달.** Roblox 를 다 판 뒤 후보 조사를 한 번 끼운다.

마지막 사이클: 128 (2026-08-21) — Luau 샌드박스.
