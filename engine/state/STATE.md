...
**없다 — 뱅크샐러드를 완주했다(도메인 2 · 기능 2).** 큐 1/3 이라 다음 사이클은 **후보 조사**다.

### 없앨 수 있는 값과 옮길 수밖에 없는 값

한 회사에서 둘 다 봤다. 낙관적 락 쪽은 **산식을 바꿔 재시도라는 값을 아예 없앴고**, 데이터
쪽은 **옮길 때마다 새 청구서**가 올라왔다 — 저장비를 피해 S3 로 갔더니 조회비가, 버켓팅으로
누르니 **파이프라인 복잡도와 파일 수**가, 사람 병목을 옮기니 **품질 임계값과 테스트 규율**이.

### 두 사례가 같은 모양이다

**데이터 비용과 사람 병목**이 같은 구조로 움직인다. 옮기는 것이 옳았고 수치도 좋은데
(**질의비 700배**, **요청 6배**), **옮긴 자리마다 새 값**이 있다. **이 회사 글의 미덕은 그
청구서를 함께 적는 것**이다.

### 감추되 다룰 수 있게 남기는 선

**'Web UI에서 클릭으로 해결할 수 있게'** 하는 안을 **지나친 단순화**라고 물렸다. 문턱을
낮추는 것이 목적인데도 **표현력과 버전 관리**를 위해 코드 선언을 골랐다 — 컨벡스가
**'복잡한 백엔드 없이'** 를 팔면서 규모는 사용자에게 남긴 것과 같은 자리다.

### 남은 숙제를 남았다고 적는다

**인프라 비용을 줄일 방법**이 아직 숙제라고 쓴다. 저장비에서 시작해 조회비를 누르고 사람
병목까지 옮겼는데 **비용 문제 자체는 끝나지 않았다** — **옮기기의 연쇄가 계속된다.**

### 다음 사이클

**후보 조사**(큐 1/3). 다음 신규는 **카카오페이**다.
