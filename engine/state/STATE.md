...
**없다 — 카케하시를 완주했다(도메인 2 · 기능 2).** 큐 0/3 이라 다음 사이클은 **후보 조사**다.

### 이 회사는 지식을 남길 때 읽는 쪽에 AI 를 넣고 생각한다

의미 문서는 **'사람도 AI 도 읽을 수 있는 곳'**(깃허브·노션)에 두고, 설계 원칙은 **문서가
아니라 코딩 에이전트용 플러그인(kamae-ts)** 으로 남긴다. **두 도메인이 같은 방향**이다 —
**머릿속에 있던 것을 밖으로 꺼내 AI 가 닿게 한다.**

### 근거가 성능이 아니라 '무엇과 맞느냐'다

SSR 을 고른 이유가 **OIDC 가 애초에 리다이렉트와 폼 전송으로 도는 규약**이라는 것이다 —
**결을 따르면 단계마다 단순해진다.** Gunosy 가 **늘릴 때 손이 얼마나 드느냐**로 골랐던 것과
같은 종류의 근거다.

### 모으면 없는 능력이 그대로 구멍이 된다

기반 넷을 한 팀에 몰아 두니 **'프런트엔드에 정통한 멤버가 적은 구성이라 생기는 과제'** 가
생겼다 — **접근성과 레거시 환경 동작성에 지견이 없다**고 스스로 적고 **대책은 안 나온다.**

### 두 글이 같은 경고로 끝난다

**'추상도를 너무 올리면 팀의 학습 비용과 가독성에 영향을 준다'**(함수형)와 **'타입 파라미터·
Conditional Types 남용'** — **여러 제품이 공유하는 기반이라 읽는 사람이 많다**는 것이
그 선을 긋는 이유다.

### 규모 수치가 통째로 없다

**몇 개 제품을 떠받치는지, 팀이 몇 명인지, 연구회에 몇 명이 오는지**가 다 없다. 데이터
쪽도 **저장 중복 크기·재생성 시간**이 없다 — **'이렇게 했다'는 촘촘한데 '얼마인가'가 비었다.**
