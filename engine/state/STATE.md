...
**없다 — Metabase 를 완주시켰다(도메인 1 · 기능 1 · `status: done`).** 회사 163개, **큐 0/3.**

### 하나만으로는 사고가 아니었다

이번 기능(`four-weak-layers-in-a-row`)의 축이다. 열린 스키마도, 넉넉한 매개변수 전달도, ORM
조회도, `raw` 통로도 **그 자체로는 버그로 보고되지 않는다** — 이어졌을 때만 취약점이 된다.
그래서 고침도 한 층이 아니라 **세 겹**에 두고 같은 부류를 넓게 강화했다.

### 리팩터가 조용히 문을 연다

원인 중 하나가 **자기들이 2025년 11월에 한 인증 리팩터**다(`token`·`password` 만 넘기던 것을
본문 전체로). 그때는 무해해 보였을 변경이다 — 아래 계층이 모르는 키를 어떻게 다루는지에 달려
있었으니까(재구성). **언제 열렸는지를 사후 기록에 적는 것**이 이 글의 값이다.

### 두 고객에게서 같은 순서

발견이 구체적이다 — **한 IP 가 서로 다른 두 고객 인스턴스에서 같은 엔드포인트 순서로 API 키를
만든** 것을 알아채고 제로데이를 역설계했다.

### 사후 기록이 말하지 않는 것을 여섯 가지 세웠다

닫힌 스키마 검증의 정의·구현 · `raw` 를 아예 금지했는지 · 제3자 포렌식이 찾은 것 · 패치와 공개
사이 3주의 이유 · 남은 미해결 목록 · 어떤 감시 도구로 봤는지. **좋은 사후 기록에도 빈칸이
있다는 것**을 `failure` 도해로 남겼다.

### 다음 사이클 — ⚠️ 후보 조사 (큐가 0/3 이다)

**의료 축이 계속 비어 있다**(Medplum 접음). `blocked` — Sourcegraph(403×2) · incident.io(404×3) ·
Guardian · Flexport · Cal.com.
