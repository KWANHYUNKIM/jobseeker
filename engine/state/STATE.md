...
**없다 — Perplexity 를 완주했다.** 도메인 **3** · 기능 **3** · `status: done`. 회사 111개. ⚠️⚠️ **큐 0/3 — 다음 사이클은 반드시 후보 조사다.**

### 이번 사이클 — `NIC 에 안 묶이는 점대점 통신 (fabric-lib)`

⭐ **이 회사에서 유일하게 1차 자료로 판 도메인이다** — 블로그는 403 이지만 **arXiv 2510.27656(MLSys 2026)과 GitHub 은 직접 열린다.**

⭐ **기여가 성능이 아니라 이식성이다**(재구성). 막힌 길이 하나씩 적혀 있다 — **`DeepEP 는 ConnectX 만 제공하는 GPU 개시 RDMA 를 요구하고, NVSHMEM 은 EFA 에서 성능이 크게 떨어지며, Mooncake 와 NIXL 은 EFA 를 지원하지 않거나 아직 초기 단계다.`** ⚠️ **AWS 에서 돌리는 쪽에는 선택지가 없었다는 뜻이다**(재구성).

건진 것 넷:

- ⭐ **더 빠른 길을 알면서 안 갔다** — **`GDA 는 호스트 CPU 를 우회해 지연을 줄이지만 아직 대부분의 클라우드 인스턴스에서 쓸 수 없다. 우리의 호스트 프록시 설계는 현세대 하드웨어를 감당하면서 이식성을 지킨다.`** ⚠️ **대가가 수치로 남아 있다** — 프록시 오버헤드 **p50 8.5µs(CX-7) · 27.9µs(EFA)**, **EFA 디코드가 DeepEP 보다 약 30% 뒤진다.**
- ⭐ **전제를 낮추고 순서를 다른 층에서 얻는다** — 완료를 **순서가 아니라 횟수(`ImmCounter`)** 로 세고, 가시성은 **`CPU 가 목표 ImmCount 를 관측한 뒤에는 이어지는 CPU-GPU 트랜잭션이 앞선 NIC-GPU 데이터 쓰기 뒤로 PCIe 스위치에 의해 정렬된다`** 로 보장한다. ⭐ **네트워크에서 못 얻는 성질을 PCIe 에서 구했다.**
- ⭐ **전송을 계산 뒤에 숨긴다** — **`각 레이어의 어텐션 뒤에 GPU 가 UVM 카운터를 올리면 CPU 쪽 콜백이 발화해 그 레이어의 WriteImm 을 제출하고, 그동안 다음 레이어가 계산된다.`**
- ⭐ **못 하는 것을 못 한다고 적는다** — **`전송에 청킹이 없어, 디코드에 최적화된 우리 커널의 메모리 부담이 배포 가능한 모델의 집합을 제한한다`**(프리필).

**수치** — 조 단위 파라미터 **RL 가중치 갱신 1.3초**(기존 대비 100배 이상; Kimi-K2-1T 에서 언샤딩 518ms·동기화 357ms·RDMA 제출 26ms, **파이프라인 밖 전송은 42ms 뿐**). **MoE 디코드 H200 EFA 66.8 tok/s vs pplx-kernels 21.0(배치 2) — 3~6배**, **`EFA 에서 실시간 추론을 처음으로 현실화했다.`** 대역폭 **64KiB 페이지 Write 로 CX-7 370Gbps · EFA 364Gbps**(⚠️ **단일 256KiB Write 는 116/54Gbps** — 쪼개야 대역폭이 난다).

### ⭐ 완주 관찰 — 언어 경계가 마이크로초로 확인됐다

**UVM watcher 콜백이 러스트 6.3±1.3µs(p99 12.6), 파이썬 9.8±9.0µs(p99 20.4).** ⭐ **프로파일에 `언어 경계가 곧 아키텍처`라고 적었는데, 그 경계의 값이 여기서 숫자로 나왔다** — **평균보다 편차와 p99 의 차이가 더 크다**(재구성).

**세 기능이 한 압력에서 나온다** — **사용량으로 파니 GPU 이용률이 곧 마진이다**(재구성): **앞단은 GPU 를 놀리지 않으려고 러스트로**(Ivy·Tulip), **엔진은 파이썬을 쓰면서 파이썬 값을 안 치르려고 그래프로**(ROSE), **GPU 사이는 클라우드에서 돌아가게 하려고 이식성으로**(fabric-lib).

⭐ **㊵축(`모르는 것을 어떻게 적나`)에 이 회사도 들어간다** — Block 과 같은 성질인데 **여기는 논문 형식이라 한계 절을 따로 둔다.**

### 다음 사이클 — ⚠️⚠️ 후보 조사 (큐 0/3, 바닥)

**먼저 `name_en` 목록 출력으로 시작한다.** ⭐ **`큰 이름인데 아직 없는 회사` 각도가 네 조사 연속 통했다** — ⏳ **Anthropic · Databricks · Snowflake · Anduril · Figma 외 디자인 툴 · Ramp 외 핀테크.**

⏳ **Perplexity 보강 거리** — `Lily`(애플 실리콘 로컬 추론) · `pplx-kernels`·CuTeDSL(**GitHub 은 직접 열린다**) · 검색 본업(색인·랭킹·인용 검증) 자료.
