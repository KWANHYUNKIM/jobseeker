...
**Let's Encrypt** — 첫 기능 `turn-it-off-in-three-steps`(끄기 전에 대체 주소를 먼저 넣는다)
를 끝냈다. **도메인 2 · 기능 1.** 큐 0/3. 남은 도메인은 **인증서를 짧게 만들어 폐기가 덜
필요하게 한다.**

### 없애는 일에도 순서가 있고, 그 순서가 곧 안전이다

일정이 세 걸음이다. **2025-01-30** 새 Must-Staple 요청 차단(전에 발급한 계정만 예외) →
**2025-05-07** **CRL URL 을 먼저 넣은 뒤 OCSP URL 을 뺀다** → **2025-08-06** 응답자 종료.
같은 날에도 순서가 있고, 서버는 **3개월 뒤에** 끈다.

### 이미 나간 것은 회수할 수 없다

인증서는 발급되면 **유효 기간이 끝날 때까지** 그때의 정보를 담고 산다. 그래서 URL 을
바꿔도 옛 인증서들은 여전히 옛 주소를 가리키고, **없애려는 서비스를 3개월 더 돌려야**
한다. curl 이 "자기 파일이 남의 손에 다시 빌드되어 퍼져 손댈 수 없다"고 적은 것과 같은
전제 위에 있다.

### 표준은 만드는 것보다 채택되는 것이 어렵다

**OCSP Must Staple 은 여러 해가 지나도록 넓은 브라우저 지원을 얻지 못했다.** 프라이버시
문제를 서버 쪽으로 옮기는 정공법이었는데 생태계가 안 따라왔다. 기다리지 않고 **함께
끊었다** — 그 확장을 쓰던 소수에게는 대안이 없다.

### 대가를 안 적은 자리를 세어 뒀다

**성능 비교가 없다**(OCSP 지연 대 CRL 조회 시간). **CRL 배포 구조·갱신 주기·클라이언트가
어떻게 소비하는지**도 없다. 수치라고는 '상당한 자원'과 '아주 작은 비율'뿐이다. 없는 것을
지어내지 않고 **failure 도해와 open_questions 에 이름 대어 적었다.**

### 다음 사이클

확장 — 남은 도메인(수명 단축). 10년 치 자료가 있지만 **45일 글에는 물린 대안도 정량
비용도 없다.** `why-90-days`(2015)가 원래 결정의 근거를 담고 있어 그쪽이 두껍다.
