...
**없다 — 문체 보수 사이클이었다.** Wise 를 배민 문체로 다시 썼다(2.2 → 0.0). 회사 115개, 큐 2/3. 58곳 남았다.

### 증상이 원인을 안 가리킬 때 — 둘이 같은 자리다

Wise 의 세 번째 기능이 그 이야기다. Thanos 에서 Mimir 로 옮긴 것은 성공이었는데 그 뒤에
병목이 조용히 자랐다. 증상이 넷이었고 흩어져 있었다. 긴 범위 질의의 5XX 와 지연, 읽기 경로의
컴퓨트 증가, 가용영역 간 전송 비용 상승, S3 저장 비용의 불균형한 증가. 회사가 그 상태를
이렇게 적는다 — 이 증상들은 처음에 압축이 병목이라고 말해 주지 않았고, 그것이 원인일 수
있다는 생각조차 떠오르지 않았다.

이 엔진의 Anthropic 이 같은 자리에 있다. 품질 저하 신고도 세 원인이 서로 다른 트래픽 조각에
서로 다른 일정으로 닿아 합치면 넓고 일관성 없는 저하로 보였다. 둘 다 합계 지표로는 영영 안
보였을 자리이고, 둘 다 변경 목록을 거꾸로 훑어서 찾았다.

### 이번 사이클 — Wise

사업 모델에서 전송의 약 65%가 즉시 도착한다는 비율 자체가 제품의 값이라는 것을 앞세웠다.
그 즉시를 만들려면 각 나라의 결제망에 직접 붙어야 하고, 그래서 영국과 헝가리와 호주에는
물리 데이터센터가 필요하다.

도메인 경계의 근거를 적었다. 표준을 정하는 일과 퍼뜨리는 일을 따로 볼까 했으나 이 회사에서는
퍼뜨리는 방법이 곧 표준의 형태라 한 경계로 뒀다.

영문 원문이 열넷 있었고 전부 풀었다. 관점 전환을 말한 문장은 배포를 단순한 거래로 보던
것에서 조율된 사건의 흐름으로 보는 것으로 바꾼 것이라고 옮겨 도메인 이름과 이어지게 했다.
