...
**QuestDB — 기능 하나를 채웠다(`qwp`, 넣기와 꺼내기를 한 프로토콜로 합친다).** 회사 122개,
큐 1/3. 빈 도메인 둘 — JVM · 시계열 표현.

### 표준을 형식으로는 안 쓰고 경계에서만 쓴다

Arrow IPC 를 채택하지 않은 이유를 밀도로 적는다 — 자체 컬럼 형식이 Arrow IPC 가 허용하는 것보다
더 촘촘히 담기 때문이다. 그런데 Arrow 를 버리지도 않는다. **널이 없는 DOUBLE 컬럼은 수신 프레임을
그대로 가리키는 Arrow 버퍼가 되어 복사 없이 넘어간다.** 형식은 자기 것으로, 상호운용은 경계에서만.

이 엔진의 프로토콜 축에서 세 회사가 이제 각자 다른 답을 갖게 됐다 — DuckDB(형식 통제권 때문에 자체
프로토콜) · Dolt(생태계 때문에 흉내) · QuestDB(밀도 때문에 자체 형식 + 경계에서 표준).

### 정확성을 못 주면 도구를 준다

재전송이 최소 한 번이라 클라이언트는 **서버가 못 받은 것과 커밋했는데 응답이 죽은 것을 구분하지
못한다**고 회사가 그대로 적는다. 그래서 중복을 막는 책임이 사용자에게 넘어간다 — `DEDUP UPSERT
KEYS(...)` 를 서버 쪽에 선언해 두라고 요구한다.

배압도 같은 태도다. 큐 한도를 넘으면 조용히 버리지 않고 쓰는 쪽을 멈추게 한다. 대신 오류가 호출한
자리가 아니라 오류 핸들러로 온다.

### 수치와 조건

수집 초당 530만 → 1,900만 행, 결과 초당 2억 2천만 행, 자동 플러시 75,000행·1,000ms → 1,000행·100ms.
다만 **벤치마크의 하드웨어와 데이터셋이 이 글에 없다** — 별도 글이 있다고만 적는다.

### 다음 사이클 — 확장 (QuestDB 의 빈 도메인)

JVM 밑바닥 글 셋과 WINDOW JOIN 글의 주소를 QUEUE 에 적어 뒀다.
