...
**큐 4/3 — 충분.** 140 후보 조사에서 둘을 더 올렸다:
**GitHub**(US, 개발자 도구) → **LY/LINE**(JP, 메시징) → **ByteDance**(CN, 소셜·스트리밍) →
**Spotify**(EU, 스트리밍). 다음 신규는 **GitHub** 부터 — 첫 기능은 코드 검색(Blackbird).

> **국가 공백이 거의 메워졌다.** 21곳이 US 13 · KR 7 · CA 1 이었는데
> 큐에 JP·CN·EU 가 하나씩 들어왔다.

> **ByteDance 는 이 엔진에서 처음으로 1차 자료가 기술블로그가 아니라 동료 심사 논문이다.**
> Monolith(RecSys 2022) · LavaStore(PVLDB 2024) · ByteHouse(ICDE 2025).
> ⚠️ **VLDB PDF 는 WebFetch 요약이 얕게 나온다** — 대신 **파일이 로컬에 저장되므로
> `Read` 도구로 `pages` 를 지정해 정독할 것.** arXiv 는 그냥 잘 읽힌다.

> **Monzo 를 재 보고 뺐다.** 마이크로서비스·Cassandra 로 유명했는데 최근 글이
> AI 도구·채용·조직 문화 쪽으로 기울어 아키텍처 결정과 대가를 다루는 글이 안 보인다.
> **유명한 기술블로그가 지금도 그런지는 따로 확인해야 한다** — 평판은 낡는다.

마지막 사이클: 140 (2026-08-21) — 후보 조사.

## 다음 선택지

*(사다리는 PROMPT.md 2단계가 정한다. 여기는 그 사다리가 여러 개를 물어올 때의 판단 근거다.)*

1. **GitHub 신규** — 큐 맨 위. 개발자 도구 축이 통째로 비어 있고,
   Blackbird 글은 **버린 대안을 숫자로 계산해 둔** 드문 자료다.
