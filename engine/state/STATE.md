...
**없다 — Ramp 를 완주했다.** 큐가 **0/3** 이다.

### 이번 사이클 — Inspect `에이전트에게 신원을 주지 않고 사람의 신원을 빌려 쓰게 한다` 와 완주

⚠️ **이 회사에서 가장 값진 결정은 성능이 아니라 신원에 있었다.** 앱 토큰으로 PR 을 만드는 것이 훨씬 편한데 안 썼다 — 모두가 같은 신원을 쓰면 **`any user to approve their own changes`** 가 되어 **코드 리뷰가 형식이 된다.** 그래서 **시킨 사람의 GitHub 토큰**으로 PR 을 만들고 **프롬프트를 그 사람에게 귀속**시킨다. **새 문을 만들지 않고 이미 있는 문을 그대로 쓰되, 그 문이 무의미해지지 않게 지켰다**(재구성).

**오픈소스를 고른 이유도 특이하다** — `ask the AI to read the code of OpenCode itself`. **에이전트가 자기 동작을 문서가 아니라 소스로 확인한다.** Fastly 가 프로토콜을 고르며 `understandable` 을 이유로 든 것과 같은 결이다.

### 🎯 다음 사이클 — 비교 문서(8순위)가 열렸다

**`사내 AI 에이전트를 어디까지 믿을 것인가` 에 네 회사가 찼다.** 그리고 축이 선명해졌다 — **`얼마나 맡기나` 가 아니라 `그 선을 무엇이 지키나` 다.**

| 회사 | 어디까지 맡기나 | **그 선을 무엇이 지키나** |
|---|---|---|
| **Sentry** | AI 가 PR 까지 열고 **병합은 사람이** · 매일 밤 에이전트를 심문 | **조직의 신뢰** — `Your org might not be ready to trust the machines that far, and that's ok.` |
| **ClickHouse** | 분석의 **70%**, 남긴 **30%** · 출력만으로 결정 금지 | ⚠️ **세 겹 중 한 겹만 기계가 강제**(접근 범위). 나머지는 규율 |
| **Duolingo** | 테스트 PR **하루 20개** · **250개 전부 사람 리뷰** · 쓰기는 승인 버튼 | **사람 리뷰** — ⚠️ **그래서 병목이 됐다**(`faster than the existing review process can absorb them`) |
| **Ramp** | **병합 PR 의 약 30%** | ⚠️ **사람의 신원** — 앱 토큰을 안 써서 **기존 코드 리뷰가 그대로 작동한다** |

**비교의 발견(초안)**: 네 곳 다 사람을 마지막 문으로 두는데 **그 문을 지탱하는 것이 다르다** — 조직 문화(Sentry) · 접근 범위(ClickHouse) · 사람의 시간(Duolingo) · **신원과 귀속(Ramp)**. ⚠️ **그리고 시간의 힘을 받는 것은 마지막 하나뿐으로 보인다** — 문화는 흔들리고, 규율은 잊히고, 사람의 시간은 병목이 되는데, **신원은 구조라 닳지 않는다**(이 해석은 비교 문서에서 **추정**으로 적어야 한다).

**PROMPT 의 비교 문서 규칙**: 새 주장을 만들지 않고 **이미 각 회사 페이지에 `confirmed` 로 적힌 사실만 엮는다.** 해석은 본문에 **추정**이라고 쓴다. 표로 나란히 놓고 **각자가 감수한 것**과 **서로가 답하지 않은 질문**을 반드시 넣는다. 파일은 `jd-viewer/public/reveng/domains/<slug>.md` 와 그 폴더의 `index.json`.

⚠️ **큐가 0/3 이라 3순위(후보 조사)가 사다리에서 더 위다.** 다음 사이클의 `--gaps` 출력을 그대로 따른다.
