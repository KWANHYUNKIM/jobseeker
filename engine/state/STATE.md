...
**GitHub — 진행 중. 22번째 회사다.** 도메인 4개 중 **1개 완료** — 코드 검색(`blackbird`).
남은 것: Git 저장·복제 · 데이터 계층 · CI 실행.
...
> Discord(비상장)·Slack(인수 후 미공시)에 이어 세 번째 경우다.

> **Blackbird 글이 이 엔진에서 만난 것 중 가장 좋은 자료 축에 든다.** 버린 대안을
> **숫자로 계산해 뒀다** — ripgrep 방식은 2048코어 완전 병렬로도 0.01 QPS 라
> 경제적으로 불가능. 저장소 4,500만·115TB → 색인 25TB · 문서 155억 · p99 100ms.
> **Git 블롭 ID 로 샤딩하면 중복이 저절로 사라진다**는 것이 이 설계의 핵심이다.

> **Vitess 네 번째 사례다** — Figma·Notion 이 버리고 Slack 이 골랐던 그것.
> GitHub 은 **도입 초기라 망설였고 그래서 쓰기 전환 스크립트를 위험 완화책으로 따로 만들었다**
> (수십 밀리초에 테이블 130개 전환). **하나에 전부 걸지 않은 사례**로 비교 문서에 쓸 값이 있다.
...
마지막 사이클: 142 (2026-08-21) — Blackbird.

> **Dropbox 에서 세운 규칙(한 도메인당 최소 두 글)이 곧바로 값을 했다.**
> 2023년 아키텍처 글 + **2026년 케이스 폴딩 글**을 겹치니 **3년치 성장**이 드러났다 —
> 저장소 4,500만 → **1억 8천만**, 115TB → **480TB**(둘 다 4배). 한 글만 읽었으면
> 2023년 숫자를 현재로 적을 뻔했다. **오래된 글을 쓸 때는 최신 글로 규모를 갱신할 것.**

> **새 축 — '최적화가 더 큰 최적화를 막고 있을 때.'** 케이스 폴딩 루프의 조기 종료가
> 벡터화를 통째로 막고 있었다. **분기만 없애고 break 를 남긴 중간 단계는 오히려 느려진다**
> (3.1 → 2.6 GiB/s) — 옳은 방향으로 한 걸음 갔을 때 지표가 나빠지는 함정이다.
> 답은 **'무엇이 느린가'가 아니라 '무엇이 켜지지 않았는가'** — 벡터 명령 개수를 셌다(0 → 41).
> Dropbox 'SSD 캐시를 뗀다'와 같은 결의 축이라 비교 문서에 함께 쓸 수 있다.
