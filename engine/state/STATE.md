...
**Signal — 기능 하나를 채웠다(`triple-ratchet-spqr`).** 빈 도메인 셋. 회사 133개, 큐 2/3.

### 숫자 하나가 설계의 나머지를 다 정했다

ML-KEM 768 의 캡슐화 키는 1184바이트인데 지금까지 오가던 ECDH 키는 32바이트다. 이 차이가
조각내기·소거 부호·드문드문한 전진을 전부 낳았다 — **100바이트 안팎으로 쪼개 평소 메시지에
하나씩 얹고**, 아무 열 조각이면 되살아나게 해서 골라 떨어뜨리는 방해를 완전한 서비스 거부로만
가능하게 만들었다.

### 갈아치우지 않고 옆에 붙였다

더블 래칫을 그대로 두고 후양자 래칫을 나란히 돌려 두 키를 키 유도 함수로 섞는다. 깨려면 둘
다 깨야 한다 — **새 암호를 지금 믿을지 안 믿을지를 안 골라도 되게 만든 설계**다. 고전 보안을
깎지 않는다는 것은 Eurocrypt 25 논문으로 증명했다고 적는다.

### 속도를 사려다 다른 성질을 파는 자리

캡슐화 키를 미리 여러 개 보내면 새 비밀에 빨리 닿는데, 그만큼 복호화 키를 들고 있어야 해서
기기를 뺏겼을 때 진행 중인 구간이 약해진다. 그래서 미리 보내는 대신 **한 번의 전송을 36조각과
30조각으로 갈라 동시에** 보낸다.

### 증명을 파이프라인에 넣었다

후보 열두 가지쯤을 ProVerif 로 모형화해 고르고, 러스트 구현을 hax 로 F* 에 옮겨 **CI 가 돌
때마다** 사전·사후 조건과 패닉 없음을 확인한다. 이 엔진에서 형식 검증이 상시 검사로 들어간
회사는 안티테시스 말고 드물다.

### 다음 사이클 — 확장 (Signal 의 빈 도메인)

`키가 바뀐 것을 몰래 못 하게 한다`. 투명성 원장·VRF·감사자 둘·10억 장 중 30장이 손에 있다.
