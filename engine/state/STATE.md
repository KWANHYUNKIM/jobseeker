...
**Meituan(美团, 52번째) `in_progress` — 도메인 1개 · 기능 1개.** 300 에서 도메인 ① 의 첫 기능
`semantic-before-text`(**글자가 안 겹쳐도 뜻이 통하면 올린다**)를 썼다. **결정 8개.**

**🔴 300 의 가장 중요한 발견 — 이 블로그는 옛 글을 내린다.**
목록과 검색에는 제목·주소가 남아 있는데 **열면 404 다.** 확인한 것:
`/2022/08/11/coarse-ranking-exploration-practice.html` · `/2022/02/17/exploration-and-practice-of-query-rewriting-in-meituan-search.html` ·
**`/2025/05/22/meituan-ab-online-controlled-experiment-01.html`** · `/tags/搜索推荐.html` · `/tags/数据库.html`.
`/history.html` 에도 검색 관련은 한 편만 남아 있다.
**⚠️ 즉 299 에서 '주소까지 확인해 뒀다'고 적은 도메인 후보 넷 중 데이터베이스와 실험 백서는
본문을 못 읽는다.** **살아 있는 것은 2025년 10월 이후 글로 보인다**(추정).

**⭐ 그래서 300 은 두 편 규칙을 지킬 수 없었다.** 두 번째 편을 읽을 방법이 없어
**한 편에 기대고 그 사실을 밝히고 썼다**(도메인 `why` · `research.hard_problems` ·
`open_questions` 세 곳에 적었다). 대신 그 한 편이 8천~1만 자에 결정·대가·수치를 다 담고 있고,
**⑫ 대로 결정 단위로 다시 읽으니 결정이 8개에서 11개로 늘었다**(재독이 새 결정을 낸 **열여덟
번째**다).

**재독에서 새로 나온 것**
- **표현을 온라인에서 안 만든다** — 미리 계산해 Hive 에 두고 서빙 때 **10개 구간 버킷**으로
  넣어 **TP 지연 비용이 없다**(대가는 이산화, 학습 가능한 12차원 버킷 임베딩으로 메운다)
- **뜻밖의 결과를 그대로 적는다** — *"LoRA 在 NDCG 指标上优于全参数微调"*(LoRA 가 전체
  미세조정보다 NDCG 에서 낫다). **왜 그런지는 안 적는다.**
- **값이 가장 큰 결정은 손실 함수** — 분류에서 랭킹으로 옮기고 어려운 음성 표본을 붙이자
  **주문 쪽 이득이 클릭 쪽의 2.3배**(Click +4.85pp / Order +11.02pp)
- **끼울 자리를 네 군데 재고 숫자를 다 적는다** — 하단 +7bp / 하단+코사인 +18bp /
  출력탑 앞 +8bp / **PEPNet 게이팅 +25bp**
- **⭐ 결론에 유효 기간을 붙인다** — 중간 크기 모델이 최적이라면서 *"这一结论有阶段局限性"*
- **🔴 못 푼 것 넷을 논문 이름까지 대며 적는다** — 음성 표본 품질(Qwen3 거짓 음성 마스킹 ·
  KALM v2 focal 재가중 · Nemotron 유사도 임계 필터링) · MRL 저차원 퇴화 진단 불가 ·
  상품 전용 표현 모델 미학습 · **랭킹이 배운 것이 표현으로 돌아오지 않는다**

**다음 사이클 — 새 도메인을 연다. 읽을 수 있는 주소만 쓴다(아래는 300 에서 홈에서 직접 얻은
href 라 살아 있다).**
- **⭐ 1순위: 무엇을 잘한다고 부를지 재기** — 재료가 가장 두껍다.
  `/2026/08/07/Agent-Evaluation.html`(`Agent评测漫谈`) ·
  `/2026/07/24/LongCat-LoHoSearch.html` · `/2026/06/29/LongCat-VitaBench-2.0.html` ·
  `/2026/06/12/LongCat-WBench.html` · `/2026/07/24/LongCat-MineExplorer.html`.
  **⭐⭐ 이 축은 Snap 에서 이미 본 것이다** — Snap 의 가장 센 패턴이 *자를 어떻게 세울지가
  반복해서 문제가 되고 네 방식이 다 나온다* 였다. **비교 문서 재료가 될 수 있다.**
- **2순위: AI 로 코드를 대량 수정하기** — `/2026/05/07/Agent-AI-Coding.html`
  (`用Agent评测思路管理AI Coding：31万行代码AI重构的实践`). **31만 줄 AI 리팩터링**이고
  **Wix 의 `compile-then-publish`(코드 샘플 2,000개)와 같은 축이다** — 비교 재료로도 좋다.
- **3순위: 자체 대규모 모델** — `/2026/06/30/LongCat2.0.html` ·
  `/2026/07/12/LongCat-2.0-Open-source.html`(**국산 칩 추론**) · `/2026/04/02/LongCat-Next.html` ·
  `/2026/01/26/EvoCUA.html` · `/2026/07/28/CatPaw-LongCat.html`.
- **🔴 새 도메인을 열면 `tech` 를 함께 채우고 `index.json` 최상위 `domains` 배열에 넣는다**
  (회사 항목의 `domain` 은 블로그 주소 문자열이다 — ⑳).

**그 밖에**
- **⚠️ 부문별 매출 구성 미확인.** ⚠️ **배차·경로·지도 같은 본업 기술이 목록에 안 보인다.**
- **큐 0/3** — Meituan 을 다 판 뒤 후보 조사. **⚠️ ⑲ 를 먼저.**
- **📌 Grab 보강 후보**(이미 `done`) — Palana 2부작 · Agent platform Part 1 · Iceberg ·
  Counter Service.
- **📌 kakao·roblox 가 `in_progress`** — 각각 `hold_reason` 달린 도메인 하나 때문이고
  **새 단서 없으면 hold 유지한 채 닫는 것도 정당하다**(290).
- **⚠️ 분포** — 52곳. **한국 편중 아홉. 빈 자리는 라틴아메리카 하나뿐이고 재시도 금지.**
  **중국은 이제 ByteDance 와 Meituan 둘.**
- **⭐⭐⭐ 비교 문서가 계속 밀려 있다.** `에이전트에게 무엇을 못 하게 하는가` — 재료 일곱,
  세 갈래(**병합 제한** monday.com Morphex·Snap CodePal·Snap Casper / **접근·행동 제한**
  Wix AirBot·Wix 자가치유 두뇌 / **반대편** Wix Agent Ready Repos·Wix 컴파일러 문지기·
  Adevinta). **Wix 하나가 세 갈래에 다 걸린다.** 또는 기존 `reversible-first.md` 보강.
