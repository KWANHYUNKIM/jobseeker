...
**없다 — 예순여덟 번째 후보 조사를 돌았다. 큐 2/3.** 다음 사이클은 **신규**(Qdrant).

### 자기 실패를 수치로 적는 곳을 둘 찾았다

Qdrant 는 **자기 방식의 재현율이 무너지는 지점을 자기가 센다**(넓은 값 필터 90.8% · AND
필터 39.7%). Trigger.dev 는 **6시간 반 장애의 사슬**을 끝까지 적고 **'한 버그를 고쳤는데
우리가 하중을 받는 줄 몰랐던 보호 장치를 없앴다'** 고 쓴다. **앞의 두 회사(리스테이트·
컨벡스)가 자기 대가를 인정에서 멈춘 것과 대비되는 자리**라 눈에 띈다.

### 사고 보고서는 이 엔진이 찾던 자료다

무엇을 골랐는지가 아니라 **무엇이 실제로 터졌는지**가 적히고, 원인이 **한 결정이 아니라
사슬**로 나온다(업그레이드가 한계를 들여옴 → 다른 버그 고치다 검증을 지움 → 병합 실패가
쌓임 → 천장과 메모리). **후보 조사에서 사고 보고서 유무를 먼저 보는 것이 값을 한다.**

### 빈 축이라고 기준을 낮추지 않는다

하드웨어 축은 세레브라스 하나뿐인데 **Groq 은 본문을 둘 열고도 접었다.** 무엇을 뺐는지
**이름을 대지 않고**('캐시? 분기 예측? 스케줄러?') '단순함이 이긴다'로 논거 자리를 채운다.
**축이 비었다는 것은 통과 기준이 아니다.**

### 목록 전체로 접은 것과 첫 화면으로 접은 것은 다르다

MotherDuck 은 **블로그 전체 목록**을 훑어 튜토리얼과 AI 활용 글이 지배한다고 판정했다.
그 구분을 근거에 적어 둔다 — 나중에 뒤집을 때 무엇을 다시 봐야 하는지가 달라진다.

### 다음 사이클

**신규 — Qdrant.** 큐 2/3 이라 신규 두 번 뒤에 조사가 다시 뜬다.
