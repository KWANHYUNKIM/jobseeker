...
**Railway 완주(기능 3개).** 회사 136개, 큐 1/3 — Jane Street·Zig 이 남았다.

### 이번 기능 — `hikari-wasm-edge`

남의 CDN 을 쓰면 **우리가 고칠 수 없는 것으로 우리가 욕을 먹는다**(네트워크 문의가 전체 티켓의
약 20%, 대부분 CDN 설정과 네임서버). 더 큰 이유는 위치다 — 오리진이 우리 기계라 위경도와
고도를 아는데, 남의 CDN 을 쓰면 **그 정보를 버리고 추측하게 된다.**

### 마이크로초에서 갈린다

요청마다 WASM 게스트를 새로 띄우면 약 10μs 이고 백만 RPS 에서는 그게 곧 벽이다. 중앙 무잠금
큐로 모으려다 CPU 캐시 라인 경합을 보고 물렸고, **코어마다 스레드 로컬 풀**로 공유 자체를
없앴다. 업무 논리(캐시 판단·WAF·속도 제한)는 데이터면 밖 게스트에 둔다.

### 제목의 숫자를 자기가 먼저 깎는다

3천만 RPS 는 **DDoS 를 받아 낸 값**이고 평상시 최대는 약 100만, 이상적 조건 벤치마크는 1억
5천만이라고 본문에 적는다. 30일도 하드웨어 주문 뒤의 소프트웨어 기간이고 POP 의 40%는 아직
배송 중이다 — 이 엔진에서 자기 홍보 문구를 본문에서 되깎는 회사는 드물다.

### 엣지를 다 가져도 마지막 구간은 남의 손

BGP 가 애초에 애니캐스트를 위해 만들어진 규약이 아니라 부수 효과로 쓰는 것이라고 인정하고,
커뮤니티로 조정해도 **최적화를 거부하는 ISP**(리버티 글로벌)를 이름 대어 적는다.

### 세 기능이 한 방향을 가리킨다

랙을 세우고(1), 그 위에서 빌드를 돌리고(2), 엣지까지 자기 것으로 바꾼다(3). 손버릇도 같다 —
중간에 낀 것을 지운다: 빌더는 네 겹을 한 프로세스로, CDN 은 앤서블 배포를 제어면으로.

### 다음 사이클 — 신규 (대기 맨 위 Jane Street)

⚠️ 사업이 공개되지 않는 회사다 — 수익원 서술을 얇게 잡고 기술 도메인 위주로 판다.
