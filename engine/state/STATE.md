...
**없다 — 헨리를 완주했다(도메인 2 · 기능 2).** 큐 1/3 이라 다음 사이클은 **신규**(Classi)다.

### 조문보다 관계를 먼저 본다

법 개정 소식에서 시작하지 않고 **'우리가 어느 쪽인가'**(위탁이냐 제3자 제공이냐)에서
시작한다. 그러니 **개정이 우리 자리를 안 바꾼다**는 결론이 바로 선다 — **특례를 근거로
넓혔다가 되돌리는 일이 없다.**

### 잣대를 새로 만들지 않고 현장에서 빌린다

병원이 쓰는 **인시던트 레벨 5단계**를 QA 우선순위로 그대로 가져왔다. 자체 등급을
정의했으면 **현장과 다른 언어**가 됐을 텐데, **도메인이 이미 가진 말**을 쓰니 설명할 필요가
없다. 그러자 판정 질문이 **'무엇을 고쳤나'에서 '환자에게 어떤 영향이 나오나'** 로 바뀌고,
**처방전 서식에 텍스트 하나 더하는 변경이 선택 QA 에서 필수 QA** 로 올라갔다.

### 주관적인 것을 들이면서 객관적인 것을 버리지 않았다

환자 영향 평가는 **자동화에 저항한다**(주관적이다). 그래서 **변경 내용 채점을 남기고 옆에
뒀다** — 자동화되는 축이 있어야 **사람은 판단이 필요한 자리만** 본다.

### 판단은 정리했는데 코드가 지키는 방법이 없다

두 글 다 **'무엇이 옳은가'는 촘촘한데** 테넌트 격리·목적 태깅·감사 로그 같은 **강제 수단이
안 나온다.** 컨벡스가 **클라이언트를 DB 에서 떼어 틀릴 자리를 없앤 것**과 대비된다 —
**한쪽은 구조로, 한쪽은 계약으로** 막는다.

### 다음 사이클

**신규 — Classi.** 큐 1/3 이라 그 뒤에 조사가 다시 뜬다.
