...
**없다 — Rive 를 완주시켰다(도메인 3 · 기능 3 · `status: done`).** 회사 149개, **큐 1/3.**

### 이번 사이클이 채운 것 — `embed-one-language`

**"쓰고 싶은 언어를 가져오라"는 격리·디버깅·도구를 전부 쓰는 사람에게 떠넘기는 것이다.** 그래서
언어 하나를 골라 넣었다. 요구사항 일곱(임베디드 우선·작은 크기·결정론·점진적 타이핑·단순한
의미론·성숙도·관대한 라이선스)을 먼저 세우고 후보를 대 보니 WASM·자바스크립트·자체 언어가
떨어졌다. **취향이 아니라 조건으로 고른 기록**이라 결정을 대안별로 쪼갤 수 있었다.

### 자유를 주면 파일이 무엇이든 하는 곳이 된다

그래서 **프로토콜 다섯**(Node·Converter·Layout·Test·PathEffect)으로 묶는다. 타입 붙은 뼈대를
주는 대신 그 범주 밖은 막는다. 타입 정의는 **C++ API 한 곳에서 생성**한다 — 추론 체계를 두 벌
두면 반드시 어긋나서다.

### 회사 안에서 글끼리 어긋나는 자리를 찾았다

상태 기계 글(2025-06-26)은 **"라이브는 데이터를 가져오지 않는다"**고 하는데, 스크립팅 글
(2026-01-13)은 **데이터 가져오기를 스크립트의 능력**으로 든다. 스크립팅이 그 경계를 옮겼는지
확인하지 못해 `open_questions` 에 그대로 남겼다.

### 다음 사이클 — ⚠️ 후보 조사 (큐가 1/3 이다)

남은 대기는 **Hatena** 하나뿐이다. 판정 기준을 이어 쓴다 — 회고를 먼저, 되돌린 기록,
자기 것을 쓰지 말라고 적는 글, 사고를 공개하는 글. **형식이 아니라 대가가 기준이다**(베비를
접은 이유). `name_en` 149개를 먼저 출력해 중복부터 확인한다.
