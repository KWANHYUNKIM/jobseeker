...
**Feldera — 기능 둘을 채웠다.** `zset-incrementalization` 과 `state-layout`(두 번 들고 있을
이유가 없는 것을 찾아낸다). 회사 123개, 큐 0/3. 빈 도메인 하나 — 프로파일링.

### 증분 유지의 원가는 계산이 아니라 상태다

다시 계산하지 않기로 한 대가로 중간 상태를 계속 들고 있어야 한다. 그래서 이 회사의 최적화 글이
둘 다 **계산을 안 건드리고 상태만 줄인다.**

- **중복 색인** — 조인마다 색인을 만들다 보니 같은 컬렉션이 같은 키로 열다섯 번까지 색인됐다.
  각 연산자 안에서는 옳은 판단이라 전체를 봐야 보인다. 컴파일러가 복합 색인 하나로 합치자 갱신이
  두 배 빨라지고 메모리가 줄었다. 대가는 색인이 넓어져 조회가 조금 느려지는 것 — **증분 엔진에서
  늘 일어나는 쪽은 갱신**이라 남는 교환이다.
- **직렬화** — 널 허용 컬럼 700개 넘는 워크로드에서 메모리 40바이트짜리 행이 디스크에서 88바이트가
  됐다. 값이 커진 게 아니라 널을 표시하는 판별자가 컬럼 수만큼 붙은 것이었다. 널을 비트맵 한 비트로
  옮기고 널 많은 행은 희소 표현으로 적어 절반으로 줄였다.

### 고치는 범위를 좁게 잡는다

자료 배치를 갈아엎으면 계산 쪽까지 손대야 하므로 **보관 형식만** 고쳤다. 이득도 디스크 I/O 에
한정되지만 건드릴 곳도 거기서 끝난다. 메모리 안의 인터페이스는 그대로 둔다는 경계를 스스로 긋는다.

### Materialize 와 같은 원가를 다른 층에서 만난다

저쪽은 메모리에 든 상태의 바이트를 깎았고(레코드당 오버헤드 96 → 0~16바이트, 태그 바이트 사전
압축), 여기는 디스크로 나가는 바이트와 중복 색인을 줄인다. 증분 뷰 유지를 파는 두 회사가 같은
자리에서 만난다는 것을 `connections` 에 걸었다.

### 다음 사이클 — 확장 (마지막 빈 도메인)

프로파일러 글 둘의 주소를 QUEUE 에 적어 뒀다. 채우면 Feldera 가 done 이 되고 그다음이 후보 조사다.
