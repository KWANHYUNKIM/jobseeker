...
**없다 — Cockroach Labs 를 완주시켰다(도메인 2 · 기능 2 · `status: done`).** 회사 167개, **큐 0/3.**

### 새로 만들지 않는 것이 가장 큰 설계 결정일 때가 있다

C-SPANN 의 전부를 정하는 한 줄은 **"파티션 하나가 레인지 안의 연속된 키-값 행 집합"** 이다.
**색인을 위해 만든 것이 하나도 없다** — 분할도 재분배도 복제도 이미 있던 것을 그대로 쓴다.
그 대신 색인은 데이터베이스의 성질을 통째로 물려받는다.

### 이름 대신 조건으로 물린다

HNSW 니 IVF 니 거명하지 않고 **중앙 조정자 · 큰 인메모리 구조 · 순차 네트워크 순회 · 샤딩
불일치**로 적었다. 이름을 대면 그 알고리즘과 싸우는 글이 되고, 조건으로 적으면 **다른
회사가 자기 사정에 대 보고 판단할 수 있는 글**이 된다. 이 엔진이 옮겨 쓸 만한 방식이다.

### 같은 회사 안에서도 글마다 근거 밀도가 다르다

Pebble 글은 47% 를 내놓고 **작은 값에서는 효과가 덜하다**는 조건까지 달았는데, C-SPANN 글은
**회수율도 지연도 처리량도 없다.** 근사 검색에서 회수율 없이 '높은 정확도'라고 쓰는 것은
딥엘이 FP8 판정에 perplexity 를 낸 것과 대비된다. **회사 단위로 '자료가 두껍다'고 말하면
안 된다는 사례**를 하나 얻었다.

### 두 도메인이 위아래로 맞물린다

임베딩은 전형적인 큰 값(약 3KB)이다. 색인이 RaBitQ 로 94% 줄이는 것과 Pebble 이 256바이트
이상을 blob 으로 빼는 것이 **같은 압박의 위아래 대응**이고, 색인이 **KV 행으로** 저장되니
저장 엔진의 개선이 검색 성능에 그대로 온다.

### 심은 대비

**turbopuffer**(오브젝트 스토리지 위에 벡터 검색을 따로 판다) ↔ **Cockroach**(이미 있는
트랜잭션 데이터베이스 안에 넣는다). 앞은 검색만 파고들어 싸게 만들고, 뒤는 **두 시스템
사이의 일관성 문제를 아예 없앤다.**

### 다음 사이클 — 후보 조사 (큐가 0/3 이다)

지난 두 조사에서 확인된 방법이 있다 — **접은 곳 목록에서 근거가 "편수"나 "첫 화면 인상"인
것을 다시 연다.** 넷을 그렇게 되살렸다(Fastmail · DeepL · Proton · Cockroach).
**의료 축은 일곱 번째로 비어 있다.** 못 읽은 곳: Lunit(medium 403) · Blender(403) ·
dbt Labs · incident.io(404 네 번) · Guardian · Flexport · Cal.com.
