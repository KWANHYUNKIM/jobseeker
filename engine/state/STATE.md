...
**없다 — 1Password 를 완주했다.** 큐는 **1/3**(DoorDash). 다음 사이클은 `--gaps` 가 **신규(6순위)**를 부른다.

### 이번 사이클 — 프로비저닝 `암호화는 올바른 공개 키를 전제로 하는데, 그 키를 나눠 주는 쪽이 서버다` 와 완주

⚠️ **예고한 대로 기능보다 자료를 먼저 찾았고, 네 번째로 통했다**(Fastly 보안 · Monzo 플랫폼 · ClickHouse 엔진에 이어). **`tech` 가 0 이던 도메인이 이 회사에서 가장 값진 기능이 됐다.**

- ⚠️ **앞의 두 도메인이 못 덮은 자리를 세 번째가 드러낸다** — 금고는 안전하고 개인 키는 기기를 안 떠나는데, **남에게 무언가를 암호화해 주려면 그 사람의 공개 키가 필요하고 그것은 서버가 준다.** **서버가 거짓 키를 주면 평문을 한 번도 안 보고도 읽을 수 있는 것을 만들 수 있다**(재구성). 회사가 그 신뢰를 명시적으로 끊는다 — PKV 와 **변조가 드러나는 사슬**(Account Trust Log).
- ⚠️ **제로 지식을 지키는 비용을 누가 내는가** — 예전에는 고객이 **SCIM 브리지를 자기 환경에 배포·유지**했다(`real operational costs`). **자기 구조를 지키려고 남의 운영 부담을 늘린 셈이고, 엔클레이브가 생기고서야 되가져왔다**(설치 약 5분).
- **세 도메인이 인과로 이어진다** — 못 읽게 막았더니 처리할 방법이 없어졌고, 그 방을 실제로 쓰는 첫 제품이 프로비저닝인데 **거기서 사각지대가 드러난다.**

**완주 판정**: 도메인 3/3 · 고아 없음 · 수익원 둘 다 연결. **검증 경고도 2건에서 1건으로 줄었다**(남은 하나는 youtube 의 기존 건).

### 다음 사이클 — 신규(DoorDash)

큐 맨 위는 **DoorDash** 다 — ⚠️ **AI 에이전트 축의 다섯 번째 회사**(한 달 13만 작업 · 주당 코드 리뷰 2.5만 건). 모놀리스 회고에 **기술 선택 비교**(Python 대 Java · REST 대 gRPC · RabbitMQ 대 Kafka/Cadence)가 있고 `Reducing the Migration's Pain Points` 처럼 **아픈 곳을 제목에 건 글**도 있다.

⚠️ **비교 문서는 여전히 사다리에 막혀 있다**(8순위, 그 위의 3·6순위가 계속 채워진다). **초안 표는 이 파일 아래 `## 배운 것` 쪽 기록과 앞 사이클 STATE 에 남아 있다** — 사다리가 손질되면 바로 쓸 수 있다.
