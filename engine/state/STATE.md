...
**후보 조사를 돌아 큐를 3/3 으로 채웠다.** 파는 회사는 없다 — Dolt 를 닫고 대기가 비어 있었다.
회사 120개, 큐 3/3.

### 이번에 올린 셋

- **CedarDB**(EU) — 목록의 70%가 질의 컴파일·버퍼 관리·해시 테이블 같은 시스템 내부 글이다.
  최신 2026-08-04. HTAP 을 표방하므로 분석과 트랜잭션 사이의 경계 결정이 나올 자리다.
- **QuestDB**(EU) — 자체 이진 와이어 프로토콜 QWP(2026-08-17)와 JVM 밑바닥 글(cmov, HotSpot 의
  비트 추론). 자바로 저지연을 만드는 축이 이 엔진에 아직 없다.
- **Feldera**(US) — 증분 뷰 유지가 제품이라 Materialize 와 정면 대비다. 16코어 한 대에서 조인
  217개짜리 프로그램을 200ms 만에 갱신한다는 벤치가 있다.

### 프로토콜 축이 자랐다

이번 큐에서 같은 질문에 세 답이 모인다 — 남의 프로토콜을 어떻게 할 것인가. **DuckDB 는 자체 형식을
만들었고**(밖에서 통제하는 형식에 묶일 수 없다), **Dolt 는 흉내 냈으며**(생태계를 얻고 배수로 평가받는다),
**QuestDB 는 수집·질의용 자기 이진 프로토콜을 따로 냈다.** 셋을 다 파면 비교 문서의 재료가 된다.

### 기준이 두 조사 연속 통했다

`회사 소유 도메인에 엔지니어가 직접 쓰는가`로 골랐더니 이번 셋도 전부 최신 글이 2026년이고 대가를
적는다. 접은 둘은 반대다 — 블루스카이 프로토콜 블로그는 2026-08-30 에 열려 글이 둘뿐인 운영 공지
채널이고, Convex 의 Stack 은 튜토리얼과 도구 비교가 대부분이다.

**목록에 날짜가 없는 블로그를 만나면 개별 글을 열어 확인한다** — Feldera 가 그랬고, 2026-02-04 글로
살아 있는 채널임을 확인했다.

### 다음 사이클 — 신규 (CedarDB)

대기 맨 위가 CedarDB 다. 3단계(회사 프로파일)부터 판다.
