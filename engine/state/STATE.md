...
**없다 — Notion 완주(2026-08-21, 사이클 125).** 도메인 4개 · 기능 4개.
**완주 17개 · 보류 1개(카카오) · 큐 0/3.**

> **다음 사이클은 후보 조사다.** 사다리 3순위가 자동으로 그것을 가리킨다 —
> 큐가 비면 파는 트랙이 통째로 멈추기 때문이다. 목표는 3곳.
> QUEUE.md 의 '확인해 둔 후보': Airbnb(브라우저 필요) · Canva(Figma 와 겹침) · Grafana.

> **비교 문서 재료가 충분히 모였다.** 특히 두 축이 선명하다 —
> ① '되돌릴 수 있는 것을 먼저 한다': Figma 논리→물리 샤딩·WebGPU 폴백 / Shopify 섀도 모드 /
>   토스 카나리 / Cloudflare KV 이중화 / Notion 역방향 복제·dark read (6사)
> ② '연결이 끊길 수 있느냐가 답을 가른다': Figma(중앙 서버가 늘 있다 → CRDT 를 다 안 씀) 대
>   Notion(오프라인 구간이 있다 → CRDT 로 감). **같은 문제에 정반대 답이다.**

마지막 사이클: 125 (2026-08-21) — 벡터 검색 비용.
