...
**Val Town 완주(기능 2개).** 회사 141개, **큐 0/3 — 다음 사이클은 후보 조사다.**

### 고정은 반쪽이었다

버전을 붙들어 모듈 요청을 80% 줄이고 워커 초기화 지연 중앙값을 50% 낮췄는데, **모듈을 나르는
곳이 남의 서비스**였다. 그쪽이 URL 구조를 바꾸고 리액트 자동 버전 고정을 멈추자 사용자 약 2%가
약 16시간 영향을 받았다 — 잠금 파일이 있어도 깨진 것이다.

그 뒤 "손님의 백엔드 의존성을 전부 자기 인프라에 복제하겠다"고 적는다. 아직 계획이다.

### 편의보다 보안을 앞세운 자리를 명시한다

잠금 파일 재생성이 계정 안에서만 일어나서, 친구와 서로의 val 을 고쳐 가며 쓸 때는 다시 저장해야
한다. 불편을 글에 적고 **그래도 이 쪽을 골랐다고 밝힌다** — 남의 코드가 내 실행에 조용히
들어오지 않게.

### 알림의 값은 평소와 사고 때 다르다

잠금 파일이 안 맞으면 메일로 알리는 것이 옳은데, 사고 때는 같은 메일이 여러 통씩 가서 부담이
됐다. 오전 10시 39분에 불필요한 알림을 끄고 상류 관리자와 함께 되돌린 뒤 오후 4시께 손으로
복구했다.

### 두 기능이 한 쌍이다

프로세스로 가둬도 안에서 부르는 것이 바뀌면 소용없고, 고정해 둬도 나르는 층이 남의 것이면
흔들린다. **격리와 고정이 함께 있어야 실행이 재현된다.**
