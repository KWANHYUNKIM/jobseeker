...
**Val Town — 기능 하나를 채웠다(`untrusted-code-runtime`).** 빈 도메인 하나. 회사 141개, 큐 0/3.

### 이 엔진에서 드문 것 — feature history 를 세웠다

런타임 네 판에 시기가 붙어 있어 **3세대까지 `history` 로 남겼다**(2022-07~09 노드 vm,
2022-09~2023 vm2, 2023~2024 데노 웹 워커). 3세대의 끝만 재구성이다 — 글(2024-02)이 다음 판을
지금 구조로 소개한다는 사실로 잡았고 그 근거를 tradeoff 에 적었다.

### 언어 안에서는 언어를 가둘 수 없다

프록시로 구멍을 막을수록 새 구멍이 나왔고(vm2 는 결국 폐기됐다), 워커로 옮겨도 **워커 안에서
워커를 띄워 빠져나갔다.** 결국 운영체제 경계로 나왔다 — 임시 데노 하위 프로세스에 웹소켓
postMessage 다.

### 단위가 작으면 확실한 방법을 못 쓴다

val 하나가 실행 단위라 격리 비용이 요청마다 붙는다. 그래서 **val 마다 도커를 물렸다**(엄청나게
느리고 부담이 크다). 이 엔진의 Depot·Ubicloud 가 VM 을 쓰는 것과 갈리는 자리다.

### 호환을 버리면 제품이 아니다

QuickJS 로 가면 격리가 쉬워지지만 NPM·노드 호환과 거리가 멀다. 네 판이 전부 노드·데노 계열인
이유가 그것이다 — **바꾼 것은 격리 방식이지 런타임 계열이 아니다.**

### 다음 사이클 — 확장 (Val Town 의 마지막 빈 도메인)

`무엇을 임포트했는지 고정한다`. 잠금 파일·무결성 검사·모듈 요청 80% 감소·초기화 지연 50%
감소와, 계정 밖 협업의 불편이 손에 있다.
