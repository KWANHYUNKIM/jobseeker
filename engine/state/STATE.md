...
**Astral**(US · 파이썬 도구 체인) — 회사 프로파일까지 썼다. **도메인 둘 · 기능 0.** 큐 0/3.

### 사업 모델 칸을 못 채운 회사다

도구 셋(Ruff · uv · ty)이 **전부 오픈소스 허용적 라이선스**이고 홈에 유료 상품 설명이
없다. 대신 결말이 적혀 있다 — **2026년 3월 19일 OpenAI Codex 팀 합류 합의.** 지어내지 않고
`revenue_streams` 의 첫 줄을 **'확인되지 않음'** 으로 두고 두 번째 줄을 재구성으로 달았다.

### 남의 인터프리터에서 온 파서를 쓰는 한 남는 간극

파서 교체의 이유 셋 중 마지막이 가장 값어치 있다 — **파이썬 인터프리터와 린터는 필요가
다르고 이상적인 AST 도 다르다.** 앞의 둘(생성기의 문법 제약, 통제 불가)은 도구 문제지만
이건 **애초에 남의 목적에 맞춰진 것을 쓰고 있었다**는 인식이다.

### 다이제스트가 그대로인 공격

uv 의 ZIP 파싱 차이는 **해시로 무결성을 확인하는 통상적인 방어가 통하지 않는다.** 같은
파일이 파서마다 다르게 풀리는데 다이제스트는 유지된다. 그리고 회사가 **자기 설계(효율을
위해 중앙 디렉터리를 안 보는 스트리밍 파서)를 근본 원인으로 지목한다.**

### 고치면서 깨질 것을 먼저 셌다

엄격해진 거부 규칙을 넣기 전에 **상위 15,000개 PyPI 패키지를 검사해 무해한 인코딩 오류
셋만 걸린다**는 것을 확인하고, 그래도 **탈출구**(`UV_INSECURE_NO_ZIP_VALIDATION=1`)를 남겼다.
보안을 조이면서 생태계가 얼마나 깨지는지 재는 방식이다.

### 한 도구가 고쳐서 끝나지 않는다고 적는다

원인의 절반이 **ZIP 규격 자체의 모호함**이라, 회사도 비슷한 차이가 다른 생태계와 파서에도
있을 것이며 **업계 차원의 조율이 필요하다**고 쓴다.

### 다음 사이클

빈 도메인 둘 중 하나. 자료 두께는 비슷하다 — 파서 쪽은 물린 이유 셋과 수치가, ZIP 쪽은
사후 기록의 전 과정(신고자·조율·고침 넷·측정·탈출구)이 있다.
