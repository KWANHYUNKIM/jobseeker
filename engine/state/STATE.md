...
**없다. 298 은 후보 조사였다** — Wix 를 닫아 진행 중인 회사가 없었고 `--gaps` 가 다섯 번째로
[후보 조사]를 가리켰다. **큐 0/3 → 1/3.**

**⚠️ ⑲ 를 먼저 돌렸다** — 완료 **51곳**이고 목록을 회사 단위로 대조했다.
국가 분포는 **US 22 · KR 9 · EU 7 · JP 3 · 기타 3 · CN·CA·AU·SG·IN·AE·NG 각 1** 이다.
**중국이 ByteDance 하나뿐이고 동남아도 Grab 하나뿐이다.**

**여섯 곳을 보고 하나만 올렸다**(근거는 QUEUE 의 '확인해 둔 후보'에 전부 적었다)
- **✅ Meituan (`tech.meituan.com`, CN·커머스)** — **본문 한 편을 실제로 열어 확인했다**
  (`美团搜索3.0`, 8천~1만 자). **버린 대안 셋에 이유가 각각 있고**(세 번의 forward pass 비효율 /
  복잡한 지시문이 표현 품질을 떨어뜨림 / CPV 를 넣었더니 평가가 나빠져 빼고 *"信息质量比信息量
  更重要"* 로 정리), **한계를 목록으로 적으며**(*"四个明显短板"*, 게다가 *"这一结论有阶段局限性"*
  으로 결론의 유효 기간까지 밝힌다), **수치가 촘촘하다**(주문 +0.20% · 롱테일 NDCG@5 +2.21pp ·
  Q2I-Order-AUC +11.02pp · 질의 커버리지 81.24%→98.92%). 최신 글 2026-08-27.
- **❌ Dream11** — 1차 자료이나 **최신 2024-10**, 본문을 열어 보니 **버린 대안 0 · 대가 0**,
  수치만 있다. **셋 중 하나만 넘겨 떨어뜨렸다**(297 에서 Wix 두 편을 떨어뜨린 것과 같은 기준).
- **❌ Rakuten** — 1차 자료이나 **최신 2023-10, 대부분 2021년**. 사실상 멈췄다.
- **❌ Meesho** — `www.meesho.io/blog` **403**.
- **❌ Tokopedia / GoTo** — 공식 블로그가 **Medium 호스팅**.
- **❌ Sea / Shopee** — **1차 블로그 주소 자체가 없다.** 두 번째 실패라 `재시도 안 함` 으로 옮겼다.
- **❌ VNG** — **엔지니어링 블로그가 없다.**

**⭐ 이번 조사에서 굳어진 것** — **"1차 자료가 있다"와 "쓸 수 있다"는 다르다.** 여섯 곳 중
넷은 블로그가 있었는데도 떨어졌다(둘은 Medium, 둘은 낡거나 얇아서). **주소를 찾는 것보다
본문 한 편을 여는 것이 판정을 결정한다** — 이번에도 제목만 봤으면 Dream11 을 올렸을 것이다.

---

**다음 사이클** — 큐가 1/3 이라 사다리 3순위(큐 0)는 안 걸린다. `--gaps` 출력을 그대로 따른다.
아마 **보강** 또는 **QUEUE 신규(Meituan 착수)** 를 가리킬 것이다.

- **Meituan 을 시작한다면** 회사 프로파일부터다(PROMPT.md 3단계). `tech.meituan.com` 은
  중국어이고 WebFetch 로 목록과 본문이 다 열린다. **298 에서 읽은 검색 3.0 편은 이미 재료가
  있다** — 도메인 후보는 **검색 랭킹**이다. 다른 갈래도 목록에 보인다(Spark 벡터화 · 에이전트
  평가 · KDD 논문). **회사 하나를 완주할 때까지 갈아타지 않는다.**
...
- **⚠️ 분포** — **한국 편중 아홉.** **빈 자리는 라틴아메리카 하나뿐이고 재시도 금지.**
  동남아는 Grab 하나인데 **Shopee·Tokopedia·VNG 가 이번에 다 막혔다.**

**⭐⭐⭐ 비교 문서는 여전히 밀려 있다. 축과 재료가 다 준비됐다.**
`에이전트에게 무엇을 못 하게 하는가` — 재료 일곱, 세 갈래:
...
`jd-viewer/public/reveng/domains/<slug>.md` 에 쓰고 커밋은 `docs(reveng): 비교 문서 <slug>`.
