...
**후보 조사를 돌아 큐를 3/3 으로 채웠다.** 파는 회사는 없다 — Feldera 를 닫고 대기가 비어 있었다.
회사 123개, 큐 3/3.

### 이번에 올린 셋 — 축을 일부러 벌렸다

지난 두 조사에서 올린 여섯이 **전부 데이터베이스·데이터 인프라**였다. 기준(회사 소유 도메인에
엔지니어가 직접 쓰는 블로그)은 그대로 두고 분야만 벌렸다.

- **Bun (Oven)**(US) — 자바스크립트 런타임. `Rewriting Bun in Rust`(2026-07-08)가 이 엔진에 없던
  기록이다: 엔지니어 한 명과 병렬 Claude 인스턴스 64개로 **11일**, API 가격 기준 약 **16만 5천 달러**,
  알려진 회귀 19건, 메모리 6,745MB → 609MB, unsafe 가 코드의 약 4%. 거절한 대안도 이름을 댄다
  (Zig + 스타일 가이드 · C++ · 자체 스마트 포인터). 사람만으로 하면 셋이 1년이고 그동안 기능이
  얼어붙는다는 것이 거절 이유다.
- **Tigris Data**(US) — FoundationDB 위의 다중 리전 객체 저장소. 이 엔진의 `객체 저장소를 진실
  원천으로` 축(WarpStream·Neon·Cursor)의 **반대편**이다 — 저쪽은 쓰는 쪽이고 여기는 만드는 쪽이다.
- **Chroma**(US) — 벡터 데이터베이스(엔진에 없던 축). WAL 설계 글에서 카프카·WarpStream·CDC 를
  이름 대며 물린다. 다만 수치가 없다.

### 접은 둘과 이번 조사의 교훈

Warp 는 터미널 내부 글이 옛날 것뿐이고 최근 목록이 전부 제품 글이다. incident.io 는 엔지니어링
블로그 주소가 404 라 규칙대로 접었다.

**날짜 없는 목록이 두 번째로 나왔다.** Feldera 에 이어 Chroma 도 그렇다 — 개별 글을 열어 확인하는
절차를 그대로 썼고, Chroma 는 팔 때 날짜를 한 번 더 확인해야 한다고 큐에 적어 뒀다.

### 다음 사이클 — 신규 (Bun)

대기 맨 위가 Bun 이다. 3단계(회사 프로파일)부터 판다.
