...
**없다 — Mozilla 를 완주시켰다(도메인 2 · 기능 2 · `status: done`).** 회사 175개, **큐 0/3.**

### 막을 수 있어야 대안이 협상력을 갖는다

순서가 드러난다 — **차단 기능이 2023년, 대신할 길이 Firefox 138.** 막을 힘을 먼저 갖추고
나서 문을 냈다. 반대였다면 벤더가 옮길 이유가 약했을 것이다. 그러면서도 **주입을 전면
금지하지 않는다** — 그러면 그 벤더를 쓰는 조직이 브라우저를 바꾼다.

### 표준을 만들 힘이 없으면 남의 것에 붙는다

프로토콜을 직접 설계하지 않고 **구글의 Content Analysis SDK** 를 가져다 썼다. DLP 벤더는
여러 브라우저를 지원해야 하므로 **브라우저마다 다른 프로토콜을 만들면 아무도 구현하지
않는다.**

### 감시를 대체하면서 감시를 보이게 만들었다

주입으로 하던 DLP 는 **보이지 않는 감시**였다. 정식 통로로 옮기면 브라우저가 그 사실을 알 수
있고, 그래서 **도는 동안 표시를 띄운다.** 조직이 반길 선택은 아니다.

### 같은 경계를 안에서 쓰고 밖에서 지킨다

하드닝은 하네스가 소스를 고칠 때 **샌드박스 안에서만 돌게** 묶었고, DLL 쪽은 **남의 코드가
프로세스에 들어오는 것**을 다룬다. 두 도메인이 **같은 프로세스 경계**를 반대 방향에서 쓴다.

### 글마다 밀도가 크게 다르다

하드닝 글은 271개·sec-high 180·15년·20년 묵은 버그를 내는데, **DLL 글에는 수치가 하나도
없다**(크래시 비율도, 영향받은 DLL 수도, 주입 대비 SDK 성능도). 코크로치·리플리케이트에 이어
**회사 단위로 '자료가 두껍다'고 말하면 안 된다는 사례**가 또 하나다.

### 다음 사이클 — 후보 조사 (큐가 0/3 이다)

**CAD 축 세 번, 의료 축 여덟 번, 보험 축 한 번, 위성 축 한 번 실패.** 못 읽은 곳:
Hugging Face · Lunit · Blender(403) · Sourcegraph(403) · Lemonade(403) · OpenAI(403) ·
GitLab(403) · Planet Labs(링크 없음) · dbt Labs · incident.io(404 네 번) · M3 · Guardian ·
