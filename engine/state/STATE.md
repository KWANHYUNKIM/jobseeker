...
**없다 — 문체 보수 사이클이었다.** Rightmove 를 배민 문체로 다시 썼다(3.4 → 0.0). 회사 115개, 큐 2/3. 63곳 남았다.

### 문체 보수에서 걸러야 할 것이 하나 더 있었다 — 영문 원문

Rightmove 에는 회사 글의 영어 문장이 본문에 그대로 남아 있었다. 변환기는 백틱만 걷으므로
이건 안 잡힌다. STYLE.md 6번의 첫 줄이 한국어이고 넷째 줄이 남의 글을 옮기지 않는다인데
둘 다 어긋난 자리다.

셋을 풀었다.

- 자바 코드와 HTML 을 서로 엮어 짜는 방식(JSP 를 설명하는 회사의 말)
- 성능 병목과 운영 위험을 만든다(레거시 상태를 적은 대목)
- 우리 통제 밖에 있던 요구사항이 뒤늦게 바뀐 것(재구축의 어려움)

sources 의 원문 제목은 그대로 뒀다. 그건 옮긴 글이 아니라 가리키는 글이다.
앞으로 문체 보수에서 영문 문장도 함께 훑는다.

### 이번 사이클 — Rightmove

사업 모델에서 매출이 거래 건수가 아니라 회원 수와 단가로 움직인다는 것을 앞세웠다.
그리고 이 사업이 검색 유입 위에 서 있다는 것 — 트래픽의 85% 이상이 직접 방문이나 자연
검색에서 오므로 색인이 곧 인프라다.

견줄 자리를 둘 넣었다. 검색을 다루는 자리가 회사마다 다른데, Perplexity 는 검색 자체를
팔고 Atlassian 은 사내 검색 색인을 내부 문제로 다루는 반면 여기는 남의 검색 엔진에 잘
보이는 것이 곧 매출이다. 그리고 낡은 것을 어떻게 바꾸느냐에서 Twilio·Atlassian·Modal·Turso
와 나란히 놓았다.

기능 이름에 핵심을 붙였다. `밤새 다시 세던 것을 바뀔 때마다 고쳐 쓴다 — 색인이 곧 매출인 회사`,
`자바와 HTML 을 풀어낸다 — 기술 부채가 사람 문제로 나타날 때`.
