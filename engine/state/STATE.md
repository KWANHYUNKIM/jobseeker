...
**없다. 304 에서 Meituan(美团, 52번째)을 `done` 으로 닫았다 — 도메인 4개 · 기능 4개.**

**닫은 근거** — 남은 글을 한 편 더 **열어 보고** 판단했다(⑫ 대로 예측이 아니라 본문으로).
`LongCat-VitaBench-2.0`(2026-06-29)은 수치가 풍부한데(사용자 56명 · 과제 819개 · 동적 선호
2,000개 이상 · 도구 66개 · 사용자당 상호작용 2,093건 · 시간 폭 1,580일) **버린 대안이 없고
자기 기준의 한계도 사실상 없다.** **LoHoSearch · LongCat-2.0 발표문과 같은 계열**이고,
**남은 글 다섯 편도 전부 같은 성격**이라 더 파지 않았다.

**`business_model` 에 반복 패턴 넷을 적었다**(전부 이 사이트의 정리라고 밝혔다)
1. **⭐⭐⭐ 판단을 믿는 대신 확인할 수 있는 형태를 만든다 — 네 도메인에서 네 번.**
   의미 유사도를 **10칸 버킷 번호**로 · 애매한 기준을 **예·아니오·모름으로 이진화**(일치율
   85~90% 문턱) · **제출 전 AI 감사**를 통과해야 사람 리뷰로 · **샌드박스 실행 결과**만 학습
   데이터로. **넷의 대가가 같은 방향이다 — 확인할 수 있는 것만 다뤄지고, 그 몫이 얼마인지는
   어디에도 없다.**
2. **⭐⭐ 기준을 하나로 만드는 일이 자동화보다 먼저다.** *"1个'独裁者'好过10个'民主者'"* ·
   *"先'人人对齐'，再'人机对齐'"* · *"真正的瓶颈在人，不在工具"*. **회사가 두 일이 같은
   논리라고 스스로 적는다.**
3. **⭐ 결론에 유효 기간을 붙인다** — *"这一结论有阶段局限性"*.
4. **⚠️ 글마다 솔직함이 갈리는데 그 선이 보인다** — **'푸는 글' 은 못 한 것을 적고 '내놓는
   글' 은 적지 않는다.** 평가 방법론(한계 다섯·미해결 다섯)과 EvoCUA(버린 대안 셋·한계 넷)
   대 벤치마크·모델 발표문 셋(**자기 한계 0줄**). **같은 회사, 같은 해, 이웃한 팀인데도 그렇다.**

**⚠️ 못 채운 것** — **부문별 매출 구성 미확인** · **배차·경로·지도 같은 본업 기술이 최근
목록에 없다**(읽은 것은 전부 검색·평가·AI 코딩·모델이다) · **옛 글이 전부 404 라 데이터베이스
자율 운영 시리즈와 `可信实验白皮书` 여덟 편을 못 읽었다**(㉑).

---

**다음 사이클은 후보 조사다.** 진행 중인 회사가 없고 큐가 0/3 이라 `--gaps` 가 그렇게
가리킨다. **⚠️ ⑲ 를 반드시 먼저 돌린다** — 282 에서 이미 `done` 인 회사 파일을 덮어쓴 사고가
났다.
```
...
**후보 조사에서 볼 곳**(298 에서 여섯 곳을 보고 하나만 올렸다 — **"1차 자료가 있다"와 "쓸 수
있다"는 다르다**)
- **아직 안 본 곳** — 중국 쪽에 Meituan 말고도 1차 블로그가 있을 수 있다(Xiaohongshu ·
  Kuaishou · Alibaba 계열은 안 봤다). 일본은 JP 3곳(cyberagent · mercari · ly-corporation)
  뿐이라 여지가 있다. **US 는 22곳으로 이미 포화다.**
...
- **📌 kakao·roblox 가 `in_progress` 로 남아 있다** — 각각 `hold_reason` 달린 도메인 하나
  때문이고 **새 단서가 없으면 hold 를 유지한 채 닫는 것도 정당하다**(290). **이 둘을 닫으면
  완료 수가 맞아떨어진다.**
- **⚠️ 분포** — 52곳. **한국 편중 아홉. 빈 자리는 라틴아메리카 하나뿐이고 재시도 금지.**
  동남아는 Grab 하나(Shopee·Tokopedia·VNG 가 298 에서 다 막혔다). **중국은 이제 둘.**

**⭐⭐⭐ 비교 문서가 계속 밀려 있다. 축이 셋이고 셋 다 굵다.**
1. **`문지기를 무엇으로 세우는가` — 재료 여섯으로 가장 굵다.** Wix 컴파일러(타입 정의로
   컴파일해 통과한 것만) · Meituan AI 감사(제출 전) · Meituan 샌드박스 실행(학습 데이터) ·
   Meituan 루브릭 이진화(기계 채점의 조건) · Wix AirBot 타입 강제(Pydantic) ·
   monday.com Morphex. **⭐ 대가가 한 방향으로 모인다 — 검사할 수 있는 것만 검사된다.**
   그리고 **문지기의 성질이 갈린다** — 컴파일러는 못 잡는 범위가 분명한데 AI 문지기는
   무엇을 못 보는지가 흐릿하다.
...
3. **`자를 어떻게 세우는가`** — Snap 넷(골든셋 만들기 / 못 잰다고 적기 / 자를 만들고 결과가
   나쁘다고 적기 / 범위를 다른 편에서 밝히기) + Meituan 둘(자가 무뎌지면 새로 만든다 /
   이진화해 기계에게 맡긴다).
**또는 기존 `reversible-first.md` 보강** — Wix 두 이관 기능이 그 문서의 양극을 한 회사
