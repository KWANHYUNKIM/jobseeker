...
**Bun — 회사 프로파일까지 썼다(in_progress).** 회사 124개, 큐 2/3.

도메인 셋을 세웠고 기능은 아직 0개다.

- **스크립트 언어의 도구를 시스템 프로그래밍으로 다시 푼다** — `Behind The Scenes of Bun Install`(2025-09-10)
- **메모리 안전을 규율이 아니라 언어로 강제한다** — `Rewriting Bun in Rust`(2026-07-08)
- **남의 API 를 흉내 내 생태계를 그대로 받는다** — 제품 페이지(Node.js API 호환, V8 없이 V8 API)

### 병목을 다시 정의한 회사다

자바스크립트 도구는 대개 자바스크립트로 짜여 병목을 볼 수 있는 층이 거기서 끝난다. 이 회사는
**이제 병목은 I/O 가 아니라 시스템 콜**이라고 다시 적고 그 층까지 내려간다. 같은 설치에서 yarn 이
시스템 콜 400만 회(61.76%가 futex), npm 이 99만 회, pnpm 이 45만 회, Bun 이 16만 회(futex 0.46%)다.
이진 매니페스트 캐시로 캐시된 설치가 9밀리초(npm 363밀리초), 배열 구조체 배치로 캐시 라인 한 번에
패키지 여덟 개, 맥은 clonefile 한 번에 디렉터리 복사, 리눅스는 하드링크에서 일반 복사까지 다섯 단계
폴백.

### 파는 것이 없는 회사

유료 제품도 클라우드도 가격도 없다. 대신 팀이 **앤트로픽에 합류했다**고 제품 페이지에 적혀 있다.
이 엔진에서 인수된 회사가 둘이 됐는데 방향이 다르다 — Cursor 는 SpaceX 의 사업부가 됐고, 여기는
도구를 만들던 팀이 그 도구를 쓰는 회사로 들어갔다.

### 다음 사이클 — 확장 (Bun 의 빈 도메인)

러스트 이전 글이 이 엔진에 없던 축이다 — **엔지니어 한 명과 병렬 LLM 인스턴스 64개로 11일,
API 가격 기준 약 16만 5천 달러, 회귀 19건.** 사람만으로 하면 셋이 1년이고 그동안 기능이
얼어붙는다는 것이 그 방식을 고른 이유다. 그것을 먼저 판다.
