...
**comma.ai — 프로파일만 썼다.** 회사 139개, 큐 2/3. 도메인 셋이 다 비어 있고 셋 다 자료를 확보했다.

### 차를 안 만들고 남의 차에 얹는다

comma four 899달러(기기 반납 시 699달러)에 하네스 99달러, 큰 모델을 돌리는 연산 장치 chestnut
249달러부터. 27개 브랜드 325대 넘는 차종에서 조향·가속·제동을 한다.

그 자리가 제약을 정한다 — **라이다도 고정밀 지도도 없고**, 연산은 상자 안에서 끝나야 하며,
차종마다 다른 하네스와 신호를 받아야 한다.

### 회고가 자료의 중심이다

CTO 회고 한 편이 2017~2026년 버그 여섯의 근본 원인을 담고 있어 프로파일의 뼈대가 됐다 —
기기 온도에 따라 변하는 자이로 편향(2017), 보닛 반사를 차선으로(2019), 헤드라이트 반사를
추적한 시각 주행거리계(2020), 새 기기에서 오히려 흐려진 자동초점 보정(2021), 시뮬레이터의
피치-가속 결합이 비현실적이라 목표 속도에 못 붙은 것(2022~2025), **학습과 추론의 이미지 패딩이
달라 여러 해 반복된 회귀**(2021~2026).

2017년에는 닫힌 루프 시뮬레이션 시험도 없었고 학습 지표가 단위 없는 val loss 하나였다고 적는다.

### 라벨을 모델이 만든다

운전자 감시 모델의 라벨을 사람이 아니라 시각언어모델로 만든다 — 휴대폰을 쓰는지 예/아니오로
묻고 토큰 확률을 잰다. 검증은 운전자 이미지 1천 장과 **거치대에 물린 휴대폰 80장짜리 어려운
음성 집합**이다.

### 발열이 곧 기능의 한계다

주행 중 문턱을 75도에서 85도로 올려 30일 동안 발열로 막힌 기기가 125대에서 11대로 줄었다.
상자 안에서 다 해야 하는 제품이라 온도 몇 도가 그대로 가용성이 된다.

### 다음 사이클 — 확장 (comma.ai 의 빈 도메인)

`카메라만 보고 어디로 갈지 정한다` 부터. 회고의 버그 여섯이 다 이 도메인에 걸려 있다.
