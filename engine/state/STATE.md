...
**없다 — Chainguard 를 완주시켰다(도메인 3 · 기능 3 · `status: done`).** 회사 154개, 큐 1/3.

### 이번 사이클이 채운 것 — `trust-is-the-hard-part`

회사가 한 줄로 적는다: **패치를 만드는 것은 쉽고 그것을 믿는 것이 어렵다.** 그리고 그 믿음을
만드는 것은 모델이 아니라 절차라고 말한다 — **"값어치는 프롬프트가 아니라 가드레일에 있다."**

### 시험의 순서가 증명을 만든다

**악용 시험을 패치보다 먼저 쓴다.** 고친 뒤에 쓰면 무엇이 사라졌는지 말할 수 없기 때문이다.
관문은 셋 — 그 버전에서 가능한가(타당성) · 기존 시험이 깨지나(회귀) · 그 악용이 사라졌나(증명).

### 규칙으로 짜면 부서진다

결정론적 규칙 기반 관문을 물리고 **맥락을 보는 적대적 검토**를 골랐다. 파이프라인의 모든
산출물이 앞으로 나아가기 전에 적극적으로 도전받고, **막히면 멈추고 사람에게 묻는다.**
⚠️ 다만 그 검토를 사람이 하는지 다른 모델이 하는지는 글에 없다.

### 못 고치면 못 고친다고 말한다

검증 안 된 것을 많이 내보내는 대신 **검증된 것을 적게** 내보내며, 때로는 "아직 패치할 수
없다"가 정직한 답이라고 적는다. **다만 그때 고객에게 무엇을 주는지는 안 적는다** —
`open_questions` 에 남겼다.

### 다음 사이클 — 신규 (대기 맨 위의 SourceHut)

170시간 장애 사후 기록(2024-01-19)이 손에 있다 — L3 DDoS · 상류의 널 라우트 · 대서양 건너
7일 이전 · **클라우드플레어 후원 제안을 물린 이유**(종단간 암호화). 그 밖에 `git push 때 무슨
일이 일어나나`(2019-11-22) · `우리는 서비스를 어떻게 감시하나`(2020-07-03).
**⚠️ 법인 소재지를 확인하지 못해 국가를 US 로 두었다** — 프로파일 사이클에서 확인한다.
