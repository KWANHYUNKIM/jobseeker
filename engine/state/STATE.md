...
**Rive — 프로파일만 썼다(도메인 셋, 기능 0).** 회사 149개, 큐 1/3.

### 어디서든 안 되면 아무 데서도 못 낸다

자체 렌더러를 만든 이유가 이거다. Skia 와 HTML 캔버스에 기대던 때, **어떤 백엔드에서 렌더러를
쓸 수 없으면 그 기능을 어디에도 낼 수 없었다.** 그래서 그리는 층을 통째로 안고 오픈소스로 열었다
— 벡터 경로를 삼각형 패치로 환원해 GPU 에 이미 있는 래스터화 파이프라인을 쓴다(120fps).

### 정보를 없앤 뒤 나중에 추측할 이유가 없다

페더링 글의 문장이다. 보통은 벡터를 굽고 흐리는데, 그러면 확대해도 안 깨지는 성질이 사라진다.
그래서 **정규 분포 함수의 적분으로 덮임을 구해** 벡터 속성으로 넣었다 — 가장자리에서 정확히
50%다. 가우시안 블러는 애초에 부드러운 가장자리용이 아니라 확률 분포용이었다고 짚는다.

### "쓰고 싶은 언어를 가져오라"는 부담을 남에게 넘기는 것이다

그래서 Luau 하나를 골라 런타임에 넣었다. 물린 대안 셋을 이유와 함께 적는다 — WASM(선형 메모리라
풍부한 객체를 안전하게 노출하기 까다롭고 디자이너용 언어를 또 얹어야 한다) · 자바스크립트(엔진이
크고 결정론적 제어가 까다롭다) · 자체 언어(맞춤 언어 연구소가 될 여력이 없다). **프로토콜**
(Converter·Node·Layout·Test·PathEffect)로 스크립트를 용도에 묶는다.

### 다음 사이클 — 확장 (첫 도메인)

`그리는 층을 남에게 맡기지 않는다` 가 자료가 가장 두껍다(렌더러 + 페더링 두 글). **⚠️ 두 글
모두 성능 수치가 120fps 말고는 없다** — 없는 것을 지어내지 말고 `limits` 로 적는다.

### ⚠️ 날짜를 두 번 확인한 사이클이었다

블로그 목록 페이지가 같은 글에 다른 연도를 보였다(Luau 글이 2024/2025 로 엇갈렸다). **인용한
세 글은 각 글 페이지에서 날짜를 다시 받아 왔다.** 목록의 날짜를 그대로 믿지 않는다.
