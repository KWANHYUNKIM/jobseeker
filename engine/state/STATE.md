...
**없다 — 문체 보수 사이클이었다.** Ably 를 배민 문체로 다시 썼다(2.3 → 0.0). 회사 115개, 큐 2/3. 56곳 남았다.

### 영문 원문이 스물하나 — 회사 글이 영어면 이 값이 커진다

Temporal 25, Ably 21, Wise 14, Rightmove 3 이었다. 회사가 영어로 쓰면 결정적인 문장을
그대로 붙여 둔 흔적이 많이 남는다. 한국어 자료로 판 회사(Cygames·Trendyol)에는 거의 없다.

푸는 방식은 세 사이클째 같다. 긴 것부터 치환하고, 직역이 아니라 앞뒤와 이어지게 녹인다.
Ably 에서는 특히 조율과 지연의 맞바꿈을 말한 문장을 엄격한 메시지 순서는 조율을 요구하고
그것이 전달을 늦출 수 있다로 옮겨 도메인 이름과 붙였다.

### 값을 매기는 단위가 아키텍처를 드러낸다 — 둘이 같다

Ably 는 연결과 채널을 시간으로 판다. 100만 분당 얼마다. 그것은 동시성을 유지하는 것 자체가
원가라는 뜻이다. 이 엔진의 Temporal 도 같은 자리인데, 액션은 워크플로가 한 걸음 나아갈 때
세는 단위이고 저장이 활성과 보존으로 갈린 것은 완료된 이력을 객체 저장소로 옮기는 계층 저장과
짝을 이룬다.

둘 다 요금표를 읽으면 시스템의 모양이 보인다. 문체 보수를 돌면서 사업 모델을 다시 쓸 때마다
이 대목이 눈에 걸린다.

### 이번 사이클 — Ably

이 회사가 파는 것이 기능이 아니라 약속이라는 것을 앞세웠다. 가동률 99.999% 를 계약으로 걸고
순서 보장과 정확히 한 번 전달을 판다.

전제가 특이하다는 것도 문장으로 세웠다. 한 곳이 죽는다가 아니라 여러 곳이 동시에 죽는다에서
시작하고, 로드밸런서로 리전을 나누는 흔한 답을 버리는 이유가 로드밸런서 자신도 어느 리전엔가
있어서 못 쓰게 될 수 있다는 것이다. 장애를 막는 장치가 그 자신도 장애의 대상이다.

정확히 한 번 축에도 넣었다. Flipkart 는 적어도 한 번을 받아들이고 배치가 갚고 Klaviyo 는
실패를 성격으로 갈라 다른 저장소로 보내는데, 여기는 그것을 계약으로 팔기 때문에 값을 지연으로
내는 쪽이다.
