...
**쿡패드**(JP · 레시피 UGC 커뮤니티) — 회사 프로파일까지 썼다. **도메인 둘 · 기능 0.** 큐 0/3.

### 다국어 하나로 더 빠른 시스템을 접었다

일본판 **Voyager**(Solr·Ruby·ECS)가 **p50 이 더 빨랐는데도** 접혔다 — **'가장 결정적인
것은 다국어 대응이 Voyager 에서는 어려웠다는 점이다. GS2 는 이미 약 30개 언어를 지원한다.'**
초기 벤치마크에서 **약 4배 느린 것을 알고도** 옮겼다.

### 중앙값을 내주고 꼬리를 벌었다

최적화 뒤 **p50 은 22ms → 33ms 로 느려지고 p95 는 80ms → 50ms 로 빨라졌다.** 성능이
'좋아졌다/나빠졌다'로 안 갈리는 자리를 **두 수치로 그대로 적는다.**

### A/B 를 쓸 수 없을 때 무엇으로 확인하나

**'제품 기반을 통째로 옮기고 있어서 일부만 사용자에게 내보내 검증할 수 없었다.'** 그래서
**순위 자체를 견줬다** — RBO 로 신착순 **97.2%**, 인기순 **95.1%**, 상위 **2만 키워드**를
매일 감시. **다만 옛 시스템을 기준으로 삼았으니 더 나아진 부분은 이 방법으로 못 잰다.**

### 이관이 끝나면 견줄 방법이 다시 필요해진다

그래서 **인터리빙**인데, **무한 스크롤과 함께 쓰면 같은 문서가 두 번 나온다.** 1페이지만
하는 안도, 전체 페이지 캐시도 못 쓴다 — **첫 질의를 캐시하고 ES 를 두 번 직렬로** 무는
쪽으로 갔다.

### 오타를 하나 잡았다

프로파일 본문에 **'클라우드패드'** 라고 쓴 자리가 있어 고쳤다. 검증은 이런 것을 못 잡으니
**회사 이름은 눈으로 확인해야 한다.**

### 다음 사이클

확장 — 빈 도메인 둘 중 하나. **이관 쪽이 수치가 촘촘하다**(4배 · p50/p95 · RBO 97.2/95.1 ·
30개 언어 · 2만 키워드). 큐가 0/3 이라 그 뒤에 후보 조사가 뜬다.
