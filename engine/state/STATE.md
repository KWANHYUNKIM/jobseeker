...
**Qdrant**(EU·베를린 · 벡터 검색 엔진) — 회사 프로파일까지 썼다. **도메인 둘 · 기능 0.** 큐 1/3.

### 무너지는 지점을 먼저 내놓는다

두 도메인이 같은 모양이다. 필터가 붙으면 **자기 방식이 못 덮는 두 경우를 재현율로 센다**
(90.8% · 39.7%). RAM 을 넘으면 **질의가 4ms 에서 43ms 로 뛴다**고 적고, 재채점이 왜 느린지도
숫자로 댄다(**0.30GB 대신 2.98GB**). **앞의 두 회사(리스테이트·컨벡스)가 대가를 인정에서
멈춘 것과 정면으로 갈린다.**

### 하나를 고르지 않는 것이 답이 되기도 한다

사전·사후·제자리 필터링이 **셋 다 어디선가 깨진다.** 그래서 **질의마다 네 전략 중 하나를
고르는 계획기**를 뒀다. 다만 **무엇을 보고 고르는지는 이 글에 없다** — 갈림의 비율만 있다
(500개 중 471 대 29).

### 다이얼을 많이 주되 멈출 자리도 적는다

양자화 넷, 메모리 계층 셋, 재채점, 오버샘플링까지 전부 사용자가 고른다. **엘라스틱이
다이얼을 안 주는 쪽이라면 여기는 정반대**다. 그런데 **어디서 멈추라고도 적는다** —
**'오버샘플링 1 이후로는 후보를 더 봐도 재현율은 거의 안 오르고 디스크 읽기만 는다.'**
**'재채점이 공짜라고 여기지 마라.'**

### 있는 것으로 안 돼서 만들었다

2021년에 매칭 엔진을 만들려다 **'FAISS 같은 것들 중 기능과 확장성 요건을 만족하는 것이
하나도 없다'** 는 결론에 이르러 밑바닥부터 지었다. **리스테이트가 '그냥 포스트그레스를
써라'를 물린 것과 같은 자리**인데, 이쪽은 **살펴본 것을 이름 대어 든다.**

### 다음 사이클

확장 — 빈 도메인 둘 중 하나. **필터 쪽이 근거가 두껍다**(세 갈래의 실패 · 자기 구멍의
재현율 · 계획기). 메모리 쪽도 표가 있어 얇지 않다. 후보 글이 아홉 편 더 남았고
**엘라스틱과 직접 견주는 글**(`benchmark-elastic-diskbbq`)이 그중에 있다.
