...
**Monzo** (EU·UK · 핀테크) — 도메인 3개 · **기능 2개**. 남은 것은 데이터 하나다.

### 이번 사이클 — 카산드라 `한 대로 시험한 것이 여섯 대에서는 다른 일이 된다`

**이 회사에서 대가가 가장 진하게 적힌 자리를 팠다.**

건진 것 넷:

- ⚠️ **정족수는 대수가 아니라 다수결이라, 대수를 바꾸면 성질이 바뀐다** — 시험은 **1대**, 운영은 **6대**였다. 1대일 때는 `we'd have agreement from the other two servers` 로 문제가 덮인다. 6대를 넣으니 **파티션당 2~3벌이 빈 노드로 옮겨져 `없음` 이 다수가 됐다.** **시험이 규모를 안 바꾸면 그 시험은 다른 것을 시험한 것이다.**
- ⚠️ **설정 하나가 두 가지를 정하고 있었다** — `auto_bootstrap: false` 가 데이터 스트리밍만이 아니라 **새 노드가 활성으로 합류하는지도** 함께 정했다. **문서를 반만 읽으면 절반은 반대로 동작한다.**
- ⚠️ **담당자가 자기 작업을 배제한 순간이 가장 값지다** — 13:24 에 `All of these metrics appear normal and unaffected by the operation`. **지표가 정상인 것과 원인이 아닌 것은 다르다.** 그리고 중간에는 **모순된 증거**까지 나온다(같은 서비스에서 어떤 읽기는 성공하고 어떤 읽기는 실패).
- ⚠️ **고치러 가는 길이 같은 이유로 막혔다** — 설정 서비스가 카산드라에 걸려 있어 **내부 edge 가 404 를 내고 배포와 고객 지원이 함께 멈췄다.**

**복구를 가능하게 한 것은 데이터베이스 밖에 저장해 둔 이벤트였다** — 평소에는 비용이던 것이 유일한 복구 수단이 됐다(15:08~23:00, 8시간).

**두 기능이 되돌리기 속도로 이어졌다** — 라이브러리 이전은 **설정 60초**로 되돌리는데 이 장애는 **노드당 8~10분**이었다. **상태를 가진 것은 빨리 되돌릴 수 없다.**

⚠️ **Wise 의 압축 백로그와 같은 결론에 이른다** — 애플리케이션 증상은 보이는데 **원인 컴포넌트를 지목하는 지표가 없었고**, 둘 다 사고 뒤에 그 알림을 만든다.

**닫은 물음 하나** — 쪼개기 계획이 실행됐는지 몰랐는데, 석 달 뒤 글에서 **다중 클러스터의 흔적**을 찾았다(`monzo.com/cluster`). **어디까지 갔는지는 여전히 모른다.**

### 다음 사이클 — 데이터 도메인, 그러면 완주가 보인다

`--gaps` 가 **팀 100개가 만드는 데이터에 계약을 건다** 를 부른다. 자료는 있다(데이터 메시 글). ⚠️ **다만 그 글에는 `무엇을 잃었나` 가 약하다** — 전제(구조화된 이벤트)와 원칙(`Be opinionated`)은 적지만 버린 대안이 없다. **기능을 쓰기 전에 자료를 더 찾는 것이 이 회사에서 두 번 통했다.**

그 기능을 쓰면 **도메인 3/3 이 채워져 완주 기준을 잰다.**
