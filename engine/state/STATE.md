...
**Signal — 기능 셋.** 남은 빈 도메인은 비용 하나. 회사 133개, 큐 2/3.

### 도와줄 수 있다는 말은 우리가 열 수 있다는 뜻이다

백업의 열쇠는 기기에서 만든 64자 복구 키뿐이고 서버와 공유하지 않는다. 그래서 **잃으면 영영
못 열고 시그널도 도와줄 수 없다**고 미리 못 박는다. 편의와 원칙이 같은 손잡이에 달려 있어
한쪽을 고르면 다른 쪽이 사라지는 자리다.

### 내용을 가려도 크기가 남는다

첨부는 백업마다 다른 키로 한 번 더 암호화하고 **덧붙임을 넣어 원래 크기까지 가린다.** 모양으로
새는 것을 막는 손질이다.

### 돈을 받으면 익명성에 구멍이 생긴다

월 1.99달러 100GB 등급을 만든 순간 결제 기록이라는 새 단서가 생겼다. 그래서 아카이브를
**계정·결제와 곧바로 잇지 않게** 저장한다 — 그룹에 쓰던 영지식 방식을 그대로 가져왔다.

### 프로파일을 고쳤다 (PROMPT 2″단계)

프로파일에 "파는 것이 없다"고 적었는데 **유료 백업이 있었다.** 기부 말고 처음 생긴 수입원이라
`business_model` 과 `revenue_streams` 를 고치고 `open_questions` 에 적었다. 비중은 공개돼
있지 않다.

### 백업은 사라짐의 약속과 부딪힌다

한 번 보기와 24시간짜리 메시지는 아예 담지 않는다. 오래 남기려고 만든 기능이 사라진다는
약속을 깨지 않게 한 경계다.

### 다음 사이클 — 확장 (Signal 의 마지막 빈 도메인)

`비용이 곧 원칙이다`. 항목별 비용(인프라 1,400만·인건비 1,900만·2025년 5,000만·인원 50명)과
연락처 탐색 600대 → 10대가 손에 있다.
