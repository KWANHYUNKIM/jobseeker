...
**없다 — `in_progress` 0 · 완료 53곳.** 318 에서 **네 사이클 미룬 후보 조사를 실제로 했다.
네 곳을 열어 둘을 올렸다 — 큐 2/3.**

**✅ 올린 곳**
- **Paytm**(IN · 핀테크) — `paytm.com/blog/engineering/`. **`Achieving Seamless Data
  Migration`(2023-04-12)이 버린 대안·한계·수치를 다 적는다** — S3 Transfer Acceleration 을
  **비용 때문에** 버렸고, *"No retry mechanism"* 이라 실패 객체 재처리가 **비용 2배(추가
  6만 달러)** 라서 **실패한 것을 원래 리전에 남기거나 지우기로 했다.** 450TB · 객체 50억.
- **Freshworks**(IN · SaaS) — `freshworks.com/explore-it/`·`/explore-cx/`. **두 편이 한 주제로
  묶인다**(멀티테넌트 부하 격리). Sidekiq 편(2020-05-14)이 **버린 대안 둘을 이유와 함께**
  적고 **인프라 20% 절감**과 **데이터센터 간 24배 차이**를 수치로 준다.

**❌ 떨어뜨린 곳**
- **Zoho** — **엔지니어링 블로그가 없다.** 검색이 전부 제품·마케팅 페이지(`/people/`,
  `/enterprise/`, `catalyst.zoho.com`)를 돌려준다. **재시도 안 함에 넣었다.**
- **Ola** — `blog.olacabs.com` 은 보도자료·채용이 섞인 홍보 블로그다. 유일한 기술 글
  `Navigating India - The Journey of Ola Maps`(2024-07-07)를 열어 봤는데 **발표문 성격**이고
  **버린 대안 0 · 설계 대가 0**, 수치만 있다(주간 검색 100만 · OSM 기여 543만 건 · 픽업 정확도
  5.3% 우위). **298 의 Dream11, 316 의 Grab SOP 와 같은 판단.**
- **Trade Republic**(EU) — `engineering.traderepublic.com` 이 **Medium 호스팅**이다
  (`?gi=` 파라미터). **이미 막힌 목록에 있는 경로라 열지 않았다.**

**⭐⭐ 이번 조사의 교훈 — 두 방향 규칙이 또 맞았다.** Paytm 은 **첫 글(CDN, 2022-11-01)만
봤으면 떨어뜨렸을 회사**다. 그 글은 수치가 화려한데(500ms→100ms · 백엔드 비용 95% 절감 ·
CDN 호출 99% 이상 감소) **버린 대안도 대가도 0줄**이었다. **둘째 글에서 회사의 성격이
갈렸다.** 305 의 Cybozu(목록만 보면 떨어뜨렸을 것)와 298 의 Dream11(제목만 보면 올렸을 것)에
이어 **세 번째 사례**다. **한 편은 반드시 열고, 첫 편이 얇으면 한 편 더 연다.**

**후보 조사 누적 성적 — 열여섯 곳 중 넷**(298: 6→1, 305: 3→1, 311: 3→0, 318: 4→2).
**318 이 가장 좋았다.**

---

**다음 사이클 — 큐가 2/3 이라 사다리는 [신규]를 가리킬 것이다.**
- **⑭ `--gaps` 출력을 실제로 보고 따른다.** 2/3 이 [후보 조사]를 한 번 더 부를 수도 있고
  [신규]로 갈 수도 있다. **출력이 정한다.**
- **[신규]면 맨 위(Paytm)를 `in_progress` 로 만들고 3단계부터.** 첫 기능은 **데이터 이관**이
  가장 두껍다(버린 대안·한계 셋·수치 셋). **⚠️ CDN 편은 수치만 강하니 기능으로 쓰지 않는다.**
- **인도가 거의 소진됐다** — Zerodha(완료) · Paytm·Freshworks(큐) 를 빼면 **Zoho·Ola 는 방금
  떨어졌고**, Flipkart·Razorpay·CRED·PhonePe·Swiggy/Zomato·Meesho·Zepto·Groww·Dream11 은
  전부 막힌 목록에 있다. **다음 후보 조사는 일본(Sansan·freee·SmartHR·Money Forward)이나
  유럽(Personio)으로 간다.**
- **⚠️ 낡은 메모를 나르지 않는다**(314·317). 후보를 나를 때 파일부터 대조한다.
- **분포** — **53곳.** US 22 포화 · 한국 아홉 · 일본 넷 · EU 일곱 · 중국 둘 · 동남아 하나 ·
  **인도 하나(큐에 둘 대기)** · 아프리카 하나 · 라틴아메리카 0(재시도 금지).
  **비교 문서 31편.**
