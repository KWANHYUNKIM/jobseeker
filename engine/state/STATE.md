...
**없다 — 일흔세 번째 후보 조사를 돌았다. 큐 1/3.** 다음 사이클은 **신규**(Gunosy).

### medium 기반 블로그가 이 엔진의 사각지대다

이번에 **일곱 곳을 두드려 여섯이 막혔는데** 그중 셋(**mixi · 리디 · 여기어때**)이
**medium 에서 403** 이다. 앞서 루닛에서도 같은 벽을 만났다. **자료가 없는 것이 아니라 이
도구로 못 읽는 것** — 계속 갈라 적되, **이 벽이 반복된다는 것 자체를 기록해 둔다.**

### 하테나 계열부터 보는 것이 값을 한다

쿡패드에 이어 Gunosy 도 **연·월별 사이트맵**이 그대로 열렸다. 일본 기술 블로그를 훑을 때
**하테나 블로그인지 먼저 보는 것**이 시간을 아낀다.

### 물린 방침을 적으면 통과다 — 수치가 없어도

Gunosy 는 **시청 시간을 직접 최적화하려다 학습이 안정되지 않아** 방침을 바꿨다고 적는다.
그런데 **A/B 결과는 '큰 개선'으로만** 쓴다. **쿡패드도 같은 모양**('캐시 비대화도 지연
증가도 인정되지 않았다')이었고 같은 기준으로 통과시켰다 — **물린 것을 이름 대어 적으면
수치가 없어도 판정 기준을 넘는다**는 선을 두 번 연속 확인했다.

### 다음 사이클

**신규 — Gunosy.** 큐 1/3 이라 신규 한 번 뒤에 조사가 다시 뜬다.
