...
**Cybozu(サイボウズ, 53번째) `in_progress` — 도메인 2개 · 기능 2개.** 308 에서 **새 도메인 ②
`테넌트가 사만 개일 때 문을 여는 법` 을 열고**(`tech` 둘 채움, `index.json` 최상위 `domains`
에도 넣음) **기능 `issuer-from-domain`(주소에서 발급자를 조립한다)까지 썼다. 결정 7개.**

**⭐ 세 편을 읽고 하나만 썼다**(근거는 `open_questions` 에 적었다)
- **✅ OIDC 편**(`/entry/2026/08/06/090000`) — **버린 대안·한계·수치 셋을 다 넘겼다.**
  테넌트가 **4만 건을 넘고 동적으로 늘고 줄어서** *"各テナントのOIDC設定を管理するのは
  現実的ではありませんでした"*. 그래서 **설정을 저장하는 대신 주소(`kintone_domain`)에서
  발급자를 조립하고** Discovery 로 종점을 받는다 — **관리 대상이 4만 개에서 0개가 된다.**
  **🔴 그 값을 회사가 스스로 적는다** — 로그인 전 사용자가 **매번 주소를 손으로 넣어야** 하고
  *"一般的なSSOの「ボタン一つでログイン」という体験に比べると…スムーズではありません"*.
  **운영의 부담을 없앤 값을 사용자가 치른 셈이다.**
- **❌ 지표 수집기 샤딩 편**(`/entry/2026/08/06/113000`) — **버린 대안 둘에 이유가 각각 있고**
  (DaemonSet 샤딩은 노드 장애 때 결손, 자동 샤딩은 갱신 중 결손) **대가도 적지만**(여러
  Deployment 간 샤드 구성 일관성, Jsonnet 생성으로 완화) **결정이 네 개 남짓이고 수치가 거의
  없다.**
- **❌ Cattage 편**(307) — **버린 설계 대안이 없고 수치가 0.**
- **둘 다 도메인 ② 의 `tech` 에 기록만 남겼다.**

**⭐⭐ 이 회사에서 굳은 관찰 — 글의 두께가 크게 다르다.** 같은 시기의 글인데 OIDC 편과
쿠버네티스 이관 편은 **버린 대안·대가·수치가 다 있고**, Cattage 와 샤딩 편은 얇다. 게다가
최근 목록의 절반 넘게가 행사·채용·회고다. **그래서 이 회사는 목록이 아니라 본문으로만
판단할 수 있다** — 305 에서 목록만 봤으면 회사 자체를 떨어뜨렸을 것이다.

**다음 사이클 — 남은 묶음을 확인하거나 Cybozu 를 닫는다.**
- **안 본 묶음**(제목·주소만 확인) — **프론트엔드와 접근성**
  (`/entry/2026/08/04/170000` 대화 상자 포커스 · `/entry/2026/08/04/090000` kintone 프론트엔드
  표준 · `/entry/2026/07/29/130254` 스크롤 표 헤더 · `/entry/2026/07/29/090000` 블로그 접근성)
  · **kintone 개발**(`/entry/2026/07/24/113000` React 그래프 로직 리팩터링 ·
  `/entry/2026/07/30/090000`) · **보안**(`/entry/2026/08/05/113000` govulncheck 오탐 줄이기).
- **⚠️ 이 블로그는 얇은 글이 섞이므로 한 편 열어 보고 판단한다.** 결정이 5개 안 나오면
  **Cybozu 를 닫는다** — 도메인 2개 · 기능 2개면 닫기에 충분하다(297·304 처럼).
- **닫는다면 `business_model` 에 적을 관찰** — **⭐ 20년 넘은 제품을 아직 팔기 때문에 멈추지
  않고 계속 옮겨야 한다**(옛 가정을 지키며 새 바닥에 올리는 이야기, `fcntl` 잠금 때문에
  **가용성을 얻고 분산을 내줬다**) · **⭐ 관리 대상이 너무 많으면 저장을 없앤다**(4만 테넌트를
  주소에서 조립. **그 값을 사용자가 치른다고 스스로 적는다**) · **⭐ 문서를 믿지 않고 커널까지
  내려가 재 본다**(*"カーネルの挙動を調べました"*) · **⚠️ 글의 두께가 크게 다르다.**

**그 밖에**
- **⚠️ 확인 못 한 것** — 2026년 영업이익 전망이 **+4.1%** 인 이유(매출은 +12.7%) ·
  제품별 매출 내역 · 요금 구조 · 부문별 이익률.
- **큐 1/3** — Cybozu 를 닫으면 **후보 조사**.
...
- **⭐⭐⭐ 비교 문서 축 넷이 다 밀려 있다.**
  ...
  ④ **`무중단으로 갈아 끼우기`** — Wix 두 편(인프라 층·앱 층) + **Cybozu**(애플리케이션이
  딛는 바닥). 기존 `reversible-first.md` 와도 이어진다.
