...
**CedarDB — 도메인 셋을 다 채우고 done 으로 닫았다.** `colibri` · `encode-always-compress-sometimes` ·
`adaptive-compilation`. 회사 121개, 큐 2/3.

완주 기준 셋을 다 넘겼다 — 도메인마다 기능 1개 이상, 기능 셋이 이어져 고아가 없고, 수익원이
기능과 닿는다.

### 세 도메인이 전부 같은 문장으로 풀린다 — 고르지 않는다

- 행이냐 컬럼이냐 → 한 나무에 둘 다 매달고 나이를 온도로 삼는다.
- 인코딩이냐 압축이냐 → 질의가 닿는 곳에는 인코딩, 디스크로 나갈 때만 압축.
- 해석이냐 컴파일이냐 → 빨리 준비되는 코드로 시작해 놓고 오래 돌면 좋은 코드로 갈아탄다.

셋 다 **미리 고르지 않고 시간에 맡긴다**는 점이 같다. 데이터는 식으면 형식이 바뀌고, 질의는 오래
돌면 코드가 바뀐다. 예측 대신 관찰로 푼 구조다(재구성).

### 수치가 방식의 차이를 보인다

해석 10 GB/s 대 컴파일 65 GB/s, 컴파일 값 약 1밀리초, ASM 백엔드가 컴파일 시간의 1%로 처리량
90% 가까이. 분기 하나를 산술로 바꿔 10억 개 정수 필터가 13.17초에서 8.21초(사이클당 명령 수
1.48 → 2.98). 이 회사는 밑바닥 수치를 잘 대는데, **자기가 감수하는 것은 잘 안 적는다.**

### 이번 큐가 남긴 서술의 대비

DuckDB · Dolt · 안티테시스는 한계를 먼저 적었고 CedarDB 는 성능을 먼저 적는다. 그래서 이 회사에서는
난제 칸이 우리가 채운 관찰로 채워졌다 — 오래된 행을 자주 고칠 때의 왕복, 쓰기 경로의 값, 컴파일이
실행보다 커지는 경계. 다만 마지막 것은 회사도 한계로 명시한다.

### 다음 사이클 — 신규 (QuestDB)

큐가 2/3 이고 대기 맨 위가 QuestDB 다. 자체 이진 프로토콜 QWP 가 이번 큐의 프로토콜 축에서
세 번째 답이 된다.
