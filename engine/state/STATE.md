...
**comma.ai — 기능 하나를 채웠다(`train-reality-mismatch`).** 빈 도메인 둘. 회사 139개, 큐 2/3.

### 여섯이 사실은 한 종류였다

9년치 회고의 버그 여섯을 늘어놓으니 **거의 다 모델의 실력 문제가 아니었다.** 모델에 들어가는
재료가 학습 때와 달랐거나 현실과 달랐다 — 센서와 현실(온도에 따라 변하는 자이로 편향, 새
기기에서 흐려진 자동초점), 학습 분포와 실제 장면(보닛 반사, 헤드라이트 반사), 시뮬과 현실
(피치-가속 결합), 그리고 **학습 코드와 추론 코드**(패딩 replicate 대 constant).

### 어긋남은 오류로 나타나지 않는다

패딩이 다르거나 보정이 낡아도 프로그램은 정상으로 돈다. 주행이 조금 나빠질 뿐이라 **가장 오래
산 것은 2021년부터 2026년까지 살았다.** 고쳐도 스택이 갈려 있으면 또 갈라진다.

### 그래서 초기의 사정이 결정적이었다

2017년에는 닫힌 루프 시뮬레이션 시험이 없었고 학습 지표가 단위 없는 val loss 하나였으며 로깅
품질도 낮았다 — **어긋남이 생겨도 알 방법이 없었다는 뜻**이다. 지금은 명령과 실측을 견주는
횡방향 시험 모드까지 만든다.

### 이 엔진에서 견줄 자리

지그가 "감춰진 것을 만들지 않는다"로 층을 정리했다면, 여기는 **감춰진 어긋남을 9년에 걸쳐
찾아낸 기록**이다. 둘 다 조용한 실패를 어떻게 다루느냐의 이야기다.

### 다음 사이클 — 확장 (comma.ai 의 빈 도메인)

`운전자를 감시하는 모델을 따로 둔다`. VLM 라벨링·검증 집합 1천 장·거치된 휴대폰 80장·회전 중
경고 7% 감소가 손에 있다.
