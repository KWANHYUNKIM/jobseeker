...
**없다 — Redpanda 를 완주시켰다(도메인 2 · 기능 2 · `status: done`).** 회사 173개, **큐 0/3.**

### 새로 만들 것을 줄이는 쪽을 골랐다

Cloud Topics 의 핵심 결정이다. 데이터를 Raft 에서 빼내면서 **메타데이터는 남겼다** — 전부
빼면 트랜잭션과 멱등성을 처음부터 다시 증명해야 하는데, **위치만 Raft 에 태우면 기존 논리를
그대로 물려받는다.** 수파베이스가 'Postgres 를 포크하지 않는다'로 같은 종류의 선택을 한 것과
겹친다.

### 비용은 사라지지 않고 형태가 바뀐다

교차 AZ 네트워크 요금을 없애려고 오브젝트 스토리지로 갔더니 **S3 가 PUT 요청마다 돈을
받는다.** 그래서 여러 파티션을 묶어 올리고(0.25초 또는 4MB), 그러면 **L0 에 파티션이 섞여
읽기가 흩어지고**, 그래서 배경 Reconciler 가 L1 으로 재정리한다. **하나를 없애면 다음이
나온다.**

### 기능을 바꾸는 대신 손잡이를 늘린다

쓰기 캐싱도 Cloud Topics 도 **기존 방식을 걷어내지 않고 하나의 바이너리에 함께** 뒀다. 이
회사가 반복해서 고르는 형태이고, 그래서 스스로를 **'적응형 스트리밍 엔진'** 이라 부른다.

### 두 글이 같은 자리를 비운다

성능은 **인스턴스 종류·파티션 수·프로듀서 수·처리량까지** 갖춰 재면서, **내구성과 가용성
쪽에는 같은 정성이 없다** — 쓰기 캐싱 글은 기본 플러시 값도 복구 절차도 없고, Cloud Topics
글은 **오브젝트 스토리지가 죽으면 어떻게 되는지**가 없다. 가용성의 성질이 바뀌는 변경인데도
그렇다.

### 심은 대비

**워프스트림**(처음부터 S3 위에 프로토콜을 얹었다) ↔ **Redpanda**(Raft 로 복제하던 엔진에서
데이터만 빼냈다) — 그래서 이쪽은 **메타데이터에 Raft 가 남아** 트랜잭션 논리를 물려받는다.
**출발점이 결과의 성질까지 갈랐다.** **티커비틀**(모든 거래를 잃지 않는 전제) ↔ Redpanda
(모든 스트림이 기록 시스템은 아니라는 전제).

### 다음 사이클 — 후보 조사 (큐가 0/3 이다)

**CAD 축 세 번, 의료 축 여덟 번 실패**는 그대로다. 못 읽은 곳: Hugging Face · Lunit ·
Blender(도메인 전체 403) · Sourcegraph(도메인 전체 403) · dbt Labs · incident.io(404 네 번) ·
M3 · Guardian · Flexport · Cal.com. **이 세션의 WebSearch 예산은 끝났다.**
