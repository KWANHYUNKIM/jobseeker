...
**없다 — 이번 사이클은 후보 조사였다.** 큐 **0/3 → 2/3.** ⚠️ **목표는 3곳인데 둘에서 멈췄다.** 회사 97개.

### 이번 사이클 — 두 곳을 올리고 하나를 접었다

- **✅ Airbnb** — 새 이름. **2026년 글이 이전·마이그레이션 이야기로 줄지어 있다.** ✅ 본문 확인(`OpenTelemetry 와 vmagent 로 메트릭 파이프라인`, 2026-04) — ⚠️ **거절한 대안을 다섯 개 나열한다**(Veneur · recording rules · OTel Collector 자체 집계 · Vector · m3aggregator). **대가도 명시한다** — **고카디널리티 서비스를 델타 시간성으로 바꾸자 장애 때 이상한 스파이크 대신 눈에 보이는 데이터 공백이 생기는데, 메모리 압박을 다루려고 의도적으로 감수한 절충이다.** **초당 1억 샘플 이상** · JVM 메트릭 CPU **10% → 1% 미만** · **비용이 대략 한 자릿수 규모로 감소.** ⭐ **Razorpay 와 같은 vmagent** 를 쓴다.
- **✅ Pinterest** — 새 이름. ✅ 본문 확인(`조건부 학습 검색의 확장`, 2026-08-26) — **`충분한 암기와 견딜 만한 충돌을 위해 20GB 크기의 id 임베딩 테이블을 사전학습하게 됐다`** 를 시맨틱 ID 로 바꾸고, **조건별 모델을 통합해 `학습 파이프라인·피처 업그레이드·실험·서빙 설정·인덱싱에 걸친 중복`** 을 없앴으며, 서빙을 GPU 로 옮겨 **`7자리 비용 절감과 p90 모델 지연 85% 감소(80ms → 12ms)`** 를 얻었다.
- **⛔ Klarna — 두 번 두드리고 접었다.** 목록은 열리는데 **최신이 2025-06 이고 일반 주제가 섞였으며**, ⚠️ **두 번의 조사에서 모두 개별 글의 정확한 주소를 못 받았다.** **Medium 해시를 추측하면 안 되므로 판정할 수 없다** — 새 단서 전에는 안 꺼낸다.

### ⚠️ 이번 조사에서 3곳을 못 채웠다 — 그 사실과 이유를 적는다

**없는 축(헬스케어 · 물류 B2B · 산업/제조 · 통신 인프라)을 검색으로 찾는 시도가 두 번째로 실패했다.** ⚠️ **`회사명 + engineering blog + 키워드` 검색은 이미 유명한 엔지니어링 블로그만 걸린다** — 그 축들은 애초에 블로그가 없거나 검색에 안 잡힌다(재구성). **다음 조사에서는 다른 각도를 쓴다** — **컨퍼런스 발표 목록**(CEDEC · QCon · SREcon)이나 **이미 판 회사의 글이 언급하는 다른 회사**를 따라간다. QUEUE 에도 적었다.

### ⭐ 이번에 보인 대비 — 같은 도구, 다른 문제

**Razorpay 와 Airbnb 가 둘 다 vmagent 를 쓴다.** ⚠️ **Razorpay 는 `쓰는 사람이 없는 메트릭을 찾아 지우는` 문제였고, Airbnb 는 `StatsD 와 자체 Veneur 파이프라인이 확장 한계에 닿은` 문제였다.** ⭐ **⑩축(`관측 비용을 어디까지 줄이나`)이 사는 쪽 둘 + 파는 쪽 둘로 채워진다.**

### 다음 사이클 — 신규(6순위)

`--gaps` 가 **큐 맨 위(Airbnb)를 `in_progress` 로 만들고 3단계부터** 를 부를 것이다. ⚠️ **`medium.com/airbnb-engineering` 은 목록이 헤더만 오니 글 주소를 검색으로 받는다.**

⏳ **보강 거리 셋** — Grafana Labs 인용 대조 · Razorpay 보안 트리아지 글 · Flipkart Rate Card 엔진 글.
...
**③ `자기 성과를 어디까지 주장하나` 5곳 + ⏳ Airbnb** — Snyk / Grafana Labs / ScyllaDB / Razorpay / Flipkart.
...
**⑨ `한 번에 갈아엎을 것인가 목 졸라 죽일 것인가`** — Twilio(두 방향) / Etsy / Plaid / Paystack / ⏳ **Airbnb**(이중 파이프라인 검증과 느린 폐기).

**⑩ ⭐ `관측 비용을 어디까지 줄이나` — 사는 쪽과 파는 쪽이 다 찼다** — **Razorpay · ⏳ Airbnb**(사는 쪽, 둘 다 vmagent) / **Grafana Labs · Honeycomb**(파는 쪽).

**⑪ `깨질 걸 알면서 고른 의존을 어떻게 다루나`** — Razorpay / Plaid / Paystack / ScyllaDB.

**⑫ `빠른 숫자와 정확한 숫자를 어떻게 가르나`** — Flipkart / Zepto / Deliveroo.

**⑬ `애매할 때 어느 쪽으로 넘어지나`** — Flipkart(둘) / Razorpay / Zepto / Paystack / ⏳ **Airbnb**(스파이크 대신 데이터 공백).

**⑭ `검색 관련성을 누가 정하나`** — Etsy / Flipkart / ⏳ Zepto / ⏳ **Pinterest**.

**⑮ `언제 쪼개고 언제 합치나`** — Twilio / DoorDash / Deliveroo / ScyllaDB / ⏳ **Pinterest**(조건별 모델을 통합).
