...
**없다 — Electric 을 완주시켰다(도메인 2 · 기능 2 · `status: done`).** 회사 158개, **큐 0/3.**

### 어차피 기다리는 김에 같이 태운다

이번 기능(`fsync-once-for-many`)의 알맹이다. fsync 는 디스크가 실제로 받아들일 때까지 기다리는
일이라 요청마다 부르면 처리량이 무너진다. 그래서 **한 번의 플러시가 도는 동안 들어온 요청을
모아 두었다가 앞 묶음이 끝나면 함께 태운다** — 한 번의 기다림을 여러 쓰기가 나눠 진다.

### 같은 바이트를 여러 벌 만들지 않는다

전선 형식 그대로 담아 읽기를 **파일을 내보내는 일**로 만들고(`sendfile`·`splice` 로 커널에서),
구독자에게는 **메모리의 공유 사본 하나**로 서빙한다 — 덧붙이기 하나가 살아 있는 모든 독자를
한꺼번에 깨운다.

### 안 한 것을 이름 대어 적는다

**다중 노드 배포 · 읽기 경로의 포괄적 벤치마크 · 결함을 넣는 카오스 시험 · 스트림 메타데이터
메모리 최적화.** 수치만 내고 남은 것을 말하지 않는 글이 많은데, 이 글은 넷을 이름 대어 적는다 —
읽는 사람이 그 수치를 어디까지 믿을지 스스로 정할 수 있게 한다.

### 두 도메인이 같은 규칙 위에 있었다

위층은 shape 로그를 **불변 청크**로 나눠 CDN 이 접게 했고, 아래층은 스트림 로그를 **봉인해 객체
저장소로** 넘기고 활성 꼬리만 남긴다. 둘 다 "다 찬 것은 안 바뀐다"는 한 문장이다.

### 다음 사이클 — ⚠️ 후보 조사 (큐가 0/3 이다)

`blocked` 목록이 다음 조사의 출발점이다 — **Ghostty**(터미널 축, `/blog` 404 에 홈이 잘림) ·
**incident.io**(세 경로 404) · **Sourcegraph**(403) · **Guardian** · **Flexport**.
지난 조사에서 **Fireworks 를 그렇게 되살렸다.**
