...
**Adevinta (EU · 커머스/분류광고) — 49번째. 도메인 3개 · 기능 6개. 큐 잔량 1/3.**

**280 에서 마지막 미독 둘을 열었고 둘 다 재료가 있었다** — **"얇을 것" 이라는 예상이 여덟
번째로 틀렸다**(운영 사실 ⑫). **이제 목록의 글을 다 읽었다.**

### 280 에 쓴 것

**`fork-and-strip`(벤치마크는 좋았는데 코드가 안 맞았다, 결정 6개)** — 도메인 ① 에.
PaddleOCR 이 **벤치마크는 강한데 코드베이스가 자기 용도에 비해 지나치게 복잡**했다.
**뜯어 보고서야 보인 것들** — 추론 클래스가 **파라미터 105개를 요구하는데 70개 넘게가
무시**되고 있었고(20개로 줄였다), **언어로 `fr` 을 고르면 범용 latin 으로 매핑되고 검출은
무엇을 고르든 English** 였으며, **모델 주소가 코드와 문서에서 어긋나 일부는 아예 못
받았다.** 고친 뒤 **예측은 그대로 뒀다** — *"does not alter predictions."*
⚠️ **수치가 105→20 하나뿐이다** — 지연·처리량·정확도·기간이 하나도 없다.

**도메인 ① 의 성격이 넓어졌다** — 원래 "재본다" 였는데 이건 **재보고 나서 뜯어고친
기록**이다. `why` 에 한 줄 덧붙였다: **남의 자로 좋다고 나온 것이 자기 것이 되지는 않는다.**

**⭐ 그리고 leboncoin 글을 찾았다** — `using-rasterization-to-make-document-sharing-safer`.
문서를 공개하되 못 고치게 **이미지로 굽는다**(비밀번호는 *"super easy to circumvent"* 라
버렸다). **대가를 여섯 개 적는다** — 파일 크기 · 확대/해상도 · **접근성 전부 상실** ·
검색·복사 불가 · 하이퍼링크 불가 · 오디오·비디오 손실. **규모** — YAMS 가 **월 260억 요청 ·
초당 1만 건 이상**, 문서 변환 **약 120만**, 래스터 PDF **약 1만 4천**. **⚠️ 검토한 셋
(ImageMagick · LibreOffice+PDFtk · Poppler) 중 무엇을 골랐는지가 확인되지 않아 기능으로
쓰지 않고 도메인 ③ `tech` 에만 넣었다.**

### 다음 사이클 — Adevinta 를 닫는다

**목록의 미독이 없다.** `--gaps` 는 [신규]를 낼 것이고, **이번엔 닫는 것이 맞다.**
회사 `status` · `index.json` status · **QUEUE 진행 중 → 완료**(완료 섹션은 **불릿**,
`'\n## 완료\n'` 로 줄 경계 매치, 운영 사실 ⑥·⑦).

**⭐ 닫을 때 `business_model` 에 적을 패턴 다섯 — 근거가 다 서 있다:**
① **남이 만든 것을 그대로 받아들이지 않는다** — 도구는 **자기 백로그로 재고**(오차 원인
다섯 · 주관과 객관의 불일치를 판정 없이 남김 · **2년 전 자기 결론을 뒤집는 데이터**),
라이브러리는 **뜯어서 다시 만든다**(**105→20**, 언어 손잡이 우회). 그리고 **활동 지표를
굿하트의 법칙으로 물리친다.**
② **인프라를 고치는 대신 인프라에 대한 사실을 인프라로 승격시킨다** — 계정 생성을 예산
담당자 승인 흐름에 넣어 소유 정보가 저절로 갱신되게 한다.
③ **통일을 시도하다 막히면 포기하고 위임한다** — 공용 집합(쿼터 500) → ABAC(한계 넷) →
팀에 넘기기.
④ **값을 없애지 않고 옮기며 그렇다고 적는다** — 메모리 → API 서버(**8GB→2.7GB**).
⑤ **⚠️ 사고와 수치의 밀도가 글마다 극단적으로 다르다** — DNS 편은 **헛발 둘과 재발까지**
적고 교훈 셋이 전부 자기 실수, 정책 엔진 편에는 **OOMKill 기록**, 래스터화 편은 **대가
여섯** — 반면 거버넌스 편은 **폐기 절차도 사고도 없고**, ML 골든패스는 **한계 0줄**,
Debezium 편은 **지연 시간 0줄**, OCR 편은 **성능 수치 0줄**, data mesh 편은 **수치 0**.

**⚠️ 닫으면서 적을 것** — **매출 실액 확인 불가**(2024-06-05 상장폐지, **못 찾은 것이 아니라
공개되지 않는다**) · **장터 안쪽 글이 둘뿐**(Kleinanzeigen 의 Debezium, leboncoin 의
래스터화. 나머지 아홉 편은 전부 플랫폼·도구다).

### 큐와 다음 회사

**큐 잔량 1/3.** `--gaps` 의 안내 — **이 회사를 닫은 뒤 후보 조사 사이클을 한 번 끼운다.**
...
**📌 Roblox 단서** — `about.roblox.com/newsroom/<연>/<월>/<슬러그>` 가 최종 주소.
...
- **⭐⭐ `무엇을 좋다고 부를 것인가` / `재 보고 나서`** — Snap 다섯 + **Adevinta 가 축을
  완성한다.** ⭐ **OCR 편이 재료를 더한다** — **벤치마크에서 좋은 것과 운영에 올릴 수 있는
  것은 다르다.**
- **⭐ `기계가 그렇다는데`** — Snap 둘 + **Adevinta 하나**(유일하게 회의적이고 사는 쪽).
- **⭐ 새 축 후보 `틀린 가설을 어떻게 버리는가`** — DNS 편이 강한 첫 재료(헛발 둘 · 증상만
  눌러 재발 · *"discard one hypothesis before starting a new one"*). Careem 의 **Gzip 착시**,
  Doximity 의 **넓이 대신 반복 횟수로 읽기**가 같은 축.
- **⭐ 새 축 후보 `만드는 이야기만 있고 치우는 이야기가 없다`** — Adevinta 계정 폐기 부재가
  첫 재료, **같은 회사의 DNS·정책 엔진 편이 반례**.
- **⭐ 새 축 후보 `아낀 값은 어디로 갔는가`** — Adevinta 메모리→API 서버 · Careem 지운 것을
  치우는 비용 · Zerodha 색인 대신 압축 · Doximity 미룬 병합.
- **⭐ 새 축 후보 `가장 느린 곳이 남의 것일 때`** — Adevinta 의 외부 서비스 초당 3천 건.
- **⭐ 새 축 후보 `설정이 실제 동작을 가릴 때`** — OCR 편의 **105개 중 70개가 무시** ·
  **언어를 골라도 다른 모델이 돈다.** 이 사이트의 다른 회사에서 같은 자리를 찾아볼 값이 있다.
