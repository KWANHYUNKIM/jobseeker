...
**Framework — 프로파일만 썼다(도메인 하나, 기능 0).** 회사 160개, **큐 0/3**.

### 제품의 약속과 부품의 물리가 부딪힌다

이 회사의 약속은 **원하는 만큼 오래 쓰게 한다**는 것이고 그러려면 부품을 바꿔 끼울 수 있어야
한다. 그런데 요즘 저전력 메모리는 **낮은 전압과 큰 전력 효율을 겨냥한 구조라 신호가 가까이
닿도록 프로세서 옆에 납땜돼야 한다** — 그 순간 업그레이드가 불가능해진다.

### 거절한 값을 수치로 적는다

LPCAMM2 로 납땜을 피하는 대신 무는 것을 그대로 적는다 — 모듈은 LPDDR5x-8533 까지 가능한데
플랫폼에서 **7467 MT/s 로 제한**되고, 용량도 **최대 64GB** 라 DDR5 두 모듈의 128GB 에 못
미친다(플랫폼은 96GB 를 지원하는데도).

### 따라온 이득도 적는다

LPCAMM2 가 DDR5 보다 전자기 간섭이 적어 **메모리 슬롯 위아래의 EMI 흡수 마일라 부품을
없앴고, 그 덕에 와이파이 성능이 좋아졌다.** 계산해서 얻은 것이라기보다 옮기고 나서 따라온
쪽에 가깝다(재구성).

### 한 번 팔고 끝나는 대신 부품을 계속 판다

마켓플레이스에서 메인보드·확장 카드·메모리·키보드·공구를 따로 판다. **사업 모델과 설계 원칙이
같은 문장**이다(재구성).

### 도메인을 하나만 세웠다

확장 카드나 메인보드 설계를 다룬 기술 글을 못 찾았다 — 먹스·파이어웍스에서 한 판단을 이어 쓴다.

### 다음 사이클 — 확장 (그 하나뿐인 도메인)

`납땜을 거절하고 그 값을 적는다`. LPCAMM2 글 하나에 근본 제약·물린 값·따라온 이득·플랫폼
이력이 다 있다.

### ⚠️ 큐가 0/3 이다

Framework 를 완주시킨 뒤 **후보 조사 사이클을 끼운다.** 음성·오디오 축이 비어 있고
**Deepgram** 을 다음 목표로 적어 뒀다.
