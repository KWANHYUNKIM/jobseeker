...
**없다 — 문체 보수 사이클이었다.** 산산을 배민 문체로 다시 썼다(1.48 → 0.0, 기호 377개). 회사 115개, 큐 2/3. 17곳 남았다.

### 🔴 가 다섯 번째다 — 273개

사이보우즈(64) · 하이퍼커넥트(96) · SmartHR(158) · 컬리(165)에 이어 여기서 가장 많았다.
일본어 원문 인용 79종도 우리말로 풀었다(글 제목은 그대로).

### 이 엔진의 되돌리기가 전부 화면 안에서 끝났다는 것을 여기서 알았다

- 트레인라인은 전환 전에 CNAME TTL 을 5분으로 낮춰 되돌릴 길을 깔았다.
- Wiz 는 만든 것을 걷어냈고, 볼트는 8년 만에 옛 패턴으로 돌아왔다.
- **산산에서는 안 그렇다** — 시스템의 상태만 되돌려도 실제 원본은 이미 다른 곳에 있을지 모른다.

그래서 현장 제약이 그대로 설계 조건이 된다. 상자를 나르는 것이 일상이라 PC 를 든 채 작업하는 것이
현실적이지 않고, 어떤 날은 평소의 10배 넘는 봉투가 왔다가 다음 날은 10분의 1 이하가 된다.

### 옮기는 것보다 붙어 있던 것이 어렵다

DoorDash 가 가장 어려웠던 일로 코드가 아니라 기능을 옮기는 것을 꼽은 것과 같은 자리다.
산산은 저장소를 옮기며 같은 것을 만났다 — **모노레포로 옮기는 것보다 미묘하게 통일되지 않았던
CI/CD 잡을 옮기는 데 손이 더 갔다.**

### 넓은 도구를 좁혀서 이긴 둘, 그리고 만든 것을 안 쓴다고 적는 회사

Fastly 는 AutoML 부터 LLM 까지 다 해 보고 회귀 하나로 돌아왔고, 산산은 범용 OCR 대신 명함이라는
좁은 입력에만 통하는 궁리를 넣었다. **둘 다 일반적인 것이 낫다는 통념을 자기 문제로 반박한다.**

그리고 드문 문장이 하나 더 있다 — **만든 것을 안 쓴다고 적는다.** 초해상은 정확도를 30% 이상
올리는데도 지금 흐름에서 빠져 있다. 기술이 나빠서가 아니라 입력이 좋아져서다.

### 한 번에 못 옮기는 것을 나누는 답이 또 늘었다

Etsy(하나씩 가려다 묶인 것 발견) · Monzo(한 팀이 전부) · DoorDash(신규 개발 중단)에 이어,
산산은 **아래층부터 위층으로 순서를 정한다.** 그리고 Monzo 가 미완성 이전을 두 벌을 영원히 함께
운영하는 일이라 적은 것을 여기서는 전제로 받아들이고 시작한다.

### 다음 사이클 — 문체 보수 (사다리 3순위)

--gaps 가 정한다. 지금 맨 위는 kraken-tech(1.44)지만 그건 이 시점의 예상이지 확정이 아니다.
