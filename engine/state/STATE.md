...
**Gunosy**(JP · 뉴스 앱·광고) — 회사 프로파일까지 썼다. **도메인 둘 · 기능 0.** 큐 0/3.

### 수익 구조가 기반 선택의 근거로 그대로 나온다

**광고 로그가 과금과 직결**되어 유실이 곧 매출 손실이다. 그래서 fluentd 를 **StatefulSet**
으로 두고 **EBS 를 중간 버퍼**로 쓰는데, 바로 그 요구가 **ECS 대신 쿠버네티스를 고른
첫 번째 이유**가 된다 — **'여러 대로 스케일 아웃할 때의 운용 부하에 큰 차이가 있습니다.'**
ECS 였다면 3대 → 10대에 **EBS 7개를 손으로** 만들어야 했다.

### 남의 것을 깎아내리지 않고 고른다

**'무상태 구성이나 단순한 운용이 요구되는 환경이라면 ECS 는 매우 뛰어난 선택지'** 라고
먼저 적고 끝에도 다시 적는다. **컨벡스가 파이어베이스·수파베이스의 행 단위 보안을
'이유가 있어서 존재한다'고 한 것과 같은 태도**다.

### 목적 함수를 바꾸는 대신 표현을 세밀하게

**시청 시간을 직접 최적화하려다** 학습이 안정되지 않아 접고, **필드별 인코더와 가중치
학습**으로 갔다. **접은 길을 이름 대어 적는 것**이 이 회사 글의 값인데, **결과는 '큰
개선'으로만** 적는다.

### 다음 사이클

확장 — 빈 도메인 둘 중 하나. **쿠버네티스 쪽이 근거가 넷으로 갈려 두껍다**(StatefulSet ·
Argo Rollouts · 네임스페이스 · PDB). 추천 쪽은 **물린 길은 분명한데 수치가 없다.**
