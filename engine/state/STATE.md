...
**없다 — 예순일곱 번째 후보 조사를 돌았다. 큐 2/3.** 다음 사이클은 **신규**(Restate).

### 릴리스 공지를 열어 보는 값이 이번엔 안 나왔다

Tor 목록은 릴리스 공지 지배였지만 **Arti 는 C 를 러스트로 다시 쓴 것**이라 그 안에 결정이
있을 법했다. 열었더니 **왜 존재하는지도, C tor 를 언제 접는지도, 옮기는 비용도 없었다.**
기준은 그대로 둔다 — **열어 보는 것이 옳고, 열어 보고 접는 것도 옳다.** 접은 근거가
'릴리스 공지라서'가 아니라 **'본문에 결정이 없어서'** 가 되어야 뒤집히지 않는다.

### 한계 고지와 결정의 대가는 다르다

썬더버드가 못 하는 것을 잘 밝힌다(달력·주소록 미지원 · 프로토콜 간 계정 이전 불가).
그런데 **왜 그 구조인지, 무엇을 물렸는지가 없다.** '아직 안 됩니다'는 대가가 아니라
**현황**이다. 이 둘을 가르지 않으면 제품 공지가 다 통과한다.

### 경계선에서 기준을 넓히지 않는다

Xata 는 어려움을 인정하고**('expand-contract 는 강력하지만 까다롭다')** 물린 것도 하나
댄다. 거기서 멈춘다 — 지연·잠금 시간·실패율이 없다. **인정과 계량은 다르다.**

### 맞은편을 노려 두 곳을 얻었다

**Restate ↔ 템포럴**(지속 실행 엔진을 밑바닥부터 자기 로그로 지었다 · **워프스트림을 이름
대어** 객체 저장소만으로는 지연이 안 된다고 적는다). **Convex ↔ 엘라스틱·타이거비틀**
(안 주는 것을 안 준다고 적는다 — **'싼 척하는 원시 연산은 없다'**).

### 다음 사이클

**신규 — Restate.** 큐 2/3 이라 신규 두 번 뒤에 조사가 다시 뜬다.
