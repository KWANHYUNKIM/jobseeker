...
**Mux — 프로파일만 썼다(도메인 하나, 기능 0).** 회사 153개, **큐 0/3**.

### 한 선택이 품질과 값을 동시에 움직인다

렌디션 사다리(같은 영상의 화질 층들)를 무엇으로 구성하느냐가 시청 경험·전달 바이트·CDN 캐시
적중·고객 청구서를 한꺼번에 정한다. 원래 사다리는 480p 에서 1080p 로 뛰는 구간이 비어 있었고,
동시에 **AI 에이전트가 추론에 필요한 최소한의 픽셀만 원하는** 새 쓰임이 생겼다.

### 감으로 정하지 않으려고 도구를 만들었다

인코딩 사다리 결정 도구로 셋을 함께 봤다 — 캐시 적중 변화, 한 달치 CDN·저장·연산 로그의 값
영향, 자기 제품(Mux Data)이 재는 품질 점수와 실제 비트레이트. **270p 를 480p 로 합쳐도 이득은
조건에 따라 10% 절감에서 15% 증가까지, 사실상 본전**이었다.

### 6.5% 더 보내는데 고객은 덜 낸다

720p 를 더한 뒤 시청이 옮겨 갔다 — **720p +43.4%포인트로 전달 분수의 절반**, 480p -32.8%포인트,
1080p -10.8%포인트, 270p 는 +0.2%포인트로 그대로. 분당 바이트는 6.5% 늘었는데(제목별 인코딩이
-4.8%포인트 상쇄) **고객 청구서는 줄었다** — 양쪽이 더 싼 전달 등급인 720p 로 모여서다.

### 도메인을 하나만 세웠다

Mux Data·Player·Robots 의 내부를 다룬 글을 못 찾았다. **자료 없이 도메인 이름만 세우면 빈
도메인이 영영 남으므로** 하나만 뒀다 — 리니어에서 도메인을 둘로 둔 것과 같은 판단이다.

### 다음 사이클 — 확장 (그 하나뿐인 도메인)

`사다리를 감이 아니라 데이터로 고른다`. 물린 대안(270p 를 720p 로 맞바꾸기 — 값을 두 번 문다)과
맞바꿈 셋(단순함 대 포괄성 · 값 대 품질 · 캐시 효율 대 사용자 경험)이 그대로 재료다.

### ⚠️ 큐가 0/3 이다

Mux 를 완주시킨 뒤 **후보 조사 사이클을 끼운다.**
