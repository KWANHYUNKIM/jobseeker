...
**Polar Signals — 기능 하나를 채웠다(`eh-frame-unwinding`).** 빈 도메인 둘. 회사 132개, 큐 0/3.

### 남이 빌드해 둔 것 그대로에서 되짚어야 한다

프로파일은 표본을 뜬 순간의 호출 스택을 알아야 성립하는데, 손님에게 다시 빌드해 달라고 할
수는 없다. 그래서 **프레임 포인터를 믿지 않고** 바이너리에 이미 실려 있는 `.eh_frame` 안내를
읽는다 — 많은 컴파일러가 `-fomit-frame-pointer` 로 그 실을 끊어 놓기 때문이다.

### 두 길을 다 유지하지 않는다

프레임 포인터가 있는 바이너리에서도 그 싼 길을 안 쓴다. **따라가려는 시도조차 하지 않는다** —
두 벌을 다 맞게 유지하는 값을 안 내는 쪽을 골랐다. 이 엔진에서 "빠른 길이 있어도 하나만
남긴다"는 결정은 드물다.

### 검증기가 코드를 두 동강 낸다

되짚기는 커널 안(eBPF C)에서 그 자리에 해야 복사가 없는데, DWARF 규칙 해석은 검증기가 받아
주지 않는다. 그래서 해석·조율·네트워크는 사용자 공간 Go 로 갈라 뒀다 — **어디까지 커널에
넣을 수 있느냐가 코드 구조를 정한 자리**다.

### 미룬 것이지 푼 것이 아니다

JVM·V8·파이썬의 스택은 표현부터 달라 하나의 되짚기로 못 덮는다. 글은 이걸 범위 밖으로 빼고
다음 편으로 넘긴다 — 그 미룸이 이 회사의 남은 숙제이고, 안 읽은 연작이 거기 있다.

### 다음 사이클 — 확장 (Polar Signals 의 빈 도메인)

`언제 찍을지 커널에게 맡긴다`. perf 이벤트 · 초당 19번 · 검증기 제약이 이미 손에 있다.
