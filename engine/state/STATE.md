...
**Zepto** (IN · 커머스 — 퀵커머스) — 도메인 3개 · **기능 1개**(`inventory-changes-so-nothing-caches`). 남은 둘은 **본문 미독**이다.

### 이번 사이클 — `재고가 계속 바뀌므로 결과를 통째로 캐시할 수 없다`

건진 것 넷:

- ⚠️ **검색에서 가장 큰 이득(결과 페이지 통째 캐싱)이 막혀 있다** — **재고 상태·개인화·세션 신호·실험 배정**이 겹쳐 **한 요청당 유효한 변형이 너무 많다.** **그래서 캐시 대신 단계마다 미리 계산할 수 있는 것을 찾아 200ms 예산을 맞춘다**(재구성).
- ⚠️ **빠른 곳과 정확한 곳을 나눴다** — **색인은 재고·가격을 싣되 초~분 뒤처지고**, 마지막에 **Product Assortment Service 가 권위 있는 확인**을 한 번 건다. **그대로 믿으면 품절을 보여 주고 매번 물으면 예산이 무너진다**(재구성).
- ⚠️ **검색이 사용자를 모른다** — **`search-platform 은 순수 검색 서비스다. 사용자에 대해 아무것도 모른다.`** 검색과 랭킹을 따로 진화시키는 값을 얻고, **개인화가 후보를 좁히지 못하는 대가**를 치른다(재구성).
- ⚠️ **부익부를 이름 붙여 막았다** — **순수 exploit 검색은 `rich-get-richer` 동학을 만들어 신상품을 굶긴다.** 그래서 **explore 를 부가 기능이 아니라 검색 단계의 기본 흐름**으로 넣었다.

**그리고 버린 것들이 분명하다** — **편집 거리 교정기**(구어는 철자가 아니라 발음으로 흔들린다 → LLM + RAG 그라운딩) · **정적 의도 분류기**(카테고리가 개편될 때마다 재학습해야 한다 → KNN 벡터) · **허브별 색인**(1,139개만큼 복제된다 → city-pvid 중첩).

### ⚠️ 회사가 스스로 밝힌 미완성

**롱테일 질의의 완전한 실시간 이해는 아직 안 됐고**(지금은 가장 가까운 상위 질의의 이해를 빌려 쓴다) **후처리 규칙 계층은 여전히 운영자 설정에 기댄다.** 회사가 이 구조를 **`진행 중인 진화의 한 지점`** 이라 부른다. **미완성을 미완성이라 적는 글이 이 엔진에 가장 좋은 자료다**(재구성).

### 다음 사이클 — 같은 회사의 남은 도메인 둘

`--gaps` 가 **`10분을 지키려고 사람과 경로를 배치한다`** 를 부를 것이다. ⏳ **`r.jina.ai` + WebFetch(verbatim 인용 지정)로 읽는다** — 목록에서 제목만 확인한 상태라 **URL 을 먼저 받아야 한다**(슬러그 추측 금지):
- `Route IQ: How We Reconstruct Delivery Paths from Noisy GPS at Scale` (2026-08-24)
- `Right People, Right Shift, Right Time: Optimizing Workforce Planning` (2026-09)
- 그 다음: `A Billion Events a Day: Ads Analytics` (2026-07-27)

⚠️ **큐가 1/3 이다**(Snyk 만 남았다). 이 회사를 다 판 뒤 **후보 조사**가 걸린다.
...
**④ ⭐ 새로 열렸다 — `인도 규모에서 무엇이 달라지나` 2곳** — **Meesho**(주소가 없어 배달 로그로 위치를 배우고, 저사양 기기에 맞춰 이미지를 8배 줄인다) / **Zepto**(구어 질의를 LLM 으로 교정하고, 재고가 계속 바뀌어 캐시를 버린다). ⚠️ **둘 다 `서구의 기본 가정이 성립하지 않는다` 에서 출발하는데 막힌 지점이 다르다.**
