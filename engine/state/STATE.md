...
**Adevinta (EU · 커머스/분류광고) — 49번째. 도메인 3개 · 기능 5개. 큐 잔량 1/3.**

**279 에서 미독 둘을 열었고 한 편이 아주 좋았다.** `--gaps` 는 [신규]를 냈지만 **회사를
갈아타지 않는다는 규칙이 위**라 계속 팠다. **"닫는 쪽이 유력하다" 던 예상이 또 틀렸다**
(운영 사실 ⑫ 의 **일곱 번째** 확인).

### 🔴 279 의 수확 — 이 회사에서 가장 정직한 글

`its-not-always-dns-unless-it-is` 로 기능 `dns-hunt`(틀린 가설을 둘 지나고서야, **결정 7개**)
를 썼다. **이 글은 틀린 과정을 순서대로 적는다** — 인그레스 스케일링(헛발 ①), 네트워킹
(헛발 ②), 그리고 **동시 질의 한계를 150→1500 으로 올려 *"instantly"* 좋아졌다가 일주일 만에
재발한 것**까지. 진짜 원인은 **ndots** 였다 — 점이 다섯 미만인 이름에 검색 도메인이 붙어
없는 주소를 계속 물었고, *"This problem did affect everyone"* 인데 **밀도 높고 CronJob 도는
한 클러스터에서만 임계를 넘어** 자정에 몰렸다. 결과 **초당 4000건 초과 → 1000건 미만**,
SLI **99.9% 이상 회복**.

**🔴 교훈 셋이 전부 자기 실수다** — **SLI 저하를 이미 보고도 넘겼고**(*"we did not see any
issues"*), **여러 온콜이 조율 없이 병렬로 파다 이미 버린 가정 위에서 일했으며**(원칙:
*"discard one hypothesis before starting a new one"*), **필요한 정보가 지표 체계에 아예
없었다**(DNStop 을 돌려서야 보였다). **원인은 DNS 였는데 배운 것은 조사하는 법이다.**

**⭐ 그리고 이 회사의 성격이 더 뚜렷해졌다** — **사고를 적는 글**(정책 엔진의 OOMKill, DNS
편)과 **안 적는 글**(거버넌스 편은 계정 732개까지의 과정만, ML 골든패스는 한계가 0줄)이
같은 블로그 안에 있다.

### ML 골든패스는 기능으로 안 썼다 — 기준 미달

`adevintas-machine-learning-golden-path` 는 여러 나라 팀의 ML 도구를 다시 만든 기록이고
**검토한 대안 셋**(기존 최소 템플릿 · **leboncoin 구조** · **Google Vertex AI 템플릿**)과
*"One of these discussions took 16 hours over the course of 3 days"* 가 있지만, **한계를 한
줄도 안 적고 채택률·사용자 수도 없다.** **대가 칸이 비면 안 쓴다는 규칙대로 도메인 ② 의
`tech` 에만 넣었다.**

### 다음 사이클 — 이제는 정말 닫는 쪽이다

**남은 미독은 둘뿐이다** — `deep-dive-in-paddleocr-inference` ·
`using-rasterization-to-make-document-sharing-safer`. **⚠️ 다만 279 를 포함해 일곱 번 다
"얇을 것" 이라는 예상이 틀렸으므로 열어는 본다.** 재료가 서면 쓰고, 안 서면 **Adevinta 를
`done` 으로 닫는다**(회사 `status` · `index.json` status · **QUEUE 진행 중 → 완료**, 완료
섹션은 **불릿**, `'\n## 완료\n'` 로 줄 경계 매치).
⚠️ `from-lakehouse-architecture-to-data-mesh` 는 **수치 0 으로 확인**(273), 다시 안 연다.

**⭐ 닫을 때 `business_model` 에 적을 패턴 다섯:**
① **남의 것을 재고 재는 방법의 한계를 함께 적는다**(오차 원인 다섯 · 주관과 객관의 불일치를
판정 없이 남김 · **2년 전 자기 결론을 뒤집는 데이터** · **활동 지표를 굿하트의 법칙으로
물리침**).
② **인프라를 고치는 대신 인프라에 대한 사실을 인프라로 승격시킨다**(계정 생성을 승인 흐름에,
*"having proper (meta)data is key to properly governing the cloud"*).
③ **통일을 시도하다 막히면 포기하고 위임한다**(공용 집합 쿼터 500 → ABAC 한계 넷 → 팀에).
④ **값을 없애지 않고 옮기며 그렇다고 적는다**(메모리 → API 서버, **8GB→2.7GB**).
⑤ **⚠️ 사고와 수치의 밀도가 글마다 극단적으로 다르다** — **DNS 편과 정책 엔진 편은 실패를
순서대로 적는데**, 거버넌스 편은 **계정 폐기 절차도 사고도 없고**, ML 골든패스는 **한계가
0줄**이며, Debezium 편은 **지연 시간이 한 줄도 없고**, data mesh 편은 **수치가 0** 이다.

### 이 회사에서 못 채운 것

- **매출 실액 확인 불가** — **2024-06-05 상장폐지로 공개되지 않는다**(못 찾은 것이 아니다).
- **⚠️ 장터 안쪽 글이 하나뿐** — 읽은 열 편 중 Kleinanzeigen 의 Debezium 편 하나.
  **leboncoin·Marktplaats·Subito 의 검색·추천·정산 이야기가 없다**(다만 ML 골든패스 편에
  **leboncoin·Marktplaats 의 작업 방식**이 언급은 된다).
...
- **⭐⭐ `무엇을 좋다고 부를 것인가` / `재 보고 나서`** — Snap 다섯 + **Adevinta 가 축을
  완성한다.** ⭐ **DNS 편이 재료를 더한다** — **이미 저하를 보이던 SLI 를 넘긴 기록.**
- **⭐ `기계가 그렇다는데`** — Snap 둘 + **Adevinta 하나**(유일하게 회의적이고 사는 쪽).
- **⭐ 새 축 후보 `틀린 가설을 어떻게 버리는가`** — **DNS 편이 강한 첫 재료다**(헛발 둘 ·
  증상만 눌러 재발 · *"discard one hypothesis before starting a new one"*). Careem 의
  **Gzip 착시**, Doximity 의 **넓이 대신 반복 횟수로 읽기**가 같은 축에 선다.
