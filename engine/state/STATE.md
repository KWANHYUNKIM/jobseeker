...
**없다 — 문체 보수 사이클이었다.** Etsy 를 배민 문체로 다시 썼다(2.33 → 0.0). 회사 115개, 큐 2/3. 37곳 남았다.

### 갈아 끼우기 다섯 번째 — 데이터를 아예 안 옮긴다

첫 기능이 이전 축의 새 자리를 낸다. Etsy 는 성공의 정의를 Paystack 과 똑같이 골랐다(끝난 뒤에
아무도 눈치채지 못하는 것). 방법은 반대편이다.

- Atlassian 은 무중단을 지키려고 일부러 느리게 옮겼다.
- Honeycomb 은 무중단을 포기하고 데이터를 지켰다.
- 볼(bol)은 과거 4년 치를 함께 옮겼다.
- Etsy 는 **데이터를 안 옮기고 그 위의 접근 계층만 바꾼다**(Vitess).

**그리고 그 전략이 어디서 꺾였는지를 적은 것이 이 기능의 값이다** — 테이블을 하나씩 올리며 위험을
줄이려 했는데 27개 모델(테이블의 3%)이 트래픽의 1/3 을 차지하며 전부 트랜잭션으로 묶여 있어
함께 옮길 수밖에 없었다. 나누는 전략은 나눠지는 것에만 통한다.

### 재는 도구를 의심하는 두 회사

둘째 기능을 Anthropic 과 붙였다.

- Anthropic 은 재현 안 되는 신고 앞에서 모델이 아니라 재는 자와 무대를 의심했다.
- Etsy 는 그 의심을 자기 실험 체계에 돌려, 좋아 보여서 고른 처치일수록 효과를 부풀려 보게 된다는
  승자의 저주를 스스로 공개한다.

다른 것은 못 푸는 것을 적는 방식이다. Anthropic 은 원인을 갈라내는 쪽으로 갔고, **Etsy 는 얼마나
잘게 나눠 볼 것인가를 묻고 답하지 않은 채 남겨 둔다.**

### 사람을 먼저 세우고 기계를 맞추는 순서 — 그리고 이 엔진이 처음 만난 결과

셋째 기능을 메이퇀과 붙였다. 둘 다 사람이 기준을 먼저 만들고 거기에 기계를 정렬한다.
그런데 Etsy 에서 다른 회사들이 안 만난 일이 나온다 — **관련성이 좋아지는데 참여 지표가 떨어졌다.**
두 축이 어긋날 때 어느 쪽이 옳은지가 자명하지 않고, 회사도 답을 확정하지 않는다.

### 도메인을 제품이 아니라 묶이지 않는 것을 어떻게 다루느냐로 잘랐다

샤드로 안 묶이는 데이터, 서로 떼어 놓을 수 없는 실험, SKU 로 안 묶이는 리스팅 — 셋이 같은 성질의
다른 얼굴이다. 검색·판매자 도구·결제로 가르면 그 공통점이 사라진다.

### 다음 사이클 — 문체 보수 (사다리 3순위)

--gaps 가 정한다. 지금 맨 위는 databricks(2.32)지만 그건 이 시점의 예상이지 확정이 아니다.
