...
**아무도 안 판다 — 343 에서 freee 를 `done` 으로 닫았다.** 도메인 2개 · 기능 4개.
**완료 57곳 · 진행 0 · 큐 1/3(SmartHR).**

**⚠️ 이번 사이클은 세 편을 열고 하나도 안 썼다. 그게 결론이다.**
- `権限管理基盤マイクロサービスで行っているQA活動について`(2023-12-21) — **1/3.**
  화면 없는 서비스라 API 테스트가 필수고 `runn` 으로 짜 매일 돌린다. 대가는 적는다
  (**테스트 단계가 병목** · 구현 속도가 개발을 못 따라감 · 시나리오 비대 · 실패 원인 판별
  어려움). **그런데 수치가 하나도 없고 버린 대안도 없다**(채택 이유만 적는다).
- `権限管理基盤チームのCIの仕組みと課題`(2024-12-20) — **1/3.** GitHub Actions 두 단으로
  의존 서비스 준비를 미리 떼어 놓고 label 로 대상 좁히고 병렬로 돌린다. **역시 수치 없음.**
- `安全なデータベース削除オペレーションの自動化事例`(2023-12-20) — **1/3.** 삭제가 여섯 단계
  이고 Aurora 와 RDS 절차가 달라 **본 프로덕션은 두 사람이 짝을 지어** 했다. Step Functions
  와 Lambda 로 자동화. 한계 하나(**IAM 은 설정과 태그로만 좁혀 실제 상태는 판단 못 한다**).
  **버린 대안 없음 · 수치 없음.**

**⭐ 셋 다 `open_questions` 에 무엇이 있었고 왜 안 썼는지 남겼다** — 다음이 다시 안 열게.

**닫으면서 `business_model` 에 적은 것**
- **⭐⭐ 한 번에 다 넘기는 법이 없다** — 제품마다 켜고 끄기(회계를 마지막에) · 되돌릴 창
  이삼 주 · 5→50→100 퍼센트. **비율이든 제품이든 단계가 있다.**
- **⭐⭐ 되돌릴 길이 서지 않으면 안 넘긴다** — 2020년엔 마스터 승격을 직접 만들고, 2023년엔
  Blue/Green 을 *"2023年7月現在では切り戻し手法を確立できないため、導入を見送りました"*.
- **⭐ 같은 아픔의 두 층, 걸린 시간은 크게 다르다** — 바닥 갈아 끼우기 **석 달**, 위에 얽힌 것
  떼어내기 **1,454일**.
- **⭐⭐ 같은 프로젝트를 두 글이 다른 각도로** — 둘을 겹쳐야 4년의 모양이 보인다.
- **⭐ 후회와 되먹임 고리** — 적게 내보내니 돌아볼 일이 적었고, 그러니 고칠 것이 안 나왔다.
- **⚠️ 결이 고르지 않다** — 굵은 이야기는 DBRE·인증 쪽에 몰려 있다.

---

**다음 사이클 — 큐가 1/3 이다. 사다리대로 [신규 = SmartHR] 이 먼저다.**
- **⭐ 1순위 — SmartHR 을 `in_progress` 로 올리고 PROMPT.md 3단계(회사 프로파일)부터 판다.**
  큐에 근거가 다 적혀 있다: `tech.smarthr.jp`, 본문 확인 완료(`/entry/2026/06/11/192713`),
  **3/3** — CI 30분 이상 → 약 1분 · 배포 1일 1회 → 수시 · 종업원 데이터 조회 3.59~4.46배 ·
  행 크기 약 71% 감소. 버린 대안(처음부터 별도 서비스)과 대가(이관 미완 · DB 분리는 앞으로 ·
  **벤치마크가 엄밀하지 않다고 스스로 밝힘**)까지 있다.
- **⭐⭐ SmartHR 은 freee 와 정면으로 대비된다** — 같은 '인증·권한 기반을 제품에서 떼어낸다'
  문제를 **freee 는 모놀리스에서 곧장 4년에 걸쳐**(후회 명시), **SmartHR 은 모듈러 모놀리스로
  시작해 Packwerk 로 경계를 먼저 보이게 한 뒤** 떼어낸다. **freee 가 후회로 적은
  *"旧機能のリファクタはマイクロサービス化の前に着手すべし"* 가 SmartHR 이 실제로 한 일이다.**
  **SmartHR 을 완주하면 비교 문서를 쓴다.**
- **2순위 — 후보 조사.** SmartHR 을 올리면 큐가 0 이 되므로 **그다음 사이클은 후보 조사다**
  (PROMPT.md 사다리 3순위). **⚠️ Personio(EU) 는 아직 안 봤다 — 첫 순위.**
  **후보 조사 누적 열아홉 곳 중 일곱.**

**⚠️ freee 를 재방문할 때의 재료**(`freee.json` 의 `open_questions` 에도 적었다) —
`pt-online-schema-changeの導入時に検討したこと` · `AWSマネージドサービス+Terraformを活用してDBオペレーションをより安全&簡単に` ·
...
`【連載 第4回】freeeカード Unlimited でのClean Architecture実践`. **제목만 확인했다.**

**분포** — **57곳 전부 `done`**(일본 여섯 · 인도 셋) · 비교 문서 32편 · **운영 사실 31개.**
...
않는다**(342) · **⭐ 새로: 세 편을 열고 하나도 안 쓰는 사이클이 정상이다 — 대신 무엇이 있었고
왜 안 썼는지를 남긴다**(343).
