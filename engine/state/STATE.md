...
**없다. 310 은 비교 문서 사이클이었다** — `문지기를 무엇으로 세우는가`
(`domains/what-guards-the-gate.md`, **28번째 비교 문서**). `domains/index.json` 에도 등록했다.

**⚠️ 후보 조사를 미뤘다.** `--gaps` 는 [후보 조사]를 가리켰지만, **298·305 에서 아홉 곳을
보고 둘만 건진** 반면 **이 축은 재료가 여섯으로 무르익은 채 열 사이클 넘게 밀려 있었다.**
사다리 8순위를 앞당겨 쓴 것이고, **다음 사이클은 후보 조사로 돌아간다.**
**⚠️ ⑲ 는 돌렸다** — 53곳, 중복 없음.

**무엇을 엮었나** — 여섯 회사가 **모델을 더 좋게 만드는 대신 문을 하나 세웠고 그 재료가 다
다르다**: **컴파일러**(Wix) · **타입 정의**(Wix AirBot, Pydantic) · **AI 감사**(Meituan
Pre-PR) · **샌드박스 실행**(Meituan) · **일치율 문턱 85~90%**(Meituan 이진화) ·
**린터**(monday.com, AI 가 단 Human Todo 가 남으면 병합 차단).

**⭐ 문서의 축 둘**(둘 다 이 사이트의 정리)
1. **대가가 한 방향으로 모인다 — 검사할 수 있는 것만 검사된다.** 컴파일되지만 뜻이 틀린
   코드 · 이진화가 안 되는 지표 · AI 가 못 보는 결함 · 코드로 검증 안 되는 과제 · AI 가
   '판단이 필요하다' 고 안 본 것은 각각의 문을 그냥 지난다. **그 몫을 센 회사가 하나도 없다.**
2. **문지기의 성질이 갈린다** — **컴파일러·타입·린터는 못 잡는 범위가 정의로 알 수 있어**
   "그 밖은 사람이 본다" 는 설계가 성립하는데, **AI 감사는 무엇을 못 보는지 자체가 흐릿하다.**
   Meituan 은 그 흐릿함을 **상급 모델이 하급을 심사하고 벤더를 교차**시켜 덮는다 —
   **문지기가 못 보는 것을 다른 문지기로 덮는 셈이다.**

**⭐ 그 밖에 담은 것** — **문을 앞으로 옮긴 두 사례**(Meituan 은 리뷰가 병목이 되어, monday.com
은 앞단 검토가 *"became bottleneck, not safety mechanism"* 이라) · **Wix 의 뜻밖의 결과**
(문지기가 잡은 것이 모델의 실수가 아니라 **토큰 잘림과 낡은 타입 정의**였다) · **AirBot 의
완전 자동화율 15%**(문을 세운다고 사람이 사라지지 않는다) · **반대 방향 재료 Cybozu**
(너무 많이 잡는 문지기. **심볼 정보를 지우면 가장 안전한 쪽으로 기운다**) · **사람이 남는
자리 표**(볼 것을 목록으로 줄였다).

---

**다음 사이클은 후보 조사다.** 진행 중인 회사 없음 · 완료 53곳 · 큐 0/3.
**⚠️ ⑲ 를 먼저 돌린다.**
- **분포** — US 22 포화 · 한국 아홉 · 일본 넷 · 중국 둘 · 동남아 하나 · 인도 하나 ·
  유럽 일곱. **채울 곳은 중국·인도·유럽.**
- **아직 안 본 곳** — 중국 Kuaishou·JD·Alibaba 계열 · 일본 Sansan·freee·SmartHR·
  Money Forward · 유럽 SoundCloud·Trade Republic·Personio.
- **기준** — **버린 대안 · 대가 · 수치 중 둘.** **제목만 보고 올리지 않는다 — 한 편은 실제로
  열어 본다.** 305 에서 **목록만 봤으면 Cybozu 를 떨어뜨렸을 것**이고, 298 에서 **제목만 봤으면
  Dream11 을 올렸을 것**이다.
...
- **📌 kakao·roblox 가 `in_progress` 로 남아 있다** — 각각 `hold_reason` 달린 도메인 하나
  때문이고 **hold 를 유지한 채 닫는 것이 정당하다**(290). **닫는 것도 한 사이클이 된다.**
- **⭐⭐ 남은 비교 문서 축 셋**
  ...
  **⚠️ 310 의 `what-guards-the-gate` 와 재료가 겹치므로 겹치는 부분은 서로 가리키게 쓴다.**
  ② `자를 어떻게 세우는가` — Snap 넷(골든셋 만들기 / 못 잰다고 적기 / 자를 만들고 결과가
  나쁘다고 적기 / 범위를 다른 편에서 밝히기) + Meituan 둘(자가 무뎌지면 새로 만든다 /
  이진화해 기계에게 맡긴다).
  ③ `무중단으로 갈아 끼우기` — Wix 두 편(인프라 층은 앱이 모르게 · 앱 층은 앱이 전부 알게) +
  **Cybozu**(애플리케이션이 딛는 바닥, `fcntl` 잠금 때문에 **가용성을 얻고 분산을 내줬다**).
  **기존 `reversible-first.md` 와 이어지므로 서로 가리키게 쓴다.**
