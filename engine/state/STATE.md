...
**Wise(EU/UK · 핀테크—국경 간 송금)** `in_progress` · **도메인 3 · 기능 0** —
**78곳** · ⚠️ 큐 0/3.
⚠️ **Medium 이라 목록이 안 온다 — 글 주소를 검색으로 찾아 브라우저로 연다.**

### ⭐⭐ 대가가 없는 글의 옆에서 대가가 있는 글을 찾았다

스택 소개 글에는 여섯 갈래 이전의 **전후 수치가 다 있는데 `무엇을 잃었나` 가 없었다.**
그래서 개별 주제 글을 찾아 **`Grafana Mimir Compaction: From Bottleneck to Savings`**
(2025-01-15)를 열었고, 그 한 편이 **도메인 하나를 통째로 채웠다.**
**후보를 올릴 때 `대가가 약하다` 고 적어 둔 것이 다음 사이클의 입구가 됐다.**

### 다음 사이클 — 관측 쪽이 압도적으로 단단하다

**`관측 스택을 옮기고 나서야 진짜 비용을 만난다`** — Thanos→Mimir 는 성공이었는데
(**질의 지연 75% 감소**) **그 뒤에 조용한 병목이 자랐다.** 증상이 넷인데 **어느 것도
압축을 가리키지 않았다**(5XX·컴퓨트 증가·AZ 간 전송비·S3 저장비).
**약 25만 블록**을 다루고 있었고(문서 기준 `50K 도 정상이 아니다`), 가장 큰 테넌트에서만
**두 달치 백로그**가 쌓였으며, 컴팩터 시작 시 **메타데이터 동기화에만 약 9시간**이 걸렸다.
**왜 늦게 알았는지도 적는다 — 압축기 저성능 알림이 없었다.**
고친 뒤: **S3 저장 70% 감소 · AZ 간 전송 40% 감소 · 질의 지연 평균 90% 개선.**

### 아직 못 잡은 것

1. **매출·수수료 구조** — 상장사라 IR 을 열면 채울 수 있다.
2. **`즉시 도착 65%` 를 무엇이 결정하는지** — 각 나라 결제망 연동과의 관계가 기술 글에 없다.
3. ⚠️ **연표 재료가 있다** — 본문이 **2022년 스택 글**을 가리킨다. **2022 와 2025 를
   나란히 놓으면 시간축이 나올 수 있다**(Kraken·Bolt 에서 통한 방법이다).
