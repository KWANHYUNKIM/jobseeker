...
**없다 — 문체 보수 사이클이었다.** Fastly 를 배민 문체로 다시 썼다(1.92 → 0.0). 회사 115개, 큐 2/3. 27곳 남았다.

### 중앙을 버린 셋 — 버린 이유가 다르다

첫 기능에 축을 세웠다.

- Cloudflare 는 전용 로드밸런서를 없애고 모든 서버가 서로에게 넘기게 만들었다.
- Fly.io 는 라우팅 표를 합의 없이 퍼뜨린다.
- Fastly 는 퍼지를 중앙 큐에 안 맡긴다.

**앞의 둘은 속도 때문이고 여기는 도달 가능성 때문이다.** 퍼지는 못 받으면 옛 데이터가 계속
나가는 일이라 느린 것보다 못 닿는 것이 나쁘다. 그리고 1999년 논문을 가져온 이유 넷에
**이해할 수 있다**가 들어 있다 — 성능이 아니라 운영이 이해할 수 있는가를 기준에 넣었다.

### 무엇을 잘 맞혀야 하는지를 의심한다

둘째 기능이 재는 것을 의심하는 축에 새 자리를 낸다.

- Anthropic 은 **무대**를 의심했다.
- Etsy 는 **자기 실험 체계**를 의심했다.
- Fastly 는 **무엇을 잘 맞혀야 하는지**를 의심했다 — 정교한 모델들은 평상시를 잘 맞히는데
  정작 돈을 써야 할지가 갈리는 드문 날에 빗나갔다.

그래서 평균을 일부러 못 맞히기로 했다. 계획 결정이 일어나는 상단 구간에 가중치를 줬고,
평상시 정확도로 평가하면 저평가된다.

### 구조가 막는 데까지와 약속으로 메우는 데까지

셋째 기능을 1Password 와 붙였다. 저쪽은 맡은 것을 자기가 못 읽게 기술적으로 불가능하게 만들고
그 목록을 적는다. Fastly 의 프록시도 평문은 구조가 막지만 **목적지 호스트명과 포트는 구조적으로
알 수밖에 없어서 그쪽은 약속으로 메운다.** 그 약속 끝에 붙은 예외(남용 방지와 네트워크 운영)의
범위를 안 적는 것이 가장 큰 공백이다.

### 도메인을 매출 갈래가 아니라 한 네트워크가 파는 서로 다른 약속으로 잘랐다

셋째와 넷째가 정면으로 반대다 — **하나는 안 보겠다고 약속하고 다른 하나는 봐서 걸러 주겠다고
약속한다.** 매출 갈래(전달·보안·컴퓨트)로 가르면 그 대비가 사라진다. 넷째 도메인의 why 가
아직 자료를 못 봤다고 적혀 있던 것도 고쳤다 — 그 아래 기능이 이미 쓰여 있었다.

### 다음 사이클 — 문체 보수 (사다리 3순위)

--gaps 가 정한다. 지금 맨 위는 bolt(1.86)지만 그건 이 시점의 예상이지 확정이 아니다.
