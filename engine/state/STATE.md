...
**없다 — WarpStream 을 완주했다.** 도메인 3개 · 기능 3개. 회사 **100개.** ⚠️ **큐 0/3 — 다음 사이클은 후보 조사다.**

### 이번 사이클 — `벤치마크에서 이길 생각이 없다`

⭐ **회사가 자기 약점을 먼저 말해 버린다** — **`우리는 행 지향 형식으로 데이터를 저장하는 분산 로그에 질의 엔진을 붙여 놨다. 저장 계층이 컬럼 지향이 아니라서 우리는 어떤 벤치마크 경쟁에서도 이기지 못할 것이다. 하지만 그래도 괜찮다.`** 대신 노리는 것을 따로 적는다 — **`충분히 빠르고, 충분히 싸고, 충분히 쓰기 쉬운`**.

건진 것 넷:

- ⚠️ **관측하는 값이 비싸서 눈을 감아야 했다** — **`어떤 이들은 중앙화된 로그가 아예 없고`**, 3자 플랫폼 쪽에서는 **고카디널리티 비용 때문에 내보내는 텔레메트리 자체를 제한해야 했다.** **싸게 만드는 것이 존재 이유인 회사가 자기를 관측하는 값에 걸렸다**(재구성).
- ⚠️ **대안을 죽인 것이 한 문장이다** — **`사이드카 데이터베이스나 로그 집계 파이프라인을 더하면 운영 부담과 비용이 는다`**. ⭐ **이 회사에서는 기술 판단이 아니라 정체성 문제다** — **싸고 운영이 쉽다는 것이 파는 물건 자체이므로 관측을 위해 그것을 깨면 제품을 깨는 것이 된다**(재구성).
- ⚠️ **색인을 만들지 않고 실행 순서로 푼다** — **`Kafka 는 오프셋으로 정리되지 시간으로 정리되지 않는다`** 는 어긋남을 **타임스탬프 워터마크**와 **`황금비 기반 간격 전략`**(초기 작업을 오프셋 공간에 흩어 표집)으로 다룬다. **색인을 만들면 다시 새 저장이 생기기 때문이다**(재구성). 읽기는 **`Go 의 net.Pipe()`** 로 **`네트워크 스택을 전혀 타지 않게`** 해 **처리량 2배 이상**.
- ⚠️ **한계를 숨기는 대신 쓰임새를 좁혔다** — **`가끔 쓰는 디버깅 도구지 주된 관측 시스템이 아니다`** 이고, **클러스터당 동시 질의 최대 2개**라는 보수적 상한이 그 선언을 코드로 옮긴 것이다(재구성). 데이터 경계도 **하드코딩**이다 — **`당신 토픽의 실제 메시지나 원본 데이터는 절대 담지 않는다`**(**설정으로 열 수 있게 두면 언젠가 열린다**, 재구성).

### 완주하며 본 것 — 세 기능이 하나의 제약에 매여 있다

⚠️ **`싸고 운영이 쉽다` 는 것이 파는 물건이라, 그 하나가 세 도메인의 결정을 전부 지배한다**(재구성) — **디스크를 없앤 것**(가용영역 비용), **얇은 프록시만 놓은 것**(고객 팀을 못 움직이니), **사이드카를 거절한 것**(운영 부담). ⭐ **셋 다 `새로 들이지 않고 이미 가진 것의 성질을 다시 쓴다`** — 순서를 미뤄서, 복제 경로를 감시 경로로, 저장 엔진을 관측 저장소로.

⚠️ **그리고 셋 다 포기한 것을 세어서 적는다** — 외부 일관성(오프셋 0·멱등·트랜잭션 셋) · 되돌아갈 수 없는 컷오버(`일방통행 문`) · 벤치마크 성능. ⭐ **③축의 아주 좋은 사례다 — 이 회사는 성과를 부풀리는 대신 한계를 선점한다**(재구성).

### 다음 사이클 — ⚠️ 후보 조사 (3순위)

**큐가 0/3 이라 `--gaps` 가 후보 조사를 부른다. 목표 3곳.**

⭐ **지난 조사에서 각도별 타율이 갈렸다** — **`이미 판 회사의 글이 언급하는 회사` 는 던진 넷 중 둘**(WarpStream ✅ · VictoriaMetrics ✅), **`없는 축을 검색으로 찾기` 는 세 번째로 전멸**, **컨퍼런스 각도는 목록조차 못 받았다.** **언급 각도를 먼저 쓴다.** ⚠️ **다만 만능은 아니다** — **쓰는 쪽이 아니라 파는 쪽이면 글의 성격이 다르다**(PingCAP). ⚠️ **후보 대조는 slug 가 아니라 `name_en` 으로.**

⏳ **이번 회사가 남긴 언급 후보** — **Datadog**(있음) · ⏳ **Confluent**(모회사, 2024 인수) · ⏳ **IBM**(2026-03-17 Confluent 인수 완료).

⏳ **보강 거리 다섯** — Grafana Labs 인용 대조 · Razorpay 보안 트리아지 글 · Flipkart Rate Card 엔진 글 · Pinterest 2부 · **WarpStream 미독 셋**(감사 로그 · MCP 서버 · `DevOps 팀 없이 30개 환경`).

⏳ **WarpStream 연표 재료** — **`WarpStream is Dead, Long Live WarpStream`**(인수 뒤 방향 전환일 수 있다).
...
**③ ⭐⭐ `자기 성과를 어디까지 주장하나` 8곳** — Snyk / Grafana Labs / ScyllaDB / Razorpay / Flipkart / Airbnb / Pinterest / **WarpStream**(⚠️ **세 기능 모두 포기한 것을 세어서 적고, 아예 `벤치마크에서 이기지 못한다` 를 먼저 말해 버린다 — 한계 선점이라는 별개의 전략이다**).
...
**⑧ `제약을 없애지 못할 때 어디서 갚는가`** — Cygames / Zepto / Grafana Labs / Pinterest / WarpStream(세 겹).

**⑨ `한 번에 갈아엎을 것인가 목 졸라 죽일 것인가` 7곳** — Twilio(두 방향) / Etsy / Plaid / Paystack / Airbnb / Pinterest / WarpStream.

**⑩ ⭐ `관측 비용을 어디까지 줄이나`** — Razorpay · Airbnb · **WarpStream**(⚠️ **사는 쪽이면서 파는 쪽이다 — 3자 플랫폼 비용에 막혀 자기 것을 만들었다**) / Grafana Labs · Honeycomb · ⏳ VictoriaMetrics.
...
**⑬ `애매할 때 어느 쪽으로 넘어지나` 7곳** — Flipkart(둘) / Razorpay / Zepto / Paystack / Airbnb / Pinterest / WarpStream.

**⑭ `검색 관련성을 누가 정하나`** — Etsy / Flipkart / ⏳ Zepto / Pinterest.

**⑮ `언제 쪼개고 언제 합치나`** — Twilio / DoorDash / Deliveroo / ScyllaDB / Airbnb / Pinterest / WarpStream.

**⑯ ⭐ `빌려 쓰던 것을 언제 자기 것으로 만드나`** — Airbnb / ScyllaDB / Plaid / Cygames / Twilio / Pinterest / **WarpStream**(⚠️ **3자 관측을 떠나 자체 질의 엔진을 만들었다 — 그런데 `Datadog 에서 Husky 를 만든` 사람들이 만들었다**).
...
**⑳ ⭐ `없앨 수 있는 것을 없앤다`** — **WarpStream**(디스크도, 관측 인프라도 — **`새로 들이지 않는다` 가 이 회사의 손버릇이다**) / ⏳ TigerBeetle · Oxide Computer.

**㉑ `논문을 어디까지 그대로 쓰나`** — WarpStream(LazyLog) / ⏳ ScyllaDB · TigerBeetle · ClickHouse.

**㉒ `되돌릴 수 있는 곳과 없는 곳을 어떻게 가르나`** — WarpStream(MIGRATING 까지 자동 롤백, COMPLETE 는 일방통행) / Plaid / Airbnb / Twilio.

⏳ **㉓ 새로 보인다 — `설정으로 열 것인가 코드로 막을 것인가`** — **WarpStream**(**데이터 경계를 하드코딩했다** — 설정으로 두면 언젠가 열린다) / **Airbnb**(**하이브리드를 첫 번째 원칙으로 금지**) / ⏳ Pinterest(확인 필요). ⚠️ **선택의 자유를 주면 반드시 그 자유가 쓰인다는 판단이 공통이다**(재구성).
