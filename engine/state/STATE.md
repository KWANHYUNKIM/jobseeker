...
**카카오뱅크(KR · 핀테크) — 첫 기능 `sequence-fds`(행동의 순서로 보는 사기 탐지)를 채웠다.**
`in_progress` · **도메인 2 · 기능 1** · 결정 8 · 그림 3 · 논문 2 · 난제 1.
**완료 62곳 + 카카오뱅크 = 63곳 · ⚠️ 큐 대기 0/3 · 비교 문서 33편.**

### 다음 사이클 — ⭐ 담보심사 글부터 읽는다

`--gaps` 가 **확장 2순위**로 남은 빈 도메인
**`빌려줄지 정하는 데 사람을 기다리게 하지 않는다`(tech 비었음)** 를 지목할 것이다.
**⚠️ 두 사이클 연속 미룬 자리다.** 먼저 원문을 연다 —
`tech.kakaobank.com/posts/2605-no-wait-collateral-review-system-design/`(2026년 5월 6일).
큐 조사 메모에 **외부 워크플로우 엔진 대신 검증된 사내 Deferred Batch 를 확장했고, 그 대가로
작업 현황·오류 추적이 복잡해져 전용 대시보드를 따로 만들어야 했다**는 내용이 있으나
**원문 확인 전에는 옮기지 않는다**(341·342 — 확인 안 된 인용 금지).
읽고 나서 **도메인 `tech` 를 채우고 같은 사이클에 기능 하나**까지 간다. 그러면 카카오뱅크가 닫힌다.

### ⚠️ 큐가 0/3 이다 — 카카오뱅크를 닫는 순간 3순위가 걸린다

PROMPT 사다리에서 **큐가 0 이면 후보 조사가 3순위**다. 확장이 끝나면 곧바로 그쪽이다.
**⚠️ 지난 세션에서 WebSearch 예산이 막힌 적이 있다 — 새 세션이면 먼저 시도한다.**
**⚠️ 요약만 보고 판정하면 과대평가가 난다**(354) — **'무엇이 없는지' 를 함께 묻는다**(352).

### 이번 사이클에서 배운 것

- **⭐ 지연 예산이 모델 후보를 먼저 자른다.** RNN·LSTM·GRU 를 버린 이유가 성능만이 아니라
  **"순차 처리가 필요해 GPU 병렬화에 불리"** 하고 **"20ms 내에 다수 요청을 처리해야 하는
  요구사항"** 이었다. **비기능 요구가 후보를 자르는 순서를 결정 칸에 적는다** — 그러면
  '왜 저 모델이 아니라 이 모델인가' 가 성능 비교가 아니라 **운영 조건**으로 설명된다.
- **⭐ 무엇을 없애면 무엇을 함께 잃는지가 결정 두 개로 나뉜다.** 파생 변수를 없앤 결정과,
  그래서 **"기존의 변수 기반 XAI를 그대로 적용하기가 쉽지 않았습니다"** 를 메우려
  Gradient Attention Rollout 을 붙인 결정은 **한 덩어리가 아니라 둘**이다(STYLE.md 3번).
- **회사가 직접 이름을 댄 논문은 `confirmed` 로 쓸 수 있다.** ArcFace 가 그랬다 —
  **"Arcface: … 에서 제안된 방식을 적용했을 때 가장 뛰어난 성능"**. ViT 는 이름만 나오고
  논문 인용이 없어 `inferred` 로 뒀다. **둘을 같은 등급으로 쓰지 않는다.**

### 카카오뱅크 자료 접근

- **`tech.kakaobank.com` 은 WebFetch 로 본문까지 열린다.** 목록은 **페이지네이션이 있어 한 번에
  4건만** 보인다 — 더 필요하면 글 주소를 직접 열거나 검색으로 찾는다.
- **⚠️ 실적을 언론 보도에서 옮겼다**(아시아경제 2026-02-04). IR 원문과 대조하지 않았다.
- **⚠️ FDS 정확도 지표가 없다.** precision·recall 대신 **t-SNE 군집**으로 대신한다 —
  기능의 지표 칸을 억지로 채우지 않았다.
- **⚠️ "약 30%를 … 지속적으로 모니터링" 이 무엇의 30%인지 못 읽었다.** 지표로 쓰지 않았다.

### 비교 문서 재료 — 두 편이 쌓였다, 아직 안 썼다

**① "내가 못 고치는 것이 조용히 바뀐다"** — **ZOZO**(관리형 추천 API) · **쏘카**(장비사 50여 곳).
둘 다 **실패를 신호로 삼기 때문에 실패 없는 변경을 못 잡는다.**

**② ⭐ "정확도를 못 재는 것을 어떻게 믿는가"** — **카카오뱅크 FDS**(precision·recall 없이
t-SNE 군집) · **ZOZO 애매한 검색**(*"nDCG 같은 정량 지표가 아니라 목시 평가"*) ·
**쏘카 예약 테트리스**(*가상 예약*이라는 대리 지표). **셋 다 정답 집합을 만들 수 없어 대리
지표로 옮겨 갔고, 그 대리 지표가 무엇을 좋다고 볼지를 정한다.** **세 회사가 모였으니 쓸 수 있다** —
카카오뱅크를 닫은 뒤 후보 조사와 이 문서 중에서 고른다(사다리상 후보 조사가 먼저다).

### ⚠️ 다른 세션과 같은 큐를 잡지 않는다

`guide-engine`(쿠팡)·`study-engine`(서킷 브레이커·JWT·Docker) 둘 다 계속 커밋 중이다.
