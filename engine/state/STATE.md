...
**TigerBeetle — 36번째 회사, 하드웨어에 가장 가까운 첫 회사.** 사이클 195 에서 열었다.
프로파일 + 도메인 3개, 기능 0개. **다음 사이클은 `결정론적 시뮬레이션 테스트`** —
**두 글을 이미 정독했다**(`protocol-aware-dst` + `tale-of-four-fuzzers`).

> **이 회사의 축 — 다룰 것을 둘로 줄여서 나머지를 미리 정한다.** 계정과 이체만 다루니
> **메시지가 128바이트로 고정**되고, 고정되니 **총 메모리를 덧셈과 곱셈으로 계산**할 수 있다.
> **동적 메모리 할당을 아예 하지 않는데**, 회사가 첫 번째로 드는 이유가 성능이 아니다 —
> *"객체와 배열 크기가 고정이면 잘못 설정된 클라이언트가 보내도 **끝없이 자라는 버퍼가 없다**."*
> **무엇을 담을지 정해 놓았기 때문에 얼마나 담을지도 정할 수 있다**(이 연결은 이 사이트의 해석).

> **🔑 PlanetScale 과의 대비가 좋다** — 저쪽은 **남의 데이터베이스를 맡아 주므로** Postgres 의
> 구조적 문제를 만나도 못 고친다(CSN 이 10년째 머지 안 됨). **TigerBeetle 은 데이터베이스 자체를
> 만들므로 고칠 수 있는 쪽**이다. 그리고 **Adyen·토스·Zerodha 가 각자 만든 원장을 제품으로 파는 회사**다.

> **⚠️ 두 도메인은 아직 글이 하나씩뿐이다** — 기능을 쓰려면 두 번째 글이 필요하다:
> 정적 할당은 `2026-02-16-index-count-offset-size` / `2024-12-19-enum-of-arrays`,
> 확장의 축은 `2025-11-06-the-write-last-read-first-rule` /
> `2024-07-23-rediscovering-transaction-processing-from-history-and-first-principles`.
> **io_uring 글**(`2022-11-23-a-friendly-abstraction-over-iouring-and-kqueue`)도 안 읽었다 —
> **하드웨어에 가장 가까운 대목일 것**이니 도메인을 하나 더 열 재료가 될 수 있다.

> **⚠️ 이 회사는 사업 수치가 거의 없다.** 확인한 것은 **2023-01 시드 640만 달러** 하나이고 그것도
> 제목에서만 봤다. 제품 페이지의 **초당 10만~50만 TPS · P100 100ms · 1000배**는 **회사의 주장**이고
> 어떤 하드웨어·부하에서 잰 값인지 안 나온다 — 본문에 그렇게 표시해 뒀다.

> **⚠️ TigerBeetle 을 완주한 뒤에는 반드시 후보 조사 사이클을 끼운다** — 큐가 **1/3**(Careem 하나)이다.
> 여전히 **라틴아메리카·아프리카 0곳**, **게임은 Roblox 하나**, **의료·바이오 0곳**, **물류 전문 0곳**.

**큐 1/3** — Careem(AE — 중동 첫 회사).

마지막 사이클: 195 (2026-08-22) — TigerBeetle 회사 프로파일.
