...
**없다 — Etsy 를 완주했다.** 큐가 **0/3** 이다.

### 이번 사이클 — LLM 검색 `인기를 지우고 뜻을 세우자 참여 지표가 흔들렸다` 와 완주

⚠️ **자료부터 찾는 방식이 여섯 번째로 통했다.**

- ⚠️ **객관적인 신호가 객관적인 답을 주지 않는다** — 참여 신호(클릭·장바구니·구매)는 관련성의 대리 지표였는데 `popular listings get more clicks, **even when they're not the best match**` 다. **인기가 인기를 낳는 고리를 참여 지표만 보면 못 알아챈다**(재구성).
- ⚠️ **가장 값진 대목은 어긋남을 적는 것이다** — `we often observe **engagement metrics decline even as semantic relevance improves**`(다른 이커머스 플랫폼도 같은 패턴이라 덧붙인다). **두 축이 어긋날 때 한쪽을 버리지 않고 어긋남 자체를 연구 대상으로 삼는다.**
- ⚠️ **관련성의 뜻이 사회적 사건으로 바뀐다** — **2020년 이전의 `face masks` 는 코스튬·패션이었고 그 뒤로는 보호용이었다.** 그래서 라벨링 지침을 고정하지 않는다. **관련성은 정의해 두는 것이 아니라 계속 다시 정의하는 것이다.**
- **사람이 기준을 만들고 LLM 이 넓힌다** — `LLMs do not replace human judgment, instead they **align with and amplify** it`. 3단 증류로 정확도·지연·비용을 층마다 다르게 맞추고, **회사가 그 운영 복잡도를 다음 숙제로 적는다.**

### ⚠️ 이 회사에서 본 가장 큰 것 — 자기 도구의 편향을 두 번 공개한다

**A/B 프레임워크**는 `systematically prone to overestimating` 이라 적고, **검색의 참여 신호**는 인기에 치우친다고 적는다. **그리고 둘이 만난다** — 관련성이 오르는데 참여가 떨어진다는 관찰도 그 실험 위에서 나온 것이라, **겹쳐 읽어야 한다**(재구성).

### 다음 사이클 — 큐가 0/3 이라 후보 조사

⏳ **안 두드린 후보** — Deliveroo · Delivery Hero · Snyk/Okta(보안 둘째 자리). ⚠️ **접근 지도의 `403` 목록을 브라우저로 되짚는 것이 더 수확이 좋을 수 있다** — Etsy 가 그렇게 살아났다. 되짚을 곳: `blog.paystack.com` · `www.scylladb.com/blog` · `www.meesho.io/blog` · `blog.zepto.com` · **`engineering.plaid.com`**(내용이 좋아 보였는데 못 받았던 곳).

⚠️ **비교 문서는 여전히 사다리에 막혀 있다** — AI 에이전트 축에 다섯 회사가 찼다.
