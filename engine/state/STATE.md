...
**Let's Encrypt**(인증 기관) — 회사 프로파일까지 썼다. **도메인 둘 · 기능 0.** 큐 0/3.

### 두 도메인이 같은 문제의 두 답이다

잘못 나간 인증서를 무효로 만드는 길이 둘이다 — **잘 알리기**(OCSP·CRL)와 **덜 필요하게
만들기**(수명 단축). 이 회사는 앞엣것에서 **손을 떼고** 뒤엣것으로 **더 밀고 있다**(90일 →
45일).

### 공짜라는 것이 기술 선택을 정한다

값을 받지 않으니 **사람 손이 드는 일을 없애야** 한다. 그래서 자동화가 전제가 되고, 그
전제 위에서만 짧은 수명이 성립한다 — **"웹 전체를 HTTPS 로 옮기려면 시스템 관리자들이
갱신을 계속 손으로 처리하기를 기대할 수 없다."** OCSP 를 접는 이유에 **운영 부담**이
들어가는 것도 같은 처지다.

### 자동화와 짧은 수명은 하나다

**자동화 없이는 짧게 만들 수 없고, 짧게 만들지 않으면 자동화할 이유가 없다.** 2015년 글이
두 이유를 나란히 적은 것이 그래서다 — 피해 창을 좁히는 것과 자동화를 유도하는 것.

### 반발을 예상하고 먼저 적었다

**"이렇게 묻는 사람들은 보통 90일이 너무 짧다고 걱정하고, 다른 인증 기관들처럼 1년 이상
가는 인증서를 바란다."** 10년 전 글인데 지금 45일로 가는 중이다 — **자기가 먼저 간 길을
업계가 따라왔다.**

### 다음 사이클

빈 도메인 둘 중 하나. **OCSP 종료 쪽이 근거가 두껍다**(프라이버시 구조 · 법적 강제 가능성 ·
물린 대안 둘을 이름 대어 · 깨질 수 있는 것). 수명 단축 쪽은 10년 치 자료가 있지만
**45일 글에는 물린 대안도 정량 비용도 없다.**
