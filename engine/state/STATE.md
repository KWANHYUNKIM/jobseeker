...
**없다 — 문체 보수 사이클이었다.** Ramp 를 배민 문체로 다시 썼다(2.56 → 0.0). 회사 115개, 큐 2/3. 40곳 남았다.

### 빨리 가려고 멈춘 회사가 둘인데 멈춰서 한 일이 다르다

첫 기능에 볼(bol)과의 대비를 넣었다.

- 볼은 옛 모놀리스를 두고 갈 수 없어 4년 치 정산 데이터를 옮겼다 — **과거 때문에 멈췄다**.
- Ramp 는 앞으로 올 요구를 코드로 못 받겠다고 판단해 규칙을 표현하는 재료를 만들었다 —
  **미래 때문에 멈췄다**.

같은 기능에 Plaid 대비도 걸었다. 저쪽은 은행마다 다른 규칙을 대신 흡수해 하나의 길로 보여 주고,
여기는 차이를 감추지 않고 고객이 직접 조립하게 한다. **흡수할 수 있는 차이와 조립하게 둘 수밖에
없는 차이가 다르다.**
...
- Anthropic 은 모델이 나빠졌다는 신고를 재현하지 못해 재는 자와 무대를 먼저 의심했다.
- Ramp 는 재현을 설정 파일로 사서 코드와 파라미터를 고정했는데, 회사가 조건을 함께 적는다 —
  데이터가 안 바뀌었다면.

**고정한 것은 코드이고 흔들리는 것은 상류 데이터다.** 규제가 요구하는 재현은 대개 몇 년 전
판단의 재현이라 이 조건이 그대로 난제가 된다.

### 에이전트에게 신원을 주는 세 번째 답

셋째 기능이 기존 축에 새 자리를 낸다.

- Block·Sentry 는 에이전트가 PR 을 열게 두고 **병합에서 사람을 세운다**.
- Razorpay 는 직원마다 격리된 신원을 새로 만들어 주고 모든 호출을 실명에 귀속시킨다.
- Ramp 는 새 신원을 만들지 않고 **시킨 사람의 GitHub 토큰을 빌려 쓰게 한다.**

앱 토큰이 훨씬 편한데 안 썼다. 모두가 같은 신원을 쓰면 누구든 자기 변경을 자기가 승인할 수 있게
되어 코드 리뷰가 형식이 되기 때문이다. 신원을 사람 것으로 두니 자기 승인이 저절로 막힌다.

### 도메인을 제품이 아니라 틀을 만든 자리로 잘랐다

카드·경비 정산·회계 연동으로 가르면 이 회사에서 가장 특징적인 것이 흩어진다. 승인 로직을
그래프로, ML 배포를 YAML 로, 코딩을 사내 에이전트로 — 셋 다 직접 짜지 말고 틀을 만들어라는
한 문장의 변주다. 그 판단을 사업 모델에 적었다.

### 다음 사이클 — 문체 보수 (사다리 3순위)

--gaps 가 정한다. 지금 맨 위는 paystack(2.52)이지만 그건 이 시점의 예상이지 확정이 아니다.
