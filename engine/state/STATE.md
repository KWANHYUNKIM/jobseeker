...
**없다 — 문체 보수 사이클이었다.** Block 을 배민 문체로 다시 썼다(2.65 → 0.0, 기호 290개로 지금까지 가장 무거웠다). 회사 115개, 큐 2/3. 42곳 남았다.

### 도메인을 사업부로 자르지 않은 이유를 적었다

Block 은 Square(가맹점)·Cash App(개인)·하드웨어(Bitkey·Proto)로 사업이 셋인데 도메인은 다섯이고
그 경계가 사업부와 안 맞는다. 그 판단을 사업 모델에 남겼다 — **사업부로 가르면 확률적으로 틀리는
것과 배포로 못 고치는 것과 사람이 병목이 되는 것이 한 칸에 섞인다.** 이 회사에서 실제로 갈리는
것은 파는 상대가 아니라 고치는 방법이다. 다섯째(필드 단위 암호화)만 사업 둘에 걸치는데, 그건
Square 와 Cash App 이 같은 메시지 위에서 돌기 때문이다.

### 사내 에이전트 플랫폼 셋이 서로 다른 것을 막는다

세 번째 기능에 축을 하나 세웠다. 셋 다 에이전트를 늘리면 생기는 새 비용을 말하는데 그 비용을
찾은 자리가 다르다.

- Razorpay 는 에이전트가 무엇까지 할 수 있는지를 커널로 가둔다(권한).
- Sentry 는 에이전트가 잘못된 결론에 이르지 않았는지 매일 밤 심문한다(품질).
- Block 이 막으려는 것은 사람이다 — 사람이 에이전트 사이의 중계기가 되는 것(주의력).

첫 기능의 자가치유 루프는 앞 사이클에 Sentry 쪽에서 이미 이어 놨는데, 이번에 Block 쪽에서도
같은 대비를 걸어 양방향으로 만들었다.

### 유휴를 흡수해 파는 쪽과 안 사려고 쓰는 쪽

MeshLLM 을 Modal 과 붙였다.

- Modal 은 유휴에 값을 안 받는 대신 시작하는 시간과 오가는 시간을 자기 원가로 떠안고,
  그것을 줄이려고 서버를 사용자 옆으로 옮겼다.
- Block 은 밤과 주말의 사내 기계를 이어 한 대에 안 들어가는 모델을 돌린다.

**한쪽은 유휴를 흡수해서 팔고 다른 쪽은 유휴를 안 사려고 쓴다.**

### 보안을 파는 회사들 옆의 빈자리

COLDCARD 자문은 이 엔진에서 유일하게 대상이 회사 밖에 있는 기능이다. Snyk 는 찾아 고치는 것을
팔고 Wiz 는 위험을 그려 파는데, **Block 은 그것을 팔지 않으면서 한다.** 하드웨어 지갑은 틀리면
배포로 못 고치니 남의 결함도 자기 신뢰의 재료가 된다. 필드 단위 암호화는 1Password 와 붙였다 —
둘 다 신뢰를 조직이 아니라 구조에 맡기는데, 경계선을 저쪽은 회사 바깥에 여기는 메시지 안에 긋는다.

### 다음 사이클 — 문체 보수 (사다리 3순위)

--gaps 가 정한다. 지금 맨 위는 onepassword(2.61)지만 그건 이 시점의 예상이지 확정이 아니다.
