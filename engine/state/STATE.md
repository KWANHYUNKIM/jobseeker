...
**Block (Square)** — 회사 프로파일을 썼다. 도메인 **4개**, 기능 **0/4**. 회사 **109개.** 큐 **2/3.**

### 이번 사이클 — ⭐ 도메인이 저절로 갈린 드문 회사

**한 회사 안에 성격이 아주 다른 셋이 있다** — 가맹점에 파는 것(Square), 개인에게 파는 것(Cash App), ⭐ **기계를 파는 것**(COLDCARD 하드웨어 지갑). ⚠️ **그리고 그 셋을 떠받치는 사내 도구까지 직접 만들어 공개한다**(Buzz).

⭐ **매출로 보면 비트코인이 크고 총이익으로 보면 Cash App 이 크다** — 비트코인 생태계 매출 **18.9억 달러**(유일하게 감소, -12.8%)인데 이건 사고파는 금액이 그대로 매출에 잡히기 때문이고 남는 몫은 얇다(재구성). 총이익은 **Cash App 20억**(+31%, `Borrow`·`Card` 가 견인) 대 **Square 12억**(+13%).

세운 도메인 넷과 각각에서 건진 것:

- **확률적으로 틀리는 것을 고친다** (Moneybot) — ⭐ **회귀 테스트의 전제가 여기서 깨진다**: **`같은 행동을 해도 어떤 고객에게는 문제가 나타나고 어떤 고객에게는 안 나타난다.`** **여섯 하위 에이전트를 순서대로 돌린다**(재현자·진단자·수정자·검증자·발행자·리뷰 대응자). ⭐ **결정론적이지 않은 실패를 표본 수로 다룬다** — 스테이징 **10~20회**로 기준 실패율을 세우고 고친 뒤 **같은 표본 수로** 다시 잰다. **검증된 수정 40건 이상.**
- **키를 만드는 난수** (COLDCARD) — ⭐ **이 회사의 도메인 중 유일하게 배포로 끝나지 않는다.** **이중 안전장치가 통합 지점의 가정 하나 때문에 반대로 작동했다** — **`libngu 가 그 매크로가 켜졌는지가 아니라 정의되었는지를 검사한다.`**
- **에이전트가 팀으로 일한다** (Buzz) — **`에이전트가 서로 효율적으로 소통하지 못하면 모든 모호함이 알림이 된다.`** ⭐ **팀이 늘 이기는 게 아니다** — 몇 시간짜리 긴 작업에서만(71.5% 대 59.1%). ⭐ **돈과 점수가 따로 논다** — **가격 5.5배 차이에 점수는 8.9점.**
- **노는 컴퓨트를 모은다** (MeshLLM) — **`어느 순간에도 방 안에는 놀고 있는 컴퓨트가 잔뜩 있다.`** ⚠️ **모으는 것은 컴퓨트뿐이다** — **`에이전트 자체는 공유하지 않는다.`**

### ⭐ 이번에 건진 가장 큰 것 — 세 도메인이 같은 물음의 서로 다른 답이다

⭐ **`틀린 것을 어떻게 아나`에 대해 한 회사가 세 가지로 답한다**(재구성):

- **Moneybot** — 확률적이라 **표본을 늘려** 안다(10~20회, 개선 여지까지 감안).
- **COLDCARD** — 결정론적이라 **경우의 수를 세어** 안다(SysTick 8만~12만 가지, 2^32 스트림, 평균 2^31 시도).
- **Buzz** — 벤치마크로는 **점수가 아니라 가격까지 봐야** 안다(5.5배 대 8.9점).

⚠️ **㉘축(`맞았는지 어떻게 아나`)에 한 회사가 세 칸을 채운다** — Cursor 가 `사용자의 다음 행동`으로 답한 그 축이다. **비교 문서 2순위였던 이 축이 이제 1순위 후보다.**

### 다음 사이클 — 확장 (2순위)

**Block 기능 0/4.** ⭐ **`키를 만드는 난수`부터 판다** — **결함의 인과가 한 줄까지 좁혀져 있고 수치가 전부 있다**(2^32 · 2^31 · 8만~12만 · 40바이트 → 4바이트). ⚠️ **실패 경로 그림을 그리기에 이보다 좋은 재료가 드물다.**

⚠️ **읽을 것이 남았다** — `Square 본업`(결제·정산·대출 심사) 자료를 아직 못 찾았다. **총이익 12억 달러를 내는 본업인데 이번에 읽은 다섯 편이 전부 AI·에이전트·펌웨어였다.** ⏳ 미독 — `BlockTabBench` · `Buzz 에이전트 설정` · `Buzz 프로젝트` · `Microfilm`(안드로이드 이미지 압축 Gradle 플러그인).
