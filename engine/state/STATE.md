...
**Ubicloud — 기능 하나를 채웠다(`bdev-ubi-copy-on-access`).** 빈 도메인 셋이 남았고
자료는 다 확보돼 있다. 회사 131개, 큐 1/3.

### 안 옮긴 81GB 가 이 회사의 값이다

깃허브 러너 VM 은 15~30초 안에 떠야 하는데 디스크를 통째로 복사하면 그 단계에만 4~5분이
든다. 관찰이 답을 준다 — **85GB 러너 이미지에서 워크플로가 실제로 바꾸는 건 4GB 남짓.**
그래서 복사하지 않고 게스트가 처음 만지는 블록만 그때 바탕 이미지에서 가져온다.

### 빌린 기능은 조건까지 딸려 온다

v2.0 에서 btrfs 의 reflink 를 빌려 1초 미만을 얻었는데 **암호화를 잃고 처리량이 ext4 의
3분의 1**이 됐다. v3.0 은 파일시스템을 거치지 않고 블록 계층에서 직접 해서 그 둘을 되찾는다.
이 엔진에서 버린 것을 다음 판에 되찾은 기록은 드물다.

### 사용자 공간에 둔 값

SPDK 로 호스트 사용자 공간에 뒀기 때문에 **하이퍼바이저를 포크하지 않고 네 판을 갈아엎었다.**
커널이나 포크 쪽을 골랐으면 못 했을 반복이다 — 성능이 아니라 고쳐 볼 수 있음을 산 선택이다.

### 앞 회사와 견준 자리

바로 앞 Depot 의 두 기능과 정확히 겹친다. LVM 제어면을 다시 만든 것과 같은 문제(디스크를
초 단위로)를 푸는데 **Depot 은 보장을 버렸고 Ubicloud 는 버렸다가 되찾았다.** SOCI 로 이미지를
다 안 받고 시작하는 것과 copy-on-access 도 같은 원리를 층만 달리해 쓴 것이다.

### 다음 사이클 — 확장 (Ubicloud 의 빈 도메인)

`남의 코드를 남의 기계 위에 가둔다` 부터. 클라우드 하이퍼바이저를 고른 이유와 seccomp-bpf 를
아직 기본으로 안 켠다는 자백이 이미 손에 있다.
