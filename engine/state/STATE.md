...
**Chroma — 기능 하나를 채웠다(`objstore-index-execution`, 색인은 객체 저장소에 두고 실행은 밀어
넣는다).** 회사 126개, 큐 0/3. 빈 도메인 둘 — 로그 · 멀티테넌시.

### 비동기 색인의 시차를 질의가 메운다

가장 궁금했던 것(컴팩션이 비동기면 방금 쓴 것은 언제 검색되나)이 실행 엔진 글에서 풀렸다.
**KNN 질의가 색인을 조회하면서 동시에 아직 색인에 안 들어간 로그를 끌어와 브루트포스로 훑는다.**
쓰기가 색인 만들기를 기다리지 않아도 되고, 대신 안 색인된 구간이 길수록 질의가 느려진다.

### 계획을 둘로 줄여 여유를 스케줄링에 쓴다

SQL 처럼 무엇이든 받지 않는다 — 계획이 `GetPlan` 과 `KNNPlan` 둘뿐이다. 그렇게 좁힌 여유를
멀티테넌트 공정성과 자원 스케줄링에 쓴다고 밝힌다.

실행은 볼케이노식 끌어오기가 아니라 **밀어 넣기**이고, 제어 흐름을 스택 밖으로 꺼내 명시적으로
다룬다. 병렬도는 계획에 박지 않고 실행 중에 모젤로 나눠 정한다 — **자원이 오르내리는 멀티테넌트
환경에서는 고정 병렬도가 어느 시점에는 과하고 어느 시점에는 모자라기** 때문이다. 병렬도 고정
교환 연산자를 그 이유로 물린다.

### 이 설계 전체가 고객의 성질 위에 서 있다

객체 저장소의 35~100밀리초를 받아들이는 근거가 성능이 아니라 **누가 쓰느냐**다 — 언어 모델을 여러
번 호출해 종단 지연이 이미 긴 워크로드. 뒤집어 말하면 그 지연이 문제가 되는 곳에는 이 제품을
못 판다는 뜻이고, 회사도 그것을 감추지 않는다(재구성).

### 다음 사이클 — 확장 (Chroma 의 빈 도메인)

`wal3` 는 읽어 뒀다 — 카프카·WarpStream·CDC 를 이름과 이유를 대며 물린 글이라 다음 기능으로 두껍다.
