...
**CyberAgent (JP · 광고) — 45번째. 기능 4개 · 도메인 4개. 열어 둔 도메인이 다 찼다. 큐 잔량 0/3.**

251 에서 **네 번째 도메인 `보는 일에 드는 값을 어떻게 치를 것인가`** 를 열고 **`alert-context`
(알림이 죽지 않게 한다)** 를 썼다. 결정 6 · 그림 3 · 생각 4 · 스택 7 · 난제 2 · ui 6핀.

**관측 3편 중 둘을 열었고 둘 다 단단했다.** 그리고 **묶이는 축이 예상과 달랐다** — 도구가 아니라
**값을 무엇으로 치르느냐**였다(이 정리는 이 사이트의 것이다).
- **Vector(60707)는 돈으로 친다** — 관리형 Datadog Observability Pipelines 를 버린 이유 셋 중 첫째가
  **이중 과금**(*"취득 요금이 $0.095/GB"* + Datadog Logs **$0.10/GB**, *"토털 코스트가 높아져 버릴 것
  같습니다"*), 나머지는 **IaC 미성숙**(*"API 자체가 preview 인 것 같아 신청하지 않으면 쓸 수 없는 것
  같았습니다"*)과 **표현력**. 자기 수치가 촘촘하다 — **초당 2만 이벤트**를 **Pod 당 0.5 vCPU · 메모리
  500MB 로 4 Pod**, VRL **400줄 조금 못 미침**, 검증에서 **3000 req/s 를 0.1 vCPU · 30MB**.
  **실패도 적는다** — *"검증 과정에서 몇 번 Vector 를 크래시시켜 버렸습니다."*
- **Grafana Alert(61628)는 주의력으로 친다** — 회사가 문제에 이름을 붙였다: ***"알림의 형해화"***.
  *"Slack 의 알림 채널은 수십~수백 건의 'Pod 이상' 통지로 가득 찹니다"* 였고 실제로 놓쳤다 —
  *"일부 중요하지 않은 배치 잡이 계속 실패하고 있다는 것을 알아채는 것이 늦어진 경우도 있었습니다."*

**🔴 이 기능의 축 — 흔한 답과 반대로 갔다.** 보통은 **평소 켜 두고 정비 시간에 끄는데**(PagerDuty
Maintenance Window · Alertmanager Silence), 회사가 그것을 못 쓰는 이유를 적는다: *"업그레이드의
시작·종료 시각이 예측하기 어려운 GKE 의 사양상 이것들을 정적으로 설정하는 것은 곤란합니다."*
그래서 **평소 꺼 두고 이벤트가 오면 켠다** — **모르는 것이 시각이지 사건이 아니라는 점을 이용했다.**

**🔴 그리고 관측이 두 계열로 갈려 있다** — 광고 배송은 **Datadog**(Agent · Vector · APM), 워크로드
감시는 **Grafana Alert + PromQL**. **두 글이 서로를 언급하지 않는다.** `open_questions` 에 남겼다.

**🚨 다음 사이클 — Vector 기능을 쓰고 CyberAgent 를 닫는다.**
재료는 **60707 하나**(이미 읽었고 **도메인 `tech` 에 요약이 들어 있다**). **⚠️ 원문을 다시 읽는다**
(운영 사실 ⑫ — 223·229·243·249·250 **다섯 번 다** 새 결정을 찾았다). **특히 볼 것**: Vector 이전에
무엇을 썼는지(글에 명시가 없었다), HAProxy 로 커넥션을 L7 분산한 이유, VRL 400줄이 무엇을 하는지,
버퍼링 설정과 유실 처리. **결정이 5개 나오면 쓰고, 안 되면 도메인 `tech` 에 둔 채 닫는다.**

**닫을 때 `business_model` 에 반복 패턴을 적는다. 근거가 다섯이 됐다**(이 사이트의 정리로 밝힐 것):
① **버린 대안과 대가는 잘 적는데 자기 개선 폭은 반만 적는다** — 캐시(65145)·Vector(60707)·고속화
(64498)는 촘촘하고, 페이싱(63908)은 숫자가 **전부 타사 참고문헌**, 이설(61355)은 **글 전체에 셋뿐**,
알림(61628)은 **고치기 전만 세고 고친 뒤를 안 센다**. **버린 대안의 수와 수치의 유무가 상관없다.**
② **같은 시스템의 다른 층을 다루는 글들이 서로를 전혀 언급하지 않는다** — 페이싱↔고속화↔이설,
광고 캐시↔ABEMA 본체 캐시, 이설↔캐시, **Datadog 계열↔Grafana 계열**.
...
**⚠️ 큐가 0 이다 — CyberAgent 를 닫으면 곧바로 후보 조사다.**
...
**📌 비교 문서에 붙을 재료**
- `되돌릴 수 있는 것을 먼저 한다`(reversible-first) — **CyberAgent 이설이 반례**다. 그 문서는
  *"되돌림을 실제로 쓴 기록이 거의 없다"* 로 끝나는데 **여기는 되돌림 계획 자체가 없다.**
- **새 축 후보** — `알림을 어떻게 살려 두는가` 또는 `관측에 값을 무엇으로 치르는가`.
  CyberAgent `alert-context`(주의력) + Vector(돈) + Yelp `partition-access`(재려고 만든 것이 새 비용이
  된다 — 접근 로그가 하루 TiB) + Zerodha `log-storage`(색인 대신 압축) + Doximity `prod-profiling`
  (*"프로파일러는 쓰기에 너무 만족스러워서 첫 수로 삼고 싶어진다. 그러면 안 된다"*). **재료가 넉넉하다.**

**📌 이 회사에서 굳어진 관찰** — 위 `business_model` 패턴 ①②와 같다.
