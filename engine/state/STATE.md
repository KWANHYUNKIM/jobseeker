...
**Plaid** (US · 핀테크) — **프로파일까지 썼다.** 도메인 2개, 기능은 아직 0이다. **회사 87개.**

### 이번 사이클 — 신규(6순위), Plaid 프로파일

⚠️ **접근 지도를 두 번 고쳐야 했다** — `engineering.plaid.com`(Medium, **목록이 비어 있다**)로 접었다가 `plaid.com/blog` 를 찾았는데, **그것도 Medium 개별 글로 리다이렉트된다.** **다만 개별 글은 브라우저로 전문이 읽힌다.** **`목록이 막힌 것` 과 `본문이 막힌 것` 은 여전히 다르다.**

**TiDB 이전 글 전문을 읽었고 자료가 대단하다.**

- ⚠️ **떠날 이유가 다섯인데 그중 하나가 `우리가 쓰는 시간` 이다** — **매년 2 엔지니어-년을 Aurora 의 한계를 우회하는 설계에 썼다.** 큰 테이블에서 **약 12k writes/sec 병목**을 만났고, 온라인 스키마 변경이 무거워 **팀들이 기존 테이블을 고치는 대신 새 테이블을 추가**했다.
- ⚠️ **업그레이드를 이전으로 바꿔치기했다** — MySQL 5.7 지원이 끝나 5.7→8.0 이 큰 일이 될 것이라(**페이스북의 8.0 업그레이드를 보고 판단**) **그 노력을 플랫폼 이전에 썼다.**
- ⚠️ **설계 제약을 스스로 걸었다** — `이 프로젝트는 전사적으로 2년을 넘길 수 없다`. 그리고 **분기 하나 연구, 분기 하나 프로토타이핑**으로 시작했다.
- **신뢰성 요구가 사업에서 나온다** — 위에 얹힌 앱들이 자기 사용자에게 SLA 를 약속하므로 **DB 계층에 99.995%** 를 건다(구글 SRE 관행대로 **9 를 하나 더**).

세운 도메인 둘:
- **관계형 저장소를 통째로 갈아 끼우되 2년을 넘기지 않는다** — tech 5개를 채웠다.
- ⚠️ **남의 은행 1만 2,000곳에 대신 닿는다** — **이 회사의 본체인데 tech 가 0 이다**(검증 경고). **다음 사이클에 자료부터 찾는다.**

### 다음 사이클 — 확장

**TiDB 도메인이 압도적으로 익었다** — 4단계 이전 절차(비호환 제거 · 복제 · 검증 · 전환), **롤백 클러스터로 앞으로 실패하기**, **동적 런북(Deno·Jupyter)으로 컷오버 5배 가속**, 그리고 **잘된 것과 아쉬운 것을 각각 다섯 개씩** 적는다.

⏳ **history 2세대를 세울 수 있다** — Aurora MySQL(~2023) → TiDB(2023~). 타임라인이 분기 단위로 적혀 있다.
