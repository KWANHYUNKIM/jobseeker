...
**없다 — 일흔 번째 후보 조사를 돌았다. 큐 2/3.** 다음 사이클은 **신규**(뱅크샐러드).

### 첫 화면으로 판정하지 않는다가 또 값을 했다

뱅크샐러드 블로그 첫 화면에는 기술 글이 **세 편**뿐이었다. **'자료가 얇다'로 접을 뻔했는데
사이트맵을 열어 보니 `/tech/` 글이 58편**이었다. 이 엔진이 이미 여러 번 적어 둔 기준인데
이번에도 그대로 걸릴 뻔했다.

### 주소를 추측하지 않는다도 값을 했다

요약이 준 `corp.banksalad.com/tech/...` 가 **404** 였다. 목록 페이지에서 href 를 받아
`blog.banksalad.com/tech/...` 로 고쳤다 — **추측했으면 통과할 회사를 '못 읽음'으로
접었을 것이다.**

### 한국 축이 다시 두꺼워진다

토스·카카오뱅크 옆에 **뱅크샐러드**(마이데이터)와 **카카오페이**(결제·보험)가 붙는다.
둘 다 **자기가 낸 값을 수치로 적는 쪽**이다 — 뱅크샐러드는 **정합성 이슈 0건**, 카카오페이는
**처리량 20% 감소와 P99 두 배**를 그대로 낸다.

### 값을 내주고 안전을 사는 거래

카카오페이의 뼈대가 그것이다 — **AOP Shadow 비교로 옛 로직과 새 로직을 나란히 돌리는
대신 처리량 20%를 내준다.** 이 원장에서 **'무엇을 얼마나 내주는지'를 벤치마크로 다 적은
사례**는 드물다.

### 다음 사이클

**신규 — 뱅크샐러드.** 큐 2/3 이라 신규 두 번 뒤에 조사가 다시 뜬다.
