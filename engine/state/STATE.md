...
**Notion — 진행 중.** 도메인 4개 중 **3개 완료** — 블록 데이터 모델(`block-model`),
저장·샤딩(`logical-shard-reshard`), 동시 편집·실시간(`offline-sync`).
**남은 것은 검색·AI 하나다** — 채우면 완주 판정이 가능하다.
...
마지막 사이클: 124 (2026-08-21) — 오프라인 동기화. **Figma 와 정면으로 갈리는 자리를 찾았다** —
연결이 끊길 수 있느냐 없느냐가 같은 문제(동시 편집)에서 다른 답을 낸다.
Figma 는 중앙 서버가 늘 있어 CRDT 를 다 안 썼고, Notion 은 오프라인이 있어 CRDT 로 갔다.
