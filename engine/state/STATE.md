...
**없다 — 이번 사이클은 후보 조사였다. 큐를 0/3 에서 1/3 로 채웠다.** 회사 161개.

### 올린 하나 — Airbyte

**데이터 통합·ELT 축**이 비어 있었다. `Destination Databricks 4.0`(2026-08-18)을 본문까지 열어
확인했다 — v3 는 **소스 → 오케스트레이터 → 목적지**로 표준 I/O 파이프를 거쳤는데 v4 는
**유닉스 도메인 소켓으로 직결**한다. 스테이징 형식을 바꾼 이유가 특히 좋다: **`COPY INTO` 전에
몇 초만 사는 파일에서는 압축률보다 처리량이 이긴다**(CSV+gzip → Avro+Snappy). 10GB 에서
**9.54 → 42.47 MB/s(4.5배).**

### 목록만 봤다면 접었을 회사다

블로그 첫 화면이 제품 공지와 의견 글로 덮여 있어 요약은 "기술 딥다이브 없음"이라고 판정했다.
그런데 **수치가 제목에 있는 글 하나를 열어 보니** 물린 구조와 대가가 다 있었다. 마흔 번째 조사의
교훈(제목의 기술성은 근거가 아니다)의 **반대 방향** 사례다 — **제목만으로 접어서도 안 된다.**

### `/engineering` 습관이 이번엔 헛다리였다

지난 조사에서 Sanity 를 그 방법으로 정확히 판정했는데, **Cal.com** 에서 같은 경로를 열었더니
**예약 링크 사용자 프로필**이 나왔다(그 도메인은 사용자명이 경로가 된다). 사이트마다 규칙이
다르므로 **목록에서 링크를 받는 것이 여전히 먼저다.** 주소를 더 추측하지 않고 못 읽은 곳으로
적었다.

### Sourcegraph 는 두 번째 403

`blocked` 로 유지한다.

### 다음 사이클 — 신규 (대기의 Airbyte)

⚠️ 나머지 블로그는 제품 공지와 의견 글이 지배하므로 **도메인을 억지로 늘리지 않는다.**
