...
**Meituan(美团) — 52번째 회사, `in_progress`. 도메인 1개 · 기능 0개.**
299 는 회사 프로파일 사이클이었다(240 에서처럼 기능은 다음 사이클로 미뤘다).

**⚠️ ⑲ 를 먼저 돌렸다** — `meituan` 이 완료 목록에 없고 파일도 없다는 것을 **스크립트 안에서
assert 로 막아 두고** 썼다(282 사고 재발 방지).

**실적을 확보했다** — **2026년 2분기 매출 1,050억 위안(전년 대비 +14.4%)** 으로 시장 전망
1,010억 위안을 넘겼고 **조정 순이익 25억 위안(약 3억 7,200만 달러)** 으로 **3분기 연속 적자를
끊었다**(2026-08-28 발표). **직전 1분기는 매출 910억 위안(+5.6%)에 적자였다.**
**⭐ 흑자 전환의 이유가 기술이 아니라 경쟁의 완화다** — 2025년 초 타오바오·징둥이 즉시 배송에
뛰어들며 시작된 가격 경쟁이 식자 **보조금을 줄이고 객단가 높은 고객에 집중**할 수 있었다.
⚠️ **부문별 매출 구성은 확인하지 못했다.**

**연 도메인 하나 — `검색이 무엇을 위로 올릴지 정한다`** (`tech` 채웠다).
근거는 298 에서 읽은 `美团搜索3.0`(2026-08-20). 버린 대안 셋 · *"四个明显短板"* ·
**결론의 유효 기간까지 밝힌다**(*"这一结论有阶段局限性"*). 수치도 촘촘하다.

**⭐ 목록을 훑어 얻은 것 — 이 회사 글의 무게중심이 본업이 아니다**(이 관찰은 이 사이트의
것이다). 최근 목록에서 **LongCat 계열 자체 모델이 여덟 편 이상**으로 가장 많고, **평가
벤치마크 여섯 편 이상**, **`可信实验白皮书` 연속 여덟 편**, **데이터베이스 자율 운영 여섯 편
이상**이다. **정작 음식 배달의 배차·경로·지도 이야기는 최근 목록에 안 보인다**(추천은 있다 —
`MTGR`, 2025-05-19). `open_questions` 에 남겼다.

**다음 사이클 — 도메인 ① 의 기능을 쓴다. 두 번째 편이 필요하다.**
목록에서 확인한 같은 도메인 글 — `美团搜索粗排优化的探索与实践`(2022-08-11) ·
`大众点评搜索相关性技术探索与实践`(2022-07-06). **주소는 아직 모른다 — ⑱ 을 건다**
(`WebSearch` + `allowed_domains: ["tech.meituan.com"]`). **⑰ — 추측하지 않는다.**
**⚠️ 둘 다 2022년이라 낡았을 수 있다.** 얇으면 억지로 쓰지 말고 **도메인 후보 넷 중 하나를
연다**(아래). ⑫ 대로 **열어 보고 판단한다.**

**📌 299 에서 제목·주소만 확인해 둔 도메인 후보 넷**(본문 안 읽음)
1. **데이터베이스를 스스로 돌보게 하기** — `超大规模数据库集群保稳系列`(1·2·3편) ·
   `MySQL自治平台建设的内核原理及实践`(상·하) · `从0到1建设美团数据库容量评估系统`(2025-10-17) ·
   `美团大规模KV存储挑战与架构实践`(2024-03-15).
   **주소 확인됨** — 保稳系列 1편 `/2023/05/11/meituan-high-availability-system.html` ·
   MySQL 자치 상편 `/2023/07/06/meituan-mysql-autonomous-platform-01.html` ·
   태그 목록 `/tags/数据库.html`. **가장 두꺼운 후보다.**
2. **실험을 믿을 수 있게 만들기** — `可信实验白皮书` **연속 여덟 편**(2025-05~08).
   **주소 확인됨** — 1편 `/2025/05/22/meituan-ab-online-controlled-experiment-01.html`.
3. **무엇을 잘한다고 부를지 재기** — 평가 벤치마크 여섯 편 이상(LoHoSearch · VitaBench 2.0 ·
   WBench · AMO-Bench · UNO-Bench · `Agent评测漫谈`).
4. **자체 대규모 모델** — LongCat 계열 여덟 편 이상(국산 칩에서 돌린 조 단위 파라미터 모델 포함).
...
- **⚠️ 분포** — 완료 51 + 진행 1 = **52곳**. **한국 편중 아홉.** **빈 자리는 라틴아메리카
  하나뿐이고 재시도 금지.** 동남아는 Grab 하나인데 **Shopee·Tokopedia·VNG 가 298 에서 다
  막혔다.** **중국은 이제 ByteDance 와 Meituan 둘이다.**
- **큐 1/3** — Meituan 을 다 판 뒤 **후보 조사를 한 번 더 끼운다**(`--gaps` 가 그렇게 안내한다).
- **⭐⭐⭐ 비교 문서가 계속 밀려 있다.** `에이전트에게 무엇을 못 하게 하는가` — 재료 일곱,
  세 갈래: **병합 제한**(monday.com Morphex · Snap CodePal · Snap Casper) ·
  **접근·행동 제한**(Wix AirBot · Wix 자가치유 두뇌) · **반대편**(Wix Agent Ready Repos ·
  Wix 컴파일러 문지기 · Adevinta). **⭐ Wix 하나가 세 갈래에 다 걸린다.**
