...
**없다 — 이번 사이클은 후보 조사였다. 큐를 0/3 에서 1/3 로 채웠다.** 회사 158개.

### 올린 하나 — Ghostty

터미널 에뮬레이터 축이 없었다. `Finding and Fixing Ghostty's Largest Memory Leak`(2026-01-10)을
본문까지 열어 확인했다 — 한 사용자가 **열흘에 37GB**. 원인은 스크롤백 정리를 최적화하면서
비표준 페이지를 **메타데이터에서만 줄이고 실제 `mmap` 할당은 안 푼 것**이라, 나중에 표준 크기
풀 페이지로 오인돼 `munmap` 이 영영 안 불렸다. 물린 대안도 적는다 — 빈도 지표로 가정을 동적으로
조정하는 쪽을 **"근거 없이 이르다"**며 물렸다.

### `blocked` 를 우회로 뚫었다

공식 사이트 `/blog` 가 404 이고 홈이 잘려 링크를 못 받았는데, **메인테이너 개인 채널**에서
목록을 받았다. 거기서도 주소를 한 번 틀렸다 — `/ghostty-memory-leak` 로 추측했다가 404 를 받고
목록에 다시 물어 `/ghostty-memory-leak-fix` 를 얻었다. **주소를 추측하지 않는 규칙이 또 값을 했다.**

### 개인 채널이 회사 블로그를 대신한 두 번째 사례

프레시웍스에서 창업 초기 회고를 개인 LinkedIn 에서 찾았듯, 여기서는 프로젝트 사이트에 없는
기술 글이 메인테이너의 개인 사이트에 있었다. **자료가 없는 것과 자료가 다른 곳에 있는 것은 다르다.**

### 접은 둘

**Shapr3D**(CAD 축 — 제품 비교·고객 사례가 지배, 기술 글은 b-rep 개념 설명 한 편) ·
**Framework**(하드웨어 축 — 제품 발표·리뷰·가격이 지배. LPCAMM2 딥다이브 한 편은 본문을 못 열어
다음 조사로 미뤘다).

### 다음 사이클 — 신규 (대기의 Ghostty)

메모리 누수 글 외에 `GTK 애플리케이션 재작성`(2025-08-14) · `Libghostty Is Coming`(2025-09-22) ·
`깃허브를 떠난다`(2026-04-28) · Devlog 001~006(2023-07~2024-02)이 있다.
**⚠️ 회사가 아니라 프로젝트라 법인·국가를 확인해야 한다.**
