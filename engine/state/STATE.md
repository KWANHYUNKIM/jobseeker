...
**Snyk** (UK/IL · SaaS — 개발자 보안) — **프로파일까지 썼다**(PROMPT 3단계). 도메인 2개 · 기능 0. 회사 **92개.** ⚠️ **큐가 0/3 이 됐다.**

### 이번 사이클 — 신규(6순위). 큐의 마지막 한 곳을 올렸다

⚠️ **이 회사의 축은 `찾기` 가 아니라 `고치기` 다.** 스캐너는 시장에 많고, **문제를 목록으로 돌려주는 것과 그 문제를 없애는 코드를 써 주는 것은 다른 일이다**(재구성). **그 둘 사이에 이 회사가 놓은 것이 자기 데이터베이스** — **전문가가 쓴 실제 수정 3만 5천 건 이상**이다.

**도메인 둘** — ① **취약점을 찾는 것이 아니라 고치는 것을 판다**(✅ 본문 확인, 2026-04-27) · ② **기계가 고친 것을 믿어도 되는지 잰다**(✅ 본문 확인). **둘 다 자료를 읽고 세웠다.**

**규모** — ARR **약 3억 2,600만 달러**(2026년 2월) · 고객 **약 5,000곳** · 개발자 **250만 명 이상** · **Snyk Code 하나가 ARR 1억 달러 이상.**

### 읽어 둔 것 — 두 글에서 건진 것 넷 (다음 사이클의 재료)

- ⚠️ **버린 방식이 명시적이다** — 예전에는 **새 언어나 새 규칙마다 수천 개의 수동 학습 표본과 긴 파인튜닝**이 필요했다. 지금은 **추론 시점에 그 CWE 가 예전에 어떻게 고쳐졌는지를 넣어 주는 동적 few-shot** 이다.
- ⚠️ **버린 것의 대가도 적는다** — 예전에는 **안전하지 않은 코드가 나오면 그냥 걸러내 개발자에게 아무 수정도 안 남겼다.** 지금은 **실패한 수정을 버리는 대신 오류를 되먹여 다시 생각하게 한다**(agentic retries).
- ⚠️ **보안 수정에는 관문이 둘이다** — **`대부분의 코드 벤치마크는 코드가 도는지, 버그 리포트를 해결하는지를 본다. 보안 수정에는 둘 다 부족하다.`** **SQL 주입을 없앴는데 질의 결과가 달라졌으면 해결이 아니고, 깨끗해 보이는데 취약점이 남으면 오히려 해롭다.**
- ⚠️ **자기 벤치마크의 한계를 넷 밝힌다** — 표본이 작아 **2점 미만 차이는 못 믿고**, **단일 파일 조각이라 파일을 넘나드는 맥락을 못 재고**, **세 언어뿐이고**, **분산을 두 번 실행으로만 봤다.** 그리고 **`인텔리전스의 10점 이상 격차는 실행 간 잡음 밖이지만, 언어별 작은 차이의 순서는 아니다`** 라고 적는다.

### 다음 사이클 — 확장(2순위)

`--gaps` 가 **`취약점을 찾는 것이 아니라 고치는 것을 판다`** 를 부를 것이다. **자료를 이미 읽었으니 바로 기능을 쓴다.**

⚠️ **큐가 0/3 이다.** 이 회사를 다 판 뒤 **3순위(후보 조사)** 가 걸린다. ⏳ 되짚을 목록 — **`bytes.swiggy.com`**(⚠️ `r.jina.ai` 422, 브라우저가 네 사이클째 응답 없음) · `www.gojek.io/blog`(429) · `tech.cygames.co.jp` · `www.scylladb.com/blog`.

### ⚠️ 시간축을 못 세운 세 번째 사례

**StarCoder 기반 → 프론티어 모델 + 인텔리전스** 라는 세대 교체가 벤치마크 표에서 분명한데 **연도가 없어 `history` 를 비웠다.** **GeoIndia V1→V2, Flink→ClickHouse 에 이어 셋째다.** ⚠️ **`무엇에서 무엇으로` 는 아는데 `언제` 를 모르는 경우가 계속 쌓인다** — 세 번 다 open_questions 에 적어 정보는 남겼다.

### ⚠️ 비교 문서 재료 (초안 유지)

**① `AI 에이전트를 어디까지 믿나` — 이제 7곳이 될 수 있다** — Sentry / ClickHouse / Duolingo / Ramp / DoorDash / Deliveroo / **Snyk**. ⚠️ **Snyk 의 자리가 다르다** — 앞의 여섯은 **에이전트가 만든 코드를 어떻게 감독할까**를 묻는데 **여기서는 AI 가 고치는 대상이 보안 취약점 자체**다. **그리고 이 회사만 `기계가 고친 것을 어떻게 잴까` 에 벤치마크로 답한다.**

**② `관리형 MySQL 의 한계` 3곳** — Etsy / Plaid / Paystack.

**③ `실험 결과를 얼마나 믿나` 2곳** — Etsy / Deliveroo.
