...
**없다 — 문체 보수 사이클이었다.** Sentry 를 배민 문체로 다시 썼다(2.71 → 0.0). 회사 115개, 큐 2/3. 43곳 남았다.

### 두 회사가 정확히 같은 칸에서 멈췄는데 근거가 다르다

첫 기능에 Block 과의 대비를 넣었다. 사람을 어디에 남기나 축의 새 자리다.

- Block 의 자가치유 루프는 병합과 티켓 종료만은 에이전트에게 안 넘긴다.
- Sentry 의 Seer 도 근본 원인 분석과 수정 계획과 PR 개설까지 가고 병합에서 멈춘다.

**같은 칸인데 근거를 대는 방식이 반대다.** Block 은 병합할 수 없는 형태로 남긴다는 구조로 말하고,
Sentry 는 조직이 기계를 거기까지 믿을 준비가 안 됐을 수 있다고 적는다. 못 맡기는 것이 아니라
아직 안 믿는 것이라고 쓰는 회사는 드물다.

### 에이전트를 지켜보는 두 가지 답 — 옮기거나, 하나 더 두거나

둘째 기능에 Razorpay 와의 대비를 넣었다. 둘 다 자기 에이전트에 눈이 없다는 것을 인정한다.

- Razorpay 는 노트북 방식의 한계 셋 중 하나로 관측 없음을 꼽고 쿠버네티스로 **옮겼다**.
- Sentry 는 매일 밤 대화 800건을 훑는 **심문용 에이전트를 하나 더 뒀다**.

관측을 파는 회사가 자기 에이전트에는 그 눈이 없었다는 것이 이 기능의 출발점이다.
핵심 발견도 거기서 나온다 — 도구 오류가 난 대화만 보면 안 되고 성공한 대화도 표본에 넣어야
한다. 도구가 다 성공해도 결론은 틀릴 수 있기 때문이다.

### 도메인이 회사의 뿌리가 아닌 이유를 적었다

Sentry 의 뿌리는 오류 추적인데 도메인 둘이 다 AI 다. 2026년에 회사가 스스로 쓴 시스템 글이
거기 몰려 있어서다. **안 세운 것은 자료를 아직 안 판 것이지 없다고 판단해서가 아니라는 문장을
사업 모델에 넣었다** — 앞 사이클의 Razorpay(결제 회사인데 도메인에 결제가 없다)와 같은 처리다.

### 다음 사이클 — 문체 보수 (사다리 3순위)

--gaps 가 정한다. 지금 맨 위는 block(2.65, 290개)이지만 그건 이 시점의 예상이지 확정이 아니다.
