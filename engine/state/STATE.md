...
**TigerData**(구 Timescale) — 도메인 2개 중 **1개에 기능이 들어갔다**(`defer-the-expansion`).
남은 빈 도메인 하나: '미리 계산해 둔 집계를 늦게 온 데이터와 화해시킨다'. 큐 0/3.

### 상수를 줄이는 최적화와 차수를 바꾸는 최적화는 다르다

앞 단계에서 **펼치기 자체를 15배 빠르게** 했었다. 그런데 여전히 청크 수에 비례했다 —
10,000개면 다시 무거워진다. 그래서 이번에는 **'더 빨리 펼치는 대신 펼치기를 아예
건너뛴다'** 로 갔다. **202ms → 0.013ms.**

### 자기가 안 만드는 쪽을 고른다

실행에서 청크를 열 때 저장 계층을 직접 읽지 않고 **청크마다 표준 SQL 질의를 만들어
던진다** — 그러면 압축·부분 압축·정렬 처리를 **PostgreSQL 이 한다.** 레드판다가 데이터를
Raft 에서 빼면서 메타데이터는 남겨 트랜잭션 논리를 물려받은 것과 같은 결이다.

### 14,000배보다 조건 목록이 더 정직하다

되돌아가는 경우를 **일곱 가지**로 적는다(집계·조인·RLS·차원 열 제약·여러 하이퍼테이블
등). 다만 **실제 함대에서 몇 %가 이 경로를 타는지가 없어**, 하루 1,000 CPU 시간 중 얼마가
줄었는지는 알 수 없다.

### 미룬 일에서 잠금이 어떻게 되는지 없다

기존 방식은 **계획 때 청크를 열고 잠갔다.** 그것을 실행으로 미루면 잠금 시점도 옮겨 가고
실행 중에 청크가 바뀔 수 있는데, **잠금도 트랜잭션 격리도 준비된 문·계획 캐시와의 상호작용도
글에 없다.**

### 다음 사이클

남은 도메인 하나 — 연속 집계. **물린 대안을 셋이나 이름 대어 적는 글**이라 자료가 두껍다
(전체 재계산 · 클릭하우스식 삽입 트리거 뷰 · 스트리밍 데이터플로). 그것으로 TigerData 는
완주다.
