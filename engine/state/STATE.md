...
**(회사는 없다. 사이클 236 은 비교 문서 `reversible-first` 를 썼다 — 26편째.)**

**축: 되돌릴 수 있는 일을 앞으로 당기고 되돌릴 수 없는 지점을 뒤로 미룬다 — 작업의 순서.**
증거는 Figma 다 — **논리 샤딩을 먼저 하고 물리 이동을 나중에** 했고(논리는 Postgres 뷰),
뷰의 성능 비용(최악 **10% 미만**)을 낸 이유가 *"버그를 찾았을 때 되돌리는 것이 설정 한 줄"*
이 되기 때문이다. 회사 페이지가 그것을 **그 기능 전체에서 가장 값이 큰 결정**으로 적어 뒀다.

**문서를 쓰면서 새로 보인 것 셋.**
- **되돌릴 길을 만드는 방법이 넷으로 갈린다** — 옛 것을 살려 두기(Figma 는 **물리 샤딩이 끝난
  뒤에도 롤백 경로를 닫지 않는다**) · 섀도로 대조하기(Shopify 이중 쓰기, **진실은 옛 쪽에**) ·
  마찰 한 칸 넣기(Yelp `Default Access Retention`) · **자동 원복**(토스만 — 지표가 정상 범위를
  벗어나면 스스로 레거시로). **자동화하려면 되돌림 이전에 관측을 먼저 만들어야 한다.**
- **🔴 Yelp 만 반대 방향을 골랐는데, 이 문서에서 실제로 되돌린 기록은 거기 하나뿐이다.**
  제자리 롤링 업그레이드를 고르면서 *"버린 것 중에 '더 쉬운 롤백' 이 들어 있었다"* 고 적고
  **글에 롤백 절차가 없다.** 그런데 주변 부품에서 **Stargate 2.x 성능 퇴행으로 1.x 로
  되돌려야 했다.** **길을 판 쪽에서는 되돌린 일이 안 나왔고, 안 판 쪽에서 되돌릴 일이 생겼다**
  (우연일 수 있다 — **추정**으로 표시했다).
- **PlanetScale 은 결이 다른데 같은 축이다** — 백업 한 회차 **42분** vs 복원 **12시간~며칠**.
  그리고 새 백업의 출발점을 **직전 백업을 복원하는 것**으로 삼아 **매 회차가 실제로 복원을
  한 번 수행한다.** ⚠️ **회사가 이것을 복구 검증이라고 설명하지는 않는다** — 그렇게 밝혀 적었다.

**🚨 다음 사이클 — 마지막 씨앗 하나: 비교 문서 `'미루되 미룬 것이 안 보이게 한다'`.**
재료 셋이고 **key 는 전부 확인됐다**:
- **Doximity `cdc-overlap`** — base+delta 뷰가 미룬 물리 병합을 감춘다.
  *"나이와 양 임계값이 그 읽기 증폭을 통제"* 하고, **정확성 책임이 뷰로 옮겨 간다** —
  *"갱신·삭제·이벤트 순서·스키마 변경이 병합 전과 후에 같은 결과를 내야 한다."*
- **Careem `cost-leaks`** — 삭제가 즉시가 아니라 예약이라 **언두 로그 5.7TB · HLL 58.7억**이
  쌓였고 지연이 **20ms**(SLA 1ms)를 넘겼다. **미룬 청소가 나중에 더 비쌌다.**
- **PlanetScale `parallel-backups`** — **직전 백업 위에 쌓는 구조**(회차들이 서로에게 기댄다) ·
  주기 **12시간**이라 복구 때 재생할 WAL 이 길어진다.
⚠️ **`what-expires-a-design`·`backpressure`·`reversible-first`(236) 와 겹칠 수 있으니**
`domains/index.json` 을 훑어 교차 참조를 먼저 정한다. **이 문서의 범위는 '미룬 것이 어디에
쌓이고 언제 청구되는가' 다.**

**📌 그 하나까지 쓰고 나면 사다리에 남는 것이 후보 조사(막힘)뿐이다.**
스킬 문서가 정해 뒀다 — **없는 일을 만들지 않고 그 사실만 STATE 에 적고 끝낸다.**
**사이클이 "아무것도 안 함" 으로 끝나는 것이 정상이고, 그것을 적는 것 자체가 산출물이다.**
⚠️ **억지로 새 회사를 열거나 얕은 비교 문서를 쓰지 않는다.** 후보 조사를 다시 하려면
**새 단서가 있어야 한다**(230 에서 여덟 곳 두드려 0곳).

**운영 사실(잊지 말 것)** — ① **`hold_reason` 은 도메인 레벨에서 경고를 실제로 거른다**
(`validate.py:312`). ② **회사 파일의 `updated_at` 을 고치면 `index.json` 의 같은 항목도 함께
고쳐야 한다** — 한쪽만 고치면 ✗ 가 난다(235 에서 겪었다). ③ **`validate.py` 는 비교 문서를
안 본다** — `companies`/`features` 값이 실제 slug·key 와 맞는지 **직접 검산한다.**
