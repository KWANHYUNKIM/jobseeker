...
**Temporal(US · SaaS—내구성 있는 실행)** `in_progress` · **도메인 3 · 기능 2** —
76곳 · ⚠️ 큐 0/3.

방금 쓴 기능: **`shard-count-is-forever`(하나를 키우지 않고 개수를 늘린다 — 대신 샤드
수는 처음에 정해야 한다)**. 결정 7개가 **전부 `confirmed`**.

### ⚠️ 앞 사이클의 판단을 정정했다 — 이 회사도 수치를 적는다

앞에서 **`구조와 제약은 잘 적는데 결과를 숫자로 안 적는 회사`** 라고 STATE 에 적었는데,
**`scaling-temporal-the-basics` 에는 수치가 다 있었다** — 샤드 권장값(**512 / 4,096**) ·
**샤드 락 지연 50ms → 1ms** · 상태 전이 **150/s → 1,350/s** · **DB CPU 80% 목표에 79%** ·
요청 지연 SLO 150ms 에 **50ms 미만**. **한 편으로 회사를 판단하면 안 되는 사례가
이 회사에서도 나왔다**(여섯 번째다). `open_questions` 맨 위에 그 정정을 적었다.

### ⭐⭐ 되돌릴 수 없는 결정을 시작 시점에 밀어 놓았다

**샤드 수는 클러스터를 세운 뒤 바꿀 수 없다** — 회사가 **유일하게 못 바꾸는 설정**이라
못 박는다(`the one configuration setting which cannot (currently) be changed`).
그런데 그 수가 **락 지연과 처리량을 정한다.** 답은 **넉넉히 잡는 것**뿐이다.
`hard_problems` 로 세웠고 ⚠️ **`(currently)` 라는 단서만 있고 재샤딩 계획은 글에 없다.**

### ⭐ 경계 안에서 끝내고 밖으로는 나중에 흘린다

상태를 샤드에 묶어 트랜잭션을 얻은 대가로 **밖에는 트랜잭션이 없다.**
그래서 **2단계 커밋·Paxos·Raft 를 안 쓰고** Transfer Queue 로 푼다 — 경계 안에서
원자적으로 커밋하고 별도 스레드가 나중에 옮긴다. **지연은 주되 유실은 안 준다**
(커밋됐다면 인덱스는 결국 갱신된다).

### 다음 사이클 — 마지막 도메인

**관리형에서는 영속 계층을 직접 만든다** — `...custom-persistence-layer`(요약 확보).
동적 샤딩 · 쓰기 선행 로그 · 계층 저장, 그리고 **시끄러운 이웃 문제**.
⚠️ **그 글에는 수치가 없다** — 대신 **병목이 DB 라는 사실**(이번 기능에서 확인)과
**요금이 활성·보존 저장으로 갈린 것**을 근거로 이을 수 있다.
여기까지 채우면 **완주 조건에 닿는다.**
