...
**Prisma — 프로파일만 썼다.** 회사 145개, 큐 2/3. 도메인 둘이 비어 있고 둘 다 자료를 확보했다.

### 러스트에서 나온 쪽

이 엔진에는 러스트로 **옮긴** 기록이 둘(Bun·Ladybird) 있었는데 이제 **나온** 기록이 생겼다.
처음 러스트를 고른 이유가 "되도록 여러 언어를 위한 ORM 을 만들고 싶어서"였는데, 오년 뒤
타입스크립트가 지배적이라 그 목적은 흐려지고 **언어 경계를 넘는 값만 남았다.**

### 크기가 곧 돌 수 있는 자리다

패키지가 약 14MB(gzip 7MB)면 서버리스 콜드 스타트에 부담이고, 큰 네이티브 바이너리는 엣지
런타임과 늘 맞지도 않는다. 운영체제·OpenSSL 버전마다 바이너리를 따로 내야 하는 것도 짐이었다.

질의 컴파일러로 바꾼 뒤 **약 1.6MB(gzip 600KB)로 85~90% 줄었고** 2만 5천 건 findMany 가
185ms → 55ms(3.4배)가 됐으며, 워커스·데노·번·엣지에서 돈다.

### 공짜로 까는 것이 파는 것의 입구다

ORM 은 "공짜이고 앞으로도 늘 그럴 것"이라 적고 관리형 데이터베이스·캐시·컴퓨트를 판다.
그래서 **그 계층이 어디서 도느냐가 곧 팔 자리의 넓이**가 된다 — 크기를 줄인 일이 기술이자
영업인 셈이다(재구성).

### 다음 사이클 — 확장 (Prisma 의 빈 도메인)

`질의를 어디서 만들지 정한다` 부터. 걷어낸 이유 넷과 새 구조, 수치가 다 손에 있다.
