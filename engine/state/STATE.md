...
**CedarDB — 회사 프로파일까지 썼다(in_progress).** 회사 121개, 큐 2/3.

도메인 셋을 세웠고 기능은 아직 0개다.

- **트랜잭션과 분석을 한 엔진에 둔다** — `CedarDB Launches HTAP (10 Years Ago)`(2026-06-29)
- **바이트를 줄이는 것과 질의를 빠르게 하는 것을 가른다** — `Encoding or Compression: Why not both?`(2026-08-04)
- **하드웨어에 맞춰 실행을 짠다** — 위 글에서 이름만 확인(모젤 병렬·데이터 중심 코드 생성·SSD 버퍼 관리자)

### 학술 배경으로 판 회사가 셋이 됐다

Materialize(차분 데이터플로) · DuckDB(CWI 암스테르담) · CedarDB(TU 뮌헨의 HyPer·Umbra). 셋 다
연구를 제품으로 옮긴 회사인데 오픈소스를 다루는 방식이 갈린다 — DuckDB 는 지식재산을 비영리 재단에
두고 회비로 개발비를 대고, 여기는 무료 커뮤니티 에디션을 입구로 두고 지원과 관리형으로 값을 받는다.

### 줄이는 것과 빠르게 하는 것은 다른 목표다

이 회사의 가장 두꺼운 글이 그 구분에서 출발한다. 범용 압축은 데이터의 뜻에 관심이 없어 잘 줄이지만
질의하려면 통째로 풀어야 하고, 뜻을 아는 인코딩은 덜 줄이지만 푼 적 없이 비교한다. 수치가 그 차이를
보인다 — 문자열 동등 필터가 사전 인코딩에서 0.4ms, zstd 레벨 3에서 64.1ms(168배). 압축률은 반대다
(URL 데이터에서 13.5배 대 19.6배).

그래서 답이 둘 다다. **언제나 인코딩된 채로 두고, 디스크에서 20% 이상 더 줄 때만 압축을 얹으며,
메모리 표현은 압축 여부와 무관하게 같다.** 이 엔진에서 상태의 바이트를 깎은 회사들과 견줄 자리다 —
Materialize 는 남는 태그 바이트를 사전으로 썼고(메모리 원가가 곧 청구서), 여기는 질의 가능한 표현을
지키는 것이 먼저다.

### 다음 사이클 — 확장 (CedarDB 의 빈 도메인)

인코딩/압축 글이 수치가 가장 두껍다. 컴파일 대 실행 글은 목록에 있는데 주소를 확인하지 못했다 —
주소를 지어내지 않는다는 규칙대로 다음에 목록을 다시 연다.
