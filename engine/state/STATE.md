...
**없다 — Bluesky 를 완주시켰다(도메인 2 · 기능 2 · `status: done`).** 회사 169개, **큐 0/3.**

### 문제를 권한의 크기가 아니라 층위로 봤다

중앙 집중 모더레이션을 **"모든 분쟁을 대법원 수준에서 해결하는 것"** 에 빗댄 것이 이
설계의 출발이다. 그렇게 보면 답이 '권한을 뺏자'가 아니라 **'층을 만들자'** 가 된다.

### 관점의 다양성은 선언이 아니라 문턱의 함수다

라벨러가 되는 데 **PDS 도 클라이언트 앱도 필요 없게** 했고 도구(Ozone)까지 오픈소스로 냈다.
문턱이 높으면 라벨러가 몇 개 안 생기고, 그러면 '여러 관점'이라는 전제 자체가 무너진다.

### 고를 수 있게 하되 안 고른 사람을 버리지 않는다

기본 모더레이션 층은 회사가 **24시간 운영**한다. 그런데 그 결과 **여전히 회사가 가장 큰
모더레이터**다 — '층으로 쌓는다'는 말이 그 사실을 가릴 수 있다는 것을 `thinking` 에 적었다.

### 열어 놓고 안 채운 칸이 크다

누구나 라벨을 붙일 수 있게 해 놓고 **평판도 남용 방지도 책임도 운영 비용도 다루지 않는다.**
2023년 글은 '아직 작업 중'이라 그럴 수 있는데 **2024년 도구 공개 글에서도 같은 칸이 비어
있다.** 페더레이션 글에도 규모·비용 수치가 하나도 없다.

### 이번 사이클에 검증이 사고를 막았다

기능 스크립트를 heredoc 으로 쓰다 깨지면서 **앞서 완주한 astral.json 이 이전 스크립트로
덮어써졌다.** `validate.py` 가 `index.json` 과의 불일치(`features_done=2 인데 상세엔 0개`)로
바로 잡았고, 커밋 전이라 `git checkout` 으로 되돌렸다. **긴 스크립트는 heredoc 대신 파일로
쓴다.**

### 심은 대비

**유튜브 콘텐츠 ID**(플랫폼이 중앙에서 대조·판정해 전 사용자에게 같은 결과) ↔ **Bluesky**
(판정을 여러 개 두고 사용자가 구독) — 같은 문제에 권한 구조가 정반대다.

### 다음 사이클 — 후보 조사 (큐가 0/3 이다)

**CAD 축과 의료 축은 여전히 비어 있다.** 못 읽은 곳: Hugging Face(별도 엔지니어링 섹션
없음) · Lunit(medium 403) · Blender(403) · dbt Labs · incident.io(404 네 번) · Guardian ·
