...
**없다 — 문체 보수 사이클이었다.** Booking.com 을 배민 문체로 다시 썼다(1.83 → 0.0). 회사 115개, 큐 2/3. 25곳 남았다.

### 무엇을 먼저 옮길지 고르는 네 번째 방식 — 가장 안전한 것

첫 기능에 축을 세웠다.

- Etsy 는 트래픽이 큰 테이블부터 위험을 재며 하나씩 갔다.
- Monzo 는 한 팀이 2,800개를 전부 밀었다.
- Paystack 은 남이 정한 마감 앞에서 아무것도 안 바뀌는 것을 성공으로 삼았다.
- Booking.com 은 **가장 급한 것이 아니라 가장 안전한 것을 골랐다** — 첫 클라우드 이전에는 꽤
  단순한 서비스를 고르라는 것을 교훈으로 남긴다.

그 단순한 것 하나에 특별한 값이 있었다. **옮기는 것만으로 오래된 순환 의존성이 끊긴다** —
백업 카탈로그가 백업 대상 DB 안에 살고 있었다. 회사는 이 고리가 새 발견이 아니라고 먼저 밝히면서
왜 안 고쳐졌는지도 적는다: 안고 사는 것은 복잡함과 실용의 타협이고, 되돌아볼 시간이 늘 있지는 않다.

### 규칙을 파이프라인이 지키게 한 셋 — 그리고 그 한계

- Deliveroo 는 규정 준수를 회의에서 배포 파이프라인으로 옮겼다.
- Monzo 는 데이터 표준을 CI 에 심어 어기는 것만 자동으로 막는다.
- Booking.com 은 그 검사를 **더 앞으로 당긴다** — 코드 리뷰 하나에 몰면 병목이면서 훨씬 일찍
  보였을 문제를 늦게 잡는 자리가 된다.

다만 자동화의 한계를 함께 적는 것이 이 회사의 특징이다. **자동 검사는 지치지도 서두르지도
선별하지도 않지만 형식화한 것만 잡는다.** 그래서 사람 리뷰를 대체하지 않고 보강한다고 못 박고,
점수도 자랑거리가 아니라 품질 기준이 탁상공론이 아니라는 증거로 쓴다.

### 도메인 둘이 예약이 아닌 이유를 사업 모델에 적었다

예약·검색·가격 같은 본체를 안 세운 것은 그쪽 글을 못 읽어서다. 다만 이 둘도 사업과 무관하지 않다 —
숙소 440만 곳과 사업부 여덟이 같은 창고를 쓰는 회사에서 **작은 어긋남은 국소적으로 끝나지 않는다.**
한쪽은 복구 못 하는 재해로, 다른 쪽은 아무도 못 믿는 숫자로 돌아온다.

### 다음 사이클 — 문체 보수 (사다리 3순위)

--gaps 가 정한다. 지금 맨 위는 cybozu(1.82)지만 그건 이 시점의 예상이지 확정이 아니다.
