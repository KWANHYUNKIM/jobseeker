...
**Feldera — 도메인 셋을 다 채우고 done 으로 닫았다.** `zset-incrementalization` ·
`state-layout` · `pipeline-profiler`. 회사 123개, 큐 0/3.
...
이 회사의 세 도메인이 한 줄로 이어진다. **어떤 SQL 이든 증분 프로그램으로 바꾸기로 한 순간
사용자가 쓴 것과 도는 것이 달라진다** — SQL 한 줄이 연산자 수천 개짜리 데이터플로 그래프가 되고
여러 코어에 흩어진다. 그러면 상태가 어디에 얼마나 쌓이는지도, 어디가 느린지도 밖에서 안 보인다.

그래서 두 번째 도메인이 상태를 줄이고(중복 색인 합치기·직렬화 절반) 세 번째 도메인이 그것을
보이게 만든다(그래프 위 측정·코드 수준 마커). 연산자마다 **원래 SQL 의 소스 위치를 들고 있는 것**이
그 간극을 메우는 핵심이다 — 사용자가 고칠 수 있는 것은 SQL 뿐이기 때문이다.

### 측정하는 일이 측정 대상을 흔들지 않게

프로파일러의 저수준 마커 인터페이스는 스레드마다 파일 하나와 마커마다 시스템 콜 하나를 요구한다.
그래서 구간을 내부에 모아 두었다가 **프로파일을 받은 뒤 후처리로 집어넣는** 쪽을 골랐다. 코드
프로파일러는 지금이 LSM 병합인지 네트워크 교환인지 모르는데, 프로그램은 안다는 사실을 쓴 것이다.

성과도 도구가 아니라 결과로 말한다 — 한 고객이 80억 레코드·1TiB 백필을 20시간에서 4시간으로 줄였다.

### 다음 사이클 — 후보 조사 (큐 0/3)

지난 두 조사의 기준(회사 소유 도메인에 엔지니어가 직접 쓰는 블로그)이 여섯 회사에서 다 통했다.
같은 기준으로 다시 판다.
