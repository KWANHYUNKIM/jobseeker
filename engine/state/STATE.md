...
**Ladybird — 기능 둘을 채웠다.** `from-scratch-wpt` 와 `swift-to-rust-with-ai`(스위프트를 물리고,
사람이 지시하는 이전으로 옮긴다). 회사 128개, 큐 1/3. 빈 도메인 하나 — 프로세스 분리.

### 같은 해, 같은 이전, 정반대 방식 — Bun 과 나란히 섰다

둘 다 2026년에 LLM 을 써서 C 계열 코드를 러스트로 옮겼고 둘 다 적대적 검토를 썼다. 그런데
방식이 정반대다.

- **Bun** — 53만 5천 줄을 **한 번에**. 워크트리 4개 × 인스턴스 16개 = 64개, 11일, 약 16만 5천
  달러, 회귀 19건. 이유는 일정이었다 — 사람만으로는 셋이 1년이고 그동안 기능이 얼어붙는다.
- **Ladybird** — 자족적이고 테스트가 두꺼운 2만 5천 줄부터. 2주, **회귀 0**, 바이트 단위로 같은
  출력. 자율 생성이 아니라 **사람이 지시하는 방식**이라고 못 박고 수백 개의 작은 프롬프트를 쓴다.
  그다음부터는 서브시스템 단위로 A/B 모드에서 두 구현을 함께 돌려 견준다.

**속도를 산 쪽과 견줄 수 있음을 산 쪽**이다. 첫 대상을 고르는 기준도 갈린다 — 저쪽은 전부,
여기는 안전 이득이 큰 곳이 아니라 **견주기 쉬운 곳**을 먼저 골랐다. 첫 이전의 목적이 이득이 아니라
방법의 검증이었기 때문이다(재구성).

### 후속 언어를 두 번 골랐다

2024년 9월에 스위프트를 채택했다가 2026년 2월에 전량 제거했다 — C++ 상호운용이 끝내 충분해지지
않았고 애플 밖 플랫폼 지원이 제한적이었다. 러스트도 2024년에는 C++ 식 객체지향에 안 맞는다고
물렸던 것을 같은 글에 적는다. **되돌리기 비싼 판단을 되돌린 기록**이라 `history` 에 두 세대로 남겼다.

### 다음 사이클 — 확장 (마지막 빈 도메인)

프로세스 분리를 채우면 Ladybird 가 done 이 된다. 컴포지터로 옮긴 GPU 접근이 보안과 성능 양쪽에서
같은 경계를 쓴다는 것을 이미 걸어 뒀다.
