...
**Klaviyo** — 회사 프로파일을 썼다. 도메인 **2개**, 기능 **0/2**. 회사 **110개.** 큐 1/3.

### ⭐ 연도 확인이 먼저였고, 그게 값을 했다

QUEUE 에 **`목록의 최신 날짜가 Apr 23 로 연도가 안 붙어 있다 — 파기 전에 확인한다`** 고 적어 뒀다. ⭐ **목록 전체를 받아 보니 답이 나왔다** — 아래쪽 글들에 **2025-12 ~ 2025-05** 가 붙어 있고 연도 없는 것들이 그 위에 있으니 **2026년이다**(최신 2026-04-23). **살아 있다.**

⚠️ **개별 글 주소는 검색으로만 얻었다** — Medium 형식이라 해시가 붙는다. **추측하지 않는다는 규칙대로**, 주소를 못 받은 두 편은 `open_questions` 로 남겼다.

### 이번 사이클 — ⭐ 규모가 트래픽이 아니라 매출의 부산물인 회사

**돈이 고객 수보다 고객의 성장에서 나온다** — **순매출유지율 110%**, **연간반복매출 100만 달러 초과 고객 수가 1년 만에 두 배.** ⭐ **그리고 그 성장이 곧바로 이벤트 수가 된다**(재구성) — 고객이 크면 그 고객의 구매자·클릭·주문이 다 들어온다. **한 달 약 1000억 건, 피크 초당 17만 건, 사업체 17만 6천 곳.**

세운 도메인 둘:

- **마감 시각이 다른 일을 한 줄에서 처리한다** — ⭐ **한 파이프에 성격이 정반대인 일이 흐른다**: **`5초 안에 처리해야 하는 실시간 알림부터 20분까지 견딜 수 있는 대용량 분석 이벤트까지.`** ⚠️ **그런데 카프카는 파티션 안 순차 처리라 우선순위가 없다** — **`규모에서 DB 락 경합과 일시적 실패를 다루다 보면 지연이 선두 차단을 만들고 그것이 SLO 위반으로 이어진다.`**
- **잃지 않고 넘긴다** — ⭐ **재구축의 이유가 성능이 아니라 피크였다.** RabbitMQ 가 **`잘 버텨 줬으나`** 2024년 블랙프라이데이에서 **`선두 차단, 재시도 폭풍, 확장 병목`** 이 드러났다. **2025년 여름 AWS MSK 로 무중단·무손실 이관**, **7일치 백업·리플레이.**

### ⭐ 가장 흥미로운 것 — 우선순위를 등급이 아니라 시각으로 매긴다

**`이벤트별 SLO 마감 시각을 키로 하는 하나의 인메모리 우선순위 큐`** — **최단 마감 우선(EDF) 스케줄러**다. ⭐ **그래서 등급마다 인프라를 따로 둘 필요가 없어진다** — **`P10·P20·P30 마다 따로 두던 인프라 대신 같은 처리기 함대가 모든 우선순위를 다룬다.`** **비용 30% 감소, SLO 준수 20% 개선.**

⭐ **그리고 기성품이 없어 직접 만들었다** — **`기존 카프카 컨슈머 프록시들을 조사했으나 풀 기반 설계와 네이티브 우선순위 스케줄링을 함께 갖춘 것이 없었다.`** ⭐ **풀 기반이라 얻는 것이 하나 더 있다** — **`분배가 풀 기반이라 느린 처리기는 스스로 속도를 늦추고, 나머지가 큐를 계속 비운다.`**

### ⚠️ eras 를 비웠다 — 첫 시기의 시작 연도가 없다

2024년에 한계가 드러났고 2025년 여름에 이관을 마쳤다는 것만 확인된다. **RabbitMQ 를 언제부터 썼는지가 자료에 없다.** ⭐ **창업연도로 메우지 않는다** — 대신 **기능의 `history`** 로 다음 사이클에 쓴다(그게 정확히 기능 하나의 세대 교체다).

### 다음 사이클 — 확장 (2순위)

**Klaviyo 기능 0/2.** ⭐ **`마감 시각이 다른 일을 한 줄에서 처리한다`부터 판다** — **본문을 가장 깊게 읽은 자료이고 거절한 대안·수치·실패 모드가 다 있다**(선두 차단 · 재시도 폭풍 · 배경 작업 굶주림 · 인메모리 큐 무한 성장).

⏳ **못 읽은 자료** — `Our two lines of defense: kafka and S3`(주소 못 받음) · `Unbreakable System for Distributed ML Workloads` · `Ray Data, Train & Tune` · `Engineering Secure Agentic Systems at Scale` · `Aurora Blue/Green`. **ML 축이 별도 도메인이 될지는 본문을 봐야 안다.**
