...
**Plaid** (US · 핀테크) — 도메인 2개 · **기능 1개**(`fail-forward-not-back`). **남은 것은 핵심 제품 하나이고, 그것이 완주를 막고 있다.**

### 이번 사이클 — TiDB 이전 `되돌아가는 대신 앞으로 실패할 길을 미리 만든다`

건진 것 넷:

- ⚠️ **되돌리기를 뒤가 아니라 앞으로 설계했다** — 원본 MySQL 을 **같은 스냅샷으로 복제한 세 번째 클러스터**를 두고, 전환 뒤 문제가 생기면 거기로 **`fail forward`** 한다. **그래서 두 데이터베이스에 동시에 읽고 쓰는 방식을 안 써도 됐다**(회사가 그 방식의 신뢰성·일관성 문제를 피하려 했다고 적는다).
- ⚠️ **어차피 치를 비용을 다른 곳에 썼다** — **MySQL 5.7 지원이 끝나** 5.7→8.0 이 어차피 큰 일이 될 것이라(페이스북의 8.0 업그레이드를 근거로) **그 노력을 플랫폼 이전으로 돌렸다.** **남의 일정이 내 마감이 됐다.**
- ⚠️ **먼저 끝을 정하고 방법을 맞췄다** — `전사적으로 2년을 넘길 수 없다` 는 **설계 제약**이었고, **분기 하나 연구·분기 하나 프로토타이핑**으로 시작도 잘랐다. **기간을 먼저 정하면 방법이 그것에 맞춰진다** — 자동화(동적 런북)와 일의 중앙화가 그 제약에서 나왔다(재구성).
- ⚠️ **일을 나누는 것이 늘 효율적이지는 않다** — 처음엔 클라이언트 팀에 일부를 맡겼는데 **맥락 전환 비용이 너무 커서** 저장 팀으로 거뒀다. **6명이 41개 서비스를 떠안게 되자 자동화가 선택이 아니라 필수가 됐다.**

**동적 런북이 흥미롭다** — Markdown 설명과 TypeScript 코드를 **한 노트북에** 두고(Deno·Jupyter), 내부 SDK 가 클러스터를 추상화하며 **각 셀 실행을 슬랙에 로깅해 감사**한다. **컷오버 5배, 서비스 이전 3~4배.**

### ⚠️ 이번 사이클에 내가 한 실수 — 근거 없는 연도를 넣었다가 뺐다

**history 를 세우면서 Aurora 시기의 `from` 에 `2020` 을 적었는데 출처에 없는 값이었다.** 떠난 시점(2023년 1월)은 확실한데 **시작 연도는 글에 없다.** **검증은 통과했지만 지어낸 연도라 커밋 전에 history 를 통째로 비웠다.**

**STYLE 8번이 경고하는 바로 그 실수다** — `첫 시기의 시작 연도가 출처에 없으면 그 시기를 세우지 말고 그 내용을 다음 시기의 context 로 옮긴다`. Aurora 의 한계는 이미 business·decisions·그림에 다 있으므로 **정보는 남고 없는 연도만 사라졌다.**

### 다음 사이클 — 핵심 제품, 기능보다 자료가 먼저다

⚠️ **`남의 은행 1만 2,000곳에 대신 닿는다` 의 tech 가 0 이다**(검증 경고). **이 회사의 본체인데** 자료가 없다. **그 차례에 자료부터 찾는다** — 여섯 번 통한 규칙이다.

⚠️ **찾을 때 두 가지를 함께 한다** — 브라우저로 열고, **검색으로 현재 주소를 받는다.** 이 회사는 `plaid.com/blog` 가 **Medium 개별 글로 리다이렉트**되지만 **브라우저로는 전문이 읽힌다.**
