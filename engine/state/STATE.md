...
**엠쓰리(M3)** — 첫 기능 `delete-the-recurring-work`(반복될 일을 없앤다)를 끝냈다.
**도메인 2 · 기능 1.** 큐 0/3. 남은 도메인은 **멈출 수 없는 것을 되돌릴 수 있게 옮긴다.**

### 올리는 것은 일을 끝내는 것이 아니라 미루는 것이다

**'런타임을 올리면 이번 대응은 끝나지만 몇 년 뒤에 또 같은 대응이 필요하다'** — 이 한
문장이 판단의 축이다. 그래서 **Lambda 자체를 없앴고**, 성과를 **439줄 삭제 대 40줄 추가**로
적는다. **더한 것이 아니라 사라진 것으로** 결과를 쓰는데, 목적이 '유지 부담 제거'였으니
**잣대가 목적과 맞는다.**

### 못 할 일의 크기를 먼저 계산한다

설문 규칙의 모순을 전수로 확인하려면 **20문항에 95조 번**이다. **'불가능하다'를 셈으로
보이고** 나서 **SMT 솔버(Z3)** 로 가니 그 길을 고른 이유가 저절로 선다 — **탐색 대신
증명**이다.

### 얻은 것만 적지 않는다

솔버는 **'미응답'과 '애초에 도달할 수 없는 것'을 구분하지 못한다.** 완전성과 규모를
얻고 **구분의 세밀함을 내줬다**고 명시한다 — **잃은 것을 적어야 언제 사람이 다시 봐야
하는지가 정해진다.**

### DeNA 와 마주 보는 자리

DeNA 는 **'MySQL 5.7 은 이미 EOL'** 이라며 **관리형으로 옮겼고**, 엠쓰리는 **'몇 년 뒤에
또'** 를 이유로 **아예 없앴다.** **옮기면 다음 EOL 이 또 오고, 없애면 안 온다** — 같은
문제의 두 답이 원장에 나란히 놓였다.

### 다음 사이클

확장 — 남은 도메인(과금 이관). **아홉 단계·병행 항목 넷·쪼갤 수 없던 웹훅·조사용 콘솔**이
이미 근거로 서 있다. 큐가 0/3 이라 그 뒤에 후보 조사가 뜬다.
