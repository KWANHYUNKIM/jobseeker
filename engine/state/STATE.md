...
**Proton**(스위스 · 프라이버시 제품군) — 도메인 2개 중 **1개에 기능이 들어갔다**
(`save-the-hardware-first`). 남은 빈 도메인 하나: '암호 라이브러리를 자기가 만들어 남에게
내준다'. 큐 1/3.

### 이 선택은 기술이 아니라 공급망이 정했다

서비스보다 장비를 먼저 살린 이유가 **AI 붐으로 서버 장비가 부족해 잃으면 짧은 시간 안에
대체할 수 없다**는 것이다. 몇 시간의 장애와 몇 달의 용량 부족을 견줬다 — **같은 사고가
2년 전이었다면 다른 결정이 나왔을 것이다.**

### 이중화가 같은 시각에 같은 손을 탄다

압축기는 이중화돼 있었는데 운영사가 **양쪽 필터를 동시에 갈았다.** 이중화는 부품이
**독립적으로** 고장 난다는 가정 위에 서 있고, **정비라는 공통 원인**이 그 가정을 무너뜨린다.
게다가 그 손은 회사 밖에 있었다.

### 보호 장치가 복구 경로를 함께 막는다

네트워크 카드는 105°C 에서 스스로 꺼져 장비를 지키지만 **차가운 재기동**을 요구하고, 원격
재기동 통로는 **보안 태세상 좁혀져** 있다. **둘 다 평소에는 옳은데 그날 밤에는 함께 복구를
늦췄다.** 그런데도 통로를 열지 않았다 — 프라이버시를 파는 회사에서 그 순간의 예외가 곧
상품의 훼손이기 때문일 것이다(재구성).

### 덜 바꾸는 것이 기본값인데, 이번에는 틀렸다

프랑크푸르트 안에서 넘긴 이유가 **"트래픽 흐름을 너무 바꾸면 새 불안정이 생긴다"** 였다.
사고 대응의 상식인데 이번에는 그 가정이 틀렸다고 적는다. 다만 **그래서 다음에는 어떻게
할 것인지는 적지 않았다.**

### 돌아온 것과 안전해진 것은 다르다

01:30 에 대부분 복구됐지만 **푸시 알림과 결제는 그때도 안 됐고**, 밤새 **어떤 주 DB 는
취리히에 어떤 것은 프랑크푸르트에** 있는 '아주 비정상적인 상태'로 돌았다. 이중화 복구는
8월 27일 내내 걸렸다.

### 다음 사이클

남은 도메인 하나 — '암호 라이브러리를 자기가 만들어 남에게 내준다'(OpenPGP.js · GopenPGP ·
제3자 감사). **자료가 얇다** — 감사 기관 이름도 날짜도 없는 페이지 하나뿐이라, 라이브러리
저장소나 감사 보고서를 따로 열어야 한다.
