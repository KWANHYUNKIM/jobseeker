...
**Sansan(JP · SaaS) `in_progress` — 329 에서 프로파일을 썼다. 도메인 1개 · 기능 0개.**
**56번째 회사이고 일본 다섯째다.** `--gaps` 가 [신규]를 가리켰다(큐 3/3 → 대기 맨 위).

**블로그가 크고 살아 있다** — `buildersbox.corp-sansan.com` 최신 글이 **2026-08-28** 이고
목록도 분류도 그대로 열린다. 분류는 **R&D 347 · 개발 178 · Eight 136 · Sansan 123 ·
Bill One 116 · 머신러닝 64 · iOS 67 · 인프라 49 · Contract One 35** 다.
**⭐ Freshworks 와 정반대다** — 거긴 네 경로에 흩어져 목록이 안 열렸다.

**연 도메인 하나 — `저장소를 옮기면 붙어 있던 것이 따라오지 않는다`.** **본문 두 편으로
받쳤다**(두 편 규칙 만족).
- **모노레포 이관**(2024-05-27) — 저장소 40개를 점진적으로 합쳤다. 버린 대안 둘(빅뱅 /
  OSS 도구 대신 직접 스크립트, 이유 넷) · **대가 넷** · 수치(40개 · 20명 미만 · 100만 줄).
- **GitHub Enterprise 이관**(2025-12-19) — Team → Enterprise Cloud(EMU), **약 9개월**에
  **1,000개 중 700개**, 계정 **500개 이상**. 버린 대안 둘(non-EMU 는 계정 수작업이 영원히
  남는다 / 일괄 이관은 긴 코드 프리즈) · **대가가 굵다**(GEI 불안정 · **Secrets 와 Variables
  미이관** · Organization URL 변경으로 외부 연동 재수립 · **OIDC `sub` 클레임 변경으로 AWS
  수용 설정까지** · EMU 는 외부 협력자 불가 · **운영 조직 미정·GHAS 미완이라고 스스로 적음**).

**⭐ 두 편이 같은 결말에 닿는다** — **옮기는 것보다 옮기는 것에 붙어 있던 설정이 더 손이
갔다.** 모노레포 편이 그것을 한 문장으로 적는다(*"モノレポの移行よりも…CI/CDジョブの移行に
手間取りました"*)고, GHE 편에서는 Secrets·외부 연동·OIDC 가 그 자리다.
**둘 다 점진 이관을 골랐는데 이유가 다르다**(변경 빈도 차 / 코드 프리즈 길이).

---

**다음 사이클 — 도메인 ① 의 첫 기능을 쓴다. 회사를 완주할 때까지 갈아타지 않는다.**
재료 두 편이 이미 확정돼 있다. **결정이 5개 나오는지 먼저 센다** — 점진 이관의 두 근거 ·
직접 스크립트 대 GEI · EMU 대 non-EMU · 권한을 Terraform 으로 · 주말 작업 창 · 저장소
인벤토리로 1,000개를 700개로 줄인 것이면 쪼개서 6~8개가 나올 것으로 본다(추정).
**⚠️ ⑫ 원문을 다시 읽는다** — 320·324·325·326 에서 네 번 다 재독이 새 재료를 냈다.

**📌 이 회사에서 다음에 볼 것**
- **⭐ 사업의 심장은 아직 안 팠다.** 매출을 끄는 것은 **명함·청구서를 읽어 데이터로 바꾸는
  일**(Bill One **+41.3%**)인데 이번 도메인은 개발 환경 이야기다. **R&D 347편 · ML 64편이
  가장 큰 분류이니 거기를 본다.**
- **⭐⭐ 실물이 있는 도메인이 하나 보인다** — `サービスごとに異なる原本管理を、一つの仕組みへ`
  (2026-08-20). **되돌리기가 화면 안에서 안 끝난다** — *"システムのステータスだけを戻したと
  しても、実際の原本はもう別の場所にあるかもしれない"*. ⚠️ **대담 형식이라 버린 대안이 0 이라
  도메인으로 안 열었다.** **같은 주제의 기술 글이 하나 더 나오면 이 사이트에 없는 축이다**
  (다른 회사들의 되돌리기는 전부 화면 안에서 끝난다, **추정**).
...
- **⭐⭐ freee 와 SmartHR 이 정반대 순서로 같은 문제를 풀었다** — 인증·권한 기반을 제품에서
  떼어내는데 **freee 는 모놀리스에서 곧장 4년**(후회로 적는다), **SmartHR 은 모듈러
  모놀리스로 시작해 Packwerk 로 경계를 먼저 보이게 한 뒤** 떼어낸다. **비교 문서 재료다.**
