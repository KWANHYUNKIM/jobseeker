...
**없다 — 문체 보수 사이클이었다.** 모달을 배민 문체로 다시 썼다(1.62 → 0.0). 회사 115개, 큐 2/3. 21곳 남았다.

### 변환기가 데이터를 지운 자리를 검증이 잡았다

기호를 걷어내는 변환기가 줄머리의 `—` 를 지우는 규칙을 갖고 있는데, `solves` 값이 통째로 `—`
하나였던 칸이 있어 빈 문자열이 됐다. **검증이 그것을 오류로 잡아 커밋 전에 멈췄다.**
빈 칸을 실제 문장으로 채우고(쿠버네티스를 왜 안 골랐는지가 그 항목의 본체다) 나머지 114개 회사도
같은 종류가 없는지 훑었다 — 없었다.

**자동 변환은 이런 식으로만 데이터를 다친다는 것이 이번 사이클의 수확이다.** 검증이 없으면
조용히 빈 칸으로 남았을 자리다.

### 중앙을 버린 넷 — 버린 이유가 다 다르다

- Cloudflare — 전용 로드밸런서를 없애 모든 서버가 서로에게 넘긴다.
- Fly.io — 라우팅 표를 합의 없이 퍼뜨려 1~2초를 산다.
- Fastly — 퍼지가 못 닿는 것이 느린 것보다 나빠서 중앙 큐를 버린다.
- Cursor — 복제본을 더할수록 푸시가 느려져 합의를 버린다.
- Modal — **요금표 때문이다.** 유휴에 값을 안 받으니 시작하는 시간이 통째로 회사 원가가 되고,
  임계 경로에서 한 번 물어보는 것조차 값이 된다.

앞의 넷이 성능이나 도달 가능성 때문에 버렸다면 **여기는 파는 방식 때문에 버렸다.**

### 파는 방식이 어디를 깎을지를 정한다

- Perplexity 는 사용량으로 팔아서 GPU 이용률이 마진이 되고, 그래서 GPU 앞 대기줄을 러스트로 옮겼다.
- Modal 은 이그레스에 값을 안 받아서 데이터가 미국을 다녀오는 왕복을 자기가 문다.

유휴를 다루는 방식은 Block 과 정면으로 갈린다 — **한쪽은 유휴를 안 사려고 쓰고 다른 쪽은 유휴를
흡수해서 판다.** 흡수해서 파는 쪽은 그 값을 자기가 떠안으므로 시작과 이동 시간을 줄이는 일이
사업 그 자체가 된다.

### 도메인을 제품이 아니라 회사가 무는 두 가지 시간으로 잘랐다

함수·샌드박스·GPU 로 가르면 기술 선택이 왜 그렇게 됐는지가 안 보인다. **시작하는 시간과 오가는
시간, 둘 다 고객에게 청구되지 않는 시간이라 줄이는 것이 곧 마진이다.**

### 다음 사이클 — 문체 보수 (사다리 3순위)

--gaps 가 정한다. 지금 맨 위는 smarthr(1.55)지만 그건 이 시점의 예상이지 확정이 아니다.
