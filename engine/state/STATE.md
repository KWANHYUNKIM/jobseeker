...
**CedarDB — 기능 둘을 채웠다.** `colibri` 와 `encode-always-compress-sometimes`. 회사 121개,
큐 2/3. 빈 도메인 하나 — 실행 엔진.

### 무엇을 재느냐가 답을 바꾼다

압축률만 보면 zstd 가 낫다(URL 데이터에서 19.6배 대 사전 13.5배). 그런데 같은 데이터에 필터를
걸면 사전 인코딩이 0.4ms, 압축본이 64.1ms 로 **168배** 차이가 난다. 숫자 컬럼도 3.2ms 대 56.5ms 다.
만드는 값까지 보면 더 갈린다 — 200만 행 인코딩이 30ms 미만인데 zstd 레벨 19는 19~32초다.

그래서 둘 중 하나를 고르지 않는다. **질의가 닿는 곳에는 인코딩을 두고, 디스크로 나갈 때만 압축을
얹되 20% 이상 더 줄 때만 얹는다.** 메모리 표현은 압축 여부와 무관하게 같아서 위층은 압축이 있었는지
모른다. ClickBench 에서 둘을 겹쳐 21.4GB 를 7.88GB 로 줄였다.

### 인코딩이 질의 가능한 표현이라는 뜻

사전에서는 비싼 문자열 비교가 싼 정수 비교가 되고, 기준점에서는 블록의 최소·최대 머리말만 보고
값 하나 안 건드린 채 블록 전체를 건너뛴다. 컬럼을 약 1,000개 값 블록으로 끊는 이유도 여기 있다 —
이상치 하나가 컬럼 전체의 바이트 폭을 넓히지 못하게 한다.

### 이 회사의 대가는 다른 자리에 있다

`Colibri` 글이 그랬듯 이 글도 자기가 감수하는 것을 따로 적지 않는다. 다만 이번에는 빈 곳이
분명하다 — **쓰기 경로의 값이 없다.** 인코딩을 고르고 블록을 만들고 압축을 얹는 일이 공짜일 리
없는데, 앞 기능에서 전환이 10~40초마다 일어난다는 것과 함께 보면 그 값이 어딘가에 실린다.
난제로 남겼다.

### 다음 사이클 — 확장 (마지막 빈 도메인)

실행 엔진 도메인을 채우면 CedarDB 가 done 이 된다. 주소는 QUEUE 에 적어 뒀다.
