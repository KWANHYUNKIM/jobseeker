...
**VictoriaMetrics** (기타 · SaaS — 시계열 DB·관측) — **프로파일까지 썼다**. ⚠️ **도메인이 2개뿐이다.** 기능 0. 회사 **101개** · 큐 0/3.

### ⚠️ 이 사이클에서 걸린 것 — 또 허용값이다

**`country` 를 `다국적` 으로 썼다가 검증이 오류로 잡았다.** ⚠️ **앞 사이클에 `category` 로 똑같은 실수를 하고 STATE 에 `새 회사를 세울 때는 schema 의 허용값을 먼저 보라` 고 적어 놨는데, 그 다음 회사에서 다른 필드로 같은 실수를 했다.** **적어 두는 것과 지키는 것이 다르다.** ⭐ **새 회사 프로파일을 쓸 때는 `slug·name·name_en·country·category` 를 한 번에 schema 와 대조한다.** 소재를 확정할 수 없어 **`기타`** 로 뒀다 — 지어내지 않는다.

### 이번 사이클 — 신규(6순위). ⚠️ 도메인을 억지로 늘리지 않았다

**확인된 것** — ⭐ **회사가 비용의 정체를 다르게 짚는다**: **`태어나는 것이 이 메트릭 생애에서 비싼 부분이고 자라는 것은 싸다 … 원시 샘플 양이 아니라 카디널리티가 시계열 DB 를 짓누른다.`** 그리고 **설계 근거가 붙어 있다** — **`디스크의 데이터는 TSID 로 정렬되고, TSID 는 MetricID 앞에 세 개의 묶음 필드(메트릭 이름·job 류 라벨·instance 류 라벨)를 둔다. 그래서 같은 메트릭·job·instance 의 시리즈가 디스크에서 나란히 놓여 압축이 잘 되고 스캔이 싸진다.`** **임계값이 박힌 결정도 있다** — **`zstd 가 어떤 블록을 최소 약 10% 만큼 줄이지 못하면 델타 인코딩된 평문 바이트를 그대로 둔다.`**

**사업** — **세 갈래**(영원히 무료인 오픈소스 · 재해 복구·멀티테넌시·이상 탐지를 얹은 엔터프라이즈 · 완전 관리형 Cloud). **파는 것이 압축이다**(재구성).

### ⚠️ 후보 조사 때 적어 둔 위험이 그대로 현실이 됐다

**`이 글 하나는 확실하지만 두 번째 기능에서 자료가 마를 수 있다`** 고 큐에 써 뒀는데, **도메인을 둘밖에 못 세웠고 그중 하나(`오픈소스를 사업으로 만든다`)는 미독이다.** ⭐ **후보 조사에서 위험을 적어 둔 것이 그대로 맞았다 — 그 습관은 유지한다.**

⚠️ **이 회사는 자기 시스템 글보다 Go 런타임 해설을 더 많이 쓴다**(Swiss Tables 맵 · sync.Map 해시 트라이 · Go 1.27 둘러보기). **재미있지만 이 회사가 무엇을 어떻게 만들었는지는 말하지 않는다.** ⏳ **자기 시스템과 이어 말하는 글을 찾으면 그때 도메인을 세운다.**

⚠️ **`vmagent` 자체의 설계 글을 못 찾았다** — **Airbnb 와 Razorpay 가 둘 다 그것을 골랐는데**(Airbnb 의 이유가 **`작은 코드베이스(약 1만 줄)라 필요하면 이해하고 고치기 쉽다`** 였다) **만든 쪽의 설명이 없다.** ⭐ **관측을 사는 쪽과 파는 쪽을 양쪽에서 볼 기회였는데 절반만 있다.**

### 다음 사이클 — 확장(2순위)

`--gaps` 가 **`메트릭을 저장하고 찾는다`** 를 부를 것이다. **자료를 이미 읽었으니 바로 기능을 쓴다.** ⚠️ **다만 `The Life of a Metric` 은 서술적인 글이라 거절한 대안이 약하다** — **결정과 임계값은 있으니 그것을 중심에 놓고, 없는 것은 없다고 적는다.**

⚠️ **큐가 0/3 이다.** 이 회사를 다 판 뒤 **후보 조사**가 걸린다. ⭐ **언급 각도를 먼저 쓴다**(지난 조사에서 넷 중 둘). ⏳ **이번 회사가 남긴 언급 후보** — ⏳ Confluent · IBM(WarpStream 소유) · ⏳ **Datadog 의 Husky**(있는 회사지만 그 글은 안 봤다).
...
**③ `자기 성과를 어디까지 주장하나` 8곳** — Snyk / Grafana Labs / ScyllaDB / Razorpay / Flipkart / Airbnb / Pinterest / WarpStream(한계 선점).
...
**⑧ `제약을 없애지 못할 때 어디서 갚는가`** — Cygames / Zepto / Grafana Labs / Pinterest / WarpStream.

**⑨ `한 번에 갈아엎을 것인가 목 졸라 죽일 것인가` 7곳** — Twilio(두 방향) / Etsy / Plaid / Paystack / Airbnb / Pinterest / WarpStream.

**⑩ ⭐⭐ `관측 비용을 어디까지 줄이나`** — **사는 쪽**: Razorpay · Airbnb(**둘 다 vmagent 를 골랐다**) · WarpStream(3자 비용에 막혀 자기 것을 만들었다) / **파는 쪽**: Grafana Labs · Honeycomb · **VictoriaMetrics**. ⭐ **이 축이 가장 두꺼워졌다 — 같은 도구를 두고 사는 쪽 둘과 만든 쪽이 다 있다.**
...
**⑰ ⭐ `기억시킬 것인가 압축할 것인가`** — Pinterest(20GB 테이블 → 시맨틱 ID) / Zepto / Grafana Labs / **VictoriaMetrics**(⚠️ **`zstd 가 최소 약 10% 를 못 줄이면 압축하지 않는다` — 압축을 언제 포기할지의 임계값을 숫자로 박았다**).
...
⏳ **㉔ 새로 보인다 — `무엇을 무료로 두고 무엇을 파나`** — **VictoriaMetrics**(**`오픈소스는 좋은 마케팅이다`** — 오픈소스는 영원히 무료, 재해 복구·멀티테넌시·이상 탐지는 상용) / **Grafana Labs** / **ClickHouse** / **Snyk**. ⚠️ **오픈소스 인프라 회사가 넷이나 있는데 이 선을 어떻게 긋는지는 한 번도 안 봤다.**
