...
**없다 — 쿡패드를 완주했다(도메인 2 · 기능 2).** 큐 0/3 이라 다음 사이클은 **후보 조사**다.

### 이관 규모가 검증 방법을 정한다

**통째로 옮기기로 한 순간 A/B 가 사라진다** — **'일부만 사용자에게 내보내 검증할 수
없었다.'** 남는 것은 **옛것과의 비교**뿐이고, 그러면 **옛것이 정답 노릇**을 하게 된다.
RBO 로 **97.2%·95.1%** 를 맞췄지만 **더 나아진 것은 그 자로 잴 수 없다.**

### 자기 확신의 부족을 적는다

**'제대로 인기 키워드를 계산하고 있는 것인지 끝까지 불안이 있었다.'** 계산식 자체가
달랐고(일본은 시간당 검색수 − 평균, 글로벌은 그날 검색수 ÷ 과거 평균) 기준도 정성적이라
(**'크게 벌어지지 않으면 됐다'**) 확신이 안 섰다는 것을 그대로 남긴다. **이 원장에서 드문
대목**이라 적어 둔다.

### 공통을 고치는 대신 전용을 따로 만든다

글로벌 구조를 일본식으로 맞추면 **여러 언어에 걸쳐 품질을 다시 검증하고 파라미터를 다시
맞춰야** 한다. **코드 두 벌을 감수하고 검증 범위를 좁혔다** — 다국어 시스템에서는 그쪽이
싸다.

### 자마다 못 재는 것이 있다

RBO 는 **꼬리 질의**를 안 보고(상위 2만만), 트렌드 워드 기준은 **정성적**이며, 인터리빙은
**무한 스크롤에서 중복**을 보인다. **세 자를 다 쓰고도 빈칸이 남는다** — 이관 뒤 실제
사용자 지표(클릭·이탈)가 어디에도 없다.

### 쓰는 쪽 기록이 만드는 쪽 기록의 빈칸을 메운다

GS2 의 바탕이 **Elasticsearch** 인데, 쿡패드는 **질의 확장과 색인 튜닝으로 4배 느리던 것을
꼬리에서는 더 빠르게** 만들었다. 엘라스틱 프로파일과 이어 붙는 자리다.
