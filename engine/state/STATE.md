...
**Freshworks(IN · SaaS) `in_progress` — 323 에서 프로파일을 썼다. 도메인 2개 · 기능 0개.**
**55번째 회사이고 인도 셋째다.** `--gaps` 가 [신규]를 가리켰다(큐 1/3 → 대기 표 맨 위).

**⚠️ 목록 페이지가 404 다** — `https://www.freshworks.com/explore-it/` 를 열면 404 이고,
글 주소는 직접 열린다. **⑰ 을 지켜 검색으로 실제 경로를 찾았고, 거기서 시리즈 이름이 나왔다.**

**⭐ 엔지니어링 글은 `Rails@Scale` 시리즈다** — 회사가 취지를 *"In this Freshworks
Engineering series, Rails@Scale, we talk about some of the techniques we employ to scale up
our products"* 로 적는다. **확인된 세 편 + 안 읽은 한 편**:
- `Scaling the Freshdesk web app for high Availability`(2020-07-15) — **이번에 새로 읽었고
  셋 중 가장 두껍다.** 단일 스레드 Passenger 로 **분당 약 50만 요청**. **🔴 대가를 다섯 적는다**
  — ORM 롤백 테스트 부담 · **옛 Rails 가 연결 실패 때 오류를 삼켜 고아 레코드** ·
  **Semian 이 테이블 단위로 못 자름** · 프로세스 단위라 분산 판단 불가 ·
  **Newrelic explain 쿼리가 Semian 감시를 방해해 꺼야 했다.** 버린 대안도 있다.
- `Sidekiq queue management using custom middleware`(2020-05-14) — 버린 대안 둘 + 수치.
- `How Freshdesk uses HAProxy at scale`(2021-08-24) — 대가 두 줄.
- **`Optimizing cache with MemoizeUntil` — 아직 안 읽었다.** 고가용성 편이 링크로 건다.

**연 도메인 둘**(둘 다 본문을 읽고 열었다)
1. **하나가 느려지면 전부가 막힌다** — HAProxy 편과 고가용성 편이 **같은 문제를 다른 층에서**
   푼다(**앞에서 나누고 뒤에서 자른다**). **다음 사이클의 기능이 여기서 나온다.**
2. **큐 이백 개를 기계에 어떻게 얹는가** — Sidekiq 편. **폭발 반경이 아니라 자원이 놀거나
   몰리는 문제**라 223 규칙에 따라 갈랐다.

**⚠️ 사업의 무게와 글의 무게가 어긋난다** — 매출을 끄는 것은 **EX(ITSM) ARR +24%** 와
**Freddy AI**(신규 엔터프라이즈 거래의 **71% 이상**)인데, **글은 거의 전부 CX 쪽 Freshdesk 의
2020~2021년 Rails 운영 이야기**다. **Paytm 과 같은 어긋남이 두 회사 연속으로 나왔다.**

---

**다음 사이클 — 도메인 ①(하나가 느려지면 전부가 막힌다)의 기능을 쓴다.**
재료는 **고가용성 편**(대가 다섯 · 버린 대안 있음 · 분당 50만 요청 · 트랜잭션 최대 25초)과
**HAProxy 편**(shell 라우팅 넷 · 리포트 300ms vs 티켓 100ms). **두 편 규칙을 만족한다.**
**결정이 5개 나오는지 먼저 센다** — 서킷 브레이커 · 드라이런 · 긴 트랜잭션 중단 · 타임아웃 ·
Envoy 사이드카 · shell 라우팅 · 요청 종류 분리면 쪼개서 6~8개가 나올 것으로 본다(추정).

**📌 그 밖**
- **큐 잔량 0/3.** **다음다음 사이클은 후보 조사다** — **일본**(Sansan · freee · SmartHR ·
  Money Forward)이나 **유럽**(Personio). **⚠️ 인도는 소진됐고**(Zerodha·Paytm 완료 ·
  Freshworks 진행 중 · Zoho·Ola 는 재시도 안 함 · 나머지 전부 막힘) **Trade Republic 은
  Medium 호스팅이다.**
- **⭐ 320 의 `leave-what-costs-more`(Paytm)가 비교 문서 두 편의 빈칸과 이어진다** —
  `reversible-first.md` 의 *"되돌리지 못해 앞으로 밀고 나간 사례가 없다"* 에 **첫 재료**이고,
  `where-it-piles-up.md` 의 **갚지 않고 이사한 쪽**에서 **한 걸음 더 간다.**
- **후보 조사 누적 열여섯 곳 중 넷.**
