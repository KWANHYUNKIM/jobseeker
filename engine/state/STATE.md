...
**Duolingo** (US · 교육) — 도메인 3개 · **기능 1개**(`when-and-what-are-two-models`). 남은 도메인 둘.

### 이번 사이클 — 학습 `언제 다시 보여 줄지와 무엇을 보여 줄지를 따로 푼다`

**예고한 대로 기능보다 자료를 먼저 찾았고, 절반만 건졌다.**

- ✅ **Birdbrain 1급 글을 찾았다** — 하루 **5억 개 이상**의 연습을 먹고 **학습자 숙련도와 연습 난이도를 함께** 배운다. ⚠️ **핵심은 대체가 아니라 보완이라는 것이다** — `This complements our other personalization system that figures out how well a learner knows each particular word.` **`언제` 와 `무엇` 이 다른 질문이라 두 시스템으로 남았다.**
- ✅ **적응형 레슨 글에서 긴장을 얻었다** — 학습자는 `hesitant to jump out of their comfort zone` 인데 최선의 학습은 익숙함과 새로움 사이다. **밀어내되 발판(스캐폴딩)을 함께 준다.**
- ❌ **2016년 논문 본문은 못 읽었다** — PDF 를 두 번 열어 두 번 다 파싱 실패.

### ⚠️ 이번 사이클에서 가장 값진 것 — 접근 지도를 정정했다

**`PDF (IR 자료 등) — ✅ 읽을 수 있다` 가 항상 참이 아니다.** 그리고 더 위험한 일이 있었다: **첫 시도에서 그럴듯한 요약이 돌아왔다**(데이터 규모·기준선 대비 개선치·한계까지). **두 번째로 같은 파일을 열었더니 `파싱할 수 없다` 고 했다.** 첫 요약이 근거 없이 생성된 것일 수 있어 **그 수치를 하나도 쓰지 않았다.**

**교훈: 도구가 준 요약이 구체적일수록 한 번 더 물어본다.** 같은 자료에 두 번 물어 답이 갈리면 **둘 다 버린다.**

### 다음 사이클 — 에이전트 아니면 테스트

**둘 다 자료가 익었다.** 에이전트 도메인은 **버린 대안 셋에 이유가 붙어 있고 연도가 다 있다**(로컬 MCP 2024-11 → 중앙 설정 생성기 2025-05 → HTTP MCP 서버 2025-08 → 슬랙 앱) — **연표를 세울 수 있다.** 테스트 도메인은 **사고 하나와 병목 하나**가 있다.
