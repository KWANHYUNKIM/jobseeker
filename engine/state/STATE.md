...
**없다 — ScyllaDB 완주.** 도메인 2/2 · **기능 2개.** 회사 **94개.** ⚠️ **큐 1/3 이라 다음은 후보 조사(7순위) 또는 신규(6순위)다.**

### 이번 사이클 — `같은 API 를 쓰면 클라이언트는 아래가 다른 줄 모른다`

건진 것 넷:

- ⚠️ **호환 전략의 값과 대가가 같은 자리에서 나온다** — DynamoDB API 를 Rust 에서 쓰려면 AWS SDK 뿐인데 그것은 **엔드포인트 하나짜리 관리형 서비스를 겨냥해 만들어졌다.** 결과가 회사의 문장에 있다: **`모든 요청이 한 노드에 떨어지고 나머지 클러스터는 놀고 있다.`** ⚠️ **이 회사의 토대는 정반대로 모든 노드와 샤드가 동시에 일해야 값이 난다.**
- ⚠️ **대체하지도 포크하지도 않는다** — **공식 `aws-sdk-dynamodb` crate 를 감싸고 그 SDK 의 인터셉터 프레임워크에 최적화를 주입한다.** **남의 것 안에서 갈라진다**(재구성). **3노드에서 처리량 약 58% 향상, P99 안정, 헤더 45% 감소.**
- ⚠️ **회사가 이 긴장을 문장으로 적는다** — **`일반적인 AWS SDK 클라이언트는 당신의 클러스터 토폴로지를 전혀 모르므로 처리량을 남긴다.`** **완전한 호환이 근본적인 성능 제약을 가리고, 추상화 계층이 분산 최적화를 표현하지 못한다.**
- ⚠️ **재는 도구가 먼저 지치면 안 된다** — 자체 도구 Latte 를 쓴 근거가 **`YCSB 보다 훨씬 효율적이라 CPU 를 대략 절반만 쓰면서 데이터베이스를 포화시킨다`** 다. **부하를 넣는 쪽이 먼저 지치면 서버가 아니라 클라이언트의 한계를 재게 된다**(재구성).

### ⭐ Plaid 와 방향이 반대인 같은 문제

**Plaid 는 `하나의 API 뒤에 1만 2천 곳의 서로 다른 규칙` 을 문서로 노출했고**(감추면 런타임에 놀라고 다 노출하면 하나의 API 가 아니다), **ScyllaDB 는 남의 API 를 흉내 내다가 `클라이언트가 아래를 모른다` 는 벽을 만났다.** ⚠️ **한쪽은 남의 차이를 흡수하고 한쪽은 남의 인터페이스를 흉내 낸다 — 추상화가 무엇을 가리는가라는 같은 문제의 두 방향이다**(재구성). connections 에 이어 뒀다.

### 다음 사이클 — 큐 보충 또는 신규

`--gaps` 가 정한다. 큐에 **Cygames**(게임 · TiDB) 하나가 남았다. ⏳ 되짚을 목록 — `www.gojek.io/blog`(429) · ⛔ `bytes.swiggy.com`(세 번 두드리고 접었다).
...
**③ ⭐ `자기 성과를 어디까지 주장하나` 3곳 (재료 참)** — Snyk / Grafana / ScyllaDB.

**④ `인도 규모에서 무엇이 달라지나` 2곳** — Meesho / Zepto.

⭐ **⑤ `한 코어에 하나씩인가, 여러 코어가 나눠 쓰나` — 양쪽에 기능이 생겼다** — **ScyllaDB**(shared-nothing 이 대안을 거르고, 공유의 자리를 커널로 내린다) 대 **ClickHouse**.

⭐ **⑥ `복제로 버틸 것인가 로그로 버틸 것인가`** — Grafana 의 Mimir·Loki.

⏳ **⑦ 새로 보인다 — `추상화가 무엇을 가리는가`** — **Plaid**(남의 차이를 흡수하고 예외를 문서에 적는다) / **ScyllaDB**(남의 API 를 흉내 내니 클라이언트가 토폴로지를 모른다) / ⏳ **Paystack**(은행 하나가 API 모양을 바꾼다).
