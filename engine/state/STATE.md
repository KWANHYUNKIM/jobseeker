...
**Qdrant** — 첫 기능 `repair-at-index-or-at-query`(고침을 색인에 넣을까 질의에 넣을까)를
끝냈다. **도메인 2 · 기능 1.** 큐 1/3. 남은 도메인은 **RAM 을 넘어서는 컬렉션에 계층을 준다.**

### 색인이냐 질의냐는 곧 빌드냐 지연이냐다

필터가 그래프를 섬으로 만든다(**`m=16` 에서 96% 를 거르면 노드당 링크가 평균 1개 미만**).
고칠 자리가 둘인데 **어느 쪽으로 옮겨도 사라지지 않는다** — 색인에 간선을 더 놓으면
**빌드가 4.4~5.6배**(100만 점에 116초 → 507~652초), 질의 때 이웃의 이웃을 보면
**지연이 2.1~2.9배**(실전에서 5.4배·6.7배). **양쪽 값을 같은 표에 올려야 고를 수 있다.**

### 구멍의 모양이 달라서 둘 다 갖는다

색인 쪽은 **AND 조합**에서 무너지고(간선을 **필드마다 만들지 조합마다 만들지 않는다** →
4% AND 교집합에서 재현율 **39.7%**), 질의 쪽은 **아주 좁은 필터**에서 무너진다(1% 필터
정체, **67.7%**). **겹치지 않으니 서로를 메운다** — 그래서 계획기가 매번 고른다.

### 자기 것이 나쁜 칸을 표에 그대로 둔다

39.7% 도 67.7% 도 자기 수치다. 그리고 **아직 못 고친 자리에 후속 논문 이름까지 적는다**
(RACORN-1). 리스테이트가 분산 배치를 '정말 어렵다'고 적은 것과 같은 결이다.

### 시간은 촘촘히 재고 공간은 안 잰다

밀리초와 재현율은 표로 다 나오는데 **여분 간선과 ACORN 자료구조의 메모리 부담이 어느 표에도
없다.** 간선을 더 놓는 방식이니 메모리가 늘 텐데 그 칸이 비어 있다 — **다음 사이클(메모리
도메인)에서 이 빈칸과 이어질 자리**라 적어 뒀다.

### 논문 링크를 확정하지 못했다

글에 arXiv 주소가 셋 걸려 있는데 **어느 것이 ACORN 이고 어느 것이 RACORN-1 인지 대응을
읽어내지 못했다.** **추측해 적지 않고 `research.papers` 를 비웠다** — 검증이 url 없는 논문
인용을 오류로 잡아 준 자리이기도 하다.

### 다음 사이클

확장 — 남은 도메인(메모리 계층). 표가 이미 있고(`when-your-collection-outgrows-ram`)
`turboquant-quantization` 과 `memory-tiers-in-qdrant-what-to-use-and-when` 이 더 있다.
