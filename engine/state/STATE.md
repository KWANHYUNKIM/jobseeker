...
**Tailscale(CA/US · SaaS—네트워킹)** `in_progress` · **도메인 3 · 기능 2** —
73곳 · ⚠️ 큐 0/3.

방금 쓴 기능: **`json-file-to-etcd`(이십 줄짜리 지름길이 백오십 메가에 닿을 때까지
버틴다)**. 결정 7개가 **전부 `confirmed`**.

### ⭐⭐ 네 후보를 버린 이유가 전부 성능이 아니다

MySQL·PostgreSQL 은 **Docker 테스트 인프라가 필요**하고 HA 시맨틱이 트랜잭션 동작을
예측하기 어렵게 만들어서, SQLite 는 창업자가 **`couldn't bring myself to make an
argument`** 라서, CockroachDB 는 **`relatively new for a database`** 라서 떨어졌다.
그리고 etcd 를 고른 이유 셋 중 하나가 **`Go 로 쓰여 테스트에 직접 링크할 수 있다`** 다.

### ⭐⭐ 같은 기준이 두 번 이겼다 — 그것을 hard_problem 으로 세웠다

**처음 20줄 지름길을 고른 이유가 `외부 의존 없는 테스트`** 였고, **9년쯤 뒤 etcd 를
고른 이유 중 하나도 테스트**다. **성능이 아니라 개발 고리의 길이가 아키텍처를 정했다.**
⚠️ 그렇게 고르면 후보가 크게 줄어든다는 지적은 재구성으로 달았다.

### ⭐ 쉬운 수를 먼저 써 보고 실패한 것도 적는다

파일을 **중요한 것과 임시(tmpfs) 것 둘로 갈랐는데** 안 됐다 — 150MB 라 **디스크 I/O 가
허락하는 최대 속도**로 쓰고 있었기 때문이다. **쪼개는 것으로는 못 넘는 벽이었다.**

### 다음 사이클 — 마지막 도메인

**두 구현을 함께 이고 간다** — `tailscale-rs-...`(전문 읽음). **점진적 in-place 재작성을
`the worst of all worlds` 라며 버렸고**, 자기 구현이 미완이라고 목록으로 밝힌다
(P2P · NAT 순회 · DNS · exit node · SSH · Taildrop · 보안 감사).
여기까지 채우면 **완주 조건에 닿는다.**

### 아직 못 잡은 것

1. **매출·사용자 수 비공개.**
2. ⚠️ **저장소 이전의 연도를 모른다** — etcd **3.4.3**(Jepsen 리포트는 2020년)이라는 단서뿐이다.
