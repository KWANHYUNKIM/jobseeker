...
**없다 — 쉰여섯 번째 후보 조사를 돌았다. 큐 1/3.** 다음 사이클은 **신규**(Astral).

### 릴리스 공지라는 이유로 걸러 내면 그 안의 설계 글을 놓친다

Astral 을 "릴리스 공지 위주"라며 접었는데, **그 릴리스 공지 안에 설계 결정이 있었다.**
`ruff-v0.4.0` 은 파서를 LALRPOP 에서 손으로 쓴 재귀 하강으로 옮긴 글이고, 물린 이유를 셋으로
적는다 — **"생성된 파서가 아이러니하게도 유지하기 더 어려워졌다"** · 생성 코드라 도메인
지식을 쓸 기회가 없었다 · **인터프리터와 린터는 이상적인 AST 가 다르다.**

### 형식이 아니라 대가다 — 세 번째 시험

지그 재단은 릴리스 노트에 거절한 기능을 적어 통과했고, 베비는 같은 형식인데 대가가 없어
접혔다. 이번 Astral 은 **접었다가 뒤집혔다.** 기준 자체는 그대로인데, **목록 단계에서 형식을
보고 거르면 안 된다**는 것이 새로 붙었다.

### 되살리기가 늘 되는 것은 아니다

이번에 다시 연 셋 중 하나만 뒤집혔다. **Shapr3D** 는 기술 글이 한 편이 아니라 넷이었지만
본문이 **CAD 일반의 개념 설명**이었고, **LiveKit** 은 유망해 보인 글이 **사용자 코드
문제 해결 안내서**였다. 접은 근거를 고쳐 적었다. 방법이 만능이라서가 아니라 **근거를 적어
뒀기 때문에 검사할 수 있는 것**이다.

### 접음과 못 읽음을 계속 가른다

**Hugging Face** 는 첫 화면에서 자기 인프라 글을 못 찾았는데, **첫 화면으로 판정하지
않는다는 규칙대로 '못 읽음'으로 적었다.** 아카이브와 검색을 쓸 수 없다(이 세션의
WebSearch 예산이 끝났다). **CAD 축은 계속, 의료 축은 일곱 번째로 비어 있다.**
