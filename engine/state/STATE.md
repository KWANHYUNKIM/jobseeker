...
**없다 — 문체 보수 사이클이었다.** Monzo 를 배민 문체로 다시 썼다(2.03 → 0.0). 회사 115개, 큐 2/3. 29곳 남았다.

### 이전을 누가 미느냐 — 답이 셋이 됐다

첫 기능에 축을 세웠다.

- Etsy 는 테이블을 하나씩 올리다 **가장 위험한 것들이 묶여 있다는 것을 발견**했다.
- DoorDash 는 아예 **신규 기능 개발을 전부 멈추고** 재설계에 들어갔다.
- Monzo 는 조직 쪽에서 답을 냈다 — **한 팀이 2,800개를 전부 민다.**

그 값을 숨기지 않는다. 한 팀이 다 하면 그 팀이 병목이고, 팀마다 하면 아무도 안 끝낸다.
분산으로 해 봤더니 끝나지 않은 이전들이 남았기 때문에 고른 쪽이다 — **미완성 이전은 안 끝난 일이
아니라 두 벌을 영원히 함께 운영하는 일이다.**

### 다 통과하고도 사고가 났다 vs 시험한 것이 다른 것이었다

- Anthropic 은 여러 겹의 리뷰와 시험과 자체 사용을 **다 통과하고도** 버그가 운영에 닿았다고 적는다.
- Monzo 는 통과한 것이 아니라 **시험한 것이 다른 것이었다** — 1대로 시험했는데 정족수는 대수가
  아니라 다수결이라, 6대를 넣자 파티션당 2~3벌이 옮겨져 합의가 깨졌다.

그리고 Wise 의 압축 백로그와 같은 결론에 이른다 — **겪기 전에는 어떤 컴포넌트에 신호가 없는지
알 방법이 없다.**

### 규칙을 사람이 아니라 파이프라인이 지키게 한 둘

- Deliveroo 는 규정 준수를 회의에서 배포 파이프라인으로 옮겼다.
- Monzo 는 데이터 표준을 CI 에 심어 어기는 것만 자동으로 막는다.

둘 다 문지기가 되지 말고 자동화하라는 같은 문장을 다른 도메인에서 쓴다. 다만 값이 따라온다 —
**막는 것은 자동인데 정하는 것은 여전히 중앙이다.** 선택지를 줄이는 것이 곧 팀 자율을 줄이는 것이고,
그 규칙을 코드로 만들어 계속 유지해야 한다.

### 도메인을 제품이 아니라 작게 쪼갠 값을 어디서 치르느냐로 잘랐다

계좌·카드·대출로 가르면 셋이 흩어진다. 3,000개를 같은 모양으로 유지하는 값, 그 뒤의 상태를 한
곳에 모아 둔 값, 팀 100개의 데이터가 서로를 깨뜨리는 값 — **셋 다 쪼개기로 한 하나의 결정이 만든
청구서다.**

### 다음 사이클 — 문체 보수 (사다리 3순위)

--gaps 가 정한다. 지금 맨 위는 trainline(2.02)이지만 그건 이 시점의 예상이지 확정이 아니다.
