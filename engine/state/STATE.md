...
**없다 — Block 을 완주했다.** 도메인 **5** · 기능 **5** · `status: done`. 회사 109개. ⚠️ **큐 2/3 — 다음 사이클은 후보 조사다.**

### ⭐ 지난 사이클에 안 닫힌다고 적어 둔 구멍이 이번에 닫혔다

지난 사이클에서 **`커머스 인에이블먼트(Square)` 수익원이 어느 기능과도 안 이어진다**고 적고 `done` 을 미뤘다. ⭐ **그 메모가 이번 사이클의 대상을 정했다** — `--gaps` 는 신규를 지목했지만 **`한 회사를 완주할 때까지 다음 회사로 안 넘어간다`** 가 위다.

⚠️ **찾는 방식이 문제였다** — 앞선 사이클들에서 블로그 첫 페이지만 봤다. **2·3페이지를 넘겨 50편 목록을 훑으니** 결제 본업 인프라가 나왔다. ⭐ **`Square 결제`로 검색해서는 안 나오고 목록을 넘겨야 나왔다** — **회사가 본업을 본업의 이름으로 부르지 않는다**(재구성).

### 이번 사이클 — `메시지 안에서 필드를 잠근다` (새 도메인 `누가 어디까지 볼 수 있나`)

⭐ **결제 회사에서 `이 서비스가 이 데이터를 봐도 되나`는 조직도로 답할 수 없다** — 하나의 거래 메시지 안에 **금액과 우편 주소와 은행 계좌 정보**가 같이 들어 있기 때문이다(재구성). ⭐ **회사는 접근선을 서비스가 아니라 필드에 긋는다:**

**`웹 UI 서비스가 고객의 우편 주소를 읽지 못하고 은행 계좌 정보에는 더더욱 접근하지 못하면서도 기본 거래 내역은 보여 줄 수 있다.`**

건진 것 넷:

- ⭐ **인프라 보안으로 충분하다고 하지 않는다** — **`통신 채널을 지키고 인프라 수준의 통제를 두는 것도 중요하지만, 이벤트 기반 시스템은 메시지 계층의 추가 보호에서 이득을 볼 수 있다.`** ⚠️ **Kafka 메시지는 원래 여러 소비자에게 가라고 만든 것이라 채널을 지켜도 `누가 무엇까지 읽었나`가 안 갈린다**(재구성).
- ⭐ **키와 데이터의 관계를 스키마에 적는다** — 커스텀 proto 옵션 **`key_alias`** 로 묶어 **코드에서 검색 가능**하게. ⚠️ **그러면 스키마 변경이 곧 보안 변경이 된다.**
- ⭐ **키 없음을 실패가 아니라 상태로 다룬다** — **무시할 키 목록**을 주면 일부만 풀고도 성공하고, **`decryptPartially`** 는 **부분 복호화된 메시지와 접근 못 한 키 이름 목록을 함께** 돌려준다. **이걸 예외로 다루면 이 구조가 아예 안 돈다**(재구성).
- ⭐ **키 계층을 설계하지 않고 드러나게 뒀다** — **`이 동작이 자연스럽게 키 계층 구조를 만들고, 암호화 키를 조직하는 더 논리적인 구조를 드러낸다.`** **조직도가 아니라 데이터 접근 패턴이 구조를 만든다.**

규모: **하루 18테라바이트**(1년 반 전 9TB에서) · **Tink 키 1000개 가까이** · **300개 서비스와 워크로드.**

### ⭐ Block 완주 관찰 — `못 하는 것을 못 한다고 적는 회사`

**다섯 기능 전부가 자기 한계를 문장으로 남긴다**(재구성):

- **Moneybot** — **`관측할 수 없는 행동을 고치려 들지 않는다.`**
- **COLDCARD 자문** — **`완전한 경험적 시험은 하지 않았다`**(그래도 악용이 진행 중이라 이르게 낸다).
- **벌 계층** — **`열두 구성 중 어느 것도 값을 생각하면 솔로를 앞서지 못했다`**(자기 가정을 깬다).
- **MeshLLM** — **`믿는 사람과 공유하라`**(프라이버시를 기술로 못 풀었다).
- **필드 암호화** — **키가 없어 못 푼 것을 감추지 않고 `접근 못 한 키 이름`으로 돌려준다.**

⭐ **새 비교 축 ㊵ — `모르는 것을 어떻게 적나`.** ⚠️ **그리고 ㉘축(`맞았는지 어떻게 아나`)에 이 회사 혼자 세 칸을 채웠다**(표본 수 · 경우의 수 · 가격과 벽시계). **비교 문서 1순위가 더 굳었다.**

### 다음 사이클 — 후보 조사 (3순위)

**큐 2/3.** ⚠️ **먼저 `name_en` 목록 출력으로 시작한다.** ⏳ 대기에 남은 둘 — **Klaviyo**(⚠️ 연도 확인 먼저) · **Perplexity**. ⭐ **`큰 이름인데 아직 없는 회사` 각도가 네 조사 연속 통했다** — ⏳ Anthropic · Databricks · Snowflake · Anduril · Ramp 외 핀테크.

⏳ **Block 보강 거리** — `BlockTabBench` · `Buzz 프로젝트` · `Microfilm` · `Who's Asking?`(에이전트·서비스 메시 신원 위임) · `CodeCrucible`(LLM 기반 SAST) · `18Mb of HashMap overhead` · `From Polyrepo Fragmentation to Monorepo Leverage`. **⭐ 남은 물음 — Bitkey 는 난수를 어떻게 만드나.**
