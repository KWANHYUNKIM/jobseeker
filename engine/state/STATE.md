...
**Etsy** (US · 커머스) — 도메인 3개 · **기능 2개**. **남은 것은 LLM 검색 하나이고, 그것이 완주를 막고 있다.**

### 이번 사이클 — 실험 `부분의 합이 전체가 아니라는 것을 분기마다 실제로 잰다`

⚠️ **자료를 찾다가 목록에 없던 글을 하나 더 발견했다**(`Understanding the collective impact of experiments`, 2022-12). **자기 A/B 프레임워크의 편향을 공개하는 글이다.**

건진 것 넷:

- ⚠️ **자율이 상호작용을 낳는다** — 팀마다 자기 실험을 돌리면 속도는 나는데 **같은 페이지에서 처치들이 섞인다.** 회사가 한 문장으로 적는다: **`It would be naive to assume that the impact of the whole was just the sum of the isolated parts.`**
- ⚠️ **소통으로 막을 수 있는 건 한계가 있다** — 눈에 보이는 충돌(주황 내비바 + 주황 장바구니)은 조율로 피할 수 있지만, **검색·광고·추천이 사용자의 제한된 주의를 두고 서로 트래픽을 잠식하는 것**은 `hard to anticipate and to quantify` 다. **그래서 막는 대신 재기로 했다.**
- ⚠️ **자기 측정 도구가 자기에게 유리하게 치우친다는 것을 공개한다** — `we are **systematically prone to overestimating** the impact sizes of treatments whose positive effects led us to select them for deployment`(승자의 저주). **좋아 보여서 골랐다는 사실 자체가 그 효과를 부풀린다.**
- **재는 방법이 단순하다** — 분기 내내 **약 8% 를 모든 실험에서 빼 두었다가**, 분기 말에 **그 절반에게 채택된 모든 처치의 조합**을 몇 주간 보여 준다. ⚠️ **그리고 총 효과와 개별 합의 간극을 `실험 관행의 결함을 찾는 신호`로 쓴다.**

**공개한 한계가 더 있다** — 지표에 안 잡히는 **중립적 처치가 잡음**이 되고(디자인 리뷰나 사용자 조사로 채택되거나 **다음 개선의 디딤돌**인 것들), **해상도에 정답이 없으며**(전사 하나는 신호 희석, 페이지별은 분석 파편화), 남은 큰 물음은 **외적 타당도**(시간과 공간의 외삽)라고 스스로 적는다.

### 다음 사이클 — LLM 검색, 기능보다 자료가 먼저다

⚠️ **`tech` 가 1 이고 그것도 제목 수준이다**(검증 경고는 안 뜨지만 근거가 얇다). `--gaps` 가 **1억 개가 서로 다른 물건에서 원하는 것을 찾게 한다** 를 부를 텐데, **기능을 쓰기 전에 자료를 찾는다** — **여섯 번 통한 규칙**이다.

찾을 곳: 검색 관련성(2026-01) · 재고 이해(2025-10) · 구매자 프로파일(2025-09) · `Shaping Product Understanding with Contrastive Reinforcement Learning`(2026-05) · `Making Ads Count: MMoE`(2026-03). ⚠️ **브라우저로 열어야 한다.**
