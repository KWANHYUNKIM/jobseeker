...
**없다 — Trigger.dev 를 완주했다(도메인 2 · 기능 2).** 큐 0/3 이라 다음 사이클은 **후보 조사**다.

### 한 회사의 두 도메인이 같은 손버릇을 보인다

사고 보고서가 **'우리 복구 자동화가 상황을 더 나쁘게 만들었다'** 고 적듯, 성능 글은
**`--bytecode` 가 정상 상태 성능을 오히려 나쁘게 했다**고 적는다. **재 보고 되돌린 것을
남기는 것**이 두 도메인에 공통이다 — 도메인을 따로 읽으면 안 보이는 자리다.

### 런타임을 두 번째로 둔 순서가 중요하다

**자기 코드 문제를 런타임 탓으로 돌리지 않았다.** 시간의 31%를 먹던 SQLite 질의를 먼저
없애 **2.2배**를 벌고 나서 Bun 으로 옮겨 **추가 2.1배**를 벌었다. 순서가 반대였으면
**런타임이 벌어 준 몫을 과장했을 것**이다.

### 고치는 대신 한도를 건다

로그 222k 개를 잘 그리는 방법을 찾는 대신 **25,000 에서 자르고 나머지는 내려받게** 했다.
**꼬리를 감당하려다 몸통을 잃지 않는 쪽**이고, **사용자가 잃는 것을 그대로 적는다.**
컨벡스가 `count()` 를 안 준 것과 같은 결인데 **이쪽은 한 번 터지고 나서 건 한도**다.

### 만든 쪽 글에는 없는 목록

Bun 으로 옮기며 깨진 것을 **성과보다 길게** 적는다 — 프로미스 결말 의무(안 지키면 상태가
영원히 남는다 · RSS 250MB), prom-client 지표 넷 무효, **AbortSignal 이 두 번 뜸**,
쿠버네티스 CA. **번(Bun) 자체 프로파일과 마주 보는 자리**로 걸어 뒀다.

### 고친 뒤의 값은 안 잰다

이벤트 루프 글이 **배포 뒤 CPU 가 얼마나 줄었는지, 위반이 얼마나 뜸해졌는지**를 재지 않는다.
**측정 도구 자체의 오버헤드**도 없다 — 사고 보고서에 **평소 기준선이 없는 것**과 같은 빈칸이다.
