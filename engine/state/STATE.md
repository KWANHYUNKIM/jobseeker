...
**없다 — 이번 사이클은 후보 조사였다. 큐를 0/3 에서 2/3 로 채웠다.** 회사 153개.

### 올린 둘 — 축을 벌렸다

**Chainguard(US)** — 소프트웨어 공급망·재현 가능한 컨테이너 빌드 축이 없었다. `This Shit is
Hard` 연작이 이 엔진이 찾는 바로 그 형태다: 스탬피드를 길들이며 **물린 대안 셋을 이름 대어**
적고(지터 · 두 레인 워크큐 + 리컨실러별 리더 선출 · 지연 전달 큐 파킹) 수치를 붙인다(50만 키가
**분당 약 350키의 배경 잡음**으로). **SourceHut(US)** — 작은 팀이 자기 하드웨어로 포지를 돌리는
축. **170시간 장애를 통째로 공개**하고 클라우드플레어 후원 제안을 물린 이유(종단간 암호화)를 적는다.

### 목표를 못 채운 것을 채운 척하지 않았다

3곳이 목표인데 **2곳만 올렸다.** 큐가 2/3 이라 다음 조사 사이클이 다시 뜬다 — 후보를 억지로
통과시키는 것이 이 데이터를 망치는 길이다.

### 접은 하나와 못 읽은 셋을 갈라 적었다

접은 곳 — **Neo4j**(GenAI·제품·고객 사례가 지배하고 **발행일조차 없다**). 못 읽은 곳 —
**incident.io**(홈이 `/blog` 를 가리키는데 404) · **Sourcegraph**(403) · **Fireworks**(목록이
모델 공지에 지배돼 커널 글 한 편만 보였고 본문을 못 열었다).

### 본문을 열어 판정한 것이 또 값을 했다

Chainguard 는 열어서 통과가 굳었고, **SourceHut 은 열어서 '최근 글이 얇다'는 첫인상을 뒤집었다**
(최근은 ToS·가격인데 사고 사후 기록이 두껍다). 반대로 Neo4j 는 목록만으로 접혔다.

### 다음 사이클 — 신규 (대기 맨 위의 Chainguard)

`Taming the Thundering Herd`(2026-09-02) · `1 billion build manifests`(2026-09-03) ·
`고칠 수 없는 취약점 패치하기`(2026-08-17) · `Athena 샌드박싱`(2026-07-29)이 손에 있다.
