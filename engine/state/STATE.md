...
**없다 — 문체 보수 사이클이었다.** Wiz 를 배민 문체로 다시 썼다(3.87 → 0.0). 회사 115개, 큐 2/3. 69곳 남았다.

### 절차가 셋으로 굳었다

Neon 은 손으로 다 고쳤고 Cursor 에서 변환기를 만들었는데, Wiz 를 하며 순서가 확실해졌다.

1. 변환기를 돌린다 — 기호·백틱 인용·굵게가 0 이 된다. 이름 앞뒤 공백도 이때 정리한다.
2. 읽는 사람이 먼저 보는 곳을 손으로 다시 쓴다 — 사업 모델, 도메인 why, 기능 이름과 business.why.
3. 조사가 붕 뜬 자리를 찾아 고친다. 정규식으로 "한글 뒤에 이고/를/가" 가 이어지는 자리를 훑으면
   대부분 걸린다. Wiz 에서 아홉 군데였다.

세 번째가 이 작업의 실제 비용이다. 인용부호를 풀면 "에이전트 없이 는 이 회사의 기술 선택이자
이름이었다" 처럼 조사가 뜬 문장이 남는데, 기호만 걷고 여기서 멈추면 읽기가 더 나빠진다.

### 이번 사이클 — Wiz

사업 모델에서 이 회사의 기술 선택이 곧 영업 속도였다는 것을 앞세웠다. 워크로드마다 에이전트를
심지 않으니 계정만 연결하면 몇 시간 안에 위험 그래프가 나오고, 배포할 것이 없다는 점이 도입
장벽을 없앴다. ARR 1억 달러까지 18개월이 걸렸다.

앞선 회사와 견주는 문장을 넣었다. 이 엔진의 Snyk 도 같은 보안 축인데 저쪽은 개발자의
의존성에서 시작하고 여기는 클라우드 자원에서 시작한다.

도메인 경계의 근거도 적었다. 데이터를 찾는 일과 그 결과를 판정하는 일을 따로 볼까 했으나
회사가 되돌린 결정이 정확히 그 경계에서 일어나 한 도메인으로 뒀다.

기능 이름에 핵심을 붙였다. `먼저 제약 없이 만들고 그다음 AI 를 걷어낸다 — 되돌린 결정`,
`에이전트를 안 쓰던 회사가 에이전트를 낼 때 — 이름과 부딪히는 제품`,
`위험이 정당화하는 만큼만 깊이 판다 — 값비싼 모델을 어디에 쓸까`.

### 다음 사이클 — 문체 보수 (사다리 3순위)

무거운 순으로 coinbase(3.83) · victoriametrics(3.78) · vercel(3.75) 이 남았다.
