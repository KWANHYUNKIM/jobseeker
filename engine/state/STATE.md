...
**Ente — 프로파일만 썼다(도메인 셋, 기능 0).** 회사 147개, **큐 0/3**.

### 파는 것이 기능이 아니라 못 보는 것이다

종단간 암호화와 영지식을 내세우는 순간 서버는 사진을 열 수 없다. 그러면 보통 서버가 하던
일 — 얼굴 묶기 · 자연어 검색 · 비슷한 사진 찾기 — 이 **전부 기기로 밀려난다.** 5만 장
라이브러리에서 25억 번을 대조하는 일이 그대로 폰에 얹힌다.

### 그래서 세 도메인이 한 줄로 이어진다

암호가 서버를 눈멀게 하고(**열 수 없게 만들고 그것을 증명한다** — X25519+ML-KEM-768 혼합,
캐스트 흐름부터) → 계산이 기기로 가고(**서버가 못 하는 계산을 기기에서 한다** — USearch
HNSW 512차원, 30분→30초) → 서버에 남는 일은 **안 잃는 것**뿐이다(**잃지 않는 것으로 값을
받는다** — 클라우드 셋에 세 벌, 한 벌은 불변 규정 잠금).

### 자기 숫자를 여는 회사다

2026년 7월부터 매달 자동 공개한다 — 활성 구독 매출 78만 → **133만 6,896달러**, 유료 고객
12,018 → **20,703명**, 등록 계정 29만 → 46만. 벤처 자본에 안 기댄다고 밝힌다. **비용·인원은
안 낸다** — 매출은 알지만 남는지는 모른다.

### 다음 사이클 — 확장 (Ente 의 첫 도메인)

`서버가 못 하는 계산을 기기에서 한다` 가 자료가 가장 두껍다(벡터 DB 글에 수치가 다 있다).
CLIP 검색(2023-11-18)과 ML 출시(2024-11-30)를 더 읽으면 층이 는다.

### ⚠️ 큐가 0/3 이다

Ente 를 완주시킨 뒤 **후보 조사 사이클을 끼운다.** 지난 조사에서 얻은 판정 기준을 이어 쓴다 —
회고를 먼저 찾고, 되돌린 기록을 찾고, **자기 것을 쓰지 말라고 적는 글**을 찾는다(프로토맵스가
그 하나로 자격을 얻었다).
