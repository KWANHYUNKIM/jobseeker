...
**없다 — `in_progress` 0 · 완료 55곳.** 327 에서 **Freshworks 를 닫았다**(도메인 3개 · 기능 3개).

**닫은 근거 — 남은 글 셋을 열어 보고 전부 못 읽었다.**
- **Mozilla rr 메모리 손상 디버깅** — **주소를 두 형태로 시도해도 404**(`/saas/<slug>/` 와
  `/saas/eng-blogs/<slug>/`). **검색 조각으로는 내용이 보인다** — 스테이징에서 재현이 안 되는
  충돌, rr 이 Redis·MySQL·gRPC 호출까지 기록해 되돌려 재생하므로 워치포인트와 역방향 실행이
  되고, 원인은 **`Delayed::MessageSending` 의 상위 클래스가 마이너 GC 마크 단계에서 표시되지
  않아 스윕에 해제된 것**, 권고는 **syck → psych**. **⚠️ 그러나 1차 자료의 본문이 아니라
  기능으로 안 썼다**(325 의 MemoizeUntil 과 같은 판단). **⭐ 주소가 살아나면 값이 크다** —
  Doximity `prod-profiling` 의 **재는 행위 자체가 값을 치른다**는 축과 정면으로 만난다.
- **기능 토글** — **마케팅 허브로 리다이렉트**된다(MemoizeUntil 과 같다).
- **⭐ 그리고 네 번째 경로를 새로 봤다** — `freshworks.com/eng-blogs/managing-rails-
  application-secrets-with-encrypted-credentials-blog`. **아직 안 열어 봤다.**

**닫으면서 `business_model` 에 적은 것**(이 사이트의 정리라고 밝혔다)
- **⭐ 대가를 적는 정도가 글마다 크게 갈린다** — 고가용성 편 **다섯**(Newrelic explain 쿼리가
  Semian 감시를 방해해 껐다) · 업그레이드 편 **다섯**(실제로 터진 사고 셋) · HAProxy 편
  **두 줄** · Sidekiq 편 **없음**. **글쓴이에 따라 갈리는 것으로 보인다**(추정).
- **⭐⭐ 그런데 대가를 가장 많이 적은 두 편이 결과의 근거는 가장 적게 적는다** — 업그레이드
  편은 **무중단·회귀 0** 이라면서 트래픽도 오류율도 전환 기간도 없고, 고가용성 편의 효과는
  **그래프 두 장**뿐이다. 설정값도 **단위 없는 25** 하나가 전부다.
- **⚠️ 블로그가 네 경로에 흩어져 있고 목록 페이지가 안 열리며 살아 있는 글과 죽은 글이
  섞여 있다** — **글 전체 목록을 끝내 확인하지 못했다.**

---

**다음 사이클 — `--gaps` 가 [후보 조사]를 가리킨다. 큐 0/3 이고 `in_progress` 도 0 이다.**
**이번엔 미룰 이유가 없다**(318 에서 네 번 미룬 뒤 실제로 했고 넷 중 둘을 올렸다).
- **일본** — Sansan · freee · SmartHR · Money Forward. 일본은 이미 넷이라 우선순위가 낮지만
  **접근이 잘 되는 편이다.**
- **유럽** — Personio.
- **⚠️ 인도는 소진됐다**(Zerodha·Paytm·Freshworks 완료 · Zoho·Ola 는 재시도 안 함 · 나머지는
  전부 막힌 목록). **Trade Republic 은 Medium 호스팅이다.**
- **방법은 318 과 같다** — **⑱ `WebSearch` + `allowed_domains`**, **⑰ 주소를 추측하지 않는다**,
  **⚠️ 한 편은 반드시 열고 첫 편이 얇으면 한 편 더 연다**, 기준은 **버린 대안·대가·수치 중 둘**,
  **⑲ 올리기 전에 `index.json` 완료 목록과 대조.**
- **⭐ 327 이 새 교훈을 하나 더 준다** — **주소가 열리는지도 후보 조사에서 확인할 항목이다.**
  Freshworks 는 글이 있는데 **절반이 안 열렸다.** 318 에서는 본문 두 편을 읽어 올렸고 그건
  맞았지만, **회사 전체의 접근성은 그때 안 봤다.**

**📌 그 밖**
- **⭐ 비교 문서와 이어지는 자리 셋** —
  ① 320 `leave-what-costs-more`(Paytm) → `reversible-first.md` 의 *"되돌리지 못해 앞으로 밀고
  나간 사례가 없다"* 와 `where-it-piles-up.md` 의 **갚지 않고 이사한 쪽**.
  ② 326 `know-both-versions`(Freshworks) → `reversible-first.md` 의 *"언제 롤백 경로를 닫는지
  정한 곳이 없다"* 에 **첫 재료**(끝나고 if/else·몽키패치·버전별 캐시 키를 지웠다고 적는다).
  ③ 326 은 `swap-while-running.md` 의 **앱이 아는가 모르는가** 축에서 **앱이 아는 쪽**의
  재료이기도 하다(**추정** — 그 문서를 다시 읽고 확인한다).
  **⭐⭐ 비교 문서 사이클을 한 번 끼울 만큼 재료가 모였다.**
- **후보 조사 누적 열여섯 곳 중 넷.**
- **⚠️ 낡은 메모를 나르지 않는다**(314·317·325). **⚠️ 두 방향 규칙**(298·305·318·321).
- **분포** — **55곳 전부 `done`.** US 22 포화 · 한국 아홉 · 일본 넷 · EU 일곱 · 중국 둘 ·
