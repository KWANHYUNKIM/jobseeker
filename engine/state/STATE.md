...
**없다 — Supabase 를 완주시켰다(도메인 2 · 기능 2 · `status: done`).** 회사 171개, **큐 0/3.**

### 비싼 것이 다 결함은 아니라는 것을 명시한다

'구독자 100명이면 INSERT 하나에 인가 질의 100개'는 보통 **결함으로 읽히는 수치**다. 그런데
회사가 먼저 **"전형적인 워크로드에서 정확성과 낮은 지연을 생 처리량보다 앞세운 의도적인
설계 선택"** 이라고 적는다. 이 엔진이 170개 회사를 파면서 **비싼 값을 설계 선택이라고 이름
붙인 글은 드물었다.**

### 한도가 곧 용도의 선언이다

Broadcast Replay 는 **3일에 25개, 비공개 채널만**이다. 넉넉히 줄 수도 있었는데 그러면
**사람들이 Realtime 을 복제 도구로 쓰기 시작한다.** 한도를 좁게 둔 것이 곧 '이건 따라잡기용
이지 복제용이 아니다'라는 말이다.

### 기능으로 못 막는 오용은 글로 막는다

Realtime 으로 내구성 복제를 짜면 **끊기지 않는 환경에서는 진짜로 잘 동작한다.** 그래서
테스트를 통과하고 프로덕션에서, 그것도 **로그에 오류 없이** 데이터가 비는 방식으로 드러난다.
**개발 환경이 프로덕션과 다르다는 사실 자체가 이 버그의 원인**이라 남는 방법이 문서뿐이었다.

### 두 도메인이 같은 재료를 반대로 쓴다

한쪽은 **복제를 HA 의 재료로**, 다른 쪽은 **복제 슬롯을 밖으로** 쓴다. 둘 다 '수정하지 않은
Postgres 가 주는 것'만으로 지어서, **WAL 보존 한도 같은 Postgres 의 성질이 그대로 제품의
한계**가 된다.

### 안 다룬 질문 하나가 크다

**저지연과 내구성이 둘 다 필요한 경우**를 글이 정면으로 답하지 않는다. 둘을 각각 쓰라는
뜻으로 읽히는데, 그러면 **같은 변경이 두 경로로 두 번 나간다** — 그 비용과 정합성 문제를
적지 않았다.

### 다음 사이클 — 후보 조사 (큐가 0/3 이다)

**CAD 축 세 번, 의료 축 여덟 번 실패.** 못 읽은 곳: Hugging Face · Lunit(medium 403) ·
Blender(도메인 전체 403) · Sourcegraph(도메인 전체 403) · dbt Labs · incident.io(404 네 번) ·
M3 · Guardian · Flexport · Cal.com. **이 세션의 WebSearch 예산은 끝났다** — 목록 페이지에
href 를 물어 받는 방법만 남았다.
