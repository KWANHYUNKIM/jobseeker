...
**Monzo** (EU·UK · 핀테크) — **프로파일까지 썼다.** 도메인 3개, 기능은 아직 0이다.

### 이번 사이클 — 신규(6순위), Monzo 프로파일

**앞의 핀테크들과 갈리는 지점이 규모가 아니라 성격이다** — 송금이나 결제 중개가 아니라 **은행 그 자체**라서, 멈추면 사람들이 카드를 못 긋고 잔액을 못 본다. 2019년 장애 글이 그 목록을 그대로 적는다(로그인 불가·카드 거래 실패·잔액 누락·지원 채팅 중단). **가용성이 제품의 일부가 아니라 제품 자체인 자리다.**

세운 도메인 셋:
- **서비스 3,000개를 한 저장소에서 같은 모양으로 굴린다** — 모노레포 · 도구를 감싼 자체 서비스 · **다자 승인**. `treating platform tooling as a product`. **작게 쪼개기로 한 순간부터 진짜 일은 쪼갠 것들을 같은 모양으로 유지하는 쪽으로 옮겨 간다.**
- **은행의 상태를 한 클러스터에 몰아 두고 그 대가를 치른다** — 카산드라 21노드. ⚠️ **`auto_bootstrap: false` 가 하는 일이 하나가 아니었다.**
- **팀 100개가 만드는 데이터에 계약을 건다** — dbt 12,000개 · 4계층 · Modelgen · **CI 가 PR 을 막는다.**

⚠️ **매출 구성을 못 얻었다** — 연차보고서 요약 페이지가 **총액(12억 파운드·+48%)과 성장률만** 적는다. 3자 출처들이 순이자수익을 £164.2m 로 말하는데 **총매출과 견주면 비율이 이상해** 쓰지 않았다. **PDF 본문을 열어야 한다.**

### 다음 사이클 — 확장

**장애 회고 도메인을 먼저 파는 것이 낫다**(추천). ⚠️ **이 회사에서 대가가 적힌 곳이 거기다** — **플랫폼 글에는 버린 대안이 하나도 없다**(모노레포도, 감싸는 방식도, 다자 승인도 왜 다른 길이 아니었는지가 없다). 데이터 글은 전제와 원칙은 적지만 `무엇을 잃었나` 가 약하다.

**한 회사 안에서 글의 결이 갈리는 여덟 번째 사례이고, 이번에는 그것을 프로파일 단계에서 미리 알고 들어간다.**

⏳ 안 본 축 — `building-agent-chip` · `engineering-the-future-of-customer-operations`(**LLM 에이전트의 평가 체계**를 만들었다고 적는다 — **Sentry 와 비교할 재료**).
