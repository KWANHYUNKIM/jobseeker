...
**Cockroach Labs**(US · 분산 SQL) — 회사 프로파일까지 썼다. **도메인 둘 · 기능 0.** 큐 0/3.

### 한 대에서는 쉬운 것을 흩어진 상태에서 다시 푼다

이 회사를 세우는 축이다. PostgreSQL 호환을 앞세우니 **한 대짜리 데이터베이스에서 당연한
것을 분산 위에서 다시 만들어야** 하고, 그 값이 엔지니어링 글마다 나온다. 도메인 둘이 각각
그 사례다 — 저장 엔진의 컴팩션과, 트랜잭션 안에 얹는 벡터 색인.

### 대가를 세 겹으로 세는 글

Pebble 값 분리 글이 그렇다. 쓰기를 줄였더니 ① **읽기에 간접 참조**가 붙고 ② 시간이 지나면
**지역성이 무너지고** ③ 죽은 값이 쌓여 **공간 증폭**이 생긴다. 셋을 다 적고 각각에 대응책을
붙인다 — 그리고 그 대응책이 다시 컴팩션이라는 것도 읽힌다.

### 자기 이득을 자기가 깎는다

같은 글에서 MVCC 가비지 분리의 이득을 **"시스템 상위의 몇몇 미묘한 사정 때문에 아직 온전히
실현하지 못한다"** 고 적고, **`AS OF SYSTEM TIME` 질의가 읽기 대역폭을 지배해 이론적 이득의
상당 부분을 상쇄한다**고 밝힌다. 47% 라는 수치를 내놓은 같은 글에서다.

### 기성 방법이 안 되는 이유를 조건으로 적는다

C-SPANN 글은 HNSW 니 IVF 니 이름을 대는 대신 **왜 안 맞는지를 조건으로** 적는다 — 중앙
조정자를 요구하거나, 큰 인메모리 구조에 기대거나, 순차 네트워크 순회가 필요하거나, 샤딩과
맞지 않는다. **이름이 아니라 제약으로 물리는 방식**이라 다른 회사에도 그대로 옮겨 읽힌다.

### 등급마다 과금 단위가 다르다

Basic 은 **요청 단위(RU)**, Standard·Advanced 는 **vCPU 시간**이다. 위로 갈수록 파는 것이
계산량이 아니라 **약속**이 된다 — 99.999% SLA, CMEK, PCI·HIPAA, 그리고 **업그레이드 일정을
고객이 정하는 권한**.

### 다음 사이클

빈 도메인 둘 중 하나. 자료 두께는 **Pebble 쪽이 낫다**(대가 셋 · 수치 · 자기 이득을 깎는
대목). 아직 안 연 글 둘(복원력 스트레스 시험 2025-06-11 · MOLT Verify 2025-04-02)이
세 번째 도메인이 될 수 있다.
