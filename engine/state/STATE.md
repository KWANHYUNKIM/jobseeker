...
**없다 — 문체 보수 사이클이었다.** 메이퇀을 배민 문체로 다시 썼다(1.31 → 0.0, 기호 462개). 회사 115개, 큐 2/3. 15곳 남았다.

### 🔴 가 여섯 번째다 — 220개

사이보우즈·하이퍼커넥트·SmartHR·컬리·산산에 이어 여섯 번째다. 중국어 원문 인용 60여 종도 우리말로
풀었다(글 제목과 제품 이름 — 数字站长·图灵评测·千面代码 같은 것 — 은 그대로 뒀다).
엔진 내부 번호(`223 의 규칙`)도 걷어냈다.

### 재는 것 자체를 의심한 회사가 다섯이 됐다

- Anthropic — 무대.
- Etsy — 자기 실험 체계.
- Fastly — 무엇을 맞혀야 하는지.
- 하이퍼커넥트 — 목표 그 자체.
- 메이퇀 — **재는 대상이 커졌다는 사실.** 모델 하나에서 모델과 시스템과 도구와 절차로 옮겨 갔고,
  물음도 정확한가에서 결과를 안정적으로 내놓을 수 있는가로 바뀌었다.

답이 소박하다 — **애매한 기준을 예와 아니오로 쪼갠다.** 지표가 복잡할수록 실행하기도 맞추기도
어렵기 때문이다.

### 사람을 먼저 세우는 순서가 셋이 됐다

Etsy(사람 골든 라벨에 LLM 정렬) · 산산(사람과 기계가 섞인 공정을 잰다) · 메이퇀. 여기는 그것을
한 문장으로 못박는다 — **먼저 사람끼리 맞추고 그다음에 사람과 기계를 맞춘다. 진짜 병목은 도구가
아니라 사람에 있다.** 기술 부채를 업무 요구에 딸린 곁작업으로 쪼개 리팩터링 전용 시간을 하루도
안 받았다는 대목도 드물다.

### 말이 되는지와 실제로 되는지

터소가 재현을 설계에 넣은 것과 같은 자리인데 방향이 다르다. 여기는 **가르칠 데이터를 만들 때
검증을 함께 만든다** — 기존 보상 모델은 지시가 글로 말이 되는지만 보고 실제로 실행될 수 있는지는
못 보기 때문에, 없는 버튼을 누르라는 지시가 걸러지지 않는다.

### 도메인을 사업이 아니라 무엇을 좋다고 부를지 정하는 자리로 잘랐다

검색이 무엇을 올릴지, 모델을 무엇으로 잘한다 할지, AI 가 쓴 코드를 받아들일지, 만든 지시가 실제로
되는지 — **넷 다 판정의 문제다.** 그리고 이 회사의 손익은 기술보다 경쟁 상황에 먼저 흔들린다는 것도
사업 모델에 남겼다.

### 다음 사이클 — 문체 보수 (사다리 3순위)

--gaps 가 정한다. 지금 맨 위는 cyberagent(1.3, 437개)지만 그건 이 시점의 예상이지 확정이 아니다.
