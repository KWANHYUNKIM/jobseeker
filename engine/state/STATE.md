...
**아무 회사도 안 판다 — 후보 조사 사이클이었다.** 큐 0/3 → **3/3**. 회사 141개.

### 이번에 올린 셋

- **AuthZed(SpiceDB)**(US) — 인가 축이 없었다. 질의 계획기 벤치마크(2026-05-20)가 시나리오
  (30단계 중첩·팬아웃)와 최적화(화살표 뒤집기·가지 재정렬·회전·가지치기), 수치(11배·47%·62%),
  대가(망 비용과 캐시의 맞바꿈, 지금은 과하게 디스패치한다)를 다 적는다.
- **turbopuffer**(US) — 검색 엔진 내부 축이 없었다. 객체 저장소 위의 역색인, BM25 질의 길이와
  속도, MAXSCORE 대 WAND, SIMD 대 러스트 추상화.
- **Deno**(US) — **방금 판 Val Town 의 런타임을 만드는 쪽**이다. 믿을 수 없는 코드 격리를
  양쪽에서 볼 수 있는 첫 사례다.

### 교훈 — 부품이 된 회사를 반대편에서 본다

이 엔진에는 남의 글에 부품으로 등장한 회사가 여럿이다(템포럴이 Railway 의 제어면에, 타이거비틀이
Zig 재단 후원사로, 클릭하우스가 PostHog 의 저장소로). Deno 는 그중 처음으로 **같은 문제를
만드는 쪽과 쓰는 쪽 양쪽에서** 팔 수 있는 대상이다.

### 접은 것과 못 읽은 것을 구분한다

**접음** — Matrix(주간 소식·릴리스 위주) · Home Assistant(릴리스·제휴 소식).
**못 읽음** — GitLab 의 엔지니어링 블로그가 403 이다. 자료가 없는 게 아니라 도구가 막힌 것이라
따로 적었다(연표 스킬의 `none`/`blocked` 구분과 같다).

### 다음 사이클 — 신규 (대기 맨 위 AuthZed)
