...
**Signal — 프로파일만 썼다.** 회사 133개, 큐 2/3. 도메인 넷 중 셋은 자료를 확보했고
하나(서버에 맡기는 것)는 자료가 모자라 tech 한 칸을 `unknown` 으로 남겼다.

### 수익원이 없다는 것이 기술 문제를 정한다

파는 것이 없다. 비영리로 기부와 초기 대출로 굴리며 **비용을 메우려고 감시 데이터를 모으지
않는다**고 못 박는다. 그러면 남들이 서버에 물어보고 끝내는 일(누가 내 연락처인가, 이 키가
진짜인가, 백업을 어디에 두나)을 전부 암호로 다시 지어야 한다.

이 엔진의 수익원 없는 회사 셋(덕DB 재단·Bun 인수·레이디버드 수익화 거부)과 다르다 —
여기는 **없는 것이 제품의 정의**다.

### 값을 항목별로 공개한다

저장 130만 · 서버 290만 · 등록 문자 600만 · 대역폭 280만 · 기타 70만 = 인프라 연 1,400만
달러, 인건비 1,900만, 2025년 약 5,000만 달러. 상근 50명(라인 3,100명, 카카오톡 4,000명과
견준다). **가장 큰 인프라 항목이 우리가 못 정하는 값**(통신사 문자 수수료)이다.

### 벌써 눈에 걸린 것 둘

**갈아치우지 않고 나란히 돌린다** — 후양자 래칫을 더블 래칫과 함께 돌려 키를 섞는다. 깨려면
둘 다 깨야 한다. 대신 ML-KEM 키가 1184바이트(ECDH 는 32바이트)라 소거 부호로 쪼개 보낸다.

**공개 원장인데 누가 누구인지 안 보인다** — 키 변경을 로그 트리에 적되 식별자는 VRF, 값은
키 해시로 가린다. 감사자는 클라우드플레어와 트레일오브비츠 둘. 10억 장 원장에서 확인할 건
30장이다.

### 다음 사이클 — 확장 (Signal 의 빈 도메인)

`세션 열쇠를 계속 갈아 끼운다` 부터. 주소를 제목으로 추측하면 404 가 난다 — 목록의 href 를
그대로 쓴다.
