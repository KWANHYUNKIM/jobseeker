...
**카카오페이**(KR · 핀테크) — 회사 프로파일까지 썼다. **도메인 둘 · 기능 0.** 큐 0/3.

### 가장 무서운 것은 실패가 아니라 모름이다

여러 서비스에 걸친 결제에서 응답을 못 받으면 **성공했는지 실패했는지 도달조차 안 했는지**
알 수 없다 — **'결제가 실패했는지 성공했는지 알 수 없을 때'** 가 가장 심각하다고 적는다.
그래서 결과를 **성공·실패·모름 셋**으로 가르고 **모름을 오류로 뭉뚱그리지 않는다.**

### 자동화의 끝을 사람으로 둔다

멱등 API(`tx_key`)와 한 번의 재시도로 걸러 내고, 남으면 **Unknown 으로 표시해 수동 대사와
배치 대사로 넘긴다.** 그리고 **끝까지 못 가는 자리도 인정한다** — 보상 트랜잭션마저
타임아웃되면 **다시 모름이 생기는 무한 회귀**가 있다. **완결을 주장하지 않는 설계**다.

### 첫 화면 함정이 연속 두 번이었다

뱅크샐러드가 첫 화면 3편(실제 58편)이었는데, 카카오페이는 **첫 화면 5편(실제 162편)** 이다.
**한국 회사 기술 블로그에서 반복되는 형태**로 보인다 — 앞으로 목록은 무조건 사이트맵으로
확인한다.

### 못 읽은 것을 못 읽었다고 적는다

**회사 소개·법인 정보 페이지를 찾지 못했다**(사이트가 JS 로만 그려지고 사이트맵에 서비스
페이지뿐이다). 대표·주소·사업자번호·매출·직원 수를 다 비웠다 — **잘 아는 회사라고 출처
없이 적지 않는다.**

### 조직이 섞여 있을 가능성

슬러그에 **`kakaopayins-`** 가 붙은 글들이 **카카오페이손해보험** 조직으로 보이는데
**본문에 조직명이 있는지 확인하지 못했다.** 한 기술 블로그에 여러 법인의 글이 섞여 있을
수 있다는 것을 open_questions 에 남겼다.

### 다음 사이클

확장 — 빈 도메인 둘 중 하나. **AOP Shadow 쪽이 수치가 촘촘하고**(처리량 20% 감소 · P99
548 → 1,305ms), 트랜잭션 쪽은 **정량이 하나도 없다.** 큐가 0/3 이라 그 뒤에 후보 조사가 뜬다.
