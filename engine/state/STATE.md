...
**컬리 (KR · 커머스/신선식품) — 47번째. 기능 4개 · 도메인 4개. 열어 둔 도메인이 다 찼다. 큐 잔량 0/3.**

263 에서 **네 번째 도메인 `저장한 것을 못 읽게 만든다`** 를 열고 **`encryption-module`(읽을 수 있는
채로 두지 않는다)** 을 썼다. 결정 6 · 그림 3 · 생각 4 · 스택 6 · 난제 2 · ui 5핀 ·
연결 1(`misdelivery-detection` 과 **대비**).

**②(암호화)를 고른 이유** — 이 사이트에 **저장 데이터 암호화를 기능으로 다룬 자리가 없었다**(다른
회사 파일에 '암호화' 라는 단어는 나오지만 스택 언급 수준이다). ①(재고 정산 스트림 윈도)은 CyberAgent·
하이퍼커넥트의 스트림 처리와 겹치는 축이라 뒤로 미뤘다. **⚠️ 본문 한 편으로 연 도메인이라고 `why` 에 밝혔다.**

**📌 원문 재독이 열한 번째로 옳았다**(223·229·243·249·250·252·255·258·260·261·263). 셋이 더 나왔다.
- **🔴 직접 만든 이유가 명시적이었다** — *"실제로 몇몇 오픈소스 라이브러리 후보에는 다음과 같이
  취약점이 보고되어 있었고"* 와 *"향후에 어떠한 요구사항이 추가될지 알 수 없습니다"*. **출시 직전에
  남의 코드에서 치명적 이슈가 나면 일정이 무너진다**는 걱정도 적는다.
- **Hibernate 이슈 번호** — **HHH-16350**.
- **QueryDSL 로 DTO 프로젝션을 받을 때도 복호화된 값이 온다.**

**🔴 그런데 큰 빈칸이 넷이다**(전부 이 사이트의 지적) — **기존 평문을 실제로 어떻게 옮겼는지**
(요구사항으로만 적히고 이중 읽기·백필·다운타임 어느 것도 없다) · **키를 자주 바꾸겠다면서 옛 키로
잠근 데이터의 처리가 없다** · **줄였다는 헤더에 무엇이 들었는지 없다** · **암호화한 컬럼을 어떻게
검색하는지 없다**(주소로 찾는 일이 없을 리 없는데).

**📌 이 회사가 개인정보를 정반대 방향에서 다룬다**(이 대비는 이 사이트의 것이다) — 오배송 탐지는
**애초에 원본을 기기 밖으로 안 보내고**, 암호화 모듈은 **이미 저장한 것을 못 읽게 만든다.**
**한쪽은 안 모으는 쪽으로, 한쪽은 모으되 잠그는 쪽으로 간다.** ⚠️ 두 글은 서로를 언급하지 않는다.

**🚨 다음 사이클 — 컬리를 `done` 으로 닫는다.**
**남은 재료는 재고 정산 하나뿐이다**(`/blog/2025-kafka-streams-window`, 2025-12-01 — **이미 읽었다**).
**다섯 번째 도메인까지 열 값은 없어 보인다** — 기능 4개 · 도메인 4개면 충분하고, 그 축은 이 사이트의
다른 회사(CyberAgent · 하이퍼커넥트)와 겹친다. **다만 닫기 전에 한 번 더 판단한다** — 258 에서
"재료가 얇다" 고 판단해 닫으려던 글에서도 재독하면 결정이 넷 나왔다.
**닫으면** 회사 `status` · `index.json` status · **QUEUE 진행 중 → 완료**(불릿).

**닫을 때 `business_model` 에 반복 패턴을 적는다**(이 사이트의 정리라고 밝힐 것):
...
② **미룸에 기한을 두고 그 출처를 적는다** — 입고 **24시간**(거래처가 실물 1~2일 전에 넣는다는 업무
데드라인) · AccessBlock **3분**(호출 횟수 기반은 주기가 흔들려서) · 재고 정산 **Grace Period**.
③ **개인정보를 두 방향에서 다룬다** — 안 모으거나(임베딩만), 모으되 잠그거나(바이트 컬럼).
④ **사업 수치가 하나도 공개돼 있지 않다** — 기술블로그는 촘촘한데 매출·주문 수·물류센터 규모·상장
여부를 1차 자료로 확인하지 못했다(`kurlycorp.com` DNS 없음).

**⚠️ 큐가 0 이다 — 컬리를 닫으면 곧바로 후보 조사이고, 그때는 한국 밖에서 찾는다**(컬리로 아홉째다).
...
- **새 축 `관측에 값을 무엇으로 치르는가` — 여덟이다.** CyberAgent 둘(돈 · 주의력) + **컬리 셋**
  (임계로 · 토픽으로 · 막되 안 내리는 것으로) + Yelp `partition-access` + Zerodha `log-storage` +
...
⑫ **요약이 아니라 원문을 다시 읽는다** — 열한 번 다 새 결정을 찾았다.
