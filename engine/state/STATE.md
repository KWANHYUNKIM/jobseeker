...
**없다 — Flipkart 완주.** 도메인 3/3 · **기능 3개.** 회사 **96개.** ⚠️ **큐 1/3**(Twilio).

### 이번 사이클 — `5분이 하루가 되는 곳에서 누구를 먼저 태울지 정한다`

⚠️ **첫 글에는 버린 대안과 대가가 없었다.** 앞의 Cygames 때처럼 **같은 도메인의 두 번째 글을 함께 읽어** 채웠다 — **계획 글은 결과를, 용량 글은 제약을 말한다.**

건진 것 넷:

- ⚠️ **이 도메인의 시간은 연속적이지 않다** — **`주문이 오후 6시 5분에 들어오면 다음 날 아침까지 처리할 수 없다.`** **웨이브와 트럭 출발이라는 경계 때문에 5분이 배송 하루가 된다**(spillage).
- ⚠️ **늘릴 수 있는 것이 거의 없다** — **`운영 제약이 하루 6~8회 마감으로 우리를 제한하고`** **인력 추가는 비용상 금지적**이다. **그러면 남은 방법은 같은 용량을 언제 누구에게 쓸지 정하는 것뿐이다**(재구성).
- ⚠️ **공평을 포기했다** — **`공평한 FIFO 처리보다 시간에 민감한 경로를 우대한다.`** **먼저 온 주문보다 지금 안 태우면 하루를 잃는 주문이 급하다**(재구성). 그리고 **세일 때 spillage 가 `불가피` 해지면 고가치 코호트를 보호한다** — **모두를 구할 수 없다는 것을 먼저 인정하고 순서를 미리 정한다**(재구성).
- ⚠️ **부분의 개선이 전체를 나쁘게 만든다** — **`한 영역의 「승리」 가 종종 다른 영역에 병목을 만들었다`**(창고 포장이 빨라지면 운송이 안 맞을 때 출고 도크가 넘친다). 그래서 계획을 한곳으로 모았고 **계획 주기가 10일에서 1일**이 됐다.

**수치** — 등록 사용자 **5억 명** · 하루 배송 **400만 건** · 핀코드 **1만 9,000개 이상** · **1일 배송 +50% · 2일 배송 +43%** · 불건전 재고 **50% 감소** · 필요 인력 **10% 감소.**

### ⚠️ 이 회사의 세 기능이 서로 다른 시간 축에 있다

**밀리초**(광고 예산, 초당 100만 이벤트) · **배치**(검색 관련성 라벨링, 22주 검증) · **하루와 몇 달**(공급망 계획, 10일 → 1일). ⭐ **프로파일에 적은 `두 극단 사이에 걸쳐 있다` 가 세 사이클로 확인됐다**(재구성).

### 다음 사이클 — 신규(6순위)

`--gaps` 가 **큐 맨 위(Twilio)를 `in_progress` 로 만들고 3단계부터** 를 부를 것이다. ⚠️ **큐가 1/3 이라 그 뒤에 후보 조사가 걸린다.**

⏳ **보강 거리 셋** — Grafana Labs 인용 대조 · Razorpay 보안 트리아지 글 · **Flipkart 의 Rate Card 엔진 글**(어느 도메인인지 본문을 봐야 안다).
...
**④ ⭐ `인도 규모에서 무엇이 달라지나` 4곳 — 가장 두껍다** — Meesho / Zepto / Razorpay / Flipkart.
...
**⑬ ⭐ `애매할 때 어느 쪽으로 넘어지나` — 재료가 넘친다** — Flipkart 광고(덜 쓰는 쪽) / **Flipkart 공급망(공평 대신 급한 쪽)** / Razorpay(수집을 멈추는 쪽) / Zepto(마지막에 확인) / Paystack(금액이 틀리면 실패).
