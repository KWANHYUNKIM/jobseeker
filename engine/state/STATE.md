...
**Vinted — 33번째 회사.** 사이클 183 까지 프로파일 + 도메인 3개 + 기능 2개
(`item-search` · `vitess-sharding`). **마지막 남은 도메인은 `자체 데이터센터 망`** —
자료 둘을 이미 확인했다: `2020/09/14 data-center-network-at-vinted`(세 물리 위치의 큰 평면
서브넷 → **2계층 Clos + eBGP · 랙마다 고유 AS · EVPN/VxLAN · 애니캐스트**, **버린 대안 둘**을
명시한다 — 큰 L2 + MLAG, 서브넷 안 나눈 EVPN 오버레이. *"MLAG·VRRP 같은 주-보조 구조를 피한다"*.
규모는 *"수백 대 서버 · 수십 기가비트"*) 와 `2025/07/15 clocksource-performance`(같은 사양인데
느린 서버 — **HPET vs TSC**, 커널 로그 *"TSC 왜곡 120 사이클"*, **원인은 끝내 확정 못 함**).
**채우면 Vinted 를 done 으로 닫는다.**

> **`vitess-sharding` 에서 나온 것 — 되돌릴 길을 만드는 것이 일의 대부분이었다.**
> 떠나온 이유가 속도가 아니라 **되돌릴 수 없다는 것**이었다(수직 샤딩 네 단계가 전부 사람 손,
> *"오타·잘못된 형식·순서가 어긋난 명령이 다운타임으로"*, 서버 42대 수작업). 그래서 만든 것이
> **카나리 · 분석→비동기→주앱 순서 전환 · 되돌릴 수 있는 컷오버**다.

> **🔑 가장 눈에 띄는 역설 — 사람 손을 없애러 갔는데 가장 위험한 자리에는 사람을 남겼다.**
> Orchestrator 자동 페일오버가 테스트에서 **스플릿 브레인**을 내서 **수동 페일오버를 유지**했고,
> 그 결과 **3년간 긴급 reparent 는 1회**였다. **자동화가 틀리면 결과가 되돌릴 수 없는 쪽**이기
> 때문으로 읽힌다. ⑮ '되돌릴 수 있는 방향이 한쪽뿐일 때' 축의 가장 좋은 재료다.

> **⑫ '해 보고 되돌린 것' 재료도 하나** — MySQL **5.7.31 버그(PS-3951)로 5.7.21-20 으로 다운그레이드**.

**큐 2/3** — Zerodha(IN) · PlanetScale(US).

마지막 사이클: 183 (2026-08-22) — Vinted 데이터베이스 샤딩.
