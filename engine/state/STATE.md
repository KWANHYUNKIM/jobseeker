...
**없다 — Fastmail 을 완주시켰다(도메인 2 · 기능 2 · `status: done`).** 회사 165개, **큐 0/3.**

### 팔지 않는 것을 먼저 말한다

이번 기능의 축이다. 저장 위치를 고르게 파는 기능인데, 같은 글에 **"데이터가 EU 안에만
남는다는 보장이 필요하다면 우리에게는 그런 것이 없다. 그렇게 짐작하도록 두느니 직접
말하겠다"** 고 대표가 적는다. **호주 법인이라 관할은 고를 수 없다** — 데이터 위치가 법적
관할을 바꾸지 않는다는 것, 이것은 기술로 풀리는 문제가 아니다.

### 새는 곳을 항목으로 세어 공개했다

비상 백업(모든 사용자 필라델피아) · 메일 주소와 사용자 메타데이터 · 시스템 로그 ·
디버깅·청구·지원 제공자. 세어 놓고 보면 **EU 에 있는 것은 주 사본과 앱 접속뿐**이다.
그런데도 세어 공개한 것이 사고 글에 자기 PR 번호를 적은 것과 같은 태도다.

### 경계와 가용성은 같이 못 가진다

장애가 나면 **가용성을 우선한다**고 명시하고 EU 계정도 미국으로 넘긴다. '리전'이라는 이름이
주는 인상은 봉쇄인데 실제로는 **우선**이다.

### 두 도메인이 한 사고로 묶여 있다

폴더를 지운 그 작업이 바로 **EU·비EU 계정을 가르는 일**이었다. 한 회사 안에서 도메인
하나가 다른 하나를 깨뜨린 관계를 데이터에 심었다.

### 앞 사이클에서 열어 둔 질문이 닫혔다

회사 프로파일 때 소재지를 확인 못 해 `country` 를 '기타'로 두고 open_questions 에 적어
뒀는데, EU 리전 글에서 **호주 법인**이라는 것이 나왔다. **모르는 것을 적어 두면 다음 자료가
답을 준다** — 이번 사이클에서 그 값을 확인했다.

### 심은 대비

**엔테**(종단간 암호화로 '우리도 못 본다'를 만들어 관할을 비껴간다) ↔ **Fastmail**(관할이
그대로 남는다는 것을 인정하고 적는다). 같은 요구에 답이 반대다. **백블레이즈**(하드웨어가
죽는 것) ↔ Fastmail(우리가 지운 것).

### 다음 사이클 — 후보 조사 (큐가 0/3 이다)

**의료 축이 여섯 번째로 비어 있다.** 못 읽은 곳 — Lunit(medium.com 403) · Blender(403) ·
dbt Labs(홈에 블로그 링크 없음) · incident.io(404 네 번) · Sourcegraph(403 두 번) ·
Guardian · Flexport · Cal.com. **이 세션의 WebSearch 예산은 이미 다 썼다** — 목록 페이지에
직접 물어 href 를 받는 방법만 남았다.
