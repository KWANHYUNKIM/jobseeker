...
**Zig Software Foundation — 프로파일만 썼다.** 회사 138개, 큐 0/3.
도메인 넷이 다 비어 있고 근거는 0.16.0 릴리스 노트 한 편에 몰려 있다.

### 릴리스 노트가 설계 문서다

서술형 심층 글이 드문 대신 릴리스 노트에 **만든 이유와 안 만든 이유가 함께** 적혀 있다.
이 엔진에서 릴리스 노트를 1차 자료로 삼은 첫 대상이라, 큐에도 그 사정을 적어 뒀다.

### 감춰진 것을 만들지 않는다

도메인 넷이 한 원칙에서 갈라진다. 입출력은 **막히는 연산마다 `Io` 를 넘기게** 해서 방식을
라이브러리가 아니라 부르는 쪽이 고르게 하고(구현 넷: 스레드·초록 스레드·io_uring·kqueue),
할당자도 인자로 다닌다 — 아레나를 잠금 없이 다시 쓰고 `ThreadSafeAllocator` 감싸개를 없앴다.

### 안 만든 이유를 이름 대어 적는다

`@Float` 은 실행시 부동소수 타입이 다섯뿐이라, `@Array` 는 보통 배열 문법으로 되니까,
`@ErrorSet` 재구성은 언어를 단순하게 하려고 안 만든다. **기능을 더하는 대신 형식적으로 명세할
수 있는 언어를 지키는 쪽**이다.

### 감싸면 남의 버그까지 받는다

자체 deflate 는 zlib 대비 9.7% 빠르고 캐시 미스가 47.3% 적되 압축률이 1% 나쁘다 — 수치와
대가를 같이 적는다. 윈도우는 NtDll 로 내려가 상위 DLL 의 버그·성능 함정·빠진 기능을 피한다.

### 후원사가 이 엔진 안에 있다

기업 후원사로 **타이거비틀**이 올라 있다. 이 엔진에 이미 판 회사가 다른 대상의 재원으로
등장하는 자리다.

### 다음 사이클 — 확장 (Zig 의 빈 도메인)

`입출력을 인터페이스로 만든다` 부터. 구현 넷과 그 성숙도 차이가 손에 있다.
