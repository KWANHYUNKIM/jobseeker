...
**TigerData**(구 Timescale · Postgres 시계열) — 회사 프로파일까지 썼다. **도메인 둘 · 기능 0.**
큐 0/3.

### 남의 계획기 안에서 고쳐야 한다

Postgres 확장으로 얹는다는 선택이 이 회사의 자리 전체를 정한다. 계획기·트랜잭션·SQL 을
그대로 물려받는 대신, **그 안에서 풀어야 하는 문제**를 진다 — 청크가 늘 때의 계획 비용,
늦게 온 데이터의 재계산. 코크로치처럼 **처음부터 새로 짓는 쪽과 다른 자리**다.

### 물린 대안을 셋이나 이름 대어 적는다

연속 집계 글이 그렇다 — **매번 전체 재계산**("한 행이 바뀌었든 백만 행이 바뀌었든 매 실행마다
전체 재구축 값을 치른다"), **클릭하우스식 삽입 트리거 뷰**("'과거는 바뀌지 않는다'는 설계라
이력을 맞추는 일이 수동이고 자원을 많이 먹는 작업이 된다"), **스트리밍 데이터플로 시스템**
("자기 일관성 모델과 자기 운영 표면을 함께 지고 온다"). 그리고 자기 대가도 한 문장으로
적는다 — **"일은 작게 유지되고 이미 돌리는 데이터베이스 안에 머문다. 그 대신 누군가는 그
창의 올바른 너비를 골라야 한다."**

### 조용히 바뀌는 숫자

여는 장면이 이 도메인의 전부다 — 버퍼에 담겼던 공장 데이터가 원래 타임스탬프로 도착하자
집계가 **41,900 에서 43,100 으로 조용히 올라갔다.** 갱신 창 밖이면 **정책은 그 버킷을 영영
다시 보지 않고 집계는 도착 전 값을 계속 보고한다.**

### 회사가 자기 정체를 설명하지 않는다

`timescale.com` 이 `tigerdata.com` 으로 **301** 인데, 홈은 **그 관계를 리브랜딩이라고도 별도
제품군이라고도 말하지 않는다.** 요금도 라이선스도 소재지도 없다. `open_questions` 에 그대로
적었다.

### 다음 사이클

빈 도메인 둘 중 하나. **연속 집계 쪽이 자료가 두껍다**(물린 대안 셋 · 두 오프셋 · 최악 지연
공식 · 조용히 바뀌는 숫자). 계획 쪽은 14,000배와 조건 다섯이 선명하다.
