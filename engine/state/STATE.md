...
**Moniepoint — 41번째 회사.** 사이클 217 까지 프로파일 + `ui_map` + **도메인 2개** + **기능 1개**(`critical-path`).

> **`critical-path` 의 축 — 무엇이 핵심인지 먼저 정하고, 나머지가 핵심을 흔들지 못하게 떼어 놓는다.**
> 세 글이 같은 판단을 반복한다 — **저축은 부가라서** 결제의 임계 경로에서 뺐고
> (*"모든 생산자 팀이 자기 의존성 그래프에 SAYT 를 넣게 된다"*), **헬스체크의 의존성 검사는 부가라서**
> 빼서 일시적 실패에 서비스가 튕기지 않게 했으며, 반대로 **Kafka 는 핵심이라서** 관리형에 안 맡기고
> Strimzi 로 직접 운영한다(글이 관리형을 권하는 조건이 *"Kafka 가 주변적이라면"* 이다).

> **눈에 걸린 것 넷** — ① **읽지 않고 넣어서 중복을 판정한다**(insert-first) — 2단계는
> *"왕복이 두 번 들고 읽기와 쓰기 사이에 경쟁 구간이 열린다"*. 그리고 **판정이 발행보다 먼저**라
> 크래시에도 유령 이중 입금이 없다. ② **늦게 온 것은 아예 안 한다** — 신선도 창이 **분 단위**이고
> *"약간의 완전성을 훨씬 큰 신뢰와 맞바꿨다"*. ③ **건강을 판정하는 장치가 가용성을 깎고 있었다** —
> 기본 헬스체크가 의존성의 일시적 실패로 서비스를 튕겨냈다. ④ **배포에 거절할 근거를 만들었다** —
> *"지연 곡선이 피크의 2배까지 평평한 것을 보여 줄 수 있을 때"* 확장 가능하다고 본다.

> **📌 새 비교 문서 축 — '무엇을 잃기로 정했는가'.** 세 회사가 **잃을 것을 먼저 고르고** 설계했다:
> **Moniepoint `critical-path`**(*"약간의 완전성을 훨씬 큰 신뢰와"*) · **Doximity `cdc-overlap`**
> (중복은 견디고 빠짐은 못 견딘다) · **Yelp `partition-access`**(놓친 접근은 '안 쓰임' 쪽으로 기운다).
> **셋 다 오차의 방향을 한쪽으로 몰아 놓았다는 공통점이 있다.** 재료가 셋이다.

> **다음 사이클** — 도메인 **`돈의 기록이 맞다는 것을 어떻게 아는가`** 의 기능 하나.
> 대사 글(**1,000억 건 이상** · *"모두에게 SQL 을 배우게 하는 것"* 을 버렸다 · *"거버넌스를 나중에
> 끼워 넣는 일은 비싸고 느리고 부서지기 쉽다"*)은 읽었고,
> **`system-migration-data-accuracy`(2025-06, 99.9%)는 제목만 확인했으니 반드시 먼저 읽는다.**

> **📌 큐 2/3** — Target(US · 리테일 물류) · bol.com(EU/NL, ⚠️ **최신 글 2024-11 이라 멈췄을 수 있음**).
> **라틴아메리카는 접었고**(새 단서 없으면 재시도 금지) **게임은 Roblox 하나**(Unreal 403).

**큐 2/3** — Target(US) · bol.com(EU/NL).

마지막 사이클: 217 (2026-08-27) — Moniepoint 핵심이 아닌 것이 핵심을 흔들지 못하게 한다.
...
3. **비교 문서 '무엇을 잃기로 정했는가'** — **새 축.** Moniepoint `critical-path`(*"약간의 완전성을
