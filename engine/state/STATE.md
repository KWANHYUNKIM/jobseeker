...
**없다 — Gunosy 를 완주했다(도메인 2 · 기능 2).** 큐 0/3 이라 다음 사이클은 **후보 조사**다.

### 사업 조건이 기반 선택까지 내려온다

**광고 로그가 과금과 직결** → **무상태로 둘 수 없다** → **StatefulSet + EBS 버퍼** →
**늘릴 때 손이 드는 구조는 못 쓴다** → **쿠버네티스.** 사슬이 한 줄로 이어진다. 고른 이유
넷이 전부 **사람이 손으로 해야 할 일이 얼마나 남느냐**를 센다 — 성능도 비용도 아니다.

### 검증은 시간이 아니라 사람이 끝낸다

CodeDeploy 는 **'타이머 진행이 기본'** 이라 **사람이 못 본 채 넘어간다.** Argo Rollouts 로
**무기한 멈춰** 두는 것이 그 차이다 — **멈출 수 없으면 검증이라 부를 수 없다.**

### 조건을 밝혀 적으면 남이 다르게 고를 수 있다

**'무상태 구성이나 단순한 운용이 요구되는 환경이라면 ECS 는 매우 뛰어난 선택지'** 를
앞뒤로 두 번 쓴다. **컨벡스가 파이어베이스·수파베이스를 '이유가 있어서 존재한다'고 한
것과 같은 태도**다.

### 손이 드는 자리는 장면으로 그리는데 자기가 낸 값은 안 잰다

**볼륨 7개를 손으로 만드는 장면**은 선명한데 **쿠버네티스 학습 비용**은 '들지만'으로 끝나고,
**추천 자동화가 공수를 얼마나 줄였는지**도 없다. **문제는 손으로 재고 답은 안 재는 것**이
이 회사 글의 일관된 모양이다.

### 억지로 붙이지 않았다

`android_compose_column_impression`(임프레션 계측)이 **광고 지표라 과금 축과 이어지지만**
이번 도메인(기반 선택)과는 다른 이야기라 붙이지 않고 후보로만 남겼다 — 카카오페이에서
`event-driven-architecture` 를 안 붙인 것과 같다.
