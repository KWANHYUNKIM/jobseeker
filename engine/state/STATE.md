...
**없다 — ClickHouse 를 완주했다.** 큐는 **2/3**(Duolingo · Ramp). 다음 사이클은 `--gaps` 가 **신규(6순위)**를 부른다.

### 이번 사이클 — AI `무엇을 기계에 안 맡길지 먼저 정한다` 와 완주

⚠️ **이 도메인의 본체는 넘긴 70% 가 아니라 남긴 30% 다.** 그리고 경계가 **세 겹**이었다 — **일의 종류**(공인 재무 지표·실시간 모니터링·표준 보고는 사람에게) · **데이터 범위**(PII·고객 데이터·민감 소통은 아예 안 보여 준다) · **결론의 지위**(출력만으로 중요한 결정 금지, 주석 달린 SQL 로 검증).

⚠️ **그중 기계가 강제하는 것은 가운데 한 겹뿐이다** — 접근 가능한 마트를 잘라 두는 것. **나머지 둘은 규칙이고 사람이 지킨다**(재구성). **AI 를 어디까지 믿을지에 대한 답이 대부분 사람의 규율에 걸려 있다는 것이 이 사례의 정직한 요약이다.**

건진 것 둘 더:
- **모델이 아니라 맥락이 문제였다** — 스키마는 이름과 타입만 말한다. **사람이 쓴 위키**(필드 정의·업무 맥락·값의 범위·그 데이터를 만드는 업무 과정·원천 간 관계)가 이 시스템의 재료다. ⚠️ **문서가 낡으면 AI 가 낡은 뜻으로 답하는데 그 갱신 보장은 글에 없다.**
- **글쓴이가 자기 회의부터 적는다** — `I had tried early LLMs with data sources and found the results disappointing and unreliable`. ⚠️ **그런데 환각에 대한 대책은 `검증하게 한다` 뿐이다** — 셋 중 맥락만 다뤘다(재구성).

### 다음 사이클 — 신규(Duolingo), 그리고 비교 문서가 가까워졌다

큐 맨 위는 **Duolingo** 다(교육 축 첫 자리 · Temporal + S3 + Claude Code 로 iOS 테스트 생성 · **SwiftLint 실패율 13.6%** · **리뷰 병목** · **프로덕션 파일을 고친 사고**).

⚠️ **`사내 AI 에이전트를 어디까지 믿을 것인가` 축에 이제 세 곳이 찼다** — Sentry(**AI 가 PR 까지 열고 병합은 사람이**, 매일 밤 심문) · ClickHouse(**분석의 70%**, 남긴 30%, 경계 세 겹). Duolingo 와 Ramp 까지 채우면 **네 곳이다.** **비교 문서(8순위)의 재료가 갖춰진다** — 같은 질문에 네 회사가 각각 다른 선을 그었고, **그 선을 무엇이 강제하는가**(기계냐 규율이냐)가 좋은 축이 될 것이다.
