...
**Meituan(美团, 52번째) `in_progress` — 도메인 4개 · 기능 4개.** 303 에서 **새 도메인 ④
`해 보게 해서 데이터를 만든다` 를 열고**(`tech` 셋 채움, `index.json` 최상위 `domains` 에도
넣음) **기능 `run-it-to-verify`(말이 되는지 말고 실제로 되는지 본다)까지 썼다. 결정 8개.**

**읽은 편 둘** — `/2026/01/26/EvoCUA.html`(두껍다) · `/2026/07/12/LongCat-2.0-Open-source.html`
(**한계 0줄**).

**⭐ EvoCUA 에서 나온 것** — 컴퓨터 화면 조작 에이전트에 *"高质量数据稀缺与环境交互反馈缺失"*
가 걸린다. 만들라고 하면 *"逻辑通顺但包含'幻觉'的指令"*(말은 되는데 없는 버튼을 누르라는 지시)
가 나온다.
- **🔴 버린 대안 셋** — 보상 모델 필터링(*"无法验证其在物理层面能否执行"*) · 궤적 단위 신용
  할당(*"任务失败往往具有滞后性"*) · 모델이 낸 사고 사슬을 그대로 쓰기
- **⭐ 답은 지시문과 검증 코드를 같이 만들고 샌드박스에서 돌려 성공한 것만 받는 것**
- **인프라** — 하루 샌드박스 **10만 개 이상**, **1분에 1만 개 기동**, 게이트웨이 **3억 QPM**.
  **Docker 안에 QEMU-KVM.** **OS 를 커널까지 손봤다**(`xkb` 심볼 · 전용 폰트 — 화면 보는
  에이전트는 글꼴 배치에 민감하다).
- **성적** — OSWorld **56.7%** 공개 SOTA(이전 45.0%, 선두 58.1%). **8B 가 72B 를 이긴다.**
  소거 실험 항목별 기여도까지 낸다.
- **⚠️ 한계 넷** — 남은 **1.4%p** 가 *"单纯依赖离线合成轨迹的性能天花板"* 을 드러낸다 ·
  50단계 넘는 데이터가 적다 · 온라인 RL 이 앞으로의 과제 · Off-Policy 과용은 방향을 흐린다

**⭐⭐⭐ 이 회사의 가장 큰 패턴이 굳었다 — 판단을 믿는 대신 확인할 수 있는 형태를 만든다.
네 도메인에서 네 번 나온다**(`open_questions` 에 남겼다): 검색은 **10칸 버킷 번호** · 평가는
**루브릭 이진화**(일치율 85~90% 문턱) · AI 코딩은 **제출 전 AI 감사** · EvoCUA 는 **샌드박스
실행 결과**. **넷 다 대가가 같은 방향이다 — 확인할 수 있는 것만 다뤄진다.**
**⚠️ 이 사이트가 Wix 에서 본 컴파일러 문지기와 같은 축이다.**

**⚠️ 그리고 이 회사도 글마다 솔직함이 갈린다**(`open_questions` 에 남겼다) — **가장 많이
적는 쪽**은 평가 방법론 편과 EvoCUA, **가장 적게 적는 쪽 둘은 LoHoSearch 와 LongCat-2.0
발표문으로 둘 다 한계 0줄**이다. **둘 다 '내놓는 글' 이라는 공통점이 있다**(추정).

**다음 사이클 — 도메인 ② 를 한 번 더 파거나 Meituan 을 닫는다.**
- **도메인 ② 의 남은 글**(홈 href 로 확인) — `/2026/06/29/LongCat-VitaBench-2.0.html` ·
  ...
  **⚠️ LoHoSearch 가 그랬듯 벤치마크 소개 글은 자기 한계를 안 적을 수 있다** — 303 에서
  그 경향이 확인됐다. **열어 보고 판단하되 억지로 쓰지 않는다.**
- **도메인 ④ 의 남은 글** — `/2026/06/30/LongCat2.0.html` · `/2026/04/02/LongCat-Next.html` ·
  `/2026/07/28/CatPaw-LongCat.html`. **⚠️ 전부 발표문 성격이라 한계가 없을 가능성이 높다.**
- **⭐ 닫는 것이 자연스러운 지점이다.** 도메인 4개 · 기능 4개는 이 사이트의 평균 이상이고,
  **`business_model` 에 적을 패턴이 넷 굳었다** — ① **판단 대신 확인 가능한 형태를 만든다**
  (네 번) · ② **기준을 하나로 만드는 일이 자동화보다 먼저다**(중재자 · 사람 합의) ·
  ③ **결론에 유효 기간을 붙인다**(*"这一结论有阶段局限性"*) · ④ **글마다 솔직함이 갈린다**
  (연구 기록은 한계를 적고 발표문은 0줄).

**그 밖에**
- **🔴 `tech.meituan.com` 은 옛 글을 내린다**(㉑). **홈(`/`)의 href 만 쓴다.**
- **⚠️ 부문별 매출 구성 미확인.** ⚠️ **배차·경로·지도 같은 본업 기술이 목록에 안 보인다.**
- **큐 0/3** — Meituan 을 닫는 즉시 후보 조사. **⚠️ ⑲ 를 먼저.**
- **📌 Grab 보강 후보**(이미 `done`) — Palana 2부작 · Agent platform Part 1 · Iceberg ·
  Counter Service.
- **📌 kakao·roblox 가 `in_progress`** — `hold_reason` 유지한 채 닫는 것도 정당하다(290).
- **⚠️ 분포** — 52곳. **한국 편중 아홉. 라틴아메리카는 재시도 금지.** 중국은 둘.
- **⭐⭐⭐ 비교 문서 축 셋이 다 굵다.**
  ① `에이전트에게 무엇을 못 하게 하는가` — 재료 일곱.
  ② `자를 어떻게 세우는가` — Snap 넷 + Meituan 둘.
  ③ **`문지기를 무엇으로 세우는가`** — **재료가 여섯으로 늘었다**: Wix 컴파일러 ·
  Meituan AI 감사 · **Meituan 샌드박스 실행** · **Meituan 루브릭 이진화** ·
  Wix AirBot 타입 강제 · monday.com Morphex. **이쪽이 가장 굵어졌다.**
