...
**Prisma — 기능 하나를 채웠다(`rust-to-typescript-query-compiler`).** 빈 도메인 하나. 회사 145개, 큐 2/3.

### 오년 전의 이유가 사라지면 핵심도 걷어낸다

여러 언어를 위한 ORM 을 만들려고 러스트 엔진을 별도 바이너리로 뒀는데, 오년 뒤 타입스크립트가
지배적이 되자 **그 목적은 남지 않고 값만 남았다** — 언어 경계 직렬화, 14MB 번들, 운영체제·
OpenSSL 버전마다 따로 내야 하는 바이너리.

### 이긴 자리와 진 자리를 같은 표에 적는다

다대다 포함 2천 건 질의가 1,539ms → 136ms(11.32배), 2만 5천 건 findMany 가 163ms → 77ms(2.12배).
그런데 **작은 질의 셋은 오히려 느려졌다**(19→25ms, 1→2ms, 9→11ms). 감추지 않고 적으면서
절대 시간이 1~2밀리초대라 실무에서 의미 있는 차이가 아니라고 함께 설명한다.

### 언어 경계는 큰 데이터에서 값을 청구한다

작은 질의에서는 티가 안 나다가 결과가 커지면 옮겨 담는 값이 지배한다 — 11.32배가 그 크기다.
그래서 계획은 WASM 에 두고 **실행은 전부 타입스크립트로** 옮겨 경계를 없앴다.

### 돌 수 있는 자리가 곧 팔 수 있는 자리다

큰 네이티브 바이너리는 엣지·서버리스 런타임이 안 받아 준다 — 성능 문제가 아니라 그 자리에서는
제품이 없는 것이 된다. 바이너리를 없애 워커스·데노·번·엣지까지 넓혔다.

### 다음 사이클 — 확장 (Prisma 의 마지막 빈 도메인)

`어디서든 도는 데이터 계층을 만든다`. 요금 구조는 확보했고 Accelerate 내부가 `unknown` 이다.
