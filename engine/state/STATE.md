...
**Paystack** (NG · 핀테크) — **프로파일까지 썼다.** 도메인 2개, 기능은 아직 0이다. **회사 88개 · 아프리카 축 첫 자리.**

### 이번 사이클 — 신규(6순위), Paystack 프로파일

⚠️ **이 회사의 기술 이야기에서 눈에 띄는 것은 시작점이 자기 안이 아니라는 것이다** — **아마존이 2024년 10월 Aurora MySQL v2 지원 중단을 발표하자** 아프리카 전역의 가맹점 결제를 받치는 데이터베이스를 올려야 했다. **남이 정한 일정이 이 회사의 공사 일정이 된다.**

**이전 글 전문을 읽었고 첫 문단부터 값이 있다.**

- ⚠️ **가장 먼저 한 일이 계획을 쓰는 것이 아니라 `성공` 을 다시 정의하는 것이었다** — **`정비 시간 안에 끝내는 것은 성공이 아니다`.** 이전 업그레이드에서 **직후에는 멀쩡해 보이다가 나중에 문제가 드러났고**, **어떤 질의는 잘못된 결과를 돌려주기 시작했다**(트래픽이 재개된 뒤에야 조사했다). 그래서 **속도가 아니라 정확성과 안정성**을 성공으로 삼았다.
- ⚠️ **하위 호환을 가정하지 말고 검증하라** — MySQL 8.0 에서 **5.7 때 되던 테이블·컬럼 이름이 예약어가 됐고**, 그것은 **업그레이드 후에야 오류로 나타난다.**
- ⚠️ **자신 있을 때 가장 어려운 것부터** — 11개 중 **12개 넘는 중요 서비스를 받치는 가장 복잡한 클러스터를 먼저** 옮겼다. 여러 번의 드라이런과 외부 검증이 있었기 때문이고, **그 첫 이전이 나머지의 청사진이 됐다.**
- ⚠️ **쓰지 않은 안전장치의 값을 적는다** — 명확한 롤백 경로를 만들어 두었는데 **결국 쓰지 않았다.** 그런데 `있다는 것을 아는 것이 자신 있게 나아가게 했다` 고 적는다.

**가장 좋은 대목은 예상 못 한 실패다** — **가장 쉬울 거라 봤던 클러스터**에서 늦게 터졌다. 컬럼을 큰 정수형으로 바꾸는 변경을 함께 했는데 **드라이런에서 복제가 테이블마다 다르게 동작**했다. **멈추고 재평가해 더 신중한 복제 전략을 설계했고**, 회사가 그것을 **드라이런의 값**으로 적는다.

세운 도메인 둘:
- **남이 정한 지원 종료 일정에 맞춰 결제 데이터베이스를 올린다** — tech 5개.
- ⚠️ **아프리카에서 사업체가 돈을 받게 한다** — **이 회사의 본체인데 tech 가 0 이다**(검증 경고). **블로그의 시스템 글이 DB 이전과 SAST 둘뿐이다.**

### 다음 사이클 — 확장

**이전 도메인이 압도적으로 익었다** — 5단계 절차와 분 단위 타임라인(T-2h · T-0 · T+15m · T+30m · T+45m · T+2h), 배운 것 다섯, **첫 2년 17만 달러 이상 절감**.

⚠️ **핵심 제품 도메인은 자료부터 찾아야 한다** — 다만 **이 회사는 글이 드문드문해서** 못 찾을 가능성이 있다. **그때는 완주를 미루고 `확인했고 없었다` 를 기록한다**(PROMPT 2‴단계).
