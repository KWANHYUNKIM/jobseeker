...
**없다 — Mapbox 를 완주시켰다(도메인 1 · 기능 1 · `status: done`).** 회사 176개, **큐 0/3.**

### 거절 이유가 성능이 아니라 크기다

이번 기능의 축이다. 자동 백엔드 선택을 안 하는 이유가 **느려서가 아니라 두 백엔드를 다
담아야 해서**다 — **실어 보내는 무게를 원가로 보는 회사**의 사고방식이 여기서 드러난다.
그래서 선택을 **빌드 시점**으로 옮겼다.

### 편의를 주려면 무게를 실어야 한다

자동 선택은 개발자에게 가장 편한데, 그 무게는 **자동 선택의 이득을 못 보는 사용자까지
전부** 진다. 편의와 크기가 정면으로 부딪히는 자리이고, 이 회사는 **크기 쪽**을 골랐다.

### 그 대가가 가장 나쁜 실패다

지원 밖 기기에서 **조용히 느려지는 것이 아니라 지도 초기화가 실패한다.** 폴백을 넣으면
다시 두 백엔드를 다 담는 것과 가까워지는데 — **그 긴장을 글이 풀지 않고 '계획'으로만
남긴다.**

### 같은 원칙이 두 곳에서 다른 모양으로

웹은 **모듈을 나중에 불러오고**, 모바일은 **빌드 때 하나를 고른다.** 웹은 실행 중에 고칠 수
있고 모바일은 배포된 뒤에는 못 고치기 때문이다.

### 개선이 가장 필요한 곳의 수치가 없다

갤럭시 탭 S9 는 성능이 좋은 기기다. **정작 프레임이 아쉬운 저사양 기기의 결과가 없어**,
10% 라는 숫자가 어디까지 일반화되는지 알 수 없다. 배터리·발열도, iOS/Metal 도 없다.
...
HashiCorp(`/blog/all` 500) · Planet Labs(링크 없음) · dbt Labs · incident.io(404 네 번) ·
