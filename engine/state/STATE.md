...
**QuestDB — 도메인 셋을 다 채우고 done 으로 닫았다.** `qwp` · `offheap-histogram` ·
`window-join`. 회사 122개, 큐 1/3.

완주 기준 셋을 다 넘겼다 — 도메인마다 기능 1개 이상, 기능 셋이 이어져 고아가 없고, 수익원이
기능과 닿는다.

### 저장의 정렬이 실행 계획을 가능하게 한다

시간 창 조인의 뼈대가 그것이다. 두 표가 지정 타임스탬프 순서로 디스크에 놓여 있어서 **워커마다
이진 탐색 한 번으로 오른쪽 조각을 잡는다.** 그 하나가 병렬 계획을 성립시킨다고 회사가 적는다.
전용 연산자가 없던 시절에는 같은 질문이 ASOF 조인과 범위 조인으로 쪼개져 가격 표를 각자 훑었다.

수치가 그 차이를 보인다 — 체결 5,000만 건 대 가격 1억 5,000만 건에서 13.69초. 단일 스레드
SIMD 67.97초, ClickHouse 의 window-over-UNION 347.63초, DuckDB 는 30분을 넘겨 미완주,
Timescale 은 디스크 넘침.

### 자기 이득의 천장을 스스로 지목한다

코어 수만큼 안 나온 이유를 감추지 않는다. 바깥의 ORDER BY LIMIT 이 직렬이라 이 규모에서는
정렬+리밋이 병렬 구간과 맞먹고, 그래서 5.0배에 그쳤다고 **암달의 천장이라는 이름으로** 적는다.
이 엔진에서 병렬 이득의 상한을 스스로 대며 다음 과제로 남긴 드문 기록이다.

### 세 기능이 같은 습관을 보인다

알고리즘을 바꾸는 대신 **배치를 바꾼다.** 집계 상태를 힙 밖으로 옮겨 병렬 그룹바이에 들어가게
하고, 흩어진 값을 키별 연속 버퍼로 모아 SIMD 에 먹이고, 결과를 행으로 실체화하지 않고 프레임을
가리키는 버퍼로 넘긴다. 자바로 데이터베이스를 짜면서 런타임에 맡기지 않기로 한 자리들이다.

### 다음 사이클 — 후보 조사 (큐 1/3)

대기에 Feldera 하나가 남았다. 목표(3)에 못 미치므로 후보 조사를 한 번 끼운다.
