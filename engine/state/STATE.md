...
**Deno — 프로파일만 썼다.** 회사 144개, 큐 0/3. 도메인 둘이 비어 있고 둘 다 자료를 확보했다.

### 부품이 된 회사를 반대편에서 판다

방금 판 Val Town 의 격리가 바로 이 회사의 런타임이다. 이 엔진에서 **같은 문제(믿을 수 없는
코드 실행)를 쓰는 쪽과 만드는 쪽 양쪽에서** 보는 첫 사례다 — Val Town 은 네 판을 갈아엎어
데노 하위 프로세스에 닿았고, 여기는 그 아이솔레이트를 파는 쪽이다.

### 격리를 여러 겹으로 쌓는다

배포마다 V8 아이솔레이트를 **자기 프로세스** 안에 두고, 러너가 아이솔레이트 하이퍼바이저 노릇을
하며, seccomp 로 시스템콜을 좁히고, 파일시스템 쓰기를 막고, 할당량을 넘기면 종료한다. 엣지
30곳 이상에 애니캐스트로 붙이고 Host·SNI 로 어느 배포인지 정한다.

### 런타임을 만들면 언어의 규칙을 정하게 된다

중앙 레지스트리 없이 URL 로 바로 가져오는 HTTP 임포트를 골랐다가 **"상당한 대가가 따랐다"**고
되짚는 글을 냈다(라이언 달, 2024-07-29). 문제는 넷 — 주소가 길고, 한 버전에 묶여 변종이
여럿 생기고, 아무 서버에나 있어 CI 와 배포를 망가뜨리고, deps.ts 가 package.json 보다 번거롭다.

JSR 과 임포트 맵으로 옮기되 **좋았던 것(큰 타르볼 없이 잘게 받기)은 물려받았다**고 적는다.

### 다음 사이클 — 확장 (Deno 의 빈 도메인)

`모듈을 어디서 가져올지 정한다` 부터. 되짚은 회고라 feature history 를 세울 자료가 있다.
