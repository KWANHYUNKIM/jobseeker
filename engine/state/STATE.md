...
**Ferrous Systems — 기능 둘을 채웠다.** 빈 도메인 하나(`인증 문서까지 열어 놓고 판다`).
회사 148개, 큐 2/3.

### 인증은 고정된 것에만 붙는데 언어는 6주마다 바뀐다

이번에 쓴 `downstream-of-a-moving-language` 의 축이다. **3개월 주기**를 세워 상류 여러 판을
한꺼번에 담는다 — 26.05.0 이 러스트 1.93.0부터 1.95.0까지 다섯 판을 담았다. 그리고 고정한
판은 **2년 동안 패치**한다(기업 요금제는 요청하는 어느 판이든).

### 우회하면 판마다 그 우회를 다시 들고 가야 한다

그래서 상류 버그를 만나면 **자기 쪽에서 피하지 않고 상류를 고친다.** 질의 순환 하나에 일주일
넘게 썼고 일정이 상류 검토에 매인다 — 그래도 그쪽을 고른다. **포크도 하지 않는다.**

### 시험이 멈춰 있는 것은 결과가 아니다

무한 재귀가 개발용 파드를 죽인 뒤 얻은 교훈을 그대로 적는다 — **시험 기반 자체에 버그가 있을
때는 올바름만으로 부족하다.** CI 에 명시적 시간·메모리 한계를 두기로 했다.

### 다음 사이클 — 확장 (마지막 빈 도메인)

`인증 문서까지 열어 놓고 판다`. 재료는 손에 있다 — MIT/Apache-2.0 에 **인증 문서 포함**,
TÜV SÜD(ASIL D · SIL 3 · Class C), 월 25유로/연 240유로, 기업 요금제의 **인증 서명과 평가자
보고서·인증 지원**, 공개 문서(public-docs.ferrocene.dev)와 고객 포털의 갈림.
**⚠️ 오픈소스인데 무엇으로 값을 받는지 회사가 직접 설명하지 않는다** — 지어내지 말고
재구성으로 적는다.
