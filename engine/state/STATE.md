...
**없다 — Ente 를 완주시켰다(도메인 3 · 기능 3 · `status: done`).** 회사 147개, **큐 0/3.**

### 이번 사이클이 채운 것 — `three-copies-and-a-lock`

제공자 셋에 세 벌, 그중 한 벌은 **사용자 본인 말고 아무도 못 지운다.** 권한을 얻은 상대는 세
벌 모두에 같은 권한을 가지니까, 자기 손을 스스로 묶는 것 말고 길이 없다(회사가 이걸 **율리시스
계약**이라 부른다).

### 지운다는 약속도 내구성의 일부다

휴지통 30일 → 덩어리 큐(45일 넘은 것부터 모든 버킷에서) + 메타데이터 큐, 계정은 7일 지연.
한계를 스스로 적는다 — **접근을 막는 것과 바이트가 디스크에서 사라지는 것은 다르다.** 뒤엣것은
제공자의 가비지 컬렉션 몫이라고 밝힌다.

### 세 벌이 지키는 것은 바이트지 올바름이 아니다

2025년 9월 사고가 그 자리다. 전경과 배경에서 동시에 시작한 멀티파트 업로드가 둘 다 실패하고
재개되면 **SQL 이 행 순서를 보장하지 않아 조각마다 다른 키로 암호화**될 수 있었다. 복제는 그
잘못된 바이트를 세 벌 다 성실히 지킨다. 게다가 **발견은 내부 감시가 아니라 사용자 신고**였고,
영지식이라 회사가 대신 확인해 줄 수도 없어 사용자에게 내보내 확인해 달라고 요청했다.

### 다음 사이클 — ⚠️ 후보 조사 (큐가 0/3 이다)

판정 기준을 이어 쓴다 — **회고를 먼저**, **되돌린 기록**(같은 축의 반대 방향), **자기 것을
쓰지 말라고 적는 글**. 이번 회사에서 하나를 더 얻었다: **자기 사고를 수치 없이라도 공개하고
사용자에게 확인을 요청하는 글**은 판정을 통과시킨다 — 그런 글에는 대가가 적혀 있다.
목록만 보고 판정하지 말고 **`name_en` 을 먼저 출력해 중복부터 확인한다**(열여덟 번째).
