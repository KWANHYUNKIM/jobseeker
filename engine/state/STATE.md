...
**Kraken Technologies(EU/UK · 에너지)** `in_progress` · **도메인 3 · 기능 1** —
67곳 · ⚠️ 큐 0/3.

방금 쓴 기능: **`ship-100-a-day`(승인 하나로 하루 백 번을 스물다섯 나라 환경에 내보낸다)**.
**쪼개지 않기로 한 것이 먼저이고, 배포를 잘하는 일은 그 값이다.** 결정 8개 중 **7개가
`confirmed`** — 이 회사는 자기 파이프라인의 구멍까지 적는다(테스트 환경이 프로덕션 같지
않다 · 안 쓰는 플래그가 쌓인다 · 암묵적 머지 충돌은 도구로도 안 없어진다).

### ⚠️ 앞 사이클의 관찰을 정정했다 — 줄 수는 글마다 다른 게 아니라 자라고 있었다

**400만(2024-01) → 600만(2024-08) → 900만(2025-02) → 1500만 가까이(2026-03).
2년에 약 3.7배다.** 앞 사이클에서 "글마다 다르다"고 적었는데, **글을 더 읽으니 시간축이
보였다.** 타입 글의 `5 million` 만 공백·주석을 뺀 수라 결이 다르다.
**한 덩어리가 이 속도로 자라는데 쪼개지 않는다** — 이 회사에서 가장 눈여겨볼 대목이다.

### ⚠️ 연표를 못 세운 자리 하나 (규칙대로 버렸다)

**전 환경 동시 배포 → 일부 먼저** 로 바뀐 것이 두 글의 대조로 확인된다. 그런데
**언제 바뀌었는지는 두 글 사이라는 것밖에 모른다.** `history` 에 `from` 없이 쓰려다
검증에 걸렸고(**연도를 모르면 항목을 쓰지 않는다**), **두 시점의 사실만 기능 본문
(flow·decisions)에 남기고 `open_questions` 에 적었다.** 정보는 남고 없는 연도만 사라졌다.

### 다음 사이클 — 남은 두 도메인

- **1500만 줄에 타입을 뒤늦게 입힌다** — `static-typing-python-at-scale` +
  `mypy-imports`(2024-08) + `linting-and-formatting`(2024-01). **세 편이 같은 줄기라
  자료가 두껍다.** Import Linter 를 사내에서 만들었다는 대목도 여기 붙는다.
- **남의 전기 사업을 같은 코드 위에 올린다** — `upgrading-rabbit-mq...` + 제품 소개 +
  `django-kubernetes-health-checks`(2026-02-11, 아직 안 읽음).

### 아직 못 잡은 것

- **Octopus Energy 와의 관계를 1차 문장으로 못 찾았다** — 제품 사이트가 고객사로만 적는다.
- **과금 방식·가격·매출 비공개.**
- **연표(eras) 비었다.** ⚠️ 다만 블로그가 **2015년부터** 있다 — `django-elb-health-checks`
  (2016)와 `django-project-structure`(2018) 같은 옛 글이 **연도가 붙은 재료**가 될 수 있다.
