...
**ZOZO(JP · 커머스) — 364 에서 회사 프로파일까지 썼다.** `in_progress` · 도메인 1개 · **기능 0.**
**완료 60곳 + ZOZO = 61곳 · 큐 대기 2(쏘카 · 카카오뱅크) · 비교 문서 33편.**

### 다음 사이클은 ZOZO 의 첫 기능이다

도메인 `애매한 말로 찾기` 안에서 **WEAR 의 애매한 검색**을 기능으로 판다. 자료는 이미 확인해 뒀다 —
`techblog.zozo.com/entry/wear-hybrid-search`(2026-08-27). **결정 재료가 최소 여섯이다**:
하이브리드 결합(전문+벡터), 점수 정규화·가중치, SQfp16 양자화(프로덕트·이진을 버린 이유),
derived_source, warmup 순서(영구화 전 그래프에는 안 먹는다), 0.5초 타임아웃과 폴백.
**실패 경로 그림의 재료도 있다** — 리인덱싱 직후 p99 가 200ms 에서 20초까지 튀는 자리다.

**그 다음 후보**(제목만 확인) — `日本語CLIPを活用した検索ランキング学習`(2026-09-01) ·
`Thompson Samplingのコールドスタート対策`(2026-08-31). **랭킹·추천 도메인이 여기서 열린다.**
