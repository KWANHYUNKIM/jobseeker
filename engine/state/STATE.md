...
**없다 — 후보 조사 사이클이었다.** 회사 **114개**, 큐 **0 → 3/3.**

### 이번 사이클 — 서른세 번째 후보 조사

⚠️ **`큰 이름인데 아직 없는 회사` 각도가 다섯 조사를 버티고 이제 말랐다.** ⭐ **그래서 새 각도 셋을 썼고 셋 다 통했다:**

- **✅ Turso**(US · SaaS) — **오픈소스를 내는 인프라 회사** 각도. ⭐ **`우리는 Postgres 를 러스트로 짓고 있다 — 데이터베이스의 LLVM 을 써서`**(2026-07-16). **형식 검증으로 SQLite 버그를 찾는 글**과 동시 쓰기·멀티테넌시 구조. 최신 2026-09-03.
- **✅ Materialize**(US · SaaS) — **학술 배경이 있는 회사** 각도(차분 데이터플로). ⭐ **사전 압축으로 피크 메모리 981.0 → 428.4 GiB(56% 감소).** **Misra-Gries 스트리밍 알고리즘으로 적재 중 인기 값을 찾아** LSM 수명주기에 걸쳐 통계를 유지한다. ⚠️ **대가를 적는다** — 컬럼형만큼의 압축률은 못 내고, **압축 효율이 나빠지면 `OOM 루프`에 빠질 위험.**
- **✅ Zed**(US · SaaS) — **이미 판 회사와 정면으로 대비되는 회사** 각도. ⭐ **Cursor ↔ Zed.** `Zed Decoded` 자료구조 연작(`Rope & SumTree`·`Text Coordinate Systems`), 자체 모델(`Zeta2`), `Sandboxing`. 최신 2026-09-01.

### ⭐⭐ 가장 큰 교훈 — `첫 화면으로 판정하지 않는다`가 두 방향으로 값을 한다

**Supabase 에서는 개별 글(`OrioleDB Public Alpha`)을 열어 `없다`를 확정했고**, **Materialize 에서는 개별 글(`Dictionary Compression`)을 열어 `있다`를 찾았다.**

⚠️ **Materialize 는 목록 8편 중 7편이 제품·개념 글이라 그냥 접었으면 놓쳤을 후보다.** ⭐ **같은 규칙이 접는 데도 살리는 데도 쓰인다** — **목록은 회사가 무엇을 홍보하는지를 보여 주고, 개별 글은 회사가 무엇을 아는지를 보여 준다**(재구성).

### ⭐ Zed 가 특히 값진 이유

**Cursor 와 같은 자리에서 다른 답을 낼 가능성이 크다** — Cursor 는 **`AI 가 코드를 쓰자 사람 속도로 만든 것이 다 무너졌다`**로 갔는데, Zed 는 **러스트·GPUI·CRDT 협업**이라는 다른 밑바닥에서 시작한다. ⭐ **㉘축(맞았는지 어떻게 아나)에도 자체 모델(`Zeta2`) 이야기가 있다.**

### 다음 사이클 — 신규 (6순위)

**큐 3/3 이 찼다.** ⭐ **Turso 부터 판다** — **`Postgres 를 러스트로, 데이터베이스의 LLVM 으로`가 이 엔진에 없는 각도이고**, **형식 검증 글이 ㊵축(모르는 것을 어떻게 적나)의 새 재료다.** ⚠️ **최근 연작이 튜토리얼 성격이라 심층 글은 앞쪽에서 찾는다.**
