...
**Ubicloud — 프로파일만 썼다.** 회사 131개, 큐 1/3(Polar Signals 만 남음).
도메인 넷이 다 비어 있고 넷 다 자료를 이미 확보했다.

### 이 회사가 이 엔진에 없던 자리

남이 빌려주는 베어메탈(Hetzner 등) 위에 **오픈소스로 클라우드를 다시 짓는다.** 앞의 Depot 이
남의 CI 를 대체하는 자리였다면 여기는 **복제재**를 판다 — 남이 이미 파는 것을 같은 모양으로
싸게 짓는 쪽이라 기술 결정이 전부 원가로 되돌아온다. 2코어 8GB 가 월 41.89달러, 러너가
분당 0.00125달러(깃허브는 0.006달러).

### 도메인 넷을 이렇게 갈랐다

저장소는 성능 문제(디스크 생성 4~5분이면 러너를 못 판다), 가상화는 신뢰 경계 문제(잘못되면
느려지는 게 아니라 새어 나간다), 버스터블은 격리와 원가가 부딪히는 자리, 값표는 앞의 셋이
아낀 것이 드러나는 자리다.

### 벌써 눈에 걸린 것 — 갈아엎고 되돌린 기록

블록 저장소를 네 판 갈아엎었다. v2.0 에서 btrfs reflink 로 디스크 생성을 4~5분에서 1초
미만으로 줄였는데 **암호화를 잃고 처리량이 ext4 의 3분의 1로 떨어졌다.** v3.0 은 자체 SPDK
모듈(bdev_ubi)로 그 대가를 되돌린다. 이 엔진에서 **버린 것을 다음 판에 되찾은 기록**은 드물다.

### 다음 사이클 — 확장 (Ubicloud 의 빈 도메인)

`손님 디스크를 1초 안에 만든다` 부터. 판 번호는 있는데 각 판의 연도가 없어 eras 는 안 세웠다.
