...
**Rightmove(EU/UK · 커머스—부동산 포털)** `in_progress` · **도메인 2 · 기능 0** —
**72곳** · ⚠️ 큐 0/3. **부동산 축 첫 회사다.**

### ⭐ 사업 구조가 기술의 무게를 정한다

**트래픽의 85% 이상이 직접·자연 유입**이고 포털 체류 시간 10분 중 **9분**을 가져간다.
광고를 사서 손님을 데려오는 회사가 아니라는 뜻이고, **그래서 사이트맵이 잡무가 아니라
매출 경로다.** 도메인 이름을 그 사실로 잡았다 — `자연 유입이 매출의 바탕이라 색인을
직접 관리한다`. 매출은 거래가 아니라 **회원 수 × 단가**로 움직인다(중개 지점 16,591곳 ·
ARPA 1,726파운드 · 유지율 96%).

### 다음 사이클 — 사이트맵 쪽이 훨씬 단단하다

**`from-legacy-to-resilience...`** — RFC 로 대안을 **장단점 표**로 놓고 고르며
**버린 쪽의 장점까지 적는다**(추가 저장소 불필요 · 사내 기존 패턴 · **디버깅이 쉽다**).
버린 이유도 명확하다(`a single instance meant any failure disrupted the process`).
**수치** — 지역 **266,569개** · 생성 **5시간 → 30분** · REST 요청 수십만 건 제거.
**인정한 대가** — 로컬 상태 유지의 복잡함 · 데이터 일관성 · 추가 저장소.

### ⚠️ 이 회사에서 조심할 것

1. **재무를 3자 매체로만 확인했다** — `plc.rightmove.co.uk` 1차 IR 을 안 읽었다.
   **다음에 열면 `inferred` 를 `confirmed` 로 올릴 수 있다.**
2. **두 글의 결이 크게 다르다** — 프런트엔드 재구축 글은 **수치가 하나도 없다.**
   Nubank·Bolt 에 이어 **세 번째 사례다.**
3. **다시 지은 앱이 무엇인지 글이 이름을 밝히지 않는다** — 보고서·권한·대량 온보딩으로
   보아 중개사용 사내 도구로 보이지만 확정하지 못했다.
4. **기술 글이 5페이지 분량인데 두 편만 읽었다** — 나머지에서 도메인이 더 나올 수 있다.
