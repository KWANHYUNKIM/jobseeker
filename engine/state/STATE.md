...
**Razorpay** (IN · 핀테크) — 도메인 3개 · **기능 1개**(`check-nobody-uses-it-then-cut`). 남은 둘은 **본문 미독**이다(CI 스팟 · AI 조사 도구).

### 이번 사이클 — `쓰는 사람이 없다는 것을 확인하고 나서 줄인다`

건진 것 넷:

- ⚠️ **`아마 안 쓸 것이다` 와 `확인했더니 안 썼다` 는 다른 근거다** — **`경보 규칙과 대시보드를 감사해 분 미만 해상도에 의존하는 것이 있는지 확인했다. 하나도 없었다.`** 그러고 나서 간격을 30초 → 60초로 바꿔 **인프라 메트릭 수집을 절반**으로 줄였다.
- ⚠️ **더 크게 줄일 수 있는 길을 조직 때문에 거절했다** — 도구 교체는 **`서비스 디스커버리, relabel 파이프라인, 스크레이프 설정을 팀 전체에 걸쳐 바꿔야 했을 것이다. 그것은 모니터링 이전을 큰 조율 프로젝트로 만들었을 것이다.`** **한 팀이 끝낼 수 있는 일과 온 조직이 움직여야 하는 일은 값이 다르다**(재구성).
- ⚠️ **이 기능에서 가장 값진 한 줄** — **`위험한 실패 모드는 스플릿 브레인이다: 두 파드가 모두 자기가 리더라고 믿고 같은 타깃을 수집하는 것.`** 그래서 **`시스템은 중복을 감수하기보다 수집을 멈추는 쪽으로 치우쳐 있다`.** ⚠️ **비용을 줄이려고 만든 장치가 실패할 때 비용이 아니라 공백 쪽으로 넘어지게 설계했다**(재구성).
- ⚠️ **`안 쓰인다` 를 세 곳으로 판정한다** — **`어디에도 쓰이지 않는 메트릭 167개를 찾았다: 경보에도, 대시보드에도, 관측된 질의에도.`** 90일 관찰, 상위 500개 고카디널리티 중. **각 팀에 요청하는 대신 중앙 relabel 로 버렸다** — 코드 릴리스를 기다리지 않으려고.

### ⚠️ 수치 하나를 인용에 맞춰 내렸다

**첫 요약에서 `하루 4,500억 샘플` 로 받았는데 원문 인용은 `roughly 400 billion daily samples` 였다.** ⚠️ **인용을 따라 `약 4,000억` 으로 적었다** — **요약이 준 숫자와 원문이 다르면 원문을 쓴다.**

### 다음 사이클 — 확장(2순위)

`--gaps` 가 **`빌드 파이프라인을 싼 자원 위에 올린다`**(CI 를 스팟 인스턴스로, 2026-08-05)를 부를 것이다. ⏳ **본문 미독** — **URL 은 목록에서 받아 뒀다**(추측 금지).

⚠️ **큐는 2/3**(Flipkart · Twilio). ⏳ **보강 거리** — Grafana Labs 정본과 중복 파일 인용 대조.

### ⚠️ 비교 문서 재료 (초안 유지)

**① `AI 에이전트를 어디까지 믿나` 8곳 + ⏳ Razorpay** — Sentry / ClickHouse / Duolingo / Ramp / DoorDash / Deliveroo / Snyk / Cygames.
...
⭐ **⑩ `관측 비용을 어디까지 줄이나` — 재료가 붙기 시작했다** — **Razorpay**(사는 쪽: 쓰는지 확인하고 줄인다, **실패는 공백 쪽으로**) / **Grafana Labs**(파는 쪽: TCO 25% 감소) / **Honeycomb**. ⚠️ **사는 쪽과 파는 쪽이 같은 문제를 반대편에서 본다**(재구성).
