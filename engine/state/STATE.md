...
**Trainline(EU/UK · 커머스—철도·버스 예약)** `in_progress` · **도메인 1 · 기능 0** —
**71곳** · ⚠️ 큐 0/3.

### ⚠️ 도메인이 하나뿐인 이유 — 자료가 한 편이다

`medium.com/trainline` 은 **발행물 홈도 태그 경로(`/tagged/...`)도 목록이 안 온다.**
브라우저로도 제목·팔로워 수·탭 이름만 오고 글이 비어 있다. 검색으로도 다른 기술 글을
못 찾았다. **읽은 글은 CDN 이전 한 편(2024-01)뿐이고 블로그가 지금도 사는지 모른다.**
**PayFit 처럼 도메인 1개로 완주하는 형태가 될 수 있다.**

### ⭐ 대신 사업 쪽은 1차 자료로 단단하다

`trainlinegroup.com` 의 **FY2026 실적을 브라우저로 읽었다** — 순 티켓 판매 **63.19억
파운드(+7%)** · 매출 **4.53억 파운드(+2%)** · 조정 EBITDA **1.77억(+11%)** · 영업이익
**1.22억(+43%)**. **⚠️ 이 회사의 특이점은 수수료율을 자기가 못 정한다는 것**이다 —
영국 B2C 온라인 기본 커미션이 **5% → 4.5%** 로 내려갔다(2022년 3월 예고, 2025년 4월 발효).
**판매액은 +7% 인데 매출이 +2% 인 이유가 그것이다.**

### ⚠️ 못 판 자리를 도메인 지도에 그려 뒀다

**Platform One** — 실적 각주에 이름만 나오고 국제 사업이 내부 거래 수수료를 내고
접근한다는 것뿐이다. **표를 모으고 값을 매기고 재고를 다루는 쪽이 이 회사의 핵심일 텐데
기술 자료가 없다.** 도메인 지도에 점선 상자로 남겼다.

### 다음 사이클 — 하나뿐인 도메인

**45개국의 콘텐츠를 자기 손으로 배달한다** — CDN 이전 글(전문 읽음).
**실패가 둘 다 "작게만 시험한 대가"다**: 리다이렉트 100개에서 청구가 튀었고
(Lambda@Edge → CloudFront Functions), 그 대안도 수천 개에서 타임아웃이 났다.
**수치도 좋다** — 도메인 300곳 이상 · 주 도메인이 트래픽 70%+ 이고 설정 JSON 8,500줄+ ·
9개월 미만 · 가중 DNS 5→10→25→50→75→100% · 롤백 위해 TTL 5분.
