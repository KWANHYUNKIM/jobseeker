...
**없다 — Let's Encrypt 를 완주했다(도메인 2 · 기능 2).** 큐 0/3 이라 다음 사이클은 **후보 조사**다.

### 그 30일이 안전 여유의 전부다

수명이 90일인데 **갱신은 60일에 시키라고 말한다.** 그 차이가 **자동화가 깨졌을 때 사람이
손으로 갱신할 시간**이다 — **'필요하면 손으로 갱신할 시간이 충분하다.'** 자동화를
전제하면서도 **자동화가 깨질 것을 계산에 넣은** 유일한 자리다. **45일이 되면 그 여유가
얼마인지는 글이 밝히지 않는다.**

### 불편함이 부작용이 아니라 목적이다

이유를 둘 적었는데 하나가 **'자동화를 유도한다'** 다. **손으로 못 하게 만드는 것 자체가
노린 것**이라, '짧아서 불편하다'는 반발은 애초에 설계가 부른 반응이다. curl 이 부담을
지는 쪽을 바꿔 문을 닫았다면, 여기는 **부담을 남겨 자동화를 강제한다** — 둘 다 불편함을
설계 수단으로 쓴다.

### 자기 논거가 밖으로 나가면 설명할 것이 사라진다

2015년 글은 **왜 90일인지**를 자기 이유로 적었다(피해 제한 + 자동화 유도 · 파이어폭스
텔레메트리 29%). 2025년 글은 **업계 표준이 요구해서** 45일로 간다 — **물린 대안도 정량
비용도 없고 일정만 있다.** 자기가 먼저 간 길을 남들이 따라오자 근거의 출처가 밖으로 나갔다.

### 두 도메인이 서로를 떠받친다

알리는 길을 접었으니 **수명이 실질적인 방어**가 되고, 수명이 짧아질수록 **실시간으로
알릴 필요가 준다.** 한 회사가 같은 문제의 두 답을 반대 방향으로 민 예다.

### 브라우저가 잰 것이 웹의 사실이 된다

'90일은 새롭지 않다'의 근거가 **파이어폭스 텔레메트리 29%** 다. 모질라가 모은 수치를
남이 정책 논거로 쓴다 — **모질라 프로파일과 이어 붙는 자리**다.

### 다음 사이클

**후보 조사**(큐 0/3). 인증 기관 축은 이제 하나 찼다.
