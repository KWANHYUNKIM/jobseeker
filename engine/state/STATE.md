...
**없다 — DeNA 를 완주했다(도메인 2 · 기능 2).** 큐 0/3 이라 다음 사이클은 **후보 조사**다.

### 조사기와 사고 보고서는 다르다

MHA 글은 **원인을 2015년 커밋까지 밝히고** 발생 조건까지 못 박는데, **대응은
'나중에 글로 쓸 수 있으면 좋겠다'** 에서 멈춘다. 트리거닷데브가 **고칠 것 여섯 가지를
같은 글에** 적은 것과 갈린다 — **'왜'와 '그래서 무엇을'은 다른 글이 될 수도 있다.**
원장에 둘 다 있으니 **어느 쪽이 더 쓸모 있는지 견줄 수 있게 됐다.**

### 얹혀 있는 도구는 밑이 바뀌면 조용히 어긋난다

**MySQL 5.7.9(2015)** 에서 mysqlbinlog 가 relaylog 의 슬레이브 FDE 를 무시하게 바뀌었는데
**MHA 는 첫 FDE 만 쓰는 옛 구현 그대로**였다. **10년 동안 아무 일 없다가** 페일오버라는
드문 상황에서 드러났다 — **원인의 나이가 10년**이라는 것이 이 조사의 값이다.

### 도구가 안 알려 주면 형식까지 내려간다

오류 메시지 → MHA 소스 → MySQL 소스와 binlog 규격 → **hexdump 로 바이너리** → **5.7.8과
5.7.25 동작 비교.** 그리고 빈도를 **'이 한 번밖에 일어나지 않았다'** 로 그대로 적는다 —
부풀리지도 축소하지도 않는다.

### 같은 손버릇이 15년 이어진다

2010년 첫해 글이 **HandlerSocket** 이다 — **MySQL 의 SQL 파서를 통째로 건너뛰어** 엔진
성능을 끌어냈다. **엔진 내부에 붙는 방식**인데, **2026년에 터진 자리도 같은 층**이다.
얻은 것과 대가가 15년 간격으로 한 회사 안에 나란히 있다.
