...
**Electric — 기능 하나를 채웠다(`serve-reads-from-immutable-chunks`).** 빈 도메인 하나.
회사 158개, **큐 0/3**.

### 읽기를 캐시할 수 있게 만드는 것이 저장 설계의 문제였다

읽을 때 계산해 내주면 CDN 이 접을 수 없다. 그래서 **무거운 계산을 쓰기 시점으로 밀어내고**
로그를 고정 크기 청크로 나눠 **다 찬 청크는 불변**으로 둔다 — 불변인 조각은 잠금 없이 읽히고
CDN 이 같은 요청을 중복 제거해 접는다.

### 색인을 촘촘히 하면 쓰기가 죽는다

옛 엔진(CubDB)은 **삽입마다 내부 구조를 갱신**해 CPU 를 과하게 먹었고 큰 트랜잭션이 읽기를 막아
P95 가 치솟았다. 새 엔진의 색인은 **희소하다** — 청크 경계 포인터만 담고, 포인터가 언제나 끝에만
붙어 **잠금이 필요 없다.** 실사용이 이걸 드러냈다: Trigger.dev 의 **초당 2만 건**.

### 읽는 쪽을 떼어 내니 두 가지가 따라왔다

보조 서버가 로그를 고치지 않고 읽으니 **수평 확장**과 **무중단 배포**가 함께 생겼다 — 옛
구조에서는 둘 다 불가능했다.

### 언제 갈아엎을지를 스스로 적는다

처음에 CubDB 를 쓴 것은 같은 언어(엘릭서)에 두어 v1.0 을 빨리 내려는 선택이었고, 성능을 손대기
전에 버그를 많이 잡고 실제 부하를 배웠다고 적는다. 그리고 못 박는다 — **범위가 좁고 팀에 그
전문성이 있을 때만 통하는 전략**이다.

### 데이터에 심은 대비

**프로토맵스** — 지도 타일을 읽기 전용 파일 하나로 만들어 범위 요청으로 내주는 쪽. 둘 다
**불변이 곧 캐시 가능성**이라는 같은 문장 위에 있다.

### 다음 사이클 — 확장 (마지막 빈 도메인)

`내구성을 커널 속도로 만든다`. Durable Streams 글이 재료다 — 카프카식 비동기 복제를 물리고
단일 노드 fsync + 그룹 커밋 WAL, 전선 형식 그대로 저장, `sendfile(2)`·`splice(2)`,
4vCPU 에 초당 86만 append.

### ⚠️ 큐가 0/3 이다

Electric 을 완주시킨 뒤 **후보 조사 사이클을 끼운다.**
