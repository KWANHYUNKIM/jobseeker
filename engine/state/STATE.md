...
**Ramp** (US · 핀테크) — 도메인 3개 · **기능 1개**(`workflows-as-graphs-not-code`). 남은 도메인 둘.

### 이번 사이클 — 워크플로 `고객마다 다른 규칙을 코드로 짜지 않고 그래프로 만든다`

건진 것 넷:

- ⚠️ **실패를 `안 돌아간다` 가 아니라 `더 이상 우아하지 않다` 로 적는다** — `no longer elegant`. 이유 둘이 **다 표현의 문제**다: 선형으로 쓰는 논리를 규칙 형태로 옮기는 **번역 코드**가 필요했고, **액션과 액션 사이의 의존**이 `exceedingly difficult to represent` 였다. **돌아가지 않아서가 아니라 계속 만들 수 없어서 갈아엎었다**(재구성).
- ⚠️ **쓰기와 실행 중 실행을 단순하게 골랐다** — `code that is easy to execute but difficult to create`. **하루 100만 건이 도는 쪽이 실행이라 맞는 선택인데, 대가가 실제로 나타났다 — 쓰기가 한때 20초를 넘었다**(지금 200ms 미만).
- ⚠️ **Celery 를 버린 근거가 `1만 개당 7개`** 다. **99.93% 는 도니까 지표로는 멀쩡해 보이는데, 승인 시스템에서 빠진 7개는 누군가의 지출이 영영 처리되지 않는 것이다**(재구성). 직접 만든 포스트그레스 큐가 **회사 전체 작업 신뢰성까지** 올렸다.
- **사람은 선형으로 쓰고 기계는 그래프로 돈다** — `elif` 조차 그래프에 없는 개념이라 **부정 조건을 겹쳐** 만든다. **대가는 표현력이 컴파일 대상에 묶인다는 것**이고, 실제로 **루프와 펍섭 트리거가 아직 없다.**

**연표는 세우지 않았다** — **2022년 가을**(고객이 초기 가정을 깨기 시작한 시점)은 나오는데 **첫 설계의 시작과 그래프로 넘어간 시점이 없다.** 연도를 지어내지 않았다.

### 다음 사이클 — ML 배포 아니면 에이전트

**둘 다 자료가 익었다.** Turbo 는 **버린 길이 명시적이고**(표준·코드 리뷰로 다스리기) 수치가 극적이다(**넉 달·3,000줄 → 5분 미만·약 40줄**, 개발자 9명이 모델 약 20개). Inspect 는 **기성품을 안 쓴 이유**(`it only has to work on your code`)와 대가 넷이 있다.

⚠️ **Inspect 를 쓰면 비교 문서의 네 번째 회사가 채워진다** — Sentry · ClickHouse · Duolingo · **Ramp**. 축은 **`그 선을 무엇이 지키나`** 다.
