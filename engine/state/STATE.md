...
**DuckDB — 기능 하나를 채웠다(`quack-protocol`, 프로세스 밖으로 나가되 형식은 우리 것으로).**
회사 118개, 큐 2/3. 빈 도메인 둘이 남았다 — 원격 읽기 · 파서.

### 전제를 어디까지 물릴지를 정하는 기능이다

이 프로젝트의 정체성이 서버가 되지 않는 것인데, 같은 파일에 여러 프로세스가 쓰려는 순간 그
전제가 깨진다. 그래서 이 기능은 능력을 더하는 일이 아니라 **어디까지 물러날지 선을 긋는 일**이다.
답은 프로세스 밖으로 나가되 형식은 내주지 않는 쪽이었다.

### 이름을 대며 두 표준을 물린다

포스트그레스 와이어는 행 단위라 대량 전송에 가망이 없다고 적는다(76GB 에 158.37초 대 4.94초).
Arrow Flight SQL 은 질의마다 왕복이 최소 둘이고 **밖에서 통제하는 형식에 스스로를 묶을 수
없다**는 이유로 물린다. 전송만 표준(HTTP)에 얹어 로드밸런서·방화벽 인프라를 얻고, 몸통은 WAL 이
몇 년째 쓰던 내부 직렬화를 그대로 재사용한다.

상호운용성을 혁신 속도와 바꿨다고 스스로 적는다. 이 엔진에서 **남의 규칙을 다루는** 축의 여덟
번째 답이다 — Zed 가 LSP 의 UTF-16 을 요약에 박아 두고 늘 유지하는 쪽이었다면, 여기는 표준을
쓰지 않고 전송 계층만 빌린다.

### 안 되는 지점을 벤치마크로 드러내고 그대로 적는다

병렬 삽입이 8스레드에서 초당 5,434 트랜잭션으로 PostgreSQL(4,320)을 넘지만 그 위에서 정체하고
그다음부터는 진다. 프로토콜이 아니라 엔진의 한계라고 밝히고 다음 일로 남긴다.

### 다음 사이클 — 확장 (DuckDB 의 빈 도메인)

원격 읽기(비동기 I/O)와 파서 중 `--gaps` 가 고르는 대로 간다.
