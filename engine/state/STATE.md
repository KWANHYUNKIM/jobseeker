...
**Render** — 첫 기능 `three-generations-of-routing-state`(같은 문제를 세 번 다시 짓는다)를
끝냈다. **도메인 2 · 기능 1.** 큐 0/3. 남은 도메인은 **무너져도 도는 것은 돌게 한다.**

### 세대를 가르지 않았으면 그 문장이 안 나온다

**'더 공격적으로 캐시하는 것과 갱신을 더 빨리 퍼뜨리는 것 사이의 고전적 긴장'** — 단면도
하나였으면 '지금 이렇게 되어 있다'에서 끝났을 것이다. **무엇이 한계여서 다음으로 갔는지를
적으니 문제 자체가 이름을 얻었다.** 앞 사이클(후보 조사)에서 세대를 나눠 적는 글이 이
엔진에 잘 맞는다고 한 것이 그대로 확인됐다.

### 0 을 주장하지 않고 15초라고 적는다

3세대는 **상태가 약 15초 동안 낡는다**고 밝힌다. 완벽을 말하면 어디까지 믿을지 모르는데,
**폭을 밝히면 그 위에서 무엇을 설계할지 정할 수 있다.** 큐드런트가 재현율 떨어지는 칸을
남긴 것과 같은 태도다.

### 자기가 정확히 못 하는 칸을 남에게 맡긴다

3세대 라우터를 직접 지어 놓고 **트래픽 분할은 클라우드플레어 층**에서 한다. **15초 낡는
상태 위에서 5% 를 정확히 나누기는 어렵다**는 계산일 수 있다 — **어디까지 직접 짓고
어디부터 얹을지**를 이 회사가 그은 선이다.

### 대가는 말하는데 값은 자주 비어 있다

라우터 **과잉 배치의 비용**, 파랑/초록을 **두 벌 돌리는 비용 증가분**, 2세대 **데이터베이스
부하의 크기**가 다 없다. **'무엇을 냈는지'는 적고 '얼마를 냈는지'는 자주 뺀다** —
open_questions 에 남겼다.

### 다음 사이클

확장 — 남은 도메인(숨은 의존성). **트리거닷데브의 etcd 사고와 이어 붙는 자리**이고
DNS 글이 이미 근거로 서 있다. 큐가 0/3 이라 그 뒤에 후보 조사가 뜬다.
