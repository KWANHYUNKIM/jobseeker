...
**Dolt — 기능 하나를 채웠다(`prolly-tree`, 같은 데이터가 저절로 같은 청크가 되게 한다).**
회사 120개, 큐 0/3. 빈 도메인 둘 — SQL 문법 · 와이어 호환.

### 조회를 내주고 차이를 샀다

B-트리는 두 트리의 차이를 구하려면 양쪽을 훑어야 해서 트리 크기에 비례한다. 프롤리 트리는 루트부터
내려가며 해시가 같은 가지를 통째로 건너뛰므로 **차이의 크기에 비례**한다. 이 회사가 파는 것이 바로
그 차이라 교환이 성립한다 — 대신 평범한 조회·갱신에서 MySQL 보다 약 2배 느리고, 그 숫자를 자료구조
소개 글에 그대로 적는다.

손댄 자리가 구체적이다. 원래 Noms 구현의 롤링 해시는 작은 청크를 잔뜩 만들고 큰 청크를 몇 개
남기는데, 청크 안에서는 이진 탐색이라 큰 청크가 곧 지연이다. 누적분포로 목표 확률을 계산해 4KB 근처
정규분포로 몰았고, 경계를 정할 때 값은 안 보고 키만 봐서 같은 크기의 값 갱신이 경계를 밀지 않게 했다.

### 자기 제품을 다룬 논문을 공개로 반박한다

ACM 저널에 실린 프롤리 트리 논문이 DoltHub 워크로드에서 삽입 30~50% 개선을 주장했는데, 회사가
읽고 조목조목 답한다 — 제안 셋이 이미 있거나(트리 머지 최적화는 2025년 7월에 넣었다) 성능을 설명하지
못하고, **30~50%라는 수치는 논문의 벤치마크가 전부 메모리 안에서 돈 코드라서 나온 것**이다. 돌트는
디스크에 영속한다. 저자들의 연구와 코드 기여는 인정한다고 함께 적는다.

이 엔진에서 남이 자기 제품을 연구 대상으로 삼았을 때의 응답이 기록으로 남은 첫 사례다(재구성).

### 두 글이 어긋나는 것을 덮지 않았다

2024년 자료구조 글은 누적분포로 정규분포를 만든다고 하고, 2026년 논문 검토 글에는 레벨을 소금으로
쓰는 해싱이 기하분포를 만들어 조회에 불리하다는 대목이 있다. 같은 층을 말하는지 다른 층인지 못
가렸으므로 **2024년 글을 따르고 그 사실을 `open_questions` 에 적었다**(PROMPT 2″단계).

### 다음 사이클 — 확장 (Dolt 의 빈 도메인)

큐가 0이다. Dolt 를 다 판 뒤 후보 조사를 끼운다.
