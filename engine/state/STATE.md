...
**없다 — 문체 보수 사이클이었다.** Temporal 을 배민 문체로 다시 썼다(2.4 → 0.0). 회사 115개, 큐 2/3. 60곳 남았다.

### 영문 원문이 지금까지 가장 많았다 — 스물다섯 조각

Rightmove 에서 셋을 풀었는데 Temporal 은 스물다섯이었다. 회사 글이 영어이고 그 문장이
결정적일 때 그대로 붙여 둔 흔적이다. 전부 한국어로 풀었다.

푼 방식은 직역이 아니라 문장에 녹이는 것이다. 예를 들어 워크플로 크기 제한 문장은
워크플로 하나하나는 크기를 제한하되 개수는 무한히 늘릴 수 있다로 옮겨 앞뒤 문장과 이어지게 했다.

긴 것부터 치환해야 한다. 짧은 조각이 긴 조각의 일부인 경우가 많아 순서를 뒤집으면 문장이 깨진다.

### 결과에 이르는 기록을 진실로 삼는다 — 셋이 같은 자리다

Temporal 은 상태를 저장하지 않고 상태에 이르는 길을 저장한다. 오류가 나면 이벤트 이력을 읽어
각 단계를 다시 밟는다. 이 엔진에 같은 발상이 둘 더 있다.

- Neon 은 로그를 데이터베이스로 삼고 데이터 파일을 그 캐시로 둔다.
- Databricks 는 커밋을 세이프키퍼 정족수에서 끝내고 페이지서버가 뒤에서 페이지를 만든다.

셋 다 결과가 아니라 결과에 이르는 기록을 진실로 삼는다. 그리고 셋 다 같은 값을 치른다 —
읽을 때 다시 만들어야 한다.

### 이번 사이클 — Temporal

사업 모델에서 요금 단위가 아키텍처를 그대로 드러낸다는 것을 앞세웠다. 액션은 워크플로가 한
걸음 나아갈 때 세는 단위이고, 저장이 활성과 보존으로 갈린 것은 완료된 워크플로의 이력을
객체 저장소로 옮기는 계층 저장과 짝을 이룬다.

시끄러운 이웃을 어디서 막느냐도 견줬다. Atlassian 은 클라이언트 쿼터로 카프카 상류에서 막고
여기는 영속 계층을 자기 것으로 만들어 바닥에서 막는다.
