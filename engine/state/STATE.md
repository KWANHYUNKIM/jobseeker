...
**1Password** (CA · 보안) — 도메인 3개 · **기능 1개**(`two-secrets-and-no-way-back`). 남은 도메인 둘.

### 이번 사이클 — 제로 지식 `둘을 합쳐야 열리게 만들고, 잃으면 회사도 못 돌려준다`

건진 것 넷:

- ⚠️ **거부한 모델을 인용문으로 남긴다** — `trust the server, use access controls, audit the logs, and **hope** the infrastructure isn't compromised. We weren't satisfied with that.` **네 장치가 다 사후적이다** — 뚫린 뒤에는 되돌릴 것이 없다. **이 회사는 뚫리는 것을 막는 대신 뚫려도 소용없게 만들기로 했다**(재구성).
- ⚠️ **같은 성질이 방어이자 위험이다** — 둘을 합쳐야 열리므로 **한쪽만 새서는 안 열리고, 한쪽만 잃어도 못 연다.**
- ⚠️ **이 회사에서 가장 무거운 문장** — `we cannot return them to you, **because we don't have them**`. **못 도와주는 것이 실패가 아니라 설계의 증명이다** — 되돌려 줄 수 있다면 **어딘가에 갖고 있다는 뜻**이고 그러면 구조 전체가 거짓이 된다.
- ⚠️ **친절해지는 모든 방법이 구조를 무너뜨린다** — 복구도, 서버 측 검색도, 실제 자격증명 유출 확인도. **우회로를 따로 만들되**(복구 코드·기기 측 검색·k-익명성 부분 해시) **셋 다 완전하지 않고, 회사가 그것을 감춘 채 팔지 않는다.**

**한 가지는 3자 자료에 기댔다** — `무작위와 구별되지 않는다` 는 **내용**에 대한 말이지 **크기·개수·시점**에 대한 말이 아니다. 3자 자료의 **2023년 말 암호화 형식 변경**(금고 크기 추정 차단)을 **회사 자료로 확인하지 못해** 그렇게 표시했다.

**공백 하나를 새로 적었다** — 개인은 `우리가 갖고 있지 않아 돌려줄 수 없다` 인데 **기업은 직원이 나갈 때 그 금고를 어떻게 하는지**가 글에 없다. **매출의 75%가 기업이라 중요하다.**

### 다음 사이클 — 봉인된 방, 그다음이 프로비저닝

`--gaps` 가 **못 읽는데 처리해야 할 때 봉인된 방을 만든다** 를 부를 것이다. **자료가 충분하다** — Nitro Enclaves · Rust(클라이언트 암호 라이브러리 재사용) · **서명 + 제3자 투명성 로그가 아니면 클라이언트가 접속을 거부** · **컴파일 타임에 강제되는 안전 로깅** · 2024년 9월 독립 감사 취약점 0.

⚠️ **프로비저닝 도메인은 여전히 자료부터다**(tech 0, 검증 경고). **그 차례가 오면 기능보다 자료를 먼저 찾는다.**
