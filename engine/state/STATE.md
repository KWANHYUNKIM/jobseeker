...
**Ferrous Systems — 프로파일만 썼다(도메인 셋, 기능 0).** 회사 148개, 큐 2/3.

### 코드가 아니라 "이걸 써도 된다"는 문서를 판다

컴파일러도 **인증 문서 전체도** MIT/Apache-2.0 으로 열려 있다. 그런데 사용자당 월 25유로를
받는다. 바이너리도 문서도 공짜로 가져갈 수 있으니 파는 것은 **그것으로 규격을 통과했다고
말할 수 있게 해 주는 관계**다(재구성 — 회사가 직접 설명하지는 않아 열린 질문에 남겼다).

### 지그 재단의 정반대 자리다

저쪽은 언어를 **만들고** 여기는 남의 언어를 **인증한다.** 상류 rustc 가 6주 주기인데 이쪽은
3개월 주기다 — 바뀌는 것과 고정해야 하는 것 사이에서 사는 회사다.

### 규격이 요구하는 것은 "좋은 코드"가 아니다

**"인증된 코드가 인증 안 된 코드를 절대 안 부른다"**는 증명이다. 그런데 러스트는 `do_io()?`
한 줄이 숨은 함수를 부르니 손으로는 못 한다. 그래서 컴파일러 드라이버 안에서 **호출 그래프의
닫힘**을 검사한다 — 검증된 함수 집합은 함수 호출 연산에 대해 닫혀 있어야 한다.

### 다음 사이클 — 확장 (첫 도메인)

`인증받은 것과 아닌 것 사이에 선을 긋는다` 가 자료가 가장 두껍다. `Callgraph analysis` 가
**물린 대안 넷을 이유와 함께** 적는다 — 클리피 린트(의존성에 재귀 적용 안 됨) · 효과 타입
시스템(새 언어를 만드는 셈) · 링커 시점 검사(`panic = "abort"` 와 안 맞음) · `cfg` 로 core
포크(유지 불가능한 타깃 변종). 두 겹 패스(THIR·MIR)의 대가도 적혀 있다.
