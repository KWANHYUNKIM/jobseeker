...
**없다 — Ferrous Systems 를 완주시켰다(도메인 3 · 기능 3 · `status: done`).** 회사 148개, 큐 2/3.

### 이번 사이클이 채운 것 — `open-the-evidence-sell-the-signature`

**증거를 다 열어 놓고 서명을 판다.** 공개 문서에 여덟 편이 다 있다 — 안전 매뉴얼 · 인증 계획 ·
언어 명세 · 평가 보고서 · 인증 보고서 · 내부 절차 · 평가 계획 · **추적성 행렬**.

### 인증이 보는 것은 코드가 아니라 연결이다

명세가 기대 동작을 못 박고, **추적성 행렬이 명세 세부와 시험 스위트를 이어** 적은 요구사항이
전부 덮이게 한다. 빈칸이 곧 인증 밖이다. 시험이 많다는 것과 요구사항이 다 덮였다는 것은
다른 말이라서다.

### 증명이냐 증거냐

CompCert 는 컴파일러의 올바름을 **증명**으로 보인다. 여기는 명세·시험·추적성이라는 **증거**로
규격이 요구하는 만큼을 보인다. 같은 문제에 두 길이 있고 이 회사는 뒤엣것을 골랐다(재구성).

### ⚠️ 재구성이라고 분명히 적은 자리

**오픈소스인데 무엇으로 값을 받는지 회사가 직접 설명하지 않는다.** 복제할 수 없는 것은 문서가
아니라 제3자 서명과 그 뒤의 책임이라고 재구성했지만, 회사 자료에 그 논리는 없다 —
`open_questions` 에 그대로 남겼다.

### 다음 사이클 — 신규 (대기 맨 위의 Rive)

`Why Scripting runs on Luau`(2025-11-20)가 거절한 대안 셋을 이유와 함께 적는다 — WASM · 자바
스크립트 · 자체 언어. **⚠️ 나머지는 사례 소개·제품 공지다.** 렌더러 관련 글이 더 있는지
목록을 더 훑어야 한다.
