...
**Backblaze — 기능 둘.** 남은 빈 도메인은 값 하나. 회사 134개, 큐 1/3.

### 세는 것으로 끝내지 않고 남에게 준다

제조사가 고장률을 안 주니 34만 대를 직접 센다. 여기서 한 걸음 더 가서 **2013년부터 13년째
분기마다 공개**한다 — 하루에 드라이브 한 대당 한 줄, 평생 드라이브-일 5억 2,996만에 고장
2만 212건. 자료는 압축 1.3GB, 푼 것 12GB 다.

### 정의를 단순하게 두어 13년을 한 줄로 잇는다

고장은 "어제 있던 게 오늘 없으면"이다. 더 정교하게 만들 수 있지만 그러면 해마다 뜻이 달라져
13년치가 안 이어진다 — 대신 **예방 교체와 갑작스러운 죽음이 안 갈린다**는 값을 문다.

### 자기 자료의 설득력을 스스로 깎는 말을 붙인다

같은 제조사의 모델끼리도 신뢰성이 다르다고 적고, 표본이 적은 모델(100대·드라이브-일 1만
미만)은 아예 표에서 뺀다. 알고 싶은 새 모델일수록 자료가 없다는 값을 감수한 자리다.

### 12GB 가 되자 형식을 늘렸다

내려받으라고만 하기엔 무거워져서 아파치 아이스버그 테이블과 읽기 전용 S3 자격 증명을 함께
준다 — 트리노·스노플레이크·덕DB 로 곧바로 질의한다. 내려받기 값을 안 받는 회사가 자기 원가
자료도 공짜로 내보내는 셈이다.

### 다음 사이클 — 확장 (Backblaze 의 마지막 빈 도메인)

`내려받기 값을 무기로 쓴다`. 3배 무료·GB당 0.01달러·파트너 CDN 여덟 곳·오버드라이브 등급이
손에 있다.
