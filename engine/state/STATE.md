...
**Zepto** (IN · 커머스 — 퀵커머스) — **프로파일까지 썼다**(PROMPT 3단계). 도메인 3개 · 기능 0. 회사 **91개.**

### 이번 사이클 — 신규(6순위)

⚠️ **이 회사의 기술 문제는 전부 `10분` 이라는 한 줄에서 나온다.** 회사가 검색 글에서 그것을 정확히 적는다 — **전통적인 검색에서는 `상품이 있거나 없거나` 인데 이곳의 재고는 `주문이 들어올 때마다 계속 바뀐다`.** **그 한 줄이 표준적인 설계 가정을 무효로 만든다**(재구성).

**도메인 셋** — ① **재고가 계속 바뀌는 시장에서 검색한다**(✅ 본문 확인, 2026-06-11 + ⏳ 의미 검색 2026-02-09) · ② **10분을 지키려고 사람과 경로를 배치한다**(Route IQ 2026-08-24 · 인력 배치 2026-09, ⏳ 미독) · ③ **광고를 팔려면 하루 10억 이벤트를 세야 한다**(2026-07-27, ⏳ 미독).

**규모** — 다크스토어 **1,139개 · 66개 도시** · 연간 거래 사용자 **4,797만 명** · 2026년 3월 분기 **하루 평균 233만 건** · FY26 매출 **₹22,624 Cr(+103.6%)** · ⚠️ **손실 ₹5,905 Cr** · **광고 매출 1억 7,100만 달러(+151%)** — **가장 빨리 크는 것은 배달이 아니라 광고다.**

### 읽어 둔 것 — 검색 글에서 건진 것 넷 (다음 사이클의 재료)

- ⚠️ **결과 페이지 전체 캐싱을 버렸다** — **재고 상태·개인화·세션 신호·실험 배정**이 겹쳐 **한 요청당 유효한 변형이 너무 많다**(카디널리티).
- ⚠️ **검색 플랫폼은 사용자를 전혀 모른다** — **`search-platform 은 순수 검색 서비스다. 사용자에 대해 아무것도 모른다.`** 검색과 랭킹을 따로 진화시키려고 끊었다.
- ⚠️ **정적 분류기를 KNN 벡터 검색으로 바꿨다** — **새 상품이 들어오거나 분류가 개편돼도 의도 모델을 재학습할 필요가 없기 때문**이다.
- ⚠️ **순수 exploit 는 부익부를 만들어 신상품을 굶긴다** — 그래서 **exploit 와 explore 후보 흐름을 둘 다** 만든다. 그리고 **편집 거리 교정기는 인도 구어 발음에 안 통해** LLM 교정으로 갔는데 **없는 상품을 지어내지 않게** 막아야 했다.

### 다음 사이클 — 확장(2순위)

`--gaps` 가 **`재고가 계속 바뀌는 시장에서 검색한다`** 를 부를 것이다. **자료를 이미 읽었으니 바로 기능을 쓴다.**

⚠️ **큐가 1/3 로 떨어졌다**(Snyk 만 남았다). 이 회사를 다 판 뒤 **후보 조사**가 걸린다. ⏳ 되짚을 목록 — `bytes.swiggy.com`(⚠️ `r.jina.ai` 가 422, 브라우저 필요) · `www.gojek.io/blog`(429) · `tech.cygames.co.jp` · `www.scylladb.com/blog`.
...
⏳ **넷째 축이 열릴 수 있다 — `인도 규모에서 무엇이 달라지나`** (Meesho · Zepto). **둘 다 저사양 기기·구어 질의·주소 부재를 다루는데 푸는 방식이 다르다.**
