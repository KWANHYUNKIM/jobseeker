...
**아무 회사도 안 판다 — 후보 조사 사이클이었다.** 큐 0/3 → **3/3**. 회사 138개.

### 이번에 올린 셋

- **comma.ai**(US) — 자율주행. CTO 가 9년치 버그 여섯을 근본 원인까지 적은 회고(2026-07-21)가
  있다: 기기 온도에 따라 변하는 자이로 편향, 보닛 반사를 차선으로 오인, 헤드라이트 반사를
  추적한 시각 주행거리계, **학습과 추론의 이미지 패딩이 달라 몇 해에 걸쳐 반복된 회귀.**
- **PostHog**(US) — 이 엔진에 ClickHouse 는 **만드는 쪽**으로 있는데 **쓰는 쪽**이 없었다.
  ClickHouse 옆에 DuckDB 를 두고 Duckgres 를 만든 이유와 대가를 가른다(2026-05-01).
- **Val Town**(US) — 런타임을 vm → vm2 → 워커 → 프로세스로 네 번 갈아엎은 기록과 장애 사후
  기록들. 다만 심층 글이 2024년에 몰려 있어 그 사정을 큐에 적었다.

### 교훈 — 회고는 최신성 판정을 통과시킨다

comma.ai 의 나머지 글은 제품 공지인데 **회고 한 편이 9년치 대가를 담고 있었다.** 지금까지
"최신성만 보면 낚인다"와 "제목의 기술성도 근거가 아니다"를 배웠는데, 여기에 하나를 더한다 —
회사의 최신 글이 아니라 **그 회사가 쓴 회고**를 찾는 것이 이 엔진의 판정에 더 맞는다.

### 주소를 세 번째로 못 믿었다

comma.ai 의 글 주소가 제목과 다르고(`/driving-bugs/`) Increase 는 404 였다. 목록에서 href 를
받아 오는 절차가 이제 기본이다.

### 접은 곳 셋

**Algolia**(엔지니어링 블로그가 글을 하나도 안 보여 준다) · **Increase**(주소 404, 추측 안 함) ·
**Bluesky**(두 번째 확인인데 여전히 제품·조직 소식 위주).

### 다음 사이클 — 신규 (대기 맨 위 comma.ai)

프로파일부터 판다. 회고와 릴리스 글에서 기술 대목만 고른다.
