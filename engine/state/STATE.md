...
**없다 — DeepL 을 완주시켰다(도메인 3 · 기능 3 · `status: done`).** 회사 164개, **큐 1/3.**

### 잣대를 갈아치운 이야기가 세 번 반복된다

이 회사를 다 파고 나서야 보이는 것이다. 세 도메인이 전부 **무엇으로 재느냐**에서 갈렸다 —
문서 생김새를 픽셀(SSIM)로 재다 버렸고, 번역 품질을 자기 모델에게 채점시키지 않기로 했고,
FP8 로 갈지를 **학습 손실이 아니라 검증 perplexity 로** 판정했다. 만드는 법이 아니라 **재는
법**이 이 회사의 이야기다.

### 잣대를 바꿔 판정하는 것의 위험도 같이 적었다

FP8 은 학습 손실만 보면 BF16 이 나았다. 거기서 잣대를 바꿔 통과시켰는데, 이것은 **원하는
답이 나올 때까지 잣대를 바꿀 수 있다**는 뜻이기도 하다. 회사가 그 위험을 적지는 않아서
`thinking` 에 재구성으로 남겼다.

### 추론에서 아낀 것은 곧 매출이다

FP8 추론이 **같은 지연에서 처리량 2배**다. 학습에서 아낀 것은 회사 비용이지만 추론에서
아낀 것은 파는 것이 두 배가 된다는 뜻이다 — API 를 파는 회사에서 이 숫자가 FP8 결정의
진짜 근거일 것이다(재구성).

### 심은 대비

**파이어웍스**(커널을 손으로 짜서 같은 하드웨어에서 더 뽑는다) ↔ **DeepL**(수의 정밀도를
깎아 뽑는다) — 같은 NVIDIA 위에서 손대는 층이 다르다. **세레브라스**(칩을 통째로 키워
쪼개지 않는다) ↔ DeepL(남의 칩 72개를 묶고 수를 줄인다). **앤티시시스**(자기 시스템을
스스로 깬다) ↔ DeepL(자기가 자기를 보면 못 본다며 밖에 심판을 세운다).

### 다음 사이클

큐에 **Fastmail** 하나가 남았다(1/3). 신규로 그것을 파고 나면 후보 조사가 다시 뜬다.
