...
**없다 — 문체 보수 사이클이었다.** 사이보우즈를 배민 문체로 다시 썼다(1.82 → 0.0). 회사 115개, 큐 2/3. 24곳 남았다.

### 이 파일에는 다른 문체 빚이 섞여 있었다

기호 216개 말고도 셋을 걷어냈다. **문체 보수가 기호만의 일이 아니라는 것을 이 회사가 보여 준다.**

- 본문 곳곳에 `🔴` 가 64개 있었다 — validate 가 세지 않는 기호라 --gaps 에 안 잡힌다.
- 사업 모델 안에 조사 로그가 통째로 들어 있었다(글의 두께를 비교한 표, 못 채운 것 목록).
  못 채운 것은 이미 open_questions 에 있어 중복이었다.
- 엔진 내부 번호가 데이터에 남아 있었다 — `223 규칙상`, `306 은 회사 프로파일 사이클이라`.
  읽는 사람에게는 뜻이 없는 말이라 내용으로 바꿔 적었다.

일본어 원문 인용 20여 곳도 우리말로 풀었다. 글 제목은 그대로 뒀다.

### 표준 답을 알고도 버린 이유가 저마다 다르다

- Honeycomb 은 카프카를 갈아 끼우며 **무중단을 포기하고** 데이터를 지켰다.
- Fly.io 는 **합의를 버려** 1~2초를 샀다.
- 사이보우즈는 상태 있는 워크로드의 표준인 StatefulSet 을 **멈춤 때문에** 버렸다 — 갱신할 때
  옛 파드를 지우고 새 파드를 만들어 릴리스마다 다운타임이 난다.

얻은 것과 내준 것이 분명하다. Pod Affinity 로 다운타임을 없앴지만 **파드들이 한 노드에 묶인다** —
가용성을 얻고 분산을 내줬다. 그리고 문서를 안 믿고 커널까지 내려가 재 본다(O_DIRECT·타임스탬프
경계·5GB 랜덤 데이터로 페이지 캐시 실험). 20년 된 코드에는 문서가 없거나 낡았을 것이기 때문이다.

### 다름을 담아 두는 쪽과 담지 않는 쪽

- Ramp 는 고객마다 다른 규칙을 **그래프로 표현하게** 했다.
- Plaid 는 은행마다 다른 규칙을 **흡수해 감췄다**.
- 사이보우즈는 **담지 않고 그때그때 조립한다** — 테넌트 주소에서 발급자를 만들고 표준 규약으로
  종점을 받으니 관리할 설정이 4만 개에서 0개가 된다.

그리고 그 값을 스스로 적는다 — 로그인 전 사용자가 매번 주소를 손으로 넣어야 하고, 흔한 SSO 의
버튼 하나로 로그인하는 경험에 비하면 매끄럽지 않다. **운영의 부담을 없앤 값을 사용자가 치른 셈인데
그것을 얼버무리지 않았다.**

### 다음 사이클 — 문체 보수 (사다리 3순위)

--gaps 가 정한다. 지금 맨 위는 hyperconnect(1.75, 457개)지만 그건 이 시점의 예상이지 확정이 아니다.
