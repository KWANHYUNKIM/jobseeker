...
**freee(JP · SaaS) `in_progress` — 도메인 1개 · 기능 1개.** 339 에서
**`refactor-before-you-cut`(떼어내기 전에 옛것을 정리했어야 했다)** 를 썼다.
**결정 8개(최대) · 그림 3장 · 생각 4개 · 난제 2건.** 재료는 인증·인가 기반 마이크로서비스화
회고(2022-08-31).

**⑫ 가 일곱 번째로 값했다 — 재독이 새 재료를 다섯 냈다.**
- **⭐ 이중 읽기의 기본이 반대였다** — *"基本は新基盤の結果を返しますが、差分があればエラー検知
  SaaSであるbugsnagに通知して旧基盤の結果を返します"*. **평소엔 새 답을 주고 어긋날 때만 옛
  답을 준다.**
- **⭐⭐ 회사가 되먹임 고리를 명시한다** — *"リリース回数が少ないために振り返りの頻度も多くなく、
  改善施策が出しづらい状況になった"*. **크게 잡으면 릴리스가 적고, 릴리스가 적으면 회고가
  적고, 회고가 적으면 개선이 안 나온다.** 그 끝이 정신의 마모다.
- **원인이 하나 더 있었다** — *"ユーザ・事業所のデータモデルが少し難しく、権限の付与や所属管理の
  機能を新基盤で模倣することが困難"*.
- **구체적 교훈** — *"対象となるテーブル数を最初は少なくする、移行するにも短期的なゴールを持つ
  ことが重要"*.
- **결말** — *"freeeは新認証認可基盤をベースとして動いており、特に大きな影響もなく稼働している状況"*.
  그리고 **옛 구조가 돌던 기간이 5년**이었다.

**⭐ 기능의 축** — **떼어내는 일이 아니라 순서의 이야기다.** 교훈 둘이 다 순서에 대한 것이고
핵심은 **옛 로직을 먼저 정리했어야 한다**는 것이다 — *"古いロジックは意図しない挙動を前提と
しているケースが多々あり、切り出した先で模倣するのが大変困難"*.

**⭐⭐ 값이 가장 큰 두 자리**
- **옛 시스템의 관대함이 사양의 일부가 돼 있었다**(이 표현은 이 사이트의 것) — 개발 초기에
  검증이 느슨해 들어온 데이터를 **옛 기반은 흘려보내는 처리를 품고 있었고 새 기반은 못 그런다.**
  **새 기반이 옳게 동작하는 것이 곧 회귀로 보인다.**
- **나눌 수 있는 것과 없는 것이 갈린다** — **읽기는 제품마다 플래그로 켜는데 쓰기는 한 번에
  뒤집어야** 한다(Auto Increment ID 발번이 한 곳이어야 해서).

**⚠️ 도메인을 글 한 편으로 썼고 `why` 에 밝혔다** — 두 번째 후보(`権限管理基盤` · `認証基盤が
入れ替わってる`)는 아직 본문을 안 읽었다.

**⭐⭐ SmartHR 과의 정반대 순서 대비를 `open_questions` 에 남겼다** — **freee 가 후회로 적은
것**(*"旧機能のリファクタはマイクロサービス化の前に着手すべし"*)**이 SmartHR 이 실제로 한
일이다**(모듈러 모놀리스로 시작 → Packwerk 로 경계를 보이게 → HTTP 로 인터페이스를 미리
기름 → 떼어냄. CI 30분 이상→약 1분 · 조회 3.59~4.46배 · 행 크기 약 71% 감소).
**SmartHR 은 큐에서 대기 중이라 둘을 다 판 뒤 비교 문서 재료가 된다.**

---

**다음 사이클 — 열린 도메인이 찼다. 두 번째 도메인을 찾거나 두 편 규칙을 채운다.**
- **⭐ 1순위 — 같은 도메인의 두 번째 글을 연다.** `freee の権限管理基盤マイクロサービスの今を
  語ろう！`(권한 관리 기반) 또는 `これってもしかして……認証基盤が入れ替わってる〜？`(인증 기반
  교체). **⑰ 주소는 ⑱ `WebSearch` + `allowed_domains: ["developers.freee.co.jp"]` 로 확인한다.**
  **223 규칙으로 같은 문제인지 실물로 정한다** — 같으면 **두 번째 기능이자 두 편 규칙 충족**,
  다르면 새 도메인.
- **2순위 — 다른 축.** `確定申告を乗り越えるDBパフォーマンス改善 - Aurora 移行の舞台裏`
  (**확정신고라는 연 1회의 부하 정점**이 있는 것으로 보인다, 추정) ·
  `Langfuse のバージョンアップに伴い ClickHouse を社内初導入した話`(PostgreSQL 은 트랜잭션,
  ClickHouse 는 트레이스 집계) · `【連載 第4回】freeeカード Unlimited でのClean Architecture実践`.
- **⚠️ QA 분류가 95편으로 큰데 한 편도 안 읽었다** — `ソフトウェアアーキテクチャに基づいた
  自動テスト戦略と実装ガイドライン` · `テストアーキテクチャの実践`. **테스트를 주제로 한
  도메인이 설 수도 있다**(추정).
- **⭐ AI 계열 글들**(2026년 8월에 몰려 있다)**이 비교 문서 `에이전트에게 무엇을 못 하게
  하는가` 와 이어질 수 있다** — 본문을 안 읽었으므로 아직 안 적었다.
