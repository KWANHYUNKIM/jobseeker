...
**Turso** — 기능 **1/2.** 회사 115개. 큐 2/3. ⚠️ **남은 도메인 하나**(`동기 인터페이스를 비동기로 연다`)를 채우면 완주다.

### 이번 사이클 — `시드를 주면 같은 버그가 다시 난다`

⭐⭐ **결정 여덟 개가 하나의 물음에서 갈라져 나온다** — **`고쳤다고 말할 근거를 어떻게 만드나`**(재구성).

- **테스트를 물려받을 수 없어서 코드를 다시 썼다** — **`SQLite 의 테스트 스위트가 독점이고, C 구현이라 자신 있게 진화시키기 어려웠다.`** ⭐ **다시 쓰기로 한 것이 코드가 아니라 검증 능력이었다.**
- **질의만이 아니라 환경까지 흔든다** — 워크로드 분포, **페이지 크기**, **리더와 라이터의 수.** ⚠️ **그러면 원인 공간이 넓어져 시드 재현이 필수가 된다**(재구성).
- ⭐ **무작위인데 조건이 있다** — **`빈 데이터베이스에서 시작해 워크로드 분포에 따라 무작위 상호작용을 더하되 속성의 불변식을 지킨다.`** **`삽입-선택` 속성이 성립하려면 중간에 오류가 없고, 그 행을 지우거나 갱신하지 않고, 테이블을 드롭·변경하지 않아야** 한다. ⚠️ **완전한 무작위는 아무것도 검증하지 못한다**(재구성).

### ⭐⭐⭐ 이번에 건진 가장 큰 것 — 결정론 자체를 검사한다

**`--doublecheck 는 계획을 두 번 돌려 출력이 같은지 확인한다.`**

⭐ **재현을 설계로 만들면 그 설계 자체가 검증 대상이 된다**(재구성) — **결정론이 깨지면 시드도 버그 데이터베이스도 차분 테스트도 다 무의미해진다.** ⚠️ **대신 모든 계획을 두 번 도는 값이 든다.**

### ⚠️ 정답지가 SQLite 라는 것의 두 얼굴

**`같은 상호작용 계획을 림보와 SQLite 양쪽에서 돌리고 결과를 비교하며 어느 쪽에서든 패닉이나 오류가 나는지 확인한다.`** ⭐ **호환이 목표이므로 원본이 곧 정답이다.**

⚠️ **그런데 대가가 둘이다** — **SQLite 가 틀린 자리는 못 잡고**, **SQLite 에 없는 기능은 대조할 대상이 없다**(재구성). ⭐ **원본을 넘으려고 다시 썼는데 검증은 원본에 기댄다.**

### ⭐ 그리고 시뮬레이터 밖을 인정한다

**시뮬레이터는 자기가 만든 세계 안에서만 참이다**(재구성). 그래서 **Antithesis 가 `결정론적 하이퍼바이저에서 코드를 돌리며 결함을 주입하고 시스템 속성을 계속 검증`** 해 **`시뮬레이터가 놓칠 수 있는 버그까지`** 잡는다. ⚠️ **바깥 회사에 기대는 자리다.**

### 다음 사이클 — 확장, 그리고 완주 (2순위)

**Turso 기능 1/2.** ⏳ 마지막 — **`동기 인터페이스를 비동기로 연다`**(`sqlite3_step` 이 제어를 안 돌려주는 문제 · `io_uring` · 파일 형식 호환 유지 · 620ns→506ns). ⚠️ **완주 판정 때 수익원 둘**(관리형 DB · 오픈소스)**이 기능과 이어지는지 본다.**

⏳ **못 읽은 것** — SQLite 암호화 연작 3편 · `Turso v0.7.0` 릴리스 노트 · **`데이터베이스의 LLVM`의 뜻**(글이 404) · **축소(shrinking)를 하는지**(길이 1000~5000짜리 계획을 그대로 들고 디버깅하는지).
