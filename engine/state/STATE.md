...
**Deliveroo** (UK · 커머스) — 도메인 3개 · **기능 2개.** 남은 도메인은 **`결제를 승인하면서 사기를 막는다`** 하나다(⏳ 본문 미독).

### 이번 사이클 — `실험을 안 하기로 정하는 것도 실험 문화다`

건진 것 넷:

- ⚠️ **이 글의 본체는 도구가 아니라 문화다** — 표준 플랫폼으로 옮긴 뒤에도 **`성공적인 실험은 기술과 도구만으로는 안 된다`** 며 다섯 원칙을 성문화했다. **매년 수백 건**을 돌리고 **A/B 외에 interleaving·multi-armed bandit·switchback** 을 쓴다.
- ⚠️ **가장 값진 대목은 실험을 건너뛴 사례를 적은 것이다** — `favourites` 를 **실험 없이 전량 출시**했고 이유 넷을 적는다(확신이 높았다 · **일관성 없는 UX 는 비싸서 도로 걷는 것이 고통스럽다** · 다음 실험의 전제라 빨리 채택시키는 값이 있었다 · **채택할 시간을 먼저 줘야 해서 실험이 아주 오래 걸렸을 것이다**). **성숙도는 실험을 많이 하는 것이 아니라 안 할 때를 판단할 수 있는가로 드러난다**(재구성).
- ⚠️ **원칙 1과 원칙 5가 서로를 견제하도록 짝지어져 있다** — 5번(실험은 안내하지 지배하지 않는다)은 **`결과가 마음에 안 들면 맥락 탓` 이라는 문**을 열 수 있는데, 1번(**성공 기준을 결과 보기 전에 못 박는다**)이 그 문을 막는다. **회사가 그 유혹을 이름 붙여 적는다** — `It may be tempting to decide on the roll out decision rule once results are observed`.
- ⚠️ **넓히려면 먼저 규격을 세워야 한다** — 콘텐츠 팀이 **코딩 없이** 실험하게 하려고 실험 백엔드를 콘텐츠 도구와 통합했고, 표현도 하나로 고정했다(**여러 방식은 오해 위험을 키운다**). **자유롭게 하려고 규격을 세운다.**

### ⚠️ Etsy 와 견주면 빈 자리가 보인다 (비교 문서 재료 셋째 축)

**Etsy 는 분기마다 약 8% 를 홀드아웃으로 빼 `부분의 합이 전체가 아니다` 를 실제로 재고, 자기 실험 도구가 효과를 체계적으로 과대평가한다(승자의 저주)고 공개한다.** **딜리버루의 다섯 원칙은 개별 실험의 규율에 집중하고 누적·상호작용 효과를 어떻게 다루는지는 말하지 않는다.** ⚠️ **`실험을 얼마나 믿나` 라는 축이 두 회사로 열렸다** — 한쪽은 도구의 편향을 재고, 한쪽은 사람의 규율을 성문화한다. open_questions 에 적어 뒀다.

### 다음 사이클 — 마지막 도메인, 그리고 완주 판정

`--gaps` 가 **확장**을 다시 부른다(`결제를 승인하면서 사기를 막는다`). ⏳ `https://deliveroo.engineering/2023/05/17/3ds-optimisation.html` — **`r.jina.ai` 를 앞에 두고 브라우저 `get_page_text` 로 받는다.** 자료가 서면 **같은 사이클에서 완주 판정까지** 한다(도메인 3/3 · 고아 없음 · 수익원 셋 다 연결). ⏳ 보강 재료로 `deliveroo-ml-platform`(2025-07-02)도 남아 있다.
...
**① `AI 에이전트를 어디까지 믿나` 6곳** — Sentry / ClickHouse(**세 겹 중 한 겹만 기계가 강제**) / Duolingo(사람 리뷰 → 병목) / Ramp(사람의 신원) / DoorDash(primitives) / **Deliveroo(강제 지점을 배포 파이프라인에 — `검사 가능한 것만 강제된다`)**. ⚠️ **ClickHouse 와 Deliveroo 가 같은 발견의 두 판본이다.**

**② `관리형 MySQL 의 한계` 3곳** — Etsy(Vitess) / Plaid(TiDB) / Paystack(MySQL 8.0 제자리). ⚠️ **Plaid 와 Paystack 은 같은 마감에 정반대 답.**

**③ ⭐ 새로 열렸다 — `실험 결과를 얼마나 믿나` 2곳** — **Etsy**(홀드아웃 8%·승자의 저주 공개·**부분의 합이 전체가 아니다**) / **Deliveroo**(다섯 원칙·**안 하기로 정하는 법**·불확실성 표현 고정). **도구로 재는 쪽과 규율로 세우는 쪽이다.**
