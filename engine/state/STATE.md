...
**DeNA**(JP · 게임·헬스케어·모빌리티) — 회사 프로파일까지 썼다. **도메인 둘 · 기능 0.** 큐 0/3.

### 기술 축의 뿌리가 블로그 첫해에 남아 있다

기술 블로그가 **2010년부터 1,857개 URL** 인데 **그 첫해 글이 `handlersocket-plugin-for-mysql`
과 `mysql-for-socialgame`** 이다. **소셜 게임 트래픽을 MySQL 로 받아 내던 시절의 기록**이
그대로 있고, 지금 도메인 둘이 다 그 자산에서 나온다 — **옮기는 일**과 **터진 것을 파는 일.**

### 도구가 10년째 못 따라온 자리

MHA 버그의 원인이 **2015년 MySQL 5.7.9 의 변경**이었다. mysqlbinlog 가 **relaylog 의
슬레이브 메타데이터를 읽어도 아무것도 하지 않게** 바뀌었는데 **MHA 는 첫 FDE 만 쓰는 옛
구현 그대로**였다. **얹혀 있는 도구의 전제가 조용히 깨진** 자리이고, 그것이 **게임 이벤트
중 페일오버**에서 드러났다.

### 어디까지 파는가

운영 데이터 불일치 → **MHA 소스** → **MySQL 소스와 binlog 규격** → **hexdump 로 바이너리** →
**5.7.8 과 5.7.25 동작 비교.** 그리고 빈도도 정직하게 적는다 — **'우리 환경에서도 이 한 번밖에
일어나지 않았다.'**

### 요금을 양쪽 다 적는 회사

이관에서 **월 $721 → $1,017(약 1.4배)** 를 내고, **문서값과 실측을 나란히** 놓는다
(페일오버 문서 60초 / 실측 약 40초). **저장 공간은 오히려 줄였다**(1500 → 1150 GiB).

### 같은 회사 안에서 글마다 두께가 또 갈린다

`game-development-sre-first-year` 는 **50% 규칙**(운영 절반·개선 절반)과 관점 전환을 적지만
**성과 지표가 없다.** 뱅크샐러드·카카오페이에 이어 **세 번째로 같은 관찰**을 하게 됐다.

### 다음 사이클

확장 — 빈 도메인 둘 중 하나. **MHA 쪽이 추적의 깊이로 두껍고**(다만 대응이 후속 글로
미뤄져 있다), 이관 쪽은 **요금·실측 수치**가 촘촘하다. 큐가 0/3 이라 그 뒤에 후보 조사가 뜬다.
