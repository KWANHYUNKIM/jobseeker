...
**Depot 완주(기능 3개).** 회사 130개, 큐 2/3 — 다음은 Ubicloud 또는 Polar Signals.

### 이번 기능 — `per-second-billing` (파는 단위를 초로 잡는다)

파는 것이 시간인데 제품이 하는 일은 그 시간을 줄이는 것이다. 이 회사는 그 긴장을 **단위를 바꿔서**
푼다 — 상대가 4분 6초를 5분으로 올려 받는 자리에서 초·vCPU 로 재면(0.00005달러) 그 차액이
그대로 옮겨 올 이유가 된다. 한 빌드에 0.012~0.014달러 대 0.04달러.

**자기 매출을 깎는 숫자를 자기 손으로 계산해 내건다** — 월 8,680달러가 4,200달러가 된다고.
그 숫자가 없으면 옮겨 올 이유도 없기 때문인데, 쓰는 양이 늘지 않으면 절감액이 그대로 손실이다.

### 과금 단위가 인프라 요구를 정했다

초로 받기로 한 이상 기계가 노는 시간은 파는 쪽이 문다. **마이크로VM 을 1초 안에 띄우는 일이
제품 기능이 아니라 원가 구조의 문제**가 되는 자리 — 요금표가 LVM 을 다시 만들게 한 셈이다.

속도의 재료도 값이다. 캐시 100GB·초당 1GB 로 빠르게 하고 그 저장을 GB·월 0.20달러로 따로 받는다.

### 벤치마크에 스스로 선을 그은 회사

피보나치 20번째 수를 러너 18개에 나눠 구하는 장난으로 콜드 스타트를 잰다. 평균은 123.8초 대
142.2초로 크게 안 갈리는데 **폭이 13초 대 148초**로 갈린다. 그러고는 진지한 벤치마크가 아니고
CI 전체 효율이 아니라 시작 성능을 잰다고 스스로 적는다 — 앞의 회사들과 견주면 드문 태도다.

### 다음 사이클 — 큐가 2/3 이므로 후보 조사
