...
**없다 — 문체 보수 사이클이었다.** 프레시웍스를 배민 문체로 다시 썼다(1.0 → 0.0). 회사 115개, 큐 2/3. 9곳 남았다.

### 핵심이 아닌 것이 핵심을 흔들지 못하게 하는 넷

- Moniepoint — 일시적 실패가 서비스를 로테이션에서 튕기지 않게 헬스체크를 직접 만든다.
- Monzo — 상태를 한 클러스터에 모아 둔 값을 장애로 치른다.
- 메르카도리브레 — 셀로 감싼다.
- 프레시웍스 — **앞에서 나누고 뒤에서 자른다**(요청을 갈라 두고 느린 쪽의 회로를 끊는다).

고른 장치의 한계를 그대로 적는 것이 이 기능의 값이다 — 일부 테이블만 실패시키도록은 설정할 수
없고, 프로세스 단위로 돌며, 분산 계측은 아직 개발 중이다. **끊는 장치를 들이면 애플리케이션과
ORM 이 롤백과 연결 실패에 대해 시험돼 있어야 한다**는 조건도 함께 적는다.

### 도구를 바꾸는 쪽으로 안 간 판단

큐 200개가 층에 무작위로 흩어져 어떤 층은 사용률 10% 미만, 어떤 층은 60~70% 였다. 그런데 회사가
먼저 적는 문장이 **새 큐 시스템으로 옮긴다고 이 문제가 풀리지는 않는다**다 — 문제가 큐가 아니라
배치에 있다는 판단이다. 그리고 제품 팀이 쓰던 방식을 안 바꾸게 하려고 **밀기 직전에 이름을 갈아
끼우는** 길을 골랐다.

### 두 판을 함께 굴린 넷 — 굴리는 자리가 다르다

- freee — 신·구 인증 기반을 **둘 다 읽어** 차이를 오류로 잡는다.
- 산산 — 옛 판과 새 판을 **같은 화면 안에** 공존시킨다.
- 크라켄 — DB 마이그레이션을 **앞뒤 모두 호환**으로 만든다.
- 프레시웍스 — **한 코드베이스가 Rails 두 버전을 동시에 알게** 하고 환경변수로 정한다.

풀 리퀘스트마다 두 버전에서 테스트를 돌려 폴백이 늘 안정되게 해 놓고도 회사가 남기는 문장이
태도를 말해 준다 — **시험해 둔 폴백이 있어도 무엇이 깨질지는 결코 알 수 없다.**

### 도메인을 제품이 아니라 한 덩어리가 어디서 터지느냐로 잘랐다

기술 블로그가 사업 이야기를 안 해서, 이 문서의 도메인은 파는 것이 아니라 **한 덩어리 Rails 앱 위에
여러 고객을 얹은 값**을 따라간다는 것을 사업 모델에 적었다.

### 다음 사이클 — 문체 보수 (사다리 3순위)

--gaps 가 정한다. 지금 맨 위는 grab(0.96)이지만 그건 이 시점의 예상이지 확정이 아니다.
