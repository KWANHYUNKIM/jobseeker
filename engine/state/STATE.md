...
**Fastmail** — 도메인 2개 중 **1개에 기능이 들어갔다**(`backup-that-outlives-a-bad-command`).
남은 빈 도메인 하나: '메일함을 나라 경계 위에 올려놓는다'. 큐 0/3.

### 명령은 모든 사본에 똑같이 도착한다

이번 기능의 축이다. 복제본을 늘리는 것은 **같은 종류의 위험만** 줄인다 — 하드웨어는 서로
독립적으로 죽지만 **명령은 세 벌 모두에 똑같이 간다.** 그래서 사본을 늘려도 이 사고는
막히지 않았고, 살린 것은 별개의 불변 백업이었다.

### 안전장치를 비껴간 경로

새로 얻은 알맹이다. 정기 롤링 복제는 **즉시 지우는 법이 없다 — 일주일쯤 뒤 정리하도록
표시만 한다.** 그런데 이동 시스템은 그 경로를 타지 않았다. **평소에 안전하다는 경험이
쌓일수록 예외 경로의 위험은 눈에 안 띈다.**

### 자기 수정이 틀렸다고 적는다

사고를 적는 글은 많지만 **첫 수정(PR 5961)이 모자랐다고 적는 글은 드물다.** PR 5988 이
실제로 일어난 패턴을 그대로 재현하는 테스트와 함께 닫았다. 이것이 이 회사를 통과시킨
이유다.

### 근본 원인은 미뤄 뒀고, 그것을 적었다

사용자명이 메일 주소이고 복제가 `mboxname` 을 쓴다 — 이름으로 사람을 가리키면 이름이
재사용될 때 사람이 뒤바뀐다. 받은편지함 uniqueid 기준으로 옮기는 것을 앞으로 볼 일로 두되
**"몇 년은 걸릴 일"** 이라고 적는다. 포기가 아니라 **일정의 정직함**이다.

### 심은 대비

**백블레이즈**(드라이브 고장 통계 — 하드웨어가 죽는 것) ↔ **Fastmail**(하드웨어는 멀쩡한데
우리가 지운 것). 복제로 막는 것과 백업으로 막는 것의 경계가 여기서 갈린다. **메타베이스**는
원인을 네 겹, 여기는 세 겹으로 나눠 적는다 — 둘 다 **자기들이 한 변경**을 원인으로 지목한다.

### 사후 기록이 말하지 않는 것 일곱 가지

**시간선이 통째로 없다** — 언제 알아챘는지, 누가 발견했는지, 복구에 얼마나 걸렸는지.
그 밖에 영향받은 사람에게 알렸는지 · 5통을 건진 도구가 무엇인지 · 재패킹이 무엇인지 ·
보안 감사를 했는지. `failure` 도해에 세웠다.

### 다음 사이클

남은 도메인 하나 — '메일함을 나라 경계 위에 올려놓는다'. 그것으로 Fastmail 은 완주이고,
그 다음은 **후보 조사**(큐 0/3).
