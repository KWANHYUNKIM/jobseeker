...
**Supabase**(Postgres 플랫폼) — 도메인 2개 중 **1개에 기능이 들어갔다**
(`postgres-os-without-forking`). 남은 빈 도메인 하나: '같은 데이터를 보내는 두 길을 갈라
놓는다'. 큐 0/3.

### 어려운 문제를 떠안았는데 그 부분이 가장 얇다

이번 기능에서 가장 눈에 띄는 것이다. Postgres 를 포크하지 않기로 한 대가로 **바깥에서
엄격한 일관성을 만드는** 어려운 문제를 떠안았다 — 커밋이 어디까지 갔는지를 엔진 밖에서
알아야 하고, 스플릿 브레인에서도 이미 성공한 커밋을 잃지 않아야 한다. 그런데 **정작 그
부분이 글에서 가장 얇다**: '일반화 합의'라는 **이름만 대고** 알고리즘도 정족수 규칙도
스플릿 브레인 해법도 없다.

### 오류 대신 지연을 시스템이 흡수한다

장애 전환 중에 게이트웨이가 **요청을 붙든다.** 오류를 돌려주는 것이 정직하지만 그러면
**모든 애플리케이션이 재시도 로직을 따로 짜야** 한다. 다만 **얼마나 붙드는지, 장애 전환에
얼마나 걸리는지가 없어** 쓰는 쪽은 타임아웃을 얼마로 둘지 모른다.

### 미룬 것을 앞에 적는다

**"샤딩(장차 대표 기능)은 이 릴리스에 없다"** 를 앞에 두고, 프로덕션 미준비 · CR API 불안정 ·
하위 호환 미보장 · **성능 벤치마크까지 미뤘다**고 밝힌다. 초기 수치는 오래 따라다니니
신중한 것일 수도, 자신이 없는 것일 수도 있다 — 이 글로는 알 수 없다.

### 심은 대비

**플래닛스케일**(MySQL 위의 Vitess 를 상품으로 만든 쪽) ↔ **Multigres**(그 **Vitess 를
만든 사람**이 Postgres 판을 짓는다) — 회사가 사명을 'Vitess 급'이라 적어 연결이 확정이다.
**코크로치**(처음부터 분산으로 설계하고 PostgreSQL 호환을 위에 얹는다) ↔ Supabase(단일 노드
Postgres 를 그대로 두고 분산을 바깥에 얹는다) — **같은 목적지에 반대 방향으로 간다.**

### 다음 사이클

남은 도메인 하나 — Realtime 과 Pipelines. 자료가 두껍다(구조 표 · 100ms 미만 대 초 단위 ·
구독자 100명이면 인가 질의 100개 · Replay 는 3일에 25개 · **"Realtime 은 이 일을 위해
만들어지지 않았다"**). 그것으로 Supabase 는 완주다.
