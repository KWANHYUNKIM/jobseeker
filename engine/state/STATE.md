...
**없다 — Qdrant 를 완주했다(도메인 2 · 기능 2).** 큐 1/3 이라 다음 사이클은 **후보 조사**다.

### 앞 사이클의 판단을 좁힌다

앞에서 **'시간은 촘촘히 재고 공간은 안 잰다'** 고 적었는데, 양자화 글에는 **압축비(8~32배)와
벡터당 4바이트**가 있다. 없는 것은 **절대 메모리(GB)** 다. **'공간을 안 잰다'가 아니라
'비율은 재고 절대량은 안 잰다'** 가 맞다. 리스테이트에서 **'정량이 없다'를 좁힌 것과 같은
수정**이 두 번째로 나왔다 — **회사 단위로 뭉뚱그리면 반드시 틀린다.**

### 전처리 하나로 운영의 짐 여럿이 사라진다

무작위 직교 회전으로 **펴 놓고 자르면 자르는 규칙이 하나로 족하다.** 그래서
**'데이터셋별 학습도, 보정 집합도, 저장해 둘 코드북도 없다.'** 압축 방식을 고른 값이
정확도가 아니라 **운영에서 나온 자리**다.

### 공짜라고 뭉뚱그리지 않는다

덧댄 것마다 값을 나눠 적는다 — 길이 재정규화 **벡터당 4바이트**(기존 L2 노름 재사용),
좌표별 보정은 **검색 때 공짜**(비대칭 채점), 보정 스캔은 **세그먼트당 몇 초**.

### 직접 지을지 얹을지가 칸마다 다르다

엔진은 **밑바닥부터** 지었다면서(FAISS 로는 안 됐다) **양자화는 구글 리서치의 것을 가져다**
쓴다. 리스테이트가 **로그는 직접 짓고 버전 고정은 람다에 얹은 것**과 같은 모양이다.

### 못 하는 것을 성질에서 끌어내 적는다

**L1 거리 미지원** — 회전이 **L1 노름을 보존하지 않아** 점수마다 벡터를 통째로 복원해야
하고 그러면 속도 이점이 사라진다. **한계를 성질에서 끌어내면 나중에 뒤집힐 자리도
분명해진다.**

### 다음 사이클

**후보 조사**(큐 1/3). 남은 Qdrant 글 중 **`benchmark-elastic-diskbbq`(엘라스틱과 직접
견준다)** 는 나중에 비교 문서 사다리에서 쓸 수 있다.
