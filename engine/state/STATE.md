...
**없다 — 문체 보수 사이클이었다.** WarpStream 을 배민 문체로 다시 썼다(2.6 → 0.0). 회사 115개, 큐 2/3. 59곳 남았다.

### 객체 저장소를 진실 원천으로 삼는 넷이 서로 다른 이유로 갔다

WarpStream 이 이 축의 원형인데, 다시 읽으니 넷이 같은 자리에 서 있으면서 이유가 다 다르다.

- WarpStream 은 가용영역 사이의 네트워크 비용을 없애려고 갔다. S3 를 저장 계층이 아니라
  네트워킹 계층으로 쓴다.
- Neon 은 역사를 남기려고 갔다. 로그가 진실이면 브랜치가 포인터 하나가 된다.
- Cursor 는 합의를 없애려고 갔다. 복제본을 더할수록 푸시가 느려지던 것을 끊었다.
- Databricks 는 정족수 합의를 쓰기 경로에 남긴 채로 갔다. 커밋이 세이프키퍼에서 끝난다.

같은 도구를 쓰는데 푸는 문제가 다르면 설계도 갈린다. 넷 중 셋만 합의를 버렸고 하나는 남겼다.

### 관측 비용에 세 회사가 다른 자리에 선다

VictoriaMetrics 는 카디널리티가 비용의 정체라며 그것을 파는 쪽이고, Databricks 는 그것에
깔려 시계열 DB 를 포크한 쪽이며, WarpStream 은 자기 제품이 싼 것이 존재 이유라서 관측
비용을 고객에게 넘길 수 없는 쪽이다. 그래서 새 인프라를 더하지 않고 이미 있는 저장 계층을
쓰는 쪽을 골랐고, 벤치마크에서 이길 생각을 접었다.

### 이번 사이클 — WarpStream

사업 모델에서 줄이는 대상이 특이하다는 것을 앞세웠다. 저장 비용이 아니라 가용영역 사이의
네트워크 비용이다.

기능 이름에 핵심을 붙였다. `순서는 읽히기 전에만 정해지면 된다 — 필요한지가 아니라 언제
필요한지`, `도착지의 느림을 미리 겪게 한다 — 옮겨 오게 만드는 법`,
`벤치마크에서 이길 생각이 없다 — 관측을 싸게 만드는 쪽`.
