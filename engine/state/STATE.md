...
**Convex** — 첫 기능 `read-set-is-the-blast-radius`(읽은 것이 곧 다시 도는 범위다)를 끝냈다.
**도메인 2 · 기능 1.** 큐 0/3. 남은 도메인은 **싼 척하는 원시 연산은 주지 않는다.**

### 반응성은 공짜가 아니라 미리 낸 것이다

트랜잭션이 어차피 쌓는 **읽기 집합**을 구독에 두 번 쓰는 영리함인데, 그래서 **읽기 집합의
넓이가 그대로 청구서**가 된다. **'그 질의가 참조한 문서들에 어떤 변화가 생겨도 결과가
자동으로 갱신된다'** — 넓게 읽으면 상관없는 쓰기에도 깨어난다.

### 같은 회사가 두 잣대를 쓴다

`select`·`count()` 는 **손에서 뺏었는데**, 넓은 읽기 집합은 **막지 않고 문서로 안내한다**
(인덱스·페이지 나누기·문서 가르기). 앞엣것은 **틀린 줄 모르고 쓰기 쉬운 것**이고 뒤엣것은
**필요해서 넓어질 수도 있는 것**이라 그런 듯하다. **어디를 잠그고 어디를 열지가 회사마다
다르다** — 엘라스틱과 견줄 자리다.

### 비용은 숫자로 대고 이득은 안 댄다

**'인덱스가 16개 있는 문서를 하나 넣는 것은 문서 17개를 넣는 것에 맞먹는다'**, 문서를
가르면 **질의마다 읽기가 하나 더**, 하트비트 10초에 사용자 10명이면 **초당 1회**·100명이면
**0.1초마다.** 그런데 **인덱싱이 질의 지연을 얼마나 줄이는지는 없다.** 리스테이트와
**반대 방향의 편향**이다(거긴 남의 비용을 셌다).

### 파는 문장과 이 글 사이의 거리

**'복잡한 백엔드 엔지니어링 없이'** 가 파는 문장인데, 인덱스를 걸고 페이지를 나누고 문서를
가르는 일은 전부 응용 쪽 일이다. **쉬움은 트랜잭션과 반응성까지고, 규모는 다시 사용자에게
온다.**

### 다음 사이클

확장 — 남은 도메인(안 주는 것). 이미 `select`·`count()` 글이 근거로 서 있고,
`why-convex-doesnt-need-row-level-security` 와 **`how-hard-is-it-to-migrate-away-from-convex`**
(자기 제품에서 떠나는 비용을 자기가 적는 드문 글)가 남았다.
