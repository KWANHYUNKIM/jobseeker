...
**Materialize — 도메인 둘을 다 채우고 done 으로 닫았다.** `tag-byte-dictionary` 와 `ivm-replica`. 회사 116개, 큐 1/3.

완주 기준 셋을 다 넘겼다 — 도메인마다 기능 1개 이상, connections 에 고아 없음, 수익원 둘이 다 기능과 이어진다.

### 늘려도 안 되는 이유를 성능이 아니라 구조로 적는다

무거운 읽기에 흔한 답은 읽기 복제본을 늘리는 것인데, 회사가 그것이 왜 답이 아닌지를 구조로 적는다 —
**복제본은 원본과 같은 구조와 같은 데이터 배치를 그대로 물려받는다.** 그러니 복잡한 읽기는 거기서도
느리고, 늘려도 뿌리가 그대로여서 원가만 치솟는다.

이 엔진에서 원본 데이터베이스를 지키려던 회사들과 자리가 갈린다.

- SmartHR — 쪼개지 않고 **앞에 커넥션 풀러를 세워 버텼다.**
- Monzo — 상태를 한 클러스터에 모아 둔 값을 장애로 치르고 **쪼갰다.**
- Materialize — **읽기 자체를 다른 엔진으로 옮긴다.** 원본은 쓰기만 하게 두고 읽을 답을 옆에서
  미리 만들어 둔다.

### 파는 쪽이 자기 물건이 안 맞는 자리를 먼저 적는다

원시 데이터의 탐색적 질의에는 창고가 맞다고 못 박는다. 이 구조가 겨냥하는 것은 **모양이 예측
가능한 질의**(웹 서비스·운영 대시보드)다. 이 엔진에서 드문 자리다.

성과도 두 수치로 드는데 둘 다 회사가 고른 사례의 것이다 — 읽기 위주 워크로드에서 1000배, 사례
하나에서 큰 읽기 복제본 여덟 대를 하나로 줄여 비용 90% 감소.

### 두 도메인이 같은 손잡이의 양쪽이다

색인을 깊이 할수록 지연이 줄고 **들고 있어야 할 상태가 는다.** 그 상태가 곧 컴퓨트 크레딧이고,
그 바이트를 깎는 것이 앞 기능(사전 압축)이다. **이쪽이 원가를 만들고 저쪽이 그 원가를 깎는다** —
회사 안에서 두 도메인이 이렇게 맞물리는 경우가 드물어 연결로 걸어 뒀다.

### 다음 사이클 — 후보 조사 (큐 1/3)

빈 도메인이 없어졌다. 큐에 Zed 하나만 남아 목표(3)에 못 미치므로 다음은 후보 조사다.
