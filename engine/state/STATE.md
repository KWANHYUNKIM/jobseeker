...
**하이퍼커넥트 (KR · 소셜/실시간 영상) — 46번째. 회사 프로파일 완료. 도메인 2개 · 기능 0개. 큐 1/3.**

**본문 두 편을 열었고 둘 다 이 사이트 평균보다 단단했다.**

**도메인 ① `모델은 좋아졌는데 서비스가 안 좋아진다`** — 추천 태그 8편 중 **셋이 이 주제로 묶인다**
(아자르 타겟 지표 2024-04-26 · train/eval 설계 2025-11-28 · 비즈니스 문제를 AI 문제로 2026-01-12).
**2년에 걸쳐 같은 문제를 세 번 다시 썼다.** 읽은 한 편(2025-11-28)이 문제를 한 문장으로 적는다 —
*"비즈니스 목표가 아닌 잘못된 지표를 최적화 하는 경우, 모델 성능을 계속 높여도 서비스에서 아무런
효과가 발생하지 않는 상황을 겪기도 됩니다."* **버린 접근이 셋이고 각각 인과적 이유가 붙는다**
(단순 지도학습은 *"유저와 아이템이 서로 독립이라고 가정"*, 유저 단위 랭킹은 아이템 교란, 무작위화
없는 아이템 단위 랭킹은 시간에 따라 변하는 숨은 교란). **대가를 여덟 줄 적는다.**
**이 사이트의 다른 회사와 접근이 다르다** — 정확도가 아니라 **인과 추론(PGM)** 이다.

**도메인 ② `서로 다른 시각에 도착한 것을 이어 붙인다`** — Flink 스트림 조인(2025-06-11).
**⭐ 이 회사에서 버린 대안이 가장 많은 글이다 — 다섯이고 이유가 각각 다르다**(Spark Streaming 의
마이크로배치 지연 · Kafka Streams 의 *"Event Time 기반 타임아웃 등 정밀 제어의 한계"* · Flink Window
API 는 *"불특정한 시점에 도착할 이벤트의 경우를 기다리기 어렵습니다"* · Kafka Sink Exactly Once 는
*"Checkpoint 주기마다 메시지를 발행하게 되어, 실시간성이 크게 저하"* · 중복 제거 Flink 앱을 따로 두면
*"추가적인 리소스 사용과 운영 비용"*). **대가도 여섯.** 수치는 **중복 제거 평균 300ms → 3ms 미만**.
**배포까지 설계에 들어온다** — Flink 에 무중단 배포가 없어 Spinnaker + Blue-Green 을 직접 만들었고,
이유가 *"배포 중 발생하는 잠깐의 서비스 중단도 유저 경험과 서비스 품질에 영향을 줄 수 있습니다"* 다.

**🔴 사업 수치를 1차 자료로 확인하지 못했다.** 하이퍼커넥트는 비상장이고 자체 IR 이 없다.
**Match Group 자회사로 알려져 있으나 Match Group IR 홈에 하이퍼커넥트도 아자르도 나오지 않는다** —
**소유 관계 자체를 확인하지 못했다.** 받은 것은 **LTM 매출 $3.5B**(2026-06-30 종료 분기 기준)뿐이고
브랜드별 구성이 없다. **그 값을 아자르의 규모로 읽으면 안 된다고 명시했다.**
다음에 `ir.mtch.com/investor-relations/news-events/quarterly-results/` 나 SEC 파일링을 연다.

**🚨 다음 사이클 — `--gaps` 가 [진행 중 확장]을 가리킬 것이다. 도메인 하나의 기능을 쓴다.**
**도메인 ②(Flink)가 더 유력하다** — 한 편만으로도 결정이 6~7개 선다(Window API 대신 KeyedProcessFunction ·
Exactly Once 대신 실시간성 · Redis 중복 제거 · Blue-Green 직접 구성 · 타이머 만료 처리 · RocksDB 상태).
**⚠️ 다만 기능을 쓸 때는 두 편 이상 읽는다**(열여덟 번 확인된 규칙) — 같은 계열의
`/2022/01/24/event-driven-recsys.html`(이벤트 기반 라이브 스트리밍 추천)을 연다. **4년 전 글이라
최신성을 함께 밝힌다.** **⚠️ 그리고 원문을 다시 읽는다**(운영 사실 ⑫ — 여섯 번 다 새 결정을 찾았다).
**도메인 ①을 쓴다면** 나머지 둘(`/2024/04/26/azar-aha-moment.html` · `/2026/01/12/how-to-relax-a-business-problem.html`)을 먼저 읽는다.
...
**미개봉이고 둘이 2020·2023 이라 최신성을 확인해야 한다.**

**⚠️ 다음 회사는 컬리다(큐 1/3).** 컬리는 **본업 재료가 따로 있다** — `/blog/image-based-misdelivery-detection`
...
**⚠️ 한국 편중 — 이 둘을 다 파면 아홉이 된다**(NAVER · Kakao · Toss · 당근 · 배민 · 쿠팡 · 무신사 + 2).
**자리는 겹치지 않지만**(실시간 영상 매칭 · 신선식품 콜드체인) **다음 후보 조사는 한국 밖에서 한다.**
...
**📌 246·253 이 확인한 것** — 403 · DNS 없음 · JS 렌더링이 절반을 넘는다. 그래도 **다섯에 둘, 여덟에
하나는 나온다.** 목록을 계속 내려간다.
...
**📌 비교 문서 재료(27편 있음, 둘이 찼다 — 큐가 비면 이걸 쓴다)**
- **새 축 `관측에 값을 무엇으로 치르는가`** — CyberAgent `observability-pipeline`(돈: 이중 과금
  **$0.095/GB + $0.10/GB** 를 피해 직접 굴리고 **관측 도구 자신의 로그를 뺀다**) + CyberAgent
  `alert-context`(주의력: *"알림의 형해화"*) + Yelp `partition-access`(접근 로그 **하루 TiB**) +
  Zerodha `log-storage`(**행당 340바이트에서 17.38바이트**) + Doximity `prod-profiling`.
- **`되돌릴 수 있는 것을 먼저 한다`(reversible-first)에 CyberAgent `env-split-migration` 이 반례** —
  **되돌림 계획 자체가 없다.**
- **📌 새 재료** — 하이퍼커넥트의 목표 지표 도메인은 기존 비교 문서 **`무엇을 좋다고 부를 것인가`** 에
  붙을 수 있다. **인과 추론으로 지표를 고른 사례는 이 사이트에 처음이다.**
...
⑧ **대기 표는 5칸**(`회사 | 국가·분류 | 1차 자료 | 접근 | 왜 이 회사인가`), **진행 중 표는 3칸**.
...
⑫ **요약이 아니라 원문을 다시 읽는다** — 223·229·243·249·250·252 여섯 번 다 새 결정을 찾았다.
