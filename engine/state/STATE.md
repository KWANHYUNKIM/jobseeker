...
**Roblox — 진행 중(사이클 127 에 시작).** 도메인 4개 중 **서비스 발견 하나 완료**
(`consul-outage-cells`). 남은 것: 게임 서버 운영 / 콘텐츠 저장·전송 / 안전·모더레이션.
...
> **게임 서버 운영 도메인이 Cloudflare Workers 와 비교할 자리다** — 둘 다 남의 코드를
> 자기 기계에서 돌리는데, Roblox 는 **상태가 있고 실시간**이라 세션을 옮기거나
> 다시 시작할 수 없다. 다음에 팔 때 그 축을 볼 것.
...
마지막 사이클: 127 (2026-08-21) — Roblox 프로파일 + 73시간 장애.
