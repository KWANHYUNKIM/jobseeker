...
**없다 — 문체 보수 사이클이었다.** ClickHouse 를 배민 문체로 다시 썼다(2.15 → 0.0). 회사 115개, 큐 2/3. 31곳 남았다.

### 나중에 합치는 셋 — 그 값을 누가 내느냐가 갈린다

첫 기능에 축을 세웠다. 셋 다 완성된 상태를 만들지 않고 무엇이 바뀌었는지만 먼저 적는다.

- Neon 은 로그가 데이터베이스이고 파일은 그것의 캐시일 뿐이라고 말한다.
- Databricks 의 Lakebase 는 커밋이 무엇을 기다릴지를 바꿔 전체 페이지 쓰기를 없앴다.
- ClickHouse 는 패치를 따로 써 두고 백그라운드 머지가 나중에 접는다.

**앞의 둘은 그 값을 뒤의 인프라가 치르는데 여기는 질의하는 사람에게 넘긴다** — 아직 안 합쳐진
것을 어떻게 읽을지 사용자가 알아야 한다. 오픈소스로 뿌리는 엔진이라 그 선택이 문서의 문제가 된다.

### 자기 제품을 자기가 쓰는 회사 — 그 값을 적는 방식

- Honeycomb 은 관측을 파는데 자기 카프카가 나빠진 구간을 남겼다.
- Sentry 는 오류를 보여 주는 것을 파는데 자기 에이전트에는 그 눈이 없었다고 적는다.
- ClickHouse 는 한발 더 간다 — **자기 창고 인프라 비용을 월 약 1,500달러라 적으면서 가장 큰
  항목인 ClickHouse Cloud 는 사내 서비스라 뺐다고 단다.**

그래서 이 수치는 남이 같은 구조를 지을 때의 비용이 아니다. 자기 제품을 쓰는 사례가 그 제품이
얼마나 싼지를 증명하지 못하는 이유가 여기 있다 — 장부에서 가장 큰 칸이 사라진다.

### 마지막 한 걸음을 남기는 방식과 비율을 먼저 정하는 방식

셋째 기능을 사람을 어디에 남기나 축에 붙였다. Anthropic·Block·Sentry·Trendyol 이 **마지막 한
걸음**(되돌릴 수 없는 행동, 병합, 최종 컷오버)을 남기는 쪽이라면, **ClickHouse 는 넘길 비율을 먼저
정하고(70%) 닿을 수 있는 데이터까지 미리 잘라 둔다.** 같은 축의 다른 종류다.

### 도메인을 제품이 아니라 자기 제품과 어떤 관계에 서 있느냐로 잘랐다

파는 엔진 안의 결정, 그 엔진을 자기가 써서 먼저 벽에 부딪힌 이야기, 그 위에 자기 팀을 위해 얹은 것.
제품으로 가르면 이 회사의 특징 — **파는 것과 쓰는 것이 같다는 사실** — 이 사라진다.

### 다음 사이클 — 문체 보수 (사다리 3순위)

--gaps 가 정한다. 지금 맨 위는 turso(2.05)지만 그건 이 시점의 예상이지 확정이 아니다.
