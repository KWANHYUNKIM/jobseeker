...
**Anthropic** — 기능 **2/3.** 회사 112개. 큐 2/3. ⚠️ **남은 도메인 하나**(`틀린 것을 공개한다`)를 채우면 완주다.

### 이번 사이클 — `무엇을 떼어 놓고 무엇을 붙여 두나`

⭐ **같은 문제가 층을 바꿔 두 번 나왔고 답이 같았다** — **인프라를 갈랐더니**(뇌·손·세션) **역할도 갈라야 했다**(계획자·생성자·평가자). **두 자료를 한 기능으로 묶었다.**

**인프라 층** — **`하네스가 더는 컨테이너 안에 살지 않는다. 하네스는 컨테이너를 다른 도구와 똑같이 부른다.`** 그래서 **컨테이너 실패가 도구 호출 오류**가 되고, **하네스 자신도 상태가 없어 세션 아이디로 깨워 이어간다.** **컨테이너를 도구 호출이 있을 때만 띄워 p50 TTFT -60%, p95 -90% 이상.**

**역할 층** — ⚠️ **자기 일을 자기가 채점하면 `사람이 보기에 명백히 평범한 품질에도 확신에 차서 칭찬한다.`** 가르는 것이 **`강한 지렛대`**였다. ⚠️ **값은 비싸다** — 솔로 20분 **$9** 대 전체 하네스 6시간 **$200**(20배 이상).

### ⭐ 이번에 건진 가장 큰 것 — 하네스는 자라기만 하는 것이 아니다

**`하네스의 모든 구성 요소는 모델이 혼자 못 하는 것에 대한 가정을 담고 있다.`**

⭐ **그래서 Opus 4.6 이 좋아지자 스프린트 구조를 없앴다** — Opus 4.5 는 스프린트 분해가 필요했고 문맥 불안이 강했는데 **4.6 은 스프린트 없이 두 시간 넘는 일관된 빌드를 감당했다.** 비용도 **6시간 $200 → 약 4시간 $125.** ⚠️ **평가자의 가치조차 `과제가 모델이 혼자 안정적으로 할 수 있는 것에 비해 어디에 놓이느냐에 달렸다.`**

### ⭐ 문맥 불안 — 성능 문제가 아니라 믿음 문제다

**`모델이 자기 문맥 한계라고 믿는 지점에 다가가면 일을 조기에 마무리하기 시작한다.`** ⭐ **그래서 흔한 해법이 안 듣는다** — **`압축은 이력을 지키지만 불안은 그대로 둔다.`** **창을 통째로 비우고 새 에이전트로 시작한다.**

⚠️ **그런데 비우면 이력이 사라진다** — **세션이 `창 밖에 사는 문맥 객체` 노릇으로 그 자리를 메운다.** ⭐ **`창은 비우고 이력은 밖에 둔다`가 이 도메인의 한 줄이다**(재구성).

### ⚠️ 한 회사 안에서 두 결정이 반대로 당긴다

**여기서는 역할을 갈라 여러 에이전트를 쓰는 것이 품질을 올리는데**, 지난 사이클의 평가 글에서는 **다중 에이전트가 오염 노출을 3.7배 키웠다.** ⭐ **`좋은 구조`와 `재기 좋은 구조`가 다르다**(재구성). `connections` 에 남겼다.

### 다음 사이클 — 확장, 그리고 완주 (2순위)

**Anthropic 기능 2/3.** ⏳ 마지막 — **`틀린 것을 공개한다`**(4월 23일 포스트모템 · 겹친 버그 셋 · 왜 못 잡았나). ⚠️ **완주 판정 때 수익원 셋**(API·기업 계약 / Claude Code / 소비자 구독)**이 기능과 이어지는지 본다** — Block 에서 그것 때문에 사이클이 하나 더 들었다.

⏳ **못 읽은 것** — `Claude Code auto mode`(⭐ **손이 무엇까지 해도 되나 — 이 도메인의 권한 쪽 면일 수 있다**) · `How we contain Claude across products` · **학습 인프라 자료**(⚠️ 아직 못 찾았다).
