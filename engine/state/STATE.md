...
**없다 — 문체 보수 사이클이었다.** Deliveroo 를 배민 문체로 다시 썼다(2.2 → 0.0). 회사 115개, 큐 2/3. 47곳 남았다.

### 사람을 어디에 남기나 — 네 번째 답은 사람이 아니었다

Deliveroo 의 첫 기능이 이 축에 새 자리를 낸다. 규정 준수를 회의에서 배포 파이프라인으로
옮겼다. 사람이 검토해서 막는 것이 아니라 파이프라인이 막는다.

- Anthropic 은 되돌릴 수 없는 행동을 사람이 쥔다. PR 병합과 티켓 종료를 안 넘긴다.
- Trendyol 은 최종 컷오버라는 돌아올 수 없는 지점에 전략적 거부권을 준다.
- Block 의 자가치유 루프도 병합과 종료를 사람에게 남긴다.
- Deliveroo 는 사람이 아니라 파이프라인에 그 선을 심는다.

앞의 셋은 사람이 서는 자리를 정하는 답이고 Deliveroo 는 그 자리를 코드로 옮기는 답이다.
근거도 반대다. 회사는 대부분의 회사가 혁신과 거버넌스는 대립한다고 생각한다고 적으면서,
자기들은 마찰을 더하는 대신 없앴다고 주장한다.

### 이 엔진이 처음 만난 종류의 벽

deliveroo.engineering 이 국가 단위로 한국에서의 접근을 막는다. 403 은 도구를 바꾸면
뚫렸지만 이것은 도구의 문제가 아니다. Perplexity 의 403 이나 Databricks 의 트래커 픽셀과는
성격이 다르다. 그 사실을 사업 모델에 그대로 남겼다.

### 이번 사이클 — Deliveroo

이 회사를 고른 이유가 규모가 아니라 각도라는 것을 앞세웠다. 같은 그룹이 된 도어대시가
에이전트가 설 자리를 워크플로가 아니라 기본 재료로 만든 것과 달리, 여기는 규정 준수 자체를
코드로 심는다.

결제 쪽 문장도 살렸다. 막는 쪽과 여는 쪽이 같은 손잡이에 달려 있고, 회사가 밈 한 장으로
적는다 — 우리는 모든 사기를 없앴다, 모든 주문을 없애서.
