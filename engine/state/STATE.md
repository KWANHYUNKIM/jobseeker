...
**Klaviyo** — 기능 **1/2.** 회사 110개. 큐 1/3. ⚠️ **남은 도메인 하나**(`잃지 않고 넘긴다`)를 채우면 완주다.

### ⚠️ 검증이 규칙 하나를 잡았다 — `from` 없는 시기는 세우지 않는다

RabbitMQ 세대를 `history` 항목으로 세웠는데 **시작 연도를 몰라 `from` 을 비웠다.** 검증이 잡았다: **`from 없음 — 시간축인데 시간이 없으면 남는 것이 없다. 연도를 모르면 이 항목을 쓰지 않는다.`**

⭐ **고치는 방법이 이미 규칙으로 있었다** — **그 시기를 세우지 말고 내용을 다음 시기의 `그전까지는 ~였다`로 옮긴다.** **정보는 그대로 남고 없는 연도만 사라졌다.** ⚠️ **프로파일에서 `eras` 를 비운 이유와 같은 이유인데, 기능 `history` 에서 같은 실수를 한 번 더 했다** — **한 곳에서 지킨 규칙이 다른 필드에서 저절로 지켜지지 않는다.**

### 이번 사이클 — `마감 시각으로 줄을 세운다 (EDF 우선순위 큐)`

⭐ **우선순위를 등급이 아니라 시각으로 매긴다** — **`이벤트별 SLO 마감 시각을 키로 하는 하나의 인메모리 우선순위 큐 — 최단 마감 우선(EDF) 스케줄러다.`** ⭐ **등급은 상대적이고 SLO 는 절대적이다**(재구성) — **`5초`와 `20분`은 등급이 아니라 시각이라 같은 자로 잴 수 있다.**

건진 것 넷:

- ⭐ **그래서 등급마다 두던 인프라가 없어졌다** — **`P10·P20·P30 마다 따로 두던 인프라 대신 같은 처리기 함대가 모든 우선순위를 다룬다.`** **비용 30% 감소, SLO 준수 20% 개선** — **보통은 하나를 주고 하나를 얻는데 둘 다 좋아졌다**(재구성).
- ⭐ **밀어 주지 않고 가져가게 했다** — **`분배가 풀 기반이라 느린 처리기는 스스로 속도를 늦추고, 나머지가 큐를 계속 비운다.`** ⚠️ **밀어 주면 느린 놈에게도 같은 양이 쌓이고 그게 다시 선두 차단이 된다**(재구성).
- ⭐ **두 순환이 동시에 돈다** — **인테이크 루프**(폴링 → 큐 채우기 → 안전한 오프셋 커밋)와 **서브 루프**(논블로킹 `다음 것 달라` → 처리기가 당겨 감 → ACK/NACK).
- ⚠️ **역압을 카프카로 되돌린다** — **`상한 워터마크에 닿으면 카프카 인테이크가 자동으로 멈추고, 하한 아래로 떨어지면 소비가 재개된다.`** ⭐ **대가가 미묘하다** — **창 안에서만 EDF 가 성립하므로 창이 좁으면 뒤에 있던 더 급한 이벤트를 못 본다**(재구성).

### ⭐ 남는 실패 모드를 회사가 스스로 든다

**`높은 우선순위 부하에서 배경 워크로드의 굶주림`** — 함대를 합친 대가다. ⚠️ **그리고 자료에 없는 것 둘** — **처리기가 일 도중 죽었을 때의 재전달·중복 규약**과 **마감 시각을 이벤트에 붙이는 방법**(큐의 키인데 어디서 오는지가 없다). `open_questions` 로 남겼다.

### ⭐ 이 엔진에서 처음 만난 축 — `실시간 스케줄링 이론을 파이프라인으로 옮기기`

**Liu & Layland(1973)의 EDF 가 단일 처리기에서 최적임을 보인 고전이다.** 회사가 그 논문을 언급하지는 않지만 **큐의 키가 정확히 EDF 다.** ⚠️ **원래 EDF 는 단일 처리기 가정 위에 서 있고 여기는 처리기 함대다** — **`풀 기반 분배`가 그 간극을 메운다**(재구성). `research.papers` 에 `inferred` 로 달았다.

### 다음 사이클 — 확장, 그리고 완주 (2순위)

**Klaviyo 기능 1/2.** ⏳ 마지막 도메인 — **`잃지 않고 넘긴다`**(RabbitMQ → AWS MSK 무중단·무손실 이관, 7일 리플레이, 설계 원칙 넷). ⚠️ **완주 판정 때 Block 에서 겪은 것을 먼저 본다 — 수익원 둘(구독·SMS)이 기능과 이어지는지.**

⏳ **못 읽은 자료** — `Our two lines of defense: kafka and S3`(주소 못 받음) · `Unbreakable System for Distributed ML Workloads` · `Ray Data, Train & Tune` · `Engineering Secure Agentic Systems at Scale` · `Aurora Blue/Green`.
