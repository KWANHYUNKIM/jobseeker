...
**CyberAgent (JP · 광고) — 45번째. 기능 1개 · 도메인 2개. 큐 잔량 0/3.**

248 에서 **`ad-pacing`(고르게 뿌린다)** 를 썼다. 결정 7 · 그림 3 · 생각 4 · 스택 5 · 난제 2 ·
ui 6핀(`kind: web`).

**🔴 관찰 하나를 정정했다 — "이 회사는 자기 수치를 안 낸다" 가 틀렸다.**
광고 배송 글 **넷 중 둘**은 자기 값을 낸다. **필터링 고속화**(64498)가 **2코어에서 약 700 RPS 한계**,
**Marshal 관련 CPU 시간 약 32% → 약 16%** 를 적고, **광고 배송 캐시**(65145)도 촘촘하다.
**안 내는 쪽은 페이싱**(63908, 숫자가 전부 타사 참고문헌)**과 플랫폼 이설**(61355, 기간 약 반년뿐).
**글쓴이·팀에 따라 관행이 다른 것으로 보인다**(추측으로 적었다).

**🔴 그리고 세 편이 서로를 전혀 언급하지 않는다.** 페이싱(2026-05) · 필터링 고속화(2026-07) ·
플랫폼 이설(2025-12) 이 **같은 시스템의 다른 층**인데 어느 글도 다른 글을 가리키지 않는다.
캐시 글 둘(광고 배송 ↔ ABEMA 본체)도 그랬다. **이 회사의 성격으로 보인다.**

**📌 223 의 규칙을 적용했다 — 서로 다른 문제를 한 기능에 뭉개지 않았다.** 세 편이 같은 "광고 배송"
이지만 층이 다르다: **페이싱 = 언제 얼마나** · **고속화 = 얼마나 빨리** · **이설 = 어디서**.
페이싱만 기능으로 쓰고 나머지 둘은 세 번째 도메인 재료로 남겼다.

**`ad-pacing` 의 축은 정확도가 아니라 운용 비용이다.** 회사가 그렇게 적는다 —
*"배송 단위마다 이득 튜닝이 불필요해지고, 운용 비용이 배송 단위 수에 의존하지 않게 됩니다."*
**G 가 10³ 에서 10⁶ 까지 갈리는데(진폭 1000배) 손잡이 하나로 다루는 것**이 목표다. 그래서 오차를
**비로 무차원화**한다(e = −log(N/r)). ⚠️ **도입 전후를 재지 않아 실제로 나아졌는지 이 글로는 모른다.**

**🚨 다음 사이클 — `--gaps` 는 [진행 중 확장]으로 도메인 ②(캐시)의 기능을 가리킬 것이다.**
재료는 **65145**(광고 배송 L1+L2 Valkey, **버린 대안 둘 + 대가 다섯 줄 + 자기 수치 촘촘**)와
**64293**(ABEMA 본체 다층 캐시, **한 달 인턴 글이라 버린 대안 0 · 수치 0**). **둘 다 이미 읽었다.**
**65145 하나로 결정 5개가 서는지 세어 본다** — 서면 쓰고, 안 서면 억지로 늘리지 않는다.
**두 글이 같은 문제(Pod 이 늘면 캐시가 흩어진다)를 각각 풀고 서로를 언급하지 않는다는 것이 축이다.**

**세 번째 도메인 후보(재료 확인됨)** — `광고 배송을 어디서 어떻게 굴릴 것인가`:
**64498**(필터링 고속화 — 700 RPS 한계 · vtprotobuf 로 Marshal CPU 32%→16% · 버린 대안 하나
easyproto · **대가 하나**: 고부하 시 논블로킹 큐가 상한에 닿아 *"예상 이상의 로그가 결손"*) +
**61355**(플랫폼 이설 — **버린 대안 넷에 이유가 각각**: 기존 구성 개수 · PSA+VPN(다운타임) ·
Cloud Run Domain Mapping(헤어핀 라우팅 지연 · Cloud Armor/CDN 불가) · Routes 기반 VPN 직결
(Pod CIDR 이 BGP 로 안 퍼진다) · **대가 다섯**: DMS 제약 · Memorystore 직결 곤란 · 쓰기 실패 시
CrashLoopBackOff 위험 · 인증서 최대 72시간 · TCP 프록시는 임시 우회로).
...
⑪ **mermaid 라벨에 수식 기호(=, ×, ÷, ⁻)를 넣지 않는다** — 말로 푼다(248 에서 그렇게 썼다).
