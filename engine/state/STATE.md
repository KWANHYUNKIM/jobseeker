...
**쏘카(KR · 모빌리티) — 첫 기능 `discount-self-heal`(할인권 자가복구 에이전트)을 채웠다.**
`in_progress` · **도메인 2 · 기능 1** · 결정 8 · 그림 3 · 지표 7.
**완료 61곳 + 쏘카 = 62곳 · 큐 대기 1(카카오뱅크) · 비교 문서 33편.**

### 다음 사이클 — 남은 도메인의 `tech` 를 채운다

**`차 한 대의 생애를 얼마로 만들 것인가` 가 비어 있다**(검증이 경고로 잡는다).
`--gaps` 가 **확장** 2순위로 이 도메인을 지목할 것이다.
실적 발표문의 문장으로 **경계만** 잡아 뒀고 무슨 기술로 푸는지는 아직 안 읽었다 —
**차량 1대당 생애주기 매출총이익 1,420만 원 · 가동률 37.8% · '쏘카 2.0' = "데이터 기반 수요
예측과 카셰어링과 쏘카플랜간 차량 운영을 최적화한"** 까지가 확인된 전부다.
**기술블로그의 데이터 카테고리가 24편으로 가장 크므로** 거기서 수요 예측·차량 배치 글을 찾는다.
`tech.socar.kr` 목록에서 URL·날짜·카테고리를 함께 받을 수 있다.

### ⚠️ 이 회사는 기능 하나가 도메인 하나를 다 가져갔다

`discount-self-heal` 이 **에이전트 설계 전체**(프레임워크 선택 · 단계 고정 · 검증 분리 ·
한도 · 쿨다운 · Draft PR 경계 · 근거 기록)를 통째로 가져갔다. **같은 도메인에 기능을 더 쓰려면
축이 겹친다** — 쓴다면 **기준선 스키마 관리**(자동 갱신 파이프라인이 아직 없다고 회사가 적었다)
쪽이어야 한다. 지금은 자료가 없다.

### 쏘카 자료 접근 — 다음 실행이 헛돌지 않게

- **`tech.socar.kr` 은 WebFetch 로 본문까지 열린다.** (`tech.socarcorp.kr` 에서 301)
  카테고리별 편수: **데이터 24 · 백엔드 19 · 웹 프론트엔드 7 · Mobility 5 · Design 5 ·
  Product 4 · QA 3 · 보안 2 · 디자인 시스템 2.**
- **IR 은 회사 블로그가 대신한다.** `socarcorp.kr/blog/689`(2026-02-25)에 FY2025 수치가 표로 있다.
  ⚠️ **부문별 매출·차량 대수·회원 수는 거기에 없다.**
- **⚠️ `eras` 는 여전히 비어 있다.** '쏘카 2.0' 이 2024년부터의 전환으로 보이지만 **그때 기술적으로
  무엇을 갈아엎었는지를 적은 자료를 못 찾았다.** 두 번째 도메인을 파다가 걸리면 그때 세운다.

### 이 세션에서 배운 것 — 다음 회사에도 걸린다

- **제목의 마지막 낱말이 기능으로 쓸 수 있는지를 가른다**(ZOZO). 「検証」은 오프라인,
  「構築と運用」·「対策」은 운영이었다. **본문에서 '본번·릴리스·A/B' 를 먼저 찾는다**(353 의 연장).
- **남에게 의존하는 자리는 그 자체로 도메인이 된다.** ZOZO 는 빌린 모델을, 쏘카는 남의 정산
  장비를 상대한다. **둘 다 '내가 못 고치는 것이 조용히 바뀐다' 가 문제의 본체이고, 둘 다
  실패를 신호로 삼기 때문에 실패 없는 변경을 못 잡는다.** 비교 문서 재료다.
- **글이 한도와 쿨다운을 적어 두면 결정이 두 개 더 나온다.** '언제 포기하는가' 와 '얼마나 자주
  시도하는가' 는 흔히 안 쓰이는데, 쏘카는 둘 다 숫자로 적었다.

### ⚠️ 다른 세션과 같은 큐를 잡지 않는다

**사용자가 여러 세션을 동시에 돌리고 있다.** `guide-engine`(쿠팡)·`study-engine` 둘 다 계속
커밋 중이다. **사이클 시작 전에 `git log --oneline -3` 을 보고, 커밋할 때 `git add -A` 를 쓰지
않는다** — 다른 세션이 작업 중인 파일이 워킹트리에 함께 떠 있다.
