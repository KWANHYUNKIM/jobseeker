...
**bol(EU · 커머스) `in_progress` — 도메인 2개 · 기능 2개.** 352 에서 **새 도메인**
`모델이 먹을 데이터를 층으로 나눠 만든다` 를 열고 그 안에
**`slow-down-to-go-fast`(빨리 가려다 느려지는 자리에서 먼저 멈춘다)** 를 썼다.
**결정 6개 · 그림 3장 · 생각 4개 · 난제 2건 · 연결 1건.**

**⑭ `--gaps` 는 [신규]** 를 가리켰다(bol 의 첫 도메인이 차서 사다리가 내려갔다).
**회사 완주 규칙이 위라 bol 을 계속 팠다**(CLAUDE.md).

**재료와 판정** — `Fast is slow, slow is fast - rethinking Our Data Engineering Process`
(2024-11-07, `/en/blog/fast-is-slow-slow-is-fast-rethinking-our-data-engineering-process/`,
**⑱ 로 실제 href 를 확인했다**). **2/3** — 버린 대안(**빨리 띄우고 리팩터는 나중에** 하는 길을
명시적으로 거부) · 대가(회사가 직접 적는다) · **수치는 약하다**(반복 시간 몇 시간에서 몇 분,
시작 2023년 4월뿐).

**⚠️ 223 규칙으로 갈랐다 — 새 도메인이다.** 첫 도메인은 **거래가 끝난 뒤의 정산**이고
이 글은 **데이터 사이언스를 떠받치는 파이프라인**이다. 완전히 다른 문제다.

**⭐⭐ 352 의 핵심 — 같은 회사 안에서 검증이 정반대다**(이 대비는 이 사이트의 것이고
`connections` 와 `open_questions` 에 적었다). 데이터 사이언스 쪽은 **쓰고 검사하고
내보내는 흐름**으로 단계마다 품질을 보고 **중요한 필드의 음수까지 잡아냈다**고 적는데,
**3TB 를 옮긴 이관에는 행 수 대조도 체크섬도 정합성 절차도 나오지 않는다.**
**⚠️ 다만 다른 팀의 글이고 목적도 달라 회사의 방침으로 읽지는 않았다.**

**352 가 새로 낸 것**
- **⭐ 가장 아픈 증상** — *"you need to use a debugger to carefully examine the code to
  figure out what inputs the model is actually using"*. **모델이 무엇을 먹는지 알려면 코드를
  열어 봐야 했다.** **계보가 문서가 아니라 코드 실행 경로가 되면 아는 방법이 읽는 것이 아니라
  돌려 보는 것이 된다**(이 사이트의 정리 — 난제로 세웠다).
- **빠른 길을 버린 이유가 한 문장이다** — *"there's nothing as permanent as a temporary
  solution"*. **자기 약속을 못 믿기로 한 셈이다.**
- **층이 답이다** — DAG 하나가 데이터 층 하나(원천과 중간 · 프라이머리 · 피처 스토어 ·
  모델 입력 · 모델 출력 · 리포팅), 태스크 하나가 한 가지 일, 디렉터리가 그 층 구조 그대로.
- **🔴 대가를 회사가 직접 적는다** — 잘게 쪼개면 **매 단계마다 데이터를 남겨야 해 저장과
  처리 시간이 늘고**, 태스크가 지나치게 많으면 특히 그렇다. **읽기 쉬움과 실행 비용이 같은
  손잡이의 양쪽이다**(이 사이트의 정리 — 난제로 세웠다).
- **⚠️ 그런데 균형의 기준이 없다** — *"strike a balance"* 라고만 한다. **그래서 이 방식은
  다른 팀이 그대로 옮겨 쓰기 어렵다**(이 사이트의 판단).
- **⚠️ 문제를 세지 않고 고쳤다** — 중복 파이프라인이 몇 개였는지 없고, 성과도 **반복 시간
  하나**뿐이며 전후 비교 지표가 없다. 팀 규모·DAG 수·오버헤드 실측도 없다.

---

**다음 사이클 — 두 도메인 다 한 편씩만 읽었다. 두 편 규칙을 어느 쪽이든 채운다.**
- **⭐ 1순위 — 데이터 도메인 쪽.** `Database Setup So Easy, Your Cat Could Do It: Docker
  and Flyway Edition`(2024-08-02). **Flyway 가 351 의 이관에도 352 의 파이프라인에도 나왔다 —
  어느 쪽에 붙을지 실물로 정한다**(223 규칙). **⚠️ 주소는 ⑱ 로 확인한다.**
- **⭐ 2순위 — 배포 축.** `Bol's journey in shifting left and shifting right: our Vision` 과
  `... Canary Releases`(둘 다 2024-05-22). **두 편이 짝이라 한 도메인을 두 편으로 채우기 좋다.**
  **⚠️ 다만 Vision 편은 일반론일 위험이 있다**(336 의 Sansan 처럼) — **Canary Releases 편을
  먼저 연다.**
- **3순위 — 다른 도메인이 될 것들** — `How we built near real-time X for you recommender
  systems at Bol`(2024-04-09) · `Navigating chaotic times: forecasting amid the pandemic`
  (2024-04-18). **⭐ 리테일 추천과 수요 예측은 이 사이트에 얇은 축이다.**
- **4순위 — 닫는다.** 도메인 둘 다 기능이 있고 `connections` 로 이어졌다(완주 기준 충족).
- **⚠️ 블로그 최신이 2024-11-07 이다.**

**⚠️ 큐 1/3 — bol 을 완주하면 후보 조사를 한 번 끼운다.** 대기는 **PayFit** 하나
(`backstage.payfit.com`, 2/3, 얇고 2024-07 에서 멈춤). **후보 조사 누적 스물세 곳 중 아홉.**

**⭐⭐ 비교 문서 재료 넷**(회사를 파는 중에는 손대지 않는다):
① **freee 대 SmartHR 의 순서** — freee 가 후회로 적은 순서를 SmartHR 이 실제로 밟아 보고
**모듈러 모놀리스로 경계만 정리하는 안을 버렸다.**
② **`미루되 미룬 것이 안 보이게 한다`** 에 SmartHR 두 사례 — 미룬 쪽은 **놀고 있는 접속**,
미리 잰 쪽은 **연 1회 정점의 부하 시험**.
③ **bol 이 ②의 셋째 사례** — 버린 첫 대안이 **중개 앱을 2031년까지 유지하는 것**이고,
352 의 *"there's nothing as permanent as a temporary solution"* 이 **같은 회사에서 같은 판단을
두 번 한 증거다**(이 연결은 352 에서 새로 보였다). ⚠️ **둘 다 값을 숫자로 적지는 않는다.**
④ **`되돌릴 수 없는 것이 어디에 있는가` 에 bol** — **적어도 한 번** 처리를 골랐고 정산에서
중복은 크레딧을 두 번 내는 일인데 **이관 쪽에 검증 이야기가 아예 없다.**
...
나온다 — 요약은 '없는 것' 을 잘 떨어뜨린다**(351) · **⭐ 새로: 글을 열 때 '무엇이 없는지' 를
함께 물으면 대가 칸이 훨씬 정확해진다**(352 에서 처음부터 그렇게 물었다).
