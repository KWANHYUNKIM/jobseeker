...
**아무것도 안 판다 — 253 은 후보 조사였다. 다섯 곳을 두드려 둘을 건졌다. 큐 잔량 2/3.**

**✅ 큐에 올린 둘**
- **하이퍼커넥트**(KR · 소셜/실시간 영상) `hyperconnect.github.io` — **2018~2026 에 150편 넘게 쌓였고
  최신이 2026-04-22.** 묶음이 넷으로 뚜렷하다: 추천/ML 23편+ · 실시간 인프라 15편+(Flink 스트림 조인
  2025-06) · 쿠버네티스 17편+(**1년에 워크로드 절반을 ARM64 로** · **5일 만에 프로덕션 클러스터 이전**) ·
  데이터 파이프라인 10편+. **자리** — 이 사이트에 **실시간 1:1 영상 매칭이 없다**(당근은 지역 중고거래,
  Discord 는 음성 커뮤니티). ⚠️ **본문을 아직 안 열었다** — 파기 전에 한 편 확인한다.
- **컬리**(KR · 커머스/신선식품) `helloworld.kurly.com` — **본문을 열어 확인했다.**
  `/blog/2026-delivery-domain-rag` 가 **세 단계를 거치며 앞의 둘을 왜 버렸는지 인용문으로 적는다**
  (역색인은 *"색인 유지 자체가 부채"*, 전문 임베딩은 **문서의 85% 가 512 토큰 한계를 넘겨** 잘렸다).
  **남는 대가도 적는다** — *"summary 품질 편차가 남습니다"*. **⚠️ 다만 그 글은 사내 개발 도구다.**
  **본업 재료는 따로 있다** — 오배송 On-device 탐지(2026-04-28) · 입고 시스템 outbox 동기화(2026-01-12).
  **도메인을 열 때 이 둘부터 본다.**

**⚠️ 한국 회사가 이미 일곱이다**(NAVER · Kakao · Toss · 당근 · 배민 · 쿠팡 · 무신사). **둘을 더 올리면
아홉이 된다** — 편중이 분명하다. **다만 자리는 겹치지 않는다**(실시간 영상 매칭 · 신선식품 콜드체인).
**다음 후보 조사는 한국 밖에서 찾는다.**

**❌ 이번에 막힌 셋(🚫 확정)**
- **Cygames** `tech.cygames.co.jp` — **WebFetch 로도 브라우저 `get_page_text` 로도 본문이 안 나온다**
  (*"No text content found"*, `/archives/` 는 404). **두 번 다른 방법으로 시도해 두 번 다 실패했다.**
  **게임 자리를 메울 유일한 실마리였는데 접는다.**
- **Gojek** `www.gojek.io/blog` — **429 가 세 번째다**(246 에 두 번). 일시적 제한이 아닌 것으로 보인다.
- **Agoda** `careersatagoda.com/blog` — **403**.

**🚨 다음 사이클 — `--gaps` 가 [신규]를 가리킨다. `## 대기` 맨 위인 하이퍼커넥트를 `in_progress` 로
만들고 PROMPT.md 3단계(회사 프로파일)까지 판다. 46번째 회사다.**
**⚠️ 본문을 아직 한 편도 안 열었다** — 도메인 수를 정하기 전에 태그별 목록을 보고 한 편을 실제로 연다.
태그 URL: `/tag/machine-learning.html` · `/tag/kubernetes.html` · `/tag/recommender-system.html` ·
`/tag/flink.html` · `/tag/real-time-join.html`.
**확인된 글 URL** — `/2025/06/11/azar-flink-real-time-stream-join-service.html`(Flink 스트림 조인) ·
`/2025/08/26/azar-recsys-experiment-framework.html`(추천 실험 시스템) ·
`/2024/11/19/azar-recommendation-model.html`(아자르 추천 모델) ·
`/2025/05/15/Building-Resilient-High-Performance-ScyllaDB-Clusters-with-Super-Disk.html` ·
`/2023/07/25/migrate-half-of-workload-in-a-year.html`(ARM64) ·
`/2020/11/02/cluster-migration-in-5-days.html`.
**business_model 은 1차 자료로** — 하이퍼커넥트는 **Match Group 자회사**다(확인 필요). 상장사가 아니라
IR 이 없을 수 있다 — **못 찾으면 지어내지 말고 `open_questions` 에 남긴다.**
...
**비어 있는 자리** — 라틴아메리카 0(접었다 · 재시도 금지) · **게임 1(Roblox — Cygames 를 접어 실마리가
없다)** · 아프리카 1(Moniepoint) · 인도 1(Zerodha) · 여행 1(trivago). **동남아는 Grab 이 있다.**

**`hold_reason` 두 곳** — LinkedIn `랭킹 모델을 얼마나 빨리 다시 만들 수 있는가` ·
kakao `동기화·다중 기기`.

**📌 비교 문서 재료(27편 있음, 둘이 찼다 — 큐가 비면 이걸 쓴다)**
- **새 축 `관측에 값을 무엇으로 치르는가`** — CyberAgent `observability-pipeline`(돈: 관리형의 이중 과금
  **$0.095/GB + $0.10/GB** 를 피해 직접 굴리고 **관측 도구 자신의 로그를 관측에서 뺀다**) +
  CyberAgent `alert-context`(주의력: *"알림의 형해화"*) + Yelp `partition-access`(접근 로그가 **하루 TiB**) +
  Zerodha `log-storage`(색인 대신 압축 — **행당 340바이트에서 17.38바이트**, 시간당 **$2.67 → $1.14**) +
  Doximity `prod-profiling`(*"프로파일러는 … 첫 수로 삼고 싶어진다. 그러면 안 된다"*). **다섯이라 넉넉하다.**
- **`되돌릴 수 있는 것을 먼저 한다`(reversible-first)에 CyberAgent `env-split-migration` 이 반례** —
  그 문서는 *"되돌림을 실제로 쓴 기록이 거의 없다"* 로 끝나는데 **여기는 되돌림 계획 자체가 없다.**
