...
**없다 — 문체 보수 사이클이었다.** ScyllaDB 를 배민 문체로 다시 썼다(2.4 → 0.0). 회사 115개, 큐 2/3. 51곳 남았다.

### 남의 인터페이스를 그대로 받고 그 아래를 다시 짓는다 — 둘이 같다

ScyllaDB 는 카산드라와 DynamoDB 의 API 를 그대로 받아 주면서 그 아래를 다시 지었다.
이 엔진의 Turso 도 같은 자리인데, SQLite 의 SQL 방언과 파일 형식과 C API 를 묶어 두고
그 아래를 러스트로 다시 지었다.

둘 다 호환을 얻고 자유를 내주는 거래인데, 그 자유가 어디서 다시 아쉬워지는지가 다르다.
Turso 는 파일 형식이 영원한 제약으로 남는다. ScyllaDB 는 클라이언트 SDK 가 문제다.
DynamoDB SDK 가 엔드포인트 하나짜리 관리형 서비스를 겨냥해 만들어져서 모든 요청이 한 노드에
떨어지는데, 이 회사의 토대는 정반대로 모든 노드와 샤드가 동시에 일해야 값이 난다.

그래서 그냥 갈아 끼우면 된다는 약속에 단서가 붙는다. 애플리케이션을 안 고쳐도 된다는 값은
남지만 성능을 다 쓰려면 손을 대야 한다.

### 원칙이 대안을 거르는 자가 되는 회사

이 회사의 기술 이야기가 대부분 아무것도 공유하지 않는다는 원칙으로 돌아온다. 새 기능을 넣을
때마다 이것이 그 원칙을 깨는지를 먼저 묻고, 실제로 공유 io_uring 인스턴스를 성능이 아니라
철학을 위반한다는 이유로 거절했다.

그리고 자기 방식이 진 경우를 그대로 적는다. 새 백엔드가 순수 I/O 워크로드에서는 기존
방식만큼의 처리량을 내지 못한다고 밝힌다. 모르는 것을 어떻게 적나 축의 자료다.
