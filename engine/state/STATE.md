...
**없다 — Meesho 완주.** 도메인 4/4 · **기능 4개.** 회사 **90개.** ⚠️ **큐 2/3 이라 다음은 7순위(버퍼 보충) 또는 6순위(신규)다.**

### 이번 사이클 — `변형본이 어디 있는지 묻지 않으려고 경로를 계산한다`

건진 것 넷:

- ⚠️ **조회를 줄인 게 아니라 조회라는 계층을 없앴다** — **`주어진 이미지와 변형 조합의 경로가 언제나 같다면, 변형본이 어디 있는지 찾아볼 데이터베이스가 필요 없다.`** 대가는 **경로 규칙이 사실상 스키마가 되어 바꾸기 어렵다는 것**이다(재구성).
- ⚠️ **작고 명확한 설계가 규모에서 틀린 설계가 된 전형** — 이전 서비스는 **변형본이 이미 있는데도 매 요청마다 원본을 확인**했다. **가장 흔한 경로에 가장 불필요한 일이 붙어 있었다.**
- ⚠️ **직관과 반대로 간 자리** — 클라이언트가 끊겨도 **변환을 끝내고 캐시에 넣는다.** **`첫 요청이 비용을 치렀고, 두 번째 요청이 그 보상을 거둔다.`**
- ⚠️ **한계가 CPU 가 아니라 메모리에서 정해진다** — **프록시는 흘려보낼 수 있는데 변환기는 libvips 가 연속 바이트를 요구해 버퍼링을 피할 수 없다.** 스트림 하나가 약 4MB 라 세마포어로 묶는다.

**수치** — CDN 히트율 **약 99%**(초당 약 60만 요청이 서버에 안 온다) · **6MB → 750KB(8배)** · 프록시 약 6K RPS·변환기 약 660 RPS.

### ⚠️ 이 회사에서 네 도메인이 서로 물렸다

**한 사이클씩 파는 동안 연결이 드러났다** — **가짜 주소로 보내는 RTO 사기**(사기 도메인)와 **진짜 주소를 찾는 지오코딩**이 같은 지점에서 만나고, **공급자 카탈로그 사진**이 이미지 도메인에서는 보내는 대상인데 사기 도메인에서는 **공급자끼리를 잇는 엣지(BEiT)** 가 된다. 그리고 **RTO 는 결국 판매자 정산의 공제로 돌아온다**(회사가 직접 적는다). ⭐ **회사 하나를 여러 사이클에 걸쳐 파야 보이는 종류의 발견이 이번에 셋 나왔다.**

### 다음 사이클 — 큐 보충 또는 신규

`--gaps` 가 정한다. 큐에 **Zepto**(퀵커머스)와 **Snyk**(개발자 보안)이 있다. ⏳ **Swiggy 는 브라우저가 살아나면 다시 본다** — `r.jina.ai` 가 Medium 계열에는 422 를 낸다.

⚠️ **`r.jina.ai` + WebFetch 조합이 이번 회사 전체를 읽어 냈다** — 브라우저 `get_page_text` 가 세 사이클 연속 응답하지 않는 동안, **`verbatim 으로 인용하라` 를 지정한 WebFetch 가 원문 문장을 돌려줬다.** 이 조합을 기본 수단으로 삼아도 될 만큼 통했다.
