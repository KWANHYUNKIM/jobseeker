...
**Flipkart** (IN · 커머스) — 도메인 3개 · **기능 1개**(`at-least-once-then-reconcile`). 남은 둘은 **본문 미독**이다(검색 관련성 · 공급망).

### 이번 사이클 — `정확히 한 번을 포기하고 두 계층으로 갚는다`

⭐ **앞 사이클에서 `제목만 보고 세운 도메인` 이라고 적어 뒀는데, 본문이 그 이름과 정확히 맞았다.**

건진 것 넷:

- ⚠️ **이 도메인에서는 지연이 곧 돈이다** — **`광고 노출/클릭과 예산 차감 사이의 지연은 직접적인 금전적 위험을 뜻한다.`** 그리고 **세 압력이 서로 반대로 당긴다** — **초과 소진 · 미달 소진 · 중복 제거 세금.**
- ⚠️ **가장 옳아 보이는 보장을 거절했다** — **`정확히 한 번` 은 높은 지연을 도입하고 가용성을 해친다.** 그래서 **적어도 한 번**을 고르고 **틀릴 수 있음을 받아들인 뒤 나중에 고칠 자리를 따로 만들었다.**
- ⚠️ **계층을 나눠 둘 다 얻는다** — **`Flink 가 p99 지연을 강제하고, 병렬 배치 계층이 최종 정산의 권위 있는 진실 원천을 제공한다.`** ⭐ **Zepto 가 시간당 분석과 3분 지출 계산을 가른 것과 같은 답이다** — **`빠른 숫자와 정확한 숫자는 같은 작업에서 나오지 않는다`.**
- ⚠️ **애매하면 덜 쓰는 쪽으로 넘어진다** — 장애 중에는 **워터마크 페일세이프가 초과 소진을 막는다.** **초과는 광고주에게 돌려줘야 하는 돈이고 미달은 못 번 돈이라 성질이 다르다**(재구성). ⭐ **Razorpay 가 `중복을 감수하기보다 수집을 멈추는 쪽으로 치우쳐 있다` 고 적은 것과 같은 결이다.**

### ⚠️ 시스템의 시간 창을 회계가 정했다

**늦게 온 이벤트를 60분까지 받는데, 그 값이 기술에서 나온 것이 아니다** — **시간 단위 정산에 맞춘 값**이다. **모바일 앱이 백그라운드에서 도는 탓에 순서를 어기는 것이 기본인데, 어디서 끊을지는 회계가 정한다**(재구성).

### 다음 사이클 — 확장(2순위)

`--gaps` 가 **`검색이 무엇을 관련 있다고 할지 정한다`**(LLM 관련성 라벨링, 2026-07-31)를 부를 것이다. ⏳ **URL 은 목록에서 받아 뒀다.** ⭐ **Etsy 의 3단 증류(사람이 기준을 만들고 LLM 이 넓힌다)와 견줄 자리다.**

⚠️ **큐는 1/3**(Twilio). ⏳ **보강 거리 둘** — Grafana Labs 인용 대조 · Razorpay 보안 트리아지 글.
...
**③ `자기 성과를 어디까지 주장하나` 4곳** — Snyk / Grafana Labs / ScyllaDB / Razorpay.

**④ ⭐ `인도 규모에서 무엇이 달라지나` 4곳 — 가장 두꺼운 축이 됐다** — Meesho / Zepto / Razorpay / **Flipkart**.
...
⭐ **⑫ `빠른 숫자와 정확한 숫자를 어떻게 가르나` — 재료가 찼다** — **Flipkart**(속도 계층 + 배치 조정, 적어도 한 번) / **Zepto**(시간당 분석 + 3분 지출, 원본을 남기고 질의 시점 중복 제거) / ⏳ **Deliveroo**(불확실성 표현을 하나로 고정). ⚠️ **셋 다 `하나의 파이프라인으로는 안 된다` 에 도달했다.**

⏳ **⑬ 새로 보인다 — `애매할 때 어느 쪽으로 넘어지나`** — **Flipkart**(덜 쓰는 쪽) / **Razorpay**(수집을 멈추는 쪽) / **Zepto**(품절을 보여 주느니 마지막에 확인) / **Paystack**(금액이 틀리면 실패시킨다).
