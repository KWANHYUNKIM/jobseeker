...
**Mapbox**(지도 플랫폼) — 회사 프로파일까지 썼다. **도메인 하나 · 기능 0.** 큐 0/3.

### 파는 코드가 남의 기기에서 돈다

이 회사를 세우는 축이다. 지도 데이터만이 아니라 **그것을 그리는 코드**를 파는데, 그 코드는
고객의 브라우저와 폰에서 돈다. 그래서 **무엇을 내려받게 하느냐**(번들·time-to-idle)와
**어떻게 그리느냐**(프레임 시간·CPU·메모리)가 곧 상품성이 된다. 지도 로드와 MAU 로 값을
받으니 **느린 지도는 고객 이탈이자 이쪽 매출 감소**다.

### 고르게 하는 것과 둘 다 담는 것은 다르다

Vulkan 전환에서 OpenGL 백엔드를 남겨 개발자가 고르게 했지만, **자동 선택은 거절했다** —
**"자동 백엔드 선택은 두 백엔드를 모두 포함해야 하는데 그것이 바이너리 크기에 영향을
준다."** 선택지를 주는 것과 둘 다 실어 보내는 것을 갈라 놓은 자리다.

### 과금 단위가 제품마다 다르다

지도 로드 · MAU · 좌석 · 요청 · 타일 요청 · 세션 · 주행 건수 · 처리 MB · 컴퓨트 유닛 ·
호스팅 일수까지. **같은 타일인데 종류마다 28배 차이**가 난다(벡터 $0.25 대 실내 $7.00).
**영구 지오코딩만 무료 구간이 없고**, **지원 자체가 상품**이다(비즈니스는 월 지출의 10%).

### 도메인을 하나만 세웠다

블로그에서 찾은 자기 시스템 글이 **렌더링·번들 쪽 둘뿐**이다 — 타일 파이프라인도 지오코딩도
내비게이션 라우팅도 못 찾았다. **억지로 늘리지 않는다**(먹스·딥그램·메타베이스·고도와 같은
처리).

### 다음 사이클

도메인 하나에 기능이 없다 — `--gaps` 가 확장으로 부를 것이다. **Vulkan 쪽이 근거가
두껍다**(성능 이유를 구조로 · 지킨 것과 버린 것 · 거절 이유). 모듈화 글은 수치는 있지만
물린 대안이 없다.
