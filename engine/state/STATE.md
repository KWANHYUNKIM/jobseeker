...
**AuthZed 완주(기능 2개).** 회사 142개, 큐 1/3 — turbopuffer·Deno 가 남았다.

### 빠진 것은 알고리즘이 아니라 데이터의 모양이었다

재귀 스키마와 깊은 중첩, 수천수만을 잇는 관계에서 순진한 그래프 탐색이 무너졌다. 예전 평가에
없던 것은 **손님 데이터의 모양**이라 통계를 그래프 옆에 두고 조언자(정적·질의 통계)가 순서를
고르게 했다 — 화살표 뒤집기·가지 재정렬·회전·도달 가능성 가지치기.

### 수치를 두 갈래로 낸다

알고리즘만 재면 11배(772→70µs), 포스트그레스 저장소를 끼면 47%(19.55→10.41ms)다. **어디까지가
계획기 덕이고 어디부터가 아래층 몫인지**가 그 두 숫자에 드러난다.

### 자기 약점을 먼저 적는다

"질의 계획기는 95%에서 아주 잘 돌고 나머지 5%에서 이상하거나 완전히 미친 동작을 보이곤 한다" —
파는 제품의 소개 글에 이 문장이 있다. 지금은 하위 문제를 과하게 흩어 보낸다는 것, 통계 출처와
성숙도가 남았다는 것도 함께 적는다.

### 두 기능이 한 자원을 놓고 다툰다

계획기가 하위 문제를 흩어 캐시해 얻는 이득이, 부르는 쪽이 일관성 수준을 올리는 순간 사라진다.
**빠르게 훑는 일과 최신을 보는 일이 같은 캐시를 나눠 쓴다** — 이 회사를 한 줄로 꿰는 긴장이다.

### 다음 사이클 — 신규 (대기 맨 위 turbopuffer)

객체 저장소 위의 전문 검색과 벡터. BM25·MAXSCORE·SIMD 글이 이어진다.
