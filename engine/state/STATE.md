...
**아무 회사도 안 판다 — 후보 조사 사이클이었다.** 큐 0/3 → **3/3**. 회사 135개.

### 이번에 올린 셋

- **Railway**(US) — 남의 클라우드를 떠나 **자기 데이터센터와 자기 오케스트레이터**를 짓는다.
  Ubicloud 가 남의 베어메탈을 빌리는 쪽이라면 여기는 랙까지 직접 세운다. 2026년 심층 글이
  여섯 편 넘고 쿠버네티스를 안 쓴다고 못 박는다.
- **Jane Street**(US) — OCaml 을 고쳐 쓰고(OxCaml) ASIC 을 뜯는다. 저지연 트레이딩과
  언어 자체를 고치는 축이 이 엔진에 없었다. 사업이 안 보이는 회사라 수익원 서술은 얇게 잡는다.
- **Zig Software Foundation**(US·비영리) — 릴리스 노트가 사실상 설계 문서다. `Io` 를 인터페이스로
  바꾸고 아레나 할당자를 잠금 없이 다시 썼으며, **거절한 기능을 이름 대어 적는다.**

### 교훈 — 제목의 기술성도 판정 근거가 아니다

지난 조사에서 "최신성만 보면 낚인다"를 배웠는데 이번엔 한 겹 더다. Baseten 은 목록에서 가장
기술적으로 보이는 글을 열었더니 **수치가 하나도 없는 개념 정리**였다. 접은 넷 가운데 셋이
목록만으로는 통과했을 곳이다.

### 주소를 또 틀렸다

`railway.com/blog` 가 404 였다. 규칙대로 추측을 접고 검색으로 `blog.railway.com` 을 얻었다 —
시그널에서 겪은 것과 같은 종류의 실패다.

### 다음 사이클 — 신규 (대기 맨 위 Railway)
