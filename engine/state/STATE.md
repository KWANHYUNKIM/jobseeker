...
**없다 — 이번 사이클은 후보 조사였다. 큐를 0/3 에서 3/3 으로 채웠다.** 회사 147개.

### 올린 셋 — 축을 또 벌렸다

**Ferrous Systems(Ferrocene, EU)** — 이 엔진에 **인증받은 컴파일러** 축이 없었다. 자기 린트
(`ferrocene::unvalidated`)에서 시작해 러스트 질의 시스템의 버그 셋까지 파고, **우회하지 않고
상류를 고치기로 한 결정**을 적는다. **Rive(US)** — 스크립팅 언어를 고르며 거절한 대안 셋을
이유와 함께 적는다. **Hatena(JP)** — 이 엔진에 **Perl 을 아직 쓰는 곳**이 없었다.

### 접은 셋과 그 이유

**Bevy** — 릴리스 노트를 실제로 열어 봤는데 **기능 목록이었다.** **Proton** — 소비자 대상
글이 지배한다. **오늘의집** — 회사 페이지에 기술 블로그 링크가 없어 주소를 추측하지 않았다.

### 이번 조사의 교훈 둘

**형식이 아니라 대가가 판정 기준이다.** 지그 재단은 릴리스 노트에 거절한 기능을 이름 대어
적고 수치를 붙여서 후보가 됐는데, 베비는 같은 형식인데 대가가 없다. 텍스트 시스템을 통째로
갈아탄 대목조차 "문서가 낫고 쓰기 좋았다"에서 끝난다.

**글 한 편이 판정을 통과시키는 경우가 또 나왔다.** 프로토맵스가 "쓰지 말라"는 글 하나로
자격을 얻었듯, 라이브는 **거절한 대안 셋을 이유와 함께 적은 글 하나**로 통과했다. 편수가
아니라 그 한 편이 무엇을 담았는가다.

### 다음 사이클 — 신규 (대기 맨 위의 Ferrous Systems)

`Query cycles: A compiler murder mystery`(2026-07)와 `Callgraph analysis`(2026-04)가 손에
있다. **⚠️ 나머지는 릴리스·행사·채용 공고다** — 팔 때 기술 글만 고른다. 도메인은 인증
(무엇을 인증받았고 무엇이 인증 밖인가)과 컴파일러 수정 쪽으로 갈릴 것 같다(재구성).
