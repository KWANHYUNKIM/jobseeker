...
**37signals — 프로파일만 썼다(도메인 셋, 기능 0).** 회사 151개, 큐 2/3.

### 파는 것이 아니라 쓰는 것을 공개한다

이 엔진에서 드문 자리다. 2022년 클라우드 청구서를 **서비스별로 통째로** 냈다 — 합계 연
**3,201,564달러**, 그중 파일 저장(S3)이 907,838달러이고 HEY 한 제품이 1,066,150달러다.
그리고 줄이겠다고 적은 뒤 실제로 되돌렸다.

### 값만이 아니라 운영이 이유였다

되돌리기 글이 그걸 분명히 한다 — **얻는 것보다 쓰는 것이 많았고 그것이 운영에서도 그랬다.**
ECS·GKE·EKS 를 함께 다루며 쌓인 복잡도, 오토스케일러와 인그레스 컨트롤러 같은 딸린 시스템의
부담이 근거다. 쿠버네티스 대신 **복잡도가 낮은 MRSK(Kamal)** 를 골랐고 배포가 몇 분에서 약
1분이 됐다.

### 계산 다음이 저장이다

2026년에 **10페타바이트(중복 제거 5PB) · 파일 50억 개**를 S3 밖으로 옮겼다. **이중 쓰기**로
무중단, 협상한 90일 창에 실제 전송은 10일 미만. 그런데 **대조가 복사보다 자원을 더 먹었고**,
100기가비트 회선을 80기가비트 넘게 쓰자 다른 트래픽을 방해했다.

### 되돌린 것과 안 되돌린 것을 구분해 적었다

CDN·DNS 는 클라우드플레어로, CI 는 GitHub Actions 로 — **전부 되돌린 것이 아니다.** 이 구분을
`tech.limits` 에 그대로 적었다.

### 다음 사이클 — 확장 (첫 도메인)

`클라우드에서 나온다` 가 자료가 가장 두껍다(지출 글 + 되돌리기 글). **⚠️ 실제 절감액을 적은
글을 아직 못 찾았다** — 연말에 보고하겠다는 약속만 읽었으니, 확장 사이클에서 그 글을 먼저 찾는다.
없으면 없는 대로 `open_questions` 에 남긴다.
