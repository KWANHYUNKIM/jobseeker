...
**Wayfair (US · 커머스/대형 물품) — 44번째. 기능 4개 · 도메인 3개. 아직 `in_progress`.**

244 에서 **Data Science 태그를 처음 열었다.** 결과: **태그가 Computer Vision 과 거의 겹친다** —
1쪽 9편이 이미 읽은 것들이었다. **"288 total results" 는 태그 카운터일 뿐 새 재료가 아니다.**
새로 보인 것은 셋뿐 — 고객 해결책 추천(2026-07) · **Wilma**(공급업체 이관 개입, 2026-01) ·
개발 도구 글(결이 다르다). **`how-we-used-ai-as-a-lab-assistant`(태그 검증 94% 절감)는 1쪽에 없었다.**

**두 편이 묶여서 세 번째 도메인을 열었다 — `주문 뒤에 생긴 문제를 누가 어떻게 풀 것인가`.**
같은 일을 **누가 하느냐**로 갈린다: Wayfair 상담원이 처리하면 해결책 추천, **공급업체가 직접
처리하면(STP)** Wilma 가 그 대화를 지켜본다. STP 정의가 그 대비를 만든다 — *"공급업체가 고객과
상호작용하고, 주문 후 문제를 진단하고, 적절한 해결책을 처리한다."* **둘 다 사람을 대체하지 않는다.**

**기능 `resolution-choice` 를 썼다.** 결정 6 · 그림 3 · 생각 4 · 스택 3 · 난제 2 · ui 4핀(`web`).

**🔴 이 회사에 대한 관찰 하나가 깨졌다 — 정정한다.**
"같은 문제를 다루는 시스템들이 서로를 언급하지 않는다"고 다섯 편을 근거로 적었는데,
**해결책 추천 글이 Saffron 을 본문에서 링크로 건다.** 나머지 짝들(창고↔고객 사진, 시각 탐색↔
고객 관심, 치수↔손상)은 여전히 안 잇는다. **앞 단계의 판정이 뒷 단계의 입력이 되는 자리에서는
잇는다** — 나란한 시스템은 안 잇고, 이어지는 시스템은 잇는다(이 정리는 이 사이트의 것).

**🔴 이 기능의 축은 정확도가 아니라 데이터의 모양이다.** 회사가 두 문장으로 적는다 —
*"우리 과거 데이터는 불완전하다. 최종적으로 선택된 해결책만 기록한다"*, *"다른 해결책을 골랐다면
결과가 어땠을지 우리는 모른다."* **반사실이 없다.** 그래서 모델이 배우는 것이 *무엇이 옳았나* 가
아니라 **과거 상담원이 무엇을 골랐나** 에 가까워지고, 회사는 **고르지 않고 좁히는** 쪽을 골랐다 —
*"명백히 더 나은 대안이 있을 때만 후보에서 제거된다."* **정답을 모르니 오답을 지운 셈이다.**
개선폭도 그만큼이다 — **FCR 약 2% 상대 개선**(고객은 수억 명이지만 **회사가 곱셈을 안 했으므로
이 사이트도 안 한다**).

**🚨 다음 사이클 — Wilma 를 쓰고 Wayfair 를 닫는다.**
`flag-early-fix-faster-real-time-intervention-for-supplier-transfers-with-wilma`(2026-01-15).
**⚠️ 본문을 아직 안 읽었다.** 확인된 것은 **초기 70% 정확도 설계에서 모델 환각**(*"댓글 작성자에게
해당되지 않는 개입 사유 코드를 잘못 예측"*)과 스택(**LangGraph · Pub/Sub · SupportHub · Jinja2 ·
Arize · MCP**)뿐이다. **요약에 65% · 80% · 90% · 95% · 97% · 99.8% · 99.7% · 29% · 3.1 · 3.8 ·
96% · 2.8% 이 나오는데 각각이 무엇의 값인지 모른다 — 원문을 반드시 다시 읽는다.**
**⚠️ 버린 대안이 없을 가능성이 높다** — 이 회사의 성격이다. **결정 5개가 안 나오면 억지로 쓰지
말고 도메인 `tech` 에 남긴 채 닫는다.**

**닫을 때 `business_model` 패턴을 판단한다.** 네 기능이 이렇게 읽힌다(가설) — **소프트웨어가
사람의 말과 물리 세계 사이의 간극을 판정한다**(손상·치수·분류) **그리고 그 판정이 정해지면
무엇을 해 줄지를 또 판정한다**(해결책). **적는다면 이 사이트의 정리라고 밝히고, 얇으면 안 적는다.**

**📌 이 회사에서 여섯 편을 읽고 굳어진 관찰**
- **버린 대안을 거의 안 적는다** — 여섯 편 중 **치수 검증 한 편만** 숫자로 비교했다.
- **대가의 밀도가 글마다 크게 다르다** — 창고 손상(여섯 줄) · 치수(다섯 줄) · **해결책 추천
  (다섯 줄)** 은 풍부한데, 고객 사진 진단은 **수치가 하나도 없고** 시각 탐색·고객 관심은
  **새 시스템의 한계가 한 줄도 없다.**
- **나란한 시스템은 서로를 안 잇지만 이어지는 시스템은 잇는다**(위 정정 참고).

**⚠️ 큐가 0 이다 — Wayfair 를 닫으면 곧바로 후보 조사다.**

**아직 안 열어 본 후보** — SmartNews · PayPay · **CyberAgent 의 ABEMA 태그** · Sea/Shopee ·
GoTo/Gojek · Tokopedia · Traveloka · Robinhood · Plaid · Ramp · Adevinta · Revolut · Skyscanner.
**인프라(Vercel·Neon·Elastic·MongoDB·Confluent)는 뒤로**(238 의 관찰).
...
④ **회사를 새로 열 때는 중복 검사부터.**
⑤ **태그 목록의 총 건수를 새 재료의 양으로 읽지 않는다** — 244 에서 태그가 거의 겹쳤다.
