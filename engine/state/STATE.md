...
**Mozilla**(브라우저) — 회사 프로파일까지 썼다. **도메인 둘 · 기능 0.** 큐 0/3.

### 안에서 곪는 것과 밖에서 들어오는 것

이 회사가 동시에 감당하는 두 가지가 그대로 도메인이 됐다. 하나는 **20년 넘게 쌓인 코드**
안의 잠복 취약점(15년 된 `<legend>` 버그 · 20년 된 XSLT)이고, 다른 하나는 **제3자가 밀어
넣는 DLL** 이다.

### 막기만 하면 떠난다

DLL 주입 글의 핵심은 막는 방법이 아니라 이 인식이다 — 관리자에게 쓸 만한 메커니즘이 없으면
**"DLL 주입 같은 불투명하고 오류가 나기 쉬운 방법에 손을 대거나, Firefox 를 다른 브라우저로
바꿔 버린다."** 그래서 Firefox 138 부터 **주입 없이 통신하는 길**(Content Analysis SDK)을
열었다. **대신할 길을 줘야 막을 수 있다.**

### 자기 릴리스 속도가 문제의 원인이다

주입된 코드는 **문서화되지 않은 내부에 기댄다.** 그런데 Firefox 는 **월 단위로** 릴리스하며
그 내부를 계속 고친다 — **주기가 짧을수록 밀어 넣은 코드가 자주 깨진다.** 문제의 절반이
자기 쪽 속도에서 온다.

### 찾은 개수보다 받는 쪽의 비용

하드닝 글의 알맹이는 271개가 아니라 **비대칭 비용**이다 — 그럴듯하지만 틀린 보고를 처리하는
일이 관리자에게 부담을 지운다. **도구가 싸질수록 받는 쪽이 비싸진다.**

### 다음 사이클

빈 도메인 둘 중 하나. **하드닝 쪽이 수치가 두껍다**(271개 · sec-high 180 · 15년·20년 묵은
버그 · 물린 정적 분석 · 100명). DLL 쪽은 **수치가 하나도 없지만** 통찰이 선명하다.
