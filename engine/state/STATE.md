...
**없다 — 문체 보수 사이클이었다.** Twilio 를 배민 문체로 다시 썼다(3.57 → 0.0). 회사 115개, 큐 2/3. 64곳 남았다.

### 낡은 것을 어떻게 바꾸나 — 이 축이 데이터에 들어갔다

Twilio 는 이 축의 좋은 자리다. 공개된 자기 시스템 이야기 둘이 서로 반대 방향인데, Segment 의
알림 서비스는 모놀리스를 세 책임으로 쪼갰고 콘솔은 마이크로 프론트엔드를 다시 합쳤다.
그런데 방법은 둘 다 같다 — 한 번에 갈아엎지 않는다.

이 엔진의 다른 회사들과 견주면 갈래가 보인다.

- Twilio 는 이중 쓰기를 유지하며 한 조각씩 옮긴다(Strangler Fig).
- Klaviyo 는 RabbitMQ 를 Kafka 로 무중단 이관했다.
- Atlassian 은 색인 변경을 의도적으로 느리게 돌렸다.
- Modal 과 Turso 는 아예 처음부터 다시 지었다.

앞의 셋은 옮기는 쪽이고 뒤의 둘은 다시 짓는 쪽이다. Turso 가 다시 지은 이유가 특히 다른데,
성능이 아니라 남의 테스트 스위트를 물려받을 수 없어서였다.

### 이번 사이클 — Twilio

사업 모델에서 자기 시스템 이야기 둘이 반대 방향이라는 것을 앞세웠다. 그리고 도메인 경계의
근거를 적었다 — 쪼개는 일과 옮기는 일을 따로 볼까 했으나 이중 쓰기가 그 둘을 한 절차로 묶어
한 경계로 뒀다.

기능 이름에 어느 쪽 이야기인지를 붙였다. `의도와 실행 사이에 선을 긋는다 — 모놀리스를 쪼갠 쪽`,
`자율을 주면 격차가 생긴다 — 쪼갠 것을 다시 합친 쪽`.

인용을 풀면서 「빅뱅」 같은 일본식 낫표와 무엇/어떻게 같은 한 낱말 인용이 특히 어색해졌다.
무엇을 할지와 어떻게 할지로 풀어 문장에 녹였다.
