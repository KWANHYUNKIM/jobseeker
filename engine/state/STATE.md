...
**없다 — 후보 조사 사이클이었다.** 큐를 **0/3 → 3/3** 으로 채웠다(**목표 달성**). 다음 사이클은 `--gaps` 가 **신규(6순위)**를 부른다 — 대기 맨 위는 **ClickHouse** 다.

### 이번 사이클 — 후보 조사 (열일곱 번째)

**세 곳을 올렸고 그중 둘은 앞선 판정을 뒤집은 것이다.**

- **✅ ClickHouse (❌ → ✅)** — 앞 조사에서 떨어뜨리며 **`ai-first-data-warehouse` 는 결이 다를 수 있다** 고 주소까지 남겨 뒀고, **그 메모가 맞았다.** 자기 내부 창고 **2.1PB** 이야기다.
- **✅ Duolingo (❌ → ✅)** — ⚠️ **두 편을 읽고 판정했는데 그것으로 부족했다.** `⏳ 안 읽은 두 편` 이라 적어 둔 쪽에 **버린 대안과 대가와 사고가 다 있었다.**
- **✅ Ramp** — 새 이름. ⚠️ **주소를 추측했다가 404 를 받고 검색으로 찾았다**(`engineering.ramp.com`).

### 이번에 굳힌 것 둘

⚠️ **`남겨 둔 메모` 가 이 엔진에서 가장 수확이 좋은 자산이다.** 이번 두 개가 다 그렇게 살아났고, 앞서 **Temporal**(안 읽은 한 편)·**Booking.com·Trainline**(못 읽는다고 적어 둔 목록)·**Tailscale·Ably**(첫 화면으로 판정한 의심 목록)도 그랬다. **떨어뜨릴 때 무엇을 안 읽었는지 적는 습관이 몇 사이클 뒤에 회사를 돌려준다.**

⚠️ **`두 편을 읽었으니 충분하다` 가 틀렸다.** Duolingo 를 두 편으로 판정했는데 나머지 둘에 답이 있었다. **한 회사 안에서 글의 결이 갈린다는 것을 여덟 번 확인해 놓고도 표본 수를 늘리지 않았다.** 앞으로 **떨어뜨릴 때는 남은 글의 제목을 반드시 본다.**

### 다음 사이클 — 신규(ClickHouse)

⚠️ **큐의 세 곳이 모두 `사내 AI 에이전트` 축이라 서로 겹친다.** 보통은 피하지만 이번에는 그것이 값이다 — **Sentry · Duolingo · Ramp · ClickHouse 가 `AI 를 어디까지 믿을 것인가` 에 각각 다른 선을 그었다**(병합은 사람이 / 리뷰가 병목 / 병합 PR 의 30% / 분석의 70%, 나머지 30%는 일부러 사람에게). **네 곳이 채워지면 비교 문서(8순위)의 재료가 된다.**

⏳ **Capital One 은 세 번 확인하고 세 번 미달이다**(마지막 주소는 404). **다음에 `## 재시도 안 함` 으로 옮긴다.**
