...
**turbopuffer — 기능 하나를 채웠다(`object-storage-first-engine`).** 빈 도메인 둘. 회사 143개, 큐 1/3.

### 두 자릿수 차이는 최적화로 못 메운다

문서 1억 건에 벡터 검색을 붙이려니 월 2만 달러가 넘었다(관계형 데이터베이스는 5천 달러).
원인은 구조다 — 색인을 램에 두면 **GB 당 2달러 이상, 객체 저장소는 0.02달러**다. 묶어서 보면
램 캐시+SSD 3중이 TB·월 1,600달러, S3+SSD 캐시가 70달러다.

### 계층화를 물린 이유

차가운 데이터만 나중에 객체 저장소로 내리는 방식으로는 **뜨거운 쪽 값이 그대로**다. 진실 자체를
거기 두지 않으면 값의 바닥이 안 내려간다 — 그래서 저장 엔진을 처음부터 다시 만들었다.

### 내주는 것의 크기를 적는다

"값을 위해 지연을 내준다"고 쓰고 그 크기를 밝힌다 — 차가운 벡터 질의 p90 444밀리초, 따뜻한
p90 10밀리초. 왕복 한 번이 약 100밀리초라 **차가운 질의를 왕복 세 번으로 묶어** 최악에 상한을
건다. 대부분의 검색 용도에서는 절약이 지연을 감수할 만하다고 말한다.

### 싼 저장은 작은 쓰기를 싫어한다

객체 저장소는 쓰기 횟수에 값이 붙으니 LSM 으로 모아 내려쓰고 네임스페이스당 초당 WAL 한
항목으로 묶는다 — 커밋까지 최대 1초라는 대가가 거기서 나온다.

### 다음 사이클 — 확장 (turbopuffer 의 빈 도메인)

`차가운 질의와 따뜻한 질의를 갈라 판다`. 세 층 캐시·강한 일관성 기본값·최종 일관성의 한 시간
낡음이 손에 있다.
