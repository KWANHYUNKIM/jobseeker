...
**없다 — 일흔여섯 번째 후보 조사를 돌았다. 큐 2/3.** 다음 사이클은 **신규**(헨리).

### 사각지대의 경계를 확정했다

앞 사이클에서 얻은 방법(**WebFetch 가 막으면 curl**)을 medium 에 시험했다. 결과는
**Cloudflare 차단 페이지**다 — **medium 과 리디는 이 도구로 못 읽는 것이 확정**이다.
**다음부터 다시 두드리지 않는다.** 못 여는 것을 못 연다고 확정해 두는 것도 기록이다.

### 일본 후보는 하테나부터 본다

이번에 네 곳(Classi·Henry·Kakehashi·BASE)이 **전부 사이트맵 200**. 쿡패드·Gunosy·엠쓰리에
이어 **패턴이 자리를 잡았다** — 일본 회사를 볼 때 하테나 블로그인지부터 확인한다.

### 앞 사이클에서 열어 둔 물음이 다음 회사에서 채워졌다

엠쓰리를 완주하며 **'의료 도메인 자체의 기술 글은 못 봤다 — 규제가 기술에 어떻게 걸리는지
열려 있다'** 고 적었는데, **헨리가 정확히 그 자리**다. 개인정보보호법 개정을 놓고 **위탁과
제3자 제공을 가르고**, **법무 문서를 '프로덕트 그 자체의 일부'** 로 둔다.

### 멈춰 있던 이유를 적는 글은 드물다

Classi 가 리아키텍트 프로젝트를 **왜 몇 년 멈춰 있었는지**부터 쓴다 — **핵심 인력이
로테이션될 때마다 동력을 잃었고, 한 사람의 지속적 집중에 기대지 않는 중간 목표가
없었다.** 실패의 원인을 **조직 구조**에서 찾고, 고침도 **목표를 쪼개는 것**으로 한다.

### 다음 사이클

**신규 — 헨리.** 큐 2/3 이라 신규 두 번 뒤에 조사가 다시 뜬다.
