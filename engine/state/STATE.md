...
**Doximity — 40번째 회사.** 사이클 212 까지 프로파일 + `ui_map` + **도메인 3개** + **기능 1개**(`cdc-overlap`).

> **`cdc-overlap` 의 축 — 이 파이프라인은 중복은 견디고 빠짐은 못 견딘다. 그래서 모든 결정이 겹치는 쪽으로 기운다.**
> 회사가 인계 지점에서 그것을 한 문장으로 적는다 — 겹침은 *"결국 수렴"* 하므로 받아들이고,
> *"스냅샷이 가장 오래된 보존 이벤트보다 오래돼서 어느 쪽에도 없는 변경이 생기는 상황은 견딜 수 없다"*.
> **겹친 것은 시간이 지우지만 빠진 것은 시간이 못 만든다**(이 대비는 이 사이트의 정리다).
> 나머지 결정도 같은 기준이다 — 미룬 병합은 **뷰가 덮어 감추고**, binlog 에 **없는 삭제는 만들어 내며**,
> 만들지 못한 것은 **운영이 지켜본다**(건너뛴 캐스케이드 모니터링).

> **눈에 걸린 것 셋** — ① **정확성 책임이 뷰로 옮겨 갔다**(*"갱신·삭제·이벤트 순서·스키마 변경이
> 병합 전과 후에 같은 결과를 내야 한다"*) — 미루는 설계의 진짜 값은 읽기 증폭이 아니라 이것이다.
> ② **CDC 가 스냅샷을 대체하지 않았다** — *"관련된 두 CDC 테이블이 정확히 같은 순간을 나타내지는
> 않을 수 있다."* **신선함과 시점 일관성은 다른 물건**이라 두 벌을 함께 굴린다(`hard_problems` 로 남겼다).
> ③ **측정 범위를 회사가 스스로 좁혀 적는다** — 95%/8분·99%/13분은 *"Kafka 발행 전 시간이 들어가지 않고
> 미뤄진 물리 병합도 기다리지 않는"* 값이다. **좋아 보이는 숫자의 경계를 자기가 그었다.**

> **📌 '두 번째 글을 읽는다' 규칙이 또 값을 했다** — `a-smarter-way-to-deploy-data-pipelines` 에서
> **버린 대안(git-sync — *"여러 저장소를 다루는 한계"*)과 수치(20개 팀 · 900+ DAG · 배포 20~30분 → 1분 미만)**
> 가 나와 결정 하나를 더 세울 수 있었다. 세 사이클 연속이다(209 · 210 · 212).

> **다음 사이클** — 도메인 **`루비 모놀리스를 프로덕션에서 굴린다`** 의 기능 하나.
> 재료는 **본문을 이미 확인한** `profiling-performance-bottlenecks-in-production`
> (계측형 프로파일러를 버린 이유 · 잡 p95 **214초 → 69초** · 4.5만 건 **8.95초 → 1.44초**).
> **⚠️ 규칙대로 두 번째 글을 함께 읽는다** — `modularizing-rails-monoliths-one-bite-at-a-time` 이나
> `5-sre-tips-for-rails-workloads-in-kubernetes` · `taking-our-ruby-kafka-stack-to-the-next-level`.

> **📌 Doximity 를 완주하면 큐가 1/3(Moniepoint 뿐)이라 후보 조사가 필요하다.**
> **여전히 라틴아메리카 0곳 · 물류 전문 0곳**, 게임은 Roblox 하나.
> `임상 현장의 AI` 는 읽은 한 편에 **버린 대안이 없어** 기능 전에 두 편을 더 읽어야 한다.

**큐 1/3** — Moniepoint(NG · 핀테크).

마지막 사이클: 212 (2026-08-27) — Doximity 중복은 견디고 빠짐은 못 견딘다.
