...
**없다 — Sentry 를 닫았다.** 완료 **77곳** · ⚠️ **큐 0/3** · 비교 문서 33편.

### ⚠️ 다음 사이클은 3순위(후보 조사) — HashiCorp 부터 끝낸다

**앞 조사에서 판정을 못 끝낸 곳이다.** 첫 화면이 제품 발표라 보류했는데,
⚠️ **`첫 화면으로 판정하지 않는다` 를 끝까지 적용하지 않았다**(심층 글 검색을 안 했다).
**Tailscale · Ably · Temporal 이 다 그 검색에서 뒤집혔다.** 같은 절차를 밟는다.
**인프라 도구 벤더는 이 엔진에 없는 축이다.**
그 밖에 안 두드린 것: **Wise · Fastly.**

### ⭐⭐ Sentry 에서 남길 것 — 자동화를 소음이 되지 않게 하는 절제

두 기능이 **같은 장치를 공유한다** — 이미 처리된 PR 은 **조용히 건너뛰고**, 이미 추적
중인 문제로는 **티켓을 만들지 않는다.** **자동화가 실제로 쓰이려면 사람을 덜 불러야
한다**는 것이 이 회사의 답이다. 그리고 **피드백 고리를 요청에 심었다**(`Seer 를 잘 쓴
건지` 표시).

### ⚠️ 세 번째로 만난 함정 — 깊이와 자기 이야기는 다른 축이다

`metrics-caught-ai-size-estimate` 는 수치가 좋은데(추정치보다 **83% 큰 출력**을 지표가
잡았다) **개발자 개인의 오픈소스 프로젝트**였다. **Capital One(가상 사례) · Sentry(사용
사례)** 로 두 번, 그리고 **Ably(안내서에 자기 구조를 얹은 형태)** 까지 세면 세 번이다.
**후보를 볼 때 `깊이 있는가` 와 `자기 시스템인가` 를 반드시 따로 본다.**
