...
**ZOZO(JP · 커머스) — 도메인 2 · 기능 2.** `in_progress` ·
`fuzzy-search`(애매한 검색 · 결정 8) + `model-drift-watch`(빌린 추천 모델 감시 · 결정 7) ·
그림 6 · 새 도메인 `빌린 모델이 조용히 나빠질 때`. **완료 60곳 + ZOZO = 61곳 ·
큐 대기 2(쏘카 · 카카오뱅크) · 비교 문서 33편.**

### ⚠️ `--gaps` 와 PROMPT 사다리가 어긋난다 — 사다리를 따랐다

`--gaps` 는 **신규**(6순위)를 가리켰는데, PROMPT 2단계 사다리의 **2순위 조건은
`status: in_progress` 인 회사가 있다** 이고 ZOZO 가 거기 해당한다. **도구는 '빈 도메인' 만
세느라** 도메인이 하나뿐인 회사를 확장 대상으로 못 잡는다. **자료가 더 있는데 회사를
갈아타면 얕게 훑은 데이터가 된다**(PROMPT: 확장 중에는 회사를 갈아타지 않는다).
**다음 실행도 같은 자리에서 같은 판단을 하게 된다 — 사다리가 우선이다.**

### 다음 사이클 — ZOZO 를 닫을지 여기서 정한다

**`Thompson Samplingのコールドスタート対策`(2026-08-31, `/entry/thompson-sampling-cold-start`)
하나만 읽으면 된다.**
- **프로덕션에서 도는 것이면** → 세 번째 도메인(**탐색과 활용** 축)을 열고 기능 하나를 판 뒤 `done`.
- **오프라인 검증뿐이면** → 열지 말고 `open_questions` 에 적고 **ZOZO 를 `done` 으로 닫는다.**
  완주 기준은 이미 충족했다 — 도메인 2개가 각각 기능 1개 이상, 고아 없음, 수익원 연결됨.

**⚠️ 랭킹 도메인은 열지 않았다.** `日本語CLIPを活用した検索ランキング学習の精度改善検証`
(2026-09-01)은 **오프라인 정확도 검증**이고 프로덕션 도입·지연·비용 서술이 없다 —
**실행 안 한 평가는 기능으로 못 쓴다**(353). 확인된 내용은 `open_questions` 에 옮겨 뒀으니
**배포가 확인되면 그때 연다.**

### 이번 사이클에서 배운 것

- **`techblog.zozo.com` 은 제목만으로 판단하면 안 된다.** 「精度改善検証」은 검증이고
  「構築と運用」은 운영이다. **제목의 마지막 낱말이 기능으로 쓸 수 있는지를 가른다.**
- **이 회사 글은 자기 한계를 적는다.** 감시 글은 **"종류·분포·건수가 모두 정상인 채 추천의
  내용만 열화하는 경우는 추천 API 의 로그로는 검지할 수 없다"** 를 스스로 적었다 —
  그 문장 하나가 `hard_problems` 의 `current_best` 가 감수하는 것이 됐다.
- **관리형 서비스를 쓰는 회사는 '감시'가 도메인이 된다.** 빌리면 내부가 안 보이고,
  안 보이는 것을 밖에서 재는 일이 따로 시스템이 된다. **다른 회사에서도 같은 축을 찾을 수 있다.**

### ZOZO 자료 접근 — 다음 실행이 헛돌지 않게

- **기술 블로그 `techblog.zozo.com` 은 WebFetch 로 본문까지 열린다.** `/archive` 로 글 목록과
  URL·날짜를 받을 수 있다. **일본어로 질문하면 원문 인용이 잘 온다.**
- **⚠️ IR 은 막혀 있다.** `corp.zozo.com` 의 결산단신 PDF 는 **압축·인코딩되어 텍스트가 안 나오고**,
  IR 페이지는 **링크만 있고 표가 없다.** 그래서 **매출·영업이익은 유통 전문지(`ryutsuu.biz`) 기사에서 옮겼다.**
- **취급고 숫자가 둘이다**(6,660억 총 / 6,039억 ZOZOTOWN+LINE야후+BtoB). 대조 못 했다 — `open_questions` 에 적었다.
...
(`7bf8079`). `guide-engine` 은 포티투닷을 계속 커밋 중이다.
