...
**Wayfair (US · 커머스/대형 물품) — 44번째. 회사 프로파일까지 썼다. 기능은 0개다.**

**중복 검사를 먼저 했다**(226 의 교훈) — `index.json` 에도 `companies/` 에도 없었다.

**⚠️ 태그별 목록이 URL 로 열린다** — `?tag=<이름>&p=<쪽>`. 이걸로 묶음을 셌다.
태그 편수는 **Data Science 90 · ML/AI 68 · Computer Vision 10 · Cloud 8**, 최신 글 **2026-07-17**.

**도메인 2개. 둘 다 본문을 한 편씩 읽고 열었다.**
1. **`창고와 사진에서 물건의 상태를 판정한다`** — Computer Vision 태그에 **같은 주제 글이 셋**이다:
   **창고 컨베이어 포장 손상**(2026-03, 본문 확인) · **고객 사진으로 손상 진단**(2026-01, 미독) ·
   **멀티모달 GenAI 로 제품 치수 검증**(2026-01, 미독). **나가기 전 · 도착한 뒤 · 등록될 때 —
   같은 물음을 세 자리에서 묻는다.**
2. **`분류가 아니라 생김새로 찾게 한다`** — `seeing-beyond-labels`(2026-06, 본문 확인) +
   `smarter-shopping-starts-here`(2025-12, 미독).

**이 회사의 자리** — **파는 것이 크고 무겁고 잘 깨진다.** Target 은 매장이 배송 거점이고
쿠팡은 자체 물류센터인데, 여기는 **물건의 물리적 상태를 판정하는 일이 소프트웨어 도메인**이 된다.
그리고 **카테고리 경계 자체가 쇼핑을 방해한다** — *"사용자의 사진을 러브시트로 분류하면
검색을 그 카테고리로만 제한한다."*

**IR 에서 눈에 걸린 것** — FY2025 매출 **$12.5B**(+5.1%)인데 **활성 고객은 2,130만으로 0.5%
줄었다.** 그 자리를 **고객당 LTM 매출 $586**(+5.6%)이 메운다. *"적게 사지만 비싼 것을 산다"*
로 읽었는데 **회사가 그 이유를 적지 않아 이 사이트의 해석이라고 밝혔다.**

**🚨 다음 사이클 — 도메인 `창고와 사진에서…` 의 첫 기능.**
**⚠️ 쓰기 전에 `vision-language-models-in-the-wild-damage-diagnostics-from-customer-photos`
(2026-01-30)를 반드시 읽는다** — 기능은 그 도메인의 글을 두 편 이상 읽고 쓴다(209~229 에서
열세 번 확인된 규칙). 포장 손상 글은 이미 읽었다. **두 글이 같은 문제를 앞뒤에서 보는 것이
확인되면 축이 선다.**

**⚠️ 큐가 0 이다 — Wayfair 를 완주하면 곧바로 후보 조사다.**

**아직 안 열어 본 후보** — SmartNews · PayPay · **CyberAgent 의 ABEMA 태그**(블로그는 활발한데
연 글이 얇았다) · Sea/Shopee · GoTo/Gojek · Tokopedia · Traveloka · Robinhood · Plaid · Ramp ·
Adevinta · Revolut · Skyscanner. **인프라(Vercel·Neon·Elastic·MongoDB·Confluent)는 뒤로 미룬다**
— 238 의 관찰대로 개발자 대상 제품 회사의 블로그는 대개 영업 채널이다.
