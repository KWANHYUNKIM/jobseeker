...
**Antithesis — 기능 둘을 채웠다.** `deterministic-hypervisor` 와 `ordinary-workload-rare-race`
(평범한 작업만 돌려 2010년부터 숨어 있던 경쟁을 깨운다). 회사 119개, 큐 1/3. 빈 도메인 하나 — 되돌려 보기.

### 특별한 것은 작업이 아니라 순서였다

SQLite 의 WAL 재설정 경쟁을 찾은 방식이 이 도메인의 요지를 그대로 보인다 — **결함을 따로 주입하지
않았다.** 운영에서 늘 일어나는 동시 쓰기와 체크포인트만 돌렸고, 확인한 속성도 커밋된 쓰기가 사라지지
않는다와 손상되지 않는다 둘이었다. 그 경쟁은 2010년부터 있었고 SQLite 팀은 수정을 확인하려고 그
상황을 일부러 일으키는 시험 로직을 따로 넣어야 했다. 여기서는 첫 실행 15분이었다.

### 이 엔진의 회사가 피해자로 등장했다

**Tailscale 이 그 버그로 반년 동안 가동 문제를 겪은 뒤에야 SQLite 팀이 알게 됐다.** 같은 버그를
두 곳이 다르게 만난 셈인데, 차이를 만든 것은 알고리즘이 아니라 그 세계가 다시 돌릴 수 있는
세계였다는 점이다. 앞 기능(하이퍼바이저)이 왜 그 값을 치렀는지가 여기서 증명된다.

### 찾지 못하는 것을 세 갈래로 갈랐다

속성에 없으면 지나가고, 워크로드가 안 밟으면 지나가고, 방 안에서 일어날 수 없는 일이면 지나간다.
그리고 **탐색이 무엇을 신호로 삼는지 이 회사는 공개하지 않는다** — 못 찾은 것이 없어서인지 안
봐서인지를 밖에서 구분할 수 없고, 그 자리를 찾은 버그를 계속 내놓는 것으로 메운다(재구성).

### 다음 사이클 — 확장 (마지막 빈 도메인)

`Debugging in the Multiverse` 를 이번에 읽어 뒀다. 되감아 죽은 프로세스에 디버거를 붙이고,
처음부터 잡고 있었던 셈 치고 네트워크 덤프를 뒤늦게 받고, 결함 주입을 꺼 놓고 같은 지점을 다시
밟아 변인을 가르는 이야기다. 그것으로 마지막 도메인을 채우면 Antithesis 가 done 이 된다.
