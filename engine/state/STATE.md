...
**Doximity — 40번째 회사.** 사이클 213 까지 프로파일 + `ui_map` + **도메인 3개** + **기능 2개**
(`cdc-overlap` · `prod-profiling`).

> **`prod-profiling` 의 축 — 재는 도구가 답을 주지 않는다. 같은 그림을 다른 축으로 읽어야 답이 나온다.**
> 플레임그래프를 *"약 40% / 60%"* 로 읽으면 손댈 곳이 없는데, **"그 두 단계가 각각 60번 돈다"**
> 를 알아채자 *"다른 그림이 떠올랐다"*. 그리고 **원인은 느린 코드가 아니라 캐시가 놓인 층**이었다 —
> *"한 단계 낮은 데 걸린 캐시"*. 레코드마다 새로 만들어지는 객체 안의 `||=` 는 캐시가 아니다.
> 부모로 올리자 **Redis 조회 4.5만 번 → 1번**, 잡 **p95 214초 → 69초**.

> **눈에 걸린 것 넷** — ① **재현이 안 돼서 프로덕션에서 쟀는데, 재는 행위가 읽기 전용이 아니다**
> (*"실제 DB 에 10만 건을 만드는 잡을 프로파일링하는 것은 읽기 전용 작업이 아니다"*).
> ② **계측형은 *"싼 것들을 비싸 보이게"* 만든다** — 정확도를 팔아 갈 수 있는 곳을 샀다.
> ③ **진단과 검증을 분리했다** — 프로파일러를 끄고 다시 쟀다. ④ **관점 전환이 답을 주지는 않는다** —
> 반복을 보고 처음 떠올린 답은 병렬화였지만 실제로 고친 것은 캐시의 층이었다.
> **관점은 답이 있는 자리를 알려 줄 뿐이다**(이 정리는 이 사이트의 것이다).

> **📌 '두 번째 글' 규칙이 네 사이클 연속(209 · 210 · 212 · 213) 값을 했다** — 다만 이번엔 **첫 후보가 빗나갔다.**
> `Modularizing Rails Monoliths` 는 **수치도 대가도 없어** 결정으로 못 썼고, 그래서 `5 SRE tips` 로 옮겨
> 배포 전략 둘(Recreate · 노드 그룹)을 건졌다. **후보를 하나 열어 보고 아니면 다음 것을 여는 절차가 실제로 필요했다.**

> **다음 사이클** — 도메인 **`임상 현장의 AI`**. **⚠️ 읽은 한 편(`doximity-ask`)에 버린 대안이 없었으니
> '읽는 사이클' 로 쓴다** — `how-we-re-thinking-about-agentic-systems`(2026-06-15)와 `beyond-accuracy` 를
> 먼저 읽고, **재료가 있으면 기능을 쓰고 없으면 `hold_reason` 을 붙이고 회사를 닫는다.**
> Ask 글에서 이미 확인된 것: 150개 넘는 헬스 시스템 · 상위 20개 병원 중 8곳 · PeerCheck 의사 1.1만 명 ·
> 리뷰 5만 건 · 블라인드 감사 4.6/5(50건) · 시나리오 약 2,000개 · 저널 2,000곳 이상.
> 한계도 스스로 적는다 — *"환각은 현재 모델 구조의 불가피한 결과"*, *"표본이 제한적이고 결과는 예비적"*.
...
마지막 사이클: 213 (2026-08-27) — Doximity 프로덕션에서만 보이는 것을 프로덕션에서 잰다.
