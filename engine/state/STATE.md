...
**Elastic**(검색 엔진) — 도메인 2개 중 **1개에 기능이 들어갔다**(`three-judges-no-averaging`).
남은 빈 도메인 하나: '필드마다 다른 방식으로 압축한다'. 큐 0/3.

### 평균이 극단을 지운다

**"상쇄가 없다 — 각 판정자가 따로 작동한다"** 는 한 문장이 이 설계의 전부다. 여러 지표를
하나로 합치는 것은 편하지만 **합치는 순간 각 지표가 무엇을 말하려 했는지가 사라진다.**

### 노드가 죽는 것과 느려지는 것을 갈랐다

**힙만 하드 제약**(`YES`/`NO`)이고 쓰기 부하는 선호(`NOT_PREFERRED`)다. 모두 하드로 두면
배치할 곳이 없어지고 모두 선호로 두면 죽는다 — 그래서 **판정값이 셋**이 됐다.

### 가장 뜨거운 샤드를 옮기지 않는다

제일 큰 것을 옮기는 것이 직관적인데 **`½ × 최대 부하` 를 임계로 삼은 중간 구간**에서 고른다.
제일 큰 것을 옮기면 **받는 노드가 곧 새 핫스팟**이 되기 때문일 것이다 — **핫스팟을 옮기는
것과 없애는 것은 다르다.**

### 불균형이 자동 확장을 막는다

오토스케일러가 **자원을 노드에 걸쳐 합산**해서, 한 노드가 뜨거워도 다른 노드에 여유가 있으면
전체로는 여유가 있어 보인다. **"밸런서가 먼저 부하 분산을 잘해야 하고, 그다음에 오토스케일러가
필요할 때 작동한다."** 밸런서 안에서는 합산을 버렸는데 **밖에서는 여전히 합산된 값에 기대고
있다.**

### 하나씩 켜야 무엇이 무엇을 고쳤는지 안다

IndexBalance → 쓰기 부하가 고르게, HeapUsage → **OOM 이 거의 0 으로**, WriteLoad → 핫스팟
교정 시간 크게 감소. 단계적 롤아웃 덕에 각 단계의 효과를 따로 적을 수 있었다.

### 다음 사이클

남은 도메인 하나 — 적응형 압축 코덱. **타이거데이터와 정확히 반대되는 결정**(설정을 사용자에게
주지 않는다)이 거기 있다. 그것으로 Elastic 은 완주다.
