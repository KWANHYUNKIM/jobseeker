...
**Polar Signals 완주(기능 3개).** 회사 132개, **큐 0/3 — 다음 사이클은 후보 조사다.**

### 이번 기능 — `sample-storage-economics`

표본 수로 값을 받으니(1억 표본에 50달러) **쌓는 값이 받는 값을 넘으면 사업이 안 된다.**
그래서 저장 엔진을 직접 만들었고, 그것이 무너진 이야기를 스스로 적었다.

### 성장이 설계를 깨뜨린 기록

값싼 선점형 인스턴스에 메모리로 모았다가 객체 저장소로 흘려보내는 구조였는데, **고객이 늘자
종료 유예 안에 다 못 내보내 데이터가 사라졌다.** WAL 로 막으니 복구 시간이 고객 수와 함께
자랐고 재생 중엔 질의를 못 받았다. SSD 로 올려 시간을 벌었지만 그건 비용 효율이라는 다른
목표를 깎아 산 시간이었다. 여러 노드가 동시에 선점되면 복구·OOM·재복구의 나선이 돌았다.

원래 목표 셋(저장·컴퓨트 분리, 비용 효율, 쉬운 운영)을 **표로 놓고 전부 실패로 매긴 뒤
고쳐 쓰기가 아니라 다시 쓰기를 골랐다.** 카프카는 작은 회사에 과하다며 일부러 피했다.

### 자기가 파는 도구로 자기를 재서 형식을 바꿨다

질의 CPU 를 파케이→애로우 변환이 지배하고 있었다. 밀어내기로는 일반적으로 못 없앤다고 보고
보텍스로 옮겨 **평균 70%** 를 얻었다(압축 크기는 3% 손해). 프로파일링 회사다운 결론이다.

### 앞 회사와 견준 자리

Ubicloud 는 btrfs 로 잃은 것을 다음 판에 되찾았고, 여기는 **되찾을 수 없다고 판단하고
버렸다.** 둘 다 갈아엎었지만 하나는 보수, 하나는 포기다.

### 다음 사이클 — 후보 조사 (큐 0/3, 비었다)

기준은 그대로다: 회사 소유 도메인 · 엔지니어가 직접 쓴 글 · 2026년 최신 · 개별 글로 확정.
축을 벌린다 — 최근 넉 달이 빌드·클라우드·프로파일링으로 몰렸다.
