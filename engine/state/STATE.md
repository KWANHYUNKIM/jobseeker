...
**bol(EU · 커머스) `in_progress` — 59번째 회사.** 350 에서 **프로파일**을 썼다
(PROMPT.md 3단계). **도메인 1개 · 기능 0개.** 다음 사이클부터 기능이다.

**⑭ `--gaps` 가 [신규] 를 가리켰고 대기 맨 위가 bol 이었다.**

**열어 확인한 것** — `Splitting the data of the monolith`(2024-11-07,
`/en/blog/splitting-the-data-of-the-monolith-because-who-needs-to-sleep-anyway/`).
**⭐ ⑫ 가 또 값을 냈다** — 349 의 큐 메모에 없던 **사업 맥락**이 나왔다:
이 모놀리스가 맡던 것은 **financial fulfillment** 이고 회사는 그것을
*"Financial fulfilment is a grouping around transactions and connects trigger events,
like a delivery with billing."* 라 설명한다. 그리고 **왜 과거를 못 버리는지**가 분명하다 —
*"Without them, our new applications cannot process correctly the new events of the old orders."*
(플레이스테이션 5 를 받았다가 반품하면 크레딧 노트를 내야 한다는 예를 든다.)
모놀리스가 산 기간은 **2017~2024년**이다.

**연 도메인 하나** — `거래는 끝나도 정산은 계속된다`(**넓게 지었다**). `tech` 셋을 그 한
편에서만 채웠다 — 관리형 이관 서비스로 복제 후 승격 · Flyway 와 SQL 로 스키마 변환 ·
준비와 실행과 복구로 나눈 작업 창. **⚠️ 두 편 규칙이 아직 안 채워졌다.**

**⭐ `tech` 에서 건진 것 하나** — **자원을 더 쓰는 것과 빨라지는 것이 같지 않았다**
(이 정리는 이 사이트의 것이다). 상태 테이블 단계에서 DB CPU 가 **4퍼센트**밖에 안 쓰였고,
병렬 처리로 **15퍼센트**까지 올렸는데도 6시간짜리 테이블이 **약 7시간**으로 되레 늘었다.
결정적이었던 것은 **스크립트의 O(N제곱) 서브셀렉트를 임시 테이블로 O(2N) 으로 바꾼 것**이고,
그러자 손을 못 대던 테이블이 **10~12시간**에 끝났다.

**사업 수치**(모회사 **Ahold Delhaize 2024년 연차보고서**에서 확인) — 고객 **1,360만 명
이상** · 판매 파트너 **47,000곳** · 취급 상품 **4,200만 종** · **1999년 첫 판매**(2024년 25주년).
**⚠️ 소매와 마켓플레이스의 매출 비중, 연간 GMV 는 확인하지 못해 `unknown` 으로 뒀다.**

**⚠️ 350 에서 validate 가 한 번 ✗ 를 냈다** — `category` 를 `리테일` 로 썼는데
**허용값은 핀테크·커머스·소셜·메시징·스트리밍·검색·광고·모빌리티·게임·SaaS·기타 뿐**이다.
`커머스` 로 고쳤다. (**country 허용값은 KR·US·CN·JP·EU·CA·AU·SG·IN·AE·NG·기타.**)

---

**다음 사이클 — bol 의 첫 기능. 두 편 규칙부터 채운다.**
- **⭐ 1순위 — 이미 읽은 이관 글로 첫 기능을 쓴다.** 재료가 이미 3/3+ 이고 도메인도 열려 있다.
  **버린 대안 셋**(중개 앱을 2031년까지 유지 · 스케줄 잡으로 밀기 · 전 물류 이벤트 리플레이) ·
  **실패 셋**(사흘 넘게 돌다 미완 · DMS 가 18시간 뒤 외래키로 반복 실패해 2주 조사 ·
  인덱스 생성 1시간 초과로 Flyway 롤백 재시작 루프) · **수치**(4TB 중 3TB · 2TB 와 1TB ·
  CPU 4→15 퍼센트 · 6→7→10~12시간 · DMS 잡당 19~23시간이고 순차만 · 준비 1~2시간 ·
  실행 1~2일 · 복구 3~6시간 · 전체 2~3개월)로 **결정 6~8개가 넉넉히 나온다.**
- **⭐ 2순위 — 두 편 규칙을 채울 다음 글.** `Fast is slow, slow is fast - rethinking Our Data
  Engineering Process`(2024-11-07) 가 같은 날 글이라 이어질 가능성이 있다(**추정**).
  그 밖에 `Bol's journey in shifting left and shifting right`(Vision 과 Canary Releases,
  둘 다 2024-05-22) · `How we built near real-time X for you recommender systems`(2024-04-09) ·
  `Database Setup So Easy, Your Cat Could Do It`(2024-08-02) ·
  `Navigating chaotic times: forecasting amid the pandemic`(2024-04-18).
  **⚠️ 추천 시스템과 예측은 다른 도메인일 것이다 — 223 규칙으로 실물을 보고 정한다.**
- **⚠️ 블로그 최신이 2024-11-07 이다** — 그 뒤가 없어 보인다(이 사이트가 확인한 범위 안에서).

**⚠️ 큐 2/3 — bol 을 완주하면 후보 조사를 한 번 끼운다.** **후보 조사 누적 스물세 곳 중 아홉.**
대기 두 번째는 **PayFit**(`backstage.payfit.com`, 2/3, 얇고 2024-07 에서 멈춤).

**⭐⭐ 비교 문서 재료가 셋으로 늘었다**(신규를 파는 중에는 손대지 않는다):
① **freee 대 SmartHR 의 순서** — freee 가 *"旧機能のリファクタはマイクロサービス化の前に
着手すべし"* 라 후회한 순서를 SmartHR 이 실제로 밟아 보고 **모듈러 모놀리스로 경계만
정리하는 안을 버렸다**(인가가 과금·테넌트에 의존해야 해서 **의존 허용 규칙이 곧 경계 설계가
되고** 배포 독립성과 가용성은 규칙으로 안 풀린다).
② **`미루되 미룬 것이 안 보이게 한다`** 에 SmartHR 두 사례 — 미룬 쪽은 **놀고 있는 접속**,
미리 잰 쪽은 **연 1회 정점의 부하 시험**.
③ **⭐ bol 이 ②의 셋째 사례가 될 수 있다** — 버린 첫 대안이 **중개 앱을 2031년까지 그대로
돌리는 것**인데, **미루는 쪽을 값으로 계산해 명시적으로 거부한 사례**다.
**⚠️ 다만 그 값을 숫자로 적지는 않는다**(`bol.json` 의 `open_questions` 에 적었다).

**분포** — 59곳(**US 22 · KR 9 · JP 7 · EU 8 · IN 3** · 기타 3 · CN 2 · CA·AU·SG·AE·NG 각 1) ·
비교 문서 32편 · **운영 사실 45개.**
...
않는다**(342·345) · **세 편을 열고 하나도 안 쓰는 사이클이 정상이다**(343) ·
...
다시 본다**(348) · **자체 도메인처럼 보여도 URL 끝에 16진수 해시가 붙으면 Medium 커스텀
도메인이다**(349) · **⭐ 새로: `category`·`country` 는 허용값 집합이 있다 — 새 회사를 열 때
`engine/validate.py` 의 `CATEGORIES`·`COUNTRIES` 를 먼저 본다**(350).
