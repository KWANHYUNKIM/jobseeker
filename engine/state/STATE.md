...
**없다 — Klaviyo 를 완주했다.** 도메인 **2** · 기능 **2** · `status: done`. 회사 110개. ⚠️ **큐 1/3 — 다음 사이클은 후보 조사다.**

### ⭐ 못 찾던 글을 찾았다 — 목록의 제목과 글의 제목이 달랐다

지난 사이클에 **`Our two lines of defense: kafka and S3` 의 주소를 못 받아** `open_questions` 로 남겼다. ⭐ **이번에 제목이 아니라 본문 문구(`two lines of defense`)로 검색해서 찾았다** — 실제 제목이 **`Building a Resilient Event Publisher with Dual Failure Capture`** 였다.

⚠️ **목록의 제목이 글의 제목과 다를 수 있다**(Medium 이 부제나 옛 제목을 보여 준 것으로 보인다). **제목으로 못 찾으면 본문에 있을 법한 문구로 찾는다.**

### 이번 사이클 — `실패를 두 겹으로 받는다 (이중 실패 포착)`

⭐ **출발점이 `돌려줄 데가 없다`는 것이다** — **`실패를 호출자에게 되돌려 재시도를 기대하는 것은 그냥 선택지가 아니다.`** 던지고 잊는 API 라 호출자는 이미 떠났다. ⚠️ **그리고 전에는 실제로 잃고 있었다** — **`발행자가 로그를 남겨 두긴 했지만, 실제로는 발행 실패가 곧 이벤트 유실을 뜻하는 경우가 많았다.`**

⭐ **핵심은 실패 경로를 성격으로 가른 것이다**(재구성):

- **재시도로 풀리는 것**(카프카 타임아웃 · MSK 불가용) → **자체 호스팅 카프카의 `failed-writes`** 에서 자동 재시도, 계속 실패하면 **`failed-writes-DLQ` 에 7일 + 경보.** **석 달 동안 약 2만 건이 자동 재발행에 성공.**
- ⚠️ **재시도로 안 풀리는 것**(직렬화 오류 — **`카프카와는 전혀 무관하고 자체 파이프라인 코드의 버그`**) → **곧장 S3.** **무한 보존, 원인 수정 후 수동 재발행.** **석 달 4천 건, 대부분 이것.**
- ⭐ **재시도 클러스터가 본 파이프와 다른 클러스터다** — **`MSK 클러스터 불가용`이 실패 원인 목록에 있으므로, 같은 곳에 두면 원인과 대피처가 같이 죽는다**(재구성).

### ⭐ 거절한 대안을 이유와 함께 적는 드문 글이다

**아웃박스 패턴** — **`영구 기록과 무한 재시도 능력을 얻지만, 파이프라인에 홉이 하나 늘고 데이터베이스 계층에 병목이 생길 수 있다.`** ⚠️ **초당 17만 건 앞에 DB 를 세우는 값이 크다**(재구성).

**실패한 것만 저장** — 속도는 지키지만 **`어느 데이터스토어를 쓸지, 재시도를 언제 어떻게 할지, 보존 정책은 어떻게 할지`** 가 남았다. ⭐ **두 곳으로 나눈 답이 그 셋을 한꺼번에 푼다** — 재시도는 카프카가, 보존은 S3 가 맡는다.

⚠️ **거절의 대가도 분명하다** — **`모든 이벤트의 영구 기록`을 포기했다.** 실패한 것만 남고, **발행 직전에 죽으면 남는 기록이 없다**(재구성). ⭐ **얻은 것은 `정상 경로에 지연 0`이다.**

### ⭐ Klaviyo 완주 관찰 — 두 도메인이 같은 이야기의 앞뒤다

**`잃지 않고 넘긴다`가 먼저이고 `마감 시각으로 줄을 세운다`가 그 위다**(재구성). ⚠️ **그리고 둘이 서로를 방해할 수 있다** — **대량 복구를 하면 이미 지난 마감 시각을 단 이벤트들이 한꺼번에 EDF 큐에 들어온다**(재구성). `connections` 에 남겼다.

⭐ **이 회사가 이 엔진에 더한 축 — `실시간 스케줄링 이론을 파이프라인으로 옮기기`**(Liu & Layland 1973 EDF). **비교 문서 재료로 ㊶번 축이다.**

### 다음 사이클 — 후보 조사 (3순위)

**큐 1/3.** ⚠️ **먼저 `name_en` 목록 출력으로 시작한다.** ⏳ 대기에 남은 하나 — **Perplexity**. ⭐ **`큰 이름인데 아직 없는 회사` 각도가 네 조사 연속 통했다** — ⏳ Anthropic · Databricks · Snowflake · Anduril · Ramp 외 핀테크.

⏳ **Klaviyo 보강 거리** — `Unbreakable System for Distributed ML Workloads` · `Ray Data, Train & Tune` · `Engineering Secure Agentic Systems at Scale` · `Aurora Blue/Green` · **이관의 기술 구현 후속 글**(회사가 예고했다).
