...
**Sentry(US · SaaS—오류 추적·관측)** `in_progress` · **도메인 2 · 기능 1** —
77곳 · ⚠️ 큐 0/3.

방금 쓴 기능: **`ai-opens-human-merges`(AI 는 PR 까지 열고 병합은 사람이 한다)**.
결정 7개가 **전부 `confirmed`**.

### ⭐⭐ 선의 근거가 기술이 아니라 조직이다

`Your org might not be ready to trust the machines that far, and that's ok.`
**믿음이 준비되지 않았다는 것을 그대로 인정하고 거기서 멈춘다.**
그리고 그 선을 지키는 장치가 구체적이다 — **4시간 무반응**이 개입 기준이고(매시간 확인),
붙이기 전에 **PR 이 아직 살아 있는지 확인해 조용히 건너뛰며**, 리뷰어는 **Seer 에게
물어** 그 코드와 가장 가까운 사람으로 고른다.

### ⭐⭐ 피드백 고리를 요청에 심었다

엔지니어에게 **세 가지**를 요청한다 — 병합하거나 닫기 · **한 줄 피드백** ·
**`Seer 를 잘 쓴 건지` 표시.** **마지막 하나가 자동화를 스스로 평가받게 만든다.**

### ⭐ 같은 숫자가 반대 뜻을 가질 때

**병합 없이 닫힌 PR 이 12.5% 늘었다** — 나쁜 신호로 읽히기 쉬운데, 안을 보니 **중복
수정이거나 리뷰어가 더 포괄적인 수정을 고른 경우**였다. **숫자를 변호하려면 안을 봐야
한다**는 사례로 실패 그림에 그렸다. ⚠️ **다만 자기 지표를 자기가 해석하는 자리라
검증하지 않았다고 밝혔다.**

### 다음 사이클 — 남은 도메인

**자기 에이전트를 감시하는 에이전트를 둔다** — `claude-routines-agent-triage`(전문 읽음).
**수치가 더 세다** — 매일 밤 대화 **약 800건** · 표본 **445~551건** · 도구 호출 **약
11,000건** · **오류율 21%** · **30~56초 지연 행**. 그리고 **핵심 발견**이 있다:
**성공한 대화도 표본에 넣어야 한다**(오류 없이 끝나도 잘못된 결론에 이를 수 있다).
여기까지 채우면 **완주 조건에 닿는다.**
