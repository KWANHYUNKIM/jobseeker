...
**Deno — 기능 하나를 채웠다(`isolate-cloud-defense-layers`).** 빈 도메인 하나. 회사 144개, 큐 0/3.

### 두 회사가 같은 결론에 다른 길로 도착했다

발타운은 노드 `vm` → `vm2` → 웹 워커를 거치며 탈출을 겪고 **프로세스로 나왔다.** 데노는 파는
쪽에서 아이솔레이트를 첫 겹으로 두되 **그것만 믿지 않고** 프로세스·네임스페이스·seccomp·cgroup·
감시 서비스를 겹으로 쌓는다. 언어 안에서는 못 가둔다는 결론이 양쪽에서 같다.

### 격리로 못 막는 것을 따로 본다

막으려는 것을 넷으로 가른다 — 다른 테넌트, 운영체제, 자원 고갈, **채굴 같은 오용**. 마지막
것은 규칙을 어기지 않고 한도 안에서 자원만 쓰는 종류라 샌드박스가 못 잡는다. 그래서 보는 눈
(감시 서비스)을 따로 둔다.

### 이 엔진 기준으로 드문 침묵

방어는 겹겹이 늘어놓는데 **스펙터·부채널·샌드박스 탈출은 이름조차 없고** 남은 위험도, 메모리·
CPU 한도 숫자도 안 적는다. 같은 문제를 쓰는 쪽인 발타운이 vm2 탈출과 워커 속 워커 탈출을
이름 대어 적은 것과 갈리는 자리라 결정 칸에 그대로 세웠다.

### 다음 사이클 — 확장 (Deno 의 마지막 빈 도메인)

`모듈을 어디서 가져올지 정한다`. HTTP 임포트에서 JSR 로 넘어온 회고라 **feature history 를
세울 수 있는 자료**다 — 이 엔진에서 두 번째다(첫 번째는 발타운의 런타임 네 판).
