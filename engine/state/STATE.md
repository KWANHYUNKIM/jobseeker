...
**없다 — Vercel 을 완주했다.** 도메인 3개 · 기능 3개. 회사 **107개.** ⚠️ **큐 0/3 — 다음 사이클은 후보 조사다.**

### 이번 사이클 — `0 이 아니라 1 로 내린다`

⭐⭐ **이번 사이클의 가장 큰 발견은 회사 하나가 아니라 두 회사의 대비다** — **Neon 은 `일이 멈추면 컴퓨트가 정지한다` 를 정의적 기능으로 삼는데, Vercel 은 `0 으로 내리는 대신 1 로 내린다`.** ⚠️ **다루는 것이 다르기 때문이다** — Neon 은 **수천 개의 산발적으로 쓰이는 데이터베이스**(대부분이 놀고 있다), Vercel 은 **사용자가 지금 기다리는 웹 요청**(첫 인상이 콜드 스타트면 안 된다). ⭐ **같은 문제에 정반대 답이고 둘 다 맞다**(재구성).

건진 것 넷:

- ⭐ **거스른 이유가 역설적이다** — **`앱이 트래픽을 아주 적게 받으면, 0 으로 내리는 것은 거의 모든 첫 상호작용이 콜드 스타트라는 뜻이 된다.`** ⚠️ **0 으로 내리는 것의 값을 가장 크게 치르는 쪽이 가장 작은 앱이다**(재구성).
- ⭐ **그 값을 감당하는 방법이 앞뒤로 맞물린다** — **`많은 요청이 하나의 인스턴스에서 돌지, 각각이 자기 것을 띄우지 않는다`**(수십~**250개 이상**). **하나를 살려 두는 값이 그 하나가 많은 일을 하기에 감당된다**(재구성). 그리고 **`Active CPU 과금으로 기다리는 동안은 CPU 값을 받지 않으니`** 기다리는 요청이 자리를 덜 차지한다(재구성).
- ⭐ **머신과 일의 관계를 뒤집는다** — **`몇 년 동안 머신은 고정된 것이었고, 당신이 자기 일을 거기에 맞췄다. Fluid 에서는 당신이 일을 기술하면 머신이 그것을 둘러싸고 형성된다.`** ⚠️ **Neon 이 `인스턴스 크기를 고르는 것은 낡은 패턴이다` 라고 한 것과 같은 방향인데 유휴 처리만 정반대다**(재구성).
- ⚠️ **에이전트가 요구하는 속도가 사람이 요구하던 속도와 다르다** — **`에이전트에게는 클라우드조차 너무 느리다. 표준 VM 은 그들이 일하는 방식을 따라갈 만큼 빠르게 프로비저닝될 수 없다`** 라서 **`Hive 가 완전한 VM 을 밀리초 안에`** 만든다.

⚠️ **`언제나 하나` 에는 시간 창이 있다** — 운영은 **최근 14일 안에 호출됐으면**, 브랜치는 **엔터프라이즈에서 최대 3일**. **완전히 안 쓰는 것까지 떠안지는 않는다**(재구성).

### 완주하며 본 것 — 세 기능이 `지킬 것을 없애는` 쪽이다

⭐ **웜풀에서는 `잃으면 안 되는 것을 캐시에 두지 않는다`, 샌드박스에서는 `보관할 자격증명을 만들지 않는다`, 컴퓨트에서는 `고를 인스턴스 크기를 만들지 않는다`**(재구성). ⚠️ **다만 `0 으로 내리지 않는다` 만은 반대 방향이다** — **없애지 않고 하나를 남긴다.** ⭐ **없애는 것이 늘 답은 아니라는 자기 반례를 같은 회사가 갖고 있다**(재구성).

⚠️ **글마다 정직함이 크게 갈린다** — **웜풀 글은 밀리초로 실패를 적는데**(P90 약 17배, 버린 해법과 이유까지) **샌드박스 글과 Fluid 글은 대가도 한계도 하나 없다**(재구성). ⭐ **③축에서 한 회사 안의 편차가 가장 큰 사례다.**

### 다음 사이클 — ⚠️ 후보 조사 (3순위)

**큐 0/3. 목표 3곳.** ⚠️ **먼저 `name_en` 목록 출력으로 시작한다.**

⭐ **`큰 이름인데 아직 없는 회사` 각도가 두 조사 연속 통했다** — ⏳ **Supabase · Robinhood · Block/Square · Klaviyo · Railway · Cursor · Linear.**

⏳ **Vercel 보강 거리** — Run SDK · v0 의 OAuth 인증 · Agent Plugins · 소프트웨어 팩토리.

⏳ **보강 거리** — Neon(브랜칭 심화) · Atlassian(Events Rail 등) · Trendyol(Helyx 4편 등) · Grafana Labs 인용 대조 · Razorpay 보안 트리아지 · Flipkart Rate Card · Pinterest 2부 · WarpStream 미독 셋.

### ⚠️ 절차 (이 세션에 실수로 배운 것)

**후보 조사는 `name_en` 목록 출력으로 시작한다.** **새 회사 프로파일 전에는 `engine/validate.py` 의 `COUNTRIES`·`CATEGORIES` 를 직접 본다**(⭐ 다섯 번 연속 오류 0). **`git add -A` 를 쓰지 않는다.** **슬러그를 추측하지 않는다.** **첫 화면으로 회사를 판정하지 않는다.** **제목만 보고 `자료가 없다` 고 적지 않는다.** **그림은 최소 하나가 `failure` 여야 한다.** ⭐ **자료가 얇으면 두 번째 자료를 찾는다**(이번에도 통했다 — Fluid 글 하나로는 결정이 5개였는데 `scale to one` 글을 더해 8개가 됐다).

### ⚠️ 비교 문서 재료 (초안 유지)

**① `AI 에이전트를 어디까지 믿나` 14곳** — Sentry / ClickHouse / Duolingo / Ramp / DoorDash / Deliveroo / Snyk / Cygames / Razorpay / Wiz / Trendyol / Coinbase / Atlassian / Vercel.

**② `관리형 MySQL 의 한계` 3곳 + Cygames** — Etsy / Plaid / Paystack.

**③ ⭐⭐ `자기 성과를 어디까지 주장하나` 15곳** — Snyk / Grafana Labs / ScyllaDB / Razorpay / Flipkart / Airbnb / Pinterest / WarpStream / VictoriaMetrics / Wiz / Trendyol / Coinbase / Atlassian / Neon / **Vercel**(⭐ **한 회사 안의 편차가 가장 크다 — 웜풀 글은 밀리초로 실패를 적고 샌드박스·Fluid 글은 대가가 하나도 없다**).

**④ `인도 규모에서 무엇이 달라지나` 4곳** — Meesho / Zepto / Razorpay / Flipkart.

**⑤ `한 코어에 하나씩인가, 여러 코어가 나눠 쓰나`** — ScyllaDB / ClickHouse / Grafana Labs / VictoriaMetrics / **Vercel**(⭐ **함수 하나에 요청 250개 — 서버리스에서 이 축을 다시 연다**).

**⑥ `복제로 버틸 것인가 로그로 버틸 것인가`** — Grafana Labs / WarpStream / Trendyol / Atlassian / Neon.

**⑦ `추상화가 무엇을 가리는가`** — Plaid / ScyllaDB / ⏳ Paystack / Wiz / Trendyol / Vercel(`격리` 가 나가는 쪽을 안 막는다는 것을 가린다).

**⑧ `제약을 없애지 못할 때 어디서 갚는가`** — Cygames / Zepto / Grafana Labs / Pinterest / WarpStream / VictoriaMetrics / Atlassian / Neon / Vercel.

**⑨ `한 번에 갈아엎을 것인가 목 졸라 죽일 것인가` 10곳** — Twilio(두 방향) / Etsy / Plaid / Paystack / Airbnb / Pinterest / WarpStream / Trendyol / Atlassian / Vercel(다섯 단계·섀도 읽기).
...
**⑬ `애매할 때 어느 쪽으로 넘어지나` 14곳** — Flipkart(둘) / Razorpay / Zepto / Paystack / Airbnb / Pinterest / WarpStream / VictoriaMetrics / Wiz / Trendyol / Coinbase / Atlassian / Neon / Vercel.
...
**⑲ `신뢰의 뿌리를 어디에 두나`** — Pinterest / Coinbase / Vercel(OIDC) / ⏳ Plaid · Snyk.

**⑳ ⭐⭐ `없앨 수 있는 것을 없앤다`** — WarpStream(디스크) / Wiz(에이전트) / Neon(컴퓨트·인스턴스 크기) / **Vercel**(⭐ **세 기능이 `지킬 것을 없애는` 쪽인데 `0 으로 내리지 않는다` 만은 반대다 — 같은 회사가 자기 반례를 갖고 있다**, 재구성) / ⏳ TigerBeetle · Oxide Computer.

**㉑ `논문을 어디까지 그대로 쓰나`** — WarpStream(LazyLog) / Neon / ⏳ ScyllaDB · TigerBeetle · ClickHouse.

**㉒ `되돌릴 수 있는 곳과 없는 곳을 어떻게 가르나`** — WarpStream / Plaid / Airbnb / Twilio / Trendyol / Atlassian / Neon / Vercel.

**㉓ `설정으로 열 것인가 코드로 막을 것인가`** — WarpStream / Airbnb / Wiz / Trendyol / Coinbase / Vercel(`적히지 않은 의존`) / ⏳ Pinterest.

**㉔ `무엇을 무료로 두고 무엇을 파나`** — VictoriaMetrics / Neon / ⏳ Vercel(Next.js 주간 2억) / ⏳ Grafana Labs · ClickHouse · Snyk.

**㉕ `새 일을 만들 것인가 도는 일에 얹을 것인가`** — VictoriaMetrics / WarpStream / Wiz / Coinbase / Atlassian / ⏳ Etsy · Plaid.

**㉖ `기술 글을 왜 쓰나`** — VictoriaMetrics / ScyllaDB / Grafana Labs · Honeycomb / Wiz / Trendyol / Coinbase / Atlassian / ⏳ Vercel · Airbnb · Pinterest.

**㉗ `인수된 뒤에 무엇이 달라지나`** — Wiz(구글) / WarpStream / Twilio / Neon(Databricks) / ⏳ Trendyol · PlanetScale · Snyk.

**㉘ `맞았는지 어떻게 아나`** — Wiz / Flipkart / Snyk / Trendyol(반대) / Coinbase / Vercel(섀도 읽기) / ⏳ Duolingo.
...
**㉜ ⭐ `흔들리는 매출과 안 흔들리는 매출`** — Coinbase / Twilio / VictoriaMetrics / Atlassian / Neon / **Vercel**(⭐ **Active CPU 로 기다리는 시간에는 안 받는다 — 과금 모델이 워크로드의 모양을 따라갔다**) / ⏳ Adyen · Stripe.

**㉝ `무엇이라 부르느냐가 요구 사항을 정한다`** — Coinbase / Trendyol / Neon / Vercel(`부채`·`적히지 않은 의존`) / ⏳ Plaid · Etsy.

**㉞ `비용을 옮기면 위험도 옮겨 간다`** — Atlassian / WarpStream / Razorpay·Airbnb / Vercel / ⏳ Zepto.

**㉟ `무엇이 제품이고 무엇이 구현 세부인가`** — Atlassian / WarpStream / Coinbase / Neon / ⏳ Plaid.

**㊱ ⭐⭐ `고객이 사람이 아닐 때`** — Neon(프로비저닝의 80% 이상) / **Vercel**(⭐ **에이전트가 남의 코드를 남의 자격증명으로 돌릴 때 · `에이전트에게는 클라우드조차 너무 느리다`**) / WarpStream / Coinbase·Atlassian / ⏳ Temporal.

**㊲ `지킬 것을 없애는 쪽으로 푼다`** — Vercel(세 기능 중 둘) / Atlassian(되돌릴 것을 안 만든다) / Neon(아무것도 덮어쓰지 않는다) / ⏳ TigerBeetle.

⏳ **㊳ 새로 보인다 — `0 으로 내릴 것인가 1 을 남길 것인가`** — ⭐ **Neon 과 Vercel 이 정반대 답을 내고 둘 다 맞다** — Neon 은 **수천 개가 대부분 놀고 있는 데이터베이스**, Vercel 은 **사용자가 지금 기다리는 웹 요청.** ⏳ Fly.io · Cloudflare · Temporal. ⚠️ **`무엇이 기다리고 있느냐` 가 답을 가른다**(재구성).

## 지금의 진짜 상태

- **회사 파일 — 비한국 위반 0.** 한국 회사 **237건**(hyperconnect 127 · kurly 110)은
  **한국어 원문 인용이라 의도적 보존.**
- **비교 문서 — 남은 1건은 위반이 아니다.** `reversible-first.md` 의 토스 인용이고
  **원문으로 확인했다.**
- **별표 세 개 회귀 0** — 361 에서 되돌린 상태가 유지된다.

---

**다음에 이어받을 것 — 셋 중 하나.**
- **⭐ 1순위 — 후보 조사.** 큐가 0/3 이다. **⚠️ 지난 세션에서 WebSearch 가 막혔는데 새 세션이면
  풀렸을 수 있다 — 먼저 한 번 시도해 본다.** 안 되면 아래로.
  **⚠️ 요약만 보고 판정하면 과대평가가 난다**(354) — **'무엇이 없는지' 를 함께 묻는다**(352).
- **2순위 — `되돌릴 수 없는 것이 어디에 있는가`(`when-you-cannot-undo.md`) 에 bol 을 더한다.**
  bol 은 **적어도 한 번** 처리를 골랐고(*"we must stop the old application, while we are
  collecting messages in the new applications to process everything at least once"*)
  **정산에서 중복은 크레딧을 두 번 내는 일**인데 **이관 쪽에 검증 이야기가 아예 없다.**
  ⭐ **Paytm 도 50억 개를 옮기고 검증이 한 문장이었다.**
  **⚠️ 갱신하면 `domains/index.json` 의 그 `docs` 항목에 `bol` 과 `move-four-years-of-past` 를
  더하고 최상위 `updated_at` 도 올린다.**
- **3순위 — 한국 회사 파일 인용 237건 전수 확인**(hyperconnect 127 · kurly 110).
  **표본 열여섯 개는 다 원문이었으나 전수는 안 봤다.** **⭐ 362 에서 배운 것이 여기에 그대로
  걸린다 — 한국어라고 다 원문인 것이 아니라, 의역이거나 이 사이트가 쓴 문장일 수 있다.**

**⭐ 이 세션의 작업은 Notion `📋 JobSeeker 작업 기록` 데이터베이스에도 남긴다**
(`collection://aa9cef95-a04c-47c6-b7fd-67a28c727053`). 사용자가 여러 세션을 동시에 돌리고 있어
거기서 진행을 본다.

**분포** — **60곳 전부 `done`**(US 22 · KR 9 · EU 9 · JP 7 · IN 3 · 기타 3 · CN 2 ·
CA·AU·SG·AE·NG 각 1) · **비교 문서 33편** · **운영 사실 72개.**

**⚠️ 나르지 않는다** — 낡은 메모(314·317·325·348·349·358) · 두 방향 규칙(298·305·318·321·354) ·
주소가 열리는지도 확인한다(327·328) · **한 글에 없다고 회사에 없는 것이 아니다**(331→332) ·
**도메인 이름은 넓게**(334→335) · **자기 문서 인용도 우리말은 굵게**(337) · **각도가 다르면
다른 기능**(339→340) · **층이 다르면 다른 도메인**(341) · **확인 안 된 인용 금지**(341·342) ·
**읽은 글을 미독 목록에 남기지 않는다**(342·345) · **여러 편을 열고 하나도 안 쓰는 사이클이
정상**(343·353) · **`domain_map`·`diagrams` 는 객체**(344) · **`features_done` 을 같이
올린다**(345) · **정반대 답은 `connections` 에**(346) · **셸에 백틱 금지**(347·356) ·
**닫을 때 낡은 문장 다시 본다**(348) · **URL 끝 16진수 해시는 Medium**(349) ·
**`category`·`country` 허용값**(350) · **세 번째 읽어도 새 사실**(351) · **'무엇이 없는지' 를
함께 묻는다**(352) · **실행 안 한 평가는 기능으로 못 쓴다**(353) · **요약만 보고 판정하면
과대평가**(354) · **버린 대안 없는 글에서도 결정을 세운다**(355) · **검사가 0 을 내면
스크립트부터 의심한다**(356) · **기계적 보수는 예외를 먼저 가른다**(357) · **비교 문서의 인용은
하나씩 확인하고 쓴다**(358) · **어떤 파일을 편집하면 그 파일에 남은 유산도 함께 본다**(359) ·
**예외를 가르는 근거를 데이터에서 얻는다**(360) · **정규식이 인접 표기까지 먹는지 본다**(361) ·
**JSON 은 원문을 건드리지 않는다**(356·361) · **대규모 보수는 되돌릴 기준선을 정해 둔다**(361) ·
**⭐⭐ 새로: 표적 수정과 일괄 치환을 한 스크립트에서 돌리면 뒤가 앞을 덮는다 — 순서를 뒤집거나
표적을 치환 대상에서 뺀다**(362) ·
**⭐⭐ 새로: 한국어 인용이라고 다 원문인 것이 아니다 — 의역이거나 이 사이트가 쓴 문장일 수
있다. 원문을 열어 대조하면 반만 원문인 것도 나온다**(362).

## 🚨 361 이 고친 것 — 내가 만든 회귀 두 가지

### ① 정규식이 굵게 표기까지 먹었다

357·360 의 치환 정규식 `\*"(...)"\*` 이 **`**"..."**` 안쪽에도 매치됐다.**
그래서 `**"X"**` 가 `***X***` 로 바뀌었다. **회사 파일 215건 · 비교 문서 13건.**

**고친 방법** — 357 이전 커밋(`db269c8`)을 기준선으로 삼아 **그때 없던 `***X***` 만**
`**"X"**` 로 되돌렸다. hyperconnect(17) · monday(62) · cyberagent(1)의 `***`는
**원래 있던 것**이라 건드리지 않았다. **대조 결과 불일치 0.**

### ② 되돌리다가 JSON 을 깼다

처음에 **파일 원문에** 따옴표를 그대로 넣어 **JSON 문법이 깨졌다**(validate 가
회사 42개 · 오류 36건). `git checkout` 으로 되돌리고 **JSON 을 파싱해 값 위에서** 다시 했다.
**⚠️ 356 에서 이미 배운 것을 361 에서 또 틀렸다** — JSON 은 원문을 건드리면 안 된다.

### ③ 그리고 검사기 자체가 틀렸었다

같은 이유로 **혼용 검사가 `**"..."**` 를 위반으로 세고 있었다.** 앞뒤 별표를 배제한
정규식으로 다시 재니 **357·360 이 보고한 수가 부풀려져 있었다.**

```
python3 - <<'PYEOF'
import io,re,glob,json
# 앞뒤에 별표가 하나 더 있으면 굵게이므로 위반이 아니다
PAT=re.compile(r'(?<!\*)\*"([^"]{1,300})"\*(?!\*)', re.S)
HAN=re.compile(r'[가-힣]')
KR={'baemin','coupang','daangn','hyperconnect','kakao','kurly','musinsa','naver','toss'}
def walk(o,out):
...
for f in sorted(glob.glob('jd-viewer/public/reveng/companies/*.json')):
    slug=f.split('/')[-1][:-5]
    v=[]; walk(json.load(io.open(f,encoding='utf-8')),v)
    h={q for q in PAT.findall('\n'.join(v)) if HAN.search(q)}
    if h and slug not in KR: print('회사', slug, len(h))
for f in sorted(glob.glob('jd-viewer/public/reveng/domains/*.md')):
    s=io.open(f,encoding='utf-8').read()
    h={q for q in PAT.findall(s) if HAN.search(q)}
    if h: print('문서', f.split('/')[-1], len(h))
PYEOF
```
...
- **회사 파일 — 비한국 위반 0.** 한국 회사 **237건**(hyperconnect 127 · kurly 110)은
  **한국어 원문 인용이라 의도적으로 보존**한 것이다.
- **비교 문서 — 13건 남았다.** `reversible-first` **9** · `build-or-buy` **4**.
  **둘 다 한국 회사가 섞여 있어**(toss · coupang) **인용마다 어느 회사 것인지 보고 갈라야 한다.**
  **⚠️ 360 이 보고한 20건 중 일부는 위 오탐이었다** — 실제로는 13건이다.

---

**다음에 이 루프를 다시 돌린다면 — 셋 중 하나.**
- **⭐ 1순위 — 남은 13건을 손으로 가른다.** `reversible-first`(9)에서 **토스 인용 하나**
  (*"PREPARE 대비 CONFIRM 비율이 정상 범위를 벗어나면 자동으로 레거시로 원복"*)는 **그대로
  두고**, Figma·Shopify·Yelp 인용은 굵게로 바꾼다. `build-or-buy`(4)는 Adyen·Cloudflare·
  Spotify 인용이라 다 굵게로 바꾸면 된다(**쿠팡 인용이 그 넷에 있는지 앞뒤 문장으로 확인한다**).
  **⚠️ 애매하면 두고 적는다.**
- **2순위 — `되돌릴 수 없는 것이 어디에 있는가`(`when-you-cannot-undo.md`) 에 bol 을 더한다.**
  bol 은 **적어도 한 번** 처리를 골랐고 **이관 쪽에 검증 이야기가 아예 없다.**
  ⭐ **Paytm 도 50억 개를 옮기고 검증이 한 문장이었다.**
  **⚠️ 갱신하면 `domains/index.json` 의 그 `docs` 항목에 `bol` 과 `move-four-years-of-past` 를
  더하고 최상위 `updated_at` 도 올린다.**
- **3순위 — 한국 회사 파일 인용 237건 전수 확인**(hyperconnect 127 · kurly 110).
  **표본 열여섯 개는 다 원문이었으나 전수는 안 봤다.**

**⚠️ 후보 조사는 이 세션에서 막혔다** — WebSearch 예산 소진(200회). **주소를 아는 글만
WebFetch 로 열 수 있다.** 큐가 0/3 이지만 채울 방법이 없다. **새 세션이면 다시 된다.**

**분포** — **60곳 전부 `done`**(US 22 · KR 9 · EU 9 · JP 7 · IN 3 · 기타 3 · CN 2 ·
CA·AU·SG·AE·NG 각 1) · **비교 문서 33편** · **운영 사실 69개.**
...
함께 묻는다**(352) · **실행 안 한 평가는 기능으로 못 쓴다**(353) · **요약만 보고 판정하면
과대평가**(354) · **버린 대안 없는 글에서도 결정을 세운다**(355) · **검사가 0 을 내면
스크립트부터 의심한다**(356) · **기계적 보수는 예외를 먼저 가른다**(357) · **비교 문서의
인용은 하나씩 확인하고 쓴다**(358) · **어떤 파일을 편집하면 그 파일에 남은 유산도 함께
본다**(359) · **예외를 가르는 근거를 데이터에서 얻는다**(360) ·
**⭐⭐ 새로: 기계적 치환을 돌리기 전에 그 정규식이 인접 표기까지 먹는지 본다 —
`\*"..."\*` 는 `**"..."**` 안쪽에도 걸린다. 앞뒤 별표를 배제해야 한다**(361) ·
**⭐⭐ 새로: JSON 은 원문을 절대 건드리지 않는다 — 파싱해서 값 위에서 고치고 다시 덤프한다.
356 에서 배우고 361 에서 또 틀렸다**(361) ·
**⭐ 새로: 대규모 보수는 되돌릴 기준선(직전 커밋)을 정해 두고 시작한다 — 361 에서 그 덕에
내가 만든 것만 골라 되돌릴 수 있었다**(361).

## 🚨 356 에서 두 가지가 드러났다 — 다음 사이클은 이것부터다

### ① 이 사이트의 혼용 검사가 내내 헛돌고 있었다

**JSON 은 큰따옴표를 이스케이프한다.** 그래서 파일 **원문**에 정규식을 걸면 **아무것도 안
걸린다.** 이 세션 내내 그 방식으로 검사해 놓고 **0 을 보고 안심했다.**
**⭐ 올바른 방법은 JSON 을 파싱한 뒤 문자열 값들 위에서 검사하는 것이다:**
...
for f in sorted(glob.glob('jd-viewer/public/reveng/companies/*.json')):
    vals=[]; walk(json.load(io.open(f,encoding='utf-8')),vals)
    t='\n'.join(vals)
    hits={q for q in re.findall(r'\*"([^"]{1,300})"\*', t) if re.search(r'[가-힣]', q)}
    if hits: print(f.split('/')[-1], len(hits))
PYEOF
```

### ② 그렇게 재 보니 인용 규칙 위반이 유산으로 쌓여 있다

**고유 위반 문장 1,704개 · 파일 35개.** 우리말을 원문 인용 표시 안에 넣은 것들이다.
많은 쪽부터 — **doximity 182 · target 166 · wayfair 142 · cyberagent 139 · mercari 133 ·
hyperconnect 128 · kurly 110 · linkedin 110 · yelp 101 · moniepoint 99 · zerodha 72 · allegro 59.**

**⭐ 그런데 이건 규칙 이전의 유산이다**(이 판단은 이 사이트의 것이다). 인용 규칙은 **337** 에서
세웠고, **그 뒤에 쓴 회사들은 거의 깨끗하다** — freee **0** · bol **0** · payfit **0** ·
freshworks **0** · smarthr **1** · sansan **1** · paytm **1**.

**⚠️ 이건 한 사이클로 못 끝낸다.** 다음 사이클부터 **보수 모드로 다룬다** —
**한 사이클에 한두 파일씩, 많은 쪽부터.** 고치는 법은 우리말을 감싼 원문 인용 표시를
**굵게로 바꾸는 것**이고, **원문이 영어·일본어인 자리는 원문을 확실히 아는 경우에만 원문으로
되돌린다.** **⚠️ 모르면서 원문을 지어내지 않는다**(341).
**⚠️ `engine/validate.py` 는 이것을 오류로 잡지 않는다** — 그래서 이 메모가 유일한 기록이다.

---

## 356 사이클 자체의 기록

**⚠️ WebSearch 예산이 소진됐다**(세션 200회). **그래서 355 의 1순위였던 JetLang 본문을 끝내
못 찾았다** — 검색 없이는 주소를 찾을 방법이 없다. **앞으로 이 세션에서는 주소를 아는 글만
열 수 있다.** 후보 조사 사이클은 사실상 막혔다.

**열어 본 글** — `Exploring our tech architecture vision`(2024-01-24). **판정 1/3.**
**수치는 아주 강한데**(고객 **10,000** · 종업원 **200,000** · **3개국**(프랑스·스페인·영국) ·
**2023년 앱 성능 30퍼센트 개선** · 필수 경험 속도 **70퍼센트 향상**) **버린 대안이 없고 대가가
아예 없다** — 이관 비용도 위험도 재교육도 일정도 되돌릴 길도 안 적고 전환을 좋은 일로만 그린다.
**기능으로 쓰지 않고 프로파일 보강에 썼다**(353 의 방법).

**⭐⭐ 거기서 JetLang 의 실체가 한 걸음 나왔다** — *"The cohabitation between our internal
low-code system, JetLang, and our more common full-code system is challenging."*
**노동법을 담으려고 만든 로우코드가 풀코드와 동거하며 책임을 흐리고 진실의 원천을 여럿으로
만들었다.** 그리고 **JetLang Action 안에 UI 와 비즈니스 로직과 데이터 모델링이 섞여 있다.**
⚠️ **노동법이 어떻게 실행 가능한 코드가 되는지는 네 편을 읽고도 못 찾았다.**

**⚠️ 354 의 서술을 정정했다**(PROMPT.md 2″단계) — 354 에 **고객 수는 3자 정리마다 어긋나
쓰지 않았다**고 적었는데 **1차 자료인 회사 기술 블로그에 10,000곳이라고 있었다.**
**⭐ 3자 추정을 견주느라 1차 자료를 늦게 본 셈이다.**

---

**다음 사이클 — 보수다.**
- **⭐ 1순위 — 인용 규칙 위반 보수.** 위 ②. **많은 쪽부터 한두 파일씩.** doximity(182) 부터.
  **⚠️ `key`·`slug`·`name` 은 건드리지 않는다**(PROMPT.md 2′단계). **한 사이클 = 한 대상.**
- **2순위 — 후보 조사.** 큐가 0/3 인데 **⚠️ WebSearch 가 막혀 사실상 불가능하다.**
- **3순위 — 비교 문서.** 검색 없이 가능하다. **재료가 다섯 서 있다**(아래).
...
**⭐ 세 회사가 다 갖춰졌다 — 지금 쓸 수 있다.**
② **`미루되 미룬 것이 안 보이게 한다`** 에 SmartHR 두 사례(놀고 있는 접속 / 연 1회 정점의 부하 시험).
③ **bol 이 ②의 셋째 사례**(중개 앱 2031년 유지를 버림 · 임시 해법만큼 영구적인 것은 없다).
④ **`되돌릴 수 없는 것이 어디에 있는가` 에 bol**(적어도 한 번 처리인데 이관 쪽에 검증이 없다).

**분포** — **60곳 전부 `done`**(US 22 · KR 9 · EU 9 · JP 7 · IN 3 · 기타 3 · CN 2 ·
CA·AU·SG·AE·NG 각 1) · 비교 문서 32편 · **운영 사실 58개.**
...
**셸 명령에 백틱을 넣지 않는다**(347 · **356 에서 또 걸렸다 — 코드펜스도 백틱이다.
긴 문서를 쓸 때는 Write 툴로 스크립트를 만들어 셸 파싱을 피한다**) ·
**프로파일의 '아직 한 편만 읽었다' 류 문장은 닫을 때 다시 본다**(348) ·
**URL 끝에 16진수 해시가 붙으면 Medium 커스텀 도메인이다**(349) ·
...
**버린 대안이 없는 글에서도 결정을 세울 수 있다**(355) ·
**⭐⭐ 새로: 검사 스크립트가 0 을 내면 스크립트부터 의심한다**(356).
...
| `engineering.razorpay.com` | ⚠️ WebFetch 403 → ✅ **`r.jina.ai` 로 목록·본문이 온다** (2026-09-06 판정 뒤집힘). 최신 글이 살아 있고 수치가 붙는다 |
| `blog.flipkart.tech` | ⚠️ WebFetch 403 → ✅ **`r.jina.ai` 로 목록이 온다** (2026-09-06 판정 뒤집힘) |
| `engineering.klarna.com` | ⚠️ WebFetch 403 → ⚠️ **`r.jina.ai` 로 목록은 오는데 최신이 2025-06 이고 일반 주제가 섞였다.** 본문 판정 아직 |
| `twilio.com/en-us/blog` | ✅ WebFetch 로 본문이 온다. ⚠️ **대부분 고객용 아키텍처 가이드** — 자기 시스템 글은 검색으로 골라야 한다 |
| `www.scylladb.com` | ⚠️ **판정 뒤집힘**(2026-09-06) — 첫 화면은 고객 사례·제품 글이지만 **엔지니어링 글은 대안 비교와 진 결과까지 적는다.** ✅ WebFetch 로 본문이 온다. ⚠️ **슬러그 추측 금지** — 날짜 경로를 지어내면 404 다 |
| `tech.cygames.co.jp` | ⚠️ WebFetch 는 헤더만 온다 → ✅ **`r.jina.ai` 로 목록·본문이 온다** (2026-09-06 판정 뒤집힘) |
| `grafana.com/blog` | ✅ WebFetch 로 본문이 온다. ⚠️ **`/blog/tags/engineering/` 는 404** — 심층 글은 검색으로 주소를 받는다 |
| `bytes.swiggy.com` | ⛔ **못 읽는다 (2026-09-06 세 번째 시도로 확정)** — WebFetch 빈 응답 · **`r.jina.ai` 는 목록·개별 글 모두 422** · 브라우저 응답 없음. ⚠️ **원 도메인 `swiggybytes.medium.com` 은 `r.jina.ai` 로 열리지만 인물·커리어 글뿐이고 최신이 2024-07** 이다(시스템 글과 다른 퍼블리케이션). **도구 한계이지 회사 사정이 아니다** |
| **`r.jina.ai/<주소>`** | ⭐ **막힌 곳을 여는 세 번째 수단.** 국가 차단(딜리버루)뿐 아니라 **403 에도 통한다**(2026-09-06 Meesho·Zepto 되짚기 성공). ⚠️ **Medium 계열에는 422 를 낸다**(bytes.swiggy.com). WebFetch 로 부르면 요약이 오니 **원문 인용이 필요하면 브라우저 `get_page_text`** 로 받는다 |
| `www.meesho.io/blog` | ⚠️ WebFetch 403 → ✅ **`r.jina.ai` 로 목록·본문 모두 온다** (2026-09-06 판정 뒤집힘) |
| `blog.zepto.com` | ✅ **`r.jina.ai` 로 목록·본문이 온다** (2026-09-06 판정 뒤집힘). Medium 계열이라 개별 글은 브라우저도 된다 |
| `bytes.swiggy.com` | ⚠️ **아직 못 읽었다** — WebFetch 는 빈 응답, **`r.jina.ai` 는 422**(Medium 계열). 브라우저로 다시 시도할 것 (2026-09-06) |
...
| `hashicorp.com/en/blog` | ❌ **판정 확정**(2026-09-06) — 심층 글을 검색해도 **조직·문화·커리어 글**뿐이다. 자기 시스템의 설계 회고가 없다 |
| ~~`hashicorp.com/en/blog`(보류)~~ | ⏳ **판정 보류** — 첫 화면은 제품 발표. **심층 글 검색을 아직 안 했다**(2026-09-06) |
...
| `paystack.com/blog/category/engineering-design` | ✅ **브라우저로 목록·본문이 온다.** ⚠️ **옛 판정(`blog.paystack.com` 403)은 주소가 바뀐 탓이었다**(사이클 스물하나) — **주소가 바뀐 두 번째 사례** |
...
| **PDF (IR 자료 등)** | ⚠️ **항상 읽히는 것은 아니다.** 바이너리는 `tool-results/` 에 저장되지만 **본문 파싱이 실패할 수 있다**(사이클 스물둘, ACL 논문 PDF). **더 위험한 것 — 첫 시도에서 그럴듯한 요약이 돌아왔는데 두 번째 시도에서 같은 파일을 파싱할 수 없다고 했다. 같은 자료에 두 번 물어 답이 갈리면 둘 다 버린다** |
| `fastly.com/blog` | ✅ WebFetch 로 목록·본문 다 온다. **첫 화면은 보안 리서치·제품 발표라 심층 글은 검색으로 찾는다**(`/blog/category/engineering` 경로도 있다) |
| `monzo.com/blog` | ✅ WebFetch 로 목록·본문 다 온다. **연도별 장애 회고 주소가 살아 있다**(`/blog/YYYY/MM/DD/...`) |
| `blog.duolingo.com` | ✅ 읽힌다 — 다만 목록은 `/tag/engineering/` 이 아니라 **`/hub/engineering/`** 이다. **자료가 아니라 결이 기준 미달이었다** |
| `open.nytimes.com` | ❌ **WebFetch 가 받지 못한다**(`unable to fetch`, 사이클 열여섯). **NYT Open 은 Medium 기반으로 보인다**(추정) |
| `fastly.com/documentation` | ✅ **1급 출처로 쓸 수 있다.** 속도 제한 문서가 계수 범위·오차 한도·윈도우 맞바꿈을 예시까지 들어 적는다 — **블로그보다 대가가 정직한 드문 경우**(사이클 열아홉) |
| `engineering.ramp.com` · `builders.ramp.com` | ✅ WebFetch 로 본문이 온다. ⚠️ **`ramp.com/blog/engineering` 은 404** — 주소를 추측하지 말고 검색으로 받는다(사이클 열일곱) |
| `clickhouse.com/blog` | ✅ WebFetch 로 본문이 온다. **글마다 결이 크게 다르다** — 일반론 교육 글과 자기 시스템 글이 섞여 있다 |
...
| **⚠️ 되짚을 때의 순서** | ⚠️ **① 브라우저로 열어 본다(403 을 뚫는다) ② 검색으로 현재 주소를 다시 받는다.** Plaid 는 **뚫는 문제가 아니라 주소가 바뀐 문제**였다 — `engineering.plaid.com`(Medium, 비어 있다) 이 아니라 **`plaid.com/blog`** 로 옮겼다(사이클 스물). **접근 지도의 ❌ 는 그 시점의 사실이지 영원한 사실이 아니다** |
| `plaid.com/blog` | ⚠️ **개별 글이 `engineering.plaid.com`(Medium)으로 리다이렉트된다** — 그런데 **브라우저로는 전문이 읽힌다.** 목록은 비어 있고 **개별 글 주소는 검색으로 받는다**(사이클 스물하나) |
| `www.scylladb.com/blog` | ⚠️ **브라우저로는 열리는데 목록이 렌더링되지 않는다**(제목 하나만 잡힌다). **개별 글 주소를 검색으로 받아야 한다** |
| `deliveroo.engineering` | ✅ 자체 호스팅. 최신 글이 2026-02-27 로 살아 있다 |
