...
**하이퍼커넥트 (KR · 소셜/실시간 영상) — 46번째. 기능 2개 · 도메인 2개. 열어 둔 도메인이 다 찼다. 큐 1/3(컬리).**

256 에서 **`stream-join`(기다렸다가 이어 붙인다)** 을 썼다. 결정 8 · 그림 3 · 생각 4 · 스택 6 ·
난제 2 · ui 6핀 · 연결 1(`objective-relaxation` 과 **추정**).

**🔴 이 기능의 축은 정확성과 지연 사이의 선택이고, 결정들이 사슬로 이어진다**(이 정리는 이 사이트의 것이다).
**Exactly Once 를 껐다**(*"2 Phase Commit(2PC) 매커니즘을 사용하기 때문에 Checkpoint 주기마다 메시지를
발행하게 되어, 실시간성이 크게 저하되는 문제가 있었습니다"*) → **중복이 나갈 수 있게 됐다** →
**Flink 안에서는 못 거른다**(*"독립된 두 서비스 간 중복 발행 여부를 판단할 수 없었습니다"*) →
**Redis 로 밖에서 거른다**(SET NX · Lua, **평균 300ms 에서 3ms 미만**). **보장이 프레임워크에서
인프라로 옮겨 갔고, 그래서 Redis 가 이 파이프라인의 정확성 경계가 됐다.**
⚠️ **Redis 키 설계도 TTL 도, 죽었을 때의 동작도 글에 없다.**

**🔴 배포가 설계의 일부다.** Flink 에 무중단 배포가 없어 Savepoint + Blue-Green + Spinnaker 를 직접
만들었고, 이유가 *"배포 중 발생하는 잠깐의 서비스 중단도 유저 경험과 서비스 품질에 영향을 줄 수
있습니다"* 다. **그리고 상태는 얼릴 수 있어도 시계는 못 얼린다**(이 표현은 이 사이트의 것이다) —
Savepoint 이후 복구까지 흐른 시간 때문에 타이머가 한꺼번에 만료돼 *"복구 시점에 이벤트 조합 성공률이
낮아지는 것을 확인"* 했고, **CheckpointedFunction 으로 복구 시각을 기록해 배포 전에 걸린 타이머만
기간을 연장**한다.

**📌 이 회사에서 굳어진 관찰(둘 다 두 번씩 확인됐다)**
- **설계의 근거는 잘 적고 결과의 크기는 잘 안 적는다.** 목표 지표 3부작은 **버린 접근 여덟에 개선 폭 0**,
  Flink 글은 **버린 대안 다섯에 개선 하나(300ms→3ms)뿐이고 초당 이벤트 수·병렬도·파티션 수·상태
  크기가 전부 없다**(본문의 1초·3초는 설명용 예시값).
- **다만 실패는 적는다** — DAU 목표에서 *"PUR 이 떨어졌습니다"*, 복구 시 조합 성공률 하락.
- **글끼리의 연결은 갈린다** — **목표 지표 3부작은 서로를 링크하는데**(2026-01 이 2024-04 을 건다)
  **Flink(2025-06)와 이벤트 기반 추천(2022-01)은 서로를 언급하지 않는다.** 이어지는지 확인 못 했다.

**🚨 다음 사이클 — 세 번째 도메인을 열지 회사를 닫을지 정한다.**
**후보: 쿠버네티스/인프라 3편(미개봉)**
```
/2025/05/15/Building-Resilient-High-Performance-ScyllaDB-Clusters-with-Super-Disk.html   (RAID 로 성능 유지하며 장애 복구 시간 단축)
/2023/07/25/migrate-half-of-workload-in-a-year.html                                       (1년에 워크로드 절반을 ARM64 로)
/2020/11/02/cluster-migration-in-5-days.html                                              (5일 만에 프로덕션 클러스터 이전)
```
**⚠️ 셋 중 둘이 2020·2023 이다.** **2025 것부터 연다** — 그것이 단단하면 도메인을 열고, 나머지 둘은
**날짜를 반드시 밝히며** 보조로 쓴다. **얕으면 억지로 열지 않고 하이퍼커넥트를 `done` 으로 닫는다.**
**닫는다면 `business_model` 에 위의 관찰 셋을 이 사이트의 정리로 적을지 판단한다** — 근거가 넷(목표
지표 3부작 + Flink)이라 적을 만하다.

**⚠️ 사업 수치를 못 찾았다** — Match Group IR 홈에 하이퍼커넥트도 아자르도 없다. 받은 것은
**LTM 매출 $3.5B**(2026-06-30 종료 분기)뿐. **단서는 2026-01 글이 Match Group 의 2023 2Q 실적을
참조한다는 것뿐이고, 소유 관계를 그것으로 단정하지 않았다.** 다음에 `ir.mtch.com/investor-relations/news-events/quarterly-results/`
나 SEC 파일링을 연다.
...
**⚠️ 한국 편중 — 둘을 다 파면 아홉이다. 다음 후보 조사는 한국 밖에서 한다.**
...
- **`되돌릴 수 있는 것을 먼저 한다`에 CyberAgent `env-split-migration` 이 반례**(되돌림 계획 자체가 없다)
  **이고, 하이퍼커넥트 `stream-join` 은 정반대 사례다** — **되돌리는 길(Savepoint 복원)이 배포 절차
  자체이고 매 배포마다 실제로 밟는다.**
- **📌 하이퍼커넥트 `objective-relaxation` 이 `무엇을 좋다고 부를 것인가` 와 `재 보고 나서` 를 크게
  보강한다** — 두 문서 다 *"잘못 고른 사례가 없다"* 로 끝나는데 **여기가 그 반례다**(DAU 를 올렸더니
  PUR 이 떨어져 목표를 다시 짰다).
