...
**Feldera — 기능 하나를 채웠다(`zset-incrementalization`, 삭제를 셈으로 다뤄 어떤 질의든
증분으로 만든다).** 회사 123개, 큐 0/3. 빈 도메인 둘 — 상태의 표현 · 프로파일링.

### 증분 유지의 진짜 문제는 삭제다

회사가 어려움의 정체를 정확히 짚는다 — 집계나 EXCEPT 같은 연산자는 단조가 아니라서 **입력에
삽입만 들어와도 출력에 삭제를 만든다.** 합계가 바뀌면 옛 값을 지우고 새 값을 넣어야 하므로 변경
하나가 두 행이 된다.

답은 삭제를 특별한 사건으로 두지 않는 것이다. 행마다 정수 개수를 붙여(양수는 추가, 음수는 제거)
삽입과 삭제를 같은 값의 부호 차이로 만들고, **질의 계획의 모든 연산자가 예외 없이 그 위에서
계산한다.** 예외를 하나만 둬도 거기서 증분이 끊겨 전체 재계산으로 떨어지기 때문이다.

### 그래서 구현 부담이 곧 진입 장벽이다

모든 연산자를 음수 가중치를 다루도록 고쳐야 하고, 회사는 그것이 표준 질의 엔진이 복잡한 질의의
증분 뷰 유지를 지원하지 않는 이유라고 본다. 이 회사가 이론을 앞세우는 이유가 여기서 설명된다 —
부분적으로 지원한다는 말이 성립하지 않는 문제라서 전부 되거나 아니거나다(재구성).

### 증거의 모양이 판매 문장과 같다

속도가 아니라 프로그램의 크기를 낸다 — 조인 217개·집계 27개·선형 연산자 287개짜리 실제 운영
프로그램을 16코어 한 대에서 돌려 200ms 에 갱신한다. 다른 시스템과 같은 자로 견주기는 어려워지지만
파는 것(무엇까지 되느냐)과 증거가 일치한다.

### Materialize 와 나란히 놓았다

둘 다 SQL 뷰를 증분으로 유지하는데 바탕이 갈린다 — 차분 데이터플로 대 DBSP. 파는 이야기도 갈린다:
Materialize 는 유지하는 상태의 **메모리 원가**를 깎는 글을 쓰고(과금 단위가 컴퓨트 크레딧이다),
Feldera 는 **무엇까지 증분화되는지**를 말한다. 같은 문제의 두 답으로 `connections` 에 걸었다.

### 다음 사이클 — 확장 (Feldera 의 빈 도메인)

직렬화 글(2026-03-02)은 이미 읽어 뒀고 프로파일러 글 셋의 주소도 받아 뒀다.
