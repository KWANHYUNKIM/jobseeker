...
**Datadog — 진행 중.** 도메인 4개 중 **3개 완료** — 저장·멀티테넌시(`husky-exactly-once`),
수집(`demand-driven-ingestion`), 실시간 시계열(`monocle-timeseries`).
**남은 것은 질의·대시보드 하나다** — 채우면 완주 판정이 가능하다.

> **이 회사의 축.** 원가가 **고객의 사정**에 달려 있다 — 고객이 사고를 겪으면 로그가
> 폭증하는데, 하필 그때가 그 고객이 우리 화면을 가장 절실히 보는 때다.
> **가장 값이 필요한 순간에 가장 비싸진다.** 그리고 받은 데이터가 곧 청구서라
> 중복이 그래프가 아니라 금액의 문제가 된다.

> **작업 메모:** `datadoghq.com/blog/engineering` 은 WebFetch 로 잘 읽힌다.
> IR 보도자료 본문은 **WebFetch 가 타임아웃** 났다 — 검색 결과 스니펫으로 수치를 잡았다.

**큐 1/3 — 목표 미달.** Notion 하나만 남았다. Datadog 을 다 판 뒤 후보 조사를 한 번 끼운다.

마지막 사이클: 120 (2026-08-21) — Monocle 시계열. 논문 2편(**DDSketch 는 Datadog 이
직접 쓴 논문**이라 confirmed). 117~119 는 0편이었다 — 있으면 넣고 없으면 비우는 것이
실제로 작동하고 있다.
