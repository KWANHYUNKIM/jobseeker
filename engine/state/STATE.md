...
**아무도 안 판다 — 360 은 보수 사이클이었다.** 359 에서 드러난 **비교 문서의 인용 유산**을
**한국 회사가 안 섞인 문서만 골라** 정리했다.
**완료 60곳 전부 `done` · 큐 0/3 · 비교 문서 33편.**

**⑭ `--gaps` 는 [후보 조사]** 를 가리켰다. **⚠️ WebSearch 예산 소진으로 불가능해** 보수로 갔다.

### 무엇을 고쳤나 — 9편 · 88건

**⭐ 예외를 먼저 가른 뒤에 돌렸다**(357 의 규칙). 각 문서의 `domains/index.json` 항목에서
`companies` 를 읽어 **한국 회사가 섞였는지**부터 확인했다.

**✅ 고친 9편**(비교 대상에 한국 회사가 없어 우리말 인용은 전부 번역이다):
**which-way-it-leans 25 · machine-says-so 25 · after-you-measure 19 · who-has-to-change 7 ·
what-expires-a-design 5 · what-they-did-not-do 5 · count-not-size 3 · move-the-axis 3 ·
what-guards-the-gate 2**. **전부 0 이 됐다.**

**⭐ 두 편은 인용을 눈으로 전수 확인한 뒤 돌렸다** — which-way-it-leans(Moniepoint·Doximity·
Yelp·Target)와 machine-says-so(LinkedIn·Doximity·Moniepoint). **원문이 영어인 회사들뿐이라
한글 인용은 전부 이 사이트의 번역이었다.** 나머지 일곱은 `companies` 목록으로 갈랐다.

### ⚠️ 일부러 안 고친 6편 · 20건 — 한국 회사가 섞였다

**reversible-first 9**(toss) · **build-or-buy 6**(coupang) · **wrong-yardstick 2**(baemin) ·
**backpressure 1**(baemin) · **payments 1**(naver·toss) · **spatial-units 1**(baemin·coupang).

**⚠️ 이건 기계적으로 못 고친다.** 한 문서 안에 여러 회사 인용이 섞여 있어서 **인용마다 앞뒤
문장을 보고 어느 회사 것인지 갈라야 한다.** 한국 회사 것이면 **한국어 원문 인용이라 그대로
두는 것이 맞다.** **⚠️ 정당한 원문 인용을 굵게로 바꾸면 되돌리기 어렵다** — 그래서 안 건드렸다.

---

**다음 사이클 — 셋 중 하나.**
- **⭐ 1순위 — 남은 6편 20건을 손으로 가른다.** 많은 쪽부터 **reversible-first(9, toss 포함) ·
  build-or-buy(6, coupang 포함)**. **인용마다 앞뒤 문장에서 어느 회사 이야기인지 보고**,
  한국 회사 것이면 두고 그 밖이면 굵게로 바꾼다. **⚠️ 애매하면 두고 STATE 에 적는다.**
- **2순위 — `되돌릴 수 없는 것이 어디에 있는가`(`when-you-cannot-undo.md`) 에 bol 을 더한다.**
  ...
  **⚠️ 갱신하면 `domains/index.json` 의 그 `docs` 항목에 `bol` 과 `move-four-years-of-past` 를
  더하고 최상위 `updated_at` 도 올린다.** (그 문서의 인용 유산은 **360 에서 이미 0** 이다 —
  `when-you-cannot-undo` 는 남은 목록에 없다.)
- **3순위 — 한국 회사 파일 인용 전수 확인**(hyperconnect 128 · kurly 110). 357 에서 일부러
  안 고쳤고 **표본 열여섯 개는 다 원문이었으나 전수는 안 봤다.**
...
CA·AU·SG·AE·NG 각 1) · **비교 문서 33편** · **운영 사실 66개.**
...
**비교 문서의 인용은 하나씩 확인하고 쓴다**(358) · **어떤 파일을 편집하면 그 파일에 남은
유산도 함께 본다**(359) ·
**⭐ 새로: 예외를 가르는 근거를 데이터에서 얻는다 — 360 에서 문서마다 `domains/index.json` 의
`companies` 를 읽어 한국 회사 포함 여부를 갈랐다. 눈으로 세는 것보다 빠르고 안전하다**(360).
