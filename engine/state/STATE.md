...
**없다 — `in_progress` 가 0 이다.** 322 에서 **Paytm 을 `done` 으로 닫았다. 완료 54곳.**

**닫은 근거** — ⑫ 대로 도메인 ②(거래 기록)의 원문을 한 번 더 열어 세고 나서 닫았다.
`How Paytm Handles Millions of Digital Transactions Safely`(2022-09-16)는 **약 500단어
개요글**이고 **결정 다섯 중 대가가 딸린 것은 둘뿐**이다 — Paxos(*"We found that Paxos was
easier to implement than Raft"*)와 LSM(*"flushing can cause more I/Os"*). 나머지 셋은
**근거만 있고 대가가 없고**, **수치는 하나도 없다.** STYLE.md 의 **대가 칸이 비면 그 결정은
쓰지 않는다** 를 지키면 결정이 다섯이 안 된다. **도메인 ②·③ 은 기술 재료만 있는 채로 남겼다.**

**닫으면서 `business_model` 에 적은 것**(이 사이트의 정리라고 밝혔다)
- **⭐ 도메인 넷 중 둘이 기능 없이 남았다.** 열 편 중 **의사결정과 대가를 함께 적은 글은
  둘뿐**(S3 이관 · Prometheus)이고 나머지는 **정책 안내문 · 사용자 안내문 · 수치만 있는
  발표문**이다. **글의 두께가 극단적으로 갈린다.**
- **⭐⭐ 그 두꺼운 두 편이 서로 반대다.** 이관 글은 **대가를 아낌없이 적는데 검증이 한
  문장**이고(50억 개를 옮기고 개수 대조도 체크섬도 없다), 관측 글은 **수치를 앞세우는데**
  (40개월 100% · 비용 2% 미만 · 한 사람) **그 100% 를 어떻게 쟀는지 안 적는다.**
  **둘 다 자기가 낸 결과를 무엇으로 확인했는지에서 멈춘다.**
- **⭐ 없는 도구는 파이썬으로 짧게 만들어 붙인다** — 접근 로그 분석기와 카디널리티 감시기.
  **두 번 다 살 것이 없어서가 아니라 있는 것으로 안 돼서다.**
- **⚠️ 사업의 심장이 글에 없다** — 가맹점 대출 중개(FY26 1분기 561 크로르, **+100%**)와 정산을
  다루는 글이 한 편도 없고, **블로그가 2023년 4월에서 멈췄다.**

---

**다음 사이클 — 큐 1/3 이라 사다리가 [신규 = Freshworks]를 가리킬 것이다.**
**⚠️ 그런데 후보 조사를 먼저 끼우는 편이 낫다** — PROMPT.md 가 직접 그 이유를 적는다:
*"바닥을 치고 나서 채우면 팔 회사가 떨어질 때마다 사이클을 하나씩 버리게 된다."*
**⑭ 대로 `--gaps` 출력을 실제로 보고 정한다.**
- **[신규]면** Freshworks(IN · SaaS)를 열고 프로파일부터. 재료는 318 에서 확인해 뒀다 —
  `Sidekiq queue management using custom middleware`(2020-05-14, 버린 대안 둘 + 인프라 20%
  절감 + 데이터센터 간 24배 차이)와 `How Freshdesk uses HAProxy at scale`(2021-08-24).
  **두 편이 멀티테넌트 부하 격리로 묶인다.**
- **[후보 조사]면** **일본**(Sansan · freee · SmartHR · Money Forward)이나 **유럽**(Personio).
  **인도는 소진됐고**(Zerodha·Paytm 완료 · Freshworks 큐 · Zoho·Ola 는 재시도 안 함 · 나머지는
  전부 막힌 목록) **Trade Republic 은 Medium 호스팅이다.**

**📌 그 밖**
- **⭐ 320 의 `leave-what-costs-more` 가 비교 문서 두 편의 빈칸과 이어진다** —
  `reversible-first.md` 의 **아직 없는 것**에 있는 *"되돌리지 못해 앞으로 밀고 나간 사례가
  없다"* 에 **첫 재료**이고, `where-it-piles-up.md` 의 **갚지 않고 이사한 쪽**(Careem
  270TB→78TB)에서 **한 걸음 더 간다 — 옮기다 실패한 것을 아예 두고 갔다.**
  **다음 비교 문서 사이클이 두 문서를 갱신할 수 있다.**
- **후보 조사 누적 열여섯 곳 중 넷.**
- **⚠️ 낡은 메모를 나르지 않는다**(314·317) — 후보를 나르기 전에 파일부터 대조한다.
- **⚠️ 두 방향 규칙**(298·305·318·321) — **한 편은 반드시 열고, 첫 편이 얇으면 한 편 더 연다.**
  **321 이 반대 방향도 보여 줬다** — 넷을 다 열어 셋을 떨어뜨렸는데 **안 열었으면 Redis 글을
  제목만 보고 올렸을 것이다**(수치는 굵다).
- **분포** — **54곳 전부 `done`.** US 22 포화 · 한국 아홉 · 일본 넷 · EU 일곱 · 중국 둘 ·
  동남아 하나 · **인도 둘** · 아프리카 하나 · 라틴아메리카 0(재시도 금지). **비교 문서 31편.**
