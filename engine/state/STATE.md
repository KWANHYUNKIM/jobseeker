...
**ScyllaDB** (IL/US · SaaS — 데이터베이스) — 도메인 2개 · **기능 1개**(`move-the-sharing-into-the-kernel`). 남은 도메인은 **`남의 API 를 그대로 받아 주면서 그 아래를 다시 짓는다`** 하나다(⏳ 미독).

### 이번 사이클 — `공유를 없애지 못하면 한 층 아래로 내린다`

건진 것 넷:

- ⚠️ **shared-nothing 은 성능 기법이 아니라 제약이다** — 샤드가 **`자기 협력형 스케줄러(리액터)를 돌리고 자기 메모리와 자료구조를 소유해 비싼 샤드 간 잠금과 원자적 연산의 필요를 없앤다`**. **그러니 나눠 쓰자는 제안은 그 값을 도로 내놓자는 말이 된다**(재구성). 실제로 **공유 io_uring 인스턴스가 성능이 아니라 `Seastar 의 shared-nothing 철학을 위반한다` 는 이유로 거절됐다**(락프리는 **기술적으로 불가능**하다고 판단).
- ⚠️ **이 기능의 답 — 공유를 없애지 못하면 그 자리를 한 층 아래로 내린다.** **샤드마다 자기 io_uring 인스턴스**를 두고 **커널 워커 스레드 풀만 `IORING_SETUP_ATTACH_WQ` 로 공유**한다. **유저 공간에는 동기화가 없다.**
- ⚠️ **부분적으로만 지켜지는 원칙은 원칙이 아니다** — **동기 I/O 를 바로 실행하던 `빠른 길 추측 경로` 를 제거**했다. 이유가 명확하다: **`애플리케이션 샤드가 동기 I/O 시스템 호출을 계속 직접 실행하면, I/O 를 네트워킹 코어로 넘기는 목적 자체가 무너진다.`** **최적화 하나를 내주고 보장을 샀다.**
- ⚠️ **맡긴 곳이 값을 부른다** — 유저 공간의 공유를 커널에 맡겼는데 **바로 그 커널이 하는 버퍼 복사가 병목이 된다.** **단일 네트워킹 코어가 주기의 40~50% 를 메모리 복사에 쓰고**, 회사가 스스로 정리한다 — **`제로 카피 수신 파이프라인이 없으면, 시스템 호출을 피해 얻은 이득이 메모리 전송 비용에 부분적으로 상쇄된다.`**

### ⚠️ 진 벤치마크를 그대로 싣는다

**`순수 I/O 위주 벤치마크에서는 비대칭 백엔드가 전통적인 linux-aio 백엔드를 이기지 못한다`** — 그리고 왜 지는지까지 적는다. **계산이 섞이면 뒤집힌다**(요청당 2마이크로초면 처리량·지연 모두 앞선다). **디스크는 차이가 없고 `한계가 Seastar 가 아니라 디스크로 보인다`** 고 적는다. **버퍼 링은 `Seastar 의 버퍼 관리에 더 큰 아키텍처 변경이 필요해` 미뤘다고 밝힌다.** ⭐ **비교 문서 ③축(`자기 성과를 어디까지 주장하나`)의 세 번째 사례가 이렇게 확인됐다.**

### 다음 사이클 — 마지막 도메인, 그리고 완주 판정

`--gaps` 가 **`남의 API 를 그대로 받아 주면서 그 아래를 다시 짓는다`** 를 부를 것이다. ⏳ **본문 미독** — **`DynamoDB API 용 새 Rust 드라이버로 처리량 58% 향상`**(2026-08-27)과 **`Rust ↔ C# 자체 비동기 FFI 프레임워크`**(2026-08-31). ⚠️ **슬러그 추측 금지 — 검색으로 주소를 먼저 받는다.** 자료가 서면 **같은 사이클에서 완주 판정까지** 한다(도메인 2/2 · 고아 없음 · 수익원 둘 다 연결).

⚠️ **큐는 1/3**(Cygames).

### ⚠️ 비교 문서 재료 (초안 유지)

**① `AI 에이전트를 어디까지 믿나` 7곳** — Sentry / ClickHouse / Duolingo / Ramp / DoorDash / Deliveroo / Snyk.

**② `관리형 MySQL 의 한계` 3곳 + ⏳ Cygames(TiDB)** — Etsy / Plaid / Paystack.

**③ ⭐ `자기 성과를 어디까지 주장하나` 3곳 — 재료가 찼다** — **Snyk**(`언어별 작은 격차의 순서는 잡음 밖이 아니다`) / **Grafana**(`더 확장성이 좋다고는 말하지 않겠다`) / **ScyllaDB**(**진 벤치마크와 그 원인을 그대로 싣는다**). ⏳ Etsy(승자의 저주) · Deliveroo(불확실성 표현 고정)도 같은 결이다.

**④ `인도 규모에서 무엇이 달라지나` 2곳** — Meesho / Zepto.

⏳ **⑤ `한 코어에 하나씩인가, 여러 코어가 나눠 쓰나`** — ScyllaDB 대 ClickHouse. ⭐ **ScyllaDB 쪽 기능이 이번에 하나 생겼다.**
