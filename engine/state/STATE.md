...
**없다 — Duolingo 를 완주했다.** 큐는 **1/3**(Ramp). 다음 사이클은 `--gaps` 가 **신규(6순위)**를 부른다.

### 이번 사이클 — 테스트 `파이프라인이 리뷰가 삼킬 수 있는 속도보다 빨리 만든다` 와 완주

⚠️ **앞 사이클이 기능을 쓰고 검증까지 통과한 채 커밋 전에 끊겼다.** `git status` 로 확인해 이어받았다 — **한 사이클 = 한 커밋 규칙이 이런 상황을 위해 있다.**

건진 것 넷:

- ⚠️ **자동화가 성공할수록 병목이 사람 쪽으로 옮겨 간다** — `The pipeline can produce PRs faster than the existing review process can absorb them`. **250개 전부를 사람이 리뷰했고**, 회사의 답은 문을 여는 것이 아니라 **앞을 거르는 것**이다(두 번째 리뷰어 에이전트). **그러면 그것은 누가 보는가가 남는다**(재구성).
- ⚠️ **도구를 위해 제품 코드를 바꿨다** — AI 가 테스트를 잘 쓰게 하려고 **구체 인스턴스 대신 프로토콜**로 코드베이스를 옮겼다. **이 기능에서 가장 큰 대가다.**
- ⚠️ **쉬운 파일부터 간다** — 파일 타입 **2.5x** 대 커버리지 공백 **0.5x**. **가장 테스트가 필요한 코드가 뒤로 밀린다**(재구성).
- ⚠️ **고칠 권한이 곧 위험이다** — CI 자동 수정 에이전트가 **프로덕션 파일을 고친 사고**. 대응은 아직 진행 중(`We're actively adding guardrails`). **사고가 나고 나서 규칙을 세우는 것이 Monzo·Wise 와 같은 모양이다.**

### 비교 문서가 준비됐다 — 다음 사이클의 유력 후보

⚠️ **`사내 AI 에이전트를 어디까지 믿을 것인가` 축에 세 회사가 찼다.**

| 회사 | 어디까지 맡기나 | **무엇이 그 선을 강제하나** |
|---|---|---|
| **Sentry** | AI 가 PR 까지 열고 **병합은 사람이** · 매일 밤 에이전트를 심문 | 조직의 신뢰(`Your org might not be ready to trust the machines that far`) |
| **ClickHouse** | 분석의 **70%**, 남긴 **30%** · 출력만으로 결정 금지 | ⚠️ **세 겹 중 한 겹만 기계가 강제**(접근 범위) |
| **Duolingo** | 테스트 PR **하루 20개** 자동 생성 · **250개 전부 사람 리뷰** · 쓰기는 승인 버튼 | 사람 리뷰 — **그래서 병목이 됐다** |

**축은 `얼마나 맡기나` 가 아니라 `그 선을 무엇이 지키나` 다** — 규율이냐, 접근 범위냐, 사람의 시간이냐. **Ramp 를 파면 네 곳이 된다**(병합 PR 의 30%를 에이전트가 쓴다).

**다음 사이클은 신규(Ramp)가 사다리 위에 있다.** 그것을 완주한 뒤 비교 문서(8순위)를 쓰는 것이 순서다.
