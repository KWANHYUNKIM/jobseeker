...
**없다. 305 는 후보 조사였다.** Meituan 을 닫아 진행 중인 회사가 없었고 `--gaps` 가
[후보 조사]를 가리켰다. **큐 0/3 → 1/3.**

**⚠️ ⑲ 를 먼저 돌렸다** — 완료 목록을 회사 단위로 대조했다. **52곳**이고 분포는
**US 22 · KR 9 · EU 7 · JP 3 · CN 2 · 기타 3 · CA·AU·SG·IN·AE·NG 각 1** 이다.
**`in_progress` 로 남아 있는 것은 kakao 와 roblox 둘뿐이다**(각각 `hold_reason` 달린 도메인
하나 때문 — 290 에서 확인).

**셋을 보고 하나만 올렸다**(근거는 QUEUE 의 '확인해 둔 후보'에 적었다)
- **✅ Cybozu (`blog.cybozu.io`, JP · SaaS)** — **본문 한 편을 실제로 열어 확인했다**
  (`20년 넘은 SQLite 기반 앱을 쿠버네티스로`, 2026-08-27). **기준 셋을 다 넘는다** —
  **StatefulSet 을 버린 이유가 명시되고**(릴리스마다 다운타임이 난다) **Deployment +
  Pod Affinity + ReadWriteOnce** 로 갔다 · **한계 여럿**(같은 볼륨의 배타 제어가 되려면
  **파드가 같은 노드에** 있어야 한다 · 파일이 **수억 개**라 정기 점검 시간 안에 다 못 옮긴다 ·
  커널 동작을 직접 조사해야 했다) · **수치**(약 **300만 파일을 15분**에 전송 · 캐시 검증에
  5GB 랜덤 데이터 · 파드 메모리 상한 1Gi). **⭐ 20년 된 C++ CGI 앱을 옮긴 이야기라 이
  사이트에 없는 종류다.**
- **❌ Xiaohongshu** — **전용 엔지니어링 블로그가 없다**(WeChat 공식계정 위주로 보인다 — 추정).
- **❌ HelloFresh** — `engineering.hellofresh.com` **403**.

**⭐ 298 에서 굳은 것이 또 맞았다** — **"1차 자료가 있다"와 "쓸 수 있다"는 다르다.** 그리고
**Cybozu 는 목록만 봤으면 떨어뜨렸을 것이다** — 최근 글 다섯 중 넷이 행사·채용·회고였고
기술 글은 하나뿐이었다. **그 하나를 열었더니 기준을 다 넘었다.** ⑫ 의 반대 방향 사례다.

---

**다음 사이클** — 큐가 1/3 이라 사다리 3순위(큐 0)는 안 걸린다. `--gaps` 는 **[신규]**
(Cybozu 착수)를 가리킬 것이다. **출력을 실제로 보고 따른다.**
- **Cybozu 를 시작하면** 회사 프로파일부터다(PROMPT.md 3단계). **⚠️ ⑲ 를 스크립트 안
  `assert` 로 막고 쓴다**(299 에서 그렇게 했다). 도메인 후보 하나는 이미 있다 —
  **20년 된 앱을 멈추지 않고 쿠버네티스로 옮기기**(`/entry/2026/08/27/090000`).
  **⚠️ 두 번째 편이 필요하다** — 목록에 기술 글이 드무니 `blog.cybozu.io` 를 더 뒤져야 한다.
  Cybozu 는 사내 쿠버네티스 기반(Neco)을 오래 공개해 왔으니 그쪽을 찾는다.
- **📌 Grab 보강 후보**(이미 `done`) — Palana 2부작 · Agent platform Part 1 · **Iceberg** ·
  **Counter Service 저장소 이전**. 뒤 둘은 기존 기능의 후속이다.
- **📌 kakao·roblox 를 닫는 것도 한 사이클이 된다** — 각각 `hold_reason` 이 달려 있어
  `--gaps` 는 확장 대상에서 빼지만, **hold 를 유지한 채 `done` 으로 닫는 것이 정당하다**(290).
  **닫으면 완료 수가 정리된다.**

**⚠️ 분포** — 52곳. **US 22 로 포화. 한국 아홉으로 편중.** Cybozu 가 들어가면 **일본이 넷**이
된다. **빈 자리는 라틴아메리카 하나뿐이고 재시도 금지.** 동남아는 Grab 하나
(Shopee·Tokopedia·VNG 가 298 에서 다 막혔다). **중국은 둘이고 Xiaohongshu 는 블로그가 없다.**

**⭐⭐⭐ 비교 문서가 계속 밀려 있다. 축 셋 다 굵고 하나는 재료가 여섯이다.**
1. **`문지기를 무엇으로 세우는가` — 가장 굵다.** Wix 컴파일러(타입 정의로 컴파일해 통과한
   것만) · Meituan AI 감사(제출 전, 사람 리뷰어는 업무 로직만) · Meituan 샌드박스 실행
   (학습 데이터) · Meituan 루브릭 이진화(일치율 85~90% 문턱) · Wix AirBot 타입 강제
   (*"LLMs are inherently non-deterministic, but SRE automation demands reliability"*) ·
   monday.com Morphex. **⭐ 축의 핵심 둘** — **① 대가가 한 방향으로 모인다: 검사할 수 있는
   것만 검사되고 그 몫이 얼마인지는 어디에도 없다** · **② 문지기의 성질이 갈린다: 컴파일러는
   못 잡는 범위가 분명한데 AI 문지기는 무엇을 못 보는지가 흐릿하다.**
2. **`에이전트에게 무엇을 못 하게 하는가`** — 재료 일곱, 세 갈래.
3. **`자를 어떻게 세우는가`** — Snap 넷 + Meituan 둘.
**또는 기존 `reversible-first.md` 보강**(Wix 두 이관 기능이 양극을 한 회사 안에서 보여 준다).
