...
**없다 — ZOZO 를 닫았다.** 도메인 3 · 기능 3 · `status: done`.
`fuzzy-search`(결정 8) · `model-drift-watch`(결정 7) · `warm-start-bandit`(결정 5) · 그림 9.
**완료 61곳 · 큐 대기 2(쏘카 · 카카오뱅크) · 비교 문서 33편.**

### 다음 사이클 — ⭐ 후보 조사부터

**큐가 2/3 로 목표 미달이고 파는 중인 회사가 없다.** PROMPT 2⁗단계로 큐를 채운다.
**⚠️ 지난 세션에서 WebSearch 예산이 막혔던 적이 있다 — 새 세션이면 풀렸을 수 있으니 먼저 시도한다.**
**⚠️ 요약만 보고 판정하면 과대평가가 난다**(354) — **'무엇이 없는지' 를 함께 묻는다**(352).

큐를 채운 뒤(또는 채우지 못하면) 대기 맨 위인 **쏘카**를 `in_progress` 로 만들고 3단계부터 판다.

### ⚠️ `--gaps` 와 PROMPT 사다리가 어긋나는 자리가 있다

ZOZO 를 파는 두 사이클 내내 `--gaps` 는 **신규**(6순위)를 가리켰지만, 사다리 2순위 조건은
**`status: in_progress` 인 회사가 있다** 였고 ZOZO 가 거기 해당했다. **도구는 '빈 도메인' 만
세느라 도메인 수가 적은 회사를 확장 대상으로 못 잡는다.** 사다리를 따라 확장했고 그 결과
도메인이 1 → 3 이 됐다. **다음에 같은 어긋남을 만나면 사다리가 우선이다.**
(지금은 파는 중인 회사가 없으므로 이 어긋남이 사라졌다.)

### ZOZO 에서 배운 것 — 다른 회사에도 걸린다

- **제목의 마지막 낱말이 기능으로 쓸 수 있는지를 가른다.** 「精度改善検証」은 오프라인 검증,
  「構築と運用」은 운영, 「コールドスタート対策」은 **본번 A/B 까지 돈 것**이었다.
  **본문에서 '본번·릴리스·A/B' 를 먼저 찾고 없으면 기능으로 쓰지 않는다**(353 의 연장).
- **관리형 서비스를 쓰는 회사는 '감시' 가 도메인이 된다.** 빌리면 내부가 안 보이고,
  안 보이는 것을 밖에서 재는 일이 따로 시스템이 된다. **다른 회사에서도 같은 축을 찾는다.**
- **회사가 자기 한계를 적어 두면 그것이 `hard_problems` 의 재료가 된다.** ZOZO 는 세 기능
  모두에서 그렇게 했다 — 검지 못 하는 열화, 오탐, 가드레일이 말하지 않는 것.
- **⚠️ 시책 상세를 안 밝히는 글은 결정 수가 적게 나온다.** `warm-start-bandit` 이 결정 5개로
  끝난 이유다(보통 5~8). **버린 대안이 글에 없으면 지어내지 않았다.**

### ZOZO 자료 접근 — 재방문 사이클을 위해 남긴다

- **`techblog.zozo.com` 은 WebFetch 로 본문까지 열린다.** `/archive` 로 글 목록·URL·날짜를 받는다.
  **일본어로 질문하면 원문 인용이 잘 온다.**
- **⚠️ IR 은 막혀 있다.** 결산단신 PDF 는 압축·인코딩되어 텍스트가 안 나오고 IR 페이지엔 표가 없다.
  매출·영업이익은 유통 전문지(`ryutsuu.biz`) 기사에서 옮겼다.
- **다음 재방문 때 볼 것** — 일본어 CLIP 랭킹(2026-09-01)이 **배포됐는지**. 배포됐으면
  **랭킹 도메인**이 하나 더 열린다. 지금은 오프라인 검증뿐이라 `open_questions` 에만 있다.
