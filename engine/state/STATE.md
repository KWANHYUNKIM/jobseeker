...
**없다 — 문체 보수 사이클이었다.** Paystack 을 배민 문체로 다시 썼다(2.52 → 0.0). 회사 115개, 큐 2/3. 39곳 남았다.

### 갈아 끼우는 이야기가 넷이 됐는데 성공의 정의가 저마다 다르다

첫 기능에 이전 4사 축을 세웠다.

- Atlassian 은 무중단을 지키되 일부러 느리게 옮겼다.
- Honeycomb 은 무중단을 포기하고 데이터를 지켰다.
- 볼(bol)은 과거 4년 치를 두고 갈 수 없어 함께 옮겼다.
- Paystack 은 **끝난 뒤에 아무것도 안 바뀌어 있는 것**을 성공으로 삼았다.

넷 중 가장 소극적으로 들리는데 실은 가장 까다롭다. 제시간에 끝냈다는 것은 즉시 검증되지만
아무것도 안 바뀌었다는 것은 시간이 지나야 알기 때문이다. 같은 아마존 마감을 맞은 Plaid 는
플랫폼을 통째로 갈아탔고(TiDB) 여기는 제자리에서 올렸다(MySQL 8.0).

### 남의 규칙을 다루는 세 가지 답

둘째 기능에 축을 하나 더 세웠다.

- Plaid 는 은행마다 다른 규칙을 대신 흡수해 하나의 길로 보여 준다.
- Ramp 는 흡수하지 않고 고객이 규칙을 직접 조립하게 한다.
- Paystack 은 흡수할 수도 조립하게 둘 수도 없는 것 — 통신사가 정한 180초 — 을 문서에 그대로
  적고 그 한계를 고객에게 알리라고 말한다.

같은 나이지리아의 Moniepoint 와도 갈린다. 저쪽은 가용성을 상품으로 삼아 통제할 수 있는 것을
끝까지 쥐었고, **여기는 통제할 수 없는 시계를 인정해 API 의 계약으로 만들었다.**

### 고친 것 하나 — country 가 `기타`였다

본문이 라고스·나이지리아를 계속 말하는데 `country` 가 `기타`로 들어가 있었다(허용값에 NG 가 있다).
회사 파일과 index.json 을 함께 NG 로 고쳤다. **새 조사가 아니라 문서가 이미 말하고 있던 것과
메타데이터를 맞춘 것이다.** 앞 사이클에 적어 둔 1Password 의 `기타`(CA·SaaS 로 보인다)는 본문에
소재지 언급이 없어 그대로 뒀다 — 그쪽은 자료 확인이 필요하다.

### 다음 사이클 — 문체 보수 (사다리 3순위)

--gaps 가 정한다. 지금 맨 위는 doordash(2.38)지만 그건 이 시점의 예상이지 확정이 아니다.
