...
**QuestDB — 회사 프로파일까지 썼다(in_progress).** 회사 122개, 큐 1/3.

도메인 셋을 세웠고 기능은 아직 0개다.

- **넣는 길과 꺼내는 길을 하나로 만든다** — `QWP: 자체 이진 와이어 프로토콜`(2026-08-17)
- **자바 위에서 사이클을 센다** — cmov·JIT·off-heap 글 셋(제목과 날짜만 확인)
- **시계열의 모양을 표현과 연산에 넣는다** — 심볼 사전·고릴라 압축(QWP 글) + WINDOW JOIN 글(제목만)

### 프로토콜 축의 세 번째 답이 들어왔다

이번 큐에서 같은 질문에 세 회사가 다르게 답했다.

- **DuckDB** — 자체 형식을 만들었다. 밖에서 통제하는 형식에 묶일 수 없다는 이유.
- **Dolt** — 남의 것을 흉내 냈다. 생태계를 얻고 배수로 평가받는다.
- **QuestDB** — 남의 것 셋을 쓰다가(쓰기는 ILP, 읽기는 PGWire, 그리고 REST) **자기 것 하나로 합쳤다.**
  이유가 특이하다 — 데이터베이스가 수집 파이프라인과 하류 소비자 사이에 놓이면서 **병목이 계산이
  아니라 결과를 옮기는 일**이 됐기 때문이다. 수집 초당 530만 → 1,900만 행, 결과 초당 2억 2천만 행.

### 오픈코어의 경계가 유난히 뚜렷하다

오픈소스판은 단일 인스턴스이고 복제도 페일오버도 없으며 전송이 암호화되지 않는다. 회사가 그것을
평가·프로토타입·파일럿용이라고 스스로 적는다. 기술 글에도 그 선이 나온다 — 자체 프로토콜의
고가용성 기능을 설명하면서 오픈소스판은 복제 계층이 없어 독립 노드로의 페일오버만 된다고 밝힌다.

이번 큐에서 오픈소스를 다루는 방식이 넷째로 갈렸다. DuckDB(비영리 재단 + 회비) · Dolt(아파치 코어 +
호스팅) · CedarDB(커뮤니티 입구 + 지원) · QuestDB(**운영하려면 사야 한다**).

### 자바로 저지연을 만드는 축은 이 엔진에 없었다

보통 이 자리의 회사들은 C·C++·러스트로 간다. 여기는 JVM 위에서 조건부 이동 명령이 분기보다 비쌀 수
있다는 이야기, JIT 이 비트를 추론하면 마스크가 사라진다는 이야기, 히스토그램을 힙 밖으로 옮겨 8.9배를
얻은 이야기를 쓴다. 다음 사이클들에서 이 글들을 연다.

### 다음 사이클 — 확장 (QuestDB 의 빈 도메인)

QWP 글이 수치와 대가를 함께 적어 가장 두껍다.
