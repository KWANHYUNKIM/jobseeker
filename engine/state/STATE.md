...
**Target (US · 커머스/리테일 물류) — 42번째. 기능 3개 · 도메인 3개. 셋 다 찼다.**

- 220 **`split-shipments`** — 폭발이 두 번(**2ⁿ−1** 배송 후보 · **벨 수 Bₙ** 이행안), 그리디가
  지는 이유는 **계단형 비용 함수**. 그래서 그리디를 **출발점으로만** 쓴다. 시간 8% 로 격차 91%.
- 221 **`infra-showback`** — **차지백을 명시적으로 버렸다.** 점수는 **제품 단위**, 재료는 **ITDP**.
  주문당 비용 4년 **−33%**. DBaaS 글과 겹쳐 읽어 **보여주는 층과 막는 층(쿼터)이 따로**임을 봤다.
- 222 **`layout-bandits`** — **A/B 테스트와 정적 배치를 각각 이유를 대고 버렸다.**
  **LinUCB** · 릿지 회귀 · **p95 25ms** · **증분 수요 $50M**. 대상은 지면 전체가 아니라 캐러셀 하나.

**222 에서 두 글을 열어 보고 하나만 썼다.** `targetrun-shopping-offers`(2026-05)는
**매장·재고·이행과 이어지지 않고**(채널 예측은 *향후 과제*) **대가가 한 줄도 없어**
기능으로 쓰지 않고 **도메인 `tech` 한 칸으로만** 넣었다(Bi-LSTM+어텐션 · Focal Loss ·
정확도 +15% · 과대예측 −44% · 저사용 손님 사용률 +78%). **밴딧 글이 훨씬 단단했다.**

**⚠️ 다음 사이클의 선택 — 셋 다 찼으니 `--gaps` 는 [신규]를 가리킬 것이다. 그래도
회사를 다 판 것은 아니다.** 남은 재료 둘:
- **`주문을…쪼갤 것인가` 의 두 번째 기능** — `math-behind-inventory-delivery-optimization`
  (2025-11, *들어오기 전* 재고 배치). 하루 약 20만 품목 · 수조 변수 → 수백만 변수 ·
  품절 −25~30% · 배송비 −0.4% · 과포장 −1.7%. ⚠️ **버린 대안이 명시돼 있지 않다 —
  쓰기 전에 결정이 5개 나오는지 세어 보고 안 나오면 무리하지 않는다.**
- **`무엇을 언제 보여줄지…` 의 두 번째 기능** — 아직 안 읽은
  `accessory-recommendations-with-llms`(2025-07) · `buy-it-again-part-2`(2025-03).
- **또는 Target 을 done 으로 닫는다.** 세 도메인이 다 찼으니 닫아도 말이 된다.

**🚫 분류별 목록은 못 찾았다** — `?category=Infrastructure` **404**, 첫 화면 분류 링크는
**href 가 본문에 안 실린다**(JS). **페이지 URL 형식도 모른다. 새 단서 없이 다시 안 두드린다.**

**읽었지만 안 쓴 글** — `synthetic-digital-orders`(2025-07-22, 합성 주문 · 통합 2배 ·
배분 정확도 약 40%). **버린 대안도 대가도 없어 얕다.**
