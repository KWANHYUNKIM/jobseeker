...
**Dolt — 회사 프로파일까지 썼다(in_progress).** 회사 120개, 큐 0/3.

도메인 셋을 세웠고 기능은 아직 0개다.

- **버전을 저장 자료구조에서 만든다** — `Prolly Trees`(2024-03-03)
- **버전 관리를 SQL 문법 안으로 넣는다** — README(시스템 테이블·함수·프로시저)
- **남의 데이터베이스인 척 붙는다** — `Doltgres is as Fast as MySQL is Slow`(2026-08-13)

### 버전 관리를 기능이 아니라 표현의 문제로 옮겼다

애플리케이션 층에서 버전을 만들면 과거를 복사하거나 변경 로그를 따로 쌓게 되고, 두 버전의 차이를
물으면 양쪽을 다 훑어야 한다. 이 회사는 그 일을 저장 자료구조로 내려 보낸다 — 프롤리 트리는 청크
경계가 내용으로 정해지므로 같은 데이터가 저절로 같은 청크가 되고, **차이를 트리 크기가 아니라 차이의
크기에 비례하는 시간에** 구한다. B-트리라면 양쪽을 훑어야 한다는 대비를 회사가 직접 적는다.

손댄 곳도 구체적이다. 원래 Noms 구현의 롤링 해시는 청크 크기가 기하분포가 되는데, 큰 청크는 그 안의
이진 탐색이 느리다. 누적분포를 써서 4KB 근처 정규분포로 몰았고, 경계를 계산할 때 값은 보지 않고 키만
봐서 같은 크기의 값 갱신이 경계를 밀지 않게 했다.

### 흉내 내는 쪽은 언제나 배수로 견주어진다

MySQL·Postgres 와이어 호환을 택한 대가가 그것이다. Postgres 15.5 기준 지연 배수로 Doltgres v1.0 이
2.6배(v0.56.3 의 4.4배에서), 돌트가 1.92배다. 고친 것을 수치와 함께 나열한다 — 128행씩 모아 보내는
버퍼드 플러시 200%+, 고루틴 셋으로 나눈 스풀링 50%, 커버링 인덱스 버그 21.5%, 선택도 낮은 필터에서
인덱스 대신 스캔을 고르게 한 것이 중앙값 43.8%. 그러고도 아직 갈 길이 멀다고 적는다.

### 자기가 몇 배 느린지를 자기 소개 글에 적는다

자료구조를 소개하는 글 안에 sysbench 에서 MySQL 보다 약 2배 느리다는 문장이 그대로 있다. 원인을
자료구조가 아니라 구현 언어(Go 대 C)로 돌리기는 하지만, 숨기지 않는다. DuckDB·안티테시스에 이어
**대가를 먼저 적는 회사가 셋 연속**이다 — 이번 후보 조사에서 고른 기준(회사 도메인에 엔지니어가
직접 쓰는 블로그)이 그런 글을 골라낸 것으로 보인다(재구성).

### 다음 사이클 — 확장 (Dolt 의 빈 도메인)

큐가 0이 됐다. Dolt 를 다 판 뒤 후보 조사를 끼운다.
