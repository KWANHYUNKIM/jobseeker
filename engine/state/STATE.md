...
**없다 — 문체 보수 사이클이었다.** 사이버에이전트를 배민 문체로 다시 썼다(1.3 → 0.0, 기호 437개). 회사 115개, 큐 2/3. 14곳 남았다.

### 보는 값을 어떻게 치르느냐 — 여섯 번째 답

- VictoriaMetrics — 카디널리티가 비용의 정체다.
- Honeycomb — 그 카디널리티를 무제한으로 열어 상품 쪽에 놓는다.
- Datadog — 보고 있는 것만 모은다.
- Razorpay — 감사로 안 쓰이는 것을 찾아 버린다.
- Databricks — 버리되 되찾을 수 있게 버린다.
- 사이버에이전트 — **보내기 전에 걸러 낸다.** 관리형에 맡기면 총비용이 높아진다고 보고 수집기를
  직접 굴린다.

그리고 이 회사의 글 둘이 서로 다른 화폐로 같은 값을 치른다 — **하나는 돈이고 하나는 주의력이다.**

### 알림의 형해화 — 컬리와 같은 이름, 다른 원인

- 컬리는 오탐이 잦으면 알럿을 대수롭지 않게 여기는 **양치기 소년 현상**이라 부른다.
- 사이버에이전트는 노드를 올릴 때마다 Slack 이 수백 건의 Pod 이상 통지로 차는 것을
  **알림의 형해화**라 부른다.

막으려는 것이 감시 체계가 아니라 사람의 습관이라는 점은 같은데, **여기는 정상적인 작업이 만드는
소음이라 임계를 낮추는 것으로는 안 풀린다.**

### 몰림을 시간에 걸쳐 다루는 셋

Klaviyo(마감 이른 것부터) · 컬리(고정 구간에 모아서) · 사이버에이전트(**내보내는 쪽의 몰림**).
그리고 이 문제가 자기 미디어 때문에 더 커진다 — 편성이 있는 실시간 스트리밍이라 시청이 시각에
몰리고 **그 파도가 그대로 광고 요청의 파도가 된다.** 광고와 미디어를 같이 가진 회사에서만 나오는
모양이다.

### 도메인을 사업이 아니라 광고와 미디어를 같이 가져서 생기는 문제로 잘랐다

언제 얼마나 낼지, 얼마나 빨리 읽을지, 옮기는 동안에도 나가게 할지, 그 전부를 보는 값을 어떻게
치를지. 무중단의 근거도 다르다 — **크라켄은 계약이 요구하고 여기는 손익이 요구한다.**

### 다음 사이클 — 문체 보수 (사다리 3순위)

--gaps 가 정한다. 지금 맨 위는 wayfair(1.25)지만 그건 이 시점의 예상이지 확정이 아니다.
