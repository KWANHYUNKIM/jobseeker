...
**Jane Street — 프로파일만 썼다.** 회사 137개, 큐 1/3(Zig 만 남음).
도메인 셋 중 둘은 자료를 확보했고 하나(하드웨어)는 자료를 안 읽어 tech 두 칸이 `unknown` 이다.

### 사업이 안 보이는 회사를 어떻게 적었나

자기 자본으로 거래하는 곳이라 요금표도 매출도 없다. 큐에 미리 적어 둔 대로 **수익원 서술을
얇게 잡고**(`revenue_streams` 를 `unknown`) 기술 도메인 위주로 세웠다. 이 엔진의 다른 회사들이
"무엇을 팔아서 이 기술이 필요한가"로 읽힌다면 여기는 **제약이 기술을 정하는 쪽**이다.

### 남이 정한 기본값을 어디까지 받아들일 것인가

도메인 셋이 층을 따라 내려간다. 언어를 고쳐 쓰고(OxCaml — 타입으로 데이터 경쟁을 막고
레이아웃·할당을 통제), 커널의 기본값을 의심하고(NFS·GSS 순번 캐시 버그를 파서 리눅스 6.16 에
패치를 올렸다), 하드웨어까지 내려간다(Hardcaml·FPGA·ASIC).

### 자기 확장에 안정성을 약속하지 않는다

OxCaml 은 상위 OCaml 과 하위 호환이지만 **자기 확장에 대해서는 안정성도 하위 호환도 약속하지
않고**, 일부 사내 라이브러리는 OxCaml 에서만 돈다. 실서비스 컴파일러를 실험실이라 부르는 자리다.

### 재현이 일의 본체였다

권한이 멀쩡한데 파일 복사가 드물게 권한 오류로 실패했다. FUSE 파일시스템을 짜고, bpftrace 로
계측하고, 와이어샤크 플러그인을 만들고, NFQUEUE 로 패킷을 늦춰 불러냈다 — **결정적인 것은
지연을 충분히 만들어 주는 아주 작은 시험용 NFS 서버**였다.

### 다음 사이클 — 확장 (Jane Street 의 빈 도메인)

`언어를 고쳐 성능을 만든다` 부터. OxCaml 의 네 갈래와 대가가 손에 있다.
