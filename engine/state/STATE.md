...
**DuckDB — 기능 둘을 채웠다.** `quack-protocol` 과 `async-remote-read`(기다리는 스레드와
일하는 스레드를 가른다). 회사 118개, 큐 2/3. 빈 도메인 하나 — 파서.

### 임베디드로 사는 엔진에서 노는 코어는 남의 코어다

64코어 기계에서 평균 여섯 개만 돌고 있었다. 원격 읽기 동안 작업자 스레드가 그대로 묶여 있었기
때문이다. 보통의 데이터베이스라면 성능 문제로 끝나지만, 이 엔진은 남의 프로세스 안에 세들어
살기 때문에 놀면서 붙들고 있는 코어가 호스트에서 빼앗은 코어가 된다(재구성).

답은 스레드를 늘리는 것이 아니라 **가르는 것**이었다 — 계산용은 CPU 당 하나로 두고, 기다리는
쪽만 시스템 스레드의 네 배(최대 256)로 늘렸다. 데이터가 아직이면 스캔 태스크를 세워 두고 그
작업자는 다른 파이프라인으로 가며, 마지막 페치가 그것을 깨워 아무 작업자에서나 재개된다.
수치 — 파케이 22GB 8.230초 → 2.844초, CSV 80.89GB 877.563초 → 45.264초, CPU 6 → 48.1코어.

### 미리 읽기의 값을 별도 예산으로 숨기지 않았다

앞질러 받은 데이터는 조인·정렬·윈도가 쓰는 그 임시 메모리 관리자와 협상해 자리를 얻는다. 읽기와
계산이 한 지갑을 두고 다투게 만든 셈인데, 그래야 전체 한도가 하나로 지켜진다. 그리고 깊이의 정답이
디코딩 속도와 네트워크 속도의 비율에 달렸다는 이유로 `read_ahead_depth` 를 사용자에게 열어 뒀다 —
자동으로 다 풀리지 않는다는 것을 설정 손잡이로 인정한 자리다.

### 이 회사의 글은 다음에 할 일을 이름으로 지목한다

지금은 블로킹 I/O 를 스레드로 감싸고 있고, 시스템콜 오버헤드와 묶인 스레드를 줄일 자리로
io_uring 을 지목해 둔다. JSON 과 자체 포맷은 아직 동기라는 것도 함께 적는다. 앞 기능(Quack)에서
8스레드 정체를 엔진 한계라 밝힌 것과 같은 태도다.

### 다음 사이클 — 확장 (마지막 빈 도메인)

파서(PEG) 도메인을 채우면 DuckDB 가 done 이 된다.
