...
**Jane Street — 기능 둘.** 남은 빈 도메인은 하드웨어 하나(자료 미독). 회사 137개, 큐 1/3.

### 증상은 위층에, 원인은 아래층에

거래 데이터를 나르는 시스템에서 **권한이 맞는데 권한 오류가 났다.** 그러면 그건 권한 이야기가
아니다 — 커버로스 NFS 가 응답 지연 때 같은 XID 에 새 GSS 순번을 붙여 재전송하고, 늦게 온
원래 응답을 새 순번으로 검사해 실패하고 있었다. **RFC 는 순번 캐시를 두라고 했는데 리눅스가
그걸 구현하지 않았다.**

### 재현이 일의 본체였다

드문 버그는 고쳐도 고쳐졌는지 알 수 없다. 그래서 방아쇠를 손에 쥐었다 — 무작위 데이터를 주는
FUSE 파일시스템에 병렬 rsync, **지연을 충분히 만드는 아주 작은 시험용 NFS 서버**(결정적 돌파),
bpftrace 계측, 직접 만든 와이어샤크 플러그인, NFQUEUE 로 패킷 늦추기.

### 덮지 않고 상류로 되돌린다

재시도로 덮으면 빈도만 낮아지고 우회하면 다른 것을 잃는다. 패치 둘을 **리눅스 6.16** 에 올려
사내 커널에만 두지 않았다 — 쓰는 쪽이자 고치는 쪽이 되는 자리다.

### 두 기능이 같은 손버릇을 가리킨다

언어가 주는 수명 관리를 안 쓰고 타입으로 다시 정한 것과, 커널이 표준대로 안 한 것을 찾아
고친 것. **남이 정한 기본값을 어디까지 받아들일 것인가**가 이 회사의 공통 질문이다.

### 다음 사이클 — 확장 (Jane Street 의 마지막 빈 도메인)

`하드웨어까지 내려간다` — 자료를 아직 안 읽었다. ASIC 역설계(2026-08-05)와 Hardcaml 대수적
효과(2026-01-06)를 열어야 tech 두 칸의 `unknown` 이 풀린다.
