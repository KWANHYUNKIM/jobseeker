...
**없다 — 이번 사이클은 후보 조사였다.** 큐 **0/3 → 3/3.** 회사 89개.

### 이번 사이클 — ⭐ 새 도구가 접근 지도를 다시 썼다

⚠️ **딜리버루의 국가 차단을 우회하려고 찾은 `r.jina.ai` 가 403 에도 통한다.** 이번 조사의 발견은 회사가 아니라 이것이다 — **접근 지도의 ❌ 를 다시 두드릴 세 번째 수단이 생겼고, 두 곳이 곧바로 뒤집혔다.**

- **✅ Meesho** (❌ 403 → ✅) — **최신 글 2026-08-31**, 17편이 거의 전부 자기 시스템 글. 본문 확인(`building-reviews-at-scale`) — **거부한 대안과 감수한 대가가 명시적이다.** ⚠️ **다만 수치가 약하다.** ⭐ **인도 주소·지오코딩과 하이퍼그래프 RTO 예측은 이 사이트에 없는 도메인이다.**
- **✅ Zepto** (❌ → ✅) — **최신 글이 이틀 전.** 본문 확인(`Building Search for a 10-Minute World`) — **MoE 랭킹 → 피처 스토어 재랭킹 → 재고·가격 권위 해석**으로 이어지고 **색인이 초~분 뒤처진다는 것을 인정하고 설계에 반영한다.** 분당 **100만 요청** 이상. ⭐ **`10분 배송` 이라는 제약이 새 축이다.**
- **✅ Snyk** (⏸️ 보류 → ✅) — **첫 화면으로 판정하지 않는다는 규칙대로 지목해 둔 글을 열었더니 결이 달랐다.** **버린 방식이 명시적**(정적 파인튜닝 → 동적 few-shot)이고 **대가도 적는다**(예전에는 안전하지 않은 코드를 걸러내 **개발자에게 아무 수정도 안 남겼다**). **벤치마크 약 150건에서 74.6% → 85.4%.**
- **⏳ Swiggy — 판정 못 했다.** **`r.jina.ai` 가 Medium 계열에는 422** 를 내고 **브라우저 도구가 이번 사이클에 응답하지 않았다.** ⚠️ **회사 사정이 아니라 이쪽 도구 문제다** — 다음 조사에서 다시 본다.

### ⚠️ 이번에 확인한 규칙 두 개

- **접근 지도의 ❌ 는 그 시점의 사실이지 영원한 사실이 아니다** — 이번엔 **주소가 바뀐 것도 아니고 도구가 늘어서** 뒤집혔다. **되짚기가 새 이름을 찾는 것보다 수확이 좋다는 것이 다시 확인됐다.**
- **첫 화면으로 판정하지 않는다** — Snyk 를 접지 않고 **글 하나를 지목해 보류로 남긴 판단이 옳았다.** ⚠️ **보류는 판정을 미루는 것이지 포기가 아니다.**

### 다음 사이클 — 신규(6순위)

`--gaps` 가 **큐 맨 위(Meesho)를 `in_progress` 로 만들고 3단계(회사 프로파일)부터** 를 부를 것이다. ⏳ 판다면 후보 글: `TrustMesh`(사기) · `GeoIndia`(주소) · `커스텀 롤아웃 서비스`.

### ⚠️ 비교 문서 재료 세 축 (초안 유지)

**① `AI 에이전트를 어디까지 믿나` 6곳 + ⏳ Snyk** — Sentry / ClickHouse(**세 겹 중 한 겹만 기계가 강제**) / Duolingo(사람 리뷰 → 병목) / Ramp(사람의 신원) / DoorDash(primitives) / Deliveroo(**검사 가능한 것만 강제된다**). ⏳ **Snyk 가 일곱 번째가 될 수 있다 — 앞의 여섯과 달리 AI 가 고치는 대상이 보안 취약점이다.**

**② `관리형 MySQL 의 한계` 3곳** — Etsy(Vitess) / Plaid(TiDB) / Paystack(MySQL 8.0 제자리).

**③ `실험 결과를 얼마나 믿나` 2곳** — Etsy(홀드아웃 8%·승자의 저주 공개) / Deliveroo(다섯 원칙·안 하기로 정하는 법).
