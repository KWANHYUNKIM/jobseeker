...
**Ladybird — 회사 프로파일까지 썼다(in_progress).** 회사 128개, 큐 1/3.
**회사가 아닌 조직을 처음 판다** — 501(c)(3) 비영리다.

도메인 셋을 세웠고 기능은 아직 0개다.

- **남의 엔진 코드를 쓰지 않고 처음부터 짓는다** — 소개 + 2026-08 소식지
- **언어를 바꿔 가며 옮긴다** — 2026-07·08 소식지(C++ → 러스트, A/B 로 두 파서 대조)
- **주 스레드에서 일을 떼어 놓는다** — 컴포지터 프로세스 · WasmCompiler 서비스

### 팔 것이 없어야 성립하는 주장

브라우저 엔진은 대개 검색 계약이나 광고에 얹혀 있고 그러면 엔진의 결정이 그 사업을 따라간다.
이 조직은 **무엇을 안 하는지를 먼저 적는다** — 기본 검색 계약도, 코인도, 사용자 수익화도 없다.
후원은 전부 조건 없는 기부이고 이사회 자리는 팔지 않는다. 웹 표준이 이끌고 이해 충돌에서 자유롭다는
문장이 참이려면 팔 것이 없어야 한다는 구조다(재구성).

이 엔진에서 수익원이 없는 세 번째 자리인데 이유가 다 다르다 — DuckDB(IP 를 재단에 두고 회비로) ·
Bun(무료로 주다 팀이 앤트로픽에 합류) · Ladybird(**수익화를 안 하는 것이 제품의 근거**).

### 배수로 말하는 것이 이 조직의 서술 방식이다

포크가 아니므로 오랫동안 기존 엔진 대비 배수로 이야기된다. 그 배수를 매달 공개한다 — 캔버스
fillStyle 파싱이 크로미움 대비 약 90배에서 2배로, replaceChildren() 이 13배에서 4배 미만으로,
리스트 삽입·삭제가 120배 넘던 것에서 20배 미만으로. 벤치마크도 함께 움직였다(StyleBench 3.5 → 83).

### 다음 사이클 — 확장 (Ladybird 의 빈 도메인)

러스트 이전이 이 엔진의 **Bun 과 정면 대비**다. 저쪽은 11일에 병렬 LLM 64개로 한 번에 옮겼고,
여기는 서브시스템 단위로 **두 구현을 함께 돌려 대조하며** 넘긴다. 같은 이전을 정반대 방식으로 한
두 기록이 나란히 서게 된다.
