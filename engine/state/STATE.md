...
**Ente — 기능 둘을 채웠다.** 빈 도메인 하나(`잃지 않는 것으로 값을 받는다`). 회사 147개, **큐 0/3**.

### 못 본다는 말은 남이 확인해 주기 전에는 말일 뿐이다

이번에 쓴 `prove-you-cannot-open-it` 의 축이다. 구조로 한 번(비밀번호를 안 보내는 SRP —
Argon2id v1.3 · 4096비트 군 · 서버엔 verifier 만), 감사로 또 한 번 증명한다.

### 감사 셋을 연표로 세울 수 있었다

**2023-03** Cure53 + Symbolic Software, 설계와 전 클라이언트 소스 — **고위험 1 · 중위험 2**,
보고서 공개. **2025-11** CERN 후원 Cure53 5명 2주, 서버와 인프라 — 무결성 0건, **보안 15건 중
12건을 감사 중에 수정**. **2026-04** 순수 러스트 `ente-core` 를 winfunc(LLM 기반 신생)가 —
중·저위험만.

### 자기 결함을 공개 기록으로 남긴다

2023년 고위험 1건은 **암호가 아니라 사람 쪽 구멍**이었다 — 웹 앱에서 약한 비밀번호를 정할 수
있었다. 그걸 그대로 적고 패치를 감사자에게 확인받았다.

### 스스로 그은 선도 적는다

SRP 능력을 다 쓰지 않고 **서버 진위 확인은 TLS 에 맡긴다**(복잡도를 줄이려고). 후양자는
**대칭키를 안 건드리고** 한시적인 캐스트 흐름부터 — 오래가는 공유는 아직 고전이다.

### 다음 사이클 — 확장 (Ente 의 마지막 빈 도메인)

`잃지 않는 것으로 값을 받는다`. 복제 3판 글(2023-01-07)이 손에 있다 — 클라우드 셋에 세 벌,
한 벌은 불변 규정 잠금, 1·2판의 한계(직렬 복제·자격 증명 유출), 율리시스 계약.
**⚠️ 복제 비용·복구 시간 수치가 그 글에 없다** — `How Ente deletes data`(2024-05-19)를 함께
읽으면 삭제 쪽 층이 는다.

### ⚠️ 큐가 0/3 이다

Ente 를 완주시킨 뒤 **후보 조사 사이클을 끼운다.**
