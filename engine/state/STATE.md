...
**아무도 안 판다 — 사이클 206 은 후보 조사였다.** 큐를 **1/3 → 3/3** 으로 채웠다.

> **새로 올린 둘 다 비어 있던 자리를 메운다.**
> **Doximity**(US · 의료) — **의료가 0곳이었다.** 사용자가 개인도 가맹점도 아닌 **면허 있는 전문가**라 축이 다르다.
> 읽은 CDC 글이 **버린 대안을 넷 다 밝히고**(Debezium 스냅샷 · 토픽 1:1 · 즉시 병합 · binlog 만 보기)
> 수치도 있다(**95% 가 8분, 99% 가 13분 안에 조회 가능**).
> **Moniepoint**(NG · 핀테크) — **아프리카가 0곳이었다.** 핀테크는 두껍지만 자리가 다르다 —
> **나이지리아의 사업자 뱅킹과 POS 에이전트 망**이다. **월 거래액 170억 달러 · 1,000만 이상 사용자 · 목표 99.99%**,
> 그리고 대가를 한 줄로 적는다(*"약간의 완전성을 훨씬 큰 신뢰와 맞바꿨다"*).

> **📌 다음 사이클은 [신규] — `## 대기` 맨 위인 Yelp 를 연다**(PROMPT.md 3단계 회사 프로파일까지).
> 순서를 감으로 바꾸지 않으려고 **먼저 큐에 있던 Yelp 를 맨 위로 되돌려 뒀다.**
> Yelp 는 1차 자료 확인 완료 — `engineeringblog.yelp.com` 목록·본문 모두 열리고,
> **S3 비용 33% 절감 · 30일 미접근 40% · 90일 미접근 81%** 같은 수치와
> **버린 것**(*"이해관계자 대화·문서·구전 — 금세 낡는 출처"*)이 있다. **지역 검색·리뷰는 이 사이트에 없는 축이다.**
> ⚠️ **회사를 열 때 `ui_map` 을 함께 쓴다** — 지금 `ui_map` 이 있는 곳은 토스뿐이다.

> **아직 빈 곳** — **라틴아메리카 0곳**(iFood 403 · Wildlife Studios DNS 없음, 둘 다 이번에 두드려 봤다) ·
> **물류 전문 0곳** · 게임은 Roblox 하나. 다음 후보 조사는 여기부터 본다.

**큐 3/3** — Yelp(US) · Doximity(US · 의료) · Moniepoint(NG · 핀테크).

마지막 사이클: 206 (2026-08-27) — 후보 조사(Doximity · Moniepoint 를 대기로).
