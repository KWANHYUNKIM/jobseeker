...
**comma.ai — 기능 둘.** 남은 빈 도메인은 상자 하나. 회사 139개, 큐 2/3.

### 경고는 많아도 적어도 실패한다

주행 보조가 사람이 보고 있다는 전제 위에 서니 감시 모델이 필요한데, **거짓 양성이 잦으면 사람이
무시하거나 꺼 버려 감시 자체가 무의미해진다.** 못 잡으면 전제가 무너지고. 한 손잡이로 두 방향을
동시에 못 맞추는 자리다.

### 헷갈리는 것만 따로 모아 잰다

검증 집합 1천 장에 더해 **거치대에 물린 휴대폰 80장짜리 어려운 음성 집합**을 따로 둔다. 평균
성능 뒤에 숨는 가장 성가신 오탐을 정면으로 재는 장치다 — 그 80장은 전부 음성으로 유지됐다.

### 라벨을 사람이 아니라 모델이 붙인다

시각언어모델에 휴대폰을 쓰는지 예/아니오로 묻고 **토큰 확률을 재서** 라벨을 만든다. 빠르고 많이
만들 수 있는 대신 **라벨 모델의 편향이 그대로 내려간다**(재구성) — 앞 기능의 어긋남과 같은
종류의 위험이다.

### 상황을 아는 임계값

돌 때 고개가 돌아가는 것은 한눈팔기가 아니므로 **조향각 방향으로 머리 자세 허용치를 넓혀**
회전 중 경고를 약 7% 줄였다. 넓힌 그 방향의 실제 한눈팔기는 놓칠 수 있다는 대가가 붙는다.

### 모델 성능이 아니라 행동을 잰다

기기 71대의 현장 데이터로 오픈파일럿을 켰을 때 휴대폰 사용이 크게 줄었다고 밝힌다. 검증 점수가
아니라 사람이 실제로 앞을 더 보는지를 보는 자리다.

### 다음 사이클 — 확장 (comma.ai 의 마지막 빈 도메인)

`상자 하나 안에서 끝낸다`. 발열 문턱 75→85도(막힌 기기 125대→11대), chestnut 249달러,
자동초점 보정의 실패가 손에 있다.
