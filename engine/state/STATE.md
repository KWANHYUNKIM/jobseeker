...
**없다 — 문체 보수 사이클이었다.** Databricks 를 배민 문체로 다시 썼다(2.32 → 0.0). 회사 115개, 큐 2/3. 36곳 남았다.

### 카디널리티를 다루는 답이 다섯이 됐다

둘째 기능에서 이 엔진에서 가장 붐비는 축이 채워졌다.

- VictoriaMetrics — 비용을 짓누르는 것은 샘플 양이 아니라 카디널리티다.
- Honeycomb — 그 카디널리티를 무제한으로 열어 상품 쪽에 놓는다.
- Datadog — 보고 있는 것만 모은다는 원칙을 수집기에 심는다.
- Razorpay — 감사로 안 쓰이는 것을 찾아 수집 계층에서 버린다.
- Databricks — **버리되 되찾을 수 있게 버린다.** 수집에서 라벨을 떨어뜨려 시계열 DB 를 지키고,
  떨어뜨린 차원은 레이크하우스에서 되찾는다.

앞의 넷이 무엇을 안 볼지 정하는 이야기라면 여기는 **안 보기로 한 것을 어디에 두느냐**의 이야기다.
창고를 파는 회사라서 나올 수 있는 답이다.

### 겹치는 사례를 견주는 상대에서 뺐다

Lakebase 는 이 엔진의 Neon 과 구조가 같다 — Databricks 가 Neon 을 인수했기 때문이다.
그래서 첫 기능의 대비를 Neon 이 아니라 **Cursor·터소**로 걸고, 겹침은 본문에 명시했다.
셋의 출발점이 다르다 — Cursor 는 에이전트 속도가 감당이 안 돼서, 터소는 재현이 안 돼서,
여기는 팔려는 물건의 쓰기 값이 비싸서 구조를 바꿨다.

### 도메인을 제품 갈래가 아니라 성장이 어디를 먼저 깨뜨렸느냐로 잘랐다

레이크하우스·SQL·AI 로 가르면 둘의 공통점이 사라진다. 하나는 팔려고 다시 지은 것이고 다른 하나는
자기가 쓰려고 지킨 것인데, **둘 다 잘 팔릴수록 비싸지는 구조**라는 점이 같다 — AI 워크로드가 매출의
약 26%인데 카디널리티를 만드는 것도 바로 그 워크로드다.

### 다음 사이클 — 문체 보수 (사다리 3순위)

--gaps 가 정한다. 지금 맨 위는 duolingo(2.24)지만 그건 이 시점의 예상이지 확정이 아니다.
