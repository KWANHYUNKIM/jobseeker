...
**Zulip — 프로파일만 썼다(도메인 하나, 기능 0).** 회사 157개, **큐 0/3**.

### 유연함의 값이 매 메시지에 붙는다

채팅은 권한 확인이 **메시지마다** 일어난다. 짧은 DM 하나를 처리·전달하는 데 **25~50밀리초**에
이미 **질의 7개**가 들어 있고, 질의 하나가 더 붙으면 **0.3~2밀리초**가 는다. 그런데 이 회사는
남들이 택한 **단일 부모 제약을 버리고** 임의 중첩을 골랐다 — 유연함 쪽으로 한 걸음 더 갔다.

### 남의 문서를 인용해 물린다

깃허브("각 하위 팀은 부모 팀이 하나뿐이다")와 깃랩("하위 그룹은 직속 부모 하나에 속한다")을
**그대로 인용해** 무엇을 안 따랐는지 밝힌다. 대신 비순환 하위 그룹 그래프로 갔다.

### 소유 구조가 특이하다

2026년 5월, **비영리 재단이 Kandra Labs 를 완전히 소유**하게 됐다 — 다른 주주도 부채도 없다.
모질라·시그널·위키피디아를 본떠 **한 사람의 재임 기간을 넘어 사명을 지키려는 것**이라고 적는다.
이 엔진에 시그널·지그 재단이 있는데 **회사를 재단이 통째로 소유하는 형태는 처음**이다.

### ⚠️ 지도부 넷이 앤트로픽으로 옮긴다

같은 발표에 적혀 있다. 개발 속도가 어떻게 되는지는 확인하지 못해 `open_questions` 에 남겼다.

### 다음 사이클 — 확장 (그 하나뿐인 도메인)

`지연에 민감한 곳에서 권한을 유연하게 짓는다`. 권한 글 하나에 물린 대안·수치·대가가 다 있다.

### ⚠️ 큐가 0/3 이다

Zulip 을 완주시킨 뒤 **후보 조사 사이클을 끼운다.**
