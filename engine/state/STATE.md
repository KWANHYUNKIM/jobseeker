...
**Ghostty — 기능 둘을 채웠다.** 빈 도메인 하나(`몇 주씩 켜 두는 프로그램의 메모리를 지킨다`).
회사 159개, **큐 0/3**.

### 본업이 아닌 것을 다들 각자 만든다

이번 기능(`extract-the-proven-core`)의 축이다. **수백 개 프로그램이 터미널 에뮬레이션을
구현하는데** 그들에게 그것은 본업이 아니라 그때그때 필요한 만큼만 만들어진다. 그래서 까다로운
자리에서 무너진다 — **Jediterm 은 중간 문자를 잘못 다뤄 커서 모양 시퀀스가 글자를 삼키고,
애플 터미널은 DCS 시퀀스를 출력으로 그냥 흘려보낸다.**

### 새로 쓰지 않고 뽑아낸다

라이브러리용으로 다시 설계하는 대신 **Ghostty 의 검증된 코어를 그대로 뽑았다** — 여러 해 실제로
돌아간 것이라는 근거가 그대로 남는다. 의존성은 없다: **libc 조차 필요 없다.**

### 무엇부터 뗄지도 근거가 있다

GPU 렌더링도 입력 처리도 아니고 **시퀀스 해석과 상태 유지**부터다 — 이름 댄 버그들이 전부 그
자리에서 났다(재구성). 나머지(GPU·입력·GTK 위젯·Swift·윈도·임베디드·WASM)는 **"더 긴 안목의
앞일"이라고 목록으로 적는다.**

### 안 된 것을 적는 회사가 또 하나

일렉트릭이 "다중 노드·카오스 시험은 아직"이라고 적었듯, 여기도 공개 알파임을 밝히고 안 들어간
것을 이름 대어 적는다. **성능 수치와 시험 묶음이 없다는 것도 그대로 남겼다.**

### 다음 사이클 — 확장 (마지막 빈 도메인)

`몇 주씩 켜 두는 프로그램의 메모리를 지킨다`. 누수 글 하나에 원인·발견 방법·고침·물린 대안·
교훈이 다 있다 — **열흘에 37GB**, 메타데이터만 줄이고 `mmap` 은 안 푼 것, macOS 가상 메모리
태그로 격리, 커뮤니티의 재현 케이스, "비표준 페이지는 절대 재사용하지 않는다".
