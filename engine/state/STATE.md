...
**컬리 (KR · 커머스/신선식품) — 47번째. 기능 1개 · 도메인 2개. 큐 잔량 0/3.**

260 에서 **`misdelivery-detection`(문 앞을 알아본다)** 을 썼다. 결정 8 · 그림 3 · 생각 4 · 스택 6 ·
난제 2 · ui 5핀.

**⚠️ 두 번째 글은 다른 문제였다.** `딜리버리 암호화 모듈 개발기`(2025-06-02)를 열었는데 **저장된
개인정보(주소·연락처)의 암호화**이고 **배송 완료 사진과 무관**해서 이 도메인에 안 넣었다(223 의 규칙).
**그 자체로는 재료가 좋다** — 버린 대안 둘에 이유가 각각 있고(JPA `@PostLoad` 는 *"값이 바뀌면서
dirty로 체크되었기 때문입니다"*, Hibernate PreLoad 는 *"6.2.0에서야 고쳐졌고"* 일정에 못 맞았다)
수치도 촘촘하다(**300만 개 암호화 5.58초 · 복호화 5.36초 · 헤더 10~30% 절감 · base64 는 원본보다
0.33배 크다**). **세 번째 도메인 후보로 남겼다.**

**📌 원문 재독이 아홉 번째로 옳았다**(223·229·243·249·250·252·255·258·260). 오배송 글에서 넷이 더 나왔다.
- **🔴 On-device 의 이유가 셋이었다 — 프라이버시가 빠져 있었다.** *"원본 데이터가 기기 외부로
  전송되지 않고 단말기 내부에서 모든 추론 연산이 완결되므로 개인정보 보호에 매우 강력한 이점을
  가집니다"*, 그리고 *"원본 이미지로 역산할 수 없는 비가역적인 숫자 배열(Embedding Vector) 형태만을
  사용하므로 프라이버시 침해 가능성을 구조적으로 방지"*.
- **제로샷** — 본 적 없는 주소도 지난 사진 몇 장이면 추론한다(⚠️ **이력이 아예 없는 새 주소의 실제
  처리는 안 적혀 있다**).
- **갤러리는 매일 갱신된다** — *"Airflow 기반의 일배치 파이프라인"*.
- **Hard Negative Mining 의 정의** — *"기준 장소(Anchor)의 중심점과 잠재 공간상 가장 가까워 모델이
  혼동하기 쉬운 어려운 샘플에 집중하여 페널티를 부여"*.

**🔴 이 기능의 축 — 질문을 바꿔 희소성을 피했다.** 오배송이 *"매우 낮은 비율로만 발생"* 해 분류로는
안 되니, **'오배송인가' 대신 '늘 보던 문 앞인가'** 를 묻는다. 그러면 **정상 사진만 많으면 되고**
주소당 10장으로 학습된다. **불균형을 이기는 대신 불균형이 걸리지 않는 형태로 옮긴 셈이다.**

**🔴 그리고 한 판정기에 두 문턱을 뒀다** — 앱은 정밀도 우선(보수적으로 높게), 운영 채널은 재현율
우선(사람이 검토). 근거가 *"양치기 소년 현상"* 이다. **CyberAgent 의 *"알림의 형해화"* 와 같은
문제인데 답이 다르다**(저쪽은 규칙을 재웠다 깨운다).

**⚠️ 그런데 놓친 오배송(거짓 음성)이 통째로 안 다뤄진다.** 오탐의 값은 이름까지 붙이면서 **못 잡은
것의 값은 한 줄도 없고**, **운영에서 몇 건을 잡았는지도 없다**(지표가 전부 테스트셋). 모델 백본·크기·
기기 지연·두 채널의 실제 임계·PK 값도 비공개다.

**🚨 다음 사이클 — `--gaps` 는 도메인 ②`남의 물건을 대신 받아 준다` 를 가리킬 것이다.**
**두 번째 글 후보**: `/blog/2025-kafka-streams-window`(Kafka Streams 윈도우 도입기, 2025-12-01).
**⚠️ 같은 도메인인지 실물로 정한다** — 입고 동기화와 스트림 윈도우가 같은 문제인지 다른 층인지.
**다르면 outbox 글 하나로 쓴다 — 결정이 6개는 선다**(버린 대안 하나에 이유 명시 · Outbox 채택 ·
SINGLE_TOPIC · retryKafkaTemplate 분리 · TopicNameSet 으로 알림 선별 · 145회/10분/24시간 설계).
**⚠️ 어느 쪽이든 원문을 다시 읽는다**(⑫가 아홉 번 다 맞았다).

**남은 재료** — 엔지니어링만 **7페이지(70편 이상)**. 1쪽의 본업 글: `access-block` 2부작(무중단 점검,
2025-10-10, **그 자체로 묶음이라 도메인 후보**) · OMS MSA(2025-05-09) · 딜리버리 암호화(2025-06-02).
**AI 도구 글이 전체 목록의 절반**인데 물류·커머스와 이어지는 서술은 아직 못 봤다.
...
- **새 축 `관측에 값을 무엇으로 치르는가` — 이제 여섯이다.** CyberAgent `observability-pipeline`(돈) +
  `alert-context`(주의력) + **컬리 `misdelivery-detection`(주의력 — *"양치기 소년 현상"*, 답이 다르다:
  채널마다 임계를 나눈다)** + Yelp `partition-access`(하루 TiB) + Zerodha `log-storage` +
...
⑫ **요약이 아니라 원문을 다시 읽는다** — 223·229·243·249·250·252·255·258·**260** 아홉 번 다 새 결정을 찾았다.
