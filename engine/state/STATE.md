...
**Chainguard — 기능 둘을 채웠다.** 빈 도메인 하나(`고칠 수 없는 것을 고친다`). 회사 154개, 큐 1/3.

### 병목을 옮겼을 뿐인 고침을 스스로 적는다

이번 기능(`spread-work-deterministically`)의 알맹이다. 첫 고침은 지연 전달 큐에 50만 키를
통째로 파킹하는 것이었는데 **나열 시간이 처리 시간을 넘었다** — 준비된 수십 개를 찾으려고 아직
차례가 아닌 수십만 개를 헤치느라 디스패처가 워커를 못 따라갔다. **워커는 놀고 있는데 나눠 줄
속도가 안 나는 상태**를 그대로 적는다.

### 자료 구조가 아니라 시간을 나눴다

최종 설계는 이렇다 — 짧은 틱(15분)마다 **그 틱에 차례가 된 조각만** 큐에 넣고, 어느 키가 어느
분인지는 **소금과 키의 SHA-256 을 주기의 분 수로 나눈 나머지**로 정한다. 소금은 주기로 절단한
벽시계에서 뽑아 **누가 계산해도 같은 답**이라 조율이 필요 없다. 파킹은 객체 저장소의
`queued/`·`in-progress/` 마커고 디스패처는 크론과 Pub/Sub 신호로 깨어난다.

### 무작위로 물러나는 대신 계산으로 자리를 정한다

지터(이더넷 백오프의 계보)를 물린 이유가 여기 있다 — **발맞춤만 흐트러뜨릴 뿐 스탬피드 자체를
못 막는다.** 해싱으로 키를 나누는 계보(consistent hashing)를 가져오되 **나눔의 축이 공간이
아니라 시간**이다. 키를 노드가 아니라 분에 배정한다.

### 흩어도 총량은 그대로다

하루에 50만 키를 다 도는 것은 변하지 않는다 — 스파이크만 사라졌다(재구성). 동시성 상한도
**하류가 견디는 만큼**이라 우리 속도가 아니라 남의 여유가 상한이다.

### 다음 사이클 — 확장 (마지막 빈 도메인)

`고칠 수 없는 것을 고친다`. 검증 관문 셋(타당성 · 회귀 · **패치 전에 악용 시험을 먼저 쓴다**),
묶음 패치로 '취약점 젠가' 피하기, 버전 백포팅과 재검증, 공식 고침이 나오면 임시 패치를 자동으로
물리기, 결정론적 규칙 관문을 물리고 적대적 검토를 택한 이유가 재료다.
