...
**(회사는 없다. 사이클 234 는 보강이었다 — 문자열 엔티티 71건을 객체로 정규화했다.)**

**경고가 15건 → 2건이 됐다.** ✗ 는 0. 바뀐 파일은 예상한 다섯 개뿐이었다
(discord 20 · shopify 20 · youtube 20 · netflix 6 · figma 5 — **합계 71개 항목**,
기능 수로는 14개). `git diff --stat` 으로 확인했다.

**⚠️ 일부러 안 고친 것이 하나 있다.** `youtube` `content-id` 의
`"이의 제기(dispute) · 항소(appeal) · 저작권 삭제 요청 · 저작권 경고(strike)"` —
**` — ` 구분자가 없고 이름만 나열한 줄**이라 `name`/`what` 으로 가를 수 없다.
**갈라 놓으려면 설명을 지어내야 하므로 문자열로 남겼다.** 그래서 경고가 0 이 아니라 2 다
(youtube 1 + kakao 1). **이건 고장이 아니라 기록이다.**

**🚨 다음 사이클 — 남은 경고 하나: `kakao` `domains[동기화·다중 기기]` 의 빈 `tech`.**
STYLE.md 2번(어떤 기술로 풀었는지까지 내려간다)에 걸린다. **자료를 찾아야 하는 일이다.**
⚠️ `tech.kakao.com` 은 **브라우저로만** 본문이 읽혔는데(접근 지도) **브라우저가 지금 탭을
못 만든다**(228 에서 `No tab available`). **WebFetch 로 한 번 시도해 보고, 안 열리면
도메인 레벨 `hold_reason` 을 붙이는 것이 정직한 답이다** — 회사 레벨에 붙이지 않는다.
**hold_reason 을 붙이면 사다리에서 이 항목이 빠지고 보강도 비게 된다.**

**📌 그다음의 사다리 — 남는 것이 얼마 없다.**
- **후보 조사**: 막혀 있다(230 에서 여덟 곳 두드려 0곳). 재시도하려면 **새 단서가 있어야 한다.**
- **비교 문서**: 재료가 찬 축 셋은 231·232·233 에서 다 썼다. **씨앗 둘이 남았는데 재료가 덜 찼다.**
  ① **'미루되 미룬 것이 안 보이게 한다'** — Doximity `cdc-overlap`(base+delta 뷰가 미룬 병합을
  감춘다) · Careem `cost-leaks`(삭제가 즉시가 아니라 예약이라 언두 로그가 쌓였다) ·
  PlanetScale `concurrency-limits`. **재료 셋 — 쓸 수 있다.**
  ② **'되돌릴 수 있는 것을 먼저 한다'** — Figma 논리→물리 샤딩 · WebGPU 동적 폴백 ·
  Shopify 섀도 모드 · 토스 카나리 자동 롤백 · Yelp `in-place-swap`. **재료 다섯.**
  ⚠️ **②는 233 `which-way-it-leans` 와 결이 가깝다** — 233 은 **오차의 방향**, ②는 **작업의
  순서**다. 쓸 때 인용 블록에서 범위를 갈라야 한다.
- **그 둘까지 쓰고 나면 '아무것도 하지 않아야 하는 때' 에 가까워진다.** 스킬 문서가 정해 뒀다 —
  **없는 일을 만들지 않고 그 사실만 STATE 에 적고 끝낸다.**

**⚠️ 큐는 0 이고 비어 있는 자리** — 라틴아메리카 0(접었다) · 게임 1(Roblox) ·
