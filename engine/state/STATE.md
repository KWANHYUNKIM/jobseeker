...
**Razorpay** (IN · 핀테크) — 도메인 3개 · **기능 2개.** 남은 도메인은 **`사람의 조사 시간을 AI 로 줄인다`** 하나다(⏳ 미독).

### 이번 사이클 — `깨질 걸 알면서 좁게 의존한다`

⭐ **이번 세션에서 실패담이 가장 풍부한 자료였다.**

건진 것 넷:

- ⚠️ **취약한 줄 알면서 고르되 폭발 반경을 한정했다** — 제대로 된 방법(AWS 중단 이벤트 상관관계)은 **두 달**이 들어서, **러너 로그의 문자열 하나를 찾는 `취약하지만 즉시 유용한 탐지기`** 를 골랐다. **그리고 위험을 미리 적었다** — **`깃허브가 로그 형식을 바꾸면 우리 탐지기는 깨진다.`**
- ⚠️ **미리 적은 위험이 그대로 터졌다** — **러너 업데이트가 형식을 바꾸자 정리 작업이 한 시간 안에 활성 파드를 전부 지웠다.** ⚠️ **답이 탐지기를 튼튼하게 만드는 것이 아니라 `한 번에 20% 상한` 이었다** — **깨지는 것을 막는 대신 깨졌을 때의 크기를 묶었다**(재구성).
- ⚠️ **정상일 때 맞던 값이 몰림 앞에서 전부 틀린 값이 됐다** — **200개가 동시에 죽자 재시도 폭풍이 레이트 리밋에 걸렸고**, **200ms 차이로 도착한 웹훅이 100ms 중복 제거 창을 우회**했다. 창을 **30초**로 늘리고 분산 락을 걸었다.
- ⚠️ **복구 시간이 곧 개발자의 대기 시간이다** — 더 단순한 폴링을 버린 이유가 **탐지·분류·재시도 각 단계에서 `작업당 몇 분`** 이 더해지기 때문이다. **대가는 공개 엔드포인트와 멱등성 처리.**

**수치** — 온디맨드 **4~5배** 비쌈 · 스팟 **70~90%** 절감 · **CI 컴퓨트 지출 60% 절감**·**워크로드의 80% 를 스팟에서**·**작업 성공률 99.2%** · 하루 **1만 건 이상**에서 **수동 재시도 0**·온콜 CI 경보 **사실상 사라짐.**

### ⚠️ 이 회사의 두 기능이 같은 문제를 다르게 만난다

**둘 다 고카디널리티를 다룬다** — 관측 쪽은 **안 쓰는 메트릭을 지웠고**, 여기서는 **개별 작업 메트릭을 히스토그램으로 접었다.** ⚠️ **그리고 둘 다 `비용을 줄이되 볼 것은 잃지 않는다` 를 성공의 정의로 삼는다**(재구성). connections 에 이어 뒀다.

### ⭐ 회사가 남은 일을 스스로 적는다

**일시적 실패와 영구적 실패를 갈라 헛된 재시도를 줄이는 것 · 팀별 비용 귀속 · 멀티 클러스터.** **완성된 이야기로 마무리하지 않는다** — ⚠️ **Cygames 의 컨퍼런스 요약이 `겪은 문제도 다르게 하겠다도 없다` 였던 것과 정확히 반대다.**

### 다음 사이클 — 마지막 도메인, 그리고 완주 판정

`--gaps` 가 **`사람의 조사 시간을 AI 로 줄인다`** 를 부를 것이다. ⏳ **본문 미독** — **`Hermes: 망에서 격리된 자가 개선 사내 두뇌`**(2026-07-22, URL 확보) · **보안 트리아지 750시간 → 2시간** · **조사 30분 → 90초.** 자료가 서면 **같은 사이클에서 완주 판정까지** 한다.

⚠️ **큐는 2/3**(Flipkart · Twilio). ⏳ **보강 거리** — Grafana Labs 정본과 중복 파일 인용 대조.

### ⚠️ 비교 문서 재료 (초안 유지)

**① `AI 에이전트를 어디까지 믿나` 8곳 + ⏳ Razorpay(Hermes)** — Sentry / ClickHouse / Duolingo / Ramp / DoorDash / Deliveroo / Snyk / Cygames.

**② `관리형 MySQL 의 한계` 3곳 + Cygames** — Etsy / Plaid / Paystack.

**③ `자기 성과를 어디까지 주장하나` 3곳 + ⭐ Razorpay** — Snyk / Grafana Labs / ScyllaDB. ⚠️ **Razorpay 는 여기에 `실패담을 적는다` 는 결로 붙는다.**

**④ `인도 규모에서 무엇이 달라지나` 3곳 + ⏳ Flipkart** — Meesho / Zepto / Razorpay.
...
⏳ **⑪ 새로 보인다 — `깨질 걸 알면서 고른 의존을 어떻게 다루나`** — **Razorpay**(로그 문자열 + 20% 상한) / **Plaid**(은행 예외를 문서에) / **Paystack**(Kuda 는 다른 필드) / **ScyllaDB**(SDK 확장 지점 안에서만).
