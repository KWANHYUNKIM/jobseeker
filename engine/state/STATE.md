...
**없다 — Godot 을 완주시켰다(도메인 1 · 기능 1 · `status: done`).** 회사 170개, **큐 0/3.**

### 가장 빠른 길을 알고도 안 갔다

이번 기능의 축이다. `raw input` 만 받으면 문제가 깨끗이 사라지는데 그러지 않았다 —
**"제목 표시줄을 끌어 창을 옮기는 것처럼 사용자가 기대하는 애플리케이션의 여러 부분이
깨진다."** 성능 문제를 풀기 전에 **무엇을 지켜야 하는지를 먼저 정한** 결정이다. 엔진은
게임만 위한 것이 아니라 **창을 가진 프로그램**이기도 하다.

### 주기가 다른 두 세계를 잇는 일

마우스는 초당 8,000번까지 말하고 화면은 480Hz 다 — **16배 차이**이고, 폴링 주기는 사용자
장비가 정하는 것이라 엔진이 통제할 수 없다. 그대로 흘려보내면 **프레임 하나를 그리는 동안
수십 개가 쌓인다.** 버퍼드 읽기·프레임당 1회·입력 누적이 그 대응이다.

### 표를 통째로 냈다

가장 극적인 칸(1 FPS 미만 → 1703 FPS)만 뽑아 쓸 수도 있었는데, **1kHz 에서는 거의 같다**는
칸과 **안 움직일 때 4097 대 4061 FPS** 라는 칸을 함께 남겼다. 뒤엣것이 있어야 **회귀가
없다**는 것이 보인다.

### 고친 뒤에도 남는 몫을 밝힌다

**"프로젝트는 여전히 `_input()` 과 `_unhandled_input()` 을 최적화하는 데 주의해야 한다."**
480Hz 수직동기만으로도 초당 480번은 여전히 불린다. 그리고 **게임패드는 다루지 않는다고
명시**하고 이유까지 적는다(폴링이 1kHz 를 거의 안 넘는다) — 키보드는 있다고만 하고 안 쟀다.

### 앞 사이클에서 내가 틀렸던 것을 데이터에 적었다

회사 프로파일 때 **"고친 뒤 수치를 밝히지 않는다"** 고 `open_questions` 에 적었는데, 본문을
끝까지 열어 보니 **고치기 전후를 나란히 놓은 표**가 있었다. 요약만 보고 적은 것이다. 그
줄을 지우고 **틀렸다는 사실을 남겼다** — 같은 실수를 다음 사이클이 반복하지 않게.

### 심은 대비

**프레임워크**(LPDDR 은 납땜돼야 한다는 제약을 풀며 속도와 용량을 내줬다) ↔ **Godot**
(레거시 이벤트를 못 끈다는 제약을 받아들이고 횟수를 묶었다) — 둘 다 제약을 없애는 대신
**값을 치르고 우회한다.**

### 다음 사이클 — 후보 조사 (큐가 0/3 이다)

**CAD 축 세 번, 의료 축 여덟 번 실패**가 원장에 쌓였다. 못 읽은 곳: Hugging Face ·
Lunit(medium 403) · Blender(403) · dbt Labs · incident.io(404 네 번) · M3(링크 없음) ·
Guardian · Flexport · Cal.com. Mastodon 의 안 연 글 둘(`5.0: Laying the foundation` ·
`Discovery Week 2026`)도 목록에 있다. **이 세션의 WebSearch 예산은 끝났다.**
