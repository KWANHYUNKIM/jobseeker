...
**Etsy** (US · 커머스) — 도메인 3개 · **기능 1개**(`coupled-anyway-despite-going-slow`, **history 3세대**). 남은 도메인 둘.

### 이번 사이클 — Vitess `천천히 가려 해도 묶여 있는 것은 함께 움직인다`

**연도가 있어 기능 안에 3세대 연표를 세웠다** — 2010 자체 ORM+인덱스 DB → 2018 Vitess 를 앞에 두되 샤드는 ORM 이 지정 → 2026 커스텀 vindex.

건진 것 넷:

- ⚠️ **기성품이 더 좋은데 못 썼다** — 해시 vindex 는 별도 저장소 의존을 없애 주니 가장 원하던 것인데, **ORM 의 매핑이 무작위라 알고리즘이 아니다.** 그대로 쓰려면 **수동 리샤딩에 수년**이 걸려서, **기존 로직을 이식한 커스텀 vindex** 로 **데이터를 하나도 안 옮기고** 갈아 끼웠다.
- ⚠️ **가장 안전하려던 전략이 가장 위험한 묶음을 만들었다** — 테이블 하나씩 가는 것이 위험을 줄이는 방법이었는데, **27개 모델(테이블의 3%)이 DB 트래픽의 1/3 을 차지하고 전부 트랜잭션으로 연결돼 있었다.** 회사의 문장 그대로다: `some of the riskiest changes were **tightly coupled anyway**`. **나누는 전략은 나눠지는 것에만 통한다**(재구성).
- ⚠️ **좋은 도구를 기본값에서 뺐다** — scatter 질의는 한 모델의 시간을 **약 2초에서 약 20ms** 로 줄이는데, **의도치 않은 비싼 질의가 1,000 샤드로 동시에 가면 곧 문제가 된다.** 그래서 **기본적으로 끄고 명시적으로 허용**하게 했다.
- ⚠️ **인프라 교체에서 성능이 그대로인 것이 성공 기준이다** — `rendering the change invisible to our end users`. **얻으려던 것은 속도가 아니라 단일 실패점 제거·확장 시간 단축(수개월 → 며칠)·개발자 부담 감소**였다.

**제품 실험 도구가 인프라 이전의 안전장치가 됐다** — 테이블마다 트래픽을 **1%→10%→50%→100%** 로 올리며 옛 방식과 비교하고 문제가 나면 즉시 0으로. **모든 질의를 미리 시험하지 않는 위험을 롤백 속도로 상쇄한 것이고, 회사가 그 교환을 그대로 적는다.**

### 다음 사이클 — 실험 아니면 LLM 검색

⚠️ **두 도메인 다 tech 가 얇다**(실험 2 · LLM 1). **그 차례에 자료부터 찾는다** — 다섯 번 통한 규칙이다.

찾을 곳: **실험** — control variates(2021·2025 후속) · 실험자 정의 세그먼트(2025-10) · **승자의 저주**(2022-12). **LLM** — 검색 관련성(2026-01) · 재고 이해(2025-10) · 구매자 프로파일(2025-09). ⚠️ **브라우저로 열어야 한다** — 이 블로그는 WebFetch 가 403 이다.

⏳ **결제 이전 3부작도 남아 있다** — Part 3 `Reducing Cutover Risk` 에 **질의 비호환을 찾아 고친 방법**이 있다고 vindex 글이 가리킨다. **회사 단위 연표(eras)를 세울 재료이기도 하다.**
