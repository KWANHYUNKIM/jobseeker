...
**CedarDB — 기능 하나를 채웠다(`colibri`, 뜨거운 행과 식은 컬럼을 한 나무에 매단다).**
회사 121개, 큐 2/3. 빈 도메인 둘 — 인코딩·압축 · 실행 엔진.

### 나이를 온도의 대리 지표로 쓴다

한 테이블을 두 형식으로 담되 사본은 하나다. 뼈대는 **행 ID 로 색인한 B+ 트리**인데, 새 행일수록 큰
ID 를 받으므로 나무가 저절로 나이순으로 갈린다 — 접근 빈도를 따로 세지 않고 공짜로 얻는 신호를 쓴
셈이다. 뜨거운 쪽은 압축 없는 행 블록이라 갱신이 싸고, 식은 쪽은 압축 컬럼 청크라 스캔이 5배 넘게
빠르다.

전환 시점을 정한 방식이 이 회사답다. 고정 임계값 대신 **1987년의 5분 규칙을 지금 SSD 값으로 다시
풀어 10~40초**를 얻었다. 40년 된 규칙의 상수를 바꿔 쓴 자리다.

### 불변을 지키려고 갱신을 되돌린다

컬럼 청크는 고치지 않는다. 식은 레코드를 고치면 삭제와 삽입의 쌍으로 다뤄 뜨거운 행 영역으로
되돌린다. 그 덕에 컬럼 블록이 불변이 되고, 불변이라 객체 저장소와 성질이 맞아 **로깅도 형식별로
갈린다** — 행은 레코드 단위로 전부, 컬럼은 나무 연산만.

### 이번 큐에서 서술이 갈리는 첫 회사다

DuckDB 는 미리 읽기가 메모리에 쌓인다고 적었고, Dolt 는 자기가 몇 배 느린지를 적었으며, 안티테시스는
카운터가 1조에 하나 어긋난다고 적었다. **CedarDB 의 Colibri 글은 자기 구조가 감수하는 것을 적지
않는다** — 두 범주에서 동시에 3배 이상 앞선다는 주장으로 끝난다. 오래된 행을 자주 고치는 워크로드의
왕복, 되돌아온 자리의 회수 같은 것을 `research` 의 난제와 `open_questions` 로 남겼다.

### 다음 사이클 — 확장 (CedarDB 의 빈 도메인)

지난 사이클에 주소를 못 찾은 글들을 목록에서 받아 QUEUE 에 적어 뒀다 — `/blog/compilation/`(2025-04-02),
`/blog/reducing_branches/`(2025-01-29), `/blog/simple_efficient_hash_tables/`(2024-06-05).
