...
**Backblaze — 기능 하나를 채웠다(`vault-17-3-erasure`).** 빈 도메인 둘. 회사 134개, 큐 1/3.

### 죽지 않게 하는 대신 죽어도 되게 만든다

값싼 드라이브를 쓰기로 한 이상 고장은 전제다. 파일을 조각 20개(데이터 17·패리티 3)로 나눠
파드 스무 대에 흩으면 **아무 17개만 있으면 되살아난다** — 파드 세 대, 하드 180개가 같은
순간에 죽어도 한 바이트도 안 잃는다.

복제였다면 두 벌에 100% 손해인데 여기는 **17.6%**다. 예전 RAID6(13+2)의 15.4%와 거의 같다.

### 전제부터 틀렸다고 적는다

성능 때문에 C 로 짜야 한다고 여겼는데 재 보니 자바가 C 만큼 나왔고(단일 스레드 초당 149MB)
자기네 스택이 자바였다. 최적화도 **행렬 곱에만** 하고 나머지는 단순하게 뒀다 — 조용히 틀리면
데이터가 사라지는 코드라 읽기 쉬운 편이 낫다는 판단이다. MIT 로 공개했다.

### 부호화하고도 원본이 그대로 남는다

결과의 앞부분이 입력과 같게 행렬을 짜서 평소 읽기에는 복원 계산이 아예 안 든다. 잃었을 때만
그 행을 지우고 역행렬을 구한다.

### 앞 회사와 견준 자리

Signal 이 "서버를 안 믿는다"는 제약에서 네 도메인을 뽑았다면, 여기는 **"디스크는 죽는다"**는
전제 하나에서 셋이 나온다 — 덮는 일(부호), 세는 일(통계), 파는 일(내려받기 값).

### 다음 사이클 — 확장 (Backblaze 의 빈 도메인)

`디스크가 얼마나 죽는지 센다`. 34만 1,263대·분기 1.24%·모델별 성적·공개 데이터셋이 손에 있다.
