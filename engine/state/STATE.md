...
**없다 — 쉰다섯 번째 후보 조사를 돌았다. 큐 2/3.** 다음 사이클은 **신규**(Proton 또는 Cockroach Labs).

### 접은 곳 목록이 곧 후보 목록이다

두 사이클 연속으로 같은 일이 일어났다. 쉰네 번째에서 Fastmail·DeepL 을 되살렸고 이번에
Proton·Cockroach Labs 다. **넷 다 접은 근거가 "편수" 아니면 "첫 화면 인상"** 이었는데,
이 엔진은 그 두 가지가 틀린 근거라고 이미 여러 번 적어 뒀다. 자기가 적어 둔 규칙을
자기가 어긴 자리를 찾아가는 것이 지금 가장 수확이 좋은 방법이다.

### 접을 때 근거를 적어 둔 덕이다

Proton 을 접는 문장 안에 **"장애 사후 기록 한 편은 있다"** 는 단서가 남아 있었고, Cockroach
쪽에는 아예 **"다음 조사에서 개별 글을 열어 결론을 낸다"** 고 적혀 있었다. 근거 없이 "접음"만
적었다면 둘 다 영영 안 돌아봤을 것이다.

### 첫 화면은 100쪽 중 1쪽이다

Cockroach Labs 블로그는 **1페이지 / 총 100페이지**였다. 그 1쪽만 보고 "엔지니어링 글이
하나뿐"이라고 적었던 것이다. `/blog/engineering/` 태그를 받아서야 Pebble 글이 나왔다.

### 자기 이득을 자기가 깎아 말하는 글

Cockroach 의 Pebble 값 분리 글은 47% 처리량을 내놓고도 **"시스템 상위의 사정 때문에 이
이득을 온전히 누리지 못한다"** 고 적는다. Proton 은 자기들이 세운 가정("프랑크푸르트를
되살리는 게 취리히로 넘기는 것보다 빠를 것이다")이 **틀렸다**고 적는다.

### 못 읽은 곳 — Sourcegraph 는 목록에서 뺀다

**세 번째 403 이고 이번에는 홈까지 막혔다.** 이 도구로는 그 도메인 전체가 안 열린다.
나머지는 그대로 — Lunit(medium 403) · Blender(403) · dbt Labs · incident.io(404 네 번) ·
Guardian · Flexport · Cal.com. **의료 축은 여섯 번째로 비어 있다.**
