...
**없다 — Grafana Labs 완주.** 도메인 2/2 · **기능 2개.** 회사 **93개.** ⚠️ **큐 2/3**(ScyllaDB · Cygames).

### 이번 사이클 — `색인하지 않은 대가는 읽을 때 치른다`

건진 것 넷:

- ⚠️ **넣을 때 싸게 하면 읽을 때 비싸진다** — 본문을 색인하지 않는 설계의 값이 숫자로 나온다: **청크의 최대 97% 가 로그 본문이고 질의에 쓸 수 있는 구조화 메타데이터·타임스탬프는 3% 뿐이다.** ⚠️ **색인을 더하면 이 시스템이 그 시스템이 아니게 되므로 개선의 방향이 `읽어야 할 양을 줄이는 쪽` 으로만 열려 있다**(재구성) — 새 데이터 포맷과 새 질의 엔진이 그 답이고 **스캔 20배 감소 · 실행 10배 가속**을 얻었다.
- ⚠️ **복제 자체보다 복제가 어긋날 때가 문제였다** — **인제스터가 동기에서 벗어나면 객체 스토리지에 중복이 남고 그것을 질의 시점에 지워야 했다.** **쓰기에서 아낀 일을 읽기가 갚는 구조가 여기서도 반복된다**(재구성).
- ⚠️ **개수를 고르게 나누는 것과 부하를 고르게 나누는 것은 다르다** — **해시 ID 로 스트림을 분산하니 처리량이 큰 덩어리 스트림이 같은 인제스터에 몰렸다.** 볼륨 기준으로 바꿨다.
- ⚠️ **전제에 기댄 절약은 전제가 흔들리는 만큼만 실현된다** — 파일 이름이 같으면 중복이 접힌다는 전제였는데 **시간 동기 편차로 부분만 통했고, 실제 저장 오버헤드가 명목 3배가 아니라 2.3배**였다. ⚠️ **이 수치는 3자 매체(InfoQ) 보도라 `inferred` 로 내렸다** — 회사 발표 전사에서는 확인하지 못했다.

### ⭐ 한 회사의 두 제품이 같은 결론에 도달했다

**Mimir 와 Loki 가 진단도 답도 같다** — **`인제스터가 읽기 경로와 쓰기 경로 사이의 약한 고리`**(Mimir) / **`무거운 질의가 들어오면 쓰기가 느려지고 사용자에게 500 이 돌아간다`**(Loki), 그리고 둘 다 **내구성을 사본에서 Kafka 로 옮겼다.** 성과를 말하는 언어도 같다 — **TCO 25% 감소**(Mimir) / **큰 클러스터에서 최대 30% 감소**(Loki). ⚠️ **다른 데이터, 같은 병목이다** — 비교 문서 ⑥축의 재료가 한 회사 안에서 둘 다 나왔다.

### 다음 사이클 — 신규(6순위)

`--gaps` 가 **큐 맨 위(ScyllaDB)를 `in_progress` 로 만들고 3단계부터** 를 부를 것이다. ⚠️ **ScyllaDB 는 슬러그 추측이 404 를 내는 곳이다** — **검색으로 주소를 먼저 받는다.**
...
**③ `실험 결과를 얼마나 믿나` 3곳** — Etsy / Deliveroo / Snyk. ⏳ **`자기 성과를 어디까지 주장하나` 로 넓힐 수 있다**(Snyk · Grafana 가 자기에게 유리한 주장을 스스로 접는다).

**④ `인도 규모에서 무엇이 달라지나` 2곳** — Meesho / Zepto.

⏳ **⑤ `한 코어에 하나씩인가, 여러 코어가 나눠 쓰나`**(ScyllaDB 대 ClickHouse).

⭐ **⑥ `복제로 버틸 것인가 로그로 버틸 것인가` — 재료가 찼다.** **Grafana 의 Mimir 와 Loki 가 같은 답을 냈고**, ⏳ **Zepto 의 `집계해 버린 원본은 되돌릴 수 없다`(Kafka→ClickHouse)와 Paystack·Plaid 의 이전 이야기가 옆에 놓일 수 있다.**
