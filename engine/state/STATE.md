...
**아무 회사도 안 판다 — 후보 조사 사이클이었다.** 큐 0/3 → **3/3**. 회사 132개.

### 이번에 올린 셋 — 축을 크게 벌렸다

최근 넉 달이 빌드·클라우드·프로파일링으로 몰려 있었다. 일부러 멀리 갔다.

- **Signal**(US·비영리) — 암호 프로토콜과 메타데이터 최소화. 자동 키 검증(2026-08-11), 희소
  후양자 래칫(2025-10-02), 암호화 백업(2025-09-08). 이 엔진에 없던 축이다.
- **Backblaze**(US) — 분기마다 **디스크 고장률을 공개 데이터로** 낸다. 저가 하드웨어 위에서
  내구성을 만드는 이야기가 이 엔진에 없었다.
- **Rerun**(EU) — 로보틱스·다중모달 데이터. 열 청크로 적재 100배·메모리 35배를 얻고
  **질의 속도를 내줬다**고 대가까지 적는다.

### 교훈 — 최신성만 보면 낚인다

접은 넷(LiveKit·Modular·Astral·Zoo)은 **전부 2026년 글이 있었다.** 갈린 것은 날짜가 아니라
자기 시스템의 의사결정과 대가를 적었느냐다. 제품 발표와 사용 가이드는 아무리 새것이어도
역설계할 재료가 아니다.

### 보류 하나 — 깊이는 있는데 대상이 다르다

Memfault 의 Interrupt 는 펌웨어 글의 깊이가 확실한데 **자기 제품이 아니라 일반 임베디드
교육 글**이다. 이 엔진의 단위는 "이 회사가 어떻게 만들어졌나"이므로 자기 시스템 내부를 적은
글을 찾은 뒤에 올린다.

### 다음 사이클 — 신규 (대기 맨 위 Signal)

프로파일부터 판다. 정책·옹호 글이 섞여 있으니 기술 글만 고른다.
