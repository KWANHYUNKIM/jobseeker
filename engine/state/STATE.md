...
**후보 조사를 돌아 큐를 3/3 으로 채웠다.** 파는 회사는 없다 — Inngest 를 닫고 대기가 비어 있었다.
회사 129개, 큐 3/3.

### 이번에 올린 셋 — 또 축을 벌렸다

- **Depot**(US) — CI·빌드 인프라. 마이크로VM 을 1초 안에 초당 200건까지 띄워야 하는데 LVM 이
  볼륨 그룹 전체 잠금으로 연산당 100ms 를 먹어 **직접 다시 구현했다.** ZFS·Btrfs·Stratis·루프 장치를
  이름 대며 물리고, **버린 보장을 명시한다**(크래시 안전성·삭제 복구·메타데이터 보존). 근거가 분명하다 —
  하이퍼바이저가 죽으면 어차피 휘발성 VM 이 사라진다. 커널 device-mapper 는 그대로 두고 제어면만
  바꿔 약 100배.
- **Ubicloud**(US) — 베어메탈 위에 오픈소스로 클라우드를 짓는다. SPDK 블록 저장소 글이 얻은 것과
  잃은 것을 함께 적는다(프로비저닝 4~5분 → 1초 미만, 대신 암호화를 잃고 처리량이 ext4 의 3분의 1).
- **Polar Signals**(EU) — eBPF 지속 프로파일링. 스택 언와인딩·하드웨어 타이머·V8·CUDA 연작이 있다.

### 관측 축에 열 번째 답이 붙는다

이 엔진에서 관측 비용을 다룬 회사가 아홉이었다(여덟 + 안티테시스의 "남기지 않고 다시 만든다").
Polar Signals 는 **프로파일을 상시로 남긴다**는 답이다 — 무엇을 남길지가 아니라 얼마나 싸게
남기느냐로 문제를 옮긴 자리다(재구성).

### 인수·합류는 판정 기준이 아니다

Polar Signals 가 2026-08-17 에 Dash0 합류를 발표했다. Bun(앤트로픽)·Cursor(스페이스X) 때와 같게
다룬다 — 이미 쓴 자료가 팔 수 있는지가 기준이다. 다만 블로그가 마를 수 있다는 것을 큐에 적었다.

### 다음 사이클 — 신규 (Depot)

대기 맨 위가 Depot 이다. 버린 보장을 이름 대며 적는 회사라 도메인을 가르기 좋다.
