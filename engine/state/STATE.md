...
**Jane Street 완주(기능 3개).** 회사 137개, 큐 0/3 — 다음 사이클은 후보 조사다.

### 기다림의 길이가 도구를 정한다

FPGA 합성이 몇 시간, ASIC 은 몇 달이다. 소프트웨어처럼 고쳐 가며 만들 수 없으니 **시뮬레이션이
사실상 개발 환경**이 되고, 그래서 테스트벤치를 다시 쓰는 일이 개발 속도가 된다.

### 모나드 겉치레를 걷어냈다

예전에는 주기 뒤에 올 계산을 표현할 방법이 없어 클로저로 감쌌다. 대수적 효과는 그 '앞으로 올
계산'을 일급으로 줘서 `step` 을 부르고 다음 줄을 그냥 쓰게 한다 — 게다가 **모나드로는 못 쓰던
언박싱 타입이 함께 쓰인다.**

### 자기가 만든 모드가 두 번째로 돌아왔다

파이썬 참조 계수에 쓴 `@ local` 과 유일성이 여기서는 **효과 핸들러와 연속을 가두는 데** 쓰인다.
컴파일러를 직접 만든 값이 서로 다른 두 자리에서 회수되는 셈이다.

### 이 글만 대가를 안 적는다

앞의 두 기능(참조 계수·커널 버그)은 대가를 또렷이 적었는데 이 글은 컴파일 시간도 디버깅도
배우는 값도 적지 않고 효과가 더 우아하다는 쪽으로만 쓴다. **같은 회사 안에서 글의 결이 갈리는
자리**라 그대로 결정 칸에 남겼다.

### 세 기능이 한 질문을 가리킨다

남이 정한 기본값(언어의 수명 관리 · 커널의 구현 · 합성 도구의 주기)을 어디까지 받아들일
것인가. 사업이 안 보이는 회사라 **제약이 기술을 정하는 쪽**으로 읽었다.

### 다음 사이클 — 후보 조사 (큐 0/3, Zig 하나만 남았다)
