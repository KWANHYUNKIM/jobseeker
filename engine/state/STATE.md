...
**없다 — 예순 번째 후보 조사를 돌았다. 큐 2/3.** 다음 사이클은 **신규**(Replicate).

### 자료가 있는 곳을 파니 쌍이 따라왔다

지난 조사에서 **마스토돈으로 마주 보는 쌍을 억지로 만들려다 못 얻었다.** 이번에는 그냥
자료가 두꺼운 곳을 골랐는데 **쌍이 저절로 생겼다** — **워프스트림**(S3 위에 카프카
프로토콜을 새로 얹었다) ↔ **Redpanda**(C++ 엔진을 이미 가진 쪽이 오브젝트 스토리지를
나중에 들였다). 같은 목적지(교차 AZ 비용 제거)에 **출발점이 반대**다.

### 바꾼 것을 이전 구조와 갈라 적는 글

Redpanda 의 Cloud Topics 글이 그렇다 — 원래는 **"데이터와 메타데이터를 Raft 로 쓰고
복제"** 해 전체 페이로드가 로컬 Raft 로그에 들어갔는데, 지금은 **데이터는 오브젝트
스토리지, 메타데이터만 로컬**이다. 대가도 수치다: **0.25초 또는 4MB** 배칭, 캐시 미스 때
**L0 파일들에 흩어진 읽기.**

### 품질이 바뀐다는 것을 인정하고 보상책을 적는다

Replicate 의 파인튠 글이 **"양자화가 출력을 약간 바꾸지만 품질에 미치는 영향은 작다고
봤다"** 고 적고, 그 위에 **`go_fast` 를 켜면 `lora_scale` 을 1.5배 자동으로 올린다**는
보상책까지 밝힌다 — **빨라진 대가로 무엇이 달라졌고 그것을 어떻게 메웠는지**가 다 있다.

### 국가를 또 비워 뒀다

블로그 목록만 보고 회사 소개를 안 열었으므로 큐의 국가 칸을 둘 다 **'소재지 미확인'** 으로
뒀다. 고도에서 근거 없이 적었던 실수를 되풀이하지 않는다.
