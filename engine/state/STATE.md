...
**Sansan(JP · SaaS) `in_progress` — 도메인 3개 · 기능 3개.** 332 에서 **세 번째 도메인
`사람과 기계가 같이 도는 공정을 잰다` 를 열고 기능 `measure-the-mixed-line`(섞여 도는 공정을
한 자로 잰다)을 썼다. 결정 6개 · 그림 3장 · 생각 4개 · 난제 2건 · 연결 1건.**

**🔴 그리고 331 의 판단을 정정했다**(PROMPT.md 2″단계). 331 에서 *"데이터화 공정에서 사람이
무엇을 하는지 글에 없다"* 고 적었는데 **연재 제1회에 없었을 뿐 다른 글에는 있었다.**
- **연재 제2회를 찾았다** — `SansanにおけるエッジAIの活用事例`(2022-11-01). **GEES 에 사람이
  확인하는 단계가 있어 시간이 걸리고, 그 지연이 바로 단말에서 즉시 결과를 주기로 한 이유다.**
  ⚠️ **다만 이 글 자체는 버린 대안도 대가도 없어**(수치만: Core ML 로 **300% 고속화** ·
  검출 **0.01~0.02초**) **기능으로는 안 썼다.**
- **데이터 기반 글을 찾았다** — `Sansanのデータ化オペレーションを支えるデータ基盤hydra`
  (2026-03-18). 공정을 *"AIによる自動処理と人による検証・補正を組み合わせたHuman-in-the-loop"*
  로 적고 **사내 오퍼레이터에 더해 외부 파트너 회사**가 섞인다고 밝힌다. **3/3 통과.**

**⭐ 기능의 축** — **사람이 섞이면 실적이 곧 돈이 되고, 돈이 되면 재는 일에 정확성이 필요해진다.**
Digitization 부문의 시스템이 **10개 이상**인데 *"作業実績データ…は各システムに閉じたままで"*
라 인력 배치도 못 하고 **파트너 지급액을 손으로 셈했다.** 답은 **중앙 공통 스키마**(하류 변환을
버린 이유가 *"スキーマ変更が検知されないまま下流に伝播し"*)와 **두 경로**(스트리밍은 보기 위해,
일 배치는 셈하기 위해)다. **중간 스트림 부품을 안 끼운 것도 앞의 결정 덕이다.**

**⭐⭐ 331 의 기능과 `connections` 로 이었다** — `narrow-to-win` 은 **기계가 얼마나 잘 읽는지**를
재고(주소 98.3%), 이 기능은 **공정이 얼마나 돌았는지**를 잰다. **빠른 답과 정확한 답이 다른
경로로 나오고 그 사이를 사람이 메운다.** ⚠️ **즉시 준 답과 확정된 답이 다를 때 사용자에게
무엇이 보이는지는 어느 글에도 없다.**

**⚠️ 안 쓴 수치 하나** — 검색 조각에 **GEES 가 AI 와 오퍼레이터를 조합해 정확도 99.9%** 라는
서술이 보였는데 **본문에서 확인 못 해 안 썼다**(325·327 과 같은 판단). **사실이라면 값이
크다** — 모델 쪽 확인값이 98.3% 이니 **차이를 사람이 메운다는 뜻**이 된다(추정).

---

**다음 사이클 — 열린 도메인 셋이 다 찼다.**
- **⭐ 1순위 — 실물이 있는 도메인.** 332 의 검색에서 **재료가 여럿 보였다**(제목만 확인) —
  `当たり前を作っていくオペレーション：Bill Oneスキャンセンターの挑戦`(2024-10-23) ·
  `コードの向こう側に事業と現場がある。Digitization部の魅力`(2026-05-25) ·
  `インフラの枠を超えて。データと仕組みで、Digitization部の未来をつくる`(2026-07-15) ·
  `AIと並走して作る、現場と地続きのシステム設計`(2026-06-26) ·
  이미 읽은 `サービスごとに異なる原本管理を、一つの仕組みへ`(2026-08-20).
  **⚠️ 뒤의 셋은 제목이 대담·소개 성격으로 보인다** — **스캔 센터 편(2024-10-23)을 먼저 연다.**
  **되돌리기가 화면 안에서 안 끝나는 축은 이 사이트에 없는 종류다.**
- **2순위 — 안 읽은 이관 글들.** Swift Concurrency(2026-03-19) · Jetpack Compose(2025-11-17) ·
  Eight 의 EC2 탈출과 모듈러 모놀리스화 · 레이어드 아키텍처 회고(2019-04-21) ·
  모바일 개발 대담 2부작. **⚠️ 223 규칙으로 기존 도메인에 붙는지 정한다.**
- **3순위 — 닫는다.** 닫을 때 `business_model` 에 적을 관찰 — **⭐ 이 회사는 자기가 만든 것을
  안 쓴다고 적는다**(초해상). **⚠️ 가장 아프다고 말한 항목(이메일·전화)의 정확도는 공개하지
  않는다.** **⭐ 그리고 글의 성격이 부문마다 갈린다** — 개발 환경 쪽은 대가를 아낌없이 적고,
  제품 기술 쪽은 성과 중심이며, **Digitization 쪽은 대담이 많다**(추정).

**📌 그 밖**
- **큐 2/3**(freee · SmartHR). **⚠️ Personio(EU)는 아직 안 봤다.**
