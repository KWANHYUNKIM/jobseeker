...
**Bluesky**(US · 분산 소셜) — 도메인 2개 중 **1개에 기능이 들어갔다**(`relay-and-prism`).
남은 빈 도메인 하나: '모더레이션을 한 겹의 권한이 아니라 층으로 쌓는다'. 큐 0/3.

### 모으는 쪽을 고른 이유가 흩어질 자유를 위해서다

이번 기능의 축이다. 보통 중앙 집계는 **분산의 후퇴**로 읽힌다. 그런데 서버 대 서버로 가면
각 PDS 가 네트워크 전체의 부하를 지고, 그러면 개인이 자기 서버를 못 돌리고, 못 돌리면
**떠날 자유가 말뿐**이 된다. **자유의 조건이 부하의 크기**라는 판단이다.

### 모으는 권한과 고르는 권한을 갈라 놓았다

릴레이는 **거르지 않는다** — 날것의 파이어호스를 내보내고 고르는 일은 하류(앱뷰·피드
생성기·라벨러)가 맡는다. 가운데를 두면서도 그 가운데가 **무엇을 보일지 정하지 못하게** 한
것이다. 그래서 가운데가 커져도 편집권은 안 따라온다.

### 후보까지만 받는다

피드 생성기는 **"피드의 후보를 제시하는 것으로 제약된다."** 제3자가 만들 수 있는 경험의
폭은 좁아지지만, 앱뷰가 마지막 책임을 져서 모더레이션과 사용자 설정이 뒤에 남는다.

### 자유를 위해 중심을 두는 역설

난제 칸의 첫 줄이다. 가운데를 안 두면 각자가 감당 못 하고, 두면 릴레이가 **'꽤 자원을 많이
요구하는 서비스'** 라 소수만 남는다. 대응은 셋 — 릴레이가 못 거르게 하고, 프로토콜에
서버 대 서버를 남겨 두고, **작은 맞춤 릴레이**가 조각을 맡게 한다.

### 규모와 비용이 하나도 없다

저장 용량도 대역폭도 하드웨어 사양도 요금도 없다. 이 글이 **샌드박스를 열며 쓴 계획서**라
그럴 수 있는데, **3년이 지난 지금까지 그 수치를 담은 후속 글을 못 찾았다는 것**은 다른
문제다. `failure` 도해에 안 밝힌 것 여덟 가지로 세웠다.

### 다음 사이클

남은 도메인 하나 — 모더레이션. 라벨러가 이미 이 기능의 하류 부품으로 이어져 있어 **연결이
확정(confirmed)** 이다. 다만 **커뮤니티 라벨링이 2023년 당시 미완성**이었으니 그 뒤를
확인해야 한다(Ozone 오픈소스 글 2024-03 을 아직 안 열었다).
