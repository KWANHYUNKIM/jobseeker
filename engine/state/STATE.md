...
**하이퍼커넥트 (KR · 소셜/실시간 영상) — 46번째. 기능 3개 · 도메인 3개. 열어 둔 도메인이 다 찼다. 큐 1/3(컬리).**

257 에서 **세 번째 도메인 `바닥을 바꾸면서 서비스를 멈추지 않는다`** 를 열고 **`raid-recovery`
(스물네 시간을 한 시간으로 줄인다)** 를 썼다. 결정 6 · 그림 3 · 생각 4 · 스택 6 · 난제 2 · ui 6핀 ·
연결 1(`stream-join` 과 **대비**).

**🔴 이 회사에 대한 관찰 둘이 정정됐다.**
- **'결과의 크기를 잘 안 적는다' 는 제품 쪽 글에만 맞다.** ScyllaDB 글(2025-05)은 **복구 18~24시간에서
  약 1시간(약 20배) · 노드 교체 자동화 98% 이상 · PoC 7억 행(노드당 약 1.5TB) · EBS 3,492GB ·
  i4i.4xlarge** 를 다 적고, ARM64 글(2023-07)도 **1년에 EC2 청구의 47% 이상 · 100개 넘는
  마이크로서비스**를 적는다. **인프라 쪽이 제품 쪽보다 수치가 많다**(추측으로 적었다).
- **'글끼리 서로를 언급하지 않는다' 도 깨진다.** ARM64 글이 *"10+개 이상의 microservice가 Flink를
  사용하여 실시간 streaming data 처리"* 하고 **아자르의 match 서버가 Flink 를 쓴다**고 적는다.

**🔴 이 기능의 축 — 빠른 것과 안 사라지는 것 중 고르지 않고 묶었다.** 로컬 NVMe 는 *"disk 데이터가
전부 유실된다는 문제가 존재합니다"* 이고 EBS 단독은 읽기가 느리다(**Discord 의 경험을 인용해 버린다** —
*"Read latency 지연 문제를 겪었다고 합니다"*). **묶음이 성립한 이유가 저장 엔진에 있다** — LSM-tree 가
쓰기를 모아 내리므로 양쪽에 써도 *"write 성능이 저하되지 않습니다"*.

**📌 남의 공개 기록이 다른 회사의 설계 근거가 된 자리다** — 이 사이트에 **Discord 가 있다.**
직접 재지 않고 후보를 버렸고, 아낀 값을 **7억 행 PoC** 에 썼다(이 해석은 이 사이트의 것이다).

**🚨 다음 사이클 — 하이퍼커넥트를 `done` 으로 닫는다.**
**남은 재료가 얇다** — ARM64 글은 **이미 도메인 `tech` 에 넣었고 버린 대안이 명시돼 있지 않다**(다른
비용 절감 수단과 비교하지 않는다). 실패 목록은 길지만(**SIGSEGV · byte order 로 utf8 처리 실패 ·
빌드 10배 지연 · Istio 비공식 빌드 · Fluent-bit 한글 인코딩 패치 · Netty 네이티브 로드 실패 ·
Flink 1.14+ 강제**) **결정 5개가 서지 않을 가능성이 높다.** 클러스터 이전 글(2020-11)은 **6년 전**이다.
**억지로 쓰지 말고 닫는다.** 회사 `status` · `index.json` status · **QUEUE 진행 중 → 완료**(불릿).

**닫으면서 `business_model` 에 반복 패턴을 적는다. 근거가 여섯 편이라 적을 만하다**(이 사이트의 정리로 밝힐 것):
① **제품 쪽 글과 인프라 쪽 글의 성격이 다르다** — 제품 쪽(목표 지표 3부작 · Flink 조인)은 **버린 대안이
많고 개선 폭이 없거나 하나뿐**이고, 인프라 쪽(ScyllaDB · ARM64)은 **수치가 촘촘하다.**
② **실패는 양쪽 다 적는다** — *"PUR(Purchase User Rate)이 떨어졌습니다"* · *"복구 시점에 이벤트 조합
성공률이 낮아지는 것을 확인"* · SIGSEGV 와 빌드 10배 지연.
③ **실시간이라는 조건이 세 층을 관통한다** — 무엇을 목표로 삼을지(며칠 뒤의 리텐션을 지금 재려고 아하
모먼트를 만들었다) · 재료를 얼마나 빨리 만들지(Exactly Once 를 껐다) · 바닥이 흔들릴 때(복구를 20배로 줄였다).

**⚠️ 사업 수치를 끝내 못 찾았다** — Match Group IR 홈에 하이퍼커넥트도 아자르도 없다. 받은 것은
**LTM 매출 $3.5B**(2026-06-30 종료 분기)뿐. 단서는 2026-01 글이 **Match Group 의 2023 2Q 실적**을
참조한다는 것뿐이고 **소유 관계를 그것으로 단정하지 않았다.** 닫을 때도 그대로 둔다.
...
**⚠️ 한국 편중 — 컬리까지 파면 아홉이다. 다음 후보 조사는 한국 밖에서 한다.**
...
- **`되돌릴 수 있는 것을 먼저 한다`** — CyberAgent `env-split-migration` 은 **반례**(되돌림 계획 자체가
  없다)이고, 하이퍼커넥트는 **두 가지 정반대 사례**를 준다: `stream-join` 은 **되돌리는 길(Savepoint)이
  배포 절차 자체**이고, `raid-recovery` 는 **늘 두 벌을 유지해 되돌릴 것도 없이 채운다.**
