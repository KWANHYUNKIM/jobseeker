...
**Meesho** (IN · 커머스 — 소셜 커머스) — **프로파일까지 썼다**(PROMPT 3단계). 도메인 **4개** · 기능 0. 회사 **90개.**

### 이번 사이클 — 신규(6순위). 큐 맨 위를 올렸다

⚠️ **이 회사의 축은 `커미션을 안 받는다` 는 결정 하나다.** **인도 커머스 중 처음으로 판매 수수료를 없앴고**, 대신 **물류(Valmo)와 광고**로 번다(광고가 가장 빨리 큰다). **그 결정이 기술 문제를 바꿔 놓는다** — **거래마다 떼는 몫이 없으니 정산은 `얼마를 뗐나` 가 아니라 `왜 이만큼인가` 를 설명해야 하고, 물류를 직접 하니 주소를 읽는 일까지 이 회사의 문제가 된다**(재구성).

**도메인을 넷 세웠다** — 보통 셋인데 넷으로 간 것은 **각 도메인마다 글이 둘 이상 확인됐기 때문**이다. ⚠️ **Paystack 에서 자료 없는 도메인을 세웠다가 완주가 막힌 적이 있어, 이번엔 목록에서 근거를 먼저 셌다.**

- **① 판매자 정산** — ✅ **본문 확인함**(2026-05-26). 아래 참고.
- **② 주소를 읽는다** — `GeoIndia`(2026-05-07) · `배송 기사가 이미 아는 것을 지오코더에 가르친다`(2026-05-25). ⭐ **이 사이트에 없는 도메인이다.**
- **③ 사기 네트워크** — `TrustMesh`(2026-07-15) · `숨은 연결을 드러낸다`(2025-07-31) · `모든 주문에는 위험이 따른다`(2026-06-05).
- **④ 이미지·저사양 기기** — `픽셀 격차`(2026-06-24) · `알맞은 픽셀`(2026-08-25) · `XML 레이아웃의 350KB`(2026-08-17).

### 읽어 둔 것 — 정산 글에서 건진 것 넷 (다음 사이클의 재료)

- ⚠️ **매일 14만 명 넘는 판매자에게 돈이 나가는데 그들이 그 숫자를 이해하지 못했다** — 공제가 **물류·세금·광고·인플루언서 마케팅·대출 상환**처럼 다른 시스템에서 오는데 화면에는 **`총 미결제 잔액` 한 줄**이었다.
- ⚠️ **모든 판매자 결제가 게이트웨이 하나만 지나갔다** — 대체 경로가 없었다. 두 번째 결제 파트너를 붙여 **실패율 3% → 0.9~1.2%.**
- ⚠️ **`빠른데 틀린 정산은 여전히 나쁜 정산이다`** — 속도보다 **처음에 맞게 주는 것**(FTRP)을 앞에 뒀고 **95% 이상**으로 올렸다. **모든 회수를 정산 전에 끝낸다.**
- ⚠️ **내부 팀이 스스로 회수 로직을 정의하는 셀프서브 플랫폼**을 만들자 **예정대로 회수되는 비율이 사실상 0 에서 98% 이상**이 됐다. **P90 처리 시간 35~40시간 → 약 25시간.**

### 다음 사이클 — 확장(2순위)

`--gaps` 가 **`판매자에게 언제 얼마를 왜 주는지 설명한다`** 를 부를 것이다. **자료를 이미 읽었으니 바로 기능을 쓴다.**

⚠️ **읽는 법** — `www.meesho.io/blog` 는 **WebFetch 403** 이고 **`r.jina.ai` 를 앞에 두면 온다.** 원문 인용이 필요하면 브라우저 `get_page_text` 로 받는다.

### ⚠️ 비교 문서 재료 세 축 (초안 유지)

**① `AI 에이전트를 어디까지 믿나` 6곳 + ⏳ Snyk** — Sentry / ClickHouse / Duolingo / Ramp / DoorDash / Deliveroo(**검사 가능한 것만 강제된다**). ⏳ **Snyk 가 일곱 번째가 될 수 있다.**
