...
**Rightmove(EU/UK · 커머스—부동산 포털)** `in_progress` · **도메인 2 · 기능 1** —
72곳 · ⚠️ 큐 0/3.

방금 쓴 기능: **`sitemap-by-events`(밤새 다시 세던 것을 바뀔 때마다 고쳐 쓴다)**.

### ⭐⭐ 이 글의 미덕 — 버린 것을 장점 칸에 남겨 뒀다

RFC 표에 옛 배치의 **장점이 셋**이나 적혀 있다: 추가 저장소 불필요 · 사내 기존 패턴 ·
**디버깅이 쉽다.** 그럼에도 **회복력 하나**가 그것을 이겼다
(`Not resilient: A single instance meant any failure disrupted the process.`).
**장점 셋을 내주고 회복력 하나를 산 거래**를 그림으로 그렸다.

### ⭐ 5시간이 30분이 된 이유는 빨라져서가 아니다

**이미 RX Java 로 병렬화하고 캐시를 쓰고도 5시간**이었다 — 같은 방식 안의 최적화는
한계에 닿아 있었다. 바뀐 것은 방향이다: **수십만 건의 REST 로 물어보던 것을 이벤트로
받아 두는 쪽으로.** 그리고 **시간이 길다는 것은 실패할 창이 길다는 뜻**이기도 했다
(이 해석은 재구성이다).

### 다음 사이클 — 남은 도메인

**자바와 HTML 이 얽힌 화면을 걷어낸다** — `monolith-to-microservice...`(전문 읽음).
⚠️ **수치가 하나도 없는 글이다**(개발 기간·성능·사용자 수 전부). 그래도 **기술 부채가
사람 문제로 나타난 자리**라 쓸 값이 있다: JSP 는 `a pattern that interweaves Java code
with HTML` 이라 **화면을 고치려면 자바를 알아야 했다.**
여기까지 채우면 **완주 조건에 닿는다.**

### 아직 못 잡은 것

1. **재무를 3자 매체로만 확인했다** — `plc.rightmove.co.uk` 1차 IR 을 열면 `confirmed` 로 올릴 수 있다.
2. **기술 글이 5페이지 분량인데 두 편만 읽었다.**
3. **다시 지은 앱의 이름을 글이 안 밝힌다.**
