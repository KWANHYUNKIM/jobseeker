...
**PayFit(EU · SaaS) `in_progress` — 60번째 회사.** 354 에서 **프로파일**을 썼다
(PROMPT.md 3단계). **도메인 1개 · 기능 0개.** 다음 사이클부터 기능이다.
**완료 59 + 진행 1 = 60곳 · 큐 0/3 · 비교 문서 32편.**

**⑭ `--gaps` 가 [신규] 를 가리켰고 대기 맨 위가 PayFit 이었다.**

**⚠️⚠️ 이번 사이클의 가장 큰 결과는 정정이다**(`payfit.json` 의 `open_questions` 에 적었다).
349 는 `How we managed to scale-up the PayFit architecture`(2021-10-07)를 **2/3** 으로 봤는데,
본문을 다시 읽으니 **1/3 이다.**
- **버린 대안에 이유가 없다** — 다른 회사는 단일 저장소를 유지하기도 한다고만 하고
  **재고 버린 대목이 없다.**
- **대가는 아예 안 적는다** — 349 가 대가로 본 *"We're only at the beginning of our vertical
  transformations"* 는 **대가가 아니라 진행 상황**이다. 재독의 답은 *"Trade-offs admitted: None"*.
- 수치도 창업자 3명·2021년 채용 목표 250명·연도들뿐이고 **쪼갠 뒤 무엇이 나아졌는지가 없다.**
**⭐ 교훈 — 후보 조사 때 요약을 근거로 판정하면 과대평가가 난다.** 349 는 WebFetch 요약만
보고 2/3 을 매겼다. **다음 후보 조사부터는 '무엇이 없는지' 를 함께 물어 판정한다**(352 의 방식).

**세 편을 열어 하나만 썼다**
- `How we managed to scale-up the PayFit architecture`(2021-10-07) — **1/3.** 위 참조.
- `Of PostgreSQL, Indexes, and Foreign Keys`(2023-01-27) — **대가와 수치는 있는데**(인덱스가
  읽기를 78퍼센트로 개선하고 쓰기는 101~106퍼센트로 느려진다, 벤치마크를 일반 지표로는 못
  믿는다고 스스로 밝힌다) **회사 시스템 이야기가 아니라 일반 PostgreSQL 벤치마크 해설이다.**
  **353 의 교훈대로 구현 층이 없어 기능으로 못 쓴다.**
- **✅ `PayFit on the Domain-Driven Design Journey`(2023-06-02) — 2/3, 회사 자기 시스템의
  서사라 이것으로 도메인을 열었다.**

**연 도메인 하나** — `급여라는 도메인을 어디서 자를 것인가`(**넓게 지었다**). `tech` 셋을 그 한
편에서만 채웠다 — 이벤트스토밍과 워크숍 · 바운디드 컨텍스트 캔버스와 메시지 흐름도 ·
도메인에 맞춘 팀 편성. **⚠️ 두 편 규칙이 아직 안 채워졌다.**

**⭐⭐ 이 회사의 증상이 아주 구체적이다** — **Pay Period 라는 말을 팀마다 다르게 써서**
어긋났다. 그래서 **Payroll Period** 를 갈라냈고, 갈라 놓고 보니 **계산과 명세 생성이 같은 것이
아니었으며 그것이 사용자 경험에도 영향을 줬다.** 회사의 문장 — *"Naming is hard and it is a
key driver for all kinds of decisions, both business and technological."*

**사업 수치는 거의 확인하지 못했다** — 회사 자체 발표의 **€254M 조달 · €1.82B 밸류에이션**만
썼다. **고객 수는 3자 정리마다 크게 어긋나**(5천 대 2만 이상) 쓰지 않았고, 매출과 진출 국가
목록도 1차 자료로 확인하지 못했다.

---

**다음 사이클 — PayFit 의 첫 기능. 두 편 규칙부터 채운다.**
- **⭐ 1순위 — JetLang 본문을 찾는다.** **이 회사의 기반인데 한 줄 설명만 확인했다** —
  ...
  **⚠️ ⑰ 주소를 추측하지 않는다. ⑱ WebSearch + allowed_domains 로 찾고, 블로그에 없으면
  컨퍼런스 발표나 채용 페이지까지 본다**(단, **1차 자료가 아니면 그렇게 밝힌다**).
  **⭐ 이게 이 사이트에 없는 결이다** — 규칙을 코드로 만드는 도메인.
- **2순위** — `Exploring our tech architecture vision`(2024-01-24). **⚠️ 비전 글이라 일반론일
  위험이 있다**(336·353 의 전례).
- **3순위** — `Our Data Assistant Adventure @ PayFit`(2024-04-17) ·
  `AI at PayFit: Empowering Through Education and Innovation (Vol. 1)`(2024-07-08) ·
  `An introduction to load testing and k6`(2022-07-13).
- **4순위 — 닫는다.** 도메인 하나에 기능 하나만 있어도 완주 기준은 찬다.
- **⚠️ 엔지니어링 태그가 여덟 편뿐이고 최신이 2024-07-08 이다. 도메인 한둘이 한계로 보인다.**

**⚠️ 큐가 0/3 이 됐다 — PayFit 을 완주하면 곧바로 후보 조사다.**
**후보 조사 누적 스물세 곳 중 아홉.** **⚠️ 다음 후보 조사에서는 요약이 아니라 '무엇이 없는지'
까지 물어 판정한다**(354 의 정정에서 나온 규칙).

**⭐⭐ 비교 문서 재료 다섯**(회사를 파는 중에는 손대지 않는다):
① **freee 대 SmartHR 대 PayFit — 같은 문제, 세 가지 답.** 셋 다 급여·인사 SaaS 이고 **제품이
늘며 경계가 무너진** 같은 문제를 겪었는데, freee 는 **떼어냈고**(4년, 후회를 명시한다),
SmartHR 은 **Packwerk 로 규칙을 긋다 안 되어 프로세스로 나눴고**, PayFit 은 **이름부터 다시
정했다**(2년). **⭐ 349 에서 두 회사였던 것이 354 에서 세 회사가 됐다.**
② **`미루되 미룬 것이 안 보이게 한다`** 에 SmartHR 두 사례 — 미룬 쪽은 **놀고 있는 접속**,
미리 잰 쪽은 **연 1회 정점의 부하 시험**.
③ **bol 이 ②의 셋째 사례** — 중개 앱 2031년 유지를 버린 것과
...
**354 에서 또 값을 냈다 — 첫 편이 얇아 두 편을 더 열었고 세 번째가 재료였다**) ·
...
나온다**(351) · **글을 열 때 '무엇이 없는지' 를 함께 묻는다**(352) · **기준을 넘어도 실제로
실행하지 않은 평가는 구현 층이 없어 기능으로 못 쓴다**(353) · **⭐ 새로: 후보 조사 때 요약만
보고 판정하면 과대평가가 난다 — 349 의 2/3 이 354 에서 1/3 로 내려갔다**(354).
