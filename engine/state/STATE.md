...
**(회사는 없다. 사이클 235 로 보강이 끝났다 — 경고가 15 → 2 → **1건**이 됐다.)**

**kakao `domains[동기화·다중 기기]` 에 도메인 레벨 `hold_reason` 을 붙였다.** 채우지 못한 이유가
분명해서다 — **이 도메인은 `tech` 도 `features` 도 둘 다 비어 있었다.** 초기 사이클이 문제의
경계만 열어 두고 채우지 못한 자리라 **기능에서 도메인 층위로 끌어올릴 기술 서술이 아예 없다.**
1차 자료도 막혔다: `tech.kakao.com` 은 **목록에 최근 3편만 렌더링되고**(2026-08 · Airflow 테스트
환경, Kanana 음성 생성) **다중 기기 동기화·읽음 처리 글이 안 보이며 본문도 WebFetch 로 안 열린다.**
브라우저는 여전히 탭을 못 만든다(228). **한 번만 두드리고 접었다.**

**확인한 것 둘.**
- **`hold_reason` 은 도메인 레벨에서 경고를 실제로 거른다** — `validate.py:312`
  (`if not techs and not dom.get("hold_reason")`). `--gaps` 만 거르는 게 아니다.
- **`updated_at` 은 회사 파일과 `index.json` 양쪽을 같이 고쳐야 한다.** 한쪽만 고쳤더니
  검증이 ✗ 로 잡았다(`index.json.companies[kakao]: updated_at 가 상세와 다르다`).
  **다음에 회사 파일을 손대면 index 도 함께 본다.**

**남은 경고 1건은 고치지 않는다** — youtube `content-id` 의
`"이의 제기(dispute) · 항소(appeal) · 저작권 삭제 요청 · 저작권 경고(strike)"` 는
**구분자가 없고 이름만 나열한 줄**이라 `name`/`what` 으로 가르려면 설명을 지어내야 한다(234).
**이건 고장이 아니라 기록이다.**

**🚨 사다리에서 보강이 비었다. 다음 사이클은 비교 문서 씨앗이다.**
**②를 먼저 쓴다**(재료가 더 많다).
② **'되돌릴 수 있는 것을 먼저 한다'**(재료 다섯) — Figma 논리→물리 샤딩 · WebGPU 동적 폴백 ·
   Shopify 섀도 모드 · 토스 카나리 자동 롤백 · Yelp `in-place-swap`.
   **⚠️ key 이름을 전부 파일에서 확인할 것** — 기억으로 쓰지 않는다.
   **⚠️ 233 `which-way-it-leans` 와 결이 가깝다**: 233 은 **오차의 방향**, 이건 **작업의 순서**다.
   인용 블록에서 범위를 갈라야 한다. `parallel-cutover`(두 시스템을 나란히 돌려 옮기기)와도
   겹칠 수 있으니 **`domains/index.json` 을 먼저 훑어 교차 참조를 정한다.**
① **'미루되 미룬 것이 안 보이게 한다'**(재료 셋) — Doximity `cdc-overlap`(base+delta 뷰가 미룬
   물리 병합을 감춘다 · *"나이와 양 임계값이 그 읽기 증폭을 통제"*) · Careem `cost-leaks`
   (삭제가 즉시가 아니라 예약이라 **언두 로그 5.7TB · HLL 58.7억**이 쌓였다) ·
   PlanetScale `concurrency-limits`(**⚠️ key 이름 확인**).

**📌 그 둘까지 쓰고 나면 사다리에 남는 것이 후보 조사(막힘)뿐이다.**
스킬 문서가 그 경우를 정해 뒀다 — **없는 일을 만들지 않고 그 사실만 STATE 에 적고 끝낸다.**
**채울 게 없을 때 지어내는 것이 이 데이터를 망치는 가장 빠른 길이다.**
후보 조사를 다시 하려면 **새 단서가 있어야 한다**(230 에서 여덟 곳 두드려 0곳).
...
**🔴 비교 문서 절차(231~233)** — 기존 문서 하나를 통째로 읽어 짜임을 따르고, 재료는 회사
파일에서 직접 읽고, **`companies`/`features` 값이 실제 slug·key 와 맞는지 직접 검산한다**
(`validate.py` 는 비교 문서를 안 본다).
