...
**없다 — 문체 보수 사이클이었다.** 컬리를 배민 문체로 다시 썼다(1.5 → 0.0, 기호 489개로 최다). 회사 115개, 큐 2/3. 19곳 남았다.

### 🔴 가 네 번째다

사이보우즈(64) · 하이퍼커넥트(96) · SmartHR(158)에 이어 여기서 **165개**. 사업 모델 안의 조사
로그(①~④)와 엔진 내부 번호(`261 에서`)도 함께 나왔다. 네 회사 연속이다.

### 드문 것을 잡는 문제 — 정확도가 아니라 희소성이 벽이다

- Meesho 는 착불 반송이 10% 미만이라 표본이 불균형하고 사기꾼이 깨끗한 이력을 먼저 쌓는다.
- 컬리의 오배송도 전체 배송 대비 매우 낮은 비율이라 **정답 사례를 모으는 것부터 안 된다.**

그리고 개인정보를 이 회사 안에서 정반대로 다룬다 — 오배송 탐지는 **애초에 안 모으고**(역산 안 되는
임베딩만), 암호화 모듈은 **모으되 잠근다.**

### 남의 규칙을 받아 주는 답이 여섯이 됐다

Plaid(흡수해 감춘다) · Ramp(조립하게 둔다) · Paystack(못 정하는 시계를 적는다) · 볼트(맞춰 본다) ·
누방크(한 벌로 만든다) · **컬리(기다린다)**. 그 기한의 근거가 업무에서 온다는 점이 드물다 —
실물이 물류센터에 닿기까지 걸리는 1~2일이 곧 데드라인이다. **기술이 아니라 물건이 기한을 정한다.**

### 조직의 경계가 아키텍처를 정한 자리

AccessBlock 의 출발점은 기술이 아니다 — **개발팀에 nginx 접근 권한이 없다.** 인프라팀에 부탁해
라우팅을 바꾸고 재시작할 때쯤이면 상황이 끝나 있어서 차단을 애플리케이션 안으로 가져왔다.
DoorDash 가 가장 어려웠던 것이 사람이었다고 적은 것과 같은 결인데, **여기서는 그것이 회고가 아니라
설계로 나온다.**

### 그 밖의 두 축

평문을 누가 못 보게 하느냐에서 네 번째 자리다(1Password·LY·Block 다음으로 **저장된 것을 못 읽게**).
스트림 창은 하이퍼커넥트와 같은 자리인데 방향이 다르다 — Klaviyo 가 먼저 처리할 것을 고르는
문제라면 **여기는 언제 처리를 끝냈다고 할 것이냐의 문제다.**

### 다음 사이클 — 문체 보수 (사다리 3순위)

--gaps 가 정한다. 지금 맨 위는 mercadolibre(1.48)지만 그건 이 시점의 예상이지 확정이 아니다.
