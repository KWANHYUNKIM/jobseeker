...
**없다 — Classi 를 완주했다(도메인 2 · 기능 2).** 큐 0/3 이라 다음 사이클은 **후보 조사**다.

### 원인을 어디서 찾느냐가 해법을 정한다

멈춘 프로젝트를 **의지 부족이나 견적 실패로 적지 않았다.** **'핵심 인력이 로테이션될 때마다
동력을 잃었다'** 는 **구조 진단**이고, 그러니 고침도 **구조**로 간다 — **중간 목표를 한
사람에 안 기대게, 마감이 있는 일을 떼어 내고, 소유를 흩는다.**

### 되돌릴 수 없으면 막고, 즉시 드러나면 맡긴다

두 도메인의 AI 기준이 한 짝이다. **틀리면 안 되는 자리**(영업 리포트의 SQL — 학교에 직접
전달된다)는 **손에서 빼고**, **틀리면 바로 드러나는 일**(타입 정의 — 구현 중 타입 오류로
나온다)에는 **맡긴다.**

### AI 는 코드베이스를 흉내 내는 존재다

**'좋은 예를 먼저 배운다'** — 팀 관례를 사람이 먼저 익히고 나서 맡긴다. 안 그러면
**코드베이스 다른 곳의 레거시 패턴을 재생산**하고 **기존 버그도 그대로 가져온다.**

### 그대로 옮기기를 물렸다

**'현재 동작을 그대로 보존한다'** 를 거부하고 **이관을 제품 개선의 기회**로 썼다. 검수는
어려워진다 — **같은지만 보면 되는 것**에서 **무엇이 옳은지를 매번 정하는 것**으로 바뀌기
때문이다. **카카오페이가 Shadow 비교로 '같음'을 증명한 것과 정반대 방향**이다.

### 4개월·한 명·2,300줄

Backbone 화면 하나를 그렇게 끝냈다. 그런데 **리아키텍트 쪽에는 수치가 없다**(일정도 화면
수도) — **같은 회사 안에서 글마다 수치의 밀도가 다르다.**
