...
**freee(JP · SaaS) `in_progress` — 도메인 1개 · 기능 2개.** 340 에서
**`swap-without-anyone-noticing`(아무도 모르게 갈아 끼우고 한 번만 멈춘다)** 을 썼다.
**결정 6개 · 그림 3장 · 생각 4개 · 난제 2건 · 연결 1건.**
재료는 `これってもしかして……認証基盤が入れ替わってる〜？`(2022-11-29). **판정 3/3.**

**✅ 두 편 규칙을 채웠고 도메인 `why` 의 "글 한 편으로 썼다" 문장을 고쳤다**(PROMPT.md 2″단계).

**⭐ 같은 프로젝트를 두 각도로 봤다** — 339 는 **왜 4년이 걸렸나**(순서), 340 은 **어떻게
사용자가 모르게 갈아 끼웠나**(전환의 기법).

**340 이 새로 낸 것**
- **1,454일** 이라는 구체적 숫자. 그중 **마지막 1년을 차분 오류 수정에만** 썼다.
- **다운타임은 심야 점검 한 번뿐** — Auto Increment 발번 전환. **거꾸로 돌리면 ID 충돌 위험**
  이라 이 한 곳만 무중단을 포기했다.
- **전환이 배포가 아니라 설정이다** — *"環境変数の切り替えも無停止で出来るような形をとって
  いました"*, EKS 라 즉시 전파. **켜는 것이 설정이면 되돌리는 것도 설정이 된다.**
- **트래픽이 가장 많은 회계를 마지막에** 넘겼다 — 앞의 열세 제품이 시험대가 됐다.
- **⭐⭐ 차분 알림이 원인을 안 알려준다** — *"差分があるぞってだけ出てくるんですけど、何が
  どうなって差分があったのかっていうのは教えてくれない"* 이고 *"普通にローカルでやってると
  今後再現しないやつばっかり"* 라 **손으로 데이터베이스를 뒤졌다.** **검증 장치를 만드는 것과
  그 결과를 해석하는 것은 다른 일이다**(이 표현은 이 사이트의 것).
- **샤딩과 Aurora 이관을 먼저 해 봤고 부족했다**(회고 편에는 없던 사실).
- **세션 관리가 인증 라이브러리에서 KVS 를 직접 쳤다** — 그래서 고치기 어려웠다.

**⭐⭐ 두 글을 겹치면 4년의 모양이 드러난다**(`connections` 와 도메인 `why` 에 적었다) —
회고는 **크게 잡아 릴리스가 적었다**고 하고 전환 쪽은 **마지막 1년을 차분 수정에만 썼다**고
한다. **앞의 3년이 만든 빚을 마지막 1년이 갚은 셈이다.** 그리고 **두 교훈이 같은 곳을
가리킨다** — *"旧機能のリファクタはマイクロサービス化の前に着手すべし"*(로직)와
*"古い良くないデータっていうのをちゃんと最初に整理すべき"*(데이터).

---

**다음 사이클 — 열린 도메인이 찼다. 다른 축을 열거나 닫는다.**
- **⭐ 1순위 — 다른 축.** `確定申告を乗り越えるDBパフォーマンス改善 - Aurora 移行の舞台裏`
  (**확정신고라는 연 1회의 부하 정점**이 있는 것으로 보인다, 추정). **⚠️ 340 에서 확인한
  사실이 하나 있다 — 인증 기반 이관 전에 샤딩과 Aurora 이관을 먼저 해 봤고 부족했다.**
  **그 Aurora 이관이 이 글일 가능성이 있다**(추정 — 열어서 정한다). 그러면 **223 규칙상 같은
  도메인일 수도, 부하 정점이라는 다른 문제일 수도 있다.**
- **2순위** — `Langfuse のバージョンアップに伴い ClickHouse を社内初導入した話`(PostgreSQL 은
  트랜잭션, ClickHouse 는 트레이스 집계) · `【連載 第4回】freeeカード Unlimited でのClean
  Architecture実践` · `freee流、クレジットカードのマイクロサービス設計構築術`.
- **⚠️ 3순위 — QA 축. 분류가 95편인데 한 편도 안 읽었다.** 340 의 검색에서 관련 글을 봤다 —
  `権限管理基盤マイクロサービスで行っているQA活動について` · `ソフトウェアアーキテクチャに
  基づいた自動テスト戦略と実装ガイドライン` · `テストアーキテクチャの実践`.
  **⭐ 인증 기반 이관의 QA 가 14제품 633 테스트 케이스로 사내 최대였다는 사실과 이어질 수
  있다**(추정).
- **4순위 — 닫는다.** 닫을 때 `business_model` 에 적을 관찰: **⭐ 이 회사는 후회를 적고
  되먹임 고리까지 짚는다**(릴리스가 적으니 회고가 적고 회고가 적으니 개선이 안 나온다).
  **⭐ 그리고 같은 프로젝트를 두 글이 다른 각도로 다룬다** — 이 사이트가 읽은 회사 중 드물다.
  **⚠️ 최근 글의 결은 AI 쪽으로 크게 기울어 있어 이런 굵은 회고는 2022년 언저리에 몰려 있다**(추정).

**📌 그 밖**
- **큐 1/3**(SmartHR). **⚠️ Personio(EU)는 아직 안 봤다 — 다음 후보 조사의 첫 순위.**
- **⭐⭐ freee 와 SmartHR 의 정반대 순서 대비**는 `freee.json` 의 `open_questions` 에 적어 뒀다 —
  **freee 가 후회로 적은 것이 SmartHR 이 실제로 한 일이다.** **SmartHR 을 판 뒤 비교 문서 재료.**
- **후보 조사 누적 열아홉 곳 중 일곱.**
- **⚠️ 낡은 메모를 나르지 않는다**(314·317·325). **⚠️ 두 방향 규칙**(298·305·318·321).
  ...
  **⭐ 도메인 이름이 좁으면 다음 재료가 안 들어온다**(334→335).
  **⭐ 자기 문서 인용도 우리말은 굵게**(337).
  **⭐ 같은 프로젝트라도 각도가 다르면 다른 기능이다**(339→340, 새 교훈).
- **분포** — **57곳.** US 22 포화 · 한국 아홉 · **일본 여섯** · EU 일곱 · 중국 둘 ·
  동남아 하나 · 인도 셋 · 아프리카 하나 · 라틴아메리카 0(재시도 금지). **비교 문서 32편.**
