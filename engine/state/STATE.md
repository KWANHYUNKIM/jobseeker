...
**37signals — 기능 하나를 채웠다(`leave-the-cloud`).** 빈 도메인 둘. 회사 151개, 큐 2/3.

### 복잡도의 값은 청구서에 안 찍힌다

이번 기능의 축이다. 연 320만 달러가 눈에 보이는 값이라면, ECS·GKE·EKS 를 함께 다루며 쌓인
복잡도와 **오토스케일러·인그레스 컨트롤러 같은 딸린 시스템을 돌보는 시간**은 어디에도 항목으로
안 나온다. 회사가 나온 이유의 첫 줄이 **"값만이 아니라 운영에서도"** 인 것이 그래서다.

### 되돌리기의 순서가 방법이었다

무중단 요구가 없는 Tadalist 부터(6주 미만) → 같은 방식을 재사용한 Writeboard(2주 미만) →
메일 파이프라인이 붙은 Backpack(3주쯤). **첫 앱은 전환 중 중단을 감수했다.**

### 되돌리지 않을 것을 정한 것도 결정이다

CDN·DNS 는 클라우드플레어로, CI 는 GitHub Actions 로 남겼다. 근거가 청구서에 있다 —
**CloudFront 는 66,742달러로 항목 중 가장 작았다.** 되돌려서 얻을 게 없는 자리다(재구성).

### 시설은 직접 운영하지 않는다

애시번·시카고 두 곳, 48U 캐비닛 여덟 대, 서버 약 180대(전부 델), 시설당 40kW. **Deft 와
코로케이션**이고 10년 넘게 고객이던 곳이다. 클라우드에서 나왔다고 전부 자기 손으로 하는 것은
아니라는 자리를 데이터에 남겼다.

### ⚠️ 못 찾은 것을 못 찾았다고 적었다

2023년 1월 글이 **연말에 실제 절감액을 보고하겠다고 약속**했는데 `dev.37signals.com` 목록에
그 후속 글이 없다. 다른 채널에 있을 수 있으나 확인하지 않았다 — `open_questions` 에 그대로 적었다.

### 다음 사이클 — 확장 (남은 두 도메인 중 하나)

`페타바이트를 되찾는다` 가 자료가 두껍다(10PB·50억 파일·이중 쓰기·90일 창에 10일 미만·대조가
복사보다 비쌌다·100Gb 회선이 80Gb 에서 간섭·5,500 GET/s 제한).
