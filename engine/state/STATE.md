...
**Prisma 완주(기능 2개).** 회사 145개, 큐 1/3 — Protomaps·Ente 가 남았다.

### 바이너리를 없앤 다음의 벽은 연결이었다

엣지·서버리스에서 돌게 만들고 나면 **연결 고갈이 실제로 나는 고장**이 된다. 함수는 요청마다
뜨고 지는데 데이터베이스의 동시 연결 수는 우리가 못 늘리는 자원이다. 그래서 15개 넘는 지역에
전역 연결 풀을 두고, 300곳 넘는 지점에서 질의 결과를 캐시한다.

### 신선도를 질의마다 붙이게 한다

어떤 질의가 얼마나 낡아도 되는지는 파는 쪽이 모른다. `cacheStrategy` 로 TTL 과
stale-while-revalidate 를 질의에 붙이게 하고(예: 60초 TTL, 10초 SWR) 무효화는 시간에 맡긴다 —
바뀌자마자 비우는 길은 문서에 없다.

### 자기 데이터베이스에는 안 주는 캐시

질의 캐싱은 **Prisma Postgres 에서 지원하지 않고 당장의 계획에도 없다**고 적는다. 독립 제품
으로서의 Accelerate 도 2026-12-01 에 접는다 — 파는 무게 중심이 계층에서 데이터베이스로 옮겨
가는 신호로 읽힌다(재구성).

### 두 기능이 한 방향이다

러스트를 걷어내 **돌 수 있는 자리를 넓히고**(1), 그 자리에서 실제로 쓰이게 **연결과 거리를
메운다**(2). 기술 결정이 곧 팔 자리의 넓이였다는 것이 이 회사를 한 줄로 꿴다.

### 다음 사이클 — 신규 (대기 맨 위 Protomaps)

`You Might Not Want PMTiles` 로 후보 자격을 얻은 곳이다.
