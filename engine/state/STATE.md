...
**Paytm(IN · 핀테크) `in_progress` — 319 에서 프로파일을 썼다. 도메인 3개 · 기능 0개.**
54번째 회사이고 **인도 두 번째**다. `--gaps` 가 [신규]를 가리켰다(큐 2/3 이라 7순위 후보
조사보다 6순위 신규가 위다).

**연 도메인 셋**(전부 본문을 읽고 열었다, 제목만 보고 연 것 없음)
1. **옮기는 값과 남기는 값을 견준다** — `Achieving Seamless Data Migration`(2023-04-12).
   **다음 사이클의 기능이 여기서 나온다.** 버린 대안(S3 Transfer Acceleration, 비용) ·
   한계 셋(객체 단위 페일오버 · 하드코딩 S3 URL · **재시도 없음**) · 수치(450TB · 50억 ·
   15억 삭제 · 추가 6만 달러)가 다 있다.
2. **거래 기록을 어디까지 맞출 것인가** — `How Paytm Handles Millions of Digital
   Transactions Safely`(2022-09-16). Paxos 단일 리더(Raft·다중 리더를 버림) · 논리 DB 층 ·
   LSM · InnoDB · 강/약/중 일관성. ⚠️ **수치가 하나도 없다** — 기능을 쓰기 전에 한 편 더 읽는다.
3. **앞단에서 끝낸다** — `Optimizing Trillion CDN Hits`(2022-11-01)를 **기술 재료로만** 썼다.
   ⚠️ **그 글은 버린 대안도 대가도 0줄이라 기능으로 쓰지 않기로 했다**(`open_questions` 에
   남겼다). **기능은 아직 안 읽은 Redis 클러스터 글(2023-01-05)에서 나올 가능성이 크다**(추정).

**⚠️ 이 회사에서 먼저 알아야 할 것 둘**
- **블로그가 2023년 4월에서 멈췄다.** 글이 열 편뿐이고 그 뒤가 없다. **재료의 총량이
  정해져 있고 재방문으로 새 것이 나올 가능성이 낮다.**
- **⭐ 사업의 심장이 글에 없다.** 매출이 두 배로 는 것은 **가맹점 대출 중개**(FY26 1분기
  561 크로르, +100%)인데 **심사·한도·상환·DLG 를 다루는 글이 한 편도 없다.** 정산도 없다.
  **공개된 것은 저장·전달·기기 보안 같은 주변부뿐이다.**

**안 읽은 글 넷** — Redis 클러스터(2023-01-05) · 루팅/탈옥 기기 차단(2022-11-16) ·
가짜 SMS 탐지(2022-03-03) · Prometheus 관측(2022-08-30). **뒤의 둘이 한 주제로 묶이면
네 번째 도메인 감이다**(기기와 문자를 얼마나 믿을 것인가, **추정**).

---

**다음 사이클 — Grab·Wayfair 때처럼 이 회사를 완주할 때까지 갈아타지 않는다.**
**도메인 ① 의 기능 하나**를 쓴다(PROMPT.md 4단계, 다섯 층 전부). **결정이 5개 나오는지
먼저 센다** — 이 글은 버린 대안 하나와 한계 셋이 있으니 쪼개면 나올 것으로 본다(추정).

**📌 그 밖**
- **큐 잔량 1/3**(Freshworks 만 남았다). **Paytm 을 다 판 뒤 후보 조사 사이클을 한 번 끼운다**
  — `--gaps` 가 그렇게 안내한다.
- **후보 조사 누적 열여섯 곳 중 넷.** 다음 조사는 **일본**(Sansan·freee·SmartHR·Money
  Forward)이나 **유럽**(Personio). **⚠️ 인도는 소진됐고 Trade Republic 은 Medium 호스팅이다.**
