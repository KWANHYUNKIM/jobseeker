...
**없다 — 문체 보수 사이클이었다.** 페이티엠을 배민 문체로 다시 썼다(1.08 → 0.0). 회사 115개, 큐 2/3. 12곳 남았다.

### 이전 이야기 중 여기만 완주를 포기했다

Atlassian(일부러 느리게) · Etsy(데이터는 두고 계층만) · Paystack(아무도 눈치 못 채게) · 볼(과거를
함께) · 트레인라인(되돌릴 길을 먼저) · 크라켄(SLA 라 무중단) · 산산(아래층부터)에 이어,
페이티엠은 **수백만 객체를 안 옮긴 채로 앞으로 갔다.** 실패한 것만 다시 돌릴 방법이 없어 재실행에
6만 달러가 더 들기 때문이다.

그것이 가능한 이유는 원본이 아직 뭄바이에 남아 있어서다. **대가는 두 곳에 걸친 상태가 남는 것**이고,
남긴 것이 정말 안 쓰이는지 무엇으로 확인했는지는 글에 없다. 50억 개를 옮기고 적은 검증도 한
문장뿐이다.

### 보는 값을 치르는 일곱 번째 답 — 사서 쓰던 것을 버리고 직접 굴린다

VictoriaMetrics · Honeycomb · Datadog · Razorpay · Databricks · 사이버에이전트에 이어,
페이티엠은 유료 도구를 버리고 직접 굴려 **총비용을 그 값의 2% 미만으로** 낮췄다. 다만 버린 이유가
값만은 아니다 — 크기와 애플리케이션마다 다른 요구를 함께 적는다. **사서 쓰는 도구는 남이 정한
모양을 따라야 하는데 필요한 것이 제각각이면 그 모양이 안 맞는다.**

그리고 직접 굴리자 **카디널리티가 가장 큰 난제로 돌아왔다** — 앞의 여섯과 같은 벽이다.

### 도메인이 회사가 파는 것이 아니라 회사가 글로 쓴 것을 따라간다

매출의 무게중심은 금융(가맹점 대출 중개)으로 옮겨 갔는데 기술 글에는 대출 심사도 정산도 한 편이
없다. **그 사실을 사업 모델에 먼저 밝혔다.** 그렇게 잡고 나면 공통점이 보인다 — 옮길지 남길지,
얼마나 맞출지, 어디서 끝낼지, 재는 일을 살지 만들지. **넷 다 무엇을 안 할지 정하는 자리다.**

### 다음 사이클 — 문체 보수 (사다리 3순위)

--gaps 가 정한다. 지금 맨 위는 target(1.06)이지만 그건 이 시점의 예상이지 확정이 아니다.
