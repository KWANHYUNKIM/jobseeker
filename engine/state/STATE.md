...
**Booking.com(EU/NL · 커머스—여행)** `in_progress` · **도메인 2 · 기능 1** —
70곳 · ⚠️ 큐 0/3. **본문은 브라우저(`get_page_text`)로만 온다.**

방금 쓴 기능: **`break-the-backup-loop`(백업을 지키는 카탈로그를 백업 대상 밖으로
꺼낸다)**. 결정 8개가 **전부 `confirmed`** 다.

### ⭐⭐ 이 글의 가장 좋은 대목은 고리를 발견한 장면이 아니다

**`Of course this observation is not new`** — 알고 있었다고 먼저 밝힌다. 그리고
**왜 안 고쳐졌는지**를 적는다: 순환 의존성을 안고 사는 것은 복잡함과 실용의 타협이고,
시스템이 커지는 동안 되돌아볼 시간이 늘 있지는 않으며, **클라우드 이전 같은 큰 계기가
왔을 때** 고친다고. ⚠️ **계기 없이 고치는 방법은 이 글에도 없다** — `hard_problems` 로 세웠다.

### ⭐ 통념을 알고도 어긴 자리

보통 이전은 겹치지 말라고 한다. 그런데 **나란히 세우는 방식이라면** OS(CentOS 8 EOL →
OL9) · Bacula(v12 → v16) · DB(온프렘 → RDS)를 **한 번에** 새로 세우는 편이
`intermediate hell` 을 피해 오히려 안전했다. 대가는 **전체가 더 오래 걸린다** 하나다.

### ⚠️ 실제로 아팠던 두 곳을 실패 그림에 그렸다

**KMS 키는 생성 시점에만 다중 리전으로 정할 수 있다** — 나중에 붙이려다 새 키 생성 →
스냅샷 → 복호화 → 재암호화로 클러스터를 다시 만들었다(`a quite painful lesson`).
그리고 **공유 호스트의 스키마는 자원 사용량을 잴 수가 없다**(CPU·IOPS 가 호스트 단위,
QPS 가 자원으로 선형 변환 안 됨) — 그래서 크게 잡고 2개월 뒤 줄였다.

### 다음 사이클 — 남은 도메인

**품질을 마지막 관문이 아니라 생애주기로 옮긴다** — `Broken Windows of Data`(전문 읽음).
**수치가 좋다**(dbt 모델 약 1,000개 · 검사 통과 95% 초과 · 승인 중앙값 2시간 → 1시간
미만 · MR 물량 7개월에 3배 이상) 그리고 **너무 엄하게 시작해 마찰이 생겼던 것까지 적는다.**
여기까지 채우면 **완주 조건에 닿는다.**

### 아직 못 잡은 것

1. **Booking.com 단독 매출** — 공개 자료가 모회사 기준이다.
2. **Perl 모놀리스 → Java·Kotlin 이전**을 1차 자료로 확인 못 했다(확인하면 `eras` 재료).
3. **온프렘 카탈로그 구조의 시작 연도가 없어** 기능 `history` 를 못 세웠다.
