...
**Wise(EU/UK · 핀테크—국경 간 송금)** `in_progress` · **도메인 3 · 기능 1** —
78곳 · ⚠️ 큐 0/3.

방금 쓴 기능: **`standards-as-dependencies`(표준을 문서가 아니라 의존성으로 배포한다)**.

### ⭐⭐ 축은 하나다 — 지키라고 말하지 않고 올리면 따라오게 한다

서비스가 천 개면 **문서로 알린 표준은 검증할 방법이 없다.** 그래서 표준을 **아티팩트로**
배포한다(`shipped as an artifact`). ⚠️ **복사해 시작하는 템플릿이면 시간이 지나 갈라지고,
갈라진 것은 한꺼번에 못 고친다** — 이 대비는 재구성으로 표시했다.

### ⭐⭐ 한 곳을 바꿔 전부에 닿는 통로를 미리 깔아 뒀다

GitHub Actions 워크플로를 **Gradle 플러그인이 표준화**해서 **버전 하나로 조직 전체가
바뀐다** — SLSA 도입을 **저장소 700개 이상에 `effortless`** 하게 굴렸다.
⚠️ **그런데 같은 통로로 실수도 700개에 간다.** `hard_problems` 로 세웠고,
**단계적 롤아웃과 되돌리는 경로가 글에 없다**고 적었다.

### ⚠️ 이 기능의 대가는 거의 다 재구성이다

⚠️ **스택 소개 글이라 회사가 잃은 것을 안 적는다.** 그래서 이 기능의 `tradeoff` 칸은
**대부분 이 사이트의 재구성**이고 그렇게 표시했다 — 기본값 밖의 서비스, 플러그인이 단일
지점이 되는 것, 샤시가 모든 서비스의 상류가 되는 것.
**다음 기능(관측)은 반대다 — 회사가 자기 실수를 직접 적는다.**

### 다음 사이클 — 관측 쪽이 압도적으로 단단하다

**`관측 스택을 옮기고 나서야 진짜 비용을 만난다`** — Mimir 압축 글(전문 읽음).
증상 넷이 **어느 것도 압축을 안 가리켰고**, **약 25만 블록**(문서 기준 `50K 도 정상이
아니다`), 가장 큰 테넌트에 **두 달치 백로그**, 컴팩터 메타데이터 동기화에만 **약 9시간**.
**왜 늦게 알았는지도 적는다 — 알림이 없었다.** 고친 뒤 **S3 70%↓ · AZ 간 전송 40%↓ ·
질의 지연 90%↓.** 여기까지 채우면 **완주 조건에 닿는다.**
