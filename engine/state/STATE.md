...
**Replicate**(ML 추론 배포) — 회사 프로파일까지 썼다. **도메인 둘 · 기능 0.** 큐 1/3.

### 요금표가 기술 과제를 그대로 드러낸다

이 회사를 세우는 축이다. 비공개 모델은 전용 하드웨어가 **켜져 있는 모든 시간**(준비·대기·
처리)을 청구하는데, **'빠르게 부팅하는 파인튠'만 예외로 처리 시간만 청구한다.** 즉
**부팅을 줄이는 일이 원가 절감이자 그 자체로 상품**이다 — 그래서 기술 글이 콜드 부트와
컴파일 캐시로 모인다.

### 빨라진 대가로 달라진 것을 메운다

두 번째 도메인의 문제는 '빠르게 만들 수 있는가'가 아니라 **'빨라진 대가로 달라진 것을
어떻게 메우는가'** 다. 회사가 **"양자화는 출력을 약간 바꾼다"** 고 인정하고, 보상책으로
**`go_fast` 일 때 `lora_scale` 을 자동으로 1.5배 올린다**고 밝힌다. **왜 1.5배인지는
적지 않는다.**

### 초 단위로 파니 로딩 시간이 곧 돈이다

H100 초당 $0.001525, 8×H100 시간당 $43.92. 이 단가 위에서 **첫 실행의 모델 로딩 2.5초**
같은 숫자가 무게를 갖는다.

### 밝히지 않은 것을 그대로 적었다

소개 페이지에 **창업자도 창업 연도도 본사 위치도 없다.** 그리고 **"지금 채용은 Cloudflare 의
채용 페이지를 통해 이뤄진다"** 고만 적혀 있고 **그 관계의 성격을 밝히지 않는다** — 인수인지
제휴인지 이 자료로는 알 수 없어 **그대로만 적었다.**

### 심은 대비

**모달**(서버리스 컨테이너의 콜드 스타트) ↔ **Replicate**(모델 컴파일 캐시와 라우팅) — 같은
자리 다른 층. **딥엘**(FP8 로 갈지를 검증 perplexity 로 **판정**했다) ↔ **Replicate**(양자화로
바뀐 출력을 **입력 조정으로 보상**한다).

### 다음 사이클

빈 도메인 둘 중 하나. **콜드 부트 쪽이 수치가 더 두껍다**(120→60초 · 400→150초 · 추론 자체
30% 이상). 양자화 쪽은 대가와 보상책이 선명하지만 **개선 전 수치가 없다.**
