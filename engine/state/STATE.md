...
**없다 — 문체 보수 사이클이었다.** 메르카도리브레를 배민 문체로 다시 썼다(1.48 → 0.0). 회사 115개, 큐 2/3. 18곳 남았다.

### 사내 플랫폼을 제품처럼 다루는 셋

- Monzo 는 서비스 3,000개를 같은 모양으로 유지하려고 플랫폼 팀이 도구를 API 가진 서비스로 감싸 판다.
- 누방크는 중앙 게이트웨이를 둬서 엔지니어가 나라별 프로토콜을 몰라도 되게 한다.
- 메르카도리브레는 목표를 **지표가 아니라 장면으로 적는다** — 세 번 눌러 앱이 뜨고, 커피 한 잔
  내리는 동안 받아서 고치고 시험한다.

대가도 분명하다. 스택과 언어와 프레임워크를 플랫폼이 정하니 **자유를 표준으로 바꾼 거래**다.
다만 배포 전략에서는 자유를 돌려준다 — 하나로 다 되는 방식은 없다며 위험도로 고르게 한다.

### 단일 데이터베이스가 전부를 멈춘다 — 답이 셋이 됐다

- Monzo 는 한 클러스터에 모아 둔 값을 장애로 치르고 **쪼갰다**.
- SmartHR 은 쪼개지 않고 **앞에 커넥션 풀러를 세워 버티게 했다**.
- 메르카도리브레는 **쪼개되 안쪽은 쪼개지 않는다** — 재고 도메인이 극히 낮은 지연과 강한 일관성을
  요구해 의도적 모놀리스를 핵심에 두고 그 주위를 셀로 감쌌다.

회사의 문장이 판단을 요약한다 — 핵심은 모놀리스냐 마이크로서비스냐가 아니라 사업이 필요로 하는
구조를 고르는 것이었다. 그리고 **완벽하지 않다고 먼저 말한다**(브로드캐스트 부담을 내고 안전한
이전 경로를 샀다).

### 남의 코드를 안 건드리는 것이 제약이자 방법이다

Etsy 는 16년 된 샤딩에서 데이터를 안 옮기고 접근 계층만 바꿨고, 메르카도리브레는 **3만 개 서비스를
고쳐야 하는가**를 모든 선택의 첫 질문으로 삼는다. FaaS 를 접은 결정적 이유가 성능이 아니라 그것이고,
셀에 라우팅 키를 안 넣은 이유도 같다 — **한쪽은 컴퓨팅 모델에서, 한쪽은 데이터 라우팅에서 같은
벽을 만났다.**

### 도메인을 사업부가 아니라 3만 개가 이미 돌고 있다는 제약으로 잘랐다

마켓플레이스·핀테크·물류로 가르면 그 공통 관문이 안 보인다.

### 다음 사이클 — 문체 보수 (사다리 3순위)

--gaps 가 정한다. 지금 맨 위는 sansan(1.48)이지만 그건 이 시점의 예상이지 확정이 아니다.
