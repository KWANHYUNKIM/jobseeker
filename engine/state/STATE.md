...
**Fireworks AI — 프로파일만 썼다(도메인 하나, 기능 0).** 회사 156개, 큐 1/3.

### 모델은 남이 만든다

그러니 파는 것은 **같은 모델을 얼마나 빠르고 싸게 돌리느냐**다. 값도 토큰 단위로 매긴다(GLM
5.2 입력 100만 토큰당 1.4달러·출력 4.4달러). 그러면 사업이 **커널 한 줄의 문제로 내려온다** —
어떤 루프를 바깥에 둘 것인가, 부분 결과를 언제 흩을 것인가.

### 병목이 다르면 답도 다르다

Q-outer 와 KV-outer 를 고르는 문제를 취향이 아니라 **병목의 종류**로 푼다 — 앞엣것은 L2
대역폭에, 뒤엣것은 HBM 대역폭에 묶인다. 그래서 교차점을 수식으로 낸다: **`nsb / N < 2.85`**.

### 커널 안에서 흩어 쓰지 않는다

오픈소스 MSA 는 어텐션 에필로그에서 부분 결과를 (질의, 순위) 자리에 흩어 쓰는데, 이쪽은 **타일
순서 그대로 원본으로 저장하고 흩기와 정규화를 결합 커널로 미룬다** — 커널 안의 흩어 쓰기가
명령 지연에 민감해 파이프라인을 통째로 멈추기 때문이다.

### 커널 배수와 모듈 배수를 나눠 적는다

기준선(FlashInfer) 대비 **1.9~2.4배**, 오픈소스 MSA 대비 **약 1.6배**, 그런데 **모듈 전체로는
1.18~1.43배**. 커널만 빨라져도 전체는 그만큼 안 는다는 것을 스스로 나눠 보인다.

### 도메인을 하나만 세웠다

훈련·배포 형태의 내부를 다룬 글을 못 찾았다. **자료 없이 이름만 세우면 빈 도메인이 영영
남으므로** 하나만 뒀다 — 먹스에서 한 판단을 이어 쓴다.

### 다음 사이클 — 확장 (그 하나뿐인 도메인)

`남의 모델을 남보다 빠르게 돌린다`. 커널 글 하나에 물린 대안과 수치가 다 있다.
