...
**Ably(EU/UK · SaaS—실시간 인프라)** `in_progress` · **도메인 2 · 기능 1** —
74곳 · ⚠️ 큐 0/3.

방금 쓴 기능: **`assume-simultaneous-failure`(장애를 막는 장치도 장애 대상이라고 보고
배치한다)**. 결정 7개가 **전부 `confirmed`**.

### ⭐⭐ 흔한 답 하나를 그 자리에서 버린다

리전 간 로드밸런서 — `the load balancer itself exists in some region and could become
unavailable`. **장애를 막으려고 둔 장치가 그 자신도 장애의 대상**이라는 문장이 이 도메인
전체를 관통한다. 그리고 Raft/Paxos 도 같은 이유로 버린다(리전을 넘으면 지연 때문에
효율이 무너진다) — **알고리즘이 나빠서가 아니라 전제하는 네트워크가 다르다.**

### ⭐⭐ 가장 아픈 인정 — 고치려는 행동이 병을 키운다

**내결함성 기법 자체가 자원을 쓴다.** 그런데 장애의 원인이 자원 고갈이면 **대응하려는
바로 그때 CPU 와 메모리가 없다.** 게다가 단순한 기법일수록 **O(N²) 이상**으로 튄다.
`hard_problems` 로 세웠고, **부분 열화**(`working some of the time`)도 함께 넣었다 —
죽었다고 보면 멀쩡한 용량을 버리고 살았다고 보면 실패가 새어 나온다.

### ⭐ 약속의 근거가 확률이다

감지·합의 시간 + 역할 재배치 시간 + AZ 실패율로 **복합 장애 확률을 모델링**한다.
**감으로 약속하지 않는다.** ⚠️ 대신 **모델의 가정에 기댄다**는 대가를 재구성으로 달았다.

### 다음 사이클 — 남은 도메인

**순서를 지키려면 속도를 내준다** — `chat-architecture-reliable-message-ordering`(요약 확보).
각 방법의 대가가 다 붙어 있다(전역 시퀀스는 지연 · 타임스탬프는 시계 표류 · 섞으면
고부하에서 복잡한 조율 · 버퍼는 동적 확장 필요).
여기까지 채우면 **완주 조건에 닿는다.**

### ⚠️ 아직 못 잡은 것

1. **`eight 9s` 와 `99.999%` 가 같은 것인지 모른다** — 두 글의 표현이 다르다.
   **확인 전에는 둘을 나란히 쓰지 않는다**(metrics 에 그렇게 적어 뒀다).
2. **매출·고객 수 비공개.** **연표는 못 세울 가능성이 높다**(글이 회고형이 아니다).
