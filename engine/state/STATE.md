...
**Doximity — 40번째 회사, 의료 첫 곳.** 사이클 211 에서 **프로파일 + `ui_map`** 까지 썼다(기능 0개).

> **이 회사의 자리 — 사용자가 개인도 가맹점도 아니라 '면허' 다.**
> *"전 진료과에 걸쳐 미국 의사의 85% 이상"* 이 회원이고, 회사가 세는 것은 방문자가 아니라
> **분기 순 활성 provider**(의사·NP·CRNA·PA·약사·의대생)이며, **세는 방식도 '봤나' 가 아니라 '했나' 다**
> — Dialer 통화·팩스·Doximity GPT 프롬프트·PeerCheck 검토·Amion 온콜·Scribe 노트.
> **Yelp 와 구조가 닮았지만 반대편이다** — Yelp 는 840만 중 51만이 내는 **넓고 얕은** 쪽,
> Doximity 는 **TTM $500K 초과 고객 수**를 세는 **좁고 깊은** 쪽이다.

> **📌 IR 접근법이 하나 뚫렸다 — Q4 Inc 계열 IR 사이트는 브라우저 `get_page_text` 로 전문이 온다.**
> `press.doximity.com` 은 본문이 비었지만
> **`investors.doximity.com/news/news-details/<연도>/<제목>/default.aspx` 는 표까지 통째로** 읽혔다.
> **Yelp 에서 막혔던 것과 같은 종류라, 그쪽도 이 방법으로 다시 볼 수 있다**(매출 부문별 구성이 아직 비어 있다).

> **다음 사이클** — 도메인 **`배치에서 실시간에 가까워지기`** 의 기능 하나.
> 재료는 **본문을 이미 확인한** `from-batch-snapshots-to-near-real-time-data`
> (버린 대안 넷: Debezium 스냅샷 · 토픽 1:1 · 즉시 병합 · binlog 만 보기 / 95%가 8분 · 99%가 13분).
> **⚠️ 규칙대로 두 번째 글을 함께 읽는다** — `a-smarter-way-to-deploy-data-pipelines` 나
> `the-art-of-data-management` · `stars-and-dimensions` 중에서 고른다.

> **⚠️ 이 블로그는 성격이 다르다** — 약 92편인데 **Ruby/Rails 공예 글과 커리어·컨퍼런스 글이 많고**
> 대규모 분산 시스템 글은 적다. **묶음 셋은 제목으로 센 것이고 본문은 네 편만 읽었다.**
> `임상 현장의 AI` 는 읽은 한 편에 **버린 대안이 없어**, 기능을 쓰려면
> `how-we-re-thinking-about-agentic-systems` 와 `beyond-accuracy` 를 먼저 읽어야 한다.

> **📌 Doximity 를 완주하면 큐가 1/3(Moniepoint 뿐)이 되므로 후보 조사가 필요하다.**
> **여전히 라틴아메리카 0곳 · 물류 전문 0곳**이고, 게임은 Roblox 하나다.
> 비교 문서 두 축('재 보고 고른다' 3곳 · '되돌릴 수 있는 것을 먼저 한다' 4곳)은 재료가 찬 채로 대기 중이다.

**큐 1/3** — Moniepoint(NG · 핀테크).

마지막 사이클: 211 (2026-08-27) — Doximity 회사 프로파일 + `ui_map`.
