...
**없다 — Airbyte 를 완주시켰다(도메인 1 · 기능 1 · `status: done`).** 회사 162개, **큐 0/3.**

### 값비싼 쪽만 도려냈다

이번 기능(`stop-serializing-four-times`)에서 가장 좋은 대목이다. 오케스트레이터를 통째로 없애지
않았다 — **데이터를 만지는 부분과 제어를 다루는 부분을 갈라**, 데이터는 소스에서 목적지로 직결하고
제어(로그·상태·메타데이터)는 **가벼운 Bookkeeper** 에 남겼다. **데이터 채널은 Bookkeeper 를
거치지 않는다.**

### 같은 바이트를 네 번 만지고 있었다

3판의 낭비가 분명하다 — 레코드마다 **JSON 직렬화 → 오케스트레이터 파싱 → 재직렬화 → 목적지
파싱.** 4판은 Protobuf-over-유닉스 도메인 소켓의 병렬 채널로 바꿨다.

### 수명이 다른 파일에 다른 저울

**`COPY INTO` 전에 몇 초만 사는 스테이징 파일에서는 압축률보다 처리량이 이긴다** — 그래서
CSV+gzip 을 Avro+Snappy 로 바꿨다. 오래 보관하는 파일이었다면 반대 판단이 나왔을 것이다(재구성).

### 안 적힌 것을 여섯 가지 적었다

동시 쓰기 성능 · 약 250MB 넘는 배치 · 순환 참조와 중첩 스키마 변경 · 이관 실패 롤백 · 타임
트래블과의 상호작용 · **소켓을 파이프나 공유 메모리 대신 고른 이유.** 4.5배라는 수치 뒤에 남은
물음을 `failure` 도해에 그대로 세웠다.

### 심어 둔 대비 둘

**Electric**(전선 형식 그대로 저장해 `sendfile`·`splice` 로 커널에서 내보낸다) ↔ **Airbyte**
(Protobuf 소켓 직결로 네 번 만지던 직렬화를 없앤다) — **복사를 없애는 것이 곧 성능.**
**SourceHut**(깃 작업을 웹 워커에 맡겼다 걷어냈다) ↔ **Airbyte**(오케스트레이터를 데이터
경로에서 뺐다) — 둘 다 **모으려다 만든 병목**을 되돌린 기록이다.

### 다음 사이클 — ⚠️ 후보 조사 (큐가 0/3 이다)

`blocked` — Sourcegraph(403 두 번) · incident.io(404×3) · Guardian · Flexport · Cal.com.
**다섯 사이클 연속 이 목록에서 하나씩 되살렸다**(Fireworks · Ghostty · Framework · Deepgram ·
그리고 Airbyte 는 '목록만 보고 접을 뻔한' 반대 사례).
