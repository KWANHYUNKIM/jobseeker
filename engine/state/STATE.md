...
**Ramp** (US · 핀테크) — 도메인 3개 · **기능 2개**. 남은 것은 코딩 에이전트 하나이고, **그것을 쓰면 비교 문서의 네 번째 회사가 채워진다.**

### 이번 사이클 — Turbo `규제가 요구하는 재현성을 설정 파일로 사는데, 그 약속은 데이터에 걸려 있다`

건진 것 넷:

- ⚠️ **안 만드는 길을 문장으로 적고 나서 버린다** — `We could solve all this without building an abstraction...we could create standards and templates to follow, enforcing these standards at review time`. **버린 이유 셋이 전부 사람 문제다**(인수인계 유지보수 · 배포에 전문성이 필요해 생기는 속도 제약 · 반복의 마찰). **표준은 지켜야 지켜지고 틀은 안 지킬 수가 없다**(재구성).
- ⚠️ **약속에 조건이 붙어 있다** — `as long as your data hasn't changed`. **재현성이 코드가 아니라 상류 데이터에 걸려 있는데, 규제가 묻는 것은 대개 몇 년 전 판단이다.** 회사가 조건을 스스로 적는다는 점은 정직하지만 **그 구멍을 메우는 방법은 글에 없다.**
- **병목이 학습이 아니라 배포였다** — 넉 달이 걸린 것은 모델을 만드는 일이 아니라 **내보내는 일**이었고, 그래서 **적재부터 모델 레지스트리까지 끝까지 덮어야** 값이 났다(재구성).
- ⚠️ **만들기 쉬워질수록 만든 사람이 한계를 모른다** — 모델을 학습시켜 본 적 없는 개발자도 5분이면 배포한다. **그런데 설명 가능성은 사람이 답하는 일이고 틀이 그 답까지 만들어 주지는 않는다**(재구성).

**두 도메인이 같은 판단의 변주다** — 승인 로직은 그래프로, ML 파이프라인은 YAML 로. **두 번 다 `직접 짜게 두면 끝나지 않는다` 에서 출발한다.**

### 다음 사이클 — Inspect, 그러면 완주와 비교 문서가 함께 온다

`--gaps` 가 **코딩 에이전트를 사서 쓰지 않고 직접 만든다** 를 부른다. **자료가 이미 다 있다** — 기성품을 안 쓴 이유(`it only has to work on your code`) · Modal 샌드박스(30분마다 굽는 이미지·파일시스템 스냅샷) · Cloudflare Durable Objects 로 세션마다 SQLite · 슬랙·웹(VS Code 내장)·크롬 확장 · **병합 PR 의 약 30%** · 대가 넷(스냅샷 30분 낡음 · 동기화 전 편집 차단 · 후속 프롬프트 큐잉 · 멀티플레이어에 GitHub 인증).

⚠️ **그 기능을 쓰면 곧바로 8순위(비교 문서)가 열린다.**

| 회사 | 어디까지 맡기나 | **무엇이 그 선을 지키나** |
|---|---|---|
| **Sentry** | AI 가 PR 까지 열고 병합은 사람이 | 조직의 신뢰 |
| **ClickHouse** | 분석의 70%, 남긴 30% | **세 겹 중 한 겹만 기계가 강제**(접근 범위) |
| **Duolingo** | 테스트 PR 하루 20개 · 250개 전부 사람 리뷰 | 사람 리뷰 — **그래서 병목이 됐다** |
| **Ramp** | **병합 PR 의 약 30%** | ⏳ 다음 사이클에 확인 |
