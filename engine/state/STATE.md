...
**Proton**(스위스 · 프라이버시 제품군) — 회사 프로파일까지 썼다. **도메인 둘 · 기능 0.** 큐 1/3.

### 광고를 안 받으면 무엇을 대신 떠안는가

이 회사를 세우는 축이다. 비영리 재단이 최대 주주이고 광고가 없으니 **구독으로만** 버틴다.
그러면 두 가지가 따라온다 — 믿음이 곧 상품이라 **암호 라이브러리까지 만들어 공개해야**
하고, 남의 클라우드 대신 **자기 데이터센터**를 져야 한다. 도메인 둘이 각각 그것이다.

### 우리가 저지른 사고와 남이 낸 사고

**Fastmail** 이 자기들이 지운 데이터를 백업으로 되살린 이야기라면, Proton 은 **데이터센터
운영사가 한밤중에 예고 없이 이중화된 압축기 양쪽 필터를 동시에 갈아** 벌어진 일이다.
자기 잘못이 아닌데도 사후 기록을 쓰고, 자기 쪽 한계까지 같이 적는다.

### 보호가 곧 복구 지연이 된다

두 자리에서 그렇다. 네트워크 카드는 105°C 에서 스스로 꺼져 장비를 지키는데 **차가운
상태에서 완전 재기동해야만** 살아난다. 그리고 **자기들의 보안 태세 때문에 대역외 컨트롤러
접근이 막혀** 사람을 더 깨워야 했다. 보안과 복구 속도가 정면으로 부딪힌 자리다.

### 공개와 감사는 다른 일인데 한 문장에 섞여 있다

오픈소스 페이지가 **"전 세계 수천 명의 전문가가 독립적으로 감사했다"** 고 쓴다. 저장소를
공개한 것과 제3자가 감사한 것은 다른데 한 문장에 있고, **기관 이름도 날짜도 방법론도 그
페이지에는 없다.** 도메인의 `limits` 에 그대로 적었다.

### 블로그에 엔지니어링 카테고리가 없다

있는 것은 업데이트·뉴스·가이드·의견이다. 그래서 자기 시스템을 다룬 글이 **사후 기록
한 편**이었다 — 후보 조사에서 이 회사를 접었던 이유가 그것이고, 그 한 편이 판정을
뒤집은 것도 그래서다.

### 다음 사이클

빈 도메인 둘 중 하나. **사후 기록 쪽이 압도적으로 두껍다**(타임스탬프 다섯, 온도 수치,
물린 선택 셋, 스스로 인정한 한계).
