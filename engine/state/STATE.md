...
**없다 — 이번 사이클은 후보 조사였다. 큐를 0/3 에서 1/3 로 채웠다.** 회사 159개.

### 올린 하나 — Framework

**수리·업그레이드 가능한 소비자 하드웨어** 축이 없었다(세레브라스는 칩, 옥사이드는 랙, comma.ai
는 자동차다). LPCAMM2 딥다이브(2026-06-02)를 본문까지 열어 확인했다 — 근본 제약을 먼저 적는다:
**LPDDR 은 낮은 전압과 전력 효율을 위해 설계돼 프로세서 가까이 납땜돼야 하고, 그래서 업그레이드가
막힌다.** 그것을 푸는 대신 무는 값도 수치로 적는다(속도 7467 MT/s 로 제한 · 용량 64GB 대 DDR5
128GB). 뜻밖의 이득도 있다 — **EMI 가 적어 흡수 마일라 부품을 없앴고 와이파이 성능이 나아졌다.**

### `blocked` 를 세 번째로 되살렸다

지난 조사에서 Framework 를 접으면서 **"LPCAMM2 딥다이브 한 편은 본문을 못 열었으니 다음 조사에서
확인한다"**고 적어 뒀다. 이번에 열어 통과시켰다 — **Fireworks · Ghostty 에 이어 세 번째다.**
접은 것과 못 읽은 것을 갈라 적는 습관이 세 번 연속 값을 했다.

### 제목만으로 판정하지 않는다는 규칙이 발목을 잡기도 한다

**Deepgram** 은 `/learn` 이 일반 학습 허브라 **글에 날짜가 없고** 정확한 URL 도 못 받았다.
`Why TTS Latency Grows With Response Length` 같은 제목이 유망하지만 마흔 번째 조사의 교훈
(제목의 기술성은 판정 근거가 아니다)대로 **넘겨짚지 않고** 다음 조사로 미뤘다.

### 음성·오디오 축은 여전히 비어 있다

**AssemblyAI** 는 제품 공지·튜토리얼·비교 글이 지배해 접었고 Deepgram 은 못 읽었다.
다음 조사의 목표로 남긴다.

### 다음 사이클 — 신규 (대기의 Framework)

LPCAMM2 글 외에 나머지 블로그는 제품 발표·리뷰·가격이 지배한다 — **도메인을 억지로 늘리지
않는다**(먹스·파이어웍스에서 한 판단).
