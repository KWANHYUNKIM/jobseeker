...
**아무것도 안 판다 — Wayfair 를 닫았다(44번째, 기능 5개 · 도메인 3개). 큐가 0 이다.**

**245 에서 Wilma 본문을 읽었고 예상이 빗나갔다.** "이 회사는 버린 대안을 거의 안 적으니 결정
5개가 안 나올 것" 이라 예상했는데 **여섯이 나왔다.** 이 글은 **버린 설계를 이유 셋과 함께 적는다** —
초기에 **프롬프트 하나로 전부 처리해 정확도 70%** 였고, 버린 이유가 *"모델 환각으로, 모델이 댓글
작성자에게 해당되지 않는 개입 사유 코드를 잘못 예측했다"* · *"단일 통합 프롬프트라 입력 토큰 비용이
더 들었다"* · *"format string 템플릿은 정적인 방법만 제공해 비효율적이었다"* 다. 고쳐서 **96%**.

**수치의 소속을 전부 확인했다**(요약만 보고 인용하지 않기를 잘했다) — **STP 티켓의 약 65% 자동
종결** · **사람 기준선 목표 정밀도 80% / 재현율 90%** · **달성 95% · 97% · 사유 정확도 99.8% ·
적시 플래깅 99.7%** · **해결 시간 29% 단축** · **공급업체 만족도 3.1 → 3.8(5점)** · **재오픈율
2.8% 감소** · **초기 70% → 최종 96%**.

**🔴 이 회사에서 가장 큰 발견 — 사람을 뺄 수 있느냐가 '잴 자가 있느냐' 로 갈린다.**
다섯 기능 중 **Wilma 만 사람 없이 종결하고(65%), Wilma 만 사람의 성적을 숫자로 갖고 있다**
(*"원래 목표였던 정밀도 80% · 재현율 90% 의 사람 기준선"*). 나머지 넷에는 넘었다고 말할 대상이
없다 — 창고 손상은 **사람 라벨이 16% 에서 갈렸고**, 시각 탐색은 **LLM 이 채점했으며**, 해결책
추천은 *"다른 해결책을 골랐다면 결과가 어땠을지 우리는 모른다"* 이고, 치수 검증의 5,000건 정답셋은
**사람의 성적이 아니라 사물의 값**이다. **이것을 `business_model` 에 이 사이트의 정리로 적었다.**

**같은 도메인 안에서 두 기능이 정반대다** — 해결책 추천은 아무것도 자동 처리하지 않고 좁히기만
하는데 Wilma 는 65% 를 닫는다. `connections`(대비)로 이었고 **두 글이 서로를 언급하지 않는다**는
것을 `contract` 에 밝혔다.

**🚨 다음 사이클 — 후보 조사다. `--gaps` 가 그렇게 가리킨다.**
**목표 3곳.** `확인해 둔 후보` 의 **Cookpad**(`techlife.cookpad.com`, 과거 글 목록을 더 열어 볼
여지) · **CyberAgent**(블로그는 활발한데 연 글이 얇았다 — **ABEMA 태그**를 아직 안 봤다)부터 본다.

**아직 안 열어 본 후보** — SmartNews · PayPay · Sea/Shopee · GoTo/Gojek · Tokopedia · Traveloka ·
Robinhood · Plaid · Ramp · Adevinta · Revolut · Skyscanner.
**📌 238 의 발견(유효)** — 개발자 대상 제품 회사(관측·엣지·워크플로·DB)의 블로그는 대개 영업
채널이다. **자체 서비스를 굴리는 회사부터 본다.** 239 에서 이 기준으로 첫 시도에 Wayfair 가 나왔다.
**인프라(Vercel·Neon·Elastic·MongoDB·Confluent)는 뒤로.**
**📌 237→238 의 교훈** — "사다리에 남는 게 없다" 고 성급히 적었다가 정정했다. **문을 열세 개
두드린 것과 후보가 없는 것은 다르다. 막힌 게 아니라 아직 못 찾은 것이다.**

**비어 있는 자리** — 라틴아메리카 0(접었다 · 재시도 금지) · **게임 1(Roblox — 후보가 계속 안 나온다)** ·
아프리카 1(Moniepoint) · 인도 1(Zerodha) · 여행 1(trivago). **동남아는 Grab 이 있다.**

**`hold_reason` 두 곳** — LinkedIn `랭킹 모델을 얼마나 빨리 다시 만들 수 있는가` ·
kakao `동기화·다중 기기`. **새 글 URL 이 나오면 다시 연다.**

**비교 문서 27편.** 이번 회사에서 재료가 는 축 — **`무엇을 잃기로 정했는가`** 에 Wayfair
`supplier-intervention`(자동 종결 65% 의 대가를 **재오픈율**로 센다)이 붙을 수 있고,
**`기계가 그렇다는데`** 에 **사람 기준선을 자로 쓴 유일한 사례**가 붙는다(지금 그 문서의
*"재는 사람 자신을 의심한 곳은 한 곳뿐"* 절과 맞물린다).
...
⑤ **태그 목록의 총 건수를 새 재료의 양으로 읽지 않는다**(244 — Wayfair 태그가 거의 겹쳤다).
