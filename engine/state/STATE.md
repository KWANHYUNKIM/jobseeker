...
**없다 — Modal 을 완주했다.** 도메인 **2** · 기능 **2** · `status: done`. 회사 114개. ⚠️⚠️ **큐 0/3 — 다음 사이클은 반드시 후보 조사다.**

### 이번 사이클 — `데이터는 지역으로 흩고 제어는 중앙에 남긴다`

⭐ **지연의 출처가 컨테이너 위치가 아니라 데이터 경로였다** — **`모든 함수의 입력과 출력이 늘 us-east 의 I/O 플레인을 거쳐, 호출자와 컨테이너의 지역에 따라 함수 호출마다 수백 밀리초까지 붙을 수 있었다.`** ⚠️ **컨테이너를 그 지역에서 돌려도 데이터가 미국을 다녀왔다**(재구성). **`약 200밀리초가 임베딩 모델을 돌리는 비용을 압도했다.`** 결과 **p50 에서 80밀리초 감소.**

건진 것 넷:

- ⭐ **평면을 갈랐다** — **I/O 는 흩고 컨트롤 플레인은 중앙에 남긴다.** 지역 서버가 **실행 중 입력 수·백로그 크기·평균 실행 시간**만 올리고 중앙이 자동 확장을 정한다. ⭐ **배치는 흩고 확장 판단은 모은다**(재구성) — **컨테이너를 얼마나 늘릴지는 전역을 봐야 안다.**
- ⭐ **지역 선택을 사용자에게 맡겼다**(`routing_region`) — ⚠️ **`지리적 제한과 가용 용량 사이에 유의할 트레이드오프가 있고 비용에도 영향이 있다.`** **좁게 묶을수록 빠르지만 쓸 수 있는 기계가 준다** — **그 균형은 워크로드마다 다르다**(재구성).
- ⭐ **파이썬을 버린 이유가 구체적이다** — **`비동기 파이썬은 협조적 단일 스레드`**, **`네트워크 페이로드 전송 같은 CPU 집약 작업에서 지연을 겪는다.`** ⚠️ **그런데 이 회사의 차별점이 `순수 파이썬 통합`이다** — **사용자 언어와 인프라 언어를 갈랐다**(Perplexity 와 같은 모양이다).
- ⭐ **좋아 보이는 것을 재 보고 거절한 기록** — Valkey 가 **`I/O 멀티스레딩으로 단일 실행 스레드의 활용률을 높인다`**는데, **`이후 버전에서 관찰한 CPU 급등 문제 때문에 Redis 7.1 에 머물렀다.`**

### ⭐⭐ Modal 완주 관찰 — 같은 수법, 다른 답

**두 기능이 똑같이 `임계 경로에서 뭔가를 걷어낸다`** — 저기는 **데이터 저장소**, 여기는 **자동 확장 통계와 대시보드 이벤트.**

⚠️ **그런데 중앙에 대한 답이 갈린다** — **스케줄링은 `모든 워커가 자기 자신의 진실 원천`으로 갔고, I/O 는 `제어는 중앙에 남긴다`로 갔다.** ⭐ **같은 회사가 한쪽은 분산하고 한쪽은 중앙을 지켰다** — **배치는 지역 정보로 충분하고 확장은 전역을 봐야 하기 때문이다**(재구성). **비교 문서 재료로 좋다.**

⭐ **그리고 원가 구조가 두 도메인을 다 설명한다** — **유휴에 값을 안 받고 이그레스도 안 받으니, `시작하는 시간`과 `오가는 시간`이 회사의 원가다.** **두 도메인이 정확히 그 둘이다.**

### ⏳ 보강 거리 — 세 번째 축이 남아 있다

**`시작 시간을 미리 벌어 둔다`**(CRIU 메모리 스냅샷). **`import torch` 예제 5초 → p50 1.05초·p0 0.69초**, 초기화가 무거운 함수는 **3~10배**, 워커 메모리 분산 파일 캐시는 **3~5배.** ⚠️ **도메인을 억지로 늘리지 않고 보강 사이클 대상으로 남겼다** — PROMPT 가 **`done 인 회사도 보강 사이클의 대상이 된다`**고 못박아 뒀다.

그 밖에 — gVisor 격리 설계(3자 자료로만 확인) · `rtnl` 락을 무엇으로 바꿔 풀었는지.

### 다음 사이클 — ⚠️⚠️ 후보 조사 (큐 0/3, 바닥)

**먼저 `name_en` 목록 출력으로 시작한다.** ⚠️ **`큰 이름인데 아직 없는 회사` 각도로 던질 이름이 줄고 있다** — Snowflake·Anduril·Railway·Linear·Robinhood·Reddit·Supabase·Klarna 를 접었다. ⭐ **새 각도가 필요하다** — 이 세션에서 통했던 것들: **오픈소스를 내는 인프라 회사**(WarpStream·VictoriaMetrics·Modal), **논문을 내는 회사**(Perplexity), **포스트모템을 쓰는 회사**(Anthropic).
