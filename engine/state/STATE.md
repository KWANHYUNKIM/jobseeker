...
**DoorDash** (US · 커머스) — 도메인 3개 · **기능 1개**(`extract-function-not-code`) · eras 3시기. 남은 도메인 둘.

### 이번 사이클 — 마이크로서비스 `코드를 옮기는 것과 기능을 옮기는 것은 다른 일이다`

**기능의 이름을 회사가 준 문장에서 그대로 가져왔다** — 가장 어려웠던 일로 꼽는 것이 `extracting the functionality, **as opposed to just extracting the code**` 다.

건진 것 넷:

- ⚠️ **코드를 옮기는 것은 파일 이동이고, 기능을 옮기는 것은 새로 생긴 서비스들 사이의 상호작용 면을 다시 정의하는 일이다.** 그리고 그것을 **스택을 바꾸면서, 새 기능도 만들면서** 해야 했다 — 추출 기간 내내 동결할 수는 없었기 때문이다.
- ⚠️ **모놀리스에서 공짜였던 것이 전부 비용이 된다** — 내부 호출에 **네트워크 지연 없음** · API 하위 호환 걱정 없음 · 재시도·서킷 브레이킹·부하 차단 불필요 · **여러 도메인을 한 번의 질의로 집계.** **넷 다 쪼개는 순간 값이 붙는다.**
- ⚠️ **안티패턴을 감수한 것을 그대로 적는다** — 하류 API 가 없으면 **데이터베이스를 직접 봤고**, 진행은 됐지만 **접근 패턴이 흐려져 테이블 이전이 더 어려워졌다.** 임시 API 도 부채였다.
- ⚠️ **회고가 결정의 결과는 적는데 이유는 안 적는 자리가 있다** — `REST 대 gRPC` 를 검토했다면서 **무엇을 견줘 왜 gRPC 인지가 없다.**

**난제 하나를 새로 세웠다 — `고치는 일과 원인을 묻는 일은 다르다`.** 2018년에 코드 프리즈까지 하고도 **`왜 아키텍처가 그렇게 취약한지에 대한 분석은 포함되지 않았다`.** **증상이 사라지면 물음도 사라지고**, 이 회사에서는 **사람이 바뀌어야 물음이 바뀌었다**(2019년 새 부사장).

### 다음 사이클 — Flux 아니면 배차

**Flux 는 자료가 익었다**(앞 사이클에 전문을 읽었다) — 노트북의 한계 셋 · **버린 대안이 명시적**(호스팅 에이전트는 `민감한 코드를 3자에 보내거나 3자가 내부로 들어오는 길을 열거나` 를 강요한다) · Firecracker microVM(p95 5초 미만) · Agent Gateway · 플레이북 · 호출 표면.

⚠️ **배차·주문 이벤트 도메인은 tech 가 0 이다** — **이 회사의 본체인데** 아직 자료가 없다. **그 차례에는 기능보다 자료를 먼저 찾는다**(네 번 통한 규칙). **브라우저로 열어야 한다** — 이 블로그는 WebFetch 가 403 이다.
