...
**ClickHouse** (US · SaaS) — 도메인 3개 · **기능 2개**. 남은 것은 AI 하나이고, **자료는 이미 다 모여 있다.**

### 이번 사이클 — 자기 창고 `하나로 굴리던 적재를 아홉으로 쪼갠다`

**두 편이 시간축을 이룬다** — 초기 구조(1편)와 **1년 뒤 회고**(2편).

건진 것 넷:

- ⚠️ **회사가 실패를 두 갈래로 정확히 가른다** — `The insert job was taking too long to execute, and just one error would jeopardize insertion of all data because of dependencies among different data models`. **시간이 문제고, 결합이 문제다.** 앞의 것은 나누면 되는데 **뒤의 것은 의존을 끊어야** 풀린다.
- ⚠️ **한 결정이 1년 뒤의 병목과 이어진다** — `we don't need to perform ETL transformations outside of the database` 로 층을 하나 줄였는데, **밖으로 뺐다면 변환 실패와 적재 실패가 갈렸을 것**을 안에 두었기에 **하나의 DAG 로 묶기 쉬웠다**(재구성).
- ⚠️ **자기 제품을 쓰면 그 비용이 장부에서 사라진다** — 인프라 비용을 **월 약 $1,500** 이라 적으면서 `excluding ClickHouse Cloud as internal service` 를 단다. **가장 큰 항목이 사내 서비스라는 이유로 빠지므로, 이 수치는 남이 같은 구조를 지을 때의 비용이 아니다.**
- **주기가 곧 비용이 되는 자리** — `Most of these jobs still run every hour, meaning that our ClickHouse Cloud service has to be running 24x7`. **저장과 컴퓨트를 따로 파는 제품에서 이것은 컴퓨트 요금이다**(재구성).

**연표는 세우지 않았다** — 두 시점의 사실은 다 있는데 **1편의 발행일을 확인하지 못했다.** `A year later` 라는 제목만으로 연도를 지어내지 않았다.

**모순 하나를 발견해 적어 뒀다** — AI 글은 창고를 **2.1PB** 라 하고 2편은 **2년 누적 470TB(압축)** 라 한다. **압축 기준인지 시점 차이인지 확인하지 못했다.**

### 다음 사이클 — AI 도메인, 그러면 완주가 보인다

`--gaps` 가 **분석 일의 70% 를 AI 에 넘기고 30% 를 일부러 남긴다** 를 부른다. **자료가 이미 다 있다**(DWAINE 사용량 · 남긴 30% 의 세 갈래 · **출력만으로 결정 금지** 가드레일 · LibreChat·MCP·Bedrock 선택). **더 찾을 필요 없이 바로 쓸 수 있다.**

그 기능을 쓰면 **도메인 3/3 이 채워져 완주 기준을 잰다.** 그리고 **큐가 2/3 이라 그다음은 신규(Duolingo)**다.
