...
**Rerun 완주(기능 3개).** 회사 135개, **큐 0/3 — 다음 사이클은 후보 조사다.**

### 이번 기능 — `query-result-cache`

창을 끌 때마다 100만 점에 600밀리초면 그건 느린 게 아니라 못 쓰는 것이다. 원인을 넷으로 갈랐고
(타임스탬프 장부 · CPU 캐시 · 움직이는 창의 중복 계산 · 픽셀보다 많은 점) **그중 둘이 자기
데이터 모형이 만든 것**이었다. 모형을 굳히지 않고 읽는 쪽에서 갚아 20밀리초(30배)로 내렸다.

### 무효화가 아무 일도 하지 않는다

변경 기록을 구독하되 **더럽다는 표시만 세우고** 캐시는 질의 때 게으르게 세운다. 기록이 쏟아지는
동안은 아무 일도 안 하고, 그사이 쌓인 변경이 한 번에 처리되는 미세 배치가 공짜로 따라온다.

### 가장 쉬운 답을 물린 자리

지표만 특별 취급하면 금방 빨라지지만 점구름과 IMU 표본은 그대로 느리다 — 그러면 이 도구가
시계열 뷰어가 되어 버린다. 일반적으로 풀어서 어려웠다고 스스로 적는다.

### 세 기능이 한 줄로 꿰인다

성분마다 따로 적는 유연한 모형(기능 1)이 값을 만들고, 저장 단위를 키워(기능 2) 적재를,
질의 캐시로(기능 3) 읽기를 갚는다. **서로의 대가를 서로가 받아 준다** — 청크를 키워 느려진
질의를 캐시가 지우고, 캐시가 느리게 만든 적재는 다음 판의 숙제로 적어 뒀다.

### 다음 사이클 — 후보 조사 (큐 0/3, 비었다)

기준은 그대로 — 회사 소유 도메인 · 엔지니어가 직접 쓴 글 · 자기 시스템의 의사결정과 대가.
최근에 로보틱스·저장·암호로 벌려 놨으니 또 다른 축을 찾는다.
