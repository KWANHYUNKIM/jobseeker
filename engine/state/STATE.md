...
**Hatena — 기능 하나를 채웠다(`one-base-many-tenants`).** 빈 도메인 둘. 회사 150개, **큐 0/3**.

### 층마다 다르게 가른다

테넌트 독립성과 값이 정면으로 부딪히는 자리다. 답은 **한 축으로 정하지 않는 것** — 도메인과
인증서는 사일로(테넌트 전용 CloudFront), 로드 밸런싱과 프록시는 풀(공유 ALB·nginx), 차이는
엣지 함수가 흡수한다. 트래픽을 모으니 **10배 급증이 최대 2.7배 변동**으로 잦아들었다.

### 같은 문제가 규모만 바꿔 되풀이된다

GigaViewer 는 테넌트가 스무 곳인데 하테나 블로그 커스텀 도메인은 **만 단위**다. 그래서 저쪽은
서비스마다 배포를 찍어 내고, 이쪽은 **템플릿 하나에 도메인마다 테넌트를 매단다.**

### 무효화의 단위가 테넌트 설계를 정했다

연표 2세대가 여기서 나온다. **2025-09** 와일드카드를 테넌트 하나로 묶었는데, **캐시 무효화에
Host 헤더 지정이 없어** 블로그 하나만 골라 지울 수가 없었다. 그래서 **2026-03** 블로그 하나 =
테넌트 하나 = 커스텀 도메인 하나로 갔다 — 대신 테넌트 수가 쿼터에 가까워진다.

### 이관에서 실제로 터진 것을 다 적는다

DNS 플래핑으로 테넌트 생성·삭제가 반복(재시도로 막음) · 사용자가 이미 가진 배포와 CNAME 충돌
(사전 점검) · ACM 이 못 하는 ccTLD 는 Let's Encrypt 임포트 · 네이키드 도메인은 Anycast 고정 IP
가 필요하고 사용자가 직접 DNS 를 바꿔야 해 이관이 길어진다 · **지금 무효화 빈도를 그대로 옮기면
요금이 병목**이 될 전망.

### 다음 사이클 — 확장 (남은 두 도메인 중 하나)

`무엇을 쓸지 매달 재서 고른다` 가 수치가 가장 많다(Fargate 대 MI 벤치마크 하나에 정규화 점수·
시간당 값·값 대비 성능·손익분기 25대·꼬리 시간 88.5초 대 72.2초가 다 있다).
