...
**없다 — 문체 보수 사이클이었다.** 아데빈타를 배민 문체로 다시 썼다(0.6 → 0.0, 기호 308개). 회사 115개, 큐 2/3. **1곳 남았다.**

### 재는 자를 만들되 벤치마크를 안 쓴다

메이퇀(평가 기준 여섯 편)과 스냅(3D 추론을 재는 자)에 이어, 아데빈타는 **원래 백로그에 있던 실제
업무 165건으로 잰다** — 이 시험을 위해 따로 고른 일이 아니라고 못 박고, 한계를 먼저 적는다(주관적
판단, 팀당 도구 하나, 한 달).

결론도 승자를 고르는 것이 아니다 — **스스로 빠르게 직접 재 보는 능력을 갖추는 것이 단기 승자를
찾는 것보다 결국 더 값지다.** 그리고 가장 아픈 관찰이 따로 있다 — **가장 덜 필요한 사람에게 가장
도움이 되고, 가장 필요한 사람에게 가장 덜 도움이 되는 것으로 보인다.**

### 사람보다 무언가가 빨리 늘어나는 자리 — 여기는 계정이다

Monzo(서비스 3,000개) · 스냅(코드)에 이어, 아데빈타에서는 **계정이 사람보다 빨리 늘었다**(2019년
150개 미만·3명 → AWS 732개와 GCP 425개를 12명이). 위키 표에 담당자를 적어 두던 방식이 왜 안 됐는지,
인사 조직도가 왜 실제 기능 구조와 다른지까지 적고 **누가 쓰는지를 문서가 아니라 인프라로 옮겼다.**

### 미리 갖고 있을 것인가 그때 물어볼 것인가 — 여기는 반대편을 골랐다

먼데이닷컴(미리 복제) · 누방크(미리 접어 둠)와 달리, 아데빈타는 미리 동기화해 둔 인벤토리가 파드
급증에 메모리를 다 먹자 **그때그때 API 로 가져오는 쪽**으로 옮겼다. 그 값도 적는다 — 메모리는 줄고
API 서버 부하는 는다.

### 원인을 가리키는 신호가 없어 헤맨 셋

Monzo(카산드라) · Wise(압축 백로그)에 이어, 아데빈타는 틀린 가설을 둘 지나고서야 닿는다 — 그리고
**옳은 결론으로 이끈 정보가 우리 지표 시스템에는 없었다**고 적는다. 겪기 전에는 어떤 층에 신호가
없는지 알 수 없다는 결론이 세 회사에서 같다.

### 다음 사이클 — 문체 보수의 마지막 한 곳

--gaps 가 정한다. linkedin(0.56) 하나가 남았다. 그것까지 끝나면 사다리가 3순위에서 내려가
**5순위(재방문) 또는 6순위(보강)** 로 옮겨 간다 — 오랫동안 안 가 본 자리다.
