...
**SmartHR(JP · SaaS) `in_progress` — 도메인 2개 · 기능 2개.** 346 에서 **새 도메인**
`한 덩어리를 그대로 두고 버티게 만든다` 를 열고 그 안에
**`share-a-few-connections`(놀고 있는 접속을 걷어내고 몇 개를 돌려 쓴다)** 를 썼다.
**결정 6개 · 그림 3장 · 생각 4개 · 난제 2건 · 연결 1건.**

**⑭ `--gaps` 는 [후보 조사]**(큐 0/3)를 가리켰다. **회사 완주 규칙이 위라 SmartHR 을 계속
팠다**(CLAUDE.md).

**재료와 판정** — `SmartHR最大のRailsアプリケーションにコネクションプーラーを導入しました
── 2年前の障害対応から学んだスケーラビリティと運用のリアル`(2026-08-04,
`/entry/2026/08/04/091858`). **3/3** — 버린 대안(PgBouncer · 관리형 · App Engine·Cloud
Run·사이드카·GKE · Compute Engine 표준 배포 기능) · 대가(트랜잭션 모드 제약 · 두 경로 유지 ·
**PgCat 개발 중단**) · 수치(접속 **4분의 1 이하** · 메모리 **13% 감소** · 105k QPS · 90초).

**⚠️ 223 규칙으로 갈랐다 — 새 도메인이다.** 기존 도메인은 **떼어내는** 이야기인데 이 글은
**그대로 두고 버티게 하는** 이야기다. **⭐ 이름을 넓게 지었다.**

**⭐⭐ 346 의 핵심 — 두 도메인이 같은 문제에 정반대로 답한다**(이 대비는 이 사이트의 것이고
`connections` 에도 적었다). 인증 쪽은 **단일 실패점을 떼어내서** 풀려 하고, 여기는
**단일 실패점 앞에 무언가를 세워** 버티게 한다. **그리고 버티는 쪽이 훨씬 빨리 끝났다** —
재발 방지가 목표라 **단기 완료가 조건이었고 그 조건이 기술 선택까지 좁혔다**(GKE 대신
Compute Engine).

**346 이 새로 낸 것**
- **⭐ 원인이 뜻밖이다** — *"データベースのコネクションの大半は、クエリを処理していない idle
  コネクションでした"*. **오토스케일이 늘린 것은 처리량이 아니라 접속 수였다**(이 사이트의 정리).
- **⭐⭐ 반성을 못 박는다** — *"今回の反省は、データベースが単一障害点だと把握していながら、
  障害の頻度がいつ上がるのか見当がつかないまま、対応を先延ばしにしていたことです"*.
  **미룬 이유가 '모른다' 였다.**
- **셀프호스트를 고른 값이 뒤에 드러났다** — **PgCat 은 지금 개발이 멈췄고 회사는 이미
  관리형으로 옮겼다.** 글이 그 사실을 스스로 적는다. ⚠️ 옮기는 데 든 값은 안 적는다.
- **접속을 모으면 세션이 사라진다** — 어드바이저리 락 · 커서 · 프리페어드 스테이트먼트 ·
  세션 변수. **경로를 둘로 나누고**(사용자는 풀러, 마이그레이션은 직결) 세션 변수는 롤 레벨로.
  ⚠️ **어디로 보낼지의 판단이 사람 손에 남는다.**
- **배포도 직접 만들었다** — 장시간 트랜잭션이 있어 강제 절단을 피해야 했는데 표준 기능이
  요건에 안 맞아(셧다운 스크립트 **약 90초** 제한) **active·standby 두 그룹 전환**으로 풀었다.
- **관리형이 아니어서 운영이 손에 남았다** — Container-Optimized OS 가 Ops Agent 미지원이라
  OpenTelemetry Collector 기반 에이전트로 메트릭 길을 따로 냈다.

**⭐⭐ 비교 문서 재료가 하나 생겼다**(`open_questions` 에 적었다) — `미루되 미룬 것이 안
보이게 한다 — 그럼 그것은 어디에 쌓이는가` 의 표에 **SmartHR 이 들어갈 자리가 있다**:
미룬 것이 **놀고 있는 접속**에 쌓였고, 청구서는 **장애 빈도가 월 1회에서 동시다발로 오르며**
왔다. **⚠️ 그 문서를 갱신할지는 SmartHR 을 완주한 뒤 정한다.**

---

**다음 사이클 — 새 도메인의 두 편 규칙을 채운다.**
- **⭐ 1순위** — `開発チームにノウハウが残るSREの関わり方 —— 年末調整のピークに備えた負荷試験`
  (2026-07-24, `/entry/2026/07/24/120000`). **같은 도메인일 가능성이 높다**(그대로 두고
  버티게 하는 일). **⭐⭐ 그리고 freee 의 확정신고 정점과 대비된다** — 둘 다 **연 1회 날짜로
  정해진 부하 정점**이다(추정).
- **2순위 — 첫 도메인을 더 판다.** 345 의 글이 참조하는 과거 글 셋(**주소 미확인 — ⑰ 추측
  금지, ⑱ WebSearch + allowed_domains**): **Policy-as-Code**(2025-08-08) ·
  **새 권한 기반 설계**(2026-01-07) · **모듈러 모놀리스화**(2026-05-18).
- **3순위** — `横断フロントエンドユニットの取り組み`(`/entry/2026/08/03/112823`) ·
  `不具合情報の集約基盤を整え、品質改善に活かすまで`(`/entry/2026/08/24/133642`) ·
  `Sandboxテスト`(`/entry/2026/08/17/080914`).
- **4순위 — 닫는다.** 도메인 둘 다 기능이 있고 연결도 이어졌다.
- **⚠️ 이 블로그는 결이 넓다** — 사내 행사·커리어·LT 보고가 절반쯤 섞여 있다. 갈라서 고른다.

**⚠️ 큐 0/3 — SmartHR 을 완주하면 곧바로 후보 조사 사이클을 끼운다.**
**⚠️ Personio(EU) 는 아직 안 봤다 — 첫 순위.** **후보 조사 누적 열아홉 곳 중 일곱.**

**⭐⭐⭐ freee 와의 대비**(`smarthr.json` 의 `open_questions`) — freee 는 **옛 기능 리팩터를
마이크로서비스화보다 먼저 했어야 한다**고 후회했고, **SmartHR 은 그 순서를 실제로 밟아 보고
그것만으로는 부족하다는 결론을 적는다.** **비교 문서의 심장. 완주하면 쓴다.**

**분포** — 58곳(**일본 일곱** · 인도 셋) · 비교 문서 32편 · **운영 사실 37개.**
...
않는다**(342·345 — 346 에서는 지웠고 확인했다) · **세 편을 열고 하나도 안 쓰는 사이클이
정상이다**(343) · **`domain_map`·`diagrams` 는 객체다**(344) · **`features_done` 을 같이
올린다**(345 — 346 에서는 같이 올렸다) · **⭐ 새로: 같은 회사 안에서 두 도메인이 같은 문제에
정반대로 답하면 그 대비를 `connections` 에 적는다**(346).
