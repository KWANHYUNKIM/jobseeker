...
**Deepgram — 프로파일만 썼다(도메인 하나, 기능 0).** 회사 161개, **큐 0/3**.

### 파는 것이 사실상 지연이다

대화형 음성에서 사람이 참는 시간은 짧다 — **ITU-T G.114 는 150밀리초 미만을 선호하고
150~400밀리초를 저하로 본다.** 그런데 합성은 응답이 길수록 오래 걸리고, 전부 계산한 뒤 내주면
**긴 문장일수록 첫 소리가 늦다.** 데모에서는 안 보이고 실제 대화에서만 드러나는 자리다.

### 그래서 전체를 보지 않기로 했다

토큰 단위로 흘려보내고 턴을 넘어 운율을 잇는다 — TTFB p95 200밀리초 미만(정상 상태 약 90ms).
**대가를 스스로 적는다: 배치보다 문맥을 5~20배 적게 보고 그래서 고유명사 발음이 흔들린다.**

### 데모가 감추는 항을 짚는다

**총 지연 = 네트워크 + TTFB + 오디오 합성**이고 마지막 항이 감춰진다고 적는다. 70 QPS 에서
짧은 입력은 136.56ms 격차인데 **7초 오디오는 925.51ms** 로 벌어진다.

### 날짜 없는 자료를 어떻게 다뤘나

`/learn` 은 글에 발행일을 붙이지 않는다. 그래서 **`business_model` 에 그 사실을 적고
`open_questions` 첫 줄에 두었으며, 시대 구분과 세대 교체를 세우지 않았다.** 없는 연도를 지어내지
않는다는 규칙을 회사 단위로 적용한 첫 사례다.

### 다음 사이클 — 확장 (그 하나뿐인 도메인)

`말이 끝나기 전에 소리를 내보낸다`. TTS 지연 글 하나에 물린 대안·대가·수치·판정 기준이 다 있다.

### ⚠️ 큐가 0/3 이다

Deepgram 을 완주시킨 뒤 **후보 조사 사이클을 끼운다.**
