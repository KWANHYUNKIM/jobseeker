...
**없다 — Zepto 완주.** 도메인 3/3 · **기능 3개.** 회사 **91개.** ⚠️ **큐가 1/3 이라 다음은 후보 조사(7순위)다.**

### 이번 사이클 — `집계해 버린 원본은 되돌릴 수 없다`

건진 것 넷:

- ⚠️ **이 기능의 이름이 회사가 적은 교훈의 첫 줄이다** — **`원본 자료를 집계해 없애지 마라.`** 처음의 Flink 파이프라인은 **5분 윈도로 미리 집계해 Postgres 에 넣었는데**, 그 순간 **재생으로 생긴 중복을 못 지우고, `한 광고의 첫 클릭에만 과금한다` 는 규칙을 적용할 수 없고, 클릭 하나하나가 사라져 디버깅이 불가능해졌다.**
- ⚠️ **도구가 주는 기능을 이유를 대고 거절했다** — `ReplacingMergeTree` 는 **중복 제거가 파트 병합 시점에 일어나는데 이 수집 속도에서 병합 빈도가 보장되지 않는다.** 그래서 **질의 시점 `argMax`** 로 갔다. **자동으로 되는 것처럼 보이는 기능이 언제 되는지 보장하지 않으면 그것은 안 되는 것이다**(재구성).
- ⚠️ **구조가 먼저이고 튜닝은 그다음이다** — 회사가 못 박는다: **파티션 키와 정렬 키가 그 뒤에 한 어떤 질의 최적화보다 영향이 컸다.**
- ⚠️ **정확한 숫자와 지금의 숫자는 같은 작업에서 나오지 않는다** — 시간당 집계만으로는 **캠페인이 최대 60분 동안 예산을 넘겨 썼다.** 그래서 **무거운 시간당 분석과 가벼운 3분 지출 계산을 갈랐고**, 3분 쪽은 **과금 관련 이벤트만**(CPC 는 클릭, CPM 은 노출) 읽어 약 1분에 끝난다.

### ⚠️ 시간축을 못 세운 두 번째 사례

**Flink → ClickHouse 라는 세대 교체가 분명한데 글이 연도를 밝히지 않아 `history` 를 비웠다.** **GeoIndia V1 → V2 에 이어 두 번째다.** ⚠️ **`무엇에서 무엇으로 갔는지` 는 아는데 `언제` 를 모르는 경우가 쌓이고 있다** — 두 경우 다 **implementation 과 open_questions 에 내용을 남겨 정보는 잃지 않았다.**

### ⭐ 이 회사에서 나온 대비 둘

- **Meesho 와 같은 답에 다른 길로** — Meesho 는 **연속 배달로 주소를**, Zepto 는 **궤적 합의로 경로를** 배운다. **둘 다 개별 신호를 못 믿어 집단으로 갔다.**
- ⚠️ **ClickHouse 를 만드는 쪽과 쓰는 쪽이 이 사이트에 함께 있다** — **만드는 쪽은 엔진의 한계를 말하고, 쓰는 쪽은 `병합 시점을 믿을 수 없다` 는 운영의 현실을 말한다.** connections 에 재구성으로 이어 뒀다.

### 다음 사이클 — 후보 조사

⚠️ **큐가 1/3 이다**(Snyk). ⏳ 되짚을 목록 — **`bytes.swiggy.com`**(⚠️ `r.jina.ai` 가 422 라 브라우저가 필요한데 세 사이클째 응답이 없다) · `www.gojek.io/blog`(429) · `tech.cygames.co.jp` · `www.scylladb.com/blog`.

⭐ **`r.jina.ai` + WebFetch(verbatim 인용 지정) 조합이 이제 기본 수단이다** — 브라우저 없이 Meesho 와 Zepto 두 회사, 기능 일곱 개를 이 조합으로 읽었다.
