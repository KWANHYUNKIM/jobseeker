...
**아무도 안 판다 — 357 은 보수 사이클이었다.** 356 에서 드러난 **인용 규칙 위반 유산을
한 번에 정리했다.** **완료 60곳 전부 `done` · 큐 0/3 · 비교 문서 32편.**

**⑭ `--gaps` 는 [후보 조사]** 를 가리켰다. **⚠️ 그런데 WebSearch 예산이 소진돼**(세션 200회)
후보 조사가 불가능하다. **그 사실을 기록하고 보수로 갔다.**

### 무엇을 고쳤나

**우리말을 원문 인용 표시 안에 넣은 자리를 굵게로 바꿨다.** 인용 규칙(337)은
**원문 인용 표시는 원문 전용, 우리말은 굵게** 다.

- **고친 파일 33개**(doximity 포함) — 치환 **8,000회 이상**(같은 `sources[].summary` 가 여러
  곳에 복사돼 있어 한 문장이 수십 번 나온다). 많은 쪽부터 **cyberagent 1882 · wayfair 1399 ·
  target 1377 · doximity 1429 · linkedin 1157 · yelp 735 · moniepoint 705 · mercari 630 ·
  tigerbeetle 376 · planetscale 233 · zerodha 224 · vinted 202 · allegro 123 · careem 76 ·
  trivago 59 · adyen 47** …
- **✅ 비한국 회사 파일의 위반이 0 이 됐다.** 검증도 통과(**회사 60개 · 경고 1건**).

### ⚠️ 일부러 안 고친 것 — 한국 회사 파일

**hyperconnect 128 · kurly 110, 합쳐서 238개가 남아 있는데 이건 위반이 아니다.**
**한국 회사의 한국어 블로그를 인용한 것이라 원문 인용 표시가 맞다.** 표본을 눈으로 확인했다 —
하십시오체의 기술 블로그 문장들이다.

**⚠️ 그래서 앞으로 이 검사는 한국 회사를 빼고 읽어야 한다.** 한국 회사 목록(`index.json` 의
`country` 가 KR): **baemin · coupang · daangn · hyperconnect · kakao · kurly · musinsa ·
naver · toss.**

**⚠️ 다만 확인 못 한 것이 하나 있다** — 그 238개 중 **이 사이트가 스스로 쓴 우리말 정리를
인용 표시로 감싼 것**이 섞여 있을 수 있다(337 의 규칙상 그건 굵게여야 한다). **표본 열여섯
개는 다 원문으로 보였지만 전수는 안 봤다.** 재방문 때의 일거리다.

### 검사 스크립트 (JSON 을 파싱해서 본다)
...
**⚠️ 파일 원문에 정규식을 걸면 안 된다** — JSON 이 큰따옴표를 이스케이프해서 **늘 0 이 나온다**
(356 에서 겪었다).

---

**다음 사이클 — 비교 문서다.** 보수가 끝났고 후보 조사는 막혔다.

**⭐ 재료가 다 갖춰진 것 — `freee 대 SmartHR 대 PayFit: 같은 문제, 세 가지 답`.**
셋 다 급여·인사 SaaS 이고 **제품이 늘며 경계가 무너진** 같은 문제를 겪었는데 답이 다르다.
- **freee** — **떼어냈다.** 인증 기반을 회계 DB 에서 4년(1,454일)에 걸쳐 뽑아냈고
  **후회를 명시한다**: *"旧機能のリファクタはマイクロサービス化の前に着手すべし"*.
- **SmartHR** — **규칙으로 긋다 안 되어 프로세스로 나눴다.** Packwerk 로 경계를 보이게 하는
  방법이 **새 영역에서는 됐는데**(SaaS 관리 유닛) **이미 얽힌 인증에서는 안 됐다** — 인가가
  과금·테넌트에 의존해야 해서 **의존 허용 규칙이 그대로 경계 설계가 되고**, 배포 독립성과
  가용성은 규칙으로 안 풀린다.
- **PayFit** — **이름부터 다시 정했다.** 2021년부터 2년, 이벤트스토밍과 워크숍으로
  Payroll Registry · Payslips · Payroll Period · Declaration 을 갈랐다. **급여 명세는 전에
  *"considered as an internal implementation detail"* 이었다.** ⚠️ **성과 수치가 하나도 없다.**

**⚠️ 비교 문서 규칙**(PROMPT.md) — **새 주장을 만들지 않는다.** 각 회사 페이지에 `confirmed`
로 적힌 사실만 엮고, **비교에서 나오는 해석은 추정이라고 쓴다.** 표로 나란히 놓고 **각자가
감수한 것과 서로가 답하지 않은 질문**을 반드시 넣는다. 파일은
`jd-viewer/public/reveng/domains/<slug>.md` 이고 **그 폴더 `index.json` 의 `docs` 배열에
한 줄을 더한다**(키가 `docs` 다). **H1 을 갖는다.**

**그 밖의 비교 문서 재료 셋**
② **`미루되 미룬 것이 안 보이게 한다`** 에 SmartHR 두 사례 — 미룬 쪽은 **놀고 있는 접속**
(단일 실패점인 줄 알면서 언제 심해질지 몰라 미뤘다), 미리 잰 쪽은 **연 1회 정점의 부하 시험**.
③ **bol 이 ②의 셋째 사례** — 중개 앱을 2031년까지 유지하는 안을 버렸고
*"there's nothing as permanent as a temporary solution"* 라 적는다.
④ **`되돌릴 수 없는 것이 어디에 있는가` 에 bol** — **적어도 한 번** 처리를 골랐는데 이관 쪽에
검증 이야기가 아예 없다.

**⚠️ 후보 조사는 이 세션에서 막혔다** — WebSearch 예산 소진. **주소를 아는 글만 WebFetch 로
열 수 있다.** 큐가 0/3 이지만 채울 방법이 없다.

**분포** — **60곳 전부 `done`**(US 22 · KR 9 · EU 9 · JP 7 · IN 3 · 기타 3 · CN 2 ·
CA·AU·SG·AE·NG 각 1) · 비교 문서 32편 · **운영 사실 60개.**
...
**코드펜스도 백틱이다. 긴 문서는 Write 툴로 스크립트를 만들어 셸 파싱을 피한다**) ·
**닫을 때 낡은 문장 다시 본다**(348) · **URL 끝 16진수 해시는 Medium**(349) ·
**`category`·`country` 허용값**(350) · **세 번째 읽어도 새 사실**(351) ·
**'무엇이 없는지' 를 함께 묻는다**(352) · **실행 안 한 평가는 기능으로 못 쓴다**(353) ·
**요약만 보고 판정하면 과대평가**(354) · **버린 대안 없는 글에서도 결정을 세운다**(355) ·
**검사가 0 을 내면 스크립트부터 의심한다**(356) ·
**⭐ 새로: 기계적 보수는 예외를 먼저 가른 뒤에 돌린다 — 한국 회사의 한국어 인용은 정당하므로
같은 치환을 걸면 안 된다**(357).
