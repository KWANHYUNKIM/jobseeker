...
**Monzo** (EU·UK · 핀테크) — 도메인 3개 · **기능 1개**(`one-team-drives-all-migrations`). 남은 도메인 둘.

### 이번 사이클 — 이전 `이전을 팀마다 맡기지 않고 한 팀이 전부 민다`

⚠️ **앞 사이클에서 `플랫폼 글에 버린 대안이 없다` 고 적어 뒀고, 그래서 기능을 쓰기 전에 자료를 더 찾았다. 그게 맞았다** — `how-we-run-migrations-across-2800-microservices` 를 찾았고 **버린 대안과 대가가 명시적이다.** **한 도메인의 자료가 얇다고 그 도메인이 얇은 것은 아니다.**

건진 것 넷:

- ⚠️ **중앙집중이 비싸다는 것을 알면서 골랐다** — 분산으로 해 봤더니 `unfinished migrations and a lot of coordination effort` 가 남았다. 중앙은 **`high price for one team`** 을 치른다. **한 팀이 다 하면 그 팀이 병목이고, 팀마다 하면 아무도 안 끝낸다** — 이 회사는 병목 쪽을 골랐다.
- ⚠️ **의존 그래프가 부분 이전을 막는다** — `all services it might (transitively) call need to also support the new functionality`. **전이적**이라는 조건 때문에 **전 서비스가 준비된 뒤에야 첫 번째를 켤 수 있고**, 그래서 대량 배포 도구가 선택이 아니라 **전제**가 된다.
- **되돌리는 속도의 차이가 설계를 정했다** — **설정 갱신 60초 대 배포 몇 분.** 그래서 새 구현을 **꺼진 채로 배포**해 두고 켜는 일만 설정으로 한다. **은행에서 그 차이는 사고 시간의 차이다.**
- ⚠️ **회사가 스스로 이 방법의 한계를 적는다** — **모노레포와 일관된 스택이 없으면 성립하지 않는다.** 한 커밋 대량 리팩터링과 CI 전사 강제는 **앞서 치른 비용이 능력으로 돌아온 것**이다(재구성).

**같은 방법이 두 도메인에 나타난다** — 여기서는 `semgrep` 이 옛 라이브러리를 막고, 데이터 쪽에서는 표준이 PR 을 막는다. **자율을 두되 어기는 것만 자동으로 막는 모양이다.**

### 다음 사이클 — 카산드라 아니면 데이터

`--gaps` 가 남은 둘 중 하나를 부른다. **자료는 둘 다 있다** — 장애 회고(2019)와 데이터 메시 글(2026).

⏳ **새로 확보한 자료** — `we-built-network-isolation-for-1-500-services`(서비스 1,500개·고유 연결 9,300개 → **평균 6개**로. **버린 대안 셋이 이유와 함께 적혀 있다**). **플랫폼 도메인에 기능을 하나 더 쓸 재료이거나, 별도 도메인의 씨앗이다.**

⏳ 아직 안 본 것 — `Building a Modern Bank Backend`(2016) · `Autoscaling Monzo`(2020) · AI 축 두 편. **2016년 글과 2026년 글을 나란히 놓으면 연표가 나올 수 있다.**
