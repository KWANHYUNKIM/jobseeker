...
**없다 — 일흔일곱 번째 후보 조사를 돌았다. 큐 1/3.** 다음 사이클은 **신규**(카케하시).

### 비어 있던 축이 세 사이클 만에 채워졌다

엠쓰리를 완주하며 **'의료 도메인 자체의 기술 글은 못 봤다'** 고 적었는데, 그 뒤로
**헨리**(전자카르테·규제)와 **카케하시**(약국 재고 예측)가 붙어 **의료 축이 셋**이 됐다.
**열어 둔 물음을 적어 두면 다음 조사가 그것을 향한다.**

### 다시 쓰지 않기 위해 층을 가른다

카케하시의 통과 근거다. 스키마를 바꾸면 **과거 예측 데이터 자체를 다시 써야** 했고 그것이
**위험하고 감사 추적을 흐렸다.** 그래서 **쌓는 층은 불변**으로 두고 **분석 층만 재생성**
가능하게 만들었다. **스키마 버전마다 전체 정의를 유효 기간과 함께 남기고 델타를 순서대로
적용**한다 — **마이그레이션 대신 이력을 쌓는 쪽**이다.

### 물류·제조 축은 여전히 비어 있다

오픈로지와 모노타로가 **연결 자체가 안 된다**(000). 하테나가 아닌 자체 도메인은 여전히
확률이 낮다.

### 다음 사이클

**신규 — 카케하시.** 큐 1/3 이라 신규 한 번 뒤에 조사가 다시 뜬다.
