...
**SmartHR(JP · SaaS) `in_progress` — 도메인 1개 · 기능 1개.** 345 에서 첫 기능
**`when-a-rule-is-not-enough`(규칙으로 그은 경계가 못 하는 것이 있다)** 를 썼다.
**결정 6개 · 그림 3장 · 생각 4개 · 난제 2건 · 연결 0건**(아직 같은 회사에 이을 기능이 없다).

**⑭ `--gaps` 가 [확장] smarthr 을 정확히 지목했다.**

**재료와 판정** — `認証がプロダクトの足かせにならない世界へ —— 認証・認可基盤ユニットの発足と挑戦`
(2026-08-12, `/entry/2026/08/12/102806`). **2/3** — **버린 대안이 아주 구체적이고**(모듈러
모놀리스로 경계만 정리하는 안을 실제로 검토했고 왜 안 되는지를 적는다) **대가도 회사가
직접 적는다**(운영 복잡성 · 네트워크 통신 비용 · 이중 유지보수). **⚠️ 수치는 거의 없다** —
발족이 2026년 2월이고 팀은 *"少人数のプロダクトエンジニアで構成"* 이라고만 한다.

**⚠️ 223 규칙으로 갈랐다 — 같은 도메인의 두 번째 편이다.** 둘 다 한 덩어리에서 떼어내는
이야기이고 스트랭글러를 쓴다. **✅ 두 편 규칙을 채웠고 도메인 `why` 를 고쳐 썼다.**

**⭐⭐ 345 의 핵심 — 같은 회사가 같은 도구를 두 방향으로 썼고 결과가 갈렸다**(이 해석은
이 사이트의 것이다). **새로 만드는 쪽**(SaaS 관리 유닛)에서는 한 덩어리 안에서 시작해
Packwerk 로 경계를 보이게 한 뒤 떼어내는 것이 **됐다.** **이미 안에 있던 것**(인증)에서는
**그 방법으로 안 되는 것이 있다고 회사가 적는다.**

**왜 안 됐나** — 업무상 인가를 하려면 **인가가 과금·테넌트에 의존해야 하는데**, 그 의존을
Packwerk 에서 허용하면 *"認可よりも変更頻度の高い領域に認可が引っ張られてしまいます"*.
**의존을 허용하는 규칙이 그대로 책임 경계 설계가 되어 버린다.** 게다가 모듈러 모놀리스로는
*"デリバリ（リリースの独立性）と可用性を解消できません"*.

**345 가 새로 낸 것**
- **⭐ 경계가 없으면 검사 범위도 못 좁힌다** — 같은 프로세스라 *"モノリス内のどのコードも
  原理的には認証周りの内部状態（セッショントークン）にアクセスできてしまいます"* 이고,
  **침투 테스트나 제3자 감사를 인증 주변만 떼어 심사할 수 없다.**
- **가용성** — *"認証周りが止まれば、SmartHRのすべてのプロダクトにログインできなくなります"*.
- **대가를 회사가 못 박는다** — *"マイクロサービス化は運用の複雑さやネットワーク境界をまたぐ
  通信のコストを新たに持ち込みます。それでも、可用性と独立したデプロイがもたらす価値がその
  代償を上回ると判断しました"*.
- **기능 개선을 섞지 않는다** — 동작을 유지한 채 떼어내는 **리프트를 먼저** 하고 개선은 그다음.
  **떼어낸 직후에는 사용자에게 아무것도 안 나아진다**(이 사이트의 정리).
- **가장 어려운 것을 뒤로 미뤘다** — 도메인을 인증·IDaaS·인가 셋으로 갈라 **인증부터** 하는데,
  **유닛 이름에 인가가 들어 있는데도 인가는 남았다.** ⚠️ 언제 할지는 안 적는다.
- **기술이 아니라 주인의 문제였다** — 멀티프로덕트화로 여러 팀이 손댈 수 있게 됐고
  **오너십 부재인 채 개별 요건에 특화된 구현이 쌓였다.**

---

**다음 사이클 — SmartHR 의 두 번째 기능. 새 도메인이 열릴 것이다.**
- **⭐ 1순위** — `SmartHR最大のRailsアプリケーションにコネクションプーラーを導入しました
  ── 2年前の障害対応から学んだスケーラビリティと運用のリアル`(2026-08-04,
  `/entry/2026/08/04/091858`). **장애에서 출발한 글이라 대가와 수치가 있을 가능성이 높다**
  (추정). **다른 도메인일 것이다 — 이름을 넓게 짓는다.**
- **⭐ 2순위** — `開発チームにノウハウが残るSREの関わり方 —— 年末調整のピークに備えた負荷試験`
  (2026-07-24, `/entry/2026/07/24/120000`). **⭐⭐ freee 의 확정신고 정점과 대비된다** —
  둘 다 **연 1회 날짜로 정해진 부하 정점**을 앞두고 하는 일이다(추정).
- **3순위 — 같은 도메인을 더 판다.** 345 의 글이 참조하는 과거 글 셋이 있다(**주소 미확인**):
  **Policy-as-Code**(2025-08-08) · **새 권한 기반 설계**(2026-01-07) ·
  **모듈러 모놀리스화**(2026-05-18).
- **4순위** — `横断フロントエンドユニットの取り組み`(`/entry/2026/08/03/112823`) ·
...
**⭐⭐⭐ freee 와의 대비가 345 에서 한 층 깊어졌다**(`smarthr.json` 의 `open_questions`) —
freee 는 **옛 기능 리팩터를 마이크로서비스화보다 먼저 했어야 한다**고 후회했고,
**SmartHR 은 그 순서를 실제로 밟아 보고 그것만으로는 부족하다는 결론을 적는다.**
**비교 문서의 심장이 될 재료다. SmartHR 을 완주하면 쓴다.**

**분포** — 58곳(**일본 일곱** · 인도 셋) · 비교 문서 32편 · **운영 사실 35개.**
...
않는다**(342 — **345 에서도 한 번 남아서 고쳤다. 두 번째다**) · **세 편을 열고 하나도 안 쓰는
사이클이 정상이다**(343) · **`domain_map` 은 객체다**(344) · **⭐ 새로: 기능을 더하면
`index.json` 의 `features_done` 도 같이 올린다 — 345 에서 빠뜨려 validate 가 ✗ 를 냈다.**
