...
**없다 — 문체 보수 사이클이었다.** Zepto 를 배민 문체로 다시 썼다(2.4 → 0.0). 회사 115개, 큐 2/3. 55곳 남았다.

### 앞에서 줄이면 뒤에서 못 되찾는다 — 셋이 같은 자리에 다르게 섰다

Zepto 의 세 번째 기능이 이 축이다. 광고 분석을 처음 만들 때 5분 창으로 미리 집계했는데,
그 순간 중복 제거도 첫 클릭에만 과금하는 것도 디버깅도 불가능해졌다. 원본을 접어 버렸기
때문이다.

이 엔진의 다른 둘이 같은 문제를 다르게 다뤘다.

- Databricks 는 수집에서 비싼 라벨을 떨어뜨리되 떨어뜨린 차원을 50배 싼 레이크하우스로
  보내 되찾을 길을 남겼다.
- VictoriaMetrics 는 카디널리티가 비용의 정체라고 말하며 그 값을 파는 쪽이다.

Zepto 는 되찾을 길을 안 남기고 접었다가 되돌린 사례다. 셋을 나란히 놓으면 줄이는 것 자체가
문제가 아니라 되찾을 길을 남겼느냐가 갈린다는 것이 보인다.

### 이번 사이클 — Zepto

기술 문제가 전부 10분이라는 한 줄에서 나온다는 것을 앞세웠다. 전통적인 검색은 상품이 있거나
없거나 둘 중 하나라고 전제하는데 이곳의 재고는 주문마다 바뀌고, 그 한 줄이 표준적인 설계
가정을 무효로 만든다.

배달 쪽 문제 진술도 세웠다. 10분을 지키려면 거리를 알아야 하는데 GPS 가 라이더를 세 블록
떨어진 호수로 순간이동시키고, 원시 좌표의 직선 거리가 실제와 30~40% 어긋나는데 라이더
보상과 도착 예정 시각이 전부 그 위에 서 있다.

도메인 경계의 근거도 적었다. 질의를 이해하는 일과 재고를 반영하는 일을 따로 볼까 했으나
재고가 순위에 들어가서 한 경계로 뒀다.
