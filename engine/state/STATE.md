...
**쏘카(KR · 모빌리티) — 회사 프로파일까지 썼다.** `in_progress` · **도메인 2 · 기능 0.**
**완료 61곳 + 쏘카 = 62곳 · 큐 대기 1(카카오뱅크) · 비교 문서 33편.**

### 다음 사이클은 쏘카의 첫 기능이다

도메인 **`남의 화면이 바뀌면 우리가 환불한다`** 안에서 **주차 할인권 자가복구 에이전트**를 판다.
자료는 이미 본문까지 읽었다 — `tech.socar.kr/dev/2026/07/22/agentic-engineering-in-production`.
**결정 재료가 최소 여섯이다**: 벤더 SDK(OpenAI·Claude Agent SDK)를 버리고 LangGraph 를 고른 이유
(**"특정 벤더에 묶이거나 상태·라우팅을 저수준에서 쥐기 어려워"**), 원인별 에이전트 분리를 버린 이유
(**"어느 에이전트에게 보낼지 라우팅하려면 이미 상당한 분석이 끝나 있어야"**), 다섯 단계로 고정한 것
(login → search → discount → check → compare), 화면을 **접근성 트리 기반 구조화 스냅샷**으로 받는 것,
실패했을 때만 analyze 를 도는 것, 그리고 비용·검증 시간을 어디까지 감수할지.
**실패 경로 그림의 재료도 있다** — **실패 없는 장비사의 조용한 변경은 미탐지**다.

**수치는 이미 확보** — 연간 할인권 적용 실패 약 30만 건 · 환불로 이어진 것 약 16.5만 건 ·
장비사 50여 곳 · 사람이 하던 평균 4시간 → 복구 기준 5분 이내 · 2개월간 7,267건 복구 ·
복구 한 건에 LLM 비용 대략 몇백 원 · 프롬프트 한 줄 변경에 검증 30분 · PoC 원인 분류 정확도 100%.

### 그 다음 — 두 번째 도메인의 `tech` 를 채운다

**`차 한 대의 생애를 얼마로 만들 것인가` 는 `tech` 를 비워 뒀다**(검증이 경고로 잡는다, 의도한 것이다).
실적 발표문의 문장으로 **경계만** 잡았고 무슨 기술로 풀었는지는 아직 안 읽었다.
**기술블로그의 데이터 카테고리가 24편으로 가장 크므로** 거기에 재료가 있을 것으로 본다.

### 쏘카 자료 접근 — 다음 실행이 헛돌지 않게

- **`tech.socar.kr` 은 WebFetch 로 본문까지 열린다.** (`tech.socarcorp.kr` 에서 301) 목록에서 글
  URL·날짜·카테고리를 함께 받을 수 있다. 카테고리별 편수: **데이터 24 · 백엔드 19 · 웹 프론트엔드 7 ·
  Mobility 5 · Design 5 · Product 4 · QA 3 · 보안 2 · 디자인 시스템 2.**
- **IR 은 회사 블로그가 대신한다.** `socarcorp.kr/blog/689`(2026-02-25)에 FY2025 수치가 표로 있고
  **WebFetch 로 열린다.** ⚠️ **부문별 매출·차량 대수·회원 수는 거기에 없다** — `open_questions` 에 적었다.
- **⚠️ `eras` 는 세우지 않았다.** '쏘카 2.0' 이 2024년부터의 전환으로 보이고 회사가 2022~2023 대비
  2024~2025 를 비교하지만, **그때 기술적으로 무엇을 갈아엎었는지를 적은 자료를 못 찾았다.**
  연도만 있고 바뀐 내용이 없으면 시대로 자르지 않는다(STYLE.md 8번).

### ZOZO 에서 배운 것 — 다음 회사에도 걸린다

- **제목의 마지막 낱말이 기능으로 쓸 수 있는지를 가른다.** 「精度改善検証」은 오프라인 검증,
  「構築と運用」은 운영이었다. **본문에서 '본번·릴리스·A/B' 를 먼저 찾고 없으면 기능으로 쓰지 않는다**(353 의 연장).
- **관리형 서비스를 쓰는 회사는 '감시' 가 도메인이 된다.** 쏘카의 첫 도메인도 성질이 같다 —
  **내가 못 고치는 남의 시스템에 매출이 걸려 있고, 그것이 조용히 바뀐다.**
- **⚠️ `--gaps` 와 사다리가 어긋날 수 있다.** ZOZO 를 파는 두 사이클 내내 도구는 **신규**를 가리켰지만
  사다리 2순위 조건(`status: in_progress` 인 회사가 있다)이 맞았다. **도구는 '빈 도메인' 만 센다.**
  지금은 쏘카에 빈 도메인(기능 0)이 있으므로 어긋나지 않는다.

### ⚠️ 다른 세션과 같은 큐를 잡지 않는다

**사용자가 여러 세션을 동시에 돌리고 있다.** 이번 세션에서 실제로 부딪혔다 — `study-engine` 의
`인터럽트` 문서를 다 쓰고 커밋하려다 **다른 세션이 몇 분 전 이미 커밋한 것**을 발견하고 되돌렸다
(`7bf8079`). `guide-engine`·`study-engine` 둘 다 계속 커밋 중이다.
**사이클 시작 전에 `git log --oneline -3` 을 보고, 커밋할 때 `git add -A` 를 쓰지 않는다** —
다른 세션이 작업 중인 파일이 워킹트리에 함께 떠 있다.
