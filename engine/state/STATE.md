...
**Target (US · 커머스/리테일 물류) — 42번째. 기능 1/2 도메인.**

사이클 220 에서 **`split-shipments`(주문 분할)** 를 썼다. 결정 7 · 그림 3 · 생각 4 · 난제 2 · ui 4핀.

**축은 '정확한 답을 포기하고 얼마에 사느냐'다.** 폭발이 두 번이라는 것이 근거다 —
**배송 후보가 2ⁿ−1**(10품목이면 1,023)이고 그것을 묶는 **이행안이 벨 수 Bₙ** 으로 자란다.
그리디가 지는 이유도 기계적으로 적혀 있다 — *"이행 비용 함수는 계단형이고, 일정 임계를
넘으면 비용이 급격히 오른다."* 그래서 **그리디를 버리지 않고 출발점으로 쓴다.**
결과가 **계산 시간 8% 로 격차 91%**.

**⚠️ 다음 사이클이 이어받을 것 — 남은 도메인 `비용을 청구하지 않고 보여준다` 는 본문을
한 편(`infra-showback`, 2025-04)만 읽었다. 기능을 쓰기 전에 반드시
`blog/accelerating-developer-velocity-dbaas`(2025-10-13)를 읽는다.**
(도메인은 한 편이면 열지만 기능은 두 편 이상 — 사이클 209~219 에서 아홉 번 확인됐다.)

**또는 같은 도메인의 두 번째 기능**도 있다 — `math-behind-inventory-delivery-optimization`
(2025-11, 재고 배치)은 *주문이 들어오기 전* 문제라 `split-shipments` 와 섞지 않고 따로 뒀다.
**하루 약 20만 품목 · 수조 변수 → 수백만 변수 · 품절 25~30% 감소**가 재료다.

**사이클 220 에서 새로 읽은 글** — `blog/synthetic-digital-orders`(2025-07-22). *"프로파일 생성,
프로세스 테스트, 예측 모델링"* 에 쓰는 **합성 주문**(Demand Profiler)이고 **이행센터 단위 통합
2배**, **향후 유닛 배분 정확도 약 40% 개선**을 적는다. ⚠️ **이행 최적화 글과 서로를 언급하지
않아 두 시스템이 이어지는지는 확인하지 못했다** — 회사 `open_questions` 에 남겼다.
**세 번째 기능 후보로는 쓸 만하지만 버린 대안도 대가도 안 적혀 있다.**

**⚠️ 아직 안 본 것** — 목록을 **첫 페이지 15편만** 셌다. 전체는 **7페이지(약 100편)** 이고
**분류별 목록(Artificial Intelligence · Cybersecurity · Data Sciences · Documentation ·
Infrastructure · Open Source · Product Engineering)을 아직 안 봤다.**
**추천·개인화 4편은 일부러 미뤘다** — 자료가 없어서가 아니라 이 사이트에 그 축이 이미
두꺼워서다. **세 번째 도메인을 연다면 거기가 1순위다.**
