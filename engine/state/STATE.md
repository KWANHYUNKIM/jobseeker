...
**Protomaps — 기능 하나를 채웠다(`single-file-tile-archive`).** 빈 도메인 하나. 회사 146개, 큐 1/3.

### 저장이 아니라 요청이 형식을 정했다

타일 3억 개를 낱개로 올리면 **요청 요금만 1,500달러가 들고 며칠이 걸린다.** 저장 단가가 싼 게
문제가 아니라 요청 단가가 비싸서, 피라미드 전체를 파일 하나로 묶고 중복을 지운다(70%+ 감소).

### 서버가 없으니 클라이언트가 색인을 읽는다

HTTP 범위 요청으로 헤더·디렉터리를 먼저 읽고 그 타일의 바이트 범위만 가져온다. 그래서 **타일을
힐베르트 곡선 순서로 늘어놓아** 공간의 가까움을 파일의 가까움으로 옮긴다 — 지도를 끌 때 받는
범위가 붙어 있게 하려는 것이다.

### 규모가 커지면 집합부터 안 된다

지역을 뽑으려면 어떤 타일이 그 안에 드는지 알아야 하는데, 좌표를 해시맵에 담으면 남아메리카
줌 17에서 **13,176MB 와 103초**가 든다. 힐베르트 ID 로 색인한 로어링 비트맵으로는 **5MB 와
95밀리초**다 — 규모의 문제가 아니라 자료 구조의 문제였다.

### 터보퍼퍼와 같은 문제의 지도판

둘 다 진실을 객체 저장소에 두고 범위 요청으로 적게 읽는다. 저쪽은 포스팅 블록 크기로, 여기는
힐베르트 순서로 **"한 번에 필요한 것이 붙어 있게"** 만든다.

### 다음 사이클 — 확장 (Protomaps 의 마지막 빈 도메인)

`안 맞는 자리를 스스로 적는다`. 적격 조건 셋과 권하는 대안 셋, 경제성이 무너지는 지점이 손에
있다.
