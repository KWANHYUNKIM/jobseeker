...
**Yelp — 39번째 회사.** 사이클 209 까지 프로파일 + `ui_map` + **도메인 3개** + **기능 2개**
(`partition-access` · `in-place-swap`).

> **`in-place-swap` 의 축 — 되돌릴 길의 폭이 곧 걸리는 시간이다.**
> 같은 회사의 두 이관이 **정반대 답**을 냈다. **Cassandra 는 되돌리기 쉬운 방법을 알고도 버렸다** —
> 별도 DC 방식이 *"트래픽을 옛 DC 로 돌려 더 쉬운 롤백"* 을 줬는데, 몇 주 · `EACH_QUORUM` 강등 ·
> 노드 2배 비용이 더 무거웠다. **그래서 그 글에는 롤백 절차가 없다.**
> **PostgreSQL → MySQL 은 반대로 모든 단계에서 되돌릴 수 있게** 이중 쓰기로 갔다 — **거의 1년 걸렸다.**

> **눈에 걸린 것 넷** — ① **DB 를 바꾼 이유가 성능이 아니라 '아는 사람 수' 였다**
> (*"장애 대응을 할 만큼 Postgres 를 잘 아는 사람은 오래 근무한 소수뿐"*).
> ② **숫자가 셋 다 달랐다** — 공식 기대 p99 21~60% · 자체 벤치 **4%** · 프로덕션 **최대 58%**.
> ③ **규칙이 코드에서 데이터로 옮겨 갔다** — 중복 예약을 막던 트리거를 **15분 블록 수백만 행**으로.
> ④ **마지막 걸림돌이 애플리케이션이 아니었다** — *"분석 파이프라인이 Postgres 에서 직접 가져가고 있었다."*

> **📌 비교 문서 재료가 두 축 모두 찼다.**
> **'재 보고 고른다'(3)** — Careem `cost-leaks` · Zerodha `log-storage` · Yelp `partition-access`.
> **'되돌릴 수 있는 것을 먼저 한다'(4)** — Figma 논리→물리 샤딩 · Shopify 섀도 모드 ·
> 토스 카나리 자동 롤백 · **Yelp `in-place-swap`**. Yelp 쪽은 **같은 회사가 두 답을 낸 사례**라
> 그 글에서 가장 단단한 재료가 된다. 둘 다 아직 안 쓰였다.

> **다음 사이클** — 도메인 **`광고비가 매출인 회사의 매출 운영`**. **⚠️ 읽은 한 편(백테스팅)에 버린 대안이
> 없었다.** 그러니 **`Revenue Automation Series` 세 편을 먼저 읽는 사이클로 쓴다** —
> `2024/12 modernization-of-yelp's-legacy-billing-system` · `2025/02 revenue-automation-series-building-revenue-data-pipeline` ·
> `2025/05 revenue-automation-series-testing-an-integration-with-third-party-system`.
> **읽고 나서 수치와 버린 대안이 있으면 기능을 쓰고, 없으면 `hold_reason` 을 붙이고 회사를 닫는다.**

> **⚠️ 접근법** — Yelp 블로그는 **`/page/N/` 로만 넘어간다**(끝 슬래시 없으면 http 로 301). **43페이지 중 3페이지만 봤다.**
> IR 의 **분기 실적·보도자료는 JS 렌더링이라 WebFetch 로 안 온다**.
> **⚠️ 식별자 필드(`connections[].to`·`key`·`name`·`domain`)에 백틱·별표를 넣지 않는다** — 검증이 경고한다(208).

**큐 2/3** — Doximity(US · 의료) · Moniepoint(NG · 핀테크).

마지막 사이클: 209 (2026-08-27) — Yelp 돌고 있는 것을 그대로 둔 채 갈아 끼운다.
