...
**Ghostty — 프로파일만 썼다(도메인 셋, 기능 0).** 회사 159개, **큐 0/3**.

### 흉내 내지 않고 진짜를 쓴다

터미널은 대개 자기가 그린 화면 안에 탭도 분할도 **글자로 흉내 낸다.** 이 프로젝트는 반대다 —
macOS 는 Swift/AppKit/SwiftUI, 리눅스는 Zig/GTK4 로 **진짜 네이티브 UI 부품**을 쓴다. 그러면
플랫폼마다 다른 껍데기를 유지해야 하고, 그래서 코어와 껍데기가 갈린다.

### 남들이 각자 만들던 것을 그만두게 하려 한다

**수백 개 프로그램이 어떤 형태로든 터미널 에뮬레이션을 구현한다.** 대부분 임시변통이라
불완전하고 버그가 있고 느리다(Jediterm 이 중간 문자를 잘못 다뤄 글자를 삼키는 예를 든다).
그리고 대다수 개발자에게 그것은 **자기 사업의 핵심이 아니므로 시간 낭비**다 — 그래서 검증된
코어를 `libghostty-vt` 로 떼어 낸다(의존성 0 · SIMD 파싱 · 퍼징과 발그린드).

### 켜 두고 잊는 프로그램의 메모리

**열흘에 37GB.** 1.0 때부터 있던 버그가 **특정 부하 패턴**(클로드 코드 CLI)이 나타나서야
드러났다. 원인은 스크롤백 정리 최적화에서 비표준 페이지를 메타데이터상으로만 줄이고 실제
`mmap` 을 안 푼 것 — 나중에 표준 풀 페이지로 오인돼 `munmap` 이 영영 안 불렸다.

### 회사가 아니라 프로젝트다

지그 재단·레이디버드와 같은 자리다. 파는 것이 없고 **자유 시간에 사랑으로 하는 일**이라고
적는다. **⚠️ 라이선스와 법인은 About 에 없어 확인하지 못했다.**

### 다음 사이클 — 확장 (첫 도메인)

`몇 주씩 켜 두는 프로그램의 메모리를 지킨다` 가 자료가 가장 두껍다(누수 글 하나에 원인·발견
방법·고침·물린 대안·교훈이 다 있다). `네이티브처럼 보이게 만든다` 는 GTK 재작성 글을 읽어야
층이 선다.

### ⚠️ 큐가 0/3 이다

Ghostty 를 완주시킨 뒤 **후보 조사 사이클을 끼운다.**
