...
**없다 — 문체 보수 사이클이었다.** Tailscale 을 배민 문체로 다시 썼다(2.1 → 0.0). 회사 115개, 큐 2/3. 48곳 남았다.

### 요금 단위가 아키텍처를 드러내는 세 번째 회사

Ably 는 연결과 채널을 시간으로 팔아 동시성 유지 자체가 원가임을 드러냈고, Temporal 은 액션을
워크플로가 한 걸음 나아갈 때 세며 저장을 활성과 보존으로 갈랐다. Tailscale 은 기기가 아니라
사람으로 센다. 무료 등급도 기기는 무제한이다.

이유가 구조에 있다(재구성). 트래픽이 중앙을 안 지나므로 기기가 늘어도 회사가 무는 값이 늘지
않는다. 제어 평면은 사실상 트래픽을 나르지 않고 실제 트래픽은 기기끼리 직접 간다. 그러니
기기로 값을 매길 근거가 없다.

요금표를 읽으면 시스템의 모양이 보인다는 것이 이제 셋에서 확인됐다.

### 이번 사이클 — Tailscale

중앙을 두되 그 중앙이 못 보게 만든 구조를 앞세웠다. 개인키가 절대로 노드를 떠나지 않고
릴레이 서버가 트래픽을 복호화할 길이 아예 없다.

일부러 두 구현을 이고 가는 결정도 살렸다. 본체를 조금씩 러스트로 바꾸는 길을 모든 세계의
최악이 될 것이라며 버리고, 두 구현을 나란히 두는 것을 이념적인 선택이 아니라 대가가 따르는
실용적인 선택이라 부른다.

영문이 서른 조각이었다. Temporal 25, Ably 21 에 이어 세 번째로 많았다. 한 문장이 여러 곳에
반복돼 나오는 경우가 많아 긴 것부터 치환하는 규칙이 특히 중요했다.
