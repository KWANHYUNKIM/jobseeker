...
**없다 — 문체 보수 사이클이었다.** 1Password 를 배민 문체로 다시 썼다(2.61 → 0.0). 회사 115개, 큐 2/3. 41곳 남았다.

### 평문을 누가 못 보게 하느냐 — 선을 긋는 위치가 셋 다 다르다

첫 기능에 축을 하나 세웠다. 같은 종류의 결정인데 신뢰에서 빼는 대상이 점점 커진다.

- Block 은 한 회사 안의 서비스가 서로의 필드를 못 읽게 메시지 안에 긋는다.
- LY 코퍼레이션은 Kafka 에 종단간 암호화를 얹어 브로커가 평문을 못 보게 한다.
- 1Password 는 회사 자신이 못 읽게 한다.

**앞의 둘은 시스템 안의 한 부품을 빼는 일이고 여기는 사업하는 주체를 통째로 빼는 일이다.**
그래서 이 회사만 팔 수 있는 기능이 줄어든다 — 복구도, 서버 측 검색도, 실제 자격증명으로 하는
유출 확인도 못 한다.

### 한 회사가 버린 대안을 다른 회사가 답으로 골랐다

둘째 기능이 Block 의 MeshLLM 과 정확히 맞물린다.

- Block 은 남의 기계에서 모델을 돌릴 때 프롬프트가 그 기계 주인에게 보인다는 것을 인정하면서
  기밀 컴퓨팅을 **버린 대안**으로 적고, 믿는 사람과만 공유하라는 문장으로 남겼다.
- 1Password 는 바로 그 길을 골라 봉인된 처리실을 지었다.

갈린 지점이 기술이 아니다. **안 풀면 팔 수 없느냐다** — 저쪽의 공개 메시는 실험이고 여기의
기업 리포팅은 매출의 75%다.

### 검증의 방향이 거울처럼 반대인 두 회사

셋째 기능을 LY 와 붙였다. 저쪽은 서버가 이 요청이 진짜 그 앱에서 왔는지 확인하고, 여기는
클라이언트가 이 공개 키를 서버가 제대로 준 것인지 확인한다. 한쪽은 서버가 클라이언트를,
다른 쪽은 클라이언트가 서버를 의심한다.

### 도메인을 제품이 아니라 제약이 조여 오는 순서로 잘랐다

개인 금고·기업 관리·개발자 도구로 가르면 같은 이야기가 세 번 나온다. 이 회사의 자료는 어느
것이든 제로 지식이라는 한 제약으로 돌아오기 때문이다. 그래서 **먼저 스스로 못 읽게 막고,
그다음 못 읽는데 처리해야 할 때를 풀고, 마지막으로 기업 자동화를 그 안에서 해내는** 순서로
경계를 잡았다는 것을 사업 모델에 적었다.

### 남은 흠 — country·category 가 `기타`다

1Password 의 국가와 분류가 둘 다 `기타`로 들어가 있다(허용값에 CA 와 SaaS 가 있다).
문체 보수 사이클은 새 조사를 안 하므로 손대지 않았다. **회사 자료로 소재지를 확인하는
보수 사이클이 따로 필요하다.**

### 다음 사이클 — 문체 보수 (사다리 3순위)

--gaps 가 정한다. 지금 맨 위는 ramp(2.56)지만 그건 이 시점의 예상이지 확정이 아니다.
