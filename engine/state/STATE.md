...
**Flipkart** (IN · 커머스) — **프로파일까지 썼다**(PROMPT 3단계). 도메인 3개 · 기능 0. 회사 **96개.**

### 이번 사이클 — 신규(6순위)

⚠️ **이 회사의 기술 이야기는 두 극단 사이에 걸쳐 있다** — 한쪽에 **`5억 고객을 위한 공급망 계획`** 같은 몇 달 단위의 예측이 있고, 다른 쪽에 **`초당 100만 이벤트에서 광고 예산 정확도 보장`** 같은 밀리초 단위의 정확성이 있다. **같은 회사가 두 시간 축을 동시에 다룬다**(재구성). 도메인을 그 결대로 셋 세웠다.

**규모** — 월간 활성 사용자 **2억 2,000만~2억 4,000만 명** · 판매자 **약 45만 곳** · 상품 **1억 5,000만 개 이상**(80개 넘는 범주) · **인도 이커머스 총거래액의 약 50~60%** · 2025년 매출 **약 86억 달러** · **월마트가 약 85% 보유.**

### ⚠️ 이번 프로파일의 약한 곳을 먼저 적는다

**도메인 셋이 전부 제목·날짜·URL 만 보고 세운 것이다.** ⚠️ **Meesho 때는 `각 도메인마다 글이 둘 이상 확인됐다` 를 근거로 삼았는데, 여기서는 공급망만 둘이고 나머지는 하나씩이다.** **첫 확장에서 본문이 기대에 못 미치면 도메인을 고친다** — QUEUE 에도 그렇게 적었다.

⏳ **요율표(Rate Card) 엔진 재설계 글**(2026-02-24)이 남아 있는데 **어느 도메인에 속하는지 본문을 봐야 안다** — 물류 요금이면 공급망, 판매자 수수료면 마켓플레이스다.

### 다음 사이클 — 확장(2순위)

`--gaps` 가 **`광고 예산을 초당 100만 이벤트에서 지킨다`** 를 부를 것이다. ⏳ **URL 은 목록에서 받아 뒀다**(추측 금지). ⭐ **Zepto 의 `하루 10억 이벤트 광고 분석` 과 나란히 놓을 자리다** — **한쪽은 세는 이야기, 한쪽은 지키는 이야기다**(재구성).

⚠️ **큐는 1/3**(Twilio). ⏳ **보강 거리 둘** — Grafana Labs 인용 대조 · Razorpay 보안 트리아지 글.

### ⚠️ 비교 문서 재료 (초안 유지)

**① `AI 에이전트를 어디까지 믿나` 9곳** — Sentry / ClickHouse / Duolingo / Ramp / DoorDash / Deliveroo / Snyk / Cygames / Razorpay(**격리를 커널에**).

**② `관리형 MySQL 의 한계` 3곳 + Cygames** — Etsy / Plaid / Paystack.

**③ `자기 성과를 어디까지 주장하나` 4곳** — Snyk / Grafana Labs / ScyllaDB / Razorpay.

**④ ⭐ `인도 규모에서 무엇이 달라지나` 3곳 + Flipkart** — Meesho / Zepto / Razorpay. **넷이 되면 가장 두꺼운 축이다.**
...
⏳ **⑫ 열릴 수 있다 — `광고를 세는 일과 지키는 일`** — **Zepto**(하루 10억 이벤트를 세어 대시보드와 과금에) / ⏳ **Flipkart**(초당 100만 이벤트에서 예산을 넘기지 않기).
