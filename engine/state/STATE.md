...
**Meesho** (IN · 커머스) — 도메인 4개 · **기능 3개.** 남은 도메인은 **`10억 대 기기의 인터넷에 물건을 보여 준다`** 하나다(⏳ 미독).

### 이번 사이클 — `깨끗한 이력을 만든 사기꾼은 이력으로 못 잡는다`

건진 것 넷:

- ⚠️ **이 기능의 이름이 나온 자리** — **정교한 사기꾼은 먼저 합법적인 이력을 쌓는다.** 그러면 이력만 보는 모델은 그를 정상으로 읽는다. **절제 실험이 그것을 숫자로 보여 준다 — RTO 이력이 전혀 없는 사용자 코호트에서 AUCPR 27 → 36.** **본인의 과거는 꾸며도 공유하는 계좌·기기·전화는 꾸미기 어렵다**(재구성).
- ⚠️ **반대 방향의 실패도 있다** — 그래프는 **사기가 갑작스러운 폭발로 나타나는 시간 변동에 약하다.** 회사의 설계 원칙: **`그래프 위상과 행동 시퀀스는 상보적인 정보를 담고 있고, 둘 다 필요하다.`**
- ⚠️ **이번 조사에서 가장 값진 문장** — **`소수 클래스 재현율에서 가장 큰 향상은 RTO 비율 가중 샘플링 전략에서 왔는데, 그것은 아키텍처 변경이 아니라 학습 트릭이다.`** **모델을 바꾸기 전에 무엇을 보여 줄지를 바꾸는 것이 더 컸다.**
- ⚠️ **좋은 모델이 고정된 지점이 아니다** — **오탐(정상 주문 차단)과 미탐(RTO 통과)의 비용이 운영 맥락에 따라 바뀌므로** 임계값을 주기적으로 재보정한다. **재학습은 6~8개월마다(회당 약 150~200달러)이고 그 사이는 임계값과 감시가 메운다.**

**수치** — p99 **50ms 미만**(피처 스토어 10ms + 추론 30ms) · 학습 1,060만/테스트 100만 · AUCPR 0.47 · 정밀도 68.3%(XGBoost 대비 +9.6%p) · **실제 A/B 24주(5%→95%)에서 고정 재현율 정밀도 +9.6%p, 운영비 5%p 절감.**

### ⚠️ 두 도메인이 같은 지점에서 만났다

**RTO 사기의 수법 중 하나가 가짜 주소로 보내게 하는 것**이고, 앞 사이클에 쓴 지오코딩이 하는 일은 **진짜 주소를 찾는 것**이다. **한쪽은 주소를 읽고 한쪽은 주소를 의심한다** — connections 에 재구성으로 이어 뒀다. ⭐ **회사 하나를 여러 사이클에 걸쳐 파야 보이는 연결이다**(딜리버루에서 두 글의 어긋남을 잡은 것과 같은 종류).

### ⚠️ 이 사이클에서도 지킨 선

**논문(AAAI 2026 채택)의 주소를 못 받아 `research.papers` 를 비웠다.** 블로그가 **전체 기술 세부는 논문에 있다**고 가리키지만 **그럴듯한 주소를 지어내지 않는다.** open_questions 에 남겼다.

### 다음 사이클 — 마지막 도메인, 그리고 완주 판정

`--gaps` 가 **`10억 대 기기의 인터넷에 물건을 보여 준다`** 를 부를 것이다. ⏳ **`r.jina.ai` + WebFetch(verbatim 인용 지정)로 읽는다:**
- `https://www.meesho.io/blog/the-pixel-gap-building-an-image-service-for-indias-billion-device-internet` (2026-06-24)
- `https://www.meesho.io/blog/the-right-pixels-optimising-images-at-meeshos-scale` (2026-08-25)

자료가 서면 **같은 사이클에서 완주 판정까지** 한다(도메인 4/4 · 고아 없음 · 수익원 셋 다 연결).

⚠️ **큐는 2/3**(Zepto · Snyk). ⏳ **Swiggy 는 브라우저가 살아나면 다시 본다.**
