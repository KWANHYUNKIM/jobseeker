...
**Razorpay** (IN · 핀테크 — 결제 게이트웨이) — **프로파일까지 썼다**(PROMPT 3단계). 도메인 3개 · 기능 0. 회사 **95개.**

### 이번 사이클 — 신규(6순위)

⚠️ **이 회사의 공개 기술 이야기는 결제 자체가 아니라 자기 비용과 사내 도구다**(2026년 글 기준) — **메트릭 비용 62% 절감 · CI 를 스팟 인스턴스로 · 데이터 웨어하우스 갱신 10배 · 보안 트리아지 750시간을 2시간으로.** **결제 규모가 커질수록 결제를 받치는 것들의 비용이 이야깃거리가 된다**(재구성). **도메인을 그 결대로 셋 세웠다.**

**규모** — 연환산 총결제액 **약 1,500억~1,800억 달러** · 가맹점 **1,200만 곳 이상** · **인도 온라인 결제 게이트웨이 시장의 약 55%** · 누적 거래 **74억 건** · 유지율 **94%** · 밸류에이션 **92억 달러** · **2026년 IPO 준비 중.**

### 읽어 둔 것 — 메트릭 글에서 건진 것 넷 (다음 사이클의 재료)

- ⚠️ **줄이기 전에 `아무도 안 쓰더라` 를 확인했다** — **경보 규칙과 대시보드를 감사해 분 미만 해상도에 의존하는 것이 있는지 봤고 하나도 없었다.** 그러고 나서 **스크레이프 간격을 30초 → 60초**로 바꿔 **인프라 메트릭 수집을 절반으로** 줄였다(하루 샘플 **약 4,500억 → 1,700억**).
- ⚠️ **도구를 통째로 바꾸는 길을 거절했다** — **팀 간 조율이 필요하기 때문.** 대신 **리더 선출 사이드카로 active-standby vmagent 쌍**을 만들어 **한 번에 하나만 수집**하게 했다. **대가는 페일오버 로직과 스플릿브레인 방지의 운영 복잡도** — **장애 유형에 따라 페일오버 7~22초, 리스 만료 15초에 갱신 2회 실패가 임계.**
- ⚠️ **각 팀에 요청하는 대신 중앙에서 버렸다** — **코드 릴리스를 기다리지 않고 vmagent relabel 규칙으로** 안 쓰는 고카디널리티 메트릭을 지웠다. **90일간 상위 500개 중 167개가 어디에도 안 쓰였고 하루 500억 샘플이 사라졌다.** **대가는 중앙 통제점 대 분산된 소유권**이다.
- ⚠️ **제목이 결론을 담는다** — **`대시보드를 하나도 잃지 않고`**. **관측 비용을 줄이는 이야기에서 가장 흔한 반론이 `그러다 못 보게 된다` 인데 그것을 먼저 막았다**(재구성).

### 다음 사이클 — 확장(2순위)

`--gaps` 가 **`관측을 싸게 유지한다`** 를 부를 것이다. **자료를 이미 읽었으니 바로 기능을 쓴다.**

⚠️ **큐는 2/3**(Flipkart · Twilio).

⏳ **보강 거리도 남아 있다** — **Grafana Labs 정본의 `mimir-next-gen` 과 중복 파일에서 확인했던 인용 대조.**

### ⚠️ 비교 문서 재료 (초안 유지)

**① `AI 에이전트를 어디까지 믿나` 8곳 + ⏳ Razorpay** — Sentry / ClickHouse / Duolingo / Ramp / DoorDash / Deliveroo / Snyk / Cygames. ⏳ **Razorpay 의 `망에서 격리된 사내 AI` 는 금융 규제 아래의 답이라 새 자리다**(재구성).

**② `관리형 MySQL 의 한계` 3곳 + Cygames(TiDB)** — Etsy / Plaid / Paystack.

**③ `자기 성과를 어디까지 주장하나` 3곳** — Snyk / Grafana Labs / ScyllaDB.

**④ `인도 규모에서 무엇이 달라지나` 2곳 + Razorpay · ⏳ Flipkart** — Meesho / Zepto. ⭐ **넷이 되면 가장 두꺼운 축이 된다.**
...
⏳ **⑩ 새로 보인다 — `관측 비용을 어디까지 줄이나`** — **Razorpay**(쓰는지 확인하고 줄인다) / **Grafana Labs**(파는 쪽에서 TCO 를 줄인다) / **Honeycomb**. ⭐ **사는 쪽과 파는 쪽이 같은 문제를 반대편에서 본다**(재구성).
