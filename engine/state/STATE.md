...
**없다 — Razorpay 완주.** 도메인 3/3 · **기능 3개.** 회사 **95개.** ⚠️ **큐 2/3**(Flipkart · Twilio).

### 이번 사이클 — `격리는 애플리케이션이 아니라 인프라의 속성이다`

⭐ **`AI 를 어디까지 믿나` 축에서 아홉 번째이자 또 하나의 새 답이다.**

건진 것 넷:

- ⚠️ **노트북을 버린 이유가 셋이다** — **사람이 있을 때만 돌고, 관측이 없고, 지속적인 맥락이 쌓이지 않는다.** 서버로 올리자 **주인이 자는 동안에도 돈다** — 기록된 한 사용자의 에이전트가 **8주에 15,039 세션**을 돌렸고 그중 **13,570 건(약 90%)이 자거나 회의 중일 때**였다.
- ⚠️ **이 기능의 이름이 회사의 한 문장이다** — **`격리는 인프라의 속성이지 애플리케이션의 속성이 아니다.`** **애플리케이션 코드로 막으면 에이전트가 영리해질수록 우회 가능성이 생기는데, 커널에서 강제하면 그 영리함이 소용없다.** **행동을 제한하는 대신 환경을 제한한다**(재구성).
- ⚠️ **나가는 길을 둘로 좁혔다** — **감사 프록시와 DNS 만 허용하고 테넌트 간 트래픽은 기본 거부.** **나가는 길이 하나뿐이면 그 길만 보면 된다**(재구성) — **월 1,500만 건을 LLM 기반 프록시로 훑어 21만 9천 건을 막고 모든 호출을 실명에 귀속시킨다.**
- ⚠️ **인프라의 단순함은 어딘가에서 값을 치른다** — **문서화되지 않은 `골든 핵` 넷**(자격증명 권한 · WebSocket 로컬호스트 릴레이 · 워크스페이스 복구 · 다중 라이브러리 CA 신뢰)이 있어야 잠긴 환경에서 강화 이미지가 돌았다. **회사가 그것을 공개한 것 자체가 이 자료의 값이다**(재구성).

### ⭐ ClickHouse 와 정확히 대비되는 자리

**ClickHouse 는 `세 겹 중 한 겹만 기계가 강제한다` 고 적었다.** ⚠️ **Razorpay 에서는 기계가 강제하는 그 겹이 커널이다.** **같은 축의 양 끝**이라 비교 문서 ①에 그 대비를 적어 뒀다.

### ⚠️ 이 회사의 세 기능이 한 방향을 가리킨다

**`막을 수 없으면 크기를 묶는다`** — 관측은 **쓰는지 확인하고 줄이되 실패는 공백 쪽으로**, CI 는 **깨질 걸 알면서 20% 상한으로**, 에이전트는 **커널이 나가는 길을 둘로 좁혀서**. ⭐ **회사 하나를 세 사이클에 걸쳐 파야 보이는 결이다**(재구성).

### 다음 사이클 — 신규(6순위)

`--gaps` 가 **큐 맨 위(Flipkart)를 `in_progress` 로 만들고 3단계부터** 를 부를 것이다. ⚠️ **`blog.flipkart.tech` 는 `r.jina.ai` 를 앞에 둔다.**

⏳ **보강 거리 둘** — **Grafana Labs 정본과 중복 파일 인용 대조** · **Razorpay 의 보안 트리아지 글**(750시간 → 2시간, 미독).

### ⚠️ 비교 문서 재료 (초안 유지)

**① ⭐ `AI 에이전트를 어디까지 믿나` 9곳 — 양 끝이 찼다** — Sentry / **ClickHouse(세 겹 중 한 겹만 기계가)** / Duolingo / Ramp / DoorDash / Deliveroo / Snyk / Cygames(**에이전트끼리 서로 비판**) / **Razorpay(격리를 커널에)**.

**② `관리형 MySQL 의 한계` 3곳 + Cygames** — Etsy / Plaid / Paystack.

**③ `자기 성과를 어디까지 주장하나` 4곳** — Snyk / Grafana Labs / ScyllaDB / **Razorpay(실패담을 적는다)**.
