...
**없다 — 일흔한 번째 후보 조사를 돌았다. 큐 1/3.** 다음 사이클은 **신규**(쿡패드).

### 사이트맵부터 여는 습관이 자리를 잡았다

앞 두 사이클에서 **첫 화면 함정**에 연속으로 걸릴 뻔했는데(뱅크샐러드 3편/실제 58편,
카카오페이 5편/실제 162편), 이번에는 **처음부터 사이트맵을 열었다.** 쿡패드는 **월별
사이트맵이 2008년부터 2026년까지** 뻗어 있어 **17년치**라는 것이 바로 보였다. RSS 만
봤으면 최근 30편으로 판단했을 것이다.

### 물린 안을 이름 대어 적는 글

쿡패드가 통과한 근거다. 인터리빙을 무한 스크롤과 함께 쓸 때 **같은 문서가 두 번 나오는**
문제를 놓고, **'1페이지일 때만 인터리빙'** 과 **'전체 페이지 캐시'** 를 각각 왜 못 쓰는지
적는다. **첫 질의는 ES 에 직렬로 두 번** 물어야 한다는 대가도 밝힌다.

### 다만 결과가 인상으로 끝난다

**'캐시 비대화도 문제가 되지 않았고 특별한 지연 증가도 인정되지 않았습니다'** — **수치가
없다.** 대가는 구조로 설명하는데 값은 재지 않는 형태로, 이 원장에서 자주 보는 모양이다.

### 못 찾은 것과 없는 것을 가른다

**오늘의집**은 홈에 기술 블로그 링크가 없고 `/tech/` 는 404 다 — **자료가 없는 것이 아니라
입구를 못 찾은 것**으로 적었다. **엔씨소프트**도 `ncsoft.github.io` 가 404 다. **한국
게임사 축이 통째로 비어 있는데**(넥슨·엔씨·크래프톤) 아직 열지 못했다.

### 다음 사이클

**신규 — 쿡패드.** 큐 1/3 이라 신규 한 번 뒤에 조사가 다시 뜬다.
