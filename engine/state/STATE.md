...
**없다 — 문체 보수 사이클이었다.** Klaviyo 를 배민 문체로 다시 썼다(2.16 → 0.0). 회사 115개, 큐 2/3. 32곳 남았다.

### 시계가 여럿인 파이프 — 시계를 쥔 쪽과 못 쥔 쪽

첫 기능을 Paystack 과 붙였더니 정반대가 나왔다.

- Paystack 은 채널마다 다른 시계(임시 계좌 15분~8시간, Pesalink 25분, 모바일 머니 180초)를
  **자기가 정하지 못해서** 문서에 그대로 적고 한계를 고객에게 알리라고 말한다.
- Klaviyo 는 시계를 **자기가 쥐고 있으므로** 마감이 이른 것부터 꺼내는 규칙(EDF)으로 줄을 다시 세운다.

문제의 모양은 배민의 배차와 닮았다. 늦으면 음식이 식고, 늦으면 알림이 쓸모없어진다. 다른 것은
무엇이 밀리느냐다 — **배민은 사람과 차가 밀리고 여기는 20분짜리 집계가 5초짜리 알림 앞을 막는다.**

### 반드시 한 번은 가야 하는 메시지 — 그런데 돌려줄 곳이 없다

둘째 기능에 축을 세웠다.

- 배민의 주문 접수는 반드시 한 번만 울려야 한다.
- Paystack 은 거의 모든 채널이 비동기라 웹훅을 전제로 깐다.
- Moniepoint 는 일시적 실패가 서비스를 튕겨내지 않게 헬스체크를 직접 만들었다.
- Klaviyo 의 제약이 그중 가장 좁다 — **API 가 던지고 잊는 방식이라 실패를 돌려줄 곳이 아예 없다.**

전에는 발행자가 로그를 남겼는데도 발행 실패가 곧 유실이었다는 것을 회사가 함께 적는다.
**로그는 남았지만 이벤트는 없어졌다는 뜻이다.**

### 도메인을 제품이 아니라 한 파이프에서 무엇이 어긋나느냐로 잘랐다

이메일·SMS·CDP 로 가르면 이 회사가 공개한 것이 전부 한 칸에 들어간다. 하나는 순서의 문제이고
다른 하나는 존재의 문제다 — **늦게 가는 것과 아예 안 가는 것은 다른 실패다.**

### 다음 사이클 — 문체 보수 (사다리 3순위)

--gaps 가 정한다. 지금 맨 위는 clickhouse(2.15)지만 그건 이 시점의 예상이지 확정이 아니다.
