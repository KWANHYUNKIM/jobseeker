...
**없다 — 문체 보수 사이클이었다.** Meesho 를 배민 문체로 다시 썼다(2.2 → 0.0). 회사 115개, 큐 2/3. 46곳 남았다.

### 커미션을 안 받겠다는 결정이 기술 문제를 바꾼다

Meesho 는 인도 커머스 중 처음으로 판매 수수료를 없앴고 물류 이행 수수료와 광고로 번다.
그 결정이 도메인 넷 중 둘을 직접 만든다.

- 거래마다 떼는 몫이 없으니 정산 화면이 얼마를 뗐는지가 아니라 왜 이만큼인지를 설명해야 한다.
  공제가 물류·세금·광고·인플루언서 마케팅·대출 상환에서 따로 오는데 화면에는 총 미결제
  잔액 한 줄뿐이었다. 매일 14만 명에게 돈이 나가면서.
- 물류를 직접 하니 인도 주소를 읽는 일이 이 회사의 문제가 된다. 통일된 형식이 없어
  뭄바이·첸나이·시골 비하르 주소가 서로 전혀 다르게 생겼다.

사업 모델을 배경으로 적고 도메인을 따로 세우는 회사가 많은데, 여기는 사업 모델이
도메인 경계를 그은 흔치 않은 경우다(재구성).

### 한 신호를 못 믿을 때 여럿의 흔적을 믿는다 — 인도에서 두 번

주소 축에 Zepto 와의 대비를 넣었다. 같은 나라 같은 문제인데 방향이 반대다.

- Zepto 는 GPS 가 라이더를 세 블록 떨어진 호수로 순간이동시켜서, 여러 사람이 다닌 길로
  좌표를 고쳤다. 기계 신호를 못 믿고 사람의 흔적으로 고친 것이다.
- Meesho 는 주소 텍스트를 못 믿어 배달 기사가 실제로 간 곳을 정답으로 삼는다.
  V1 은 텍스트만 읽었고 V2 는 기사가 다닌 길을 함께 읽는다.

둘 다 한 번의 신호를 버리고 여럿의 흔적을 택했다.

### 사기 쪽은 벽이 여섯이다

착불이 기본인 시장에서 가장 아픈 남용은 반송이다. 반송이 10% 미만이라 표본이 불균형하고,
수법이 바뀌며, 주체가 여럿이고, 사기꾼이 깨끗한 이력을 먼저 쌓으며, 신규 사용자는 주문이
두세 건뿐이고, 갑자기 폭발한다. 이력으로 잡는 방법이 통하지 않는 이유가 넷째 벽에 있다.

### 다음 사이클 — 문체 보수 (사다리 3순위)

--gaps 가 정한다. 지금 맨 위는 razorpay(2.81)지만 그건 이 시점의 예상이지 확정이 아니다.
