...
**없다 — 예순두 번째 후보 조사를 돌았다. 큐 1/3.** 다음 사이클은 **신규**(Mozilla).

### 비대칭 비용을 한 문장으로 적는다

Mozilla 의 Firefox 하드닝 글에서 가장 값어치 있는 대목이다 — **"그럴듯해 보이지만 틀린
보고를 다루는 일은 프로젝트 관리자에게 비대칭적인 비용을 지운다: LLM 에게 코드에서
'문제'를 찾으라고 시키는 것은 싸고 쉬운데, 거기에 대응하는 것은 느리고 비싸다."** 도구가
싸질수록 **받는 쪽의 부담이 커진다**는 구조를 정확히 짚는다.

### 자기 코드를 AI 로 감사한 기록

이 엔진에 새로운 종류의 자료다. 퍼징 인프라 위에 **에이전틱 하네스**를 얹어 Firefox 150 에서
**버그 271개**(sec-high 180)를 찾았고 **15년 된 `<legend>` 버그와 20년 된 XSLT 취약점**까지
나왔다. 물린 것도 적는다 — GPT-4·Sonnet 3.5 로 한 정적 분석은 **오탐이 많아 규모를 키우기에
비현실적**이었다.

### 축을 벌리려 했는데 세 곳이 연달아 막혔다

최근 통과가 전부 데이터 인프라라 일부러 다른 곳을 골랐는데 — **Planet Labs**(위성, 완전히
빈 축)는 홈에 엔지니어링 블로그 링크가 없고, **OpenAI** 와 **GitLab** 은 **403** 이다.
GitLab 은 특히 아쉽다(깃허브·소스헛과 마주 보는 자리인데 못 읽었다).

### 403 이 쌓여 간다

Lemonade · OpenAI · GitLab, 앞서 Blender · Sourcegraph. **이 도구로 못 여는 곳의 목록이
길어지고 있다** — 접은 것과 계속 갈라 적는다.
