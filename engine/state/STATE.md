...
**없다 — 엠쓰리를 완주했다(도메인 2 · 기능 2).** 큐 0/3 이라 다음 사이클은 **후보 조사**다.

### 가른 기준은 '지금 멈출 수 있느냐'였다

같은 회사가 **남이 정한 수명**(EOL·deprecated) 앞에서 두 갈래로 갔다. **없앨 수 있으면
지우고**(Lambda), **멈출 수 없으면 겹으로 만든다**(과금·시청 기록). **두 도메인을 가른 것이
기술이 아니라 '멈출 수 있느냐'** 라는 것이 이 프로파일의 알맹이다.

### 넓히는 장치를 줄이는 데도 쓴다

**'플래그를 넘긴 뒤의 정리가 한꺼번에 하는 작업이 되기 쉬운데, 정리 쪽도 단계적으로 할 수
있는 형태로 해 두니 끝까지 마음이 편했다.'** 대부분은 전환용 플래그만 만들고 **뒷정리는
한꺼번에** 하는데, 그것이 **가장 위험한 순간**이라는 것을 알고 미리 쪼갰다.

### 재시도의 상한은 관측 창이 정한다

늘릴 수 없으니 **전송을 4.5초 앞당겨** 여유를 **시간 축에서 벌어 왔다.** 그리고 두 번째 겹은
새로 만들지 않고 **이미 1분마다 흐르던 경과 시간 로그**를 판정에 끌어다 썼다.

### 이상이 아직 없다는 것을 적는다

**'서버 사이드 시청 추적이 이상적인데 아직 만들지 않았다'** — 지금 것이 최선이 아니라는
것을 남기면 **다음 사람이 어디를 볼지** 안다. 첫 실패의 **55%를 회수**한다는 말은 곧
**나머지는 보조 판정에 기댄다**는 뜻이기도 하다.

### 열어 둔 채로 완주했다

**의료 도메인 자체의 기술 글은 못 봤다** — 두 도메인이 다 **일반 인프라·과금**이다.
규제(의료 정보 취급)가 기술에 어떻게 걸리는지가 열린 채로 남았다.
