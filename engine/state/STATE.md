...
**없다 — 후보 조사 사이클이었다.** 회사 **108개**, 큐 **0 → 3/3.**

### 이번 사이클 — 서른한 번째 후보 조사

⭐ **`큰 이름인데 아직 없는 회사` 각도가 네 조사 연속 3곳을 채웠다.** Coinbase → Atlassian·Neon·Vercel → Cursor → 이번 셋. ⚠️ **지역 축과 언급 각도가 말랐던 것과 대비가 이제 뚜렷하다** — **이 엔진이 108개를 파는 동안 정작 큰 이름들이 남아 있었다**(재구성).

올린 세 곳(근거는 QUEUE 대기 표에):

- **✅ Block (Square)**(US · 핀테크) — ⭐ **사내 에이전트 플랫폼 `Buzz` 를 다섯 편 연작으로 쓴다**(MeshLLM 컴퓨트 공유 · 릴레이 · 토큰 효율 · 에이전트 설정 · 프로젝트). ⭐ **결제·비트코인 하드웨어 펌웨어(COLDCARD RNG 폴백)·사내 에이전트가 한 회사 안에 있어 도메인이 저절로 갈린다.** 최신 2026-09-01.
- **✅ Klaviyo**(US · SaaS) — ⭐ **`카프카로 분산 우선순위 큐 만들기` — 이 엔진에 없는 축이다**(큐는 많지만 우선순위를 붙인 이야기는 없다). ⚠️ **목록 날짜에 연도가 없다**(`Apr 23`) — **파기 전에 개별 글에서 연도를 확인한다.**
- **✅ Perplexity**(US · 검색) — ⭐ **자체 서빙 엔진 `ROSE` 를 만들고, 유연한 곳은 파이썬·PyTorch, 빠른 곳(서빙 로직·배치 스케줄링)은 러스트로 언어를 가른다.** **RDMA 대역폭 효율 97.1%, 2025-11 arXiv 논문, `fabric-lib`·`Lily` 오픈소스.**

### ⭐ 판정을 미루는 것보다 닫는 것이 낫다 — Supabase 를 접었다

**세 사이클을 끌었다.** ⭐ **`첫 화면으로 판정하지 않는다` 는 규칙대로 `engineering` 카테고리를 따로 열고, 거기서도 확신이 안 서 `OrioleDB Public Alpha` 개별 글까지 열었다.** 결과는 **없다** — 본문에 스토리지 엔진 내부가 없고 **`Postgres 의 플러그형 스토리지를 쓰는 확장`이라는 소개와 외부 링크뿐**이다. ⚠️ **핵심 기술 문서가 OrioleDB 팀 쪽에 있지 Supabase 자신의 글이 아니다.**

⭐ **여기서 `두 번째 자료를 찾는다` 가 반대로 값을 했다** — 보통은 얇아 보이는 회사를 살리는 규칙인데, 이번엔 **없다는 것을 확정하는 데 썼다.** ⚠️ **⏸️ 로 남겨 두면 매 사이클 비용이 다시 든다.** 같은 이유로 Railway 도 404 를 보고 바로 접었다.

### 다음 사이클 — 신규 회사 (6순위)

**큐 3/3 이 찼다.** ⭐ **Block 부터 판다** — 셋 중 도메인이 가장 뚜렷하게 갈리고(결제 · 하드웨어 펌웨어 · 사내 에이전트) 글이 가장 두껍다. ⚠️ **Klaviyo 를 고를 때는 연도 확인이 먼저다.**

⏳ **보강 거리** — Cursor(MoE 메가커널 · AIUC-1 · Firetiger) · Vercel(Run SDK · v0 OAuth) · Neon(브랜칭 심화) · Atlassian(Events Rail) · Trendyol(Helyx 4편) · Grafana Labs 인용 대조 · Razorpay 보안 트리아지 · Flipkart Rate Card · Pinterest 2부 · WarpStream 미독 셋.
