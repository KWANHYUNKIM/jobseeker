...
**없다 — 문체 보수 사이클이었다.** Honeycomb 을 배민 문체로 다시 썼다(2.8 → 0.0). 회사 115개, 큐 2/3. 44곳 남았다.

### 남들이 비용이라 부르는 것을 상품 쪽에 놓았다

관측을 파는 회사가 이 엔진에 셋이 됐는데 값을 매기는 자리가 서로 다르다.

- VictoriaMetrics 는 비용을 짓누르는 것이 샘플 양이 아니라 카디널리티라고 말한다.
- Datadog 은 제품을 여러 갈래로 나눠 판다.
- Honeycomb 은 좌석도 쿼리도 안 받고 이벤트 수로만 받는다. **고카디널리티를 아무 조합으로나
  훑는 것이 곧 제품이라, 남들이 비용이라 부르는 것을 여기서는 무제한으로 연다.**

요금 구조가 기술 요구를 정하는 회사라 도메인도 파는 것이 아니라 그 약속을 떠받치는 것으로 잡았다.
제품 화면(트레이스·SLO·경보)을 도메인으로 세우지 않은 이유를 사업 모델에 적어 뒀다.

### 안 만든 것이 특징인 설계 — 무엇을 안 읽을지 정하는 회사들과 반대다

첫 기능에 대비 축을 세웠다. 같은 문제를 셋이 반대 방향으로 푼다.

- Datadog 은 100조 개를 안 읽는 방법을 질의 계층에 넣었다.
- ClickHouse 는 엔진을 오픈소스로 뿌려 그 판단을 남에게 넘긴다.
- Honeycomb 은 색인도 스키마도 사전 집계도 없이 다 훑는다.

**그 대가가 예상 못 한 축에서 돌아왔다** — 데이터의 양이 아니라 고객의 서비스 수가 늘었을 때
세그먼트 디렉터리가 함께 늘어 질의가 느려졌다.

### 도는 것을 갈아 끼우는 세 번째 답 — 무중단을 포기한다

둘째 기능이 기존 축에 새 자리를 낸다.

- Atlassian 은 무중단을 지키되 일부러 느리게 옮겼다.
- Modal 과 Turso 는 아예 처음부터 다시 지었다.
- Honeycomb 은 생산자 전환과 소비자 전환 사이에 멈추는 창을 받아들이고 데이터를 지켰다.

관측을 파는 회사가 잠깐 안 보이는 것보다 잘못 보이는 것을 더 무서워한 셈이다.
회사가 자기 시스템이 나빠진 구간(브로커 교체 8~12시간 → 48~72시간)을 그대로 적는 것도 같은 태도다.

### 다음 사이클 — 문체 보수 (사다리 3순위)

--gaps 가 정한다. 지금 맨 위는 sentry(2.71)지만 그건 이 시점의 예상이지 확정이 아니다.
