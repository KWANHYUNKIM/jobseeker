...
**없다 — 문체 보수 사이클이었다.** PayFit 을 배민 문체로 다시 썼다(2.7 → 0.0). 회사 115개, 큐 2/3. 50곳 남았다. 절반을 지났다.

### 지금까지와 다른 문제가 나왔다 — 사이클 로그가 데이터에 섞여 있었다

PayFit 의 business_model 과 도메인 why 에 조사 과정이 그대로 적혀 있었다. 어느 사이클에서
무엇을 정정했고 검색 예산이 언제 떨어졌는지 같은 것들이다. 읽는 사람에게는 뜻이 없는 내부
로그인데 사업 설명 자리에 있었다.

내용은 지우지 않았다. 사업 이야기는 business_model 에 남기고 조사의 한계는 open_questions
로 옮겼다. 문체 보수의 원칙이 문장만 바꾸고 사실은 남기는 것이므로 자리를 옮기는 것까지가
할 수 있는 일이다.

사이클 번호도 걷어냈다. 354 에서, 356 에서, 334→335 의 교훈 같은 표현은 이 엔진 안에서만
뜻이 있다. 조사를 이어 가며 같은 말로 바꿨다.

그리고 이탤릭 인용이 남아 있었다. 변환기가 굵게만 걷고 별표 하나로 감싼 인용은 안 걷는다.
정규식을 하나 더 걸어 처리했다.

### 이름이 곧 경계인 회사

PayFit 의 답이 특이하다. 급여 도메인의 경계를 다시 그으면서 코드가 아니라 이름부터 정했다.
Pay Period 라는 용어를 팀마다 다르게 쓰던 것이 증상이었고, 급여 명세가 내부 구현 상세로
여겨지던 동안에는 그것이 별도의 경계라는 것이 안 보였다.

같은 문제에 이 엔진의 다른 둘이 다르게 답했다. freee 는 떼어냈고 SmartHR 은 규칙으로 긋다
안 되어 프로세스로 나눴다. 셋 다 급여·인사 SaaS 이고 셋 다 제품이 늘며 경계가 무너졌다.
