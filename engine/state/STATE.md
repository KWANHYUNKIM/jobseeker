...
**Databricks** — 기능 **1/2.** 회사 113개. 큐 1/3. ⚠️ **남은 도메인 하나**(`카디널리티에서 시계열 DB 를 지킨다`)를 채우면 완주다.

### 이번 사이클 — `쓰기에서 전체 페이지를 빼고 읽기를 유계로 지킨다`

⭐ **결정 사슬이 세 단계로 완전히 드러난 드문 자료다:**

1. **왜 끄나** — **`레이크베이스 구조에는 찢어질 로컬 디스크 페이지가 없으므로, 전체 페이지 쓰기가 막으려던 실패 모드 자체가 존재하지 않는다.`** ⚠️ **상속받은 안전장치가 새 구조에서는 순수한 비용이었다**(재구성). **WAL 이 트랜잭션당 58KB → 4KB 미만.**
2. ⚠️⚠️ **그런데 그냥 끄면 읽기가 무너진다** — **`로그에 주기적인 전체 페이지 이미지가 없으면, 스토리지 계층이 페이지를 복원하려고 끝없이 긴 작은 델타 사슬을 재생해야 하고, 체크포인트 빈도에 묶인 유계 재생 대신 무계 재생 사슬이 된다.`** ⭐ **전체 페이지 쓰기는 쓰기 비용이 아니라 읽기 상한을 사는 값이었다**(재구성).
3. ⭐ **그래서 끄는 게 아니라 옮긴다** — **이미지 생성 푸시다운.** **`지능을 컴퓨트 노드에서 스토리지 계층으로 옮긴다.`** 페이지서버가 **`이미지 없이 임계값보다 많은 델타 레코드가 쌓이면`** 전체 이미지를 만든다.

**결과가 한쪽만 좋아지지 않았다** — **쓰기 4.5배**(32-vCPU)와 **읽기 P99 30~50%·P50 약 30% 개선**이 함께 왔다. 어느 고객은 **WAL 초당 30MB → 1MB**, 수집 **17,000 → 62,000행/초.**

### ⭐ 이번에 건진 가장 큰 것 — 판단의 위치를 옮기는 것이 답일 때가 있다

**컴퓨트는 무엇이 쌓였는지 모르고 페이지서버는 안다**(재구성). ⭐ **그리고 기준도 바꿨다** — **`임의의 체크포인트가 아니라 실제 변경이 쌓인 정도에 따라`** 만든다. **안 바뀌는 페이지에는 이미지를 안 만든다.**

⚠️ **대가 셋을 그대로 적었다** — **임계값이라는 새 손잡이**(낮으면 이미지 쓰기가 늘고 높으면 사슬이 길어진다), **`컴퓨트 계층의 복잡성을 얼마간 받아들인 것`**, 그리고 전제 하나: **`분산 스토리지가 따로 확장할 수 있어 단일 노드 Postgres 가 감당 못 하던 복원 작업을 흡수하기에 성립한다.`** ⭐ **뒤집으면 단일 노드에서는 이 거래가 성립하지 않는다.**

### ⚠️ 문서 주소도 추측하면 404 다

`docs.databricks.com/.../branching` 을 짐작해 열었더니 없는 글이었다. ⭐ **`architecture` 페이지는 검색으로 얻은 주소였다.** **문서 사이트라고 주소 규칙을 넘겨짚지 않는다.** `open_questions` 에 남겼다.

### 다음 사이클 — 확장, 그리고 완주 (2순위)

**Databricks 기능 1/2.** ⏳ 마지막 — **`카디널리티에서 시계열 DB 를 지킨다`**(Thanos 포크 Pantheon · 하루 10조 샘플 · 활성 시계열 50억 · 다운타임 5분의 1 · **레이크하우스로 고차원 되찾기**).

⚠️ **완주 판정 때 수익원 넷**(플랫폼 사용량 · 데이터 웨어하우징 · AI 제품 · Lakebase)**이 기능과 이어지는지 본다.** ⏳ 못 읽은 것 — `Specialized GPU Kernel Generation`(⭐ **AI 제품이 매출의 26%인데 그 원가 쪽**).
