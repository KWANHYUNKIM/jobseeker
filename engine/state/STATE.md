...
**Convex**(소재지 미확인 · 백엔드 플랫폼) — 회사 프로파일까지 썼다. **도메인 둘 · 기능 0.** 큐 0/3.

### 쉬움을 어디서 만들고 어디서 거두는지가 갈린다

파는 것은 **'복잡한 백엔드 엔지니어링 없이'** 다. 그런데 그 쉬움을 **트랜잭션 쪽에서
만들고**(사용자 코드를 DB 안에서 직렬화 가능하게 돌린다 · 읽기 집합을 그대로 구독에 써서
반응성이 따라온다) **싸 보이는 연산 쪽에서 거둬들인다**(`select` 도 `count()` 도 없다 ·
카운터를 손으로 올려라). **한 회사 안에서 방향이 반대인 두 도메인**이다.

### 공짜로 얻은 것에는 조건이 붙는다

반응성이 공짜인 이유는 **트랜잭션이 이미 읽기 집합을 쌓기 때문**이다. 대신 함수가
**결정적**이어야 한다 — 결과가 **'인자와 데이터베이스 읽기만으로 완전히 정해져야'** 하고,
그래서 **뮤테이션에서 `fetch` 를 막고 V8 의 거동까지 통제**한다. **바깥을 부르는 일은
다른 자리로 밀려난다.**

### 값을 기능이 아니라 운영에서 받는다

무료 단에 **벡터 검색·텍스트 검색·데이터 지역 선택**까지 넣고, 돈은 **백업·로그 스트리밍·
규정 보고서·SLA** 에서 받는다. 개발자당 월 $25, 기업은 월 최소 $2,500.

### 소재지를 정하지 않았다

법인명 **Convex, Inc.** 와 저작권 표기뿐이고 소개·투자자·약관에 주소가 없다(약관 본문은
JS 로만 그려진다). **투자자가 미국 인사들이라는 것으로 국가를 정하지 않았다** — 고도에서
한 번 겪은 실수라 '기타'로 뒀다.

### 다음 사이클

확장 — 빈 도메인 둘 중 하나. **트랜잭션 쪽이 근거가 두껍다**(OCC 커밋 규약 · 읽기 집합
재사용 · 결정성 강제 · 직접 만든 것과 V8 을 가른다). 큐가 0/3 이라 그 뒤에 후보 조사가 뜬다.
