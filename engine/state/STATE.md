...
**뱅크샐러드**(KR·서울 마포 · 핀테크/마이데이터) — 회사 프로파일까지 썼다.
**도메인 둘 · 기능 0.** 큐 1/3.

### 규제가 제품을 바꾼 자리가 있다

**마이데이터**로 오면서 **'스크래핑으로 데이터를 연결하던 환경을 API로 전환하는 대규모
프로젝트'** 를 치렀고, 그 과정에서 **자체 인증서**까지 만들었다. **제도가 기술 축을 갈아
치운 사례**라 한국 회사에서만 볼 수 있는 자리다.

### 비용을 옮기면 다른 비용이 올라온다

MySQL 저장비가 커져 S3 로 옮겼더니 **조회가 262,800 오브젝트**가 되어 API 요금이
폭증했다. **user_id 버켓팅 + 일 단위 파티션**으로 **365개**로 줄여 질의비를 **약 700배**
낮췄는데, **거기서도 새 비용이 올라온다**(스파크·아테나 불일치 → CTAS 임시 테이블,
executor 마다 버킷당 파일 하나 → 파일 폭증).

### 락으로 막지 않고 도메인 성질로 넘긴다

낙관적 락을 고른 근거가 **기술이 아니라 도메인**이다 — **'특정 유저의 데이터를 수많은
타인이 동시에 수정할 일이 없으며'**, **'게임에서 가장 중요한 것은 유저의 흐름이 끊기지
않는 것.'** 그리고 보상을 **시간 차로 계산해 놓친 갱신이 자연히 따라잡히게** 만들어
**재시도 복잡도를 구조로 없앴다.**

### 같은 회사 안에서 글마다 두께가 크게 다르다

버켓팅 글은 **물린 것 셋(STS·Delta·INSERT INTO)과 대가 둘**을 다 적는데, `dataserving`
글은 **DocumentDB 를 고른 이유도 비용도 실패 양상도 수치도 없다.** **회사 단위로 '자료가
두껍다'고 말하면 안 된다**는 기준이 한 회사 안에서도 성립한다.

### 다음 사이클

확장 — 빈 도메인 둘 중 하나. **버켓팅 쪽이 수치가 촘촘하고**(262,800 → 365 · 700배 ·
3배), 낙관적 락 쪽은 **물린 대안 둘과 결과 0건**이 있다. 후보 글이 열 편 넘게 남았다.
