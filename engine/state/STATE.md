...
**없다 — SourceHut 을 완주시켰다(도메인 2 · 기능 2 · `status: done`).** 회사 155개, **큐 0/3.**

### 이번 사이클이 채운 것 — `publish-the-wreckage`

무너진 날(170시간)과 **무너질 뻔한 날**(디스크 4.6GiB)을 둘 다 적어 낸다. 뒤엣것은 장애가
아니었는데도 사후 기록으로 냈다 — **GC 크론잡을 넣으면서 스냅숏 보존 정책을 안 고쳤다**는
자기 실수까지 공개 기록으로 남긴다.

### 경로 결정이 남의 손에 있다

상류(Cogent·Level 3)가 공격에 대응해 **서브넷 전체를 널 라우트로 막으면** 아래에서 할 수 있는
일이 없다. 게다가 제공자의 티켓 시스템은 인수를 거치며 폐기돼 연락도 안 닿았다. 그래서 기다리는
대신 대서양 건너로 7일에 걸쳐 옮겼다.

### 지표는 있는데 눈먼 자리가 생긴다

저장 모형이 덧붙이기에서 삭제로 바뀌자 스냅숏끼리 차이가 커졌는데, 그 변화는 며칠에 걸쳐
나타나 **15일치 보존으로는 안 보였다.** 고친 것은 보존을 60일로 늘린 것뿐이고, **저장 지표
세분화와 정책 재검토는 미뤘다고 스스로 적는다.**

### 원칙의 예외도 적는다

100% 자유 소프트웨어를 내세우는데 긴급 경보는 **트윌리오와 Automate 라는 비공개 소프트웨어**에
기댄다. 그 사실을 적고 오픈소스 대안 제안을 받는다 — 바꾸기 어렵다고 밝히면서.

### 다음 사이클 — ⚠️ 후보 조사 (큐가 0/3 이다)

판정 기준을 이어 쓴다 — 회고를 먼저, 되돌린 기록, 자기 것을 쓰지 말라고 적는 글, 사고를
공개하는 글, 상류에 고쳐 보낸 기록. **형식이 아니라 대가가 기준이고, 본문을 열어 판정한다.**
지난 조사에서 **못 읽은 곳 셋(incident.io 404 · Sourcegraph 403 · Fireworks 본문 미확인)**을
다시 시도해 볼 수 있다. `name_en` 155개를 먼저 출력해 중복부터 확인한다.
