...
**헨리(Henry)** — 첫 기능 `one-build-many-hospitals`(한 벌로 만들고 다름은 밖에서 받는다)를
끝냈다. **도메인 2 · 기능 1.** 큐 1/3. 남은 도메인은 **맡아 둔 데이터는 맡긴 사람의 목적
안에 머문다.**

### 리프트와 시프트를 가른 것이 이 글의 알맹이다

기간 업무 시스템의 **여섯 세대**를 적는데, 그중 **클라우드 리프트(EC2·RDS 로 그대로,
코드는 거의 그대로)** 와 **클라우드 시프트(무상태·수평 확장·무중단 배포를 위해 다시 설계)**
를 나눈 것이 핵심이다. **둘 다 '클라우드'라 불리는데 얻는 것이 전혀 다르다** — 이름을
나눠야 **지금 어디 있는지**를 속이지 않는다.

### 다름을 없앨 수 없으니 위치를 옮긴다

멀티테넌트로 가면 **고객마다 코드를 못 고친다.** 그런데 병기마다 흐름이 통째로 달라
**'방대한 기능 세트'** 를 요구받는다. 답은 **밖에서 받는 것** — **'서비스에 직접 손을
대는 것이 아니라, 서비스가 데이터를 교환하는 구조를 제공함으로써 종래에 하던 독자 운용을
실현한다.'** 다만 **밖에서 못 하는 것의 경계**는 안 적혀 있다.

### 방향을 밖에서 정해 준다

**일본 정부 표준이 '멀티테넌트 방식의 클라우드 네이티브형'을 요구한다** — 이 회사가 고른
것이 아니다. **뱅크샐러드가 마이데이터로 스크래핑을 API 로 바꾼 것**과 같은 결이고,
같은 회사의 다른 도메인(개인정보보호법)도 같은 자리에서 나온다.

### 세대 서술이 이 엔진에 잘 맞는다는 것이 또 확인됐다

렌더의 **1→2→3세대**가 '캐시와 전파 사이의 고전적 긴장'을 이름 붙였듯, 헨리의 여섯 세대는
**리프트와 시프트의 차이**를 분명히 한다.

### 다만 자기 구조가 없다

**업계 관찰은 두꺼운데 헨리 자신의 아키텍처는 이 글에 없다** — 세대를 잘 갈라 놓고
**'우리는 여기 있다'** 를 적지 않는다. 채용 글의 성격이 섞여 있어 보인다.

### 다음 사이클

확장 — 남은 도메인(규제). **위탁/제3자 제공 구분 · 이용 범주 셋 · 문서별 역할**이 이미
근거로 서 있다. 큐가 1/3 이라 그 뒤에 조사가 다시 뜬다.
