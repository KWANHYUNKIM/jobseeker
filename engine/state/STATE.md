...
**Twilio** (US · SaaS — 통신 API) — **프로파일까지 썼다**(PROMPT 3단계). 도메인 2개 · 기능 0. 회사 **97개.** ⚠️ **큐가 0/3 이 됐다.**

### 이번 사이클 — 신규(6순위). 큐의 마지막 한 곳을 올렸다

⚠️ **이 회사의 공개된 자기 시스템 이야기 둘이 서로 반대 방향이다.**

- **Segment 의 알림 서비스**(2025-09-22) — **모놀리스를 세 책임으로 쪼갰다**(API 운영 · Kafka 이벤트 처리 · 시간 기반 스케줄링).
- **콘솔**(2021-04-07) — ⚠️ **마이크로 프론트엔드를 모놀리식 Jamstack 으로 합쳤다.** 팀 간 결합을 풀려고 넣은 **프록시가 성능 문제와 파편화를 낳았고 최악의 경우 첫 바이트까지 1초**가 걸렸다.

⭐ **둘 다 `한 번에 갈아엎지 않는다` 는 같은 방법을 쓰면서 반대 방향으로 갔다** — 하나는 **Strangler Fig 로 이중 쓰기**를 하며 쪼갰고, 하나는 **페이지 라우트 하나씩** 옮기며 합쳤다. **도메인을 그 대비로 둘 세웠다**(재구성).

**규모** — 2026년 1분기 매출 **14억 달러(+20%)** · 2025년 **활성 고객 계정 40만 2,000곳 이상**·**상호작용 2조 5,000억 건 이상**(⚠️ **2026년 1분기부터 활성 계정 공시 중단**) · 고객들이 **거의 10조 행**을 웨어하우스로 동기화.

### ⚠️ 이번 프로파일의 약한 곳

- **통신 API 본체의 기술 자료를 못 찾았다** — **2조 5,000억 건을 어떻게 처리하는지**가 비어 있다. ⚠️ **Paystack 에서 본체 자료가 없어 완주가 막힌 전례가 있어 그 도메인은 세우지 않았다.**
- **콘솔 글이 2021년 4월이라 낡았다** — **5년이 지났고 그 뒤가 어떻게 됐는지 모른다.** 도메인 why 에 그렇게 적었다.
- **알림 글에 수치가 하나도 없다** — 결정과 대가는 명확한데 **처리량·지연·이전 기간이 없다.**

### 다음 사이클 — 확장(2순위)

`--gaps` 가 **`알림을 보내는 시스템을 책임별로 쪼갠다`** 를 부를 것이다. **자료를 이미 읽었으니 바로 기능을 쓴다.** ⭐ **비교 문서 ⑨축(`한 번에 갈아엎을 것인가 목 졸라 죽일 것인가`)의 주인공이다.**
...
**⑨ ⭐ `한 번에 갈아엎을 것인가 목 졸라 죽일 것인가` — 주인공이 들어왔다** — **Twilio**(Strangler Fig + 이중 쓰기, 그리고 **반대 방향으로 합칠 때도 페이지 하나씩**) / Etsy(테이블마다 1%→100%) / Plaid(앞으로 실패하는 길) / Paystack(제자리 업그레이드).
...
⏳ **⑮ 새로 보인다 — `언제 쪼개고 언제 합치나`** — ⭐ **Twilio 한 회사 안에 두 방향이 다 있다**(알림은 쪼개고 콘솔은 합쳤다) / DoorDash(모놀리스 → 마이크로서비스 약 50개) / Deliveroo(팀마다 만든 것을 중심 인프라로 모았다).
