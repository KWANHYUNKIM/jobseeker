...
**없다 — 이번 사이클은 후보 조사였다. 큐를 0/3 에서 2/3 로 채웠다.** 회사 155개.

### 올린 둘 — 축을 벌렸다

**Fireworks AI(US)** — LLM 추론 커널 축. Q-outer 와 KV-outer 를 견주고 **교차점을 수식으로
도출한다**(`nsb / N < 2.85`). 오픈소스 커널이 어텐션 에필로그 안에서 흩어 쓰는 것을 물린 이유도
적는다 — **흩어 쓰기가 명령 지연에 민감해 파이프라인을 멈춘다.** **Zulip(US)** — 지연에 민감한
제품 **안에서** 권한 모델을 짓는 축. 깃허브·깃랩 문서를 인용해 단일 부모 제약을 물렸고,
마이그레이션 800여 개 중 **115개**가 이 작업이었다.

### 지난 조사의 '못 읽은 곳'이 이번 조사의 목록이 됐다

Fireworks 는 지난번에 **"본문을 못 열었다"**고 적어 뒀는데 이번에 그 한 편을 열어 통과시켰다.
**`blocked` 로 구분해 적는 습관이 실제로 값을 했다.**

### 만드는 쪽과 쓰는 쪽 — 다섯 번째

**AuthZed**(인가 시스템을 파는 쪽) ↔ **Zulip**(제품 안에서 권한 모델을 짓는 쪽).
turbopuffer↔Linear, 클릭하우스↔PostHog, 데노↔Val Town, 템포럴↔Railway 에 이은 다섯 번째다.

### 접은 하나와 못 읽은 둘

**Raspberry Pi** — 실리콘 축을 노렸는데 뉴스가 교육 프로젝트·커뮤니티 빌드·제품 발표가
지배한다. **incident.io** 는 `/blog`·`/blog?tag=engineering`·`/blog/engineering` **세 경로가
모두 404** 다(세 번째 확인). Sourcegraph 는 지난번 403 이후 이번엔 시도하지 않았다.

### 다음 사이클 — 신규 (대기 맨 위의 Fireworks AI)

커널 최적화 글(2026-07-10)이 손에 있다. **⚠️ 나머지 목록은 모델 공지가 지배하므로** 프로파일을
쓸 때 도메인을 억지로 늘리지 않는다 — 리니어·먹스에서 한 판단을 이어 쓴다.
