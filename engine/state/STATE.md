...
**Temporal(US · SaaS—내구성 있는 실행)** `in_progress` · **도메인 3 · 기능 1** —
76곳 · ⚠️ 큐 0/3.

방금 쓴 기능: **`replay-must-match`(다시 밟은 길이 처음 길과 한 걸음도 달라선 안 된다)**.
결정 7개가 **전부 `confirmed`**.

### ⭐⭐ 편의를 저장에서 코드로 옮긴 설계다

상태 스냅샷을 두지 않고 **이력을 재생한다.** 되살리기는 확실해지지만 **워크플로 코드가
결정적이어야 한다** — `any time your Workflow code is executed it makes the same Workflow
API calls in the same sequence, given the same input`.

### ⭐⭐ 금지되는 것이 매일 쓰는 것들이다

**시스템 시간 직접 읽기 · 난수 · 로컬 시계로 분기 · API·DB·LLM 호출**(액티비티로 옮겨야
한다). **`now()` 와 `random()` 이 금지된다는 것**이 이 설계의 진짜 값이다.

### ⭐⭐ 가장 무서운 자리는 배포다

비결정성 오류는 `typically from code changes made to running Workflow Executions` 다.
그래서 문서가 **안전하지 않은 수정 목록**을 따로 둔다 — 커맨드 재정렬·추가·제거,
**타이머 시간을 0으로/0에서**(Java·Python·Go, .NET 은 -1), 액티비티·자식 워크플로
타입 ID 변경. **워크플로가 몇 달씩 사는 시스템에서 코드는 계속 바뀐다** — `hard_problems`
로 세웠다. ⚠️ **버전 관리 장치가 있는지는 이 문서에 없다.**

### ⚠️ 침묵 하나를 기록했다 — 이벤트 이력의 크기 제한

**문서에 없다.** 그런데 **이력이 곧 저장이고 저장이 곧 요금**이다(활성 $0.042/GBh ·
보존 $0.00105/GBh). 관리형의 **계층 저장**(완료되면 객체 저장소로)이 답의 일부인데,
**끝나지 않고 계속 도는 워크플로는 어떻게 되는지 확인하지 못했다.**

### 다음 사이클 — 남은 두 도메인

- **확장의 단위를 워크플로 하나로 잡는다** — `workflow-engine-principles`(요약 확보).
  **제약을 먼저 인정하고 단순하게 푸는 결이 가장 뚜렷하다.**
