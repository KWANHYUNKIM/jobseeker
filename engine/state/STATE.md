...
**없다 — Protomaps 를 완주시켰다(도메인 2 · 기능 2 · `status: done`).** 회사 146개, 큐 1/3.

### 이번 사이클이 채운 것 — `say-when-not-to-use-it`

파는 쪽이 **"당신은 이걸 원하지 않을 수도 있다"**를 자기 블로그 제목으로 단다. 적격 조건
셋(웹에서 보는 것 · 수 메가바이트 초과 · 갱신 하루 한 번 이하)을 못 박고, 아니면 남의 것을
이름 대어 권한다 — 작고 고정은 GeoJSON, 자주 바뀌면 PostGIS, 데스크톱 탐색은 GeoParquet.

### 형식을 가변으로 만드는 대신 합성으로 풀었다

읽기 전용을 지키면서 여러 아카이브를 하나로 내야 하자 `pmtiles merge`(2025-12)를 냈다.
**겹치는 타일이 0이어야 하고** 클러스터·타일 종류·압축이 같아야 한다. 겹치면 거절하고
rio-pmtiles·Tippecanoe·Planetiler 로 돌려보낸다 — 겹침을 다루는 건 사실상 렌더링 결정이라서다.

### 이 회사가 이 엔진에 준 축

**자기 제품의 경계를 스스로 긋는 회사**다. 후보 조사 때 이 글 하나로 자격을 얻었고, 판고 나니
그게 도메인 하나가 됐다. 앞으로 이 축(안 맞는 자리를 적는가)을 후보 판정에 계속 쓴다.

### 다음 사이클 — 대기의 Ente 로 넘어간다

IN/US · 소비자 종단간 암호화 사진 + **온디바이스 ML**. 시그널(메시징)·백블레이즈(저장 내구성)와
다른 자리다. 자료는 큐에 여덟 편 적어 뒀다 — 후양자 혼합 암호(2026-08-18) · 기기 위의 CLIP
검색(2023-11-18) · 3중 복제(2023-01-07)가 중심이다. **⚠️ 최근 목록은 제품 공지가 많다.**
