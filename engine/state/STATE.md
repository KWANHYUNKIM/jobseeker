...
**없다 — 예순다섯 번째 후보 조사를 돌았다. 큐 1/3.** 다음 사이클은 **신규**(curl).

### 만드는 쪽과 받는 쪽을 나란히 얻었다

**모질라**는 AI 하네스로 Firefox 버그 271개를 **만들어 내며** 적었다 — "문제를 찾으라고
시키는 것은 싸고 쉬운데 대응하는 것은 느리고 비싸다." **curl** 은 **받는 쪽에서 문을
닫았다** — 2026년 7월 한 달간 취약점 접수를 멈췄고, 대가를 **CNA 규정 준수의 사소한 위험
대 메인테이너 번아웃 회복**으로 적는다. 같은 문제의 양쪽 끝이 이제 데이터에 있다.

### 불균형을 정면으로 적는다

**"여러분의 보고를 받는 프로젝트에는 그 보고들을 처리하는, 과부하에 걸린 소수의
메인테이너뿐이다. 이 불균형을 생각하라."** 보고하는 사람에게 대놓고 쓴 문장이다.

### 개인 채널이 프로젝트 블로그를 대신한 세 번째 사례

`curl.se` 홈에 **블로그 링크가 아예 없고** 기술 글은 **`daniel.haxx.se`** 에 있다.
프레시웍스(창업 초기 회고가 개인 LinkedIn)·고스티(메인테이너 개인 사이트)에 이어서다 —
**회사 사이트에 없다고 자료가 없는 것이 아니다.**

### 접은 곳 둘

**Twitch**(먹스와 마주 볼 스트리밍 축) — 엔지니어링 카테고리가 없고 크리에이터·이벤트
글이 지배한다. **Bitwarden**(1Password 와 마주 볼 자리) — 제로 지식 암호화 **개념 소개**
한 편 정도이고 자기 구현을 판 글이 없다.
