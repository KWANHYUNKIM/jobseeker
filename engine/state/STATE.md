...
**없다 — 문체 보수 사이클이었다.** 윅스를 배민 문체로 다시 썼다(0.95 → 0.0, 기호 414개). 회사 115개, 큐 2/3. 6곳 남았다.

### 이 파일이 영문 인용 최다였다 — 215종

지금까지 중 가장 많았다. 그리고 인용을 풀면 조사가 붕 뜨는 자리가 30곳 넘게 생겨서, **한글 뒤에
공백을 두고 떨어져 있는 조사를 한 번에 붙이는 정규식**으로 정리했다(한국어에서 조사는 앞말에
붙여 쓰므로 안전하다). 문체 보수를 계속 돌 거라면 이 정리를 변환기에 넣는 것이 맞다.

### 에이전트에게 이유를 대게 하는 자리

Sentry(매일 밤 심문) · 그랩(혼란에 빠질 수 있다고 전제하고 가둔다) · Razorpay(커널로 권한을 막는다)에
이어, 윅스는 **모든 도구 호출에 이유를 적게 한다** — 표면의 키워드를 맞춰 보는 대신 무엇을 찾는지
실제로 생각하게 만들려는 것이다. 그리고 자기 자동화를 평가하는 방식이 정직하다 — 늘 맞지는
않지만 평균 해결 시간을 줄일 만큼은 자주 맞고, **전후 그래프가 딸린 깔끔한 사례 연구는 없다.**

### 되돌릴 길을 언제까지 남기느냐 — 문장으로 가른다

트레인라인은 전환 전에 TTL 을 낮춰 길을 깔았고 크라켄은 아예 앞으로 고치는 쪽을 택했는데,
윅스는 **되돌릴 수 있는 구간과 없는 구간을 문장으로 가른다** — 어느 지점부터는 되돌리려면 새
클러스터에만 커밋된 데이터를 잃어야 한다. 그리고 이관의 성공을 순간이 아니라 구간으로 적는다 —
**운영 읽기를 옮긴 정점의 순간 같은 것은 없었다.**

챗룸 단위로 나눠 옮길 수 있었던 이유도 적혀 있다 — **다행히 메시지가 불변이다.**

### 에이전트가 설 자리를 만든 셋, 그리고 코드베이스를 고치는 넷째

DoorDash(환경·도구·권한·제약) · 그랩(주위를 상자에 미리) 다음으로, 윅스는 **코드베이스 자체를
고친다.** 마이크로서비스 2,500개와 엔지니어 수천 명 앞에서 그냥 AI 코딩 도구를 쓰라는 것은
전략이 아니라고 적고, **하네스를 팀의 결정이 아니라 플랫폼의 책임으로 다루라**고 남긴다.

거르는 방식도 한 줄이다 — 컴파일이 통과해야 내보내고, **상류의 자동화 비율을 높이기 전에 리뷰
층에 투자하라.**

### 다음 사이클 — 문체 보수 (사다리 3순위)

--gaps 가 정한다. 지금 맨 위는 grafana-labs(0.87)지만 그건 이 시점의 예상이지 확정이 아니다.
