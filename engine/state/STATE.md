...
**아무도 안 판다 — 349 는 후보 조사 사이클이었다.** **네 곳을 보고 둘을 올렸다.**
**완료 58곳 전부 `done` · 큐 2/3 · 비교 문서 32편.**

**⑭ `--gaps` 가 [후보 조사]** 를 가리켰고 그대로 따랐다(PROMPT.md 2⁗단계).
**⚠️ 그리고 낡은 메모를 하나 걷어냈다** — STATE 가 *"Personio 는 큐의 확인해 둔 후보에
올라 있다"* 고 나르고 있었는데 **실제 QUEUE 에는 없었다.** 기억이었다(314·317·325·348 에
이어 다섯 번째다).

**✅ 올린 둘**
- **bol**(EU · 리테일) — `techlab.bol.com`. **이번 조사에서 가장 강하다.**
  `Splitting the data of the monolith`(2024-11-07)를 열어 봤다. **4TB 중 3TB 를 두
  마이크로서비스로 옮긴 이야기**인데 **버린 대안 셋에 각각 이유가 붙고**(중개 앱을 2031년까지
  유지 · 스케줄 잡으로 밀기 · 전 이벤트 리플레이), **실패를 셋 적고**(한 테이블이 사흘 넘게
  돌다 미완으로 멈춤 · DMS 가 18시간 뒤 외래키로 실패해 2주 조사 · 인덱스 생성 데드락으로
  재시작 루프), **수치가 촘촘하다**(CPU 4→15 퍼센트 · 한 테이블 6→7→10~12시간 · DMS 잡당
  19~23시간 · 전체 2~3개월). **⚠️ 최신 글이 2024-11 이다.**
- **PayFit**(EU · HR SaaS) — `backstage.payfit.com`. **2/3**(버린 대안·대가는 있는데 수치가
  2021년 채용 250명 하나뿐). `How we managed to scale-up the PayFit architecture`(2021-10-07)를
  열어 봤다 — 단일 저장소를 **수직 슬라이스**로 쪼갠 이야기다.
  **⭐⭐ 축이 freee·SmartHR 과 정확히 겹친다** — 급여·인사 SaaS 이고 **JetLang 이라는 로우코드
  언어로 노동법을 코드로 추상화**한다(**본문 미확인**). **🔴 얇고 멈춰 있다** — 엔지니어링 태그
  여덟 편, 최신 2024-07-08. **도메인 두세 개가 한계일 것으로 보인다**(추정).

**❌ 못 올린 둘 — 둘 다 Medium 이었다**(`## 재시도 안 함` 으로 옮겼다)
- **Personio**(EU · HR SaaS) — 엔지니어링 글이 `medium.com/inside-personio` 다.
  **⚠️ HR 축이라 가장 아까웠다.** `www.personio.com/blog` 은 HR 마케팅 블로그다.
- **Deezer**(EU · 음악) — `deezer.io` 가 자체 도메인처럼 보이는데 **URL 이 Medium 커스텀
  도메인 형태**(`/slug-11d3084cfd70`)다.

**⚠️ 목표 3곳에 하나 못 미친다.** 억지로 채우지 않았다 — 근거 없는 후보 하나가 다음 사이클을
통째로 버리게 만든다. **다음 후보 조사에서 한 곳을 더 채운다.**
**후보 조사 누적 — 스물세 곳 중 아홉.**

---

**다음 사이클 — 신규다.** `--gaps` 가 [신규] 를 가리킬 것이다(큐 2/3, 진행 0).
**⭐ 대기 맨 위는 bol 이다.** `## 진행 중` 으로 옮기고 `in_progress` 로 만든 뒤
**PROMPT.md 3단계(회사 프로파일)** 부터 판다 — `business_model` · `products` ·
`revenue_streams` · 도메인 후보 · `domain_map`. **기능은 그다음 사이클.**
- **⚠️ 큐에 적은 것도 본문을 다시 열어 확인하고 쓴다**(⑫ — 재독이 새 사실을 낸 것이 여러 번이다).
- **사업 수치는 회사 IR 쪽을 따로 찾는다** — bol 은 Ahold Delhaize 계열이다(**미확인 — 확인하고 쓴다**).
- **⭐ 도메인 이름은 넓게 짓는다**(334→335). **새 도메인은 `index.json` 최상위 `domains` 에도.**
- **⚠️ `domain_map` 은 `{title, question, code}` 객체다**(344).

**분포** — 58곳(**US 22 · KR 9 · JP 7 · EU 7 · IN 3** · 기타 3 · CN 2 · CA·AU·SG·AE·NG 각 1) ·
비교 문서 32편 · **운영 사실 43개.**

**⭐⭐ 비교 문서 재료가 둘 서 있다**(신규를 파는 중에는 손대지 않는다):
① **freee 대 SmartHR 의 순서** — freee 는 *"旧機能のリファクタはマイクロサービス化の前に
着手すべし"* 라 후회하고, SmartHR 은 **그 순서를 실제로 밟아 보고** 모듈러 모놀리스로 경계만
정리하는 안을 검토했다가 버렸다(인가가 과금·테넌트에 의존해야 해서 **의존 허용 규칙이 곧 경계
설계가 되고**, 배포 독립성과 가용성은 규칙으로 안 풀린다). **후회를 실행해 보니 부족했다는 이야기다.**
② **`미루되 미룬 것이 안 보이게 한다`** 에 SmartHR 두 사례를 더한다 — 미룬 쪽은 **놀고 있는
접속**에 쌓였고 청구서는 장애 빈도가 오르며 왔으며, **미리 잰 쪽**은 연 1회 정점을 부하 시험으로
앞당겨 봤다. **⭐ bol 을 파고 나면 여기에 셋째 사례가 붙을 수 있다**(2031년까지 중개 앱을
유지하는 안을 버린 것이 바로 '미루지 않기로 한 결정' 이다 — **추정**).

**⚠️ 나르지 않는다** — **낡은 메모**(314·317·325·348·**349 에서 다섯 번째**) ·
두 방향 규칙(298·305·318·321) · 주소가 열리는지도 확인한다(327·328) ·
인용문에 다른 문자가 섞인다(330) · **한 글에 없다고 회사에 없는 것이 아니다**(331→332) ·
**도메인 이름이 좁으면 다음 재료가 안 들어온다**(334→335) · **자기 문서 인용도 우리말은
굵게**(337) · **같은 프로젝트라도 각도가 다르면 다른 기능이다**(339→340) · **같은 회사의
같은 아픔이라도 층이 다르면 다른 도메인이다**(341) · **확인 안 된 인용을 지어내지
않는다**(341·342) · **읽은 글을 미독 목록에 남기지 않는다**(342·345) · **세 편을 열고
하나도 안 쓰는 사이클이 정상이다**(343) · **`domain_map`·`diagrams` 는 객체다**(344) ·
**`features_done` 을 같이 올린다**(345) · **두 도메인이 같은 문제에 정반대로 답하면
`connections` 에 적는다**(346) · **`-c "…"` 안에 백틱 금지**(347) · **프로파일의 '아직 한
편만 읽었다' 류 문장은 닫을 때 다시 본다**(348) · **⭐ 새로: 자체 도메인처럼 보여도 URL 끝에
16진수 해시가 붙어 있으면 Medium 커스텀 도메인이다**(349 의 `deezer.io`).
