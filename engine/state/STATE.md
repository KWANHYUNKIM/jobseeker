...
**DeepL**(EU · 기계번역) — 회사 프로파일까지 썼다. **도메인 셋 · 기능 0.** 큐 1/3.

### 잣대를 틀렸다고 적는 회사다

이 회사 자료에서 가장 값어치 있는 대목은 번역 품질이 아니라 **품질을 재는 자를 갈아치운
이야기**다. 문서 생김새를 픽셀 비교(SSIM)로 재다가 버렸다 — **단어가 줄 넘어간 사소한 일과
글자가 페이지에서 사라진 큰 일을 못 가르고**, 배경 이미지가 크면 점수가 부풀려졌다. 대신
페이지를 의미 영역으로 잘라 겹침을 재는 자체 지표를 만들었다.

### 수식을 포기한 자리

글꼴 크기를 언어별 팽창률로 계산하려다 물렀다 — **"10.385 크기의 Times New Roman 같은 것은
없다."** 그래서 0.5pt 씩 오르내리는 반복 최적화로 갔다. **세상의 단위가 연속이 아니라서
수식이 안 되는 자리**를 이렇게 정확히 적은 자료는 드물다.

### 자기가 자기를 채점하지 못하게 한다

번역 모델에게 스스로 채점시키는 길을 버렸다 — **"LLM 이 번역을 검사하면 자기 모델의 결과를
편들고 그 약점을 놓친다."** 메타베이스가 사고 원인으로 **자기들의 리팩터**를 지목한 것과
같은 결이다. 자기 것을 자기가 볼 때의 눈먼 자리를 **구조로** 막는다.

### 다듬은 것을 버린 시대 하나

시대를 하나 세웠다(2025-07~). Mercury(H100 · GPU 8개)에서 Arion(GB200 · GPU 72개)으로 가며
**오래 다듬어 온 자기들만의 맞춤 seq2seq 구조를 버렸다** — "AI 연구의 쓴 교훈". 대가도
적힌다: 합성 데이터를 1000배로 불려야 하고, 학습 손실만 보면 BF16 이 FP8 보다 조금 낫다.

### 다음 사이클

도메인 셋 중 아무 곳에도 기능이 없다. `--gaps` 가 확장으로 부를 것이다. 문서 생김새 쪽이
자료가 가장 두껍다(SSIM 폐기 · 바운딩박스 지표 · 0.5pt 반복 · 우선순위 넷 · PDF 60%).
