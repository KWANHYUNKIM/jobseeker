...
**Chainguard — 프로파일만 썼다(도메인 셋, 기능 0).** 회사 154개, 큐 1/3.

### 파는 것이 산출물이 아니라 상태다

이미지 하나가 아니라 **그 이미지가 늘 최신이라는 상태**를 판다. 그 일을 세는 단위가 빌드
매니페스트인데(새 릴리스 · 보안 패치 재빌드 · 새 아키텍처 변종 · SBOM 재생성마다 하나)
**6개월 만에 5억에서 10억으로** 두 배가 됐다.

### 한 번에 맞히기를 요구하지 않는다

Factory 2.0 의 설계 원칙이 그렇다 — 실패한 작업을 다시 시도할 수 있게 두고 **옳은 결과로
수렴**하게 한다. AI 는 구조가 없는 판단(부품 추론·CVE 백포팅)에만 골라 쓰고 나머지는 검증
가능한 구조적 도구로 둬 환각을 막는다고 적는다.

### 끊임없이 돌면 반드시 몰린다

50만 개 넘는 키를 한꺼번에 다시 방문하면 스파이크가 된다. 흔한 답들을 이름 대어 물린다 —
**지터는 발맞춤만 흐트러뜨리고**, 두 레인 워크큐와 리더 선출은 "관리는 됐지만 없애지는
못했다". 답은 시간에 흩어 파킹하는 것이었다: **50만 키 스파이크 → 분당 약 350키의 배경 잡음**,
"하루까지 50만 키 대기" → "약 5,000키 15분 미만".

### 누가 이걸 고칠 것인가

최신 AI 모델이 **유지보수자가 고칠 수 있는 속도보다 빠르게** 취약점을 찾아낸다. 그래서 기다리는
대신 직접 고침을 쓴다 — 다만 **"패치를 만드는 것은 그중 가장 작은 일"**이고 남의 코드를 공개
유예 아래 올바르다고 증명하는 부담이 전부다. **패치하기 전에 악용 시험을 먼저 써 둔다.**

### 다음 사이클 — 확장 (첫 도메인)

`끊임없이 다시 짓는다`. **⚠️ 10억 매니페스트 글에 운영 수치가 없다**(하루 빌드 수·지연·값)
— 없는 것을 지어내지 말고 `limits` 로 적는다. `한꺼번에 몰리는 일을 길들인다` 는 수치가 가장
두껍다.
