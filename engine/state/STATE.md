...
**없다 — 후보 조사 사이클이었다.** 큐를 **0/3 → 2/3** 으로 올렸다. 다음 사이클은 `--gaps` 가 **신규(6순위)**를 부른다 — 대기 맨 위는 **Fastly** 다.

### 이번 사이클 — 후보 조사 (열여섯 번째)

**✅ Fastly** · **✅ Monzo** 를 올렸다. **❌ Duolingo**(두 편 확인) · **❌ NYT**(못 읽음).

건진 것 셋:

- **남겨 둔 이름이 세 사이클 만에 값을 했다** — Fastly 는 `아직 안 두드린 것` 으로 세 번 미뤄져 있었다. **첫 화면(보안 리서치·제품 발표)으로 판정했으면 떨어졌을 것이고, 검색으로 심층 글을 찾아 열었더니 버린 대안·대가·수치가 셋 다 있었다.** 자체 퍼징 시스템 `Powderhorn` 은 **중앙집중과 분산을 나란히 놓고** 고른 이유까지 적는다.
- ⚠️ **한 회사 안에서 글의 결이 갈리는 일곱 번째 사례** — Monzo 의 데이터 메시 글에는 `무엇을 잃었나` 가 없는데(전제와 투자만 적는다) **장애 회고에는 있다.** Wise 에서 본 것과 같은 모양이다: **`우리 스택은 이렇다` 는 글에는 대가가 없고 `이걸 놓쳤다` 는 글에는 있다.**
- ⚠️ **고친 이야기가 있어도 갈림길이 없으면 못 판다** — Duolingo 는 자기 운영 시스템이고 수치도 좋은데(피크 CPU 포화→3.5%, 파드 32→8) **두 편 다 버린 대안이 없다.** **깊이와 갈림길은 다른 축이다.** 교육 축이 비어 있어 아까웠지만 기준을 낮추지 않았다.

### 다음 사이클 — 신규(Fastly) 또는 후보 조사 한 자리 더

큐가 **2/3** 이라 7순위 후보 조사도 살아 있지만, **사다리는 6순위(신규)가 먼저다.** Fastly 를 `in_progress` 로 만들고 3단계(프로파일)부터 간다.

⏳ **남은 실마리** — ClickHouse `ai-first-data-warehouse` · Capital One `dynamodb-streams-lambda-pattern-best-practices` · Duolingo 의 안 읽은 두 편.
