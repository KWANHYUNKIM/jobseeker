...
**하이퍼커넥트 (KR · 소셜/실시간 영상) — 46번째. 기능 1개 · 도메인 2개. 큐 1/3(컬리).**

255 에서 **`objective-relaxation`(가정을 쌓아 목표를 옮긴다)** 을 썼다. 결정 8 · 그림 3 · 생각 4 ·
스택 6 · 난제 2 · ui 6핀.

**⚠️ `--gaps` 가 도메인 ①(목표 지표)을 지목해서 그것을 따랐다.** 프롬프트는 ②(Flink)를 시켰지만
**감으로 바꾸지 않았다**(스킬 규칙). 대신 **②용으로 읽은 두 편을 소스와 도메인 `tech` 에 반영해 뒀으니
다음 사이클이 바로 쓸 수 있다.**

**⭐ 이 회사가 지금까지 판 46곳 중 가장 특이한 자리에 있다**(이 정리는 이 사이트의 것이다).
- **세 글이 서로를 명시적으로 링크한다** — 2026-01 글이 2024-04 아하 모먼트 글을 본문에서 건다.
  **CyberAgent·Wayfair 는 같은 시스템의 다른 층을 다루는 글끼리도 서로를 언급하지 않았다.**
- **가정의 대가에 이름을 붙였다** — ***"가정은 AI 문제에서 일종의 기술 부채입니다."*** 그리고 왜
  낡는지까지 적는다: 사용자 제품은 *"본질적으로 비정상성(non-stationary) 분포를 따르기 때문에"*
  *"한 번 성립했던 가정이 계속 유효하다고 보장할 수 없습니다"*.
- **🔴 가정이 실제로 깨진 기록이 있다** — DAU 를 목표로 삼았더니 *"PUR(Purchase User Rate)이
  떨어졌습니다"* 라서 **구매 사용자 중심으로 다시 짰다.** 그리고 예외로 적지 않는다 —
  *"여러 가정이 동시에 깨지는 경우가 빈번"*. **이 사이트의 여러 비교 문서가 *'재 보고 골랐는데
  틀렸다는 글을 아직 못 찾았다'* 로 끝나는데, 여기가 그 반례다.**

**🔴 관찰 하나** — **이 회사는 설계의 근거를 잘 적고 결과의 크기를 잘 안 적는다.** 목표 지표 3부작은
**버린 접근을 여덟 개나 대는데 개선 폭이 없고**, Flink 글은 **중복 제거 지연(300ms 에서 3ms 미만)만
있고 초당 이벤트 수·병렬도·파티션 수가 하나도 없다.** 다만 **실패는 적는다**(PUR).

**📌 Flink 글 재독으로 셋을 더 찾아 소스에 반영했다**(운영 사실 ⑫가 일곱 번째로 맞았다) —
① **복구 시 타이머 문제의 원인과 해법**(*"Savepoint가 생성된 이후 복구 시점 사이에 만료될 타이머가
존재하기 때문입니다"* → **CheckpointedFunction 의 `initializeState` 에서 복구 시각을 기록해 두고**
*"배포 시점 이전에 설정된 타이머라면 기간을 연장"*) · ② **Exactly Once 를 버린 진짜 이유**(*"2 Phase
Commit(2PC) 매커니즘을 사용하기 때문에 Checkpoint 주기마다 메시지를 발행하게 되어"*) ·
③ **Blue-Green 절차**(Savepoint 로 상태와 오프셋 저장 → 새 서비스가 복원 → 동기화 완료 후 전환 →
기존 종료, **Spinnaker 로 자동화한 이유는** *"Savepoint 생성 이후 신규 서비스 배포까지의 시간을
최소화"*). ⚠️ **Redis 키 설계·TTL 은 없고, 상태 크기 상한도 정리 정책도 없다.**

**🚨 다음 사이클 — 도메인 ②`서로 다른 시각에 도착한 것을 이어 붙인다` 의 기능을 쓴다.**
**재료를 이미 둘 다 읽었고 도메인 `tech` 에 넣어 뒀다** — Flink 스트림 조인(2025-06-11, 재독 완료) +
이벤트 기반 라이브 스트리밍 추천(2022-01-24, **⚠️ 4년 전 글이라 최신성을 밝힌다**).
**결정이 6~8개 선다** — Window API 대신 KeyedProcessFunction · Exactly Once 대신 실시간성 ·
Redis 중복 제거(300ms 에서 3ms 미만) · Blue-Green 을 직접 구성 · 복구 시 타이머 연장 ·
RocksDB 상태 백엔드 · (2022 쪽) API 대기와 DB 공유를 버리고 이벤트 기반으로.
**⚠️ 2022 글은 스트림 조인을 언급하지 않는다** — 두 글이 이어지는지 이 사이트는 확인하지 못했다.

**세 번째 도메인 후보** — 쿠버네티스/인프라 3편: `/2025/05/15/Building-Resilient-High-Performance-ScyllaDB-Clusters-with-Super-Disk.html`
· `/2023/07/25/migrate-half-of-workload-in-a-year.html`(ARM64) · `/2020/11/02/cluster-migration-in-5-days.html`.
**미개봉이고 둘이 2020·2023 이다.**

**⚠️ 사업 수치를 못 찾았다** — Match Group IR 홈에 하이퍼커넥트도 아자르도 없다. 받은 것은
**LTM 매출 $3.5B**(2026-06-30 종료 분기)뿐. **단서 하나** — 2026-01-12 글이 **Match Group 의 2023 2Q
실적**을 참조한다. **정황이지만 소유 관계를 그것으로 단정하지 않았다.** 다음에 `quarterly-results` 나
SEC 파일링을 연다.

**⚠️ 다음 회사는 컬리다(큐 1/3).** 본업 재료 둘 — `/blog/image-based-misdelivery-detection`
(**배송 완료 사진으로 오배송을 On-device 로 탐지**, 2026-04-28) · `/blog/2026-outbox-pattern-and-retry-topic`
(입고 시스템의 외부 인입 데이터 동기화, 2026-01-12). **도메인을 열 때 이 둘부터 본다.**
253 에서 읽은 `/blog/2026-delivery-domain-rag` 는 **사내 개발 도구라 본업이 아니다.**

**⚠️ 한국 편중 — 이 둘을 다 파면 아홉이다. 다음 후보 조사는 한국 밖에서 한다.**
...
- **새 축 `관측에 값을 무엇으로 치르는가`** — CyberAgent `observability-pipeline`(돈) +
  `alert-context`(주의력) + Yelp `partition-access`(하루 TiB) + Zerodha `log-storage` + Doximity
  `prod-profiling`. **다섯이라 넉넉하다.**
- **`되돌릴 수 있는 것을 먼저 한다`에 CyberAgent `env-split-migration` 이 반례** — 되돌림 계획 자체가 없다.
- **📌 새 재료 — 하이퍼커넥트 `objective-relaxation` 이 기존 `무엇을 좋다고 부를 것인가` 를 크게 보강한다.**
  **인과 추론으로 지표를 고른 사례이자, 고른 지표가 실제로 틀렸던 사례**(PUR)다. 그 문서와
  `재 보고 나서`(after-you-measure) 둘 다 *"잘못 고른 사례가 없다"* 로 끝나는데 **여기가 그 반례다.**
...
⑫ **요약이 아니라 원문을 다시 읽는다** — 223·229·243·249·250·252·**255** 일곱 번 다 새 결정을 찾았다.
⑬ **JS 로 그려지는 목록은 브라우저 `get_page_text` 로도 안 나올 수 있다**(253, Cygames). **두 번 실패하면 접는다.**
⑭ **`--gaps` 가 프롬프트의 지시와 다른 도메인을 가리키면 `--gaps` 를 따른다**(255). 프롬프트용으로 읽은 재료는 소스·`tech` 에 넣어 다음 사이클에 넘긴다.
