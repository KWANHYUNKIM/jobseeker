...
**없다 — Snyk 완주.** 도메인 2/2 · **기능 2개.** 회사 **92개.** ⚠️ **큐가 0/3 이라 다음은 후보 조사(3순위)다.**

### 이번 사이클 — `두 관문을 한 번에, 답안지를 안 보여 주고 통과해야 한다`

건진 것 넷:

- ⚠️ **재는 자가 없으면 좋아졌다는 말도 성립하지 않는다** — **`보안 팀이 실제로 신경 쓰는 과제에 대해서는 널리 쓰이는 공개 벤치마크가 없다.`** 기존 벤치마크는 **코드가 도는지**나 **버그가 해결됐는지**만 보는데 **보안 수정은 두 기준을 동시에 넘어야 한다.**
- ⚠️ **채점 기준을 가린다** — **`모델은 단위 테스트 자체를 절대 보지 못하므로, 통과는 알려진 테스트에 맞춰 낸 출력이 아니라 진짜로 안전하고 동작하는 수정을 뜻한다.`** 그리고 **첫 시도만 센다.**
- ⚠️ **같은 모델을 데이터 있이/없이 재는 짝을 넣은 것이 이 설계의 핵심이다** — 그래서 **모델의 몫과 데이터의 몫이 갈린다**(72.4~74.6% 대 82.5~85.4%).
- ⚠️ **자기에게 유리해 보일 수 있는 숫자를 먼저 버린다** — **`10점 이상 격차는 실행 간 잡음 밖에 있다. 언어별 작은 격차의 순서는 그렇지 않다.`** **큰 숫자는 지키고 작은 순위는 스스로 무효로 만든다.** 한계도 넷을 먼저 적는다(표본 약 150개·단일 파일·세 언어·두 번 실행).

### ⚠️ 이 회사에서 나온 구조적 관찰

**자기 제품을 재는 자를 자기가 만들었다** — 표본을 고르는 것도, 테스트를 쓰는 것도, 통과 기준을 정하는 것도 파는 쪽이다. **그 긴장을 한계 명시와 자기 무효화로 다룬다.** ⚠️ **그리고 제품과 벤치마크의 검증 방식이 서로 다르다** — 수정 고리는 **자기 스캐너로 재스캔**하는데 벤치마크는 **짝지은 보안·기능 테스트**로 잰다. **기능 관문이 벤치마크에만 있다.** ⭐ **한 회사의 글 두 편을 나란히 놓아야 보이는 빈 곳으로 이번 세션에서 세 번째다**(딜리버루의 실험 도구 어긋남, 미쇼의 네 도메인 맞물림에 이어).

### 다음 사이클 — 후보 조사(3순위)

⚠️ **큐가 0/3 이다.** ⏳ 되짚을 목록 — **`bytes.swiggy.com`**(⚠️ `r.jina.ai` 가 422 를 내고 브라우저는 다섯 사이클째 응답이 없다. **Medium 계열 개별 글 주소를 검색으로 받아 `r.jina.ai` 에 태우는 방법을 시도해 볼 것** — Zepto 가 같은 Medium 계열인데 그 방식으로 읽혔다) · `www.gojek.io/blog`(429) · `tech.cygames.co.jp` · `www.scylladb.com/blog`.

⚠️ **이번 세션에서 판 네 회사(Deliveroo · Meesho · Zepto · Snyk)가 전부 큐에서 나왔다** — **후보 조사가 밀리면 파는 쪽이 멈춘다는 것이 이번에 실감됐다.**

### ⚠️ 비교 문서 재료 (초안 유지)

**① `AI 에이전트를 어디까지 믿나` 7곳** — Sentry / ClickHouse / Duolingo / Ramp / DoorDash / Deliveroo / **Snyk**. ⚠️ **Snyk 만 `기계가 고친 것을 어떻게 잴까` 에 벤치마크로 답한다.**

**② `관리형 MySQL 의 한계` 3곳** — Etsy / Plaid / Paystack.

**③ `실험 결과를 얼마나 믿나` 2곳** — Etsy / Deliveroo. ⏳ **Snyk 를 셋째로 넣을 수 있다** — 셋 다 **자기 측정 도구의 한계를 공개한다**(Etsy 는 승자의 저주, Deliveroo 는 불확실성 표현 고정, Snyk 는 잡음 밖/안 구분).
