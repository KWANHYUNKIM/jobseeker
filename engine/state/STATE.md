...
**없다 — 문체 보수 사이클이었다.** 볼트를 배민 문체로 다시 썼다(1.86 → 0.0). 회사 115개, 큐 2/3. 26곳 남았다.

### 설정으로 다 되게 만들려던 시도를 되돌린 둘

첫 기능에 축을 세웠다. 되돌린 결정을 글로 남기는 회사가 드물다.

- Wiz 는 먼저 제약 없이 AI 로 만들고 그다음 그것을 걷어냈다.
- 볼트는 **엔지니어 없이 설정으로 한다는 발상이 오히려 엔지니어의 부담을 늘렸다**고 적으며
  8년 만에 빌더 패턴으로 돌아왔다.

그리고 이 길이 Ramp 의 반대편이다. **Ramp 는 고객마다 다른 규칙을 코드로 못 받겠다고 판단해
그래프라는 틀을 만들었고, 볼트는 같은 발상을 8년 굴려 보고 되물렀다.** 결론은 설정 주도를
부정하는 것이 아니라 자리를 가리는 것이다.

### 남의 규칙을 다루는 네 번째 답 — 흡수하고 나서 맞춰 본다

- Plaid 는 흡수해 감춘다.
- Ramp 는 고객이 조립하게 둔다.
- Paystack 은 못 정하는 시계를 문서에 그대로 적는다.
- 볼트는 흡수한 뒤에 **은행 명세와 맞춰 본다** — 예상과 실제가 어긋나는 것이 곧 사기의 신호다.

원본을 손대지 않는 규칙도 적었다. ClickHouse(고치는 대신 무엇을 고쳤는지만)와 Neon(로그가 진실)과
같은 결인데, **여기는 중복까지 그대로 받는다.**

### 같은 데이터가 다른 숫자를 내는 문제 — 그리고 지시 없이 시작한 이전

- Trendyol 은 같은 필드를 서비스마다 다르게 읽던 것을 **공유된 착각**이라 불렀다.
- 볼트는 쓰는 엔진과 읽는 엔진이 달라 생긴 것을 **신뢰 침식**이라 부른다.

그리고 이 이전이 앞의 이전들과 다른 점이 있다. Monzo 는 한 팀이 전부 밀었고 DoorDash 는 신규
개발을 멈췄는데, **여기는 위에서 내려온 지시가 없었다.** 엔진 소유 팀의 회의를 같은 실험을 더
무거운 규모에서 한 번 더 돌려 넘었다 — 논쟁을 말이 아니라 반복으로 끝냈다.

### 도메인을 사업이 아니라 한 번 고른 구조를 어떻게 되돌리느냐로 잘랐다

승차·배달·킥보드로 가르면 공통점이 흩어진다. 셋 다 앞선 결정을 되무르는 글이다.

### 다음 사이클 — 문체 보수 (사다리 3순위)

--gaps 가 정한다. 지금 맨 위는 bookingcom(1.83)이지만 그건 이 시점의 예상이지 확정이 아니다.
