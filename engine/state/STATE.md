...
**Zerodha — 34번째 회사, 인도 첫 회사이자 증권 첫 회사.** 사이클 185 에서 열었다.
**`IN` 을 validate.py COUNTRIES 와 index.json countries 에 추가했다.** 프로파일 + 도메인 3개, 기능 0개.
**다음 사이클은 `규제 문서 대량 생성`** — 자료 정독 완료(`1-5-million-pdfs-in-25-minutes`).
그 뒤 `과거 거래·리포트 데이터`(working-with-postgresql, 정독 완료), `로그 저장`(logging-at-zerodha, 정독 완료).

> **이 회사의 축 — 매출이 거래대금이 아니라 주문 건수에 붙는다.** 현물은 **무료**, 옵션은 **건당 ₹20 정액**.
> 그래서 **건수가 폭발하는 옵션이 중심**이고 시스템이 감당할 것도 금액이 아니라 건수다. 그리고
> **규제가 매출을 직접 움직인다** — 회사가 *"주간 옵션 중단 검토"* 를 위험으로 직접 들고,
> **2025년 6월 분기 위탁수수료가 전년 대비 약 40% 줄었다**고 밝힌다.

> **⑱ '남에게 안 맡긴다' 축이 선명해졌다 — 거절하는 층이 회사마다 다르다.**
> Adyen 은 **클라우드 자체**를, Vinted 는 **데이터센터와 망**을, Allegro 는 **저장 위치**를,
> Zerodha 는 **관리형 서비스**를 거절한다(*"모든 DB 인스턴스를 맨 EC2 위에서 직접 호스팅·튜닝·관리"* —
> **인프라는 AWS 위에 있다**). **이 차이가 축의 본체다.**

> **⑫ '해 보고 되돌린 것' 재료가 무더기로 들어왔다** — 렌더러를 **Puppeteer → pdflatex → lualatex → Typst**
> 로 세 번 갈아탔고(2,000쪽에서 lualatex 18분 → Typst 약 1분), 저장은 **FaaS · EFS · FSx · EBS** 를 각각
> 다른 이유로 버렸으며, 로그는 **Grafana Loki 를 두 번 검토해 두 번 다 버렸다**, DB 는 **Citus 를 스키마
> 수정 요구 때문에 거부**했다. **한 회사에서 '버린 대안'이 이렇게 많이 기록된 곳은 처음이다.**

> **⚠️ Zerodha 를 완주한 뒤에는 반드시 후보 조사 사이클을 끼운다** — 큐가 **1/3**(PlanetScale 하나)이다.
> 여전히 **라틴아메리카·중동·아프리카 0곳**, **하드웨어에 가까운 회사 0곳**.

마지막 사이클: 185 (2026-08-22) — Zerodha 회사 프로파일.
