...
**없다 — 일흔네 번째 후보 조사를 돌았다. 큐 1/3.** 다음 사이클은 **신규**(엠쓰리).

### 막힌 곳 목록을 '자료가 없는 곳'으로 굳히지 않은 것이 값을 했다

**엠쓰리(M3)** 는 앞서 **막힌 곳**으로 적어 뒀던 회사다. 그런데 그때 본 것은 **medium**
이었고, 이번에 **`www.m3tech.blog`(하테나)** 로 들어가니 그대로 열렸다. **막힌 이유를
'못 읽음'으로 적어 둔 덕에 다시 두드릴 수 있었다** — `none`(자료 없음)으로 굳혔으면
영영 안 봤을 것이다.

### 비어 있던 의료 축이 열렸다

닥시미티 하나뿐이던 자리다. **일본 의료 정보 회사**가 붙는다.

### 올릴 것인가 없앨 것인가

통과 근거가 제목부터 트레이드오프다. Lambda 런타임 EOL 을 맞아 **'런타임을 올리면 이번
대응은 끝나지만 몇 년 뒤에 또 같은 대응이 필요하다'** 며 **Lambda 자체를 없앴다.**
**DeNA 가 EOL 을 맞아 옮긴 것과 마주 본다** — 한쪽은 옮기고 한쪽은 없앤다.

### 다음 사이클

**신규 — 엠쓰리.** 큐 1/3 이라 신규 한 번 뒤에 조사가 다시 뜬다.
