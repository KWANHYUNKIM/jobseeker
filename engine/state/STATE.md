...
**Anthropic** — 회사 프로파일을 썼다. 도메인 **3개**, 기능 **0/3**. 회사 **112개.** 큐 2/3.

### 이번 사이클 — ⭐ 남의 제품에 들어가는 부품이라 `재는 법`과 `밝히는 법`이 본업이 된 회사

**런레이트 650억 달러**(2025년 말 90억에서), **2분기 첫 영업이익**, ⭐ **그중 약 80%가 소비자 구독이 아니라 API 와 기업 계약이다.** 기업 고객 30만 곳 이상, **연 10만 달러 이상 고객이 1년에 7배.**

⚠️ **이 사업 구조가 기술 도메인을 정한다**(재구성) — **`품질이 조금 나빠졌다`가 곧 남의 제품 수천 개가 나빠지는 일이 된다.** ⭐ **그래서 공개 기술 자료가 유난히 `재는 법`과 `틀렸을 때 밝히는 법`에 몰려 있다.**

세운 도메인 셋:

- **자 자체를 의심한다** — ⭐ **에이전틱 평가에서는 무대가 배우의 일부다**: **`인프라가 수동적인 그릇이 아니라 능동적인 구성 요소가 된다.`** **모델을 안 바꾸고 설정만 바꿔도 점수가 6%포인트 갈린다** — 리더보드 상위 간격보다 크다.
- **뇌와 손을 떼어 놓는다** — ⭐ **운영체제에서 착상했다**(프로세스·파일 추상). **뇌·손·세션 셋으로 갈라** **`하네스가 더는 컨테이너 안에 살지 않는다`**. **p50 TTFT -60%, p95 -90% 이상.**
- **틀린 것을 공개한다** — ⚠️ **재현하지 못한 신고에서 시작한다.** 버그 셋이 겹쳐 **`합쳐 놓으면 넓고 일관성 없는 저하처럼 보였다.`**

### ⭐ 이번에 건진 가장 큰 것 — 어디까지가 잡음이고 어디부터가 실력인가

인프라 오류율이 **1배 5.8% → 3배 여유 2.1% → 상한 없음 0.5%.** ⭐ **그런데 성공 점수는 다르게 움직인다** — **1배에서 3배까지는 잡음 범위 안에서 흔들리고(p=0.40)**, **3배에서 상한 없음 사이에서 거의 4%포인트 뛴다.**

⭐ **그 경계 위로는 자원이 `에이전트가 전에 못 풀던 문제를 풀도록 돕기` 시작한다** — **더는 잡음 보정이 아니라 실력을 사 주는 것이다.** ⚠️ **그래서 권고가 값 하나가 아니라 띠다** — **보장 할당과 종료 임계값을 과제마다 함께 명시하고, `바닥과 천장의 점수가 서로 잡음 범위 안에 들도록` 맞춘다.**

⚠️ **교란 요인이 더 있다** — **`통과율이 하루 중 시간대에 따라 흔들리는데 API 지연이 트래픽 패턴과 사고에 따라 달라지기 때문으로 보인다.`**

### ⭐ ㉘축이 이 회사에서 한 겹 더 깊어진다

**Cursor 는 `사용자의 다음 행동`으로, Block 은 `표본 수와 잔여 여지`로 답했다.** ⭐ **Anthropic 은 그 앞 단계를 묻는다 — `재는 자와 그것이 서는 무대를 믿을 수 있나`.** ⚠️ **그리고 자기 결론을 자기에게도 적용한다** — **`리더보드에서 3%포인트 미만의 차이는 평가 설정이 문서화되고 맞춰지기 전까지는 의심해야 한다.`**

⭐ **㊵축(`모르는 것을 어떻게 적나`)에도 정면이다** — 포스트모템이 **`사람과 자동 코드 리뷰 여러 겹, 단위 시험, 종단 시험, 자동 검증, 자체 사용에도 불구하고`** 못 잡았다고 적고, **길이 제한의 3% 하락은 사고 뒤에야 잡혔다**고 밝힌다.

### 다음 사이클 — 확장 (2순위)

**Anthropic 기능 0/3.** ⭐ **`자 자체를 의심한다`부터 판다** — **수치가 가장 조밀하고**(5.8/2.1/0.5, 6%포인트, p=0.40, 1.54%포인트) **거절한 대안과 권고가 명확하다.**

⏳ **못 읽은 것** — `How we contain Claude across products`(격리) · `Claude Code auto mode` · `Harness design for long-running apps` · **`Eval awareness in Claude Opus 4.6's BrowseComp performance`**(⭐ `자 자체를 의심한다`의 다른 면일 수 있다) · **학습 인프라 자료**(⚠️ **Block 에서 겪은 것과 같은 모양 — 본업 자료가 안 보이면 목록을 더 넘긴다**).
