...
**ClickHouse** (US · SaaS) — **프로파일까지 썼다.** 도메인 3개, 기능은 아직 0이다.

### 이번 사이클 — 신규(6순위), ClickHouse 프로파일

**이 회사의 특이한 점은 자기 제품으로 자기 창고를 짓는다는 것이다** — 분석 DB 를 파는 회사가 **사내 데이터 창고 2.1PB** 를 자기 엔진으로 굴린다. 그러면 **고객이 만나기 전에 자기가 먼저 벽에 부딪히고**, 실제로 1년 뒤 회고에 그 문장이 있다: **시간당 적재 DAG 하나로 굴리던 초기 구조가 원천이 늘자 확장되지 않았고 인서트가 너무 오래 걸렸다.**

세운 도메인 셋:
- **갱신을 제자리에서 하지 않고 나중에 합친다** — 백그라운드 머지 · 목적별 엔진 · `FINAL`. ⚠️ **대가가 읽는 쪽으로 넘어간다** — 정확성이 기본값이 아니라 **선택지**다(쓰면 정확하고 느리며, 안 쓰면 빠르고 틀릴 수 있다).
- **자기 제품으로 자기 창고를 짓고 한계를 먼저 만난다** — 2.1PB. **자기 제품을 쓰는 회사의 값은 자랑이 아니라 무너진 구조를 적는 문장에 있다**(재구성).
- **분석 일의 70% 를 AI 에 넘기고 30% 를 일부러 남긴다** — DWAINE. ⚠️ **핵심은 넘긴 70% 가 아니라 남긴 30%** 다(공인 재무 지표·실시간 운영 모니터링·반복 표준 보고). **출력만으로 중요한 결정을 못 하게** 막아 두었다.

⚠️ **요금 단가를 얻지 못했다** — 요금 페이지가 **저장과 컴퓨트를 따로 판다는 철학만** 적고 숫자를 안 준다. **`요금 단위가 아키텍처를 드러낸다` 를 이 회사에서는 절반만 쓸 수 있다.**

⚠️ **매출도 3자 수치가 갈린다**(ARR 2.5억 vs 3.5억, 고객 3,000 vs 4,000). **회사가 직접 밝힌 것만 썼다** — 관리형 고객 3,000곳 이상, ARR 250% 이상 성장, 시리즈 D 4억 달러.

### 다음 사이클 — 확장

⏳ **첫 일이 정해져 있다** — **`building-a-data-warehouse-with-clickhouse-part-2`(A year later) 본문을 읽는다.** 초기 구조가 무너진 것까지는 확인했고 **어떻게 넘었는지가 남았다.** 그게 `자기 창고` 도메인의 기능이 된다.

**AI 도메인도 자료가 이미 충분하다**(남긴 30% · 가드레일 · LibreChat/MCP 선택). **엔진 도메인은 자료를 한 편 더 찾는 것이 낫다** — 지금 근거가 일반론 성격의 글 하나뿐이다.
