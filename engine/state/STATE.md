...
**Deno 완주(기능 2개).** 회사 144개, **큐 0/3 — 다음 사이클은 후보 조사다.**

### 자기 근본 설계를 틀렸다고 적은 회사

레지스트리 없이 URL 로 모듈을 가져오게 한 것이 이 런타임의 출발이었는데, 창업자가 **"상당한
대가가 따랐다"**고 되짚는 글을 냈다(2024-07-29). 문제 넷을 이름 대어 적는다 — 주소가 길고,
한 버전에 묶여 변종이 여럿 생기고, 아무 서버에나 있어 CI 와 배포를 망가뜨리고, `deps.ts` 가
`package.json` 보다 번거롭다.

### 버리지 않고 감쌌다

JSR 은 **뒤에서 HTTP URL 로 가는 똑똑한 리다이렉트**다. 유의적 버전·중앙 호스팅·자동 중복
제거를 주되, 정말 좋았던 것(큰 타르볼 없이 잘게 받기)은 그대로 물려받았다고 적는다.

### 죽으면 안 되는 경로에는 자기 코드를 안 둔다

모듈 서빙을 구글 인프라에 맡기고 **그 뜨거운 경로에 자기 코드를 하나도 두지 않는다** — 구글이
죽어야 JSR 도 죽는다고 말한다. 가동률 목표도 모듈 서빙 100%, 나머지 99.9% 로 가른다.

### 탐침을 물린 이유가 물리다

확장자를 바꿔 가며 파일을 찾는 방식은 HTTP 위에서 읽기마다 수십 밀리초가 들어 파일 수백 개면
감당이 안 된다. 그래서 **올릴 때 임포트를 명시적으로 고쳐 써서** 찾을 필요 자체를 없앤다.

### 만드는 쪽과 쓰는 쪽을 함께 판 첫 쌍

Val Town(쓰는 쪽)과 Deno(만드는 쪽)가 같은 문제 — 믿을 수 없는 코드 실행과 의존성 — 를 양쪽에서
다룬다. 발타운은 잠금 파일로 붙들었고 데노는 레지스트리 설계로 바꿨다.

### 다음 사이클 — 후보 조사 (큐 0/3, 비었다)

기준은 그대로 — 회사 소유 도메인 · 엔지니어가 직접 쓴 글 · 자기 시스템의 의사결정과 대가.
회고를 먼저 찾는다.
