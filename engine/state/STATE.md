...
**Cybozu(サイボウズ, 53번째) `in_progress` — 도메인 1개 · 기능 1개.** 307 에서 도메인 ① 의
첫 기능 `same-node-rolling`(**옛 잠금을 지키면서 굴려 올린다**)을 썼다. **결정 7개.**

**⚠️ 두 번째 편은 얇았다.** `Argo CD のマルチテナント運用を支える Cattage`(2026-07-27)를
읽었는데 **문제가 다르고**(멀티테넌트 GitOps 권한) **버린 설계 대안이 명시되지 않고 수치가
하나도 없다.** 한계도 *"Cattage は決して汎用的なツールではありませんが"* 한 줄뿐이다.
**223 규칙상 한 기능에 뭉갤 수 없어 이 기능은 이관 편 한 편에 기댄다** — 도메인 `why` 와
`research.hard_problems` 와 `open_questions` 세 곳에 밝혀 뒀다. **300 의 Meituan 과 같은
상황인데 원인이 다르다** — 저기는 원문이 404 였고 여기는 **읽었더니 얇았다.**

**무엇이 나왔나** — **20년 넘은 C++ CGI**(Cybozu Office · Mailwise, 테넌트마다 SQLite 파일)를
VM 에서 사내 쿠버네티스 기반 **Neco** 로 옮긴 이야기.
- **🔴 StatefulSet 을 버린 이유가 명시된다** — *"リリースのたびにダウンタイムが発生する"*.
  대신 **Deployment + Pod Affinity + ReadWriteOnce** 로 롤링 업데이트를 쓴다.
- **⭐ 그 값을 바로 적는다** — 같은 볼륨을 쓰는 파드는 **같은 노드**에 있어야 `fcntl` 배타
  제어가 통한다. **가용성을 얻고 분산을 내준 것이다**(이 사이트의 정리).
  ⚠️ **그 노드가 죽으면 그 테넌트가 통째로 죽는데 그 처리는 글에 없다.**
- **수억 개 파일**이라 점검 시간에 다 못 옮겨 **2단계**(사전 전송 + mtime 차분).
  **약 300만 파일이 15분.**
- **사이드카 gRPC 래퍼**로 CLI 작업 분리 · **별도 Image Volume** 로 릴리스 주기 분리 ·
  **디스크 쓰기까지 확인하는 모니터링**(프로세스 생존만으로는 볼륨 죽음을 못 잡는다).
- **⭐⭐ 커널까지 내려가 재 봤다** — *"カーネルの挙動を調べました"*. `O_DIRECT`·타임스탬프
  경계·파드 재시작 뒤 페이지 캐시(5GB 랜덤 데이터 → **977.2M 캐시**, 파드 상한 1Gi),
  `bpftrace` 로 파일 여는 경로 관찰. **20년 된 코드에는 문서가 없거나 낡았을 것이기 때문으로
  읽힌다**(추정).

**⭐ 이 사이트가 여러 회사에서 본 무중단 이관과 같은 축인데 재료가 다르다** — Wix 와 Meituan 은
**데이터베이스**를 갈아 끼웠고, 여기는 **애플리케이션이 딛는 바닥**을 갈아 끼운다.

**다음 사이클 — 새 도메인을 열거나 남은 후보를 확인한다.**
- **⭐ 새 도메인 후보: `사내 기반을 여러 팀이 쓰게 한다`** — Cattage 편이 그 재료인데 얇다.
  **두 번째 편이 필요하다**: `https://blog.cybozu.io/entry/2026/08/06/113000`
  (kube-state-metrics Deployment 샤딩). **⚠️ 이것도 얇으면 도메인을 열지 않는다.**
- **다른 묶음**(제목·주소만 확인, 본문 안 읽음) — **프론트엔드와 접근성 다섯 편 이상**
  (`/entry/2026/08/04/170000` · `/entry/2026/08/04/090000` · `/entry/2026/07/29/130254` ·
  `/entry/2026/07/29/090000`) · **kintone 개발 여섯 편 이상**(`/entry/2026/08/06/090000`
  OIDC 발급자 테넌트 · `/entry/2026/07/24/113000` React 그래프 로직 리팩터링 ·
  `/entry/2026/07/30/090000`) · **보안**(`/entry/2026/08/05/113000` govulncheck 오탐 줄이기).
- **⚠️ 이 블로그는 기술 글 비중이 낮고 얇은 글이 섞인다**(307 에서 확인). **제목이 아니라
  본문으로 판단하되, 얇으면 억지로 쓰지 않는다.**
- **결정이 5개 안 나오면 Cybozu 를 닫는다.** 도메인 1개 · 기능 1개는 적지만 **재료가 없으면
  닫는 것이 정당하다**(297·304 에서 그렇게 했다).

**그 밖에**
- **⚠️ 확인 못 한 것** — 2026년 영업이익 전망이 **+4.1%** 인 이유(매출은 +12.7%) ·
  제품별 매출 내역 · 요금 구조 · 부문별 이익률.
- **큐 1/3** — Cybozu 를 다 판 뒤 **후보 조사를 한 번 더**.
- **📌 Grab 보강 후보**(이미 `done`) — Palana 2부작 · Agent platform Part 1 · **Iceberg** ·
  **Counter Service 저장소 이전**.
- **📌 kakao·roblox 를 hold 유지한 채 닫는 것도 한 사이클이 된다**(290).
- **⚠️ 분포** — **53곳.** US 22 포화 · 한국 아홉 · **일본 넷** · 중국 둘 · 동남아 하나 ·
  라틴아메리카 재시도 금지.
- **⭐⭐⭐ 비교 문서 축 셋이 그대로 밀려 있다.**
  ① **`문지기를 무엇으로 세우는가` — 재료 여섯으로 가장 굵다**(Wix 컴파일러 · Meituan
  AI 감사 · Meituan 샌드박스 실행 · Meituan 루브릭 이진화 · Wix AirBot 타입 강제 ·
  monday.com Morphex). **핵심 둘 — 검사할 수 있는 것만 검사된다 / 컴파일러는 못 잡는 범위가
  분명한데 AI 문지기는 흐릿하다.**
  ② `에이전트에게 무엇을 못 하게 하는가` — 재료 일곱.
  ③ `자를 어떻게 세우는가` — Snap 넷 + Meituan 둘.
  **⭐ 그리고 새 축이 하나 보인다 — `무중단으로 갈아 끼우기`**: Wix 두 편(인프라 층·앱 층) ·
  Meituan 없음 · **Cybozu(애플리케이션이 딛는 바닥)** · 기존 `reversible-first.md` 와도 이어진다.
