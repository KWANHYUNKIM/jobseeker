...
**없다 — 문체 보수 사이클이었다.** Neon 을 배민 문체로 다시 썼다. 회사 115개, 큐 2/3.

### 사다리에 문체 보수 단이 생겼다

사용자가 배민(초반에 쓴 회사)과 최근 회사들을 견주게 했고, 문체가 흘렀다는 것이 드러났다.
초반 회사들(배민·토스·네이버·쿠팡·스트라이프)은 평문인데 중간에 경고 기호가 들어오고 나중에
별표까지 붙어, 본문 1000자당 강조 기호가 2~3개인 회사가 절반을 넘었다. 그 기호를 따라 눈이
튀어서 문장이 이어지지 않는다.

STYLE.md 6번에 기준 문서를 baemin.json 으로 고정했고, validate.py 가 본문 1000자당 기호를
재서 0.5 이상을 문체 빚으로 잡는다. 사다리 3순위(재방문 위)에 문체 보수 단을 넣었다.
재방문은 바깥이 변해서 도는 무한 루프이고 이쪽은 끝나는 일이라 먼저 끝낸다. 지금 72곳이 걸렸다.

### 이번 사이클 — Neon (4.29 → 0.0)

고친 것 넷이다.

첫째, 강조 기호를 본문에서 전부 걷어냈다. 중요한 것은 문단의 첫 문장에 놓거나 앞 문장과
대비시켜 드러낸다. 굵게는 한 문단에 한 번까지만 남겼다.

둘째, 백틱 인용을 풀어 썼다. 회사가 한 말을 통째로 붙이면 조사 기록이지 설명이 아니다.
원문은 sources 의 한 줄 요약에만 두고 본문은 내 말로 다시 썼다. 인용 밀도도 0 이 됐다.

셋째, 앞선 회사와 견주는 문장을 넣었다. Databricks 가 이 회사를 인수해 Lakebase 가 됐다는
사실을 프로파일에 적고 두 항목을 별개 사례로 세지 말라고 못박았다. S3 를 진실 원천으로 삼는
축에서 WarpStream·Cursor 와 견주고, 유휴를 다루는 축에서 Vercel·Cursor·Modal 과 견줬다.

넷째, 도메인 경계의 근거를 적었다. 저장 구조와 브랜칭을 따로 볼까 했으나 브랜치가 결국
특정 LSN 을 가리키는 포인터라 같은 자료구조 위에 서 있어 한 경계로 뒀다는 것, 자동 확장과
0 으로 내리는 것이 같은 관측 신호 위에 서 있어 한 경계로 뒀다는 것이다.

기능 이름도 배민을 따랐다. `로그가 데이터베이스다 — 파일은 그것의 캐시일 뿐`,
`크기를 미리 고르기 — 81초마다 바뀌는 값을 사람이 정하는 일`.

내용은 지우지 않았다. 수치와 거절한 대안과 대가를 그대로 두고 문장만 다시 썼다.

### 다음 사이클 — 문체 보수 (사다리 3순위)

71곳이 남았다. 무거운 순으로 cursor(4.2) · wiz(3.87) · coinbase(3.83) · victoriametrics(3.78)
· vercel(3.75) 순이다. 새 조사를 하지 않는 사이클이라 읽고 다시 쓰기만 하면 된다.
