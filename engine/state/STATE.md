...
**없다 — Astral 을 완주시켰다(도메인 2 · 기능 2 · `status: done`).** 회사 168개, **큐 0/3.**

### 해시는 바이트가 바뀌었는지만 본다

이 회사에서 가장 값어치 있는 대목이다. 공급망 방어의 바닥에 **다이제스트 검증**이 있는데,
uv 의 ZIP 파싱 차이는 **바이트를 바꾸지 않는다.** 같은 파일을 두 파서가 다르게 해석할 뿐이라
해시도 서명도 걸러 내지 못한다 — **무결성과 해석이 다른 문제**라는 것이 드러난 자리다.

### 규격이 모호하면 한 구현이 고쳐도 안 끝난다

중앙 디렉터리 오프셋이 절대값인지 EOCDR 기준인지 **규격이 정하지 않는다.** uv 가 아무리
엄격해져도 다른 파서가 다르게 읽으면 차이는 남는다. 회사도 생태계 조율이 필요하다고 적는데
**pip 쪽 병행 수정은 언급하지 않는다.**

### 조이기 전에 깨질 폭을 먼저 쟀다

보안을 조이는 변경은 **막는 것보다 잘못 막는 것이 무섭다** — 설치 도구가 멀쩡한 패키지를
거부하면 생태계가 멈춘다. 상위 15,000개로 먼저 돌려 무해한 오류 셋만 걸린다는 것을 확인하고,
그래도 탈출구를 남기되 **이름에 `INSECURE` 를 박았다.**

### 자기 판단과 매긴 등급이 다르다는 것을 그대로 쓴다

**"실제 영향은 낮다고 보지만 지나칠 만큼 조심해서"** 보통으로 뒀다. 낮게 매기고 조용히
넘어가는 길이 있었는데 그러지 않았다.

### 읽는 사람이 가장 먼저 묻는 질문이 답 없이 남았다

제3자 인덱스는 점검하라면서 **이미 설치되었거나 캐시된 패키지에 대해서는 완전히 침묵한다.**
좋은 사후 기록에도 빈칸이 있다는 사례를 또 하나 얻었다(메타베이스 여섯 가지 · Fastmail
일곱 가지에 이어).

### 심은 대비

**체인가드**('패치를 만드는 것은 쉽고 믿는 것이 어렵다') ↔ **Astral**(**믿는 방법 자체가
통하지 않는 경우**를 보여 준다). **지그 재단** ↔ Astral(릴리스 공지 안에 설계 결정을 담는
같은 부류).

### 다음 사이클 — 후보 조사 (큐가 0/3 이다)

되살리기가 다섯 번 값을 했지만 지난 조사에서 **셋 중 하나만 뒤집혔다**는 것도 기록에 있다.
**CAD 축과 의료 축은 여전히 비어 있다.** 못 읽은 곳: Hugging Face(첫 화면만 봤다) ·
Lunit(medium 403) · Blender(403) · dbt Labs · incident.io(404 네 번) · Guardian ·
Flexport · Cal.com. **이 세션의 WebSearch 예산은 끝났다.**
