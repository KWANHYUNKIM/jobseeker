...
**없다 — Anthropic 을 완주했다.** 도메인 **3** · 기능 **3** · `status: done`. 회사 112개. 큐 2/3.

### 이번 사이클 — `흐릿한 신고에서 세 원인을 갈라낸다`

⭐ **`품질이 나빠졌다`는 신고는 티켓이 아니라 안개다**(재구성) — **`각 변경이 서로 다른 트래픽 조각에 서로 다른 일정으로 영향을 주어, 합쳐 놓으면 넓고 일관성 없는 저하처럼 보였다.`** ⚠️ **처음에는 `사용자 피드백의 정상적인 변동과 구별하기 어려웠고, 내부 사용도 평가도 재현하지 못했다.`**

세 원인과 각각이 통과한 방어:

- **03-04 추론 노력 `high` → `medium`** — 근거가 있었다: **`대부분 작업에 약간 낮은 지능과 훨씬 적은 지연`**. ⭐ **평가가 틀린 게 아니라 목적함수가 틀렸다** — **`내부 평가가 지연보다 지능을 원하는 사용자 선호를 담지 못했다.`**
- **03-26 캐싱 버그** — 한 번만 지워야 할 것을 **`남은 모든 턴에서 계속 지웠다.`** ⚠️⚠️ **못 잡은 이유가 시험 부족이 아니었다** — **`메시지 큐잉과 관련한 내부 전용 서버 실험`** 과 **`사고를 표시하는 방식의 직교한 변경이 대부분의 CLI 세션에서 이 버그를 감췄다.`** **낡은 세션이라는 구석 사례**에서만 났다.
- **04-16 길이 제한 프롬프트** — **`몇 주에 걸친 내부 시험과 회귀 없음`**을 통과하고도 나중 절제 연구가 **Opus 4.6·4.7 양쪽에 3% 하락**을 드러냈다.

### ⭐ 이번에 건진 가장 큰 것 — 관련 없는 변경이 방어가 아니라 은폐가 된다

**그래서 대책이 `시험을 더`가 아니라 `내부에서 공개 빌드를 쓴다`이다** — **`새 기능을 시험하는 버전이 아니라 공개된 바로 그 빌드`.** ⭐ **실험이 얹히지 않은 것을 봐야 한다**(재구성).

⭐ **바꾸기로 한 것이 앞의 실패 하나씩에 대응한다** — 선호를 못 담은 평가 → **침지 기간·점진 배포**, 실험이 가림 → **공개 빌드 자체 사용**, 좁은 평가 묶음 → **모델별 넓은 평가**, 한 줄의 영향을 모름 → **절제 연구·프롬프트 감사 도구**, 여러 모델에 닿음 → **의도한 모델만 겨냥하는 지침.**

### ⭐⭐ Block 과 정면으로 갈리는 자리를 찾았다

**Block 의 Moneybot 은 `관측할 수 없는 행동을 고치려 들지 않는다`며 재현을 조건으로 걸었다.** ⚠️ **Anthropic 은 재현 없이 들어갔다** — 합계로는 영영 안 보였을 것이기 때문이다.

⭐ **어느 쪽이 옳은지가 아니라 무엇을 잃을 수 있느냐가 다르다**(재구성) — **저기는 `고치지도 않고 고쳤다고 적히는 것`을 막았고, 여기는 `영영 안 보이는 저하`를 막았다.** **비교 문서 재료로 아주 좋다** — ㊵축(`모르는 것을 어떻게 적나`)의 정면 대비다.

### ⭐ Anthropic 완주 관찰 — 세 도메인이 한 문장으로 이어진다

**재는 법**(자 자체를 의심한다) · **돌리는 법**(뇌와 손을 떼어 놓는다) · **틀렸을 때 밝히는 법**(틀린 것을 공개한다). ⚠️ **그리고 셋이 서로를 물고 있다** — **평가를 넓히는 것이 대책인데 그 평가가 인프라 잡음을 타고**, **다중 에이전트가 품질을 올리는데 평가 오염을 3.7배 키우고**, **하네스 부품이 품질을 올리려다 세 버그 중 둘이 됐다.**

### 다음 사이클 — 신규 (6순위)

**큐 2/3.** ⏳ 대기 — **Databricks**(⚠️ **본문 접근 경로를 먼저 확인한다** — `r.jina.ai` 가 트래커 픽셀을 준다. `docs.databricks.com` 이 열릴 수 있다) · **Modal**.

⏳ **Anthropic 보강 거리** — `Claude Code auto mode`(손이 무엇까지 해도 되나) · `How we contain Claude across products` · `Building a C compiler with a team of parallel Claudes` · **학습 인프라 자료**(⚠️ 여전히 못 찾았다).
