...
**없다 — Deliveroo 완주.** 도메인 3/3 · **기능 3개.** 회사 **89개.** ⚠️ **큐가 0/3 이라 다음 사이클은 3순위(후보 조사)다.**

### 이번 사이클 — `사기를 다 없애는 방법은 주문을 다 없애는 것이다`

건진 것 넷:

- ⚠️ **전부 검사하면 두 번 손해다.** 첫째는 **마찰로 잃는 주문**(주문량·이익·유지·경험이 다 떨어지고 피해는 대부분 정상 고객에게 간다). 둘째가 뜻밖이다 — **3DS 비율이 극단적으로 높으면 발급사가 그 가맹점을 위험하다고 평가해 승인률이 오히려 떨어진다.** **안전하려는 행동이 안전하지 않아 보이게 만든다**(재구성).
- ⚠️ **`사기를 얼마나 막았나` 로 재면 규칙은 언제나 옳아 보인다** — **잃은 주문은 어느 장부에도 안 남기 때문이다.** 그래서 판정 기준을 식으로 세웠다 — **추가 영업이익 + 절약된 3DS 수수료 − 추가 차지백 − 추가 보상.** 회사의 문장: **`물론 사기를 최대한 줄이고 싶지만 그렇게 하는 것이 회사 전체에 옳은 결정이 아닐 수 있다.`**
- ⚠️ **막는 쪽의 자산은 낡는데 공격하는 쪽은 옮겨 다닌다** — **오래 산 규칙은 사기꾼이 다른 벡터로 가면서 점점 무용해진다.** 그래서 이 도메인에서는 **검증이 한 번의 일이 아니라 상시 업무**가 된다(재구성).
- **결과 — 3DS 비율을 40% 가까이 낮췄고 사기 위험은 늘지 않았다.** 2022년 4분기 대규모 A/B(대조군은 기존 규칙대로, **실험군은 그 규칙에서 면제**). ⚠️ **대가가 무거운 실험이다 — 실험군을 실제 사기에 노출시킨다.**

### ⚠️ 이번에 발견한 회사 안의 어긋남

**같은 회사가 다른 글에서 `공통 도구와 표준 절차로 팀 간 비교를 쉽게 한다` 는 원칙을 세웠는데, 이 3DS 실험은 사내 실험 플랫폼이 아니라 벤더(Ravelin) 위에서 돌았다.** 얻은 것은 **피처 플래그·로깅 같은 엔지니어링 작업 없이 실험할 수 있다는 것**이고, 잃은 것은 **이 결과를 다른 팀 실험과 나란히 놓기 어렵다는 것**이다(재구성). ⚠️ **한 회사의 글 두 편이 서로 다른 말을 하는 것을 잡아 낸 사례로 기록해 둔다 — 회사 하나를 여러 사이클에 걸쳐 파야 보이는 종류의 발견이다.**

### 다음 사이클 — 후보 조사(3순위)

⚠️ **큐가 0/3 이다.** ⏳ 되짚을 목록 — `www.meesho.io/blog` · `blog.zepto.com` · `bytes.swiggy.com` · `www.gojek.io/blog`(429) · `tech.cygames.co.jp` · `www.scylladb.com/blog`. ⏸️ **Snyk 보류** — `New Agentic Architecture for Snyk Agent Fix` 를 열고 결론 낸다.

⭐ **이번 회사에서 새 도구를 얻었다** — **`r.jina.ai` 프록시.** 국가 차단·403 으로 접었던 곳을 다시 두드릴 때 **브라우저 다음의 세 번째 수단**으로 쓴다. ⚠️ **WebFetch 로 부르면 요약이 오니 브라우저 `get_page_text` 로 받아야 원문 인용을 쓸 수 있다.**

⏳ **보강 재료가 하나 남아 있다** — `deliveroo-ml-platform`(2025-07-02). 회사가 done 이 된 뒤의 보강 사이클 몫이다.
...
**③ `실험 결과를 얼마나 믿나` 2곳** — **Etsy**(홀드아웃 8%·승자의 저주 공개) / **Deliveroo**(다섯 원칙·**안 하기로 정하는 법**·그리고 **3DS 에서는 그 규율을 벤더 도구 위에서 돌렸다**). **도구로 재는 쪽과 규율로 세우는 쪽이다.**
