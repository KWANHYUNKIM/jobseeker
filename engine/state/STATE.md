...
**Gunosy** — 첫 기능 `what-counts-as-good`(좋은 추천인지 무엇으로 판정할까)을 끝냈다.
**도메인 2 · 기능 1.** 큐 0/3. 남은 도메인은 **늘릴 때 손이 얼마나 드느냐로 기반을 고른다.**

### 두 문제가 한 뿌리였다

**모델 쪽**은 시청 시간을 목적 함수에 직접 넣으려다 **학습이 안정되지 않아** 접었고,
**판정 쪽**은 CTR 이 올라도 **'리스트를 하나하나 눈으로 확인해야 하고 그 공수가
비대해진다'** 였다. 둘 다 **좋은 추천의 정의가 없어서** 생긴다 — 목적 함수로도 못 박고
사람 눈으로도 다 못 본다. 그래서 **모델은 입도를 세밀하게, 판정은 LLM 을 자로** 우회했다.

### 문제를 공수로 정의하면 자동화의 대상이 분명해진다

'맞는 추천인가'가 아니라 **'눈으로 보는 데 공수가 비대해진다'** 로 적으니 무엇을 자동화할지가
나온다. **그런데 답을 공수로 재지 않는다** — 얼마나 줄었는지도, **LLM 판정과 사람 판정의
일치율**도 없다. **자를 바꿨으면 새 자의 정확도를 재야** 하는데 그 칸이 비어 있다.

### 요금제가 아키텍처를 정한 드문 사례

**'추천 리스트의 정성 평가에 Copilot 을 쓰면 Premium Request 가 순식간에 사라진다'** —
**능력이 아니라 요율 제한**이 도구를 갈랐다. 컨벡스는 **설계자가 `count()` 를 막았는데**
여기는 **요금제가 막는다.**

### 이 회사는 근거를 반복해서 '사람 손'에서 찾는다

기반 선택은 **3대 → 10대에 EBS 7개를 손으로**, 판정 자동화는 **리스트를 하나하나 눈으로.**
**두 도메인의 결정 근거가 같은 자리**에 있다.

### 다음 사이클

확장 — 남은 도메인(쿠버네티스). **근거가 넷으로 갈려 있다**(StatefulSet · Argo Rollouts ·
네임스페이스 · PDB). 큐가 0/3 이라 그 뒤에 후보 조사가 뜬다.
