...
**없다 — 문체 보수 사이클이었다.** 웨이페어를 배민 문체로 다시 썼다(1.25 → 0.0, 기호 270개). 회사 115개, 큐 2/3. 13곳 남았다.

### 같은 판정을 두 자리에서 서로 다른 예산으로 한다

창고 컨베이어는 1초 안에 끝내야 하고, 고객이 보낸 사진은 꼬리에서 몇 초를 받아들이는 대신
비동기로 돈다. **무엇을 판정하느냐가 아니라 어디서 판정하느냐가 지연 예산을 정한다.**

드문 것을 잡는다는 점에서는 컬리·Meesho 와 같은 자리인데 답이 하나 더 있다 — **미결정을 정식
결과로 스키마에 둔다.** 확신이 없을 때 억지로 고르지 않는 길을 데이터 모델에 새겼다.

### 카탈로그의 칸이 검색을 방해한다 — 답이 갈린다

- Etsy 는 리스팅 1억 개가 SKU 로 안 묶여서 **사람 골든 라벨에 LLM 을 정렬**해 관련성을 다시 정의했다.
- 웨이페어는 러브시트와 2인용 소파의 경계가 파는 쪽 분류일 뿐이라 **분류 체계 자체를 검색에서
  떼어 낸다.** 붙어 있으면 사업이 카테고리를 바꿀 때마다 모델을 다시 학습시키는 값을 낸다.

### 남이 적어 보낸 숫자를 못 믿는 셋 — 틀렸을 때 오는 것이 다르다

볼트(결제대행사 보고서)와 컬리(3PL 입고 데이터)는 틀리면 **돈이 안 맞는데**, 웨이페어가 맞춰 보는
것은 물건의 치수라 **틀리면 트럭이 두 번 온다.**

### 반사실을 못 보는 평가

재는 것을 의심한 회사가 이 엔진에 다섯인데, 웨이페어의 벽은 형태가 다르다 — **다른 해결책을
골랐다면 결과가 어땠을지 모른다.** 그래서 사람이 하던 선택을 기준선으로 삼는다. 짝을 이루는 다음
기능에서는 그것이 더 분명해진다 — **사람이 하던 일의 점수(정밀도 80%·재현율 90%)가 기계의
합격선이 된다.** Trendyol 이 사람에게 거부권을 남긴 것과는 다른 방향이다.

### 도메인을 제품이 아니라 물건이 크다는 사실이 만드는 문제로 잘랐다

상태를 판정하는 일, 분류로는 못 찾는 것을 찾게 하는 일, 문제가 생긴 뒤를 수습하는 일.
매장이 없고(↔Target) 보내는 것이 소파와 침대다(↔쿠팡).

### 다음 사이클 — 문체 보수 (사다리 3순위)

--gaps 가 정한다. 지금 맨 위는 paytm(1.08)이지만 그건 이 시점의 예상이지 확정이 아니다.
