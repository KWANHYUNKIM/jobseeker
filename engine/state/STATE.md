...
**DeepL**(EU · 기계번역) — 도메인 3개 중 **1개에 기능이 들어갔다**(`layout-preserving-document-translation`).
남은 빈 도메인 둘: '모델이 자기 번역을 자기가 채점하지 못하게 한다' · '정밀도를 깎아서 처리량을 산다'.

### 잣대를 갈아치운 이야기가 본체다

번역문을 원본 자리에 다시 앉히는 기능인데, 글의 절반이 **무엇으로 재느냐**다. 픽셀
비교(SSIM)를 버린 이유가 구체적이다 — **줄 넘김과 글자 소실을 못 가르고**, 배경 이미지가
크면 안 바뀐 픽셀이 많다는 이유로 점수가 부풀려진다. 새 지표는 의미 영역의 겹침이다.

### 세상의 단위가 이산이라 수식이 안 됐다

이번 기능에서 가장 값어치 있는 결정이다. 언어별 팽창률로 최적 글꼴 크기를 계산하려다
물렀다 — **"10.385 크기의 Times New Roman 같은 것은 없다."** 알고리즘의 한계가 아니라
**세상의 단위**가 한계였던 자리다. 대신 0.5pt 씩 오르내린다.

### 순위를 못 박은 것

모두 지킬 수 없을 때 무엇을 먼저 버리는지를 적어 뒀다 — **같은 페이지 > 같은 자리 >
요소 간 크기 비율 > 크기의 일관성.** 순위가 없으면 무엇을 내줬는지 아무도 설명하지 못한다.

### 새 지표가 새 실패 지점을 들여왔다

픽셀을 버리고 의미 영역으로 갔더니 **분할기(open-parse)가 틀리면 점수도 틀린다.** 그 틀림은
점수만 봐서는 안 보인다. 난제 칸에 이것을 적었다.

### 글이 말하지 않는 것 여섯 가지

**아홉 개 지표가 무엇인지 끝내 안 밝힌다.** 그 밖에 어느 OCR 라이브러리인지 · 허용하는 비율
창이 얼마인지 · 아랍어처럼 방향이 바뀌면 어떻게 하는지 · 여러 단은 어떻게 자르는지 · 반복
조정이 속도를 얼마나 먹는지. `failure` 도해에 함께 세웠다.

### 다음 사이클

남은 빈 도메인 둘 중 하나. 자기 채점 쪽(TQE)은 자료에 수치가 하나도 없고 발행일도 없다 —
FP8 쪽이 수치가 두껍다.
