...
**DeepL**(EU · 기계번역) — 도메인 3개 중 **2개에 기능이 들어갔다**(`independent-judge-model` 추가).
남은 빈 도메인 하나: '정밀도를 깎아서 처리량을 산다'. 큐 1/3.

### 성능 문제가 아니라 이해 상충 문제로 다뤘다

자기 모델의 자기 채점을 **더 좋은 모델로** 풀지 않고 **누가 채점하느냐**를 바꿔 풀었다.
근거도 회사가 직접 적는다 — **"LLM 이 번역을 검사하면 자기 모델의 결과를 편들고 그 약점을
놓친다."** 사람 조직에서 감사를 분리하는 것과 같은 이유다.

### 자를 대 보고 칸이 빈 것을 발견했다

MQM 을 기계 번역에 얹었더니 오타 칸이 통째로 비었다 — **"기계 번역에는 오타가 없다."**
자가 안 맞는다는 발견이 곧 **기계 번역의 오류는 다른 종류다**라는 발견이었다. 대신 걸리는
것은 **암묵적 지식의 부족**이고, 원문 자체가 헷갈릴 때(문법 오류·일부러 쓴 문체) 더 크게
드러난다.

### 사람에게 무시할 권한을 준다

고치기 · 해결로 표시 · **그냥 무시하기**가 같은 자리에 있다. 모델의 표시는 제안이지 판정이
아니다. 오탐을 걷어낼 곳을 남겨 둔 것이다.

### 닫힌 고리를 한 번 더 열었다

이 채점기를 내부 도구로 두지 않고 **고객에게 테스트 환경과 API 로 연다.** 자기 채점을
막아 놓고 자기 채점기를 안에만 두면 결국 고리가 하나 남는다.

### 지표는 평균을 보고 사람은 자기가 걸린 한 문장을 본다

연구 책임자의 말을 그대로 옮겼다 — **"사람들은 언제나 잘 안 맞는 작은 부분을 찾아낸다."**
그래서 새 언어를 낼 때 연구팀과 제품팀을 서로 밀게 한다(스위스 독일어에 여섯 달).

### 마주 보는 쌍을 하나 심었다

**앤티시시스**(결정론적 시뮬레이션으로 자기 시스템을 스스로 깨는 쪽) ↔ **DeepL**(자기가
자기를 보면 못 본다며 밖에 심판을 세우는 쪽). 같은 문제에 해법이 반대다.

### 다음 사이클

남은 도메인 하나 — '정밀도를 깎아서 처리량을 산다'(FP8 · Arion). 수치가 두껍다. 그것으로
DeepL 은 완주다.
