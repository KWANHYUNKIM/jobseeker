...
**PostHog 완주(기능 3개).** 회사 140개, 큐 0/3 — 다음 사이클은 후보 조사다.

### 미들웨어가 되지 않으려고 엔진을 하나 더 들인다

웨어하우스를 붙인 원래 목적은 손님이 **첫 데이터 엔지니어를 뽑는 순간을 늦추는 것**이었다.
그런데 회사가 30~50명을 넘으면 그 엔지니어가 들어와 데이터를 바깥으로 내보내고, 그러면
포스트혹은 중간에 낀 미들웨어가 된다.

### 쓰고 있는 데이터베이스의 약점을 이름 대어 적는다

다중 테넌시 탓에 며칠짜리 질의를 못 돌리고, 비용 기반 최적화가 없고, 스키마가 뻣뻣하고,
S3·아이스버그 카탈로그가 미흡하고, 버전마다 질의가 어긋난다 — 다섯을 적고도 **클릭하우스를
빼지 않는다.** 큰 이벤트 위의 빠른 분석은 여전히 거기가 낫다.

### 남이 아는 규약을 흉내 내 생태계를 얻는다

자체 질의 언어를 dbt 가 지원하지 않는 문제를 **포스트그레스 와이어 프로토콜**로 푼다.
이 엔진의 Dolt(MySQL 프로토콜)와 같은 수법이 다른 자리에서 또 나왔다.

### 격리의 단위가 커지는 순서

토픽(넘침) → 클러스터(워크로드) → **엔진(조직마다 단일 테넌트 덕DB)**. 방해하지 않게 떼어 놓는
같은 손버릇이 세 층에서 반복된다 — 이 회사를 한 줄로 꿰는 것이 그 순서다.

### 다음 사이클 — 후보 조사 (큐 0/3, Val Town 만 남았다)
