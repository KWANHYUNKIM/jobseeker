...
**Materialize — 첫 기능을 팠다.** 도메인 ①(미리 계산해 둔 상태를 메모리에 들고 있는다)에
`tag-byte-dictionary`(안 쓰던 태그 바이트를 사전으로 쓴다). 회사 116개, 큐 1/3.

### 새로 만든 것이 아니라 낭비되고 있던 자리를 셌다

행은 값마다 태그 한 바이트가 붙는데 **256개 중 약 128개만 쓰고 있었다.** 남는 태그를 자주 나오는
값에 배정하면 반복되는 문자열이 한 바이트가 된다. 새 형식을 들이면 바뀐 행에 바로 닿는 성질을
잃는데, 태그를 빌려 쓰면 그 성질을 안 건드린다 — **거의 공짜인 대신 컬럼 엔진만큼의 압축률은 못
낸다는 것을 회사가 먼저 적는다.**

인기 값은 Misra-Gries 요약으로 적재 중에 찾는다. **메모리를 줄이려는 기능이 세는 데 메모리를 크게
쓰면 앞뒤가 안 맞으므로**, 정확한 빈도를 포기하고 고정된 자리로 순위만 얻는다.

### 성과를 바이트가 아니라 클러스터 크기로 보인다

1600cc 에서 피크 981.0 → 428.4 GiB(56% 감소)인데 하이드레이션은 2708.6 → 2665.6초로 그대로다.
800cc 로 내리면 돌긴 하는데 3839.5초로 늘고, **400cc 는 OOM 루프에 빠진다 — 메모리가 아니라 뒤를
받치는 디스크까지 압축된 데이터를 못 담는다.**

**과금 단위가 컴퓨트 크레딧인 회사에서 압축의 값어치는 한 단계 아래 클러스터로 내려갈 수 있느냐다**
(재구성). 실패한 실험까지 같은 표에 적는 것이 이 글의 태도다.

### 이 엔진의 압축·상태 축에 붙는 자리

- VictoriaMetrics — 비용의 정체는 샘플 양이 아니라 카디널리티다.
- ClickHouse — 갱신을 나중에 합치며 그 값을 질의하는 쪽에 넘긴다.
- 누방크 — 읽기를 싸게 하려고 쓸 때 미리 접어 둔다.
- **Materialize — 넘길 데가 없다.** 미리 계산해 둔 상태를 들고 있는 것 자체가 제품이라, 줄이려면
  그 상태의 바이트를 직접 깎아야 한다.

그리고 앞선 작업과의 순서가 남는다 — **먼저 껍데기를 깎고(레코드당 96바이트 → 0~16바이트)
그다음 알맹이를 줄인다.** 그때 CPU 오버헤드를 이유로 미뤄 둔 것이 이번 사전 압축이다.

### 난제 둘을 회사가 열어 둔 채로 남긴다

- **압축 효율이 데이터와 함께 변할 때 그것이 어떻게 드러나는가** — 운영 시스템은 95%의 시간 동안
  싸고 나머지 5%에 멎어 있다고 해서 온전한 점수를 받지 못한다고 적는다. 400cc 의 실패가 그 방향의
  첫 증거다.
- **전체를 훑지 않으면서 컬럼 압축의 압축률을 얻는 방법** — 이번에 얻은 것은 약 2배다.

### 다음 사이클 — 확장 (사다리 2순위)

도메인 ②(일을 쓰는 쪽에서 할 것인가 읽는 쪽에서 할 것인가)가 비어 있다. --gaps 가 정한다.
