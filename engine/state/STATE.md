...
**없다 — Fireworks AI 를 완주시켰다(도메인 1 · 기능 1 · `status: done`).** 회사 156개, 큐 1/3.

### 어느 쪽이 빠르냐가 아니라 무엇에 묶이느냐

이번 기능의 축이다. Q-outer 와 KV-outer 중 하나가 언제나 낫지 않다 — 앞엣것은 **L2 대역폭**에,
뒤엣것은 **HBM 대역폭**에 묶인다. 그래서 교차점을 수식으로 낸다: `nsb / N < 2.85`
(`nsb` = 고른 KV 블록의 서로 다른 개수, `N` = 질의 토큰 수).

### 계산이 아니라 접근 패턴이 병목이었다

오픈소스 MSA 는 어텐션 에필로그에서 부분 결과를 흩어 쓴다. 그 **한 줄이 파이프라인을 세운다** —
흩어 쓰기가 명령 지연에 민감해서다. 그래서 타일 순서로 원본을 저장하고 **흩기와 정규화를 결합
커널로 옮겼다**(질의 위치로 모아 읽어 LSE 병합).

### 전부 읽지 않는 것이 출발점이다

희소성의 알갱이는 **128토큰 블록**이고 GQA 그룹마다 **상위 16개**만 고른다. 이 엔진에 같은
문장이 또 있다 — turbopuffer 의 포스팅 리스트도 "무엇을 안 읽을지"가 성능을 정한다.

### 커널 배수와 모듈 배수를 나눠 적는다

기준선 대비 **1.9~2.4배**, 오픈소스 MSA 대비 **약 1.6배**, 그런데 **모듈 전체로는 1.18~1.43배.**
가장 큰 배수만 내세우지 않는 회사의 자리를 데이터에 남겼다.

### 다음 사이클 — 신규 (대기 맨 위의 Zulip)

`Engineering flexible permissions`(2025-03-20, Tim Abbott)가 손에 있다 — 깃허브·깃랩의 단일
부모 제약을 인용해 물리고 임의 중첩을 택했고, 짧은 DM 하나에 25~50ms·질의 7개, 마이그레이션
800여 개 중 115개가 이 작업이었다. **⚠️ 나머지는 릴리스·보안 공지가 지배한다.**
