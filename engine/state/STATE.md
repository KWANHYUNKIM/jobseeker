...
**Polar Signals — 기능 둘.** 남은 빈 도메인은 값표 하나. 회사 132개, 큐 0/3.

### 이번 기능 — `perf-event-sampling`

늘 켜 두는 프로파일러라 찍는 값이 티가 안 나야 한다. 그래서 둘을 정했다 — **언제 깨울지는
커널 타이머에 맡기고**(우리가 시계를 들면 재는 일부터 값이 든다), **깨어난 자리에서 밖으로
안 나간다**(eBPF 로 대상 프로세스 주소 공간에서 그대로 읽는다).

설정이 그대로 보인다: 소프트웨어 CPU 클록 카운터, 빈도 방식으로 초당 19번, 꺼진 채로 걸어
두었다가 켜기, 모든 논리 코어에 거는 시스템 전역.

### 손잡이 하나에 정확도·부담·값이 다 묶였다

초당 19번을 올리면 잘 보이지만 손님이 느려지고 청구서도 는다 — vCPU 하나가 월 600만 표본,
1억 표본이 50달러다. **표본 주기가 요금표와 같은 손잡이**라는 것이 이 회사 구조의 요점이다.

### 사이클이 아니라 흘러간 CPU 시간으로 센다

사이클로 세면 코어 속도가 바뀔 때 표본의 뜻이 달라진다. 대신 캐시 미스 같은 하드웨어 카운터의
이야기는 이 설정으로 안 나온다.

### 이 방식이 못 보는 것

온-CPU 만 본다 — 잠금이나 입출력을 기다리며 서 있는 시간에는 표본이 안 뜬다. 그리고 표본은
사실이 아니라 추정이라 짧게 스치는 함수는 놓친다. 오버헤드 수치는 두 글 모두 안 적는다.

### 다음 사이클 — 확장 (Polar Signals 의 마지막 빈 도메인)

`남기는 양이 곧 값이다`. 요금표는 확보했다 — 표본 과금·짧은 보관·오픈소스 파르카의 자리.
