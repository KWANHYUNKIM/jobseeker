...
**없다 — 쉰여덟 번째 후보 조사를 돌았다. 큐 1/3.** 다음 사이클은 **신규**(Godot Engine).

### 원하는 쌍이 있다고 해서 없는 자료를 만들 수는 없다

이번 조사의 목표는 **Mastodon** 이었다. Bluesky 를 방금 완주했으니 **같은 문제에 정반대
구조(ActivityPub 서버 대 서버)를 고른 쪽**을 나란히 두면 이 엔진이 계속 심어 온 '마주 보는
쌍'의 교과서적 사례가 된다. 그런데 본문을 열어 보니 **기능 공지이고 왜·무엇을 물렸는지가
하나도 없었다.** 짝이 아깝다고 통과시키면 그때부터 데이터가 아니라 이야기가 된다.

### 대신 게임 엔진 축이 열렸다

**Godot** 은 폴링이 빠른 마우스 문제를 고치며 **물린 대안을 이유와 함께** 적는다 — raw
input 만 요청할 수도 있지만 **제목 표시줄을 끌어 창을 옮기는 것 같은 기대 동작이 깨진다.**
8kHz 캡처 모드에서 **초당 1프레임 미만**이라는 수치와, 고친 뒤에도 남는 부담(초당 480회
또는 8,000회 호출)까지 적는다.

### 의료 축 여덟 번, CAD 축 세 번

M3(일본 최대 의료 정보 플랫폼)를 열었는데 **홈에 기술 블로그 링크 자체가 없었다.** 이쯤이면
**이 도구로 닿는 범위 안에 그 축의 회사가 없다**는 것이 결론에 가깝다 — Medplum(접음) ·
Lunit(medium 403) · M3(링크 없음) · Shapr3D(개념 설명) · Zoo(제품 업데이트).
