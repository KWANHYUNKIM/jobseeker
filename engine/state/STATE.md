...
**Allegro — 32번째 회사.** 사이클 177 까지 프로파일 + 도메인 3개 + 기능 1개(`search-relevance-judge`).
**다음 기능은 `프론트엔드 배포`** — 자료 정독 완료(`2026/08 how-we-achieved-independent-deployments-in-a-micro-frontend-architecture`:
**opbox-web 하나에 126팀 · 의존성 1,206개**, 하루 두 번 릴리스 트레인이 병목, **FaaS 컴포넌트를
먼저 시도했다 네트워크 왕복 때문에 접었고**, 기본 비활성 버전 + 레지스트리 + 단계적 활성화로 옮겼다.
**2026년 6월 한 달 240.92 man-day(약 11 FTE) 절감**, 개발자 만족도 8.7/10 · NPS +64).
그 다음이 `저장을 어디에 둘 것인가`(back-to-dc). **둘 다 끝나면 Allegro 를 done 으로 닫는다.**

> **`search-relevance-judge` 에서 나온 것 — 평균 점수 하나로 모델을 고르면 두 가지 방식으로 속는다.**
> ① **실패가 안 보인다** — Qwen 3.6 27B 는 κ-quad **0.70 으로 표 1등**인데 **응답 5%가 파싱 실패**였고,
> 점수는 파싱된 95%에서만 나온 값이다. ② **다수 클래스만 맞혀도 오른다** — PLLuM 8B 는 정확도 0.84 인데
> **순서형 일치도로 재자 0.17** 이었다. 그래서 **1등을 포기하고 안 깨지는 Gemma 4 26B**(파싱 오류 0.2% 미만)를 골랐다.

> **🔑 새 비교 축 후보 — '작년에 통한 것이 올해 반대가 된다'.** 같은 회사가 **2025년 3월**에
> 합성 주석자를 만들다 **Cohen's κ 0.2** 에서 멈추고 *"아직 사람 주석을 쓴다"* 고 적었는데,
> 그 글의 결론은 **5-shot 이 낫다 · 맥락을 더하면 낫다** 였다. **2026년 8월 검색 적합성 글은 정확히
> 반대다** — *"few-shot 을 빼자 성능이 올랐고"* 부서·카테고리 맥락은 도움이 안 됐다. **회사가 그 차이를
> 설명하지 않는다**(open_question 에 남겼다). **프롬프트 기법은 회사의 자산이 아니라 그 과제·그 모델의 성질**로 보인다.

**큐 1/3 — Vinted(LT) 하나뿐이다. Allegro 를 완주한 뒤 반드시 후보 조사 사이클을 끼운다.**

마지막 사이클: 177 (2026-08-22) — Allegro 검색 적합성 평가.
