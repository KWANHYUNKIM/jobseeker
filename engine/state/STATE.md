...
**Turso** — 회사 프로파일을 썼다. 도메인 **2개**, 기능 **0/2**. 회사 **115개.** 큐 2/3.

### ⭐⭐ 이번 사이클의 핵심 — 남의 테스트를 못 쓰면 남의 코드도 못 고친다

**재작성의 진짜 이유가 성능이 아니었다.** 시작은 벡터 검색이었다 — **`확장으로 하고 싶지 않았고 더 자연스러운 문법을 원했는데, 색인된 벡터 검색을 제대로 넣으려면 바이트코드에 침습적인 변경이 필요해 결국 덜 우아한 조인 기반 방식으로 타협했다.`**

⭐ **그런데 포크(libSQL)를 접은 진짜 이유는 따로 있었다** — **`SQLite 의 테스트 스위트가 독점이고, C 구현이라 자신 있게 진화시키기 어려웠다.`** ⚠️ **고칠 수는 있어도 고쳤다고 말할 근거를 못 만든다**(재구성).

⭐ **그래서 `결정론적 시뮬레이션 테스트와 러스트의 메모리 안전을 처음부터 구조에 넣었다.`** **다시 쓰기로 한 것이 코드가 아니라 검증 능력이었다**(재구성).

### 이번 사이클 — 세운 도메인 둘

- **재현을 설계로 만든다** — ⭐ **`시뮬레이터가 버그를 잡으면 같은 시드를 넘겨 정확히 같은 사건 순서를 언제나 재현할 수 있다.`** **`서로 다른 사건 순서로 수년치 실행을 시험할 수 있다.`** **차분 테스트로 `같은 상호작용 계획을 림보와 SQLite 양쪽에서 돌려 결과를 비교`** 하고, **결함 주입 하이퍼바이저(Antithesis)로 시뮬레이터가 놓칠 버그까지** 잡는다.
- **동기 인터페이스를 비동기로 연다** — **동기가 둘 다 구체적이다**: **`모든 SQLite 질의가 빠른 것은 아니다 — 큰 데이터에 대한 집계 같은 것은 데이터가 완전히 로컬이어도 늘 느리다`**, 그리고 **`HTTP 기반 SQLite 나 S3 를 뒤에 둔 구현처럼 분산 배포`**. **동기적 `sqlite3_step` 과 달리 `데이터가 곧바로 준비되지 않으면 질의가 제어를 돌려줄 수 있게` 한다**(리눅스는 `io_uring`). **620ns → 506ns(20%).**

### ⭐⭐⭐ ㊵축(`모르는 것을 어떻게 적나`)에 네 번째 답이 왔다 — 그리고 가장 다르다

**같은 물음에 네 회사가 다르게 답한다**(재구성):

- **Block (Moneybot)** — **재현을 조건으로 건다.** **`관측할 수 없는 행동을 고치려 들지 않는다.`**
- **Anthropic** — **재현 없이 들어간다.** 합계로는 영영 안 보였을 것이기 때문이다.
- **Databricks·Modal** — **부하 시험으로 확인한다**(Valkey 를 거절한 근거가 그것이다).
- ⭐ **Turso** — **재현을 설계로 만든다.** **시드를 주면 같은 사건 순서가 다시 나온다.** ⚠️ **앞의 셋은 `이미 있는 시스템에서 재현을 구했고`, 여기는 `재현되는 시스템을 지었다`**(재구성).

⭐ **비교 문서 재료로 아주 좋다** — 그리고 **`재현 가능성을 위해 무엇을 포기했나`가 이 회사에서는 `기존 코드베이스 전체`다.**

### ⚠️ 차분 테스트의 한계도 분명하다

**SQLite 를 정답지로 쓴다** — 그래서 **SQLite 가 틀린 자리는 못 잡고**, **SQLite 에 없는 기능은 대조할 대상이 없다**(재구성). ⭐ **원본을 이기려고 다시 썼는데 검증은 원본에 기댄다.**

### ⚠️ 목록이 준 주소가 404 였다

**`우리는 Postgres 를 러스트로 짓고 있다 — 데이터베이스의 LLVM 을 써서`(2026-07-16)** 가 안 열린다. ⭐ **GitHub 저장소 설명에는 그 문구가 있다** — **`SQLite 호환이며 이제 Postgres 도 (실험적으로) 말한다. 데이터베이스의 LLVM.`** ⚠️ **뜻을 짐작만 할 수 있어 도메인으로 세우지 않았다** — **프런트엔드(SQL 방언)와 백엔드(엔진)를 가르는 것으로 보이지만 본문 근거가 없다.**

### 다음 사이클 — 확장 (2순위)

**Turso 기능 0/2.** ⭐ **`재현을 설계로 만든다`부터 판다** — **이 회사의 존재 이유이고 ㊵축의 네 번째 답이다.**

⏳ **못 읽은 것** — SQLite 암호화 연작 3편(2026-09-01~03, 튜토리얼 성격 확인 필요) · `Turso v0.7.0` 릴리스 노트(동시 쓰기·멀티테넌시가 있을 수 있다) · **`데이터베이스의 LLVM`의 뜻**(다른 경로로 찾는다).
