...
**Pinterest** (US · 소셜) — 도메인 3개 · **기능 2개**. 남은 하나는 **인프라 자원 프로비저닝**(본문 미독)이다.

### 이번 사이클 — `참여와 유지는 다른 것이다`

⭐ **앞 기능이 남긴 물음에 이 글이 정확히 답했다** — 조건부 학습 검색의 `조건이 무엇인가` 에 대한 답이 **UIC(사용자 관심 클러스터)** 다. **정적인 `팔로우한 관심사` 를 동적인 UIC 로 바꿨다.**

건진 것 넷:

- ⚠️ **문제의 뿌리가 지표에 있다** — **`참여와 유지는 다른 것이다.`** 회사가 든 장면: **`사용자가 오늘 사워도우 레시피 열 개를 저장하고도 다음 달에 이탈할 수 있다.`** **이미 좋아하는 것을 더 주는 데는 뛰어났지만 오래 남게 할 새 관심사를 찾아 주지는 못했다.** 그리고 관계가 비선형이다 — **`유지 이득이 최상위 십분위에서 가파르게 가속된다.`**
- ⚠️ **참여 최적화가 유지를 갉는 고리를 찾았다** — `팔로우한 관심사` 를 조건으로 쓰니 **`지배적 관심사 쪽으로 크게 치우쳐 작거나 떠오르는 관심사에는 표현이 거의 남지 않았다`.** **큰 관심사가 작은 관심사를 굶기는데, 유지를 만드는 것은 바로 그 작은 관심사들이 자라는 일이다**(재구성).
- ⚠️ **단위가 카탈로그가 아니라 사람의 목표다** — **`각 클러스터는 사용자가 실제로 추구하고 있는 쓰임새에 대응하지, 전역 콘텐츠 카탈로그의 한 영역이 아니다.`** **전역 공간이 아니라 그 사용자가 참여한 핀들만으로** 완전 연결 군집화(최근 500개 행동, **`모든 점이 모든 점과 충분히 유사할 때만`** 병합). **클러스터 수는 사람마다 다르다**(2개~15개).
- ⚠️ **표현만 바꿔서는 안 됐다** — 검색 조건·L1 할인·생애주기별 랭킹·UIC 인지 다양성·공유 피처 저장소 **다섯 계층을 함께 바꿨다.** **한 곳만 고치면 다른 곳에서 되돌아온다**(재구성).

⚠️ **이 글에는 수치가 하나도 없다** — 정성적 결과만 적는다. **③축(`자기 성과를 어디까지 주장하나`)의 반대편 사례로 좋다** — 앞 기능(검색)은 `7자리 절감·85% 감소` 를 대는데 같은 회사의 이 글은 아무 숫자도 대지 않는다.

### 다음 사이클 — 확장(2순위)

`--gaps` 가 **`인프라 자원을 안전하게 내준다`**(자원 프로비저닝 파이프라인 RPP, 2026-07-22)를 부를 것이다. ⏳ **본문 미독** — **URL 확보해 뒀다.** **이걸 쓰면 Pinterest 완주다.**

⚠️ **큐가 0/3 이다.** 완주 뒤 **3순위(후보 조사)** 가 걸린다. ⚠️ **지난 조사에서 3곳을 못 채웠으니 다음엔 다른 각도를 쓴다** — 컨퍼런스 발표 목록이나 **이미 판 회사의 글이 언급하는 다른 회사**.

⏳ **보강 거리 넷** — Grafana Labs 인용 대조 · Razorpay 보안 트리아지 글 · Flipkart Rate Card 엔진 글 · **Pinterest 2부(예측 UIC)**.

### ⚠️ 비교 문서 재료 (초안 유지)

**① `AI 에이전트를 어디까지 믿나` 9곳 + ⏳ Pinterest 2부** — Sentry / ClickHouse / Duolingo / Ramp / DoorDash / Deliveroo / Snyk / Cygames / Razorpay. ⏳ **Pinterest 2부가 `사용자가 신호하기 전에 미래 좌표를 생성` 한다 — LLM 기반 추론 포함.**

**② `관리형 MySQL 의 한계` 3곳 + Cygames** — Etsy / Plaid / Paystack.

**③ ⭐ `자기 성과를 어디까지 주장하나` 7곳** — Snyk / Grafana Labs / ScyllaDB / Razorpay / Flipkart / Airbnb / **Pinterest**(⚠️ **같은 회사 두 글이 갈린다** — 검색 글은 `7자리 절감·p90 85% 감소`, UIC 글은 수치 0).
...
**⑭ ⭐ `검색 관련성을 누가 정하나`** — Etsy / Flipkart / ⏳ Zepto / **Pinterest**(⚠️ **`카탈로그의 분류가 아니라 그 사람의 목표` 라고 답한다** — 이 축에서 가장 뚜렷한 입장이다).
...
⏳ **⑱ 새로 보인다 — `무엇을 최적화할지를 바꾼 순간`** — **Pinterest**(참여 → 유지) / **Duolingo**(⏳ 확인 필요) / **DoorDash**(⏳ 확인 필요). ⚠️ **지표를 바꾸면 시스템 다섯 계층이 따라 바뀐다는 것이 이 축의 값이다.**
