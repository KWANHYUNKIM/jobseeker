...
**아무것도 안 판다 — 하이퍼커넥트를 닫았다(46번째, 도메인 3개 · 기능 4개). 큐 1/3(컬리).**

258 에서 **`arch-migration`(쓰던 것을 다른 칩 위로 옮긴다)** 을 쓰고 회사를 닫았다.
결정 6 · 그림 3 · 생각 4 · 스택 6 · 난제 2 · ui 6핀 · 연결 1(`raid-recovery` 와 **대비**).

**📌 원문 재독이 여덟 번째로 옳았다**(223·229·243·249·250·252·255·258). "버린 대안이 없다" 고 판단해
닫으려던 글에서 넷이 나왔다.
- **QEMU 를 버린 이유가 둘 명시돼 있다** — *"apt-get 관련된 명령어를 수행할 때 **10배 이상 느려지는
  경우**도 있었습니다"* 와 *"QEMU는 완벽하지 않습니다…**segmentation fault** 등이 발생하는 경우가
  있습니다"*. **버린 대안이 아예 없는 게 아니라 '무엇으로 옮길까' 층에만 없었다.**
- **Istio 비공식 빌드를 받아들인 판단** — *"물론 이론과 실제는 다를 수 있어 걱정이 되었지만"*, 근거는
  **아래층(envoy · golang)이 이미 ARM64 를 지원한다는 것**이었고 결과가 *"30+개의 cluster에서 어떤
  문제도 일으키지 않고 잘 작동"*.
- **옮기는 순서에 기준이 있다** — 관리형 서비스가 *"상대적으로 쉽습니다"* 라 먼저, 지표를 이해하는
  팀은 카나리, *"상대적으로 중요도가 떨어지며 Java/Kotlin만을 사용"* 하는 것은 DevOps 단독.
- **🔴 롤백 기준이 통째로 없다** — 관찰 창(한 시간에서 사흘)은 있는데 **중단 임계도 되돌림 조건도
  없어서 판정이 "문제가 안 보였다" 로 남는다.** 200개 넘는 서비스를 1년에 옮기는 일에서 큰 빈칸이다.

**🔴 회사를 닫으며 `business_model` 에 패턴 넷을 적었다**(이 사이트의 정리라고 밝혔다).
① **실시간이라는 조건이 세 층을 관통한다** — 무엇을 목표로 삼을지(시차 때문에 아하 모먼트가 필요했다) ·
재료를 얼마나 빨리 만들지(Exactly Once 를 껐다) · 바닥이 흔들릴 때(하루짜리 복구가 위험 구간이었다).
② **제품 쪽과 인프라 쪽 글의 성격이 다르고, 인프라 쪽도 갈린다** — ScyllaDB 는 촘촘하고 ARM64 는
**비중(47% 이상)만 있고 실제 절감액이 없다.**
③ **실패는 어느 쪽이든 적는다** — PUR 하락 · 복구 시 조합 성공률 하락 · SIGSEGV 와 빌드 10배 지연.
④ **글끼리의 연결이 고르지 않다** — 목표 지표 3부작은 서로를 링크하고 ARM64 글은 Flink 와 아자르
match 서버를 언급하는데, Flink 조인 글과 이벤트 기반 추천은 서로를 모르고 **ScyllaDB 글은 아자르도
Flink 도 언급하지 않는다.**

**안 연 글** — 클러스터 이전(2020-11, **6년 전**). 열지 않고 닫았다.

**⚠️ 사업 수치를 끝내 못 찾은 채로 닫았다** — Match Group IR 홈에 하이퍼커넥트도 아자르도 없다.
받은 것은 **LTM 매출 $3.5B**(2026-06-30 종료 분기)뿐이고, 단서는 2026-01 글이 **Match Group 의 2023 2Q
실적**을 참조한다는 것뿐이라 **소유 관계를 단정하지 않았다.** `open_questions` 에 남겼다.

**🚨 다음 사이클 — `--gaps` 가 [신규]를 가리킨다. `## 대기` 맨 위인 컬리를 `in_progress` 로 만들고
PROMPT.md 3단계(회사 프로파일)까지 판다. 47번째 회사다.**
**⚠️ 본업 재료 둘부터 본다** — `/blog/image-based-misdelivery-detection`(**배송 완료 사진으로 오배송을
On-device 로 탐지**, 2026-04-28) · `/blog/2026-outbox-pattern-and-retry-topic`(**입고 시스템의 외부
인입 데이터 동기화**, 2026-01-12). **253 에서 읽은 `/blog/2026-delivery-domain-rag` 는 사내 개발
도구라 본업이 아니다**(다만 **버린 두 단계에 이유가 각각 있어 재료로는 좋다** — 역색인은 *"색인 유지
자체가 부채"*, 전문 임베딩은 **문서의 85% 가 512 토큰 한계를 넘겨** 잘렸다).
**카테고리 링크** — `/engineering` · `/ai-data` · `/product`. **⚠️ 244 의 교훈: 태그 총 건수를 새 재료의
양으로 읽지 않는다.** **자리** — 쿠팡은 종합 익일배송, 배민은 음식 배달인데 **여기는 신선식품 콜드체인**이라
**문 앞에 두고 가는 배송의 판정**이 소프트웨어 문제가 된다(확인되면). 국가 `KR`, 카테고리 **커머스**.
**business_model 은 1차 자료로** — 컬리는 **상장사다**(확인 필요). IR 을 찾아보고 **못 찾으면 지어내지
말고 `open_questions` 에 남긴다.**

**⚠️ 한국 편중 — 컬리를 파면 아홉이다. 다음 후보 조사는 한국 밖에서 한다.**
...
- **`되돌릴 수 있는 것을 먼저 한다`** — CyberAgent `env-split-migration` 은 **반례**(되돌림 계획이 없다),
  하이퍼커넥트 `stream-join` 은 **되돌리는 길이 배포 절차 자체**, `raid-recovery` 는 **늘 두 벌을 유지해
  되돌릴 것도 없이 채운다**, `arch-migration` 은 **롤백 기준이 통째로 없다.** **한 회사에서 세 가지가
  다 나온다.**
- **`무엇을 좋다고 부를 것인가` 와 `재 보고 나서`** — 둘 다 *"잘못 고른 사례가 없다"* 로 끝나는데
  **하이퍼커넥트 `objective-relaxation` 이 반례다**(DAU 를 올렸더니 PUR 이 떨어져 목표를 다시 짰다).
...
⑫ **요약이 아니라 원문을 다시 읽는다** — 223·229·243·249·250·252·255·**258** 여덟 번 다 새 결정을 찾았다. **"재료가 얇다" 고 판단해 닫으려던 258 에서도 넷이 나왔다.**
