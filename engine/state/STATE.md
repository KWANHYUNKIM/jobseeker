...
**Cerebras — 회사 프로파일까지 썼다(in_progress).** 회사 127개, 큐 2/3.
**이 엔진에 처음 들어온 하드웨어 회사다.**

도메인 셋을 세웠고 기능은 아직 0개다.

- **웨이퍼 한 장을 칩 하나로 쓴다** — `Hot Chips 2026 딥다이브`(2026-08-25)
- **가중치를 움직이지 않는 것을 성능의 정의로 삼는다** — `GPT-5.6 Sol 초당 750토큰`(2026-08-27)
- **모델이 커지는 방향에 맞춰 담을 곳을 늘린다** — 같은 글의 CS-5·CS-6 계획

### 지금까지 판 회사들과 층이 다르다

이 엔진의 회사들은 남이 만든 칩 위에서 소프트웨어로 값을 깎았다 — Materialize 는 바이트를,
CedarDB 는 사이클을, QuestDB 는 시스템 콜을. **여기는 칩 자체를 다시 그린다.** 그래서 글에
밀리미터와 페타바이트가 같이 나온다(전력 변환기가 웨이퍼에서 0.5mm, 온웨이퍼 패브릭 53.5 PB/s).

### 문제를 다시 정의한 자리가 분명하다

느린 이유가 연산 부족이 아니라 **가중치가 칩 밖 메모리에서 되풀이해 건너오는 것**이라고 못 박고,
GPU 를 더 붙이는 답을 물린다 — 칩을 더할수록 그 단계가 비싸지면서 연산은 줄어든다. 그래서 웨이퍼
한 장에 SRAM 44GB 를 코어 90만 개 옆에 두고(총대역폭 21 PB/s), 모델은 층 단위로 굵게 나눠
**가중치는 연산 옆에 두고 활성값만 옮긴다.**

이 엔진에서 병목을 다시 정의한 회사가 셋이 됐다 — Bun(I/O 가 아니라 시스템 콜) · Chroma(값을 위해
지연을 받아들인다) · Cerebras(연산이 아니라 가중치 이동).

### 다음 사이클 — 확장 (Cerebras 의 빈 도메인)

메모리 벽 글이 거절한 대안과 수치를 함께 적어 가장 두껍다. 못 본 것도 적어 뒀다 — 제조 수율과
결함 우회, 가격, 배치와 투기적 디코딩, 학습 쪽 자료.
