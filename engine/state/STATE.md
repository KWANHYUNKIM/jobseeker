...
**없다 — 문체 보수 사이클이었다.** Vercel 을 배민 문체로 다시 썼다(3.75 → 0.0). 회사 115개, 큐 2/3. 66곳 남았다.

### 유휴를 다루는 네 답이 이제 데이터 안에서 이어진다

STATE 에만 적어 두던 축을 데이터로 옮기는 중이다. 유휴 자원을 어떻게 할 것인가에 이 엔진의
회사들이 각자 다르게 답했다.

- Neon 은 0 으로 내린다. 로그가 진실이고 S3 에 있으니 컴퓨트를 꺼도 잃을 것이 없다.
- Vercel 은 하나를 남긴다. 트래픽이 적을수록 거의 모든 첫 상호작용이 콜드 스타트가 되기 때문이다.
- Cursor 는 재운다. 놀리는 비용과 재구성 시간 사이를 잡는다.
- Modal 은 아예 유휴에 값을 안 받는다. 그래서 유휴가 아니라 깨어나는 시간이 원가가 된다.

Neon 과 Vercel 양쪽에 이 견줌을 넣었으니 어느 쪽에서 읽어도 나머지 셋이 보인다.
Cursor 와 Modal 은 아직 안 넣었다 — 그 회사들의 문체 보수 사이클에서 넣는다.

Vercel 과 Cursor 가 격리에서도 갈린다. Vercel 은 격리가 이그레스 통제 없이는 결과를 못
가둔다며 자기 클라우드 안에 경계를 세웠고, Cursor 는 실행을 고객 네트워크로 옮기되 이그레스
통제와 비밀 삭제를 따라 보낸다. 경계를 세우는 쪽과 경계를 옮기는 쪽이다.

### 이번 사이클 — Vercel

사업 모델에서 유통의 뿌리가 오픈소스라는 것을 앞세웠다. Next.js 주간 다운로드가 2억 회이고
31만 9,000개 넘는 회사가 쓴다. 전략이 프런트엔드 클라우드에서 AI 클라우드로 옮겨 갔고
v0 하나가 ARR 의 21% 안팎이다.

기능 이름에 핵심을 붙였다. `캐시에 잃으면 안 되는 것을 두고 있었다 — 상태가 부채가 될 때`,
`프로세스를 가두는 것과 결과를 가두는 것은 다르다 — 격리의 한계`,
`0 이 아니라 1 로 내린다 — 적게 쓸수록 나빠지는 구조를 깬다`.

### 다음 사이클 — 문체 보수 (사다리 3순위)

무거운 순으로 atlassian · cygames · payfit · trendyol · twilio 가 남았다.
