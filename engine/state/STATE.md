...
**컬리 (KR · 커머스/신선식품) — 47번째. 회사 프로파일 완료. 도메인 2개 · 기능 0개. 큐 잔량 0/3.**

**본문 두 편을 열었고 둘 다 단단했다.**

**도메인 ① `문 앞에 두고 간 것이 맞는지 판정한다`** — 배송 완료 사진으로 오배송을 **기기 안에서**
탐지한다(2026-04-28). 문제를 사용자 쪽에서 적는다 — *"분명 '배송 완료' 알림을 받았는데 문 앞이
텅 비어있거나, 다른 곳으로 잘못 배송되어"*. **어려움은 정확도가 아니라 희소성이다**(*"오배송이 전체
배송 건수 대비 매우 낮은 비율로만 발생"*). **On-device 이유가 둘 명시돼 있다** — *"통신 환경이
불안정한 배송 현장(지하 주차장, 엘리베이터 등)"* 과 추론 서버가 필요 없다는 것.

**🔴 회사가 오탐의 값에 이름을 붙였다** — *"정상 배송임에도 오배송 알럿이 빈번하게 발생하면 매니저의
업무 흐름이 끊기고, 결국 알럿을 대수롭지 않게 여기는 **'양치기 소년' 현상**"*. **CyberAgent 가 붙인
*"알림의 형해화"* 와 같은 문제이고, 답도 비슷하다 — 다만 컬리는 **채널마다 임계를 다르게 뒀다**
(앱 알럿은 정밀도 우선, Slack 알럿은 재현율 우선 + 사람 검토). **한 판정에 두 개의 임계를 쓴다.**

**또 하나의 대가가 특이하다** — *"서버의 Python 환경과 모바일의 On-device 환경에서 동일한 이미지에
대해 동일한 임베딩 값을 얻어야 합니다"* 인데 *"이미지 전처리 과정은 사용하는 라이브러리나 플랫폼별
연산 방식에 따라 미세한 결과 차이를 보입니다"*. 그래서 **전처리까지 ONNX 에 묶었다.**

**도메인 ② `남의 물건을 대신 받아 준다`** — **1P(직매입)에서 3PL(제3자 물류)로 사업이 넓어지면서**
입고 예정 데이터가 *"외부의 여러 채널에서 다양한 규격과 형태로 수신되게 되었습니다"*. **버린 대안
하나에 이유가 명시적이다** — 배치가 인터페이스 테이블을 직접 훑는 안은 *"가장 확실하게 찾아낼 수
있고, 로직이 직관적"* 인데도 **소비자가 발행자의 저장 계층에 결합된다**는 이유로 버렸다. 재시도는
**총 145회 · 10분 간격 · 24시간 창**이고, **거래처는 보통 실물 도착 1~2일 전에 데이터를 넣는다** —
그게 처리 기한이다.

**⚠️ 사업 수치를 하나도 못 찾았다.** `www.kurlycorp.com` 은 **DNS 가 없고**, `www.kurly.com` 회사
소개에는 수치가 없다. **매출·주문 수·활성 고객·물류센터 규모도, 상장 여부도 모른다.** 3PL 매출
비중도 공개되지 않았다. **지어내지 않고 비워 뒀다** — 하이퍼커넥트에서와 같다.

**🚨 다음 사이클 — `--gaps` 가 [진행 중 확장]을 가리킬 것이다. 도메인 하나의 기능을 쓴다.**
**⚠️ 각 도메인의 두 번째 글을 아직 안 읽었다**(스무 번 확인된 규칙: 기능을 쓸 때는 두 편 이상).
엔지니어링 카테고리가 **7페이지(70편 이상)** 이고 1쪽에만 본업 글이 여섯이다:
```
/blog/2025-kafka-streams-window        Kafka Streams 윈도우 도입기 (2025-12-01)   ← 도메인 ②와 이어질 수 있다
/blog/delivery-encryption-module       딜리버리 암호화 모듈 개발기 (2025-06-02)   ← 도메인 ①과 이어질 수 있다
/blog/oms-msa-architecture-1           OMS 의 최적화된 MSA 디자인 (2025-05-09)
/blog/access-block-1  ·  /blog/access-block-2   nginx 설정 없이 우아하게 서비스 점검하기 上·下 (2025-10-10, 2부작)
```
**두 번째 글을 열어 보고 같은 도메인이면 기능을 쓰고, 다른 문제면 세 번째 도메인 재료로 남긴다**
(223 의 규칙 — 서로 다른 문제를 한 기능에 뭉개지 않는다). **`access-block` 2부작은 그 자체로 묶음이라
세 번째 도메인 후보다**(무중단 점검).

**AI 도구 글이 전체 목록의 절반**이다(RAG 2026-05-21 · 오케스트레이션 2026-04-03 · 클로드 코드 셋).
**사내 개발 도구 이야기이고 물류·커머스와 이어지는 서술은 아직 못 봤다** — 도메인으로 열지는
더 읽어 보고 정한다. **다만 RAG 글은 재료가 좋다**(버린 두 단계에 이유가 각각, *"색인 유지 자체가
부채"*, 문서의 85% 가 512 토큰 한계를 넘겨 잘렸다).
...
- **새 축 `관측에 값을 무엇으로 치르는가`** — CyberAgent `observability-pipeline`(돈) +
  ...
  `prod-profiling`. **📌 컬리의 오배송 탐지가 여기 붙는다** — *"양치기 소년 현상"* 은 CyberAgent 의
  *"알림의 형해화"* 와 같은 문제인데 **답이 다르다**(저쪽은 규칙을 재웠다 깨우고, 여기는 채널마다
  임계를 다르게 뒀다). **여섯이 된다.**
- **`되돌릴 수 있는 것을 먼저 한다`** — CyberAgent `env-split-migration` 은 **반례**, 하이퍼커넥트
  한 회사가 **세 가지를 다 준다**(`stream-join` 은 되돌리는 길이 배포 절차 자체, `raid-recovery` 는
  늘 두 벌을 유지, `arch-migration` 은 롤백 기준이 통째로 없다).
...
⑫ **요약이 아니라 원문을 다시 읽는다** — 223·229·243·249·250·252·255·258 여덟 번 다 새 결정을 찾았다.
