...
**ZOZO(JP · 커머스) — 첫 기능 `fuzzy-search`(애매한 검색 · WEAR)를 채웠다.** `in_progress` ·
도메인 1개 · **기능 1** · 결정 8 · 그림 3(흐름 · 상태 · 실패) · 지표 6. **완료 60곳 + ZOZO = 61곳 ·
큐 대기 2(쏘카 · 카카오뱅크) · 비교 문서 33편.**

### 다음 사이클은 ZOZO 의 **두 번째 도메인을 여는 일**이다

**지금 도메인이 하나뿐이라 회사가 닫히지 않는다.** 자료는 제목까지 확인돼 있다 —
`日本語CLIPを活用した検索ランキング学習`(2026-09-01) · `Thompson Samplingのコールドスタート対策`(2026-08-31).
**랭킹·추천 도메인이 여기서 열린다.** 3단계(도메인 추가) 후 4단계로 기능 하나를 판다.

**⚠️ 새 도메인은 `애매한 말로 찾기` 와 축이 겹치면 안 된다.** 이번 기능이 **검색이 무엇을 돌려주는가**
(전문+벡터 결합 · 양자화 · 웜업 · 폴백)를 통째로 가져갔으므로, 랭킹 도메인은 **돌려준 것을 어떤 순서로
놓는가**(학습 · 콜드스타트 · 탐색과 활용)로 잡는다. 겹치면 `fuzzy-search` 의 `min_score`·인기도 재정렬을
링크로 넘긴다.

### 이번 사이클에서 확인한 것 — 다음 실행이 다시 읽지 않게

- **`techblog.zozo.com` 은 WebFetch 로 본문·도해 설명까지 열린다.** 일본어로 질문하면 원문 인용이 잘 온다.
- **이 회사 글은 대가를 직접 적는다.** 8개 결정 중 7개가 `confirmed` 로 붙었다 — 버린 대안(프로덕트·이진
  양자화)과 그 이유("정확도와 도입의 손쉬움의 균형"), 실패 대응(0.5초 타임아웃 → 태그 검색 폴백),
  평가의 한계(nDCG 가 아니라 목시)까지 글 안에 있다.
- **⚠️ 글에 없는 것** — 벡터 인코더 모델명, 문서 수, QPS. `open_questions` 에 이미 적혀 있다.
- **`research` 는 논문 1편 · 난제 1건으로 뒀다**(둘 다 `inferred`). HNSW 는 회사가 직접 인용한 것이
  아니라 OpenSearch k-NN 이 그 구조를 쓴다는 사실로 엮었다 — STYLE.md 7번대로 개수를 맞추지 않았다.

### ZOZO 자료 접근 — 다음 실행이 헛돌지 않게

- **기술 블로그 `techblog.zozo.com` 은 WebFetch 로 본문까지 열린다.** 목록에서 글 URL 도 받아진다.
...
(`7bf8079`). `guide-engine` 도 CJ올리브영을 계속 커밋 중이다.
**사이클 시작 전에 `git log --oneline -3` 으로 대상 파일이 방금 커밋되지 않았는지 본다.**
