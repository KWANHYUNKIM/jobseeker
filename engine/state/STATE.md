...
**아무것도 안 판다 — 265 는 후보 조사였다. 여덟 곳을 두드려 하나를 건졌다. 큐 잔량 1/3.**
**한국 밖에서만 찾았다**(컬리로 한국 회사가 아홉이 됐다).

**✅ 큐에 올린 곳 — Snap (US · 소셜/카메라·AR).** `eng.snap.com`
**본문을 실제로 열어 확인했다**(`/codepal`, AI 코드 리뷰어, 2026-06-05). **벤더를 버린 이유가 둘이다** —
**통합 깊이**(*"Snap's engineering infrastructure is extensive and deeply interconnected… A generic tool
with only a few configuration knobs wasn't going to work"*)와 **속도**(*"We shipped a working end-to-end
demo in two weeks. A vendor procurement cycle hadn't even finished"*). **대가 다섯**(다중 패스라 *"It
costs more compute"* · 큰 저장소는 설정이 더 필요 · *"AI review doesn't yet replace human review, it
reshapes it"* · 리뷰당 **약 $0.40**). **수치가 아주 촘촘하다** — PR 의 **90%** · 4개월 **20만+ 리뷰** ·
중앙값 **10분** 대 사람 첫 리뷰 **약 5시간** · 채택률 한 분기에 **0%→90%** · 재현율 **30%→80%** ·
골든셋 **오탐 0%** · PR 증가율 **연 60%**.

**⭐ 비교 축이 이미 서 있다** — **LinkedIn `ai-code-review` 와 같은 결정을 다른 이유로 했다.**
LinkedIn 은 *"벤더의 모델을 카나리로 돌릴 수 없고 두 번째 리뷰어로 페일오버할 수도 없다"* 였고,
Snap 은 **통합 깊이와 속도**다. **그리고 재는 방식도 다르다** — LinkedIn 은 **병합된 코드와 대조한
채택률 63.9%**, Snap 은 **골든셋 오탐 0% 와 재현율 30%→80%**.

**자리** — 이 사이트에 **카메라 중심 소셜도 AR 글라스도 없다**(인스타그램은 피드, Discord 는 음성,
하이퍼커넥트는 실시간 영상 매칭). ⚠️ **다만 블로그 1쪽이 AI 개발 도구에 쏠려 있다**(Code Search ·
Casper · CodePal · Spatial Intelligence). **8페이지이니 뒤쪽에 Snapchat 본체의 시스템 글이 있는지
확인하고 도메인을 연다.**

**❌ 이번에 막힌 여섯**
- **Atlassian** — `/engineering` 이 **일반 블로그로 계속 리다이렉트된다**(두 번 따라갔다). **전용
  엔지니어링 블로그가 없어진 것으로 보인다.**
- **N26** `tech.n26.com` — **DNS 없음.**
- **Robinhood** — 뉴스룸에 **엔지니어링 섹션이 없다**(Crypto·Design·Education·Ideas·News 뿐).
- **Wolt** — 기술 블로그를 `careers.wolt.com/en/blog` 로 옮겼는데 **채용·문화 중심이고 기술 글이 하나뿐**.
- **Twitch** — 목록이 **JS 로 그려져 안 온다**.
- **eBay** `innovation.ebayinc.com/tech/` — **60초 타임아웃**.

**📌 뜻밖의 수확 — Grab 보강 후보.** 이미 `done` 인데 `engineering.grab.com` 최신이 **2026-08-21** 이고
...
(2026-07-03). **이 사이트의 Grab 페이지가 `counter-service`·`iceberg-lake`·`dispatchgym` 인데
앞의 둘에 후속 글이 나온 셈이다.** **2부작이 있어 파기 좋다**(262 의 발견).

**🚨 다음 사이클 — `--gaps` 가 [신규]를 가리킨다. `## 대기` 맨 위인 Snap 을 `in_progress` 로 만들고
회사 프로파일까지 판다. 48번째 회사다.**
**⚠️ 도메인을 정하기 전에 8페이지를 훑어 Snapchat 본체 글이 있는지 본다** — 1쪽은 AI 개발 도구에
쏠려 있다. **AI 도구만 있으면 그것으로 도메인을 열되 그 사실을 밝힌다.**
**business_model 은 1차 자료로** — Snap 은 **미국 상장사(NYSE: SNAP)** 다. IR 에서 **매출·DAU·
Snapchat+ 구독자·Specs 관련 수치**를 찾고 **못 찾으면 지어내지 말고 `open_questions` 에 남긴다**
(하이퍼커넥트·컬리에서 그렇게 했다).
...
- **`관측에 값을 무엇으로 치르는가`(새 축, 여덟)** — CyberAgent 둘(돈 · 주의력) · **컬리 셋**(임계로 ·
  ...
  Doximity `prod-profiling`. **컬리 한 회사에서 같은 생각이 세 번 다른 방식으로 나온 것이 핵심 재료다.**
...
- **📌 새 재료 — `기계가 그렇다는데`(machine-says-so)에 Snap `codepal` 이 붙는다.** 그 문서는
  LinkedIn 의 두 시스템이 **정반대 답**(거르기 vs 사후 재기)을 쓴다는 것이 축인데, **Snap 은 세 번째
  답을 준다** — **골든셋으로 오탐을 0% 로 만들고 재현율을 올린다.**
...
⑬ **JS 로 그려지는 목록은 브라우저 `get_page_text` 로도 안 나올 수 있다**(253). **두 번 실패하면 접는다.**
⑭ **`--gaps` 가 프롬프트의 지시와 다른 도메인을 가리키면 `--gaps` 를 따른다**(255).
⑮ **2부작·연작 글은 도메인을 열기에 좋다**(262).
⑯ **리다이렉트를 두 번 따라가도 목적지가 아니면 접는다**(265, Atlassian). 엔지니어링 경로가 일반 블로그로 밀리는 것은 그 블로그가 없어졌다는 신호다.
