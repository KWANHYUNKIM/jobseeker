...
**없다 — 후보 조사 사이클이었다.** 회사 **111개**, 큐 **0 → 3/3.**

### 이번 사이클 — 서른두 번째 후보 조사

⭐ **`큰 이름인데 아직 없는 회사` 각도가 다섯 조사 연속 통했다.** Coinbase → Atlassian·Neon·Vercel → Cursor → Block·Klaviyo·Perplexity → 이번 셋.

올린 세 곳(근거는 QUEUE 대기 표에):

- **✅ Anthropic**(US · SaaS) — ⭐ **포스트모템을 쓴다**(`An update on recent Claude Code quality reports`, 2026-04-23). ⭐ **그리고 `Quantifying infrastructure noise in agentic coding evals` 가 있다** — **평가 자체의 잡음을 정량화하는 글이다.** ⚠️ **㉘축(맞았는지 어떻게 아나)에 이보다 정면인 자료가 드물다** — Cursor 는 `사용자의 다음 행동`으로, Block 은 `표본 수와 가격`으로 답했는데 **여기는 자 자체를 의심한다**(재구성).
- **✅ Databricks**(US · SaaS) — ⭐ **⑥축(`복제로 버틸 것인가 로그로 버틸 것인가`)의 정면 사례에 수치까지 있다.** **컴퓨트와 스토리지를 WAL 스트림 하나로만 잇고, safekeeper 무리가 Paxos 정족수로 복제하며, 커밋은 객체 저장소가 아니라 정족수 확인으로 끝난다.** **델타만 보내 WAL 트래픽 94% 감소, 쓰기 5배.** ⚠️ **Neon 과 같은 구조라 겹침 주의**(Databricks 가 Neon 을 인수했다).
- **✅ Modal**(US · SaaS) — **`초 단위로 100만 동시 샌드박스`·`전선 속도에 가까운 서버리스`.** ⭐ **㊳축(유휴를 어떻게 다루나)의 새 답 후보다** — Neon 은 0 으로 내리고 Vercel 은 1 을 남기고 Cursor 는 재우는데 **여기는 서버리스 GPU 다.**

### ⚠️ 접은 둘 — 둘 다 이미 배운 규칙에 걸렸다

- **❌ Snowflake** — ⭐ **Databricks 와 대비되는 큰 이름이라 노렸는데** 회사 자신이 쓰는 채널이 안 잡힌다. 블로그는 **쿠키 배너만** 오고, 검색에 걸리는 것은 **Medium 의 커뮤니티 기고(2025)와 3자 정리**다. ⚠️ **`회사가 스스로 쓰는 채널이 없으면 파다가 마른다`**(Trendyol·Rivian 에서 배운 것).
- **❌ Anduril** — 블로그가 **404**. **주소를 추측하지 않는다.** ⚠️ **산업/제조·방산 축은 이제 네 번째 실패다.**

### ⭐ `본문이 안 열리면 3자 경로로 판정한다`가 두 번째로 값을 했다

Perplexity 에서 처음 썼고 이번엔 **Databricks** 에서 썼다 — `r.jina.ai` 가 개별 글 본문 대신 **트래커 픽셀**을 주는데, 검색으로 3자 정리를 받아 **safekeeper·Paxos 정족수·pageserver·94%·5배**까지 확인했다.

⚠️ **다만 판정과 파는 것은 다르다** — **Databricks 를 팔 때 본문 대신 무엇을 읽을지 미리 정해야 한다**(`docs.databricks.com` 이 열릴 수 있다). **Perplexity 에서 `개별 글은 열린다`고 넘겨짚어 다음 사이클에 정정한 일을 반복하지 않는다.**

### 다음 사이클 — 신규 (6순위)

**큐 3/3 이 찼다.** ⭐ **Anthropic 부터 판다** — **1차 자료가 직접 열리고**(블로그가 403 이 아니다), **㉘축·㊵축의 재료가 가장 두껍다.** ⚠️ **Databricks 를 고를 때는 본문 접근 경로를 먼저 확인한다.**
