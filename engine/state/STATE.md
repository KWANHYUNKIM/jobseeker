...
**Slack — 진행 중. 20번째 회사다.** 도메인 4개 중 **3개 완료** — 비동기 작업(`kafka-job-queue`),
가용성·장애 격리(`az-siloing`), 실시간 메시지 전달(`unified-grid`). 남은 것: **저장·샤딩** 하나.

> **실시간 전달은 Unified Grid 로 팠다.** 회사의 실시간 전달 글이 구성요소 나열이라
> 대가·실패가 안 보였는데, Unified Grid 글(2024-07)이 그 자리를 채웠다 —
> **2013년 '데이터는 워크스페이스 하나의 것' 전제를 11년 만에 걷어낸 이야기.**
> 남은 저장·샤딩은 **Vitess** 글이 자리다. Unified Grid 가 이미 Vitess 재샤딩을
> 전제 조건으로 깔아 뒀으니 이어진다.
...
마지막 사이클: 132 (2026-08-21) — Unified Grid.

> **비교 축이 하나 더 굳었다 — '셀로 나눈 이유'.** Roblox(127)와 Slack(131)이 둘 다
> 장애를 겪고 셀·사일로로 갔는데 **장애의 성격이 정반대다** — Roblox 는 작은 실패가
> 전체로 **너무 빨리 번진** 것(73시간), Slack 은 **무엇이 고장인지 판단이 갈린** 것(회색 장애).
> 그래서 답도 다르다: Roblox 는 번지는 범위를 자르고, Slack 은 **끊는 동작을 확실하게** 만들었다.

> **새 축 후보 — '맞았던 전제가 언제 문제가 되나'.** Slack 2013년 워크스페이스 샤딩,
> Figma 논리→물리 샤딩, Notion 블록 테이블 단일 샤드. 셋 다 **그때는 맞았고** 규모나
> 쓰임새가 옮겨 가면서 어긋났다. 공통점은 **임시방편이 한동안 통했다**는 것 — 그래서 늦어진다.
