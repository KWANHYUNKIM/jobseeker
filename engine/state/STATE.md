...
**freee(JP · SaaS) `in_progress` — 도메인 2개 · 기능 4개.** 342 에서 DB 도메인
`멈출 수 없는 데이터베이스를 돌아가는 채로 바꾼다` 의 **둘째 기능**
**`send-only-the-writes`(읽기를 옆으로 보내고 쓰기만 원래 자리로 보낸다)** 를 썼다.
**결정 6개 · 그림 3장 · 생각 4개 · 난제 1건 · 연결 2건.**

**⑭ `--gaps` 는 [신규 = SmartHR]** 을 가리켰다(큐 1/3). **회사 완주 규칙이 위라 freee 를
계속 팠다**(CLAUDE.md).

**재료와 판정** — `freee人事労務の給与計算ロジックにLocal Write Forwardingを導入した話`
(2024-09-19, `/entry/introduce-local-write-forwarding`). **3/3** — 버린 대안(Aurora
Limitless 를 기다리는 안 · 병렬도를 낮추는 안) · 대가(못 쓰는 SQL · 접속 상한 · 한 건 3~4초) ·
수치(reader CPU 5→35 · writer 20→40 · 6,000건 · 5→50→100 퍼센트).

**⚠️ 223 규칙으로 갈랐다** — **같은 도메인의 둘째 기능**이다. 돌아가는 채로 **쿼리의
행선지를 바꾸는** 이야기고, 단계 릴리스와 병행 비교까지 이 도메인의 결 그대로다.

**342 가 새로 낸 것**
- **⭐⭐ 341 이 남긴 구멍을 정확히 메운다** — 2020년 이관은 **읽기를 갈라내려고** 한 일인데,
  급여계산처럼 **트랜잭션이 읽기와 쓰기를 묶는 잡에는 그 갈라냄이 닿지 않았다.**
  **레플리카를 늘려도 안 쓰는 곳이 있었다.** ⚠️ **그리고 이것도 쓰기가 갈라진 것은 아니다** —
  갈라진 것은 트랜잭션 안의 읽기다.
- **기법** — 읽기 쪽에 트랜잭션을 걸고 쓰기만 전달한다:
  *"Local Write Forwardingとは reader に向けて貼った transaction の中で、write の query だけを writer instance に転送してくれる仕組み"*. 정합성은 SESSION.
- **⭐ 검증 기법이 새롭다** — 본 프로덕션 **5퍼센트**에서 **켠 채 실행해 결과만 담고 롤백**,
  **끈 채 다시 실행해 커밋**, 둘을 비교한다.
- **⭐⭐ 340 의 난제와 정면으로 짝이 된다** — 인증 기반 쪽은 **차분의 원인을 안 알려 줘**
  마지막 1년을 손으로 뒤졌는데, 여기서는 **차분이 두 실행 사이의 사용자 조작**(출퇴근 기록
  변경 · 인사 정보 갱신)**때문임을 규명했다.** ⚠️ 다만 대상이 다르다 — 한쪽은 옛 데이터,
  한쪽은 계산 로직이다. **`connections` 에 그렇게 적었다.**
- **우려를 목록으로 적고 각각 처리한다** — 못 쓰는 SQL(급여계산은 안 씀) · 기능 고유 버그
  (검증 기간) · 레이턴시(병렬도로 상쇄) · **Aurora Limitless 로 가면 헛수고가 될 위험**
  (장래를 검토할 필요 없다고 판단). **⚠️ 마지막 판단의 근거는 글에 없다.**
- **🔴 걸린 곳** — 전달되는 쓰기의 접속에 별도 상한이 있다
  (`aurora_fwd_writer_max_connections_pct`, 초기값 10퍼센트). **스테이징에서 상한에 닿았다.**
- **다음 계획** — *"writer instance に生まれた余裕を給与計算 Job 用 replica を増やすことで利用し、ユーザーの給与計算待ち時間を短縮する予定"*.

**⚠️ 인용은 두 문장만 원문으로 썼다** — WebFetch 요약과 검색 스니펫에서 같은 문장이 서로
다르게 나온 것이 있어(`すべてのクエリを…`) **확실히 인용부호 안에 있던 것만** `*"..."*` 로
쓰고 나머지는 우리말 굵게로 옮겼다(341 의 교훈).

---

**다음 사이클 — freee 를 닫거나 한 축을 더 연다.**
- **1순위 — DB 도메인의 셋째 기능.** 제목만 확인한 것들:
  `pt-online-schema-changeの導入時に検討したこと` · `安全なデータベース削除オペレーションの自動化事例`
  (⭐ 비교 문서 `되돌릴 수 없는 것이 어디에 있는가` 와 이어질 수 있다, 추정) ·
  `複数の検証環境でのDB相乗り化` · `Aurora 3.04.2 での DDL の予期しない挙動と Rails での対策` ·
  `freee人事労務開発 〜 従業員一覧画面のパフォーマンス改善・リアーキテクチャ`.
  **⑰ 주소를 추측하지 않는다. ⑱ WebSearch + allowed_domains.**
  **⚠️ 다만 이 도메인은 이미 기능 둘·재료 셋이다 — 넷째 재료가 같은 결이면 안 쓴다.**
- **⚠️ 2순위 — QA 축. 분류가 95편인데 아직 한 편도 안 읽었다.**
  `権限管理基盤マイクロサービスで行っているQA活動について` ·
  `ソフトウェアアーキテクチャに基づいた自動テスト戦略と実装ガイドライン` · `テストアーキテクチャの実践`.
  **⭐ 인증 기반 이관의 QA 가 14제품 633 테스트 케이스로 사내 최대였다는 사실과 이어질 수 있다**(추정).
- **3순위** — `Langfuse のバージョンアップに伴い ClickHouse を社内初導入した話` ·
  `【連載 第4回】freeeカード Unlimited でのClean Architecture実践`.
- **4순위 — 닫는다.** 그때 `business_model` 에 적을 관찰(이 사이트의 정리라고 밝힌다):
  **⭐ 후회를 적고 되먹임 고리까지 짚는다** —
  *"完了までに時間がかかったことを踏まえるとこれを良しとするのはよくありません"* ·
  *"リリース回数が少ないために振り返りの頻度も多くなく、改善施策が出しづらい状況になった"*.
  **⭐⭐ 같은 프로젝트를 두 글이 다른 각도로 다룬다** — **둘을 겹쳐야 4년의 모양이 보인다.**
  **⭐⭐ 되돌릴 길이 서지 않으면 안 넘긴다** — 2020년엔 마스터 승격을 직접 만들었고,
  2023년엔 Blue/Green 을 *"切り戻し手法を確立できないため、導入を見送りました"*.
  **⭐⭐ 그리고 이 회사는 단계로 연다** — 인증은 제품마다, 급여계산은 5→50→100 퍼센트.
  **비율이든 제품이든 '한 번에 다'는 안 한다**(342 에서 굳어졌다).
  **⭐ 같은 아픔을 두 층에서 풀었다** — 바닥 갈아 끼우기 석 달, 위에 얽힌 것 떼어내기 1,454일.
  **⚠️ 최근 글의 결은 AI 쪽으로 크게 기울어 있고 이런 굵은 회고는 2022년 언저리에 몰려 있다**(추정).

**⚠️ 큐 1/3 — SmartHR 하나뿐이고 Personio(EU) 는 아직 안 봤다.** freee 를 닫으면
**후보 조사 사이클**을 한 번 끼운다. **후보 조사 누적 열아홉 곳 중 일곱.**

**⭐⭐ freee 와 SmartHR 의 정반대 순서 대비**(`freee.json` 의 `open_questions`) — freee 가
후회로 적은 *"旧機能のリファクタはマイクロサービス化の前に着手すべし"* 가 SmartHR 이 실제로
한 일이다. **SmartHR 을 판 뒤 비교 문서 재료.**

**분포** — 57곳(일본 여섯 · 인도 셋) · 비교 문서 32편 · **운영 사실 29개.**

**⚠️ 나르지 않는다** — 낡은 메모(314·317·325) · 두 방향 규칙(298·305·318·321) ·
...
(341) · **확인 안 된 인용을 지어내지 않는다**(341) · **⭐ 새로: 읽은 글을 '안 읽은 목록' 에
남겨 두지 않는다 — 342 에서 치환이 빗나가 한 번 남았다.**
