...
**Airbnb** (US · 커머스) — 도메인 3개 · **기능 2개.** 남은 도메인은 **`신원 그래프를 벤더에서 자기 것으로 옮긴다`** 하나다(⏳ 미독).

### 이번 사이클 — `도메인마다 고르게 하되 한 도메인 안에서는 섞지 않는다`

건진 것 넷:

- ⚠️ **두 극단이 다 틀렸다** — **전부 분리하면 도메인마다 같은 로직이 중복되고**, **전부 통합하면 다루기 어려워지고 제품 고유의 속성에 잘 맞지 않는다.** 회사가 둘을 명시적으로 거절했다.
- ⚠️ **그래서 선택을 도메인에 맡기되 첫 번째 원칙으로 하이브리드를 금지했다** — **각 도메인은 완전히 분리되거나 완전히 통합돼야 한다.** **선택의 자유를 주면 반드시 `조금씩 섞은` 구조가 나오고 그것이 가장 설명하기 어렵다**(재구성). **자유를 주되 그 선택이 보이게 만든다** — 식별자 이름(`id_experience` 대 `id_product_listing` + 제품 타입 컬럼)과 네임스페이스로.
- ⚠️ **판단 기준이 데이터의 모양이 아니라 사람의 행동이다** — **개념이 제품마다 다르면 분리**(서비스의 `제공 항목`은 다대일, 가용성이 캘린더가 아니라 영업시간, 위치가 점이 아니라 반경), **흐름이 제품을 넘나들면 통합**(메시지 스레드를 나누면 대화가 조각난다).
- ⚠️ **같은 결정에 다른 종류의 이유가 섞여 있다** — 분리한 넷 중 **셋은 개념 때문이고 게스트 하나는 데이터 양·성능 때문**이다(재구성).

### ⚠️ 끝나지 않았다고 밝히는 대목

**`처음 출시하고 몇 달이 지난 지금도 우리는 번역 계층을 계속 다듬고 레거시 자산을 조심스럽게 이전하고 있다.`** ⚠️ **새 구조를 세우는 일보다 옛것을 걷는 일이 오래 남는다**(재구성). **그리고 이 글에도 수치가 하나도 없다** — 규모·데이터 양·성능 개선 없이 **구조와 규칙만 말한다.**

### 다음 사이클 — 마지막 도메인, 그리고 완주 판정

`--gaps` 가 **`신원 그래프를 벤더에서 자기 것으로 옮긴다`**(2026-05)를 부를 것이다. ⏳ **본문 미독** — **URL 은 확보해 뒀다.** ⚠️ **3자 요약에 `같은 질의가 자체 인프라와 3자 벤더에서 성능이 크게 달라 JanusGraph 쪽과 클라이언트 쪽 양쪽을 최적화해야 했다` 는 대목이 있어 결정이 나올 것 같다.** 자료가 서면 **같은 사이클에서 완주 판정까지** 한다.
...
**⑨ `한 번에 갈아엎을 것인가 목 졸라 죽일 것인가`** — Twilio(두 방향) / Etsy / Plaid / Paystack / **Airbnb**(번역 계층을 두고 몇 달째).
...
**⑮ `언제 쪼개고 언제 합치나`** — Twilio / DoorDash / Deliveroo / ScyllaDB / ⏳ Pinterest / ⭐ **Airbnb**(도메인마다 고르게 하되 섞지 않는다 — **이 축에 규칙을 준 유일한 사례다**).
