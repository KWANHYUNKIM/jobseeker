...
**Meituan(美团, 52번째) `in_progress` — 도메인 2개 · 기능 2개.** 301 에서 **새 도메인 ②
`무엇을 잘한다고 부를지 정한다` 를 열고**(`tech` 둘 채움, `index.json` 최상위 `domains` 에도
넣음) **그 기능 `binarize-the-rubric`(애매한 기준을 예와 아니오로 쪼갠다)까지 썼다. 결정 8개.**

**읽은 편 둘**(둘 다 살아 있는 주소)
- `/2026/08/07/Agent-Evaluation.html` — `Agent评测漫谈`. **이번 세션에서 가장 두꺼운 글이다.**
- `/2026/07/24/LongCat-LoHoSearch.html` — 자체 공개 벤치마크.

**⭐ 무엇이 나왔나** — 재는 대상이 모델 하나에서 *"模型 + 系统 + 工具 + 流程"* 으로 옮겨 갔다.
- **🔴 안 되는 방식을 다섯 적는다** — 답만 보기(*"只看最终答案，这两者会被误判为同一水平"*) ·
  처음부터 복잡하게 설계하기(*"绝大部分新上手Agent评测团队都有一个误区"*) · 여럿의 민주적
  합의(*"各自为政"*) · 모델 지표를 사업 지표에 직접 잇기 · 사람만으로 규모 감당하기
- **⭐ 가장 실용적인 결정이 루브릭 이진화** — 애매한 기준을 **예/아니오/모름**으로 쪼개고
  **일치율 85~90%** 를 넘을 때까지 고친다. **Beam 62%→92%**, 数字站长 **99%**.
  **이것이 기계에게 채점을 맡길 수 있는 조건이 된다.**
- **한 명의 중재자** — *"1个'独裁者'好过10个'民主者'"*. **대가를 회사가 스스로 적는다** —
  사용자층이 바뀌면 판단이 어긋나는데 **감지할 방법이 없다고 미해결에 넣는다.**
- **한계 다섯 · 미해결 다섯.** 장기 평가 인프라에 필요한 능력 **일곱**이 아직 개발 중이다.
- **LoHoSearch** — BrowseComp 이 포화되자 **지식 그래프(개체 762만·간선 2억 6,500만)에게
  문제를 내게 했다.** **GPT-5.5 가 34.74%**(같은 모델이 BrowseComp 에서는 **80% 이상**).
  도구 호출이 35회→61회. 그래프 구조(8.01%)가 트리(11.89%)보다 어렵다.

**⭐⭐ 비교 문서 재료가 여기서 크게 굵어졌다**(`open_questions` 에 남겼다). **Snap 의 '자를
어떻게 세우는가' 네 방식에 Meituan 이 둘을 더한다** — **자가 무뎌지면 새로 만든다** ·
**애매한 기준을 이진화해 기계에게 맡긴다**. **⚠️ 그리고 같은 회사 안에서 솔직함이 갈린다** —
방법론 편은 한계를 다섯 적는데 **자를 만든 LoHoSearch 편은 자기 기준의 한계를 한 줄도 안 적는다.**

**다음 사이클 — 도메인 ② 를 한 번 더 파거나 새 도메인을 연다. 살아 있는 주소만 쓴다.**
- **도메인 ② 의 남은 글**(전부 홈 href 로 확인) — `/2026/06/29/LongCat-VitaBench-2.0.html` ·
  `/2026/06/12/LongCat-WBench.html` · `/2026/07/24/LongCat-MineExplorer.html`.
  **⚠️ LoHoSearch 처럼 벤치마크 소개 글은 버린 대안이 없을 수 있다** — 열어 보고 판단한다.
- **⭐ 새 도메인 후보 1순위** — `/2026/05/07/Agent-AI-Coding.html`
  (`用Agent评测思路管理AI Coding：31万行代码AI重构的实践`). **31만 줄 AI 리팩터링**이고
  **Wix 의 `compile-then-publish`(코드 샘플 2,000개)와 같은 축이다.** 제목상 **평가 방식으로
  AI 코딩을 관리한다**는 것이라 도메인 ② 와 이어질 수도 있다 — **223 규칙대로 실물로 정한다.**
- **새 도메인 후보 2순위 — 자체 대규모 모델**: `/2026/06/30/LongCat2.0.html` ·
...
**그 밖에**
- **🔴 `tech.meituan.com` 은 옛 글을 내린다**(㉑). 2022~2025-05 글과 `/tags/` 목록이 **전부
  404** 다. **검색이 주는 옛 주소를 믿지 말고 홈(`/`)의 href 만 쓴다.**
...
- **⚠️ 분포** — 52곳. **한국 편중 아홉. 빈 자리는 라틴아메리카 하나뿐이고 재시도 금지.**
  **중국은 ByteDance 와 Meituan 둘.**
- **⭐⭐⭐ 비교 문서 축이 이제 둘 다 굵다.**
  ① `에이전트에게 무엇을 못 하게 하는가` — 재료 일곱, 세 갈래(**병합 제한** monday.com
  Morphex·Snap CodePal·Snap Casper / **접근·행동 제한** Wix AirBot·Wix 자가치유 두뇌 /
  **반대편** Wix Agent Ready Repos·Wix 컴파일러 문지기·Adevinta).
  ② **`자를 어떻게 세우는가` — Snap 넷 + Meituan 둘 = 여섯 방식.** **이쪽이 새로 굵어졌다.**
