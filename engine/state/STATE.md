...
**Ghostty — 기능 하나를 채웠다(`embrace-the-platform-type-system`).** 빈 도메인 둘.
회사 159개, **큐 0/3**.

### 버그가 하나가 아니라 부류였다

리눅스 쪽 초기 구현은 GTK 의 타입 시스템(GObject)을 **피하려** 했다. 그 회피가 만든 것이
**"버그의 한 부류"** — Zig 메모리나 GTK 메모리 가운데 **한쪽만 해제되는** 것이다. 하나씩
고쳐 살 수도 있지만 원인이 회피 자체라면 끝나지 않는다. 그래서 2025년 8월에 통째로 다시 썼다.

### 피하는 대가는 기능이 막히는 것이었다

시그널·프로퍼티·액션을 **아예 못 썼다** — GUI 상호작용과 기능 개발에 결정적인 것들이다.
받아들이자 Blueprint 같은 요즘 도구도 함께 열렸다.

### 설정 하나 다시 읽는 일이 그 증상이었다

옛 구조에서는 옛 설정을 해제하려고 **인터페이스 전체가 반영됐는지 확인**해야 했다 — 복잡하고
CPU 를 먹고 실수하기 쉬웠다. 이제 `Config` 가 `GhosttyConfig` GObject 가 되어 프로퍼티 알림이
변화를 저절로 퍼뜨린다.

### 대가를 안 적는 글도 그대로 적는다

이 글은 **명시적인 대가를 적지 않는다** — 줄 수·인원·기간도, 참조 계수를 받아들일 때의 순환
참조 처리도 없다. `failure` 도해와 `open_questions` 에 **없다는 사실 자체**를 남겼다.

### 다음 사이클 — 확장 (남은 두 도메인 중 하나)

`몇 주씩 켜 두는 프로그램의 메모리를 지킨다` 가 자료가 두껍다(열흘 37GB · `mmap`/`munmap`
불일치 · 가상 메모리 태그 · 재현 케이스 · 물린 대안 · 교훈 셋).
`터미널 코어를 떼어 남에게 준다` 는 libghostty 글이 재료다.
