...
**없다 — Elastic 을 완주시켰다(도메인 2 · 기능 2 · `status: done`).** 회사 177개, **큐 0/3.**

### 하나의 예외가 전체 비용을 정하지 않게 한다

압축 글 안에서 **같은 생각이 두 번** 쓰인다. 비트 패킹은 블록의 모든 값에 같은 비트 수를
써서 **"큰 점프 하나가 나머지 전부의 비용을 정한다"** — 그래서 **시계열 경계에서 블록을
나눈다.** 그리고 ALP 에 안 맞는 값도 **블록의 나머지를 망치지 않게** 따로 떨어뜨린다.

### 정렬 순서가 압축을 정한다

시계열 인덱스가 **`_tsid` 먼저, 타임스탬프 나중**으로 정렬된다는 것을 알고 나면 고정 크기
블록이 왜 나쁜지가 바로 보인다 — **데이터의 배치를 아는 것이 압축의 절반**이다.

### 줄인 것과 못 줄인 것을 같이 낸다

전체 doc values **−33.6%** 를 내면서 **차원 필드가 저장의 44% 인데 최적화를 안 받았다**는
것을 함께 적는다. 그 사실을 안 밝혔다면 33.6% 가 더 커 보였을 것이다.

### 다이얼을 줄 것인가 — 두 회사가 반대로 갔다

**타이거데이터**: "누군가는 그 창의 올바른 너비를 골라야 한다"(사용자에게 준다).
**Elastic**: "그 판단을 할 준비가 가장 안 된 사람들에게 떠넘기는 일"(주지 않는다).
어느 쪽이 옳은지는 **사용자가 누구냐**에 달렸을 것이다 — 시계열 전문가가 쓰는 도구와 관측
데이터를 그냥 넣는 팀이 쓰는 도구는 다르다.

### 합산이 정보를 지운다 — 두 도메인이 같은 말을 한다

샤드 쪽은 **가중합을 버리고 판정자를 나눴고**, 압축 쪽은 **획일적 인코딩을 버리고 필드마다
골랐다.** 그러면서 **둘 다 그 판단을 사용자에게 넘기지 않는다.**

### 다음 사이클 — 후보 조사 (큐가 0/3 이다)

**CAD 축 세 번, 의료 축 여덟 번, 보험 한 번, 위성 한 번 실패.** 못 읽은 곳: Hugging Face ·
Lunit · Blender(403) · Sourcegraph(403) · Lemonade(403) · OpenAI(403) · GitLab(403) ·
HashiCorp(500) · Algolia(목록이 안 뜬다) · Planet Labs(링크 없음) · dbt Labs ·
incident.io(404 네 번) · M3 · Guardian · Flexport · Cal.com. **WebSearch 예산은 끝났다.**
