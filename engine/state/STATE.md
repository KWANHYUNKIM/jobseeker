...
**PostHog — 기능 둘.** 남은 빈 도메인은 덕DB 하나. 회사 140개, 큐 1/3.

### 튜닝 대신 자원을 갈랐다

초기의 단일 클러스터에서 무거운 배치가 대화형 질의와 다투며 성능이 나빠졌다. 질의를 아무리
다듬어도 그 배치가 도는 동안은 소용이 없어서, **워크로드마다 전용 클러스터**를 뒀다 — 대화형·
적재·리플레이·로그·배치 내보내기·API·내부 도구.

### 격벽을 두 겹으로 친다

클러스터 경계만 믿지 않는다. 같은 클러스터 안에서도 **제품마다 클릭하우스 사용자를 따로 두고
설정 프로파일로 자원 제약**을 건다. 미국과 유럽은 아예 독립된 거울 배포다.

### 데이터베이스의 성질이 운영 주기를 정한다

갱신과 삭제가 비싸니 변화를 새 행으로 쌓고, **GDPR 삭제는 주마다 한 번으로 몰아서** 한다 —
즉시성을 내주고 다시 쓰는 값을 줄인 자리다.

### 조용히 멈춰 있는 것들

주키퍼에 남은 고아 레코드 탓에 **뮤테이션이 몇 달씩 멈춰 있었다**(고칠 파트가 이미 없는데 아직
있다고 여겼다). 적재를 멈추려고 뗀 뷰가 재시작 때 다시 붙어 왜 안 멈췄는지 헷갈린 일도 있다 —
`PERMANENTLY` 를 안 붙였기 때문이다. 겪은 것을 핸드북에 그대로 적는다.

### 다음 사이클 — 확장 (PostHog 의 마지막 빈 도메인)

`한 데이터베이스로 안 되는 것을 옆에 둔다`. 클러스터로 갈라도 안 되는 것을 다른 엔진(덕DB)으로
빼는 마지막 단계다.
