...
**Zed — 기능 둘을 채웠다.** `os-sandbox` 와 `zeta-distillation`(선생 모델을 베껴 작은 모델을
가르친다). 회사 117개, 큐 0/3. 빈 도메인 하나가 남았다 — 글자를 무엇에 담을 것인가.

### 데이터를 못 쓰는 제약이 구조를 정했다

편집 예측의 가장 좋은 학습 데이터는 사용자가 실제로 친 코드인데 그것을 쓰면 남의 코드로 모델을
만드는 회사가 된다. 그래서 **시작 상태만 가져가고 정답은 큰 모델에게 시킨다** — 커서 위치와 최근
편집 이력을 선생(Claude Sonnet 4.6)에게 던져 받은 완성본이 학습의 정답 자리에 앉는다. 증류를
성능이 아니라 데이터 권리 때문에 쓴 사례다.

대가가 곧바로 따라온다. 학생의 천장이 선생이 되고, 선생의 나쁜 습관도 물려받는다 — 반전 문제가
그것이다. 사용자가 몇 글자 치면 모델이 그걸 오타로 보고 지우려 들었는데, 원인이 학생이 아니라
선생이 지저분한 코드를 정리하려 든 데 있었다. 고친 곳도 학생이 아니라 선생의 프롬프트였다.

### 재는 자를 스스로 만들면 잘함의 정의가 바뀐다

수락률은 사용자가 눌러 줘야 나오는 숫자라 실험 전에는 아무것도 모른다. 그래서 선생과의 일치도를
재는 자체 지표(deltaChrF)를 만들었다 — 정답이 하나가 아니어서 예제마다 선생 완성본 셋과 견준다.
그 순간 **잘한다는 것이 선생을 닮는 것으로 정의된다.** 이 엔진에서 재는 것을 의심한 회사들
(앤트로픽의 자 자체를 의심한다, Etsy 의 실험 체계) 옆에 놓을 자리다.

### 토큰을 3분의 1로 줄이고 수락률은 0.51% 올랐다

출력 형식을 멀티 리전으로 바꿔 바꿀 곳 주변만 내놓게 했다. 평균 출력 토큰 270→90, p50
189ms→136ms, 같은 트래픽에 서버 30% 감소, 거절률 4.10% 감소. 품질을 산 것이 아니라 원가와
지연을 샀다 — 무료 등급이 수락된 예측만 세는 과금 구조에서는 그쪽이 더 급했을 수 있다.

세대 교체가 연도와 함께 확인돼 `history` 를 두 칸 채웠다(Zeta2 2026-03 → Zeta2.1 2026-05).
앞 세대(Zeta1)의 시작 연도는 자료에 없어 다음 칸의 설명으로 옮겼다.

### 다음 사이클 — 확장 (마지막 빈 도메인)

`글자를 무엇에 담을 것인가`(Rope & SumTree)를 채우면 Zed 가 done 이 되고, 그다음이 후보 조사다.
