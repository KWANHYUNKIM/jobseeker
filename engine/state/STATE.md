...
**없다 — 37signals 를 완주시켰다(도메인 3 · 기능 3 · `status: done`).** 회사 151개, 큐 2/3.

### 이번 사이클이 채운 것 — `build-instead-of-buy`

만들었다고 자랑만 하지 않는 것이 이 회사의 특징이다. **읽기가 40% 느려졌다고 적고**, 그 대신
얻은 것도 수치로 적는다 — 캐시 6배 · 저장 값 80% 저렴 · 며칠이 아니라 몇 달치 ·
베이스캠프 요청 지연 **p95 375ms → 225ms.**

### 부분이 느려지는데 전체가 빨라진다

읽기 지연만 보면 레디스가 이긴다. 근거를 이렇게 댄다 — **메모리 접근은 캐시 한 번에 드는
시간의 일부일 뿐**이고 네트워크·직렬화·압축이 붙으며 데이터베이스도 자체 메모리 캐시를 갖는다.
그러니 캐시가 커져 적중이 늘면 전체가 이긴다.

### 살 것인가 만들 것인가 — 막힌 자리를 이름 대어 적는다

핑돔을 떠난 이유 셋: **체크를 맞춤할 수 없다 · 인증 브라우저 흐름은 돈을 더 내야 한다 ·
어디서 도는지 통제할 수 없다.** 그래서 Upright 를 만들고 **값도 적는다**(다섯 사이트 월 약
110달러, 최소 구성은 20달러 미만).

### 유연함 대신 단순함을 골랐다고 밝힌다

ONCE 는 앱에 제약을 건다 — `/up` 응답 · `/storage` 저장 · SQLite. 설치가 문서 세 쪽짜리면
기술자가 아닌 사람은 못 쓴다는 것이 이유다. **자기가 무엇을 버렸는지 적는 회사**가 이 엔진의
데이터에 또 하나 늘었다(프로토맵스·라이브에 이어).

### 다음 사이클 — 신규 (대기 맨 위의 Linear)

`Rebuilding Linear's delta sync read path`(2026-08-18)가 **이미 판 turbopuffer 를 쓰는 쪽**의
기록이다. 20TB 넘는 동기화 액션의 캐치업 지연을 다룬다. ⚠️ 나머지는 체인지로그·고객 사례다.
