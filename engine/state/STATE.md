...
**없다 — 후보 조사 사이클이었다.** 큐를 **0/3 → 2/3** 으로 채웠다. 다음 사이클은 `--gaps` 가 **신규(6순위)**를 부른다 — 대기 맨 위는 **1Password** 다.

### 이번 사이클 — 후보 조사 (열여덟 번째)

**남은 실마리가 다 소진돼 새 이름으로 갔다.**

- **✅ 1Password** — **보안 축 첫 자리.** ⚠️ **대가가 이 엔진에서 본 것 중 가장 극단적이다** — 복호화 키가 서버에 안 가서 회사가 **정책이 아니라 기술적으로**(`technically incapable`) 금고를 못 읽고, **그래서 못 하는 기능을 목록으로 적는다**(비밀번호 복구 불가 · 서버 측 검색 불가 · 실제 자격증명으로 유출 확인 불가). 그 제약을 우회하려 **기밀 컴퓨팅 엔클레이브**를 만들었다 — **구조로 막아 놓고 그 구조를 다시 구조로 뚫는 자리다.**
- **✅ DoorDash** — ⚠️ **AI 에이전트 축의 다섯 번째 회사**(한 달 13만 작업 · 주당 코드 리뷰 2.5만 건). 모놀리스 회고에 **기술 선택 비교**가 있고 `Reducing the Migration's Pain Points` 처럼 **아픈 곳을 제목에 건 글**도 있다.
- **❌ Etsy** — `etsy.com/codeascraft` **403**.

### ⚠️ 구조적 관찰 — 비교 문서는 이 사다리에서 영영 안 온다

**비교 재료가 갖춰졌는데도 쓸 차례가 오지 않는다.** 사다리에서 비교 문서는 **8순위**이고, 그 위에 **3순위(큐 0이면 후보 조사)**와 **6순위(큐에 회사가 있으면 신규)**가 있다. 그러면 이렇게 돈다:

> 큐가 비면 → 후보 조사로 채운다(3순위) → 큐에 회사가 있으니 신규로 판다(6순위) → 다 파면 큐가 비고 → 다시 후보 조사(3순위) → …

**8순위는 `위가 전부 비었을 때` 인데, 3순위가 큐를 다시 채우므로 그 조건이 성립하지 않는다.** ⚠️ **이 엔진은 바깥에서 가져오는 일과 파는 일 사이를 영원히 왕복하도록 되어 있고, 가진 것을 엮는 일은 구조적으로 못 한다.**

**이번 사이클도 그 고리를 한 바퀴 더 돌렸다** — 재료가 다 있는 것을 알면서 큐를 채웠다. 사다리가 그렇게 정해 두었기 때문이다.

⚠️ **이것은 데이터 문제가 아니라 `engine/PROMPT.md` 의 문제이고, 사이클이 고칠 수 있는 것이 아니다**(코드·문서 변경은 사이클에 섞지 않는다). **사람이 사다리를 손봐야 열린다** — 예를 들어 `같은 도메인에 회사가 N곳 이상 차면 비교 문서를 3순위 위로 올린다` 같은 규칙이 필요하다.

### 비교 문서 초안 (쓸 차례가 오면 이대로)

| 회사 | 어디까지 맡기나 | **그 선을 무엇이 지키나** |
|---|---|---|
| **Sentry** | AI 가 PR 까지 열고 **병합은 사람이** | **조직의 신뢰** — `Your org might not be ready to trust the machines that far, and that's ok.` |
| **ClickHouse** | 분석의 **70%**, 남긴 **30%** | ⚠️ **세 겹 중 한 겹만 기계가 강제**(접근 범위) |
| **Duolingo** | 테스트 PR **하루 20개** · **250개 전부 사람 리뷰** | **사람 리뷰** — ⚠️ **그래서 병목이 됐다** |
| **Ramp** | **병합 PR 의 약 30%** | ⚠️ **사람의 신원** — 앱 토큰을 안 써서 기존 리뷰가 그대로 작동 |
| **DoorDash** | 한 달 **13만 작업** · 주당 **코드 리뷰 2.5만 건** | ⏳ 아직 안 팠다 |

**해석(추정으로 적을 것)**: 다섯 곳 다 사람을 마지막 문으로 두는데 **그 문을 지탱하는 것이 다르다.** 그리고 **시간의 힘을 받는 것은 신원뿐으로 보인다** — 문화는 흔들리고, 규율은 잊히고, 사람의 시간은 병목이 되는데 **신원은 구조라 닳지 않는다.**
...
| `careersatdoordash.com/blog` | ✅ 자체 호스팅. **검색으로 글 주소가 잘 나온다** |
