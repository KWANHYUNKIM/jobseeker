...
**AuthZed — 프로파일만 썼다.** 회사 142개, 큐 2/3. 도메인 둘이 비어 있고 둘 다 자료를 확보했다.

### 모든 요청 앞에 서는 층

권한은 느리면 제품 전체가 느려지고, 틀리면 남의 것이 보이며, 멈추면 아무것도 안 된다. 그래서
이 회사의 기술 글이 **마이크로초와 캐시 이야기**로 채워진다.

핵심 엔진(SpiceDB)은 아파치 2.0 으로 공짜이고 파는 것은 대신 굴려 주는 일이다 — 관리형이
시간당 2달러부터, 자체 호스팅은 지역당·vCPU 당 연간 라이선스다.

### 한곳에 모으는 값을 스스로 적는다

서비스마다 권한을 구현하면 구현이 둘이 되어 버그 기회가 두 배가 되고 어긋난다. 그래서 한곳에
모으라고 권하면서 **대가도 같이 적는다** — 이중 쓰기 문제를 풀어야 하고 권한이 단일 장애
지점이 된다. 물린 대안 셋(서비스별 구현·DB 조인·게이트웨이 JWT 범위)도 이유와 함께 든다.

### 30단계 중첩과 팬아웃을 시나리오로 만든다

관계로 표현된 권한은 깊게 중첩되고 넓게 퍼진다. 질의 계획기가 조언자(정적·통계)를 두고 화살표
뒤집기·가지 재정렬·회전·가지치기로 훑는 순서를 고른다. 수치를 두 갈래로 낸다 — 알고리즘만
11배(772→70µs), 포스트그레스 저장소로는 47%·62%.

### 지금이 최선이 아니라고 적는다

디스패치는 망 비용을 조금 내주고 캐시 이득을 얻는 맞바꿈인데, **지금 계획기는 하위 문제를
과하게 흩어 보낸다고 스스로 밝힌다.** 초기 벤치마크라는 것도 제목에 적어 둔다.

### 다음 사이클 — 확장 (AuthZed 의 빈 도메인)

`권한 그래프를 빨리 훑는다` 부터. 시나리오·최적화·수치·남은 과제가 다 손에 있다.
