...
**없다 — 문체 보수 사이클이었다.** SmartHR 을 배민 문체로 다시 썼다(1.55 → 0.0). 회사 115개, 큐 2/3. 20곳 남았다.

### 🔴 가 세 번째로 나왔다

사이보우즈(64) · 하이퍼커넥트(96)에 이어 여기서 **158개**가 나왔다. validate 가 세지 않는 기호라
--gaps 에는 안 잡힌다. 사업 모델 안의 조사 로그와 엔진 내부 번호(`344 에는 아직 한 편만 읽었다`)도
같이 나왔다. **세 번 연속이면 이건 그 시기에 쓴 문서들의 공통 습관이다.**

일본어 원문 인용 25곳을 우리말로 풀었다. 글 제목은 그대로 뒀다.

### 같은 문제에 정반대로 답하는 두 도메인

이 회사가 특이한 것은 한 회사 안에서 대칭이 선다는 점이다. 둘 다 하나가 멈추면 전부 멈춘다는
문제인데,

- 인증 쪽은 **떼어내서** 장애를 격리하려 한다.
- 가장 큰 Rails 애플리케이션은 **그대로 둔 채 앞에 커넥션 풀러를 세워** 버티게 한다.

그리고 버티는 쪽이 훨씬 빨리 끝났다 — 재발 방지가 목표라 단기 완료가 조건이었고, 그 조건이
GKE 대신 Compute Engine 이라는 기술 선택까지 좁혔다.

### 나누는 전략이 꺾이는 자리를 적은 둘

- Etsy 는 테이블을 하나씩 올리다 **가장 위험한 것들이 묶여 있다**는 것을 발견했다.
- SmartHR 은 규칙으로 경계를 긋는 방법이 **새 영역에서는 됐는데 이미 얽힌 인증에서는 안 됐다**고
  적는다. 되는 사례와 안 되는 사례를 같은 블로그에 나란히 둔다.

볼트와도 닿는다 — 저쪽은 설정 주도를 8년 굴려 보고 되물렀고 여기는 실행 전에 버렸다.
**한쪽은 겪고 나서 알았고 다른 쪽은 검토하다 알았다.**

### 단일 실패점을 알면서 미룬 둘, 드문 날을 미리 재는 둘

- Monzo 는 상태를 한 클러스터에 모아 둔 값을 장애로 치렀고, SmartHR 은 **알면서도 언제 심해질지
  몰라 미뤘다**고 반성으로 적는다. 그다음이 갈린다 — 저쪽은 쪼갰고 여기는 앞을 막았다.
- Fastly 는 엣지 용량 때문에 드문 날을 고르는 모델로 갔고, SmartHR 은 정점이 오기 전에 부하 시험을
  돌린다. 여기엔 축이 하나 더 있다 — **언제 손을 떼는가를 설계 대상으로 삼는다**(SRE 의 EXIT
  로드맵). 그런데도 전환 시점 판단은 어려웠다고 적는다.

### 다음 사이클 — 문체 보수 (사다리 3순위)

--gaps 가 정한다. 지금 맨 위는 kurly(1.5, 489개)지만 그건 이 시점의 예상이지 확정이 아니다.
