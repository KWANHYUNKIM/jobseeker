...
**Perplexity** — 기능 **1/3.** 회사 111개. 큐 0/3. ⚠️ **`--gaps` 가 STATE 계획(RDMA)과 다른 도메인을 지목했고 출력을 따랐다.**

### ⚠️⚠️ 지난 사이클의 메모가 틀렸다 — 개별 글도 403 이다

프로파일에 **`개별 글 주소는 열린다`**고 적었는데 **직접 열어 보니 개별 글도 403 이다.** ⭐ **실제로는 3자 기사(MarkTechPost)가 원문을 인용한 것을 읽고 있었다.**

⚠️ **`열린다`와 `내용을 얻었다`를 구별하지 않은 것이 원인이다**(재구성) — 3자 기사에서 주소를 얻었으니 그 주소가 열린다고 넘겨짚었다. ⭐ **그래서 이 회사의 인용은 전부 `회사가 말한 것을 매체가 옮긴 것`이고, 출처에 매체를 정확히 적었다.** **arXiv 와 GitHub 은 직접 열린다.**

### 이번 사이클 — `GPU 앞의 일을 러스트로 옮긴다 (Ivy · Tulip)`

⭐ **GPU 가 도는 시간만 원가가 아니다** — **JSON 파싱·토큰화·입력 템플릿·배치 쪼개기**가 다 GPU 앞의 CPU 일이고 그동안 GPU 는 논다(재구성). ⚠️ **사용량으로 파는 회사라 GPU 이용률이 곧 마진이다**(재구성).

건진 것 셋:

- ⭐ **선착순 스케줄러를 쓰면서 근거를 모델의 비용 구조에서 댄다** — **`퍼플렉시티가 서비스하는 시퀀스 길이에서 작은 임베딩 모델은 조밀 레이어의 선형 비용이 어텐션의 이차 비용을 압도한다.`** **그래서 지연이 시퀀스 수가 아니라 토큰 수에 비례하고, 정교하게 섞어도 얻을 게 없다**(재구성). ⚠️ **어텐션이 지배하는 자리였다면 이 결정이 틀렸을 것이다.**
- ⭐ **채운다고 계속 좋아지지 않는다는 것을 수치로 안다** — **`배치가 GPU 를 포화시킨 뒤에는, 10억 파라미터 미만 모델에서 약 512 토큰인데, 시퀀스를 더 채워 넣어도 효율이 나아지지 않는다.`** **그 뒤로는 처리량은 그대로고 지연만 는다**(재구성).
- ⭐ **분산 단위를 요청이 아니라 청크로 내렸다** — **`큰 배치 요청을 청크로 쪼개 복제본들에 부하를 분산하는데, 이는 프로덕션 페이로드의 크기가 제각각일 때 생기는 부하 불균형을 바로잡기 위해서다.`** ⚠️ **대가는 한 요청의 조각이 흩어져 가장 느린 조각이 응답 시각을 정한다는 것이다**(재구성).

### ⚠️ 러스트를 고른 이유는 회사가 안 적었다

**`CPU 쪽 일`이라는 서술과 언어 선택이 나란히 놓여 있을 뿐**이라 그 결정을 `inferred` 로 달았다. ⭐ **다만 대비는 뚜렷하다** — 앞단(Ivy·Tulip)은 러스트, 엔진(ROSE)은 파이썬이고, **모델 정의는 자주 바뀌고 앞단은 안 바뀐다**(재구성).

### 다음 사이클 — 확장 (2순위)

**Perplexity 기능 1/3.** ⏳ 남은 둘 — `모델을 GPU 위에서 돌린다`(ROSE · CUDA 그래프 지연 캡처 · LazyTensor) · **`GPU 끼리 직접 말하게 한다`**(fabric-lib · **arXiv 본문이 직접 열려 근거가 가장 두껍다**).

⚠️ **큐가 0/3 이다** — 이 회사를 완주한 뒤 **후보 조사**를 반드시 끼운다. ⏳ **`큰 이름인데 아직 없는 회사`** — Anthropic · Databricks · Snowflake · Anduril · Ramp 외 핀테크.
