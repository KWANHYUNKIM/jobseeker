...
**Sentry(US · SaaS—오류 추적·관측)** `in_progress` · **도메인 2 · 기능 0** —
**77곳** · ⚠️ 큐 0/3.

### ⚠️ 읽은 글 셋 중 하나가 자기 시스템이 아니었다 — Capital One 의 함정을 또 만났다

`metrics-caught-ai-size-estimate`(2026-09-01)는 **개발자 개인의 오픈소스 프로젝트**가
Sentry 제품을 쓴 사례였다(추정치보다 83% 큰 출력 파일을 지표가 잡아낸 이야기).
**수치는 좋은데 이 회사의 시스템이 아니라 프로파일에서 뺐다.**
**`깊이 있는가` 와 `자기 시스템인가` 를 따로 보는 규칙이 두 번째로 값을 했다.**

### ⭐⭐ 두 도메인이 한 줄로 이어진다 — AI 를 들이면 AI 를 지켜보는 일이 생긴다

Seer 가 이슈를 고치는데(도메인 1), **그 에이전트가 잘 돌고 있는지 보는 일**이 새로
생겼다(도메인 2). 그 전에는 **일회성 흐름을 손으로 돌리고 JSON 을 눈으로 파싱**했다.

### ⭐⭐ 핵심 발견 하나 — 성공한 대화도 표본에 넣어야 한다

`Sampling both success and failure conversations, not just the ones with tool errors,
mattered more than expected.` **오류 없이 끝나도 잘못된 결론에 이를 수 있고, 그것은
추론을 검증해야만 보인다.** ⚠️ 대신 **볼 것이 두 배가 된다.**

### 다음 사이클 — 감시 쪽이 수치가 더 세다

**`claude-routines-agent-triage`**(전문 읽음) — 매일 밤 대화 **약 800건** · 표본 기간
**445~551건** · 도구 호출 **약 11,000건** · **오류율 21%**(표본 400 중 83) ·
**30~56초 지연 행**. 구조도 뚜렷하다 — **MCP 가 끌어오고 거르고, Claude 는 판단만**
맡으니 출력이 일관됐다.

다른 도메인(경계 긋기)도 자료가 있다 — PR 액션률 **+21%** · 48시간 응답률 **+13%** ·
**병합 없이 닫힌 PR +12.5%**(회사가 이것을 **성공으로 변호한다**).

### ⚠️ 아직 못 잡은 것

1. **Seer 의 내부 구조** — 어떻게 분석하고 고치는지가 읽은 글에 없다.
2. **오류 추적 제품 자체의 구조** — 이 회사의 뿌리인데 읽은 글이 전부 AI 워크플로다.
   **오픈소스이므로 자료가 있을 것이다.**
