...
**Wayfair (US · 커머스/대형 물품) — 44번째. 기능 1개 · 도메인 2개.**

241 에서 **`damage-judgment`** 를 썼다. 결정 8 · 그림 3 · 생각 4 · 스택 6 · 난제 2 · ui 4핀.

**두 번째 글을 먼저 읽은 것이 축을 정했고, 세워 둔 가설이 그대로 맞았다.**
두 글이 **같은 판정(손상)을 정반대 조건에서** 한다.
- **창고(Tarragon)** — 고정 카메라 여섯 각도 · 고정 조명 · **전체 1초 SLA**. 파인튜닝한 분류기.
- **고객 사진(Saffron)** — 아무 폰 · 아무 조명. *"단일 모델 API 호출이 꼬리에서 몇 초"* 를
  받아들이고 **비동기 전처리 + 캐시된 조회**로 실시간 경로에서 뺐다.

**🔴 가장 눈에 걸린 것 — 수치 밀도가 정반대다.** 창고 글은 숫자가 빽빽한데
**고객 사진 글에는 수치가 하나도 없다**(정확도·처리량·실제 지연·비용·오탐률 전부).
**통제된 환경이라야 잴 수 있기 때문으로 보인다** — 추측이라고 밝혀 적었다.
회사도 그 자리를 한 줄로 짚는다: *"GenAI 에서는 대개 쓸 만한 단일 스칼라 점수가 없다."*
**그래서 숫자 대신 구조로 안전을 산다** — 스키마로 답을 가두고, **'미결정' 을 정식 결과**로
두고, *"불확실할 때는 개입을 피하고 더 안전한/기본 흐름으로 넘긴다"*.

**또 하나** — 카메라 노출이 일정하게 어긋난 것을 **소프트웨어로 덮지 않고 고쳤다**:
*"고치는 데 하드웨어 벤더와 몇 달간 조율이 필요했다."* **물리 세계를 판정하는 시스템에서는
고칠 곳이 코드 밖에도 있다.**

**⚠️ 남은 도메인 `분류가 아니라 생김새로 찾게 한다` 는 0개다.** 본문을 한 편
(`seeing-beyond-labels`, 2026-06)만 읽었으니 **기능을 쓰기 전에 두 번째 글
`smarter-shopping-starts-here-how-ai-understands-what-youre-looking-for`(2025-12-01)를 읽는다.**
같은 도메인의 세 번째 글 `how-we-validate-product-dimensions-at-scale-with-multimodal-genai`
(2026-01-15)도 아직 미독이라, **손상 도메인의 두 번째 기능**으로 쓸 여지가 있다.
**Data Science 태그 90편은 아직 한 편도 안 봤다** — 세 번째 도메인이 열린다면 거기일 것이다
(`how-we-used-ai-as-a-lab-assistant-to-run-110-experiments-in-three-days`, 2026-06-15 이 눈에 띈다).

**⚠️ 큐가 0 이다 — Wayfair 를 완주하면 곧바로 후보 조사다.**
...
**인프라(Vercel·Neon·Elastic·MongoDB·Confluent)는 뒤로 미룬다**(238 의 관찰 — 개발자 대상
제품 회사의 블로그는 대개 영업 채널이다).
