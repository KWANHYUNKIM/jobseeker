...
**Zed — 기능 하나를 채웠다(`os-sandbox`, 에이전트의 손을 커널이 잡는다).** 회사 117개, 큐 0/3.
빈 도메인 둘이 남았다 — 다음 편집을 미리 맞힌다 · 글자를 무엇에 담을 것인가.

### 가두는 답이 셋이 됐고, 여기가 유일하게 파는 제품 안이다

Razorpay 는 격리를 커널의 속성이라고 못 박았고, 그랩은 에이전트가 뚫리거나 말을 안 들을 수
있다고 전제하고 제어면을 바깥에 뒀다. 둘 다 사내 플랫폼이다. Zed 는 같은 결론을 **사용자
랩톱에서 도는 상품** 안에 넣어야 해서 제약이 다르다 — 승인 흐름과 승인 피로가 여기서만 나온다.

근거를 대는 방식도 다르다. Zed 는 정렬 문제로 시작한다 — 에이전트가 사용자가 승인할 행동인지
스스로 가릴 수 없는 한 가두는 것이 최선이고, 그것은 답이 아니라 유예라고 적는다.

### 버린 답과 못 덮는 곳을 함께 적은 것이 이 글의 값어치다

명령어 패턴 금지는 셸 서브셸·환경변수·다른 인터프리터로 우회되므로 버렸다. 막을 것을 세는
방식은 다 셌는지를 증명할 수 없다는 데서 무너진다. 그래서 굵은 선을 긋는 쪽으로 갔다 —
프로젝트 밖 쓰기 금지, 네트워크 기본 거부, 그리고 `.git` 쓰기는 승인 흐름에서 아예 뺐다
(훅이 울타리 밖에서 돌기 때문).

덮지 못하는 곳도 회사가 먼저 나열한다 — 편집 도구·일반 터미널·LSP·MCP·build.rs·절차
매크로·.bashrc·깃 훅. 목록을 공개하면 공격 안내서가 되지만 덮는다고 믿게 두는 편이 더
위험하다는 선택이다.

### 좁은 창은 방어가 아니다

승인과 실행 사이에 디렉터리를 심링크로 바꿔치기하는 경합을 스스로 예로 든다. 창이 마이크로초
단위인데도 병렬 하위 에이전트로 몇 번이고 다시 노릴 수 있으므로, 닫는 쪽으로 실패하게 만들었다.
**좁다는 것을 방어로 치지 않는다** — 이 엔진에서 확률적 안전을 이렇게 명시적으로 거절한 글이 드물다.

### 다음 사이클 — 확장 (같은 회사의 빈 도메인)

회사를 갈아타지 않는다는 규칙이 위다. 큐가 0이지만 Zed 를 끝낸 뒤에 후보 조사로 간다.
