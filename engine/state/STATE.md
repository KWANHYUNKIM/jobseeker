...
**없다 — `in_progress` 0 · 완료 56곳.** 337 에서 **비교 문서 `when-you-cannot-undo`
(되돌릴 수 없을 때 무엇을 하는가 — 취소가 세상을 되돌리지 않을 때)를 썼다. 비교 문서 32편.**

**⚠️ `--gaps` 는 [신규 = freee]를 가리켰는데 한 번 미뤘다.** 근거 — **재료 다섯이 320·326·
333·334·335 다섯 사이클에서 연달아 나왔고 전부 한 축에 모였다.** 새 회사를 열면 그 재료가
다시 몇십 사이클 묵는다. **⚠️ 다음 사이클은 반드시 freee 를 연다.**

**⭐ 실제로 선 축 셋**
- **되돌릴 수 없는 것이 어디에 있는가** — **재처리 비용 안**(Paytm: S3 Batch 가 50억 개를
  훑어 요금을 매겨 재시도가 2배) · **데이터 형식 안**(Freshworks: 새 판이 만든 것을 옛 판이
  못 읽으면 롤백이 종이 계획) · **중간 상태 안**(Sansan 앱: 11줄 대 1줄) · **구조 안**
  (Compose: 다시 만들 수 없다) · **⭐ 화면 밖**(Sansan 실물: 상자가 이미 옮겨졌다).
  **마지막이 이 사이트에 없던 종류다.**
- **값을 낼 것인가 안 낼 것인가** — **Paytm 만 안 냈다**(두고 갔다). 그리고 그것이 가능한
  조건이 하나 있다 — **원본이 아직 옛 리전에 있다.** **되돌릴 수 없는 것과 잃어버린 것은
  다르다**(이 관찰은 이 사이트의 것).
- **못 내면 무엇으로 대신하는가** — **애초에 틀리지 않게 만든다**(버튼 감추기 · 통합 ID ·
  규칙 표준화). ⚠️ 그래도 남는 것은 **현장 규칙**으로 간다.

**⭐⭐ 다른 문서의 빈칸 둘을 채웠다** — `reversible-first.md` 의 **되돌리지 못해 앞으로 밀고
나간 사례가 없다**(→ Paytm)와 **언제 롤백 경로를 닫는지 정한 곳이 없다**(→ **Freshworks 가
적었다** — if/else·몽키패치·버전별 캐시 키·`Gemfile_next.lock` 을 지웠다). **반대쪽에 Sansan
앱이 있다** — 정리가 마지막 단계인데 **아직 시작도 안 했고** 전체가 약 60% 다.
**⭐ 그래서 축이 하나 더 생겼다 — 닫는 것을 계획에 넣은 쪽과 마지막으로 미뤄 둔 쪽.**

**기존 문서 셋과 갈랐다** — `reversible-first.md`(되돌릴 길을 **어떻게 만드는가**) ·
`swap-while-running.md`(**앱이 아는가**) · `where-it-piles-up.md`(**미룬 것이 어디 쌓이는가`).
**여기는 그 앞의 질문** — 길을 만들 수 있는지가 이미 정해졌을 때 **값을 낼 것인가**다.
머리말에서 서로 가리키게 했다.

---

**다음 사이클 — [신규] freee(JP · SaaS)를 연다. 더 미루지 않는다.**
- **⑭ `--gaps` 출력을 실제로 보고 따른다.** 큐 2/3 이므로 [신규]가 나올 것이다.
- **PROMPT.md 3단계(회사 프로파일)부터** — 319·323·329 와 같은 방식. **비즈니스 모델 · 제품 ·
  도메인 후보까지 쓰고 커밋, 기능은 다음 사이클.**
- **재료는 328 에서 확인해 뒀다**(본문을 열었다, 추측 아님) —
  `https://developers.freee.co.jp/entry/migrate-to-auth-microservice` (2022-08-31,
  **인증·인가 기반 마이크로서비스화 4년의 회고**). **⭐ 이 사이트에 드문 종류다 — 후회를
  적는다**: DDD 재설계로 *"旧基盤との挙動の違いが大量に発生しました"*, 레거시의 부정 데이터가
  새 기반에서 에러, 견적 부족으로 연기 다발, *"開発メンバーの精神が摩耗する状態となっていました"*.
  교훈이 **"잘게 릴리스했어야"** 와 **"마이크로서비스화 전에 옛 기능을 리팩터했어야"** 다.
  **수치** — 2019~2022.6 · QA 가 **14제품 633 테스트 케이스**로 사내 최대.
  다른 재료 — **MySQL→Aurora 이관과 샤딩** · **권한 관리 기반 마이크로서비스** ·
  Langfuse v2→v3 에서 **ClickHouse 사내 첫 도입**.
- **⚠️ 목록과 주소는 ⑰·⑱ 로 확인한다.** 블로그는 `developers.freee.co.jp` 다.
...
- **⭐⭐ freee 와 SmartHR 이 정반대 순서로 같은 문제를 풀었다** — 인증·권한 기반을 떼어내는데
  **freee 는 모놀리스에서 곧장 4년**(후회로 적는다), **SmartHR 은 모듈러 모놀리스로 시작해
  Packwerk 로 경계를 먼저 보이게 한 뒤** 떼어낸다. **둘을 다 파고 나면 비교 문서 재료다.**
- **후보 조사 누적 열아홉 곳 중 일곱.**
- **⚠️ 낡은 메모를 나르지 않는다**(314·317·325). **⚠️ 두 방향 규칙**(298·305·318·321).
  ...
  **⭐ 이 사이트 자기 문서를 인용할 때도 우리말은 굵게 쓴다**(337 에서 넷이 걸렸다 — 새 교훈).
- **분포** — **56곳 전부 `done`.** US 22 포화 · 한국 아홉 · 일본 다섯 · EU 일곱 · 중국 둘 ·
