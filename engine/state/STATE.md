...
**없다 — Deepgram 을 완주시켰다(도메인 1 · 기능 1 · `status: done`).** 회사 161개, **큐 0/3.**

### 처리량이 3천 배 큰 쪽을 물렸다

이번 기능(`stream-before-you-know-the-end`)에서 가장 좋은 대목이다. 병렬 디코딩은 초당 50만+
타임스텝이고 자기회귀는 172 다. 그런데 **전부 계산한 뒤에야 내주므로 첫 소리가 늦다** — 그래서
느린 쪽을 골랐다. **저울의 눈금이 처리량이 아니라 첫 소리 시각**이라는 것이 이 도메인의 전부다.

### 원인을 구조에서 찾는다

길수록 느려지는 것이 버퍼도 보코더도 아니라 **프레임이 다음 프레임의 입력이 되는 구조**라고
짚는다 — 하드웨어를 늘려도 순차성은 남는다(재구성).

### 대가를 적고 완화책이 없다는 것도 적는다

**문맥 5~20배 감소 → 성급한 음소 결정 → 고유명사 발음 정확도 하락.** 그리고 이 글은 그것을
어떻게 눅이는지 **제시하지 않는다** — 그 사실을 `limits` 와 `open_questions` 에 남겼다.

### 재는 법까지 알려 준다

**총 지연 = 네트워크 + TTFB + 오디오 합성**이고 마지막 항이 데모가 감추는 것이라며, 턴마다
**타임스탬프 넷**(텍스트 준비·첫 바이트·첫 재생·마지막 재생)에 상관 ID 를 붙이고 p50~p99 와
RTF 를 보며 **길이와 동시성을 바꿔 가며** 재라고 적는다.

### 심어 둔 대비 둘

**Fireworks**(커널의 루프 순서를 병목으로 고르며 처리량을 잰다) ↔ **Deepgram**(같은 저울의 반대
쪽 끝인 첫 소리 시각). 그리고 **Mux**("실제로 무엇이 전달됐는지는 플레이어만 안다") ↔
**Deepgram**("총 지연의 마지막 항이 데모가 감추는 것") — 둘 다 **측정 지점을 바꿔야 진짜가
보인다**는 이야기다.

### 다음 사이클 — ⚠️ 후보 조사 (큐가 0/3 이다)

`blocked` — incident.io(404×3) · Sourcegraph(403) · Guardian · Flexport. **네 사이클 연속 이
목록에서 하나씩 되살렸다**(Fireworks · Ghostty · Framework · Deepgram).
