...
**Block (Square)** — 기능 **4/4**, 도메인 전부 찼다. ⚠️ **그런데 `done` 이 아니다** — 아래 이유. 회사 109개. 큐 2/3.

### ⚠️ 완주 기준 셋 중 하나가 안 닫혔다 — 그래서 `in_progress` 를 유지했다

- ✅ **모든 도메인에 기능 1개 이상** — 4/4.
- ✅ **connections 고아 없음** — 네 기능이 서로와 릴레이·사람·제조사로 이어진다.
- ⚠️ **각 수익원이 최소 한 기능과 연결** — **`커머스 인에이블먼트(Square)` 가 비어 있다.** **총이익 12억 달러를 내는 본업인데, 이번에 읽은 여섯 편이 전부 AI·에이전트·펌웨어였다.**

⚠️ **그리고 `--gaps` 는 이걸 못 집는다** — 빈 도메인이 없으니 `정리할 것이 없다` 로 넘어간다. **엔진의 눈이 도메인까지만 닿고 수익원까지는 안 닿는다.** ⭐ **그래서 다음 사이클 대상을 여기 적어 둔다** — **Square 본업(결제 처리·정산·대출 심사) 기술 자료를 찾아, 있으면 도메인을 하나 더 세우고 없으면 `그 회사가 본업을 안 쓴다`는 사실로 닫는다.** (코드는 고치지 않는다.)

### 이번 사이클 — `층을 흩어 큰 모델을 돌린다 (MeshLLM)`

⭐ **관찰 하나에서 출발한다** — **`어느 순간에도 방 안에는 놀고 있는 컴퓨트가 잔뜩 있다. 방이 아니면 집이나 사무실에, 아니면 밤이나 주말에 안 쓰는 용량이 있을 것이다.`**

- **모델을 미리 층으로 쪼개** 각 기계가 **자기에게 필요한 층만** 내려받는다. 메시가 층을 파이프라인 단계에 배정하고 **가장 낮은 지연 경로**로 잇는다. **5천억~1조 파라미터 규모**를 돌렸다(Qwen · Inkling).
- ⭐ **보상과 대가를 한 문장에 같이 적는다** — **`보상은 훨씬 큰 모델을 돌릴 수 있다는 것이고, 대가는 단계 사이의 통신 지연이며, 그것이 추론 파이프라인의 일부를 놀게 둘 수 있다.`**
- **놀리느니 검증시킨다** — **드래프트 모델·n-gram 매칭·MTP** 가 **뒤 단계가 검증할 후보 토큰**을 만든다. ⚠️ **그래서 상한을 정하는 것이 하드웨어 총량이 아니다** — **`실효 처리량이 네트워크와, 예측기가 각 단계를 얼마나 잘 바쁘게 유지하느냐에 크게 좌우된다.`**

### ⭐ 기술로 못 푼 것을 못 풀었다고 적는다

⚠️ **경계를 그어도 데이터는 넘어간다** — **`프롬프트와 다른 기계로 보낸 어떤 데이터든 그 기계 소유자에게 보이고, 당신은 돌아오는 결과를 믿는 것이다.`** ⭐ **회사가 내놓는 답이 프로토콜이 아니라 문장이다** — **`믿는 사람과 컴퓨트를 공유하라.`** 그리고 **공개 메시는 `엄격히 최선 노력이고 아무 보장이 없다`** 고 못 박아 **진지한 용도를 사설 메시로 밀어낸다.**

⭐ **아직 안 정한 것도 그대로 적는다** — **`작은 모델 여럿으로 시작하는가, 아니면 프런티어급 모델을 위한 메시가 만들어질 만큼 피어가 모일 때까지 참을성 있게 기다리는가?`** ⚠️ **분산 추론의 어려움이 성능이 아니라 기다림의 설계라는 뜻이다**(재구성).

### ⭐ Block 네 기능을 한 줄로 꿰면 — `못 하는 것을 못 한다고 적는 회사`

**네 기능 전부가 자기 한계를 문장으로 남긴다**(재구성):

- **Moneybot** — **`관측할 수 없는 행동을 고치려 들지 않는다`**(재현 안 되면 세워 둔다).
- **COLDCARD 자문** — **`악용 가능성을 확인할 완전한 경험적 시험은 하지 않았다`**(그래도 이르게 낸다).
- **벌 계층** — **`열두 구성 중 어느 것도 값을 생각하면 솔로를 앞서지 못했다`**(자기 가정을 깬다).
- **MeshLLM** — **`믿는 사람과 공유하라`**(프라이버시를 기술로 못 풀었다).

⭐ **비교 문서 재료로 이만한 회사가 드물다** — ㉘축(`맞았는지 어떻게 아나`)에 세 칸을 채우고, 여기에 **`모르는 것을 어떻게 적나`** 라는 새 축(㊵)이 하나 더 선다.

### 다음 사이클 — ⚠️ Block 의 Square 본업 (위 이유)

**그다음** 후보 조사(큐 2/3). ⏳ 미독 — `BlockTabBench` · `Buzz 프로젝트` · `Microfilm`. **⭐ 남은 물음 — Bitkey 는 난수를 어떻게 만드나.**
