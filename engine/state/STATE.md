...
**WarpStream** (US · SaaS) — 도메인 3개 · **기능 2개**. 남은 하나는 **관측**(Events·감사 로그·MCP, 본문 미독)이다. 회사 100개 · 큐 1/3.

### 이번 사이클 — `도착지의 느림을 미리 겪게 한다`

⭐ **앞 기능과 정확히 이어졌다** — 앞 기능이 **지연을 70% 줄인 이야기**인데, **그래도 남은 격차(한 자릿수 밀리초 대 수백 밀리초)가 이번 기능의 최대 위험이다.** ⚠️ **자기가 못 없앤 대가가 영업의 장벽이 됐다.**

건진 것 다섯:

- ⚠️ **판정 기준이 기술적 가능성이 아니라 남을 움직일 수 있는지였다** — 커스텀 팻 클라이언트는 **`모든 언어에서 동작해야 하고`** 무엇보다 **`서로 다른 많은 팀을 설득해 클라이언트를 갈아 끼우게 할 수 없었다`**. 피처 플래그도 **`개별 고객의 애플리케이션을 우리가 다시 쓸 수는 없다`**. **그래서 자기 쪽에 얇은 프록시를 놨다** — **옮기는 쪽이 아니라 옮겨지는 쪽의 사정에 맞춘 것이다**(재구성).
- ⭐ **되돌릴 수 없는 순간의 앞쪽으로 고칠 시간을 옮겼다** — **`프록시는 전달하는 모든 produce 요청마다 WarpStream 자신이 그 배치를 내구성 있게 저장하는 데 얼마나 걸렸을지 추정하고, 그만큼 시간이 지날 때까지 응답을 붙들고 있는다`** 라서 **`프로듀서들이 미리 튜닝된 상태가 된다 — 컷오버 뒤에 불을 끄는 대신에`**. **일부러 느리게 만드는 결정이다.**
- ⚠️ **손실과 중복 사이의 원칙이 앞 기능과 같다** — **`잃은 데이터를 가끔의 중복과 맞바꾸는 것이 옳은 절충이다`** 이고 근거가 **`성공했을지도 모르는 쓰기를 성공했다고 프로듀서에게 말하는 것이야말로 우리가 막으려던 바로 그 손실이다`**. **일관성은 내주면서 손실은 안 내준다**(앞 기능의 `데이터 손실 제로` 와 같은 자리).
- ⚠️ **중앙이 못 미치는 곳을 시간으로 다스린다** — **`전달할 권한이 몇 초 안에 만료되는 리스이고, 갱신하는 유일한 방법은 그 토픽의 현재 상태를 가져오는 것이다. 분단된 에이전트는 갱신할 수 없으니 스스로 전달을 멈춘다.`**
- ⚠️ **이미 있는 복제 경로가 감시 경로가 됐다** — **`프록시가 모든 레코드에 작은 내부 헤더를 붙이고, Orbit 이 소스 토픽의 모든 레코드를 보기 때문에 어떤 것이 프록시를 지나왔고 어떤 것이 직접 쓰였는지 알 수 있다.`** 크론잡 같은 것을 위해 **24시간 스냅숏 대조**까지 둔다.

⚠️ **롤백이 있는 곳과 없는 곳을 갈랐다** — MIGRATING 까지는 **제한 시간 초과 시 자동으로 PROXY 로 되돌아가는데**, COMPLETE 는 **`일방통행 문이다 — 토픽이 한 번 컷오버하면 쉽게 돌아갈 방법이 없다`**.

### 다음 사이클 — 확장(2순위)

`--gaps` 가 **`클러스터 안에서 무슨 일이 있었는지 본다`** 를 부를 것이다. ⏳ **본문 미독 · URL 확보** — `Kafka Isn't a Database, But We Gave It a Query Engine Anyway`(2026-03-20) · 감사 로그(2026-02-13) · MCP 서버(2026-04-16). **이걸 쓰면 WarpStream 완주다.** ⭐ **연결 재료도 이미 있다** — 오프셋이 0 으로 오고 순서가 배경에서 붙으니 **진행 상황을 다른 데서 알아야 한다**(재구성).

⏳ **연표 재료** — **`WarpStream is Dead, Long Live WarpStream`**.
...
**⑧ ⭐ `제약을 없애지 못할 때 어디서 갚는가`** — Cygames / Zepto / Grafana Labs / Pinterest / **WarpStream**(⚠️ **세 겹이 됐다** — 더 빠른 저장소 · 기다림을 줄이는 구조 · **그래도 남은 느림을 고객에게 미리 겪게 하기**).

**⑨ ⭐ `한 번에 갈아엎을 것인가 목 졸라 죽일 것인가` 7곳** — Twilio(두 방향) / Etsy / Plaid / Paystack / Airbnb / Pinterest / **WarpStream**(⚠️ **남의 시스템을 옮겨 오는 쪽이라 각도가 다르다** — **자기 일정이 아니라 남의 일정에 자기를 맞춘다**).
...
**⑬ ⭐ `애매할 때 어느 쪽으로 넘어지나` 7곳** — Flipkart(둘) / Razorpay / Zepto / Paystack / Airbnb / Pinterest / **WarpStream**(⚠️ **두 기능이 같은 답을 낸다 — 일관성은 내주고 손실은 안 내준다**).

**⑭ `검색 관련성을 누가 정하나`** — Etsy / Flipkart / ⏳ Zepto / Pinterest.

**⑮ `언제 쪼개고 언제 합치나`** — Twilio / DoorDash / Deliveroo / ScyllaDB / Airbnb / Pinterest / WarpStream(내구성과 순서를 갈랐다).
...
**⑳ `없앨 수 있는 것을 없앤다`** — WarpStream(디스크) / ⏳ TigerBeetle · Oxide Computer.

**㉑ `논문을 어디까지 그대로 쓰나`** — WarpStream(LazyLog) / ⏳ ScyllaDB · TigerBeetle · ClickHouse.

⏳ **㉒ 새로 보인다 — `되돌릴 수 있는 곳과 없는 곳을 어떻게 가르나`** — **WarpStream**(MIGRATING 까지는 자동 롤백, COMPLETE 는 **`일방통행 문`**) / **Plaid**(다운타임 0 이전) / **Airbnb**(**`먼저 메트릭을 흐르게 하고 그다음 대시보드`**) / **Twilio**(피처 플래그로 읽기·쓰기 따로). ⚠️ **되돌아갈 곳이 있다는 것이 자동화의 전제다**(재구성).
