...
**없다 — 문체 보수 사이클이었다.** 하이퍼커넥트를 배민 문체로 다시 썼다(1.75 → 0.0, 기호 457개로 최다). 회사 115개, 큐 2/3. 23곳 남았다.

### 앞 사이클과 같은 종류의 빚이 또 나왔다

사이보우즈에서 본 것이 여기서 반복된다. **`🔴` 가 96개** 있었고(validate 가 안 세는 기호다),
사업 모델 안에 조사 로그가 통째로 들어 있었으며(글의 성격을 비교한 ①~④), 엔진 내부 번호가
데이터에 남아 있었다(`257 에서 정정`). 한국어 인용을 푼 자리의 조사도 손봤다.

**두 회사 연속으로 나왔으니 우연이 아니다** — 문체 보수는 기호만의 일이 아니고, `--gaps` 가
집어 주는 수치는 그중 한 종류만 센다.

### 무엇을 맞혀야 하는지를 의심한 회사가 넷이 됐다

- Anthropic — 재는 자와 무대.
- Etsy — 자기 실험 체계.
- Fastly — 평상시가 아니라 드문 날.
- 하이퍼커넥트 — **목표 그 자체.** 잘못된 지표를 최적화하면 모델을 아무리 높여도 서비스에
  효과가 없다고 적는다.

그리고 Duolingo 와 같은 시차 문제를 만난다 — **좋은 매칭이었는지를 그 사람이 다시 오는가로만
알 수 있다.** 그 시차가 아하 모먼트라는 중간 지표를 만들었다.

### 정확히 한 번을 지키는 쪽과 포기하는 쪽

- Datadog 은 Husky 에서 정확히 한 번과 멀티테넌시를 같은 자리에서 지킨다.
- 하이퍼커넥트는 두 서비스 사이의 중복 발행을 판단할 수 없어 **정확히 한 번을 포기했다** —
  재료를 제때 만드는 것이 먼저였다.

Klaviyo 는 줄 세우기를 자기가 쥐고 마감 순으로 다시 세웠는데, **여기서는 멈춤 자체가 매칭을 끊는다.**

### 복구 속도가 곧 가용성이다

같은 카산드라 계열을 두고 답이 셋이다 — Discord 는 **떠났고**(러스트로 다시 씀), Monzo 는 한
클러스터에 **모아 둔 값을 장애로 치렀고**, 하이퍼커넥트는 **되살아나는 시간을 24시간에서 1시간으로**
줄였다. 낯선 사람을 즉시 잇는 서비스에서는 복구 중인 시간이 곧 매칭이 끊긴 시간이다.

ARM64 이전은 Booking.com 과 같은 순서였다(쉽고 위험 낮은 것부터). 다만 SIGSEGV 와 빌드 10배
지연을 그대로 적는다 — 허니컴·도어대시와 같은 자리다.

### 다음 사이클 — 문체 보수 (사다리 3순위)

--gaps 가 정한다. 지금 맨 위는 nubank(1.72)지만 그건 이 시점의 예상이지 확정이 아니다.
