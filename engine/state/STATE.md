...
**헨리(Henry)**(JP · 병원 SaaS — 전자카르테·레세콘) — 회사 프로파일까지 썼다.
**도메인 둘 · 기능 0.** 큐 1/3.

### 제도가 두 겹으로 걸리는 회사

한쪽은 **돈 계산의 규칙** — 레세콘이 **'진료보수제도라는 거대하고 복잡한 규칙'** 안에서
돌아야 한다. 다른 쪽은 **데이터 취급의 규칙** — 맡아 둔 환자 데이터가 **의료기관이 정한
목적 안에** 머물러야 한다. **두 도메인이 다 남이 정한 규칙에서 나온다.**

### 기록과 청구를 한 시스템에 두는 값

전자카르테는 **'환자에게 무엇을 했는가'**, 레세콘은 **'그것이 제도상 얼마가 되는가'** —
둘을 함께 가지니 **'병원의 한복판을 쥐고 있는'** 위치가 된다. 대가는 **방대한 기능 세트**
다(만성기·회복기·급성기마다 흐름이 통째로 다르다).

### 동의 면제가 설명 면제는 아니다

개인정보보호법 개정으로 특례가 생겼지만 **'개정 뒤에도 SaaS 사업자가 맡아 둔 환자
데이터를 자유롭게 통계 작성이나 AI 학습에 이용할 수 있게 되는 것은 아닙니다.'** 가르는
축은 **위탁이냐 제3자 제공이냐**다. 그리고 못 박는다 — **'본인 동의가 필요 없어지는 것과,
데이터 이용에 대해 누구에게도 설명하지 않아도 되는 것은 전혀 다릅니다.'**

### 법무 문서를 제품의 일부로 둔다

**약관·DPA·프라이버시 정책이 각각 다른 역할**을 하고, **프라이버시 정책에 '익명 통계를
만든다'고 적어 두는 것이 테넌트를 가로지르는 이용을 허락하지 않는다.** VP of Technology
가 쓴 글이 **법무 문서를 '프로덕트 그 자체의 일부'** 라 부른다 — **이 원장에서 처음
보는 자리**다.

### 법인 정보를 거의 못 읽었다

회사 사이트가 **JS 로만 그려지고** 제품 도메인 루트는 **404** 다. **설립 연도·대표·주소·
직원 수·도입 병원 수·매출**이 전부 비어 있다 — 법인명 **株式会社ヘンリー** 만 확인했다.

### 다음 사이클

확장 — 빈 도메인 둘 중 하나. **규제 쪽이 인용이 촘촘하고**(위탁/제3자 제공 · 이용 범주 셋 ·
문서별 역할), 제품 쪽은 **왜 둘 다 가지는지**가 분명하다. 큐가 1/3 이라 그 뒤에 조사가
다시 뜬다.
