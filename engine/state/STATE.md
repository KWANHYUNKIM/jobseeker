...
**Redpanda**(US · 스트리밍 엔진) — 회사 프로파일까지 썼다. **도메인 둘 · 기능 0.** 큐 0/3.

### 모든 스트림이 기록 시스템은 아니다

이 회사를 세우는 전제다. 스트리밍 시스템은 보통 **하나의 내구성 보장**에 전부를 맞추는데,
여기는 **잃어도 되는 스트림과 한 바이트도 잃으면 안 되는 스트림이 같은 클러스터에 살 수
있다**고 본다. 그러면 각각 다른 값을 치르면 된다 — 쓰기 캐싱은 EBS 에서 **P99 를 113ms 에서
8.5ms 로** 줄이는 대신 **상관 장애 때 소량 손실**을 감수한다.

### 언제 쓰지 말아야 하는지를 못 박는다

**"기록 시스템 스트림, 즉 인프라 장애 앞에서도 모든 바이트를 지키는 것이 최우선인 곳에는
맞지 않는다."** Supabase 가 "Realtime 은 이 일을 위해 만들어지지 않았다"고 적은 것과 같은
태도다 — **하나의 답을 강요하지 않고 경계를 그어 준다.**

### 같은 위험이라도 기본값이 다르면 기대가 다르다

회사는 **카프카의 `acks=all` 도 비슷한 위험을 안고 있다**며 이 대가가 새로운 종류가
아니라고 적는다. 맞는 말인데, **기본값과 기대는 다른 문제**라는 것을 글이 다루지 않아
`limits` 에 재구성으로 남겼다.

### 두 도메인이 같은 말을 한다

보장을 토픽마다 고르는 것(쓰기 캐싱)과 데이터를 어디 둘지 토픽마다 고르는 것(Cloud Topics).
**둘 다 '같은 클러스터 안에서 토픽마다 다르게'** 다.

### 심은 대비

**워프스트림**(S3 위에 카프카 프로토콜을 **새로 얹어** 시작했다) ↔ **Redpanda**(C++ 엔진을
이미 가진 쪽이 오브젝트 스토리지를 **나중에 들였다**) — 같은 목적지에 출발점이 반대다.

### 다음 사이클

빈 도메인 둘 중 하나. **쓰기 캐싱 쪽이 수치가 두껍다**(113ms→8.5ms · CPU 10% · 토픽별 설정 ·
쓰지 말아야 할 곳). Cloud Topics 쪽은 대가는 적지만 **물린 대안을 이름 대어 적지 않는다.**
아직 안 연 글 둘(Seastar 메모리 프로파일링 · WASM 변환)이 세 번째 도메인이 될 수 있다.
