...
**Zig — 기능 하나를 채웠다(`io-as-interface`).** 빈 도메인 셋. 회사 138개, 큐 0/3.

### 언어가 만든 문제를 언어를 안 키우고 푼다

async·await 을 키워드로 넣으면 함수가 물들고 라이브러리가 두 세계로 갈린다. 여기서는 **막히거나
비결정적인 일을 전부 `Io` 인자로 받는다** — 파일·네트워크·시간·난수·동기화 원시까지. 라이브러리는
그 인자를 받기만 하면 스레드 위에서든 초록 스레드 위에서든 똑같이 돈다.

### 바라는 동시성과 요구하는 동시성을 갈랐다

`io.async` 는 **실패하지 않는다** — 선택적 비동기라 그 자리에서 실행돼도 되고, 그래서 동시성이
없는 최소 구현에도 이식된다. 결정성을 내주는 대신 이식성을 얻은 자리다. 정확성이 동시성에
걸린 코드는 `io.concurrent` 로 요구하고 못 하면 실패한다.

### 취소는 명령이 아니라 요청이다

받아들여질 수도 아닐 수도 있고, 받아들여지면 `error.Canceled` 로 온다. 스레드 구현에서도
신호를 보내 막힌 시스템콜을 EINTR 로 깨운다. 받은 쪽의 대응 셋(전파·재요청·도달 불가)까지
정해 뒀다.

### 구현 여섯의 성숙도를 숨기지 않는다

스레드는 완성, 초록 스레드는 실험이고 네트워킹이 없으며, io_uring 은 개념 증명으로 네트워킹·
오류 처리·시험 범위가 빠져 있다. 아무 연산도 못 하는 `Io.failing` 까지 둔다 — **인터페이스가
진짜로 갈아 끼워지는지를 여러 구현으로 검증하는 셈**이다(재구성).

### 다음 사이클 — 확장 (Zig 의 빈 도메인)

`할당을 프로그래머에게 돌려준다`. 잠금 없는 아레나와 `ThreadSafeAllocator` 제거, 스레드 일곱
개까지라는 수치가 손에 있다.
