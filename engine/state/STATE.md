...
**Ladybird — 도메인 셋을 다 채우고 done 으로 닫았다.** `from-scratch-wpt` ·
`swift-to-rust-with-ai` · `off-main-thread-processes`. 회사 128개, 큐 1/3.

완주 기준 셋을 다 넘겼다 — 도메인마다 기능 1개 이상, 기능 셋이 이어져 고아가 없고, 수익원(기부와
후원)이 기능과 닿는다.

### 같은 경계가 두 값을 낸다

컴포지터를 별도 프로세스로 빼고 GPU 접근을 거기로 몰았다. **성능 결정이 그대로 보안 결정이 되는
자리다** — 주 스레드가 비어 스크롤과 애니메이션이 살아나고, 동시에 페이지 프로세스가 뚫려도 GPU 로
곧장 가지 못한다. 새로 짓는 엔진이라 아직 모르는 결함을 가두는 일이 함께 필요했다는 점에서 두
이유가 겹친다(재구성).

나누는 값도 함께 치른다 — 백킹 스토어 관리가 경계를 넘고, 죽는 자리가 늘어 **컴포지터가 죽어도
페이지가 살아남는 복구**를 넣어야 했다.

### 아직 안 한 일을 순서로 적는다

사이트 격리는 아직이다. 대신 **세션 이력과 이동을 브라우저 프로세스로 옮기는 전제 작업**을 먼저
했다 — 페이지 프로세스가 언제든 갈릴 수 있으려면 그 상태가 밖에 있어야 하기 때문이다. 못 한 일을
숨기지 않고 그 앞 단계를 보이는 방식이다.

### 이 회사가 남긴 세 가지 대비

1. **Bun 과** — 같은 해 같은 러스트 이전, 정반대 방식(한 번에 vs 서브시스템 단위).
2. **DuckDB·Bun 과** — 수익원이 없는 세 번째 자리인데 이유가 다 다르다(재단+회비 / 인수 / 수익화를
   안 하는 것이 제품의 근거).
3. **Zed 와** — 샌드박스를 커널에 맡기는 계보가 같다(Landlock·seccomp·Seatbelt).

### 다음 사이클 — 신규 (Inngest)

큐가 1/3 이고 대기 맨 위가 Inngest 다. 지속 실행 축이라 Temporal 과 정면으로 붙고, FoundationDB
이전은 Tigris 와, 공정 큐는 Chroma 에서 보류한 자리와 겹친다.
