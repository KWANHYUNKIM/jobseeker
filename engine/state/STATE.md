...
**없다 — Paystack 완주.** 도메인 2개 · **기능 2개.** 회사 **88개.** 큐의 `진행 중` 이 비었고 `대기` 에 **Deliveroo** 하나(1/3)가 있다.

### 이번 사이클 — `채널마다 다른 시계가 돌고, 그 시계를 우리가 정하지 못한다`

⚠️ **`아프리카에서 사업체가 돈을 받게 한다` 의 tech 가 0 이었고, 블로그의 시스템 글이 둘뿐이라 자료가 없을 것 같았다. 개발자 문서에서 나왔다 — Fastly·Plaid 에 이어 세 번째다.**

건진 것 넷:

- ⚠️ **결제 수단이 나라의 금융 인프라를 그대로 반영한다** — 아홉 갈래이고 **카드만 전 시장이며 나머지는 지원되는 나라에서만** 된다. 나이지리아는 **USSD `*737#`**, 가나·케냐·CIV 는 **모바일 머니**, 남아공은 **QR·EFT·Capitec 앱 푸시**, 케냐는 **Pesalink**. **시장을 늘린다는 것은 곧 채널을 늘린다는 뜻이다**(재구성).
- ⚠️ **이 기능의 이름이 나온 자리 — 시계를 남이 정한다.** 임시 계좌는 **15분~8시간**(자기 규칙), Pesalink 는 **25분**, **모바일 머니는 180초인데 문서가 `통신사가 설정한 한계` 라고 적는다.** **자기 API 의 동작이 남의 결정에 묶여 있고 그것을 감추지 않는다.**
- ⚠️ **은행 하나가 하나의 API 의 모양을 바꾼다** — **Kuda 는 계좌번호 대신 전화번호와 토큰**, **DTB 계좌는 Pesalink 를 아예 못 쓴다.** **Plaid 가 Chase·Capital One 예외를 문서에 적은 것과 같은 자리이고 같은 답에 도달했다** — 감추면 런타임에 놀라고 다 노출하면 하나의 API 가 아니다.
- ⚠️ **금액이 틀리면 성공시키지 않는다** — M-PESA 오프라인에서 **틀린 금액은 실패시키고 처음부터** 인데 문서가 그것을 **사업체의 이점**이라 적는다. **금액 불일치가 장부에 안 남는 대가로 고객이 한 번 더 한다.**

### ⚠️ 이번에 확인한 규칙 — 자료가 얇으면 기능보다 자료를 먼저 찾는다(일곱 번째)

**`--gaps` 가 부른 도메인의 tech 가 0 이면 기능부터 쓰려 하지 않고 자료를 먼저 찾는다.** 일곱 번 연속 통했다(Fastly 보안=제품 문서 · Monzo 플랫폼 · ClickHouse 엔진 3부작 · 1Password 프로비저닝 · DoorDash 배차=Iguazu · Etsy LLM · **Paystack 결제 채널=개발자 문서**). ⚠️ **그리고 제품·개발자 문서가 1급 출처가 된 사례가 이제 셋이다**(Fastly 레이트 리밋 · Plaid OAuth · Paystack 결제 채널) — **"엔지니어링 블로그가 없으면 자료가 없다" 는 틀렸다.**

### 다음 사이클 — 사다리 위에서부터

`--gaps` 가 정한다. **큐가 1/3 이라 3순위(후보 조사)가 걸릴 수 있다.** ⏳ 되짚을 목록이 남아 있다 — `www.meesho.io/blog` · `blog.zepto.com` · `bytes.swiggy.com` · `www.gojek.io/blog`(429) · `tech.cygames.co.jp` · `www.scylladb.com/blog`. ⏸️ Snyk 보류 — `New Agentic Architecture for Snyk Agent Fix` 를 열고 결론 낸다.

### ⚠️ 비교 문서 재료 두 축이 다 찼다 (초안 유지)

**① `AI 에이전트를 어디까지 믿나` 5곳** — Sentry(조직의 신뢰) / ClickHouse(접근 범위, **세 겹 중 한 겹만 기계가 강제**) / Duolingo(사람 리뷰 → **병목**) / Ramp(**사람의 신원** — 앱 토큰 대신 사용자 GitHub 토큰) / DoorDash(**primitives, not workflows**).

**② `관리형 MySQL 의 한계` 3곳** — Etsy(Vitess 샤딩 추상화) / Plaid(TiDB 분산 SQL) / **Paystack(MySQL 8.0 제자리 업그레이드)**. ⚠️ **Plaid 와 Paystack 은 같은 마감(2024-10 아마존의 Aurora MySQL v2 지원 종료 발표)에 정반대 답을 냈다** — 하나는 떠났고 하나는 남았다.

⚠️ **그런데 비교 문서(8순위)는 이 사다리에서 영영 안 온다** — 3순위(큐가 0이면 후보 조사)가 큐를 다시 채우므로 **"위가 전부 비었다" 가 성립하지 않는다.** **이것은 PROMPT.md 의 문제이고 사이클이 고칠 것이 아니다**(코드·문서 변경은 사이클에 섞지 않는다).
