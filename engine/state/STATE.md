...
**후보 조사를 돌아 큐를 3/3 으로 채웠다.** 파는 회사는 없다 — Zed 를 닫고 대기가 비어 있었다.
회사 117개, 큐 3/3.

### 이번에 올린 셋

- **DuckDB**(EU) — CWI 에서 나온 임베디드 분석 데이터베이스. 비동기 I/O 글 하나에 스레드 풀을
  둘로 가른 이유와 미리 읽기의 대가, 그리고 S3 파케이 22GB 8.230초에서 2.844초 같은 수치가 다 있다.
- **Antithesis**(US) — 결정론적 시뮬레이션 테스트가 제품인 회사. 라프트 구현 하나에서 한 시간에
  버그 셋을 갈라낸 기록이 있고, 형식 검증은 모델의 정확성이지 구현의 충실성이 아니라는 한계도 적는다.
- **Dolt**(US) — 깃처럼 브랜치와 머지가 되는 SQL 데이터베이스. 프롤리 트리의 청크 경계를 값이 아니라
  키로만 정한 이유가 있고, MySQL 보다 약 2배 느리다는 것도 그대로 적는다.

### 각도가 바뀌었다 — 이름의 크기가 아니라 채널의 주인

지난 다섯 조사를 버틴 `큰 이름인데 아직 없는 회사` 각도가 말랐다고 적어 뒀는데, 이번에 그것을
확인했다. 접은 셋(Riot Games · Redpanda · Cockroach Labs)이 전부 큰 이름이거나 유명한 제품인데
**채널이 마케팅으로 넘어갔거나 회사 뉴스로 흡수됐다.** 리엇은 기술 블로그가 뉴스 페이지로 합쳐졌고
최신 기술 글이 2025-10-16 이다.

올린 셋은 반대다. 이름값은 작아도 **블로그가 회사 소유 도메인에 있고 엔지니어가 직접 쓴다.** 세
회사 모두 최신 글이 2026년 8~9월이다. 이것을 다음 조사의 첫 기준으로 삼는다.

### 다음 사이클 — 신규 (DuckDB)

대기 맨 위가 DuckDB 다. 3단계(회사 프로파일)부터 판다.
