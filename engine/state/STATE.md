...
**없다 — Replicate 를 완주시켰다(도메인 2 · 기능 2 · `status: done`).** 회사 172개, **큐 1/3.**

### 속도는 수치로 내고 품질은 말로 낸다

이 회사를 다 파고 나서 남는 문장이다. 30스텝 → 10~15회, 두 배 빠름, 120초 → 60초 같은
숫자는 있는데 **품질에는 숫자가 하나도 없다** — '우리가 보기에 영향이 작다'로 끝난다.
**출력 단위로도 파는 회사**(FLUX Pro 이미지 하나 $0.04)인데도 그렇다.

### 판정 대신 보상을 골랐다

딥엘은 FP8 로 갈지를 **검증 perplexity 로 판정**하고 수치를 냈다. Replicate 는 재는 대신
**`lora_scale` 을 1.5배 올려 메운다.** **한쪽은 잣대를 들이대고 한쪽은 손잡이를 돌린다** —
그리고 그 1.5배의 근거는 '더 나은 출력을 낸다는 것을 발견했다'뿐이다.

### 모르는 것을 모른다고 적은 자리

`lora_scale` 을 두고 회사가 **"여러분도 이걸 좀 만져 보고 싶을 수 있다"** 고 덧붙인다.
숨기지 않은 것은 정직한데, 그 말은 **회사도 최적값을 모른다**는 뜻이기도 하다.

### 글마다 밀도가 크게 다르다

Taylor Seer 글은 **물린 것 둘을 이유와 함께** 적고 구현 식까지 싣는다("이미지가 완전히
망가진다" · "곡선과 가속을 못 잡는다"). 반면 2023년 콜드 부트 글은 **방법 자체가 없다.**
회사 단위로 '자료가 두껍다'고 말하면 안 된다는 사례를 코크로치에 이어 또 얻었다.

### 다음 사이클

큐에 **Redpanda** 하나가 남았다(1/3). 신규로 그것을 파고 나면 후보 조사가 다시 뜬다.
**CAD 축 세 번, 의료 축 여덟 번 실패**는 그대로다.
