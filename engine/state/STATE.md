...
**PostHog — 기능 하나를 채웠다(`overflow-instead-of-dropping`).** 빈 도메인 둘. 회사 140개, 큐 1/3.

### 거절하지 않고 옆길로 보낸다

손님 쪽 버그 하나로 한 사용자 식별자에서 이벤트가 몰아칠 수 있다. 흔한 답은 속도 제한을 걸고
거절하는 것인데, 그러면 손님 라이브러리가 오류를 받고 **그 데이터는 영영 없다.**

이 회사는 한 (프로젝트, distinct_id) 흐름이 분당 약 5천 건을 지속해서 넘기면 **넘침 파이프라인
으로 우회**시킨다 — 버리는 것 없이 캡처 API 는 여전히 200 을 돌려준다.

### 버리지 않으려면 하나는 내줘야 한다

내주는 것이 **순서**다. 우회하는 동안 그 식별자의 처리 순서 보장이 느슨해진다. 지연이든 순서든
하나는 포기해야 하는 자리에서 무엇을 골랐는지가 분명하다.

### 성질이 다르면 줄도 다르다

토픽을 셋으로 갈랐다(보통·넘침·과거 데이터). 클러스터를 워크로드마다 가른 것과 같은 손버릇이다 —
**한 종류의 부하가 다른 종류를 늦추면 안 된다.**

### 편한 구조에 조용한 함정

카프카 테이블로 적재하면 편한데, 핸드북이 경고한다 — **평범한 SELECT 하나가 오프셋을 진행시켜
데이터를 지운다.** 구조로 못 막으니 문서로 막는 셈이다.

### 다음 사이클 — 확장 (PostHog 의 빈 도메인)

`워크로드마다 클러스터를 가른다`. 단일 클러스터 시절의 자원 다툼, 계층 저장, 분산 테이블,
갱신·삭제가 비싸서 새 행으로 모형화하는 스키마가 손에 있다.
