...
**없다 — 문체 보수 사이클이었다.** 크라켄 테크놀로지스를 배민 문체로 다시 썼다(1.44 → 0.0). 회사 115개, 큐 2/3. 16곳 남았다.

### 쪼개지 않기로 한 둘 — 그 값을 다르게 갚는다

- 메르카도리브레는 재고 도메인이 극히 낮은 지연과 강한 일관성을 요구해 의도적 모놀리스를 핵심에
  두고 **그 주위를 셀로 감쌌다.**
- 크라켄은 개발자 500명 이상이 같은 코드베이스를 만지는 채로 **하루 100번 넘게 내보내는 쪽**으로
  갚는다.

그리고 되돌리기를 기본으로 두지 않는다는 점이 드물다. 앞으로 고치는 쪽을 택하고 문제가 생긴
버전은 환경을 고정해 더 퍼지지 않게 막는다. **그러니 DB 마이그레이션이 앞뒤 모두 호환이어야
한다는 규칙이 따라 나온다** — 트레인라인이 되돌릴 길을 먼저 깐 것과 정반대다.

### 사람이 못 보는 것을 도구가 대신 보게 하는 일 중 가장 긴 것

Monzo 는 데이터 표준을 CI 에 심었고 Booking.com 은 검사를 더 앞으로 당겼는데, 크라켄은
**2.5년에 걸쳐 1500만 줄에 타입을 뒤늦게 입혔다.** 만지는 함수부터 요구를 넓히고 빠진 주석 수를
대시보드로 세어 스스로를 붙들어 맸다. 쉬운 길(Any 를 뿌리면 하루)을 안 갔고, 결과가 절제를
보여 준다 — 500만 줄에 무시 주석이 5천 개뿐이다.

### 멈출 수 없다는 것이 여기서는 계약이다

Honeycomb(무중단 포기) · Etsy(데이터는 두고 계층만) · 트레인라인(되돌릴 길을 먼저)과 달리,
크라켄에서 무중단은 기술 판단이 아니라 **SLA 다.** 계약이 구조를 정한다.

그리고 고객사가 느는 것이 곧 배포가 느려지는 것이었다 — 옛 구조에서는 머지된 PR 이 차례로
배포되며 서로를 기다렸는데, 지금은 환경마다 자기 배포기가 자기 구역을 봐서 **전부 독립적으로
병렬로 나간다.**

### 도메인을 제품이 아니라 쪼개지 않기로 한 결정이 만든 값으로 잘랐다

하루 100번 내보내는 일도, 1500만 줄에 타입을 입히는 일도, 환경 25곳을 각자 돌게 하는 일도
**전부 그 하나의 결정에서 나온 청구서다.**

### 다음 사이클 — 문체 보수 (사다리 3순위)

--gaps 가 정한다. 지금 맨 위는 meituan(1.31, 462개)이지만 그건 이 시점의 예상이지 확정이 아니다.
