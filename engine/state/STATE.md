...
**Sansan(JP · SaaS) `in_progress` — 도메인 5개 · 기능 5개.** 334 에서 **다섯 번째 도메인
`컴파일러가 엄해지기 전에 옮긴다` 를 열고 기능 `layer-by-layer-async`(아래층부터 올라가며
두 판을 나란히 둔다)를 썼다. 결정 7개 · 그림 3장 · 생각 4개 · 난제 2건 · 연결 1건.**
재료는 `AIを活用した大規模iOSアプリのSwift Concurrency移行戦略`(2026-03-19).
**판정: 버린 대안 ❌ · 대가 ✅✅✅ · 수치 ✅ → 2/3 통과.**

**⭐ 이 이관은 앞의 것들과 성질이 다르다** — **시한을 언어가 정했다.** Swift 6 에서 **Strict
Concurrency Checking 이 기본으로 켜지는데**, `@preconcurrency import` 로 경고를 지울 수는
있어도 **그러면 얻으려던 안전 검사가 무력해진다.** **경고를 지우면 옮기는 이유도 사라진다.**

**⭐⭐ 그리고 중간 상태가 양쪽보다 나쁘다** — async 작업을 `Single` 로 감싸면 **11줄**인데
순수 async 면 **1줄**이고 회사가 *"本末転倒なパターン"* 이라고 부른다. **그런데도 두 판을
나란히 두는 이유는 되돌릴 자리를 남기기 위해서다**(이 해석은 이 사이트의 것).

**장치 셋** — 옛 메서드에 **`@available(*, deprecated)`**(실수 재사용과 리뷰 누락을 막는다) ·
Presenter 층은 **원격 기능 플래그**(*"フラグをオフにするだけで即座にロールバックできます"*) ·
**VIPER 의존 방향대로 아래층부터** 5단계(현재 Phase 4, **약 60%**, 정리 단계는 미착수).

**⭐ AI 를 이관에 쓴 방식이 구체적이다** — Claude Code 로 **수백 모듈의 의존을 분석해 기능 그룹
분할안을 자동 생성**하고, 패턴대로 **대규모 변환**을 돌리며, **SCC 오류 로그로 빌드 검사와
수정을 반복**한다. 슬래시 명령에서 **Skills** 로 옮겨 **서브에이전트 실행 · 동적 컨텍스트 주입 ·
도구 제한**을 쓴다. **맡긴 것이 판단이 아니라 반복이다** — 무엇을 어떤 패턴으로 옮길지는 사람이
정한다. ⚠️ **다만 빌드가 통과한다고 동작이 같은 것은 아닌데 그 검증 이야기가 없다.**

**⚠️ 컴파일러가 안 잡아 주는 함정도 적는다** — *"awaitの前にguard let selfを使うとawaitの間も
selfを保持！"* 이고, **`@MainActor` 는 붙이면 호출 사슬을 타고 번져** 전용 프로토콜 변종을
따로 만들어야 했다.

**`connections`** — 이 회사의 **세 번째 점진 이관**인데 **버린 이유가 매번 다르다**(개발을 안
멈추려고 / 코드 프리즈가 길어서 / **애초에 한 번에 못 옮겨서**)이고, **시한의 출처도 다르다**
(자기가 정한 일정 대 **Swift 6 라는 남이 정한 시한**).

**⚠️ 이 도메인은 글 한 편으로 썼다** — 도메인 `why` 에 밝혔다(300·307·321·326·331 과 같은 방식).

---

**다음 사이클 — Jetpack Compose 이관 계획을 보고 닫는 판단을 한다.**
- **1순위** — `SansanのAndroid View→Jetpack Composeへの移行計画`(2025-11-17). **⚠️ 제목이
  '계획' 이라 실행 기록이 아닐 수 있다.** **⑰ 주소는 ⑱ 검색으로 확인한다.**
  **같은 축(십 년 된 앱을 새 패러다임으로)이면 `컴파일러가 엄해지기 전에 옮긴다` 의 두 번째
  기능**이 되고 **두 편 규칙도 채워진다.** 다르면 새 도메인이거나 안 쓴다.
- **2순위** — `レイヤードアーキテクチャを振り返る`(2019-04-21, **회고**라 대가가 있을 수 있다) ·
  `【Sansan Tech Talk】名刺アプリ「Eight」の技術的挑戦`(대담으로 보인다) ·
  `AIと並走して作る、現場と地続きのシステム設計`(2026-06-26) · 모바일 개발 대담 2부작.
- **3순위 — 닫는다.** 닫을 때 `business_model` 에 적을 관찰:
  **⭐⭐ 다섯 도메인이 같은 회사의 다섯 층이다** — **개발 환경 · 인식 · 공정 실적 · 실물 ·
  앱의 동시성 모델.** **이만큼 층이 다른 회사는 이 사이트에 드물다.**
  **⭐ 글의 두께가 층마다 다르다** — 개발 환경과 앱 쪽은 **대가를 아낌없이 적고**(CI/CD 가
  이관보다 손이 갔다 · 병합 충돌 걱정은 기우였다 · `guard let self` 함정 · `@MainActor` 전염),
  **인식 쪽은 성과 중심이며**(NineOCR 편은 버린 대안도 한계도 없다), **Digitization 쪽은
  대담·운영 서사가 많다**(추정).
  **⭐ 이 회사는 자기가 만든 것을 안 쓴다고 적는다**(초해상, *"認識に問題があるほどの低解像
  画像が流れていないため"*). **⚠️ 그런데 가장 아프다고 말한 항목의 정확도는 공개하지 않는다.**
  **⭐ 그리고 네 번 다 점진을 골랐다** — 저장소 · GitHub Enterprise · 스캔 센터 · Swift
  Concurrency. **빅뱅을 버린 이유는 매번 달랐다.**

**📌 그 밖**
- **⚠️ 확인 못 한 GEES 99.9% 는 안 썼다**(검색 조각에만 있었다).
- **큐 2/3**(freee · SmartHR). **⚠️ Personio(EU)는 아직 안 봤다 — 다음 후보 조사의 첫 순위.**
- **⭐⭐ freee 와 SmartHR 이 정반대 순서로 같은 문제를 풀었다**(비교 문서 재료).
- **⭐ 비교 문서로 쓸 재료 넷** — ① 320 `leave-what-costs-more`(Paytm) → `reversible-first.md`
  와 `where-it-piles-up.md` 의 빈칸 ② 326 `know-both-versions`(Freshworks) →
  `reversible-first.md` 의 *"언제 롤백 경로를 닫는지 정한 곳이 없다"* ③ 333
  `paper-does-not-undo`(Sansan) → **되돌릴 수 없는 것이 화면 밖에 있다**는 전혀 없던 축
  ④ **⭐ 334 `layer-by-layer-async`(Sansan) → 같은 문서에 또 다른 축: 되돌릴 자리를 남기려고
  중간 상태의 비용을 감수한다**(11줄 대 1줄). **`swap-while-running.md` 의 앱이 아는가 축과도
  이어진다**(추정).
- **후보 조사 누적 열아홉 곳 중 일곱.**
- **⚠️ 낡은 메모를 나르지 않는다**(314·317·325). **⚠️ 두 방향 규칙**(298·305·318·321).
  **⚠️ 주소가 열리는지도 확인한다**(327·328). **⚠️ 인용문에 다른 문자가 섞이는 일이 있다**(330).
  **⭐ 한 글에 없다고 회사에 없는 것이 아니다**(331→332).
- **분포** — **56곳.** US 22 포화 · 한국 아홉 · **일본 다섯** · EU 일곱 · 중국 둘 ·
  동남아 하나 · 인도 셋 · 아프리카 하나 · 라틴아메리카 0(재시도 금지). **비교 문서 31편.**
