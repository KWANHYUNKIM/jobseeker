...
**Restate**(베를린 · 지속 실행 엔진) — 회사 프로파일까지 썼다. **도메인 둘 · 기능 0.** 큐 1/3.

### 자기가 만든 것에 안 맞는 용도를 보고 회사를 차렸다

기원을 소개 페이지가 직접 적는다 — **'플링크를 만들고 그 사용자들과 일하면서 우리는
비분석 용도를 자주 보았다. 흥미로웠지만 플링크에 가장 잘 맞지는 않았다.'** 아파치 플링크의
공동 창시자들이 **플링크가 못 맡는 자리**를 위해 다른 엔진을 짓는다.

### 두 도메인이 층으로 쌓인다

위층은 **모든 것을 하나의 로그로 모아 조율을 없앤다**(메시지 큐도 DB 도 락 서비스도 이미
로그다 · **'선형 역사를 근거로 강제하는 단 한 곳이 있다는 것은 조율을 많이 할 필요가
없다는 뜻'**). 아래층은 **그 로그를 자기 손으로 짓는다**(포스트그레스도 카프카도 아니다).
**위에서 없앤 복잡함을 받치려고 아래를 떠안았다.**

### 문제를 옮기는 것과 없애는 것을 가른다

주키퍼 락을 더하는 식을 물리며 **'코드가 동시성을 걱정할 필요를 줄였을 뿐, 잃어버린 락과
남은 락의 정리라는 문제를 새로 들여왔다'** 고 적는다. **그 잣대를 자기에게 대면 로그의
운영이라는 문제를 새로 들인 것**인데, 그 대가는 엔진 글에서 삼각형으로 인정한다 —
**'최선의 단 하나는 없다. 트레이드오프의 스펙트럼만 있다.'**

### 통념을 이름 대어 물린다

**'새 상태 저장 시스템을 만들지 마라, 그냥 포스트그레스를 써라'** 를 인용하고 어긋난다고
적는다. 근거는 **큐 비유** — SQL 테이블로도 큐를 만들 수 있는데 메시지 큐가 따로 있는 것과
같은 이유다. **워프스트림을 이름 대어** 객체 저장소만으로는 지연이 안 된다고도 적는다.

### 소재지는 Imprint 로만 확정된다

소개 페이지는 '다국적 팀'이라고만 한다. 독일 법이 요구하는 **Impressum** 에서
**Restate GmbH · 베를린**이 나왔다. **국가 칸은 이 원장 표기대로 EU 로 적었다** —
검증이 DE 를 받지 않는다(코드는 건드리지 않았다).

### 다음 사이클

확장 — 빈 도메인 둘 중 하나. **엔진 쪽이 근거가 두껍다**(물린 대안 둘 · 삼각형 인정 ·
워프스트림을 이름 대어). 로그 쪽은 스스로 그은 한계선이 좋다(**'모든 연산에 단일 로그는
말이 되지 않는다 — 거대한 폭발 반경 하나를 만들 것이다'**).
