...
**후보 조사를 돌아 큐를 3/3 으로 채웠다.** 파는 회사는 없다 — Chroma 의 남은 도메인을 보류해
확장 대상이 비었다. 회사 126개, 큐 3/3.

### 이번에 올린 셋 — 또 축을 벌렸다

- **Cerebras**(US) — 하드웨어 축이 이 엔진에 아예 없었다. 웨이퍼 한 장을 칩 하나로 쓰고, 문제를
  GPU 메모리 벽으로 정의한 뒤 **칩을 더 붙이는 답을 물린다**(더할수록 그 단계가 비싸지면서 연산은
  줄어든다). SRAM 44GB 가 코어 90만 개 옆에, 총대역폭 21 PB/s, 초당 750토큰.
- **Ladybird**(US) — 브라우저 엔진 축. 2026년 7월에 스타일 시스템과 레이아웃을 러스트로 옮겼고,
  8월 한 달 최적화로 StyleBench 가 약 3.5에서 83이 됐다. 컴포지터를 별도 프로세스로 뺐다.
- **Inngest**(US) — 지속 실행. Temporal 과 정면 대비이고, Valkey 가 천장이 된 이유를 셋으로 적는다
  (메모리가 바닥나면 클러스터 전체가 죽는다 · 단일 스레드라 공정 분배가 안 된다 · 영속성이 복제
  구성에 달렸다).

### 축이 서로 이어진다

Inngest 가 FoundationDB 로 옮긴 이야기는 **Tigris**(FDB 위의 객체 저장소)와 같은 바닥을 쓰고,
공정 큐 이야기는 **Chroma 에서 자료가 없어 보류한 그 자리**(테넌트 공정성)와 정확히 겹친다. 한
회사에서 못 판 것을 다른 회사에서 팔 수 있게 된 셈이다(재구성).

### 회사가 아닌 조직을 처음 올렸다

Ladybird 는 비영리다. DuckDB 의 지식재산이 비영리 재단에 있는 것을 이미 판 적이 있으므로,
판정 기준(공개 자료가 의사결정과 대가를 말하는가)만 지키면 된다고 봤다.

### 다음 사이클 — 신규 (Cerebras)

대기 맨 위가 Cerebras 다. 하드웨어는 이 엔진에 처음이라 도메인을 어떻게 가를지가 이번 프로파일의
과제다.
