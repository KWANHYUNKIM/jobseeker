...
**Cygames** (JP · 게임) — 도메인 3개 · **기능 1개**(`move-the-verdict-keep-the-feel`). 남은 둘은 **트래픽 급증**(✅ 자료 읽음)과 **LLM 도구**(⏳ 미독)다.

### 이번 사이클 — `손맛을 지키면서 판정을 서버로 옮긴다`

건진 것 넷:

- ⚠️ **짧게 보면 클라이언트가 낫고 길게 보면 서버가 낫다** — 클라이언트에 두면 통신 대기가 없어 손맛이 살지만 **몇 년을 굴릴 게임에서는 유지 비용이 쌓이고 기능 확장이 어려워진다.** **한 번 내고 끝나는 제품이었다면 다른 답이 나왔을 결정이다**(재구성).
- ⚠️ **대가를 되돌리지 않고 다른 층에서 갚았다** — **`통신 대기의 발생으로 인한 UX 저하가 있었습니다`** 를 그대로 적고, **흐름과 연출을 손봐 전작보다 나은 감촉**에 도달했다. **대기 자체는 없앨 수 없으니 기다리는 동안 무엇을 보여 줄지를 바꿨다**(재구성).
- ⚠️ **가장 싼 방법이 게임의 규칙 때문에 원천적으로 막혔다** — 관전 화면을 영상으로 보내려 했지만 **`대전 중 플레이어의 화면에서는 서로 상대의 손패가 가려져 있고`** 방송에는 **양쪽이 다 보여야** 한다. **그러니 어느 플레이어의 화면도 방송 화면이 될 수 없다.** 클라이언트가 새로 그리기로 했다.
- ⚠️ **먼저 재고 나서 줄였다** — 기존 파크 지표에서 **필요한 감축량을 계산해 놓고** 무엇을 정적으로 만들지 정했다(**3D 배경과 Spine 데이터**). **줄일 곳을 고른 기준이 `게임의 정체성에서 먼 것` 이었던 셈이다**(재구성) — **카드를 다루는 감촉은 건드리지 않았다.**

### ⚠️ 이번 사이클에 조심한 것

**낙관적 UI(결과를 먼저 보여 주고 나중에 맞추기)를 왜 안 골랐는지는 글에 없다.** 그럴듯한 이유가 떠올랐지만(**서버 판정이 뒤집히면 되돌리는 장면이 카드 게임에서 치명적이다**) **글에 근거가 없어 `이 사이트의 재구성이고 글에 근거가 없다` 를 명시해 달았다.** ⚠️ **추정을 적되 추정이라고 두 번 적는 편이 낫다.**

### 다음 사이클 — 확장(2순위)

`--gaps` 가 **`출시일의 트래픽 급증을 무중단으로 넘긴다`** 를 부를 것이다. **자료를 이미 읽었으니 바로 쓴다**(출시일 50만 QPS · 피크 65만 · 40만 QPS 중 롤링 업데이트 · **통계 이력 기능을 껐다**). ⚠️ **다만 그 글은 컨퍼런스 후속 요약이라 얕다** — 결정을 다섯 개 이상 뽑을 수 있는지 먼저 본다.
...
**② ⭐ `관리형 MySQL 의 한계` 3곳 + Cygames(TiDB)** — **Plaid 와 Cygames 가 같은 TiDB 를 다른 이유로 골랐다.**
...
⏳ **⑧ 새로 보인다 — `제약을 없애지 못할 때 어디서 갚는가`** — **Cygames**(통신 대기를 연출로 갚는다) / **Zepto**(재고 지연을 마지막 권위 확인으로 갚는다) / **Grafana**(색인 없는 대가를 새 포맷으로 갚는다).
