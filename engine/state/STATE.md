...
**Backblaze 완주(기능 3개).** 회사 134개, 큐 0/3 — 다음 사이클은 후보 조사다.

### 붙잡는 값을 반대로 쓴다

큰 회사들은 GB 당 0.09달러부터 받아 나가지 못하게 한다. 이 회사는 그 자리를 뒤집어
**저장량의 세 배까지 공짜**로 내보내고 넘는 만큼만 GB 당 0.01달러를 받는다 — 나가는 값이
없다는 것이 들어올 이유가 된다.

### 공짜로 준 값은 사라지지 않고 옮겨 간다

2023년 10월에 무료 내려받기를 내면서 **저장 값을 TB 당 5달러에서 6달러로 올렸다.** 쌓아만
두고 잘 안 꺼내는 손님이 안 쓰는 몫까지 무는 구조다. 나머지는 파트너 여덟 곳의 망으로
옮기고(대역폭 동맹 창립 멤버), 그래도 많이 꺼내는 쪽에는 무제한 등급을 따로 판다.

### 공개의 기준이 회사 안에서 갈린다

고장률은 13년째 원자료까지 내놓으면서 **왜 하필 세 배인지, 자기 대역폭 원가가 얼마인지는
밝히지 않는다.** 공개가 주장을 도울 때는 열고 값 구조를 드러낼 때는 닫는다 — 같은 회사의
두 태도를 나란히 적어 뒀다.

### 세 도메인이 한 전제에서 나왔다

"디스크는 죽는다"가 덮는 일(17+3)·세는 일(드라이브 통계)·파는 일(내려받기 값)을 낳았다.
값싼 하드웨어 위에서 내구성을 만들어 아낀 것이 그대로 값의 여유가 된다.

### 다음 사이클 — 후보 조사 (큐 0/3)

Rerun 하나만 남았다. 기준은 그대로 — 회사 소유 도메인 · 엔지니어가 직접 쓴 글 · 자기 시스템의
의사결정과 대가.
