...
**아무도 안 판다 — 348 에서 SmartHR 을 `done` 으로 닫았다.** 도메인 2개 · 기능 3개.
**완료 58곳 · 진행 0 · 큐 0/3.**

**⑭ `--gaps` 는 [후보 조사]**(큐 0/3)를 가리켰다. **닫고 나니 그 지목이 그대로 다음
사이클의 대상이 된다.**

**닫은 근거** — 완주 기준을 채웠다: 도메인 둘 다 기능이 있고(1개 / 2개), `connections` 로
셋이 이어져 고아가 없으며, 수익원(구독)이 기능들과 연결된다.

**⚠️ 닫으면서 낡은 문장 하나를 정정했다**(PROMPT.md 2″단계) — 344 의 `business_model` 에
*"아직 한 편만 읽었다"* 라고 적어 두었는데 세 편을 읽었다. **그리고 그 문장의 주장 자체를
고쳐야 했다** — 순서를 밟는다는 것까지는 맞지만 **그것으로 다 되지는 않는다는 결론을 이
회사가 스스로 적는다.**

**`business_model` 에 적은 관찰**
- **⭐⭐ 두 도메인이 같은 단일 실패점에 정반대로 답한다** — 인증은 **떼어내서 격리**,
  가장 큰 Rails 앱은 **그대로 두고 앞에 세워 버틴다.** **버티는 쪽이 훨씬 빨리 끝났다** —
  단기 완료가 조건이라 **GKE 대신 Compute Engine** 을 골랐다.
- **⭐⭐ 데이터베이스에서 두 번 막혔는데 시점이 정반대다** — 접속 수는 **장애 뒤**,
  CPU 는 **정점 전.** **한 번은 늦었고 한 번은 이르다.**
- **⭐ 자기 방법의 한계를 적는다** — Packwerk 가 **새 영역에서는 됐는데 얽힌 인증에서는
  안 됐다**고 쓴다. **되는 사례와 안 되는 사례를 같은 블로그에 나란히 둔다.**
- **⭐ 자기가 고른 것이 나중에 틀린 것도 적는다** — PgCat 은 개발이 멈췄고 이미 관리형으로
  옮겼다는 사실을 도입기 안에 남긴다.
- **⭐ 언제 손을 떼는가를 설계 대상으로 삼는다** — EXIT 로드맵을 처음에 합의한다.
  ⚠️ 그래도 전환 타이밍 판단은 어려웠다고 적는다.
- **⚠️ 세 이야기가 다 진행 중이다** — DB 는 아직 함께 쓰고, 인증 리프트는 2026년 2월에
  시작했고, 부하 시험의 전사 횡전개는 결과가 없다. **끝난 이야기가 하나도 없다.**
- **⚠️ 블로그의 결이 넓다** — 사내 행사·커리어·LT 보고가 절반쯤 섞여 있다.

**안 읽고 남긴 것**(`open_questions` 에도 적었다) — `横断フロントエンドユニットの取り組み`
(2026-08-03) · `不具合情報の集約基盤を整え、品質改善に活かすまで`(2026-08-24) ·
`Sandboxテスト`(2026-08-17). 그리고 인증·인가 유닛 글이 참조하는 과거 글 셋
(**주소 미확인**): **Policy-as-Code**(2025-08-08) · **새 권한 기반 설계**(2026-01-07) ·
**모듈러 모놀리스화**(2026-05-18).

---

**다음 사이클 — 후보 조사다.** 큐가 0/3 이고 진행 중인 회사가 없다. **PROMPT.md 2⁗단계** —
**회사를 파지 않는다. 팔 수 있는지 확인해 적기만 한다.** 한 사이클에 여러 곳을 올려도 되고
**목표는 대기 3곳**이다.
- **⭐ 1순위 — Personio(EU · HR SaaS).** **아직 한 번도 안 봤다.** 큐의 '확인해 둔 후보' 에
  올라 있던 곳이다. ⭐ **SmartHR·freee 와 같은 HR/백오피스 축이라 비교가 바로 선다.**
- **⚠️ 후보마다 확인할 것** — 기술 블로그가 있는지가 아니라 **그 글이 의사결정과 대가를
  말하는지.** WebFetch 로 읽히는지, 최근 글이 언제인지, 어떤 도메인이 나올 것 같은지를
  **근거로 함께 적는다.** **⑰ 주소를 추측하지 않는다. ⑱ WebSearch + allowed_domains.**
  **⚠️ 주소가 실제로 열리는지도 확인한다**(327·328).
- **⚠️ 이미 판 회사와 겹치지 않는 축을 우선한다.** **분포가 일본 일곱으로 기울어 있다** —
  유럽·라틴아메리카·동남아는 막힌 곳이 많지만 **아직 안 본 곳을 먼저 본다.**
- **후보 조사 누적 열아홉 곳 중 일곱** — 지금까지의 타율이다.

**⭐⭐ 비교 문서 재료가 둘 서 있다**(다음다음 사이클 이후의 후보):
① **freee 대 SmartHR 의 순서** — freee 는 인증 기반을 **곧장 4년에 걸쳐** 떼어내고
*"旧機能のリファクタはマイクロサービス化の前に着手すべし"* 라 후회한다. SmartHR 은
**그 순서를 실제로 밟아 보고** 모듈러 모놀리스로 경계만 정리하는 안을 검토했다가 버렸다 —
**인가가 과금·테넌트에 의존해야 해서 의존 허용 규칙이 곧 경계 설계가 되고, 배포 독립성과
가용성은 규칙으로 안 풀린다.** **즉 후회를 실행해 보니 그것만으로는 부족했다는 이야기다.**
② **`미루되 미룬 것이 안 보이게 한다 — 그럼 그것은 어디에 쌓이는가`** 에 SmartHR 을 더한다.
**같은 회사에 두 사례가 다 있다** — 미룬 쪽(**놀고 있는 접속**에 쌓였고 청구서는 장애 빈도가
오르며 왔다)과 **미리 잰 쪽**(연 1회 정점을 부하 시험으로 앞당겨 봤다).

**분포** — **58곳 전부 `done`**(일본 일곱 · 인도 셋) · 비교 문서 32편 · **운영 사실 41개.**

**⚠️ 나르지 않는다** — 낡은 메모(314·317·325 · **348 에서도 한 번 나왔다**) ·
...
`connections` 에 적는다**(346) · **`-c "…"` 안에 백틱 금지**(347) ·
**⭐ 새로: 프로파일에 적은 '아직 한 편만 읽었다' 류의 문장은 회사를 닫을 때 반드시
다시 본다 — 348 에서 그 문장이 낡은 채로 남아 있었다.**
