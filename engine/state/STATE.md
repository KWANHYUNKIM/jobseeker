...
**Grafana Labs** (US/SE · SaaS — 관측) — 도메인 2개 · **기능 1개**(`let-the-log-remember-not-the-replicas`). 남은 도메인은 **`로그를 색인 없이 다룬다`** 하나다(⏳ 미독).

### 이번 사이클 — `사본을 세 벌 두는 대신 로그가 기억하게 한다`

건진 것 넷:

- ⚠️ **비용은 눈에 띄는 문제였고 결합이 더 깊은 문제였다** — 복제 3은 청구서로 보이지만, **`인제스터는 읽기 경로와 쓰기 경로 사이의 약한 고리이기도 하다. 흔히 겪는 문제는 고객이 던진 무거운 질의가 인제스터를 과부하시키는 것이다.`** **남이 던진 질의 하나가 수집을 멈출 수 있는 구조였다**(재구성). 그래서 **새 구조의 첫 목표가 비용이 아니라 경로 분리**다.
- ⚠️ **사본을 줄이려면 누군가는 기억해야 한다** — Kafka 를 놓고 **끊김 없는 소비**를 보장하자(**`5분 동안 내려가 있어도 시작할 때 멈췄던 자리에서 이어 받는다`**) **복제 3 → 2, 쿼럼 2 → 1** 이 가능해졌다. **내구성의 책임을 옮긴 것이지 없앤 것이 아니다.** 남은 복제 2도 **`인제스터 하나가 비정상이 될 경우를 대비해`** 둔 것이라고 명시한다.
- ⚠️ **비싼 것은 저장이 아니라 전송이었다** — **교차 AZ 전송이 GB당 0.02달러**라 **객체 스토리지에 먼저 쓴다.** 회사가 대가를 그대로 적는다 — **`지연 페널티를 받아들이는 대가로.`** **클라우드 요금표가 아키텍처를 정한 자리다**(재구성).
- ⚠️ **부품에서 역할을 덜어 냈다** — 인제스터가 **`가장 최근 데이터를 서빙하는 순수 읽기 경로 부품`** 이 됐다. **겸하던 시절에는 한쪽의 부하가 다른 쪽을 무너뜨렸다**(재구성).

### ⚠️ 이 회사가 주장하지 않은 것

**`새 아키텍처가 옛것보다 더 확장성이 좋다고는 말하지 않겠다`** — 다만 **`이제는 우리를 겁먹게 할 만한 크기가 거의 없는 지점에 도달했다`**. ⚠️ **팔기 좋은 주장 대신 실제로 얻은 것(TCO 25% 감소와 경로 분리)만 말한다.** ⭐ **Snyk 이 `언어별 작은 격차의 순서는 잡음 밖이 아니다` 라고 스스로 선을 그은 것과 같은 결이고, 이번 세션에서 두 번째다.**

### 다음 사이클 — 마지막 도메인, 그리고 완주 판정

`--gaps` 가 **`로그를 색인 없이 다룬다`** 를 부를 것이다. ⏳ **본문을 아직 안 읽었다** — **Loki 의 Kafka 기반 수집 계층 재설계**(Grafana 13, 2026). **슬러그 추측 금지 — 검색으로 주소를 먼저 받는다**(이번 사이클에 그 실수를 한 번 했다). 자료가 서면 **같은 사이클에서 완주 판정까지** 한다(도메인 2/2 · 고아 없음 · 수익원 둘 다 연결).

⚠️ **큐는 2/3**(ScyllaDB · Cygames).
...
**③ `실험 결과를 얼마나 믿나` 3곳** — Etsy / Deliveroo / Snyk. ⏳ **⭐ `자기 성과를 어디까지 주장하나` 로 넓힐 수 있다** — **Snyk 과 Grafana 가 자기에게 유리한 주장을 스스로 접는다.**

**④ `인도 규모에서 무엇이 달라지나` 2곳** — Meesho / Zepto.

⏳ **⑤ `한 코어에 하나씩인가, 여러 코어가 나눠 쓰나`**(ScyllaDB 대 ClickHouse). ⏳ **⑥ ⭐ `복제로 버틸 것인가 로그로 버틸 것인가`** — **Mimir 가 답을 냈고 Loki 도 같은 방향으로 갔다.**
