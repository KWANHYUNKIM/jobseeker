...
**freee(JP · SaaS) `in_progress` — 338 에서 프로파일을 썼다. 도메인 1개 · 기능 0개.**
**57번째 회사이고 일본 여섯째다.** 337 에서 한 번 미뤘던 [신규]를 이번에 열었다.

**블로그가 크고 살아 있다** — `developers.freee.co.jp` 최신 글이 **2026-08-20** 이고 분류가
**픽업 321 · 기술 291 · 카르처 98 · QA 95 · SRE 79 · AI 56 · 보안 39** 다.
**⚠️ 그런데 최근 글의 결이 AI 쪽으로 크게 기울어 있다** — 에이전트를 쓴 개발 프로세스 ·
사양 주도 개발 · 저비용 LLM 연구 같은 것들이라 **굵은 이관 이야기는 2022년 언저리에 몰려
있는 것으로 보인다**(추정).

**사업** — 2012년 **회계 애플리케이션**에서 시작해 백오피스 전체로 번졌다. 2025년 6월기
유료 고객 **60만 곳 이상**, **ARR 340억 엔 초과**, **창업 이래 첫 흑자**.

**⭐ 연 도메인 하나 — `모든 제품이 기대던 것을 떼어낸다`**(⭐ **이름을 넓게 지었다** —
334→335 의 교훈). 회계 애플리케이션 하나로 시작한 회사가 제품을 늘리면서 **새 제품들이 전부
회계 DB 를 거쳐 사용자·사업소·인증을 읽게 됐고**, 그래서 *"会計DBが高負荷の状態となることに
よるfreeeの全面ダウンへのリスク"* 가 생겼다. **한 제품의 데이터베이스가 회사 전체의 단일
실패점이 된 것이다.** 부하만이 아니라 **인증을 고칠 때마다 여러 제품을 함께 갱신**해야 했다.

**⭐⭐ 그리고 이 회사의 답은 '떼어냈다' 로 끝나지 않는다** — **4년이 걸렸고 그것을 잘한 일로
적지 않는다**: *"完了までに時間がかかったことを踏まえるとこれを良しとするのはよくありません"*.
**교훈 둘이 다 순서에 대한 것이다** — *"細かくリリースをできるような開発計画を組むべき"* 와
**⭐ *"旧機能のリファクタはマイクロサービス化の前に着手すべし"***.

**도메인 `tech` 넷을 채웠다** — 샤딩(수직 확장으로는 못 견뎠다) · **gRPC + Protobuf**(언어별
클라이언트 자동 생성으로 라이브러리 갱신 부담 제거) · **이중 읽기 검증**(차분을 bugsnag 로
알리고 **구 기반 결과를 돌려준다**) · **ActiveRecord 훅 비동기 동기화 + 제품별 Read 플래그와
일괄 Write 전환**(**읽기는 나눠 켤 수 있는데 쓰기는 못 나눈다** — ID 발번이 한 곳이어야 해서다).

---

**다음 사이클 — 도메인 ① 의 첫 기능을 쓴다.**
- **재료는 확정돼 있다**: `https://developers.freee.co.jp/entry/migrate-to-auth-microservice`
  (2022-08-31). **⚠️ ⑫ 원문을 다시 읽는다** — 320·324·325·326·330·331 에서 여섯 번 다 재독이
  새 재료를 냈다.
- **결정이 5개 나오는지 먼저 센다** — 샤딩 · gRPC · DDD 새 모델 · 이중 읽기 검증 · 비동기
  동기화 · 제품별 Read 플래그 · 일괄 Write 전환이면 쪼개서 6~8개가 나올 것으로 본다(추정).
- **⚠️ 두 편 규칙** — 이 도메인의 두 번째 재료는 아직 본문을 안 읽었다:
  `freee の権限管理基盤マイクロサービスの今を語ろう！`(권한 관리 기반)와
  `これってもしかして……認証基盤が入れ替わってる〜？`(인증 기반 교체). **⑰ 주소는 ⑱ 로 확인하고
  223 규칙으로 같은 문제인지 실물로 정한다.** 어기게 되면 도메인 `why` 에 밝힌다.

**📌 이 회사에서 다음에 볼 것**
- **⚠️ QA 분류가 95편으로 큰데 한 편도 안 읽었다** — 테스트를 주제로 한 도메인이 설 수도 있다
  (`ソフトウェアアーキテクチャに基づいた自動テスト戦略と実装ガイドライン` · `テストアーキテクチャの実践`).
- `確定申告を乗り越えるDBパフォーマンス改善 - Aurora 移行の舞台裏` — **확정신고라는 연 1회의
  부하 정점**이 있는 것으로 보인다(추정). MySQL→Aurora 이관과 샤딩.
- `Langfuse のバージョンアップに伴い ClickHouse を社内初導入した話` — PostgreSQL 은 트랜잭션,
  ClickHouse 는 트레이스 집계로 역할 분담.
- **⭐ AI 계열 글들이 비교 문서 `에이전트에게 무엇을 못 하게 하는가` 와 이어질 수 있다** —
  **본문을 안 읽었으므로 아직 안 적었다.**

**📌 그 밖**
- **큐 1/3**(SmartHR). **⚠️ Personio(EU)는 아직 안 봤다 — 다음 후보 조사의 첫 순위.**
- **⭐⭐ freee 와 SmartHR 이 정반대 순서로 같은 문제를 풀었다** — **freee 는 모놀리스에서 곧장
  4년**(후회로 적는다), **SmartHR 은 모듈러 모놀리스로 시작해 Packwerk 로 경계를 먼저 보이게
  한 뒤** 떼어낸다. **둘을 다 파고 나면 비교 문서 재료다.**
- **후보 조사 누적 열아홉 곳 중 일곱.**
