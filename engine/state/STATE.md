...
**Trigger.dev**(EU·영국 체셔 · 백그라운드 작업 플랫폼) — 회사 프로파일까지 썼다.
**도메인 둘 · 기능 0.** 큐 0/3.

### 두 도메인이 같은 태도의 두 얼굴이다

**터진 것도 깎은 것도 사슬과 수치로 적는다.** 사고 보고서는 원인을 **업그레이드 → 검증
삭제 → 파트 누적 → 메모리 고갈**의 사슬로 적고, 런타임 교체는 **네 단계의 처리량과 지연**을
단계마다 적는다. 남의 작업을 대신 돌려 주는 회사가 파는 것이 **믿음**이라는 것을 알고 쓰는
글이다(재구성).

### 자기 설계를 부채라고 부른다

**'임의의, 제멋대로 생긴 작업 출력을 무제한 스키마 컬럼에 저장한 것은 잠재된 책임이었다.'**
그리고 **'한 버그를 고쳤는데 우리가 하중을 받는 줄 몰랐던 보호 장치를 없앴다.'** —
**무엇이 무엇을 떠받치는지 몰랐다는 자백**이다.

### 런타임을 바꾸면 계약도 바뀐다

Bun 으로 옮겨 **처리량 5배**를 얻었는데, **대가를 성과보다 길게 적는다** — **'모든
`Promise<Response>` 는 반드시 결말이 나야 한다'**(안 그러면 상태가 영원히 메모리에 남는다 ·
RSS 250MB), prom-client 지표 넷이 못 쓰게 됨, **AbortSignal 이 두 번 뜸**, 쿠버네티스 CA
인증서, 그리고 **자기가 물린 것**(`--bytecode` 는 정상 상태 성능을 오히려 나쁘게 했다).

### 무료 단이 기능이 아니라 한도로 갈린다

작업 수는 무제한인데 **동시 실행 20 · 로그 보존 1일 · 알림 대상 1곳**으로 좁다. 컨벡스가
**기능은 다 주고 운영 도구에서 받는 것**과 또 다른 모양이다 — 여기는 **한도를 푸는 것**을 판다.

### 회사명과 제품명이 다르다

법인은 **API Hero Ltd**(잉글랜드·웨일스, 체셔 앨트린첨). **왜 바뀌었는지 어느 공개 문서도
설명하지 않는다** — Impressum 격인 개인정보처리방침에서만 알 수 있다.

### 다음 사이클

확장 — 빈 도메인 둘 중 하나. **런타임 쪽이 수치가 더 촘촘하고**(네 단계·다섯 가지 깨진 것),
사고 쪽은 **`incident-report-jun-22-2026` 을 아직 안 읽어** 두 사고가 이어진 것인지 확인이
필요하다. 큐가 0/3 이라 그 뒤에 후보 조사가 뜬다.
