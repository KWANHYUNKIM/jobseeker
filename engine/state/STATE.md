...
**없다 — Mux 를 완주시켰다(도메인 1 · 기능 1 · `status: done`).** 회사 153개, **큐 0/3.**

### 그럴듯한 근거가 재 보니 본전이었다

"층을 줄이면 캐시 적중이 오른다"는 말은 옳게 들린다. 그런데 2026년 2월 CDN·저장·연산 로그로
재니 **조건에 따라 10% 절감에서 15% 증가까지** 흩어져 사실상 본전이었다. 그래서 270p 를 안 뺐다.

### 새 쓰임이 낡은 층을 지켰다

가장 낮은 층을 살린 것은 제약 있는 시청자만이 아니다 — **추론에 필요한 최소한의 픽셀만 원하는
AI 에이전트**가 그 자리를 새로 쓴다. 물린 대안(270p 를 720p 로 맞바꾸기)은 그 쓰임까지 없애
**값을 두 번 무는** 셈이라 뺐다.

### 더 보내는 것과 더 받는 것은 같지 않다

720p 를 더하자 시청이 몰렸고(+43.4%p, 전달 분수의 절반) 분당 바이트가 **6.5% 늘었다.** 그런데
**고객 청구서는 줄었다** — 요금이 바이트가 아니라 해상도 등급으로 갈리고, 1080p 에서 내려온
쪽도 480p 에서 올라온 쪽도 더 싼 720p 로 모였기 때문이다.

### 실제로 무엇이 나갔는지는 서버가 모른다

적응형 스트리밍에서는 클라이언트가 매 순간 화질을 고른다. 그래서 **"주어진 세션에서 실제로
무엇이 전달됐는지는 플레이어만 안다"**고 적고, 플레이어가 알린 시간으로 결과를 확인한다.

### 다음 사이클 — ⚠️ 후보 조사 (큐가 0/3 이다)

판정 기준을 이어 쓴다 — 회고를 먼저, 되돌린 기록, 자기 것을 쓰지 말라고 적는 글, 사고를
공개하는 글, 상류에 고쳐 보낸 기록. **형식이 아니라 대가가 기준이고, 본문을 열어 판정한다.**
`name_en` 153개를 먼저 출력해 중복부터 확인한다.
