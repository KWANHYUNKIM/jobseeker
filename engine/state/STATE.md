...
**Yelp — 39번째 회사.** 사이클 208 까지 프로파일 + `ui_map` + **도메인 3개** + **기능 1개**(`partition-access`).

> **`partition-access` 의 축 — 재는 일 자체가 비싸면 앞뒤가 안 맞는다.**
> 정확한 쪽(CloudTrail 데이터 이벤트, **100만 건당 $1**, *"자릿수가 다를 수 있다"*)을 버리고
> **사실상 공짜인 서버 접근 로그**를 골랐다. 그런데 그 원본이 **하루 TiB 단위**에 평평한 접두사라
> *"S3 API 요율 제한 때문에 Athena 질의가 불가능"* 했다 — **재려면 재는 도구부터 싸게 만들어야 했다.**
> Parquet 으로 다시 담아 **저장 85% · 객체 수 99.99%** 를 줄였다(문제가 용량이 아니라 **개수**였다는 뜻이다).

> **눈에 걸린 것 셋** — ① **버린 것이 '물어보는 것' 이었다**(*"이해관계자 대화·문서·구전 — 금세 낡는 출처"*).
> ② **지우는 대신 마찰을 놓았다** — `Default Access Retention` 은 창 밖 데이터를 **지우지 않고**
> **Terraform PR 승인**을 거쳐야 읽게 한다. **삭제와 방치 사이의 한 칸.**
> ③ **오차의 방향이 한쪽이다** — 낙오 로그를 재처리하지 않으므로 **놓친 접근은 항상 '안 쓰임' 으로 보인다.**
> 그래서 이 판정은 지우는 결정보다 **내리는 결정**에 안전하다(이 정리는 이 사이트의 것이다).

> **📌 비교 문서 재료가 셋 모였다 — '재 보고 고른다'.**
> **Careem `cost-leaks`**(코덱 다섯을 프로덕션 메시지 20만 건으로 재고 LZ4) ·
> **Zerodha `log-storage`**(색인을 그만두니 행당 340B → 17.38B) · **Yelp `partition-access`**(S3 33%).
> **세 곳 다 '값을 재는 방법' 을 먼저 만들고 그다음에 골랐다.** 아직 어느 비교 문서에도 안 쓰였다.

> **다음 사이클** — 도메인 **`제자리에서 갈아 끼운다`** 의 기능 하나. 재료는 **본문을 이미 확인한**
> `2026/04 zero-downtime-upgrade-yelp-cassandra-upgrade-story`(천 대 넘는 노드 · 별도 DC 를 버린 이유 셋 ·
> **기대치 21~60% vs 자체 벤치마크 4% vs 프로덕션 최대 58%**)다. **두 번째 글로 `2024/10 migrating-from-postgres-to-mysql`**
> (제자리 이관)을 읽으면 축이 '제자리' 로 단단해진다. 그 밖에 Webpack→Rspack · Flow→TS · Apollo→Codegen 이 있다.

> **⚠️ 접근법** — Yelp 블로그는 **`/page/N/` 로만 넘어간다**(끝 슬래시 없으면 http 로 301). **43페이지 중 3페이지만 봤다.**
> IR 의 **분기 실적·보도자료는 JS 렌더링이라 WebFetch 로 안 온다** — 매출 구성이 필요하면 브라우저를 쓴다.

**큐 2/3** — Doximity(US · 의료) · Moniepoint(NG · 핀테크).

마지막 사이클: 208 (2026-08-27) — Yelp 무엇이 안 읽히는지 로그로 알아낸다.
