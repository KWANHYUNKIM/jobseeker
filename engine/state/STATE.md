...
**Figma — 진행 중.** 도메인 4개 중 **3개 완료** — 동시 편집(`multiplayer-doc`),
실시간 데이터 구독(`livegraph-subscription`), 렌더링·클라이언트 성능(`webgpu-renderer`).
**남은 것은 데이터 저장·확장 하나다** — 이것만 채우면 완주 판정이 가능하다.
`category` 는 `SaaS` 다 — validate.py 의 CATEGORIES 에 '협업 도구'는 없다.

마지막 사이클: 100 (2026-08-21) — WebGPU 렌더러.

## 다음 선택지

*(사다리는 PROMPT.md 2단계가 정한다. 여기는 그 사다리가 여러 개를 물어올 때의 판단 근거다.)*

1. **Figma 데이터 저장·확장** — 진행 중 회사의 마지막 빈 도메인. 단일 Postgres 에서
   수직·수평 샤딩으로 간 경로. LiveGraph 100x 글이 그 샤딩을 전제로 쓰여 있어 이어 읽기 좋다.
   **이걸 끝내면 Figma 는 완주다.**
2. **비교 문서 '브라우저가 안 열어 주는 것을 어떻게 직접 만드는가'** — 100 에서 나온 새 축.
   Figma 텍스트 레이아웃 엔진 · 렌더러. 아직 한 회사뿐이라 짝이 더 필요하다.
