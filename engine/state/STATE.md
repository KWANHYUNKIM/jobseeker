...
**없다 — 문체 보수 사이클이었다.** Perplexity 를 배민 문체로 다시 썼다(2.21 → 0.0, 기호 195개). 회사 115개, 큐 2/3. 33곳 남았다.

### 러스트로 옮긴 회사 셋 — 옮긴 이유가 다 다르다

첫 기능에 축을 세웠다.

- Discord 는 카산드라를 떠나려고 저장 계층을 다시 써 노드를 177대에서 72대로 줄였다.
- 터소는 물려받을 수 없는 테스트 스위트 때문에 데이터베이스를 통째로 다시 썼다.
- Perplexity 가 옮긴 것은 **GPU 앞의 대기줄**이다 — 파싱과 토큰화가 도는 동안 GPU 가 논다.

그리고 이 결정이 원가 구조에서 나오는 것이 Modal 과 같다. 저쪽은 유휴에 값을 안 받아서 시작·이동
시간이 원가가 됐고, 여기는 사용량으로 팔아서 GPU 이용률이 마진이 된다. **파는 방식이 어디를
깎을지를 정한다.**

### 미리 만들 것인가 그때 만들 것인가 — 두 극단 사이

둘째 기능을 두 회사 사이에 놓았다.

- Honeycomb 은 색인도 스키마도 사전 집계도 없이 **아무것도 미리 정하지 않는다**.
- Meesho 는 변형본이 어디 있는지 묻지 않으려고 **경로를 규칙으로 계산해 둔다**.
- Perplexity 는 둘 중 하나를 고르지 않는다 — CUDA 그래프를 다 미리 캡처하면 서버가 몇 분간 안 뜨고,
  안 쓰면 파이썬 오버헤드를 매번 문다.

### 묶이지 않으려는 셋 — 줄여서 사는 쪽과 만들어서 늘리는 쪽

- Airbnb 는 표준 프로토콜로 옮겨 신뢰성을 얻었다.
- Honeycomb 은 데이터 전송이 싼 길을 99.99% 가용성 약속 때문에 잘랐다.
- Perplexity 는 AWS 의 EFA 에서 안 되면 소용없다는 제약 아래에서, **NIC 에 안 묶이는 계층을 직접
  만들어 선택지를 늘렸다.**

앞의 둘은 선택지를 줄여 안전을 샀고 여기는 만들어서 늘렸다. 그리고 그것을 논문과 오픈소스로
내놓는다 — 안에서 쓰려고 만든 것이 밖으로 나가는 자리다.

### 도메인을 제품이 아니라 요청이 GPU 에 닿기까지의 구간으로 잘랐다

앞까지 나르는 일, GPU 위에서 도는 일, GPU 끼리 말하는 일. **이 경계가 곧 언어의 경계이기도 하다** —
앞과 옆은 러스트이고 위는 파이썬이다. 검색 도메인을 안 세운 것은 그쪽 글을 못 읽어서라고 적었다.

### 다음 사이클 — 문체 보수 (사다리 3순위)

--gaps 가 정한다. 지금 맨 위는 klaviyo(2.16)지만 그건 이 시점의 예상이지 확정이 아니다.
