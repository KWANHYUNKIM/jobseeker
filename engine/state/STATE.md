...
**Pinterest** (US · 소셜 — 발견·추천) — **프로파일까지 썼다**(PROMPT 3단계). 도메인 3개 · 기능 0. 회사 **99개.** ⚠️ **큐가 0/3 이 됐다.**

### 이번 사이클 — 신규(6순위). 큐의 마지막 한 곳을 올렸다

⚠️ **이 회사의 기술 이야기는 `무엇을 보여 줄지` 로 모인다** — 홈피드에 무엇을 꺼내 올지(조건부 학습 검색) · 사용자를 어떤 쓰임새로 이해할지(Pinner Progression) · 그것을 굴리는 인프라를 어떻게 안전하게 둘지(자원 프로비저닝)(재구성). 도메인을 그 셋으로 세웠다.

**규모** — 2026년 2분기 매출 **11억 8,000만 달러(+18%)** · 월간 활성 사용자 **6억 4,000만 명(+11%)** 으로 최고치 · **하위 퍼널 광고 매출의 약 30%** 가 Performance+ 를 지나고 **전달 모델 개선이 중소 광고주 시험에서 ROAS 28% 향상.**

### 읽어 둔 것 — 검색 글에서 건진 것 넷 (다음 사이클의 재료)

- ⚠️ **암기의 비용이 숫자로 나온다** — **`충분한 암기와 견딜 만한 충돌을 위해 20GB 크기의 id 임베딩 테이블을 사전학습하게 됐다.`** 그래서 **양자화 VAE 로 만든 시맨틱 ID** 와 **계층마다 하나씩 다섯 개의 작은 임베딩 테이블 + MLP 융합**으로 바꿨다 — **메모리와 표현력의 맞바꿈**이다.
- ⚠️ **조건마다 모델을 따로 두면 일이 곱해진다** — **`학습 파이프라인·피처 업그레이드·실험·서빙 설정·인덱싱에 걸쳐 일이 중복된다`**. 통합 CLR 은 **조건별 최적화를 일부 내주고 운영 단순성을 샀다.**
- ⚠️ **가장 큰 절충은 서빙 쪽이었다** — 조건마다 별도 요청으로 다루던 것을 GPU 기반 서빙으로 바꿔 **`7자리 비용 절감과 p90 모델 지연 85% 감소(80ms → 12ms)`**. **병목이 네트워크에서 CPU 로 옮겨 갔다.**
- **학습 효율 2배** — 요청 수준 중복 제거(같은 배치 안의 동일 사용자 피처를 없애 **사용자 타워를 지나는 유효 배치 크기를 크게 줄인다**)와 **M-Falcon 으로 조건 토큰을 평탄화**해 GPU 메모리를 아꼈다.

### 다음 사이클 — 확장(2순위)

`--gaps` 가 **`홈피드에 무엇을 꺼내 올지 조건에 따라 정한다`** 를 부를 것이다. **자료를 이미 읽었으니 바로 기능을 쓴다.** ⭐ **⑮축(`언제 쪼개고 언제 합치나`)에 `조건별 모델을 합쳤다` 로 붙는다.**

⚠️ **큐가 0/3 이다.** 이 회사를 다 판 뒤 **3순위(후보 조사)** 가 걸린다. ⚠️ **지난 조사에서 3곳을 못 채웠으니 다음엔 다른 각도를 쓴다** — 컨퍼런스 발표 목록이나 **이미 판 회사의 글이 언급하는 다른 회사**.

⏳ **보강 거리 셋** — Grafana Labs 인용 대조 · Razorpay 보안 트리아지 글 · Flipkart Rate Card 엔진 글.

### ⚠️ 비교 문서 재료 (초안 유지)

**① `AI 에이전트를 어디까지 믿나` 9곳** — Sentry / ClickHouse / Duolingo / Ramp / DoorDash / Deliveroo / Snyk / Cygames / Razorpay. ⏳ **Pinterest 의 `AI 팀이 된다`(2026-09)가 조직 쪽 재료가 될 수 있다.**
...
**⑮ `언제 쪼개고 언제 합치나`** — Twilio / DoorDash / Deliveroo / ScyllaDB / Airbnb / ⏳ **Pinterest**(조건별 모델을 합쳤다).

**⑯ `빌려 쓰던 것을 언제 자기 것으로 만드나`** — Airbnb / ScyllaDB / Plaid / Cygames / Twilio.

⏳ **⑰ 새로 보인다 — `기억시킬 것인가 압축할 것인가`** — **Pinterest**(20GB 임베딩 테이블 → 시맨틱 ID) / **Zepto**(색인이 초~분 뒤처지는 것을 권위 확인으로) / **Grafana Labs**(색인 없는 대가를 읽을 때 치른다).
