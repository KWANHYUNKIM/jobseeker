...
**Tigris Data — 회사 프로파일까지 썼다(in_progress).** 회사 125개, 큐 1/3.

도메인 셋을 세웠고 기능은 아직 0개다.

- **메타데이터를 한 트랜잭션으로 묶는다** — `Building a global object store on FoundationDB`(2026-08-18)
- **어디에 두고 어디서 읽을지 정한다** — 같은 글(애니캐스트·세 단계 읽기·즉시 복제되는 메타데이터)
- **남의 API 를 그대로 받아 옮겨 오게 만든다** — 제품 페이지(S3 API 90% 이상, 이그레스 무료)

### 이 엔진에서 자리가 뒤집힌 회사다

WarpStream·Neon·Cursor 는 객체 저장소를 **진실 원천으로 삼아** 그 위에 시스템을 지었다 — 저쪽에서
S3 는 주어진 바닥이었다. 티그리스는 그 바닥을 만드는 쪽이라, 저쪽이 전제로 삼던 성질을 스스로
지어야 한다. 같은 축의 반대편이 데이터에 들어온 셈이다.

### 어려운 것은 바이트가 아니라 기록이다

객체 메타데이터 쓰기와 색인 변경, 그리고 뒤에 할 백그라운드 작업까지를 **한 트랜잭션에 함께
커밋한다.** 얻는 것을 한 줄로 적는다 — 데이터베이스에는 썼는데 큐에 넣지 못하는 오류 경로가 없다.
큐를 따로 두지 않고 FDB 안에 직접 구현했고(애플의 QuiCK 에서 영감) 작업은 끝날 때까지 남아 최소
한 번 실행되며 워커가 죽으면 리스가 만료되며 넘어간다.

키 배치도 그 연장이다 — `[테넌트 | 버킷 | 서브스페이스 | 객체·색인·키]` 를 사전순으로 인코딩해
한 버킷의 모든 것이 연속 범위에 놓이게 한다.

### 무엇을 즉시 하고 무엇을 미룰지 갈랐다

**메타데이터는 리전 간에 즉시 복제하고 실제 바이트의 배치는 백그라운드로 미룬다.** 목록 조회의
전역 일관성을 쓰기 지연보다 앞에 뒀다고 회사가 밝힌다. 읽기는 로컬 캐시 → 로컬 블록 저장소 →
원격 순으로 내려가고, 원격에서 가져오면 그 뒤에 배치 작업을 큐에 넣어 캐시를 데운다.

### 다음 사이클 — 확장 (Tigris 의 빈 도메인)

⚠️ 심층 글이 아직 이 하나뿐이다. FDB 트랜잭션 묶기를 먼저 팔고, 두 번째 기능 전에 자료를 더 찾는다.
수치가 하나도 없다는 것도 그대로 적어 뒀다.
