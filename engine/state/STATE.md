...
**없다 — 쉰일곱 번째 후보 조사를 돌았다. 큐 1/3.** 다음 사이클은 **신규**(Bluesky).

### 자기 선택을 유일한 답으로 팔지 않는 글

Bluesky 의 페더레이션 구조 글에서 가장 값어치 있는 문장이다 — **"릴레이를 통해 연결한다는
우리 방식은 규범적인 것이 아니다. 프로토콜은 실제로 양쪽 다 되도록 설계돼 있다."** 서버 대
서버 대신 중앙 릴레이를 고르고, 그 대가(릴레이는 자원 요구가 커서 **소수만 존재할 것**)까지
적는다. **비어 있던 분산 소셜 축**이 열렸다.

### 사후 기록처럼 보이는 것과 진짜 사후 기록

2026년 4월 장애를 **네 편**에 걸쳐 적었길래 열었는데 **상태 공지**였다 — DDoS 였다는 것과
데이터 무단 접근 증거가 없다는 것뿐이다. Proton 의 냉각 실패 기록(온도·타임스탬프·물린
선택·틀린 가정)과 견주면 차이가 분명하다. **편수가 아니라 그 안에 무엇이 있느냐**다.

### 한 회사의 글이 세 도메인에 흩어져 있다

`atproto.com`(명세·가이드) · `bsky.network/blog`(2026년 8월 시작한 운영 공지, 글 둘) ·
`bsky.social/about/blog`(79편, 여기 기술 글이 있다). **리디렉션을 두 번 따라가고** 홈에서
href 를 받아서야 닿았다 — 주소를 추측하지 않는 규칙이 또 값을 했다.

### Hugging Face 는 이번에도 못 뚫었다

홈을 열어 href 를 받았는데 **`/blog` 하나뿐이고 별도 엔지니어링 섹션이 없다.** 아카이브를
넘길 방법이 검색뿐인데 **이 세션의 WebSearch 예산이 끝났다.** '접음'이 아니라 **'못 읽음'**
으로 계속 둔다. **CAD 축과 의료 축도 그대로 비어 있다.**
