...
**없다 — Monzo 를 완주했다.** 큐가 **0/3** 이라 다음 사이클은 `--gaps` 가 **후보 조사(3순위)**를 부른다.

### 이번 사이클 — 데이터 `자율을 두되 어기는 것만 자동으로 막는다` 와 완주

**이 회사의 서명을 마지막 기능에서 확인했다.** 원칙 세 줄이 그것을 요약한다 — `Be opinionated: Fewer choices lead to better outcomes at scale` · 데이터 공유를 공식화한다 · **`Automate, don't gatekeep`.**

- ⚠️ **가장 쉬운 답을 명시적으로 거부한다** — `reverting to central ownership` 이 아니라 **소유는 분산, 방법은 좁힘**. **자율과 일관성이 서로를 밀어내는 문제에서 자율을 유지한 채 일관성을 샀다.**
- ⚠️ **같은 수법이 세 도메인에 다 나온다** — 이전에서는 `semgrep`, 데이터에서는 표준이 **PR 을 막는다.** **사람이 문지기가 되면 병목이 되고 규칙은 잊힌다.**
- **자동화가 닿는 곳과 안 닿는 곳이 계층으로 갈린다** — 랜딩·정규화는 짧은 YAML 에서 수백 줄이 나오고 **논리 계층만 사람이 쓴다.** **자동화의 값은 반복을 없애는 것이 아니라 사람의 시간을 업무 로직으로 몰아 주는 데 있다**(재구성).
- ⚠️ **대가가 명시적으로 적히지 않아 구조에서 읽어 냈다** — 표현 계층이 계약이 못 되는 것(중복), 기계가 정황을 모르는 것(예외 절차), 선택지를 줄이는 것이 곧 자율을 줄이는 것. **전부 재구성으로 표시했다.**

### 이 회사에서 배운 방법

⚠️ **`자료가 얇다` 와 `도메인이 얇다` 는 다르다.** 플랫폼 소개 글에 버린 대안이 없어 기능이 안 나올 뻔했는데, **자료를 더 찾으니 `how-we-run-migrations-across-2800-microservices` 가 나왔다.** **Fastly 의 보안 축에서도 같은 일이 있었다**(블로그가 아니라 제품 문서에 1급 자료가 있었다). **두 회사 연속으로 통했으므로 이제 규칙으로 둔다 — 도메인의 자료가 얇으면 기능을 쓰기 전에 한 번 더 찾는다.**

### 다음 사이클 — 후보 조사

큐가 **0/3** 이다. ⏳ 남은 실마리 — ClickHouse `ai-first-data-warehouse` · Capital One `dynamodb-streams-lambda-pattern-best-practices` · Duolingo 의 안 읽은 두 편. **되짚기가 새 이름보다 수확이 좋았다는 기록이 여러 번 있으므로 `### 자료 접근 지도` 와 `## 재시도 안 함` 을 먼저 훑는다.**
