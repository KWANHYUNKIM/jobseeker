...
**없다 — 일흔두 번째 후보 조사를 돌았다. 큐 1/3.** 다음 사이클은 **신규**(DeNA).

### 비용을 달러로 적는 회사를 찾았다

DeNA 가 통과한 근거다. IaaS MySQL → Cloud SQL 이관에서 **월 $721 → $1,017(약 1.4배)** 를
**양쪽 다 내고**, 그 증가를 **두려워하던 '몇 배 규모의 증가'와 견주어** 받아들일 만하다고
적는다. **이 원장에서 요금을 숫자로 낸 사례는 드물다** — 대부분 '비용이 는다'에서 멈춘다.

### 실측을 문서값과 나란히 놓는다

페일오버가 **문서에는 60초인데 실측 약 40초**, 스펙 변경은 **약 1초 다운타임**, 부하 시험은
**기준의 3배.** **남이 적어 둔 값을 그대로 믿지 않고 재 본 것**을 적는 태도다.

### 입구를 못 찾은 것과 자료가 없는 것을 가른다

**넥슨 인텔리전스랩스**는 사이트맵에 **카테고리 7개뿐**이고 목록이 JS 로 그려져 **개별 글
주소를 얻지 못했다** — 자료가 없는 것이 아니다. **크래프톤**은 연결 자체가 안 된다.
**한국 게임사 축이 두 사이클 연속 안 열렸다.**

### 회사 단위로 팔 수 없는 자료도 있다

**데보션(SKT)** 은 커뮤니티 플랫폼이라 **여러 회사·개인의 글이 섞인다.** 통신 축을 열고
싶어도 **이 엔진의 단위(한 회사)** 와 안 맞는다 — **자료의 두께가 아니라 주인이 문제**인
경우를 처음 만나 적어 둔다.

### 다음 사이클

**신규 — DeNA.** 큐 1/3 이라 신규 한 번 뒤에 조사가 다시 뜬다.
