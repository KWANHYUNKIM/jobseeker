...
**Cygames** (JP · 게임) — 도메인 3개 · **기능 2개.** 남은 도메인은 **`게임 만드는 일에 LLM 을 넣는다`** 하나다(⏳ 미독).

### 이번 사이클 — `피크에 맞춰 늘 사 두는 대신 그때 늘린다`

⚠️ **처음 연 자료로는 결정이 넷뿐이었다** — STYLE 은 5~8개를 요구한다. **억지로 늘리는 대신 자료를 더 찾았고**(데이터베이스 벤더의 사례 연구) **거기서 진짜 이유가 나왔다.**

건진 것 넷:

- ⚠️ **탄력이 없으면 피크가 상시 비용이 된다** — **MySQL 로는 스케일 인·아웃에 몇 달의 준비**가 필요해서 **평시 20만 QPS 인데도 피크에 대비해 늘 자원을 예약해 둬야 했다.** **줄이는 것도 몇 달이 걸리니 결국 줄이지 않게 되고 그 낭비가 상시화된다**(재구성).
- ⚠️ **둘 중 하나를 고르는 문제로 두지 않았다** — 게임 업계 표준인 샤딩은 **다중 샤드 트랜잭션을 복잡하게** 만드는데, 회사가 고른 근거가 **`「성능의 스케일」과 「트랜잭션 기능」을 더 높은 수준에서 양립할 수 있다고 생각해 선택했습니다`** 다.
- ⚠️ **미리 사 두지 않고 트래픽이 도는 중에 늘렸다** — **`업데이트하던 시간대에는 40만 QPS 정도 있었지만 에러 없이 스케일 업할 수 있었습니다`**(TiKV 약 2배 사양). **출시 당일에 조작을 하는 위험을 감수한 셈이다**(재구성).
- ⚠️ **출시일에는 나중에 필요할 것을 지금 끄게 된다** — **`TiDB 의 통계 정보 「이력」을 갖고 있었는데 이것이 메모리를 압박해서 「이력」 저장은 껐습니다.`** **관측을 내주고 안정성을 샀다**(재구성).

### ⚠️ 이번 사이클에 조심한 것 둘

- **자료의 출처 성격을 본문에 밝혔다** — **평시 20만 QPS·스케일에 몇 달·다운로드 수치가 데이터베이스 벤더(PingCAP)의 사례 연구에서 왔다.** 회사 발표 요약과 겹치는 부분은 확인했지만 **`벤더 자료는 성공을 말하도록 만들어진 글`** 이라는 점을 metrics 와 open_questions 에 적었다.
- **`왜 신작에서 처음 쓰나` 에 그럴듯한 답이 떠올랐지만**(신작이라 옮길 데이터가 없고 설계를 처음부터 맞출 수 있다) **글에 근거가 없어 `이 추정은 이 사이트의 재구성이고 글에 근거가 없다` 를 명시했다.** ⚠️ **앞 사이클(낙관적 UI)에 이어 두 번째다.**

### ⚠️ 이 글이 말하지 않는 것도 그림으로 남겼다

**겪은 문제도, `다르게 하겠다` 도 없다.** 사전 준비도 **`여러 검증·조사를 거듭했다`** 로만 적힌다. **컨퍼런스 후속 요약의 한계**라 `이 이야기에서 비어 있는 곳` 이라는 실패 그림을 따로 그렸다.

### 다음 사이클 — 마지막 도메인, 그리고 완주 판정

`--gaps` 가 **`게임 만드는 일에 LLM 을 넣는다`** 를 부를 것이다. ⏳ **CEDEC 2026 글 셋 미독** — `LLM 코드 리뷰 지원 도구`(archives/11637) · `시나리오 작가용 AI 도구`(archives/77178) · `게임 개발에서의 LLM 활용`. ⚠️ **`r.jina.ai` 를 앞에 둔다.** 자료가 서면 **같은 사이클에서 완주 판정까지** 한다(도메인 3/3 · 고아 없음 · 수익원 둘 다 연결).

⚠️ **큐가 0/3 이다.** 이 회사를 다 판 뒤 **3순위(후보 조사)** 가 걸린다.

### ⚠️ 비교 문서 재료 (초안 유지)

**① `AI 에이전트를 어디까지 믿나` 7곳 + ⏳ Cygames** — Sentry / ClickHouse / Duolingo / Ramp / DoorDash / Deliveroo / Snyk.

**② ⭐ `관리형 MySQL 의 한계` 3곳 + Cygames(TiDB)** — ⚠️ **Plaid 와 Cygames 가 같은 TiDB 를 다른 이유로 골랐다** — 하나는 **관리형 MySQL 의 병목과 우회 비용** 때문에, 하나는 **스케일에 몇 달이 걸려 피크 대비가 상시 낭비가 되기** 때문에.

**③ `자기 성과를 어디까지 주장하나` 3곳** — Snyk / Grafana / ScyllaDB.

**④ `인도 규모에서 무엇이 달라지나` 2곳** — Meesho / Zepto.

**⑤ `한 코어에 하나씩인가, 여러 코어가 나눠 쓰나`** — ScyllaDB 대 ClickHouse.

**⑥ `복제로 버틸 것인가 로그로 버틸 것인가`** — Grafana 의 Mimir·Loki.

**⑦ `추상화가 무엇을 가리는가`** — Plaid / ScyllaDB / ⏳ Paystack.

**⑧ `제약을 없애지 못할 때 어디서 갚는가`** — Cygames(통신 대기를 연출로) / Zepto(재고 지연을 권위 확인으로) / Grafana(색인 없는 대가를 새 포맷으로).
