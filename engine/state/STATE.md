...
**없다. 311 은 후보 조사였고 한 곳도 못 올렸다. 대신 290 부터 밀려 있던 정리를 했다 —
kakao 와 Roblox 를 hold 유지한 채 `done` 으로 닫았다.**

**⚠️ ⑲ 를 먼저 돌렸다** — 53곳, 중복 없음.
**⭐ 이제 `in_progress` 인 회사가 하나도 없다. 53곳 전부 `done` 이다.**

**후보 조사 — 셋을 보고 하나도 못 건졌다**(근거는 QUEUE '확인해 둔 후보' 에 적었다)
- **❌ SoundCloud** — `soundcloud.com/blog` 은 **뉴스룸**이고, `developers.soundcloud.com`
  은 API 문서다.
- **❌ Kuaishou(快手)** — **전용 엔지니어링 블로그가 없다.** 오픈소스 저장소와 IR 블로그뿐.
  **중국 기업이 WeChat 공식계정만 쓰는 두 번째 사례다**(Xiaohongshu 가 첫 번째, 305).
- **❌ GetYourGuide** — `inside.getyourguide.com` 이 채용 블로그로 리다이렉트된다. 그래도
  **305 의 교훈대로 기술 글 한 편을 열어 봤는데**(`LLM evals in three days`) **버린 대안 0 ·
  수치 0** 이고 한계도 한 줄뿐이었다. **결정은 여덟인데 대가가 거의 없다.**
- **⭐ 후보 조사 누적 성적 — 열두 곳을 보고 둘만 건졌다**(298 여섯 중 하나 · 305 셋 중 하나 ·
  311 셋 중 영). **1차 자료가 있는 곳 자체가 말라 가고 있다**(이 판단은 이 사이트의 것이다).

**닫은 둘**(290 의 판단을 적용했다 — 새 단서가 없으면 hold 를 유지한 채 닫는 것이 정당하다)
- **kakao** — 도메인 7개 · 기능 6개. 남은 `동기화·다중 기기` 는 공개 1차 자료를 **두 번 찾아
  두 번 다 없었고**(사이클 028) `tech.kakao.com` 본문이 안 열린다.
- **Roblox** — 도메인 4개 · 기능 3개. 남은 `콘텐츠 저장·전송` 은 **공개 1차 자료가 없다**
  (CDN 은 커뮤니티 포럼 글뿐, 하이브리드 아키텍처 글은 회사 스스로 *"아직 실시간으로 안
  돈다"* 고 밝히는 발표문).
- **둘 다 `hold_reason` 을 지우지 않았다** — 새 발표가 나오면 지우고 다시 판다.

---

**다음 사이클 — 후보 조사가 말라 가므로 비교 문서를 우선한다.**
`--gaps` 는 큐가 0 이라 [후보 조사]를 계속 가리킬 것이다. **하지만 열두 곳 중 둘이라는
성적이면 사다리 8순위(비교 문서)를 앞당기는 것이 낫다**(310 에서 그렇게 했다).
**후보 조사를 한다면 아래를 보되, 한 번 시도해 안 되면 바로 비교 문서로 넘어간다.**
- **안 본 곳** — 인도 Zoho·Ola·Paytm·Freshworks · 유럽 Trade Republic·Personio ·
  일본 Sansan·freee·SmartHR·Money Forward(**일본은 이미 넷이라 우선순위 낮음**).
- **⚠️ 중국은 사실상 막혔다** — Xiaohongshu·Kuaishou 둘 다 블로그가 없었다. **WeChat
  공식계정 관행 때문으로 보인다**(추정). Alibaba·Tencent 는 커뮤니티 포털이라 1차 자료로
  보기 어렵다.
- **분포** — 53곳. US 22 포화 · 한국 아홉 · 일본 넷 · EU 일곱 · 중국 둘 · 동남아 하나 ·
  인도 하나 · 라틴아메리카 0(재시도 금지) · 아프리카 하나.

**⭐⭐ 남은 비교 문서 축 셋 — 재료가 다 준비돼 있다**
1. **`에이전트에게 무엇을 못 하게 하는가`** — 재료 일곱, 세 갈래: **병합 제한**(monday.com
   Morphex · Snap CodePal · Snap Casper) · **접근·행동 제한**(Wix AirBot · Wix 자가치유
   두뇌) · **반대편**(Wix Agent Ready Repos · Wix 컴파일러 문지기 · Adevinta).
   **⚠️ 310 의 `what-guards-the-gate` 와 재료가 겹친다 — 겹치는 부분은 서로 가리키게 쓴다.**
2. **`자를 어떻게 세우는가`** — Snap 넷(골든셋 만들기 / 못 잰다고 적기 / 자를 만들고 결과가
   나쁘다고 적기 / 범위를 다른 편에서 밝히기) + Meituan 둘(자가 무뎌지면 새로 만든다 /
   이진화해 기계에게 맡긴다). **기존 `what-counts-as-good.md`·`wrong-yardstick.md` 와
   서로 가리키게 쓴다.**
3. **`무중단으로 갈아 끼우기`** — Wix 두 편(인프라 층은 앱이 모르게 *"no code changes"* ·
   앱 층은 앱이 전부 알게 읽기마다 상태 조회) + **Cybozu**(애플리케이션이 딛는 바닥, `fcntl`
   잠금 때문에 **가용성을 얻고 분산을 내줬다**). **기존 `reversible-first.md`·
   `parallel-cutover.md` 와 서로 가리키게 쓴다.**

**비교 문서 쓰는 법**(310 에서 확인) — `jd-viewer/public/reveng/domains/<slug>.md` 에 쓰고
**`domains/index.json` 에도 등록한다**(`slug`·`title`·`question`·`companies`·`features`·
`file`). 머리말에 비교 대상과 *"각 회사 페이지에서 `확인`으로 정리한 사실만 엮었다. 비교에서
나오는 해석은 **추정**으로 표시했다"*, 다른 비교 문서 안내, 표로 갈라 보이기, 마지막은
`## 이 사이트에 아직 없는 것`. **회사 페이지에 `확인`으로 있는 사실만 쓴다.**
**⚠️ 인용에 원문 언어를 섞지 않는다**(310 에서 두 곳을 고쳤다).
...
- **📌 Grab 보강 후보**(이미 `done`) — Palana 2부작(06-19/21) · Agent platform Part 1(07-24) ·
  **Iceberg(07-10)** · **Counter Service 저장소 이전(07-03)**. 뒤 둘은 기존 `iceberg-lake`·
  `counter-service` 의 후속이다. **보강 사이클로 붙일 수 있다.**
- **✅ kakao·roblox hold 정리는 끝났다** — 더 이상 남은 `in_progress` 가 없다.
