...
**Block (Square)** — 기능 **1/4.** 회사 109개. ⚠️ **`--gaps` 가 STATE 계획과 다른 도메인을 지목했고 출력을 따랐다**(`키를 만드는 난수` 대신 `확률적으로 틀리는 것을 고친다`).

### 이번 사이클 — `확률적으로 틀리는 것을 고치는 자가치유 루프`

⭐ **회귀 테스트의 전제가 깨진 자리에서 시작한다** — **`LLM 으로 움직이기 때문에 품질 실패의 상당수가 확률적이다 — 같은 행동을 해도 어떤 고객에게는 문제가 나타나고 어떤 고객에게는 안 나타난다.`** 사례가 구체적이다: 송금을 묻던 고객이 **`제 계좌가 개인인가요 사업자인가요`** 라고 묻자 **도구를 부르지도 않고** **`제가 보기에 개인 계좌입니다`** 라고 답했다. ⚠️ **`때로는 도구를 확인하거나 볼 수 없다고 말하고, 때로는 짐작한다.`**

건진 것 넷:

- ⭐ **`고쳤다`를 표본 수로 만든다** — **고치기 전에 스테이징에서 10회**(버그가 한 번만 나타나면 **20회**) 돌려 **기준 실패율**을 세우고, **검증도 같은 표본 수로** 잰다. **표본 수가 다르면 좋아졌다는 말이 성립하지 않는다.**
- ⭐ **좋아졌다를 절대 증분이 아니라 잔여 여지로 잰다** — **`고친 뒤 결과가 꾸준히 강하고, 기준 대비 실질적으로 개선되었으며, 개선의 여지가 얼마나 있었는지까지 감안했을 때만 확신으로 친다.`** ⚠️ **그래서 판정이 불리언이 아니게 되고 자동 병합이 불가능해진다**(재구성).
- ⭐ **고칠 수 없다고 말하는 자리를 만들었다** — 한 번도 재현되지 않으면 **`재현 불가`로 티켓을 세워 두고 사람에게 넘긴다**: **`관측할 수 없는 행동을 고치려 들지 않는다.`** ⚠️ **대가는 드문 실재 버그가 미해결로 남는 것이고, 얻는 것은 고치지도 않고 고쳤다고 적히지 않는 것이다**(재구성).
- ⭐ **PR 을 둘 연다** — 수정 하나, **재현에 쓴 평가 하나.** 후자가 **영구 회귀 테스트**가 된다. **고친 것과 고쳤음을 아는 방법을 같이 남긴다.**

### ⭐ 루프보다 밑바닥이 어렵다 — 회사가 직접 그렇게 적는다

**`반복해서 시나리오를 재생할 수 있는 평가 프레임워크가 필요하다. 티켓마다 띄울 만큼 값싼 스테이징 환경이 필요하다. 버그 보고에서 실패한 바로 그 대화까지 거슬러 가는 추적성이 필요하다. 그리고 API 가 있는 티켓 시스템이 필요하다.`**

⚠️ **이 넷이 없으면 에이전트를 여섯 개 붙여도 돌아가지 않는다**(재구성). ⭐ **특히 `값싼 스테이징`은 취향이 아니라 전제다** — **티켓 하나에 실행이 열 번에서 스무 번 들기 때문에 값이 싸지 않으면 이 방법 자체가 성립하지 않는다.**

### ⭐ 그리고 사람의 자리를 병목이 아니라고 부른다

**`사람이 되돌릴 수 없는 행동을 쥔다 — PR 병합과 티켓 종료를 포함해.`** ⚠️ **자동 리뷰 피드백만 에이전트가 정해진 횟수 안에서 반영하고, `사람 댓글은 에이전트가 답할 수는 있어도 그로 인한 코드 변경은 사람이 운전한다.`** ⭐ **회사는 그 자리를 `가장 중요한 일`이라고 적는다** — **`무엇이 좋은 행동인지 정의하고, 변경을 검토하고, 무엇을 내보낼지 결정하는` 일.**

### 다음 사이클 — 확장 (2순위)

**Block 기능 1/4.** ⭐ **`키를 만드는 난수`**(COLDCARD) — **결함의 인과가 한 줄까지 좁혀져 있고 수치가 전부 있다**(2^32 · 2^31 · 8만~12만 · 40바이트 → 4바이트). ⚠️ **실패 경로 그림 재료로 이보다 좋은 것이 드물다.** 그다음 `에이전트가 팀으로 일한다` · `노는 컴퓨트를 모은다`.

⚠️ **읽을 것이 남았다** — `Square 본업`(결제·정산·대출 심사) 자료를 아직 못 찾았다. ⏳ 미독 — `BlockTabBench` · `Buzz 에이전트 설정` · `Buzz 프로젝트` · `Microfilm`.
