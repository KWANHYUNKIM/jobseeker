...
**없다 — Linear 를 완주시켰다(도메인 2 · 기능 2 · `status: done`).** 회사 152개, **큐 0/3.**

### 이번 사이클이 채운 것 — `swap-the-index-not-the-query`

**더 붙이는 것으로는 안 되는 문제**가 있다는 것을 수치로 보인 기록이다. 읽기 복제본을 늘려도
요청마다 드는 교집합 계산은 그대로다 — 복제본은 요청을 나눠 받을 뿐 요청 하나가 하는 일을
줄이지 않는다. 그래서 튜닝을 그만두고 **질의의 모양**을 바꿨다.

### 행을 거르는 대신 집합을 맞물린다

포스트그레스는 넓어지는 ID 범위에 배열 겹침 술어로 **상관없는 행까지 읽고 버렸다.** turbopuffer
는 속성마다 정렬된 ID 집합(포스팅 리스트)을 두고 **집합 교집합을 바로** 구한다. GIN 색인도
살펴봤지만 이 규모에서는 **쓰기 때 드는 값이 말이 안 됐다.**

### 정확성이 걸린 길은 검증하고 갈아 끼운다

그림자 모드로 두 길을 나란히 돌려 결과를 견주고, 새 색인을 못 쓰면 포스트그레스로 되돌아가며,
경계는 일부러 겹치게 두고 중복을 지운다. **복제 지연 p50 약 1초** 때문에 최근 범위는 계속
포스트그레스가 권위 있는 출처다.

### 도메인을 억지로 늘리지 않은 회사로 남겼다

기술 글이 사실상 한 편이라 **도메인 둘·기능 둘로 완주**시켰다. 없는 것을 지어내지 않는다는
규칙이 도메인 수에도 적용된다는 선례다.

### 다음 사이클 — ⚠️ 후보 조사 (큐가 0/3 이다)

남은 대기는 **Mux** 하나뿐이었는데 그것도 이제 맨 위다 — 표를 확인하고, 판정 기준을 이어 쓴다:
회고를 먼저, 되돌린 기록, 자기 것을 쓰지 말라고 적는 글, 사고를 공개하는 글, **상류에 고쳐
보낸 기록**. **형식이 아니라 대가가 기준이고, 본문을 열어 판정한다.**
