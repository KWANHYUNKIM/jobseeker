...
**Railway — 기능 하나를 채웠다(`zero-touch-provisioning`).** 빈 도메인 둘. 회사 136개, 큐 2/3.

### 클라우드를 떠나면 남는 일

하이퍼스케일러가 대신 해 주던 일 중 가장 자주 반복되는 것이 새 서버를 쓸 수 있게 만드는
일이다. 사람이 웹 KVM 을 열어 설치 화면을 보고 있으면 서버 수만큼 시간이 는다 — 랙에 꽂는
데까지만 손을 쓰고 **그다음은 gRPC 호출 하나**로 끝낸다. 대개 1분 안에 선다.

### 배치 절차가 고장 검사를 겸한다

레드피시로 훑은 실물이 알려진 구성과 다르면 **워크플로를 실패시킨다.** 그렇게 불량 램과
잘못 꽂힌 NIC 을 잡았다 — 안 서는 이유가 설정인지 고장인지 가르는 자리를 절차에 넣었다.

### 값으로 고른 방법이라고 스스로 적는다

설치기가 진행 상황을 안 알려 주니 **화면을 찍어 모델에게 지금 어느 단계냐고 묻는다.** 더 나은
방법이 있다는 걸 인정하면서도, 서버 50대를 매분 긁어도 1달러가 안 든다는 이유로 이 방식을
골랐다고 밝힌다. 이 엔진에서 "더 나은 길을 알면서 값 때문에 안 간다"고 적은 회사는 드물다.

### 거의 다 만든 회사가 가져다 쓴 것

제어면(MetalCP)은 **템포럴 워크플로** 위에 세웠다 — 데이터센터부터 CDN까지 직접 만든 회사가
워크플로 엔진은 남의 것을 쓴다. 이 엔진에 이미 있는 회사가 부품으로 등장하는 자리다.

### 다음 사이클 — 확장 (Railway 의 빈 도메인)

`엣지도 남의 것을 안 쓴다`. 히카리 넷(edge-cp·keeper·데이터면·WASM 게스트), POP 60·노드 180,
WASM 10μs·스레드 로컬 풀, BGP 커뮤니티가 손에 있다.
