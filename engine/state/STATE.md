...
**Rerun — 기능 하나를 채웠다(`timelines-latest-at`).** 빈 도메인 둘. 회사 135개, 큐 0/3.

### 맞추지 않기로 한 것이 설계의 전부다

1000Hz 와 4Hz 를 한 표에 세우려면 어딘가에 줄을 세워야 하는데 **그 순간 원래 언제 왔는지가
사라진다.** 그래서 각자 온 시각에 그대로 적고, 물어본 시각 이전의 가장 최근 값을 성분마다
모아 온다 — 정렬을 저장할 때가 아니라 읽을 때 만든다.

10Hz 점구름과 1Hz 텍스처가 한 화면에 담기는데, 그 텍스처는 몇 프레임 전 것이다.

### 시간이 하나가 아니다

기록 한 번이 켜져 있던 축 모두에 달린다 — 실제 시각·순번·프레임 번호·센서 시각. 같은 데이터를
프레임으로 훑다가 센서 시각으로 갈아탈 수 있다. 정적 데이터는 아직 만들어지지 않은 축에서도
나오고 같은 종류의 시간 데이터를 가린다.

### 만료되지 않는 값이 양날이다

성분이 안 지워지니 색만 다시 기록해도 위치가 이어진다(부분 갱신). 대신 **센서가 멎어도 마지막
값이 계속 보이고**, 순서가 뒤바뀐 기록이 앞선 질의에 어떻게 반영되는지는 문서가 다루지 않는다.

### 앞 사이클과 이어지는 자리

이 유연한 모형이 곧 느림의 원인이다 — 성분마다 장부를 뒤지느라 100만 점에 600ms 가 걸렸다.
**모형은 그대로 두고 저장(열 청크)과 질의(캐시) 쪽에서 갚는** 것이 남은 두 도메인이다.

### 다음 사이클 — 확장 (Rerun 의 빈 도메인)

`화면이 도는 동안 다시 꺼낸다`. 캐시·변경 기록 무효화·잘게 나눈 잠금·픽셀 합치기가 손에 있다.
