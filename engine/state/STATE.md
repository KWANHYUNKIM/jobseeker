...
**카카오페이** — 첫 기능 `unknown-is-a-state`(끝내 모르면 사람에게 넘긴다)를 끝냈다.
**도메인 2 · 기능 1.** 큐 0/3. 남은 도메인은 **운영을 멈추지 않고 레거시를 바꿔치운다.**

### 둘로 가르면 반드시 한쪽으로 틀린다

타임아웃을 **실패로 간주하면 성공한 거래를 되돌리고**, 성공으로 간주하면 안 된 거래를
됐다고 한다. 그래서 **모름을 상태로 만드는 것 자체가 사고를 막는 설계**다.

### 안전장치를 받는 쪽에 둔다

요청자가 아무리 조심해도 중복은 생기니 **제공자 쪽을 멱등으로**(`tx_key`) 만든다.
**리스테이트가 부르는 쪽 저널로** 같은 성질을 얻는 것과 짝이 된다 — **한쪽은 기록으로,
다른 쪽은 약속으로.**

### 완결을 주장하지 않는다

**보상 트랜잭션마저 타임아웃되면 다시 모름**이라는 무한 회귀를 적고, 자동으로 닫으려
애쓰는 대신 **끊고 사람에게 넘긴다.** 이 원장에서 드문 태도다.

### 사람을 끝에 두는 설계일수록 그 부담의 크기가 중요한데

**Unknown 이 하루 몇 건인지, 대사에 사람이 얼마나 드는지, 배치가 얼마나 자주 도는지**가
없다. 정량이 통째로 비어 있어 **감당 가능한 규모인지 밖에서 알 수 없다.**

### 제목만 보고 붙이지 않았다

`event-driven-architecture` 를 이 도메인에 붙이려다 열어 보니 **슬랙 웹훅 처리 이야기**였고
**전달 보장·순서·중복 제거를 다루지 않는다.** `r2dbc-connection-pool-missing` 도 좋은
추적기였지만 **다른 이야기**라 붙이지 않았다.

### 다음 사이클

확장 — 남은 도메인(AOP Shadow). **수치가 촘촘하다**(처리량 20% 감소 · P99 548 → 1,305ms ·
BigDecimal `100.0` ≠ `100.000`). 큐가 0/3 이라 그 뒤에 후보 조사가 뜬다.
