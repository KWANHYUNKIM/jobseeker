...
**없다 — `in_progress` 0 · 완료 53곳 · 큐 0/3.** 317 에서 **비교 문서
`what-agents-may-not-do`(에이전트에게 무엇을 못 하게 하는가 — 손을 묶었나 눈을 가렸나
머리를 좁혔나)를 썼다. 비교 문서 31편.**

**⚠️ 재료를 파일에서 다시 읽고 나서 목록이 바뀌었다**(314 의 교훈이 또 맞았다).
- **Adevinta 를 뺐다.** 메모에는 '반대편 재료' 로 적혀 있었는데 실제 기능
  (`pilot-with-real-work`·`fork-and-strip`)은 **남의 AI 도구를 자기 백로그로 재고 남의
  라이브러리를 뜯는 이야기**라 이 축과 관계가 없다. **기억이 아니라 파일이 맞았다.**
- **monday.com `Morphex` 는 기능 이름이 아니다** — 회사 파일 안의 도구 이름이고, 기능 key 는
  `agent-with-guardrails` 하나다.
- **대신 안 세던 재료가 둘 나왔다** — Snap `agentic-code-search` 의
  *"Agents inherit their human user's repo access"* 와 Wix `agent-ready-org` 의
  *"The action space is unbounded. More capability, more autonomy, more risk"*.

**실제로 선 축 넷** — ① **손을 묶는다**(닿는 범위: Grab Palana · Snap 코드 검색 · Wix AirBot)
② **머리를 좁힌다**(추론 방식: monday.com 잘게 자르기 · Wix 이유 요구 · Grab SOP)
③ **마지막 한 걸음만 안 준다**(병합·승인: Snap 90%/10분 · monday.com 최소 두 명 · AirBot 15%)
④ **반대편, 안 막고 여는 쪽**(Wix Agent Ready Repos · Grab LLM-Kit).

**⭐⭐ 가장 센 관찰 — 같은 회사가 두 방향으로 가고 그 두 글이 서로를 안 부른다.**
Grab(Palana 조임 ↔ LLM-Kit 풂) · Wix(AirBot ↔ Agent Ready Repos) · Snap(CodePal ↔ 코드 검색).
**규칙으로 읽으면 '만들 것에는 느슨하고 닿을 것에는 빡빡하다'(추정).**
**⭐ 그리고 값을 세는 방향이 한쪽뿐이다** — 푼 쪽의 이득은 숫자가 있는데(2주→1시간 ·
월 675시간 · 8 person-years→6 person-months) **조인 쪽의 대가는 여덟 중 하나도 숫자가 없다.**

**Grab SOP 는 재료로만 썼다** — 기능이 아니라 `grab.json` 의 `open_questions` 에서 인용했고,
문서 안에 **버린 대안 0 · 한계 0 이라 기능으로 안 썼다는 사실을 함께 적었다.**
**310 의 `what-guards-the-gate` 와는 머리말에서 서로 가리키게 했다**(거기는 결과 앞의 문,
여기는 일하는 동안의 제약).

---

**다음 사이클 — 이번엔 정말로 후보 조사다.**
- **⚠️ `--gaps` 가 네 사이클 연속 [후보 조사]를 가리켰고 네 번 다 미뤘다**(310·312·313·317).
  **비교 문서 재료가 이제 남아 있지 않다** — 이번에 가장 굵던 축을 썼다. **미룰 근거가 없다.**
...
- **후보 조사 성적은 나쁘다** — 누적 **열두 곳 중 둘**(298: 6→1, 305: 3→1, 311: 3→0).
  **그래도 이번엔 한다.** 목표는 3곳이고, **한 편은 반드시 열어 본다**(제목만 보면 298 의
  Dream11 을 올렸을 것이고, 목록만 보면 305 의 Cybozu 를 떨어뜨렸을 것이다).
- **⚠️ 후보를 올리기 전에 `index.json` 완료 목록과 회사 단위로 대조한다**(운영 사실 ⑲),
  **그리고 이미 있는 회사면 그 회사 파일의 `sources` 부터 대조한다**(314).
- **분포** — **53곳 전부 `done`.** US 22 포화 · 한국 아홉 · 일본 넷 · EU 일곱 · 중국 둘 ·
  동남아 하나 · 인도 하나 · 아프리카 하나 · 라틴아메리카 0(재시도 금지). **비교 문서 31편.**
