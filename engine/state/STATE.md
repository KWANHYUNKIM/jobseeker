...
**Chroma — 기능 둘을 채웠다.** `objstore-index-execution` 과 `wal3`(연결 리스트를 파일로 옮겨
로그를 만든다). 회사 126개, 큐 0/3. 빈 도메인 하나 — 멀티테넌시.

### 남의 저장소가 원자적 연산 하나를 열자 설계가 가능해졌다

객체 저장소에는 원래 원자적 갱신이 없어 로그를 얹으려면 밖에 잠금 서비스를 둬야 했다. 2024년
11월에 S3 조건부 쓰기가 나오자 **없을 때만 만들기(If-None-Match)와 그대로일 때만 덮어쓰기
(If-Match)** 가 생겼고, 그것이 곧 비교 후 교체다.

그러면 메모리에서 하던 것을 파일로 옮길 수 있다 — 조각 파일이 연결 리스트의 노드, 매니페스트가
컨테이너다. **1994년 락 프리 큐 알고리즘을 그대로 쓰고 포인터가 파일이 될 뿐**이라고 적는다.
시제품에서 운영까지 넉 달.

### 지우는 곳을 하나로 모으고 두 구현으로 감시한다

"지우는 곳이 하나뿐이라면 잃는 것도 거기"라는 문장으로 트리밍을 유일한 삭제 경로로 둔다. 그리고
로그가 조용히 틀리는 것을 잡으려고 **실제 로그를 유지하는 구현과 setsum 체크섬으로 추적하는 구현을
나란히 돌린다** — 살아 있는 것과 수거된 것의 다이제스트 합이 전체와 같아야 한다. 실제로 멱등적
수거에서 이미 지운 것을 깨끗이 감지하지 못하던 버그가 그 검사에 걸렸다.

이 엔진에서 두 구현을 나란히 돌려 서로를 검증하는 사례가 안티테시스(방 안의 결정론)에 이어 둘째다 —
저쪽은 세계를 재현해 잡고 여기는 같은 값을 두 방식으로 계산해 잡는다(재구성).

### 이름을 대며 물린 대안에 이 엔진의 회사가 있다

카프카(디스크 함대를 운영해야 한다) · **WarpStream(오픈소스가 아니다)** · CDC(처음부터 지어야 한다).
같은 문제를 푼 제품이 거절 대안으로 등장하는 자리가 이번 큐에서 둘째다(앞은 Bun 의 TigerBeetle).

### 다음 사이클 — 확장 (마지막 빈 도메인)

멀티테넌시를 채우면 Chroma 가 done 이 된다. `distributed-chroma-byoc` 과 `transactions` 가 후보다.
