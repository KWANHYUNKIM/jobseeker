...
**DuckDB — 회사 프로파일까지 썼다(in_progress).** 회사 118개, 큐 2/3.

도메인 셋을 세웠고 기능은 아직 0개다.

- **호스트 프로세스 안에 산다** — `Why DuckDB`
- **네트워크 너머를 로컬 파일처럼 읽는다** — `Asynchronous I/O in DuckDB`(2026-07-31)
- **문법을 계속 고칠 수 있게 만든다** — `Your Database Deserves a Better Parser`(2026-08-20)

### 파는 회사가 구조적으로 따로 서 있다

이 엔진의 오픈소스 인프라 회사들은 회사가 제품을 팔고 오픈소스가 그 앞단이었다(ClickHouse ·
Grafana Labs · Turso). DuckDB 는 지식재산과 상표가 **비영리 재단**에 있고 MIT 라이선스를
되돌릴 수 없이 유지한다고 못 박는다. 개발비는 회원 회비다 — 실버 연 1만 유로 이상, 골드 연
10만 유로 이상. 상업 클라우드를 파는 MotherDuck 이 회사가 아니라 **골드 회원**으로 이름을
올린 것이 그 관계를 그대로 보여 준다.

### 도메인 셋이 한 선택에서 갈라진다

서버가 되지 않기로 한 것. 프로세스 안에 사니 데이터를 옮기는 값이 사라지지만, 남의 집에 세드는
값을 낸다 — 의존성을 하나도 못 들이고, 엣지 기기부터 큰 서버까지 같은 코드가 돌아야 한다. 데이터가
S3 에 있으면 그 이득이 뒤집히므로 원격 읽기를 감추는 문제가 따로 서고(스레드 풀을 둘로 가른 이유가
그 경계다), 문법을 계속 늘리는 프로젝트라 파서를 갈아엎는 일이 개발 속도의 문제로 남는다.

### 대가를 먼저 적는 글이다

미리 읽기는 디코딩이 느리고 네트워크가 빠르면 메모리에 쌓인다고 적고 설정 손잡이를 연다. PEG 파서는
괄호가 안 맞는 질의에서 지수적으로 느려졌다고 적고 팩랫으로 메모리와 바꿨다고 밝힌다(10.6초 → 0.001초).
자동으로 다 풀리지 않는다는 것을 인정하는 자리가 두 곳 다 있다.

### 다음 사이클 — 확장 (DuckDB 의 빈 도메인)

DuckDB Labs 사이트가 403 이라 재단 회비 말고 어떤 수익원이 있는지는 못 봤다. 도구 한계로
