...
**Restate** — 첫 기능 `journal-then-replay`(먼저 저널에 적고, 되짚을 때는 시작한 코드로
되짚는다)를 끝냈다. **도메인 2 · 기능 1.** 큐 1/3. 남은 도메인은 **엔진을 밑바닥부터 자기
손으로 짓는다.**

### 적는 것이 하는 것보다 먼저다

체크포인팅은 **경계의 상태만** 담아서 연산 셋 중 둘 뒤에 죽으면 셋을 다 다시 돌린다 —
**'앞의 둘이 일어났다는 것을 아무것도 기록하지 않았기 때문이다.'** 그래서 **실행이
진행되기 전에** 복제 로그에 적고, 재시도는 끝난 단계의 결과를 받아 **안 끝난 그 단계로
빨리감기**한다. **남들을 이름 대어 든다** — LangGraph·Google ADK·Mastra.

### 대가가 즉시 따라온다 — 코드가 변하면 안 된다

되짚으려면 **'그 요청을 실행하는 코드는 결코 행동이 바뀌어서는 안 된다.'** 맨 앞에 단계
하나만 더해도 재생 중인 요청은 **저널에 그것이 없어 어떻게 진행할지 모른다.** 이 회사가
**자기 방식의 대가를 글 한 편으로 정면에서 다룬 드문 자리**다.

### 남의 값은 숫자로 세고 자기 값은 인정에서 멈춘다

템포럴 방식의 값을 **숫자로 센다** — **'요청이 한 달을 돌고 그 달에 파괴적 변경을 다섯 번
하면 워커 다섯을 동시에 돌려야 한다.'** 그런데 **자기 제안(같은 파드에 두 컨테이너)의
유지 부담은 재지 않는다.** 앞 사이클의 엔진 글도 같았다 — 삼각형은 그리는데 수치가 없다.
**이 회사 글의 일관된 모양으로 적어 뒀다.**

### 직접 지을지 얹을지가 칸마다 다르다

아래층 로그는 **직접 짓는다**면서(포스트그레스도 카프카도 아니다), 버전 고정은 **람다가
이미 가진 불변 버전에 얹는다.** 같은 회사가 칸마다 다르게 고른다.

### 다음 사이클

확장 — 남은 도메인(엔진). 물린 대안 둘과 삼각형 인정이 이미 있고, `distributed-restate-a-first-look`
과 `geo-replicated-apps` 가 남아 있다.
