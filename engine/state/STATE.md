...
**DoorDash** (US · 커머스) — 도메인 3개 · **기능 2개** · eras 3시기. **남은 것은 배차 하나이고, 그것이 완주를 막고 있다.**

### 이번 사이클 — Flux `에이전트를 만들지 않고 에이전트가 설 자리를 만든다`

건진 것 넷:

- ⚠️ **회사가 진짜 문제를 다시 정의한다** — 어려운 것은 에이전트가 코드를 쓰게 하는 일이 아니다(**`that's mostly solved`**). **환경·도구·권한·통합·제약을 주는 일이다.** 그래서 에이전트가 아니라 **그 주변의 원시 요소**를 만들었다 — 소제목이 **`Primitives, not workflows`** 다.
- ⚠️ **호스팅 에이전트가 강요하는 것을 `hard tradeoff` 라 부르며 둘 다 거부한다** — `Either send sensitive code and execution context to a third party, **or open a path from that third party back into internal systems**.` **양쪽 다 내주는 것이 있고 중간이 없다.**
- ⚠️ **노트북을 떠난 진짜 이유는 성능이 아니라 권한이다** — 노트북은 SSH 키·VPN 세션·인증된 도구에 광범위하게 닿는데, **사람이 쓸 때는 판단이 그 권한을 제한하지만 자율 에이전트에게는 그 판단이 없다.**
- ⚠️ **가장 실용적인 결정은 플레이북이 두 종류의 단계를 섞는다는 것이다** — 에이전트 단계는 **유연성과 판단**을, 결정론적 단계는 **예측 가능성·낮은 비용·쉬운 검증**을 준다. **요구가 바뀌면 로직을 둘 사이에서 옮길 수 있고 워크플로를 다시 설계하지 않아도 된다.**

**두 회사가 각각 `보이는 자리` 에서 답을 찾았다** — DoorDash 는 **비공개 채널을 공개 스레드로** 옮기자 채택 패턴이 바뀌었고, Duolingo 는 **슬랙으로 옮기며** `Convenience wins` 를 배웠다. **둘 다 기술이 아니라 조직의 발견이다.**

⚠️ **채택 수치는 많은데 품질 수치가 없다** — 한 달 13만 작업·주당 코드 리뷰 2.5만 건·플레이북 300개 이상인데 **실패율·되돌린 비율·비용이 없다.** Duolingo 는 첫 시도 CI 통과율(76%)까지 적었다.

### 다음 사이클 — 배차, 기능보다 자료가 먼저다

⚠️ **이 회사의 본체인데 `tech` 가 0 이다**(검증 경고). `--gaps` 가 **주문과 배차를 실시간으로 흘린다** 를 부를 텐데, **기능을 쓰기 전에 자료를 찾는 것이 그 사이클의 일이다** — **네 번 연속으로 통한 규칙**이다(Fastly 보안 · Monzo 플랫폼 · ClickHouse 엔진 · 1Password 프로비저닝).

찾을 곳: **`Iguazu` 이벤트 플랫폼** · 서비스 메시(Linkerd→Envoy, 2025) · Kafka 아키텍처(2026-05) · Dasher 온보딩 플랫폼(2026-03). ⚠️ **브라우저로 열어야 한다** — 이 블로그는 WebFetch 가 403 이다.
