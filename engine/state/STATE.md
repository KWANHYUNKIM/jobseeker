...
**없다 — Wise 를 완주했고 큐가 비었다(0/3).** 다음 사이클은 `--gaps` 가 **3순위(후보 조사)**를 부른다.

### 이번 사이클 — 와이즈 마지막 도메인 `관측 스택을 옮기고 나서야 진짜 비용을 만난다`

`silent-compaction-backlog` **성공한 이전 뒤에 조용히 자란 병목을 찾는다** 를 쓰고 **완주 판정까지 같은 사이클에서 끝냈다**(도메인 3/3 · 고아 연결 없음 · 수익원 둘 다 기능과 연결).

**이 글이 이 회사에서 유일하게 대가를 직접 적는 글이었다.** 앞의 두 기능(스택 소개 글 기반)은 tradeoff 가 대부분 재구성이었는데, **Mimir 압축 글 하나에서는 decisions 일곱 개가 전부 confirmed** 로 채워졌다. **한 회사 안에서 글의 종류가 등급을 가르는 것을 직접 봤다** — `우리 스택은 이렇다` 는 글에는 대가가 없고 `이걸 놓쳤다` 는 글에는 있다.

건진 것 셋:
- ⚠️ **증상 넷이 어느 것도 원인을 안 가리켰다** — `nor did it cross our minds as the likely cause`. **원인이 하나인데 증상이 흩어지면 묶는 방법이 없다**는 것을 회사가 인정한다.
- ⚠️ **감시 도구를 쓴다는 것과 그 도구가 모든 컴포넌트를 본다는 것은 다르다** — Grafana 믹신 대시보드·알림·런북을 쓰고 있었는데 압축기 신호가 없었다. **전환점은 깃허브 이슈의 댓글 하나였다.**
- ⚠️ **회사가 자기 우선순위 실수를 적는다** — `inadvertently leaving the compactor's optimisation lower on the priority list`. **새 도구를 들이면 안 보이는 컴포넌트가 생긴다.**

### 다음 사이클 — 3순위 후보 조사

큐가 **0/3** 이라 새 이름을 두드린다. 안 두드린 것: **Fastly**(CDN — Cloudflare 와 겹치지만 요금 단위가 다르다).

**되짚기가 새 이름보다 수확이 좋았다는 것을 이 세션이 여러 번 보여 줬다** — Booking.com·Trainline 은 `Medium 이라 못 읽는다` 목록에서 브라우저로 되살렸고, Tailscale·Ably·Temporal 은 `첫 화면만 보고 마케팅이라 판정한` 의심 목록을 뒤집어 건졌다. **먼저 `### 자료 접근 지도` 와 `## 재시도 안 함` 을 훑고 나서 새 이름으로 간다.**

⏳ **와이즈에 남은 두 갈래**(연표 사이클이나 보강 사이클이 맡을 것): **2022년 스택 글**(2025년 글과 나란히 놓으면 시간축) · **`our earlier post about the state of our CI/CD pipeline`**(Octopus 를 버린 대가).
