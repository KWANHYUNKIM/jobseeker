...
**없다 — `in_progress` 0 · 완료 56곳.** 336 에서 **Sansan 을 닫았다**(도메인 5개 · 기능 6개).

**닫은 근거** — 남은 글 중 가장 기술적으로 보이던 `レイヤードアーキテクチャを振り返る`
(2019-04-22)를 열어 봤는데 **특정 시스템의 회고가 아니라 계층형 아키텍처 일반에 대한
에세이**였다. 제품도 규모도 수치도 없고 예시가 *"簡単なCRUDしか行わないシンプルなアプリ
ケーション"* 같은 가정이다. **대가는 있지만**(완화책이 계층화의 이점을 잃게 한다 · 갱신에서
비즈니스 로직을 우회하면 검증이 화면마다 흩어진다 · *"レイヤーには分けられてはいるものの、
開発者への浸透がされず責務が曖昧になって後からのメンテナンスが困難になった"*) **이 엔진은
공개 자료로 그 회사가 무엇을 어떻게 만들었나를 재구성하는 것이므로 일반론은 재료가 아니다.**
**⚠️ 이 회사에서 대담·에세이 형식은 세 번 다 버린 대안이 0 이었다.**

**닫으면서 `business_model` 에 적은 것**(이 사이트의 정리라고 밝혔다)
- **⭐⭐ 다섯 도메인이 같은 회사의 다섯 층이다** — 개발 환경 · 인식 · 공정 실적 · **실물** ·
  십 년 된 앱. **대부분의 회사는 소프트웨어 안에서 끝나는데 여기는 우편물이 도착하고 사람이
  옮긴다.** 표로 정리해 넣었다.
- **⭐ 글의 두께가 층마다 다르다** — 개발 환경과 앱 쪽은 **대가를 아낌없이 적고**, 인식 쪽은
  **성과 중심**(NineOCR 편은 버린 대안도 한계도 0), Digitization 쪽은 **대담이 많다**(추정).
- **⭐ 자기가 만든 것을 안 쓴다고 적는다**(초해상). **⚠️ 그런데 가장 아프다고 말한 항목
  (이메일·전화)의 정확도는 공개하지 않는다** — 공개한 것은 주소 98.3% 하나다.
- **⭐⭐ 다섯 번 다 점진을 골랐는데 버린 이유가 매번 달랐다** — 표로 넣었다(개발을 안
  멈추려고 / 코드 프리즈가 길어서 / 위험을 나누려고 / 한 번에 못 옮겨서 / 다시 만들 수 없어서).
  **⭐ 그리고 마지막 둘은 AI 를 이관 인력으로 쓴다** — 하나는 이미 돌리고 있고, 하나는
  **앞으로의 개선에 기댄다고 적으면서 그것이 불확실하다는 것도 함께 적는다.**

---

**다음 사이클 — `--gaps` 는 [신규 = freee]를 가리킨다. 그런데 비교 문서를 먼저 쓰는 편이 낫다.**

**⭐⭐ 이관·되돌리기 축의 재료가 다섯이나 쌓였고 전부 최근 사이클에서 나왔다.**
① **320 `leave-what-costs-more`(Paytm)** — 옮기다 실패한 것을 재처리 않고 **두고 갔다**
  (재처리하면 50억 객체를 다시 훑어 비용 2배). → `reversible-first.md` 의 *"되돌리지 못해
  앞으로 밀고 나간 사례가 없다"* 와 `where-it-piles-up.md` 의 **갚지 않고 이사한 쪽**.
② **326 `know-both-versions`(Freshworks)** — 끝나고 **if/else·몽키패치·버전별 캐시 키를
  지웠다**고 적는다. → `reversible-first.md` 의 *"언제 롤백 경로를 닫는지 정한 곳이 없다"* 의
  **첫 재료**.
③ **333 `paper-does-not-undo`(Sansan)** — **되돌릴 수 없는 것이 화면 밖에 있다**
  (*"システムのステータスだけを戻したとしても、実際の原本はもう別の場所にあるかもしれない"*).
  **그 문서에 전혀 없던 축이다.**
④ **334 `layer-by-layer-async`(Sansan)** — **되돌릴 자리를 남기려고 중간 상태의 비용을
  감수한다**(async 를 Rx 로 감싸면 11줄 대 1줄, *"本末転倒なパターン"*).
⑤ **335 `coordinator-in-between`(Sansan)** — **이관 계획이 도구의 미래에 걸려 있다고
  적는다**(3년 계산 + AI 개선에 대한 기대와 그 불확실성).
**⚠️ 그리고 기존 문서 `reversible-first.md`·`swap-while-running.md`·`where-it-piles-up.md`
셋과 겹치므로 반드시 먼저 읽고 서로 가리키게 쓴다**(310·317 에서 그렇게 했다).
...
- **⭐⭐ freee 와 SmartHR 이 정반대 순서로 같은 문제를 풀었다** — 인증·권한 기반을 떼어내는데
  **freee 는 모놀리스에서 곧장 4년**(후회로 적는다), **SmartHR 은 모듈러 모놀리스로 시작해
  Packwerk 로 경계를 먼저 보이게 한 뒤** 떼어낸다. **이것도 비교 문서 재료다.**
- **후보 조사 누적 열아홉 곳 중 일곱.**
