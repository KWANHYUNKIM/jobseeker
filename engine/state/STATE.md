...
**없다.** 완료 **77곳** · **큐 1/3**(Wise) · 비교 문서 33편.

**다음 사이클은 6순위(신규) — Wise 다.** PROMPT.md 3단계부터.
⚠️ **Medium 이라 목록이 안 온다 — 글 주소를 검색으로 먼저 찾아 브라우저로 연다.**
이미 아는 것: `wise-tech-stack-2025-update`(2025-02-12, 전문 읽음).
⚠️ **그 글은 스택 소개라 대가가 거의 없다** — **기능을 쓰려면 개별 주제 글을 더 찾아야 한다**
(CI/CD 이전 · 관측 비용 최적화 · 데이터 엔지니어링 발표 등이 본문에서 링크로 언급된다).

### ⭐⭐ HashiCorp 판정이 이번엔 근거가 단단하다

앞서 `첫 화면으로 판정하지 않는다` 를 끝까지 적용 안 했다고 적어 뒀고, **이번에 검색까지
해서 확정했다** — 나오는 것이 조직·문화·커리어 글뿐이다. **절차를 밟았고 결과가 같았다.**
**Tailscale·Ably·Temporal 은 뒤집혔고 ClickHouse·Capital One·HashiCorp 는 유지됐다** —
**되짚기가 항상 뒤집는 것은 아니고, 그래서 의미가 있다.**

### Wise 가 왜 좋은가 — 이전이 여섯 개이고 전후가 다 있다

Xcodegen→Tuist(**빌드 28초 → 2초**) · JSONNET→Helm(**클러스터 6 → 20+**) ·
CircleCI→GitHub Actions(**빌드 15% 단축 · 월 1,000시간 절약**) · Octopus→Spinnaker
(**카나리 5% · 30분 · 2024년에 수백 건 차단**) · Thanos→Mimir(**초당 600만 샘플**) ·
EC2→RDS. ⚠️ **다만 `무엇을 잃었나` 가 거의 없다** — 스택 소개 글의 한계다.

### ⭐ 재미있는 연결 — Wise 가 Temporal 을 쓴다

방금 판 회사(Temporal)를 **전환·복구 시험 자동화**에 쓴다고 적는다.
**비교 문서 재료가 될 수 있다.**
