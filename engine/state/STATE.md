...
**turbopuffer — 프로파일만 썼다.** 회사 143개, 큐 1/3(Deno 만 남음).
도메인 셋 중 둘은 아키텍처 문서로 채웠고 하나(색인)는 블로그를 안 읽어 tech 두 칸이 `unknown` 이다.

### 진실을 객체 저장소에 두면 나머지가 다 정해진다

메모리에 색인을 올려 두고 그것을 진실로 치면 빠르지만 데이터가 늘수록 값이 선형으로 는다.
여기는 **S3 를 원본으로 두고 나머지를 캐시로 본다** — 그러면 저장은 싸지고 지연은 늘며, 그
지연을 메모리·NVMe 두 층으로 되산다.

### 874밀리초와 14밀리초는 다른 제품이다

문서 100만 건 기준 차가운 질의가 중앙값 874밀리초(객체 저장소 3~4 왕복, 왕복당 약 100ms),
NVMe 에 캐시된 따뜻한 질의가 14밀리초다. **60배 넘게 갈리는 두 상태**를 같은 제품이 오간다.

### 기본을 강한 일관성으로 두고 느슨함을 팔지 않는다

쓴 것이 바로 다음 질의에 보인다 — 검색이 방금 넣은 문서를 못 보여 주는 흔한 문제를 기본값에서
없앤다. 원하면 최종 일관성으로 바꿔 10밀리초 아래도 가능한데 **최악의 경우 한 시간까지
낡을 수 있다**고 적는다.

### 쓰기 지연을 대놓고 내준다

초당 벡터 1만 개 이상을 받되 쓰기 지연이 높은 것이 그 대가이고, 네임스페이스마다 초당 WAL
항목 하나로 묶어 새 묶음은 커밋까지 최대 1초가 걸릴 수 있다.

### 다음 사이클 — 확장 (turbopuffer 의 빈 도메인)

`객체 저장소를 원본으로 삼는다` 부터. 왕복 수와 지연, WAL 제한, 일관성 선택이 다 손에 있다.
