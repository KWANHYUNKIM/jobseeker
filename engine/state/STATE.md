...
**Deliveroo** (UK · 커머스 — 음식·식료품 배달) — 도메인 3개 · **기능 1개**(`governance-in-the-pipeline-not-the-meeting`). **남은 도메인 둘은 본문을 아직 안 읽었다.**

### 이번 사이클 — `규정 준수를 회의에서 배포 파이프라인으로 옮긴다`

건진 것 넷:

- ⚠️ **문제가 AI 를 만드는 것이 아니라 팀마다 따로 만든 것을 다시 모으는 일이었다** — **비슷한 관측·보고가 나란히 만들어졌고, 엔지니어링과 법무 사이의 거버넌스가 급격히 복잡해졌으며, 규정 준수가 팀별 맞춤 작업으로 자라났다.** **중복은 비용이 아니라 확장의 상한이었다**(재구성).
- ⚠️ **강제 지점을 스테이징에 뒀다** — 프로덕션에서 막으면 이미 만든 것을 되돌려야 하고 경고만 남기면 아무도 안 고친다(재구성). **회사가 `일시적으로 차단한다`(temporarily block) 고 적는 것도 같은 결 — 완전히 막는 대신 고칠 시간을 준다.** ⚠️ **다만 이것은 `현재 작업 중` 이라 지금 도는 것은 메타데이터로 준수를 증명하는 단계까지다.**
- ⚠️ **정책을 문장이 아니라 데이터셋으로 만든다** — 3면 마켓플레이스라 **고객·파트너·라이더 에이전트에 같은 톤 정책**이 걸리므로 하나의 벤치마크가 전부를 잰다. **법무·프라이버시가 프로젝트마다 회의에 들어오는 대신 버전 관리되는 데이터셋에 직접 기여한다.**
- ⚠️ **이 사이클에서 찾은 이 구조의 한계 — 강제되는 것과 안 되는 것 사이의 선을 정책의 중요도가 아니라 검사 가능성이 긋는다.** `모델 버전을 이렇게 표기하라` 는 기계가 검사하지만 `이 모델을 이 용도에 써도 되는가` 는 아니다. **회사는 그 자리에 사람의 리스크 평가를 인벤토리에 기록하게 해 뒀는데, 기록하는 것과 강제하는 것은 다르다**(재구성).

### ⚠️ 이 회사의 자료에서 눈에 띄는 것 — 수치가 하나도 없다

**에이전트 수·트레이스 양·평가 통과율·차단된 배포 수가 전부 없고, 인벤토리 화면조차 `예시` 라고 밝힌다.** **`혁신과 거버넌스는 대립하지 않는다` 는 주장을 뒷받침할 숫자를 이 글은 내놓지 않는다.** 그래서 **metrics 의 첫 줄에 `이 글에는 수치가 하나도 없다` 를 그대로 적었다** — 없는 것을 없다고 적는 것도 기록이다.

### 다음 사이클 — 같은 회사의 남은 도메인

`--gaps` 가 **확장**을 다시 부른다(`무엇이 실제로 나아졌는지 판별한다`). ⏳ **본문을 아직 안 읽었다** — `r.jina.ai` 를 앞에 두고 브라우저로 받는다:
- `https://deliveroo.engineering/2025/02/06/experimentation-principles.html` — ⭐ **Etsy 의 `부분의 합이 전체가 아니다` 와 견줄 자리**
- `https://deliveroo.engineering/2023/05/17/3ds-optimisation.html` — 승인률과 사기의 맞바꿈
- `https://deliveroo.engineering/2025/07/02/deliveroo-ml-platform.html` — 첫 도메인의 보강 재료가 될 수 있다

⚠️ **큐가 0/3 이다.** 이 회사를 다 판 뒤 **3순위(후보 조사)** 가 걸린다. ⏳ 되짚을 목록 — `www.meesho.io/blog` · `blog.zepto.com` · `bytes.swiggy.com` · `www.gojek.io/blog`(429) · `tech.cygames.co.jp` · `www.scylladb.com/blog`. ⏸️ Snyk 보류 — `New Agentic Architecture for Snyk Agent Fix` 를 열고 결론 낸다.

### ⚠️ 비교 문서 재료 두 축 (초안 유지)

**① `AI 에이전트를 어디까지 믿나` 6곳** — Sentry(조직의 신뢰) / ClickHouse(접근 범위, **세 겹 중 한 겹만 기계가 강제**) / Duolingo(사람 리뷰 → **병목**) / Ramp(**사람의 신원**) / DoorDash(**primitives, not workflows**) / **Deliveroo(강제 지점을 배포 파이프라인에 — 그리고 `검사 가능한 것만 강제된다`)**. ⚠️ **ClickHouse 의 `세 겹 중 한 겹만 기계가 강제한다` 와 Deliveroo 의 `검사 가능성이 선을 긋는다` 가 같은 발견의 두 판본이다.**

**② `관리형 MySQL 의 한계` 3곳** — Etsy(Vitess) / Plaid(TiDB) / Paystack(MySQL 8.0 제자리). ⚠️ **Plaid 와 Paystack 은 같은 마감(2024-10 Aurora MySQL v2 지원 종료 발표)에 정반대 답을 냈다.**

⚠️ **비교 문서(8순위)는 이 사다리에서 영영 안 온다** — 3순위(큐가 0이면 후보 조사)가 큐를 다시 채우므로 **"위가 전부 비었다" 가 성립하지 않는다.** **PROMPT.md 의 문제이고 사이클이 고칠 것이 아니다.**
