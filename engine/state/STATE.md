...
**없다 — 쉰네 번째 후보 조사를 돌았다. 큐 2/3.** 다음 사이클은 **신규**(DeepL 또는 Fastmail).

### 목록 한 겹 아래에 진짜 블로그가 있었다

DeepL 은 `/blog` 만 보면 제품·마케팅 글이 지배해 접을 회사였다. **네비게이션에 따로 걸린
`/blog/tech`** 를 확인하고서야 알맹이가 나왔다 — 산티에서 배운 것(`/engineering` 섹션까지
본다)이 두 번째로 값을 했다. 물린 지표가 왜 실패했는지와 **포기한 것**("10.385 크기의
Times New Roman 같은 것은 없다")까지 적는 글이다.

### 첫 수정이 틀렸다는 것까지 적는다

Fastmail 은 자기들이 지운 사용자 폴더를 PR 번호까지 대어 적고, **PR 5961 이 불완전했고
PR 5988 에서야 실제 패턴을 재현하는 테스트와 함께 닫혔다**고 쓴다. 메타베이스에 이어
**사고를 공개하는 글이 판정을 통과시킨** 사례가 또 하나다.

### 본문을 열어야 접을 수 있다

Determinate Systems 는 제목이 유망했다("lazy trees", "평가가 빨라졌다"). 열어 보니 성과
수치는 있는데 **왜 그것을 골랐고 무엇을 물렸는지가 없다.** 접는 판정도 본문을 열고서야
근거가 선다.

### ⚠️ 의료 축은 다섯 번째로 비어 있다

Medplum 은 **접었고**(자료를 봤는데 대가가 없다), Lunit 은 **못 읽었다**(medium.com 403).
둘은 다른 사실이라 계속 갈라 적는다. 새로 막힌 곳 — Blender(403) · dbt Labs(홈에 블로그
링크 자체가 없다) · incident.io(`/blog` 네 번째 404).

### 검색 예산이 떨어졌다

이 세션의 WebSearch 200회를 다 썼다. 이제 **목록 페이지에 직접 물어 href 를 받는 것**만
남았다 — 원래 규칙(주소를 추측하지 않는다)이 그대로 유일한 방법이 됐다.
