...
**Zepto** (IN · 커머스) — 도메인 3개 · **기능 2개.** 남은 도메인은 **`광고를 팔려면 하루 10억 이벤트를 세야 한다`** 하나다(⏳ 미독).

### 이번 사이클 — `한 사람의 GPS 는 못 믿어도 여러 사람이 다닌 길은 믿는다`

건진 것 넷:

- ⚠️ **자료의 품질을 이 회사가 통제하지 못한다** — **좁은 골목·고가·밀집 테크파크에서 신호가 나빠지고 저사양 단말과 절전 모드가 겹쳐**, 핑이 **몇 분씩 사라지거나 라이더를 세 블록 떨어진 호수로 순간이동시킨다.** ⚠️ **그 오차가 그대로 돈이 된다 — 원시 좌표의 하버사인 거리가 실제와 30~40% 어긋나는데 라이더 보상·ETA·최적화가 전부 그 위에 서 있다.**
- ⚠️ **판단의 근거가 판단에 의해 바뀌는 자료를 다루는 법** — **핑 하나를 빼면 이웃의 특징이 바뀌어 정상이던 핑이 이상해지거나 수상하던 핑이 정상이 될 수 있다.** 그래서 **한 번에 하나씩 빼고 다시 계산한다.**
- ⚠️ **도구가 주는 신뢰도를 그대로 쓰지 않았다** — OSRM 의 confidence 위에 **DTW 편차·타임스탬프 공백·시작/끝 지연**을 얹어 자체 점수를 만들고, **낮으면 맵매칭을 버리고 거친 하버사인으로 돌아간다.** **그럴듯하게 틀린 답보다 거친 근사가 낫다**(재구성). ⚠️ **최적 매치도 최고 신뢰도가 아니라 `정제된 핑의 하버사인 거리에 가장 가까운 것`으로 고른다** — 파편 매치를 피하려고.
- ⚠️ **이 기능의 전환점 — 개인의 관측을 집단의 사전으로 교정한다.** H3 격자 위에 HMM 을 세우고 **전이 확률을 그 구간의 모든 과거 배달에서 배워** Viterbi 로 **맞춤법 검사**하듯 개별 궤적을 다듬는다.

### ⚠️ 그리고 그 방법이 무엇을 지우는지도 함께 봤다

**합의로 고치는 방법은 다수와 다른 것을 잡음으로 본다** — 이상한 우회는 다수 경로로 다듬어지고, **25% 미만 군집은 버려지며**, **도착지를 약 201m 격자로 뭉치면 밀집 동네 안의 진짜 차이가 가려진다.** 그리고 **임계값이 전부 고정이다**(60km/h · 30m/240초 · σ 15m · DTW 10% · 공백 180초 · 25%). **완화 장치는 OD 쌍당 두 경로를 남겨 완전한 단일화를 피하는 것뿐이다**(재구성).

### ⭐ Meesho 와 같은 답에 다른 길로 도달했다

**Meesho 는 기사가 15분 안에 연달아 배달한 두 주소를 이어 `주소` 를 배웠고, Zepto 는 여러 라이더의 궤적에서 `경로` 를 배운다.** ⚠️ **둘 다 개별 신호를 못 믿어 집단으로 갔다** — 비교 문서 4축(`인도 규모에서 무엇이 달라지나`)의 가장 좋은 재료다.

### 다음 사이클 — 마지막 도메인, 그리고 완주 판정

`--gaps` 가 **`광고를 팔려면 하루 10억 이벤트를 세야 한다`** 를 부를 것이다. ⏳ **URL 을 먼저 받아야 한다**(슬러그 추측 금지) — `A Billion Events a Day: How We Built Ads Analytics That Actually Works`(2026-07-27). 자료가 서면 **같은 사이클에서 완주 판정까지** 한다(도메인 3/3 · 고아 없음 · 수익원 둘 다 연결).

⚠️ **큐가 1/3 이다**(Snyk). 이 회사를 다 판 뒤 **후보 조사**가 걸린다.
...
**④ `인도 규모에서 무엇이 달라지나` 2곳** — Meesho / Zepto. ⭐ **`개별 신호를 못 믿어 집단으로 간다` 는 같은 답을 둘 다 냈다.**
