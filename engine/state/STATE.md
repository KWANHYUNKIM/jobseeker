...
**없다 — Fastly 를 완주했다.** 큐는 **1/3**(Monzo). 다음 사이클은 `--gaps` 가 **신규(6순위)**를 부른다.

### 이번 사이클 — 보안 `빨리 세는 것과 정확히 세는 것 중 하나를 고르게 한다` 와 완주

**예고한 대로 기능보다 자료를 먼저 찾았고, 그게 맞았다.**

- ⚠️ **`deep dive` 가 붙은 글이 고객용 how-to 였다** — `preserving-analytics-attribution-bot-challenges-...-deep-dive` 는 NGWAF 가 456 을 내면 VCL 로 챌린지 페이지를 합성하는 **사용법 안내**다. **제목의 깊이와 글의 성격은 다르다**(Capital One 의 OPA 글과 같은 함정, 두 번째 사례).
- ✅ **1급 자료는 제품 문서에 있었다** — `documentation/guides/security/rate-limiting`. **대가를 이렇게 정직하게 적는 문서는 드물다**: 계수는 **POP 안에서만** 공유되고 **최대 10% 적게** 잡히며(예시까지 든다 — 100rps 한도가 **110rps 에서 걸릴 수 있다**), 윈도우는 **짧으면 빠르고 부정확·길면 느리고 정확**하다. 그리고 못을 박는다 — `No security product...will detect or prevent all possible attacks or threats.`
- **블로그가 얇을 때 문서가 답인 경우가 있다** — Stripe 문서를 1급 출처로 쓴 전례가 접근 지도에 이미 있었다. **`설계 회고가 없다` 와 `자료가 없다` 는 다르다.**

### 이 회사에서 건진 가장 큰 것

⚠️ **네 도메인이 한 판단을 네 번 반복한다 — 중앙에 모으는 편의를 버리고 그 자리에서 끝낸다.** 퍼징은 중앙 큐를, 보안은 중앙 스크러빙을 버렸다. **그리고 대가도 네 번 같다 — 전역으로 아는 곳이 없다.** 퍼지는 **언제 모두가 받았는지 몰라** 상한 대신 **평균**을 팔고, 속도 제한은 **POP 사이에 계수를 공유하지 않아 10% 적게** 센다. **한 회사의 도메인들이 같은 성질과 같은 약점을 공유하는 것을 이렇게 선명하게 본 것은 처음이다.**

### 다음 사이클 — 신규(Monzo)

큐 맨 위는 **Monzo**(UK · 네오뱅크)다. **장애 회고에 대가가 있고 데이터 메시 글에 수치가 있다** — 프로파일(3단계)부터 간다.

⏳ **큐가 1/3 이라 Monzo 를 다 판 뒤 후보 조사를 한 번 끼운다.** 남은 실마리: ClickHouse `ai-first-data-warehouse` · Capital One `dynamodb-streams-lambda-pattern-best-practices` · Duolingo 의 안 읽은 두 편.
