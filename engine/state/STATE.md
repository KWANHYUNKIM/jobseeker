...
**없다 — 문체 보수 사이클이었다.** 누방크를 배민 문체로 다시 썼다(1.72 → 0.0). 회사 115개, 큐 2/3. 22곳 남았다.

### 읽기를 싸게 하는 쪽과 쓰기를 싸게 하는 쪽 — 정확히 반대다

둘째 기능이 ClickHouse 와 거울처럼 마주 선다.

- ClickHouse 는 **쓰기를 싸게** 만들려고 패치만 적어 두고 그 값을 질의하는 사람에게 넘긴다.
- 누방크는 **읽기를 싸게** 만들려고 쓸 때 미리 접어 두고 그 값을 쓰기 쪽이 문다.

무엇을 포기할지 먼저 정한 것도 드물다 — 정해진 범위 안의 최종 일관성은 받아들이고 예측할 수 없는
지연은 안 받아들인다. 그리고 **구조를 바꾸기 전에 전술적 최적화를 다 써 봤다.** Ramp 나 DoorDash 가
크게 멈추고 다시 지은 것과 달리 막다른 곳을 확인하고 나서 옮겼다.

### 남의 규칙을 다루는 답이 다섯이 됐다

Plaid(흡수해 감춘다) · Ramp(조립하게 둔다) · Paystack(못 정하는 시계를 적는다) · 볼트(흡수한 뒤
맞춰 본다) · **누방크(연결 자체를 한 벌로 만든다)**. 핵심 문장이 성능이 아니라 **누가 무엇을 몰라도
되는가**다 — 나라가 늘어도 엔지니어가 배울 프로토콜 수는 늘지 않는다. Monzo 가 플랫폼 도구를
제품처럼 다뤄 균일함을 판 것과 같은 자리인데, 여기서는 그 균일함이 국경을 넘는다.

### 사내 에이전트 일곱 번째 — 사람이 프롬프트를 쓰는 일 자체를 없앤다

Razorpay(권한) · Sentry(품질) · Block(주의력) · DoorDash(설 자리) · Ramp(신원) · Duolingo(채택)에
이어, 누방크는 **사람이 프롬프트를 쓰는 일 자체**를 없앤다. 다만 경계는 더 단단하다 — 결정적인
일이라면 LLM 에게 안 시킨다는 원칙으로 카드 배송 절차를 하나의 결정적 API 안에서 끝낸다.
ClickHouse 가 넘길 비율을 먼저 정한 것과 같은 결이다.

**그리고 이 엔진에서 처음 보는 대가가 하나 있다 — 모델이 좋아지는 것이 곧 깨질 위험이다.**
똑똑한 모델일수록 전제하는 것이 적어서 예전 프롬프트가 기대던 암묵적 가정이 사라진다.

### 도메인을 상품이 아니라 지점이 없다는 것이 만드는 문제로 잘랐다

카드·대출·예금으로 가르면 이 회사의 가장 특징적인 제약이 안 보인다. 나라를 넘는 배선, 화면이 뜨는
속도, 사람을 늘릴 수 없는 창구 — **셋 다 지점이 있었다면 다른 모양으로 풀렸을 문제다.**

### 다음 사이클 — 문체 보수 (사다리 3순위)

--gaps 가 정한다. 지금 맨 위는 modal(1.62)이지만 그건 이 시점의 예상이지 확정이 아니다.
