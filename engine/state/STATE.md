...
**ClickHouse** (US · SaaS) — 도메인 3개 · **기능 1개**(`updates-as-patches-not-rewrites`). 남은 도메인 둘.

### 이번 사이클 — 갱신 `고치는 대신 무엇을 고쳤는지만 적어 둔다`

⚠️ **앞 사이클에 `엔진 도메인은 자료를 한 편 더 찾는 것이 낫다` 고 적어 뒀고, 찾으니 3부작이었다** — 1편(엔진으로 우회) · **2편(패치 파트로 표준 SQL UPDATE 를 만든다)** · **3편(벤치마크)**. **`자료가 얇으면 한 번 더 찾는다` 가 세 회사 연속으로 통했다**(Fastly · Monzo · ClickHouse).

**이 기능에서 처음으로 `history` 를 세웠다** — 2018년 클래식 뮤테이션 → 2026년 패치 파트. **회사가 옛 구조와 그 한계를 직접 인용하기 때문에 연도가 붙었다.**

건진 것 넷:

- ⚠️ **대가가 쓰기에서 읽기로 옮겨 간 것을 벤치마크가 그대로 보여 준다** — 가시성은 **최대 1,700배** 빨라지는데 질의는 **8~21%(평균 15.3%)** 느려진다. 폴백에서는 **39~121%**, `ReplacingMergeTree + FINAL` 은 **평균 280%** 다. **비용을 없앤 게 아니라 옮긴 것이고, 회사가 그 크기를 잰다.**
- ⚠️ **새 기계를 만들지 않고 이미 도는 기계에 얹었다** — 삽입이 원래 빠르고 **백그라운드 머지가 어차피 돌기 때문에** 패치가 `almost zero overhead` 로 접힌다. **모든 파트가 같은 키로 정렬돼 있다는 성질이 여기서 값을 한다.**
- ⚠️ **델타는 위치에 기대는데 그 위치가 움직인다** — 빠른 경로는 `_part_offset` 으로 행을 직접 맞추는데 **파트가 먼저 머지되면 그 위치가 사라져** 해시 조인 폴백으로 떨어진다(메모리에 다 들어가야 한다). **메모리에 안 묶이는 전체 머지 조인은 `not yet implemented` 라고 직접 적는다.**
- **1,000배를 자랑하는 글이 못 하는 것도 같이 적는다** — 서브쿼리·비결정성 제한 · `TOO_MANY_PARTS` 압력 · **25.7 실험적**. 그리고 선을 긋는다 — **테이블의 약 10% 까지의 작고 잦은 변경.**

⚠️ **한 가지는 그대로 받지 않았다** — `up to 4,000× faster than PostgreSQL` 은 **자기 벤치마크이고 조건이 다른 시스템 간 비교**라 그렇게 표시했다.

### 다음 사이클 — 자기 창고 아니면 AI

⏳ **첫 일은 그대로 남아 있다** — **`building-a-data-warehouse-with-clickhouse-part-2`(A year later) 본문을 읽는다.** 초기 적재 구조가 무너진 것까지만 확인했다.

**AI 도메인은 자료가 이미 충분하다**(남긴 30% · 가드레일 · LibreChat·MCP 선택). **그쪽이 불리면 바로 쓸 수 있다.**
