...
**없다. 312 는 비교 문서 사이클이었다** — `무중단으로 갈아 끼우기 — 앱이 아는가 모르는가`
(`domains/swap-while-running.md`, **29번째 비교 문서**). `domains/index.json` 에도 등록했다.

**⚠️ 후보 조사를 또 미뤘다**(310 에 이어 두 번째). `--gaps` 는 큐가 0 이라 [후보 조사]를
가리켰지만, **누적 성적이 열두 곳 중 둘이다**(298 여섯 중 하나 · 305 셋 중 하나 · 311 셋 중
영). **1차 자료가 있는 곳 자체가 말라 가는 반면 비교 문서 축은 재료가 준비돼 있다.**
**⭐ 53곳 전부 `done` 이라 `in_progress` 가 하나도 없다.**

**무엇을 엮었나** — **애플리케이션이 이관을 아는가 모르는가**로 셋을 갈랐다.
- **Wix 인프라 층**(약 200개 MySQL 클러스터) — **앱이 모른다**(*"no code changes"*, ProxySQL 뒤)
- **Wix 앱 층**(Cassandra→DynamoDB, 메시지 수 테라바이트) — **앱이 전부 안다**(읽기마다
  챗룸 상태 조회)
- **Cybozu**(20년 된 C++ CGI 를 쿠버네티스로) — **앱은 모르는데 배포 구조가 앱의 가정에
  맞춰진다**(`fcntl` 잠금 때문에 파드가 한 노드에). **앱을 안 고치는 값을 인프라가 치른
  셈이다**(이 사이트의 해석).

**⭐ 문서에 담은 갈래 여섯**(전부 이 사이트의 정리라고 밝혔다)
1. **앱이 아는가 모르는가** — 같은 회사가 두 층에서 정반대로 풀었고 **두 글은 서로를 언급하지
   않는다.**
2. **무엇을 내주고 무중단을 샀는가** — 인프라 층은 **되돌릴 수 있는 구간**과 **파티션 수 고정**
   (*"we do have data skew"*), 앱 층은 **자원과 지연**, Cybozu 는 **분산**.
3. **되돌릴 길을 언제 닫는가** — 인프라 층은 **쓰기를 넘기면 끝**, 앱 층은 **닫지 않는다**,
   Cybozu 는 **글에 롤백 이야기가 없다**.
4. **버릴 것을 어떻게 골랐는가** — 거절의 종류가 다섯으로 갈린다(**못 한다 · 안 맞는다 ·
   위험하다 · 해 봤더니 안 됐다 · 표준이지만 우리 조건에 안 맞는다**). **실제로 해 보고 버린
   기록은 Wix 앱 층 하나뿐.**
5. **확인의 대상이 다르다** — 인프라 층은 **새 쪽이 견디는지**(1시간 실트래픽 창), Cybozu 는
   **옛 쪽의 가정이 유지되는지**(커널 실험), 앱 층은 **확인 대신 데이터의 성질에 기댄다**
   (불변성을 *"Luckily"* 라고 적는다).
6. **옮기는 양이 방법을 정한다** — 둘 다 **창을 늘리는 대신 그 안에서 할 일을 줄였다**.

**⚠️ 인용 검사를 돌렸다** — 310 에서 중국어 인용에 한국어가 섞였던 일이 있어, 이번엔 정규식으로
**인용부호 안 한글 혼용 0건**을 확인하고 커밋했다.

---

**다음 사이클 — 남은 비교 문서 축 둘 중 하나.**
1. **`자를 어떻게 세우는가`**(권함) — Snap 넷(골든셋 만들고 범위 밝히기 / 못 잰다고 적기 /
   자를 만들고 결과가 나쁘다고 적기 / 범위를 다른 편에서 밝히기) + Meituan 둘(**자가 무뎌지면
   새로 만든다** — LoHoSearch 로 GPT-5.5 가 BrowseComp 80%대에서 **34.74%** 로 / **이진화해
   기계에게 맡긴다** — 일치율 85~90% 문턱, Beam 62%→92%). **기존 `what-counts-as-good.md`·
   `wrong-yardstick.md` 와 서로 가리키게 쓴다.**
2. **`에이전트에게 무엇을 못 하게 하는가`** — 재료 일곱, 세 갈래(병합 제한 monday.com
   Morphex·Snap CodePal·Snap Casper / 접근·행동 제한 Wix AirBot·Wix 자가치유 두뇌 / 반대편
   Wix Agent Ready Repos·Wix 컴파일러 문지기·Adevinta). **⚠️ 310 의 `what-guards-the-gate`
   와 재료가 크게 겹친다 — 겹치는 부분은 서로 가리키게 쓰거나, 겹침이 심하면 안 쓴다.**

**형식**(310·312 에서 확인) — `domains/<slug>.md` 에 쓰고 **`domains/index.json` 에도 등록**
(`slug`·`title`·`question`·`companies`·`features`·`file`). 머리말에 비교 대상과 *"각 회사
페이지에서 `확인`으로 정리한 사실만 엮었다. 비교에서 나오는 해석은 **추정**으로 표시했다"*,
다른 비교 문서 안내, 표로 갈라 보이기, 마지막은 `## 이 사이트에 아직 없는 것`.
**⚠️ 인용에 원문 언어를 섞지 않는다 — 쓴 뒤 정규식으로 확인한다.**
...
- **📌 Grab 보강 후보**(이미 `done`) — Palana 2부작(06-19/21) · Agent platform Part 1(07-24) ·
  **Iceberg(07-10)** · **Counter Service 저장소 이전(07-03)**. 뒤 둘은 기존 `iceberg-lake`·
  `counter-service` 의 후속이라 **보강 사이클**로 붙일 수 있다. **비교 문서 축이 다 떨어지면
  이쪽이 다음 일이다.**
- **후보 조사** — 안 본 곳: 인도 Zoho·Ola·Paytm·Freshworks · 유럽 Trade Republic·Personio ·
  일본 Sansan·freee·SmartHR·Money Forward(**일본은 이미 넷이라 우선순위 낮음**).
  **⚠️ 중국은 사실상 막혔다** — Xiaohongshu·Kuaishou 둘 다 블로그가 없었다(WeChat 공식계정
  관행으로 보임, **추정**).
