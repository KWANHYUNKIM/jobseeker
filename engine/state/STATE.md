...
**없다 — 문체 보수 사이클이었다.** Flipkart 를 배민 문체로 다시 썼다(2.5 → 0.0). 회사 115개, 큐 2/3. 57곳 남았다.

### 정확히 한 번을 어디서 보장하나 — 셋이 다른 답을 냈다

Flipkart 의 첫 기능이 이 축이다. 초당 100만 이벤트에서 정확히 한 번을 시스템이 주려 하면
높은 지연과 가용성 손실이 따라오므로, 적어도 한 번을 받아들이고 배치가 뒤에서 바로잡는다.

이 엔진의 다른 둘이 같은 물음에 다르게 답했다.

- Klaviyo 는 발행 실패를 성격으로 갈랐다. 재시도로 풀리는 것은 별도 카프카가 받고
  안 풀리는 직렬화 오류는 S3 가 받는다.
- Block 의 자가치유 루프는 아예 재현되지 않는 실패를 손대지 않는다. 관측할 수 없는 행동을
  고치려 들지 않는다는 것이다.

Flipkart 는 세 번째 자리다. 틀린 것을 받아들이고 나중에 갚는다. 셋 다 정확히 한 번이 공짜가
아니라는 데서 출발하는데, 그 값을 지연으로 낼지 복잡도로 낼지 사후 정산으로 낼지가 갈린다.

### 이번 사이클 — Flipkart

사업 모델에서 기술 이야기가 두 극단 사이에 걸쳐 있다는 것을 앞세웠다. 한쪽에는 5억 고객을
위한 몇 달 단위 공급망 계획이 있고 다른 쪽에는 초당 100만 이벤트의 밀리초 정확성이 있다.

세 도메인의 문제 진술을 문장으로 세웠다. 특히 공급망 쪽은 시간이 연속적이지 않다는 것이
핵심이다 — 주문이 오후 6시 5분에 들어오면 다음 날 아침까지 처리할 수 없고, 5분이 배송
하루를 잃게 만든다.

도메인 경계의 근거도 적었다. 세는 일과 정산하는 일을 따로 볼까 했으나 속도 계층과 배치
계층이 같은 로그를 놓고 서로를 보정해 한 경계로 뒀다.
