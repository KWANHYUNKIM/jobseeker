...
**Rerun — 프로파일만 썼다.** 회사 135개, 큐 0/3. 도메인 셋이 다 비어 있고 셋 다 자료를 확보했다.

### 큐의 경고가 그대로 맞았다 — 절대 날짜가 없다

목록에도 개별 글에도 `1년 전`·`2년 전` 같은 상대 표기만 보인다. 그래서 **이 프로파일의 어느
사실에도 연도를 붙이지 않았고** eras 도 안 세웠다. 시점은 릴리스 번호(0.12·0.13·0.18)로만
이어 뒀다 — 발행일을 못 박는 절차가 여기서는 아예 불가능하다.

### 표에 안 들어가는 데이터

로봇 데이터는 모션 1000Hz·카메라 30Hz·계획 4Hz 로 저마다 다른 속도로 비동기로 바뀐다.
게다가 원본 로그에서 뽑아 오프라인으로 넘기는 순간 **그 데이터가 무엇을 뜻하는지가 사라진다** —
온라인과 오프라인을 같은 모형으로 잇는 것이 이 회사의 문제 정의다.

### 수치가 두 번 크게 나온다

행 색인을 열 청크로 바꿔 **적재 100배·메모리 35분의 1**(스칼라 225만 점·그래프 아홉 개),
질의 캐시와 픽셀 합치기로 **100만 점 600ms → 20ms(30배)**. 둘 다 대가를 함께 적는다 —
청크가 크면 질의가 느려지고, 캐시는 순서가 뒤섞인 로그의 적재를 느리게 한다.

### 가장 쉬운 답을 물렸다

시계열만 빠르게 하려면 지표 전용 경로를 두면 된다. 그런데 점구름과 여러 필드를 가진 IMU
표본까지 다 빨라야 해서 일반적으로 풀었고, 그래서 어려웠다고 스스로 적는다.

### 다음 사이클 — 확장 (Rerun 의 빈 도메인)

`화면이 도는 동안 다시 꺼낸다` 부터. 캐시·변경 기록 무효화·잘게 나눈 잠금·픽셀 합치기가 이미
손에 있다.
