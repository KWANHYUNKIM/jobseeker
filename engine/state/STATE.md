...
**Meesho** (IN · 커머스) — 도메인 4개 · **기능 2개.** 남은 도메인 둘은 **본문 미독**이다(사기 · 이미지).

### 이번 사이클 — `주소가 아니라 배달 기사가 다닌 길에서 위치를 배운다`

⚠️ **글 두 편이 한 시스템의 두 세대였다** — 텍스트만 읽던 V1(2026-05-07)과 **배달 로그를 함께 읽는 V2**(2026-05-25). 한 기능으로 묶어 썼다.

건진 것 다섯:

- ⚠️ **못 푸는 문제를 풀 수 있는 문제로 바꿔 놓는 결정이 먼저 온다** — **위경도 회귀를 버리고 H3 격자 칸(해상도 9, 반경 약 200m)을 맞히기로** 했다. 잡음 입력에서 회귀는 성립하지 않았다.
- ⚠️ **규모가 답이 아닌 자리** — **더 큰 모델이 오히려 나빴다.** 회사가 **인도 주소의 변동성과 잡음에 과적합한 것 같다**고 적고, **LoRA 로 미세조정한 LLM 도 더 작은 모델보다 못했다.** 고른 것은 **Flan-T5-base(220M).**
- ⚠️ **인도를 하나로 다루지 않는다** — **주별 모델 29개.** 전국 모델은 데이터 불균형을 못 견뎠고, 주별이 **모든 주에서 일관되게** 나았다(복잡한 주에서 최대 35% 개선). 대가는 **모델 29개의 운영**이다.
- ⚠️ **이 기능의 전환점 — 주소 밖에서 근거를 찾았다.** 서로 다른 랜드마크로 같은 곳을 부르면 텍스트만으로는 알 수 없다. 그래서 **기사가 15분 안에 연달아 배달한 두 주소를 잇는 2년치 공간 그래프**를 만들었다. 회사의 문장: **`모든 주소를 보는 두 가지 방법을 주고 둘이 서로 대화하게 한다.`** **1km 이내 정확도 80.45% → 89.52%**(UP).
- ⚠️ **정확도를 얻고 지연을 내줬다** — V1 은 700ms 를 **약 80ms** 로 줄였는데 V2 는 **약 495ms** 다. 그리고 **새 주소에 self-loop 를 주면 89.62% → 85.30%** 로 떨어져 **ANN 으로 추론 시 그래프를 만들어 준다.**

### ⚠️ 이 사이클에서 지킨 선 — 연도를 모르면 쓰지 않는다

**두 글이 명백히 V1 → V2 인데도 `history` 를 비웠다.** V2 논문이 **CIKM '25** 에 실렸다는 것은 알지만 **V1 이 언제 프로덕션에 갔고 V2 로 언제 넘어갔는지**가 어느 글에도 없다. **논문 발표 연도를 세대 전환 연도로 옮기는 것이 STYLE 8번이 경고하는 바로 그 실수다.** open_questions 에 남겼다.

### 다음 사이클 — 같은 회사의 남은 도메인 둘

`--gaps` 가 **`사기 네트워크를 실시간으로 끊는다`** 를 부를 것이다. ⏳ **`r.jina.ai` + WebFetch(verbatim 인용 지정)로 읽는다:**
- `https://www.meesho.io/blog/trustmesh-how-we-built-a-real-time-fraud-brain-that-thinks-in-graphs-and-sequences` (2026-07-15)
- `https://www.meesho.io/blog/revealing-the-hidden-links-how-we-break-down-complex-fraud-networks` (2025-07-31)
- 그 다음: `the-pixel-gap-...`(이미지, 2026-06-24) · `the-right-pixels-...`(2026-08-25)

⚠️ **큐는 2/3**(Zepto · Snyk). ⏳ **Swiggy 는 브라우저가 살아나면 다시 본다** — `r.jina.ai` 가 Medium 계열에는 422 를 낸다.

### ⚠️ 비교 문서 재료 세 축 (초안 유지)

**① `AI 에이전트를 어디까지 믿나` 6곳 + ⏳ Snyk** — Sentry / ClickHouse / Duolingo / Ramp / DoorDash / Deliveroo.

**② `관리형 MySQL 의 한계` 3곳** — Etsy / Plaid / Paystack.

**③ `실험 결과를 얼마나 믿나` 2곳** — Etsy / Deliveroo.

⚠️ **비교 문서(8순위)는 이 사다리에서 영영 안 온다** — 3순위가 큐를 다시 채우므로 "위가 전부 비었다" 가 성립하지 않는다. **PROMPT.md 의 문제이고 사이클이 고칠 것이 아니다.**
