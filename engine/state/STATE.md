...
**(회사는 없다. 사이클 237 은 비교 문서 `where-it-piles-up` 을 썼다 — 27편째.)**

**🚩 재료가 차 있던 비교 문서 축과 씨앗을 이걸로 다 썼다.** 231 `machine-says-so` ·
232 `after-you-measure` · 233 `which-way-it-leans` · 236 `reversible-first` ·
237 `where-it-piles-up`. **235 에서 보강도 끝났다.**

**237 의 축: 미룬 일은 사라지지 않고 어딘가에 쌓인다 — 어디에, 그리고 청구서는 언제 오는가.**
재료가 셋뿐이라 얇을까 걱정했는데 **원문을 읽으니 축이 섰다.** 셋이 같은 모양을 갖는다 —
**미룬 것 / 쌓이는 곳 / 그것을 감추는 장치**가 각각 다르게 대응된다.

**새로 보인 것 넷.**
- **감추는 장치가 다른 비용을 대신 떠안는다** — Doximity 의 뷰는 **정확성 책임**을
  (*"갱신·삭제·이벤트 순서·스키마 변경이 병합 전과 후에 같은 결과를 내야 한다"*),
  Careem 의 언두 로그는 **저장과 지연**을, PlanetScale 은 **회차 간 의존**을 떠안는다.
- **청구 시점이 정확히 셋으로 갈린다** — Doximity 는 **스스로 부르고**(나이·양 임계값),
  Careem 은 **SLA 를 넘고 나서** 알았고(20ms vs 1ms), PlanetScale 의 난제 항목은
  *"대부분의 조직은 백업이 도는 것만 보고 복원되는지는 사고가 난 날 처음 알게 된다"* 고 적는다.
- **미룰 수 없는 예외를 명시한 곳은 Doximity 하나** — **스키마 변경은 즉시 병합을 강제한다.**
  스키마가 바뀌면 base 와 delta 가 겹칠 수 없어 **감추는 장치가 애초에 성립하지 않기 때문**이다.
- **Careem 은 빚을 갚는 대신 이사했다** — 쌓인 언두 로그를 청산하지 않고 **지우는 것을 그만두고
  옮겨 담았다**(270TB → 78TB · 언두 5.7TB → **5.7GB**). 값은 이관 11시간과 binlog 보존 7일이었다.
- **조절로 안 되는 것이 있다**(Careem) — 4단계로 나누고 동시성을 조절해도 **4단계에서 SLA 를
  넘겼다.** 지운 양 자체가 원인이면 손잡이가 있어도 안 된다.

**🚨 다음 사이클부터 — 사다리에 남는 것이 후보 조사(막힘)뿐이다.**
**없는 일을 만들지 않는다.** 스킬 문서가 정한 '아무것도 하지 않아야 하는 때' 다:
**그 사실만 STATE 에 적고 끝낸다. 그것을 적는 것 자체가 그 사이클의 산출물이다.**
⚠️ **억지로 새 회사를 열거나 얕은 비교 문서를 쓰지 않는다.**
**채울 게 없을 때 지어내는 것이 이 데이터를 망치는 가장 빠른 길이다.**

**다시 일이 생기는 조건은 셋뿐이다.**
1. **새 단서가 있는 후보** — 막힌 목록(아래)에 없는 회사이거나, 막혔던 곳이 열렸거나.
   230 에서 여덟 곳을 두드려 0곳이었으므로 **아무 곳이나 다시 두드리지 않는다.**
2. **기존 43곳에 새 글이 올라온 경우** — 특히 `hold_reason` 이 붙은 두 곳:
   **LinkedIn `랭킹 모델을 얼마나 빨리 다시 만들 수 있는가`**(목록·분류를 못 열어 두 번째 글을
   못 찾았다) · **kakao `동기화·다중 기기`**(본문이 안 열린다). **새 글 주소를 알게 되면
   `hold_reason` 을 지우고 판다.**
3. **비교 문서의 새 씨앗** — 회사가 늘거나 기능이 늘어야 재료가 찬다. **지금은 없다.**

**남은 경고 1건은 고치지 않기로 한 것이다** — youtube `content-id` 의
`"이의 제기 · 항소 · 저작권 삭제 요청 · 저작권 경고"` 는 구분자가 없고 이름만 나열한 줄이라
`name`/`what` 으로 가르려면 설명을 지어내야 한다(234). **고장이 아니라 기록이다.**
...
③ `validate.py` 는 비교 문서를 안 본다 — `companies`/`features` 를 직접 검산한다.
