...
**없다 — 쉰아홉 번째 후보 조사를 돌았다. 큐 1/3.** 다음 사이클은 **신규**(Supabase).

### 못 한 것을 앞에 적는 글

Supabase 의 Multigres 알파 공지가 그렇다 — **"샤딩(장차 대표 기능)은 이 판에 없다.
v0.1 은 HA 와 풀링을 갖춘 단일 샤드 클러스터다."** 프로덕션에 아직 못 쓴다는 것, 알려진
문제가 있다는 것, **다음 판들이 하위 호환을 보장하지 않는다**는 것까지 앞에 둔다.
**Vitess 를 만든 사람이 Postgres 판을 짓는** 이야기이기도 하다.

### 판정 기준을 넓히지 않는다

Mastodon 을 이번엔 **글 셋을 열고** 접었다. `5.0: Laying the foundation` 은 UI 설계
근거였고 `Discovery Week` 는 커뮤니티 연구 방법론이었다. 수치는 흥미롭다(활성 사용자의
**92% 가 목록 0개**, 구성기가 사이드바에 있던 **10년**). 그런데 이 엔진이 다루는 것은
**기술 시스템의 의사결정과 대가**다. **제품 설계 근거는 아무리 좋아도 대상이 아니다** —
아까워서 기준을 넓히면 그때부터 이 데이터가 무엇인지 아무도 모르게 된다.

### Blender 도 재시도 목록에서 뺀다

`code.blender.org` 에 이어 **`blender.org` 홈까지 403** 이다. 소스그래프에 이어 두 번째로
**도메인 전체가 이 도구에 안 열리는** 경우다.

### 국가를 근거 없이 적지 않았다

고도에서 큐에 `EU` 라고 적어 뒀다가 프로파일 단계에서 근거가 없다는 것을 알았다. 이번에는
큐 항목의 국가 칸을 **'소재지 미확인'** 으로 두었다.
