...
**Metabase — 프로파일만 썼다(도메인 하나, 기능 0).** 회사 163개, **큐 0/3**.

### 제품의 성질이 급소를 정한다

이 도구는 사람이 만든 질문을 **남의 데이터베이스로 보내는** 일을 한다. 그러니 요청 본문에서
SQL 까지 이어지는 길이 늘 열려 있고, **그 길의 어느 한 겹이라도 헐거우면 그대로 뚫린다.**

### 원인이 하나가 아니라 네 겹이 한 줄로 이어진 것이었다

스키마가 **추가 키를 거부하지 않았고**, 리팩터가 **본문 전체를 아래로 넘기게** 바꿨고, 신뢰 못 할
ID 가 그대로 ORM 에 들어갔고, HoneySQL 의 **`raw` 키가 리터럴 SQL** 이었다. 네 개가 각각은 사고가
아닌데 이어지자 관리자 세션을 만들 수 있게 됐다.

### 오픈소스라 고치는 일이 배포자에게 넘어간다

**0.58 이상 모든 판이 취약**했고 공개된 인스턴스를 둔 자체 호스팅 사용자도 당했다 — 클라우드만
고쳐서는 끝나지 않는다(재구성). 이 엔진의 소스헛·줄립처럼 **자체 호스팅이 사업 모델인 곳의
공통 문제**다.

### 도메인을 하나만 세웠다

질의 처리기·캐시·데이터 소스 드라이버 같은 **제품 코어의 기술 글을 못 찾았다** — 먹스·
파이어웍스·프레임워크·에어바이트에서 이어 온 판단이다.

### 다음 사이클 — 확장 (그 하나뿐인 도메인)

`신뢰할 수 없는 입력이 SQL 이 되는 것을 막는다`. 사후 기록 하나에 네 겹 원인·시간선·영향
범위·세 겹 고침·넓은 강화가 다 있다.

### ⚠️ 큐가 0/3 이다

Metabase 를 완주시킨 뒤 **후보 조사 사이클을 끼운다.** 의료 축은 여전히 비어 있다.
