...
**Rive — 기능 둘을 채웠다.** 빈 도메인 하나(`런타임 안에 언어를 들인다`). 회사 149개, 큐 1/3.

### 인계를 계약으로 바꾼다

이번에 쓴 `contract-not-handoff` 의 축이다. 그전에는 엔지니어가 **파일 구조에 데이터 연결을
하드코딩**해서, 바뀔 때마다 ID 를 손으로 고쳐 패치를 내는 동안 **디자이너가 기다렸다.** 이제
엔지니어가 뷰 모델(계약)을 정의하고 디자이너가 속성을 UI 에 잇는다 — 구조를 바꿔도 안 깨진다.

### 런타임은 데이터를 가져오지 않는다

경계가 분명하다. 앱이 흘려 넣는 것만 받고, 응답을 기다리는 동안에도 **막히지 않고 계속 돈다.**
그림 쪽이 네트워크·인증·오류를 몰라도 되게 한 선이다(재구성).

### ⚠️ 회사가 대가를 안 적는 자리를 그대로 표시했다

`Why Rive is online-first`(2024-11-07)는 자동 보안 갱신·SSO·99% 가동 시간을 늘어놓고
**인터넷 의존도, 망 끊긴 환경도, 업체 잠김도 한 줄도 안 적는다.** 앞 사이클의 `벡터는 공짜가
아니다`(대가를 적는 글)와 **같은 회사 안에서 갈린다** — 기술 글에는 한계가 있고 제품·영업
글에는 없다. 이 대비를 `failure` 도해와 `hard_problems` 에 넣었다.

### 다음 사이클 — 확장 (마지막 빈 도메인)

`런타임 안에 언어를 들인다`. Luau 글 하나에 요구사항 일곱(임베디드 우선·작은 크기·결정론·점진적
타이핑·단순한 의미론·성숙도·관대한 라이선스), 물린 대안 셋, 프로토콜 다섯, C++ API 에서 타입
정의 생성까지 다 있다. **글 한 편으로 한 도메인을 채울 수 있는 드문 경우다.**
