...
**Bolt(EU/에스토니아 · 모빌리티)** `in_progress` · **도메인 3 · 기능 0** — 프로파일만 세웠다.
**68곳** · 큐 1/3(Mercado Libre).

⚠️ **본문은 브라우저로만 온다** — `bolt.eu/en/blog/*` 는 WebFetch 로 목록만 오고 글은
비어 온다. 다음 사이클도 `get_page_text` 를 쓴다(탭 만들고 navigate 하고 읽는다).

### 세 도메인이 한 줄로 이어진다

**참여자가 넷인 거래**(승객·기사·배달원·식당)를 50개국에서 굴리면 —
**① 돈이 여러 손을 거치니 어디까지 왔는지 세야 하고**(PSP 마다 통화·주기·형식이 다르다),
**② 어긋난 금액이 곧 사기 신호이니 검문을 세워야 하고**(약 60개를 8년에 걸쳐),
**③ 같은 지표가 두 값을 내면 그 판단 전체가 못 믿을 것이 된다**(쓰는 엔진과 읽는 엔진이 달랐다).

### 다음 사이클 — 첫 도메인부터가 좋다

**과잉설계했다고 적고 되돌아온다** — `how-bolt-simplified-fraud-checkpoints`.
**이 엔진이 찾는 결 그 자체다.** 맞춤 코드 → 빌더 → 설정 주도 ETL → **다시 빌더**로
돌아온 네 단계를 회사가 직접 적고, **어디까지는 통했는지도 가른다**(규칙 유지를
분석가에게 넘긴 것은 성공, 생애주기 전체 설정화는 실패). 인용이 셋 다 강하다.

나머지 둘도 자료를 이미 읽어 뒀다 — `tracking-payments-at-scale` ·
`migrating-looker-to-databricks`(**월 1000만 쿼리 · 32PB · 최종 45% 빠르고 40% 저렴**,
그리고 **실패한 시도 둘**을 적는다: 정규식 자리표시자와 LLM 변환).

### ⚠️ 연표를 못 세운 이유 (Kraken 과 같은 자리)

사기 검문은 **네 단계를 지났다고 적는데 연도가 없다** — `약 60개를 지난 8년에 걸쳐`
뿐이다. **연도를 모르니 `eras` 를 비웠다.** 다만 Looker 이전 글에는 연도가 있다
(**2021** 설계 · **2025년 2월** POC 시작 · **11월** 완료) — 기능의 `history` 재료가 된다.
