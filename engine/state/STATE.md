...
**Grafana Labs** (US/SE · SaaS — 관측) — **프로파일까지 썼다**(PROMPT 3단계). 도메인 2개 · 기능 0. 회사 **93개.**

### 이번 사이클 — 신규(6순위)

⚠️ **이 회사의 기술 문제가 자기 사업 모델에서 곧장 나온다.** **100개가 넘는 데이터 소스를 지원하는 `큰 천막` 전략**으로 사용자를 모으고 **그들의 텔레메트리를 대신 저장해 준다.** **그러면 저장 비용이 곧 마진이 된다** — 그래서 **Mimir 차세대 아키텍처 이야기가 성능이 아니라 총소유비용(TCO)으로 시작하고 `TCO 25% 감소` 로 끝난다**(재구성).

**도메인 둘** — ① **메트릭을 오래, 그리고 싸게 저장한다**(✅ 본문 확인, 2026-01-02) · ② **로그를 색인 없이 다룬다**(⏳ 2차 자료로 방향만 확인). ⚠️ **Snyk 처럼 둘로 시작한다** — Tempo·Pyroscope·Alloy 는 자료를 읽고 새 도메인이 보이면 확장 사이클에서 더한다.

**규모** — 고객 **1만 곳 이상** · **ARR 6억 달러 이상**(2026년 8월; 2025년 9월에 4억 달러·7,000곳) · **엔지니어링 자원의 약 80% 를 오픈소스에** 쓴다고 밝힌다.

### 읽어 둔 것 — Mimir 글에서 건진 것 넷 (다음 사이클의 재료)

- ⚠️ **옛 구조가 카산드라에서 왔다** — **복제 계수 3 + 쿼럼 읽기·쓰기.** 회사가 대가를 그대로 적는다: **`복제 계수 3은 비싸다. 수집 경로에서 각 데이터의 사본 셋을 들고 있어야 한다.`**
- ⚠️ **더 아픈 것은 비용이 아니라 결합이었다** — **`인제스터가 과부하되면 수집 경로, 즉 쓰기 경로도 영향을 받는다.`** 새 구조의 첫 목표가 **`읽기 경로에서 무슨 일이 일어나든 쓰기 경로에 영향을 주지 않아야 하고 그 반대도 마찬가지`** 다.
- ⚠️ **Kafka 를 사이에 놓자 복제를 줄일 수 있었다** — **복제 3 → 2, 쿼럼 2 → 1.** **로그가 내구성을 대신 져 주면 사본 수를 줄일 수 있다**(재구성).
- ⚠️ **비용의 정체가 전송이었다** — **교차 AZ 전송이 GB당 0.02달러**인데, **WarpStream 이 객체 스토리지에 쓰면 스토리지가 알아서 영역을 넘어 복제하면서 교차 AZ 비용이 들지 않는다.**

### 다음 사이클 — 확장(2순위)

`--gaps` 가 **`메트릭을 오래, 그리고 싸게 저장한다`** 를 부를 것이다. **자료를 이미 읽었으니 바로 기능을 쓴다.**

### ⚠️ 시간축을 못 세운 네 번째 사례

**Mimir 글이 `10년의 아키텍처 진화` 를 말하면서 어느 구조가 언제부터였는지는 밝히지 않는다.** GeoIndia · Flink→ClickHouse · Snyk 에 이어 넷째다. ⚠️ **`무엇에서 무엇으로` 는 아는데 `언제` 를 모르는 경우가 이 세션에 네 번 나왔다** — 전부 open_questions 에 남겨 정보는 잃지 않았다.
...
**③ `실험 결과를 얼마나 믿나` 3곳** — Etsy / Deliveroo / Snyk.

**④ `인도 규모에서 무엇이 달라지나` 2곳** — Meesho / Zepto.

⏳ **⑤ `한 코어에 하나씩인가, 여러 코어가 나눠 쓰나`** (ScyllaDB 대 ClickHouse). ⏳ **⑥ ⭐ `복제로 버틸 것인가 로그로 버틸 것인가`** — **Grafana Mimir 가 복제 3에서 Kafka 로 옮긴 이야기**이고, **Loki 도 같은 방향으로 갔다.** **한 회사 안에서 같은 판단이 두 제품에 반복되는 자리다**(재구성).
