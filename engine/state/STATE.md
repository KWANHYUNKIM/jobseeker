...
**curl**(오픈소스 프로젝트) — 도메인 2개 중 **1개에 기능이 들어갔다**
(`close-the-door-for-a-month`). 남은 빈 도메인 하나: '남이 배포한 바이너리는 우리가 못
고친다'. 큐 0/3.

### 어려운 것은 실행이 아니라 결정이었다

개인 이메일로 하나가 들어왔을 때의 대응이 **"나는 그냥 그 이메일을 무시했다. 쉬웠다."**
기술적으로는 아무것도 아니었다 — **하기로 정하는 것**이 어려웠던 일이다.

### 평소에 못 하던 일의 목록이 곧 부하의 증거다

자유로워진 시간에 무엇을 했는지가 이 결정의 값을 증명한다 — **밀린 PR 검토, 소홀했던 코드,
문서 빈틈, 그리고 실제로 쉬고 여행.** 그 목록이 곧 **보고 처리가 먹고 있던 것**의 목록이다.

### 상업 지원 신규 가입 0

이 글에서 가장 아픈 숫자다. **사용자는 curl 이 안전하기를 바라지만 그 안전에 돈을 내지는
않는다** — 널리 쓰일수록 각자에게는 남의 일이 된다. 그런데 글은 이 사실을 **적기만 하고
해석하지 않는다.**

### 위험을 0 으로 만들지 않고 감당 가능한 크기로 줄였다

CNA 의 **72시간 응답 의무**는 면제되지 않는다. 그래서 **최악이면 상위 기관이 긴급 배정**을
하기로 합의해 두고 갔다 — 실제로는 쓰지 않았다.

### 한 프로젝트의 결정이 관행이 됐다

**다른 오픈소스 프로젝트 여럿이 따라 쉬었다.** 다만 **이름을 적지 않아** 얼마나 퍼졌는지는
알 수 없다.

### 닫는 것은 미루는 것이지 없애는 것이 아니다

돌아온 뒤 **밀린 보고를 어떤 순서로 처리할지도, 다음이 언제인지도 없다.** 저자도 **부하가
줄어 언젠가 필요 없어지기를 바란다**고 쓴다 — 근본 해결이 아니라 견디는 방법이다.

### 다음 사이클

남은 도메인 하나 — `system32` 의 `libcurl.dll`. **수치가 없지만** 제약이 선명하다("우리는
여러분의 윈도우 설치를 런타임에 패치해 줄 수 없다"). 그것으로 curl 은 완주다.
