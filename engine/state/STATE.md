...
**SmartHR(JP · SaaS) `in_progress` — 도메인 2개 · 기능 3개.** 347 에서 DB 쪽 도메인
`한 덩어리를 그대로 두고 버티게 만든다` 의 둘째 기능
**`measure-before-the-season`(계절이 오기 전에 재고, 재는 일을 넘긴다)** 를 썼다.
...
**재료와 판정** — `開発チームにノウハウが残るSREの関わり方 —— 年末調整のピークに備えた負荷試験`
(2026-07-24, `/entry/2026/07/24/120000`). **3/3** — 버린 대안(**스펙 증강만으로 넘기는 안**을
명시적으로 거부) · 대가(**모드 전환 타이밍 판단이 어려웠다** · 전부 콜라보레이션이면 SRE 공수가
터진다) · 수치(**첫 시험 목표의 4분의 1** · 재시험까지 **약 1주** · 계획부터 달성까지 **약 1개월** ·
SRE **2명에서 6명**).

**⚠️ 조직론 에세이일까 봐 조심했는데 아니었다** — 팀 토폴로지 세 모드를 **공정마다 붙인 표**와
**EXIT 로드맵**이 있고, 그 위에 **부하 시험의 기술과 수치**가 얹혀 있다. **223 규칙상 같은
도메인의 둘째 기능**이다(떼어내지 않고 버티게 하는 일).

**⭐⭐ 347 의 핵심 — 같은 회사가 두 번 다 데이터베이스에서 막혔는데 시점이 정반대다**
(이 대비는 이 사이트의 것이고 `connections` 와 도메인 `why` 에 적었다). 커넥션 풀러 쪽은
**접속 수**가 병목이었고 **장애가 난 뒤에** 움직였다(회사가 **언제 심해질지 몰라 미뤘다**고
반성한다). 부하 시험 쪽은 **CPU** 가 병목이었고 **정점이 오기 전에 재 봤다.**
**한 번은 늦었고 한 번은 이르다.**

**347 이 새로 낸 것**
- **⭐ 관측할 것이 없는 구간은 외삽이 안 된다** — *"9〜11月にトラフィックのピークを迎える
  一方で、それ以外の時期はアクセスがほとんどありません"*. **그래서 만들어서 잰다.**
- **버린 대안이 명시적이다** — *"クラウドサービスにも限界があり、際限なくスケールアップする
  ことはできません"*.
- **시나리오를 평균이 아니라 무거운 쪽으로 기울인다** — 과거 통계로 부양가족 있음·복수 보험
  같은 패턴 비율을 높였다. ⚠️ **그러면 여유가 얼마인지는 안 보인다**(이 사이트의 지적).
- **첫 결과를 스펙으로 덮지 않았다** — DB CPU 가 **100퍼센트**에 붙어 **캐스케이드 장애**로
  전 요청이 지연·실패했다. 개발팀은 코드와 SQL, SRE 는 인프라를 병행해 약 1주 뒤 목표 초과.
  ⚠️ **무엇을 고쳤는지는 글에 없다 — 가져갈 것이 방법이 아니라 절차뿐이다.**
- **EXIT 를 처음에 합의했다** — 모드와 관여도를 로드맵화. **⚠️ 그래도 판단은 어려웠다:**
  *"コラボレーションからファシリテーションへ切り替えるタイミングの見極めが難しかった"*.
- **의도적으로 모드를 나눈다** — *"なんでもコラボレーションモードで進めるとSREの工数が
  パンクしてしまう"*.

---

**다음 사이클 — SmartHR 을 닫거나 한 축을 더 연다.**
- **⭐ 1순위 — 닫는다.** 도메인 둘 다 기능이 둘 이상이고 연결도 이어졌다(완주 기준 충족).
  닫으면 `business_model` 에 적을 관찰(이 사이트의 정리라고 밝힌다):
  **⭐⭐ 두 도메인이 같은 단일 실패점에 정반대로 답한다** — 인증은 **떼어내서 격리**,
  가장 큰 Rails 앱은 **그대로 두고 앞에 세워 버틴다.** **버티는 쪽이 훨씬 빨리 끝났다.**
  **⭐⭐ 그리고 데이터베이스에서 두 번 막혔는데 한 번은 늦었고 한 번은 이르다.**
  **⭐ 자기 방법의 한계를 적는다** — Packwerk 가 새 영역에서는 됐는데 얽힌 인증에서는 안 됐다.
  **⭐ 자기가 고른 것이 나중에 틀린 것도 적는다** — PgCat 은 개발이 멈췄고 이미 관리형으로 옮겼다.
  **⚠️ 블로그의 결이 넓어 사내 행사·커리어 글이 절반쯤 섞여 있다.**
  닫는 절차: 회사 `status`·`index.json` status → `done` · 양쪽 `updated_at`(235 에서 ✗) ·
  **QUEUE 진행 중 행을 빼고 완료 불릿로**(⑦ 불릿이다, ⑥ `'\n## 완료\n'` 줄 경계 매치).
- **2순위 — 한 축 더.** `横断フロントエンドユニットの取り組み`(`/entry/2026/08/03/112823`) ·
  `不具合情報の集約基盤を整え、品質改善に活かすまで`(`/entry/2026/08/24/133642`) ·
  `Sandboxテスト`(`/entry/2026/08/17/080914`). 그리고 345 의 글이 참조하는 과거 글 셋
  (**주소 미확인**): **Policy-as-Code**(2025-08-08) · **새 권한 기반 설계**(2026-01-07) ·
  **모듈러 모놀리스화**(2026-05-18).

**⚠️ 큐 0/3 — SmartHR 을 닫으면 곧바로 후보 조사 사이클이다**(PROMPT.md 사다리 3순위).
**⚠️ Personio(EU) 는 아직 안 봤다 — 첫 순위.** **후보 조사 누적 열아홉 곳 중 일곱.**

**⭐⭐ 닫으면 비교 문서 재료가 둘 선다**(어느 것을 쓸지는 그때 정한다):
① **freee 대 SmartHR 의 순서** — freee 가 후회로 적은 *"旧機能のリファクタはマイクロ
サービス化の前に着手すべし"* 를 SmartHR 이 실제로 밟아 보고 **그것만으론 부족하다**는 결론을
적는다(`smarthr.json` 의 `open_questions`).
② **`미루되 미룬 것이 안 보이게 한다`** 에 SmartHR 을 더한다 — 미룬 것이 **놀고 있는 접속**에
쌓였고 청구서는 **장애 빈도가 오르며** 왔다. **⭐ 347 이 그 문서에 짝을 하나 더 준다** —
같은 회사가 **미루지 않고 미리 잰** 사례다.

**분포** — 58곳(**일본 일곱** · 인도 셋) · 비교 문서 32편 · **운영 사실 39개.**
...
**⭐ 새로: `python3 -c "…"` 안에 백틱을 쓰지 않는다 — 347 에서 셸이 명령 치환으로 먹어
명령 전체가 파스 실패했다. 확인 스크립트는 heredoc 으로 돌린다.**
