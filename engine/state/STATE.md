...
**Fastmail** — 회사 프로파일까지 썼다. **도메인 둘 · 기능 0.** 큐 0/3.

### 복제는 백업이 아니다

이 회사를 파는 축이다. 2026년 4월에 폴더가 **복사본 세 벌 모두에서** 지워졌다 — 복제는
하드웨어 고장은 견디지만 **잘못된 명령까지 충실히 복제한다.** 살린 것은 몇 시간마다 받는
추가 전용 증분 백업이었다.

### 백업의 틈에 다섯 통이 있었다

한 사용자의 **보낸 편지 5통**이 백업 주기 사이에 있었다. 그것을 Cyrus 의
`#jmapsubmission` 임시 파일에서 건졌는데, 이것은 **설계된 복구 수단이 아니라 운 좋게 남아
있던 것**이다. 그 사실을 도메인의 `limits` 에 적어 뒀다.

### 사고가 난 자리가 곧 다른 도메인이다

폴더를 지운 작업이 **EU·비EU 계정을 가르는 일**이었다. 도메인 둘이 남남이 아니라 하나가
다른 하나를 깨뜨린 관계다.

### 경계를 긋고 그 경계가 새는 곳을 적는다

EU 리전을 내면서 **대가를 숨기지 않는다** — EU 계정의 지리적 백업은 아직 미국에 있고,
비상 백업은 필라델피아, 시스템 로그는 지역과 무관하게 미국이다. 그래서 **"EU 안에만
머문다고 보장하지 않는다"** 고 명시한다. 방향도 대칭이 아니다 — EU→US 는 빠르고
US→EU 는 느리다.

### 시대를 세우지 않았다

읽은 글 넷 중 **발행 연도가 정확한 것은 하나뿐**이다(플랫폼 팀 글 2026-07-28). 사고 글에도
'4월' 말고는 시간선이 없다 — 언제 알아챘고 얼마 만에 복구했는지가 없다.

### 다음 사이클

빈 도메인 둘 중 하나. **되살리기 쪽이 자료가 두껍다**(PR 번호 셋, 원인 세 겹, 두 번의 고침,
백업 방식과 주기).
