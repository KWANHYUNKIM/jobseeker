...
**Bun — 기능 둘을 채웠다.** `bun-install` 과 `zig-to-rust`(기계적으로 옮기고 적대적으로 검토한다).
회사 124개, 큐 2/3. 빈 도메인 하나 — API 호환.

### 이 엔진에 없던 기록이 들어왔다

53만 5천 줄짜리 코드베이스의 언어를 통째로 바꾼 이야기인데, 결정의 축이 언어가 아니라 **일정**이다.
사람만으로는 엔지니어 셋이 약 1년이고 그동안 버그 수정과 기능이 얼어붙는다 — 사용자에게 아무 영향도
없는 1년은 고를 수 있는 선택지가 아니라고 적는다. 그래서 엔지니어 한 명과 병렬 LLM 인스턴스 64개로
11일에 옮겼고 API 가격 기준 약 16만 5천 달러가 들었다.

거절한 대안 셋에도 이름이 붙는다 — Zig + 스타일 가이드(TigerBeetle 의 TigerStyle 같은 것)는 린터의
최선 노력에 그치고, C++ 는 ASAN 을 써도 손상과 누수가 계속 나며, 자체 스마트 포인터는 러스트보다
쓰기 나쁘면서 보장이 없다. **이 엔진에 있는 TigerBeetle 이 남의 글에서 거절 대안으로 등장한 첫 사례다.**

### 병렬화의 전제는 속도가 아니라 합의였다

코드를 만들기 전에 Zig 패턴과 러스트 대응을 적은 지도(`PORTING.md`)와 구조체 필드마다의 수명 표
(`LIFETIMES.tsv`)를 먼저 만들고 **그것부터 적대적 검토를 받았다.** 64개가 동시에 옮겨도 소유권 결정이
갈리지 않으려면 먼저 합의가 있어야 하기 때문이다(재구성).

검토도 방식이 뚜렷하다 — 별도 인스턴스 둘에게 **구현자의 설명 없이 diff 만 주고 코드가 틀렸다고
가정하라**고 지시했다. 그렇게 병합 전에 치명적인 것 셋을 잡았다(libuv 가 참조하는 파이프의 해제 후
사용 · 음수 시간의 trunc · unwrap_or 의 양쪽 평가).

### 문법이 비슷하면 의미 차이가 가려진다

병합 뒤 회귀 19건이 전부 그 종류였다 — Zig 의 assert 는 인자를 늘 실행하는데 러스트는 릴리스에서
지우고, Zig 는 홀수 길이 슬라이스를 조용히 무시하는데 bytemuck 은 패닉이며, ReleaseFast 는 경계
검사를 없애는데 러스트는 남긴다. 임시로 64로 둔 블록 크기가 그 덕에 드러난 것은 덤이다.

### 성립한 조건을 스스로 적는다

언어와 무관한 테스트 스위트(파일 4,174개 · 케이스 60,624개 · expect 1,386,826회, 하나도 안 건드렸다),
약 53만 5천 줄이라는 크기, 저수준 구조를 그대로 두는 기계적 이식. 셋 중 하나라도 없으면 이 방식이
안 된다고 밝힌다.

### 다음 사이클 — 확장 (마지막 빈 도메인)

Node.js·V8 API 호환을 채우면 Bun 이 done 이 된다. 그 테스트 스위트가 이번 이전을 가능하게 했다는
점에서 두 도메인이 이어진다.
