...
**Tigris Data — 도메인 셋을 다 채우고 done 으로 닫았다.** `fdb-metadata` ·
`push-metadata-pull-data` · `shadow-bucket-migration`. 회사 125개, 큐 1/3.

완주 기준 셋을 다 넘겼다 — 도메인마다 기능 1개 이상, 기능 셋이 이어져 고아가 없고, 수익원이
기능과 닿는다.

### 세 도메인이 같은 원칙의 세 얼굴이다

**요청이 배치를 정한다.** 다른 리전의 블록도, 그림자 버킷에 남은 옛 객체도 요청이 와야 옮겨진다.
사내 경계(리전 사이)와 사외 경계(남의 저장소와 우리 사이)에 같은 게으름을 쓴다.

그리고 그 게으름의 대가도 같은 모양이다 — 첫 접근은 멀고, 한 번도 안 읽힌 것은 영영 안 옮겨진다.
그래서 양쪽에 손잡이를 하나씩 뒀다: 업로드 때 미리 데우는 `accelerate` 와 나머지를 밀어 넣는 CLI
능동 이전. **게으른 이전은 끝을 스스로 내지 못한다**는 것을 문서가 그대로 적는다.

### 이사는 되돌아갈 수 있느냐의 문제다

원본을 그림자로 걸고 쓰기 통과로 최신을 유지해 **엔드포인트만 되돌리면 예전으로 간다.** 저장소를
바꾸는 결정이 미뤄지는 진짜 이유가 되돌리기 어려움이라면, 쓰기를 두 번 하는 값은 마케팅비에
가깝다(재구성).

다만 반대 방향도 있다 — 조건부 쓰기를 원본보다 넓게 주면(ETag·수정 시각·태그 값) 그것을 쓴 코드는
S3 로 못 돌아간다. **호환으로 들어와 비호환으로 묶는 자리**다.

### 도구의 한계를 절차로 바꿨다

블로그 목록 페이지가 WebFetch 로 한 편만 오는 바람에 지난 사이클에 자료가 없다고 적을 뻔했다.
검색으로 개별 글을 찾아 넷을 더 얻었다 — 목록이 안 오면 검색으로 내려간다는 것을 QUEUE 에 남겼다.

### 다음 사이클 — 신규 (Chroma)

큐가 1/3 이고 대기 맨 위가 Chroma 다. 벡터 데이터베이스 축이 이 엔진에 아직 없다. 목록에 날짜가
없으니 개별 글에서 날짜를 확인하며 판다.
