...
**없다 — Zulip 을 완주시켰다(도메인 1 · 기능 1 · `status: done`).** 회사 157개, **큐 0/3.**

### 저장할 것과 계산할 것을 가른다

이번 기능의 알맹이다. **직접 하위 그룹만 저장하고** 간접 멤버십은 저장하지 않는다 — 그러면
그룹이 바뀔 때 다시 계산할 것이 없다(재구성). 대신 확인할 때마다 그래프를 걸어야 하는데,
그 걸어 가기를 **포스트그레스 재귀 CTE 로 데이터베이스 안에서 한 번에** 한다.

### 질의 하나가 0.3~2밀리초인 곳

메시지 한 통에 이미 질의가 일곱이라 하나를 더 붙일 수 없다. 그래서 **채널 객체를 가져오는
질의에 `can_send_message_group` 이름을 함께 실어** 흔한 경우(`role:everyone`·`role:nobody`)는
질의 없이 판정한다. 대가는 스스로 적는다 — **복잡도가 는다.**

### 큰 전환을 한 번에 하지 않는다

수천 개 조직이 각자 서버를 돌리므로 한 번의 큰 마이그레이션을 강요할 수 없다. **작고 안전한
전환들의 긴 연속**으로 쪼갰고(10년 약 800개 중 **115개**), 옛 역할은 **시스템 그룹**으로 새
체계 안에 표현해 코드가 새 체계 하나만 알게 했다.

### 인가를 파는 쪽과 제품 안에서 짓는 쪽

AuthZed 는 잔지바 계열 시스템 자체를 팔며 질의 계획기를 벤치마크하고, 여기는 채팅 제품 안에서
**메시지 한 통의 25~50밀리초 예산**에 권한 확인을 맞춘다. 같은 문제의 다른 자리다.

### 다음 사이클 — ⚠️ 후보 조사 (큐가 0/3 이다)

판정 기준을 이어 쓴다 — 회고 · 되돌린 기록 · 자기 것을 쓰지 말라고 적는 글 · 사고를 공개하는
글 · 상류에 고쳐 보낸 기록. **형식이 아니라 대가가 기준이고, 본문을 열어 판정한다.**
`blocked` 로 적어 둔 곳(incident.io · Sourcegraph · Guardian · Flexport)이 다음 목록이다.
