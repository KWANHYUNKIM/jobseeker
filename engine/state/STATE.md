...
**Signal 완주(기능 4개).** 회사 133개, 큐 1/3 — Backblaze·Rerun 이 남았다.

### 이번 기능 — `published-cost-map`

데이터로 돈을 안 벌기로 하면 **모든 지출이 남에게 받아야 할 돈**이 된다. 그래서 값을 항목별로
공개했다 — 저장 130만·서버 290만·등록 문자 600만·대역폭 280만·기타 70만으로 인프라 연
1,400만, 인건비 1,900만, 2025년 약 5,000만 달러, 상근 50명.

공개하는 이유를 스스로 적는다: 업계에서 돈이 어디서 나고 어디로 가는지가 금기에 가까운데,
그 지도를 그려 둬야 사생활을 지키는 대안에 무엇이 필요한지가 분명해진다는 것이다.

### 가장 큰 값을 자기가 못 정한다

인프라 최대 항목이 **등록 문자 수수료 600만 달러**인데 통신사가 값을 정하고 통행료 사기로
부풀려진다. 가짜 등록을 빨리 끊는 것 말고는 손댈 데가 없고, 쫓고 쫓기는 놀음이라고 인정한다.

### 원칙을 지키면서 값을 내리는 길은 연구뿐

연락처 탐색 서버를 **600대에서 10대로** 줄였다 — 트래픽은 오히려 늘었는데 연 200만 달러
넘게 아꼈다. 기능을 깎거나 데이터를 팔면 값은 줄지만 제품이 지키려던 것이 깨진다.

### 네 기능이 한 제약에서 갈라져 나왔다

서버를 안 믿는다는 조건 하나가 래칫(세션 안) · 투명성 원장(세션 밖) · 영지식 백업(맡기되 못
열게) · 공개 비용(수입을 데이터에서 안 찾음)을 각각 낳았다. 이 엔진에서 **제약 하나가 네
도메인을 다 설명하는 회사**는 드물다.

### 다음 사이클 — 신규 (대기 맨 위 Backblaze)

⚠️ 볼트·소거 부호 구조 글이 옛것이라 날짜를 개별로 확인하고 판다.
