...
**아무것도 안 판다 — 컬리를 닫았다(47번째, 도메인 5개 · 기능 5개). 큐 잔량 0/3.**

264 에서 **다섯 번째 도메인 `한꺼번에 몰리는 것을 모아서 받는다`** 를 열고 **`stream-window`(창이
닫혀야 값이 나온다)** 를 쓴 뒤 회사를 닫았다. 결정 7 · 그림 3 · 생각 4 · 스택 5 · 난제 2 · ui 5핀 ·
연결 1(`inbound-sync` 와 **대비**).

**📌 원문 재독이 열두 번째로 옳았다.** "얕으면 닫는다" 고 판단해 두고 열었는데 **넷이 더 나왔다.**
- **더미 이벤트의 키 설계** — *"파티션별로 분배가 되도록 키를 설정을 하거나 아래처럼 파티션 번호를
  직접 지정해서 발행해야 합니다"*, 코드가 `"dummy-key-" + context.taskId().partition()` 이다.
  **같은 키를 쓰면 한 파티션으로만 몰려 나머지 파티션의 시간이 얼어붙는다.**
- **버린 대안이 둘 더 있었다** — **`Processor.forward()`**(스트림 시간이 안 흐른다) · **WindowStore 를
  직접 훑어 강제 발행**(내부 상태가 안 바뀌어 **중복 발행**, 시도한 임계는 *"10초 이상 지난 윈도우는
  강제 발행"*). **둘 다 창을 억지로 닫으려던 시도였고 다른 이유로 실패했다.**
- **날짜 경계는 문제로 짚기만 하고 해법 없이 닫는다** — *"설계 단계에서 고려할 것이 훨씬 많습니다."*
- **집계 결과는 DB 가 아니라 다른 Kafka 토픽으로 나간다.** ⚠️ **DB 부하 감소 수치는 끝내 없다.**

**🔴 이 기능의 축 — 스트림의 시간은 시계가 아니라 이벤트가 민다.** 그래서 **한산하면 창이 안 닫히고
값이 안 나온다** — 정산처럼 '없으면 없다고 알아야 하는' 일에서는 그게 곧 고장이다. 답이 **시간을
흐르게 하려고 가짜 데이터를 만들어 넣는 것**이고, **그 가짜에도 파티션별 키가 필요하다.**

**🔴 회사를 닫으며 `business_model` 에 패턴 넷을 적었다**(이 사이트의 정리라고 밝혔다).
① **알림을 다루는 생각이 세 기능에서 반복된다** — 임계로(오배송 · *"양치기 소년 현상"*) · 토픽으로
(입고 · *"컨슘 실패 알람이 이제는 오히려 반가워졌습니다!"*) · 막되 안 내리는 것으로(AccessBlock).
**셋 다 '덜 시끄럽게, 그러나 놓치지 않게' 인데 세 글이 서로를 언급하지 않는다.**
② **미룸에 기한을 두는데 기한의 출처가 갈린다** — 입고 **24시간**(업무 데드라인) · AccessBlock
**3분**(예측 가능성) · 재고 정산 **Grace Period**(**근거가 안 적혀 있다**).
③ **개인정보를 정반대 두 방향에서 다룬다** — 오배송은 **안 모으고**(임베딩만), 암호화는 **모으되 잠근다**.
④ **사업 수치가 하나도 공개돼 있지 않다** — **하이퍼커넥트에 이어 두 번째다.**

**🚨 다음 사이클 — 후보 조사다. `--gaps` 가 그렇게 가리킬 것이다. 목표 3곳.**
**⚠️ 한국 회사가 아홉이 됐다**(NAVER · Kakao · Toss · 당근 · 배민 · 쿠팡 · 무신사 · 하이퍼커넥트 · 컬리).
**이번 후보 조사는 한국 밖에서 한다.**

**아직 안 열어 본 후보(한국 밖)** — Sea/Shopee(다른 주소를 찾아본다) · GoTo/Tokopedia · Traveloka ·
Robinhood · Plaid · Adevinta · Skyscanner · N26 · Atlassian · Carousell · VNG.
**📌 238 의 기준(유효 — 239·246·253 에서 통했다)** — **개발자 대상 제품 회사(관측·엣지·워크플로·DB)의
블로그는 대개 영업 채널이다. 자체 서비스를 굴리는 회사부터 본다.** 인프라(Vercel·Neon·Elastic·
MongoDB·Confluent)는 뒤로.
**📌 후보를 올릴 때 제목만 보지 않는다** — 목록을 열어 **최신 글 날짜**와 **두 편 이상 묶이는 주제**를
보고 **본문 한 편을 실제로 연다.** 버린 대안과 대가가 없으면 안 올린다.
**📌 246·253 이 확인한 것** — **403 · DNS 없음 · JS 렌더링이 절반을 넘는다.** 그래도 **다섯에 둘,
여덟에 하나는 나온다.** 목록을 계속 내려간다.
**📌 262 의 발견** — **2부작·연작 글이 있는 블로그가 파기 좋다.** 한 도메인에 두 편이 그 자리에서 확보된다.
...
- **새 축 `관측에 값을 무엇으로 치르는가` — 여덟이다.** CyberAgent `observability-pipeline`(돈) ·
  `alert-context`(주의력) · **컬리 셋**(`misdelivery-detection` 은 임계로 · `inbound-sync` 는 토픽으로 ·
  `access-block` 은 막되 안 내리는 것으로) · Yelp `partition-access`(하루 TiB) · Zerodha `log-storage` ·
  Doximity `prod-profiling`. **컬리 한 회사에서 같은 생각이 세 번 다른 방식으로 나온 것이 좋은 재료다.**
- **`되돌릴 수 있는 것을 먼저 한다`** — CyberAgent `env-split-migration` 은 **반례**(되돌림 계획이 없다),
  하이퍼커넥트 **한 회사가 세 가지를 다 준다**(`stream-join` 은 되돌리는 길이 배포 절차 자체 ·
  `raid-recovery` 는 늘 두 벌을 유지 · `arch-migration` 은 롤백 기준이 통째로 없다).
- **`무엇을 좋다고 부를 것인가` 와 `재 보고 나서`** — 둘 다 *"잘못 고른 사례가 없다"* 로 끝나는데
  **하이퍼커넥트 `objective-relaxation` 이 반례다**(DAU 를 올렸더니 PUR 이 떨어져 목표를 다시 짰다).
- **📌 새 축 후보 `기한을 무엇으로 정하는가`** — 컬리 셋(업무 데드라인 / 예측 가능성 / 근거 없음) +
  Moniepoint 신선도 창 + Doximity 나이·양 임계 + PlanetScale 12시간 WAL. **재료가 여섯이다.**
...
⑫ **요약이 아니라 원문을 다시 읽는다** — **열두 번 다** 새 결정을 찾았다(223·229·243·249·250·252·255·258·260·261·263·264). **"얕다"·"재료가 없다" 는 판단이 재독 앞에서 두 번(258·264) 다 틀렸다.**
⑬ **JS 로 그려지는 목록은 브라우저 `get_page_text` 로도 안 나올 수 있다**(253, Cygames). **두 번 실패하면 접는다.**
⑭ **`--gaps` 가 프롬프트의 지시와 다른 도메인을 가리키면 `--gaps` 를 따른다**(255).
⑮ **2부작·연작 글은 도메인을 열기에 좋다**(262) — 한 도메인에 두 편이 그 자리에서 확보된다.
