...
**Tigris Data — 기능 하나를 채웠다(`fdb-metadata`, 기록과 뒤에 할 일을 한 트랜잭션에 함께
적는다).** 회사 125개, 큐 1/3. 빈 도메인 둘 — 배치·캐싱 · S3 호환.

### 이중 쓰기 문제를 구조에서 지웠다

전역 저장소는 요청받은 자리에서 일을 다 끝낼 수 없다 — 복제와 캐시 데우기가 뒤에 남는다. 보통은
데이터베이스에 쓰고 큐에 넣는데, 그 둘이 다른 시스템이면 **썼는데 못 넣은 상태**를 나중에 스캔해
찾아내는 보정 작업을 짓게 된다.

이 회사는 큐를 FoundationDB 안에 직접 만들어(애플의 QuiCK 에서 영감) **PUT 하나가 객체 메타데이터와
색인 변경과 작업 항목을 한 트랜잭션에 함께 커밋**하게 했다. 얻은 것을 한 줄로 적는다 — 그 오류
경로가 아예 없다. 작업은 끝날 때까지 남아 최소 한 번 실행되고, 워커가 죽으면 리스 만료로 넘어간다
(그래서 모든 작업이 멱등해야 한다).

키 배치도 같은 결이다 — 사전순 튜플로 한 버킷을 연속 범위에 놓아 목록 조회가 범위 스캔 하나가 된다.
대신 뜨거운 버킷이 한 자리에 몰릴 수 있다.

### 어려운 부분을 남에게 맡긴 대가

FDB 를 쓰면 샤딩·합의·복제·트랜잭션을 직접 짓지 않는다. 대신 저장소의 성질이 그쪽 제약에 묶인다 —
트랜잭션 크기와 시간 제한이 위층을 정하는데, 큰 객체와 멀티파트 업로드가 그 한계와 어떻게 만나는지가
자료에 없다. 난제로 남겼다.

### 다음 사이클 — 확장, 다만 자료가 얇다

심층 글이 `fdb-krea-talk` 하나뿐이고 블로그 목록도 그것만 온다. 남은 두 도메인은 같은 글에서 뽑을 수
있는 만큼만 남았다 — 자료를 더 못 찾으면 `hold_reason` 을 달고 다음 회사로 넘어간다. 없는 것을
지어내지 않는다는 규칙이 여기서 걸린다.
