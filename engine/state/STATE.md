...
**없다 — 이번 사이클은 후보 조사였다.** 큐 **0/3 → 3/3.** 회사 94개.

### 이번 사이클 — ⭐ 접근 지도의 ❌ 가 또 뒤집혔다

⚠️ **`r.jina.ai` 가 403 을 뚫는다는 것을 이번 세션에 알았으니, 지도에 `403` 으로 적힌 판정은 전부 그 이전 것이다.** 인도 회사 셋 중 둘이 곧바로 살아났다.

- **✅ Razorpay** (403 → ✅) — **최신 8월**, 자기 시스템 글 일색, 수치가 붙는다. ✅ 본문 확인(`메트릭 비용 62% 절감`) — **결정마다 거절한 대안과 대가가 명시적이다.** ⚠️ **가장 좋은 대목은 `아무도 안 쓰더라` 를 확인하고 나서 줄인 것** — **경보 규칙과 대시보드를 감사해 분 미만 해상도에 의존하는 것이 있는지 봤고 하나도 없었다**(하루 샘플 4,500억 → 1,700억). **90일간 상위 500개 고카디널리티 메트릭 중 167개가 어디에도 안 쓰였다.**
- **✅ Flipkart** (403 → ✅) — 최신 7월, **`Re-architecting`·`Optimizing`·`Guaranteeing`** 류 제목이 줄지어 있다. ⏳ 본문 미독. ⭐ **`초당 100만 이벤트에서 광고 예산 정확도를 보장한다`(2026-01-22)가 Zepto 의 광고 분석과 나란히 놓을 자리**이고, **`검색 관련성 라벨링을 LLM 으로`(2026-07-31)는 Etsy 의 3단 증류와 견줄 자리다.**
- **✅ Twilio (Segment)** — 새 이름. ✅ 본문 확인(`Strangler Fig 로 알림 아키텍처를 바꾸다`) — **한 번에 다시 쓰는 길을 버리고 점진 교체**, **대가는 이중 쓰기**, **수십 개 독립 서비스 대신 워크플로 오케스트레이션으로 `운영 단순성` 을 `순수 마이크로서비스 모듈성` 보다 앞에** 뒀다. ⚠️ **수치가 없다.** ⭐ **`한 번에 갈아엎지 않고 목 졸라 죽이기` 라는 이전 전략이 이 사이트에 아직 없다.**

### ⚠️ 못 찾은 축과 이번에 확인한 규칙

**헬스케어(Oscar Health · Zocdoc)와 물류 B2B(Flexport · Samsara)는 심층 글을 못 찾았다.** ⚠️ **검색으로 엔지니어링 블로그를 찾는 방식이 이 축들에는 잘 안 걸린다** — 다음에는 다른 각도를 쓴다(컨퍼런스 발표·채용 브랜딩 글).

⭐ **그리고 이번 사이클에 규칙이 하나 생겼다** — **후보를 대조할 때 슬러그가 아니라 `name_en` 으로 본다.** 앞 사이클의 중복 등록이 그래서 생겼고, 이번엔 그 방식으로 40여 개 이름을 한 번에 걸렀다.

### ⚠️ 이번 사이클에 내가 한 실수 — 또 슬러그를 추측했다

**Klarna 글 주소의 Medium 해시를 지어내 404 를 받았다.** ⚠️ **이번 세션에서 두 번째다**(ScyllaDB 에 이어). **목록에서 URL 을 받아 쓰거나 검색으로 확인한다** — 접근 지도에 이미 적혀 있는 규칙인데 또 어겼다. **Klarna 는 그래서 판정을 못 끝냈다**(목록은 열리는데 최신이 2025-06 이고 일반 주제가 섞였다).

### 다음 사이클 — 신규(6순위)

`--gaps` 가 **큐 맨 위(Razorpay)를 `in_progress` 로 만들고 3단계부터** 를 부를 것이다.

⏳ **보강 거리도 남아 있다** — **Grafana Labs 정본의 `mimir-next-gen` 과 중복 파일에서 확인했던 인용을 대조하는 일**(gapless consumption · 복제 3→2 · WarpStream · `더 확장성이 좋다고는 말하지 않겠다`). **회사가 done 이므로 보강 사이클의 몫이다.**

### ⚠️ 비교 문서 재료 (초안 유지)

**① `AI 에이전트를 어디까지 믿나` 8곳** — Sentry / ClickHouse / Duolingo / Ramp / DoorDash / Deliveroo / Snyk / Cygames.

**② `관리형 MySQL 의 한계` 3곳 + Cygames(TiDB)** — Etsy / Plaid / Paystack.

**③ `자기 성과를 어디까지 주장하나` 3곳** — Snyk / Grafana Labs / ScyllaDB.

**④ `인도 규모에서 무엇이 달라지나` 2곳 + ⏳ Razorpay · Flipkart** — Meesho / Zepto. ⭐ **넷이 되면 이 축이 가장 두꺼워진다.**

**⑤ `한 코어에 하나씩인가, 여러 코어가 나눠 쓰나`** — ScyllaDB / ClickHouse / Grafana Labs(`query-sharding`).

**⑥ `복제로 버틸 것인가 로그로 버틸 것인가`** — Grafana Labs 의 Mimir·Loki.

**⑦ `추상화가 무엇을 가리는가`** — Plaid / ScyllaDB / ⏳ Paystack.

**⑧ `제약을 없애지 못할 때 어디서 갚는가`** — Cygames / Zepto / Grafana Labs.

⏳ **⑨ 새로 보인다 — `한 번에 갈아엎을 것인가 목 졸라 죽일 것인가`** — **Twilio(Strangler Fig, 이중 쓰기)** / **Etsy**(테이블마다 1%→100%) / **Plaid**(앞으로 실패하는 길) / **Paystack**(제자리 업그레이드).
