...
**Depot — 기능 하나를 채웠다(`in-process-volume-allocator`, 커널의 데이터면은 두고 제어면만
가져온다).** 회사 130개, 큐 2/3. 빈 도메인 둘 — 이미지 배포 · 시간 판매.

### 최적화의 문제가 아니라 산수의 문제였다

요구를 숫자로 적으면 도구가 걸러진다 — **1초 안에, 초당 200건.** 연산당 100밀리초 동안 볼륨 그룹
전체를 잡는 LVM 은 초당 열 번이 한계라 애초에 쓸 수 없다. 하드웨어를 더 붙여도 그 잠금은 그대로다.

### 직접 만드는 범위를 어디서 끊느냐

블록 장치를 통째로 만들지 않았다. **커널 device-mapper 데이터면은 그대로 두고 제어면만** 인프로세스
할당자로 바꿨다 — 매핑을 메모리에 들고 디스크 I/O 와 전역 잠금을 함께 없애되, 돌고 있는 워크로드는
커널이 들고 있는 매핑 덕에 견딘다. 약 100배.

### 보장을 버린 근거가 end-to-end 논증과 같다

크래시 안전성·삭제 복구·메타데이터 보존을 버렸다고 이름 대어 적고 근거를 댄다 — **하이퍼바이저가
죽으면 이 VM 도 메모리도 사라지는데 그 밑의 메타데이터만 살아남아 무엇을 하나.** 위층이 죽으면
아래층 보장이 의미를 잃는다는 오래된 논증의 실무판이다(재구성).

그래서 이 설계는 오래 사는 데이터에는 쓸 수 없다. 버린 것을 적어 두는 일이 그것을 막는다.

### 다음 사이클 — 확장 (Depot 의 빈 도메인)

SOCI 글은 읽어 뒀다 — 빌드가 이미 레이어를 손에 들고 있으니 색인도 그때 만들자는 이야기다.
