...
**Block (Square)** — 기능 **2/4.** 회사 109개. 큐 2/3.

### ⚠️⚠️ 이번 사이클에 내 프로파일의 사실 오류를 잡았다 — 원문이 정면으로 부정한다

**지난 사이클에 COLDCARD 를 Block 제품으로 적었다.** 이번에 본문을 깊게 읽으니 회사가 못을 박아 뒀다 — **`이 보고서는 Block 제품에 해당하지 않으며, Block 제품이나 고객은 이 취약점의 영향을 받지 않는다.`** **COLDCARD 는 Coinkite 제품이고, Block 의 비트코인 엔지니어링·보안 팀이 남의 기계를 파헤쳐 공개한 것이다.** Block 자신의 비트코인 하드웨어는 **Bitkey**(자기 보관 지갑)와 **Proto**(채굴 장비)다.

⚠️ **어떻게 틀렸나** — 첫 자료 읽기에서 **`블로그에 있으니 회사 제품`이라고 건너뛰었다.** ⭐ **회사 블로그에는 회사 제품 아닌 것도 올라온다** — 보안 자문이 그렇다. **`이 글이 다루는 것이 이 회사 것인가`를 프로파일 단계에서 묻는다.**

⭐ **고치고 나니 이 도메인이 더 흥미로워졌다** — **이 회사의 기능 중 유일하게 대상이 회사 밖에 있다.** 같은 종류의 기계를 파는 회사가 남의 결함을 읽어 내고, **그 자리에서 자기 제품을 팔지 않는다.**

### 이번 사이클 — `남의 기계 안 난수를 파헤친다 (COLDCARD)`

⭐ **세 겹의 방어가 차례로 통과했다** — 그림 하나가 통째로 이 이야기다:

- **1겹 컴파일 가드** — 하드웨어 TRNG 없이는 빌드가 실패하게 해 뒀는데, **가드가 `매크로가 켜졌는가`가 아니라 `정의되었는가`를 물었다.** 양산 설정이 값을 **0** 으로 두었으므로 통과.
- **2겹 심볼 해석** — 보드는 `random32()`·`random_buffer()` 로 내보내는데 라이브러리는 전역 `rng_get()` 을 참조한다. **그래서 참조가 MicroPython 폴백으로 조용히 풀렸다.**
- ⭐ **3겹 건강성 검사** — 검사가 **인접한 반복 출력**을 거부하는데 **`결정론적 PRNG 는 보통 인접한 값이 서로 다르므로 통과한다.`** **잡으라고 둔 그물이 이 결함을 정확히 통과시킨다.**

건진 것 셋 더:

- ⭐ **엔트로피는 파이프라인에서 가장 좁은 자리만큼만 남는다** — 보안 요소가 **40바이트**(SE1 32 + SE2 8)를 주는데 이중 SHA-256 으로 줄인 **앞 4바이트만** 재시드에 닿는다. **`원래 입력의 품질과 무관하게.`** 남는 것이 **최대 2^32 스트림, 평균 2^31 후보 시도.**
- ⭐ **노출을 제조일이 아니라 펌웨어가 가른다** — **`비밀이 만들어질 때 쓰던 펌웨어`.** ⚠️ **사용자가 기기를 봐서는 자기 노출 여부를 알 수 없다**(재구성).
- ⭐ **불완전함을 감추는 대신 표시했다** — **`악용 가능성을 확인할 완전한 경험적 시험은 하지 않았다. 우리는 이 자문을 이르게 공개한다, 악용이 진행 중이기 때문이다.`** 그리고 **`Coinkite 의 확정 보고서가 나오면 그것을 보라`**고 덧붙인다.

⭐ **연표를 쓸 수 있었다** — 자료에 날짜가 줄지어 있다: **2021-01-28**(취약한 가드 코드 존재) → **2021-03-01**(지갑 생성이 libngu 로 이관) → **2021-03-17**(v4.0.0 출시) → **2022-03-14**(첫 양산 Mk4 v5.0.0, 32비트 재시드) → **2026-07-30**(자문 공개). `history` 두 세대로 넣었다.

### 다음 사이클 — 확장 (2순위)

**Block 기능 2/4.** ⏳ `에이전트가 팀으로 일한다`(Buzz 벌 계층·릴레이) · `노는 컴퓨트를 모은다`(MeshLLM).

⚠️ **읽을 것이 남았다** — `Square 본업`(결제·정산·대출 심사) 자료를 아직 못 찾았다. ⏳ 미독 — `BlockTabBench` · `Buzz 에이전트 설정` · `Buzz 프로젝트` · `Microfilm`. **⭐ 새로 생긴 물음 — Bitkey 는 난수를 어떻게 만드나**(남의 기계를 파헤친 회사가 자기 기계는 어떻게 하는지가 이 자료에 없다).
