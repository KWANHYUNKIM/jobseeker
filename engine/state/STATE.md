...
**없다 — 문체 보수 사이클이었다.** Snyk 을 배민 문체로 다시 썼다(2.3 → 0.0). 회사 115개, 큐 2/3. 53곳 남았다.

### 맞았는지 어떻게 아나 — 네 번째 답은 자를 직접 만드는 것이었다

Snyk 의 두 번째 기능이 그 이야기다. 보안 수정을 재는 공개 벤치마크가 없었다. 기존 벤치마크는
코드가 도는지와 버그 리포트를 해결하는지를 보는데, 보안 수정에는 둘 다 부족하다. 보안 기준과
기능 기준을 동시에 넘어야 하기 때문이다. SQL 주입을 없앴는데 질의 결과가 달라졌다면 해결이
아니고, 깨끗해 보이는데 취약점이 남아 있다면 오히려 해롭다.

이 축에 이제 넷이 모였다.

- Cursor 는 사용자의 다음 행동에서 읽는다. 묻지도 벤치마크로 재지도 않는다.
- Block 은 표본 수와 잔여 여지로 잰다. 10~20회 돌려 기준을 세우고 같은 표본 수로 다시 잰다.
- Anthropic 은 재는 자와 그것이 서는 무대까지 의심한다. 설정만 바꿔도 6%포인트가 갈렸다.
- Snyk 은 잴 자가 없어서 자를 직접 만들었다.

앞의 셋은 있는 자를 어떻게 믿을지의 문제이고 Snyk 은 자가 아예 없던 자리다.

### 이번 사이클 — Snyk

축이 찾기가 아니라 고치기라는 것을 앞세웠다. 스캐너는 이미 시장에 많고, 문제를 목록으로
돌려주는 것과 그 문제를 없애는 코드를 써 주는 것은 다른 일이다. 그 둘 사이에 이 회사가 놓은
것이 전문가가 쓴 실제 수정 3만 5천 건 이상의 데이터베이스다.

같은 보안 축의 Wiz 와도 견줬다. 저쪽은 클라우드 자원에서 시작하고 여기는 개발자의 코드에서
시작한다. AI 를 어디까지 믿나 축에서도 자리가 다른데, 다른 곳들은 에이전트가 만든 코드를
어떻게 감독할지를 묻고 여기서는 AI 가 고치는 대상이 보안 취약점 자체다.

도메인 경계의 근거도 적었다. 찾는 일과 고치는 일을 따로 볼까 했으나 이 회사가 파는 것이
고치기라 한 경계로 뒀다.
