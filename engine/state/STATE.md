...
**QuestDB — 기능 둘을 채웠다.** `qwp` 와 `offheap-histogram`(힙 밖으로 옮기고 나서 진짜 문제를
만난다). 회사 122개, 큐 1/3. 빈 도메인 하나 — 시계열 표현.

### 병렬로 안 돌던 이유가 알고리즘이 아니라 메모리 위치였다

백분위수 집계가 사실상 단일 스레드였는데, 원인이 **병렬 그룹바이 자료구조가 힙 밖 메모리를
전제한다**는 데 있었다. 힙 위에 사는 히스토그램은 애초에 그 안에 못 들어간다. 그래서 고친 것이
알고리즘이 아니라 상태가 사는 자리다.

### 옮기는 것만으로는 이득이 아니었다

옮기고 재 보니 단일 스레드가 **0.82배로 오히려 느려졌다.** GC 압력이 사라진 자리에 새 값이
생겼기 때문이다 — 핸들의 포인터를 갈아 끼울 때마다 힙 밖에서 상태를 읽어 온다. 회사가 그것을
한 종류의 값을 다른 종류와 바꾸는 일이라고 적는다.

진짜 이득은 코드 리뷰에서 나왔다. 생성자 인자라 절대 안 바뀌는 값으로 버킷 기하를 매번 다시
계산하고 있었고(Math.log 2회·pow 1회·ceil 1회), 그것을 생성자로 끌어올리자 뜨거운 경로가
Unsafe 읽기 5회 + 수학 여러 번에서 **읽기 3회, 수학 0회**가 됐다. 단일 0.82 → 2.78배,
워커 8개 2.99 → 8.34배, 최고 8.90배.

### 분기를 없애는 것을 규칙으로 삼지 않는다

같은 회사가 조건부 이동이 분기보다 비쌀 수 있다는 것을 따로 파고들어 적는다. **JIT 은 두 갈래의
실행 비율(치우침)을 보고 고르는데 하드웨어가 값을 매기는 것은 예측 가능성이다.** 보통 데이터에서는
일치하다가 짓궂은 데이터에서 갈리고, 결과가 다음 반복으로 이어지는 루프에서만 2.9배·1.9배로
어긋난다. CedarDB 가 분기를 없애 60%를 얻은 것과 나란히 놓으면 같은 문제의 반대편 기록이다.

### 다음 사이클 — 확장 (마지막 빈 도메인)

시계열 표현 도메인을 채우면 QuestDB 가 done 이 되고, 그다음이 후보 조사다(큐 1/3).
