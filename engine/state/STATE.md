...
**없다 — 문체 보수 사이클이었다.** freee 를 배민 문체로 다시 썼다(1.0 → 0.0, 기호 182개). 회사 115개, 큐 2/3. 10곳 남았다.

### freee 의 후회를 SmartHR 이 순서로 실행한다 — 양방향으로 걸었다

freee 는 회계 DB 에 얽힌 인증을 떼어내는 데 4년을 썼고 **그것을 잘한 일로 적지 않는다**(완료까지
오래 걸린 것을 생각하면 잘했다고 할 수 없다). SmartHR 은 새 영역을 한 덩어리 안에서 시작해 경계를
도구로 보이게 만든 뒤 떼어내는데, 그것이 정확히 freee 가 후회로 적은 **먼저 정리했어야 했다**의
실행이다. SmartHR 쪽에 이미 걸어 둔 대비를 이번에 freee 쪽에서도 걸었다.

그리고 실패의 인과를 적은 것이 이 기능의 값이다 — **제대로 그리기로 한 순간 옛것과 달라졌고,
달라진 것을 전부 맞추는 일이 4년의 가장 큰 원인이 됐다.** 제대로 만드는 것과 빨리 떼어내는 것이
같은 방향이 아니었다.

### 옛 기반이 흘려보내던 것 때문에 새 기반이 회귀로 보인다

개발 초기에 검증을 느슨하게 둔 값이 몇 년 뒤 이관의 비용으로 청구된다. **옛 기반은 잘못된
데이터를 그냥 통과시키고 있었기 때문에 새 기반이 옳게 도는 것이 곧 오류로 보였다.**

### 정점이 오기 전에 바닥을 가는 셋 — 여기 시계가 가장 단단하다

Fastly(엣지 용량은 몇 달 전에) · SmartHR(정점 전 부하 시험) · **freee(확정신고라는 연 1회 정점이
날짜로 정해져 있다)**. 그래서 날짜가 기술 선택을 잘랐다 — 구성 변경이 너무 큰 길도, 되돌리는
방법을 확립 못 한 길도 접었다. **무엇이 더 나은가가 아니라 그날까지 되는가가 기준이었다.**

### 읽기와 쓰기의 값을 옮기지 않고 길만 가른다

ClickHouse 는 쓰기를 싸게 하려고 값을 질의하는 쪽에 넘겼고 누방크는 읽기를 싸게 하려고 쓸 때 접어
뒀는데, freee 는 **읽기 쪽에 트랜잭션을 걸고 그 안의 쓰기 질의만 원래 자리로 넘긴다.** 레플리카를
거의 못 쓰던 상태였으니 놀던 읽기 쪽을 쓰는 것만으로 원래 자리의 부하가 준다.

### 다음 사이클 — 문체 보수 (사다리 3순위)

--gaps 가 정한다. 지금 맨 위는 freshworks(1.0)지만 그건 이 시점의 예상이지 확정이 아니다.
