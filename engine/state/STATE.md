...
**Mercado Libre(기타/AR · 커머스)** `in_progress` · **연표 3시기 · 도메인 3 · 기능 0** —
**69곳** · ⚠️ 큐 0/3. **라틴 둘째 · 아르헨티나 첫 회사다.**
⚠️ **본문은 Medium 이라 브라우저(`get_page_text`)로만 읽힌다.**

### ⭐⭐ 첫 사이클에 연표를 세웠다 — 이 엔진에서 드문 일이다

**1999**(저장소 하나에 개발자 200명 · 단일 Oracle · 주 1회 배포) →
**2010** MeliCloud(마이크로서비스 · 인스턴스 17,500) → **2015** Fury(지금).
**시기마다 대가를 회사가 직접 적는다** — 특히 2010년대의 것이 세다:
마이크로서비스로 간 뒤를 **`chaotic freedom`** 이라 부르고, 팀이 제품 개선 대신
운영 문제 해결에 시간을 쏟았다고 인정한다.

### ⭐⭐ 세 도메인을 꿰는 축 — 남의 코드는 안 건드린다

**모든 큰 선택에 같은 관문이 있다: 3만 개 마이크로서비스와 공개 API 계약을 건드리는가.**
FaaS 를 뺀 이유도(`considerable rework on our development teams`), 셀에 라우팅 키를
안 넣은 이유도(`Introducing a new routing attribute wasn't realistic`) 같다.
그래서 나온 것이 **브로드캐스트 게이트웨이**다 — 회사가 그 값을 인정한다:
...
### 다음 사이클 — 도메인 셋 중 어디부터든 자료가 있다

- **셀로 나눠 장애 반경을 줄인다** — `from-a-single-point-of-failure...`(전문 읽음).
  **가장 단단하다**: 2024년 초 한계 · 수십 TB 이전 · **p95/p99 약 70% 감소** ·
  브로드캐스트와 ID 범위라는 두 우회 · **의도적 모놀리스**를 고른 이유까지.
- **이미 있는 것을 고치지 않고 새 구조로 간다** — 같은 글 + 쿠버네티스 글
  (`AWS Batch → Lambda → ECS → Cloud Run → Anthos → Nomad` 를 차례로 떨어뜨린
  6개월. 교훈에 **`Do not buy the buzz`** 가 있다).
- **개발자가 인프라를 몰라도 되게 만든다** — evolution 글(전문 읽음).

### 아직 못 잡은 것

1. **매출·사업부별 비중** — 기술 블로그만 읽었다. **상장사라 IR 을 열어 볼 값이 있다.**
2. **쿠버네티스로 옮긴 시점의 연도가 없다** — 평가에 6개월 걸렸다는 것만 안다.
   그래서 `2015-fury` 시기 안의 사건으로만 적었다.
