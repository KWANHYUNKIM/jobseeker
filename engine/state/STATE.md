...
**Elastic**(검색 엔진) — 회사 프로파일까지 썼다. **도메인 둘 · 기능 0.** 큐 0/3.

### 다이얼을 줄 것인가 말 것인가 — 타이거데이터와 정확히 반대다

압축 코덱 글의 물린 대안이 이번 프로파일의 알맹이다 — **"대안은 필드별 인코딩 선택을 설정
매개변수로 노출하는 것이었을 것이다... 그것은 어느 인코딩이 어느 자료형에 맞는지 아는
부담을, 그 판단을 할 준비가 가장 안 된 사람들에게 떠넘기는 일이 된다."**

**타이거데이터는 "누군가는 그 창의 너비를 골라야 한다"** 며 사용자에게 다이얼을 줬다.
Elastic 은 **주지 않기로** 했다. 같은 종류의 결정에 답이 정반대인 짝을 얻었다.

### 하나의 점수로 합치면 서로를 상쇄한다

샤드 밸런서도 같은 형태의 문제다. 가중합은 **한 지표의 높은 값이 다른 지표의 낮은 값을
지워** 아무 일도 안 하게 만들었고, 가중치를 손대면 **OOM 으로 이어질 수 있었다.** 그래서
**독립된 판정자 셋**으로 쪼개고 `NOT_PREFERRED` 를 더했다.

### 두 도메인이 같은 모양이다

**한 가지 방식을 모두에 적용하던 것을, 대상마다 다르게 판정하는 쪽으로** 바꿨다 — 샤드는
판정자 셋으로, 압축은 필드 메타데이터로. 그러면서 **그 판단을 사용자에게 넘기지는 않는다.**

### 자기 함대의 숫자를 그대로 낸다

**708TB · 인덱스 4,100개 · 샤드 3만 개 · 노드 37대 · 초당 19만 문서.** 샤드 이동을 아끼는
이유로도 **'서버 사이로 데이터를 옮기는 클라우드 인프라 비용'** 을 든다 — 관리형으로 굴리는
만큼 **자원 배치가 곧 원가**다.

### 다음 사이클

빈 도메인 둘 중 하나. **샤드 쪽이 자료가 두껍다**(실패 이유 넷 · 판정자 셋 · 감수 문장 ·
OOM 거의 0 · 이동 50% 감소 · 함대 규모). 압축 쪽은 수치가 선명하지만 **CPU 대가를 수치로
내지 않는다.**
