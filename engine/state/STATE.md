...
**없다 — Framework 를 완주시켰다(도메인 1 · 기능 1 · `status: done`).** 회사 160개, **큐 0/3.**

### 못 가는 건 모듈이 아니라 우리 보드다

이번 기능(`refuse-to-solder`)에서 가장 좋은 대목이다. 속도가 7467 MT/s 에서 멈추는 이유를
**"메인보드에 터무니없이 비싼 PCB 공정을 쓰지 않는 한"**이라고 적는다 — 모듈은 8533 까지 갈 수
있다. **한계의 주인을 정확히 짚는다.**

### 거절의 값을 먼저 적는다

속도 7467, 용량 64GB(DDR5 두 모듈은 128GB, 플랫폼은 96GB 지원), 소매 모듈 선택지도 아직 적다.
**진 자리를 먼저 적고** 그 대가로 얻은 것(업그레이드 가능)을 말한다.

### 따라온 이득도 적는다

압축 핀과 내장 금속 차폐 덕에 DDR5 보다 EMI 가 적어 **흡수 마일라 부품을 없앴고 와이파이
성능이 좋아졌다.** 계산해서 얻은 것이라기보다 옮기고 나서 따라온 쪽이다(재구성).

### 설계 원칙과 사업 모델이 같은 문장이다

납땜을 거절했기에 메모리·메인보드·확장 카드를 **따로 팔 수 있다.** 한 번 팔고 끝나는 대신
부품을 계속 파는 쪽에 건 회사다(재구성).

### 다음 사이클 — ⚠️ 후보 조사 (큐가 0/3 이다)

**음성·오디오 축이 비어 있다** — AssemblyAI 는 접었고 **Deepgram** 은 `/learn` 에 날짜가 없어
못 읽었다. 목록을 다시 받아 본문을 연다. 그 밖의 `blocked` — incident.io(404×3) ·
Sourcegraph(403) · Guardian · Flexport. **세 사이클 연속 이 목록에서 하나씩 되살렸다.**
