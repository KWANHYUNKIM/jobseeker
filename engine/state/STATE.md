...
**Rerun — 기능 둘.** 남은 빈 도메인은 질의 캐시 하나. 회사 135개, 큐 0/3.

### 장부가 데이터보다 무거워졌다

스칼라 하나를 기록하면 행이 하나 생기고, 그 행마다 색인을 달면 **8바이트 값보다 장부가 크다.**
작은 행이 많은 기록에서 색인이 적재 시간과 메모리를 지배했고 시간 점 10만에서 막혔다.

답은 데이터를 줄이는 게 아니라 **색인이 붙는 단위를 키우는 것**이었다 — 행에서 여러 행을 담은
애로우 열 청크로. 225만 점 기준 적재 100배·메모리 35배.

### 한 번 만든 것이 끝까지 그대로 흐른다

청크가 SDK 에서 저장소, 시각화까지 형태를 안 바꾸고 지나간다. 경계마다 변환하던 값이 통째로
사라지는 자리다. 질의의 종류는 그대로 둘(최신값·범위)로 두어 위층을 안 건드렸다.

### 두 작업이 서로의 대가를 받아 준다

청크를 키우면 질의가 느려지는데, 앞서 만들어 둔 최신값 캐시가 그 영향을 대체로 지운다.
적재 중 압축도 마찬가지다 — 지연과 메모리를 조금 내주고 전송 효율을 얻는다고 스스로 적는다.

### 큰 것과 작은 것을 한 구조에

점구름은 낮은 빈도의 큰 덩어리, 스칼라는 높은 빈도의 작은 값이다. 그래서 한도를 **행 수와
바이트 수 두 자**로 잰다(기본 4,096행·1MB).

### 다음 사이클 — 확장 (Rerun 의 마지막 빈 도메인)

`화면이 도는 동안 다시 꺼낸다`. 변경 기록 기반 무효화·잘게 나눈 잠금·픽셀 합치기·600ms→20ms
가 손에 있다.
