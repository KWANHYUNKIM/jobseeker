...
**SmartHR(JP · SaaS) `in_progress` — 58번째 회사.** 344 에서 **프로파일**을 썼다
(PROMPT.md 3단계). **도메인 1개 · 기능 0개.** 다음 사이클부터 기능이다.

**⑭ `--gaps` 가 [신규] 를 가리켰고 이번엔 그게 맞았다** — 진행 중인 회사가 없었다.

**열어 확인한 것** — `SaaS管理ユニットはなぜモジュラーモノリスから始め、マイクロサービスへ
移行しているのか`(2026-06-11, `/entry/2026/06/11/192713`). **판정 3/3.**
**⭐ ⑫ 가 또 값을 냈다** — 큐 메모에 없던 것이 여럿 나왔다: **2023년 10월 메타프스 클라우드
사업 양수**로 만들어진 팀이라는 것, **이관이 4단계**라는 것(Packwerk 로 의존 정리 → 메서드
호출을 HTTP 로 → 레포지토리 분리 → 데이터베이스 이관), **스트랭글러 패턴과 부패 방지층**,
**VPC 안 HTTP 통신**, **RubyDex · Sorbet · Caddy 신규 도입**.

**연 도메인 하나** — `한 덩어리 안에서 시작해 경계를 키운 뒤 떼어낸다`(**넓게 지었다**).
`tech` 셋을 그 한 편에서만 채웠다 — Packwerk · Public API 를 거쳐 HTTP 로 · 스트랭글러와
부패 방지층. **⚠️ 두 편 규칙이 아직 안 채워졌다.**

**⭐ 이 도메인의 핵심은 '떼어냈다' 가 아니라 '떼어낼 수 있게 두었다' 다** — 글의 결론이
그것이다: *"最初から完成形のアーキテクチャを選ぶことよりも、その時点で価値を届けやすい形を
選びつつ、後で変えられる余地を残しておくことの大切さです"*.

**사업 수치**(회사 보도자료 **제목에서 확인한 것까지만** 옮겼다) — **ARR 300억 엔 돌파**
(2026-07-07) · **등록 사수 80,000사 돌파 · 7년 연속 점유율 1위**(2026-04-28) ·
**2030년 매출 1000억 엔 목표**(2025-06-03) · **데이터 입력 없는 급여계산 제공 개시**
(2025-06-03) · **AI 어시스턴트를 포함한 종업원 포털이 새 사업의 기둥**. **⚠️ 영역별 매출
비중은 모른다.**

---

**다음 사이클 — SmartHR 의 첫 기능. 두 편 규칙부터 채운다.**
- **⭐ 1순위** — `認証がプロダクトの足かせにならない世界へ —— 認証・認可基盤ユニットの発足と挑戦`
  (2026-08-12). **같은 도메인일 가능성이 높고**(추정), **⭐⭐ freee 의 인증 기반 이관과 직접
  대비된다.** **⚠️ 223 규칙으로 실물을 보고 정한다** — 유닛 발족 이야기뿐이면 기준 미달일 수
  있다(**버린 대안 · 대가 · 수치 중 둘**).
- **⭐ 2순위** — `SmartHR最大のRailsアプリケーションにコネクションプーラーを導入しました
  ── 2年前の障害対応から学んだスケーラビリティと運用のリアル`(2026-08-04). **장애에서 출발한
  글이라 대가와 수치가 있을 가능성이 높다**(추정). **다른 도메인일 것이다.**
- **⭐ 3순위** — `開発チームにノウハウが残るSREの関わり方 —— 年末調整のピークに備えた負荷試験`
  (2026-07-24). **⭐⭐ freee 의 확정신고 정점과 대비된다** — 둘 다 **연 1회 날짜로 정해진
  부하 정점**을 앞두고 하는 일이다(추정).
- **4순위** — `横断フロントエンドユニットの取り組み`(2026-08-03) ·
  `不具合情報の集約基盤を整え、品質改善に活かすまで`(2026-08-24) ·
  `Sandboxテスト 〜フロントエンドとAI時代の変化の激しさへの挑戦〜`(2026-08-17).
- **⚠️ 이 블로그는 결이 넓다** — 2026년 8월에만 십수 편인데 **사내 행사·커리어 글이 절반쯤
  섞여 있다.** 재료를 고를 때 갈라야 한다.

**⚠️ 큐가 0 이 됐다 — PROMPT.md 사다리 3순위는 후보 조사다.** 다만 **회사 완주 규칙이 위라**
(CLAUDE.md) SmartHR 을 먼저 판다. **완주하면 곧바로 후보 조사 사이클**을 끼운다.
**⚠️ Personio(EU) 는 아직 안 봤다 — 첫 순위.** **후보 조사 누적 열아홉 곳 중 일곱.**

**⭐⭐ freee 와의 정반대 순서 대비**(`smarthr.json` 의 `open_questions` 에 적었다) — freee 가
후회로 적은 *"旧機能のリファクタはマイクロサービス化の前に着手すべし"* 를 이 회사는 순서로
밟고 있는 것으로 보인다(추정). **SmartHR 을 완주하면 비교 문서를 쓴다.**

**분포** — 58곳(**일본 일곱** · 인도 셋) · 비교 문서 32편 · **운영 사실 33개.**
...
않는다**(342) · **세 편을 열고 하나도 안 쓰는 사이클이 정상이다**(343) ·
**⭐ 새로: `domain_map` 은 문자열이 아니라 `{title, question, code}` 객체다 — 344 에서
문자열로 넣었다가 validate 가 죽었다.**
