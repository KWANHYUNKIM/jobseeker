...
**Ubicloud — 기능 셋.** 남은 빈 도메인은 값표 하나. 회사 131개, 큐 1/3.

### 이번 기능 — `burstable-cpu-slicing`

값으로 싸우는 회사는 한 대에 더 많이 올릴수록 이긴다. 작은 VM 들이 받은 코어를 늘 쓰지는
않으니 **안 쓰는 시간을 옆 손님에게 빌려준다** — 기준 50%, 최대 100%, 사실상 2대1.
8GB 구성이 월 100달러에서 25달러 미만이 되고, 가장 싼 칸은 월 10.47달러다.

### 싼 칸이 비싼 칸을 갉지 않는다는 것을 구조로 보인다

같은 회사가 1대1 과 2대1 을 함께 판다. cpuset 으로 **CPU 집합 자체를 갈라** 버스터블이
표준 VM 의 코어에 닿지 못하게 한다(`cpuset.cpus.partition: root`). 상품 두 개를 한 호스트에
올리려면 격리가 먼저다.

### 커널이 허용하는 만큼만 튄다

크레딧을 쌓아 몰아 쓰는 다른 클라우드의 방식 대신 `cpu.max.burst` 가 주는 짧은 구간만 쓴다 —
원장을 직접 만들지 않는 대신 **몇 분씩 몰아 쓰는 작업에는 안 맞는 상품**이 됐다.

### 불리한 숫자를 자기 입으로 적는다

경합이 가벼우면 약 30%, 모두가 꽉 채워 쓰면 0. 뒤 숫자를 빼면 더 좋아 보이는데 그러면 이
상품이 안 맞는 손님이 들어온다. 앞서 판 Depot 이 벤치마크의 한계를 스스로 적은 것과 같다.

### 다음 사이클 — 확장 (Ubicloud 의 마지막 빈 도메인)

`베어메탈 값과 클라우드 값의 차이를 판다`. 요금표는 확보했고 `ubicloud-price-adjustment-2026`
을 아직 안 읽었다 — 값을 올린 글이면 이 회사 구조의 가장 약한 데가 드러날 자리다.
