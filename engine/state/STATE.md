...
**Val Town — 프로파일만 썼다.** 회사 141개, 큐 0/3. 도메인 둘이 비어 있고 둘 다 자료를 확보했다.

### 한도가 전부 실행에 걸려 있다

무료가 하루 10만 회·실행당 1분·크론 15분, 프로가 월 21달러에 하루 100만 회·실행당 10분,
비즈니스가 월 167달러에 하루 500만 회. **파는 것이 실행이라 기술 문제도 하나로 모인다** —
남이 쓴 코드를 우리 기계에서 안전하고 싸게 돌리는 일이다.

### 네 번 갈아엎은 격리

노드 `vm`(문서가 대놓고 믿을 수 없는 코드에 쓰지 말라고 적어 둔 물건) → `vm2`(안전하지 않은
구조에 보안을 덧붙이려다 탈출이 계속 나와 폐기) → 데노 웹 워커(메모리가 새고 워커 안에서
워커를 띄워 탈출) → **임시 데노 하위 프로세스**(미리 띄운 풀로 콜드 스타트를 없애고 100밀리초
단축). 물린 대안도 이유와 함께 적는다 — QuickJS·val 마다 도커·JS 안 샌드박스·Realms.

### 격리가 완벽해도 안에서 부르는 것이 바뀌면

`npm:lodash` 처럼 고정 없는 임포트는 그때그때 최신으로 풀려 잘 돌던 val 을 깨뜨린다. 저장할 때
잠금 파일을 자동으로 만들고 무결성 검사를 붙이니 **모듈 요청이 80% 줄고 워커 초기화 지연
중앙값이 50% 떨어졌다** — 대가는 남의 val 을 임포트할 때 다시 저장해야 하는 불편이다.

### 이 엔진에서 견줄 자리

남의 코드를 대신 돌리는 회사가 여럿인데(Depot 은 빌드, Ubicloud 는 VM, Railway 는 배포) 여기는
**조각 코드 하나**가 단위다. 그래서 도커나 VM 을 일찍 물렸다.

### 다음 사이클 — 확장 (Val Town 의 빈 도메인)

`남의 코드를 우리 기계에서 돌린다` 부터. 네 판의 변천이 연도까지 붙어 있어 feature history 를
세울 수 있는 드문 자료다.
