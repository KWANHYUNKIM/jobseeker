...
**Target (US · 커머스/리테일 물류) — 42번째. 기능 2개. 열어 둔 도메인 2개가 다 찼다.**

- 220 — **`split-shipments`**(주문 분할). 폭발이 두 번(**2ⁿ−1** 배송 후보 · **벨 수 Bₙ** 이행안),
  그리디가 지는 이유는 **계단형 비용 함수**. 그래서 그리디를 **출발점으로만** 쓴다.
  **계산 시간 8% 로 격차 91%.**
- 221 — **`infra-showback`**(청구서 대신 점수판). **차지백을 명시적으로 버렸고**,
  점수는 **제품 단위**, 재료는 **ITDP** 한 곳, 축은 셋(비용·기술부채·안정성).
  **주문당 비용 4년간 33% 절감 · 기술부채 10% · 안정성 12%.**

**221 에서 겹쳐 읽어 얻은 것** — DBaaS 글이 *"쇼백 API 로 테넌트 대시보드에 사용량·비용
가시성을 제공"* 하면서 동시에 *"쿼터가 걸린 온디맨드 프로비저닝"* 을 쓴다.
**보여주는 층과 실제로 막는 층이 따로 있다.** ⚠️ **두 글이 서로를 언급하지 않으므로
이건 이 사이트의 해석이고, 그렇게 `open_questions` 와 결정의 대가 칸에 밝혀 뒀다.**
**"기능을 쓸 땐 두 편 이상" 규칙이 열한 번째로 맞았다** — 한 편만 봤으면 이 대비를 놓쳤다.

**⚠️ 다음 사이클의 선택** — 열어 둔 도메인이 둘 다 찼다. 둘 중 하나다.
- **세 번째 도메인을 연다 — 추천·개인화(1순위).** 자료가 없어서가 아니라 이 사이트에
  그 축이 이미 두꺼워 미뤄 둔 것이다. 첫 페이지에만 네 편 —
  `targetrun-shopping-offers`(2026-05) · `page-layout-optimization-bandits`(2026-04) ·
  `accessory-recommendations-with-llms`(2025-07) · `buy-it-again-part-2`(2025-03).
  **리테일 쪽 결이 있는지 먼저 한 편 열어 보고 정한다.**
- **또는 `주문을…쪼갤 것인가` 의 두 번째 기능** —
  `math-behind-inventory-delivery-optimization`(2025-11, 들어오기 전 재고 배치).
  ⚠️ **버린 대안이 명시돼 있지 않아 결정 5개를 채우기 어려울 수 있다 — 쓰기 전에 세어 본다.**
- **또는 Target 을 done 으로 닫는다.** 그러면 큐가 1/3(bol.com 뿐)이라 **곧바로 후보 조사**다.

**🚫 분류별 목록은 못 찾았다.** `?category=Infrastructure` 는 **404** 이고 첫 화면의 분류
링크는 **href 가 본문에 안 실린다**(JS 로 붙는 듯). **전체 7페이지(약 100편) 중 첫 페이지
15편만 셌고 페이지 URL 형식도 모른다.** 새 방법이 없으면 다시 두드리지 않는다.

**읽었지만 안 쓴 글** — `synthetic-digital-orders`(2025-07-22). 합성 주문(Demand Profiler),
**이행센터 단위 통합 2배 · 향후 유닛 배분 정확도 약 40% 개선**. ⚠️ **버린 대안도 대가도
없어 기능으로 쓰기 얕다.** 이행 최적화와 이어지는지도 확인 못 했다.

**⚠️ Target 을 닫으면 큐가 1/3(bol.com 뿐)이라 후보 조사 사이클이 필요하다.**
