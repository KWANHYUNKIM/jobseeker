...
**없다 — 예순세 번째 후보 조사를 돌았다. 큐 1/3.** 다음 사이클은 **신규**(Mapbox).

### 거절한 이유를 못 박는 글

Mapbox 의 Vulkan 전환 글이 통과한 근거다 — **"아니다, 자동 백엔드 선택은 없을 것이다.
자동 백엔드 선택은 두 백엔드를 모두 포함해야 하는데 그것이 바이너리 크기에 영향을
준다."** 왜 안 하는지를 **비용으로** 적는다. 성능 이유도 구조로 설명한다 — **OpenGL
드라이버는 호출마다 검증 부담을 강제하는데 Vulkan 은 그것을 최소화한다**(프레임 시간 약
10% 감소).

### 지킨 것과 버린 것을 갈라 적는다

OpenGL 백엔드를 남겨 **개발자가 고르게** 하되, 미리보기에서 커스텀 레이어와 위젯 겹치기를
빼고 지원 범위를 **Android 12+ · Vulkan 1.1+ · arm64-v8a** 로 좁혔다.

### 글 하나로 회사를 판정하면 안 된다 — 이번엔 통과 쪽에서

같은 회사의 GL JS 모듈화 글은 수치가 있는데도(번들 10~15% · time-to-idle 최대 50%)
**물린 대안 없이 이득만 적는다.** 모질라·코크로치·리플리케이트에서는 **두꺼운 글과 얇은
글**이 섞였는데, 이번에는 **어느 글을 먼저 열었느냐로 판정이 갈릴 뻔했다.**

### HashiCorp 는 못 읽었다

첫 화면이 전략·제품·고객 사례가 지배해 엔지니어링 글이 안 보였고, **첫 화면으로 판정하지
않는다는 규칙대로** `/blog/all` 을 열었더니 **500** 이다. 접은 것이 아니라 못 읽은 것이다.
