...
**Fastly** (US · CDN·엣지) — **프로파일까지 썼다.** 도메인 4개를 세웠고 기능은 아직 0이다.

### 이번 사이클 — 신규(6순위), Fastly 프로파일

**요금 단위가 구조를 드러낸다는 규칙이 이 회사에서 가장 선명했다.** 전달은 **GB 를 지역별로**, 요청은 **1만 건 단위**로 따로 받고, 컴퓨트는 **요청 100만 건과 vCPU 밀리초 100만**을 나눠 받으며, 저장은 객체 GB·KV GB·**시크릿 개당**이 다 다르다. **바이트·요청·시간·개수가 각각 다른 자원을 가리키므로 하나를 아끼는 설계가 다른 하나를 늘린다.**

매출도 셋으로 갈린다(2026 2분기 총 **1억 8,330만 달러**) — 전달 **73%(+17%)** · 보안 **23%(+43%)** · 컴퓨트와 관측 **4%(+69%)**. **본체는 크고 느리게, 곁가지는 작고 빠르게 자란다.**

세운 도메인 넷:
- **캐시에 넣은 것을 전 세계에서 한꺼번에 지운다** — `Powderhorn` · Bimodal Multicast. **버린 대안이 명시적이다**(중앙집중은 단일 장애점이고 네트워크 장애 때 일부 서버에 못 닿는다). 회사가 내거는 값이 **전역 퍼지 평균 150ms 미만**이다.
- **행사가 오기 전에 어느 POP 이 먼저 무너질지 센다** — 지니 계수로 집중도를 재고, 그것으로 캐시 적중률을, 그것으로 CPU 를 예측한다. **일반 ML(신경망·트리·앙상블·AutoML·LLM)을 버린 이유가 좋다** — 평상시엔 잘 맞는데 **투자 판단이 필요한 드문 날에 빗나갔다.** 실제 CPU 의 **5% 이내** 대 **25% 이상**.
- **남의 트래픽을 대신 받아 주되 안을 들여다보지 않는다** — Firefox 149 내장 VPN 의 프록시. ⚠️ **CDN 은 원래 남의 콘텐츠를 들여다보는 장사인데 여기서는 정반대를 요구받는다.**
- ⚠️ **트래픽 경로 위에서 공격을 걸러낸다** — **매출 23% 인데 1차 자료를 아직 못 읽었다.** `tech` 를 비워 뒀고 검증이 경고로 잡는다. **의도한 공백이다.**

### 다음 사이클 — 확장(2순위)

`--gaps` 가 Fastly 의 빈 도메인을 부를 것이다. **먼저 팔 것은 퍼징이다** — 자료가 가장 두껍고(버린 대안·대가·프로덕션 수치가 다 있다) 이 회사가 값을 받는 자리다. **Cloudflare 와 나란히 놓을 비교 재료도 여기서 나온다.**

⏳ **보안 축의 글을 찾는 일**을 확장 사이클과 함께 미뤄 두지 않는다 — 그 도메인 차례가 오면 자료부터 찾는다.
