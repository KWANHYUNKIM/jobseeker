...
**컬리 (KR · 커머스/신선식품) — 47번째. 기능 2개 · 도메인 2개. 열어 둔 도메인이 다 찼다. 큐 잔량 0/3.**

261 에서 **`inbound-sync`(늦게 오는 것을 기다린다)** 를 썼다. 결정 7 · 그림 3 · 생각 4 · 스택 6 ·
난제 2 · ui 5핀 · 연결 1(`misdelivery-detection` 과 **대비**).

**⚠️ 두 번째 글은 또 다른 문제였다.** `Kafka Streams 윈도우 도입기`(2025-12-01)는 **재고 정산**이다 —
주문 마감 시간대의 몰림을 **텀블링 윈도 5분 · grace 1분**으로 모아 DB 부하를 줄인다. 이 도메인에
안 넣었다(223 의 규칙). **버린 대안은 배치**(*"데이터가 누적될 때까지 기다려야 하므로 실시간성 확보가
어렵고"*)이고 **대가가 넷**이다(검증이 몇 분씩 걸린다 · 날짜 경계 · **Grace 밖 데이터는 유실** ·
**입력이 멈추면 창이 안 닫혀 더미 이벤트를 3분마다 넣는다**). **세 번째 도메인 후보로 남겼다.**

**📌 원문 재독이 열 번째로 옳았다**(223·229·243·249·250·252·255·258·260·261). 여섯이 더 나왔다.
- **🔴 재시도 창 24시간의 근거가 업무 데드라인이었다** — *"일반적으로 입고 예정 정보는 1~2일쯤 전에
  미리 생성을 합니다. 즉, 입고 예정 정보가 생성되고 나서 실제 상품이 물류 센터에 도착하는 데까지
  걸리는 이 1~2일의 시간, 이 시간 텀이 이 메시지가 처리되어야 할 데드라인이라고 볼 수 있습니다."*
  **못 맞추면 실물이 먼저 도착한다는 물리적 사실이 설정값이 됐다.**
- **Outbox 테이블 스키마**(`id · status · aggregate_id · event_type · payload · created_at ·
  completed_at · retry_count · next_retry_at · partition_no`)와 **라이브러리가 대신하는 범위**
  (*"아웃박스 테이블 관리, 폴링 스케줄링, 재시도 횟수/상태 관리 등을 라이브러리에 위임"*, 직접 쓰는
  것은 `OutboxRecordProcessor` 뿐).
- **버린 이유의 정확한 문장** — *"발행부와 수신부의 책임을 명확히 분리하고 싶었습니다."*
- **DLT 핸들러의 한 줄** — *"컨슘 실패 알람이 이제는 오히려 반가워졌습니다!"*
- **멱등성은 소비자에게 맡긴다** — *"Kafka 리스너에서 멱등성을 보장할 수 있는 설계가 미리 마련되어야
  합니다"* 라고만 적고 **구체적 기법은 없다.** ⚠️ **이 기능의 정확성이 이 글이 다루지 않는 곳에 있다.**
- 수치가 더 있다 — 라이브러리 기본 재시도 3회 · executor 코어 4/최대 8 · graceful shutdown 15초 ·
  죽은 인스턴스 감지 30초 · heartbeat 5초.

**🔴 두 기능이 알림을 다루는 생각이 같다**(이 대비는 이 사이트의 것이다) — 오배송 탐지는 *"양치기
소년 현상"* 을 피하려 **채널마다 임계를 다르게** 뒀고, 입고 동기화는 **재시도 중의 실패를 안 알리고
최종 DLT 만 크게** 알린다. **한쪽은 임계로, 한쪽은 토픽으로 알림을 갈랐다.** ⚠️ **두 글은 서로를
언급하지 않는다.**

**🚨 다음 사이클 — 세 번째 도메인을 열지 컬리를 닫을지 정한다.**
**후보 셋 중 둘은 이미 읽었다:**
- **재고 정산**(Kafka Streams 윈도, 2025-12-01) — 버린 대안 하나 + 대가 넷. **`inbound-sync` 와 같은
  성질을 공유한다**(미룸에 기한을 둔다: 24시간 vs Grace Period).
- **딜리버리 암호화**(2025-06-02) — **버린 대안 둘에 이유가 각각 명시적**(JPA `@PostLoad` 는 *"값이
  바뀌면서 dirty로 체크되었기 때문입니다"*, Hibernate PreLoad 는 *"6.2.0에서야 고쳐졌고"* 일정에 못
  맞았다). 수치도 촘촘(**300만 개 암호화 5.58초 · 복호화 5.36초 · 헤더 10~30% 절감**).
- **`access-block` 2부작**(무중단 점검, 2025-10-10) — **미개봉이지만 2부작이라 그 자체로 묶음이다.**
**⚠️ 셋이 서로 다른 문제다** — 정산 · 개인정보 · 점검. **한 도메인에 뭉치지 말고 하나를 골라 연다.**
**둘 다 이미 읽었으니 도메인을 열면 그 자리에서 기능까지 쓸 수 있다.**

**⚠️ 사업 수치를 하나도 못 찾았다** — `kurlycorp.com` **DNS 없음**, `kurly.com` 회사 소개에 수치 없음.
매출·주문 수·활성 고객·물류센터 규모·상장 여부·3PL 비중 전부 모른다. **비워 뒀다.**
...
- **새 축 `관측에 값을 무엇으로 치르는가` — 일곱이 됐다.** CyberAgent `observability-pipeline`(돈) +
  `alert-context`(주의력) + **컬리 `misdelivery-detection`(임계로 알림을 가른다)** +
  **컬리 `inbound-sync`(토픽으로 알림을 가른다 — *"컨슘 실패 알람이 이제는 오히려 반가워졌습니다!"*)** +
  Yelp `partition-access` + Zerodha `log-storage` + Doximity `prod-profiling`. **한 회사 안에서 같은
  생각이 두 번 다른 방식으로 나온 것이 이 축의 좋은 재료다.**
...
⑫ **요약이 아니라 원문을 다시 읽는다** — 223·229·243·249·250·252·255·258·260·**261** 열 번 다 새 결정을 찾았다.
