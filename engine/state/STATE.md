...
**Tigris Data — 기능 둘을 채웠다.** `fdb-metadata` 와 `push-metadata-pull-data`(메타데이터는 밀고
데이터는 당긴다). 회사 125개, 큐 1/3. 빈 도메인 하나 — S3 호환.

### 목록이 안 온다고 자료가 없는 것은 아니었다

지난 사이클에 심층 글이 하나뿐이라고 적었는데, 검색으로 찾으니 더 있었다 — `global-replication`
(2025-04-01) · `multi-region-dual-region-buckets`(2026-03-24) · 안티테시스로 카오스 시험한 글 · 문서.
**블로그 목록 페이지가 WebFetch 로 한 편만 돌려준 것이었다.** 목록을 못 읽으면 검색으로 개별 글을
찾는다는 절차를 하나 더 얻었다.

### 무엇을 밀고 무엇을 당길지를 갈랐다

모든 바이트를 모든 리전에 두면 저장 값이 리전 수만큼 곱해지고, 한 곳에만 두면 먼 사용자가 매번
왕복을 낸다. 답은 **가르는 것**이었다 — 어디 있는지에 대한 기록(가볍다)은 모든 리전에 밀어 두고,
실제 바이트는 요청한 사람이 있을 때만 당긴다. 메타데이터에 사본이 있는 리전 목록이 들어 있어 어느
리전이 어디서 가져올지 안다. 읽기는 FDB → SSD 캐시 → 로컬 블록 → 원격 네 층으로 내려간다.

### 자동 배치의 대가를 유형 넷으로 갚았다

접근 패턴을 따라 옮기는 것이 이 제품의 차별점인데, 규제를 받는 쪽에는 그 차별점이 그대로 결격
사유가 된다. 그래서 버킷 위치 유형을 넷으로 나눴다 — 전역·멀티 리전·듀얼 리전·싱글 리전. **같은
리전 읽기는 넷 다 강한 일관성이지만 리전을 넘으면 전역과 듀얼은 잠시 옛 사본을 돌려줄 수 있다.**
GDPR 44조를 직접 들며 거주지 요구를 멀티·듀얼이 받는다고 적는다. 가격도 갈린다(멀티만 0.025, 나머지 0.02).

복제 지연도 감추지 않는다 — 0이 아니고 대개 한 자릿수 초이며 리전이 돌아오는 동안에는 조금 느려진다.

### 다음 사이클 — 확장 (마지막 빈 도메인)

S3 호환을 채우면 Tigris 가 done 이 된다. 그리고 안티테시스 글은 **이 엔진의 회사가 남의 글에
고객으로 등장하는 자리**라 따로 적어 뒀다.
