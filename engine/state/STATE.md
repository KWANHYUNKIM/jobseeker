...
**Ramp** (US · 핀테크) — **프로파일까지 썼다.** 도메인 3개, 기능은 아직 0이다.

### 이번 사이클 — 신규(6순위), Ramp 프로파일

⚠️ **이 회사가 무엇을 파는지가 기술 구조를 그대로 정한다** — 파는 것은 카드가 아니라 **`이 지출이 되는가` 를 판정하는 규칙**이다. **고객마다 규칙이 다르므로 코드로 박을 수 없고, 그래서 그래프가 된다**(재구성). 그 엔진이 **하루 100만 건 이상**을 돈다.

⚠️ **그리고 만드는 방식에도 같은 수법을 쓴다** — 승인 로직을 그래프로 추상화하듯, **ML 배포를 YAML 로**(첫 모델 **넉 달·3,000줄 → 5분 미만·약 40줄**), **코딩을 사내 에이전트로**(병합 PR 의 약 30%) 옮겼다. **세 도메인이 모두 `직접 짜지 말고 틀을 만들어라` 의 변주다.** 회사가 그 태도를 한 문장으로 적는다 — **`We decided to slow down, in order to go fast`.**

세운 도메인 셋:
- **지출이 되는지를 그래프가 판정한다** — 액션·조건 정점의 DAG, **Postgres 에 AST 저장**(쓰기를 복잡하게, 실행을 단순하게). ⚠️ **첫 설계가 실패한 이유를 적는다**(선형 논리를 규칙 형태로 옮겨야 했고 액션 간 의존이 어려웠다). ⚠️ **Celery 를 버린 이유가 `1만 개당 7개 실패`** 다 — 승인 시스템에서 조용히 빠지는 것은 치명적이다(재구성).
- **모델 배포를 넉 달에서 5분으로 줄인다** — 금융이라 **재현성·설명 가능성이 규제 요구**다. ⚠️ **표준과 코드 리뷰로 다스리는 길을 명시적으로 버렸다.** 대가도 적는다 — `as long as your data hasn't changed`.
- **코딩 에이전트를 사서 쓰지 않고 직접 만든다** — 이유가 한 문장이다: **`it only has to work on your code`.**

⚠️ **사업 수치가 전부 3자 출처다**(Sacra·CNBC) — 연환산 매출 15억 달러 · 고객 7만 곳 이상 · TPV 570억 달러 · 밸류에이션 440억 달러. **수익 구성(인터체인지 대 구독)은 못 찾았다.**

### 다음 사이클 — 확장

**워크플로 도메인이 가장 익었다** — 버린 대안 둘(첫 설계·Celery)에 이유가 붙어 있고 수치도 촘촘하다(4,500만 건·하루 100만·쓰기 20초→200ms·실행 100ms). ⏳ **연도를 확인하면 기능 안에 `첫 설계 → 지금 구조` 연표를 세울 수 있다.**

⚠️ **비교 문서는 Ramp 완주 뒤다** — `사내 AI 에이전트를 어디까지 믿을 것인가` 에 Sentry·ClickHouse·Duolingo 가 찼고 **Ramp 가 네 번째다.** 축은 `얼마나 맡기나` 가 아니라 **`그 선을 무엇이 지키나`** 다.
