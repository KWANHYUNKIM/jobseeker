...
**Hatena — 프로파일만 썼다(도메인 셋, 기능 0).** 회사 150개, **큐 0/3**.

### 오래된 것을 굴리는 쪽과 남의 것을 대신 굴리는 쪽이 한 회사에 있다

2001년에 시작해 25년째다. 하테나 블로그는 아직 **Perl** 로 돌고, GigaViewer 는 **스무 곳 넘는
만화 서비스**를 한 기반으로 낸다. 그 사이를 잇는 것이 **"무엇을 쓸지 매달 재서 고른다"**는
습관이다 — SRE 연재가 Fargate·Aurora·GitHub Actions 러너·TiDB 를 차례로 재고 그 수치로 고른다.

### 독립성과 값이 정면으로 부딪히는 자리

테넌트마다 전담 디자이너와 독자 마케팅이 있어 **기반이 그 요구를 제약하면 안 된다.** 그런데
서비스마다 기반을 따로 두면 테넌트가 늘 때마다 값이 그대로 는다. 답은 **사일로와 풀을 섞은
다리 구조** — CDN 은 서비스별 전용(CloudFront·인증서·도메인), 로드밸런서와 nginx 는 공유.
1분에 10배 치솟던 트래픽이 **최대 2.7배 변동**으로 잦아들었다.

### 범용 벤치만 보면 잘못 고른다

이 회사의 벤치마크 글이 그걸 수치로 보인다 — c8g 는 DKbench 로 **1.03** 인데 실제 운영 코드로는
**0.91** 이다. 값 대비 성능 순위가 통째로 뒤집힌다. 그래서 범용 벤치와 자체 마이크로벤치를 같이
돌린다. **Fargate 는 CPU 뽑기라 최저 성능이 평균의 75%**까지 내려가 예측이 안 된다고 뺐다.

### 4MB 짜리 정규식

키워드 자동 링크가 **30만 개 넘는 키워드로 4MB 를 넘는 정규식**을 만든다. Perl 5.42.0 으로
올리자 컴파일에 기가바이트가 들어 OOM 킬러가 프로세스를 죽였다. 원인은 **5.29.9 의 변경** —
긴 점프가 필요한지 확인하기 전에 명령열을 거의 다 만들어 놓고, 필요하면 통째로 버리고 다시
컴파일한다. **못 고쳐서 기능을 일단 접었다가** 나중에 고쳤다.

### 다음 사이클 — 확장 (첫 도메인)

`한 기반으로 남의 서비스 스무 곳을 낸다` 가 자료가 가장 두껍다(GigaViewer 글 하나에 구조·수치·
대가가 다 있다). `무엇을 쓸지 매달 재서 고른다` 는 벤치마크 글이 수치를 잔뜩 준다.

### ⚠️ 큐가 0/3 이다

Hatena 를 완주시킨 뒤 **후보 조사 사이클을 끼운다.**
