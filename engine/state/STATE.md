...
**없다 — 쏘카를 닫았다.** 도메인 2 · 기능 2 · `status: done`.
`discount-self-heal`(결정 8) · `reservation-tetris`(결정 6) · 그림 6.
**완료 62곳 · 큐 대기 1(카카오뱅크) · 비교 문서 33편.**

### 다음 사이클 — 사다리 6순위 **신규**, 카카오뱅크

파는 중인 회사가 없고 재방문·보강도 비었으므로 `--gaps` 가 **신규**를 가리킬 것이다.
**큐 대기 맨 위 = 카카오뱅크**를 `in_progress` 로 만들고 3단계(회사 프로파일)부터 판다.
큐에 근거가 이미 적혀 있다 — `tech.kakaobank.com`, WebFetch ✅, 두 글을 본문까지 확인했다
(`2605-no-wait-collateral-review-system-design` 은 버린 대안과 대가, `2606-sequence-based-fds-model`
은 버린 대안과 수치. ⚠️ 두 글 다 정확도 지표는 없다).
**은행 라이선스를 가진 곳이 아직 없어 심사·여신·FDS 가 새 도메인이 된다.**

**⚠️ 그 다음은 후보 조사다.** 카카오뱅크를 꺼내면 큐가 **0/3** 이 된다 — 사다리 3순위가 곧바로
걸리므로, 카카오뱅크를 닫은 뒤가 아니라 **큐가 0 이 되는 순간부터** 후보 조사가 우선한다.

### 비교 문서 재료가 하나 생겼다 — 아직 안 썼다

**"내가 못 고치는 것이 조용히 바뀐다"** 가 두 회사에서 같은 모양으로 나왔다.
- **ZOZO** `빌린 모델이 조용히 나빠질 때` — 관리형 추천 API. **"종류·분포·건수가 모두 정상인 채
  추천의 내용만 열화하는 경우는 검지할 수 없다"**
- **쏘카** `남의 화면이 바뀌면 우리가 환불한다` — 장비사 50여 곳. **실패 없는 장비사의 조용한
  변경은 미탐지**

**둘 다 실패를 신호로 삼기 때문에 실패 없는 변경을 못 잡는다.** 세 번째 회사가 나오면 쓴다.

### 쏘카 자료 접근 — 재방문 사이클을 위해 남긴다

- **`tech.socar.kr` 은 WebFetch 로 본문까지 열린다.** (`tech.socarcorp.kr` 에서 301 — 리다이렉트
  주소로 다시 요청해야 한다.) 카테고리별 편수: **데이터 24 · 백엔드 19 · 웹 프론트엔드 7 ·
  Mobility 5 · Design 5 · Product 4 · QA 3 · 보안 2 · 디자인 시스템 2.**
- **IR 은 회사 블로그가 대신한다.** `socarcorp.kr/blog/689`(2026-02-25)에 FY2025 수치가 표로 있다.
- **⚠️ `eras` 를 세울 재료가 반쯤 있다.** 예약 테트리스가 **"2020년 이전"** 수동 배정 →
  **2020년 자동화 완성**이라고 적는다. **바뀐 지점과 연도가 둘 다 있는데 그 앞이 언제부터였는지가
  없어** 첫 시기를 못 열었다. 다른 글에서 잡히면 그때 세운다.
- **아직 안 판 자리** — Mobility 5편·보안 2편·QA 3편은 열어 보지 않았다. 차량 상태(정비·청결)나
  사고 대응 쪽이 세 번째 도메인이 될 수 있다.

### 이 세션에서 배운 것 — 다음 회사에도 걸린다

- **제목의 마지막 낱말이 기능으로 쓸 수 있는지를 가른다**(ZOZO). 「検証」은 오프라인,
  「構築と運用」·「対策」은 운영이었다. **본문에서 '본번·릴리스·A/B' 를 먼저 찾는다**(353 의 연장).
- **남에게 의존하는 자리는 그 자체로 도메인이 된다**(ZOZO·쏘카 둘 다).
- **글이 한도·쿨다운·실행 주기를 숫자로 적으면 결정이 두세 개 더 나온다.** 쏘카가 그랬다 —
  '언제 포기하는가'(시도 3회·3분·confidence 0.5), '얼마나 자주 도는가'(1시간에 1번).
- **⭐ 새로: 버린 대안이 인프라 선택에도 있다.** 쏘카는 **AWS SQS 를 페이로드 256KB 한계로 버리고
  GCP Pub/Sub(10MB)** 을 골랐다고 적는다. **큐·인스턴스 선택도 결정 칸에 들어간다** — 아키텍처
  이야기만 결정으로 보면 이런 줄을 놓친다.

### ⚠️ 다른 세션과 같은 큐를 잡지 않는다

**사용자가 여러 세션을 동시에 돌리고 있다.** `guide-engine`(쿠팡)·`study-engine`(JWT·Docker) 둘 다
계속 커밋 중이다. **사이클 시작 전에 `git log --oneline -3` 을 보고, 커밋할 때 `git add -A` 를
쓰지 않는다** — 다른 세션이 작업 중인 파일이 워킹트리에 함께 떠 있다.
