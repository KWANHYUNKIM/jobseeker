...
**Snyk** (UK/IL · SaaS — 개발자 보안) — 도메인 2개 · **기능 1개**(`intelligence-at-inference-not-training`). 남은 도메인은 **`기계가 고친 것을 믿어도 되는지 잰다`** 하나이고 **자료는 이미 읽었다.**

### 이번 사이클 — `모델을 고치지 않고 그 순간에 아는 것을 바꾼다`

건진 것 넷:

- ⚠️ **지원 범위가 학습 비용에 묶여 있었다** — 예전에는 **새 언어나 규칙마다 수천 개의 수동 표본과 긴 파인튜닝**이 필요했다. **제품의 넓이가 데이터 라벨링 속도의 함수가 된다**(재구성). **추론 시점 주입으로 옮기자 같은 데이터베이스가 모든 언어를 동시에 덮는다.**
- ⚠️ **파는 것이 모델이 아니라 데이터라는 것을 자기 벤치마크로 증명했다** — **모델만 놓고 보면 StarCoder 72.4% · Gemini 3.1 Pro 74.2% · Sonnet 4.6 72.4% · Opus 4.6 74.6% 로 서로 붙어 있다.** **차이는 데이터를 붙였을 때 났다** — Opus **74.6% → 85.4%**, 파이썬은 **64.0% → 88.0%.**
- ⚠️ **거르는 것과 고치는 것은 다르다** — 안전하지 않은 답은 **지금도 언제나 걸러져 고객에게 가지 않는데**, 예전에는 **거기서 끝나 개발자에게 아무 수정도 안 남았다.** 지금은 **실패 이유를 뽑아 되먹이고 다시 생각하게 한다.**
- ⚠️ **찾는 제품이 고치는 제품의 채점자가 됐다** — 만든 수정을 자기 스캐너로 다시 스캔한다. **검증이 공짜에 가까운 대신, 스캐너가 못 보는 취약점은 이 고리에서도 안 보인다**(재구성).

### ⚠️ 이번에 찾은 이 구조의 빈 곳

**스캔은 `안전한가` 만 말하고 `여전히 동작하는가` 는 말하지 않는다.** 회사가 벤치마크 글에서는 그 위험을 분명히 적는데(**SQL 주입을 없앴는데 질의 결과가 달라졌으면 진짜 해결이 아니다**), **수정 고리의 설명에는 기능 관문이 없다.** ⚠️ **한 회사의 글 두 편을 나란히 놓아야 보이는 빈 곳이다** — 딜리버루에서 두 글의 어긋남을 잡은 것과 같은 종류다.

### 다음 사이클 — 마지막 도메인, 그리고 완주 판정

`--gaps` 가 **`기계가 고친 것을 믿어도 되는지 잰다`** 를 부를 것이다. **벤치마크 글의 본문을 이미 읽었으니 바로 쓴다.** 같은 사이클에서 **완주 판정까지** 한다(도메인 2/2 · 고아 없음 · 수익원 둘 다 연결).
...
**① `AI 에이전트를 어디까지 믿나` 7곳** — Sentry / ClickHouse / Duolingo / Ramp / DoorDash / Deliveroo / **Snyk**. ⚠️ **Snyk 의 자리가 다르다** — 앞의 여섯은 **에이전트가 만든 코드를 어떻게 감독할까**를 묻는데 여기서는 **AI 가 고치는 대상이 보안 취약점 자체**이고, **유일하게 `기계가 고친 것을 어떻게 잴까` 에 벤치마크로 답한다.**
