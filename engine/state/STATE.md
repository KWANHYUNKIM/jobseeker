...
**Ubicloud — 기능 둘.** 남은 빈 도메인은 둘(버스터블 · 값표). 회사 131개, 큐 1/3.

### 이번 기능 — `layered-guest-isolation`

빌린 베어메탈 한 대에 여러 손님을 올리면 옆 손님 코드가 안 넘어온다는 것이 상품의 조건이다.
하이퍼스케일러는 전용 하드웨어로 푸는 자리를, 남의 기계를 빌리는 쪽은 **리눅스에 이미 있는
격리를 겹쳐 쌓아** 푼다 — 일반 사용자로 실행 · systemd 권한 제한 · 네트워크 네임스페이스 ·
nftables · 파일 권한.

### 재일러를 만들지 않았다

레드햇은 libvirt 가 SELinux 로, 파이어크래커는 chroot·네임스페이스·seccomp 를 묶은 자체
재일러로 가둔다. 이 회사는 그 자리를 systemd 와 일반 사용자로 메운다 — **만들 것이 없으면
유지할 것도 없지만, 남들이 한 덩어리로 묶어 둔 것을 여러 설정에 흩어 두는 셈**이다.

### 있는 방어와 켠 방어를 구분해 적는다

seccomp-bpf 가 클라우드 하이퍼바이저에 내장돼 있는데 **아직 켜지 않는다고 글에 적는다**
(프로젝트가 이미 시스템콜 표면에 민감하다는 것이 근거). 문서만 보면 구분이 안 되는 것을
스스로 밝힌 자리라 결정 칸에 그대로 남겼다.

### 파이어크래커를 물린 이유가 사업이다

윈도우를 올리고 GPU 를 통과시켜야 하는 요구가 최소 장치 모델과 정면으로 부딪힌다. 일반 목적
VMM 을 고른 값이 넓은 표면이고, 그 값을 바깥 겹으로 받친다.

### 다음 사이클 — 확장 (Ubicloud 의 빈 도메인)

`한 코어를 여럿에게 판다` — cgroups v2 버스터블. 2대1 과다할당, 경합 가벼우면 30% 이득이고
모두 꽉 채워 쓰면 0, 크레딧이 안 쌓인다는 한계까지 이미 손에 있다.
