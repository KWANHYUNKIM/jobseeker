...
**없다 — 문체 보수 사이클이었다.** Atlassian 을 배민 문체로 다시 썼다(2.7 → 0.0). 회사 115개, 큐 2/3. 62곳 남았다.

### 실패했을 때 무엇이 남느냐 — 넷이 같은 자리에 섰다

Atlassian 의 요구 하나가 이 축을 열었다. 샌드박스가 실패해도 대화는 계속되어야 한다는 것이다.
그런데 이 엔진의 다른 회사 셋이 정확히 같은 자리에 있다.

- Anthropic 은 뇌와 손과 세션을 갈랐다. 컨테이너가 실패하면 그것을 도구 호출 오류로 받고
  세션 로그는 밖에 남긴다.
- Vercel 은 격리가 이그레스 통제 없이는 결과를 못 가둔다며 자기 클라우드 안에 경계를 세웠다.
- Cursor 는 그 경계를 고객 네트워크로 옮기되 이그레스 통제와 비밀 삭제를 따라 보냈다.
- Atlassian 은 샌드박스가 죽어도 대화가 살아남게 제어 평면을 분리했다.

넷 다 에이전트를 어디까지 믿을지가 아니라 실패했을 때 무엇이 남느냐를 설계한다.
AI 를 어디까지 믿나 축과는 다른 물음이다. 저쪽은 판단의 문제이고 이쪽은 경계의 문제다.

### 이번 사이클 — Atlassian

사업 모델에서 수가 아니라 크기로 자란다는 것을 앞세웠다. 순매출유지율이 4분기 연속 120%를
웃돌고, 클라우드 ARR 1만 달러를 넘는 고객 55,913곳이 클라우드 ARR 의 85% 이상을 차지한다.
Klaviyo 110%, Databricks 140% 와 나란히 놓아 같은 방식으로 자라는 회사들이 여럿임을 보였다.

낡은 것을 바꾸는 방법 축에도 넣었다. Twilio 는 이중 쓰기로 한 조각씩 옮겼고 Klaviyo 는
무중단 이관했고 Modal 과 Turso 는 처음부터 다시 지었는데, 여기는 옮기되 일부러 느리게 옮긴다.

어색한 자리가 스물두 곳으로 지금까지 가장 많았다. 이 회사는 인용을 짧게 여러 번 쓰는
습관이 있어서 풀면 조사가 자주 뜬다.
