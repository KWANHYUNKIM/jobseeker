...
**Cybozu(サイボウズ) — 53번째 회사, `in_progress`. 도메인 1개 · 기능 0개.**
306 은 회사 프로파일 사이클이었다(240·299 에서처럼 기능은 다음으로 미뤘다).
**⚠️ ⑲ 를 스크립트 안 `assert` 로 막고 썼다.**

**실적을 확보했다** — **2025년 12월기 매출 374억 3,000만 엔(+26.1%)**, **영업이익 101억
100만 엔(+106.4%)** 으로 **이익이 한 해에 두 배**가 됐다. **kintone 이 216억 8,900만
엔(+33.9%)** 으로 절반을 넘고 2026년 상반기에 **100억 엔을 넘겼다**.
**⚠️ 그런데 2026년 전망이 조심스럽다** — 매출은 **+12.7%** 를 보는데 **영업이익은 +4.1%** 다.
**이 사이트는 그 이유를 확인하지 못했다.**

**연 도메인 하나 — `이십 년 된 것을 멈추지 않고 옮긴다`**(`tech` 채웠다).
근거는 305 에서 읽은 `20年以上続くSQLiteベースのアプリケーションをKubernetesへ移行する`.
**Cybozu Office 와 Mailwise 는 C++ CGI 이고 테넌트마다 SQLite 파일**을 쓴다.
- **버린 대안에 이유가 명시된다** — StatefulSet 은 *"リリースのたびにダウンタイムが発生する"*
- **그 선택의 제약을 바로 적는다** — 배타 제어가 되려면 **파드들이 같은 노드**에 있어야 한다
- **수억 개 파일**이라 점검 시간 안에 못 옮겨 **2단계 이관**(사전 전송 + mtime 차분).
  **약 300만 파일이 15분**
- **커널 동작을 직접 조사했다** — `O_DIRECT`·타임스탬프 경계·파드 재시작 뒤 페이지 캐시
  (5GB 랜덤 데이터 · 977.2M 캐시 · 파드 메모리 상한 1Gi)

**⭐ 이 회사의 성격이 사업과 맞물린다**(이 관찰은 이 사이트의 것이다) — **20년 넘은 제품을
아직 팔기 때문에 그것을 멈추지 않고 계속 옮겨야 한다.** 새로 만드는 이야기가 아니라
**오래된 가정을 지키면서 새 바닥 위에 올리는 이야기**다.

**⚠️ 이 블로그는 기술 글의 비중이 낮다** — 최근 목록에서 **행사·채용·회고가 절반을 넘는다.**
**305 에서 목록만 봤으면 떨어뜨렸을 것이고 본문 한 편을 열었더니 기준을 다 넘었다.**

**다음 사이클 — 도메인 ① 의 기능을 쓴다. 두 번째 편 주소를 이미 확보했다**(아카이브 href).
- `https://blog.cybozu.io/entry/2026/08/06/113000` — kube-state-metrics Deployment 샤딩
- `https://blog.cybozu.io/entry/2026/07/27/113000` — Argo CD 멀티테넌트 Cattage
**⚠️ 둘 다 사내 쿠버네티스 기반(Neco) 운영 이야기라 이관 편과 문제가 다를 수 있다** —
**223 규칙대로 실물로 정한다.** 같은 도메인이면 두 편 규칙이 채워지고, 다르면 **새 도메인**이
될 수도 있다(그때는 `tech` 를 함께 채우고 `index.json` 최상위 `domains` 에 넣는다 — ⑳).

**📌 306 에서 아카이브로 확인한 다른 묶음**(제목·주소만, 본문 안 읽음)
- **프론트엔드와 접근성 다섯 편 이상** — `/entry/2026/08/04/170000` · `/entry/2026/08/04/090000` ·
  `/entry/2026/07/29/130254` · `/entry/2026/07/29/090000`
- **kintone 개발 여섯 편 이상** — `/entry/2026/08/06/090000`(OIDC 발급자 테넌트) ·
  `/entry/2026/07/24/113000`(React 그래프 로직 리팩터링) · `/entry/2026/07/30/090000`
- **보안** — `/entry/2026/08/05/113000`(govulncheck 오탐 줄이기)

**그 밖에**
- **⚠️ 확인 못 한 것** — 2026년 이익 전망이 4.1% 인 이유 · 제품별 매출 내역 · 요금 구조 ·
  부문별 이익률.
- **큐 1/3** — Cybozu 를 다 판 뒤 **후보 조사를 한 번 더** 끼운다.
...
- **⚠️ 분포** — 완료 52 + 진행 1 = **53곳**. **US 22 포화 · 한국 아홉 · 일본은 이제 넷** ·
  중국 둘 · 동남아 하나. **라틴아메리카는 재시도 금지.**
