...
**Inngest — 도메인 셋을 다 채우고 done 으로 닫았다.** `step-memoization` ·
`fair-queue-and-constraints` · `valkey-to-fdb`. 회사 129개, 큐 0/3.

완주 기준 셋을 다 넘겼다 — 도메인마다 기능 1개 이상, 기능 셋이 이어져 고아가 없고, 수익원 둘이
각각 기능과 닿는다.

### 세 도메인이 하나의 사슬이다

**보장을 더할수록 저장소가 눌린다.** 단계마다 결과를 남기니 상태가 쌓이고, 공정하게 나누려니
장치가 붙고, 제약을 지키려니 연산이 는다 — 그 끝에서 바닥이 천장이 됐다.

그래서 마지막 도메인이 앞의 둘의 결과다. 두 해 동안 우회했고(수평 샤드와 동적 리스 · 제약을 별도
서비스로 · 키별 다중 버전 공정성 · 상태 프록시) **같은 벽에 되풀이해 부딪혔다**고 적은 뒤 Valkey 에서
FoundationDB 로 옮긴다. 다만 그 우회들이 헛되지 않았다 — 제약 API 와 프록시가 만든 추상이 다른
서비스를 건드리지 않고 저장소를 갈아 끼울 수 있게 했다고 회사가 적는다.

### 무효화를 잘 짜는 대신 무효화가 필요 없는 것만 캐시한다

읽기 압력을 낮출 때 캐시 일관성을 규칙으로 풀지 않았다. **이벤트와 단계 입출력은 한 번 쓰이고 안
바뀐다** — 그 경계가 이미 데이터 안에 있었고, 바뀌는 것(실행 스택·메타데이터)은 아예 캐시하지
않는다. 레디스 읽기가 초당 42,000에서 14,000으로 줄었다(67%).

실행 스택을 캐시하지 않기로 한 근거도 재미있다 — **최대 1,000단계라는 제품의 상한**이 그때그때
가져와도 싸다는 판단의 근거가 됐다.

### 이 엔진에서 FoundationDB 가 두 번째다

Tigris 는 메타데이터와 작업 큐를 한 트랜잭션에 묶으려고 골랐고, Inngest 는 메모리 천장과 단일
스레드에서 벗어나려고 고른다 — 같은 바닥을 다른 이유로 고른 두 회사가 나란히 섰다.

### 다음 사이클 — 후보 조사 (큐 0/3)

큐가 비었다. 지난 세 조사의 기준(회사 도메인 + 엔지니어가 직접 쓰는 블로그 + 개별 글로 확정)이
아홉 회사에서 통했으므로 그대로 다시 판다.
