...
**Pinterest** (US · 소셜) — 도메인 3개 · **기능 1개**(`one-user-many-intents`). 남은 둘은 **본문 미독**이다(사용자 표현 · 인프라 프로비저닝).

### 이번 사이클 — `사람은 한 번에 여러 의도를 갖는다`

건진 것 넷:

- ⚠️ **문제의 뿌리가 사람 쪽에 있다** — **`사용자들은 한 번에 여러 의도를 갖고 있어서 하나의 검색 임베딩으로는 그 다양성을 담기 어렵다.`** **한 사람을 벡터 하나로 요약하면 그가 지금 무엇을 하러 왔는지가 뭉개진다**(재구성). 그래서 **사용자 타워를 명시적인 검색 맥락으로 조건화**했다.
- ⚠️ **조건화의 값을 조건화가 만든 중복으로 치른다** — **조건을 넣는 순간 같은 사용자가 조건 수만큼 나타난다**(학습 배치·트랜스포머 시퀀스·서빙 요청). **그래서 중복 제거가 두 겹이다** — `user-id + 조건 식별자` 로 사용자 타워 앞에서, **한 사용자의 모든 조건을 하나의 시퀀스로 이어 붙여**(M-Falcon) 트랜스포머 앞에서. **합쳐서 학습 처리량 2배, 정확도 손실 없이.**
- ⚠️ **외울 것을 줄인 게 아니라 외우는 방식을 바꿨다** — **`id 공간이 거대해서(수십억 규모) 충분한 암기와 견딜 만한 충돌을 위해 20GB 크기의 id 임베딩 테이블을 사전학습하게 됐다`** → **다섯 계층 × 2048 코드의 시맨틱 ID 와 계층마다 작은 테이블 다섯 개.**
- ⚠️ **병목을 없앤 것이 아니라 옮겼다고 스스로 적는다** — 서빙에서 조건마다 별도 요청을 보내던 것을 모델 내부 배치로 바꿔 **`병목을 네트워크에서 CPU 로 옮기면서 지연은 중립으로 유지했다`.** 결과가 **`7자리 비용 절감과 p90 모델 지연 85% 감소(80ms → 12ms)`** 다.

### 다음 사이클 — 확장(2순위)

`--gaps` 가 **`사용자를 어떤 쓰임새로 이해할지 정한다`**(Pinner Progression, 2026-07-27)를 부를 것이다. ⏳ **본문 미독** — **URL 은 확보해 뒀다.** ⭐ **이 글이 `조건이 무엇인지` 의 답일 수 있다** — 지금 기능의 open_question 이 그것이다.
...
**③ `자기 성과를 어디까지 주장하나` 7곳** — Snyk / Grafana Labs / ScyllaDB / Razorpay / Flipkart / Airbnb / **Pinterest**(병목을 옮겼다고 적는다).
...
**⑧ `제약을 없애지 못할 때 어디서 갚는가`** — Cygames / Zepto / Grafana Labs / **Pinterest**(조건화의 값을 중복 제거로).
...
**⑭ `검색 관련성을 누가 정하나`** — Etsy / Flipkart / ⏳ Zepto / **Pinterest**(사람의 의도를 조건으로).
...
**⑰ ⭐ `기억시킬 것인가 압축할 것인가`** — **Pinterest**(20GB 테이블 → 시맨틱 ID) / Zepto(색인 지연을 권위 확인으로) / Grafana Labs(색인 없는 대가를 읽을 때).
