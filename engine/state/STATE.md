...
**없다 — 문체 보수 사이클이었다.** 볼(bol)을 배민 문체로 다시 썼다(0.73 → 0.0). 회사 115개, 큐 2/3. 2곳 남았다.

### 과거를 어떻게 대하느냐 — 여기만 버릴 수가 없다

- Etsy — 데이터를 안 옮기고 접근 계층만 바꾼다.
- 페이티엠 — 수백만 객체를 안 옮긴 채 앞으로 간다.
- Paystack — 아무것도 안 바뀌는 것을 성공으로 삼는다.
- 볼 — **과거를 버릴 수가 없다.** 옛 주문에 새 사건이 오기 때문에, 그것이 없으면 새 애플리케이션이
  그 사건을 제대로 처리하지 못한다.

그래서 4년 치 4테라바이트 중 3테라바이트를 옮겼다. 그리고 이 글이 남기는 교훈이 기술이 아니라
태도다 — **사내에서 이런 일을 해 본 사람이 아무도 없었다**고 적고, 낯선 길에서는 작은 걸음으로
가라고, 여러 사람과 여러 번 이야기하라고 남긴다.

### 빨리 가려고 멈춘 넷 — 여기가 멈춘 자리는 모델이 먹을 데이터다

Ramp(재료를 만든다) · DoorDash(신규 개발 중단) · SmartHR(순서를 먼저)에 이어, 볼은 프로젝트마다
따로 자란 파이프라인 앞에서 멈춘다. 가장 아픈 증상을 한 문장으로 적는다 — **모델이 실제로 어떤
입력을 쓰는지 알아내려면 디버거로 코드를 꼼꼼히 들여다봐야 한다.** 계보가 코드 안으로 숨은 것이다.

다만 잘게 나누는 값도 함께 적는다(단계마다 넘기고 저장해야 한다). **임시 해법만큼 영구적인 것은
없다는 문장도 같은 글에 있다.**

### 도메인을 사업이 아니라 끝난 거래가 계속 말을 거는 성질로 잘랐다

배송이 청구를 부르고 반품이 크레딧 노트를 부르고 그것이 부가세를 다시 계산하게 만든다.

### 다음 사이클 — 문체 보수 (사다리 3순위)

--gaps 가 정한다. 지금 맨 위는 adevinta(0.6)지만 그건 이 시점의 예상이지 확정이 아니다.
