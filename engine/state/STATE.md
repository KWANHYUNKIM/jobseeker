...
**ScyllaDB** (IL/US · SaaS — 데이터베이스) — **프로파일까지 썼다**(PROMPT 3단계). 도메인 2개 · 기능 0. 회사 **94개.**

### 이번 사이클 — 신규(6순위)

⚠️ **이 회사의 기술 이야기가 대부분 하나의 원칙으로 돌아온다 — 아무것도 공유하지 않는다.** 자체 프레임워크 **Seastar** 위에서 **코어마다 자기 샤드**를 갖고, **새 기능을 넣을 때마다 `이것이 그 원칙을 깨는가` 를 먼저 묻는다**(재구성). ⚠️ **실제로 어떤 대안은 성능이 아니라 그 이유로 거절된다** — 공유 io_uring 인스턴스를 버린 근거가 **`Seastar 의 shared-nothing 철학을 위반한다`** 였다.

**도메인 둘** — ① **아무것도 공유하지 않고 코어마다 하나씩 둔다**(✅ 본문 확인, 2026-07-22) · ② **남의 API 를 그대로 받아 주면서 그 아래를 다시 짓는다**(⏳ 미독). **Snyk·Grafana 처럼 둘로 시작한다.**

**규모** — 총 조달 **1억 700만 달러** · 고객 **300곳 이상**. ⚠️ **둘 다 3자 자료다** — 회사가 직접 밝힌 최근 수치를 못 찾았다.

### 읽어 둔 것 — io_uring 글에서 건진 것 넷 (다음 사이클의 재료)

- ⚠️ **세 대안을 나란히 놓고 둘을 거절한다** — ① **마스터/프록시 샤드**: **`smp::submit_to 의 성능 비용은 받아들일 수 없다. 폴링에 의존해 완료까지 수 밀리초가 걸릴 수 있다.`** ② **공유 io_uring 인스턴스**: **`liburing 이 스레드 안전하지 않아 동기화가 필요하다`** — 뮤텍스는 경합을 낳고 **철학을 위반**하며, 락프리는 **기술적으로 불가능**하다고 판단했다.
- ⚠️ **고른 답은 공유를 없앤 것이 아니라 커널로 옮긴 것이다** — **샤드마다 자기 io_uring 인스턴스**를 두고 **커널 워커 스레드 풀만 `IORING_SETUP_ATTACH_WQ` 로 공유**한다. **원칙을 지키면서 자원을 나눠 쓰는 자리를 한 층 아래로 내렸다**(재구성).
- ⚠️ **순수함을 위해 최적화를 버렸다** — **동기 I/O 호출을 바로 실행하던 `빠른 길 추측 경로`(fast-track speculation path)를 제거**해 **모든 I/O 가 네트워킹 코어에서 돌게** 보장했다.
- ⚠️ **자기 방식이 진 경우를 그대로 적는다** — **`순수 I/O 위주 벤치마크에서는 비대칭 백엔드가 전통적인 linux-aio 백엔드를 이기지 못한다.`** 원인도 밝힌다 — **네트워킹 코어가 주기의 40~50% 를 커널·유저 공간 사이 메모리 복사에 쓴다.** **계산이 섞이면(요청당 2마이크로초) 이긴다.** ⚠️ **그리고 버퍼 링 탐색은 미뤘다고 적는다** — **`해법의 성능과 정확성을 종합적으로 시험하는 것이 더 중요했다.`**

### 다음 사이클 — 확장(2순위)

`--gaps` 가 **`아무것도 공유하지 않고 코어마다 하나씩 둔다`** 를 부를 것이다. **자료를 이미 읽었으니 바로 기능을 쓴다.**

⚠️ **큐는 1/3 이 된다**(Cygames). 이 회사를 다 판 뒤 **7순위(버퍼 보충)** 가 걸릴 수 있다.
...
**③ `실험 결과를 얼마나 믿나` 3곳** — Etsy / Deliveroo / Snyk. ⏳ **`자기 성과를 어디까지 주장하나` 로 넓힐 수 있다** — ⭐ **ScyllaDB 가 세 번째다**(Snyk 은 작은 격차를 무효로, Grafana 는 확장성 주장을 접고, **ScyllaDB 는 자기 방식이 진 벤치마크를 그대로 싣는다**).

**④ `인도 규모에서 무엇이 달라지나` 2곳** — Meesho / Zepto.

⏳ **⑤ `한 코어에 하나씩인가, 여러 코어가 나눠 쓰나`** — **ScyllaDB 대 ClickHouse.** ⭐ **이제 양쪽 다 이 사이트에 있고 ScyllaDB 쪽 자료를 읽기 시작했다.**

⭐ **⑥ `복제로 버틸 것인가 로그로 버틸 것인가`** — Grafana 의 Mimir·Loki 가 같은 답을 냈다.
