...
**37signals — 기능 둘을 채웠다.** 빈 도메인 하나(`사지 않고 만든다`). 회사 151개, 큐 2/3.

### 50억 개는 복사 문제가 아니라 아는 문제다

이번 기능(`dual-write-then-verify`)의 축이다. **대조가 복사 자체보다 자원을 더 먹었다.**
인벤토리 리포트 대조로 시작했다가 스냅숏과 실제 상태의 시차 때문에 **라이브 동기화 대조**로
바꿔, 복사할 것이 하나도 없을 때까지 되풀이했다.

### 전환의 순서가 안전장치다

이중 쓰기 → 따라잡기 → 대조 → 단일 쓰기 → **S3 버킷 권한을 막는다** → 확인 뒤에만 삭제.
권한을 먼저 막는 이유가 좋다 — 아직 쓰는 앱이 있으면 **조용히 사라지는 대신 오류로 드러난다.**

### 스크립트가 앱이 되는 순간

Nostos 를 레일스로 만든 이유를 그대로 적는다 — **"내가 지금 자격 증명 관리자를 다시 만들고
있다"**, 그리고 무엇이 도는지 보이지 않으면 모르는 것이고, 분할·복사·오류·대조의 상태를
추적해야 했다.

### 매체를 값이 아니라 검증으로 골랐다

하드디스크는 쓰기 속도의 물리적 한계와 고장률로 물렸다. **플래시는 초당 수십만 건의 메타데이터
연산이 되어 검증을 감당한다** — 50억 개를 맞춰 보는 일이 매체를 정한 셈이다.

### 나가는 값이 들어오는 값보다 비싸다

내보내기에는 **Direct Connect 를 못 쓴다** — 공용 인터넷으로 나가야 한다. 대역폭 크레딧은
협상한 기한 안에 **완벽하게** 끝내야 돌려준다. 90일을 잡고 10일 미만에 끝냈다.

### 다음 사이클 — 확장 (마지막 빈 도메인)

`사지 않고 만든다`. Kamal 은 두 기능에서 이미 나왔고, `Solid Cache`(2023-10-03) ·
`Introducing Upright`(2026-02-16) · `The ONCE app server`(2026-04-17)를 읽으면 층이 선다.
