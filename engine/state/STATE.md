...
**Astral**(US · 파이썬 도구 체인) — 도메인 2개 중 **1개에 기능이 들어갔다**
(`hand-written-parser`). 남은 빈 도메인 하나: '압축을 푸는 방식이 다르면 같은 파일이 다르게
설치된다'. 큐 0/3.

### 도구를 버리는 기준이 성능이 아니었다

**"생성된 파서가, 아이러니하게도, 유지하기 더 어려워졌다."** 파서 생성기는 유지를 쉽게
하려고 쓰는 것인데, **그 목적이 뒤집힌 지점**에서 버렸다. 판단의 기준이 속도가 아니라
**원래 그 도구를 쓴 이유가 아직 남아 있는가**였다.

### 빠른 출발이 AST 의 모양까지 물려줬다

남의 인터프리터(RustPython)에서 파서를 가져온 것은 빠른 출발이었는데, 그 선택이 **트리의
모양까지** 물려줬다. **"인터프리터와 린터는 이상적인 AST 가 꽤 다르게 생겼다"** — 시간이
지나도 저절로 나아지지 않고 **갈아야만 사라지는 종류의 빚**이다.

### 파서가 틀리면 오류가 나는 게 아니라 다른 트리가 나온다

그래서 검증이 어렵다 — 무엇이 정답인지의 기준이 **옛 파서**다. 수백만 줄의 실제 코드와
퍼저 입력으로 몇 달을 견줬다고 적지만, **차등 테스트 방법과 말뭉치 구성은 밝히지 않는다.**

### 큰 변경을 두 번에 나눠 냈다

파서 교체의 가장 큰 값(**편집 중인 코드에 린트를 주는 오류 복구**)이 이 판에서는 아직
동작하지 않는다. 토대만 놓고 다음으로 미뤘다 — 일정은 밝히지 않는다.

### 심은 대비

**지그 재단**(릴리스 노트에 거절한 기능을 이름 대어 적는다) ↔ **Astral**(릴리스 공지 안에
파서 교체의 이유 셋을 적는다) — **형식이 아니라 대가**라는 기준을 두 번째로 확인시킨 짝이다.
**페러스 시스템즈**(툴체인을 인증해 밖에서 보증받는다) ↔ Astral(수백만 줄과 퍼저로 스스로
증명한다).

### 다음 사이클

남은 도메인 하나 — uv 의 ZIP 파싱 차이 CVE. 사후 기록의 전 과정이 있다(신고자·조율·고침
넷·상위 15,000 패키지 측정·탈출구·업계 조율 촉구). 그것으로 Astral 은 완주다.
