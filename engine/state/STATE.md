...
**Trigger.dev** — 첫 기능 `chain-and-self-inflicted`(사슬로 적고, 자기가 키운 부분까지
적는다)를 끝냈다. **도메인 2 · 기능 1.** 큐 0/3. 남은 도메인은 **연결을 붙들고 있는 값을
깎는다.**

### 두 사고는 이어진 것이 아니었다

앞 사이클에 열어 둔 물음의 답이다. **6월 22일은 쿠버네티스 제어 평면과 etcd, 6월 30일은
클릭하우스** — **서로 다른 시스템에서 여드레 사이에 각각 터졌다.** 한 뿌리의 재발이 아니라
**여러 층이 각자 한계에 닿은 것**이라, 고칠 것 목록도 열한 가지와 여섯 가지로 갈라진다.

### 한 고리로 적으면 남 탓이 된다

AWS 용량 부족만 적으면 외부 사고다. **사슬로 적어야 자기가 끊을 수 있었던 고리가 보인다** —
**etcd 가 5GB 로 비대**했고 **OS 와 디스크를 공유**했고 **압축이 조정되지 않았다.** 셋 다
자기 것이다.

### 장애와 대응을 나눠 적지 않으면 다음에도 같은 대응을 한다

옆 지역으로 보낸 판단을 **'이것은 큰 실수였다'** 고 쓰고, **'우리 자신의 복구 자동화가
상황을 더 나쁘게 만들었다 — 우리가 올릴 수 있는 것보다 빠르게 제어 평면을 다시
무너뜨렸다'** 고 쓴다. **설계의 한계가 아니라 판단의 실수를 적는 회사는 이 원장에 드물다.**

### 우회로가 우회로가 아니었다

옮긴 뒤에도 **큐에 걸린 실행이 동시성을 잡고 있어** 새 실행이 시작되지 않았다. 그래서 고칠
것에 **'시작 안 한 실행의 동시성 해제'** 가 들어간다 — **우회 경로의 용량과 상태를 모른 채
보내면 두 곳이 다 죽는다.**

### 정반대 방향의 선택

사고 뒤 **관리형 쿠버네티스(EKS)로 옮긴다.** 리스테이트가 **로그를 직접 지어 제어 평면을
하나로 묶은 것**과 같은 층위의 문제에 **정반대 방향**이다 — 한쪽은 통제를 가져오고 한쪽은
넘긴다.

### 기준선이 없다

사고 중 수치는 촘촘한데 **평소 값이 없다.** 88% 실패도 수백만 건 큐도 **원래 크기를 모르면
절대 규모를 가늠할 수 없다.**

### 다음 사이클

확장 — 남은 도메인(런타임). Bun 이관 글에 **네 단계 수치와 다섯 가지 깨진 것**이 이미 있다.
