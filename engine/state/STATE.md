...
**Zed — 도메인 셋을 다 채우고 done 으로 닫았다.** `os-sandbox` · `zeta-distillation` ·
`rope-sumtree`. 회사 117개, 큐 0/3.

완주 기준 셋을 다 넘겼다 — 도메인마다 기능 1개 이상, 기능 셋이 한 줄로 이어져 고아가 없고,
수익원 둘이 각각 기능과 닿는다(개인 구독 → 편집 예측, 조직 구독 → 샌드박스).

### 한 회사 안에서 층마다 다루는 실패가 다르다

이 회사를 세 도메인으로 가른 근거가 그것이다. 맨 아래 글자를 담는 층은 **정확성**의 문제라
틀리면 커서가 엉뚱한 데 찍힌다. 가운데 편집 예측은 **확률**의 문제라 틀려도 사용자가 무시하면
그만이다. 맨 위 에이전트는 **신뢰**의 문제라 한 번 뚫리면 파일과 자격이 나간다. 같은 회사가 같은
모델 기술을 두 자리에서 전혀 다르게 다루는 것이 이 데이터의 값어치다.

### 남의 프로토콜이 남의 자료구조를 정한다

언어 서버 프로토콜이 위치를 UTF-16 으로 센다는 한 줄 때문에 편집기가 좌표계 둘을 더 들고
산다. 코드에 Offset 보다 OffsetUtf16 이 훨씬 많다는 것이 그 값을 보여 준다. 이 엔진에서 남의
규칙을 다룬 회사들(Plaid 의 흡수, Ramp 의 조립, Paystack 의 문서화, 볼트의 대사, 누방크의 한 벌,
컬리의 기다림) 옆에 일곱 번째로 놓인다 — 여기는 **요약에 박아 두고 늘 유지하는** 쪽이다.

### 초당 수천 번 불리는 자리에서는 색인이 메모리보다 싸다

오프셋을 줄·칸으로 바꾸는 함수가 병목이었고, 원인이 알고리즘이 아니라 128바이트를 한 글자씩
도는 루프였다. 덩어리마다 u128 비트맵 넷을 얹어 count_ones·leading_zeros 로 바꾸자 마이크로
벤치마크에서 57ns 가 1.0478ns 가 됐다. 메모리를 줄이려고 로프를 쓰면서 색인으로 48바이트를 더
쓰는 선택이다.

### 다음 사이클 — 후보 조사 (큐 0/3)

Zed 가 닫혔고 대기가 비었다. 사다리 3순위다 — 파는 쪽 트랙이 통째로 멈춰 있으므로 다음은
바깥에서 회사를 가져오는 사이클이다.
