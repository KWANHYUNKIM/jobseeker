...
**Supabase**(Postgres 플랫폼) — 회사 프로파일까지 썼다. **도메인 둘 · 기능 0.** 큐 0/3.

### 포크하지 않고 둘레를 짓는다

이 회사의 기술 노선이다. Postgres 를 규모 있게 굴리는 일은 운영이 복잡한데, 그렇다고
포크하면 상류의 개선을 못 받고 **자기들만의 데이터베이스**가 된다. 그래서 Multigres 도
**수정하지 않은 Postgres 복제**를 그대로 쓰면서 엄격한 일관성을 지킨다. 네온·PlanetScale·
터소가 저장 계층이나 엔진을 바꾼다면, 여기는 **엔진을 건드리지 않고 바깥에서 묶는다.**

### 두 답을 동시에 팔면서 각각의 한계를 적는다

두 번째 도메인이 특이하다. Realtime 과 Pipelines 는 둘 다 '변경을 받는다'라서 개발자가
잘못 고르는데, 회사가 **자주 나는 사고를 이름 대어 적는다** — Realtime 으로 내구성 있는
복제를 하려는 것이다. **"개발 중에는 아주 잘 된다"** 가 웹소켓이 몇 초 끊기면 데이터가
사라진다. 결론이 단호하다: **"Realtime 은 이 일을 위해 만들어지지 않았다."**

### 임시 슬롯과 영속 슬롯 — 어느 쪽도 공짜가 아니다

두 제품을 가르는 근본 차이가 복제 슬롯이다. **임시**는 아무도 안 볼 때 자원을 안 쓰지만
**놓친 것을 못 되돌리고**(30초 끊기면 그 사이가 사라진다), **영속**은 아무도 안 봐도 자리를
지키지만 **밀리면 WAL 이 쌓여 데이터베이스에 부담을 남긴다.**

### 못 한 것을 앞에 적는 회사다

Multigres 알파 공지가 **"샤딩(장차 대표 기능)은 이 릴리스에 없다"** 로 시작하고, 프로덕션
미준비·알려진 문제·**하위 호환 미보장**까지 앞에 둔다. Realtime 글도 자기 제품이 못 하는
것을 수치로 적는다(구독자 100명이면 INSERT 하나에 인가 질의 100개 · Replay 는 3일에 25개).

### 국가를 또 비워 뒀다

홈에도 요금 페이지에도 소재지가 없어 `country` 를 **'기타'** 로 두고 적었다. 고도에 이어
두 번째다 — **이 도구로 받는 공개 페이지에는 법인 소재지가 잘 안 적혀 있다.**

### 다음 사이클

빈 도메인 둘 중 하나. **Realtime/Pipelines 쪽이 자료가 두껍다**(구조 표·수치·자주 나는
사고·단호한 결론). Multigres 쪽은 '일반화 합의'가 이름만 나오고 성능 수치가 없다.
