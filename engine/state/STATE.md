...
**comma.ai 완주(기능 3개).** 회사 139개, 큐 1/3 — PostHog·Val Town 이 남았다.

### 천장을 두 번 밀었다

상자 하나 안에서 끝낸다는 전제가 곧 모델 크기의 천장이 된다. 먼저 **발열로 쥐어짰고**(문턱
75→85도, 막힌 기기 125대→11대), 그래도 모자라자 **상자를 키우는 대신 밖에 하나를 더 붙였다** —
조수석 발밑의 AMD RX 9060 8GB, 시가잭 전원, 완제 799달러(GPU 있는 사람은 249달러).

얻은 것은 파라미터 30배·연산 100배인 **10억 파라미터 주행 모델**이다.

### 도메인 이름이 이번 자료로 깨졌다

프로파일에서 이 도메인을 `상자 하나 안에서 끝낸다`로 잡았는데 chestnut 은 그 상자 밖에 붙는
제품이다. 연산이 차 안에서 끝난다는 원칙은 그대로지만 '상자 하나'라는 틀은 아니다 —
`open_questions` 에 적어 뒀다(PROMPT 2″단계).

### 이 글이 안 적은 것

소비자용 그래픽카드를 조수석 발밑에 두는 값(온도·진동·전원 변동)과 시가잭이 줄 수 있는 전력,
물린 대안이 전혀 없다. 앞의 두 글(회고·릴리스)이 대가를 또렷이 적은 것과 갈리는 자리라
난제로 남겼다.

### 세 기능이 한 회사를 설명한다

차를 안 만들고 남의 차에 얹기로 한 순간 — 카메라만으로 판단해야 하고(1), 사람이 보고 있다는
전제를 지켜야 하며(2), 연산이 차 안에서 끝나야 한다(3). **제약이 기능을 낳은 회사**다.

### 다음 사이클 — 신규 (대기 맨 위 PostHog)

ClickHouse 를 만드는 쪽은 이 엔진에 있고 쓰는 쪽이 없었다.
