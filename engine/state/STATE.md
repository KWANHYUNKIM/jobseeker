...
**Modal** — 회사 프로파일을 썼다. 도메인 **2개**, 기능 **0/2**. 회사 **114개.** ⚠️ **큐 0/3 — 이 회사를 다 판 뒤 후보 조사다.**

### 이번 사이클 — ⭐ 과금 방식이 기술 도메인을 정한 회사

**서버리스 컴퓨트를 초 단위로 판다.** 시리즈 C **3.55억 달러**를 **기업가치 46.5억**으로, 연환산 매출 **약 3억 달러.**

⭐ **`유휴 시간에는 값을 안 내고` 이그레스·저장·API 호출 요금도 없다.** ⚠️ **그러면 회사가 버는 것은 `실제로 도는 시간`뿐이고**, **시작하는 시간과 네트워크를 오가는 시간은 고객이 안 내고 회사가 문다**(재구성). ⭐ **이 회사의 두 도메인이 정확히 그 둘이다.**

- **전역 조율 없이 스케줄한다** — **`강화학습과 에이전트 워크로드 때문에 사용자가 수백만 개의 샌드박스를 동시에, 수십만 개를 한꺼번에 터뜨리듯 만들기를 원하게 됐다.`**
- **서버를 사용자 옆으로 옮긴다** — **`유럽 사용자가 추론 모델을 돌리면 us-east 왕복에 드는 약 200밀리초가 임베딩 모델을 돌리는 비용을 압도했다.`**

### ⭐ 쿠버네티스를 안 쓴 이유를 수식으로 적는다

**스케줄링이 `O(n × p)` 이고 기본적으로 직렬화**돼 있다. ⭐ **더 나쁜 것은 기저 부하다** — **`각 노드가 하트비트 간격마다 최소 한 번은 etcd 에 써야 해서, 기저 부하가 파드 생성과 완전히 무관하게 O(노드) 다.`** ⚠️ **아무것도 안 만들어도 노드 수만큼 든다**(재구성).

⚠️ **자기 이전 구조도 같은 병으로 진단한다** — **`백엔드 전반의 강한 일관성 때문에 전역 조율이 필요했고, 쉽게 샤딩할 수 없는 Postgres 에 O(샌드박스) 만큼 써야 했다.`**

### ⭐⭐ 이번에 건진 가장 큰 것 — 진실 원천을 아래로 내렸다

**`새 시스템에서는 모든 워커가 자기 자신의 진실 원천이다.`** 워커가 상태를 **Redis 스트림에 발행**하고 스케줄러가 **비동기로 소비해 메모리 캐시에 대고 판단**한 뒤 **직접 RPC** 로 연락한다. ⭐ **`샌드박스 생성의 임계 경로에 데이터 저장소가 아예 없다.`**

⭐ **맞바꾼 것을 문장으로 적는다** — **`샌드박스를 만들고 돌리는 임계 경로 전반에서 전역 일관성을 확장성과 성능과 맞바꿨다.`** 결과 **100만 개를 1분 안에, 시작 중앙값 0.5초 미만, 스케줄링 지연 수십 밀리초.**

⚠️ **남은 병목도 밝힌다** — **모든 워커가 단일 Redis 스트림에 발행**하는데 **10만 워커를 훌쩍 넘겨도 유효하고, 순서에 의존하지 않으므로 스트림을 더 만들면 된다.**

### ⭐ 예상 못 한 실패 모드가 아주 좋다

**빨라진 것 자체가 새 병목을 만들었다** — **`새 스케줄러가 컨테이너를 워커로 너무 빨리 밀어 넣어, 한꺼번에 시작하는 많은 컨테이너가 네트워킹을 설정할 때 리눅스 커널의 rtnl 락을 두고 경합해 시작이 수십 초 걸렸다.`** ⚠️ **⭐ 병목을 없애면 다음 병목이 커널 안에 있다**(재구성).

### ⭐ ㊳축(`유휴를 어떻게 다루나`)의 네 번째 답이다

**Neon 은 0 으로 내리고, Vercel 은 1 을 남기고, Cursor 는 재우는데** — ⭐ **Modal 은 아예 유휴에 값을 안 받는다.** **그래서 유휴 자체가 아니라 `유휴에서 깨어나는 시간`이 원가가 된다**(재구성).

### 다음 사이클 — 확장 (2순위)

**Modal 기능 0/2.** ⭐ **`전역 조율 없이 스케줄한다`부터 판다** — **거절한 대안이 수식으로 있고**(쿠버네티스 `O(n×p)`·`O(노드)`), **맞바꾼 것과 남은 병목과 예상 못 한 실패 모드가 다 적혀 있다.**

⏳ **못 읽은 것** — gVisor 격리 설계(3자 자료로만 확인) · **1초 미만 콜드 스타트의 기법**(이미지 캐싱·스냅샷) · `rtnl` 락을 무엇으로 바꿔 풀었는지.
