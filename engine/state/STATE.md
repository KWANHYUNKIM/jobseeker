...
**없다 — 문체 보수 사이클이었다.** Trendyol 을 배민 문체로 다시 썼다(2.7 → 0.0). 회사 115개, 큐 2/3. 61곳 남았다.

### 되돌릴 수 없는 자리에 누가 서느냐 — 셋이 같은 결론에 왔다

Trendyol 의 결론 한 줄이 이 축을 열었다. 전면 자동화는 기능이 아니라 위험이다. DB 이전에서
최종 컷오버라는 돌아올 수 없는 지점에 사람을 세우고 전략적 거부권을 준다.

이 엔진의 다른 둘이 같은 자리에 있다.

- Anthropic 은 되돌릴 수 없는 행동을 사람이 쥔다고 적고 PR 병합과 티켓 종료를 넘기지 않는다.
- Block 의 자가치유 루프도 여섯 에이전트를 돌리면서 병합과 종료는 사람에게 남긴다.

셋 다 자동화를 어디까지 미느냐가 아니라 되돌릴 수 없는 자리에 누가 서느냐를 정했다.
AI 를 어디까지 믿나(판단), 실패했을 때 무엇이 남느냐(경계)에 이어 세 번째 축이다.
이쪽은 권한의 문제다.

### 이번 사이클 — Trendyol

사업 모델에서 튀르키예 시장 점유가 34~40%로 압도적이라는 것을 앞세웠다. 국제 매출이 전체의
거의 20%로 커지는 중이고 걸프 지역 총거래액이 10억 달러를 넘겼다.

도메인 경계의 근거를 적었다. 광고 상품을 정의하는 일과 파는 일을 따로 볼까 했으나 상품
정의가 곧 사업 쪽이 손댈 수 있는 범위라 한 경계로 뒀다.

기능 이름에 핵심을 붙였다. `같은 필드를 서비스마다 다르게 읽고 있었다 — 공유된 착각`,
`전면 자동화는 기능이 아니라 위험이다 — 사람에게 거부권을 남긴다`,
`고치는 시간이 아니라 찾는 시간이 문제였다 — 진단을 자동화한다`.
