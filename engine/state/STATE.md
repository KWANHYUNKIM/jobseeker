...
**Mozilla**(브라우저) — 도메인 2개 중 **1개에 기능이 들어갔다**
(`harness-that-writes-the-testcase`). 남은 빈 도메인 하나: '남이 우리 안에 들어오는 것을
막되 대신할 길을 준다'. 큐 0/3.

### 찾는 능력이 아니라 증명을 요구하는 쪽으로 갔다

정적 분석(GPT-4·Sonnet 3.5)이 **오탐이 많아 규모를 못 키운** 것을 보고, 더 잘 찾는 대신
**재현 가능한 테스트 케이스를 요구**하는 쪽으로 옮겼다. **증명이 없으면 보고가 아니라
부담**이라는 판단이다.

### 이미 가진 경계를 다른 목적에 재사용했다

샌드박스 탈출을 찾을 때 **모델이 Firefox 소스를 고치는 것을 허용**하되, **고친 코드가
샌드박스된 프로세스 안에서만 돌게** 묶었다. 위험한 선택을 **자기들이 이미 가진 경계**로
감당 가능하게 만든 자리다.

### 하네스는 재사용되지만 파이프라인은 프로젝트마다 다르다

회사가 직접 그은 선이고, 이 일의 무게중심을 알려 준다 — **찾는 도구는 남의 것을 써도 되지만
받아서 처리하는 쪽(중복 제거·분류·배포)은 각자 만들어야 한다.** 그래서 100명 넘게 붙었다.

### 271개가 하네스 덕인지 알 수 없다

**퍼징만 돌렸으면 몇 개였을지 비교가 없고 오탐 비율도 없다.** 고친 것이 새 취약점을 만들지
않았는지 확인한 방법도 없다. 수치는 크게 내면서 **견줄 대상을 주지 않는다.**

### 다음 사이클

남은 도메인 하나 — DLL 주입. **수치가 하나도 없지만** 통찰이 선명하다('막기만 하면 떠난다',
'자기 릴리스 속도가 문제의 절반'). 그것으로 Mozilla 는 완주다.
