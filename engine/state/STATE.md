...
**Dolt — 도메인 셋을 다 채우고 done 으로 닫았다.** `prolly-tree` · `version-control-in-sql` ·
`wire-compatible-doltgres`. 회사 120개, 큐 0/3.

완주 기준 셋을 다 넘겼다 — 도메인마다 기능 1개 이상, 기능 셋이 이어져 고아가 없고, 수익원이
기능과 닿는다.

### 같은 문제에 정반대 답을 낸 두 회사가 나란히 들어왔다

DuckDB 와 Dolt 를 연이어 팠는데, 남의 프로토콜을 어떻게 대할지에서 정면으로 갈린다.

- **DuckDB** — 포스트그레스 와이어와 Arrow Flight 를 이름 대며 물리고 자체 프로토콜을 만들었다.
  밖에서 통제하는 형식에 묶일 수 없다는 이유였고, 대가로 남의 클라이언트를 잃었다.
- **Dolt** — 흉내 내는 쪽을 골랐다. 기존 드라이버와 도구를 그대로 얻는 대신 **원본 대비 배수로만
  평가받는다.** Postgres 15.5 기준 Doltgres v1.0 이 2.6배(v0.56.3 은 4.4배), 돌트가 1.92배.

두 회사의 기술 글이 그 선택 그대로다. 한쪽은 형식을 새로 만든 이야기이고 다른 쪽은 배수를 깎은
이야기다 — 버퍼드 플러시 9줄로 처리량 200%+, 스풀링 50%+, 커버링 인덱스 버그 21.5%, 선택도 낮은
필터에서 스캔을 고르게 해 중앙값 43.8%.

### 최적화로 못 뚫는 바닥을 인정한다

전송과 분석기는 깎을 수 있지만 저장 자료구조가 내는 값은 그 층에서 안 나온다. 회사가 돌트의
1.92배를 사실상의 하한선으로 보는 이유이고, **버전 관리를 위해 고른 구조가 그대로 배수의 바닥이
된다**는 뜻이다. 갈 길이 멀다는 문장과 그 바닥을 같은 글에 함께 적는다.

### 다음 사이클 — 후보 조사 (큐 0/3)

지난 조사에서 세운 기준(블로그가 회사 도메인에 있고 엔지니어가 직접 쓰는가)이 세 회사 모두에서
통했다 — DuckDB·안티테시스·Dolt 가 전부 대가를 먼저 적는 글을 갖고 있었다. 같은 기준으로 다시 판다.
