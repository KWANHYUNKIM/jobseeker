...
**없다 — 예순한 번째 후보 조사를 돌았다. 큐 1/3.** 다음 사이클은 **신규**(TigerData).

### 언제 안 되는지를 조건으로 적는 글

TigerData 의 `DeferredChunkAppend` 글이 **14,000배**라는 수치를 내면서 **적용 조건을 다섯
개나 나열한다** — `LIMIT` 이 스캔에 직접 붙어야 하고, 하이퍼테이블이 하나여야 하고, 행 수준
보안이 없어야 하고, 차원 열에 제약이 없어야 하고, `ORDER BY` 가 주 시간 차원의 자연 순서만
이어야 한다. **큰 수치일수록 조건이 좁다**는 것을 스스로 밝히는 형태다.

### 문제를 `LIMIT` 의 성질에서 찾았다

**"`LIMIT` 은 몇 행이 돌아오는지만 묶을 뿐"** 다른 청크가 불필요하다는 것을 증명하지
못한다 — 그래서 올바른 계획은 그 청크들로 넘어갈 수 있어야 하고, 그 때문에 계획 단계에서
청크마다 열고 잠그고 통계를 읽었다. **하루 1,000 CPU 시간 이상**이 거기 들어가고 있었다.

### 이름이 바뀐 회사를 만났다

`timescale.com` 이 **`tigerdata.com` 으로 301** 이다. 큐에 **TigerData(구 Timescale)** 로
적어 어느 이름으로 찾아도 걸리게 했다. 리디렉션을 따라가는 손질이 블루스카이·라이브킷에
이어 세 번째로 값을 했다.

### 보험 축도 막혔다

**Lemonade** 를 처음 노렸는데 블로그가 **403** 이다 — 접은 것이 아니라 못 읽은 것이다.
의료 여덟 번, CAD 세 번에 이어 보험도 첫 시도에서 막혔다. 다만 핀테크는 이미 여럿 있으니
(토스·페이스택·라조르페이) **갈리는 것은 업종이 아니라 기술 글을 쓰는 문화**인 듯하다.
