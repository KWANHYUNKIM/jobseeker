...
**Materialize — 회사 프로파일을 세웠다.** 회사 116개, 큐 1/3. 기능은 아직 0개다.

### 왜 이 회사인가 — 과금 단위가 기술 문제를 정한다

SQL 뷰를 결과가 아니라 **살아 있는 상태로 파는** 회사다. 데이터가 바뀌면 처음부터 다시 계산하지 않고
바뀐 만큼만 반영해 두었다가, 질의가 오면 이미 끝나 있는 답을 준다. 과금 단위가 **컴퓨트 크레딧**이라
그 상태를 들고 있는 값이 그대로 고객의 청구서가 된다.

**그래서 이 회사의 공개 기술 글이 유난히 바이트를 줄이는 이야기에 몰려 있다** — 레코드당 오버헤드를
96바이트에서 0~16바이트로 줄인 기록이 있고, 그다음이 사전 압축이다.

이 엔진에서 값을 어디에 미리 쌓아 두느냐를 다룬 회사들과 자리가 다르다 — 누방크는 읽기를 싸게
하려고 쓸 때 접어 뒀고 ClickHouse 는 쓰기를 싸게 하려고 값을 질의하는 쪽에 넘겼는데, **여기는 미리
계산해 두는 것 자체가 제품이라 그 값을 옮길 데가 없다.**

### 도메인 둘

1. **미리 계산해 둔 상태를 메모리에 들고 있는다** — 저장이 아니라 붙들고 있는 값의 문제다.
   태그 바이트 재사용·Misra-Gries·로컬 SSD 페이징이 여기 있다.
2. **일을 쓰는 쪽에서 할 것인가 읽는 쪽에서 할 것인가** — 관계형 DB 는 쓰는 쪽을, 검색 색인은 읽는
   쪽을 섬긴다. 이 회사의 답은 그 사이에 증분 변환 층을 두는 것이고, 값어치를 지연으로 적는다
   (쓰기에서 맥락까지 한 자릿수 초면 에이전트가 대화형이 된다).

### 안 쓴 것

- **eras 를 비웠다.** 단일 바이너리에서 컴퓨트·스토리지를 가른 전환이 있었다는 3자 자료는 있는데
  회사 자료에서 연도와 내준 것을 함께 확인하지 못했다. **연도를 모르면 쓰지 않는다.**
- 매출·고객 수는 공개돼 있지 않아 비웠다. 확인한 것은 요금 구조뿐이다.

### 다음 사이클 — 확장 (사다리 2순위)

`--gaps` 가 이미 이 회사의 빈 도메인을 지목한다. 첫 기능은 **사전 압축 편**이다 — 쓰이지 않던 태그
바이트 약 128개를 자주 나오는 값에 배정하고, 피크 메모리 981.0 → 428.4 GiB(56% 감소), 400cc 로 더
줄이자 OOM 루프. 버린 대안(컬럼 엔진의 순차 압축)과 한계(압축률 천장, 데이터가 변할 때의 거동)가
다 적혀 있는 글이다.

큐가 1/3 로 떨어졌다 — 이 회사를 끝낸 뒤 후보 조사를 끼운다.
