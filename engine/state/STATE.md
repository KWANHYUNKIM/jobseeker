...
**아무 회사도 안 판다 — 후보 조사 사이클이었다.** 큐 0/3 → **3/3**. 회사 144개.

### 이번에 올린 셋

- **Prisma**(US) — 오년 된 **러스트 질의 엔진을 걷어내고** 타입스크립트·WASM 으로 되돌린 연작.
  질의 최대 3.4배, 번들 약 14MB → 1.6MB. 여러 언어를 받치려던 이유가 오년 뒤에는 언어 경계를
  넘는 직렬화 값만 남겼다고 적는다.
- **Protomaps**(US) — 지도 타일 축. `You Might Not Want PMTiles`(2024-05-22)가 **자기 제품을
  쓰지 말아야 할 자리**를 세 조건으로 적고 GeoJSON·PostGIS·GeoParquet 을 이름 대어 권한다.
- **Ente**(IN/US) — 소비자 E2EE 사진과 온디바이스 ML. 후양자 첫걸음(2026-08-18), 기기 위의
  CLIP 검색(2023-11-18), 3중 복제(2023-01-07).

### 교훈 — 되돌린 기록을 찾는다

"회고를 먼저 찾는다"를 한 겹 더 좁혔다. 이 엔진에는 러스트로 **옮긴** 기록이 둘(Bun·Ladybird)
있는데 러스트에서 **나온** 기록이 없었다 — Prisma 가 그 반대편이다. **같은 축의 반대 방향을
찾으면 비교가 저절로 선다.**

### 자기 제품을 쓰지 말라는 글이 판정을 통과시킨다

Protomaps 는 그 글 하나로 후보 자격을 얻었다. 파는 쪽이 안 맞는 자리를 이름 대어 적는 것은 이
엔진이 찾는 바로 그 자료다.

### 접은 곳

**Godot** — 릴리스·개발 스냅숏·커뮤니티 소식이 지배한다(최신 2026-08-26). 엔진 내부 글은
마우스 폴링 수정 한 편 정도다.

### 다음 사이클 — 신규 (대기 맨 위 Prisma)

⚠️ 목록 페이지가 글을 안 보여 줘 검색으로 주소를 얻었다 — 개별 글에서 날짜를 확정한다.
