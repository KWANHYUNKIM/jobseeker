...
**Sansan(JP · SaaS) `in_progress` — 도메인 2개 · 기능 2개.** 331 에서 **두 번째 도메인
`좁혀야 이기는 인식` 을 열고 기능 `narrow-to-win`(명함으로 좁혀 범용을 넘는다)을 썼다.
결정 5개 · 그림 3장 · 생각 4개 · 난제 2건.**

**⭐ 이 회사의 심장을 팠다.** 330 까지 판 것은 개발 환경(저장소 이관)이었는데, 이번에
**연구개발부 기술 연재 두 편**으로 받쳐 인식 쪽을 열었다 —
`【研究開発部の技術】第1回 Sansanの名刺データ化技術`(2022-10-27)과
`第3回 名刺に特化した文字認識エンジン「NineOCR」`(2022-11-17).
**⚠️ `--gaps` 는 [신규]를 가리켰지만 완주 규칙이 위라 Sansan 을 더 팠다**(CLAUDE.md).

**⭐ 기능의 축** — **한 글자만 틀려도 못 쓰는 항목이 있어서 문제를 좁혔다.**
*"Email アドレスや電話番号などを1文字間違えただけでも大問題です"* 이고,
*"名刺に特化することで既存の汎用OCRエンジンの精度を超えられる見込みがあった"* 다.
명함의 성질(**작은 이미지 · 정해진 11항목 · 여러 줄인 부서·직함·주소**)이 그대로 모델 선택의
근거가 된다 — 검출은 U-Net 에서 **다중 클래스 물체 검출**로, 인식은 CRNN 에서
**Transformer 2차원 인식**으로 갔다. **주소 문자 단위 98.3%**(무작위 1만 장), **한 장당 0.3초**.

**⭐⭐ 이 기능에서 가장 드문 대목은 정확도가 아니라 안 쓰기로 한 기술이다** —
*"ただし、実は本技術は現在のデータ化フローの中では使われていません"*. 초해상은 저해상에서
**30% 이상**을 올리는데도 뺐고, 이유가 **기술이 나빠서가 아니라 입력이 좋아져서**다
(*"認識に問題があるほどの低解像画像が流れていないため"*). ⚠️ **왜 좋아졌는지도, 다시 넣을
조건도, 입력의 해상도 분포를 계속 재는지도 글에 없다** — 난제로 적었다.

**⭐ CRNN 의 장단이 같은 가정에서 나온다는 것도 적었다** — 전이 방향을 가정하니 앞뒤 문맥으로
*"O(オー)と0(ゼロ)などの判別も可能です"* 인데, **그 가정이 곧 한 줄 제한**이다.

**⚠️ `open_questions` 에 남긴 것 둘**
- **데이터화 공정에서 사람이 무엇을 하는지 찾아봤지만 글에 없다.** 공정은 잘라내기·인식·
  백색화·초해상 넷뿐이다. **그러므로 안 적었다**(추측으로 채우지 않는다).
- **연재 제2회의 주소를 못 찾았다.** 제1회와 제3회는 열었다. **이 도메인의 세 번째 재료일 수 있다.**

---

**다음 사이클 — 열린 도메인 둘이 다 찼다. 순서는 이렇게 본다.**
- **⭐ 1순위 — 연재 제2회를 찾는다.** `【研究開発部の技術】第2回` 로 **⑱ `WebSearch` +
  `allowed_domains: ["buildersbox.corp-sansan.com"]`**. 찾으면 **도메인 `좁혀야 이기는 인식` 의
  두 번째 기능**이 될 수 있다. **⚠️ 223 규칙으로 같은 문제인지 실물로 정한다.**
- **⭐⭐ 2순위 — 실물이 있는 도메인.** `サービスごとに異なる原本管理を、一つの仕組みへ`
  (2026-08-20)는 **되돌리기가 화면 안에서 안 끝난다**는 축이다 —
  *"システムのステータスだけを戻したとしても、実際の原本はもう別の場所にあるかもしれない"*.
  **⚠️ 대담 형식이라 버린 대안 0.** **스캔 센터·물류·현장 공정을 다룬 기술 글이 하나 더 있으면
  두 편으로 연다** — 331 의 검색에서 **스캔 센터에서 원본을 관리·보존하는 운영**이 있다는
  언급을 봤다. **이 축은 이 사이트에 없는 종류다.**
- **3순위 — 안 읽은 이관 글들.** Swift Concurrency(2026-03-19) · Jetpack Compose(2025-11-17) ·
  Eight 의 EC2 탈출과 모듈러 모놀리스화 · 레이어드 아키텍처 회고(2019-04-21) ·
  AI 와 함께 만드는 시스템 설계(2026-06-26) · 모바일 개발 대담 2부작. **⚠️ 이미 연 도메인
  `저장소를 옮기면 붙어 있던 것이 따라오지 않는다` 에 붙는지 실물로 정한다.**
- **4순위 — 아무것도 안 서면 닫는다.** 닫을 때 `business_model` 에 적을 관찰 —
  **⭐ 이 회사는 자기가 만든 것을 안 쓴다고 적는다**(초해상). **⚠️ 그리고 가장 아프다고 말한
  항목(이메일·전화)의 정확도는 공개하지 않는다** — 공개한 것은 주소 98.3% 하나다.

**📌 그 밖**
- **큐 2/3**(freee · SmartHR). **⚠️ Personio(EU)는 아직 안 봤다 — 다음 후보 조사의 첫 순위.**
- **⭐⭐ freee 와 SmartHR 이 정반대 순서로 같은 문제를 풀었다** — 인증·권한 기반을 떼어내는데
  **freee 는 모놀리스에서 곧장 4년**(후회로 적는다), **SmartHR 은 모듈러 모놀리스로 시작해
  Packwerk 로 경계를 먼저 보이게 한 뒤** 떼어낸다. **비교 문서 재료다.**
- **⭐ 비교 문서로 쓸 재료 둘** — ① 320 `leave-what-costs-more`(Paytm) → `reversible-first.md`
  의 *"되돌리지 못해 앞으로 밀고 나간 사례가 없다"* 와 `where-it-piles-up.md` 의 **갚지 않고
  이사한 쪽**. ② 326 `know-both-versions`(Freshworks) → `reversible-first.md` 의 *"언제 롤백
  경로를 닫는지 정한 곳이 없다"* 에 **첫 재료**이고 `swap-while-running.md` 의 **앱이 아는가
  모르는가** 축 재료이기도 하다(**추정 — 그 문서를 다시 읽고 확인한다**).
- **후보 조사 누적 열아홉 곳 중 일곱.**
- **⚠️ 낡은 메모를 나르지 않는다**(314·317·325). **⚠️ 두 방향 규칙**(298·305·318·321).
  **⚠️ 주소가 열리는지도 확인한다**(327·328). **⚠️ 인용문에 키릴 등 다른 문자가 섞이는 일이
  있다**(330 — 혼용 검사와 함께 `[Ѐ-ӿ]` 도 본다).
