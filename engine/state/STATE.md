...
**Target (US · 커머스/리테일 물류) — 42번째. 기능 4개 · 도메인 3개.**

- 220 **`split-shipments`** — 폭발이 두 번(**2ⁿ−1** · **벨 수 Bₙ**), 그리디가 지는 이유는
  **계단형 비용 함수**. 그리디를 출발점으로만 쓴다. 시간 8% 로 격차 91%.
- 221 **`infra-showback`** — **차지백을 버렸다.** 점수는 제품 단위, 재료는 **ITDP**.
  DBaaS 글과 겹쳐 읽어 **보여주는 층과 막는 층(쿼터)이 따로**임을 봤다.
- 222 **`layout-bandits`** — **A/B 와 정적 배치를 각각 이유를 대고 버렸다.** LinUCB · p95 25ms · $50M.
- 223 **`inventory-placement`** — 주문이 오기 전 배치.

**223 에서 원문을 다시 읽은 것이 값을 했다.** 요약본에는 *"버린 대안이 없다"* 뿐이었는데,
원문에는 **제약이 축어로 다섯 줄** 있었다. 결정을 세어 보니 **6개**가 나와서 썼다.
새로 나온 둘이 특히 크다.

- **이접 제약** — *"두면 최소 수량 이상, 아니면 아예 두지 않는다."* **이 한 줄이 문제를
  연속에서 정수로 넘긴다.** 변수는 묶어서 수조 → 수백만으로 줄였지만 **정수 조건은
  그렇게 줄지 않는다** — 어려움의 진짜 근원이 여기일 수 있다(이 판단은 이 사이트의 것).
- **품목 친화도** — *"함께 사는 물건들은 이행을 단순하게 하려고 함께 저장해야 할 수 있다."*
  **배치 문제 안에 들어온 분할 문제다.** `split-shipments` 와 `connections` 로 이었다
  (⚠️ 회사가 두 글에서 이 연결을 명시하진 않았다 — 제약 목록에서 읽어 낸 것이라고 밝혔다).

**또 하나** — 수요 지역을 *"창고 위치와 수요 밀도로"* 정의하므로 **이 모델은 주어진
창고망 안에서만 최적**이고 *"창고를 어디에 둘 것인가"* 는 물을 수 없다.

**⚠️ 다음 사이클 — 남은 재료는 추천 묶음의 안 읽은 두 편뿐이다.**
`accessory-recommendations-with-llms`(2025-07-01) · `buy-it-again-part-2`(2025-03-04).
**하나 열어 보고 대가가 붙는 결정이 5개 나오면 쓰고, 얕으면 Target 을 닫는다.**
(222 에서 TargetRun 이 그래서 기능이 안 됐다 — 수치는 좋은데 대가가 한 줄도 없었다.)

**🚫 분류별 목록은 못 찾았다** — `?category=` **404**, 분류 링크 href 가 JS 로 붙어 본문에
없음, 페이지 URL 형식 모름. **새 단서 없이 다시 안 두드린다.**

**읽었지만 안 쓴 글** — `synthetic-digital-orders`(2025-07-22) · `targetrun-shopping-offers`
(2026-05-06, 도메인 tech 로만 씀). **둘 다 대가가 한 줄도 없다.**
