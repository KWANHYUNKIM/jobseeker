...
**없다 — 문체 보수 사이클이었다.** 트레인라인을 배민 문체로 다시 썼다(2.02 → 0.0). 회사 115개, 큐 2/3. 28곳 남았다.

### 파는 쪽과 사는 쪽이 마주 본다

이 엔진에 CDN 을 파는 회사가 둘 있는데, 트레인라인은 그것을 사던 쪽이었다가 자기 손으로 가져온다.

- Fastly 는 콘텐츠를 세계 83곳에 미리 갖다 놓고 팔면서 **정작 파는 것은 지우는 속도**라고 말한다.
- Cloudflare 는 전용 로드밸런서를 없애고 모든 서버가 서로에게 넘기게 만든다.
- 트레인라인은 그 물건을 쓰던 쪽에서 **설정 8,500줄을 자기가 쥐기로** 한 자리에 있다.

수수료율을 남이 정하는 회사에서(기본 커미션 5% → 4.5%) 원가를 자기 손에 쥐는 일이 왜 중요한지가
그 배경이다.

### 갈아 끼우기가 여섯이 됐고 여기만 옮기는 대상이 트래픽이다

- Atlassian — 색인을 무중단으로, 일부러 느리게.
- Honeycomb — 카프카를, 무중단을 포기하고.
- Etsy — 데이터를 안 옮기고 접근 계층만.
- Paystack — 아무도 눈치 못 채게.
- 볼(bol) — 과거 4년 치를 함께.
- 트레인라인 — **되돌릴 길을 먼저 깔아 두고.** 전환 한참 전에 CNAME TTL 을 5분으로 낮춰 뒀고,
  회사가 그 안전망이 나아갈 자신을 줬다고 적는다.

그리고 이 글이 스스로 적는 교훈이 앞 회사들의 사고와 같은 모양이다 — 시험이 대부분 소규모였고
그래서 끝에 가서야 큰 그림이 보였다. **Monzo 가 1대로 시험한 것이 6대에서 다른 일이 됐던 것과 같다.**

### 도메인이 하나뿐인 이유를 사업 모델에 적었다

회사가 그것만 해서가 아니라 읽을 수 있는 기술 글이 그것 하나여서다. 표를 모으고 값을 매기는
쪽(Platform One)은 실적 각주에 이름만 나온다.

### 다음 사이클 — 문체 보수 (사다리 3순위)

--gaps 가 정한다. 지금 맨 위는 fastly(1.92)지만 그건 이 시점의 예상이지 확정이 아니다.
