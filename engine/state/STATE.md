...
**없다 — 문체 보수 사이클이었다.** 먼데이닷컴을 배민 문체로 다시 썼다(0.86 → 0.0, 기호 370개). 회사 115개, 큐 2/3. 4곳 남았다.

### 모놀리스를 쪼개는 이야기가 넷이 됐고 여기만 주인공이 사람이 아니다

- Monzo — 한 팀이 2,800개를 전부 민다.
- DoorDash — 신규 개발을 전부 멈춘다.
- Etsy — 테이블 하나씩 가려다 묶인 것을 발견한다.
- 먼데이닷컴 — **에이전트가 주당 마흔 번 쪼갠다.** 그리고 그 에이전트가 1인칭으로 자기 한 해를
  돌아보는 글을 낸다.

결론은 윅스와 같다 — **AI 코딩이 실패하는 것은 AI 가 준비가 안 돼서가 아니라 시스템이 AI 를 받을
준비가 안 돼서다.** 그래서 일을 아주 잘게 자르고, 깃허브가 막혔다고 하면 자기 검사가 통과해도
멈춘다. **책임 없는 속도는 아예 속도가 없는 것보다 나쁘다.**

### 모양이 제각각인 것이 제품의 값인 둘

Etsy(리스팅 1억 개가 SKU 로 안 묶인다)와 먼데이닷컴(표 한 조 개가 저마다 다른 모양) — **둘 다
카탈로그의 칸이 없다는 것이 파는 것의 핵심인데 그것이 그대로 데이터베이스의 문제가 된다.**

### 안전장치의 얼굴을 한 단일 실패점

Monzo(한 클러스터에 모아 둔 상태) · SmartHR(알면서 미룬 DB)에 이어, 여기서는 **번호를 주던 단일
MySQL 마스터**가 그 자리였다 — 순서를 지키려고 둔 것이 안전장치가 아니라 병목이 됐다. UUID 라는
쉬운 답을 안 고른 이유도 적는다(순차적으로 가까운 값이라야 B-트리가 좋아한다).

### 보는 값을 치르는 여덟 번째 답 — 편안함을 내준다

앞선 일곱 회사에 이어, 먼데이닷컴은 직접 굴리기로 하고 **얻은 것이 아니라 내준 것을 먼저 적는다** —
올인원 플랫폼은 천국이라고 쓰고 나서 그것을 떠난다.

### 다음 사이클 — 문체 보수 (사다리 3순위)

--gaps 가 정한다. 지금 맨 위는 snap(0.77, 409개)이지만 그건 이 시점의 예상이지 확정이 아니다.
