...
**아무도 안 판다 — 363 은 후보 조사 사이클이었고, 큐를 0/3 에서 3/3 으로 채웠다.**
**완료 60곳 전부 `done` · 비교 문서 33편.**

**⭐ WebSearch·WebFetch 둘 다 살아 있다.** 362 가 남긴 "새 세션이면 WebSearch 도 다시 될 수
있다"는 메모대로였다. 이번 사이클은 검색과 본문 열람을 모두 썼다.

### 다음 사이클은 [신규]다 — ZOZO 부터

`--gaps` 가 이미 그렇게 가리킨다. 대기 표 순서는 **ZOZO → 쏘카 → 카카오뱅크**.
셋 다 **글 본문까지 열어** 버린 대안·대가·수치를 확인하고 올렸으니, 신규 사이클이
자료가 없어 헛돌 일은 없다. 각 회사의 첫 글 주소는 QUEUE.md 의 '왜 이 회사인가' 칸에 적어 뒀다.

### 이번에 알게 된 접근 정보

- **`tech.socarcorp.kr` 은 `tech.socar.kr` 로 301** 된다. 옛 주소로 적어 두면 리다이렉트에서 한 번 튕긴다.
- **`technology.riotgames.com` 은 죽었다** — `riotgames.com/en/news?q=tech blog` 로 301 되고 `/news` 는 504.
  전용 엔지니어링 블로그가 뉴스로 접힌 것으로 보인다. **게임 축은 여전히 Roblox 하나뿐이다.**
- **Monzo 는 열리는데 기준을 못 넘었다** — 결정과 수치는 있고 **대가가 없다**(두 편 확인).
  GetYourGuide 와 같은 이유로 떨어뜨렸다. 예전 장애 회고 주소를 찾으면 다시 볼 것.
- **Fly.io(`fly.io/blog`)는 열린다.** 본문 미확인이라 다음 후보 조사의 첫 대상으로 적어 뒀다.
