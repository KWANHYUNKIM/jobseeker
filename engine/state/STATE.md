...
**Cygames** (JP · 게임) — **프로파일까지 썼다**(PROMPT 3단계). 도메인 3개 · 기능 0. 회사 **95개.** ⭐ **게임 축의 첫 회사다.** ⚠️ **큐가 0/3 이 됐다.**

### 이번 사이클 — 신규(6순위). 큐의 마지막 한 곳을 올렸다

⚠️ **이 회사의 기술 결정에는 `오래 운영한다` 는 전제가 계속 나타난다.** 카드 로직을 클라이언트에서 서버로 옮긴 근거가 **장기 유지 비용과 기능 확장**이었고, 데이터베이스를 고른 근거도 **게임 데이터베이스의 전통적인 샤딩 복잡도를 피하는 것**이었다. **한 번 내고 끝나는 제품이 아니라 몇 년을 굴릴 제품의 설계다**(재구성).

**도메인 셋** — ① **카드 게임의 판정을 클라이언트에서 서버로 옮긴다**(✅ 본문 확인) · ② **출시일의 트래픽 급증을 무중단으로 넘긴다**(✅ 본문 확인) · ③ **게임 만드는 일에 LLM 을 넣는다**(⏳ CEDEC 2026 글 셋, 미독).

**규모** — 사이버에이전트 **SGE 부문이 Cygames 와 여섯 자회사를 합쳐 5,000명 이상** · **우마무스메 영문판(2025년 6월) 이후 해외 매출이 크게 늘었다.**

### 읽어 둔 것 — 두 글에서 건진 것 넷 (다음 사이클의 재료)

- ⚠️ **전작의 성공 요인을 지키면서 구조를 바꿔야 했다** — **`카드를 다루는 감촉`** 을 유지하면서 판정을 서버로 옮겼다. **초기에 UX 우려가 있었는데도** 그렇게 한 근거가 **장기 운영 지속 가능성**이다.
- ⚠️ **대가가 즉시 나타났고 그것을 어떻게 갚았는지도 적는다** — **서버 검증이 통신 지연을 만들었지만 워크플로 개선과 애니메이션 조정으로 다뤘고, 결국 전작보다 반응성이 좋아졌다.** **제약을 받아들이고 다른 층에서 되찾았다**(재구성).
- ⚠️ **영상 스트리밍을 버리고 클라이언트에서 동시 렌더링했다** — **다운로드 우려를 피하고 관전자에게 오픈핸드 방송 뷰를 줄 수 있다.** 대신 **3D 배경과 Spine 데이터를 정적 이미지로 바꿔** 처리 부하와 메모리를 맞췄다.
- ⚠️ **출시일 초당 50만 질의, 피크 65만을 무중단으로** 넘겼고 **40만 QPS 가 도는 중에 TiKV 노드를 롤링 업데이트**했다. **통계 정보 이력 기능은 껐다** — 메모리를 제약해서, **안정성을 앞에 뒀다.**

### 다음 사이클 — 확장(2순위)

`--gaps` 가 **`카드 게임의 판정을 클라이언트에서 서버로 옮긴다`** 를 부를 것이다. **자료를 이미 읽었으니 바로 기능을 쓴다.**

⚠️ **큐가 0/3 이다.** 이 회사를 다 판 뒤 **3순위(후보 조사)** 가 걸린다. ⏳ 되짚을 목록 — `www.gojek.io/blog`(429) · ⛔ `bytes.swiggy.com`(세 번 두드리고 접었다).

### ⚠️ 비교 문서 재료 (초안 유지)

**① `AI 에이전트를 어디까지 믿나` 7곳 + ⏳ Cygames** — Sentry / ClickHouse / Duolingo / Ramp / DoorDash / Deliveroo / Snyk. ⏳ **Cygames 가 여덟 번째가 될 수 있고 자리가 다르다 — 여기서는 LLM 이 창작 공정(시나리오)에 들어간다.**

**② ⭐ `관리형 MySQL 의 한계` 3곳 + Cygames(TiDB)** — Etsy / Plaid / Paystack. ⚠️ **Plaid 와 Cygames 가 같은 TiDB 를 다른 이유로 골랐다** — **하나는 관리형 MySQL 을 떠나며, 하나는 게임 샤딩의 복잡도를 피하려고.**
