...
**없다 — 문체 보수 사이클이었다.** Airbnb 를 배민 문체로 다시 썼다(2.4 → 0.0). 회사 115개, 큐 2/3. 49곳 남았다.

### 변환기에 둘을 더했다

PayFit 에서 손으로 하던 것을 넣었다. 별표 하나로 감싼 이탤릭 인용을 걷고, 사이클 번호가
들어간 표현을 조사를 이어 가며 같은 말로 바꾼다. 이 사이트의 것이다 라는 표현도 재구성으로
통일한다. 다음 회사부터는 손으로 안 해도 된다.

지금까지 변환기가 자란 순서는 이랬다. 기호와 백틱과 굵게(Cursor) → 공백 정리(Coinbase)
→ 이탤릭 인용과 사이클 번호(Airbnb). 매번 한 회사에서 손으로 고치고 그다음 회사부터
자동으로 넘긴다.

### 도구를 고르는 자가 회사마다 다르다

Airbnb 의 관측 이전 이야기에서 판정 기준이 특이하다. 우리가 고칠 수 있는지가 도구 선택의
기준으로 명시돼 있다. 작은 코드베이스라 필요하면 이해하고 고치기 쉽다는 것이 선택 근거에
들어가고, 남들이 쓴다는 것은 근거가 되지 못했다.

관측 비용을 다루는 자리가 이 엔진에 넷이 됐다.

- VictoriaMetrics 는 카디널리티가 비용의 정체라며 그것을 판다.
- Databricks 는 수집에서 라벨을 떨어뜨리되 50배 싼 레이크하우스로 되찾을 길을 남겼다.
- WarpStream 은 새 인프라를 안 더하고 이미 있는 저장 계층을 쓴다.
- Airbnb 는 우리가 고칠 수 있는지를 기준으로 삼는다.

### 이번 사이클 — Airbnb

2026년에 공개한 기술 이야기 셋이 모두 옮기는 이야기라는 것을 앞세웠다. 관측을 벤더에서
오픈소스로, 데이터 아키텍처를 한 제품에서 여러 제품으로, 신원 그래프를 벤더 서비스에서
자체 인프라로 옮겼다. 규모가 커진 회사가 빌려 쓰던 것을 하나씩 자기 것으로 바꾸는 국면이다.

도메인 경계의 근거도 적었다. 수집하는 일과 저장하는 일을 따로 볼까 했으나 전송 프로토콜을
바꾼 것이 저장 선택까지 밀어 한 경계로 뒀다.
