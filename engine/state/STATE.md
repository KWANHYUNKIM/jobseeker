...
**Dolt — 기능 둘을 채웠다.** `prolly-tree` 와 `version-control-in-sql`(두 종류의 커밋을 한 연결
위에 올린다). 회사 120개, 큐 0/3. 빈 도메인 하나 — 와이어 호환.

### 호환을 택한 순간 표면이 정해져 있었다

MySQL 로 붙게 만들었으므로 새 문법을 만들 수 없다. 브랜치도 머지도 전부 함수·테이블 함수·저장
프로시저·시스템 테이블 안에 들어가야 했다. 기존 드라이버가 그대로 통하는 대신 제약이 따라온다 —
테이블 함수에는 별칭을 못 붙이고 다른 테이블과 조인할 수 없으며, 표준 SQL 도구는 이것들의 뜻을 모른다.

### 커밋이 두 종류인데 한 연결 위에 있다

SQL 트랜잭션은 원자성의 경계이고 버전 커밋은 이력의 경계다. 수명이 다른 둘을 한 세션에서 다루려니
**버전 조작이 트랜잭션을 함께 커밋하도록 묶었다** — DOLT_COMMIT 은 트랜잭션도 커밋하고, 머지·브랜치·
하드 리셋은 현재 트랜잭션을 암묵적으로 커밋하고 새로 시작한다. 애매함은 사라지지만 애플리케이션의
트랜잭션 경계가 버전 조작에 끌려간다.

브랜치도 데이터가 아니라 **세션의 상태**다. 체크아웃이 현재 데이터베이스 이름까지 바꾸고, 커밋하지
않은 작업 집합은 브랜치를 따라가지 않는다. 연결을 돌려 쓰는 커넥션 풀에서는 앞사람의 체크아웃이
남는다는 뜻이라 문서가 그 부작용을 명시한다.

### 충돌을 예외가 아니라 행으로 돌려준다

base·ours·theirs 를 시스템 테이블의 행으로 주고, 머지 전에 데이터베이스를 바꾸지 않고 미리 보는
함수도 따로 둔다. 해결이 SQL 안에서 끝나는 대신 충돌이 남아 있는 동안 트랜잭션을 커밋할 수 없어
세션이 묶인다. 자동 해결은 행마다 한쪽 편을 드는 데서 멈춘다 — 그 이상은 데이터의 뜻을 아는 사람 몫이다.

버전을 다 들고 있는 데이터베이스의 대가도 문서에 있다. `DOLT_GC` 는 참조되지 않는 데이터를 치우는데
**실행하면 서버의 모든 연결이 끊긴다** — 회수와 가용성이 정면으로 부딪히는 자리다.

### 다음 사이클 — 확장 (마지막 빈 도메인)

와이어 호환 도메인을 채우면 Dolt 가 done 이 되고, 그다음이 후보 조사다(큐 0/3).
