...
**Backblaze — 프로파일만 썼다.** 회사 134개, 큐 1/3(Rerun 만 남음).
도메인 셋이 다 비어 있고 셋 다 자료를 확보했다.

### 죽는 것을 전제로 짓는다

기업용 드라이브를 사면 값이 올라 팔 이유가 사라진다. 그래서 값싼 소비자용 드라이브를 대량으로
쓰되 **죽는 것을 전제로** 위에 소프트웨어를 얹는다 — 리눅스 RAID 를 걷어내고 리드-솔로몬을
직접 짜서 볼트(파드 20대·드라이브 1,200개)에 17+3 으로 편다. 파드 세 대(하드 180개)가 같은
순간에 죽어도 한 바이트도 안 잃는다.

저장 손해는 17.6%로 예전 RAID6(13+2)의 15.4%보다 조금 크다 — 거의 같다고 스스로 적는다.

### 자기 하드웨어가 죽는 비율을 공개한다

이 엔진에 값을 공개하는 회사는 여럿이지만(Depot 은 요금, Signal 은 지출) **자기 하드웨어의
고장률을 분기마다 내는 회사**는 없었다. 34만 1,263대, 분기 1.24%·평생 1.39%, 고장 하나 없는
모델과 5.78%짜리 모델을 이름 대어 적고 자료 전체를 공짜로 내려받게 한다.

고장의 정의도 단순하다 — 어제 있던 드라이브가 오늘 없으면 고장이다.

### 내려받기 값을 반대로 쓴다

오브젝트 저장소가 손님을 붙잡는 흔한 수단이 내려받기 요금인데, 여기는 평균 저장량의 세 배까지
공짜로 주고 아예 무제한인 등급을 따로 판다. 파트너 CDN 여덟 곳을 거치면 무제한이다.

### 다음 사이클 — 확장 (Backblaze 의 빈 도메인)

`값싼 디스크 위에 내구성을 만든다` 부터. 볼트 글이 2015년 글의 2019년 갱신본이라 지금 34만 대
규모의 배치와 얼마나 같은지는 `open_questions` 에 적어 뒀다.
