...
**Railway — 프로파일만 썼다.** 회사 136개, 큐 2/3.
도메인 셋 중 둘은 자료를 확보했고 하나(오케스트레이터)는 자료가 없어 tech 한 칸을 `unknown` 으로 뒀다.

### 아래층을 통째로 가져오는 회사

구글 클라우드를 떠난 이유가 셋이다 — 내려받기 요금 탓에 값이 안 맞고, 해마다 수백만 달러를
쓰는데 지원을 못 받고, **남의 플랫폼 위에서는 원하는 것을 못 만든다.** 그 뒤로 데이터센터 ·
오케스트레이터 · CDN · 빌더를 차례로 자기 것으로 바꿨다.

이 엔진의 값싼 인프라 회사들과 층이 다르다 — Ubicloud 는 남의 베어메탈을 빌리고 Backblaze 는
값싼 디스크를 사서 덮는데, **여기는 랙과 전원과 광섬유까지 내려간다.**

### 소프트웨어가 아닌 것들의 목록

랙마다 독립 전원 두 줄, 단순 합산이 아닌 3상 계산, 전력 밀도가 하드웨어 선택의 첫 기준,
설치 한 판에 장비 60대와 케이블 300가닥 배선표, 6~14일. 겪은 사고까지 이름 대어 적는다 —
뒤집힌 PDU, 뒤섞인 WYE·델타, 거꾸로 꽂힌 공기흐름 장비, 틀린 광섬유 극성.

**테라폼 배포보다 집 짓기에 가깝다**는 문장이 이 회사의 성질을 요약한다.

### 제목의 숫자를 자기가 먼저 깎는다

CDN 글 제목의 3천만 RPS 는 **DDoS 를 받아 낸 값**이고 평상시 최대는 약 100만 RPS 라고 본문에서
스스로 구분한다. 30일도 하드웨어를 몇 달 전에 주문해 둔 뒤의 소프트웨어 기간이다. POP 의
40%는 아직 배송 중이라고도 적는다.

### 다음 사이클 — 확장 (Railway 의 빈 도메인)

`랙과 전원부터 직접 세운다` 부터. 케이지 코로케이션·전력·L3 화이트박스 망이 손에 있다.
