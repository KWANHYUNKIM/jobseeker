...
**없다 — 문체 보수 사이클이었다.** Coinbase 를 배민 문체로 다시 썼다(3.83 → 0.0). 회사 115개, 큐 2/3. 68곳 남았다.

### 변환기를 한 번 더 고쳤다

Wiz 에서 손으로 하던 공백 정리를 변환기에 넣었다. 줄머리 들여쓰기와 앞뒤 공백을 자동으로
없앤다. 다음 회사부터는 이 단계를 손으로 안 해도 된다.

남은 손일은 여전히 셋째 단계다. 인용부호를 풀면 물음 형태의 인용이 특히 어색해진다.
Coinbase 에는 "그것이 일어났다는 것을 우리가 어떻게 아나" 라는 회사의 물음이 여덟 번 나오는데,
인용을 걷으면 전부 조사가 뜬다. "그것이 일어났다는 사실을 어떻게 아느냐" 로 바꿔 문장에 녹였다.

### 이번 사이클 — Coinbase

사업 모델에서 거래와 구독이 거의 같아졌다는 것을 앞세웠다. 2026년 2분기 순매출 11억 5,000만
달러 가운데 거래가 5억 9,900만, 구독과 서비스가 5억 5,500만이다. 거래 쪽은 시장에 그대로
흔들려서 사용자와 자산이 다 줄고 순손실 3억 5,900만 달러를 냈다.

앞선 회사와 견주는 문장을 셋 넣었다. Toss·Monzo 와 규제 조건은 같은데 다루는 자산이 다르고,
Block 은 사내 에이전트를 벌 계층으로 나눴고 Anthropic 은 뇌와 손과 세션을 갈랐는데 여기는
기존 소프트웨어 규율을 그대로 가져다 붙인다. 그리고 Modal 과는 정면으로 갈린다 — 저쪽은
전역 조율을 없애고 모든 워커를 자기 진실 원천으로 만들었는데 여기는 인가와 감사를 중앙으로
모았다. 배치를 정하는 일과 권한을 정하는 일은 중앙에 두는 값이 다르다(재구성).

기능 이름에 핵심을 붙였다. `능력은 나눠 주고 통제는 모은다 — 중앙화의 양날`,
`에이전트는 소프트웨어 분야다 — 장난감으로 다루지 않는다는 뜻`.

### 다음 사이클 — 문체 보수 (사다리 3순위)

무거운 순으로 victoriametrics(3.78) · vercel(3.75) · atlassian · trendyol 이 남았다.
