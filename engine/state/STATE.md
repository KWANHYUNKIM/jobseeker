...
**없다 — 이번 사이클은 후보 조사였다. 큐를 0/3 에서 1/3 로 채웠다.** 회사 157개.

### 올린 하나 — Electric(ElectricSQL)

로컬 우선 **동기화 엔진을 파는 쪽**이 이 엔진에 없었다. `Durable Streams at kernel speed`
(2026-06-26)를 본문까지 열어 확인했다 — **카프카식 비동기 복제를 물리고 단일 노드 fsync**
내구성을 고른 뒤 그 값을 그룹 커밋 WAL 로 눅인다. 데이터를 **전선에 실리는 형식 그대로**
저장하고 `sendfile(2)`·`splice(2)` 로 커널 공간에서 내보내 복사를 없앤다. 수치도 두껍다 —
4vCPU 에 스트림 10만 개로 **초당 약 86만 append**, 메모리 중앙값 515MB, 재생 2.0GB/s.

### 마주 보는 쌍을 또 얻었다 — 여섯 번째

**Electric**(동기화 엔진을 파는 쪽) ↔ **Linear**(같은 문제를 자체 구현으로 푸는 쪽).
AuthZed↔Zulip, turbopuffer↔Linear, 데노↔Val Town, 클릭하우스↔PostHog, 템포럴↔Railway 에 이어.

### 1곳만 채운 것을 채운 척하지 않는다

목표는 3곳인데 하나만 올렸다. **WorkOS** 는 제품 공지와 OAuth·MCP 같은 업계 가이드가 지배했고,
**Servo** 는 월간 진행 요약이 지배해 특정 결정과 대가를 판 글을 못 찾았다. **Ghostty** 는
`/blog` 가 404 이고 홈이 잘려 링크를 못 받아 `blocked` 로 적었다.

### 리디렉션을 따라가야 했다

`electric-sql.com/blog` 가 `electric.ax/blog` 로 301 이었다. 도구가 자동으로 안 따라가므로
목록에서 새 주소를 받아 다시 물었다 — **주소를 추측하지 않는 규칙**이 여기서도 값을 했다.

### 다음 사이클 — 신규 (대기의 Electric)

**⚠️ 2026-08-11 에 데이터브릭스 합류를 발표했다** — 폴라 시그널스(Dash0) 때와 같게 다룬다.
법인 소재지를 확인하지 못해 EU 로 뒀으니 프로파일 사이클에서 확인한다.
