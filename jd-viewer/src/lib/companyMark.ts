 ...
 *
 * NFKC 를 먼저 거는 것이 중요하다. `㈜`(U+321C)는 한 글자로 합쳐진 호환 문자라
 * 아래의 `(주)` 세 글자 규칙에 걸리지 않는다. 그래서 `㈜와탭랩스` 는 정규화해도
 * `㈜와탭랩스` 로 남고, 그 회사의 취업 브리핑과 로고가 화면에서 조용히 사라졌다.
 * NFKC 가 하는 일이 정확히 `㈜`→`(주)`, 전각→반각 같은 호환 문자의 통일이다.
 */
