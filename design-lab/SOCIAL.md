...
- **오디오 트랙** — 기본은 무음이지만 트랙 자체가 없으면 인스타가 되돌려 보낸다.
  무음 AAC 를 깔아서 올린다(`--audio` 를 주면 그 음원이 대신 깔린다, 아래).
...
### 소리 — 넣으려면 우리 음원을 구워 넣는 수밖에 없다

**인스타그램 음원 라이브러리는 API 로 못 쓴다.** 그리고 이미 올라간 릴스에는 나중에
오디오를 붙일 수도 없다. 그러니 길은 둘뿐이다 — 무음으로 두거나, 권리를 가진 음원을
영상에 미리 구워 넣거나. (앱에서 직접 올리면 라이브러리를 쓸 수 있지만 그건 자동이 아니다.)

```bash
python -m publish.cli approve-collection week --as reel   --audio ~/music/uplifting.mp3   --audio-credit "Uplifting by Artist (CC BY 4.0)"
```
...
| | 왜 |
|---|---|
| 길이 맞추기 | 곡이 짧으면 반복하고 영상 길이에서 자른다. 중간부터 무음이 되면 안 된다 |
| 크기 고르기(`loudnorm` I=-16 LUFS) | 트랙마다 녹음 레벨이 달라, 안 맞추면 어떤 판은 안 들리고 어떤 판은 폰에서 찢어진다 |
| 양끝 페이드(0.8초 / 1.5초) | 뚝 시작하고 뚝 끊기는 소리는 그 자체로 아마추어 신호다 |
...
| 출처 | 라이선스 | 표기 |
|---|---|---|
| YouTube 오디오 보관함 | 대부분 표기 불필요 | 곡마다 다름 — 받을 때 확인 |
| Pixabay Music | Pixabay 라이선스(상업 이용 가능) | 불필요 |
| Free Music Archive | CC0 / CC-BY / CC-BY-NC 섞임 | **CC-BY 는 필수**, NC 는 우리 용도에 위험 |
| Incompetech | CC-BY | 필수 |
